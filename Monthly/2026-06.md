...
const pages = dv.pages('"Repos"').where(p => {
  ...
  return fs && fs.startsWith("2026-06");
});
...
FROM "Repos"
WHERE first_seen AND dateformat(first_seen, "yyyy-MM") = "2026-06"
...
```dataviewjs
// 計算上個月的 monthStr
const [y, m] = "2026-06".split("-").map(Number);
const prevM = m === 1 ? 12 : m - 1;
const prevY = m === 1 ? y - 1 : y;
const prevStr = prevY + "-" + String(prevM).padStart(2, "0");

const thisMonth = dv.pages('"Repos"').where(p => p.first_seen?.toString()?.startsWith("2026-06"));
...
if (lastMonth.length > 0) {
  ...
  dv.table(["分類", prevStr, "2026-06", "變化"], rows);
} else {
