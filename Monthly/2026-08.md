...
const pages = dv.pages('"Repos"').where(p => {
  ...
  return fs && fs.startsWith("2026-08");
});
...
FROM "Repos"
WHERE first_seen AND dateformat(first_seen, "yyyy-MM") = "2026-08"
...
const thisMonth = dv.pages('"Repos"').where(p => p.first_seen?.toString()?.startsWith("2026-08"));
...
if (lastMonth.length > 0) {
  ...
  dv.table(["分類", prevStr, "2026-08", "變化"], rows);
} else {
