...
NLP 是讓電腦能夠「讀懂」人類語言的技術集合，涵蓋分詞、情感分析、命名實體識別、機器翻譯等各種任務。LLM 出現之前，NLP 的每個任務都需要獨立的模型和大量特徵工程；現在很多任務直接用預訓練模型 fine-tune 就能解決。你會需要了解這個領域，通常是在處理使用者輸入、分析文件內容、或建立搜尋功能的時候。
