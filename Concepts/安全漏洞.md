...
安全漏洞是軟體設計或實作上的缺陷，讓攻擊者能做到他們本來沒有權限做的事——像是讀取別人的資料、執行任意程式碼、或讓服務掛掉。常見的類型有 SQL injection、XSS、buffer overflow、不安全的反序列化等，OWASP Top 10 是很好的入門清單。身為工程師，你不需要成為資安專家，但至少要知道哪些程式寫法容易引入漏洞，才能在 code review 時把關。
