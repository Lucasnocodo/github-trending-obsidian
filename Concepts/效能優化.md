...
效能優化是找出系統的瓶頸，讓它跑得更快或用更少資源，範圍從資料庫查詢慢、API 延遲高，到前端頁面載入久、記憶體用量暴增都算。關鍵是先量測（profiling）找到真正的瓶頸，而不是憑感覺猜，因為你以為慢的地方通常不是真正的問題所在。你會需要認真面對這個議題，通常是用戶開始抱怨、流量上來撐不住、或雲端費用燒太快的時候。
