...
Agent 框架讓 LLM 不只是回答問題，而是能自己決定「下一步要做什麼」，像是搜尋網路、呼叫 API、執行程式碼，然後根據結果繼續推進。這解決了單次問答解決不了的多步驟任務問題，例如「幫我研究競爭對手並產出報告」。LangChain、LlamaIndex、AutoGen 都是常見的框架，當你需要讓 AI 自動完成一串動作時就會碰到這個領域。
