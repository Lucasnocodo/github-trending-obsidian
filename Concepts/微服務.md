...
微服務是把一個大型應用拆成多個獨立的小服務，每個服務負責一個功能領域（例如用戶服務、訂單服務、通知服務），彼此透過 API 溝通。好處是每個服務可以獨立部署、獨立擴展，某個服務壞了不會拖垮整個系統；代價是分散式系統的複雜度——網路延遲、服務發現、分散式追蹤這些問題都要處理。團隊夠大、系統夠複雜的時候才值得導入，小專案強行拆微服務反而是自找麻煩。
