...
use_case: "一款可變形的無人機，適合多種應用場景，具備靈活的移動性和多樣的感測器。"
priority: medium
ring: assess
...
star_history: "2026-03-10:364,2026-03-11:364,2026-03-11:364,2026-03-13:380,2026-03-14:396,2026-03-15:400,2026-03-16:402,2026-03-17:410,2026-03-18:416,2026-03-19:417,2026-03-20:418,2026-03-21:419,2026-03-22:419,2026-03-23:420,2026-03-24:422"
tags:
  - github
  - "category/其他"
  - "lang/python"
aliases:
  ...
  - "一款可變形的無人機，適合多種應用場景，具備靈活的移動性和多樣的感測器。"
---

# Mercury-Transforming-Drone

**422** stars · **25** stars/天 · 建立 17 天前 · Python · CERN-OHL-S-2.0
...
> **適合** 對無人機技術有興趣並希望進行自定義開發的創客和開發者。
> **一句話重點** Mercury 的設計讓無人機技術變得更加開放和可自定義，適合探索和創新。
...
> [!abstract] 核心創新
> 這個專案的核心創新在於其開放的硬體設計，讓使用者能夠自由修改和擴展。

## 專案簡介

Mercury 是一款可變形的無人機，設計上強調靈活性與多功能性。用戶可以透過簡單的變形機制，將無人機轉換為不同的形態，適應各種使用場景。它配備 RGB、深度和熱成像攝影機，並整合 Ardupilot 與 GPS，提供穩定的自主飛行能力。使用者需將自動化軟體上傳至 Raspberry Pi 5，並透過 `start_mavproxy.sh` 和 `run.sh` 指令啟動系統，這樣便能透過網路控制無人機。這個設計的賣點在於其可擴展性，能夠根據需求添加不同的感測器和載物空間。

技術上，Mercury 使用 Python 來處理自動化邏輯，並利用 Shell 腳本來管理啟動和運行，這樣的架構使得維護和擴展變得相對簡單。與其他無人機相比，如 DJI 的產品，Mercury 提供了開放的硬體設計，讓使用者能夠自由修改和擴展，而不僅僅是使用封閉的商業解決方案。這使得它特別適合開發者和創客，能夠在小型專案或教育用途上發揮作用。使用者需注意，這款無人機的效能和穩定性在大風或複雜環境中可能會受到影響，並且需要一定的技術背景來進行設置和維護。整體來看，這是一個適合小型團隊或個人開發者的專案，對於想要探索無人機技術的人來說，這是一個值得嘗試的選擇。

**技術棧**：`Python` · `Shell` · `Raspberry Pi`

## 重點功能

- 內部載物空間 — 可容納 1 公斤的貨物，適合多種應用。
- 簡單的變形機制 — 允許快速轉換無人機形態以適應不同任務。
- 多種攝影機支援 — 包括 RGB、深度和熱成像攝影機，滿足不同的感測需求。
- 使用 Ardupilot 和 GPS — 提供穩定的自主飛行能力，適合各種環境。
- 支援 Raspberry Pi 5 — 便於上傳和運行自動化軟體，操作簡單。
...
> [!tip] 爆紅原因
> 建立 17 天內累積 422 stars（25/天），forks 52（12.3%），顯示出相對穩定的興趣增長。這個專案由 L42ARO 和 Agonat0r 共同開發，他們在開源硬體領域有一定的經驗。Mercury 解決了傳統無人機在靈活性和可擴展性上的不足，特別是對於開發者和創客來說，這是一個開放且可自定義的選擇。近期的推廣活動和社群討論也可能促進了其曝光率，吸引了更多的使用者關注。這個專案的設計理念符合當前開源硬體的趨勢，讓使用者能夠在實際應用中進行創新。

## 適合誰使用

**目標受眾**：對無人機技術有興趣並希望進行自定義開發的創客和開發者。

> [!example] 使用場景
> - 無人機愛好者用它來建造個人化的變形無人機，因為它的開放設計和多樣的感測器選擇讓他們能夠自由創作。
> - 教育機構用它來進行無人機技術的教學，因為其簡單的組裝和操作流程適合學生學習。
> - 小型企業用它來進行貨物運輸，因為內部載物空間可容納 1 公斤的貨物，適合小型物流需求。

## 架構分析

Mercury 的架構設計以 Raspberry Pi 作為核心控制單元，這樣的選擇使得整體系統具有高度的靈活性和可擴展性。資料流方面，無人機的感測器數據透過 Ardupilot 與 GPS 進行處理，並且使用 Python 進行自動化邏輯的實現。這樣的設計使得使用者能夠輕鬆地添加或更換感測器。選擇 Raspberry Pi 而非專用控制器的代價是可能在性能上有所妥協，但這也讓開發者能夠利用現有的軟體生態系統。擴展性方面，使用者可以根據需求添加不同的感測器和功能，但這也可能導致系統複雜度上升，增加維護成本。

## 技術深入分析

Mercury 的核心技術機制依賴於 Raspberry Pi 作為控制單元，並使用 Python 來處理自動化邏輯，這使得開發者能夠利用現有的軟體生態系統。效能上，這款無人機在靈活性上表現優異，但在高負載或複雜環境下可能會面臨穩定性挑戰。設計上，選擇 Raspberry Pi 而非專用控制器的好處在於開放性和可擴展性，但可能在處理速度上有所妥協。技術風險方面，隨著使用者需求的增加，系統的複雜度可能上升，這對維護和擴展提出了挑戰。整合方面，Mercury 能夠與多種感測器和模組搭配使用，但需要注意與現有工具鏈的相容性，特別是在 CI/CD 流程中的整合。整體來看，這是一個具潛力的專案，對於無人機技術的探索提供了新的可能性。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝步驟和範例，對於新手來說相對友好。安裝過程中需要注意 Raspberry Pi 的配置，可能會有一些小坑。文件沒有提供多語言版本，主要以英文為主，但內容完整，能夠幫助使用者快速上手。
