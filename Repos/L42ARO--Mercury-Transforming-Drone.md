...
**525** stars · **12** stars/天 · 建立 44 天前 · N/A · CERN-OHL-S-2.0
...
> **授權** CERN-OHL-S-2.0 · **維護** Moderate (最後推送 18 天前) · **貢獻者** 2 人 · **參與度** Medium
> **適合** 對無人機技術有興趣的開發者和研究人員，特別是需要快速原型和測試的團隊。
> **一句話重點** 這個專案的設計讓無人機的使用變得更加簡單和靈活，特別適合快速原型和測試。
...
> **結論** 花 5 小時學習，8 小時整合，得到靈活的無人機控制解決方案，值得投入。

> [!abstract] 核心創新
> 提供一種簡單易用的無人機控制解決方案，適合快速原型和測試。

## 專案簡介

Mercury Transforming Drone 是一款可變形的無人機，設計上可攜帶最多 1 公斤的內部有效載荷，並配備 RGB、深度及熱成像攝影機，適合多種應用場景。使用者只需將自動駕駛軟體上傳至 Raspberry Pi 5，並透過簡單的命令行指令啟動 Mavproxy Bridge 和主控制程式，即可開始操控無人機。這樣的設計使得無人機能夠在多種環境中靈活運作，並且透過簡單的網路設定，使用者可以在不同網路下遠程控制無人機。此專案的賣點在於其簡易的組裝和操作，讓無人機的使用門檻大幅降低。技術上，專案使用了 Python 作為主要開發語言，並依賴於 Raspberry Pi 進行運算，這使得整體架構相對輕量，適合快速部署和測試。

與其他無人機控制方案相比，如 PX4 或 DJI SDK，Mercury 提供了更高的靈活性和可擴展性，特別是在自定義功能和硬體整合方面。這使得它特別適合小型團隊或愛好者進行創新實驗。使用者在實際操作中可能會遇到網路設定的挑戰，尤其是在多網路環境下，但這可以透過設置 Tailscale 來解決。整體來看，這是一個適合快速原型開發和小型專案的無人機解決方案，未來可能會隨著社群的成長而增強功能和支援。

**技術棧**：`Python` · `Raspberry Pi`

## 重點功能

- 內部有效載荷艙 — 可攜帶最多 1 公斤的設備，適合多種應用。
- 簡單變形機構 — 便於快速轉換不同模式，提升靈活性。
- 多種攝影機配置 — 包括 RGB、深度和熱成像攝影機，滿足不同需求。
- 基於 Raspberry Pi 的自動駕駛軟體 — 提供簡單的安裝和使用流程。
- 遠程控制功能 — 支援透過網路控制，適合長距離操作。
...
> [!tip] 爆紅原因
> 建立 44 天內累積 525 stars（12.0/天），forks 63（12.0%），顯示出不錯的社群興趣。這個專案由 L42ARO 和 Agonat0r 共同維護，兩位貢獻者在無人機領域有一定的經驗。Mercury 解決了傳統無人機組裝和操作的複雜性，讓使用者能夠輕鬆上手，這在市場上是相對少見的。近期的推廣活動和社群討論也可能促進了其曝光度。技術上，無人機的可變形設計和多種攝影功能使其在應用上更具吸引力，特別是在需要靈活性和多功能性的場景中。forks/stars 比率達到 12.0%，顯示出使用者對於修改和擴展的興趣。

## 適合誰使用

**目標受眾**：對無人機技術有興趣的開發者和研究人員，特別是需要快速原型和測試的團隊。

> [!example] 使用場景
> - 無人機愛好者用它來快速組裝和測試自定義功能，因為其簡單的組裝流程和開放的軟體架構讓創新變得容易。
> - 小型團隊用它來進行環境監測，因為其具備 RGB、深度和熱成像攝影機，能夠滿足多樣化的數據收集需求。
> - 研究人員用它來進行無人機自動駕駛算法的實驗，因為其開放的軟體環境和 Raspberry Pi 的使用降低了硬體成本。

## 架構分析

Mercury 的架構設計以 Raspberry Pi 為核心，搭配自動駕駛軟體和多種攝影機配置。這種設計使得無人機的組裝和操作變得簡單，並且能夠快速部署。資料流方面，無人機的控制指令透過 Mavproxy Bridge 與飛行控制器進行通訊，並且使用 Python 處理各種感測器數據。選擇 Raspberry Pi 作為硬體平台的好處在於其廣泛的社群支持和靈活的開發環境，但也限制了處理性能，特別是在高負載情況下。擴展性方面，使用者可以根據需求添加額外的攝影機或感測器，但這可能會增加系統的複雜性。

## 技術深入分析

Mercury Transforming Drone 的核心技術機制在於其基於 Raspberry Pi 的自動駕駛系統，這使得無人機能夠在多種環境下靈活運作。使用 Python 作為開發語言，讓開發者能夠快速編寫和修改控制程式。效能方面，無人機的處理能力受限於 Raspberry Pi 的硬體性能，尤其在高負載情況下可能會影響飛行穩定性。設計上選擇 Raspberry Pi 的好處在於其廣泛的社群支持和豐富的資源，但也可能面臨性能瓶頸。技術風險方面，無人機的控制系統依賴於穩定的網路連接，任何網路中斷都可能導致控制失敗。整合方面，與其他主流無人機控制系統相比，Mercury 提供了更高的靈活性，特別是在自定義功能和硬體整合方面，這使得它適合小型團隊和愛好者進行創新實驗。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝步驟和範例，對於新手來說相對友好。安裝過程順暢，但對於網路設定仍需一定的技術背景。文件目前僅提供英文版本，可能對非英語使用者造成困難。
...
> [!danger] 缺點
> - 依賴於 Raspberry Pi，可能在性能上有所限制。
> - 對於新手來說，網路設定可能會有挑戰。
> - 目前文檔僅提供英文，對非英語使用者不太友好。

> [!warning] 注意事項
> - 需要 Raspberry Pi 5 作為運算平台。
> - 對於新手來說，網路設定可能會有一定的學習曲線。
> - 目前僅提供英文文檔，對於非英語使用者可能不太友好。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供類似的無人機控制功能，但主要針對專業用途，缺乏 Mercury 的靈活性和易用性。 |
| [PX4/PX4-Autopilot](https://github.com/PX4/PX4-Autopilot) | PX4 提供更全面的功能和社群支持，但相對於 Mercury，配置和使用上更為複雜。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [PX4/PX4-Autopilot](https://github.com/PX4/PX4-Autopilot) | PX4 提供更全面的功能和社群支持，適合專業用途，但配置和使用上更為複雜。 | 如果你的團隊需要更強大的功能和社群支持，並且能夠承擔較高的學習曲線。 | medium，因為需要重新學習 PX4 的配置和操作方式。 |
| DJI SDK | DJI SDK 提供商業級的無人機控制方案，但其封閉性限制了自定義功能。 | 如果你需要商業級的穩定性和支持，但不需要太多自定義功能。 | high，因為需要完全轉換到 DJI 的生態系統。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **Mercury-Transforming-Drone** | **PX4-Autopilot** | **DJI SDK** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | PX4 提供更全面的功能和社群支持，適合專業用途，但配置和使用上更為複雜。 | DJI SDK 提供商業級的無人機控制方案，但其封閉性限制了自定義功能。 |
> | 遷移成本 | - | medium，因為需要重新學習 PX4 的配置和操作方式。 | high，因為需要完全轉換到 DJI 的生態系統。 |
> | 適用場景 | 主要場景 | 如果你的團隊需要更強大的功能和社群支持，並且能夠承擔較高的學 | 如果你需要商業級的穩定性和支持，但不需要太多自定義功能。 |
...
- **[HIGH]** 在不同網路環境下，無法穩定連接無人機
  - 解法：設置 Tailscale 以便於遠程控制
- [MEDIUM] 使用者可能無法正確安裝依賴
  - 解法：確保在虛擬環境中安裝依賴，並檢查 requirements.txt
- **[HIGH]** 無法在 Raspberry Pi 4 上正常運行
  - 解法：確保使用 Raspberry Pi 5，因為性能要求較高

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型創業團隊進行產品原型開發 | 非常適合 | 其簡單的組裝和操作流程使得快速原型成為可能。 |
| 學術研究團隊進行無人機自動駕駛算法的測試 | 適合 | 開放的軟體環境和靈活的硬體配置能夠滿足研究需求。 |
| 大型企業進行無人機商業應用開發 | 不適合 | 可能需要更穩定和全面的商業支持。 |
| 無人機愛好者進行個人項目 | 非常適合 | 易於上手和自定義，適合個人探索。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，8 小時整合，得到靈活的無人機控制解決方案，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限，並且不存取敏感資料。依賴於 Raspberry Pi 的安全性，需確保其系統更新和安全配置。
