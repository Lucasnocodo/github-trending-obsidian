...
**538** stars · **8** stars/天 · 建立 65 天前 · N/A · CERN-OHL-S-2.0
...
> **授權** CERN-OHL-S-2.0 · **維護** Slow (最後推送 39 天前) · **貢獻者** 2 人 · **參與度** Medium
> **適合** 對無人機技術有興趣的開發者和愛好者，特別是希望進行實驗和自定義的使用者。
> **一句話重點** 這個專案提供了一個靈活的開源無人機平台，適合進行實驗和開發。
...
> **結論** 花 10 小時學習，5 小時整合，得到靈活的無人機平台，值得嘗試。

> [!abstract] 核心創新
> 提供一個簡單的變形機制和多功能攝影能力的開源無人機解決方案。

## 專案簡介

Mercury-Transforming-Drone 是一款具備變形能力的無人機，設計上可攜帶 1 公斤的內部載荷，並配備 RGB、深度和熱成像攝影機，適合多種應用場景。使用者只需將自動駕駛軟體上傳至 Raspberry Pi 5，並透過 Mavproxy 介面與飛行控制器連接，便可輕鬆操控無人機。這個工具的賣點在於其簡單的變形機制和多功能攝影能力，能夠在不同環境中靈活應用。技術上，該專案依賴於 Ardupilot 和 GPS 進行導航，並使用 Python 進行軟體開發，這使得開發者能夠快速上手並進行擴展。與其他無人機解決方案相比，如 DJI 的商業無人機，Mercury 提供了更高的自定義性和開源的靈活性，特別適合開發者和愛好者進行實驗和創新。

這個專案的社群活躍度不高，但在功能上具備一定的潛力。整體來看，這是一個適合小型團隊或個人開發者的專案，尤其是對無人機技術有興趣的人。未來六個月內，隨著更多功能的加入，這個專案可能會吸引更多的使用者和貢獻者。使用者在選擇這個工具時，應考慮其開源特性和自定義需求，特別是在需要進行實驗或開發新功能的情境下。

**技術棧**：`Python` · `Raspberry Pi 5` · `Ardupilot`

## 重點功能

- 內部載荷艙 — 可攜帶 1 公斤的內部載荷，適合多種應用。
- 變形機制 — 簡單的變形設計，方便在不同環境中使用。
- 多種攝影功能 — 配備 RGB、深度和熱成像攝影機，滿足不同需求。
- 自動駕駛能力 — 基於 Ardupilot 和 GPS，提供穩定的導航。
- 簡易軟體設置 — 透過 Raspberry Pi 5 和 Python 環境輕鬆部署。
...
```bash
# 前置條件
# 在 Raspberry Pi 上安裝 Python 環境
python3 -m venv venv
source venv/bin/activate
pip install -r requirements.txt
# 預期輸出
# 成功啟動後，終端機顯示 IP 地址以便控制無人機。
```

## 為什麼值得關注

> [!tip] 爆紅原因
> 建立 65 天內累積 538 stars（8/天），forks 63（11.7%），顯示出一定的興趣和參與度。這個專案的作者 L42ARO 和 Agonat0r 在無人機領域有一定的背景，解決了傳統無人機在可變形和自動駕駛方面的不足。無人機的靈活性和多功能性是目前市場上缺乏的，這使得該專案具備吸引力。雖然沒有明顯的觸發事件，但無人機技術的進步使得這個專案變得可行。forks/stars 比率 11.7% 表示有不少使用者在實際修改和使用這個專案。

## 適合誰使用

**目標受眾**：對無人機技術有興趣的開發者和愛好者，特別是希望進行實驗和自定義的使用者。

> [!example] 使用場景
> - 無人機開發者用它來設計和測試自定義無人機功能，因為其開源特性允許自由修改和擴展。
> - 環境監測專家用它來進行熱成像和深度攝影，因為具備多種攝影功能能夠收集多樣化的數據。
> - 教育機構用它來教授無人機技術和自動駕駛原理，因為其簡單的設置和開源軟體便於學生學習。

## 架構分析

該專案採用 Raspberry Pi 5 作為控制核心，並使用 Python 進行軟體開發，這樣的選擇使得開發者能夠快速上手並進行擴展。資料流方面，無人機的控制命令透過 Mavproxy 進行傳遞，並與飛行控制器進行實時通訊。選擇 Raspberry Pi 使得硬體成本低廉，但在處理性能上可能不及專業無人機控制板。這個架構的擴展性有限，尤其在需要處理更高頻率的數據時，可能會出現瓶頸。

## 技術深入分析

Mercury-Transforming-Drone 的核心技術機制在於其變形能力和多功能攝影設計，這使得它在不同的環境中都能發揮作用。該專案使用 Ardupilot 作為導航系統，並透過 Raspberry Pi 5 進行控制，這樣的設計使得開發者能夠快速部署和修改。效能方面，無人機的處理能力受限於 Raspberry Pi 的性能，可能在高負載或複雜任務中出現延遲。選擇 Python 作為開發語言，雖然簡化了開發流程，但在性能上可能不及 C/C++ 等低階語言。這個專案的依賴樹相對簡單，主要依賴於 Python 的標準庫和一些第三方庫，這降低了維護成本。技術風險方面，若無法有效處理高頻率的數據流，可能會影響無人機的穩定性和反應速度。整合方面，該專案與其他無人機控制系統的兼容性尚需進一步測試，特別是在與商業無人機的整合上。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了基本的安裝指引和操作步驟，但缺乏詳細的範例和說明。安裝過程相對順暢，但對於初學者來說，可能需要額外的學習時間來理解無人機的運作原理。文件沒有提供多語言支持，僅以英文為主。
...
> - 開源特性 — 使用者可自由修改和擴展功能。
> - 多功能攝影 — 能夠進行多種環境的數據收集。
> - 簡單的設置 — 對於開發者來說，快速上手和部署。

> [!danger] 缺點
> - 缺乏活躍的社群支持 — 在遇到問題時可能難以獲得幫助。
> - 對於初學者來說，學習曲線可能較陡峭。
> - 性能上可能不及商業無人機，特別是在高負載情況下。

> [!warning] 注意事項
> - 僅支援 Raspberry Pi 5，其他版本可能不兼容。
> - 對於初學者，可能需要額外的學習時間來理解無人機的運作原理。
> - 缺乏活躍的社群支持，可能在遇到問題時難以獲得幫助。
...
- **[HIGH]** 在高負載情況下，無人機可能會出現性能瓶頸。
  - 解法：減少同時運行的任務數量以提高穩定性。
- [MEDIUM] 初學者可能無法快速理解無人機的設置和操作。
  - 解法：建議參考相關的無人機學習資源。
- [MEDIUM] 缺乏活躍的社群支持，可能在遇到問題時難以獲得幫助。
  - 解法：主動尋找相關論壇或社群進行交流。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型無人機開發團隊 | 非常適合 | 開源特性和靈活性使其成為實驗和開發的理想選擇。 |
| 教育機構的無人機課程 | 適合 | 簡單的設置和多功能性有助於教學。 |
| 商業無人機應用 | 不適合 | 性能和穩定性可能無法滿足商業需求。 |
| 個人愛好者進行無人機實驗 | 非常適合 | 開源和可變形的設計非常吸引愛好者。  |
...
> [!tip] 投入 vs 回報
> 花 10 小時學習，5 小時整合，得到靈活的無人機平台，值得嘗試。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限，並且不存取敏感資料。依賴鏈的信任程度較高，但仍需注意第三方庫的安全性。
