...
star_history: "2026-03-10:364,2026-03-11:364,2026-03-11:364,2026-03-13:380,2026-03-14:396,2026-03-15:400,2026-03-16:402,2026-03-17:410,2026-03-18:416,2026-03-19:417,2026-03-20:418,2026-03-21:419,2026-03-22:419,2026-03-23:420,2026-03-24:422,2026-03-25:423,2026-03-26:425,2026-03-27:439,2026-03-28:473,2026-03-29:481,2026-03-30:487,2026-03-31:500,2026-04-01:504,2026-04-02:508,2026-04-03:511,2026-04-04:514"
tags:
  - github
  - "category/其他"
  - "lang/other"
...
**514** stars · **18** stars/天 · 建立 28 天前 · N/A · CERN-OHL-S-2.0
...
> **適合** 需要快速原型和實驗的無人機開發者，特別是小型團隊或個人開發者。
> **一句話重點** 這款無人機的變形設計和多功能攝影能力使其在特定應用中具備優勢。
...
> [!abstract] 核心創新
> 提供一個具備變形功能的無人機，能夠適應多種任務需求。

## 專案簡介

Mercury Transforming Drone 是一款具備變形功能的無人機，能夠攜帶最多 1 公斤的內部載荷。使用者只需將自動駕駛軟體上傳至 Raspberry Pi 5，並透過簡單的指令設置環境，即可啟動無人機的控制系統。該無人機配備 RGB、深度和熱成像攝影機，並整合了 Ardupilot 和 GPS 系統，能夠在多種環境下進行自主飛行。其設計的賣點在於簡單的變形機制，使其能夠適應不同的任務需求。使用者需要在兩個終端中運行 Mavproxy Bridge 和主控制程式，以便進行無人機的操作，並可透過網路連接進行遠程控制，這對於需要長距離操作的應用場景特別有用。

這種設計使得無人機在多種應用中都能保持靈活性和高效性。整體架構上，該專案依賴於 Raspberry Pi 和 Python 環境，並使用虛擬環境來管理依賴，這使得安裝和維護相對簡單。與其他無人機系統相比，Mercury 的變形能力和多功能攝影系統使其在特定任務中更具優勢，特別是在需要多種感測器的應用中。使用者在設置過程中可能會遇到網路連接的問題，但這可以通過設置 Tailscale 來解決。整體而言，這是一個適合小型團隊或個人開發者的專案，特別是對於需要快速原型和實驗的情境。
...
- 簡單變形機制 — 使無人機能夠適應不同的操作需求。
- 多種攝影機 — 包含 RGB、深度和熱成像攝影機，提供多樣化的數據收集能力。
- 基於 Raspberry Pi 的自動駕駛 — 使用者可輕鬆設置和管理無人機的飛行控制。
- 網路遠程控制 — 支援透過 Tailscale 進行長距離控制，增加操作靈活性。
...
> [!tip] 爆紅原因
> 建立 28 天內累積 514 stars（18/天），forks 60（11.7%），顯示出穩定的增長潛力。作者 L42ARO 和 Agonat0r 在開源社群中有一定的影響力，這個專案解決了無人機在多功能性和易用性上的需求，特別是對於小型專案和個人開發者來說，提供了一個靈活的解決方案。無人機技術的進步和 Raspberry Pi 的普及使得這個專案的實現變得可行。forks/stars 比率為 11.7%，顯示出使用者對這個專案的實際修改和應用需求。

## 適合誰使用

**目標受眾**：需要快速原型和實驗的無人機開發者，特別是小型團隊或個人開發者。

> [!example] 使用場景
> - 無人機愛好者用它來進行空拍，因為其具備 RGB 和熱成像攝影機，能夠在不同環境下拍攝高品質影像。
> - 小型企業用它來進行監控和巡檢，因為其可變形設計使其能夠適應多種任務需求，提升工作效率。
> - 研究人員用它來進行環境監測，因為其搭載的多種感測器能夠提供豐富的數據支持，便於進行分析。

## 架構分析

Mercury Drone 的架構基於 Raspberry Pi 5，利用 Python 來實現自動駕駛功能。其設計選擇了簡單的變形機制，這使得無人機能夠在不同的環境中靈活運作。資料流方面，無人機的控制系統依賴於 Mavproxy 進行與飛行控制器的通訊，並透過網路連接進行遠程控制。選擇 Raspberry Pi 作為核心控制單元的好處在於其易於使用和廣泛的社群支持，但代價是可能在處理大量數據時出現性能瓶頸。整體而言，這個架構適合小型無人機應用，但在大規模部署時可能需要考慮更強大的硬體支持。

## 技術深入分析

Mercury Drone 的核心技術機制是基於 Raspberry Pi 5 的自動駕駛系統，使用 Python 進行開發，並整合了 Mavproxy 來控制飛行器。這種設計使得無人機能夠靈活應對各種任務需求，並且易於擴展。效能方面，該系統能夠處理多種感測器數據，但在高負載情況下可能會遇到性能瓶頸。選擇 Raspberry Pi 作為控制單元的好處在於其廣泛的社群支持和易於使用，但這也意味著在處理大量數據時，可能需要更強大的硬體支持。設計上，該專案的依賴樹相對簡單，主要依賴 Python 的生態系統，這降低了學習曲線。技術風險方面，隨著使用者數量的增加，可能會出現性能和穩定性問題，特別是在長距離控制的情境下。整合方面，該系統能夠與多種無人機控制框架兼容，但在與現有的 CI/CD 流程整合時可能需要額外的適配工作。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝步驟和範例，對於新手來說相對友好。安裝過程順暢，但需要注意網路連接的設置。文件目前僅提供英文版本，可能對非英語使用者造成一定困難。
...
> - 具備變形功能，能夠適應多種任務需求。
> - 搭載多種攝影機，提供豐富的數據收集能力。
> - 使用 Raspberry Pi，易於設置和維護。

> [!danger] 缺點
> - 僅支援 Raspberry Pi 5，限制了使用者的選擇。
> - 設置過程對於初學者來說可能較為複雜。
> - 在高負載情況下可能會出現性能瓶頸。
