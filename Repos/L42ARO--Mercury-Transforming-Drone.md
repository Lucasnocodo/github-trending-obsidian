...
use_case: "一款可變形的無人機，適合多種應用場景，具備靈活的移動性和多種感測器。"
priority: medium
ring: assess
...
aliases:
  ...
  - "一款可變形的無人機，適合多種應用場景，具備靈活的移動性和多種感測器。"
---

# Mercury-Transforming-Drone

**481** stars · **22** stars/天 · 建立 22 天前 · Python · CERN-OHL-S-2.0
...
> **適合** 對開源硬體和無人機技術有興趣的開發者和愛好者。
> **一句話重點** Mercury 的設計不僅提供了靈活的無人機功能，還讓開發者能夠在開源的環境中進行創新。
...
> **結論** 花 5 小時學習，10 小時整合，得到靈活且可擴展的無人機，值得投入。

> [!abstract] 核心創新
> 提供了一個開放且可變形的無人機設計，適合多種應用場景。

## 專案簡介

Mercury 是一款可變形的無人機，設計上強調靈活性和多功能性，適合多種應用場景。用戶可以透過簡單的變形機制將無人機轉換為不同模式，並利用內部的 1 公斤貨物艙來攜帶各種裝置。其配備 RGB、深度和熱成像相機，並整合了 Ardupilot 和 GPS 系統，讓無人機具備自主飛行的能力。使用者需將自主軟體上傳至 Raspberry Pi 5，並透過命令行啟動 Mavproxy 來與飛行控制器進行連接，這樣可以在同一網路下透過 IP 地址進行控制。這款無人機的賣點在於其高度的適應性和可擴展性，能夠滿足不同用戶的需求。

技術上，Mercury 使用 Python 和 Shell 腳本來管理無人機的運行，並且依賴於 Raspberry Pi 作為其控制核心，這使得它在硬體需求上相對輕量。與其他無人機相比，如 DJI 的 Phantom 系列，Mercury 提供了更高的自定義性和開放性，適合開發者和愛好者進行二次開發和擴展。雖然它的功能範圍廣泛，但在使用上仍需一定的技術背景，特別是在設置和維護方面。社群活躍度良好，開發者在最近幾天內有持續的更新和維護。整體來看，這是一個適合 DIY 愛好者和開發者的無人機專案，未來可能會朝著更智能化和自動化的方向發展。

**技術棧**：`Python` · `Shell` · `Raspberry Pi`

## 重點功能

- 內部貨物艙 — 可容納 1 公斤的貨物，適合運輸小型物品。
- 簡單的變形機制 — 允許用戶根據需求快速轉換無人機模式。
- 多種感測器 — 配備 RGB、深度和熱成像相機，適合多種應用場景。
- Raspberry Pi 控制 — 使用 Python 和 Shell 腳本來管理無人機運行，降低硬體需求。
- Mavproxy 支援 — 透過命令行與飛行控制器進行連接，方便用戶控制無人機。

## 快速開始

1. 在 Raspberry Pi 上創建虛擬環境
```bash
python3 -m venv venv
```
2. 啟動虛擬環境
...
> [!tip] 爆紅原因
> 建立 22 天內累積 481 stars（22/天），forks 55（11.4%），顯示出不錯的社群關注度。作者 L42ARO 和 Agonat0r 在開源硬體領域有一定的經驗，這個專案解決了無人機在多功能性和可擴展性上的需求，特別是對於 DIY 和開發者社群。無人機的設計和功能在市場上相對獨特，吸引了對開源硬體有興趣的使用者。社群活動頻繁，且無開放的 Issues，顯示出專案的穩定性和活躍度。

## 適合誰使用

**目標受眾**：對開源硬體和無人機技術有興趣的開發者和愛好者。

> [!example] 使用場景
> - 無人機愛好者用它來建造一款可變形的無人機，因為它的設計簡單且可自定義，適合個人專案。
> - 教育機構用它來進行無人機的實驗教學，因為其多種感測器和開放的設計能夠激發學生的創造力。
> - 小型企業用它來進行物流運輸，因為其內部貨物艙能夠攜帶輕量物品，並具備自主飛行的能力。

## 架構分析

Mercury 的架構設計以 Raspberry Pi 為核心，並使用 Python 和 Shell 腳本來管理無人機的運行。這種設計使得無人機在硬體需求上相對輕量，且易於擴展。資料流方面，無人機的控制指令透過 Mavproxy 與飛行控制器進行交互，並能透過網路進行遠端控制。選擇 Raspberry Pi 作為控制核心的好處在於其開放性和社群支持，但代價是可能在運算能力上不如專業的飛行控制器。擴展性方面，使用者可以根據需求添加更多感測器或功能模組，但這也可能增加維護的複雜度。

## 技術深入分析

Mercury 的核心技術機制在於其靈活的變形設計和多種感測器的整合。它使用 Python 來管理無人機的運行，並透過 Shell 腳本來啟動和控制各項功能。這種設計使得無人機在處理各種感測器數據時能夠保持高效，並能夠快速響應用戶的指令。效能方面，無人機的運算需求相對較低，適合在 Raspberry Pi 上運行，但這也意味著在處理複雜任務時可能會有瓶頸。設計上選擇 Raspberry Pi 而非專業飛行控制器，帶來了開放性和可擴展性，但也可能在穩定性上有所妥協。技術風險方面，隨著使用者需求的增加，可能會面臨維護複雜度上升的挑戰。整合方面，與其他開源硬體和無人機系統的相容性良好，並能夠輕鬆融入現有的開發流程。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝步驟和使用指令，對於有一定技術背景的使用者來說，安裝過程相對順暢。文件中包含了必要的範例和指令，讓新手能夠快速上手。整體而言，對於有經驗的開發者來說，花 30 分鐘能夠成功運行起來。
...
> - 設置過程對初學者較為複雜。
> - 不支援即時視頻回傳，限制某些應用。
> - 需要特定硬體（Raspberry Pi 5），不具通用性。

> [!warning] 注意事項
> - 需要 Raspberry Pi 5 作為控制核心，無法在其他平台運行。
> - 對於初學者來說，設置過程可能較為複雜。
> - 不支援即時視頻回傳功能，限制了某些應用場景。
...
- **[HIGH]** 在某些網路環境下，無法穩定連接到 Mavproxy
  - 解法：檢查網路設定，確保無人機和控制設備在同一網路下
- [MEDIUM] 初次設置時可能需要多次重啟 Raspberry Pi
  - 解法：確保所有依賴已正確安裝，並重啟後再嘗試
- [MEDIUM] 感測器數據整合時可能出現延遲
  - 解法：優化程式碼，減少不必要的計算
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型團隊進行無人機開發專案 | 非常適合 | 靈活的設計和開放的架構非常適合小型團隊進行創新。 |
| 教育機構的無人機實驗課程 | 適合 | 多種感測器和開放性設計能激發學生的創造力。 |
| 大型商業無人機運輸服務 | 不適合 | 在穩定性和性能上可能無法滿足商業需求。 |
| 個人愛好者的 DIY 無人機專案 | 非常適合 | 開放的設計和可變形的特性非常符合 DIY 愛好者的需求。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，10 小時整合，得到靈活且可擴展的無人機，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：無人機本身不需要高權限操作，但需注意網路安全性，確保控制設備不被未授權訪問。
