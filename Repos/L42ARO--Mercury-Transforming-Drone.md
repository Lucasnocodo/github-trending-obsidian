...
**535** stars · **10** stars/天 · 建立 55 天前 · N/A · CERN-OHL-S-2.0
...
> **授權** CERN-OHL-S-2.0 · **維護** Moderate (最後推送 29 天前) · **貢獻者** 2 人 · **參與度** Medium
> **適合** 對無人機技術有興趣並希望進行自動化控制的開發者或愛好者。
> **一句話重點** Mercury-Transforming-Drone 的變形設計和多功能攝影能力讓它在無人機領域中具備獨特的競爭優勢。
...
> [!abstract] 核心創新
> 提供一個靈活的變形無人機設計，結合多種攝影功能和自動化控制。

## 專案簡介

Mercury-Transforming-Drone 是一款具備變形能力的無人機，設計上能夠攜帶最多 1 公斤的內部有效載荷。使用者可以透過簡單的變形機制來調整無人機的形狀，這使得它在不同的操作環境中更具靈活性。該無人機配備 RGB、深度和熱成像攝影機，並且整合了 Ardupilot 與 GPS 系統，以實現精確的自動導航和控制。使用者需將自動化軟體上傳至 Raspberry Pi 5，並透過簡單的指令來啟動 Mavproxy 介面和主控制程式，這樣便能在同一網路下控制無人機。其一句話賣點是：透過簡單的變形設計與強大的攝影功能，讓無人機在多種環境中都能發揮效用。
...
- 簡單的變形機制 — 允許無人機在不同形狀之間快速轉換。
- 多種攝影功能 — 配備 RGB、深度和熱成像攝影機，適合多種環境。
- 基於 Ardupilot 的自動化控制 — 提供精確的導航和控制能力。
- Raspberry Pi 5 支援 — 使用者可輕鬆上傳和管理自動化軟體。
...
> [!tip] 爆紅原因
> 建立 55 天內累積 535 stars（約 10 stars/天），forks 63（11.8%），顯示出不錯的社群關注度。這個專案的主要貢獻者 L42ARO 和 Agonat0r 具有開源背景，並且在無人機領域有一定的經驗。Mercury 提供了一個靈活的解決方案，解決了傳統無人機在特定環境下的使用限制，尤其是對於需要多功能攝影的需求。社群的活躍度和開發者的參與度都顯示出這個專案的潛力。

## 適合誰使用

**目標受眾**：對無人機技術有興趣並希望進行自動化控制的開發者或愛好者。

> [!example] 使用場景
> - 無人機操作員用它來在城市環境中進行多種攝影任務，因為其變形設計使得在狹小空間中操作更為靈活。
> - 環境監測專家用它來收集熱成像數據，因為其配備的熱成像攝影機能夠有效識別熱源。
> - 教育機構用它來進行無人機技術的實驗教學，因為其簡單的操作和自動化控制使得學生能夠快速上手。

## 架構分析

這個專案的架構基於 Raspberry Pi 5，使用 Python 來實現自動化控制。無人機的核心控制系統依賴 Ardupilot，這使得其具備強大的導航能力。資料流方面，無人機的攝影數據透過無線網路傳輸至使用者端，並且可以在不同的網路環境中進行控制。選擇 Raspberry Pi 5 作為控制單元的好處在於其強大的計算能力和豐富的外部接口，但代價是需要額外的硬體支援。擴展性方面，若未來需要增加更多功能或感測器，可能需要重新設計硬體架構。

## 技術深入分析

這個專案的核心技術機制是基於 Raspberry Pi 5 的自動化控制系統，使用 Python 語言來實現各種功能。無人機的攝影系統整合了 RGB、深度和熱成像攝影機，這使得它能夠在不同環境下進行有效的數據收集。效能方面，無人機的最大有效載荷為 1 公斤，這對於某些應用可能是個限制。選擇 Raspberry Pi 5 作為控制單元的原因在於其強大的計算能力和廣泛的社群支持，然而這也意味著需要額外的硬體支援。技術風險方面，若未來需要擴展功能，可能需要重新設計硬體架構，這會增加開發成本。整合方面，與主流開發框架的兼容性良好，但對於新手來說，初期的安裝和配置過程可能會有一定的學習曲線。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝步驟和必要的指令，對於新手來說相對友好。安裝過程需要一定的技術背景，尤其是在 Raspberry Pi 上的配置。文件中缺乏中文或多語言支持，可能對非英語使用者造成困難。
...
> [!danger] 缺點
> - 對於高負載應用，內部有效載荷限制可能成為瓶頸。
> - 需要穩定的網路連接以實現遠程控制。
> - 安裝和配置過程對於新手來說可能有一定難度。

> [!warning] 注意事項
> - 僅支援 Raspberry Pi 5，其他版本可能不兼容。
> - 內部有效載荷限制在 1 公斤，對於某些應用可能不夠。
> - 需要穩定的網路連接以進行遠程控制。
...
> [!tip] 採用建議
> 適合開發者進行實驗和學習，但不建議用於生產環境的核心任務。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在某些網路環境下，無法穩定連接到無人機
  - 解法：確保無人機和控制設備在同一網路下，或使用 Tailscale 進行遠程控制
- [MEDIUM] 安裝過程中可能缺少某些依賴
  - 解法：仔細檢查 requirements.txt 文件，確保所有依賴都已安裝
- **[HIGH]** 變形機制在極端環境下可能無法正常運作
  - 解法：在使用前進行充分的測試，並考慮環境因素

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型無人機開發團隊 | 非常適合 | 靈活的設計和多功能性使其適合多種開發需求。 |
| 大型企業的無人機監控系統 | 不適合 | 有效載荷限制和穩定性可能無法滿足大型企業的需求。 |
| 教育機構的無人機實驗課程 | 適合 | 簡單的操作和自動化控制能夠幫助學生快速上手。 |
| 環境監測專案 | 普通 | 雖然具備攝影功能，但有效載荷限制可能影響數據收集能力。 |
...
> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限操作，且不會存取敏感資料。依賴的開源庫需定期檢查以防供應鏈風險。
