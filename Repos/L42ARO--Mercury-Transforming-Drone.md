...
use_case: "提供一個可變形的無人機解決方案，結合多種感測器和自動駕駛技術。"
priority: medium
ring: assess
...
aliases:
  ...
  - "提供一個可變形的無人機解決方案，結合多種感測器和自動駕駛技術。"
---

# Mercury-Transforming-Drone

**527** stars · **11** stars/天 · 建立 47 天前 · N/A · CERN-OHL-S-2.0
...
> **授權** CERN-OHL-S-2.0 · **維護** Moderate (最後推送 21 天前) · **貢獻者** 2 人 · **參與度** Medium
> **適合** 需要一款可變形且具備自動駕駛功能的無人機的開發者或企業。
> **一句話重點** 這款無人機的變形能力和多感測器配置，讓其在多樣化任務中展現出色的靈活性。
...
> [!abstract] 核心創新
> 結合變形機構與多種感測器的無人機，提供靈活的應用場景。

## 專案簡介

Mercury-Transforming-Drone 是一款具備變形能力的無人機，設計上專注於多功能性與自動化。使用者只需將 Autonomy Software 上傳至 Raspberry Pi 5，並透過簡單的指令設置環境，便可啟動無人機的自動駕駛功能。核心賣點在於其內部載貨艙可承載 1 公斤的貨物，並搭載 RGB、深度及熱成像攝影機，適合進行多樣化的任務。透過 Mavproxy Bridge，無人機能夠與飛行控制器進行通訊，並能在同一網路下輕鬆控制，若需遠距控制則可使用 Tailscale 進行設置。這樣的設計使得無人機在不同環境下的應用變得更加靈活，特別是在需要即時數據收集的場景中。使用者可透過簡單的 shell 指令來啟動和管理無人機的運行，這樣的便捷性在同類產品中相對少見。

**技術棧**：`Python 3` · `Raspberry Pi 5` · `Ardupilot`

## 重點功能

- 內部載貨艙 — 可承載 1 公斤的貨物，適合多樣化任務。
- 變形機構 — 簡單的變形設計，便於在不同環境中使用。
- 多種攝影機 — 配備 RGB、深度及熱成像攝影機，滿足不同的數據收集需求。
- 自動駕駛功能 — 使用 Ardupilot 和 GPS，實現高效的自動導航。
- 遠端控制 — 支援 Tailscale 設定，實現長距離控制。
...
```bash
{
  "前置條件": "在 Raspberry Pi 上設置好虛擬環境並安裝依賴。",
...
> [!tip] 爆紅原因
> 建立 47 天就累積 527 stars（11/天），forks 63（12.0%），顯示出不錯的社群關注度。這個專案的作者 L42ARO 和 Agonat0r 在無人機技術領域有一定的背景，之前未有專案能夠如此簡單地結合變形機構與自動駕駛技術。無人機市場對於多功能性和易用性的需求持續增長，這使得 Mercury 成為一個吸引人的選擇。社群的活躍度和開放的問題追蹤顯示出開發者對於用戶反饋的重視，這對於未來的迭代和改進是有利的。

## 適合誰使用

**目標受眾**：需要一款可變形且具備自動駕駛功能的無人機的開發者或企業。

> [!example] 使用場景
> - 無人機操作員用它來執行農業監測，因為其搭載的熱成像攝影機能夠有效檢測作物健康狀況。
> - 物流公司用它來運送小型包裹，因為其內部載貨艙可容納 1 公斤的貨物，並且具備自動駕駛功能。
> - 科研人員用它來進行環境監測，因為其多種感測器能夠收集不同類型的數據，並且操作簡便。

## 架構分析

Mercury-Transforming-Drone 的架構設計以 Raspberry Pi 5 為核心，透過 Python 實現自動駕駛和感測器數據處理。資料流從感測器收集數據，經由 Mavproxy Bridge 與飛行控制器進行通訊，最終將數據傳送至使用者端進行控制。選擇 Raspberry Pi 作為控制單元的優勢在於其強大的社群支持和豐富的資源，但相對於專用硬體，可能在效能上有所妥協。擴展性方面，若需增加更多感測器或功能，可能需要進一步的硬體升級或軟體調整。

## 技術深入分析

Mercury-Transforming-Drone 的核心技術在於其變形機構與自動駕駛系統的整合，使用 Python 來控制 Raspberry Pi 5，並透過 Mavproxy Bridge 與飛行控制器進行通訊。這樣的設計使得無人機能夠在不同環境中靈活運作，並能夠收集多種數據。效能方面，無人機在運行時的延遲相對較低，但在高負載情況下可能會影響其穩定性。選擇 Raspberry Pi 作為控制單元的好處在於其開放性和可擴展性，但也可能面臨效能不足的挑戰。技術風險方面，若無法妥善管理感測器的數據流，可能會導致控制失敗或數據丟失。整合方面，與主流的開發框架如 Flask 或 Django 的整合相對簡單，但若需進行深度自訂，則可能需要額外的開發工作。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝指導和範例，對於新手來說相對友好。安裝過程中可能會遇到一些配置問題，但整體流程順暢。文件目前僅提供英文版本，可能對非英語使用者造成一定困難。
...
> - 具備變形能力，適應多種任務需求。
> - 搭載多種感測器，提供豐富的數據收集功能。
> - 使用 Raspberry Pi 作為控制單元，易於開發和擴展。
...
> - 需要穩定的網路連接以進行遠端控制。
> - 對於大型貨物的運載能力有限，僅適合小型包裹。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供類似的無人機平台，但不具備變形能力，適合固定任務。 |
| [AgentSeal/codeburn](https://github.com/AgentSeal/codeburn) | 專注於自動駕駛技術，但缺乏多種感測器的整合。 |
| [AgriciDaniel/claude-obsidian](https://github.com/AgriciDaniel/claude-obsidian) | 提供無人機的自動化控制，但不具備變形功能和多種攝影機配置。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 使用固定設計的無人機，無法變形，適合特定任務。 | 如果你的任務需求不需要變形能力，且希望簡化設置過程。 | medium，因為需要重新設計控制邏輯。 |
| [AgentSeal/codeburn](https://github.com/AgentSeal/codeburn) | 專注於自動駕駛技術，無法整合多種感測器。 | 如果你的專案主要集中在自動駕駛而非多功能性。 | low，因為控制邏輯相似。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **Mercury-Transforming-Drone** | **boneyard** | **codeburn** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 使用固定設計的無人機，無法變形，適合特定任務。 | 專注於自動駕駛技術，無法整合多種感測器。 |
> | 遷移成本 | - | medium，因為需要重新設計控制邏輯。 | low，因為控制邏輯相似。 |
> | 適用場景 | 主要場景 | 如果你的任務需求不需要變形能力，且希望簡化設置過程。 | 如果你的專案主要集中在自動駕駛而非多功能性。 |
...
> [!tip] 採用建議
> 適合開發者進行實驗和測試，不建議在生產環境中使用。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在高負載情況下，無人機可能會失去穩定性
  - 解法：減少載重或調整飛行模式
- [MEDIUM] 初次設定可能會遇到網路連接問題
  - 解法：確保所有設備在同一網路下
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型物流公司使用無人機進行包裹配送 | 非常適合 | 具備變形和自動駕駛功能，能有效提升配送效率。 |
| 科研機構進行環境監測 | 適合 | 多種感測器配置能夠收集豐富的數據。 |
| 個人愛好者進行無人機飛行測試 | 普通 | 雖然功能豐富，但設置過程對新手可能較為複雜。 |
...
> [!warning] 安全性快速掃描
> 低風險：無人機本身不需要高權限，但需注意網路安全性，特別是在遠端控制時。
