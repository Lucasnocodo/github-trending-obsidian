...
use_case: "提供一個可變形的無人機解決方案，結合多種感測器和自動化軟體。"
priority: medium
ring: assess
...
aliases:
  ...
  - "提供一個可變形的無人機解決方案，結合多種感測器和自動化軟體。"
---

# Mercury-Transforming-Drone

**523** stars · **13** stars/天 · 建立 40 天前 · N/A · CERN-OHL-S-2.0
...
> **授權** CERN-OHL-S-2.0 · **維護** Moderate (最後推送 14 天前) · **貢獻者** 2 人 · **參與度** Medium
> **適合** 對無人機開發和自動化有興趣的開發者和研究人員。
> **一句話重點** 這款無人機的變形能力和開源特性使其在多種應用場景中具備競爭優勢。
...
> [!abstract] 核心創新
> 這個專案的核心創新在於其簡單的變形機制和多感測器整合能力。

## 專案簡介

Mercury-Transforming-Drone 是一款具備變形功能的無人機，設計上可攜帶最多 1 公斤的內部載荷。用戶可以透過簡單的變形機制，將無人機轉換為不同的形態以適應不同任務。這款無人機配備 RGB、深度和熱成像攝影機，並使用 Ardupilot 和 GPS 進行導航。用戶需要將自動化軟體上傳至 Raspberry Pi 5，並透過指令 `python3 -m venv venv` 和 `pip install -r requirements.txt` 來設置環境，然後運行 `start_mavproxy.sh` 和 `run.sh` 來啟動控制系統。這個工具的賣點在於其簡單的設置流程和靈活的應用場景，能夠在多種環境下運行。

其技術架構基於 Raspberry Pi 和 Python，這使得開發者能夠輕鬆擴展功能或進行自定義。與其他無人機解決方案相比，如 DJI 的 Mavic 系列，Mercury 提供了更大的靈活性和可擴展性，特別是在自動化和開源方面。這款無人機適合需要自定義解決方案的開發者或研究人員，尤其是在小型無人機應用的開發上。使用者需要注意的是，無人機的性能可能會受到載荷和環境因素的影響，並且社群支持仍在發展中。整體來看，這是一個有潛力的專案，適合對無人機開發有興趣的團隊或個人。

**技術棧**：`Python 3` · `Raspberry Pi 5`

## 重點功能

- 內部載荷艙 — 可攜帶最多 1 公斤的貨物，適合多種任務。
- 簡單變形機制 — 允許用戶快速轉換無人機形態以適應不同需求。
- 多種感測器 — 配備 RGB、深度和熱成像攝影機，提供多樣化的數據收集能力。
- 基於 Raspberry Pi 的自動化 — 使用者可輕鬆設置和擴展功能。
- 支持遠程控制 — 可透過 Tailscale 進行長距離控制，適合多種操作環境。

## 快速開始

1. 上傳自動化軟體至 Raspberry Pi 5
```bash
scp -r Autonomy Software pi@<RaspberryPi_IP>:~
```
2. 建立虛擬環境並安裝依賴
```bash
...
> [!tip] 爆紅原因
> 建立 40 天內累積 523 stars（約 13 stars/天），forks 63（12%），顯示出不錯的社群參與度。作者 L42ARO 和 Agonat0r 在開源硬體領域有一定的經驗，這個專案解決了無人機自動化和變形的需求，過去的解決方案往往缺乏靈活性。無人機市場對於開源和可擴展的需求日益增加，這使得 Mercury 成為一個有吸引力的選擇。此專案的快速增長可能與其簡單的設置流程和多功能性有關，並且在社群中引發了討論。

## 適合誰使用

**目標受眾**：對無人機開發和自動化有興趣的開發者和研究人員。

> [!example] 使用場景
> - 無人機開發者用它來設計和測試自定義的無人機任務，因為其開源架構和靈活性使得快速迭代成為可能。
> - 學術研究人員用它來進行環境監測，因為其配備的多種感測器能夠收集不同類型的數據。
> - 業餘愛好者用它來學習無人機控制和自動化技術，因為其簡單的設置和開源特性降低了入門門檻。

## 架構分析

Mercury-Transforming-Drone 的架構基於 Raspberry Pi 5，這使得開發者能夠利用現有的硬體資源。資料流從感測器收集數據，經由 Python 程式處理後，通過 Mavproxy 與飛行控制器進行通訊。這種設計選擇使得系統能夠快速部署，但也限制了對於高性能計算的需求。選擇 Raspberry Pi 作為控制單元的代價在於可能的性能瓶頸，特別是在處理高解析度影像時。整體架構的擴展性良好，但在高負載情況下可能會遇到延遲問題。

## 技術深入分析

Mercury-Transforming-Drone 的核心技術機制是基於 Raspberry Pi 和 Python，這使得開發者能夠輕鬆集成各種感測器和控制系統。其資料流設計從 RGB、深度和熱成像攝影機收集數據，並透過 Mavproxy 與飛行控制器進行通訊。這種設計使得無人機能夠在多種環境下運行，但也可能在高負載下出現性能瓶頸。選擇 Raspberry Pi 作為控制單元的好處在於其廣泛的社群支持和豐富的資源，但在處理高解析度影像時可能會遇到延遲。技術風險方面，隨著使用者數量的增加，對於系統穩定性的需求將會提高，這可能會導致潛在的技術債。整合方面，與主流開發工具鏈的相容性良好，特別是對於 Python 生態系統，學習成本相對較低。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝指導和範例，讓新手能夠順利上手。安裝過程相對順暢，但需要注意 Raspberry Pi 的配置。文件中缺乏多語言支持，可能對非英語使用者造成困難。
...
> [!danger] 缺點
> - 性能受限於 Raspberry Pi 的計算能力。
> - 對於重載荷任務的支持有限。
> - 社群支持仍在發展中，可能缺乏即時的技術支援。

> [!warning] 注意事項
> - 僅支援 Raspberry Pi 5，無法在其他平台上運行。
> - 對於重載荷任務，無人機的性能可能會受到影響。
> - 目前社群支持仍在發展中，可能缺乏即時的技術支援。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供類似的無人機控制功能，但缺乏變形機制，適合固定任務的應用。 |
| [AgentSeal/codeburn](https://github.com/AgentSeal/codeburn) | 專注於無人機的自動化和控制，但不具備多種感測器的整合能力。 |
| [AgriciDaniel/claude-obsidian](https://github.com/AgriciDaniel/claude-obsidian) | 提供無人機的開源控制系統，但不支持變形功能，適合需要穩定控制的場景。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [DJI/Mavic](https://github.com/DJI/Mavic) | 專注於商業市場，提供高性能無人機，但缺乏開源特性。 | 如果需要高性能和商業支持的無人機解決方案。 | medium，因為需要重新適應商業產品的操作界面。 |
| [Parrot/ANAFI](https://github.com/Parrot/ANAFI) | 提供開源 SDK，但不具備變形功能，適合固定任務。 | 如果需要穩定的無人機控制，但不需要變形能力。 | low，因為有相似的開發環境和工具。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **Mercury-Transforming-Drone** | **Mavic** | **ANAFI** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於商業市場，提供高性能無人機，但缺乏開源特性。 | 提供開源 SDK，但不具備變形功能，適合固定任務。 |
> | 遷移成本 | - | medium，因為需要重新適應商業產品的操作界面。 | low，因為有相似的開發環境和工具。 |
> | 適用場景 | 主要場景 | 如果需要高性能和商業支持的無人機解決方案。 | 如果需要穩定的無人機控制，但不需要變形能力。 |
...
> [!tip] 採用建議
> 適合個人或小型團隊的開發和測試，不建議用於生產環境的核心任務。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在某些網路環境下，無法穩定連接到飛行控制器
  - 解法：確保使用穩定的 Wi-Fi 網路，或考慮使用 Tailscale 進行連接
- [MEDIUM] 在高負載情況下，無人機可能會出現性能下降
  - 解法：減少載荷或降低攝影機解析度以提高性能
- [MEDIUM] 缺乏即時的社群支持，可能遇到技術問題時無法快速解決
  - 解法：參考官方文檔和 GitHub Issues，或尋求其他開發者的幫助

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型團隊開發自動化無人機任務 | 非常適合 | 開源架構和靈活性使得快速迭代成為可能。 |
| 個人愛好者進行無人機控制學習 | 適合 | 簡單的設置流程降低了入門門檻。 |
| 大型商業無人機應用 | 不適合 | 性能和穩定性尚未達到商業標準。  |
...
Mercury-Transforming-Drone 最常與 Raspberry Pi 和 Python 生態系統搭配使用，適合在開發和測試階段進行集成。在一個使用 Raspberry Pi 的無人機專案中，開發者可以將 Mercury 的自動化軟體上傳至 Raspberry Pi，並透過簡單的指令啟動控制系統。該工具與 GitHub Actions 等 CI 工具相容，便於進行自動化測試和部署。整合過程中，最常見的問題是依賴庫的版本不兼容，建議使用虛擬環境來管理依賴。
