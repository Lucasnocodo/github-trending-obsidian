...
**524** stars · **12** stars/天 · 建立 42 天前 · N/A · CERN-OHL-S-2.0
...
> **授權** CERN-OHL-S-2.0 · **維護** Moderate (最後推送 16 天前) · **貢獻者** 2 人 · **參與度** Medium
> **適合** 對無人機技術和自動駕駛有興趣的開發者和研究者。
> **一句話重點** Mercury 的變形能力和多功能攝影設計讓它在無人機市場中脫穎而出。
...
Mercury 是一款具備變形功能的無人機，設計上能夠攜帶最多 1 公斤的內部載荷，並配備 RGB、深度和熱成像攝影機，適合多種應用場景。使用者需要將自動駕駛軟體上傳至 Raspberry Pi 5，並透過簡單的指令來啟動 Mavproxy 連接飛行控制器，這樣可以實現無人機的遠程控制。這個設計的賣點在於其簡易的轉換機制和多功能攝影能力，讓使用者能夠在不同環境中靈活運用。技術上，這個專案使用了 Python 來處理自動駕駛邏輯，並透過 Raspberry Pi 進行硬體控制，這樣的選擇使得整體架構輕量且易於部署。與其他無人機專案相比，如 0xGF/boneyard 和 AgentSeal/codeburn，Mercury 的獨特之處在於其變形能力和多種攝影功能的整合，這在傳統無人機中並不常見。

實際使用中，這款無人機能夠在各種環境下運作，但使用者需注意其載重限制和操作距離。社群活躍度良好，開發者在最近 3 天內有持續更新，顯示出對專案的投入。這個專案目前處於 alpha 階段，適合對無人機技術有興趣的開發者或研究者，但不建議用於商業生產環境。預計未來會持續增強其自動駕駛能力和攝影功能，並可能加入更多的傳感器支持。

**技術棧**：`Python` · `Raspberry Pi 5`

## 重點功能

- 內部載荷艙 — 可攜帶最多 1 公斤的設備，適合多種應用。
- 變形機制 — 簡單的轉換設計，方便在不同環境中使用。
- 多種攝影功能 — 配備 RGB、深度和熱成像攝影機，滿足多樣化需求。
- 自動駕駛能力 — 使用 Ardupilot 和 GPS，實現自主飛行。
- 移動應用支援 — 透過手機應用進行遠程控制，提升使用便利性。

## 快速開始

1. 上傳自動駕駛軟體至 Raspberry Pi 5
```bash
...
> [!tip] 爆紅原因
> 建立 42 天內累積 524 stars（12/天），forks 63（12.0%），顯示出良好的社群參與度。作者 L42ARO 和 Agonat0r 在無人機領域有一定的經驗，這個專案解決了傳統無人機在多功能性和變形能力上的不足。無人機技術的進步和 Raspberry Pi 的普及使得這個專案的實現變得可行。高達 12% 的 forks/stars 比率顯示出使用者對這個專案的實際修改和使用意願。

## 適合誰使用

**目標受眾**：對無人機技術和自動駕駛有興趣的開發者和研究者。

> [!example] 使用場景
> - 無人機愛好者用它來進行空拍和環境監測，因為其具備 RGB、深度和熱成像攝影機，能夠滿足多種拍攝需求。
> - 研究人員用它來進行實地數據收集，因為其內部載荷可攜帶各種傳感器，並且具備自動駕駛能力。
> - 開發者用它來測試自動駕駛演算法，因為其開放的軟體架構和 Raspberry Pi 支持使得實驗環境搭建變得簡單。

## 架構分析

Mercury 的架構設計選擇了 Raspberry Pi 作為控制單元，這樣的選擇使得整體系統輕量且易於部署。資料流方面，無人機的控制指令透過 Mavproxy 進行傳遞，並且使用 Python 處理自動駕駛邏輯，這樣的設計使得開發者能夠快速調整和擴展功能。選擇 Raspberry Pi 的代價是可能在處理高運算需求時出現性能瓶頸，特別是在高解析度攝影時。整體而言，這樣的架構適合小型專案，但在大型應用中可能需要更強的硬體支持。

## 技術深入分析

Mercury 的核心技術機制在於其變形設計和多種攝影功能的整合，這使得無人機能夠在不同環境中靈活運用。使用 Python 作為主要開發語言，透過 Raspberry Pi 進行控制，這樣的選擇使得開發者能夠快速調整和擴展功能。效能方面，無人機的載重限制為 1 公斤，這對於某些應用可能不足。設計上選擇 Raspberry Pi 的代價是可能在處理高運算需求時出現性能瓶頸，特別是在高解析度攝影時。這個選擇使得整體系統輕量，但在大型應用中可能需要更強的硬體支持。對於無人機的自動駕駛功能，使用了 Mavproxy 來連接飛行控制器，這樣的設計使得無人機能夠在不同網路環境下運作。整合方面，這個專案與 Raspberry Pi 的生態系統相容性良好，能夠輕鬆整合到現有的開發環境中。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝指引和範例，讓新手能夠快速上手。安裝過程相對順暢，但需要注意 Raspberry Pi 的配置。文件沒有提供多語言支持，主要以英文為主。

## 優缺點分析

> [!success] 優點
> - 變形設計使得無人機在多種環境中靈活運用。
> - 多種攝影功能滿足不同需求，適合多樣化應用。
> - 開放的軟體架構便於開發者進行擴展和修改。

> [!danger] 缺點
> - 內部載荷限制可能不適合某些專業應用。
> - 需要穩定的網路連接以進行遠程控制，增加操作複雜度。
> - 目前仍在 alpha 階段，可能存在不穩定性。

> [!warning] 注意事項
> - 僅支援 Raspberry Pi 5，其他版本可能不兼容。
> - 內部載荷限制為 1 公斤，對於某些應用可能不足。
> - 需要穩定的網路連接以進行遠程控制。
...
| 工具 | 差異 |
| --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard 專注於無人機的硬體設計，而 Mercury 提供了變形功能和多種攝影能力，適合更廣泛的應用場景。 |
| [AgentSeal/codeburn](https://github.com/AgentSeal/codeburn) | Codeburn 提供了更強的自動駕駛功能，但缺乏 Mercury 的變形設計，適合需要高自動化的專案。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於無人機的硬體設計，提供詳細的組裝指導。 | 如果你的專案需要更強的硬體設計支持，Boneyard 會是更好的選擇。 | medium，因為需要重新設計硬體架構。 |
| [AgentSeal/codeburn](https://github.com/AgentSeal/codeburn) | 提供強大的自動駕駛功能，專注於軟體層面。 | 如果你的團隊需要高自動化的無人機，Codeburn 會是更合適的選擇。 | high，因為需要重新調整整體架構。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **Mercury-Transforming-Drone** | **boneyard** | **codeburn** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於無人機的硬體設計，提供詳細的組裝指導。 | 提供強大的自動駕駛功能，專注於軟體層面。 |
> | 遷移成本 | - | medium，因為需要重新設計硬體架構。 | high，因為需要重新調整整體架構。 |
> | 適用場景 | 主要場景 | 如果你的專案需要更強的硬體設計支持，Boneyard 會是更 | 如果你的團隊需要高自動化的無人機，Codeburn 會是更合 |
...
- **[HIGH]** 在高負載情況下，無人機可能會出現性能瓶頸
  - 解法：減少載荷或降低攝影解析度以提高性能
- [MEDIUM] 網路不穩定時，遠程控制可能會失敗
  - 解法：確保使用穩定的 Wi-Fi 連接或使用 Tailscale 進行連接

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型無人機開發專案 | 非常適合 | 具備變形和多功能攝影能力，適合多樣化應用。 |
| 商業用途的無人機服務 | 不適合 | 目前仍在 alpha 階段，穩定性不足。 |
| 學術研究和實驗 | 適合 | 開放的軟體架構便於進行實驗和調整。 |
| 大型商業無人機操作 | 不適合 | 載重限制和性能瓶頸可能無法滿足需求。 |
...
> [!warning] 安全性快速掃描
> 低風險：無人機本身不需要高權限操作，但需注意網路安全，避免未授權訪問。
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["自動駕駛","無人機技術","攝影技術"];
