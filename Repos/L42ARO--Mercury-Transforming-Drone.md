...
use_case: "提供一個可變形的無人機，具備多種感測器和自動化功能，適合各種應用場景。"
priority: medium
ring: assess
...
aliases:
  ...
  - "提供一個可變形的無人機，具備多種感測器和自動化功能，適合各種應用場景。"
---

# Mercury-Transforming-Drone

**523** stars · **13** stars/天 · 建立 39 天前 · N/A · CERN-OHL-S-2.0
...
> **授權** CERN-OHL-S-2.0 · **維護** Moderate (最後推送 13 天前) · **貢獻者** 2 人 · **參與度** Medium
> **適合** 對無人機開發有興趣的獨立開發者或小型團隊，尤其是需要快速原型設計的使用者。
> **一句話重點** 這個專案的設計讓無人機的使用變得更簡單，特別適合快速原型開發和教育用途。
...
> [!abstract] 核心創新
> 簡單的變形機制和多功能感測器配置，使無人機能夠靈活應用於多種場景。

## 專案簡介

Mercury-Transforming-Drone 是一款具備變形能力的無人機，能夠攜帶最多 1 公斤的內部有效載荷，並配備 RGB、深度和熱成像相機，適合多種應用。使用者需要將 Autonomy Software 上傳至 Raspberry Pi 5，並透過兩個終端運行 Mavproxy Bridge 和主軟體，這樣便能在同一網路下控制無人機。這種設計使得無人機能夠在不同環境中靈活運用，並且透過 Tailscale 進行遠程控制，增強了其操作的便利性。此專案的賣點在於其簡單的變形機制和多功能的感測器配置，讓使用者能在多種場景下靈活應用。技術上，該專案依賴於 Raspberry Pi 和 Python 環境，並使用了 Mavproxy 來與飛行控制器進行通訊，這樣的選擇使得整體架構相對簡單且易於維護。

與其他無人機專案相比，如 0xGF/boneyard 或 AgentSeal/codeburn，Mercury 提供了更簡單的組裝和操作流程，並且在感測器的多樣性上具有優勢。實際使用中，無人機的性能和穩定性取決於所選擇的硬體和環境配置，使用者可能會遇到網路連接不穩定的問題，這需要透過適當的網路設置來解決。從成熟度來看，該專案目前處於 beta 階段，適合小型團隊或愛好者進行實驗和開發，未來可能會進一步增強自動化和智能化功能。對於需要快速部署和測試無人機應用的開發者來說，這是一個值得考慮的選擇。

**技術棧**：`Python 3` · `Raspberry Pi 5`

## 重點功能

- 內部有效載荷艙 — 可攜帶最多 1 公斤的物品，適合多種應用。
- 簡單的變形機制 — 使無人機能夠在不同環境中靈活運用。
- 多種感測器配置 — 包含 RGB、深度和熱成像相機，提供豐富的數據收集能力。
- 基於 Raspberry Pi 的自動化軟體 — 簡化了無人機的控制和操作流程。
- 支援遠程控制 — 可透過 Tailscale 進行長距離控制，增強操作便利性。

## 快速開始

1. 將 Autonomy Software 上傳至 Raspberry Pi 5
```bash
scp -r AutonomySoftware user@raspberrypi:/path/to/destination
```
2. 創建虛擬環境並安裝依賴
```bash
python3 -m venv venv && source venv/bin/activate && pip install -r requirements.txt
```
3. 啟動 Mavproxy Bridge 和主軟體
```bash
...
> [!tip] 爆紅原因
> 建立 39 天就累積 523 stars（約 13 stars/天），forks 數量為 63（12.0%），顯示出相對活躍的社群參與。專案的主要貢獻者 L42ARO 和 Agonat0r 具備相關背景，並且在無人機領域有一定的經驗。這個專案解決了無人機在多功能應用上的痛點，之前的方案往往在感測器配置或操作上較為複雜，Mercury 則提供了一個更簡單的解決方案。社群的活躍度和開發者的參與度可能是推動其快速增長的原因之一。

## 適合誰使用

**目標受眾**：對無人機開發有興趣的獨立開發者或小型團隊，尤其是需要快速原型設計的使用者。

> [!example] 使用場景
> - 無人機開發者用它來快速測試不同的感測器配置，因為其簡單的組裝和操作流程能夠節省時間。
> - 環境監測團隊用它來收集多種環境數據，因為其具備 RGB、深度和熱成像相機，能夠提供多樣的數據來源。
> - 教育機構用它來進行無人機相關的實驗和教學，因為其開放的設計和易於操作的特性適合學生使用。

## 架構分析

該專案採用 Raspberry Pi 作為控制核心，並透過 Python 實現自動化功能。資料流中，無人機的感測器數據會被收集並發送至 Raspberry Pi，然後進行處理和分析。選擇 Raspberry Pi 的原因在於其成本效益和社群支持，但這也意味著在處理性能上可能會有瓶頸，尤其在高負載情況下。擴展性方面，若需增加更多感測器或功能，可能需要進一步的硬體升級和軟體調整。

## 技術深入分析

Mercury-Transforming-Drone 的核心技術在於其基於 Raspberry Pi 的自動化系統，這使得無人機的控制和數據處理變得相對簡單。使用 Python 作為主要開發語言，這樣的選擇使得開發者能夠快速上手並進行修改。該系統能夠處理來自多種感測器的數據，並通過 Mavproxy 與飛行控制器進行通訊，這樣的架構設計使得無人機能夠在多種環境中靈活運用。效能方面，無人機的運行效率取決於 Raspberry Pi 的性能，可能在高負載情況下出現延遲。設計取捨上，選擇 Raspberry Pi 作為控制核心雖然降低了成本，但也限制了處理能力，未來可能需要考慮更高效的硬體來提升性能。技術風險方面，若無法保持穩定的網路連接，將會影響無人機的遠程控制能力，這在實際應用中需要特別注意。整合方面，該專案與現有的無人機控制系統相容性良好，但在 CI/CD pipeline 的整合上可能需要額外的調整。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了基本的安裝和使用指導，但缺乏詳細的範例。安裝過程相對順暢，對於有 Raspberry Pi 經驗的使用者來說不會有太多困難。整體來說，對於新手來說，可能需要額外的學習時間來熟悉操作流程。

## 優缺點分析

> [!success] 優點
> - 簡單易用的設計，適合初學者和小型團隊。
> - 多樣的感測器配置，能夠滿足不同的應用需求。
> - 具備遠程控制功能，增強了操作的靈活性。
...
> - 無法處理超過 1 公斤的有效載荷，限制了某些應用場景。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供更高的載重能力，但操作和組裝較為複雜，適合專業使用者。 |
| [AgentSeal/codeburn](https://github.com/AgentSeal/codeburn) | 專注於自動化和智能控制，功能更為強大，但學習曲線較陡。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 使用更高效的硬體，能夠處理更大的載重。 | 如果需要更高的載重能力和性能，且不介意較複雜的設置。 | medium，因為需要重新配置硬體和軟體環境。 |
| [AgentSeal/codeburn](https://github.com/AgentSeal/codeburn) | 專注於自動化和智能控制，提供更強大的功能。 | 如果需要更高級的自動化功能，且團隊具備相關技術能力。 | high，因為需要全面了解新的架構和功能。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **Mercury-Transforming-Drone** | **boneyard** | **codeburn** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 使用更高效的硬體，能夠處理更大的載重。 | 專注於自動化和智能控制，提供更強大的功能。 |
> | 遷移成本 | - | medium，因為需要重新配置硬體和軟體環境。 | high，因為需要全面了解新的架構和功能。 |
> | 適用場景 | 主要場景 | 如果需要更高的載重能力和性能，且不介意較複雜的設置。 | 如果需要更高級的自動化功能，且團隊具備相關技術能力。 |
...
> [!tip] 採用建議
> 適合小型團隊進行實驗和開發，但不建議用於生產環境的核心應用。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在高負載下，無人機可能出現性能下降的問題。
  - 解法：減少同時運行的感測器數量以降低負載。
- [MEDIUM] 網路連接不穩定會影響遠程控制功能。
  - 解法：確保使用穩定的網路環境，或考慮使用有線連接。
- [MEDIUM] 安裝過程中可能會遇到依賴包版本不匹配的問題。
  - 解法：仔細檢查 requirements.txt 中的版本，並根據需要進行調整。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型無人機開發團隊 | 非常適合 | 簡單的設計和易於操作的特性使其成為快速原型開發的理想選擇。 |
| 教育機構進行無人機實驗 | 適合 | 開放的設計和多樣的感測器配置適合學生學習和實驗。 |
| 需要高性能無人機的專業團隊 | 不適合 | 載重能力和性能限制無法滿足專業需求。 |
| 環境監測團隊 | 普通 | 雖然具備多種感測器，但在高負載情況下性能可能不足。 |
...
> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限，且不存取敏感資料，但在遠程控制時需確保網路安全性。

## 生態系整合

> [!abstract] 如何融入你的工具鏈

Mercury-Transforming-Drone 最常與 Raspberry Pi 和 Python 生態系統搭配使用，適合在開發和測試階段進行集成。在一個使用 Raspberry Pi 的無人機專案中，使用者可以透過 SCP 將 Autonomy Software 上傳，然後運行相應的腳本來啟動無人機控制系統。該專案與 GitHub Actions 等 CI 工具相容，但在整合時可能需要額外的配置以確保無縫運作。常見的整合問題包括依賴包版本不匹配，這需要在安裝過程中仔細檢查。
