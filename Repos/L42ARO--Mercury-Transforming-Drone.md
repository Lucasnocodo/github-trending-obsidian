...
use_case: "一款可變形的無人機，適合多種用途，具備靈活的移動性、廣泛的感測器和貨艙空間。"
priority: medium
ring: assess
...
star_history: "2026-03-10:364,2026-03-11:364,2026-03-11:364,2026-03-13:380,2026-03-14:396,2026-03-15:400,2026-03-16:402"
tags:
  - github
  - "category/其他"
  - "lang/python"
aliases:
  ...
  - "一款可變形的無人機，適合多種用途，具備靈活的移動性、廣泛的感測器和貨艙空間。"
---

# Mercury-Transforming-Drone

**402** stars · **45** stars/天 · 建立 9 天前 · Python · CERN-OHL-S-2.0
...
> **授權** CERN-OHL-S-2.0 · **維護** Active (最後推送 4 天前) · **貢獻者** 2 人 · **參與度** Medium
> **適合** 對無人機技術有興趣的開發者和研究者，尤其是需要自定義解決方案的使用者。
> **一句話重點** 這款無人機的設計不僅靈活多變，還能滿足多種實際需求，特別適合開源愛好者和研究者。
...
> [!abstract] 核心創新
> 提供了一個可變形的無人機設計，適合多種用途和自定義需求。

## 專案簡介

Mercury 是一款可變形的無人機，設計上旨在適應多種使用情境。用戶可以透過簡單的組裝和配置，將其轉換為不同的模式，滿足特定需求。其核心功能包括 1 公斤的內部貨艙、RGB、深度及熱成像攝影機，並搭載 Ardupilot 與 GPS 系統，提供高效的自動駕駛能力。使用者需將自動化軟體上傳至 Raspberry Pi 5，並透過命令行啟動 Mavproxy 進行飛行控制，這樣的設計使得無人機能夠在多種環境下運作，並且可透過網路遠程控制。這種設計選擇使得無人機在不同的應用場景中都能展現出靈活性和適應性，特別適合需要快速部署和多功能操作的情境。

與其他無人機相比，如 DJI 的 Phantom 系列，Mercury 提供了更高的自定義性和擴展性，因為用戶可以根據需求選擇不同的感測器和配件。這使得它在小型企業或研究機構中，尤其是那些需要特定任務的團隊中，顯得特別有用。使用者在實際操作中可能會遇到需要配置網路連接的挑戰，但這也提供了遠程操控的便利性。這個專案目前處於活躍開發階段，且社群活躍度良好，適合對無人機技術有興趣的開發者和研究者。預計未來將會有更多的功能和擴展，特別是在自動化和感測器整合方面。

**技術棧**：`Python` · `TypeScript` · `JavaScript` · `C++`

## 重點功能

- 內部貨艙 — 可容納 1 公斤的貨物，適合運輸小型設備或感測器。
- 簡單變形機制 — 允許用戶快速轉換無人機模式以適應不同任務。
- 多種攝影機 — 配備 RGB、深度和熱成像攝影機，滿足不同的拍攝需求。
- 自動駕駛支持 — 使用 Ardupilot 和 GPS 技術，實現高效的自動駕駛功能。
- 遠程控制功能 — 支持透過網路進行遠程控制，便於在不同環境下操作。
...
> [!tip] 爆紅原因
> 建立 9 天內累積 402 stars（44.7/天），forks 49（12.2%），這顯示出不錯的社群反響。作者 L42ARO 和 Agonat0r 具備相關技術背景，解決了無人機自定義和多用途的需求，這在市場上是相對少見的。專案的設計使得用戶能夠快速組裝和使用，這在 DIY 和開源社群中引起了興趣。社群的活躍度和開發者的反饋也促進了專案的快速成長。

## 適合誰使用

**目標受眾**：對無人機技術有興趣的開發者和研究者，尤其是需要自定義解決方案的使用者。

> [!example] 使用場景
> - 無人機愛好者用它來建造個人化的無人機，因為其模組化設計讓使用者能夠根據需求選擇不同的感測器和功能。
> - 小型企業用它來進行環境監測，因為其具備多種感測器，能夠收集不同類型的數據，並且可遠程控制。
> - 研究機構用它來進行實驗，因為其開源特性和可擴展性使得研究人員能夠快速調整和測試不同的配置。

## 架構分析

Mercury 的架構設計以 Raspberry Pi 5 為核心，所有自動化軟體和控制邏輯都運行在此平台上。資料流從感測器（如 RGB 和熱成像攝影機）收集數據，並通過 Ardupilot 進行飛行控制。這種設計使得無人機能夠在不同的環境中靈活運作，並且支持多種感測器的擴展。選擇 Raspberry Pi 5 作為控制單元的原因在於其強大的計算能力和豐富的社群支持，但這也意味著使用者需要具備一定的技術背景來配置和維護系統。擴展性方面，無人機的模組化設計使得未來可以輕鬆添加新功能或感測器，這對於需要快速迭代的應用場景非常有利。

## 技術深入分析

Mercury 的核心技術機制基於 Raspberry Pi 5，並使用 Python 和 C++ 進行控制和數據處理。無人機的感測器數據通過 Ardupilot 進行整合，這使得其在自動駕駛和環境感知方面表現出色。效能上，這款無人機能夠在多種環境下運行，並且支持 1 公斤的貨物運輸，這在同類產品中具有競爭力。設計上選擇 Raspberry Pi 5 的原因在於其強大的計算能力和廣泛的社群支持，但這也帶來了對使用者技術能力的要求。技術風險方面，使用者可能在擴展功能時遇到兼容性問題，特別是在添加新感測器時。整合方面，與主流開發工具和 CI/CD 流程的兼容性良好，但對於初學者來說，學習曲線可能較為陡峭。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝步驟和範例，但對於初學者來說，組裝和配置過程可能會有些挑戰。安裝過程相對順暢，但需要使用者具備一定的技術背景。整體來說，文件對於新手友好度尚可，但缺乏多語言支持。
...
> - 模組化設計，便於用戶根據需求進行自定義。
> - 多種感測器支持，適合不同的應用場景。
> - 開源特性，社群支持良好，便於獲取資源和幫助。

> [!danger] 缺點
> - 需要一定的技術背景來配置和維護系統。
> - 僅支援 Raspberry Pi 5，限制了硬體選擇。
> - 對於新手來說，組裝和配置過程可能較為複雜。

> [!warning] 注意事項
> - 僅支援 Raspberry Pi 5，其他版本可能無法運行。
> - 需要穩定的網路連接以進行遠程控制。
> - 對於初學者來說，無人機的組裝和配置可能有一定的學習曲線。
