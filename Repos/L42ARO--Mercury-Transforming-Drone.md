...
**518** stars · **17** stars/天 · 建立 31 天前 · N/A · CERN-OHL-S-2.0
...
> **適合** 對無人機開發和自動化有興趣的創客和開發者。
> **一句話重點** 這款無人機的設計不僅靈活，還提供了開放的開發環境，適合有創意的開發者進行實驗。
...
> [!abstract] 核心創新
> 提供簡單的變形機制和多功能的攝影系統，讓無人機的應用更加靈活。

## 專案簡介

Mercury-Transforming-Drone 是一款設計用於多種任務的可變形無人機，具備 1 公斤的內部載荷空間，並搭載 RGB、深度及熱成像攝影機。使用者可以透過 Raspberry Pi 5 來控制其自動化功能，並且需要運行 Mavproxy Bridge 和主控制軟體來進行操作。這個工具的賣點在於其簡單的變形機制和多功能的攝影系統，讓使用者能夠在不同的環境中靈活應用。技術上，該專案使用 Python 作為主要開發語言，並依賴於 Raspberry Pi 的運算能力來處理數據和控制無人機行為。與其他無人機解決方案相比，例如 DJI 的商業無人機，Mercury 提供了更高的自定義性和開放性，適合開發者和創客進行二次開發。

實際使用中，這款無人機能夠在 1 公斤的載重下運行，並且能夠進行長距離控制，若使用 Tailscale 可進一步擴展其控制範圍。雖然目前專案仍在活躍開發中，但其設計理念和功能已經相當成熟，適合小型團隊或個人開發者進行實驗和創新。未來可能會加入更多的自動化功能和擴展模組，以提升其應用場景的多樣性。對於需要進行無人機開發和測試的開發者來說，這是一個值得考慮的選擇，尤其是在需要靈活性和可擴展性的情況下。

**技術棧**：`Python`

## 重點功能

- 內部載荷空間 — 支持 1 公斤的內部載荷，適合各種任務。
- 簡單變形機制 — 便於在不同環境中快速調整形態。
- 多種攝影機 — 搭載 RGB、深度和熱成像攝影機，提供多樣化的數據收集。
- 基於 Raspberry Pi 的控制 — 使用者可以輕鬆上手並進行自定義開發。
- 支持遠程控制 — 可透過 Tailscale 擴展控制範圍，適合長距離操作。
...
> [!tip] 爆紅原因
> 建立 31 天內累積 518 stars（每天 17），forks 61（11.8%），顯示出相對活躍的社群參與。作者 L42ARO 和 Agonat0r 擁有背景知識和經驗，能夠針對無人機的自動化和變形技術進行深入開發。這個專案解決了傳統無人機在靈活性和自定義性上的不足，特別是對於開發者和創客來說，提供了一個開放的架構來進行實驗和創新。社群的活躍度和開放的設計理念使得這個專案在技術討論中受到關注。

## 適合誰使用

**目標受眾**：對無人機開發和自動化有興趣的創客和開發者。

> [!example] 使用場景
> - 無人機開發者用它來設計和測試新型無人機，因為其開放的架構和可變形設計讓創新變得容易。
> - 學術研究人員用它來進行環境監測，因為搭載的多種攝影機能夠提供多樣的數據收集方式。
> - 創客用它來進行 DIY 項目，因為提供的 STL 檔案和自動化軟體能快速組裝和測試。

## 架構分析

Mercury-Transforming-Drone 的架構設計以 Raspberry Pi 為核心，利用其運算能力來控制無人機的自動化功能。資料流從攝影機收集數據，經由 Raspberry Pi 處理後再傳送至控制端。選擇 Raspberry Pi 是因為其開放性和擴展性，雖然這意味著需要使用者具備一定的技術背景來進行設置。整體架構的設計使得無人機能夠在不同的環境中靈活應用，但也可能在高負載情況下出現性能瓶頸。未來可能需要考慮更高效的硬體來提升處理速度和穩定性。

## 技術深入分析

Mercury-Transforming-Drone 的核心技術機制是基於 Raspberry Pi 的自動化控制，利用 Python 語言來實現各種功能。其資料流從攝影機收集 RGB、深度和熱成像數據，然後經由 Raspberry Pi 進行處理，最終透過 Mavproxy Bridge 傳送至控制端。這樣的設計使得無人機能夠在不同環境中靈活運作，但也可能在高負載下出現性能瓶頸。選擇 Raspberry Pi 作為控制核心的優勢在於其開放性和可擴展性，但也意味著使用者需要具備一定的技術背景來進行設置和維護。未來的發展方向可能會集中在提升無人機的自動化程度和擴展其應用場景，特別是在環境監測和救援任務中。整合方面，這款無人機可以與其他開源硬體進行配合，但可能需要額外的適配器或轉換工具來實現無縫整合。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了基本的安裝和使用說明，但缺乏詳細的範例和文檔。安裝過程相對順暢，但對於新手來說，可能需要額外的時間來理解無人機的控制邏輯。整體上，對於有經驗的開發者來說，能夠在 30 分鐘內完成設置並運行起來。文檔目前僅提供英文版本，沒有多語言支持。
...
> - 對於新手來說，學習曲線較陡峭。
> - 目前文檔不足，可能影響使用體驗。
> - 依賴 Raspberry Pi，限制了硬體選擇。

> [!warning] 注意事項
> - 目前僅支持 Raspberry Pi 5，無法在其他平台運行。
> - 對於初學者，可能需要額外的學習時間來理解無人機的控制邏輯。
> - 缺乏詳細的文檔和使用範例，可能會影響新手的上手速度。
