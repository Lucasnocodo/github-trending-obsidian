...
use_case: "提供一個可變形的無人機，具備多種攝影功能與自動駕駛能力。"
priority: medium
ring: assess
...
aliases:
  ...
  - "提供一個可變形的無人機，具備多種攝影功能與自動駕駛能力。"
---

# Mercury-Transforming-Drone

**543** stars · **8** stars/天 · 建立 71 天前 · N/A · CERN-OHL-S-2.0
...
> **授權** CERN-OHL-S-2.0 · **維護** Slow (最後推送 45 天前) · **貢獻者** 2 人 · **參與度** Medium
> **適合** 對無人機開發有興趣，並希望探索自動駕駛與變形技術的開發者。
> **一句話重點** Mercury 的變形設計讓它在無人機市場中脫穎而出，特別適合開發者進行定制化應用。
...
> [!abstract] 核心創新
> 無人機具備簡單的變形機制，能夠根據任務需求靈活改變形狀。

## 專案簡介

Mercury-Transforming-Drone 是一款具備變形功能的無人機，設計上強調靈活性與多功能性。它的核心機制是透過簡單的變形機制來改變無人機的形狀，並能夠攜帶最多 1 公斤的內部有效載荷。無人機配備 RGB、深度及熱成像攝影機，並使用 Ardupilot 與 GPS 進行自動駕駛，這使得它在各種環境中都能有效運作。使用者需將自動駕駛軟體上傳至 Raspberry Pi 5，並透過指令 `python3 -m venv venv` 和 `pip install -r requirements.txt` 安裝依賴，這樣便可啟動無人機的控制系統。這個工具的賣點在於其靈活的變形設計與多種攝影功能，適合用於各種監控與探測任務。

在技術實作上，該專案使用 Raspberry Pi 5 作為控制單元，並依賴 Python 環境來運行其自動駕駛軟體。這樣的設計選擇使得整體系統能夠保持輕量，且易於維護。

與其他無人機系統相比，如 DJI 的商業無人機，Mercury 提供了更高的自訂性和開放性，特別適合開發者進行二次開發。相對於傳統無人機，Mercury 的變形能力使其在狹窄空間中更具靈活性。

實際使用中，無人機的效能取決於其搭載的攝影設備與自動駕駛算法，能夠在多種環境下運行，但對於高風速或複雜地形的適應性仍需進一步測試。社群活躍度尚可，開發者定期更新，且目前無開放問題，顯示出良好的維護狀態。整體來看，這是一個值得關注的專案，特別適合對無人機開發與應用有興趣的開發者。

不過，對於不熟悉 Raspberry Pi 或 Python 的使用者來說，初期的設置可能會有一定的學習曲線。未來 6 個月內，預期會有更多功能的擴展，尤其是在自動駕駛與影像處理方面的應用。

**技術棧**：`Raspberry Pi 5` · `Python`

## 重點功能

- 內部有效載荷艙 — 可攜帶最多 1 公斤的貨物，適合多種任務。
- 簡單變形機制 — 透過簡單的機械設計實現無人機形狀的變化。
- 多種攝影功能 — 配備 RGB、深度和熱成像攝影機，滿足不同的監控需求。
- 基於 Raspberry Pi 的控制系統 — 使用者可以輕鬆安裝和配置，支援 Python 環境。
- 自動駕駛功能 — 結合 Ardupilot 和 GPS，實現高效的自動導航。
...
```bash
{
  "前置條件": "需在 Raspberry Pi 上安裝 Python 環境",
  "指令": "python3 -m venv venv\nsource venv/bin/activate\npip install -r requirements.txt",
  "預期輸出": "成功啟動虛擬環境並安裝所有依賴"
}
```
...
> [!tip] 爆紅原因
> 建立 71 天內累積 543 stars（約 8 stars/天），forks 65（12%），顯示出不錯的社群關注度。這個專案的作者 L42ARO 和 Agonat0r 在無人機技術方面有一定的經驗，專案解決了市場上無人機靈活性不足的痛點，特別是在變形與多功能攝影方面。社群的反應熱烈，可能與其簡單易用的設置流程有關，並且無人機的多樣化應用場景吸引了開發者的興趣。

## 適合誰使用

**目標受眾**：對無人機開發有興趣，並希望探索自動駕駛與變形技術的開發者。

> [!example] 使用場景
> - 無人機開發者用它來設計和測試新型變形無人機，因為其開放的設計與可擴展性讓開發過程更靈活。
> - 環境監測團隊用它來進行地形勘查，因為其搭載的熱成像攝影機能夠在不同環境中提供多樣的數據。
> - 學術研究者用它來進行無人機自動駕駛算法的實驗，因為其基於 Raspberry Pi 的架構易於進行軟體修改和測試。

## 架構分析

該專案採用 Raspberry Pi 5 作為控制單元，這使得整體系統保持輕量且易於維護。資料流方面，無人機的控制系統透過 Python 腳本來實現自動駕駛與影像處理，並使用 Mavproxy 進行與飛行控制器的介面連接。這樣的設計選擇使得開發者能夠快速修改和擴展功能，但也意味著使用者需具備一定的程式設計能力。選擇 Raspberry Pi 而非專用硬體，雖然降低了成本，但在效能上可能無法與高端商業無人機相比。整體架構的擴展性良好，但在高負載情況下可能會出現性能瓶頸，特別是在同時處理多個攝影任務時。

## 技術深入分析

Mercury-Transforming-Drone 的核心技術機制在於其變形機制，這使得無人機能夠根據任務需求靈活調整形狀。它使用 Raspberry Pi 5 作為控制單元，並透過 Python 腳本來實現自動駕駛與影像處理，這樣的設計使得開發者能夠快速修改和擴展功能。效能方面，該無人機能夠處理多種攝影任務，但在高負載情況下可能會出現性能瓶頸，特別是在同時運行多個攝影設備時。選擇 Raspberry Pi 而非專用硬體，雖然降低了成本，但在效能上可能無法與高端商業無人機相比。技術風險方面，隨著使用者數量的增加，可能會出現穩定性問題，特別是在複雜的環境中運行時。整合方面，該專案的架構設計友好，能夠與現有的無人機控制系統進行有效整合，但對於不熟悉 Python 的使用者來說，學習成本仍然存在。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝步驟和使用指令，對於新手來說相對友好。安裝過程順暢，但需要一定的 Python 知識。文件中未提供多語言支持，僅有英文版本，可能對非英語使用者造成困難。
...
> [!danger] 缺點
> - 對於不熟悉 Raspberry Pi 的使用者，設置過程可能較為複雜。
> - 在高風速環境下，無人機的穩定性可能受到影響。
> - 依賴於 Python 環境，對於非開發者來說學習曲線較陡。
...
**社群活躍度**：社群活躍度尚可，開發者定期更新，且目前無開放問題。
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["無人機技術","自動駕駛","影像處理"];
