...
use_case: "提供一個可變形的無人機解決方案，適合多種環境和任務。"
priority: medium
ring: assess
...
aliases:
  ...
  - "提供一個可變形的無人機解決方案，適合多種環境和任務。"
---

# Mercury-Transforming-Drone

**542** stars · **8** stars/天 · 建立 68 天前 · N/A · CERN-OHL-S-2.0
...
> **授權** CERN-OHL-S-2.0 · **維護** Slow (最後推送 42 天前) · **貢獻者** 2 人 · **參與度** Medium
> **適合** 對無人機有興趣並希望進行多功能開發的獨立開發者或小型團隊。
> **一句話重點** Mercury 的變形機制讓無人機在多任務環境中展現出更高的靈活性。
...
> [!abstract] 核心創新
> 簡單的變形機制使無人機能夠在多樣化任務中靈活運作。

## 專案簡介

Mercury 是一款可變形的無人機，設計上具備 1 公斤的內部載貨艙，並配備 RGB、深度及熱成像攝影機，適合多種應用場景。使用者需將自動化軟體上傳至 Raspberry Pi 5，並透過 Mavproxy 介面與飛行控制器連接。這樣的設計使得無人機能夠在不同的環境中靈活運作，並且能夠透過移動應用程式進行遠端控制。其核心賣點在於簡單的變形機制，讓無人機能夠在不同的任務中快速適應。技術上，Mercury 使用 Ardupilot 作為飛行控制系統，並依賴 GPS 進行導航，這使得其在定位與導航的精確度上有保障。與其他無人機相比，Mercury 的變形機制和內部載貨艙設計使其在執行多樣化任務時更具優勢。

相較於傳統無人機，Mercury 的設計更為靈活，適合需要快速變換任務的場景。實際使用中，無人機的效能和穩定性取決於其搭載的攝影機和控制系統，這在不同環境下可能會有變化。社群活躍度不高，但開發者的持續更新顯示出對專案的承諾。適合小型團隊或個人開發者進行實驗或專案開發，未來可能會有更多功能擴展。對於需要多功能無人機的使用者，Mercury 提供了一個不錯的選擇，但對於大型商業應用可能還需進一步的穩定性測試。

**技術棧**：`Python 3` · `Raspberry Pi 5` · `Ardupilot`

## 重點功能

- 內部載貨艙 — 可容納 1 公斤的物品，適合小型運輸任務。
- 簡單變形機制 — 允許無人機在不同任務間快速適應。
- 多種攝影機配置 — 包含 RGB、深度及熱成像攝影機，適合各種環境。
- 基於 Ardupilot 的控制系統 — 提供穩定的飛行控制和導航。
- 移動應用程式控制 — 允許用戶遠端操作無人機，方便靈活。
...
> [!tip] 爆紅原因
> 建立 68 天內累積 542 stars（每天 8），forks 64（11.8%），顯示出一定的社群關注度。作者 L42ARO 和 Agonat0r 具備開發經驗，這個專案解決了無人機在多任務環境中的靈活性問題，之前的無人機多數專注於單一功能，無法快速適應不同需求。社群的反應和使用者的需求可能促進了這個專案的成長。

## 適合誰使用

**目標受眾**：對無人機有興趣並希望進行多功能開發的獨立開發者或小型團隊。

> [!example] 使用場景
> - 無人機愛好者用它來進行空拍，因為其具備多種攝影機選擇，能夠拍攝不同環境的影像。
> - 小型企業用它來進行貨物運送，因為內部載貨艙設計可容納 1 公斤的物品，適合小型配送需求。
> - 研究人員用它來進行環境監測，因為其熱成像攝影機能夠在夜間或低能見度條件下工作。

## 架構分析

Mercury 的架構設計為基於 Raspberry Pi 的控制系統，搭配 Ardupilot 進行飛行控制，這樣的選擇使得無人機能夠在多種環境中運行。資料流方面，無人機的攝影機數據透過 Mavproxy 進行處理，並將控制信號發送至飛行控制器。這種設計的優勢在於能夠快速部署並進行多任務操作，但也帶來了對 Raspberry Pi 性能的依賴。若未來需要處理更複雜的任務，可能需要考慮更強大的硬體支持。整體而言，這種設計適合小型無人機應用，但在擴展性上可能會遇到瓶頸。

## 技術深入分析

Mercury 的核心技術機制基於 Raspberry Pi 和 Ardupilot，這使得無人機能夠在多樣化的環境中運行。使用 Python 作為主要開發語言，這讓開發者能夠快速實現功能並進行調整。效能方面，無人機的運行依賴於 Raspberry Pi 的計算能力，這在處理複雜任務時可能會成為瓶頸。設計上選擇了簡單的變形機制，這樣的選擇使得無人機能夠快速適應不同的任務，但也可能限制了其在高性能需求下的表現。未來若需擴展功能，可能需要考慮更強大的硬體支持。整合方面，Mercury 能夠與主流的無人機控制系統相容，但在特定情況下可能需要額外的適配器或轉換工具。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了基本的安裝指引，但缺乏詳細的範例。安裝過程相對順暢，對於新手來說，使用 WinSCP 進行文件傳輸是個不錯的選擇。整體上，文件的清晰度有待提高，特別是在使用範例方面。
...
> - 無法在高風速或惡劣天氣下正常運行。
> - 對於長距離控制需要額外設置 Tailscale。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供更高的自動化功能，適合需要複雜任務的無人機應用。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於無人機的硬體設計，適合需要自定義硬體的開發者。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於自動化和智能控制，提供更高的自動化功能。 | 如果你的應用需要複雜的自動化任務，則應選擇這個工具。 | medium，因為需要重新設計控制邏輯。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於無人機硬體設計，提供更高的自定義選擇。 | 如果你需要自定義硬體並進行深度調整，則應選擇這個工具。 | high，因為需要重新設計整個系統架構。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **Mercury-Transforming-Drone** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於自動化和智能控制，提供更高的自動化功能。 | 專注於無人機硬體設計，提供更高的自定義選擇。 |
> | 遷移成本 | - | medium，因為需要重新設計控制邏輯。 | high，因為需要重新設計整個系統架構。 |
> | 適用場景 | 主要場景 | 如果你的應用需要複雜的自動化任務，則應選擇這個工具。 | 如果你需要自定義硬體並進行深度調整，則應選擇這個工具。 |
...
- **[HIGH]** 在高風速下無法穩定飛行
  - 解法：避免在惡劣天氣中使用
- [MEDIUM] Raspberry Pi 性能不足以處理複雜任務
  - 解法：簡化任務或升級硬體
- [MEDIUM] 移動應用程式控制延遲
  - 解法：確保網路連接穩定
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型企業的貨物運送 | 非常適合 | 內部載貨艙設計能夠運送小型物品，且具備靈活性。 |
| 無人機愛好者的空拍需求 | 適合 | 多種攝影機配置能夠滿足不同拍攝需求。 |
| 大型商業應用的環境監測 | 不適合 | 在惡劣天氣下性能不佳，且穩定性不足。 |
