...
**518** stars · **16** stars/天 · 建立 32 天前 · N/A · CERN-OHL-S-2.0
...
> **適合** 對無人機技術有興趣並希望進行實驗的開發者和愛好者。
> **一句話重點** 這款無人機的變形能力和多功能攝影系統，讓它在多樣化的應用場景中表現出色。
...
> **結論** 花 8 小時學習，10 小時整合，得到靈活的無人機解決方案，值得考慮。

> [!abstract] 核心創新
> 具備變形功能的無人機，能夠在不同任務中靈活應用。

## 專案簡介

Mercury-Transforming-Drone 是一款具備變形功能的無人機，設計上可以攜帶最多 1 公斤的內部載荷。使用者可以透過簡單的轉換機制來改變無人機的形態，這使得它在不同的任務中更具靈活性。無人機配備 RGB、深度和熱成像攝影機，並且整合了 Ardupilot 和 GPS 系統，提供高精度的導航和控制。使用者需要將自動駕駛軟體上傳至 Raspberry Pi 5，並透過命令行啟動 Mavproxy 和主程式，這樣便能在同一網路下控制無人機。這個工具的賣點在於其多功能性和易於設置的特性。技術上，該專案使用 Python 來實現自動駕駛功能，並依賴於 Raspberry Pi 的運算能力，這樣的選擇使得整體架構輕量且易於部署。

與其他無人機專案相比，如 0xGF/boneyard 和 AlpinDale/parsync，Mercury 提供了更簡單的變形機制和更強的攝影能力，特別適合需要多種視覺數據的應用場景。實際使用中，無人機的性能會受到載重和環境因素的影響，並且需要穩定的網路連接以確保遠程控制的流暢性。這個專案目前處於 beta 階段，適合小型團隊或愛好者進行實驗和開發。未來可能會增加更多的自動化功能和擴展性。對於需要在戶外進行拍攝或監控的使用者，這是一個值得考慮的選擇，但如果只是需要基本的無人機功能，則可能會覺得設置過於繁瑣。

**技術棧**：`Python 3` · `Raspberry Pi 5`

## 重點功能

- 內部載荷艙 — 可攜帶最多 1 公斤的物品，適合多種任務需求。
- 簡單變形機制 — 透過簡單的操作改變無人機形狀，提升靈活性。
- 多種攝影功能 — 配備 RGB、深度和熱成像攝影機，適合不同的拍攝需求。
- 自動駕駛系統 — 整合 Ardupilot 和 GPS，提供高精度的導航能力。
- 易於設置 — 只需將軟體上傳至 Raspberry Pi 5，並執行簡單的命令即可啟動。
...
```bash
{
  "前置條件": "在 Raspberry Pi 上安裝了 Python 環境和依賴",
...
> [!tip] 爆紅原因
> 建立 32 天內累積 518 stars（16/天），forks 61（11.8%），顯示出不錯的增長潛力。專案的主要貢獻者 L42ARO 和 Agonat0r 具備相關背景，過去在無人機和自動化領域有一定的經驗。這個專案解決了無人機在多任務環境中的靈活性問題，以往的無人機多數專注於單一功能，無法有效應對多變的需求。近期的社群討論和展示影片也引起了不少關注，進一步推動了使用者的興趣。技術上，無人機的設計和功能整合在當前的無人機市場中是相對創新的，特別是在變形能力和多種攝影功能的結合上。forks/stars 比率為 11.8%，顯示出有相當比例的使用者在實際修改和使用這個專案。

## 適合誰使用

**目標受眾**：對無人機技術有興趣並希望進行實驗的開發者和愛好者。

> [!example] 使用場景
> - 無人機愛好者用它來進行戶外拍攝，因為它具備 RGB、深度和熱成像攝影機，能夠捕捉多樣的視覺數據。
> - 小型企業用它來進行農業監控，因為其變形能力和自動駕駛功能能有效覆蓋大面積的農田。
> - 工程師用它來測試自動駕駛算法，因為其開源特性和可擴展性使得測試和修改變得簡單。

## 架構分析

該專案採用 Raspberry Pi 5 作為核心運算平台，結合 Python 開發的自動駕駛軟體，這樣的設計使得整體架構輕量且易於部署。資料流方面，無人機的控制信號通過 Mavproxy 與飛行控制器進行通訊，並透過網路將影像資料回傳給使用者。選擇 Raspberry Pi 使得硬體成本低，但也限制了運算能力，可能在高負載任務中出現瓶頸。擴展性方面，使用者可以根據需求添加額外的傳感器或改進變形機制，但這需要額外的硬體和軟體整合工作。

## 技術深入分析

Mercury-Transforming-Drone 的核心技術在於其變形機制和自動駕駛系統。使用 Python 作為開發語言，並依賴 Raspberry Pi 5 的計算能力，這使得整體架構輕量且易於部署。無人機的控制信號透過 Mavproxy 與飛行控制器進行通訊，這種設計使得使用者能夠在多種環境下靈活操作。效能方面，該無人機能夠在正常條件下運行，但在高負載情況下可能會出現運算瓶頸，特別是在同時處理多個攝影功能時。

選擇 Raspberry Pi 5 作為運算平台，雖然降低了成本，但也限制了其運算能力，對於需要高性能的應用場景可能不夠理想。技術風險方面，隨著使用者需求的增加，可能會出現性能不足的問題，特別是在進行高強度任務時。整合方面，與主流開發工具的相容性良好，但在實際使用中，使用者可能需要花時間熟悉命令行操作和設定。整體而言，這是一個具潛力的專案，適合對無人機技術有興趣的開發者進行實驗和開發。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝步驟和使用說明，對於新手來說相對友好。安裝過程中需要注意 Raspberry Pi 的設定，可能會遇到一些初學者的坑。文件中缺乏多語言支持，僅提供英文內容，但整體結構清晰，易於理解。

## 優缺點分析

> [!success] 優點
> - 多功能性 — 提供變形和多種攝影功能，適合多種應用場景。
> - 易於設置 — 簡單的安裝步驟和命令行操作，降低了使用門檻。
> - 開源社群 — 具備開源特性，使用者可以自由修改和擴展功能。

> [!danger] 缺點
> - 硬體依賴 — 需要特定的硬體（如 Raspberry Pi 5）來運行。
> - 設置複雜 — 對於初學者來說，可能需要時間來熟悉命令行操作。
> - 性能限制 — 在高負載情況下可能會遇到運算瓶頸。
...
| 工具 | 差異 |
| --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供基本的無人機功能，但缺乏變形能力和多功能攝影系統，適合需要簡單無人機操作的使用者。 |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 專注於數據同步和傳輸，無法提供變形功能，適合需要數據傳輸的場景。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於基本的無人機功能，缺乏變形能力和多功能攝影系統，適合需要簡單無人機操作的使用者。 | 如果只需要基本的飛行功能而不需要變形或多攝影能力。 | low，因為功能簡單，易於上手。 |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 專注於數據同步和傳輸，無法提供變形功能，適合需要數據傳輸的場景。 | 如果你的需求主要集中在數據傳輸而非飛行能力。 | medium，因為需要重新設計數據流和控制邏輯。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **Mercury-Transforming-Drone** | **boneyard** | **parsync** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於基本的無人機功能，缺乏變形能力和多功能攝影系統，適合需要簡單無人機操作的使用者。 | 專注於數據同步和傳輸，無法提供變形功能，適合需要數據傳輸的場景。 |
> | 遷移成本 | - | low，因為功能簡單，易於上手。 | medium，因為需要重新設計數據流和控制邏輯。 |
> | 適用場景 | 主要場景 | 如果只需要基本的飛行功能而不需要變形或多攝影能力。 | 如果你的需求主要集中在數據傳輸而非飛行能力。 |
...
> [!tip] 採用建議
> 適合小型團隊或愛好者進行實驗和開發，但不建議用於生產環境的核心任務。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在高負載情況下，無人機可能會出現性能瓶頸，影響控制精度
  - 解法：避免同時啟動多個攝影功能
- [MEDIUM] 需要穩定的網路連接以確保遠程控制的流暢性
  - 解法：在使用前檢查網路狀況
- [low] 初次設置時，可能會遇到命令行操作的困難
  - 解法：參考官方文檔中的步驟進行設置
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型企業進行農業監控 | 非常適合 | 具備變形和多種攝影功能，能有效覆蓋大面積的農田。 |
| 無人機愛好者進行戶外拍攝 | 適合 | 提供多樣的攝影選項，能滿足不同拍攝需求。 |
| 大型活動的安全監控 | 普通 | 雖然具備攝影功能，但在高負載情況下可能會影響性能。 |
| 需要高性能的無人機進行科研實驗 | 不適合 | Raspberry Pi 的性能限制可能無法滿足高強度任務的需求。 |
...
> [!tip] 投入 vs 回報
> 花 8 小時學習，10 小時整合，得到靈活的無人機解決方案，值得考慮。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具本身不需要高權限，且不存取敏感資料，但使用者需確保網路安全以防止未經授權的訪問。

## 生態系整合

> [!abstract] 如何融入你的工具鏈

Mercury-Transforming-Drone 最常與 Raspberry Pi 和各種無人機控制器搭配使用，通常在開發和部署階段進行整合。在一個使用 Raspberry Pi 的專案中，使用者可以將該無人機的自動駕駛軟體上傳至 Raspberry Pi，並透過命令行啟動控制。與主流開發工具如 GitHub Actions 和 VS Code 的相容性良好，但使用者需要熟悉命令行操作。整合過程中，最常見的問題是命令行指令的使用，建議使用者提前熟悉相關指令和操作流程。
