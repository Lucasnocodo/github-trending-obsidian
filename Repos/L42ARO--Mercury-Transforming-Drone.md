...
use_case: "提供一個可變形的無人機解決方案，適合多種拍攝需求。"
priority: medium
ring: assess
...
aliases:
  ...
  - "提供一個可變形的無人機解決方案，適合多種拍攝需求。"
---

# Mercury-Transforming-Drone

**543** stars · **8** stars/天 · 建立 69 天前 · N/A · CERN-OHL-S-2.0
...
> **授權** CERN-OHL-S-2.0 · **維護** Slow (最後推送 43 天前) · **貢獻者** 2 人 · **參與度** Medium
> **適合** 對無人機技術有興趣並希望進行多功能拍攝的開發者。
> **一句話重點** Mercury-Transforming-Drone 的可變形設計讓無人機在多功能性上具備優勢，適合多種拍攝需求。
...
> [!abstract] 核心創新
> 這個專案的創新點在於其可變形設計，能夠根據需求調整無人機的形態和載重。

## 專案簡介

Mercury-Transforming-Drone 是一款可變形的無人機，設計上特別適合攜帶多種攝影設備，包括 RGB、深度和熱成像相機。使用者只需將自動化軟體上傳至 Raspberry Pi 5，並透過簡單的指令設定，即可啟動無人機的飛行控制系統。這個設計的賣點在於其內部載重艙可承載 1 公斤的設備，並且具備簡單的變形機制，讓無人機能夠在不同的環境中靈活運用。使用者可以透過兩個終端運行 Mavproxy 及主控制程式，並在同一網路下輕鬆控制無人機，這樣的設計使得操作過程相對簡單且直觀。

這個專案的技術架構基於 Raspberry Pi 和 Ardupilot，這樣的選擇使得無人機的自動化控制更為靈活，但也需要使用者具備一定的硬體操作能力。相較於其他無人機專案，Mercury 的變形機制和內部載重設計使其在多功能性上更具優勢，但可能在穩定性和性能上有所妥協。使用者在實際操作中可能會遇到網路連接不穩定的問題，這需要透過設置 Tailscale 來解決。整體來說，這個專案適合對無人機技術有興趣的開發者，特別是那些希望在不同環境中使用無人機進行拍攝的使用者。

**技術棧**：`Raspberry Pi 5` · `Ardupilot`

## 重點功能

- 內部載重艙 — 可承載 1 公斤的設備，適合多種攝影需求。
- 簡單變形機制 — 讓無人機能夠在不同環境中靈活運用。
- RGB + 深度 + 熱成像相機 — 支援多種攝影模式，滿足不同需求。
- 基於 Raspberry Pi 5 — 便於使用者進行自定義和擴展。
- Mavproxy 介面 — 透過簡單的指令控制無人機，操作直觀。
...
# 預期輸出：安裝完成後可運行無人機控制程式
```

## 為什麼值得關注

> [!tip] 爆紅原因
> 建立 69 天就累積 543 stars（7.9/天），forks 64（11.8%），顯示出一定的社群關注度。這個專案的主要貢獻者 L42ARO 和 Agonat0r 在無人機領域有一定的背景，之前的專案也涉及自動化和無人機技術。Mercury 解決了傳統無人機在多功能性上的不足，讓使用者能夠根據需求變換無人機的形態和載重，這在市場上是相對新穎的。社群的活躍度和開放的問題回報機制也促進了這個專案的快速成長。

## 適合誰使用

**目標受眾**：對無人機技術有興趣並希望進行多功能拍攝的開發者。

> [!example] 使用場景
> - 攝影師用它來在不同環境中拍攝高品質影像，因為其可變形設計能夠適應多種拍攝需求。
> - 無人機開發者用它來測試不同的自動化控制算法，因為其開放的軟體架構便於進行修改和擴展。
> - 環境監測人員用它來進行地形勘查，因為內部載重艙可搭載多種感測器，提升數據收集的靈活性。

## 架構分析

Mercury-Transforming-Drone 的架構基於 Raspberry Pi 5 和 Ardupilot，這樣的選擇使得無人機的自動化控制更為靈活。資料流方面，使用者需要將自動化軟體上傳至 Raspberry Pi，並在該平台上運行 Mavproxy 來與飛行控制器進行通訊。這種設計的優勢在於使用者可以輕鬆地進行自定義和擴展，但也要求使用者具備一定的硬體操作能力。選擇 Raspberry Pi 而非其他控制平台的代價在於，可能會影響性能和穩定性，特別是在高負載的情況下。擴展性方面，這個架構能夠支持多種感測器和攝影設備，但在實際操作中，無人機的穩定性和控制精度可能會受到影響。

## 技術深入分析

Mercury-Transforming-Drone 的核心技術機制基於 Raspberry Pi 5 和 Ardupilot，這使得無人機能夠實現自動化控制。使用者需透過 Mavproxy 與飛行控制器進行通訊，這樣的設計使得操作過程相對簡單。效能方面，無人機的載重能力為 1 公斤，能夠支援多種攝影設備，但在高負載情況下可能會影響飛行穩定性。選擇 Raspberry Pi 而非其他控制平台的原因在於其開放性和擴展性，但這也帶來了性能上的妥協。技術風險方面，無人機的變形機制可能在操作過程中導致不穩定，這需要使用者在實際操作中謹慎。整合方面，與主流無人機控制系統的相容性良好，但在硬體組裝和設定上可能會對初學者造成挑戰。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝步驟和範例，但對於初學者來說，硬體組裝可能會有一些挑戰。安裝過程相對順暢，沒有明顯的坑。文件目前僅提供英文版本，對於非英語使用者可能會造成一定障礙。
...
> - 可變形設計，靈活適應多種拍攝需求。
> - 開放的軟體架構，便於進行自定義和擴展。
> - 支援多種攝影模式，提升使用者的創作自由度。

> [!danger] 缺點
> - 需要一定的硬體組裝能力，對初學者不太友好。
> - 變形機制可能影響飛行穩定性，需謹慎操作。
> - 依賴 Raspberry Pi，性能可能受限於硬體配置。

> [!warning] 注意事項
> - 需要 Raspberry Pi 5 作為控制單元，無法在其他平台運行。
> - 變形機制可能影響飛行穩定性，需謹慎操作。
> - 對於初學者來說，硬體組裝和設定可能有一定難度。
...
**社群活躍度**：社群活躍度中等，開放的問題回報機制促進了專案的成長。
