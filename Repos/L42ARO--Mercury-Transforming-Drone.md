...
use_case: "提供一個可變形的無人機，整合多種攝影機和自動駕駛功能。"
priority: medium
ring: assess
...
aliases:
  ...
  - "提供一個可變形的無人機，整合多種攝影機和自動駕駛功能。"
---

# Mercury-Transforming-Drone

**520** stars · **14** stars/天 · 建立 36 天前 · N/A · CERN-OHL-S-2.0
...
> **適合** 對無人機開發和自動駕駛技術有興趣的獨立開發者和小型團隊。
> **一句話重點** 這個專案的變形機制和多攝影機配置使其在無人機領域中具備獨特的競爭優勢。
...
> [!abstract] 核心創新
> 具備簡單的變形機制和多種攝影機配置，適合多樣化應用。

## 專案簡介

Mercury Transforming Drone 是一款具備變形機制的無人機，能夠在空中和地面上運行。用戶只需將 Autonomy Software 上傳至 Raspberry Pi 5，並透過簡單的指令啟動 Mavproxy Bridge 和主控制程式，即可實現無人機的控制。這個設計的賣點在於其內部載貨艙可容納 1 公斤的負載，並配備 RGB、深度和熱成像攝影機，適合多種應用場景，如搜索救援或環境監測。技術上，該專案使用 Python 作為主要開發語言，並依賴於 Ardupilot 和 GPS 進行導航，這使得無人機具備高精度的自主飛行能力。

這個專案的依賴性相對輕量，主要集中在 Raspberry Pi 環境下的 Python 庫，適合小型團隊或個人開發者使用。與其他無人機專案相比，如 0xGF/boneyard 和 AlpinDale/parsync，Mercury 提供了更靈活的變形功能和多種攝影機選擇，適合需要多樣化功能的使用者。實際使用中，無人機的效能和穩定性取決於使用者的網路環境，若要實現遠距離控制，建議使用 Tailscale 進行網路配置。這個專案目前處於早期階段，適合對無人機開發有興趣的開發者，但在生產環境中使用時需謹慎評估其穩定性。

**技術棧**：`Python`

## 重點功能

- 內部載貨艙 — 可容納 1 公斤的負載，適合多種應用。
- 變形機制 — 簡單的變形設計，能在空中和地面上運行。
- 多種攝影機配置 — 配備 RGB、深度和熱成像攝影機，滿足不同需求。
- 基於 Ardupilot 的自動駕駛 — 提供高精度的導航和控制。
- 簡單的軟體設置 — 透過 Raspberry Pi 進行簡單的軟體安裝和配置。
...
> [!tip] 爆紅原因
> 建立 36 天內累積 520 stars（約 14 stars/天），forks 數量為 62（11.9%），顯示出一定的社群關注度。L42ARO 和 Agonat0r 是主要貢獻者，過去在無人機和自動駕駛領域有相關經驗。這個專案解決了無人機在多功能性和靈活性上的需求，特別是在需要變形和多種攝影機配置的場景中。社群的活躍度和開放的問題追蹤系統顯示出開發者對於改進的重視。這個專案的設計理念符合當前對於無人機技術多樣化的需求，並且在技術上具備可行性。

## 適合誰使用

**目標受眾**：對無人機開發和自動駕駛技術有興趣的獨立開發者和小型團隊。

> [!example] 使用場景
> - 無人機開發者用它來快速原型設計多功能無人機，因為其變形機制和多種攝影機配置能夠滿足不同的應用需求。
> - 環境監測專家用它來進行空中拍攝和數據收集，因為其搭載的 RGB 和熱成像攝影機能夠提供多樣的數據來源。
> - 教育機構用它來教授無人機技術和自動駕駛原理，因為其開放的架構和簡單的設置流程適合學生實驗和學習。

## 架構分析

該專案採用 Raspberry Pi 作為控制核心，並使用 Python 進行軟體開發。資料流從攝影機收集數據，經過處理後傳送至使用者介面。選擇 Raspberry Pi 是為了降低成本和簡化硬體需求，但這也限制了處理速度和擴展性。無人機的控制系統依賴於 Ardupilot，這使得其在導航和控制上具備高精度，但也需要使用者對於這些技術有一定的了解。整體架構的設計使得無人機在功能上具備靈活性，但在高負載或複雜環境下可能會面臨性能瓶頸。

## 技術深入分析

Mercury Transforming Drone 的核心技術在於其變形機制和多種攝影機配置。該專案使用 Python 作為主要開發語言，並依賴於 Ardupilot 進行自動駕駛，這使得無人機在導航和控制上具備高精度。無人機的設計考量了多種應用場景，如環境監測和搜索救援，並透過內部載貨艙提供額外的功能。效能方面，無人機的運行依賴於 Raspberry Pi 的處理能力，這可能在高負載情況下成為瓶頸。選擇 Raspberry Pi 除了降低成本外，也使得開發者能夠快速進行原型設計，但這也限制了其在資源密集型應用中的表現。整體來看，該專案的架構設計使得無人機具備靈活性，但在實際應用中，使用者需考慮到其性能和穩定性。對於無人機的控制和導航，該專案依賴於 Ardupilot 的成熟生態系統，這使得其在技術上具備可行性，但也要求使用者具備相關知識以便進行有效的操作和維護。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝指導和範例，對於新手來說相對友好。安裝過程中，使用者需要具備 Raspberry Pi 的基本操作知識，可能會遇到一些配置上的挑戰。整體來說，花 30 分鐘內能夠完成基本設置並運行起來。
...
> - 對於初學者來說，操作和設置可能有一定難度。
> - 在高負載情況下，性能可能受到影響。

> [!warning] 注意事項
> - 僅支援 Raspberry Pi 5 環境，其他平台可能不兼容。
> - 變形機制的穩定性可能受到外部環境影響。
> - 需要使用者具備一定的無人機操作知識，初學者可能需要額外學習。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供無人機的基本控制功能，但缺乏變形機制和多攝影機配置的靈活性。 |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 專注於無人機的數據同步，但不具備變形能力和多樣化的攝影機選擇。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供無人機的基本控制功能，但缺乏變形機制和多攝影機配置的靈活性。 | 如果只需要基本的無人機控制功能，而不需要變形或多攝影機配置。 | low，因為其架構相對簡單，容易上手。 |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 專注於無人機的數據同步，但不具備變形能力和多樣化的攝影機選擇。 | 如果重點在於數據同步而非無人機的變形功能。 | medium，因為需要重新設計數據處理流程。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **Mercury-Transforming-Drone** | **boneyard** | **parsync** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 提供無人機的基本控制功能，但缺乏變形機制和多攝影機配置的靈活性。 | 專注於無人機的數據同步，但不具備變形能力和多樣化的攝影機選擇。 |
> | 遷移成本 | - | low，因為其架構相對簡單，容易上手。 | medium，因為需要重新設計數據處理流程。 |
> | 適用場景 | 主要場景 | 如果只需要基本的無人機控制功能，而不需要變形或多攝影機配置。 | 如果重點在於數據同步而非無人機的變形功能。 |
...
- **[HIGH]** 在高負載情況下，無人機可能無法穩定運行。
  - 解法：減少載重或優化飛行路徑。
- [MEDIUM] 初學者可能對於設置過程感到困惑。
  - 解法：參考官方文檔或社群論壇獲取幫助。
- **[HIGH]** 在某些環境下，無人機的變形機制可能不夠穩定。
  - 解法：在平坦的地面上進行測試，避免不平整的地形。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型無人機開發團隊 | 非常適合 | 具備變形機制和多攝影機配置，能夠快速開發多功能無人機。 |
| 教育機構進行無人機技術教學 | 適合 | 開放的架構和簡單的設置流程適合學生實驗和學習。 |
| 大型商業無人機應用 | 不適合 | 目前仍處於 alpha 階段，穩定性和性能尚未達到商業需求。 |
| 環境監測專案 | 非常適合 | 多種攝影機配置能夠提供豐富的數據來源，適合進行環境監測。 |
...
> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限操作，且不會存取敏感資料，但使用者需確保網路安全性以防止未授權訪問。

## 生態系整合

> [!abstract] 如何融入你的工具鏈

Mercury Transforming Drone 通常與 Raspberry Pi 和 Ardupilot 搭配使用，適合在開發和測試階段進行快速原型設計。在一個使用 Raspberry Pi 的專案中，你可以將 Autonomy Software 上傳至 Raspberry Pi，並透過簡單的指令啟動無人機控制系統。該專案支援與多種 CI 工具的整合，並且可以與 VS Code 等 IDE 進行協作開發。整合過程中，使用者可能需要處理一些依賴問題，特別是在不同版本的 Python 庫之間。
...
**社群活躍度**：社群活躍度中等，開放的問題追蹤系統顯示出開發者對於改進的重視。
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["自動駕駛","無人機技術","環境監測"];
