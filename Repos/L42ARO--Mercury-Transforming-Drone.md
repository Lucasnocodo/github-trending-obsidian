...
**529** stars · **11** stars/天 · 建立 50 天前 · N/A · CERN-OHL-S-2.0
...
> **授權** CERN-OHL-S-2.0 · **維護** Moderate (最後推送 24 天前) · **貢獻者** 2 人 · **參與度** Medium
> **適合** 對無人機設計和自動駕駛技術有興趣的開發者和研究人員。
> **一句話重點** 這款無人機的設計理念在於簡化使用者的操作流程，讓無人機技術更容易被廣泛應用。
...
> [!abstract] 核心創新
> 提供一個簡單的變形機制，讓無人機在多種環境下靈活運用。

## 專案簡介

Mercury-Transforming-Drone 是一款具備變形能力的無人機，設計上可以攜帶 1 公斤的內部載荷，並搭載 RGB、深度和熱成像攝影機。用戶可以透過 Raspberry Pi 5 來運行其自動駕駛軟體，並使用 Mavproxy 來與飛行控制器進行接口。這樣的設計使得無人機在多種環境下都能靈活運用，特別適合需要遠程操控的應用場景。關鍵指令包括 `start_mavproxy.sh` 和 `run.sh`，這兩個指令分別啟動無人機的控制和主軟體，讓用戶能夠在同一網路下輕鬆控制無人機。這個工具的賣點在於其簡單的變形機制和多功能的攝影能力，適合用於監控、搜救等任務。

技術上，專案使用了 Ardupilot 和 GPS 進行導航，並且依賴於 Python 的虛擬環境來管理依賴。相比其他無人機專案，Mercury 提供了更簡單的組裝和操作方式，特別適合初學者。雖然目前開源社群活躍度不高，但專案的設計理念和功能仍然具有吸引力。適合小型團隊或個人開發者進行實驗和創新，未來可能會隨著功能擴展而變得更加成熟。

**技術棧**：`Python` · `Raspberry Pi 5` · `Ardupilot`

## 重點功能

- 內部載荷艙 — 可攜帶 1 公斤的設備，適合多種應用。
- 簡單變形機制 — 使無人機能夠在不同環境中靈活運用。
- RGB + 深度 + 熱成像攝影機 — 提供多種攝影功能，適合監控和搜救任務。
- 基於 Ardupilot 的自動駕駛 — 具備穩定的導航能力，適合長距離操作。
- 支援 Raspberry Pi 5 — 方便用戶進行軟體安裝和配置。
...
> [!tip] 爆紅原因
> 建立 50 天內累積 529 stars（約 11.9/天），forks 63（11.9%），顯示出一定的社群關注。作者 L42ARO 和 Agonat0r 具備相關背景，專案解決了無人機在多功能性和易用性上的痛點，特別是對於需要簡單組裝和操作的用戶。社群的活躍度雖然不高，但無開放問題顯示出專案的穩定性。技術上，無人機的設計和功能在目前的市場上相對獨特，吸引了對無人機技術有興趣的開發者。

## 適合誰使用

**目標受眾**：對無人機設計和自動駕駛技術有興趣的開發者和研究人員。

> [!example] 使用場景
> - 無人機愛好者用它來進行空中攝影，因為其具備 RGB 和熱成像攝影功能，能夠在多種環境下拍攝。
> - 小型企業用它來進行環境監控，因為其可變形設計和自動駕駛能力能夠在狹小空間中靈活運行。
> - 研究人員用它來進行數據收集，因為其內部載荷能力和多種攝影機配置能夠滿足不同的實驗需求。

## 架構分析

Mercury-Transforming-Drone 的架構設計以 Raspberry Pi 5 為核心，搭載 Ardupilot 進行自動駕駛。這樣的設計使得整體系統具備高度的可擴展性，並且能夠輕鬆整合各種攝影設備。資料流方面，無人機的控制指令透過 Mavproxy 進行傳輸，並且在 Raspberry Pi 上運行的主軟體負責處理攝影和導航的任務。

選擇 Raspberry Pi 作為控制單元的好處在於其開放性和社群支持，但也帶來了對於硬體性能的依賴。未來若要擴展功能，可能需要考慮更高效能的硬體來支援更複雜的任務。整體而言，這個架構在小型無人機應用中展現出良好的靈活性和可擴展性。

## 技術深入分析

Mercury-Transforming-Drone 的核心技術機制在於其使用 Raspberry Pi 5 作為控制單元，並結合 Ardupilot 進行自動駕駛。這樣的設計不僅提供了穩定的導航能力，還能夠支援多種攝影機的接入。效能方面，無人機的處理能力取決於 Raspberry Pi 的性能，對於簡單的任務其反應速度和穩定性都能滿足需求，但在處理更複雜的任務時可能會遇到瓶頸。設計上選擇 Raspberry Pi 而非其他專用硬體，帶來了開放性和可擴展性，但也意味著在性能上可能有所妥協。技術風險方面，隨著使用者需求的增長，可能需要考慮更高效能的硬體來支援未來的功能擴展。整合方面，與現有的無人機控制系統相容性良好，特別是在使用 Mavproxy 進行控制時，能夠輕鬆與其他系統進行整合。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了基本的安裝指導和使用說明，但缺乏詳細的範例。安裝過程相對順暢，但對於新手來說，可能需要額外的學習時間來理解各個組件的功能。整體而言，對於有一定技術基礎的用戶來說，花 30 分鐘能夠順利運行起來。
