...
use_case: "一款可變形的無人機，適合多種應用場景，具備靈活的移動性和多種傳感器。"
priority: medium
ring: assess
...
aliases:
  ...
  - "一款可變形的無人機，適合多種應用場景，具備靈活的移動性和多種傳感器。"
---

# Mercury-Transforming-Drone

**500** stars · **21** stars/天 · 建立 24 天前 · Python · CERN-OHL-S-2.0
...
> **適合** 對無人機開發有興趣的工程師和研究人員，特別是希望在開源環境中進行創新的人。
> **一句話重點** Mercury 的開源設計讓無人機的開發和應用變得更加靈活，特別適合對 DIY 和自動化有興趣的使用者。
...
> [!abstract] 核心創新
> 這個專案的創新點在於其開源設計，允許用戶根據需求自由修改和擴展無人機的功能。

## 專案簡介

Mercury 是一款可變形的無人機，設計上支持多種應用，從內部貨艙（可承載 1 公斤）到多種傳感器（RGB、深度、熱成像攝影機），使其能夠適應不同的任務需求。用戶可以將自動化軟體上傳至 Raspberry Pi 5，並透過簡單的命令設置虛擬環境與依賴，這樣就能啟動無人機的控制系統。具體來說，使用者需運行 `start_mavproxy.sh` 和 `run.sh` 兩個腳本來啟動 Mavproxy 介面和主控制軟體，並能透過網頁控制無人機。這個設計的賣點在於其靈活性和可擴展性，讓使用者可以根據需求調整無人機的功能。技術上，該專案使用 Python 來處理自動化控制，並依賴於 Ardupilot 和 GPS 進行導航，這使得無人機在各種環境中都能穩定運行。

與其他無人機相比，如 DJI 的產品，Mercury 提供了開源的靈活性，使用者可以根據自己的需求進行修改和擴展，而不是受限於商業產品的封閉生態。這使得它特別適合開發者和愛好者，尤其是在教育和研究領域。實際使用中，無人機的效能和穩定性取決於所選擇的傳感器和載荷，這可能會影響其在特定任務中的表現。社群活躍度高，無開放問題顯示出良好的維護狀況，適合有一定技術背景的團隊使用。未來，隨著更多功能的加入，這個專案可能會成為開源無人機領域的重要參考。

**技術棧**：`Python` · `TypeScript` · `C++` · `Shell`

## 重點功能

- 內部貨艙 — 可承載 1 公斤的貨物，適合多種應用。
- 簡單的變形機制 — 使無人機能夠在不同模式間快速轉換。
- 多種傳感器 — 包括 RGB、深度和熱成像攝影機，支持多樣化的數據收集。
- 基於 Ardupilot 的導航 — 提供穩定的飛行控制和 GPS 定位。
- 移動應用控制 — 允許用戶通過手機應用進行無人機控制。
...
> [!tip] 爆紅原因
> 建立 24 天內累積 500 stars（約 21 stars/天），forks 57（11.4%），顯示出良好的使用者興趣。作者 L42ARO 和 Agonat0r 具備開源專案的背景，解決了傳統無人機在靈活性和可擴展性上的不足，讓使用者能夠根據需求進行調整。無人機的開源特性吸引了對 DIY 和自動化感興趣的開發者。社群的活躍度和無開放問題的狀況也顯示出這個專案的健康發展。這些因素共同促進了其快速增長。

## 適合誰使用

**目標受眾**：對無人機開發有興趣的工程師和研究人員，特別是希望在開源環境中進行創新的人。

> [!example] 使用場景
> - 機器人愛好者用它來製作自定義無人機，因為其開源設計讓他們能夠自由修改和擴展功能。
> - 教育機構用它來進行無人機技術的實驗和教學，因為其多種傳感器和靈活性能夠支持多樣化的學習場景。
> - 研究人員用它來進行環境監測，因為其搭載的熱成像和深度攝影機能夠提供多種數據來源。

## 架構分析

Mercury 的架構設計為一個模組化系統，允許用戶根據需求選擇不同的傳感器和控制單元。核心控制邏輯由 Python 實現，並使用 Ardupilot 進行飛行控制，這使得無人機在不同環境中都能穩定運行。資料流方面，無人機的各種傳感器數據會通過 Mavproxy 進行處理，然後再傳送至用戶的控制介面。選擇 Raspberry Pi 作為控制單元的好處在於其開源生態和強大的社群支持，但這也意味著使用者需要具備一定的 Linux 知識。擴展性方面，這個設計能夠支持多種傳感器和附加模組，但可能會增加系統的複雜性和維護成本。

## 技術深入分析

Mercury 的核心技術機制是基於 Python 的自動化控制系統，並利用 Ardupilot 進行飛行控制。這樣的設計使得無人機能夠在不同環境中穩定運行，並能夠快速響應用戶的控制指令。效能方面，無人機的載重能力為 1 公斤，這對於大多數輕型任務來說是足夠的。設計選擇上，使用 Raspberry Pi 作為控制單元的優勢在於其強大的社群支持和開源生態，但這也意味著使用者需具備一定的 Linux 知識。

這個選擇的代價是系統可能會變得較為複雜，特別是在擴展功能時。技術風險方面，若在未來的擴展中未能妥善管理依賴關係，可能會導致系統的穩定性下降。與主流框架的整合方面，Mercury 可以與各種開發工具和 CI/CD pipeline 友好整合，但仍需注意對於硬體的要求和配置。整體而言，這個專案在開源無人機領域中具備相當的潛力，未來可能會吸引更多的開發者參與。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝步驟和範例，讓使用者能夠快速上手。安裝過程相對順暢，但對於初學者來說，可能需要額外的學習時間來理解無人機的組裝和編程。文件目前僅提供英文版本，未來若能增加多語言支持將更有利於擴展使用者基礎。
...
> - 開源設計，允許用戶自由修改和擴展功能。
> - 多種傳感器選擇，適合多樣化的應用場景。
> - 社群活躍，無開放問題顯示出良好的維護狀況。

> [!danger] 缺點
> - 需要一定的技術背景來設置和操作。
> - 對於初學者來說，學習曲線較陡。
> - 硬體需求較高，需使用 Raspberry Pi 5。

> [!warning] 注意事項
> - 需要 Raspberry Pi 5 作為控制單元。
> - 對於初學者，可能需要額外的學習時間來理解無人機的組裝和編程。
> - 不支援 Windows 環境，需在 Linux 系統上運行。
