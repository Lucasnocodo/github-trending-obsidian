...
**543** stars · **8** stars/天 · 建立 72 天前 · N/A · CERN-OHL-S-2.0
...
> **授權** CERN-OHL-S-2.0 · **維護** Slow (最後推送 46 天前) · **貢獻者** 2 人 · **參與度** Medium
> **適合** 對無人機技術有興趣並希望進行自定義的開發者或愛好者。
> **一句話重點** 這款無人機的變形能力和開源設計使其在多用途應用上具備優勢，特別適合對無人機技術有興趣的開發者。
...
> [!abstract] 核心創新
> 具備簡單的變形機制，能夠快速適應不同的任務需求。

## 專案簡介

Mercury-Transforming-Drone 是一款具備變形能力的無人機，設計上能夠攜帶最多 1 公斤的內部載荷，並配備 RGB、深度及熱成像攝影機，適合各種環境監測和拍攝任務。使用者需將自動駕駛軟體上傳至 Raspberry Pi 5，並透過 Mavproxy 進行飛行控制，這種設計使得無人機能夠在多種環境中靈活運作。核心賣點是其簡單的變形機制，能夠快速適應不同的任務需求。這款無人機的架構採用開源硬體設計，使用 STL 檔案進行組裝，並提供完整的 PCB 檔案，這使得使用者可以輕鬆自製或維護設備。相較於其他無人機，Mercury 的變形功能和多重攝影機配置使其在多用途應用上具備優勢，特別是在需要多種感測器的情況下。

與傳統無人機相比，Mercury 的設計強調了模組化和可擴展性，使用者可以根據需求調整載荷和感測器配置。這款無人機的社群活躍度不高，但其開源特性使得使用者能夠自由修改和擴展功能。對於需要在特定環境下執行任務的開發者來說，這是一個值得考慮的選擇。整體來看，這款無人機適合小型團隊或個人開發者，特別是那些對無人機技術有興趣並希望進行自定義的使用者。

**技術棧**：`Python` · `Raspberry Pi 5` · `Ardupilot`

## 重點功能

- 內部載荷艙 — 可攜帶最多 1 公斤的設備，方便進行多種任務。
- 簡單變形機制 — 使無人機能夠快速適應不同的操作需求。
- 多種攝影機 — 配備 RGB、深度及熱成像攝影機，適合各種環境拍攝。
- 開源硬體設計 — 提供 STL 和 PCB 檔案，方便使用者自製或維護。
- 自動駕駛功能 — 使用 Ardupilot 和 GPS 進行精確控制，提升操作靈活性。
...
```bash
# 前置條件：在 Raspberry Pi 上安裝了 Python 和依賴
python3 -m venv venv
source venv/bin/activate
pip install -r requirements.txt
# 預期輸出：成功創建虛擬環境並安裝所需依賴。
```

## 為什麼值得關注

> [!tip] 爆紅原因
> 建立 72 天內累積 543 stars（約 8 stars/天），forks 數量為 65（12%），顯示出一定的社群關注度。作者 L42ARO 和 Agonat0r 似乎在無人機領域有一定的經驗，這個專案解決了傳統無人機在多用途應用上的不足，特別是在變形和多感測器整合方面。社群的反應可能受到其開源特性和多功能設計的影響，這使得開發者能夠根據需求進行調整和擴展。這款無人機的設計理念符合當前對於靈活性和可擴展性的需求，特別是在無人機技術逐漸普及的背景下。

## 適合誰使用

**目標受眾**：對無人機技術有興趣並希望進行自定義的開發者或愛好者。

> [!example] 使用場景
> - 無人機愛好者用它來進行戶外拍攝，因為其具備 RGB、深度和熱成像攝影機，能夠捕捉多種環境影像。
> - 小型企業用它來進行環境監測，因為其可變形設計和內部載荷能力使其能夠適應不同的任務需求。
> - 開發者用它來進行無人機自動駕駛技術的實驗，因為其開源特性和完整的硬體設計文件便於修改和擴展。

## 架構分析

Mercury-Transforming-Drone 的架構設計強調模組化和可擴展性，使用 Raspberry Pi 5 作為核心控制單元，並透過 Mavproxy 與飛行控制器進行通訊。資料流方面，攝影機捕捉的影像經由 Raspberry Pi 處理後，透過網路傳輸至使用者端進行控制。這種設計使得無人機能夠在不同環境中靈活運作，但也要求使用者具備一定的技術背景來進行設置和維護。選擇 Raspberry Pi 作為控制器的好處在於其開源生態和廣泛的社群支持，然而這也意味著使用者需要自行處理可能出現的技術問題。整體而言，這種設計能夠提供高靈活性，但在擴展性上可能會受到 Raspberry Pi 的性能限制。

## 技術深入分析

Mercury-Transforming-Drone 的核心技術機制在於其變形能力，這使得無人機能夠根據任務需求進行快速調整。使用 Raspberry Pi 5 作為控制單元，無人機能夠透過 Mavproxy 與飛行控制器進行有效的通訊，這種設計使得控制過程更加靈活。無人機的攝影機配置包括 RGB、深度和熱成像攝影機，這些感測器能夠提供多樣化的數據，適合各種環境的應用。效能方面，無人機的載重能力為 1 公斤，這使得其在執行任務時具備一定的靈活性。選擇 Raspberry Pi 5 的原因在於其開源特性和廣泛的社群支持，但這也意味著使用者需要具備一定的技術背景來進行設置和維護。設計取捨方面，雖然選擇了開源硬體，但這也可能導致在某些情況下的性能瓶頸，特別是在處理大量數據時。整體而言，這款無人機的設計強調了靈活性和可擴展性，適合對無人機技術有興趣的開發者進行實驗和開發。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了基本的安裝和使用說明，但缺乏詳細的範例和使用手冊。安裝過程相對順暢，但需要使用者具備一定的技術背景。整體來說，對於新手來說，可能需要額外的學習時間來熟悉整個系統。

## 優缺點分析

> [!success] 優點
> - 具備變形能力，適應性強，能夠執行多種任務。
> - 開源設計，使用者可自由修改和擴展功能。
> - 多種攝影機配置，適合不同環境的拍攝需求。

> [!danger] 缺點
> - 對新手來說，設置過程可能較為複雜。
> - 需要特定硬體（Raspberry Pi 5），限制了使用範圍。
> - 變形機制的耐用性尚未經過長期測試。

> [!warning] 注意事項
> - 尚未提供完整的使用手冊，對於新手可能有學習曲線。
> - 需要 Raspberry Pi 5 作為控制器，無法在其他平台上運行。
> - 變形機制的耐用性尚未經過長期測試，可能會影響使用壽命。
...
- [MEDIUM] 設置過程中可能會遇到依賴問題，特別是在安裝 Python 套件時。
  - 解法：確保所有依賴都已正確安裝，並檢查版本相容性。
- **[HIGH]** 變形機制的耐用性尚未經過長期測試，可能會導致故障。
  - 解法：在使用過程中定期檢查變形機構的狀況。
- **[HIGH]** 無法在其他平台上運行，限制了使用範圍。
  - 解法：確保使用 Raspberry Pi 5 作為控制器。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 無人機愛好者進行戶外拍攝 | 非常適合 | 具備多種攝影機配置，能夠捕捉多樣化的影像。 |
| 小型企業進行環境監測 | 適合 | 可變形設計和內部載荷能力使其能夠適應不同的任務需求。 |
| 開發者進行無人機自動駕駛技術的實驗 | 非常適合 | 開源特性和完整的硬體設計文件便於修改和擴展。 |
| 大型企業進行商業用途的無人機操作 | 不適合 | 尚未經過長期測試，穩定性和耐用性未知。 |
...
> [!warning] 安全性快速掃描
> 低風險：無人機本身不需要高權限操作，但需注意網路安全，特別是在遠程控制時。
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["無人機技術","開源硬體","自動駕駛"];
