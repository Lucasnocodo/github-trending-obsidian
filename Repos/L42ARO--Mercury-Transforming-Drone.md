...
use_case: "一款可變形的無人機，集成多種攝影功能，適合多種應用場景。"
priority: medium
ring: assess
...
aliases:
  ...
  - "一款可變形的無人機，集成多種攝影功能，適合多種應用場景。"
---

# Mercury-Transforming-Drone

**536** stars · **8** stars/天 · 建立 64 天前 · N/A · CERN-OHL-S-2.0
...
> **授權** CERN-OHL-S-2.0 · **維護** Slow (最後推送 38 天前) · **貢獻者** 2 人 · **參與度** Medium
> **適合** 對無人機開發和自動化控制有興趣的開發者和工程師。
> **一句話重點** 這款無人機的設計不僅靈活，還具備多種攝影功能，適合多樣化的應用需求。
...
> [!abstract] 核心創新
> 這款無人機的變形機制和多種攝影功能的整合，使其在靈活性和應用範圍上具備優勢。

## 專案簡介

Mercury-Transforming-Drone 是一款設計精巧的無人機，具備變形能力，能夠在不同環境中靈活運用。用戶只需將自動化軟體上傳至 Raspberry Pi 5，並透過簡單的指令啟動 Mavproxy 和主程式，即可實現無人機的控制與監控。這款無人機的「一句話賣點」是其內建的 RGB、深度及熱成像攝影機，讓用戶在多種場景下收集數據。技術上，這款無人機使用 Ardupilot 作為飛行控制系統，並結合 GPS 定位，確保其在各種環境下的穩定性。相較於傳統無人機，Mercury 的變形機制和內部載貨艙設計（可承載 1 公斤）使其在運輸和拍攝任務上更具靈活性。與其他無人機相比，如 DJI 系列，Mercury 提供了更開放的硬體設計，便於用戶自定義和擴展功能。

使用者可以透過 Tailscale 進行遠端控制，這在長距離操作時特別有用。雖然目前尚無開放的問題，但社群活躍度高，顯示出良好的維護狀況。這個專案的成熟度為 beta，適合對無人機開發有興趣的開發者和愛好者。未來可能會加入更多自動化功能和擴展模組，進一步提升其應用範圍。建議在小型專案或個人實驗中使用，對於大型商業應用則需謹慎評估其穩定性和功能完整性。

**技術棧**：`Python` · `Ardupilot` · `Raspberry Pi 5`

## 重點功能

- 內部載貨艙 — 可承載 1 公斤的物品，適合運輸任務。
- 多種攝影機 — 配備 RGB、深度及熱成像攝影機，滿足不同拍攝需求。
- 變形機制 — 簡單的變形設計，提升靈活性和適應性。
- 自動化控制 — 使用 Ardupilot 和 GPS 確保飛行穩定性。
- 遠端控制 — 支援 Tailscale 進行長距離控制，方便靈活操作。
...
# 啟動主程式
./run.sh
# 預期輸出：在終端顯示 IP 地址以控制無人機。
```

## 為什麼值得關注

> [!tip] 爆紅原因
> 建立 64 天內累積 536 stars（8.4/天），forks 63（11.8%），顯示出相對高的參與度。作者 L42ARO 和 Agonat0r 在開源社群中有一定的影響力，過去參與過多個相關專案。這款無人機解決了傳統無人機在靈活性和功能擴展上的不足，特別是對於需要多種感測器的使用場景。近期的社交媒體宣傳和示範影片也吸引了不少關注。技術上，無人機的變形設計和多功能攝影機的整合使其在市場上具備競爭力。forks/stars 比率達到 11.8%，顯示出許多用戶對其進行實際修改和使用。

## 適合誰使用

**目標受眾**：對無人機開發和自動化控制有興趣的開發者和工程師。

> [!example] 使用場景
> - 無人機愛好者用它來進行空拍攝影，因為其 RGB、深度及熱成像攝影機能提供多樣化的拍攝選擇。
> - 小型企業用它來運輸輕型物品，因為其內部載貨艙設計可承載 1 公斤的貨物，提升運輸效率。
> - 研究人員用它來進行環境監測，因為其多種攝影功能能夠收集不同類型的數據，助於分析。

## 架構分析

Mercury-Transforming-Drone 採用 Raspberry Pi 5 作為控制核心，並使用 Ardupilot 作為飛行控制系統。這樣的設計使得無人機能夠在多種環境下穩定運行，並且便於用戶進行自定義和擴展。資料流方面，無人機的攝影機收集的數據經由 Raspberry Pi 處理，並透過 Mavproxy 與飛行控制器進行通訊。

選擇 Raspberry Pi 而非專用硬體，降低了成本和開發門檻，但可能在性能上有所妥協。擴展性方面，使用者可以根據需求添加新的感測器或模組，但需自行處理相容性問題。整體而言，這種架構適合小型專案和個人實驗，但在商業應用中可能面臨性能和穩定性挑戰。

## 技術深入分析

Mercury-Transforming-Drone 的核心技術機制在於其使用 Raspberry Pi 5 作為控制核心，並結合 Ardupilot 進行飛行控制。這樣的設計使得無人機在多種環境下具備穩定性和靈活性。無人機的攝影機系統包括 RGB、深度和熱成像攝影機，能夠收集多樣化的數據，這對於環境監測和拍攝任務特別有用。效能方面，無人機的運行依賴於 Raspberry Pi 的計算能力，對於高負載任務可能會面臨性能瓶頸。設計上選擇 Raspberry Pi 而非專用硬體，降低了成本，但在性能和穩定性上可能有所妥協。技術風險方面，無人機的變形機制和多功能攝影系統在長期使用中可能會出現耐用性問題。整合方面，與現有的無人機控制系統相容性良好，但對於新手來說，學習曲線可能較陡峭。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了基本的安裝和使用說明，但缺乏詳細的範例和文檔。安裝過程相對順暢，但對於新手來說，可能需要額外的學習時間來理解如何操作。整體而言，對於有一定技術背景的使用者來說，30 分鐘內能夠完成基本的設置和運行。
...
> - 多種攝影機功能，能夠收集豐富的數據。
> - 開放的硬體設計，便於用戶自定義和擴展。
> - 使用 Raspberry Pi 降低了開發成本和門檻。

> [!danger] 缺點
> - 對新手不太友好，缺乏詳細的使用文檔。
> - 性能可能不及專用硬體，對於大型應用有挑戰。
> - 變形機制的耐用性尚未經過長時間測試。
> - 需要一定的技術背景才能進行有效的自定義。

> [!warning] 注意事項
> - 僅支援 Raspberry Pi 5，對於其他平台的兼容性尚未測試。
> - 無法在無網路環境下使用遠端控制功能。
> - 變形機制的耐用性尚未經過長時間測試。
> - 目前缺乏詳細的使用文檔和範例，對新手不太友好。
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["自動化","無人機技術","機器人技術"];
