...
use_case: "一款可變形的無人機，適合多種應用場景，具備多種感測器和貨艙空間。"
priority: medium
ring: assess
...
star_history: "2026-03-10:364,2026-03-11:364,2026-03-11:364,2026-03-13:380,2026-03-14:396,2026-03-15:400,2026-03-16:402,2026-03-17:410,2026-03-18:416,2026-03-19:417,2026-03-20:418,2026-03-21:419,2026-03-22:419,2026-03-23:420,2026-03-24:422,2026-03-25:423"
tags:
  - github
  - "category/其他"
  - "lang/python"
aliases:
  ...
  - "一款可變形的無人機，適合多種應用場景，具備多種感測器和貨艙空間。"
---

# Mercury-Transforming-Drone

**423** stars · **24** stars/天 · 建立 18 天前 · Python · CERN-OHL-S-2.0
...
> **適合** 需要快速原型開發可變形無人機的開發者或教育機構。
> **一句話重點** 這款無人機的設計理念在於靈活性和可擴展性，適合多種應用需求。
...
> [!abstract] 核心創新
> Mercury 提供了一個模組化的無人機設計，能夠根據需求靈活調整配置。

## 專案簡介

Mercury 是一款可變形的無人機，設計上考慮了多樣化的應用需求。用戶可以透過簡單的轉換機制，將無人機從飛行模式轉換為地面行駛模式，這樣的設計使得它在不同環境中都能靈活運用。用戶只需將自動化軟體上傳至 Raspberry Pi 5，並透過簡單的指令來啟動 Mavproxy 及主程式，便可輕鬆控制無人機。這個工具的賣點在於其多功能性和易於組裝的特性，特別適合需要在多種環境中運作的應用場景。技術上，Mercury 使用了 Ardupilot 和 GPS 進行導航，並配備 RGB、深度和熱成像攝影機，這讓它在環境感知上具備強大的能力。

相較於其他無人機，Mercury 的設計更注重於模組化和可擴展性，讓使用者可以根據需求添加不同的感測器或改變貨艙配置。這使得它在小型商業應用或教育實驗中都能展現出色的性能。雖然目前開源社群活躍度不高，但隨著功能的增強和使用案例的增加，未來可能會吸引更多的開發者參與。整體來說，這是一個適合小型團隊或個人開發者的專案，尤其是在需要快速原型開發的情況下。

**技術棧**：`Python` · `TypeScript` · `JavaScript` · `C++` · `Shell`

## 重點功能

- 內部貨艙（1 kg） — 可容納多種設備或貨物，適合多種應用場景。
- 簡單的轉換機制 — 用戶可輕鬆將無人機從飛行模式轉換為地面行駛模式。
- 多種感測器 — 配備 RGB、深度和熱成像攝影機，增強環境感知能力。
- 基於 Ardupilot 和 GPS 的導航系統 — 提供穩定的飛行控制和定位功能。
- 可透過 Raspberry Pi 進行軟體設置 — 方便用戶進行自定義開發和擴展。
...
> [!tip] 爆紅原因
> 建立 18 天內累積 423 stars（24/天），forks 52（12.3%），顯示出一定的社群關注度。作者 L42ARO 和 Agonat0r 具備一定的開發經驗，並針對無人機的多功能性提出了創新解決方案。這個專案填補了市場上對於可變形無人機的需求，特別是在教育和小型商業應用方面。社群的活躍度雖然不高，但隨著功能的持續增強，未來可能會吸引更多開發者的參與。這個專案的成功可能與無人機技術的進步和開源硬體的興起有關，特別是對於 DIY 愛好者和教育機構來說，這是一個值得關注的選擇。

## 適合誰使用

**目標受眾**：需要快速原型開發可變形無人機的開發者或教育機構。

> [!example] 使用場景
> - 工程師用它來開發一個環境監測系統，因為其搭載的多種感測器能夠實時收集數據。
> - 學生用它來進行無人機技術的實驗，因為其簡單的組裝和操作流程適合教學使用。
> - 小型企業用它來進行物流配送，因為其可變形設計能夠適應不同的運輸需求。

## 架構分析

Mercury 的架構設計採用模組化的方式，這使得用戶可以根據需求自由組合不同的感測器和配件。核心控制系統基於 Raspberry Pi，這樣的選擇讓開發者能夠利用 Python 和其他語言進行快速開發。資料流方面，無人機的感測器數據通過 Ardupilot 進行處理，並最終傳送至用戶端進行控制。

這樣的設計使得無人機在不同環境中都能保持高效的運作。選擇 Raspberry Pi 作為控制核心的代價是需要額外的硬體支持，但這也讓整個系統的擴展性大大提高。未來如果需要進一步擴展功能，可能會面臨硬體資源的瓶頸，特別是在處理複雜任務時。

## 技術深入分析

Mercury 的核心技術機制在於其模組化設計，這使得用戶可以根據需求自由選擇和更換感測器。使用 Ardupilot 進行導航，這是一個成熟的開源飛行控制系統，能夠提供穩定的飛行性能。無人機的感測器數據通過 Raspberry Pi 進行處理，這樣的設計使得系統能夠快速響應環境變化。效能方面，無人機的最大承重為 1 kg，這在小型應用中表現良好，但在處理更大規模的任務時可能會遇到瓶頸。選擇 Raspberry Pi 作為控制核心的好處在於其開發生態系統豐富，但也意味著需要額外的硬體支持。技術風險方面，若未來需要擴展功能，可能會面臨硬體資源的限制，特別是在處理複雜任務時。整合方面，與主流開發框架的相容性良好，但在 CI/CD pipeline 的整合上可能需要額外的適配工作。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝步驟和範例，但對於初學者來說，可能仍需一些額外的背景知識。安裝過程相對順暢，但對於 Raspberry Pi 的配置要求可能會帶來一定的挑戰。缺乏多語言支持，主要以英文為主，這可能會影響非英語使用者的理解。整體來說，花 30 分鐘能夠完成基本的設置，但深入使用可能需要更多時間學習。
...
> - 模組化設計，易於根據需求進行擴展。
> - 多種感測器配置，適合多樣化的應用場景。
> - 社群支持雖然不多，但開源特性使得使用者可以自行調整和改進。

> [!danger] 缺點
> - 對於初學者來說，安裝和配置過程可能有些複雜。
> - 目前缺乏活躍的社群支持，可能影響問題解決的速度。
> - 硬體需求較高，特別是對於感測器和控制單元的選擇。

> [!warning] 注意事項
> - 僅支援 Raspberry Pi 5，其他版本可能無法正常運行。
> - 內部貨艙最大承重僅為 1 kg，限制了可攜帶的設備。
> - 目前缺乏活躍的社群支持，可能影響問題解決的速度。
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["無人機技術","自動化","感測器整合"];
