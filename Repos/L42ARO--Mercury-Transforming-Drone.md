...
**546** stars · **7** stars/天 · 建立 77 天前 · N/A · CERN-OHL-S-2.0
...
> **授權** CERN-OHL-S-2.0 · **維護** Slow (最後推送 51 天前) · **貢獻者** 2 人 · **參與度** Medium
> **適合** 需要靈活無人機解決方案的開發者和研究人員，特別是在環境監測和救援任務中。
> **一句話重點** 這款無人機的變形能力和開放性設計使其在特定應用場景中具備優勢。
...
Mercury Transforming Drone 是一款具備變形能力的無人機，設計上可以攜帶最多 1 公斤的內部有效載荷。使用者只需將自動化軟體上傳至 Raspberry Pi 5，並透過簡單的命令行操作來啟動 Mavproxy 介面和主軟體，便可控制無人機。這種設計使得無人機能在不同的環境中靈活運用，並且搭載 RGB、深度及熱成像攝影機，適合於多種應用場景，如搜索救援或環境監測。這個工具的賣點在於其簡單的變形機制，讓無人機能夠在狹小空間中運行，並且具備良好的操作靈活性。技術上，該專案依賴於 Ardupilot 與 GPS 定位，並使用 Python 進行軟體開發，這使得其在資源需求上相對輕量，適合小型團隊或個人開發者使用。

與其他無人機方案相比，如 DJI 的商業無人機，Mercury 提供了更高的自定義性和開放性，適合需要特定功能或改裝的使用者。實際使用中，無人機的效能會受到環境因素影響，特別是在複雜地形中，可能需要額外的調整和測試。該專案目前處於活躍開發階段，社群回應良好，並且在 GitHub 上有穩定的貢獻者支持。對於小型團隊或個人開發者來說，這是一個值得考慮的選擇，尤其是對於需要靈活應對不同任務的無人機應用場景。

**技術棧**：`Python` · `Raspberry Pi 5` · `Ardupilot`

## 重點功能

- 內部有效載荷艙 — 可攜帶最多 1 公斤的設備或物品。
- 簡單變形機制 — 允許無人機在不同環境中靈活運用。
- RGB + 深度 + 熱成像攝影機 — 提供多種感測數據以應對不同任務。
- 基於 Ardupilot 和 GPS — 提供穩定的飛行控制和導航能力。
- 支援 Raspberry Pi 5 — 方便用戶進行軟體安裝和操作。
...
```bash
{
  "前置條件": "已經在 Raspberry Pi 上安裝了必要的軟體。",
  "指令": "./start_mavproxy.sh",
  "預期輸出": "應該顯示 IP 地址以便於控制無人機。"
}
```
...
> [!tip] 爆紅原因
> 建立 77 天內累積 546 stars（約 7 stars/天），forks 65（11.9%），顯示出相對活躍的社群參與。主要貢獻者 L42ARO 和 Agonat0r 具備相關背景，並且該專案解決了無人機變形的需求，之前市場上缺乏這樣的開放式解決方案。無人機的多功能性和可自定義性使其在特定應用場景中具備優勢，尤其是在環境監測和救援任務中。社群的活躍度和無開放問題的狀態也顯示出該專案的穩定性和可用性。

## 適合誰使用

**目標受眾**：需要靈活無人機解決方案的開發者和研究人員，特別是在環境監測和救援任務中。

> [!example] 使用場景
> - 無人機操作員用它來執行環境監測任務，因為其具備 RGB、深度和熱成像攝影機，能夠提供多樣化的數據。
> - 開發者用它來測試無人機的變形機制，因為其簡單的設計讓改裝和測試變得容易。
> - 研究人員用它來進行搜索救援演練，因為其可攜帶 1 公斤的有效載荷，能夠適應不同的任務需求。

## 架構分析

Mercury Transforming Drone 的架構設計為一個輕量級的無人機系統，主要依賴 Raspberry Pi 5 作為控制核心。這樣的設計使得整個系統能夠在資源有限的情況下運行，並且易於用戶進行自定義和擴展。資料流方面，無人機的感測器數據（RGB、深度、熱成像）會通過 Ardupilot 進行處理，並最終由 Raspberry Pi 控制飛行。選擇 Raspberry Pi 而非更複雜的控制系統，能夠降低成本和學習曲線，但可能在處理高負載任務時出現瓶頸。擴展性方面，該架構能夠支持額外的感測器和功能，但隨著功能的增加，可能需要更強的硬體支持。

## 技術深入分析

Mercury Transforming Drone 的核心技術機制在於其變形能力，這使得無人機能夠在狹小空間中運行。該系統使用 Raspberry Pi 5 作為控制核心，並透過 Ardupilot 進行飛行控制，這樣的選擇使得整體架構輕量且易於維護。無人機的感測器數據（RGB、深度、熱成像）能夠實時傳輸至 Raspberry Pi，並由其進行處理，這樣的設計能夠支持多種應用場景。效能方面，無人機在正常操作下能夠持續運行數小時，但在高負載情況下可能會出現性能瓶頸。選擇 Raspberry Pi 而非更複雜的控制系統，能夠降低成本和學習曲線，但可能在處理高負載任務時出現瓶頸。整體而言，該專案的設計考量了靈活性與成本的平衡，適合小型團隊或個人開發者使用。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝步驟和範例，對於新手來說相對友好。安裝過程中，使用者需要注意 Raspberry Pi 的配置，可能會有一些小坑。整體上，文件結構良好，能夠幫助使用者快速上手。
...
> [!danger] 缺點
> - 內部有效載荷限制，無法攜帶大型設備。
> - 依賴 Raspberry Pi，可能在高負載情況下表現不佳。
> - 需要穩定的網路連接以便於遠程控制。

> [!warning] 注意事項
> - 僅支援 Raspberry Pi 5。
> - 內部有效載荷限制為 1 公斤，可能不適合大型設備。
> - 需要穩定的網路連接以便於遠程控制。
