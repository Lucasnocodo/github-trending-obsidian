...
use_case: "提供一個可變形的無人機解決方案，具備多種攝影功能和自動駕駛能力。"
priority: medium
ring: assess
...
aliases:
  ...
  - "提供一個可變形的無人機解決方案，具備多種攝影功能和自動駕駛能力。"
---

# Mercury-Transforming-Drone

**535** stars · **9** stars/天 · 建立 59 天前 · N/A · CERN-OHL-S-2.0
...
> **授權** CERN-OHL-S-2.0 · **維護** Slow (最後推送 33 天前) · **貢獻者** 2 人 · **參與度** Medium
> **適合** 對無人機設計和開發有興趣的獨立開發者或小型團隊。
> **一句話重點** 這個專案的開源性和靈活性使其成為無人機開發者的理想選擇。
...
> [!abstract] 核心創新
> 提供了一個開源且可變形的無人機解決方案，具備多種攝影功能和自動駕駛能力。

## 專案簡介

Mercury-Transforming-Drone 是一款具備變形能力的無人機，設計上能夠攜帶最多 1 公斤的內部載荷，並配備 RGB、深度和熱成像攝影機，適合多種應用場景。用戶可以透過將自動駕駛軟體上傳至 Raspberry Pi 5，並運行 Mavproxy Bridge 和主控制軟體來實現無人機的遠程控制。這樣的設計使得無人機不僅能在空中飛行，還能在地面上行駛，提升了其靈活性和功能性。核心賣點在於其簡單的變形機制，讓使用者能夠在不同環境中靈活運用。技術上，該專案使用了 Ardupilot 作為飛行控制系統，並透過 GPS 確保定位精準，這樣的選擇使得無人機在各種環境下都能保持穩定性。

與其他無人機相比，如 DJI 系列，Mercury 提供了更大的自定義空間和開源的靈活性，特別適合對硬體和軟體有深入需求的開發者。使用者在實際操作中可能會遇到網路連接問題，特別是在不同網路環境下，這時候可以考慮使用 Tailscale 來解決。整體來說，這個專案在無人機領域提供了一個開源且可擴展的解決方案，適合小型團隊或個人開發者進行實驗和創新。未來 6 個月內，隨著社群的活躍，可能會有更多功能和擴展模組的推出，值得持續關注。

**技術棧**：`Python 3` · `Raspberry Pi 5` · `Ardupilot`

## 重點功能

- 內部載荷艙 — 可攜帶最多 1 公斤的貨物，適合多種應用。
- 簡單變形機制 — 允許無人機在空中和地面之間靈活轉換。
- 多種攝影功能 — 配備 RGB、深度和熱成像攝影機，滿足不同需求。
- Raspberry Pi 5 支援 — 透過簡單的指令安裝和配置自動駕駛軟體。
- Mavproxy Bridge — 透過簡單的指令啟動，實現與飛行控制器的連接。
...
> [!tip] 爆紅原因
> 建立 59 天內累積 535 stars（9/天），forks 63（11.8%），顯示出相對穩定的增長。該專案由 L42ARO 和 Agonat0r 共同維護，兩位貢獻者在無人機和開源硬體領域有一定的經驗。Mercury 解決了傳統無人機在可擴展性和自定義上的不足，特別是對於想要進行實驗的開發者來說，這是一個理想的選擇。最近的推文和社群討論也為這個專案帶來了更多的曝光。技術上，無人機的變形機制和多種攝影功能使其在市場上具備獨特性，這些特點吸引了許多開發者的注意。forks/stars 比率相對較高，顯示出使用者對該專案有實際的修改和使用需求。

## 適合誰使用

**目標受眾**：對無人機設計和開發有興趣的獨立開發者或小型團隊。

> [!example] 使用場景
> - 無人機開發者用它來設計和測試多功能無人機，因為它的開源特性和可擴展性讓他們能夠自由調整硬體和軟體。
> - 環境監測專家用它來進行空中和地面數據收集，因為其 RGB、深度和熱成像攝影機能夠提供多樣的數據來源。
> - 教育機構用它來教授學生無人機技術和自動駕駛原理，因為其簡單的組裝和操作流程適合教學使用。

## 架構分析

該專案採用 Raspberry Pi 5 作為控制核心，並使用 Python 進行軟體開發，這樣的選擇使得開發者能夠快速迭代和測試。資料流方面，無人機的控制指令通過 Mavproxy Bridge 傳遞至飛行控制器，並透過 GPS 確保定位精準。這種架構的優勢在於其靈活性和可擴展性，但代價是需要穩定的網路連接來確保控制指令的即時傳遞。擴展性方面，未來可以透過增加更多的感測器或改進算法來提升無人機的功能，但這也可能增加系統的複雜度和維護成本。

## 技術深入分析

Mercury-Transforming-Drone 的核心技術機制在於其變形能力和多種攝影功能，這使得無人機能夠在不同環境中靈活運用。使用 Python 進行開發，並搭配 Raspberry Pi 5，這樣的選擇使得開發者能夠快速迭代和測試。該無人機能夠處理的資料量取決於其攝影機的解析度和傳輸速率，具體的效能數據尚未提供，但可以推測在穩定的網路環境下能夠實現即時控制。選擇 Raspberry Pi 作為控制核心的好處在於其廣泛的社群支持和豐富的資源，但也可能面臨硬體性能的限制。技術風險方面，若未來需要擴展更多功能，可能會面臨系統複雜度增加的挑戰。與主流無人機框架的整合難度中等，特別是在 CI/CD pipeline 的友善度上，可能需要額外的配置來實現自動化部署。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝指導和範例，對於新手來說相對友好。安裝過程需要一些技術知識，但整體流程順暢。文件目前僅提供英文，缺乏多語言支持。
...
> [!danger] 缺點
> - 需要穩定的網路連接，對於無網路環境不友好。
> - 安裝和設定過程對初學者有一定難度。
> - 目前僅支援 Raspberry Pi 5，限制了硬體選擇。

> [!warning] 注意事項
> - 僅支援 Raspberry Pi 5，其他版本可能不兼容。
> - 無法在無網路環境下正常運作，需依賴穩定的網路連接。
> - 對於初學者來說，設定過程可能需要額外的技術知識。
...
> [!tip] 採用建議
> 適合個人或小型團隊進行實驗和開發，不建議用於生產環境的核心應用。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在不同網路環境下可能無法穩定控制無人機。
  - 解法：建議使用 Tailscale 來解決網路問題。
- [MEDIUM] 安裝過程中可能會遇到依賴問題。
  - 解法：確保使用正確的 Python 版本和依賴庫。
- [MEDIUM] 對於初學者，設定過程可能會感到困難。
  - 解法：建議尋找相關的教學資源或社群支持。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型無人機開發團隊 | 非常適合 | 提供開源和可擴展的解決方案，適合進行實驗和創新。 |
| 教育機構進行無人機技術教學 | 適合 | 簡單的組裝和操作流程適合教學使用。 |
| 大型商業無人機應用 | 不適合 | 目前功能和穩定性不足以支持商業級應用。 |
...
> [!warning] 安全性快速掃描
> 該工具本身不需要高權限，但在使用過程中可能會存取網路和環境變數。依賴鏈的信任程度尚可，需注意外部依賴的安全性。
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["無人機技術","自動駕駛","開源硬體"];
