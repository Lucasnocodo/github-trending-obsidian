...
star_history: "2026-03-10:364,2026-03-11:364,2026-03-11:364,2026-03-13:380,2026-03-14:396,2026-03-15:400,2026-03-16:402,2026-03-17:410,2026-03-18:416,2026-03-19:417,2026-03-20:418,2026-03-21:419,2026-03-22:419,2026-03-23:420"
tags:
  - github
  - "category/其他"
  - "lang/python"
...
**420** stars · **26** stars/天 · 建立 16 天前 · Python · CERN-OHL-S-2.0
...
> **適合** 對無人機組裝和應用有興趣的創客或業餘愛好者。
> **一句話重點** 這款無人機的設計不僅強調靈活性，還讓使用者能夠根據需求輕鬆調整配置，降低了進入門檻。
...
> [!abstract] 核心創新
> Mercury 提供了一種簡單的變形機制，讓無人機能夠適應多種任務需求。

## 專案簡介

Mercury 是一款可變形無人機，設計上強調靈活性和多功能性。用戶可以透過簡單的組裝，將其配置為不同的任務，例如搭載 RGB、深度和熱成像攝影機，或是進行自主飛行。無人機的內部貨艙可容納最多 1 公斤的載荷，這使得它在運輸和監控任務中具備實用性。使用者需將自主軟體上傳至 Raspberry Pi 5，並透過簡單的指令來啟動 Mavproxy 及主控制程式，這樣便能透過網路進行控制，甚至可利用 Tailscale 進行遠程操作。這款無人機的設計理念是為了讓任何人都能輕鬆組建並適應各種使用情境，從而降低進入門檻。其核心賣點在於靈活的變形機制和多樣的感測器配置，能夠應對不同的任務需求。

**技術棧**：`Python` · `TypeScript` · `C++`

## 重點功能

- 內部貨艙 — 可容納最多 1 公斤的載荷，適合多種應用。
- 簡單變形機制 — 讓無人機能夠快速轉換形態以適應不同任務。
- 多種感測器 — 包括 RGB、深度和熱成像攝影機，滿足不同需求。
- 基於 Ardupilot 的自主飛行 — 提供穩定的飛行控制和導航。
- 支援遠程控制 — 可透過 Tailscale 進行長距離操作。
...
```python
[
  "# 前置條件：已在 Raspberry Pi 上安裝虛擬環境並啟動",
  "python3 -m venv venv",
  "source venv/bin/activate",
  "pip install -r requirements.txt",
  "# 預期輸出：安裝所需的所有依賴"
...
> [!tip] 爆紅原因
> 建立 16 天內累積 420 stars（26/天），forks 52（12.4%），顯示出不錯的社群參與度。這個專案由 L42ARO 和 Agonat0r 共同開發，兩位貢獻者在無人機技術方面有一定的經驗。Mercury 解決了傳統無人機在靈活性和多功能性上的不足，讓使用者能夠根據需求輕鬆調整無人機的配置。近期的推廣活動和社群互動也可能促進了這一增長。

## 適合誰使用

**目標受眾**：對無人機組裝和應用有興趣的創客或業餘愛好者。

> [!example] 使用場景
> - 業餘無人機愛好者用它來組建自己的多功能無人機，因為它的組裝過程簡單且可根據需求調整配置。
> - 環境監測專家用它來搭載熱成像攝影機進行野生動物監測，因為它的貨艙能夠容納多種感測器。
> - 物流公司用它來進行小型包裹的快速運輸，因為其 1 公斤的載重能力適合短途配送。

## 架構分析

Mercury 的架構設計以 Raspberry Pi 5 為核心，結合多種感測器和控制模組。使用者需將自主軟體上傳至 Raspberry Pi，並透過 Mavproxy 與飛行控制器進行通訊。這種設計使得無人機具備良好的擴展性，使用者可以根據需求添加不同的感測器和功能模組。選擇 Raspberry Pi 5 作為控制單元的好處在於其強大的計算能力和廣泛的社群支持，但代價是需要使用者具備一定的技術背景來進行設定和維護。整體架構的擴展性良好，但在處理高負載任務時可能會遇到性能瓶頸。

## 技術深入分析

Mercury 的核心技術機制在於其變形機制和感測器整合。無人機使用 Raspberry Pi 5 作為控制單元，搭載多種感測器，包括 RGB、深度和熱成像攝影機，這些感測器能夠提供豐富的環境資訊。效能方面，無人機的最大載重為 1 公斤，這限制了其在某些應用場景的使用。設計上選擇 Raspberry Pi 5 是因為其強大的計算能力和廣泛的社群支持，但這也意味著使用者需要具備一定的技術背景來進行設定和維護。這款無人機的設計在擴展性上表現良好，但在處理高負載任務時可能會遇到性能瓶頸。整合方面，無人機的控制系統可與多種感測器和模組進行配合，並能透過 Mavproxy 與飛行控制器進行通訊，這使得使用者能夠根據需求進行調整和擴展。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝步驟和範例，但缺乏詳細的使用說明。安裝過程相對順暢，使用者只需按照指示進行操作。對於初學者來說，可能需要額外的資源來理解無人機的組裝和調試過程。文件未提供多語言版本，僅有英文。
...
> - 靈活的變形機制，適合多種應用場景。
> - 內部貨艙設計，方便搭載不同的感測器。
> - 基於 Raspberry Pi 的開放性設計，易於擴展和修改。

> [!danger] 缺點
> - 需要一定的技術背景來進行組裝和調試。
> - 最大載重限制可能不適合大型物品運輸。
> - 依賴 Raspberry Pi 5，對硬體要求較高。

> [!warning] 注意事項
> - 僅支援 Raspberry Pi 5 作為控制單元。
> - 內部貨艙最大載重為 1 公斤，對於大型物品不適用。
> - 需要一定的電子和編程基礎才能進行組裝和調試。
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["無人機技術","自主飛行","感測器整合"];
