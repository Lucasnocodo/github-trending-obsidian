...
star_history: "2026-03-10:364,2026-03-11:364,2026-03-11:364,2026-03-13:380,2026-03-14:396,2026-03-15:400,2026-03-16:402,2026-03-17:410,2026-03-18:416,2026-03-19:417,2026-03-20:418,2026-03-21:419,2026-03-22:419"
tags:
  - github
  - "category/其他"
  - "lang/python"
...
**419** stars · **28** stars/天 · 建立 15 天前 · Python · CERN-OHL-S-2.0
...
> **授權** CERN-OHL-S-2.0 · **維護** Moderate (最後推送 10 天前) · **貢獻者** 2 人 · **參與度** Medium
> **適合** 對於開源無人機技術有興趣的開發者和研究者。
> **一句話重點** Mercury 提供了一個靈活且可擴展的無人機設計，適合開發者進行多樣化的應用。
...
Mercury 是一款可變形的無人機，設計上考量了多用途的需求，內部貨艙可容納 1 公斤的有效載荷，並搭載 RGB、深度和熱成像攝影機。使用者可以透過 Raspberry Pi 5 來運行其自動化軟體，並透過 Mavproxy 連接飛行控制器，這樣的設計使得無人機能夠在各種環境中靈活運作。核心指令如 `start_mavproxy.sh` 和 `run.sh` 允許使用者在兩個終端中啟動必要的服務，並透過網頁介面進行控制。這個工具的賣點在於其可擴展性和多樣化的感測器配置，適合用於監控、搜索救援等任務。

其架構選擇 Raspberry Pi 5 作為控制核心，這不僅降低了成本，還能利用廣泛的社群支持和豐富的資源。與其他無人機相比，如 DJI 的商業產品，Mercury 提供了更大的自定義空間和擴展性，適合開發者和愛好者進行二次開發。使用者需要注意的是，該專案目前處於早期階段，可能會遇到一些技術挑戰，例如依賴於 Raspberry Pi 的性能限制和無法即時獲得技術支持。整體而言，這是一個值得探索的開源專案，特別適合對無人機技術有興趣的開發者和研究者。

**技術棧**：`Python` · `TypeScript` · `JavaScript` · `C++`

## 重點功能

- 內部貨艙 — 可容納 1 公斤的有效載荷，適合多種用途。
- 變形機制 — 簡單的變形設計，便於適應不同環境。
- 多種攝影機 — 配備 RGB、深度和熱成像攝影機，滿足不同的監測需求。
- 自動化軟體 — 使用 Raspberry Pi 5 來運行自動化軟體，支持遠端控制。
- 網頁控制介面 — 透過 IP 地址在瀏覽器中控制無人機，方便易用。
...
> [!tip] 爆紅原因
> 建立 15 天內累積 419 stars（28/天），forks 51（12.2%），顯示出不錯的增長潛力。作者 L42ARO 和 Agonat0r 具備相關背景，專注於開源硬體和無人機技術，這個專案填補了市場上對於可自定義無人機的需求。無人機技術的普及和開源硬體的興起使得這個專案更具吸引力。高 forks/stars 比率（12.2%）顯示出使用者對於這個專案的實際修改和應用需求。

## 適合誰使用

**目標受眾**：對於開源無人機技術有興趣的開發者和研究者。

> [!example] 使用場景
> - 無人機愛好者用它來建造一款可變形的無人機，因為它提供了靈活的設計和多種感測器選擇。
> - 研究人員用它來進行環境監測，因為其搭載的熱成像攝影機能夠有效收集數據。
> - 小型企業用它來進行貨物運輸，因為內部貨艙設計可容納 1 公斤的有效載荷，適合小型物流需求。

## 架構分析

Mercury 的架構基於 Raspberry Pi 5，這使得其具備了低成本和高可擴展性。資料流中，無人機的感測器數據透過 Mavproxy 連接到飛行控制器，並由主程式進行處理。這種設計選擇使得使用者能夠輕鬆地進行遠端控制和數據收集，但也可能面臨 Raspberry Pi 性能的限制，尤其是在高負載情況下。選擇 Raspberry Pi 而非專業無人機控制器，降低了成本，但在性能和穩定性上可能有所妥協。擴展性方面，這種設計能夠支持多種感測器和附加模組，但也需要使用者具備一定的技術能力來進行配置和調整。

## 技術深入分析

Mercury 的核心技術機制在於其使用 Raspberry Pi 5 作為控制單元，這使得無人機能夠利用 Python 和其他語言進行高度自定義的開發。無人機的感測器數據透過 Mavproxy 進行管理，這是一種輕量級的地面站控制軟體，能夠有效地與飛行控制器進行通訊。這種設計使得無人機在處理複雜任務時仍能保持較低的延遲和高效能。對於資料處理，無人機能夠在實時環境中運行，這對於監控和數據收集任務尤為重要。選擇 Raspberry Pi 5 而非專業控制器，雖然降低了成本，但在性能上可能會受到限制，特別是在高負載情況下。這個選擇也使得無人機的開發和維護相對簡單，因為使用者能夠利用現有的 Python 生態系統進行擴展。整體而言，這個專案在無人機技術的開發上提供了一個靈活的框架，但也需要使用者具備一定的技術能力來進行配置和調整。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝指南和範例，對於新手友好。安裝過程相對順暢，但需要注意 Raspberry Pi 的配置。文件未提供多語言支持，主要以英文為主。
...
> [!danger] 缺點
> - 性能受限於 Raspberry Pi 5。
> - 對於大型貨物的運輸能力有限。
> - 缺乏即時的技術支持和文檔更新。

> [!warning] 注意事項
> - 需要 Raspberry Pi 5 作為控制核心。
> - 對於大型貨物的運輸能力有限，僅支持 1 公斤。
> - 目前缺乏即時的技術支持和文檔更新。
