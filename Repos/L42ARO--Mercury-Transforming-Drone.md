...
use_case: "提供一款可變形的無人機，具備多種攝影功能和自主飛行能力。"
priority: medium
ring: assess
...
aliases:
  ...
  - "提供一款可變形的無人機，具備多種攝影功能和自主飛行能力。"
---

# Mercury-Transforming-Drone

**543** stars · **7** stars/天 · 建立 73 天前 · N/A · CERN-OHL-S-2.0
...
> **授權** CERN-OHL-S-2.0 · **維護** Slow (最後推送 47 天前) · **貢獻者** 2 人 · **參與度** Medium
> **適合** 需要一款可變形且具備多種攝影功能的無人機的開發者或愛好者。
> **一句話重點** Mercury 的變形設計和多功能攝影能力讓無人機操作變得更靈活，適合多樣化的應用場景。
...
> [!abstract] 核心創新
> 提供一個可變形的無人機設計，結合多種攝影功能和自主飛行能力。

## 專案簡介

Mercury 是一款變形無人機，設計上具備內部有效載荷艙（可容納 1 公斤），並搭載 RGB、深度和熱成像攝影機，能夠在多種環境下進行拍攝和數據收集。使用者需將自主飛行軟體上傳至 Raspberry Pi 5，並透過簡單的指令設置虛擬環境和安裝依賴，這樣便能啟動無人機的控制系統。其核心賣點在於簡化了無人機的操作流程，並提供了可視化的控制介面，讓使用者能夠輕鬆掌握無人機的運行狀態。技術上，Mercury 使用 Ardupilot 作為飛行控制系統，並結合 GPS 進行精確導航，這使得其在自主飛行方面表現出色。與其他無人機相比，如 DJI 的 Phantom 系列，Mercury 提供了更高的客製化空間，因為使用者可以根據需求自行設計和組裝部件。

實際使用中，無人機的性能會受到環境因素影響，例如風速和溫度，這可能會影響其飛行穩定性。社群活躍度方面，該專案在短短 73 天內累積了 543 顆 stars，顯示出其受歡迎程度。對於小型團隊或個人開發者來說，這是一個值得考慮的選擇，尤其是在需要進行特定任務的場景下。未來，隨著社群的成長，可能會有更多的功能和擴展出現，進一步提升其應用潛力。

**技術棧**：`Python 3` · `Raspberry Pi 5` · `Ardupilot`

## 重點功能

- 內部有效載荷艙 — 可容納 1 公斤的物品，適合多種任務需求。
- 多種攝影功能 — 配備 RGB、深度和熱成像攝影機，滿足不同拍攝需求。
- 簡單的變形機制 — 使無人機能夠在不同環境中靈活運行。
- 基於 Ardupilot 的自主飛行 — 提供穩定的飛行控制和導航能力。
- 支援 Raspberry Pi 5 — 簡化了軟體安裝和配置過程。
...
> [!tip] 爆紅原因
> 建立 73 天內累積 543 stars（7 stars/天），forks 65（12%），顯示出良好的社群參與度。該專案的主要貢獻者 L42ARO 和 Agonat0r 在無人機和開源硬體領域有一定的經驗，這使得專案具備了良好的技術基礎。Mercury 解決了無人機操作複雜的痛點，提供了一個簡化的解決方案，讓使用者能夠更輕鬆地進行無人機的控制和數據收集。社群的活躍度和開源特性吸引了不少開發者的注意，進一步推動了專案的發展。

## 適合誰使用

**目標受眾**：需要一款可變形且具備多種攝影功能的無人機的開發者或愛好者。

> [!example] 使用場景
> - 無人機愛好者用它來進行空拍，因為它的變形設計和多種攝影功能讓拍攝更具靈活性。
> - 小型企業用它來進行環境監測，因為內部有效載荷艙可以裝載不同的感測器，滿足多樣化需求。
> - 開發者用它來測試自主導航演算法，因為其開源特性和可客製化設計讓實驗更具彈性。

## 架構分析

Mercury 的架構基於 Raspberry Pi 5 作為控制單元，並使用 Ardupilot 作為飛行控制系統。資料流從攝影機捕獲影像，經由 Raspberry Pi 處理後，通過 Mavproxy 與飛行控制器進行通訊。這樣的設計使得無人機能夠在不同環境中靈活運行，並且能夠實現自主導航。選擇 Raspberry Pi 使得整體系統具備較高的可擴展性，但也帶來了對硬體的依賴。擴展性方面，無人機的設計能夠容納不同的攝影機和感測器，但在重量和電池續航上可能會受到限制。

## 技術深入分析

Mercury 的核心技術機制在於其變形設計和多種攝影功能的整合，使用 Raspberry Pi 5 作為控制單元，並利用 Ardupilot 進行自主導航。這樣的設計使得無人機能夠在多種環境中靈活運行，並且能夠進行複雜的任務。效能方面，無人機的最大有效載荷為 1 公斤，這對於大多數小型任務來說已經足夠，但在大型任務中可能會成為瓶頸。設計上選擇 Raspberry Pi 使得系統具備較高的可擴展性，但也增加了對硬體的依賴。技術風險方面，無人機在極端天氣條件下的表現可能不穩定，這需要在實際應用中進行測試和調整。整合方面，與主流的開發工具和 CI/CD 流程相容性良好，但對於初學者來說，學習曲線可能較為陡峭。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了基本的安裝和使用指導，但缺乏詳細的範例。安裝過程相對順暢，使用者可透過指令輕鬆設置環境。整體來說，對於有一定基礎的使用者來說，花 30 分鐘內能夠完成基本的設置。
...
> - 內部有效載荷艙容量有限，無法滿足大型任務需求。
> - 缺乏詳細的使用文檔，可能影響使用體驗。

> [!warning] 注意事項
> - 僅支援 Raspberry Pi 5，其他版本可能無法運行。
> - 內部有效載荷艙容量限制為 1 公斤，無法滿足更大型任務需求。
> - 對於極端天氣條件的適應性較差，可能影響飛行穩定性。
> - 缺乏詳細的使用文檔，初學者可能需要額外的學習時間。
...
**社群活躍度**：社群活躍度良好，短時間內累積了不少 stars 和 forks。
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["無人機技術","開源硬體","自主導航"];
