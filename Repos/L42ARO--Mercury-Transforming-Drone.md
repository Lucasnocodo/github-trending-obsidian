...
use_case: "一款可變形的無人機，能夠適應多種用途，具備多樣的感測器和貨艙空間。"
priority: medium
ring: assess
...
star_history: "2026-03-10:364,2026-03-11:364,2026-03-11:364,2026-03-13:380,2026-03-14:396,2026-03-15:400,2026-03-16:402,2026-03-17:410,2026-03-18:416,2026-03-19:417,2026-03-20:418,2026-03-21:419"
tags:
  - github
  - "category/其他"
  - "lang/python"
aliases:
  ...
  - "一款可變形的無人機，能夠適應多種用途，具備多樣的感測器和貨艙空間。"
---

# Mercury-Transforming-Drone

**419** stars · **30** stars/天 · 建立 14 天前 · Python · CERN-OHL-S-2.0
...
> **適合** 對無人機技術有興趣並希望進行自定義開發的創客或工程師。
> **一句話重點** Mercury 的設計讓無人機變得更加靈活，能夠適應多種不同的應用場景。
...
> [!abstract] 核心創新
> 這款無人機的變形機制和多樣感測器配置使其在市場上獨樹一幟。

## 專案簡介

Mercury 是一款可變形的無人機，設計上強調靈活性和多功能性，適合各種應用場景。用戶可以透過簡單的變形機制將無人機轉換為不同的形態，以適應不同的任務需求。這款無人機配備 RGB、深度和熱成像攝影機，並使用 Ardupilot 和 GPS 進行導航，讓其在各種環境下都能穩定運作。核心的控制軟體需要在 Raspberry Pi 5 上運行，使用者只需透過 SCP 將自動化軟體上傳，並設置虛擬環境安裝依賴。

關鍵的啟動指令包括 `start_mavproxy.sh` 和 `run.sh`，這樣就能在同一網路下控制無人機。這款無人機的設計考量了易用性和擴展性，使用者可以透過 Tailscale 進行遠程控制。這樣的設計使得無人機不僅適合業餘愛好者，也能滿足專業需求，特別是在需要靈活運用的場景中。整體來說，Mercury 的一句話賣點是：一款可變形的無人機，讓使用者能夠根據需求靈活調整其功能和形態。

**技術棧**：`Python` · `TypeScript` · `C++`

## 重點功能

- 內部貨艙 — 可容納 1 公斤的物品，適合小型運輸任務。
- 簡單變形機制 — 使用者可快速轉換無人機形態以適應不同任務。
- 多種感測器 — 配備 RGB、深度和熱成像攝影機，滿足多樣化的拍攝需求。
- GPS 導航 — 使用 Ardupilot 進行精確定位和導航。
- 移動應用 — 提供手機應用程式以便於遠程控制和監控。

## 快速開始

1. 在 Raspberry Pi 5 上設置虛擬環境
```bash
python3 -m venv venv
```
2. 啟動虛擬環境
...
> [!tip] 爆紅原因
> 建立 14 天內累積 419 stars（30/天），forks 51（12.2%），顯示出不錯的關注度。這個專案由 L42ARO 和 Agonat0r 共同開發，解決了傳統無人機在多功能性和靈活性上的不足。之前的無人機往往只能專注於單一任務，無法快速適應不同的需求。這個專案的出現正好填補了這一空白，並且其開源的特性讓更多人能夠參與和貢獻。社群的活躍度也相對較高，顯示出使用者對這個專案的興趣和需求。

## 適合誰使用

**目標受眾**：對無人機技術有興趣並希望進行自定義開發的創客或工程師。

> [!example] 使用場景
> - 業餘無人機愛好者用它來進行空拍和探索，因為其變形設計讓拍攝角度更加靈活。
> - 環境監測專家用它來收集不同環境下的數據，因為其多種感測器能夠提供豐富的資訊。
> - 物流公司用它來運送小型包裹，因為內部貨艙可容納 1 公斤的物品，並且具備 GPS 導航功能。

## 架構分析

Mercury 的架構設計以 Raspberry Pi 為核心，搭配多種感測器和控制軟體。這樣的設計使得無人機能夠在不同的環境中靈活運作。資料流方面，攝影機捕獲的影像會經由 Raspberry Pi 處理，並透過 Mavproxy 與飛行控制器進行通訊。選擇 Raspberry Pi 的原因在於其開放性和強大的社群支持，但這也意味著對於硬體的依賴性較高，若未來需要擴展功能，可能面臨硬體升級的挑戰。此外，無人機的變形機制雖然提供了靈活性，但在設計上也增加了結構的複雜度，可能影響到穩定性和耐用性。

## 技術深入分析

Mercury 的核心技術機制包括多種感測器的整合與 Raspberry Pi 的使用，這使得無人機能夠在多樣化的環境中運作。感測器的數據會被即時處理，並透過 Mavproxy 與飛行控制器進行通訊，確保無人機能夠精確導航。效能方面，無人機的設計考量了重量和穩定性，雖然內部貨艙容量僅為 1 公斤，但這樣的設計使得無人機在空中運行時更為靈活。選擇 Raspberry Pi 作為控制核心的好處在於其開放性和強大的社群支持，但這也意味著對於硬體的依賴性較高，未來若需要擴展功能，可能面臨硬體升級的挑戰。技術風險方面，無人機的變形機制雖然提供了靈活性，但在設計上也增加了結構的複雜度，可能影響到穩定性和耐用性。整合方面，無人機的設計使得與現有的開發工具鏈相容性良好，但初學者在設置過程中可能會遇到一些挑戰。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝指引和範例，讓使用者能夠快速上手。安裝過程中需注意 Raspberry Pi 的配置，對於新手來說可能會有一定的挑戰。整體來說，文件的結構合理，適合有一定技術背景的使用者。
...
> - 對於初學者來說，設置過程可能較為複雜。
> - 內部貨艙容量限制，無法運送較重物品。
> - 需要穩定的網路環境以確保遠程控制的流暢性。

> [!warning] 注意事項
> - 僅支援 Raspberry Pi 5，其他版本可能不兼容。
> - 內部貨艙容量限制為 1 公斤，無法運送較重物品。
> - 需要穩定的無線網路以進行遠程控制。
