...
use_case: "一款可變形的無人機，適用於多種用途，具備靈活的移動性和多樣的感測器。"
priority: medium
ring: assess
...
star_history: "2026-03-10:364,2026-03-11:364,2026-03-11:364,2026-03-13:380,2026-03-14:396,2026-03-15:400,2026-03-16:402,2026-03-17:410,2026-03-18:416,2026-03-19:417,2026-03-20:418,2026-03-21:419,2026-03-22:419,2026-03-23:420,2026-03-24:422,2026-03-25:423,2026-03-26:425"
tags:
  - github
  - "category/基礎設施"
  - "lang/python"
aliases:
  ...
  - "一款可變形的無人機，適用於多種用途，具備靈活的移動性和多樣的感測器。"
---

# Mercury-Transforming-Drone

**425** stars · **22** stars/天 · 建立 19 天前 · Python · CERN-OHL-S-2.0
...
> **適合** 對無人機技術有興趣並希望進行自訂開發的工程師和愛好者。
> **一句話重點** 這款無人機的設計不僅靈活，還能夠輕鬆適應不同的任務需求，特別適合開發者進行二次開發。
...
> [!abstract] 核心創新
> Mercury 提供了簡單的變形機制和多種感測器的整合，適合多用途應用。

## 專案簡介

Mercury 是一款可變形的無人機，設計上考慮了多用途的靈活性，具備內部載貨艙（可承載 1 公斤）、RGB、深度和熱成像攝影機，並搭載 Ardupilot 和 GPS 系統。用戶只需將自動化軟體上傳至 Raspberry Pi 5，並透過簡單的指令來啟動 Mavproxy 介面和主控制程式，便可輕鬆控制無人機。這種設計使得無人機在多種環境下都能運行，並且能夠透過網路遠端控制，這在傳統無人機中並不常見。Mercury 的核心賣點在於其簡單的變形機制和強大的感測能力，適合用於監控、搜救或運輸等多種場景。

技術上，該專案使用 Python 和 Shell 腳本來實現控制邏輯，依賴於 Raspberry Pi 的運算能力，這使得整體架構相對輕量，但也需要用戶具備一定的 Linux 環境操作知識。與其他無人機相比，如 DJI 的商業無人機，Mercury 提供了更大的自訂空間和擴展性，特別適合開發者和愛好者進行二次開發。使用者在實際操作中可能會遇到網路連接不穩定的問題，建議使用 Tailscale 來改善遠端控制的穩定性。這個專案目前仍在 alpha 階段，對於希望在無人機領域進行創新和實驗的團隊來說，值得一試。

**技術棧**：`Python` · `Shell` · `Raspberry Pi`

## 重點功能

- 內部載貨艙 — 可承載 1 公斤的貨物，適合運輸小型物品。
- 多種感測器 — 配備 RGB、深度和熱成像攝影機，適合多種監控任務。
- 簡單的變形機制 — 使無人機能夠在不同模式下運行，增加靈活性。
- 自動化控制 — 使用 Raspberry Pi 進行控制，簡化了操作流程。
- 遠端控制功能 — 可透過網路進行遠端控制，適合長距離操作。
...
> [!tip] 爆紅原因
> 建立 19 天內累積 425 stars（22/天），forks 53（12.5%），顯示出相對活躍的社群參與。作者 L42ARO 和 Agonat0r 在開源硬體和無人機領域有一定的經驗，這個專案解決了傳統無人機在多用途和可擴展性上的不足。近期的推廣活動和社群分享也可能促進了其曝光率。這個工具的設計考量到了現代無人機市場的需求，特別是在 DIY 和教育領域的應用。高達 12.5% 的 forks 表示許多開發者對於修改和擴展這個專案感興趣，顯示出其潛在的應用價值。

## 適合誰使用

**目標受眾**：對無人機技術有興趣並希望進行自訂開發的工程師和愛好者。

> [!example] 使用場景
> - 機器人愛好者用它來建造一個多功能的無人機，因為其可變形設計和多樣的感測器使得無人機能夠適應不同的任務需求。
> - 環境監測人員用它來進行野外調查，因為其搭載的熱成像攝影機能夠在夜間或低能見度的情況下運行。
> - 教育機構用它來教授學生無人機技術，因為其開源特性和簡單的組裝過程讓學生能夠輕鬆上手並進行實驗。

## 架構分析

Mercury 的架構設計為一個輕量級的無人機控制系統，主要依賴 Raspberry Pi 作為控制單元。這樣的設計使得整體系統不需要重型硬體，便於攜帶和操作。資料流方面，無人機的感測器數據會通過 Mavproxy 介面傳輸至 Raspberry Pi，並由主控制程式進行處理。選擇 Raspberry Pi 而非專用控制器的好處在於其開源和可擴展性，但代價是可能在處理速度和穩定性上有所妥協。未來擴展性方面，若需增加更多感測器或功能，可能需要進一步的硬體升級。

## 技術深入分析

Mercury 的核心技術機制在於其使用 Raspberry Pi 作為控制單元，並透過 Python 和 Shell 腳本來實現控制邏輯。這種設計使得無人機能夠在多種環境下運行，並且能夠透過網路遠端控制。效能方面，該系統的處理能力主要取決於 Raspberry Pi 的性能，對於小型任務和資料處理是足夠的，但在處理大型數據時可能會出現瓶頸。選擇 Raspberry Pi 而非專用硬體的好處在於其開源性和社群支持，但也意味著需要用戶具備一定的技術背景。技術風險方面，若未來需要擴展更多功能，可能會面臨性能不足的問題。整合方面，該系統可以與多種開源工具和平台進行整合，對於 CI/CD 流程的支持也相對友好。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝步驟和基本操作指令，對於有一定技術背景的用戶來說，安裝過程相對順暢。對於初學者，可能需要一些額外的 Linux 知識來順利完成安裝。文件中缺乏詳細的範例和使用場景，可能會讓新手感到困惑。
...
> [!danger] 缺點
> - 對於初學者有一定的學習曲線
> - 依賴 Raspberry Pi，可能在性能上有限
> - 目前功能仍在開發中，穩定性可能不足

> [!warning] 注意事項
> - 需要 Raspberry Pi 5 作為控制單元
> - 對於初學者來說，Linux 環境的操作可能有一定難度
> - 目前僅支援特定的感測器和硬體配置
...
| 工具 | 差異 |
| --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 提供類似的無人機控制功能，但專注於資料同步和傳輸，適合需要高效能數據傳輸的場景。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | 專注於無人機的隱私保護和數據加密，適合對安全性有高需求的用戶。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| DJI Mavic | 商業無人機，專注於穩定性和高效能，適合專業拍攝和商業用途。 | 如果需要高穩定性和專業級的影像品質，則 DJI Mavic 是更好的選擇。 | medium，因為需要重新學習 DJI 的控制系統和應用程式。 |
| Parrot Anafi | 提供類似的拍攝功能，但在價格上更具競爭力，適合入門級用戶。 | 如果預算有限且希望獲得良好的拍攝效果，Parrot Anafi 是不錯的選擇。 | low，因為操作界面相對簡單，容易上手。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **Mercury-Transforming-Drone** | **DJI Mavic** | **Parrot Anafi** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 商業無人機，專注於穩定性和高效能，適合專業拍攝和商業用途。 | 提供類似的拍攝功能，但在價格上更具競爭力，適合入門級用戶。 |
> | 遷移成本 | - | medium，因為需要重新學習 DJI 的控制系統和應用程式。 | low，因為操作界面相對簡單，容易上手。 |
> | 適用場景 | 主要場景 | 如果需要高穩定性和專業級的影像品質，則 DJI Mavic  | 如果預算有限且希望獲得良好的拍攝效果，Parrot Anaf |
...
> [!tip] 採用建議
> 適合個人實驗和開發，但不建議用於生產環境的核心應用。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在不同網路環境下，遠端控制可能不穩定
  - 解法：建議使用 Tailscale 來改善連接穩定性
- [MEDIUM] 初次安裝可能需要額外的 Linux 知識
  - 解法：參考網上資源或尋求社群幫助
- [MEDIUM] 感測器的兼容性可能有限
  - 解法：在購買前確認感測器是否支援
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型團隊開發無人機應用 | 非常適合 | 開源設計便於修改和擴展，適合創新應用。 |
| 大型企業的商業無人機需求 | 不適合 | 目前仍在 alpha 階段，穩定性不足。 |
| 教育機構進行無人機技術教學 | 適合 | 簡單的組裝和操作適合學生學習。 |
| 需要高穩定性的專業拍攝 | 不適合 | 與商業無人機相比，穩定性和性能不足。 |
...
> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限，並且不存取敏感資料，但使用者需注意依賴的第三方庫的安全性。
...
**社群活躍度**：社群活躍度中等，近期有持續的 commit 活動。
