...
**544** stars · **7** stars/天 · 建立 75 天前 · N/A · CERN-OHL-S-2.0
...
> **授權** CERN-OHL-S-2.0 · **維護** Slow (最後推送 49 天前) · **貢獻者** 2 人 · **參與度** Medium
> **適合** 對無人機技術有興趣並希望進行自定義開發的工程師或開發者。
> **一句話重點** 這款無人機的設計不僅靈活，還能夠滿足多樣化的應用需求，特別適合開發者進行二次開發。
...
> **結論** 花 10 小時學習，5 小時整合，得到靈活的無人機解決方案，值得考慮。

> [!abstract] 核心創新
> 提供了一個可變形的無人機設計，結合多種攝影技術和自動駕駛功能。

## 專案簡介

Mercury-Transforming-Drone 是一款具備變形功能的無人機，能夠搭載 1 公斤的內部有效載荷，並配備 RGB、深度及熱成像攝影機。使用者可以透過 Raspberry Pi 5 安裝自動駕駛軟體，並運行 Mavproxy Bridge 來與飛行控制器進行連接。這樣的設計使得無人機能夠在不同環境下靈活運作，並且可以透過簡單的命令行指令來啟動和管理。其一句話賣點是「一款可變形的無人機，結合了多種攝影技術和自動駕駛能力」。技術上，該專案使用 Python 來編寫自動駕駛軟體，並且依賴於 Ardupilot 和 GPS 進行導航。

這樣的選擇使得無人機在精確度和穩定性上有良好的表現。與其他無人機相比，如 DJI 的 Phantom 系列，Mercury 提供了更大的自定義空間，適合開發者進行二次開發和功能擴展。實際使用中，無人機的性能取決於所搭載的攝影設備和環境條件，並且需要穩定的網路連接來進行遠程控制。該專案目前處於活躍開發階段，社群活躍度良好，無開放的問題，顯示出穩定性。對於小型團隊或開發者來說，這是一個值得考慮的選擇，尤其是在需要靈活性和可擴展性的情況下。

**技術棧**：`Python`

## 重點功能

- 內部有效載荷艙 — 可容納 1 公斤的有效載荷，適合不同任務需求。
- 變形機構 — 簡單的變形設計，便於攜帶和操作。
- 多種攝影機 — 配備 RGB、深度及熱成像攝影機，提供多樣化的拍攝選擇。
- 自動駕駛軟體 — 使用 Ardupilot 和 GPS 進行精確導航，提升飛行穩定性。
- 遠程控制 — 支援透過網路進行遠程控制，適合長距離操作。
...
```bash
{
  "前置條件": "需要在 Raspberry Pi 5 上安裝自動駕駛軟體",
...
> [!tip] 爆紅原因
> 建立 75 天內累積 544 stars（約 7 stars/天），forks 數量為 65（11.9%），顯示出相對較高的關注度。專案的主要貢獻者 L42ARO 和 Agonat0r 在無人機技術領域有一定的背景，這使得專案在技術上具備可信度。這款無人機解決了市場上對於可變形無人機的需求，並且提供了開源的自動駕駛解決方案，讓開發者能夠進行自定義開發。社群的活躍程度和無開放問題的狀態也顯示出專案的穩定性和潛力。

## 適合誰使用

**目標受眾**：對無人機技術有興趣並希望進行自定義開發的工程師或開發者。

> [!example] 使用場景
> - 無人機愛好者用它來組裝和測試自定義的無人機，因為它提供了靈活的設計和多種攝影選項。
> - 小型企業用它來進行環境監測，因為其搭載的熱成像攝影機能夠有效捕捉不同環境的數據。
> - 開發者用它來開發新的自動駕駛功能，因為開源的軟體架構讓他們能夠自由修改和擴展功能。

## 架構分析

該專案採用 Raspberry Pi 5 作為控制單元，並使用 Python 開發自動駕駛軟體。資料流從攝影機獲取影像，經過處理後發送至控制器進行導航。這樣的設計使得無人機能夠在不同環境下靈活運作。選擇 Raspberry Pi 5 的原因在於其強大的處理能力和開源社群的支持，但這也意味著對於初學者來說可能有一定的學習曲線。擴展性方面，該架構能夠支持多種攝影機和感測器的添加，但需要考量電源和處理能力的限制。

## 技術深入分析

Mercury-Transforming-Drone 的核心技術機制在於其變形設計和多攝影機配置，這使得無人機能夠在不同任務中靈活應對。使用 Python 開發的自動駕駛軟體，結合 Ardupilot 和 GPS，提供了精確的導航和控制能力。該無人機能夠處理多達 1 公斤的有效載荷，並且在不同環境下的性能表現良好。選擇 Raspberry Pi 5 作為控制單元的設計，使得開發者能夠利用其強大的計算能力，但也帶來了對於電源和處理能力的需求。設計取捨方面，選擇開源架構使得開發者能夠自由修改和擴展功能，但也可能導致維護上的挑戰。技術風險方面，隨著使用者需求的增加，無人機的穩定性和性能可能會受到影響，特別是在大型有效載荷的情況下。整合方面，該專案與 Raspberry Pi 的生態系統相容性良好，但對於其他硬體的支持可能需要額外的適配。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝步驟和範例，讓新手能夠快速上手。安裝過程相對順暢，但對於初學者來說可能有些挑戰。文件中沒有提供多語言支持，但英文內容清晰易懂。整體而言，花 30 分鐘應該能夠成功運行起來。
...
- **[HIGH]** 在某些網路環境下，無法穩定連接到遠程控制界面
  - 解法：確保使用穩定的 Wi-Fi 或考慮使用 Tailscale
- [MEDIUM] 對於大型有效載荷的支持不佳，可能導致飛行不穩定
  - 解法：減少有效載荷或進行飛行測試以確保穩定性
- [MEDIUM] 初學者在設置過程中可能會遇到依賴問題
  - 解法：仔細檢查 requirements.txt 中的依賴，並確保正確安裝

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型企業的環境監測無人機 | 非常適合 | 具備多種攝影功能，能夠收集環境數據 |
| 無人機愛好者的自定義專案 | 適合 | 靈活的設計和開源架構，便於個性化開發 |
| 大型企業的商業無人機應用 | 不適合 | 對於大型有效載荷的支持有限，可能影響性能 |
| 教育機構的無人機課程 | 適合 | 開源設計適合教學和實驗 |
...
> [!tip] 投入 vs 回報
> 花 10 小時學習，5 小時整合，得到靈活的無人機解決方案，值得考慮。
...
相關專案：[[0x0funky--agent-sprite-forge|0x0funky/agent-sprite-forge]] · [[432539--gpt2api|432539/gpt2api]] · [[BigBodyCobain--Shadowbroker|BigBodyCobain/Shadowbroker]] · [[0xGF--boneyard|0xGF/boneyard]] · [[NVIDIA--NemoClaw|NVIDIA/NemoClaw]] · [[AgentSeal--codeburn|AgentSeal/codeburn]] · [[AgriciDaniel--claude-obsidian|AgriciDaniel/claude-obsidian]] · [[AlpinDale--parsync|AlpinDale/parsync]]
