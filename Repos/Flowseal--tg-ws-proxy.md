...
use_case: "提供 Telegram 的本地 MTProto 代理伺服器，部分繞過 Telegram 加載限制。"
priority: medium
ring: assess
...
aliases:
  ...
  - "提供 Telegram 的本地 MTProto 代理伺服器，部分繞過 Telegram 加載限制。"
---

# tg-ws-proxy

**4.8k** stars · **126** stars/天 · 建立 38 天前 · Python · MIT
...
> **適合** 需要在不穩定網絡環境中使用 Telegram 的用戶，特別是在某些地區受限的情況下。
> **一句話重點** tg-ws-proxy 是 Telegram 用戶在受限環境中提升使用體驗的利器，特別是在不穩定的網絡中。
...
> [!abstract] 核心創新
> tg-ws-proxy 提供了一個簡單易用的本地 MTProto 代理，專為 Telegram Desktop 設計，能夠有效繞過網絡限制。

## 專案簡介

tg-ws-proxy 是一個本地 MTProto 代理伺服器，旨在透過 WebSocket 連接來加速 Telegram Desktop 的運行。用戶只需在本地啟動代理，然後 Telegram 客戶端將流量導向 `127.0.0.1:1443`，代理會自動處理與 Telegram 數據中心的連接。這樣的設計不僅提高了連接速度，還保持了數據的加密性，避免了對第三方伺服器的依賴。使用者可以透過 CLI 指令啟動代理，並使用 `tg://proxy` 連結自動配置 Telegram。tg-ws-proxy 支援多平台，並提供 GUI 介面來簡化配置過程。這個工具的賣點在於其簡單易用和高效能，能夠在不穩定的網絡環境中提升 Telegram 的可用性。

技術上，tg-ws-proxy 使用 Python 開發，依賴於多個庫如 `pyperclip` 和 `psutil`，並針對不同操作系統提供相應的依賴。與其他代理工具相比，如 `0xGF/boneyard` 和 `AlpinDale/parsync`，tg-ws-proxy 提供了更簡單的設置流程和更好的用戶體驗，特別是在 Windows 和 macOS 環境下。使用者在配置過程中可能會遇到媒體文件無法加載的問題，這通常與 Telegram 的 IP 設置有關，需根據文檔進行調整。tg-ws-proxy 的社群活躍度良好，開發者定期更新，並解決了大部分已知問題。對於需要在不穩定網絡中使用 Telegram 的用戶，這個工具是個不錯的選擇，但在生產環境中使用時仍需謹慎評估其穩定性。

**技術棧**：`Python` · `Dockerfile`

## 重點功能

- 本地 MTProto 代理 — 透過 WebSocket 連接加速 Telegram Desktop 的加載速度。
- 自動配置功能 — 透過 `tg://proxy` 連結自動設置 Telegram 的代理。
- 多平台支持 — 提供 Windows、macOS 和 Linux 的安裝包，適用於不同操作系統。
- GUI 介面 — 提供用戶友好的圖形介面來管理代理設置和日誌。
- Fake TLS 模式 — 支持在 CLI 中使用 Fake TLS 以繞過某些網絡限制。

## 快速開始

1. 下載 Windows 版本
```bash
從 [GitHub Releases](https://github.com/Flowseal/tg-ws-proxy/releases) 下載 TgWsProxy_windows.exe
```
2. 啟動代理
```bash
執行 TgWsProxy_windows.exe，並根據提示設置 Telegram
```
3. 配置 Telegram
```bash
右鍵點擊系統托盤圖標，選擇「打開 Telegram」自動配置代理
```

## 為什麼值得關注

> [!tip] 爆紅原因
> 建立 38 天就累積 4788 stars（126/天），forks 218（4.6%），顯示出穩定的增長趨勢。作者 Flowseal 及其團隊在 Telegram 代理領域有一定的經驗，這個專案解決了 Telegram 用戶在特定地區無法順利加載的問題，之前的解決方案往往需要複雜的配置或依賴於不穩定的第三方伺服器。該專案的推出引起了 Telegram 用戶的廣泛關注，並在社群中獲得了積極的反饋。技術上，WebSocket 的使用使得連接更為高效，這在現今的網絡環境中是非常重要的。forks/stars 比率為 4.6%，顯示出有相當一部分用戶在實際修改和使用這個工具。

## 適合誰使用

**目標受眾**：需要在不穩定網絡環境中使用 Telegram 的用戶，特別是在某些地區受限的情況下。

> [!example] 使用場景
> - 獨立開發者用它來在不穩定的網絡環境中加速 Telegram 的加載速度，因為它能夠透過本地代理減少延遲。
> - 小型團隊用它來簡化 Telegram 的使用流程，因為它提供了自動配置的功能，節省了設置時間。
> - 系統管理員用它來監控 Telegram 的流量，因為它支持日誌記錄功能，方便追蹤問題。

## 架構分析

tg-ws-proxy 採用單一的 Python 應用架構，設計目的是為了簡化用戶的使用體驗。資料流從 Telegram Desktop 通過 MTProto 代理轉發到 WebSocket，這樣的設計可以減少延遲並提高速度。應用內部使用了多個庫來處理不同平台的依賴，這使得它能夠在 Windows、macOS 和 Linux 上運行。選擇 Python 作為開發語言的原因是其跨平台的特性和豐富的庫支持，但這也可能導致在性能上不如某些編譯型語言。tg-ws-proxy 的擴展性良好，能夠根據用戶需求進行功能擴展，但在高並發情況下可能會遇到性能瓶頸。

## 技術深入分析

tg-ws-proxy 的核心技術機制是使用 MTProto 協議來建立與 Telegram 數據中心的安全連接，並通過 WebSocket 進行數據傳輸。這樣的設計不僅保持了數據的加密性，還能夠在不穩定的網絡環境中提供更好的連接穩定性。tg-ws-proxy 能夠處理的數據量取決於用戶的網絡帶寬和 Telegram 伺服器的響應速度，具體的性能數據在 README 中未提供，但用戶反映在正常使用情況下延遲明顯減少。選擇 Python 作為開發語言的原因在於其開發效率和跨平台支持，但這也可能導致在高負載情況下性能不如 C/C++ 等編譯型語言。tg-ws-proxy 的依賴樹相對簡單，主要依賴於幾個常用的 Python 庫，這降低了維護的複雜性。技術風險方面，若 Telegram 改變其 API 或 MTProto 協議，可能會導致代理失效，這需要開發者及時更新。整合方面，tg-ws-proxy 可以輕鬆與現有的 Telegram 客戶端進行集成，並且支持在 CI/CD pipeline 中使用，這使得它在開發和測試環境中非常友好。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的指導和範例，安裝過程相對順暢，但 Windows 用戶可能會遇到誤報的問題。整體來說，對於新手來說，花 30 分鐘應該能夠順利運行起來。

## 優缺點分析

> [!success] 優點
> - 簡單易用，快速配置 Telegram 代理。
> - 支持多平台，適合不同操作系統的用戶。
> - 提供 GUI 介面，方便用戶管理和查看日誌。

> [!danger] 缺點
> - 在某些情況下，媒體文件可能無法加載，需要手動配置。
> - Windows Defender 可能誤報為病毒，影響下載體驗。
> - 對於高並發的使用場景，性能可能會受到限制。

> [!warning] 注意事項
> - 僅支援 Python 3.8 以上版本
> - 在某些情況下，媒體文件可能無法加載，需要手動配置 IP
> - Windows Defender 可能誤報為病毒，需要手動處理

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | boneyard 提供了更廣泛的代理選項，但設置過程較為複雜，適合有經驗的用戶。 |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | parsync 專注於高效的數據同步，而 tg-ws-proxy 專注於 Telegram 的加速，適合不同的使用場景。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | boneyard 提供了更廣泛的代理選項，但設置過程較為複雜，適合有經驗的用戶。 | 如果你的團隊需要多樣的代理選擇並且有能力進行複雜的配置。 | medium，因為需要重新配置和學習新的設置流程。 |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | parsync 專注於高效的數據同步，而 tg-ws-proxy 專注於 Telegram 的加速，適合不同的使用場景。 | 如果你的需求是高效的數據同步而非專注於 Telegram 的使用。 | low，因為功能上有明顯的差異，遷移過程相對簡單。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **tg-ws-proxy** | **boneyard** | **parsync** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | boneyard 提供了更廣泛的代理選項，但設置過程較為複雜，適合有經驗的用戶。 | parsync 專注於高效的數據同步，而 tg-ws-proxy 專注於 Telegram 的加速，適合不同的使用場景。 |
> | 遷移成本 | - | medium，因為需要重新配置和學習新的設置流程。 | low，因為功能上有明顯的差異，遷移過程相對簡單。 |
> | 適用場景 | 主要場景 | 如果你的團隊需要多樣的代理選擇並且有能力進行複雜的配置。 | 如果你的需求是高效的數據同步而非專注於 Telegram 的 |
...
- **[HIGH]** 在某些情況下，媒體文件無法加載，特別是在非 Premium 帳號上。
  - 解法：根據文檔手動配置 IP 地址。
- [MEDIUM] Windows Defender 可能誤報為病毒，影響下載體驗。
  - 解法：在下載時暫時禁用防病毒軟件。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的小型團隊使用 Telegram 進行溝通 | 非常適合 | tg-ws-proxy 能夠簡化設置過程並提高使用體驗。 |
| 需要在不穩定網絡中使用 Telegram 的個人用戶 | 非常適合 | 本地代理能有效減少延遲。 |
| 大型企業需要穩定的 Telegram 使用環境 | 不適合 | 在高負載情況下可能會遇到性能瓶頸。 |
| 開發者需要測試 Telegram 應用的功能 | 適合 | tg-ws-proxy 提供了簡單的配置和日誌功能。 |
...
> [!warning] 安全性快速掃描
> 低風險：tg-ws-proxy 不需要高權限運行，且不存取敏感資料，但用戶需注意防病毒軟件的誤報問題。

## 生態系整合

> [!abstract] 如何融入你的工具鏈

tg-ws-proxy 最常與 Telegram Desktop 搭配使用，作為本地代理來提高連接速度。在一個使用 Telegram 的團隊中，可以直接將 tg-ws-proxy 設置為代理，具體做法是啟動代理後在 Telegram 中配置相應的 IP 地址。tg-ws-proxy 支援與主流 CI/CD 工具的整合，如 GitHub Actions，並且能夠在開發環境中輕鬆使用。整合的摩擦點主要在於防病毒軟件的誤報，可能需要用戶手動處理。
