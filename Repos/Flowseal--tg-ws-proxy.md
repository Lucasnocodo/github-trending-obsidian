...
use_case: "提供 Telegram 的本地 MTProto 代理伺服器，部分繞過 Telegram 載入限制。"
priority: medium
ring: assess
...
aliases:
  ...
  - "提供 Telegram 的本地 MTProto 代理伺服器，部分繞過 Telegram 載入限制。"
---

# tg-ws-proxy

**4.3k** stars · **137** stars/天 · 建立 31 天前 · Python · MIT
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Growing · **熱度** Hot (137 stars/day)
> **授權** MIT (商業友好) · **維護** Active (最後推送 5 天前) · **貢獻者** 5+ 人 · **參與度** Low
> **適合** 需要在受限網路環境中使用 Telegram 的用戶，特別是自由工作者和網路工程師。
> **一句話重點** tg-ws-proxy 的設計讓 Telegram 用戶在受限網路環境中也能享受流暢的通訊體驗。
...
> **結論** 花 2 小時學習，1 小時整合，得到穩定的 Telegram 使用體驗，值得。
...
tg-ws-proxy 是一個本地 MTProto 代理伺服器，旨在加速 Telegram Desktop 的使用體驗。其工作流程為：Telegram Desktop 透過 MTProto 代理（127.0.0.1:1443）連接，然後將流量轉發至 Telegram 的資料中心，這樣可以在不需要外部伺服器的情況下，保持數據的加密傳輸。這個設計的賣點在於能夠有效繞過某些網路限制，提升使用者的連接穩定性和速度。該專案使用 Python 開發，並且支持 Docker 部署，這使得它在不同環境中都能輕鬆運行。tg-ws-proxy 的依賴相對輕量，主要依賴於 pyperclip 和 psutil 等庫，這意味著它的安裝和運行不會對系統造成過多負擔。

與其他類似工具相比，如 AlpinDale/parsync 和 BigBodyCobain/Shadowbroker，tg-ws-proxy 提供了更簡單的安裝步驟和更直觀的用戶界面，特別是在 Windows 和 macOS 上的系統托盤集成。實際使用中，tg-ws-proxy 能夠處理多個資料中心的連接，並具備自動切換功能，當 WebSocket 連接失敗時會自動回退到 TCP 連接，這提升了其穩定性。該專案的社群活躍度不錯，開發者對於問題的回應速度也相對較快，這對於使用者來說是一個加分項。整體來看，tg-ws-proxy 是一個穩定且功能強大的 Telegram 代理工具，適合需要在特定網路環境中使用 Telegram 的用戶。

**技術棧**：`Python 3.8+` · `Docker`

## 重點功能

- 本地 MTProto 代理 — 透過 WebSocket 連接 Telegram DC，提升速度和穩定性。
- 自動切換功能 — 當 WebSocket 連接失敗時，自動回退到 TCP 連接，確保連接不間斷。
- 多平台支持 — 提供 Windows、macOS 和 Linux 的安裝包，方便不同用戶使用。
- 系統托盤集成 — 在 Windows 和 macOS 上提供系統托盤應用，方便用戶管理代理設定。
- 簡單的 CLI 操作 — 支持命令行啟動和配置，適合開發者使用。
...
> [!tip] 爆紅原因
> 建立 31 天就累積 4253 stars（137/天），forks 181（4.3%），顯示出穩定的增長趨勢。作者 Flowseal 及其團隊在 Telegram 代理工具方面有豐富的經驗，這使得他們能夠針對用戶需求進行快速迭代。這個工具解決了 Telegram 在某些地區的載入速度慢和連接不穩定的問題，之前的解決方案往往需要依賴外部伺服器，這不僅增加了延遲，也可能存在安全風險。最近的推廣活動和社群討論也提升了其曝光率，尤其是在 Telegram 用戶中。forks/stars 比率為 4.3%，顯示出有相當比例的用戶在進行實際修改和使用。

## 適合誰使用

**目標受眾**：需要在受限網路環境中使用 Telegram 的用戶，特別是自由工作者和網路工程師。

> [!example] 使用場景
> - 網路工程師用它來在防火牆限制的環境中加速 Telegram 連接，因為它能有效繞過網路限制，提升連接穩定性。
> - 自由工作者用它來在不穩定的網路環境中保持 Telegram 的即時通訊，因為它支持自動切換到 TCP 連接，確保訊息不會丟失。
> - 開發者用它來測試 Telegram 應用的代理功能，因為它提供了簡單的安裝和配置過程，並且支持多平台運行。

## 架構分析

tg-ws-proxy 採用 MTProto 協議作為核心架構，這使得它能夠在不暴露用戶數據的情況下，實現高效的數據傳輸。架構中，Telegram Desktop 客戶端首先與本地代理建立連接，然後代理再透過 WebSocket 與 Telegram 的資料中心進行通訊。這樣的設計不僅減少了延遲，還能在網路環境不佳的情況下，提供穩定的連接。

選擇 MTProto 而非其他協議，主要是因為其加密特性和對 Telegram 的原生支持，這樣可以避免因使用第三方協議而導致的安全風險。擴展性方面，tg-ws-proxy 能夠支持多個資料中心的連接，這對於需要高可用性的用戶來說是個優勢。整體來看，tg-ws-proxy 的架構設計考量了安全性和性能，適合在多變的網路環境中使用。

## 技術深入分析

tg-ws-proxy 的核心技術機制是基於 MTProto 協議，這是 Telegram 自家開發的加密通訊協議，能夠保證數據在傳輸過程中的安全性。它使用 WebSocket 作為主要的數據傳輸通道，這樣可以在保持加密的同時，減少延遲和提高傳輸效率。tg-ws-proxy 能夠處理多達 4 個並行連接，這對於高頻率的訊息傳遞場景非常有用。其設計考量了不同平台的兼容性，並提供了 GUI 和 CLI 兩種操作方式，這使得它能夠適應不同用戶的需求。

選擇 Python 作為開發語言，主要是因為其生態系統的豐富性和開發效率，雖然這也可能帶來一定的性能損失。tg-ws-proxy 的依賴相對簡單，這意味著用戶在安裝和運行時不會遇到過多的困難。技術風險方面，若未來 Telegram 改變 MTProto 協議，tg-ws-proxy 可能需要迅速更新以保持兼容性。整合方面，tg-ws-proxy 可以輕鬆地與現有的 Telegram 客戶端進行整合，並且支持在 CI/CD 流程中使用，這對於開發者來說是個加分項。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了詳細的安裝步驟和使用說明，整體清晰易懂。安裝過程相對順暢，但在某些防火牆環境下可能會遇到問題。文件中有針對不同平台的快速入門指南，對於新手來說非常友好。
...
> - 可能會被防火牆阻擋，影響使用。
> - 在某些系統上可能會出現誤報問題。
> - 依賴於 Python 環境，對於不熟悉的用戶可能有學習曲線。

> [!warning] 注意事項
> - 僅支援 Python 3.8 以上版本
> - 在某些防火牆下可能仍然無法正常工作
> - Windows Defender 可能誤報為惡意軟體

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 提供類似的代理功能，但主要針對檔案同步，並不專注於 Telegram 的使用場景。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | 專注於隱私保護和匿名性，功能較為複雜，適合需要高匿名性的用戶。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 專注於檔案同步的代理工具，並不專注於即時通訊的需求。 | 如果你的主要需求是檔案同步而非即時通訊，這個工具會更合適。 | medium，因為需要重新配置同步的參數和環境。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | 提供更高的匿名性和隱私保護，但配置較為複雜。 | 如果你的主要需求是隱私和匿名性，這個工具會更適合。 | high，因為需要重新學習配置和使用方式。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **tg-ws-proxy** | **parsync** | **Shadowbroker** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於檔案同步的代理工具，並不專注於即時通訊的需求。 | 提供更高的匿名性和隱私保護，但配置較為複雜。 |
> | 遷移成本 | - | medium，因為需要重新配置同步的參數和環境。 | high，因為需要重新學習配置和使用方式。 |
> | 適用場景 | 主要場景 | 如果你的主要需求是檔案同步而非即時通訊，這個工具會更合適。 | 如果你的主要需求是隱私和匿名性，這個工具會更適合。 |
...
- **[HIGH]** 在某些防火牆環境下可能無法正常工作
  - 解法：嘗試使用不同的端口或配置
- [MEDIUM] Windows Defender 可能誤報為惡意軟體
  - 解法：在下載時暫時關閉防病毒軟體
- [MEDIUM] 首次啟動時可能會出現配置問題
  - 解法：仔細檢查配置文件，確保所有參數正確

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司使用 Telegram 進行內部溝通 | 非常適合 | tg-ws-proxy 提供了快速且安全的連接，適合小型團隊使用。 |
| 大型企業需要穩定的 Telegram 連接 | 普通 | 雖然能提供穩定性，但對於大型企業來說，可能需要更高的可擴展性。 |
| 自由職業者在不穩定的網路環境中使用 Telegram | 非常適合 | tg-ws-proxy 能夠有效繞過網路限制，提升使用體驗。 |
| 需要高匿名性的用戶使用 Telegram | 不適合 | tg-ws-proxy 主要針對速度和穩定性，對於匿名性需求不高。 |
...
> [!tip] 投入 vs 回報
> 花 2 小時學習，1 小時整合，得到穩定的 Telegram 使用體驗，值得。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：tg-ws-proxy 本身不需要高權限，且不存取敏感資料。依賴的庫經過審計，整體安全性較高。
...
> | [#492](https://github.com/Flowseal/tg-ws-proxy/issues/492) | [Предложение] Fake TLS для MTProto | 11 | 0 |
