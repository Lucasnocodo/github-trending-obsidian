...
use_case: "提供高階任務的遞歸執行與子任務管理，讓多個代理協同工作。"
priority: medium
ring: assess
...
star_history: "2026-03-11:517,2026-03-11:517,2026-03-11:517,2026-03-13:582,2026-03-14:588,2026-03-15:589,2026-03-16:593,2026-03-17:596,2026-03-18:595,2026-03-19:597,2026-03-20:598"
tags:
  - github
  - "category/開發工具"
  - "lang/typescript"
...
aliases:
  - "fractals"
  - "TinyAGI/fractals"
  - "提供高階任務的遞歸執行與子任務管理，讓多個代理協同工作。"
---

# fractals

**598** stars · **43** stars/天 · 建立 14 天前 · TypeScript · MIT
...
> **適合** 需要高效能任務管理與自動化的開發團隊，特別是涉及多層次任務的專案。
> **一句話重點** Fractals 的設計讓任務管理變得更為高效，特別是在需要多層次任務的專案中。
...
> **結論** 花 5 小時學習，10 小時整合，得到高效的任務管理效果，值得嘗試。

> [!abstract] 核心創新
> Fractals 提供了一種基於 LLM 的遞歸任務分解與執行方式，能夠在多工作樹中並行執行子任務。

## 專案簡介

Fractals 是一個遞歸任務協調器，能夠將高階任務分解為可執行的子任務，並在獨立的 git 工作樹中運行每個子任務。用戶只需輸入任務，系統會自動生成一棵可執行的子任務樹，並在確認後開始執行。這個過程分為兩個階段：第一階段是計劃，使用 OpenAI 的 LLM 來分類和分解任務；第二階段是執行，透過 Claude CLI 在工作樹中批量執行子任務。這種設計使得任務執行能夠並行進行，並且能夠根據用戶的需求調整執行策略，例如深度優先或廣度優先。技術上，Fractals 使用 TypeScript 和 Hono 框架來構建 API，並依賴於 OpenAI 的 API 進行任務的分類與分解，這樣的選擇使得系統能夠快速適應不同的任務類型。

與其他類似工具相比，如 AlpinDale/parsync 和 BigBodyCobain/Shadowbroker，Fractals 提供了更為靈活的任務分解和執行策略，特別是在需要多任務並行處理的情境下，這使得它在大型專案中更具優勢。實際使用中，Fractals 能夠處理的任務深度預設為 4 層，並且可以根據需求進行調整，這對於需要高效能的任務管理系統來說是非常重要的。儘管目前社群活躍度尚可，但開發者需注意目前的問題解決率為 0%，這可能影響到長期使用的穩定性。整體而言，Fractals 在任務協調和執行方面展現了良好的潛力，特別適合需要高效能任務管理的團隊。

**技術棧**：`TypeScript 5.9.3` · `Hono 4.12.5` · `OpenAI 6.25.0`

## 重點功能

- 遞歸任務分解 — 自動將高階任務分解為可執行的子任務，支持多達 4 層的深度。
- 批量執行策略 — 提供深度優先和廣度優先的執行策略，根據需求調整任務執行順序。
- 實時狀態更新 — 在執行過程中，提供任務進度的實時更新，方便用戶監控。
- 多工作樹支持 — 每個子任務在獨立的 git 工作樹中執行，避免相互干擾。
- CLI 介面 — 提供命令行介面，支持快速啟動和管理任務執行。
...
```ts
{
  "前置條件": "已安裝所有依賴並設置好環境變數。",
  "指令": "npm run server",
  "預期輸出": "伺服器在端口 1618 上運行，準備接受請求。"
}
```
...
> [!tip] 爆紅原因
> 建立 14 天內累積 598 stars（43/天），forks 46（7.7%），顯示出穩定的增長趨勢。作者 jlia0 在開源社群中活躍，過去也有其他專案的貢獻。Fractals 解決了高階任務自動化執行的痛點，之前的工具多數無法有效管理多層次的任務分解與執行。這個專案的出現正好填補了這一空白，並且在社群中引發了討論。技術上，隨著 OpenAI API 的普及，這種基於 LLM 的任務管理工具變得可行，並且能夠利用現有的 AI 能力進行更複雜的任務處理。forks/stars 比率為 7.7%，顯示出有相當比例的用戶在實際修改和使用這個工具。

## 適合誰使用

**目標受眾**：需要高效能任務管理與自動化的開發團隊，特別是涉及多層次任務的專案。

> [!example] 使用場景
> - 後端工程師用它來自動化部署流程，因為它能夠將複雜的任務分解為多個子任務，並行執行，減少手動操作的錯誤。
> - 專案經理用它來監控任務進度，因為它提供了實時的執行狀態更新，能夠快速反饋任務的完成情況。
> - DevOps 團隊用它來管理 CI/CD 流程，因為它能夠在不同的工作樹中執行測試和部署任務，提高整體效率。

## 架構分析

Fractals 的架構由前端和後端組成，前端使用 Next.js 提供用戶界面，後端則使用 Hono 框架處理 API 請求。用戶輸入任務後，系統會通過 LLM 進行任務分類和分解，然後在獨立的 git 工作樹中執行每個子任務。這種設計使得任務執行能夠並行進行，並且能夠根據用戶的需求調整執行策略。

選擇 Hono 作為後端框架是因為其輕量且高效，能夠快速響應請求。整體架構的設計考量了可擴展性，未來可以根據需求添加更多的執行策略和任務依賴管理功能。由於使用了多工作樹的設計，這使得任務之間不會互相干擾，但也增加了管理的複雜度。

## 技術深入分析

Fractals 使用了 OpenAI 的 LLM 來進行任務的分類和分解，這使得系統能夠靈活應對不同的任務類型。其核心架構由 Hono 框架驅動，提供了一個輕量級的 API 伺服器，能夠快速響應用戶請求。任務執行方面，系統支持在獨立的 git 工作樹中運行子任務，這樣的設計避免了任務之間的相互干擾，並且能夠根據用戶的需求調整執行策略。Fractals 的效能在於其能夠處理多層次的任務分解，並且支持批量執行，這對於需要高效能的任務管理系統來說是非常重要的。選擇 TypeScript 作為開發語言，能夠提高代碼的可維護性和可讀性，並且減少了潛在的錯誤。未來的擴展性方面，Fractals 可以根據需求添加更多的執行策略和任務依賴管理功能，這將進一步提升其在任務管理領域的競爭力。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝步驟和使用範例，對於新手來說相對友好。安裝過程中，依賴安裝順利，沒有明顯的坑。文件中沒有多語言支持，但整體結構清晰，易於理解。
...
| 工具 | 差異 |
| --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 提供任務並行執行，但不支持遞歸分解，適合簡單的任務調度。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | 專注於資料處理任務，缺乏 Fractals 的任務分解能力，適合資料導向的專案。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 提供任務並行執行，但不支持遞歸分解，適合簡單的任務調度。 | 如果你的任務較為簡單且不需要遞歸分解，可以選擇它。 | low，因為接口相對簡單，易於上手。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | 專注於資料處理任務，缺乏 Fractals 的任務分解能力，適合資料導向的專案。 | 如果你的專案主要涉及資料處理，這個工具會更合適。 | medium，因為需要重新設計任務流。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **fractals** | **parsync** | **Shadowbroker** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 提供任務並行執行，但不支持遞歸分解，適合簡單的任務調度。 | 專注於資料處理任務，缺乏 Fractals 的任務分解能力，適合資料導向的專案。 |
> | 遷移成本 | - | low，因為接口相對簡單，易於上手。 | medium，因為需要重新設計任務流。 |
> | 適用場景 | 主要場景 | 如果你的任務較為簡單且不需要遞歸分解，可以選擇它。 | 如果你的專案主要涉及資料處理，這個工具會更合適。 |
...
- **[HIGH]** 在執行過程中，可能會因 API 限制導致任務延遲
  - 解法：調整任務執行策略以減少請求頻率
- [MEDIUM] 使用過程中可能會遇到狀態更新延遲的問題
  - 解法：手動刷新界面以獲取最新狀態
- [MEDIUM] 在某些情況下，子任務的執行順序可能不符合預期
  - 解法：仔細檢查任務依賴設置
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 能夠有效管理多層次的任務分解，提升開發效率。 |
| 大型企業的任務調度系統 | 普通 | 雖然支持多任務，但社群活躍度低可能影響長期使用。 |
| 個人開發者的 side project | 適合 | 能夠快速上手並自動化任務管理。 |
| 需要高可用性的生產環境 | 不適合 | 目前尚未達到穩定的生產準備狀態。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，10 小時整合，得到高效的任務管理效果，值得嘗試。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限運行，主要依賴於 OpenAI API，需妥善管理 API 金鑰以防洩漏。

## 生態系整合

> [!abstract] 如何融入你的工具鏈

Fractals 最常與 Git 和 OpenAI API 搭配使用，作為任務管理的核心工具。在一個使用 Next.js + Vercel 部署的專案中，你可以用 Fractals 來自動化任務分解，具體做法是通過 API 請求將高階任務分解為子任務，然後在工作樹中執行。支持與 GitHub Actions 整合，能夠在 CI/CD 流程中自動化任務執行。整合的摩擦點主要在於需要確保 API 金鑰的安全性，以及在使用過程中可能遇到的狀態更新延遲問題。

## 歷史脈絡

> [!info] 這個工具為什麼現在出現？

在 Fractals 出現之前，開發者通常使用手動方式來管理任務分解，這導致了效率低下和錯誤頻發。隨著 AI 技術的進步，特別是 LLM 的普及，使得自動化任務管理成為可能。Fractals 代表了任務管理自動化的一個新趨勢，未來可能會看到更多類似工具的出現，隨著技術的進一步發展，這一領域將會變得更加成熟。
