...
use_case: "自動化任務分解與執行，讓多個代理協作完成複雜任務。"
priority: medium
ring: assess
...
aliases:
  - "fractals"
  - "TinyAGI/fractals"
  - "自動化任務分解與執行，讓多個代理協作完成複雜任務。"
---

# fractals

**619** stars · **13** stars/天 · 建立 46 天前 · TypeScript · MIT
...
> **適合** 需要自動化複雜任務並協調多個代理的開發團隊。
> **一句話重點** Fractals 不僅僅是一個任務執行工具，它還能自動生成任務樹，讓用戶更清楚任務之間的關係。
...
> **結論** 花 5 小時學習，8 小時整合，能夠自動化複雜任務，值得投資。

> [!abstract] 核心創新
> Fractals 提供了一個遞歸的任務協調器，能夠自動生成可執行的任務樹並在多個代理間協作。

## 專案簡介

Fractals 是一個遞歸任務協調器，能夠將高層次任務分解成可執行的子任務，並在獨立的 git 工作樹中運行每個子任務。用戶只需輸入一個任務，系統會自動生成一棵可執行的子任務樹，並在確認後執行。這個過程分為兩個階段：計劃和執行。計劃階段中，系統會使用 OpenAI 的 LLM 來分類和分解任務，然後在執行階段中，通過 Claude CLI 來批量運行子任務，並實時更新狀態。這種設計使得任務的執行可以在不同的環境中進行，並且能夠有效管理任務之間的依賴關係。核心賣點在於其遞歸性和自動化，能夠處理複雜的任務結構，並且支持多種批量執行策略，如深度優先和廣度優先。使用者可以透過 API 來初始化工作空間、開始任務分解和執行，這些操作都可以通過簡單的 HTTP 請求來完成。

**技術棧**：`TypeScript 5.9.3` · `Hono 4.12.5` · `OpenAI 6.25.0`

## 重點功能

- 遞歸任務分解 — 將高層次任務自動分解成可執行的子任務，支持最大深度設定。
- 多種批量執行策略 — 支持深度優先和廣度優先的執行策略，根據需求選擇最合適的方式。
- 實時狀態更新 — 在執行過程中，通過 API 實時獲取任務狀態，便於監控進度。
- 獨立 git 工作樹 — 每個子任務在獨立的 git 工作樹中執行，避免相互影響。
- API 支持 — 提供多個 API 端點，方便集成到現有系統中。
...
> [!tip] 爆紅原因
> 建立 46 天就累積 619 stars（約 13/天），forks 46（7.4%），顯示出一定的使用者興趣。作者 jlia0 之前在開源社群活躍，這個專案解決了傳統任務執行中缺乏自動化和協作的痛點，特別是在需要多個代理協同工作的情境下。這個工具的出現是因為現今對於自動化和智能任務管理的需求不斷上升，尤其是在開發和運維領域。forks/stars 比率適中，顯示出有一定數量的使用者在實際修改和使用這個工具。

## 適合誰使用

**目標受眾**：需要自動化複雜任務並協調多個代理的開發團隊。

> [!example] 使用場景
> - 後端工程師用它來自動化部署流程，因為它能夠將複雜的任務分解成可管理的子任務，減少手動操作的錯誤。
> - DevOps 團隊用它來協調多個服務的升級，因為它支持任務依賴管理，確保每個服務在正確的時間升級。
> - 產品經理用它來規劃大型專案的執行，因為它能視覺化任務樹，幫助團隊理解任務之間的關係。

## 架構分析

Fractals 採用 Hono 作為後端框架，並使用 Next.js 作為前端，這樣的選擇使得整體架構輕量且易於擴展。資料流中，使用者輸入任務後，系統會通過 LLM 進行分類和分解，然後生成任務樹。執行階段則通過 Claude CLI 在獨立的 git 工作樹中運行子任務。

選擇使用 Hono 的原因是其輕量和高效，適合快速開發和部署。這種架構的代價是需要用戶手動確認計劃，無法完全自動化。擴展性方面，由於使用了 git 工作樹，能夠輕鬆管理多個任務的執行，但在任務數量過多時可能會導致性能瓶頸。

## 技術深入分析

Fractals 的核心技術在於其遞歸任務分解能力，使用 OpenAI 的 LLM 來分類和分解任務，這使得它能夠處理複雜的任務結構。系統的效能在於其能夠在獨立的 git 工作樹中運行子任務，這樣的設計避免了任務之間的相互影響。選擇 Hono 作為後端框架的原因是其輕量和高效，能夠快速響應用戶請求。這個選擇的代價是需要用戶手動確認計劃，這在某些情況下可能會影響使用體驗。技術風險方面，隨著任務數量的增加，可能會出現性能瓶頸，特別是在執行大量子任務時。整合方面，Fractals 可以輕鬆與現有的 git 流程結合，但在 CI/CD pipeline 中的整合可能需要額外的適配。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝步驟和使用說明，但缺乏範例。安裝過程相對順暢，沒有明顯的坑。沒有提供多語言支持，可能對非英語使用者造成障礙。
...
> [!danger] 缺點
> - 需要手動確認計劃，增加了操作步驟。
> - 目前僅支持 OpenAI 作為 LLM，選擇有限。
> - 批量執行策略尚在開發中，功能不夠完善。

> [!warning] 注意事項
> - 目前僅支持 OpenAI API 作為 LLM，未來可能增加其他選項。
> - 批量執行策略尚在開發中，部分功能可能不穩定。
> - 需要用戶手動確認計劃，無法完全自動化。
...
| 工具 | 差異 |
| --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard 提供類似的任務管理功能，但不支持遞歸分解，適合較簡單的任務。 |
| [AgentSeal/codeburn](https://github.com/AgentSeal/codeburn) | Codeburn 專注於單一任務的執行，而 Fractals 支持任務的遞歸分解和多任務協作。 |
| [AgriciDaniel/claude-obsidian](https://github.com/AgriciDaniel/claude-obsidian) | Claude-Obsidian 提供任務執行的功能，但缺乏 Fractals 的可視化任務樹和依賴管理。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard 提供簡單的任務管理，但不支持遞歸分解，適合較簡單的任務。 | 如果你的任務結構較簡單，且不需要遞歸分解，Boneyard 可能是更輕量的選擇。 | low，因為 Boneyard 的使用方式相對簡單，轉移成本低。 |
| [AgentSeal/codeburn](https://github.com/AgentSeal/codeburn) | Codeburn 專注於單一任務的執行，而 Fractals 支持任務的遞歸分解和多任務協作。 | 如果你的專案需要處理單一任務而不需要複雜的任務分解，Codeburn 會更合適。 | medium，因為需要重新調整任務結構以符合 Codeburn 的設計。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **fractals** | **boneyard** | **codeburn** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | Boneyard 提供簡單的任務管理，但不支持遞歸分解，適合較簡單的任務。 | Codeburn 專注於單一任務的執行，而 Fractals 支持任務的遞歸分解和多任務協作。 |
> | 遷移成本 | - | low，因為 Boneyard 的使用方式相對簡單，轉移成本低。 | medium，因為需要重新調整任務結構以符合 Codeburn 的設計。 |
> | 適用場景 | 主要場景 | 如果你的任務結構較簡單，且不需要遞歸分解，Boneyard  | 如果你的專案需要處理單一任務而不需要複雜的任務分解，Code |
...
> [!tip] 採用建議
> 目前適合用於實驗性專案，不建議在生產環境中使用。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- [MEDIUM] 使用者在執行計劃時可能會遇到確認按鈕被隱藏的問題
  - 解法：調整瀏覽器視窗大小以顯示按鈕
- **[HIGH]** 在高深度任務分解時，可能會導致性能下降
  - 解法：減少最大深度設定以提高性能
- [low] 任務狀態更新可能會延遲
  - 解法：手動刷新狀態頁面以獲取最新資訊
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 能夠自動化複雜的任務分解，減少手動操作。 |
| 大型企業的 DevOps 團隊 | 適合 | 支持多任務協作和依賴管理，適合複雜的工作流。 |
| 個人開發者的簡單專案 | 普通 | 對於簡單任務，可能過於複雜。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，8 小時整合，能夠自動化複雜任務，值得投資。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：不需要高權限，僅存取用戶提供的環境變數和 git 工作樹。依賴的第三方庫需定期檢查更新以避免潛在的安全風險。

## 生態系整合

> [!abstract] 如何融入你的工具鏈

Fractals 最常與 git 和 OpenAI API 搭配使用，適合在開發和運維環境中使用。在一個使用 Next.js + Vercel 的專案中，可以用 Fractals 來自動化任務分解，具體做法是通過 API 將任務發送到 Fractals，然後在 git 工作樹中執行。支援 GitHub Actions 等 CI 工具，但可能需要額外的適配。整合的摩擦點主要在於需要用戶手動確認計劃，這可能影響使用流暢度。

## 歷史脈絡

> [!info] 這個工具為什麼現在出現？

在 Fractals 出現之前，開發者通常使用手動方式來管理任務，這導致了效率低下和錯誤頻發。隨著 AI 和自動化技術的進步，對於能夠自動化任務管理的工具需求上升。Fractals 代表了任務自動化的一個新趨勢，未來可能會有更多類似的工具出現，尤其是在需要多代理協作的場景中。
...
> [!tip] 導入策略
> 第一週：在小型專案中試用 Fractals。第二週：在內部工具中導入，收集反饋。第三週：撰寫最佳實踐文檔，分享給團隊。第四週：在主要產品中使用，監控效果。

**成功指標**：任務執行時間減少 30%，錯誤率降低至 5% 以下。

> [!warning] 退出計畫
> 所有設定存在標準 JSON 格式，可以輕鬆轉換為其他任務管理工具。
...
相關專案：[[0xGF--boneyard|0xGF/boneyard]] · [[AgentSeal--codeburn|AgentSeal/codeburn]] · [[AgriciDaniel--claude-obsidian|AgriciDaniel/claude-obsidian]] · [[BuilderPulse--BuilderPulse|BuilderPulse/BuilderPulse]] · [[VoltAgent--awesome-codex-subagents|VoltAgent/awesome-codex-subagents]] · [[AlpinDale--parsync|AlpinDale/parsync]] · [[BigBodyCobain--Shadowbroker|BigBodyCobain/Shadowbroker]] · [[CoderLuii--HolyClaude|CoderLuii/HolyClaude]]
