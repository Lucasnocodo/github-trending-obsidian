...
use_case: "自動化任務分解與執行的遞歸任務協調器，讓多個代理同時運行。"
priority: medium
ring: assess
...
star_history: "2026-03-11:517,2026-03-11:517,2026-03-11:517,2026-03-13:582,2026-03-14:588,2026-03-15:589,2026-03-16:593,2026-03-17:596,2026-03-18:595,2026-03-19:597,2026-03-20:598,2026-03-21:599,2026-03-22:599,2026-03-23:600,2026-03-24:604,2026-03-25:604"
tags:
  - github
  - "category/開發工具"
  - "lang/typescript"
...
aliases:
  - "fractals"
  - "TinyAGI/fractals"
  - "自動化任務分解與執行的遞歸任務協調器，讓多個代理同時運行。"
---

# fractals

**604** stars · **32** stars/天 · 建立 19 天前 · TypeScript · MIT
...
> **適合** 需要自動化複雜任務分解的開發者，特別是對於使用 git 的團隊。
> **一句話重點** Fractals 的創新在於自動化任務分解與執行，特別適合需要處理複雜任務的開發者。
...
> [!abstract] 核心創新
> Fractals 提供了一個自動化的遞歸任務分解與執行解決方案，並在可視化和即時狀態更新上做出了創新。

## 專案簡介

Fractals 是一個遞歸任務協調器，能夠將高層次的任務分解為可執行的子任務，並利用代理群體在獨立的 git 工作樹中執行這些子任務。用戶只需輸入一個任務，系統會生成一棵自相似的可執行子任務樹，然後在每個葉子節點上進行執行。這個設計使得任務的執行過程能夠並行化，從而提高效率。關鍵指令如 `npm run server` 啟動後端服務，並透過 HTTP 請求與前端進行交互。這個工具的賣點在於其能夠自動化任務的分解與執行，特別適合需要處理複雜任務的開發者。技術上，Fractals 使用 TypeScript 和 Hono 框架來構建 API，並整合 OpenAI 的 LLM 進行任務分類與分解。

這樣的選擇使得系統能夠快速響應並處理用戶的請求。與其他工具相比，如 AlpinDale/parsync 和 BigBodyCobain/Shadowbroker，Fractals 提供了一個更為直觀的任務視覺化界面和即時狀態更新，這對於需要即時反饋的工作流尤為重要。實際使用中，Fractals 能夠處理多達 4 層的遞歸深度，並且支持批量執行策略以應對 API 的速率限制。這些特性使得 Fractals 在面對大型專案時能夠有效管理任務依賴與優先級。儘管目前仍在開發中，社群活躍度不高，但其潛力不容小覷，未來可能會有更多功能加入。對於小型團隊或個人開發者而言，這是一個值得關注的工具，特別是當需要自動化複雜任務時。

**技術棧**：`TypeScript 5.9.3` · `Hono 4.12.5` · `OpenAI 6.25.0`

## 重點功能

- 遞歸任務分解 — 自動將高層次任務分解為可執行的子任務，並生成任務樹。
- 即時狀態更新 — 使用 HTTP 請求獲取當前執行狀態，支持實時監控。
- 多層次執行 — 支持最多 4 層的遞歸深度，適合處理複雜任務。
- 批量執行策略 — 提供深度優先和廣度優先的執行策略，應對 API 的速率限制。
- 與 OpenAI 整合 — 使用 OpenAI 的 LLM 進行任務分類與分解，提升準確性。
...
> [!tip] 爆紅原因
> 建立 19 天內累積 604 stars（32/天），forks 47（7.8%），顯示出穩定的增長潛力。這個專案的作者 jlia0 在開源社群中有一定的影響力，並且提供了一個解決高層次任務分解的有效方案，這在過去的工具中並不常見。該專案的設計考量了用戶的實際需求，並在功能上做了創新，特別是在任務的可視化和執行管理上。社群的反饋和需求也促進了這個專案的發展，未來可能會有更多功能加入以滿足用戶需求。

## 適合誰使用

**目標受眾**：需要自動化複雜任務分解的開發者，特別是對於使用 git 的團隊。

> [!example] 使用場景
> - 後端工程師用它來自動化複雜的任務分解，因為手動管理多層任務容易出錯且耗時。
> - 產品經理用它來追蹤任務進度，因為可視化的任務樹能夠幫助他們更清楚地了解進度與依賴。
> - 獨立開發者用它來快速執行多個子任務，因為它能夠在 git 工作樹中隔離每個任務的執行環境，避免衝突。

## 架構分析

Fractals 的架構設計採用前後端分離的模式，前端使用 Next.js 提供用戶界面，後端則使用 Hono 框架構建 API。這樣的設計使得前端能夠專注於用戶體驗，而後端則專注於任務的處理與執行。資料流中，任務由用戶輸入後，經過 LLM 的分類與分解，生成一棵任務樹，然後在 git 工作樹中執行。

選擇 Hono 作為後端框架是因為其輕量且高效，能夠快速響應用戶請求。這樣的設計使得系統能夠處理多個請求而不會造成延遲。擴展性方面，未來可以加入更多的執行策略和任務依賴管理，但目前的設計在處理複雜任務時仍有瓶頸，特別是在任務依賴性上。

## 技術深入分析

Fractals 的核心技術機制在於其遞歸任務分解能力，利用 OpenAI 的 LLM 進行任務的分類與分解，這使得系統能夠自動生成一棵任務樹。系統的效能特性在於能夠同時處理多個請求，並透過批量執行策略來應對 API 的速率限制。設計上選擇 TypeScript 和 Hono 框架，這不僅提高了開發效率，也使得系統在運行時能夠保持高效能。依賴樹的複雜度相對較低，主要依賴於 OpenAI 的 API，這可能帶來供應鏈風險。技術風險方面，未來在擴展功能時，任務依賴管理的實現可能會成為瓶頸，特別是在任務執行順序上。整合分析中，Fractals 與主流框架的整合難度相對較低，但在 CI/CD pipeline 的友好度上仍需進一步提升。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且包含必要的安裝步驟，但缺乏詳細的使用範例。安裝過程相對順暢，但需要注意環境變數的設置。整體來說，對於新手來說，花 30 分鐘能夠順利啟動並運行。
...
> - 目前功能尚不完善，部分功能仍在開發中。
> - 社群活躍度低，問題解決速度慢。
> - 依賴於 OpenAI 的 API，限制了使用範圍。
> - 尚未實現任務依賴管理，可能導致執行錯誤。

> [!warning] 注意事項
> - 目前僅支持 OpenAI 的 API，無法使用其他 LLM。
> - 批量執行策略尚在開發中，可能會影響執行效率。
> - 社群活躍度不高，可能影響問題的解決速度。
> - 尚未實現任務依賴管理，可能導致執行順序錯誤。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 專注於並行任務執行，但不支持任務的遞歸分解，適合簡單的任務管理。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | 提供任務調度功能，但缺乏可視化界面，使用上不夠直觀。 |
| [Flowseal/tg-ws-proxy](https://github.com/Flowseal/tg-ws-proxy) | 專注於即時通訊的任務處理，與 Fractals 的任務分解功能不同，適合不同場景。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 專注於並行任務執行，並不支持任務的遞歸分解，適合簡單的任務管理。 | 如果你的任務較為簡單，且不需要複雜的分解，則可以選擇它。 | low，因為其 API 設計較為簡單，易於上手。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | 提供任務調度功能，但缺乏可視化界面，使用上不夠直觀。 | 如果你需要一個穩定的任務調度工具，但不需要可視化功能，可以考慮它。 | medium，因為需要重新適應其任務管理方式。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **fractals** | **parsync** | **Shadowbroker** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於並行任務執行，並不支持任務的遞歸分解，適合簡單的任務管理。 | 提供任務調度功能，但缺乏可視化界面，使用上不夠直觀。 |
> | 遷移成本 | - | low，因為其 API 設計較為簡單，易於上手。 | medium，因為需要重新適應其任務管理方式。 |
> | 適用場景 | 主要場景 | 如果你的任務較為簡單，且不需要複雜的分解，則可以選擇它。 | 如果你需要一個穩定的任務調度工具，但不需要可視化功能，可以考 |
...
- **[HIGH]** 任務執行時可能會因為 API 限制而導致延遲
  - 解法：調整批量執行策略以減少請求數量
- [MEDIUM] 如果未設置環境變數，系統將無法啟動
  - 解法：確保在啟動前設置 .env 檔案
- [MEDIUM] 任務依賴管理尚未實現，可能導致執行錯誤
  - 解法：手動檢查任務依賴關係
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型團隊的任務管理 | 非常適合 | 能夠自動化任務分解，減少手動操作的錯誤。 |
| 大型專案的任務協調 | 普通 | 目前功能尚不完善，部分功能仍在開發中。 |
| 獨立開發者的個人專案 | 適合 | 支持多層次的任務執行，適合複雜專案。 |
...
> [!warning] 安全性快速掃描
> 低風險：不需要高權限，僅需設置 OpenAI API 金鑰，依賴鏈的信任程度較高。
