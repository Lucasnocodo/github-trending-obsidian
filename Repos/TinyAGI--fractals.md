...
use_case: "提供一個遞迴任務調度器，讓代理人群體自動執行高階任務的子任務。"
priority: medium
ring: assess
...
aliases:
  - "fractals"
  - "TinyAGI/fractals"
  - "提供一個遞迴任務調度器，讓代理人群體自動執行高階任務的子任務。"
---

# fractals

**616** stars · **18** stars/天 · 建立 35 天前 · TypeScript · MIT
...
> **授權** MIT (商業友好) · **維護** Slow (最後推送 34 天前) · **貢獻者** Solo (bus factor 風險) · **參與度** Low
> **適合** 需要自動化複雜任務調度的開發團隊，特別是使用 git 和 OpenAI 的專案。
> **一句話重點** Fractals 的遞迴任務調度方式能顯著提升複雜任務的執行效率，尤其是在需要多任務並行處理的情境下。
...
> **結論** 花 5 小時學習，10 小時整合，得到自動化任務調度的效果，值得嘗試。

> [!abstract] 核心創新
> Fractals 提供了一種新的遞迴任務調度方式，能夠自動生成可執行的任務樹並在獨立環境中執行。

## 專案簡介

Fractals 是一個遞迴任務調度器，能夠將高階任務轉化為可執行的子任務樹，並透過代理人群體在獨立的 git 工作樹中執行每個葉子任務。使用者只需輸入任務，系統會自動分類並分解，然後在確認計劃後執行。這個過程分為兩個階段：第一階段是計劃，使用 OpenAI 的 LLM 進行任務的分類和分解；第二階段是執行，透過 Claude CLI 在 git 工作樹中批量執行任務。這種設計使得任務執行可以在不同的環境中進行，並且支持多種批次執行策略，例如深度優先和廣度優先。技術上，Fractals 使用 TypeScript 和 Hono 框架來構建 API，並依賴 OpenAI 的 API 進行任務的智能處理。

這個工具的賣點在於其能夠自動化複雜的任務調度，特別適合需要處理大量子任務的情境。相較於傳統的任務調度工具，Fractals 透過遞迴結構和代理人群體的方式，能夠更有效地管理和執行任務，尤其在面對高複雜度的任務時。使用者可以透過簡單的 API 調用來初始化工作空間和執行任務，並且可以根據需求調整最大遞迴深度和批次策略。這使得 Fractals 在處理大規模任務時，能夠保持高效能和靈活性。

**技術棧**：`TypeScript 5.9.3` · `Hono 4.12.5` · `OpenAI API`

## 重點功能

- 遞迴任務調度 — 將高階任務轉化為可執行的子任務樹。
- 多種批次執行策略 — 支持深度優先和廣度優先的任務執行。
- 即時狀態更新 — 透過 API 獲取當前任務樹狀態和執行進度。
- CLI 支援 — 提供 CLI 介面以便於獨立執行任務。
- 環境獨立執行 — 在獨立的 git 工作樹中執行每個葉子任務。
...
> [!tip] 爆紅原因
> 建立 35 天內累積 616 stars（約 18 stars/天），forks 數量為 46（7.5%），顯示出不錯的社群關注度。作者 jlia0 在開源社區活躍，致力於開發自動化工具，Fractals 解決了傳統任務調度工具無法有效處理複雜任務的痛點。之前的方案往往需要手動管理子任務，效率低下。此專案的推出引起了開發者的興趣，尤其是在自動化和代理人技術日益普及的背景下。forks/stars 比率為 7.5%，顯示出相對穩定的使用者基礎，並且有潛在的實際修改需求。

## 適合誰使用

**目標受眾**：需要自動化複雜任務調度的開發團隊，特別是使用 git 和 OpenAI 的專案。

> [!example] 使用場景
> - 後端工程師用它來自動化複雜的任務調度，因為可以將高階任務分解為可執行的子任務，並在 git 工作樹中獨立執行。
> - DevOps 工程師用它來管理 CI/CD 流程，因為能夠根據任務依賴性自動調整執行順序，提升整體效率。
> - 產品經理用它來追蹤任務進度，因為可以視覺化任務樹，並實時更新執行狀態，讓團隊更透明化。

## 架構分析

Fractals 的架構由前端和後端組成，前端使用 Next.js 提供用戶界面，後端則基於 Hono 框架構建 API。這種分層設計使得前端能夠專注於用戶交互，而後端則處理任務的分類、分解及執行。資料流從用戶輸入任務開始，經過 LLM 的分類和分解，最終生成執行計劃並在 git 工作樹中執行。

選擇 Hono 作為後端框架是因為其輕量和高效能，能夠快速響應用戶請求。這種設計的代價是需要用戶對 git 工作樹有一定的了解，否則可能會遇到使用上的困難。擴展性方面，未來可以透過增加更多的執行器來支持不同的任務類型，但目前的設計仍然依賴於 OpenAI 的 API，這可能在未來成為一個瓶頸。

## 技術深入分析

Fractals 的核心技術機制是基於遞迴任務調度，利用 OpenAI 的 LLM 進行任務的分類和分解。這種設計使得任務可以自動生成樹狀結構，並在不同的 git 工作樹中獨立執行。效能上，由於使用了批次執行策略，能夠有效控制 API 的調用次數，避免超出速率限制。設計上選擇 TypeScript 和 Hono 框架，這使得開發過程中能夠享受靜態類型檢查的好處，並且 Hono 的輕量特性有助於提升響應速度。技術風險方面，依賴於 OpenAI 的 API 可能會導致未來的功能受限，尤其是當 API 的使用條款改變時。整合方面，Fractals 可以與現有的 CI/CD 流程無縫對接，特別是在需要自動化任務調度的場景中，能夠提升整體開發效率。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且詳細，提供了安裝步驟和使用範例。安裝過程相對順暢，但對於不熟悉 git 的使用者可能會有一些挑戰。文件目前僅提供英文版本，缺乏多語言支持。
...
> [!danger] 缺點
> - 對 git 的依賴可能增加學習曲線。
> - 目前僅支持 OpenAI 的 API，限制了使用場景。
> - 在處理極端複雜任務時，可能需要手動調整。

> [!warning] 注意事項
> - 目前僅支持 OpenAI 的 API，無法使用其他 LLM。
> - 對於非常複雜的任務，可能需要手動調整子任務的執行順序。
> - 依賴於 git 工作樹，對於不熟悉 git 的使用者可能有學習曲線。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard 提供了類似的任務調度功能，但主要集中在單一任務的執行，而 Fractals 支持遞迴結構和多任務並行執行。 |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | Parsync 專注於並行處理，但缺乏 Fractals 的任務分解和視覺化功能，適合簡單的並行任務。 |
| [NVIDIA/NemoClaw](https://github.com/NVIDIA/NemoClaw) | NemoClaw 主要針對 AI 模型的訓練與推論，而 Fractals 更加通用，適用於各種任務調度場景。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard 提供了任務調度功能，但主要集中在單一任務的執行，缺乏 Fractals 的遞迴結構。 | 如果你的需求僅限於簡單的任務執行，而不需要遞迴調度功能，Boneyard 會是更簡單的選擇。 | low，因為 Boneyard 的 API 設計相對簡單，容易上手。 |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | Parsync 專注於並行處理，但不支持 Fractals 的任務分解和視覺化功能。 | 如果你的主要需求是高效的並行處理，而不是任務的遞迴調度，Parsync 會是更合適的選擇。 | medium，因為需要重新設計任務的執行邏輯。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **fractals** | **boneyard** | **parsync** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | Boneyard 提供了任務調度功能，但主要集中在單一任務的執行，缺乏 Fractals 的遞迴結構。 | Parsync 專注於並行處理，但不支持 Fractals 的任務分解和視覺化功能。 |
> | 遷移成本 | - | low，因為 Boneyard 的 API 設計相對簡單，容易上手。 | medium，因為需要重新設計任務的執行邏輯。 |
> | 適用場景 | 主要場景 | 如果你的需求僅限於簡單的任務執行，而不需要遞迴調度功能，Bo | 如果你的主要需求是高效的並行處理，而不是任務的遞迴調度，Pa |
...
> [!tip] 採用建議
> 適合在小型專案中試用，但不建議用於生產環境的核心任務。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在高階任務的分解過程中，可能會出現無法正確識別子任務的情況。
  - 解法：手動檢查並調整任務結構。
- [MEDIUM] 使用者在確認計劃後，可能會發現執行按鈕被隱藏。
  - 解法：調整瀏覽器視窗大小以顯示按鈕。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 能夠自動化任務調度，減少手動操作。 |
| 大型企業的 CI/CD 流程 | 適合 | 支持多種任務執行策略，能夠提升效率。 |
| 小型專案的任務管理 | 普通 | 雖然功能強大，但對於簡單任務可能過於複雜。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，10 小時整合，得到自動化任務調度的效果，值得嘗試。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限，主要存取 OpenAI API，且不存取敏感資料。
...
相關專案：[[0xGF--boneyard|0xGF/boneyard]] · [[AlpinDale--parsync|AlpinDale/parsync]] · [[NVIDIA--NemoClaw|NVIDIA/NemoClaw]] · [[BigBodyCobain--Shadowbroker|BigBodyCobain/Shadowbroker]] · [[CoderLuii--HolyClaude|CoderLuii/HolyClaude]] · [[FreedomIntelligence--OpenClaw-Medical-Skills|FreedomIntelligence/OpenClaw-Medical-Skills]] · [[Flowseal--tg-ws-proxy|Flowseal/tg-ws-proxy]] · [[karpathy--agenthub|karpathy/agenthub]]
