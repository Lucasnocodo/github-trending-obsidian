...
use_case: "透過遞迴任務生成和執行，讓多個代理協作完成複雜任務。"
priority: medium
ring: assess
...
star_history: "2026-03-11:517,2026-03-11:517,2026-03-11:517,2026-03-13:582,2026-03-14:588,2026-03-15:589,2026-03-16:593,2026-03-17:596,2026-03-18:595,2026-03-19:597,2026-03-20:598,2026-03-21:599,2026-03-22:599,2026-03-23:600,2026-03-24:604,2026-03-25:604,2026-03-26:604,2026-03-27:604,2026-03-28:607,2026-03-29:608,2026-03-30:609,2026-03-31:609,2026-04-01:610,2026-04-02:611,2026-04-03:611,2026-04-04:611,2026-04-05:614"
tags:
  - github
  - "category/開發工具"
  - "lang/typescript"
...
aliases:
  - "fractals"
  - "TinyAGI/fractals"
  - "透過遞迴任務生成和執行，讓多個代理協作完成複雜任務。"
---

# fractals

**614** stars · **20** stars/天 · 建立 30 天前 · TypeScript · MIT
...
> **適合** 需要自動化任務管理和協調的開發團隊，特別是面對複雜任務結構的團隊。
> **一句話重點** Fractals 不僅是一個任務協調工具，更是一個自動化的任務生成系統，能夠有效應對複雜的任務結構。
...
> **結論** 花 5 小時學習，10 小時整合，得到自動化的任務管理，值得嘗試。

> [!abstract] 核心創新
> Fractals 的核心創新在於其能夠自動生成任務樹，並在多個代理之間協調執行。這使得複雜任務的管理變得更加高效和自動化。

## 專案簡介

Fractals 是一個遞迴任務協調器，能夠將高層次任務轉化為可執行的子任務樹，並在獨立的 git 工作樹中運行每個葉子任務。用戶只需輸入一個高層次任務，系統會自動將其分解為多個子任務，並通過 OpenAI 的 API 進行分類和分解，最終在 Claude CLI 中執行這些任務。這樣的設計使得任務執行過程中可以實現高效的資源管理和任務追蹤，特別適合需要多任務並行處理的情境。關鍵指令包括 `npm run server` 啟動後端服務和 `npm run dev` 啟動前端界面，讓用戶可以方便地進行任務輸入和狀態檢查。

技術上，Fractals 使用 TypeScript 和 Hono 框架來構建 API，並依賴 OpenAI 的 GPT 模型進行任務的分類和分解。這種選擇使得系統能夠靈活地處理複雜的任務結構，而不需要用戶手動設計每個任務的執行邏輯。與其他任務協調工具相比，Fractals 的遞迴設計使得它能夠自動生成任務樹，這在處理層級關係複雜的任務時顯得尤為重要。



在與類似工具的比較中，Fractals 的優勢在於其能夠動態生成和執行任務樹，而不是僅僅依賴靜態的任務列表。例如，與 AlpinDale/parsync 相比，Fractals 提供了更高層次的抽象，能夠自動處理任務間的依賴關係，這使得它在面對複雜的任務時更具靈活性。另一方面，像是 CoderLuii/HolyClaude 這類工具則可能需要用戶手動管理任務的執行順序，這在大規模任務處理中會增加額外的負擔。

使用 Fractals 時，可能會遇到的挑戰包括對於任務樹的管理和調整，特別是在任務數量較多時，使用者需要清楚如何設置工作環境和確認任務計劃。社群的活躍度相對較低，目前開放的 Issues 也顯示出一些功能上的需求和使用者的疑慮。

總體來看，Fractals 目前處於 beta 階段，對於需要進行複雜任務協調的團隊來說，值得一試，但在生產環境中使用時仍需謹慎考量其穩定性和社群支持的情況。未來六個月內，若能持續改進用戶體驗和增加社群互動，將有助於其成長和成熟。

**技術棧**：`TypeScript 5.9.3` · `Hono 4.12.5` · `OpenAI API 6.25.0`

## 重點功能

- 遞迴任務生成 — 根據高層次任務自動生成可執行的子任務樹。
- 多代理執行 — 在獨立的 git 工作樹中運行每個葉子任務，實現任務的並行處理。
- 即時狀態更新 — 透過 API 查詢任務樹的當前狀態，支持實時監控。
- 批次執行策略 — 支持深度優先和未來的廣度優先執行策略，以提高任務執行效率。
- 環境變數管理 — 透過 .env 文件管理 OpenAI API 金鑰和其他配置。
...
> [!tip] 爆紅原因
> 建立 30 天內累積 614 stars（20/天），forks 47（7.7%），顯示出一定的關注度。作者 jlia0 之前在開源領域有一定的貢獻，這個專案解決了多任務協調的痛點，特別是在需要遞迴處理的情境下，提供了一個新的解決方案。社群的反饋和需求也促進了這個專案的發展，特別是對於任務管理的需求越來越高。技術上，隨著 OpenAI API 的普及，這樣的工具變得更加可行，能夠利用強大的 LLM 進行任務的自動化處理。forks/stars 比率在 7.7% 屬於中等，顯示出有一定數量的用戶在實際修改和使用這個工具。

## 適合誰使用

**目標受眾**：需要自動化任務管理和協調的開發團隊，特別是面對複雜任務結構的團隊。

> [!example] 使用場景
> - 專案經理用它來自動化任務分配，因為可以快速將高層次目標轉化為具體的執行步驟，節省了手動分配的時間。
> - 開發者用它來管理多個子任務的執行，因為它能夠自動處理任務間的依賴關係，避免了手動調整的麻煩。
> - 團隊協作中，使用者用它來追蹤任務進度，因為可以即時查看任務樹的狀態，提升了透明度和協作效率。

## 架構分析

Fractals 的架構採用前後端分離的設計，前端使用 Next.js 提供用戶界面，後端則使用 Hono 框架處理 API 請求。用戶輸入任務後，系統會通過 OpenAI API 將其分類和分解，生成任務樹，然後在 Claude CLI 中執行每個葉子任務。這樣的設計使得系統能夠靈活應對不同的任務需求，並且能夠隨時調整任務的執行策略。

選擇 Hono 作為後端框架是因為其輕量和高效，適合處理高頻的 API 請求。整體架構的擴展性良好，但在面對極大規模的任務時，可能會出現性能瓶頸，特別是在任務樹的生成和管理上。使用者在設置工作環境時需要注意 git 工作樹的管理，這可能會增加一定的複雜度。

## 技術深入分析

Fractals 的核心技術機制基於遞迴任務生成，通過 OpenAI 的 API 將高層次任務分類並分解為可執行的子任務。這一過程使用了 Hono 框架來處理 API 請求，並利用 TypeScript 進行靜態類型檢查，確保代碼的穩定性和可維護性。系統的效能特性依賴於任務的複雜度和數量，對於較簡單的任務，能夠快速生成和執行，但在面對大量複雜任務時，可能會出現延遲。設計上，選擇 Hono 作為後端框架是因為其輕量和高效，能夠快速響應用戶請求，這對於需要即時反饋的任務管理工具至關重要。技術風險方面，隨著任務數量的增加，系統可能會面臨性能瓶頸，特別是在生成和管理任務樹的過程中。此外，對於外部 API 的依賴程度較高，這可能會影響系統的穩定性。整合方面，Fractals 與主流的 CI/CD 工具相容性良好，能夠輕鬆集成到現有的開發流程中，並且支持多種開發環境的配置，降低了使用門檻。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝和使用指南，包含必要的範例和指令。安裝過程相對順暢，沒有明顯的坑。文件目前僅提供英文版本，對於非英語使用者可能會有一定的學習障礙。
...
> - 目前社群支持有限，文檔不夠完善。
> - 對於過於複雜的任務，性能可能下降。
> - 僅支援 OpenAI API，缺乏其他選擇。

> [!warning] 注意事項
> - 目前僅支援 OpenAI 的 API，無法使用其他 LLM。
> - 在高層次任務過於複雜時，可能會導致性能下降。
> - 目前的社群支持和文檔相對有限，可能需要用戶自行探索。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | AlpinDale/parsync 提供了任務並行處理的功能，但不具備 Fractals 的遞迴任務生成能力，適合簡單的任務分配場景。 |
| [CoderLuii/HolyClaude](https://github.com/CoderLuii/HolyClaude) | HolyClaude 主要專注於單一任務的執行，而 Fractals 則能夠自動生成任務樹，適合複雜的任務結構。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | AlpinDale/parsync 主要專注於任務的並行處理，而 Fractals 則提供了遞迴任務生成的能力，適合更複雜的任務結構。 | 如果你的任務結構較簡單，且不需要遞迴生成，AlpinDale/parsync 會是一個更輕量的選擇。 | medium，因為需要重新設計任務的結構和執行邏輯。 |
| [CoderLuii/HolyClaude](https://github.com/CoderLuii/HolyClaude) | HolyClaude 提供了單一任務的執行，而 Fractals 則能夠自動生成任務樹，這使得 Fractals 更適合處理複雜的任務。 | 如果你的任務是單一且不需要分解的，HolyClaude 會是更簡單的選擇。 | low，因為 HolyClaude 的使用方式較為簡單，轉移到 Fractals 需要重新設計任務的結構。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **fractals** | **parsync** | **HolyClaude** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | AlpinDale/parsync 主要專注於任務的並行處理，而 Fractals 則提供了遞迴任務生成的能力，適合更複雜的任務結構。 | HolyClaude 提供了單一任務的執行，而 Fractals 則能夠自動生成任務樹，這使得 Fractals 更適合處理複雜的任務。 |
> | 遷移成本 | - | medium，因為需要重新設計任務的結構和執行邏輯。 | low，因為 HolyClaude 的使用方式較為簡單，轉移到 Fractals 需要重新設計任務的結構。 |
> | 適用場景 | 主要場景 | 如果你的任務結構較簡單，且不需要遞迴生成，AlpinDale | 如果你的任務是單一且不需要分解的，HolyClaude 會是 |
...
> [!tip] 採用建議
> 適合小型專案或內部工具試用，但不建議用於生產環境的核心任務。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在任務樹生成過程中，複雜任務可能導致性能下降。
  - 解法：簡化任務結構，或分步執行任務以減少負擔。
- [MEDIUM] 使用者可能無法即時查看任務狀態，特別是在大量任務執行時。
  - 解法：定期手動查詢任務狀態，或設置更頻繁的狀態更新。
- [MEDIUM] 對於不熟悉 git 的使用者，工作樹的管理可能會造成困難。
  - 解法：提供詳細的 git 工作樹使用指南，或考慮簡化工作樹的管理流程。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 需要自動化任務管理的開發團隊 | 非常適合 | Fractals 能夠自動生成和執行任務樹，減少手動干預。 |
| 面對複雜任務結構的專案經理 | 適合 | 能夠有效管理任務間的依賴關係，提升工作效率。 |
| 小型團隊的內部工具開發 | 普通 | 雖然功能強大，但對於小型任務可能顯得過於複雜。 |
| 單一任務執行的開發者 | 不適合 | 對於不需要遞迴生成的簡單任務，Fractals 的功能可能過於繁瑣。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，10 小時整合，得到自動化的任務管理，值得嘗試。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：Fractals 本身不需要高權限，僅需存取 OpenAI API，且不會存取敏感資料。依賴的第三方庫需定期檢查安全性。
...
**社群活躍度**：社群活躍度中等，開放的 Issues 數量顯示出使用者的需求和疑慮。
