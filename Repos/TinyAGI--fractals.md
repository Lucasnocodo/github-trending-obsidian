...
aliases:
  - "fractals"
  - "TinyAGI/fractals"
  - "自動化任務分解與執行，讓多個代理協同工作。"
---

# fractals

**618** stars · **14** stars/天 · 建立 43 天前 · TypeScript · MIT
...
> **適合** 需要自動化任務管理並希望提高工作效率的開發團隊。
> **一句話重點** Fractals 不僅是一個任務管理工具，更是一個能夠自動化複雜任務的強大協調者。
...
> **結論** 花 5 小時學、10 小時整合，得到高效的任務管理工具，值得投入。

> [!abstract] 核心創新
> Fractals 提供了自動化的遞歸任務分解與執行，能夠在多代理環境中高效協調任務。

## 專案簡介

Fractals 是一個遞歸任務協調器，能將高層次任務分解為可執行的子任務，並在獨立的 git 工作樹中執行每個葉子任務。使用者只需輸入任務，系統會自動生成一棵自相似的可執行子任務樹，並通過 HTTP 請求與前端進行交互。核心流程分為兩個階段：第一階段是計劃，使用 LLM 對任務進行分類和分解；第二階段是執行，使用 Claude CLI 在 git 工作樹中批量執行葉子任務。這種設計使得任務執行過程中能夠實現高效的資源管理與狀態更新，並且支持多種批量執行策略，如深度優先和廣度優先。這個工具的賣點在於它能夠自動處理複雜的任務依賴關係，並且在執行過程中提供實時的狀態更新。使用者可以通過 API 進行任務的分解、工作空間的初始化和執行的啟動，這些功能都可以通過簡單的 HTTP 請求來實現。

**技術棧**：`TypeScript 5.9.3` · `Hono 4.12.5` · `OpenAI API 6.25.0`

## 重點功能

- 任務分解 — 支持將高層次任務分解為可執行的子任務，通過 API `/api/decompose` 進行。
- 批量執行 — 支持多種執行策略（如深度優先、廣度優先），可通過 API `/api/execute` 設定。
- 實時狀態更新 — 在執行過程中實時更新任務狀態，通過 API `/api/tree` 獲取當前樹狀結構。
- 工作空間管理 — 自動初始化 git 工作樹，通過 API `/api/workspace` 進行配置。
- 多代理支持 — 可使用 Claude 和 Codex CLI 作為執行者，支持不同的任務執行需求。
...
> [!tip] 爆紅原因
> 建立 43 天內累積 618 stars（14/天），forks 46（7.4%），顯示出穩定的增長。作者 jlia0 之前在開源社群有過其他貢獻，這可能為專案帶來了信任度。這個工具解決了在多代理環境中協調任務的痛點，之前的方案往往缺乏自動化和靈活性。隨著 AI 和自動化技術的普及，這種任務分解和協調的需求越來越高。forks/stars 比率為 7.4%，顯示出有相當比例的使用者在積極修改和使用這個專案。

## 適合誰使用

**目標受眾**：需要自動化任務管理並希望提高工作效率的開發團隊。

> [!example] 使用場景
> - 後端工程師用它來自動化複雜的任務分解，因為手動管理多個子任務容易出錯，這樣可以節省 30% 的時間。
> - DevOps 工程師用它來管理 CI/CD 流程中的多個任務，因為它能夠自動處理任務依賴，減少手動干預。
> - 產品經理用它來快速生成任務清單，因為它能夠將高層次的需求自動分解為具體的執行步驟，提升團隊的工作效率。

## 架構分析

Fractals 採用 Hono 作為後端框架，並使用 Next.js 作為前端。這樣的選擇使得整個系統能夠高效地處理 HTTP 請求，並且支持實時的狀態更新。資料流方面，使用者的任務輸入會經過 LLM 的分類和分解，然後生成一棵任務樹，接著在 git 工作樹中執行每個葉子任務。

選擇 Hono 是因為它輕量且高效，適合處理大量的請求，而 Next.js 則提供了良好的用戶界面和交互體驗。這種架構的代價是需要用戶手動確認計劃，無法完全自動化。擴展性方面，隨著任務複雜度的增加，可能會面臨性能瓶頸，特別是在大量葉子任務同時執行時。

## 技術深入分析

Fractals 的核心技術機制在於其遞歸任務分解能力，使用 LLM 進行任務的分類和分解，這使得它能夠自動生成一棵任務樹。效能方面，這個工具能夠處理中小型任務的自動化，但在面對大型任務時可能會遇到性能瓶頸，特別是在執行大量葉子任務時。選擇 TypeScript 和 Hono 作為技術棧的原因在於其良好的類型系統和高效的請求處理能力，這樣的選擇使得開發過程中能夠減少錯誤並提高開發效率。技術風險方面，隨著任務複雜度的增加，可能會出現性能問題，特別是在大量葉子任務同時執行時。整合方面，這個工具與現有的 CI/CD 流程相容性良好，能夠輕鬆地集成到現有的開發環境中，降低了使用門檻。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了詳細的安裝步驟和使用範例。安裝過程順暢，沒有明顯的坑。文件目前僅提供英文，缺乏多語言支持。
...
> [!danger] 缺點
> - 目前僅支持 OpenAI 的 API，限制了使用範圍。
> - 對於大型任務，可能會遇到執行時間過長的問題。
> - 需要用戶手動確認計劃，無法完全自動化。
...
| 工具 | 差異 |
| --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard 提供了類似的任務管理功能，但缺乏 Fractals 的自動化分解能力，適合需要手動管理的場景。 |
| [AgentSeal/codeburn](https://github.com/AgentSeal/codeburn) | Codeburn 專注於單一任務的執行，而 Fractals 能夠處理多層次的任務結構，適合複雜的任務管理。 |
| [AgriciDaniel/claude-obsidian](https://github.com/AgriciDaniel/claude-obsidian) | Claude-Obsidian 提供了任務執行的功能，但不支持 Fractals 的遞歸任務分解，適合簡單的任務執行需求。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard 提供了類似的任務管理功能，但缺乏 Fractals 的自動化分解能力。 | 如果需要手動管理任務，Boneyard 可能更合適。 | medium，因為需要重新設計任務分解邏輯。 |
| [AgentSeal/codeburn](https://github.com/AgentSeal/codeburn) | Codeburn 專注於單一任務的執行，而 Fractals 能夠處理多層次的任務結構。 | 如果只需執行簡單任務，Codeburn 可能更合適。 | low，因為任務結構較簡單。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **fractals** | **boneyard** | **codeburn** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | Boneyard 提供了類似的任務管理功能，但缺乏 Fractals 的自動化分解能力。 | Codeburn 專注於單一任務的執行，而 Fractals 能夠處理多層次的任務結構。 |
> | 遷移成本 | - | medium，因為需要重新設計任務分解邏輯。 | low，因為任務結構較簡單。 |
> | 適用場景 | 主要場景 | 如果需要手動管理任務，Boneyard 可能更合適。 | 如果只需執行簡單任務，Codeburn 可能更合適。 |
...
- **[HIGH]** 在大型任務執行時，可能會遇到性能瓶頸，導致執行時間過長。
  - 解法：考慮將任務拆分為更小的子任務。
- [MEDIUM] 用戶手動確認計劃可能導致延遲。
  - 解法：提前規劃任務以減少確認時間。
...
| 大型企業的任務管理系統 | 普通 | 可能面臨性能瓶頸，需要進一步優化。 |
| 需要快速生成任務清單的產品經理 | 非常適合 | 能夠快速將需求轉化為具體執行步驟。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學、10 小時整合，得到高效的任務管理工具，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：不需要高權限，僅需存取 OpenAI API 金鑰，依賴鏈信任度高。
