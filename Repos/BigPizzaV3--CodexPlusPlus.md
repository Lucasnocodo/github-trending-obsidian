...
use_case: "為 CodexApp 提供增強功能的外部啟動器，提升使用體驗。"
priority: high
ring: assess
...
star_history: "2026-05-12:1040,2026-05-12:1044,2026-05-13:1526,2026-05-13:1529,2026-05-14:1898,2026-05-15:2194,2026-05-16:2415,2026-05-17:2591,2026-05-18:2763,2026-05-19:2985,2026-05-20:3192,2026-05-21:3568,2026-05-22:3970,2026-05-23:4363"
tags:
  - github
  - "category/開發工具"
  - "lang/rust"
aliases:
  ...
  - "為 CodexApp 提供增強功能的外部啟動器，提升使用體驗。"
---

# CodexPlusPlus

**4.4k** stars · **273** stars/天 · 建立 16 天前 · Rust · 未標註授權
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Recent · **熱度** Hot (273 stars/day)
> **授權** 未標註授權 (風險較高) · **維護** Active (最後推送 0 天前) · **貢獻者** 5+ 人 · **參與度** Low
> **適合** 希望提升 Codex 使用體驗的開發者和團隊。
> **一句話重點** Codex++ 的設計理念在於提供一個靈活且安全的增強工具，讓用戶能夠在不改變原始應用的情況下，享受更好的使用體驗。
...
> **結論** 花 5 小時學習，3 小時整合，得到靈活的增強功能，值得一試。

> [!abstract] 核心創新
> Codex++ 提供了一個不修改原始安裝文件的增強解決方案，靈活性高且安全性強。

## 專案簡介

Codex++ 是一個專為 Codex App 設計的外部增強啟動器和管理工具，旨在不修改原始安裝文件的情況下，通過外部啟動和注入增強腳本來提升使用體驗。用戶可以從 GitHub Releases 下載安裝包，安裝後會有兩個入口：靜默啟動入口和管理工具，後者提供了豐富的功能來啟動、檢查、修復和更新 Codex。核心賣點在於其不改變 Codex 的原始結構，並支持多種增強模式，包括中轉注入和傳統增強模式，讓用戶能夠靈活使用各種插件和功能。技術上，Codex++ 使用 Rust 作為後端，並利用 Tauri 和 React 構建管理工具，這樣的選擇使其在性能和用戶體驗上達到平衡。與其他工具相比，如 AgentSeal/codeburn 和 0x0funky/agent-sprite-forge，Codex++ 提供了更為靈活的插件管理和增強功能，特別是在 API Key 登錄模式下的插件支持上。

實際使用中，Codex++ 能夠支持多種中轉配置，並在用戶切換供應商時保持會話的可見性，這在其他工具中並不常見。需要注意的是，Codex++ 的使用可能會受到 Codex 更新的影響，因為其注入腳本依賴於 Codex 的頁面結構。這個專案目前處於活躍開發中，適合希望提升 Codex 使用體驗的開發者和團隊。未來六個月內，預計將持續優化增強功能和修復已知問題。

**技術棧**：`Rust` · `Tauri` · `React`

## 重點功能

- 靜默啟動 — 使用 Rust 實現的靜默啟動器，不依賴額外運行時。
- Tauri + React 管理工具 — 提供深色/淺色主題切換，提升用戶體驗。
- 外部 CDP 注入 — 不修改 Codex 安裝文件，安全性高。
- 中轉注入模式 — 支持多個中轉配置，靈活切換 API 提供者。
- 插件入口解鎖 — 解鎖 Codex 的插件入口，提供更多功能。
- 用戶腳本管理 — 獨立管理用戶自定義腳本，靈活性強。
- 自動更新 — 通過 GitHub Release 自動檢查和更新。
- 多平台支持 — 提供 Windows 和 macOS 的安裝包，兼容性好。
...
{
  "前置條件": "已安裝 Codex++",
  "指令": "在管理工具中添加中轉配置，並啟用中轉注入模式。",
  "預期輸出": "Codex++ 成功注入增強功能，並顯示相應的插件入口。"
}
```
...
> [!tip] 爆紅原因
> 建立 16 天就累積 4363 stars（273/天），forks 275（6.3%），顯示出穩定的增長趨勢。作者 BigPizzaV3 和其他貢獻者在開源社群中活躍，過去有多個相關專案，這使得他們對 Codex 的增強需求有深入理解。Codex++ 解決了 Codex 原生插件無法正常使用的痛點，特別是在 API Key 登錄模式下，這是許多用戶面臨的問題。最近的社群討論和反饋也促進了這個專案的曝光率，讓更多用戶了解其增強功能。技術上，Codex++ 的設計使其能夠靈活應對 Codex 的更新，這在過去的工具中並不常見，顯示出其未來的可持續性和適應性。

## 適合誰使用

**目標受眾**：希望提升 Codex 使用體驗的開發者和團隊。

> [!example] 使用場景
> - 開發者用它來管理 Codex 的增強功能，因為可以靈活配置多種插件和自定義腳本，提升工作效率。
> - 團隊協作時，使用 Codex++ 來統一管理 API Key 和會話，因為這樣可以避免因為登錄問題而導致的插件無法使用。
> - 系統管理員用它來監控 Codex 的後端狀態，因為能夠快速檢查和修復問題，減少停機時間。

## 架構分析

Codex++ 採用 Tauri 作為前端框架，Rust 作為後端，這樣的選擇使得應用在性能和安全性上達到平衡。後端負責處理增強功能的注入和管理，而前端則提供用戶友好的界面。資料流方面，Codex++ 在啟動時會檢查用戶的配置和會話狀態，並根據用戶選擇的增強模式進行相應的操作。

這樣的設計使得用戶能夠靈活切換不同的增強模式，並在需要時進行配置。選擇 Rust 作為後端語言的好處在於其高效的性能和安全性，然而這也帶來了較高的學習曲線。整體架構的擴展性良好，未來可以根據用戶需求增加更多的增強功能。

## 技術深入分析

Codex++ 的核心技術機制是通過外部啟動器和增強腳本的注入來提升 Codex 的功能。使用 Rust 作為後端語言，這使得其在性能和安全性上具有優勢，特別是在處理多線程和異步操作時。資料流方面，Codex++ 在啟動時會檢查用戶的配置，並根據選擇的增強模式進行相應的操作，這樣的設計使得用戶能夠靈活切換不同的增強模式。選擇 Tauri 作為前端框架的原因在於其能夠提供原生應用的性能，同時保持輕量級。這樣的選擇雖然提升了開發效率，但也可能帶來一定的依賴性風險。技術風險方面，Codex++ 依賴於 Codex 的頁面結構，若未來 Codex 進行重大更新，可能會影響增強功能的正常運作。整合方面，Codex++ 能夠與現有的開發工具鏈良好整合，特別是在 CI/CD pipeline 中，能夠輕鬆集成到現有的開發流程中。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝步驟和功能介紹，並附有範例配置。安裝過程相對順暢，但對於新手來說，手動配置中轉注入可能會有些困難。整體來說，花 30 分鐘應該能夠順利跑起來。
...
> [!danger] 缺點
> - 需要手動配置中轉注入，對新手不太友好。
> - Codex 更新可能影響增強功能的正常運作。
> - 某些插件可能不兼容最新版本的 Codex。
...
| 工具 | 差異 |
| --- | --- |
| [AgentSeal/codeburn](https://github.com/AgentSeal/codeburn) | 提供類似的增強功能，但不支持靜默啟動和多平台安裝。 |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於特定插件的增強，功能較為單一，缺乏靈活性。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [AgentSeal/codeburn](https://github.com/AgentSeal/codeburn) | 專注於特定插件的增強，功能較為單一，缺乏靈活性。 | 如果你的需求非常專一，且不需要多種增強功能，則可以考慮這個工具。 | low，因為功能相對簡單，轉移成本低。 |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供類似的增強功能，但不支持靜默啟動和多平台安裝。 | 如果你的團隊已經在使用這個工具，且不需要 Codex++ 的靈活性，可以考慮繼續使用。 | medium，因為需要重新配置和適應新的增強模式。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **CodexPlusPlus** | **codeburn** | **agent-sprite-forge** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於特定插件的增強，功能較為單一，缺乏靈活性。 | 提供類似的增強功能，但不支持靜默啟動和多平台安裝。 |
> | 遷移成本 | - | low，因為功能相對簡單，轉移成本低。 | medium，因為需要重新配置和適應新的增強模式。 |
> | 適用場景 | 主要場景 | 如果你的需求非常專一，且不需要多種增強功能，則可以考慮這個工 | 如果你的團隊已經在使用這個工具，且不需要 Codex++ 的 |
...
- **[HIGH]** Codex++ 菜單未出現，可能是從原版 Codex 啟動的
  - 解法：確保從 Codex++ 入口啟動
- [MEDIUM] 插件無法正常使用，可能是因為 API Key 配置錯誤
  - 解法：檢查中轉注入配置是否正確
- **[HIGH]** macOS 提示無法打開，可能是未簽名的安裝包
  - 解法：在系統設置中允許打開未簽名的應用
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型開發團隊使用 Codex 進行 API 開發 | 非常適合 | 提供靈活的插件管理和增強功能，提升開發效率。 |
| 個人開發者希望提升 Codex 使用體驗 | 適合 | 能夠靈活配置多種增強功能，滿足個性化需求。 |
| 大型企業需要穩定的 API 接入服務 | 普通 | 雖然功能強大，但可能不夠穩定，適合小範圍內部使用。 |
| 對 Codex 的增強需求不高的用戶 | 不適合 | 對於基本功能的使用，Codex 本身已經足夠。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，3 小時整合，得到靈活的增強功能，值得一試。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：Codex++ 本身不需要高權限，且不存取敏感資料。依賴的庫和框架經過審計，安全性較高。
...
> | 貢獻者 | Commits |
> | --- | --- |
> | [@BigPizzaV3](https://github.com/BigPizzaV3) | 167 |
...
> **活躍天數** 2 天 · **最新 commit** Update discussion group QR code
...
> | [#257](https://github.com/BigPizzaV3/CodexPlusPlus/issues/257) | DEEPSEEK v4 PRO  无法使用CODEX插件的原因。希望作者优化一下 | 0 | 0 |
> | [#256](https://github.com/BigPizzaV3/CodexPlusPlus/issues/256) | 悬停没有删除 | 0 | 0 |
> | [#255](https://github.com/BigPizzaV3/CodexPlusPlus/issues/255) | /goal模式在纯api和api与和官方账号混合登录的情况下均无法使用 | 0 | 0 |
...
相關專案：[[AgentSeal--codeburn|AgentSeal/codeburn]] · [[0x0funky--agent-sprite-forge|0x0funky/agent-sprite-forge]] · [[0xGF--boneyard|0xGF/boneyard]] · [[432539--gpt2api|432539/gpt2api]] · [[AgriciDaniel--claude-obsidian|AgriciDaniel/claude-obsidian]] · [[GitFrog1111--badclaude|GitFrog1111/badclaude]] · [[Gitlawb--openclaude|Gitlawb/openclaude]] · [[HKUDS--CLI-Anything|HKUDS/CLI-Anything]]
