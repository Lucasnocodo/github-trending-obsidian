...
use_case: "讓 CodexApp 更好用的增強工具，提供靜默啟動和管理功能。"
priority: high
ring: assess
...
star_history: "2026-05-12:1040,2026-05-12:1044,2026-05-13:1526,2026-05-13:1529,2026-05-14:1898,2026-05-15:2194,2026-05-16:2415,2026-05-17:2591,2026-05-18:2763,2026-05-19:2985,2026-05-20:3192,2026-05-21:3568,2026-05-22:3970"
tags:
  - github
  - "category/開發工具"
  - "lang/rust"
aliases:
  ...
  - "讓 CodexApp 更好用的增強工具，提供靜默啟動和管理功能。"
---

# CodexPlusPlus

**4.0k** stars · **265** stars/天 · 建立 15 天前 · Rust · 未標註授權
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Recent · **熱度** Hot (265 stars/day)
> **授權** 未標註授權 (風險較高) · **維護** Active (最後推送 0 天前) · **貢獻者** 5+ 人 · **參與度** Low
> **適合** 需要在 Codex 上進行擴展和自定義的開發者。
> **一句話重點** Codex++ 的設計讓用戶能夠在不改變 Codex 的情況下，靈活擴展其功能。
...
> [!abstract] 核心創新
> Codex++ 透過外部啟動器和增強腳本注入，無需修改 Codex 原始文件。

## 專案簡介

Codex++ 是一個專為 Codex App 設計的外部增強工具，透過外部啟動器來啟動 Codex，並使用 Chromium DevTools Protocol 注入增強腳本。用戶可以下載適用於 Windows 或 macOS 的安裝包，安裝後可選擇靜默啟動或使用管理工具進行配置。這個工具的賣點在於它不會修改 Codex 的原始安裝文件，從而避免了更新時的兼容性問題。Codex++ 的後端使用 Rust 實現，並搭配 Tauri 和 React 提供管理介面，這樣的選擇使得應用程式能夠輕量運行且不依賴額外的運行時。相較於 Codex 的原生功能，Codex++ 提供了更靈活的插件管理、會話刪除和 Markdown 導出等功能，並支持多個 API 的中轉注入。

這使得用戶能夠在不影響原有功能的情況下，靈活切換不同的模型提供者。使用者在操作過程中可能會遇到一些問題，例如插件無法正常顯示後端連接狀態，這時可以透過管理工具檢查日誌來排查問題。這個專案目前處於穩定階段，適合需要在 Codex 上進行擴展的開發者使用。未來可能會持續增強對新模型的支持，並改善用戶體驗。
...
- 增強功能管理 — 透過 Tauri + React 管理工具，支持深色/淺色主題切換。
- 中轉注入模式 — 支持多個 API 配置，靈活切換模型提供者。
- 插件入口解鎖 — 允許用戶在 Codex 中使用更多插件功能。
- 會話管理 — 提供會話刪除和 Markdown 導出功能，方便用戶管理資料。
...
> [!tip] 爆紅原因
> 建立 15 天內累積 3970 stars（265/天），forks 249（6.3%），這顯示出穩定的增長潛力。作者 BigPizzaV3 及其團隊在開源社群中活躍，致力於提升 Codex 的使用體驗。Codex++ 解決了 Codex 原生插件功能受限的問題，讓用戶能夠更靈活地管理 API 和插件。近期的推廣活動和社群反饋也促進了這個專案的曝光率。隨著對 AI 編程工具需求的增加，Codex++ 的出現正好填補了這一市場空白。

## 適合誰使用

**目標受眾**：需要在 Codex 上進行擴展和自定義的開發者。

> [!example] 使用場景
> - 後端開發者用它來靜默啟動 Codex，因為這樣可以避免手動操作，提高工作效率。
> - 團隊協作時，使用它來管理多個 API 配置，因為能夠快速切換不同的模型提供者，適應不同的開發需求。
> - 新手開發者用它來學習如何使用 Codex，因為提供了更友好的管理介面和增強功能，降低了學習曲線。

## 架構分析

Codex++ 採用外部啟動器的架構設計，這樣的選擇使得它能夠在不改變 Codex 原始安裝的情況下進行增強。資料流方面，Codex++ 透過 Chromium DevTools Protocol 進行增強腳本的注入，這樣的設計讓它能夠靈活應對 Codex 的更新。選擇 Rust 作為後端語言，提供了高效能和安全性，但也增加了學習曲線。未來的擴展性方面，若 Codex 的結構發生重大變化，可能需要對增強腳本進行調整。

## 技術深入分析

Codex++ 的核心技術機制是透過外部啟動器來啟動 Codex，並使用 Chromium DevTools Protocol 進行增強腳本的注入。這樣的設計使得它能夠在不修改 Codex 原始安裝的情況下，提供額外的功能和靈活性。效能方面，由於後端使用 Rust，Codex++ 在啟動和執行過程中具備高效能和低資源佔用的特性。設計選擇上，Rust 提供了安全性和性能，但也增加了開發的複雜度。技術風險方面，若 Codex 的結構發生變化，可能會影響增強腳本的有效性。整合方面，Codex++ 與現有的開發工具鏈（如 VS Code）相容性良好，能夠輕鬆融入開發流程。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且包含安裝步驟，安裝過程順暢，無明顯坑。提供了良好的入門指導，並包含多語言支持，適合新手快速上手。

## 優缺點分析

> [!success] 優點
> - 靜默啟動功能，提升使用便利性。
> - 支持多種 API 配置，靈活應對不同需求。
> - 不修改原始 Codex 安裝，避免更新時的兼容性問題。

> [!danger] 缺點
> - 目前僅支援 Windows 和 macOS，Linux 用戶無法使用。
> - 增強功能可能受到 Codex 更新的影響，需要定期維護。
> - 安裝過程中可能遇到 macOS 的安全性限制。

> [!warning] 注意事項
> - 目前僅支援 Windows 和 macOS 平台。
> - 在某些情況下，macOS 可能會因未簽名的安裝包而無法啟動。
> - 增強功能可能會因 Codex 的更新而需要調整注入腳本。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 這個工具專注於 AI agent 的創建，而 Codex++ 更加專注於增強 Codex 的使用體驗。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard 提供了 AI 模型的管理工具，但不具備 Codex++ 的靈活插件管理功能。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於 AI agent 的創建，提供了不同的功能和框架支持。 | 如果你的需求是專注於 AI agent 的開發，而不僅僅是增強 Codex 的功能。 | medium，因為需要重新適應不同的框架和功能。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供了 AI 模型的管理工具，但不具備 Codex++ 的靈活插件管理功能。 | 如果你需要一個專注於 AI 模型的管理工具，而不是增強現有應用。 | low，因為功能相似且易於上手。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **CodexPlusPlus** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於 AI agent 的創建，提供了不同的功能和框架支持。 | 提供了 AI 模型的管理工具，但不具備 Codex++ 的靈活插件管理功能。 |
> | 遷移成本 | - | medium，因為需要重新適應不同的框架和功能。 | low，因為功能相似且易於上手。 |
> | 適用場景 | 主要場景 | 如果你的需求是專注於 AI agent 的開發，而不僅僅是增 | 如果你需要一個專注於 AI 模型的管理工具，而不是增強現有應 |
...
> [!tip] 採用建議
> 適合開發者在非生產環境中試用，未來可能會進一步增強功能。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- [MEDIUM] 在某些情況下，Codex++ 菜單未顯示
  - 解法：確認從 Codex++ 入口啟動，而非原版 Codex。
- **[HIGH]** macOS 提示無法打開或已損壞
  - 解法：在系統設置中允許打開未簽名的應用。
- [MEDIUM] 插件無法正常顯示後端連接狀態
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型團隊的開發環境 | 非常適合 | 能夠靈活管理多個 API 配置，適應不同的開發需求。 |
| 個人開發者的 Codex 使用 | 適合 | 提供了增強功能和友好的管理介面，降低了使用門檻。 |
| 大型企業的生產環境 | 不適合 | 目前仍處於 beta 階段，穩定性和兼容性尚需提升。  |
...
> [!warning] 安全性快速掃描
> 低風險：Codex++ 不需要高權限，且不存取敏感資料，依賴鏈相對簡單，適合在 CI/CD 中使用。
...
> | 貢獻者 | Commits |
> | --- | --- |
> | [@BigPizzaV3](https://github.com/BigPizzaV3) | 157 |
...
> **活躍天數** 3 天 · **最新 commit** Merge pull request #224 from leeween/feat/codex-extra-args
...
> | [#234](https://github.com/BigPizzaV3/CodexPlusPlus/issues/234) | 接第三方api报错 | 0 | 0 |
> | [#232](https://github.com/BigPizzaV3/CodexPlusPlus/issues/232) | 使用问题 | 0 | 0 |
> | [#231](https://github.com/BigPizzaV3/CodexPlusPlus/issues/231) | 使用 @ai-sdk/openai-compatible的 opencode-go怎么配置 | 0 | 0 |
> | [#229](https://github.com/BigPizzaV3/CodexPlusPlus/issues/229) | 用了Codex+后，后面如果不用了，能否有一键恢复环境功能 | 0 | 1 |
