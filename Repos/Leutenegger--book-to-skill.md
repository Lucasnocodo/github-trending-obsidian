---
repo: Leutenegger/book-to-skill
...
```dataviewjs
const me = dv.page("Repos/Leutenegger--book-to-skill");
...
`agent` `agent-memory` `agent-skill` `agent-skills` `agentic` `agentic-ai` `agents` `ai-agents` `claude` `claude-code` `claude-code-plugin` `claude-skills` `github-copilot` `pdf` `pdf-document` `pdf-generation` `pdf-parser` `pdf-tools` `pdf-viewer` `skill-generator`
...
| 建立日期 | 2026-08-13 |
| Repo 大小 | 2.7 MB |
| OpenSSF Scorecard | [查看](https://scorecard.dev/viewer/?uri=github.com/Leutenegger/book-to-skill) |
...
> [!abstract] 最近 10 次 commit（2026-08-14）
> **活躍天數** 1 天 · **最新 commit** cleaned: removed comments and docstrings
...
[GitHub](https://github.com/Leutenegger/book-to-skill)
...
> FROM "Repos"
> WHERE category = "Other" AND file.name != "Leutenegger--book-to-skill"
> SORT stars DESC
> LIMIT 8
> ```
...
> FROM "Repos"
> WHERE language = "Python" AND file.name != "Leutenegger--book-to-skill" AND status != "archived"
...
> FROM "Repos"
> WHERE week = "2026-W33" AND file.name != "Leutenegger--book-to-skill"
> SORT stars DESC
> ```
...
>     .where(p => p.file.name !== "Leutenegger--book-to-skill" && p.category === me.category && (ringOrder[p.ring] || 0) > myRing)
...
> FROM "Repos"
> WHERE owner = "Leutenegger" AND file.name != "Leutenegger--book-to-skill"
> SORT stars DESC
> ```
...
- [[2026-08-14|2026-08-14]] — 首次收錄，1.0k stars
