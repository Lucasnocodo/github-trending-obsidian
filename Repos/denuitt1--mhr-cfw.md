...
use_case: "透過 GAS 和 Cloudflare Workers 轉發流量，設計用來繞過深度封包檢測（DPI）。"
priority: high
ring: assess
...
star_history: "2026-05-02:1316,2026-05-02:1319,2026-05-03:1701,2026-05-03:1703,2026-05-04:1961,2026-05-04:1961,2026-05-05:2166,2026-05-06:2333,2026-05-07:2468,2026-05-08:2584"
tags:
  - github
  - "category/安全"
  - "lang/python"
...
aliases:
  - "mhr-cfw"
  - "denuitt1/mhr-cfw"
  - "透過 GAS 和 Cloudflare Workers 轉發流量，設計用來繞過深度封包檢測（DPI）。"
---

# mhr-cfw

**2.6k** stars · **258** stars/天 · 建立 10 天前 · Python · MIT
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Recent · **熱度** Hot (258 stars/day)
> **授權** MIT (商業友好) · **維護** Active (最後推送 0 天前) · **貢獻者** 5+ 人 · **參與度** Low
> **適合** 需要在受限網路環境中安全訪問內容的開發者和研究人員。
> **一句話重點** 這個專案展示了如何利用現有的雲端服務來實現強大的隱私保護功能。
...
> **結論** 花 3 小時學習，2 小時整合，能夠有效繞過網路過濾，值得一試。

> [!abstract] 核心創新
> 利用 Google Apps Script 和 Cloudflare Workers 的組合，提供了一個創新的域名前置代理解決方案。

## 專案簡介

MHR-CFW 是一個域名前置代理工具，透過 Google Apps Script (GAS) 和 Cloudflare Workers 來隱藏實際流量。用戶的瀏覽器流量首先發送到本地代理，然後經過 Google 的基礎設施，最終到達 Cloudflare Worker，這樣網路只會看到正常的 Google 流量。這種設計使得流量不易被 DPI 檢測，因為它偽裝成合法的 Google 請求。使用者只需運行 `run.bat` 或 `run.sh` 啟動代理，並配置 Cloudflare Worker 和 Google Apps Script。這個工具的賣點在於它能夠在不暴露實際目的地的情況下，安全地繞過網路過濾。技術上，它利用了 Google 的基礎設施來隱藏流量，並且可以選擇性地使用穩定的上游轉發器來解決 CAPTCHA 問題，這在許多網站上是必要的。

這個專案的依賴相對輕量，主要依賴 Python 標準庫和一些可選的第三方庫，如 `cryptography` 和 `h2`，以支持 MITM 攔截和 HTTP/2 多路復用。與其他類似工具相比，如 `0x0funky/agent-sprite-forge` 和 `Flowseal/tg-ws-proxy`，MHR-CFW 提供了更強的隱私保護，因為它能夠隱藏實際的流量來源。使用者在使用過程中可能會遇到一些常見問題，如 SOCKS 不在 LAN 共享上工作或 502 錯誤，這些問題在 GitHub 上有相應的討論。這個專案目前處於 beta 階段，適合需要繞過網路過濾的開發者和研究人員。未來可能會增加更多的功能以增強穩定性和兼容性。

**技術棧**：`Python 3.10+` · `Google Apps Script` · `Cloudflare Workers`

## 重點功能

- 域名前置代理 — 透過 Google 基礎設施隱藏真實流量來源。
- Cloudflare Worker 支援 — 可自定義 Worker 以處理請求和響應。
- 穩定的上游轉發器 — 解決 CAPTCHA 問題，保持 IP 穩定。
- 多種部署選擇 — 支援本地和雲端部署，靈活適應不同需求。
- 簡單的安裝流程 — 只需幾個指令即可完成設置。
...
```bash
在 Cloudflare Dashboard 中創建 Worker，並將 worker.js 的內容複製到編輯器中。
```
3. 設置 Google Relay
```bash
在 Google Apps Script 中創建新專案，並將 Code.gs 的內容複製到編輯器中。
```
4. 運行代理
```bash
雙擊 run.bat (Windows) 或 run.sh (Linux) 啟動代理。
```
5. 測試連接
```bash
在瀏覽器中打開 ipleak.net 以檢查 IP 地址。
```
...
> [!tip] 爆紅原因
> 建立 10 天內累積 2584 stars（258/天），forks 246（9.5%），顯示出強烈的社群參與。作者 denuitt1 及其團隊在網路隱私和安全領域有豐富的經驗，這個專案解決了許多使用者在網路過濾和 CAPTCHA 驗證方面的痛點。這些問題在現有的工具中往往無法有效解決，MHR-CFW 提供了一個創新的解決方案。社群的活躍度和開發者的回應速度也顯示出這個專案的潛力。

## 適合誰使用

**目標受眾**：需要在受限網路環境中安全訪問內容的開發者和研究人員。

> [!example] 使用場景
> - 網路安全研究員用它來測試不同網站的流量過濾機制，因為它能夠隱藏真實流量來源，並模擬正常的 Google 流量。
> - 開發者用它來繞過公司網路的 DPI 檢測，因為這樣可以安全地訪問被封鎖的資源，而不會引起注意。
> - 獨立用戶用它來訪問受限內容，因為它能夠有效地隱藏實際請求，避免被網路監控系統檢測。

## 架構分析

MHR-CFW 的架構設計為一個本地代理，將流量轉發至 Google 的基礎設施，然後通過 Cloudflare Workers 進行處理。這樣的設計使得流量在網路上看起來像是來自 Google，從而避免了 DPI 檢測。每個請求都經過 GAS，這樣可以在不暴露實際目的地的情況下，安全地轉發流量。

選擇使用 GAS 和 Cloudflare Workers 是因為它們提供了穩定的基礎設施和強大的功能，但這也意味著依賴於這些服務的可用性和穩定性。這種設計的 trade-off 在於，使用者需要配置多個服務，增加了初始設置的複雜性，但最終能夠獲得更強的隱私保護。擴展性方面，這個架構可以輕鬆地添加更多的功能，如穩定的上游轉發器，以解決特定網站的 CAPTCHA 問題。

## 技術深入分析

MHR-CFW 的核心技術在於其域名前置代理的實現，利用 Google Apps Script 和 Cloudflare Workers 的組合來隱藏流量。這種設計使得流量在傳輸過程中不易被檢測，因為它看起來像是來自 Google 的合法請求。效能方面，這個工具能夠處理大量的請求，但具體的性能數據在 README 中並未提供。選擇 Python 作為主要開發語言使得這個專案的依賴相對輕量，並且能夠在多種平台上運行。這個選擇的好處在於 Python 的廣泛使用和豐富的庫支持，但也可能帶來性能上的瓶頸，特別是在高並發的情況下。技術風險方面，依賴於 Google 和 Cloudflare 的服務可能會導致可用性問題，特別是在這些服務出現故障時。整合方面，這個工具可以輕鬆地與現有的開發環境相容，並且支持與多種代理工具的整合，降低了使用門檻。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了詳細的安裝步驟和使用說明，對新手非常友好。安裝過程相對順暢，沒有明顯的坑。文件中包含了多語言支持，增加了可讀性。

## 優缺點分析

> [!success] 優點
> - 能有效繞過 DPI 檢測，保護用戶隱私。
> - 支持 Cloudflare Workers 的自定義配置，靈活性高。
> - 安裝和使用相對簡單，適合各種用戶。

> [!danger] 缺點
> - 需要穩定的 VPS 來運行上游轉發器，增加成本。
> - 依賴 Google 和 Cloudflare 的服務，可能面臨可用性問題。
> - 在某些網路環境中可能無法正常工作，限制了使用場景。

> [!warning] 注意事項
> - 僅支援 Python 3.10+
> - 需要穩定的 VPS 來運行上游轉發器（可選）
> - 在某些網路環境中可能無法正常工作
> - 使用 Google Apps Script 需要遵循 Google 的服務條款

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供類似的隱私保護功能，但不支援 Cloudflare Workers 的自定義配置。 |
| [Flowseal/tg-ws-proxy](https://github.com/Flowseal/tg-ws-proxy) | 專注於 Telegram 的代理功能，無法提供通用的域名前置代理解決方案。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 使用 Rust 實作，記憶體用量少，但功能上不如 MHR-CFW 靈活。 | 如果需要更高效的性能和更低的資源消耗，且不需要 Cloudflare Workers 的支持。 | medium，因為需要重新配置和學習新的工具。 |
| [Flowseal/tg-ws-proxy](https://github.com/Flowseal/tg-ws-proxy) | 專注於 Telegram 的代理功能，無法提供通用的域名前置代理解決方案。 | 如果主要需求是 Telegram 的代理功能，而不需要通用的隱私保護。 | low，因為功能較為單一，遷移成本低。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **mhr-cfw** | **agent-sprite-forge** | **tg-ws-proxy** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 使用 Rust 實作，記憶體用量少，但功能上不如 MHR-CFW 靈活。 | 專注於 Telegram 的代理功能，無法提供通用的域名前置代理解決方案。 |
> | 遷移成本 | - | medium，因為需要重新配置和學習新的工具。 | low，因為功能較為單一，遷移成本低。 |
> | 適用場景 | 主要場景 | 如果需要更高效的性能和更低的資源消耗，且不需要 Cloudf | 如果主要需求是 Telegram 的代理功能，而不需要通用的 |
...
- **[HIGH]** 在某些網路環境中可能無法正常工作，特別是企業網路。
  - 解法：嘗試在不同的網路環境下測試，或使用 VPN。
- [MEDIUM] 502 錯誤可能會發生在配置不當時。
  - 解法：檢查 Cloudflare Worker 和 Google Apps Script 的配置是否正確。
- [MEDIUM] SOCKS 不在 LAN 共享上工作。
  - 解法：檢查防火牆設置，確保端口開放。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 需要在受限網路環境中安全訪問內容的開發者 | 非常適合 | 提供強大的隱私保護功能，能有效繞過網路過濾。 |
| 小型團隊需要測試網路過濾機制 | 適合 | 簡單的設置流程和靈活的配置選項。 |
| 大型企業網路環境下的個人使用者 | 不適合 | 可能會受到企業網路政策的限制，導致無法正常工作。  |
...
> [!tip] 投入 vs 回報
> 花 3 小時學習，2 小時整合，能夠有效繞過網路過濾，值得一試。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：本工具不需要高權限運行，但使用 Google Apps Script 可能需要遵循其安全政策。依賴於 Google 和 Cloudflare 的服務，需注意其隱私政策和使用條款。

## 生態系整合

> [!abstract] 如何融入你的工具鏈

MHR-CFW 最常與 Google Apps Script 和 Cloudflare Workers 搭配使用，處於流量轉發的環節。在使用過程中，用戶需配置 Cloudflare Worker 和 GAS，具體做法是將專案中的 `worker.js` 和 `Code.gs` 複製到相應的編輯器中。這個工具與主流的 CI/CD 工具（如 GitHub Actions）相容，能夠輕鬆集成到現有的開發流程中。整合的摩擦點主要在於配置過程，特別是對於不熟悉 GAS 或 Cloudflare 的用戶，可能需要一些時間來適應。
...
> | 貢獻者 | Commits |
> | --- | --- |
> | [@denuitt1](https://github.com/denuitt1) | 48 |
...
> **活躍天數** 2 天 · **最新 commit** Update CA Certificate name
...
> | [#132](https://github.com/denuitt1/mhr-cfw/issues/132) | چطور اینو به گوشی بدم ؟ نمیشه ! کمک | 2 | 1 |
...
> - **User responsibility:** Running this project outside controlled test environments may affect 

## 延伸閱讀

相關概念：[[隱私保護]] · [[深度封包檢測]] · [[代理伺服器]]
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["隱私保護","深度封包檢測","代理伺服器"];
