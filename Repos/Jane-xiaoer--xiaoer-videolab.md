...
use_case: "一鍵將當前頁面視頻抓取到本地，支持 1800+ 個網站。"
priority: high
ring: assess
...
star_history: "2026-06-08:467,2026-06-08:467,2026-06-09:482,2026-06-09:482,2026-06-10:493,2026-06-10:493,2026-06-11:504,2026-06-12:511,2026-06-13:513,2026-06-14:516,2026-06-15:518,2026-06-16:520,2026-06-17:522,2026-06-18:523,2026-06-19:523,2026-06-20:525,2026-06-21:525,2026-06-22:525,2026-06-23:527,2026-06-24:529,2026-06-25:531,2026-06-26:530,2026-06-27:543,2026-06-28:546"
tags:
  - github
  - "category/開發工具"
  - "lang/javascript"
...
aliases:
  ...
  - "一鍵將當前頁面視頻抓取到本地，支持 1800+ 個網站。"
---

# xiaoer-videolab

**546** stars · **24** stars/天 · 建立 23 天前 · JavaScript · MIT
...
> **授權** MIT (商業友好) · **維護** Moderate (最後推送 20 天前) · **貢獻者** 3 人 · **參與度** Medium
> **適合** 需要快速下載網頁視頻但又不想使用不安全擴展的普通用戶。
> **一句話重點** 這個工具的設計理念是將視頻下載過程完全本地化，從而提高安全性和用戶隱私。
...
>     .where(p => p.subcategory === "視頻下載器" && p.file.name !== "Jane-xiaoer--xiaoer-videolab" && p.status !== "archived")
...
>       p.ring || "assess"
>     ]));
>   } else { dv.paragraph("_目前 vault 中沒有其他 視頻下載器 類工具_"); }
> }
> ```

> [!question] TL;DR — 值得投入嗎？
> **成熟度** Beta (可試用) · **安裝** Medium (需設定) · **學習** ~2h · **綁定風險** low
> **結論** 花 2 小時學習，1 小時整合，得到一個安全且強大的視頻下載工具，值得使用。

> [!abstract] 核心創新
> 這個專案的創新點在於將視頻下載過程完全本地化，避免了使用不安全的網頁擴展。

## 專案簡介

Xiaoer VideoLab 是一個簡單的工具，讓用戶可以通過點擊瀏覽器工具欄按鈕，將當前頁面上的視頻下載到本地。它依賴於一個本地的 Python daemon，該 daemon 監聽來自瀏覽器擴展的請求，並使用 yt-dlp 來處理視頻下載。用戶只需安裝 yt-dlp 和 ffmpeg，然後安裝 VideoLab，便可在每次登錄時自動啟動下載服務。這種設計的好處在於，所有的下載過程都在本地進行，避免了使用不安全的網頁擴展。用戶可以通過設置環境變數來配置下載的視頻質量、文件名前綴等，這使得工具的靈活性大大提高。核心命令包括 `brew install yt-dlp ffmpeg` 和 `./scripts/install.sh`，這些命令簡化了安裝過程。這個工具的賣點在於其簡單易用，並且支持多達 1800 多個網站，包括 YouTube、Bilibili 和 TikTok 等，讓用戶能夠輕鬆抓取各類視頻。

**技術棧**：`Python` · `JavaScript` · `Shell`

## 重點功能

- 一鍵下載 — 只需點擊工具欄按鈕即可將當前頁視頻下載到本地。
- 支持 1800+ 網站 — 包括 YouTube、Bilibili、TikTok 等主流視頻平台。
- 本地處理 — 所有下載過程在本地進行，無需依賴外部伺服器。
- 自動更新 — yt-dlp 每週自動更新，確保支持最新的網站變化。
- 靈活配置 — 可通過環境變數設置下載質量、文件名前綴等。
...
> [!tip] 爆紅原因
> 建立 23 天內累積 546 stars（24 stars/天），forks 79（14.5%），顯示出一定的社群關注度。作者 Jane-xiaoer 在開源社群中有一定的知名度，之前也有其他相關項目。這個工具解決了用戶在下載視頻時的安全性問題，因為許多現有的瀏覽器擴展需要過多的權限，容易造成隱私洩露。這個工具的設計理念是將所有下載過程本地化，減少外部依賴。社群對於這個工具的需求也反映在其熱門問題上，像是安裝報錯的問題，顯示出用戶在實際使用過程中的痛點。

## 適合誰使用

**目標受眾**：需要快速下載網頁視頻但又不想使用不安全擴展的普通用戶。

> [!example] 使用場景
> - 學生用它來下載網上課程的視頻，因為這樣可以隨時離線觀看，節省流量。
> - 內容創作者用它來抓取靈感來源的視頻，因為可以快速獲取高質量的素材。
> - 普通用戶用它來保存喜愛的網絡視頻，因為它支持多達 1800 多個網站，方便快捷。

## 架構分析

Xiaoer VideoLab 採用本地 daemon 的架構，這樣設計的目的是為了提高安全性和隱私保護。該 daemon 使用 Python 標準庫的 http.server 來處理來自瀏覽器擴展的請求，並將請求轉發給 yt-dlp 進行下載。這種設計使得所有的下載過程都在用戶的本地機器上進行，避免了將數據發送到外部伺服器的風險。

選擇 Python 作為後端語言是因為其簡單易用且擁有豐富的庫支持。這種架構的代價是需要用戶安裝額外的依賴（如 yt-dlp 和 ffmpeg），但這也使得工具的功能更強大。擴展性方面，這種設計能夠輕鬆支持更多的視頻網站，只需在 yt-dlp 中添加相應的提取器即可。

## 技術深入分析

Xiaoer VideoLab 的核心技術機制是基於 Python 的 HTTP 伺服器，這使得它能夠輕鬆處理來自瀏覽器的請求。使用 yt-dlp 作為底層下載引擎，這個工具能夠支持多達 1800 多個視頻網站，並且每週自動更新以應對網站的變化。效能方面，這個工具的下載速度取決於網絡連接和 yt-dlp 的性能，通常能夠滿足大多數用戶的需求。設計上選擇 Python 是因為其簡單性和強大的庫支持，這使得開發和維護變得更加容易。這個工具的依賴樹相對簡單，主要依賴於 yt-dlp 和 ffmpeg，這些都是廣泛使用的開源工具。技術風險方面，未來可能會面臨網站變更導致的下載失敗，這需要持續的維護和更新。整合方面，這個工具與主流的瀏覽器（如 Chrome 和 Edge）兼容良好，但在某些情況下需要手動加載擴展，這可能對新手造成困擾。

## 新手體驗

> [!info] 上手難度評估
> README 文件詳細且清晰，包含安裝步驟和配置說明。安裝過程相對順暢，但需要用戶手動安裝依賴，對於新手來說可能有一定難度。文件目前僅提供英文，缺乏多語言支持。
...
> [!danger] 缺點
> - 對於某些網站的支持有限，無法下載 VIP 內容。
> - 需要用戶手動安裝依賴，對新手不太友好。
> - 在某些情況下，可能需要額外的配置來獲取視頻。
...
| 工具 | 差異 |
| --- | --- |
| [yt-dlp/yt-dlp](https://github.com/yt-dlp/yt-dlp) | yt-dlp 是底層的視頻下載器，提供更高的靈活性和配置選項，但需要用戶手動操作，而 Xiaoer VideoLab 提供了更簡單的用戶界面。 |
| [youtube-dl/youtube-dl](https://github.com/youtube-dl/youtube-dl) | youtube-dl 是 yt-dlp 的前身，功能相似，但更新頻率較低，且支持的網站數量少於 yt-dlp。 |
| [4kdownload/4k-video-downloader](https://github.com/4kdownload/4k-video-downloader) | 4K Video Downloader 是一個商業工具，提供 GUI 介面，但需要付費，而 Xiaoer VideoLab 是開源且免費的。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [yt-dlp/yt-dlp](https://github.com/yt-dlp/yt-dlp) | yt-dlp 是一個命令行工具，提供更高的靈活性和配置選項，但不提供圖形界面，使用上需要一定的技術門檻。 | 如果你需要更高的自定義和控制，並且不介意使用命令行工具，則可以選擇 yt-dlp。 | low，因為兩者都使用 yt-dlp 作為底層下載引擎，切換相對簡單。 |
| [youtube-dl/youtube-dl](https://github.com/youtube-dl/youtube-dl) | youtube-dl 是 yt-dlp 的前身，功能相似，但更新頻率較低，且支持的網站數量少於 yt-dlp。 | 如果你需要一個成熟的工具並且不介意較慢的更新頻率，可以選擇 youtube-dl。 | medium，因為需要適應不同的命令行參數和配置方式。 |
| [4kdownload/4k-video-downloader](https://github.com/4kdownload/4k-video-downloader) | 4K Video Downloader 是一個商業工具，提供 GUI 介面，但需要付費，而 Xiaoer VideoLab 是開源且免費的。 | 如果你需要一個簡單的 GUI 工具並且願意支付費用，可以選擇 4K Video Downloader。 | high，因為需要重新適應 GUI 操作和付費模式。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **xiaoer-videolab** | **yt-dlp** | **youtube-dl** | **4k-video-downloader** |
> | --- | --- | --- | --- | --- |
> | 技術路線 | 本專案 | yt-dlp 是一個命令行工具，提供更高的靈活性和配置選項，但不提供圖形界面，使用上需要一定的技術門檻。 | youtube-dl 是 yt-dlp 的前身，功能相似，但更新頻率較低，且支持的網站數量少於 yt-dlp。 | 4K Video Downloader 是一個商業工具，提供 GUI 介面，但需要付費，而 Xiaoer VideoLab 是開源且免費的。 |
> | 遷移成本 | - | low，因為兩者都使用 yt-dlp 作為底層下載引擎，切換相對簡單。 | medium，因為需要適應不同的命令行參數和配置方式。 | high，因為需要重新適應 GUI 操作和付費模式。 |
> | 適用場景 | 主要場景 | 如果你需要更高的自定義和控制，並且不介意使用命令行工具，則可 | 如果你需要一個成熟的工具並且不介意較慢的更新頻率，可以選擇  | 如果你需要一個簡單的 GUI 工具並且願意支付費用，可以選擇 |
...
- **[HIGH]** 在某些網站上無法獲取視頻，因為這些網站的視頻地址無法單獨訪問。
  - 解法：確保在視頻播放時點擊下載按鈕。
- [MEDIUM] 安裝過程中可能出現依賴錯誤，特別是對於新手用戶。
  - 解法：仔細檢查安裝步驟，確保所有依賴都已安裝。
- [MEDIUM] 在使用過程中，可能需要手動更新 yt-dlp。
  - 解法：定期運行 `bash scripts/auto-update-ytdlp.sh` 來保持更新。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型團隊的視頻內容創作 | 非常適合 | 支持多個視頻平台，能夠快速獲取素材。 |
| 個人用戶下載網絡視頻 | 適合 | 簡單易用，適合普通用戶。 |
| 需要高頻率更新的商業視頻下載 | 不適合 | 可能需要手動更新 yt-dlp，無法自動適應所有網站變更。  |
...
> [!tip] 投入 vs 回報
> 花 2 小時學習，1 小時整合，得到一個安全且強大的視頻下載工具，值得使用。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：工具本身不需要高權限，且所有下載過程在本地進行，不會存取敏感資料。依賴鏈的信任程度高，因為主要依賴於開源的 yt-dlp 和 ffmpeg。
...
> [!note]- 直接競品（同子分類：視頻下載器）
> ```dataview
> TABLE stars, stars_per_day AS "Stars/天", install_complexity AS "難度", use_case AS "用途"
> FROM "Repos"
> WHERE subcategory = "視頻下載器" AND file.name != "Jane-xiaoer--xiaoer-videolab"
> SORT stars DESC
> ```
