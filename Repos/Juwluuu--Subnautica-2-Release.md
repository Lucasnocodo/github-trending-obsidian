...
**98** stars · **4** stars/天 · 建立 22 天前 · C++ · MIT
...
> **適合** 喜歡探索和生存冒險的玩家，特別是尋求多人合作體驗的遊戲愛好者。
> **一句話重點** 這款遊戲不僅是生存冒險的延續，更是對多人合作模式的一次創新。
...
> **結論** 花 5 小時學習，10 小時整合，得到一個具有潛力的多人合作生存遊戲，值得一試。

> [!abstract] 核心創新
> 引入了 DNA BioMod 系統和模組化潛艇，增強了遊戲的深度和自定義選項。

## 專案簡介

Subnautica 2 是一款全新的外星海洋生存冒險遊戲，玩家可以探索新星球 Zazura，建造基地、製作裝備，並在危險生物中生存。遊戲的核心機制是透過探索和生存來解鎖故事，玩家將面對各種新生物和生態系統。安裝後，使用者需運行 `Subnautica2.exe` 並根據自己的硬體調整畫質設定。這款遊戲的賣點在於其豐富的探索內容和多人合作模式，最多可支持四人同時遊玩。技術上，遊戲使用 C++ 開發，並針對 Windows 10 和 11 進行了優化，建議使用高效能的電腦以獲得最佳體驗。

相較於前作，這一代在生存機制和視覺效果上都有顯著提升，並引入了 DNA BioMod 系統和模組化潛艇等新元素。與其他生存遊戲相比，Subnautica 2 的探索深度和故事情節更為豐富，玩家不僅要面對環境挑戰，還需解開隱藏在海洋深處的謎團。實際使用中，玩家可能會遇到啟動問題或性能瓶頸，這些問題通常可以通過調整設定或更新驅動程式來解決。這款遊戲目前處於早期訪問階段，未來可能會持續更新和擴展內容。對於喜愛生存冒險的玩家來說，這款遊戲值得一試。

**技術棧**：`C++`

## 重點功能

- 多人合作 — 支持最多四位玩家同時遊玩，增強互動性。
- 全新外星海洋 — 提供豐富的探索內容和新生物。
- 進階基地建設系統 — 玩家可以自由建造和設計自己的基地。
- DNA BioMod 系統 — 引入新的生物改造機制，增加遊戲深度。
- 模組化潛艇 — 玩家可以自定義潛艇以適應不同的探索需求。
- 改進的生存機制 — 提供更真實的生存挑戰和環境互動。
- 驚人的水下視覺效果 — 利用高效能圖形提升遊戲體驗。
- 深度故事與探索 — 隱藏在海洋深處的謎團等待玩家發掘。
...
> [!tip] 爆紅原因
> 建立 22 天內累積 98 stars（4.5/天），forks 0，顯示出相對較少的實際修改需求。作者 Juwluuu 之前在遊戲開發上有一定經驗，這款遊戲解決了多人合作生存遊戲中缺乏新鮮感的問題，提供了一個全新的探索環境和機制。雖然目前沒有明顯的觸發事件，但遊戲的早期訪問模式吸引了不少玩家的關注。技術上，C++ 的使用讓遊戲在性能上有更好的表現，這也是其受歡迎的原因之一。Stars 與 forks 的比率顯示出玩家對於這款遊戲的興趣，但目前仍在觀望階段。

## 適合誰使用

**目標受眾**：喜歡探索和生存冒險的玩家，特別是尋求多人合作體驗的遊戲愛好者。

> [!example] 使用場景
> - 獨立遊戲開發者用它來測試多人合作遊戲的機制，因為這款遊戲提供了完整的基礎架構和生存系統。
> - 遊戲愛好者用它來探索新遊戲環境，因為其豐富的故事和生態系統讓玩家能夠長時間投入。
> - 社群玩家用它來組建小隊進行合作冒險，因為支持四人同時遊玩的設計增強了互動性和樂趣。

## 架構分析

這款遊戲採用 C++ 開發，旨在提供高效能的遊戲體驗。其架構設計上，使用了現代的遊戲引擎技術來實現流暢的畫面和互動。遊戲的資料流主要集中在玩家的操作和環境變化之間，利用物理引擎來模擬水下環境的真實感。

選擇 C++ 而非其他語言的原因在於其性能優勢，能夠處理更複雜的圖形和計算需求。這樣的選擇雖然增加了開發難度，但也使得遊戲在性能上更具優勢。未來擴展性方面，若要加入更多的生物和生態系統，可能需要進一步優化現有架構以避免性能瓶頸。

## 技術深入分析

Subnautica 2 的核心技術機制在於其使用 C++ 開發，這使得遊戲在性能上具有優勢，能夠處理複雜的圖形和物理模擬。遊戲的資料流設計上，強調玩家與環境的互動，使用物理引擎來模擬水下環境的真實感。效能方面，遊戲建議使用高效能的電腦，這意味著在低效能設備上可能會遇到性能瓶頸。選擇 C++ 而非其他語言的原因在於其性能優勢，能夠更好地滿足遊戲的需求。技術風險方面，遊戲仍在早期訪問階段，未來可能會面臨穩定性和性能的挑戰。整合方面，遊戲的設計考慮到了現有的遊戲生態，與主流的遊戲引擎和工具鏈相容性良好，這使得開發和維護變得更加容易。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝步驟和故障排除建議，對新手友好。安裝過程相對順暢，無明顯的坑。文件中未提供多語言支持，主要以英文為主。

## 優缺點分析

> [!success] 優點
> - 豐富的探索內容，玩家可以在全新外星海洋中自由探索。
> - 多人合作模式增強了遊戲的互動性和樂趣。
> - 進階的基地建設系統讓玩家能夠自由發揮創意。
> - 引入新的生物改造機制，增加遊戲的深度。

> [!danger] 缺點
> - 目前僅支援 Windows 10 和 11，對於其他平台的支持尚未明確。
> - 處於早期訪問階段，可能存在未解決的問題。
> - 需要高效能的電腦，對於低效能設備的支持不足。
> - 遊戲的穩定性可能會受到影響，特別是在多人模式下。
...
- **[HIGH]** 在某些高效能顯示卡上可能會出現黑屏問題
  - 解法：切換到無邊框窗口模式
- [MEDIUM] 遊戲啟動時可能需要以管理員身份運行
  - 解法：右鍵點擊並選擇以管理員身份運行
- [MEDIUM] 性能下降可能與圖形設定有關
  - 解法：降低圖形設定或解析度
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型開發團隊開發新遊戲 | 適合 | 提供了完整的多人合作框架和生存機制，便於快速開發和測試。 |
| 大型遊戲工作室進行多人遊戲開發 | 普通 | 雖然有良好的基礎，但仍在早期訪問階段，穩定性可能不足。 |
| 獨立遊戲開發者尋求新遊戲靈感 | 非常適合 | 提供了創新的生存和探索機制，適合獨立開發者借鑒。 |
| 希望在多平台上運行的遊戲開發者 | 不適合 | 目前僅支援 Windows 10 和 11，對其他平台的支持尚未明確。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，10 小時整合，得到一個具有潛力的多人合作生存遊戲，值得一試。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：遊戲本身不需要高權限運行，主要存取本地檔案系統。依賴鏈的信任程度尚可，但仍需注意安全性問題，特別是在多人模式下。
