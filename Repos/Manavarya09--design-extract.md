...
> **一句話重點** 這個工具不僅提取設計元素，還能捕捉到網站的運動語言和佈局結構，對於設計和開發團隊來說是個強大的資源。
...
> **結論** 花 3 小時學習，2 小時整合，得到全面的設計提取功能，值得投資。

> [!abstract] 核心創新
> 設計提取工具首次支持運動語言和佈局系統的全面提取。

## 專案簡介

designlang 是一個強大的工具，可以透過無頭瀏覽器爬取網站，提取所有計算樣式並生成 11 種以上的輸出文件，包括 AI 優化的 Markdown、視覺 HTML 預覽、Tailwind 配置、React 主題等。這個工具的賣點在於它不僅提取顏色和字體，還能抓取佈局模式、運動語言、元件解剖學和品牌聲音，這些都是其他工具無法做到的。使用者只需執行 `npx designlang https://stripe.com` 即可獲得完整的設計系統，或使用 `--full` 標誌來獲取更詳細的輸出。這樣的設計使得設計師和開發者能夠快速獲取網站的設計元素，並將其應用於自己的專案中。技術上，designlang 依賴 Playwright 進行網頁爬取，並使用 Node.js 20+ 作為執行環境，這使得它在性能上有著良好的表現。

與其他設計提取工具相比，designlang 提供了更全面的功能，包括佈局系統的提取和實時網站同步，這使得它在設計更新和維護上更具優勢。使用者可以通過 `designlang sync` 指令來保持本地設計標記與網站的同步，這在快速變化的設計環境中尤為重要。這個工具的社群活躍度也相對較高，開發者經常更新和修復問題，顯示出良好的維護狀態。對於需要快速提取和應用網站設計的團隊，這是一個非常值得考慮的選擇。

**技術棧**：`Node.js 20+` · `Playwright` · `TypeScript` · `Dockerfile`

## 重點功能

- 完整設計提取 — 提取顏色、字體、間距、陰影、運動等設計元素，並生成多種格式的輸出。
- 佈局系統提取 — 自動識別網頁的佈局模式，包括網格和彈性容器，並生成相應的配置。
- 運動語言支持 — 提取動畫持續時間、緩動函數、滾動連結動畫等，並生成運動標記。
- 品牌聲音提取 — 分析網站的語調和 CTA 動詞，生成品牌聲音摘要。
- 實時網站同步 — 通過 `designlang sync` 指令保持本地設計標記與網站的同步，確保設計的一致性。
...
```js
{
  ...
  "預期輸出": "生成多個設計相關的輸出文件，包括設計標記、預覽和配置文件。"
}
```
...
> [!tip] 爆紅原因
> 建立 5 天內累積 1157 stars（231/天），forks 90（7.8%），顯示出相對穩定的增長。作者 Manavarya09 是一位活躍的開發者，專注於設計系統和工具的開發。這個工具解決了設計提取過程中的多個痛點，特別是能夠提取佈局模式和運動語言，這在市場上是獨一無二的。最近的版本更新引入了新的功能，進一步提升了工具的實用性。社群的討論和反饋也促進了工具的快速迭代和改進。
...
> [!example] 使用場景
> - 前端工程師用它來從競爭對手的網站提取設計系統，因為這樣可以快速了解市場趨勢，並在自己的專案中應用類似的設計元素。
> - 設計師用它來生成 Figma 變數，因為這樣可以快速將網站的設計轉換為可在 Figma 中使用的格式，提升工作效率。
> - 產品經理用它來進行設計評估，因為它提供的 WCAG 可及性評分可以幫助團隊確保產品符合無障礙標準。

## 架構分析

designlang 採用無頭瀏覽器技術（Playwright）來爬取網站，這樣可以直接從實時 DOM 提取樣式，這種設計使得提取的數據更準確且即時。資料流中，工具首先抓取網站的 HTML 結構，然後分析 CSS 樣式，最後生成多種格式的輸出文件。這種方法的優勢在於能夠捕捉到動態生成的樣式和佈局，而不是僅依賴靜態資源。

選擇 Playwright 而非其他爬蟲框架的原因在於其強大的跨瀏覽器支持和易用性。這樣的架構設計也使得工具在處理複雜網站時能夠保持高效，並且支持多平台輸出。擴展性方面，未來可以考慮加入更多的設計規範和樣式檢查功能，以滿足不斷變化的設計需求。

## 技術深入分析

designlang 的核心技術機制基於 Playwright，這使得它能夠在無頭環境中準確地抓取網站的 DOM 結構和樣式。這種設計模式不僅支持靜態網站，也能處理動態生成的內容，這在許多傳統的設計提取工具中是無法實現的。效能上，這個工具能夠快速處理大型網站，並且在提取過程中不會造成顯著的延遲。設計選擇上，使用 TypeScript 提高了代碼的可維護性和可讀性，這對於未來的擴展和功能添加是非常有利的。技術風險方面，對於一些使用複雜 JavaScript 的網站，可能會出現提取不完整的情況，這需要在未來的版本中進行優化。整合方面，這個工具能夠輕鬆與現有的 CI/CD 流程結合，並且支持多種平台的輸出，這使得它在實際工作流中非常友好。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且詳細，提供了多個使用範例，讓新手能夠快速上手。安裝過程順暢，使用 npx 指令無需額外安裝，降低了使用門檻。文件目前僅提供英文版本，可能對非英語使用者造成一定困難。

## 優缺點分析

> [!success] 優點
> - 全面的設計提取功能，涵蓋顏色、字體、佈局等多個方面。
> - 支持運動語言和品牌聲音的提取，這在市場上是獨一無二的。
> - 實時同步功能，能夠保持本地設計標記與網站的一致性。

> [!danger] 缺點
> - 對於某些複雜網站的提取可能不完全。
> - 需要 Node.js 20 以上版本，對環境有一定要求。
> - 初次使用時需要學習 CLI 指令，對新手有一定門檻。

> [!warning] 注意事項
> - 僅支援 Node.js 20 以上版本。
> - 對於某些複雜的網站，可能無法完全提取所有樣式。
> - 需要網路連接以訪問目標網站進行提取。
...
| 工具 | 差異 |
| --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供設計元素提取，但不支持運動語言和佈局系統的提取，適合簡單的設計需求。 |
| [AgentSeal/codeburn](https://github.com/AgentSeal/codeburn) | 專注於設計回歸測試，缺乏全面的設計提取功能，適合已有設計系統的團隊。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供基本的設計元素提取，但不支持運動語言和佈局系統的提取。 | 如果只需要簡單的顏色和字體提取，而不需要其他功能。 | low，因為功能較簡單，學習成本低。 |
| [AgentSeal/codeburn](https://github.com/AgentSeal/codeburn) | 專注於設計回歸測試，缺乏全面的設計提取功能。 | 如果已有設計系統，並且需要進行設計回歸測試。 | medium，因為需要調整現有的設計流程。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **design-extract** | **boneyard** | **codeburn** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 提供基本的設計元素提取，但不支持運動語言和佈局系統的提取。 | 專注於設計回歸測試，缺乏全面的設計提取功能。 |
> | 遷移成本 | - | low，因為功能較簡單，學習成本低。 | medium，因為需要調整現有的設計流程。 |
> | 適用場景 | 主要場景 | 如果只需要簡單的顏色和字體提取，而不需要其他功能。 | 如果已有設計系統，並且需要進行設計回歸測試。 |
...
- **[HIGH]** 某些複雜網站的樣式可能無法完全提取
  - 解法：手動檢查和補充缺失的樣式
- [MEDIUM] 提取過程中可能出現性能瓶頸
  - 解法：對於大型網站，考慮分頁提取
- **[HIGH]** 需要正確配置 Node 環境，否則可能導致安裝失敗
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司前端團隊 | 非常適合 | 能快速提取設計系統，提升開發效率。 |
| 大型企業的設計部門 | 普通 | 可能需要更多的自定義和手動調整。 |
| 需要進行設計回歸測試的團隊 | 適合 | 能夠自動檢查設計變更。 |
| 對設計提取需求不高的團隊 | 不適合 | 功能過於全面，可能造成學習成本。 |
...
> [!tip] 投入 vs 回報
> 花 3 小時學習，2 小時整合，得到全面的設計提取功能，值得投資。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限，僅需訪問目標網站的權限，並且不存取敏感資料。依賴鏈中無已知的供應鏈風險，適合在 CI/CD 中使用。

## 生態系整合

> [!abstract] 如何融入你的工具鏈

designlang 可以與多種工具鏈輕鬆整合，最常與 Playwright 和 Node.js 一起使用。在一個使用 Next.js 的專案中，可以用 `npx designlang` 提取設計，並將生成的設計標記直接應用到專案中。它也支持 GitHub Actions，能在每次 PR 時自動檢查設計變更。整合過程中，最常見的問題是需要正確配置 Node 環境，這可能需要一些額外的設置。
...
> | 貢獻者 | Commits |
> | --- | --- |
> | [@Manavarya09](https://github.com/Manavarya09) | 58 |
> | [@bot-manavarya](https://github.com/bot-manavarya) | 2 |

**最新版本**：v9.0.0 — v9.0.0 — Motion · Anatomy · Voice (2026-04-20)

> [!info]- Release Notes
> **The Motion & Voice release.** Six new capabilities that push designlang past *"extract the paint"* into *"extract the feel, the anatomy, and the voice."* No competing tool does any of these. 282/282 tests passing.
> 
> ## Highlights
> 
> - **Motion language** — easing families, springs via cubic-bezier geometry, duration bucketing (`instant`/`xs`/`sm`/`md`/`lg`/`xl`/`xxl`), scroll-linked animation detection (`animation-timeline`, `view-timeline-name`, `scroll-timeline-name`), keyframe kind classification, and a one-word `feel` fingerprint — *springy* / *responsive* / *smooth* / *mechanical* / *mixed*. Emits `*-motion-tokens.json`.
> - **Component anatomy v2** — variant × size × state matrices with slot inference (icon/label/badge/heading/media/footer). Emits typed React stubs: `*-anatomy.tsx`.
> - **Brand voice** — tone (friendly/formal/technical/playful/neutral), pronoun posture (`we→you`/`you-only`/`we-only`/`third-person`), heading style, CTA verbs, microcopy patterns. Emits `*-voice.json`.
> - **\`designlang lint <file>\`** — audit your own DTCG / flat / CSS-vars token files. Color sprawl, spacing variance, contrast. Exits non-zero on errors. CI-ready.
> - **\`designlang drift <url> --tokens <file>\`** — verdict + ratio between local tokens and a live site. \`--fail-on <level>\` gates CI.
> - **\`designlang visual-diff <a> <b>\`** — single-file HTML with embedded screenshots, size deltas, and a changed-color-tokens table. Zero new deps.
> 
> ## Install
> 
> \`\`\`bash
...
> [!abstract] 最近 10 次 commit（2026-04-18 ~ 2026-04-20）
> **活躍天數** 3 天 · **最新 commit** feat(v9): motion · anatomy · voice — and lint/drift/visual-diff
...
> - **Component Anatomy v2** — every component cluster is now an *anatomy tree* with slots (label, icon, badge, heading, media), variant × size × state matrices, and an emitted `*-anatomy.tsx` file of typed React stubs you can wi

## 延伸閱讀

相關概念：[[設計系統]] · [[設計標記]] · [[無障礙設計]] · [[自動化測試]]
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["設計系統","設計標記","無障礙設計","自動化測試"];
