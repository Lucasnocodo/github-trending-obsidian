...
**4.5k** stars · **87** stars/天 · 建立 52 天前 · N/A · MIT
...
> [!info] 速覽
> **安裝難度** Easy · **專案狀態** Growing · **熱度** Growing (87 stars/day)
> **授權** MIT (商業友好) · **維護** Slow (最後推送 49 天前) · **貢獻者** 2 人 · **參與度** Medium
> **適合** 需要在 Codex 中使用專業化子代理以提升開發效率的軟體工程師。
> **一句話重點** 這個專案的價值在於其提供的專業化子代理，讓開發者能夠更有效率地處理特定任務。
...
這個專案是一個集合，包含超過 130 個 Codex 子代理，專為特定開發任務設計。使用者可以將這些子代理放置在指定的目錄中，然後在 Codex 的提示中明確調用它們，這樣可以針對不同的開發需求進行專業化的處理。每個子代理都使用 `.toml` 格式，並包含模型選擇和指令集，這樣 Codex 可以根據任務自動選擇合適的模型來執行。這些子代理的設計理念是提升 Codex 的能力，讓開發者能夠更高效地處理複雜的任務，如 PR 審查、錯誤調查和代碼規劃等。具體來說，使用者可以通過指令如 `cp categories/01-core-development/backend-developer.toml ~/.codex/agents/` 將所需的子代理複製到全局代理目錄中，並在 Codex 中進行調用。這個工具的賣點在於其專業化和靈活性，能夠根據不同的開發場景提供針對性的支援。

## 重點功能

- 專業化子代理 — 提供超過 130 種針對不同開發任務的 AI 助手，涵蓋從 API 設計到錯誤調查的各種場景。
- 靈活的子代理配置 — 使用者可以將子代理配置為全局或專案特定，根據需要進行調整。
- 自動模型選擇 — 每個子代理根據任務自動選擇合適的 Codex 模型，提升執行效率。
- 沙盒模式 — 透過 `sandbox_mode` 設定，控制子代理的檔案系統訪問權限，確保安全性。
- 簡單的安裝流程 — 使用者只需複製 `.toml` 檔案到指定目錄即可快速啟用子代理。
...
> [!tip] 爆紅原因
> 建立 52 天就累積 4506 stars（87/天），forks 522（11.6%），顯示出強勁的增長潛力。這個專案的主要貢獻者是 necatiozmen 和 haoxianhan，他們在 AI 和開發工具領域有豐富的經驗。專案解決了開發者在使用 Codex 時缺乏專業化助手的痛點，讓開發者能夠更有效地利用 AI 進行特定任務。近期的推廣活動和社群討論可能也促進了其曝光率。隨著 AI 助手的需求增加，這個工具的出現正好填補了市場的空白。
...
> [!example] 使用場景
> - 後端工程師用它來快速審查 PR，因為可以利用 `reviewer` 子代理進行安全性和正確性檢查，節省了手動審查的時間。
> - 前端開發者用它來調查錯誤，因為可以同時調用 `code_mapper` 和 `frontend_developer` 來追蹤問題並提出修復建議，提升了問題解決的效率。
> - 項目經理用它來規劃代碼庫，因為可以利用 `search_specialist` 和 `knowledge_synthesizer` 來快速定位和總結設計，減少了手動查找的時間。

## 架構分析

這個專案的架構基於 Codex 的子代理設計理念，將不同的開發任務分配給專門的 AI 助手。每個子代理使用 `.toml` 格式進行配置，這樣可以簡化管理和調用。由於子代理的設計是為了專注於特定任務，因此在性能上可以針對性優化，減少不必要的計算負擔。這樣的設計使得開發者能夠在不同的開發場景中靈活應用，並且能夠根據需求進行擴展。整體架構的可擴展性良好，但在管理多個子代理時可能會增加複雜度。

## 技術深入分析

這個專案的核心技術機制是使用 Codex 的子代理設計，將不同的開發任務分配給專門的 AI 助手。每個子代理都使用 `.toml` 格式進行配置，這樣可以簡化管理和調用。效能上，這些子代理能夠根據任務自動選擇合適的模型，從而提高執行效率。設計上，這些子代理的選擇和配置是為了專注於特定任務，這樣可以減少不必要的計算負擔。技術上，這個專案的依賴性較低，主要依賴 Codex 的能力，這使得它在整合到現有工具鏈時相對簡單。隨著開發者對 AI 助手需求的增加，這個專案的潛力和市場需求也在持續增長。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝步驟和範例，讓新手能夠快速上手。安裝過程相對簡單，主要是克隆倉庫和配置子代理。文件中沒有多語言支持，但英文內容清晰易懂。整體來說，花 30 分鐘內能夠成功配置並運行。

## 優缺點分析

> [!success] 優點
> - 提供多樣化的專業化子代理，滿足不同開發需求。
...
> - 每個子代理的功能範圍有限，可能無法涵蓋所有需求。

> [!warning] 注意事項
> - 需要手動配置子代理，Codex 不會自動生成。
> - 每個子代理的功能範圍有限，可能無法涵蓋所有開發需求。
> - 對於大型專案，管理多個子代理可能會變得複雜。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供類似的 AI 助手，但專注於遊戲開發場景，適合遊戲開發者使用。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於數據科學和分析，提供不同的功能和專業化支持。 |
| [432539/gpt2api](https://github.com/432539/gpt2api) | 提供 API 接口的解決方案，但不具備專業化子代理的靈活性和多樣性。 |
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["AI 助手","Codex","專業化子代理"];
