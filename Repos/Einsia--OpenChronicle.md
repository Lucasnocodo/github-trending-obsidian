...
use_case: "提供本地優先的記憶系統，讓 LLM 代理能夠捕捉和存儲工作上下文。"
priority: high
ring: assess
...
star_history: "2026-04-26:1126,2026-04-26:1129,2026-04-27:1422,2026-04-27:1425,2026-04-28:1593,2026-04-28:1596,2026-04-29:1809,2026-04-30:1885,2026-05-01:1933,2026-05-02:1983,2026-05-03:2024,2026-05-04:2063,2026-05-05:2090,2026-05-06:2097,2026-05-07:2105,2026-05-08:2109,2026-05-09:2158,2026-05-10:2221,2026-05-11:2341,2026-05-12:2453,2026-05-13:2591"
tags:
  - github
  - "category/開發工具"
  - "lang/python"
...
aliases:
  ...
  - "提供本地優先的記憶系統，讓 LLM 代理能夠捕捉和存儲工作上下文。"
---

# OpenChronicle

**2.6k** stars · **123** stars/天 · 建立 21 天前 · Python · MIT
...
> **授權** MIT (商業友好) · **維護** Active (最後推送 3 天前) · **貢獻者** 5+ 人 · **參與度** Low
> **適合** 需要在 macOS 上運行 LLM 代理並希望擁有本地記憶功能的開發者。
> **一句話重點** OpenChronicle 提供了一個靈活且本地優先的記憶系統，讓 LLM 代理能夠更好地捕捉和管理上下文。
...
> **結論** 花 5 小時學習、3 小時整合，得到高效的上下文記憶管理，值得考慮。
...
OpenChronicle 是一個開源的本地優先記憶系統，專為任何能夠調用工具的 LLM 代理設計。它能夠捕捉使用者的屏幕和應用程序上下文，並將其轉換為持久的 Markdown 記憶，這樣代理就能夠隨時查詢這些記憶。使用者只需在 macOS 上運行 OpenChronicle，並通過簡單的 CLI 指令來啟動和管理記憶捕捉。其核心賣點在於本地存儲和可檢查性，讓使用者能夠完全掌控自己的數據。

技術上，OpenChronicle 使用 AX Tree 作為主要信號，這樣可以降低處理成本並提高意圖捕捉的準確性，並且支持 Markdown 和 SQLite 進行存儲和索引。與 OpenAI Chronicle 相比，OpenChronicle 是開源的，並且支持多種模型選擇，這使得它更具靈活性和擴展性。使用者可以通過 MCP 協議輕鬆連接各種工具，並且能夠在本地或雲端模型中運行，這在需要快速查詢和記憶管理的情境下特別有用。這個專案目前處於早期 alpha 階段，適合對個人 AI 記憶或開放上下文基礎設施有興趣的開發者。

**技術棧**：`Python 3.11` · `Swift` · `Shell`

## 重點功能

- 本地優先存儲 — 記憶數據保存在本地，確保數據隱私。
- AX Tree 捕捉 — 使用 AX Tree 作為主要信號，降低處理成本並提高準確性。
- Markdown 記憶 — 記憶以人類可讀的 Markdown 格式存儲，便於檢查和編輯。
- SQLite 索引 — 使用 SQLite 進行快速查詢和索引，支持高效的數據檢索。
- MCP 支持 — 提供 MCP 協議的支持，方便與各種工具集成。
...
```python
{
  "前置條件": "需要在 macOS 上安裝並配置好 Xcode Command Line Tools。",
...
> [!tip] 爆紅原因
> 建立 21 天就累積 2591 stars（123/天），forks 206（8.0%），顯示出強勁的增長潛力。專案的主要貢獻者有豐富的開源經驗，並且專案解決了現有 LLM 代理缺乏本地記憶的痛點，這在當前對數據隱私越來越重視的環境中尤為重要。開發者對於本地優先的記憶系統需求日益增加，特別是在需要快速查詢和上下文記憶的場景中。這個專案的開源性和靈活性使其成為一個值得關注的選擇。

## 適合誰使用

**目標受眾**：需要在 macOS 上運行 LLM 代理並希望擁有本地記憶功能的開發者。

> [!example] 使用場景
> - 自由職業者用它來記錄每個客戶專案的上下文和決策，因為這樣能夠快速回顧過去的工作，提升工作效率。
> - 開發者用它來捕捉編碼過程中的重要上下文，因為這樣能夠減少重複工作，並提高代碼質量。
> - 產品經理用它來追蹤不同專案的進展和關鍵決策，因為這樣能夠更好地協調團隊工作，提升專案透明度。

## 架構分析

OpenChronicle 採用事件驅動架構，通過 macOS 的 AX 事件捕捉上下文，然後將這些上下文壓縮成會話，並提取持久的事實。這樣的設計使得記憶的捕捉和存儲變得高效且結構化。使用 SQLite 進行索引，能夠快速查詢和檢索記憶。

選擇 AX Tree 作為主要信號是因為它能夠提供更低的處理成本和更好的意圖捕捉，這在需要快速反應的場景中尤為重要。這種設計的代價是目前對於其他上下文捕捉方式的支持較弱，未來可能需要擴展這方面的功能。整體架構的擴展性良好，能夠支持多種 LLM 模型的集成。

## 技術深入分析

OpenChronicle 的核心技術機制在於其事件驅動的上下文捕捉，利用 macOS 的 AX 事件來捕捉使用者的操作，這樣的設計使得記憶的捕捉過程高效且結構化。它使用 AX Tree 作為主要信號，這樣可以降低處理成本並提高意圖捕捉的準確性，與傳統的屏幕截圖和 OCR 方法相比，能夠更快速地提取有用信息。記憶的存儲使用 Markdown 格式，這使得記憶不僅易於檢查，也便於後續的編輯和擴展。SQLite 的使用則提供了快速的數據檢索能力，能夠支持高效的查詢操作。設計上，這個專案選擇了本地優先的存儲方式，這在數據隱私日益受到重視的當下，顯得尤為重要。然而，這樣的設計也帶來了對於其他上下文捕捉方式支持不足的風險，未來可能需要進一步擴展功能以滿足更廣泛的需求。整體而言，OpenChronicle 在 LLM 代理的記憶管理上提供了一個創新的解決方案，並且在開源社群中具有良好的發展潛力。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝指引和使用範例，讓新手能夠快速上手。安裝過程相對順暢，但需要注意安裝 Xcode Command Line Tools。文件中包含了詳細的配置和使用說明，對於新手友好。文件目前僅提供英文版本，可能對非英語使用者造成一定困難。

## 優缺點分析

> [!success] 優點
> - 本地優先設計，確保數據隱私和安全性。
> - 開源且可擴展，適合開發者進行自定義和擴展。
> - 支持多種 LLM 模型，靈活性高。
> - Markdown 格式存儲，便於檢查和編輯。

> [!danger] 缺點
> - 目前僅支援 macOS，對於其他平台的支持有限。
> - 處於 alpha 階段，可能存在不穩定性。
> - 功能尚未全面，特別是在上下文捕捉方面。
> - 需要額外安裝 Xcode Command Line Tools。

> [!warning] 注意事項
> - 目前僅支援 macOS 13+。
> - 處於 alpha 階段，可能存在不穩定性和功能缺失。
> - 目前功能主要集中在 AX Tree 捕捉，尚未全面支持其他上下文捕捉方式。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供類似的代理記憶功能，但專注於遊戲開發場景，並不支持本地優先的存儲。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於數據存儲和檢索，缺乏 OpenChronicle 的本地優先和可檢查性特性。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於遊戲開發的記憶系統，使用 Unity 引擎實現，與 OpenChronicle 的 Python 基礎架構不同。 | 如果你的專案是遊戲開發，並且需要針對遊戲環境的記憶管理。 | medium，因為需要重新設計記憶捕捉邏輯。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於數據存儲和檢索，使用 NoSQL 數據庫，與 OpenChronicle 的 SQLite 存儲方式不同。 | 如果你的專案需要高效的非結構化數據存儲和檢索。 | high，因為需要重構數據結構和存儲邏輯。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **OpenChronicle** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於遊戲開發的記憶系統，使用 Unity 引擎實現，與 OpenChronicle 的 Python 基礎架構不同。 | 專注於數據存儲和檢索，使用 NoSQL 數據庫，與 OpenChronicle 的 SQLite 存儲方式不同。 |
> | 遷移成本 | - | medium，因為需要重新設計記憶捕捉邏輯。 | high，因為需要重構數據結構和存儲邏輯。 |
> | 適用場景 | 主要場景 | 如果你的專案是遊戲開發，並且需要針對遊戲環境的記憶管理。 | 如果你的專案需要高效的非結構化數據存儲和檢索。 |
...
- **[HIGH]** 在某些 macOS 版本上，AX 事件捕捉可能不穩定。
  - 解法：確保系統更新到最新版本，並檢查相關權限設定。
- [MEDIUM] Markdown 記憶文件可能會因為寫入錯誤而損壞。
  - 解法：定期備份記憶文件，並使用版本控制進行管理。
- [MEDIUM] CLI 指令在某些情況下可能無法正常執行。
  - 解法：檢查安裝是否正確，並參考官方文檔進行故障排除。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型開發團隊的專案管理 | 非常適合 | 能夠有效捕捉和管理專案上下文，提升團隊協作效率。 |
| 個人自由職業者的工作記錄 | 非常適合 | 本地優先設計確保數據隱私，適合記錄客戶專案。 |
| 大型企業的記憶管理系統 | 不適合 | 目前僅支援 macOS，對於跨平台需求的支持有限。 |
| 需要高效數據檢索的應用 | 適合 | 使用 SQLite 進行快速查詢，能夠滿足高效檢索的需求。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習、3 小時整合，得到高效的上下文記憶管理，值得考慮。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：本工具在本地運行，不需要高權限，且不會存取敏感資料。依賴鏈的信任程度較高，無已知的供應鏈風險，適合在 CI/CD 中使用。

## 生態系整合

> [!abstract] 如何融入你的工具鏈

OpenChronicle 最常與 LLM 代理搭配使用，特別是支持 MCP 協議的工具。在一個使用 OpenAI 或 Ollama 的專案中，使用者可以通過 MCP 端點輕鬆集成 OpenChronicle，以捕捉和管理上下文記憶。它與主流 CI 工具（如 GitHub Actions）兼容，並且可以與 VS Code 等 IDE 整合。整合過程中，最常見的問題是權限設定不當，建議在安裝前檢查相關權限。

## 歷史脈絡

> [!info] 這個工具為什麼現在出現？

在 OpenChronicle 出現之前，許多 LLM 代理依賴於雲端存儲和外部服務進行記憶管理，這樣的方案往往存在數據隱私和安全性問題。隨著對本地優先解決方案需求的增加，OpenChronicle 提供了一個靈活且安全的替代方案。技術上，macOS 的 AX 事件捕捉技術的成熟使得這種本地記憶系統成為可能，未來可能會有更多類似的工具出現，進一步推動本地優先記憶管理的發展。
...
> [!tip] 導入策略
> 第一週：在個人側項目中試用。第二週：在小型團隊內部工具中導入。第三週：收集使用反饋並進行調整。第四週：在主產品中逐步推廣使用。

**成功指標**：記憶管理效率提升 30%，團隊協作透明度提高。

> [!warning] 退出計畫
> 若需退出，所有設定和記憶數據都以 Markdown 格式存儲，易於轉移至其他工具。
...
相關概念：[[MCP Protocol]] · [[自動化測試]] · [[記憶管理]]

相關專案：[[0x0funky--agent-sprite-forge|0x0funky/agent-sprite-forge]] · [[0xGF--boneyard|0xGF/boneyard]] · [[AgriciDaniel--claude-obsidian|AgriciDaniel/claude-obsidian]] · [[432539--gpt2api|432539/gpt2api]] · [[MemPalace--mempalace|MemPalace/mempalace]] · [[milla-jovovich--mempalace|milla-jovovich/mempalace]] · [[BuilderPulse--BuilderPulse|BuilderPulse/BuilderPulse]] · [[AgentSeal--codeburn|AgentSeal/codeburn]]
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["MCP Protocol","自動化測試","記憶管理"];
