...
use_case: "為任何 LLM agent 提供本地優先的記憶系統，捕捉屏幕上下文並以 Markdown 格式儲存。"
priority: high
ring: assess
...
star_history: "2026-04-26:1126,2026-04-26:1129,2026-04-27:1422,2026-04-27:1425,2026-04-28:1593,2026-04-28:1596,2026-04-29:1809,2026-04-30:1885,2026-05-01:1933,2026-05-02:1983,2026-05-03:2024,2026-05-04:2063"
tags:
  - github
  - "category/開發工具"
  - "lang/python"
...
aliases:
  ...
  - "為任何 LLM agent 提供本地優先的記憶系統，捕捉屏幕上下文並以 Markdown 格式儲存。"
---

# OpenChronicle

**2.1k** stars · **172** stars/天 · 建立 12 天前 · Python · MIT
...
> **授權** MIT (商業友好) · **維護** Active (最後推送 7 天前) · **貢獻者** 5+ 人 · **參與度** Low
> **適合** 希望在 Mac 上使用本地記憶系統的開發者，特別是對 LLM agent 有需求的使用者。
> **一句話重點** OpenChronicle 提供了一個靈活且可擴展的本地優先記憶系統，適合對個人 AI 記憶有需求的開發者。
...
> [!abstract] 核心創新
> 提供一個開放的本地優先記憶系統，支持多種 LLM 模型，並以 Markdown 格式儲存記憶。

## 專案簡介

OpenChronicle 是一個本地優先的記憶系統，專為 LLM agent 設計，能夠捕捉用戶的屏幕上下文並將其轉換為持久的 Markdown 記憶。使用者在 Mac 上運行該工具，能夠記錄正在進行的工作、已做的決策以及使用的工具，並將這些資訊以結構化的方式儲存。其核心流程包括捕捉上下文、壓縮為會話、提取持久事實，並將記憶儲存於本地 SQLite 數據庫中。最關鍵的 CLI 指令如 `openchronicle start` 可啟動記憶捕捉，而 `openchronicle status` 則可檢查當前狀態。這個工具的賣點在於其開放性和可擴展性，允許用戶根據自己的需求進行調整和擴展。

技術上，OpenChronicle 使用了 AX Tree 作為主要信號，並輔以截圖，這樣的設計使得記憶的捕捉成本更低，且能夠更準確地捕捉用戶的意圖。與 OpenAI Chronicle 相比，OpenChronicle 提供了更大的靈活性，支持多種模型，並且是開源的，這使得它更適合開發者進行個性化的擴展。使用者可以在本地或雲端模型中進行操作，並且其記憶資料以 Markdown 格式儲存，便於檢查和修改。這個專案目前仍在 alpha 階段，適合對個人 AI 記憶系統有興趣的開發者進行探索和實驗。

**技術棧**：`Python 3.11` · `Swift` · `Shell`

## 重點功能

- 本地優先記憶 — 記憶資料儲存在本地，保障隱私與安全。
- AX Tree 捕捉 — 主要使用 AX Tree 作為上下文信號，降低處理成本。
- Markdown 格式儲存 — 記憶以人類可讀的 Markdown 格式儲存，便於檢查和修改。
- SQLite 索引 — 使用 SQLite 進行本地索引，提升查詢效率。
- 即時捕捉 — 支援事件驅動的上下文捕捉，確保記憶的即時性與準確性。
...
> [!tip] 爆紅原因
> 建立 12 天就累積 2063 stars（172/天），forks 194（9.4%），顯示出強勁的增長潛力。該專案的主要貢獻者來自於開源社群，致力於提供一個開放的記憶系統，解決了現有工具在記憶捕捉上的局限性。之前的解決方案如 OpenAI Chronicle 限於特定模型，無法靈活應用於多種工具，而 OpenChronicle 則提供了更廣泛的兼容性和可擴展性。社群的活躍度和持續的開發更新也顯示出其潛在的長期支持。這樣的增長可能受到社群對於開源和本地優先解決方案需求增加的影響。

## 適合誰使用

**目標受眾**：希望在 Mac 上使用本地記憶系統的開發者，特別是對 LLM agent 有需求的使用者。

> [!example] 使用場景
> - 自由職業者用它來記錄工作進度和決策，因為這樣可以快速回顧過去的工作內容，提升工作效率。
> - 開發者用它來整合不同工具的上下文，因為這樣能夠更好地管理多個專案的記憶，避免重複工作。
> - 研究人員用它來捕捉實驗過程中的關鍵資訊，因為這樣能夠保留實驗的詳細記錄，便於後續分析。

## 架構分析

OpenChronicle 的架構設計為本地優先，主要依賴 macOS 的 AX 事件來捕捉上下文。其資料流從事件捕捉開始，經過去重和解析，最終將記憶儲存於 SQLite 中。這樣的設計使得記憶的捕捉成本較低，並且能夠有效地提取用戶的意圖。

選擇 AX Tree 作為主要信號的原因在於其結構化文本處理的效率高於截圖，這樣能夠在記憶中保留更清晰的資訊。擴展性方面，OpenChronicle 允許用戶自定義解析器和記憶邏輯，這使得它能夠適應不同的應用場景。整體而言，這種設計在保證記憶準確性的同時，也提供了靈活的擴展能力。

## 技術深入分析

OpenChronicle 的核心技術機制依賴於 macOS 的 AX 事件來捕捉用戶的上下文，並使用 Markdown 格式來儲存記憶，這使得記憶的檢查和修改變得簡單。其效能特性允許在本地快速處理和索引記憶資料，並且在記憶的捕捉過程中，通過事件驅動的方式來保證資料的即時性。選擇 Python 和 Swift 作為主要開發語言，能夠充分利用 macOS 的生態系統，但這也意味著專案的使用範圍受到限制。技術上，這個專案的依賴樹相對簡單，主要依賴於幾個核心庫，降低了維護的複雜度。然而，這也可能導致在未來擴展時遇到瓶頸，特別是在處理更複雜的記憶邏輯時。整合方面，OpenChronicle 提供了與 MCP 協議的良好兼容性，這使得它能夠與多種工具進行整合，並且在 CI/CD 流程中也能夠輕鬆使用。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝步驟和使用範例，對於新手來說相對友好。安裝過程需要安裝 Xcode Command Line Tools，這可能對某些用戶造成困難。整體而言，文件質量良好，能夠幫助用戶快速上手。
...
> - 開放源碼，易於擴展和自定義。
> - 支持多種 LLM 模型，靈活性高。
> - Markdown 格式儲存，便於檢查和修改。

> [!danger] 缺點
> - 僅支援 macOS，限制了使用範圍。
> - 目前仍在 alpha 階段，功能不穩定。
> - 依賴於 AX Tree，對某些應用可能無法捕捉。
> - 安裝過程需要額外的 Xcode 工具。

> [!warning] 注意事項
> - 僅支援 macOS 13+。
> - 目前仍在 alpha 階段，功能可能不穩定。
> - 依賴於 AX Tree，對於不支援的應用可能無法捕捉上下文。
> - 需要 Xcode Command Line Tools 的安裝。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於生成和管理 AI agent 的記憶，與 OpenChronicle 的本地優先設計不同，後者更強調本地存儲和可檢查性。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供一個簡化的記憶管理系統，適合小型專案，而 OpenChronicle 則針對更複雜的上下文捕捉和記憶管理。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於生成和管理 AI agent 的記憶，與 OpenChronicle 的本地優先設計不同，後者更強調本地存儲和可檢查性。 | 如果需要一個專注於生成記憶的工具，而不僅僅是捕捉上下文。 | medium，因為需要重新設計記憶管理邏輯。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供一個簡化的記憶管理系統，適合小型專案，而 OpenChronicle 則針對更複雜的上下文捕捉和記憶管理。 | 如果你的專案規模較小，且需要快速上手的記憶管理工具。 | low，因為其設計較為簡單，易於適應。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **OpenChronicle** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於生成和管理 AI agent 的記憶，與 OpenChronicle 的本地優先設計不同，後者更強調本地存儲和可檢查性。 | 提供一個簡化的記憶管理系統，適合小型專案，而 OpenChronicle 則針對更複雜的上下文捕捉和記憶管理。 |
> | 遷移成本 | - | medium，因為需要重新設計記憶管理邏輯。 | low，因為其設計較為簡單，易於適應。 |
> | 適用場景 | 主要場景 | 如果需要一個專注於生成記憶的工具，而不僅僅是捕捉上下文。 | 如果你的專案規模較小，且需要快速上手的記憶管理工具。 |
...
- **[HIGH]** 安裝過程中可能會因缺少 Xcode Command Line Tools 而失敗。
  - 解法：確保先執行 `xcode-select --install` 安裝必要的工具。
- [MEDIUM] 在某些應用中，AX Tree 可能無法正確捕捉上下文。
  - 解法：檢查應用的無障礙設置，確保 AX Tree 功能正常。
- [low] 記憶資料可能因為過多截圖而變得冗長。
  - 解法：定期清理不必要的截圖，保持記憶的簡潔。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 個人開發者的本地記憶系統 | 非常適合 | 提供靈活的記憶管理和本地存儲，滿足開發者需求。 |
| 大型團隊的協作記憶系統 | 不適合 | 目前仍在 alpha 階段，功能不穩定，難以滿足團隊需求。 |
| 需要快速捕捉上下文的自由職業者 | 適合 | 能夠即時捕捉工作上下文，提升工作效率。 |
...
> [!warning] 安全性快速掃描
> 低風險：本工具不需要高權限運行，且所有資料儲存在本地，不會外洩敏感資訊。
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["MCP Protocol","記憶管理","事件驅動"];
