...
> **適合** 需要在 macOS 上運行本地記憶系統的 LLM 開發者和研究者。
> **一句話重點** OpenChronicle 的設計理念在於提供一個開放的、本地優先的記憶系統，讓 LLM 代理能夠更好地管理和利用上下文。
...
> **結論** 花 5 小時學、3 小時整合，得到一個靈活的本地記憶系統，值得考慮。

> [!abstract] 核心創新
> OpenChronicle 提供了一個開放的、本地優先的記憶系統，專為任何工具能力的 LLM 代理設計。

## 專案簡介

OpenChronicle 是一個開源的本地優先記憶系統，專為任何工具能力的 LLM 代理設計。它運行在 macOS 上，通過捕捉 AX 樹和截圖來構建持久的 Markdown 記憶，記錄用戶的工作內容、決策、使用的工具及相關人員或項目。這個系統的核心賣點在於其本地存儲和可檢查性，使用者可以隨時查閱和管理記憶。其設計選擇了 AX 樹作為主要信號，因為相較於截圖，結構化文本的處理成本較低，且能更好地捕捉用戶的意圖。這樣的設計使得記憶更易於去重、標準化和長期保存。

與 OpenAI Chronicle 相比，OpenChronicle 是開源的，並且不依賴特定的模型或應用，支持多種 LLM 提供者。它的記憶存儲格式為 Markdown 和 SQLite，提供了更大的擴展性和可定制性。使用者可以通過 MCP 協議輕鬆連接各種工具，並利用事件驅動的捕捉方式來記錄上下文。整體而言，OpenChronicle 是一個針對需要本地記憶系統的 LLM 代理的理想解決方案，特別適合開發者和 AI 研究者使用。

**技術棧**：`Python 3.11` · `Swift` · `Shell`

## 重點功能

- 本地優先記憶 — 記憶數據存儲在本地，保護用戶隱私。
- AX 樹捕捉 — 主要使用 AX 樹結構來捕捉上下文，降低處理成本。
- Markdown 格式存儲 — 記憶以 Markdown 格式保存，便於檢查和編輯。
- SQLite 索引 — 使用 SQLite 進行本地索引，提高查詢效率。
- 事件驅動捕捉 — 透過 macOS AX 事件實現即時捕捉上下文。
...
```python
[
  "# 前置條件：需要安裝 Xcode Command Line Tools",
  "openchronicle start",
...
> [!tip] 爆紅原因
> 建立 4 天內累積 1126 stars（282/天），forks 57（5.1%），顯示出穩定的增長趨勢。這個專案的主要貢獻者包括 Xiao-ao-jiang-hu 和 KMing-L，他們在開源社群中有一定的影響力。OpenChronicle 解決了現有記憶系統的封閉性和不透明性，提供了一個開放且可擴展的替代方案。這一點在當前對於 AI 代理的需求日益增加的背景下尤為重要。社群對於本地記憶系統的需求也在不斷上升，尤其是在隱私和數據控制方面的考量。forks/stars 比率為 5.1%，顯示出一定的實際使用和修改需求，這對於新專案來說是個良好的指標。

## 適合誰使用

**目標受眾**：需要在 macOS 上運行本地記憶系統的 LLM 開發者和研究者。

> [!example] 使用場景
> - AI 研究者用它來捕捉和管理實驗過程中的上下文，因為這樣可以更好地追蹤決策和使用的工具。
> - 開發者用它來記錄日常編碼過程中的重要信息，因為這能幫助他們在未來的項目中快速回顧和重用知識。
> - 產品經理用它來整理會議記錄和決策過程，因為這樣可以提高團隊的透明度和協作效率。

## 架構分析

OpenChronicle 採用事件驅動架構，通過 macOS 的 AX 事件捕捉上下文。這種設計使得系統能夠即時響應用戶行為，並將捕捉到的數據存儲為 Markdown 和 SQLite 格式。這樣的存儲方式不僅便於檢查，還能提高查詢效率。

系統的核心流程包括捕捉上下文、壓縮為會話、提取持久事實，並將記憶存儲在本地。這種設計選擇的代價在於對特定平台的依賴，未來擴展到其他操作系統可能需要重構部分代碼。整體架構的擴展性良好，能夠支持多種 LLM 提供者的集成。

## 技術深入分析

OpenChronicle 的核心技術機制是基於 macOS 的 AX 事件捕捉，這使得系統能夠即時獲取用戶的操作上下文。系統通過事件驅動的方式捕捉上下文，並將其壓縮為會話，最終存儲為 Markdown 和 SQLite 格式。這樣的設計不僅提高了記憶的可讀性，還能夠快速查詢和檢索。選擇使用 AX 樹而非截圖的原因在於其處理成本較低，且能更好地捕捉用戶的意圖。

這樣的選擇雖然在某些情況下可能無法捕捉到完整的視覺上下文，但為系統提供了一個更清晰的記憶結構。技術上，這個系統的依賴關係相對簡單，主要依賴於 Python 和 Swift 的組合，這使得開發和維護相對容易。然而，隨著使用者數量的增加，系統的性能和穩定性可能會受到挑戰，特別是在記憶數據量增大的情況下。整合方面，OpenChronicle 目前主要支持 MCP 協議，未來可能需要擴展到其他協議以滿足更廣泛的需求。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝步驟和使用範例，對於新手來說相對友好。安裝過程需要安裝 Xcode Command Line Tools，這可能對某些用戶造成困難。整體上，文件結構良好，包含了多個文檔鏈接以幫助用戶了解系統的架構和使用方法。沒有中文或多語言支持。
...
> - 開源且可擴展，適合開發者進行二次開發。
> - 支持多種 LLM 提供者，靈活性高。
> - 使用 Markdown 存儲，便於檢查和編輯。

> [!danger] 缺點
> - 目前僅支持 macOS，缺乏跨平台支持。
> - 處於 alpha 階段，功能尚不穩定。
> - 依賴於 AX 樹，可能無法捕捉所有應用的上下文。
> - 社群活躍度尚待提高，問題解決速度慢。
...
> - 依賴於 AX 樹，可能無法捕捉所有應用的上下文。
> - 目前僅支持 MCP 協議，其他協議的支持尚未完善。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard 提供了一個類似的記憶系統，但主要針對特定的應用場景，而 OpenChronicle 則是通用的記憶解決方案。 |
| [AgriciDaniel/claude-obsidian](https://github.com/AgriciDaniel/claude-obsidian) | Claude Obsidian 專注於 Obsidian 生態系統，而 OpenChronicle 則支持多種 LLM 提供者，具有更大的靈活性。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard 提供了一個類似的記憶系統，但主要針對特定的應用場景，而 OpenChronicle 則是通用的記憶解決方案。 | 如果你的需求是針對特定應用的記憶管理，Boneyard 可能更適合。 | medium - 需要重新設計記憶結構以符合 Boneyard 的格式。 |
| [AgriciDaniel/claude-obsidian](https://github.com/AgriciDaniel/claude-obsidian) | Claude Obsidian 專注於 Obsidian 生態系統，而 OpenChronicle 則支持多種 LLM 提供者，具有更大的靈活性。 | 如果你的團隊已經在使用 Obsidian 並需要深度整合，Claude Obsidian 會是更好的選擇。 | low - 由於兩者都使用 Markdown 格式，遷移相對簡單。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **OpenChronicle** | **boneyard** | **claude-obsidian** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | Boneyard 提供了一個類似的記憶系統，但主要針對特定的應用場景，而 OpenChronicle 則是通用的記憶解決方案。 | Claude Obsidian 專注於 Obsidian 生態系統，而 OpenChronicle 則支持多種 LLM 提供者，具有更大的靈活性。 |
> | 遷移成本 | - | medium - 需要重新設計記憶結構以符合 Boneyard 的格式。 | low - 由於兩者都使用 Markdown 格式，遷移相對簡單。 |
> | 適用場景 | 主要場景 | 如果你的需求是針對特定應用的記憶管理，Boneyard 可能 | 如果你的團隊已經在使用 Obsidian 並需要深度整合，C |
...
- **[HIGH]** 在某些 macOS 設定下，AX 事件捕捉可能會失效。
  - 解法：檢查系統隱私設定，確保應用有足夠的權限。
- [MEDIUM] Markdown 檔案可能在高頻寫入時出現損壞。
  - 解法：定期備份記憶檔案以防數據丟失。
- [low] 目前的文檔不夠詳盡，可能會讓新手感到困惑。
  - 解法：參考社群討論或尋求幫助。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型團隊的 AI 研究專案 | 非常適合 | 提供本地記憶系統，能夠有效管理上下文和決策。 |
| 個人開發者的日常編碼工作 | 適合 | 能夠記錄和回顧編碼過程中的重要信息。 |
| 大型企業的生產環境 | 不適合 | 目前處於 alpha 階段，穩定性不足。 |
| 需要跨平台支持的開發團隊 | 不適合 | 目前僅支援 macOS，缺乏跨平台能力。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學、3 小時整合，得到一個靈活的本地記憶系統，值得考慮。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：本工具不需要高權限，僅存取本地文件系統，且不會傳輸敏感數據。
...
相關概念：[[MCP Protocol]] · [[記憶系統]] · [[AI 代理]]

相關專案：[[0xGF--boneyard|0xGF/boneyard]] · [[AgriciDaniel--claude-obsidian|AgriciDaniel/claude-obsidian]] · [[MemPalace--mempalace|MemPalace/mempalace]] · [[milla-jovovich--mempalace|milla-jovovich/mempalace]] · [[BuilderPulse--BuilderPulse|BuilderPulse/BuilderPulse]]
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["MCP Protocol","記憶系統","AI 代理"];
