...
use_case: "為 LLM 代理提供本地、可檢查的記憶體，捕捉屏幕和應用程式上下文。"
priority: high
ring: assess
...
aliases:
  ...
  - "為 LLM 代理提供本地、可檢查的記憶體，捕捉屏幕和應用程式上下文。"
---

# OpenChronicle

**2.5k** stars · **123** stars/天 · 建立 20 天前 · Python · MIT
...
> **適合** 需要在 macOS 上運行本地記憶體系統以支持 LLM 代理的開發者。
> **一句話重點** OpenChronicle 讓用戶能夠在本地管理 LLM 代理的記憶體，這在當前的 AI 生態中是相對少見的。
...
> [!abstract] 核心創新
> OpenChronicle 提供一個本地優先的記憶體系統，專為能夠調用工具的 LLM 代理設計。

## 專案簡介

OpenChronicle 是一個開源的本地記憶體系統，專為能夠調用工具的 LLM 代理設計。它運行在 macOS 上，通過捕捉用戶的屏幕和應用上下文，將這些信息轉化為持久的 Markdown 記憶體。具體來說，使用者可以透過 `openchronicle start` 命令啟動服務，並使用 `openchronicle capture-once` 捕捉當前上下文。這樣的設計使得記憶體能夠在本地存儲，並且不依賴於雲端服務，這對於隱私和數據安全至關重要。此工具的賣點在於其本地優先的設計，讓用戶能夠完全掌控自己的數據，並且支持多種 LLM 提供者，這使得它在模型選擇上非常靈活。

技術上，OpenChronicle 利用 macOS 的 AX Tree 事件來捕捉上下文，這種方法比傳統的屏幕截圖和 OCR 更高效，因為 AX Tree 提供了結構化的文本信息，降低了處理成本。

它的存儲格式為 Markdown 和 SQLite，這使得數據易於檢索和管理。與 OpenAI Chronicle 相比，OpenChronicle 是開源的，並且不僅限於某一特定的模型或應用，這使得它的擴展性更強。

在與其他類似工具的比較中，OpenChronicle 的 AX Tree 捕捉方式使其在記憶體的準確性和可維護性上優於依賴屏幕截圖的工具，如 OpenAI Chronicle。對於需要本地記憶體的用戶，這個工具提供了一個強大的解決方案，但目前僅支持 macOS，這限制了其使用範圍。

使用者在實際操作中可能會遇到一些挑戰，例如需要安裝 Xcode Command Line Tools，這對於不熟悉 macOS 的用戶來說可能是一個障礙。整體來看，OpenChronicle 處於 alpha 階段，適合對本地記憶體系統有興趣的開發者和研究者，未來可能會進一步增強其功能和穩定性。

**技術棧**：`Python 3.11` · `Swift` · `Shell`

## 重點功能

- 本地存儲 — 記憶體數據存儲在本地，確保數據隱私。
- AX Tree 捕捉 — 利用 macOS 的 AX Tree 事件捕捉上下文，提供結構化數據。
- Markdown 格式 — 記憶體以 Markdown 格式存儲，易於閱讀和編輯。
- SQLite 索引 — 使用 SQLite 進行快速檢索和查詢。
- 多模型支持 — 支持多種 LLM 提供者，靈活性高。
...
> [!tip] 爆紅原因
> 建立 20 天內累積 2453 stars（123 stars/天），forks 204（8.3%），顯示出穩定的增長。這個專案的主要貢獻者來自於對開放記憶體系統的需求，特別是在隱私和數據控制方面。OpenChronicle 提供了一個本地優先的解決方案，這在當前的 AI 生態中是相對少見的。其開源特性和靈活的模型選擇吸引了許多開發者的注意。社群對於本地記憶體的需求正在上升，這使得 OpenChronicle 的出現恰逢其時。

## 適合誰使用

**目標受眾**：需要在 macOS 上運行本地記憶體系統以支持 LLM 代理的開發者。

> [!example] 使用場景
> - 自由職業者用它來記錄工作過程中的重要決策，因為它能夠自動捕捉上下文，減少手動記錄的負擔。
> - 開發者用它來追蹤在 IDE 中的代碼變更，因為這樣可以更清晰地了解每次提交的背景和意圖。
> - 研究人員用它來整理實驗過程中的觀察結果，因為它的 Markdown 格式使得結果易於分享和檢查。

## 架構分析

OpenChronicle 的架構設計為本地優先，利用 macOS 的 AX Tree 事件捕捉上下文，並將其轉化為持久的 Markdown 記憶體。這種設計使得用戶的數據完全存儲在本地，避免了雲端存儲的隱私風險。資料流從事件捕捉開始，經過解析、去重、存儲等步驟，最終形成可查詢的記憶體。

這樣的架構選擇使得記憶體的存取速度更快，並且能夠支持多種 LLM 提供者。選擇 AX Tree 作為主要捕捉方式的好處在於其結構化數據的處理效率，但可能在某些情況下無法提供足夠的視覺上下文。整體來看，這種設計在隱私和數據控制上取得了良好的平衡，但在擴展性上可能會受到 macOS 限制的影響。

## 技術深入分析

OpenChronicle 的核心技術機制在於利用 macOS 的 AX Tree 事件來捕捉上下文，這種方法比傳統的屏幕截圖和 OCR 更高效，因為 AX Tree 提供了結構化的文本信息，降低了處理成本。系統的效能特性使其能夠快速捕捉和存儲大量的上下文信息，並且使用 SQLite 進行快速檢索。設計上選擇 Python 和 Swift 作為主要開發語言，這使得在 macOS 環境下的整合更加順暢，但也限制了跨平台的能力。技術風險方面，隨著用戶數據量的增加，如何保持記憶體的查詢效率和存儲穩定性將是未來的一大挑戰。此外，與主流框架的整合難度相對較低，但對於不熟悉 macOS 的開發者來說，仍然需要一定的學習曲線。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝和使用指導，包含了必要的範例。安裝過程相對順暢，但需要安裝 Xcode Command Line Tools，這對新手來說可能是一個障礙。整體來說，花 30 分鐘應該能夠成功運行起來。

## 優缺點分析

> [!success] 優點
> - 本地存儲，確保數據隱私。
> - 靈活支持多種 LLM 提供者，增加使用場景。
> - Markdown 格式易於閱讀和編輯，便於用戶管理記憶體。

> [!danger] 缺點
> - 僅支持 macOS，限制了用戶群。
> - 目前處於 alpha 階段，功能尚不穩定。
> - 需要安裝 Xcode Command Line Tools，對新手不友好。

> [!warning] 注意事項
> - 僅支援 macOS 13+。
> - 目前處於 alpha 階段，功能尚不穩定。
> - 需要安裝 Xcode Command Line Tools，對新手可能造成困難。
> - 目前僅支持 AX Tree 捕捉，未來可能會增加其他捕捉方式。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於生成代理的記憶體管理，但不支持本地存儲，適合需要雲端解決方案的用戶。 |
| [AgriciDaniel/claude-obsidian](https://github.com/AgriciDaniel/claude-obsidian) | 提供類似的記憶體功能，但主要針對 Obsidian 用戶，無法直接與其他 LLM 代理整合。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供類似的記憶體功能，但主要針對雲端存儲，無法直接在本地運行。 | 如果你的團隊需要雲端解決方案並且不介意數據隱私問題。 | medium，因為需要重新設計數據存儲和訪問方式。 |
| [AgriciDaniel/claude-obsidian](https://github.com/AgriciDaniel/claude-obsidian) | 專注於 Obsidian 的集成，無法直接與其他 LLM 代理整合。 | 如果你的團隊已經在使用 Obsidian 並希望擴展其功能。 | low，因為可以直接在 Obsidian 環境中使用。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **OpenChronicle** | **boneyard** | **claude-obsidian** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 提供類似的記憶體功能，但主要針對雲端存儲，無法直接在本地運行。 | 專注於 Obsidian 的集成，無法直接與其他 LLM 代理整合。 |
> | 遷移成本 | - | medium，因為需要重新設計數據存儲和訪問方式。 | low，因為可以直接在 Obsidian 環境中使用。 |
> | 適用場景 | 主要場景 | 如果你的團隊需要雲端解決方案並且不介意數據隱私問題。 | 如果你的團隊已經在使用 Obsidian 並希望擴展其功能。 |
...
- **[HIGH]** 安裝過程中可能會遇到 Xcode Command Line Tools 未安裝的問題
  - 解法：確保運行 `xcode-select --install` 安裝命令
- [MEDIUM] 在捕捉上下文時，AX Tree 事件可能無法捕捉所有應用的狀態
  - 解法：定期檢查和更新應用的權限設置
- [low] Markdown 文件可能會因為長時間運行而變得冗長
  - 解法：定期清理和整理記憶體文件
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型開發團隊需要本地記憶體系統 | 非常適合 | 本地存儲和靈活的模型支持使其非常適合開發環境。 |
| 大型企業需要雲端解決方案 | 不適合 | 目前僅支持本地運行，無法滿足雲端需求。 |
| 對數據隱私有高要求的個人用戶 | 非常適合 | 本地優先的設計確保了數據的隱私性。 |
| 新手開發者希望快速上手記憶體系統 | 普通 | 安裝過程中需要安裝 Xcode，對新手有一定的挑戰。 |
...
> [!warning] 安全性快速掃描
> 低風險：本工具不需要高權限，數據僅存儲在本地，對外部 API 的依賴程度低。
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["MCP Protocol","記憶體管理","本地優先"];
