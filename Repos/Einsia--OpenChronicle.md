...
use_case: "提供一個本地的、可檢視的記憶體系統，讓任何工具驅動的 LLM 代理能夠捕捉和存儲上下文信息。"
priority: high
ring: assess
...
star_history: "2026-04-26:1126,2026-04-26:1129,2026-04-27:1422,2026-04-27:1425,2026-04-28:1593,2026-04-28:1596,2026-04-29:1809,2026-04-30:1885,2026-05-01:1933,2026-05-02:1983,2026-05-03:2024,2026-05-04:2063,2026-05-05:2090,2026-05-06:2097,2026-05-07:2105,2026-05-08:2109"
tags:
  - github
  - "category/開發工具"
  - "lang/python"
...
aliases:
  ...
  - "提供一個本地的、可檢視的記憶體系統，讓任何工具驅動的 LLM 代理能夠捕捉和存儲上下文信息。"
---

# OpenChronicle

**2.1k** stars · **132** stars/天 · 建立 16 天前 · Python · MIT
...
> **授權** MIT (商業友好) · **維護** Moderate (最後推送 11 天前) · **貢獻者** 5+ 人 · **參與度** Low
> **適合** 需要在 macOS 上建立本地優先的 LLM 記憶體系統的開發者。
> **一句話重點** OpenChronicle 提供了一個靈活且可檢視的本地記憶體系統，適合需要上下文捕捉的 LLM 代理。
...
> **結論** 花 5 小時學習，3 小時整合，得到一個靈活的本地記憶體系統，值得嘗試。

> [!abstract] 核心創新
> 提供一個本地優先的、可檢視的記憶體系統，專為工具驅動的 LLM 代理設計。

## 專案簡介

OpenChronicle 是一個本地優先的記憶體系統，專為工具驅動的 LLM 代理設計。它透過 macOS 的 AX 事件捕捉用戶的操作上下文，並將其轉換為持久的 Markdown 記憶體，包含用戶正在處理的內容、決策、使用的工具及相關人員或項目。這個系統的賣點在於其本地存儲和可檢視性，讓用戶能夠隨時檢查和管理自己的記憶體。技術上，OpenChronicle 使用 SQLite 進行本地索引，並支持 Markdown 格式的記憶體文件，這使得記憶體的讀取和編輯變得直觀。與 OpenAI Chronicle 相比，OpenChronicle 是開源的，並且不依賴於特定的模型或工具，這使得它在靈活性上有顯著優勢。

它的 AX Tree 優先捕捉方式相較於傳統的截圖和 OCR 方法，能夠以更低的成本和更高的準確度捕捉用戶的意圖，並且更容易進行去重和索引。實際使用中，記憶體的結構化存儲和會話感知的寫入方式能夠有效減少噪音，提升記憶體的質量。這個專案目前處於 alpha 階段，適合對個人 AI 記憶體或本地優先代理有興趣的開發者。未來的發展方向可能會集中在增強記憶體管理和擴展更多代理的整合上。

**技術棧**：`Python 3.11` · `Swift` · `Shell`

## 重點功能

- 本地優先存儲 — 記憶體數據存儲在本地，使用 SQLite 進行索引，保證數據隱私。
- AX Tree 捕捉 — 透過 macOS 的 AX 事件捕捉應用上下文，提供更準確的意圖識別。
- Markdown 格式存儲 — 記憶體以人類可讀的 Markdown 格式存儲，方便用戶檢視和編輯。
- 會話感知寫入 — 根據用戶操作的上下文進行記憶體寫入，減少噪音和冗餘。
- 多工具支持 — 兼容多種 LLM 模型，支持 MCP 協議的整合，靈活性高。
...
> [!tip] 爆紅原因
> 建立 16 天內累積 2109 stars（132/天），forks 200（9.5%），顯示出強烈的社群興趣。開發者 KMing-L 和其他貢獻者在開源領域有一定的影響力，這個專案提供了一個開放的替代方案，解決了現有記憶體系統的封閉性和模型依賴問題。近期的推廣活動和社群討論也可能促進了這個專案的關注度。技術上，macOS 的 AX 事件捕捉技術使得這個工具在本地記憶體管理上具備了前所未有的能力，這在現有市場上是相對少見的。高達 9.5% 的 forks/stars 比率顯示出許多開發者對這個專案的實際修改和使用意圖。

## 適合誰使用

**目標受眾**：需要在 macOS 上建立本地優先的 LLM 記憶體系統的開發者。

> [!example] 使用場景
> - 自由職業者用它來記錄每次與客戶的會議內容，因為它能夠自動捕捉上下文並生成 Markdown 格式的記錄，避免手動記錄的遺漏。
> - 開發者用它來追蹤在不同 IDE 中的開發進度，因為它能夠結合 AX Tree 和截圖，提供更全面的上下文記憶。
> - 研究人員用它來整理實驗過程中的關鍵決策和工具使用情況，因為它的結構化存儲方式能夠方便地進行查詢和分析。

## 架構分析

OpenChronicle 的架構設計以本地優先為核心，利用 macOS 的 AX 事件捕捉用戶操作。整個流程從捕捉上下文開始，然後將其壓縮為會話，提取持久的事實，並將記憶體存儲在本地的 SQLite 數據庫中。這樣的設計使得記憶體能夠快速檢索和查詢，並且在用戶需要時隨時可用。

選擇使用 Markdown 格式存儲記憶體，讓用戶能夠輕鬆檢視和編輯，這在其他記憶體系統中並不常見。這種設計的代價在於需要依賴 macOS 環境，限制了其跨平台的適用性。整體架構的擴展性良好，未來可以通過增加更多的上下文解析器和記憶體管理功能來進一步提升性能。

## 技術深入分析

OpenChronicle 的核心技術機制在於使用 macOS 的 AX 事件來捕捉用戶的操作上下文，這使得它能夠以低成本獲取結構化的文本信息。效能上，這種方法能夠快速處理並存儲用戶的操作記錄，並且通過 SQLite 進行高效的索引和檢索。選擇 Python 作為主要開發語言，因為它在數據處理和文本操作上有著強大的庫支持，並且 Swift 用於 macOS 特定的功能實現，這樣的選擇使得整體架構能夠保持輕量且高效。技術上，這個專案的依賴樹相對簡單，主要依賴於幾個關鍵庫，如 Typer 和 Rich，這使得維護和擴展變得容易。未來可能面臨的技術風險包括在擴展到更多平台時的兼容性問題，以及在處理大量數據時的性能瓶頸。與主流框架的整合難度較低，因為它提供了清晰的 API 和文檔，方便開發者進行集成。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝步驟和使用指令，並且有詳細的功能介紹。安裝過程相對順暢，但需要注意安裝 Xcode Command Line Tools。文件目前僅提供英文版本，對於非英語用戶可能會造成一些障礙。
...
> - 目前功能仍在開發中，可能不穩定。
> - 需要安裝 Xcode Command Line Tools，增加了安裝難度。

> [!warning] 注意事項
> - 僅支援 macOS 平台，無法在其他操作系統上運行。
> - 目前處於 alpha 階段，功能可能不穩定，API 可能會變動。
> - 需要安裝 Xcode Command Line Tools，對於不熟悉的用戶可能增加安裝難度。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於創建可視化代理，而 OpenChronicle 專注於記憶體管理和上下文捕捉。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 主要用於數據存儲和管理，缺乏 OpenChronicle 的上下文捕捉功能。 |
| [432539/gpt2api](https://github.com/432539/gpt2api) | 專注於 GPT-2 模型的 API 接入，並不具備本地記憶體管理的功能。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於創建可視化代理，缺乏 OpenChronicle 的記憶體管理功能。 | 如果你的專案需要可視化的代理而非記憶體管理，則應選擇它。 | medium，因為需要重新設計記憶體管理的邏輯。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 主要用於數據存儲和管理，缺乏上下文捕捉功能。 | 如果你的需求主要集中在數據存儲而非上下文捕捉，則應選擇它。 | low，因為兩者的功能有一定的重疊。 |
| [432539/gpt2api](https://github.com/432539/gpt2api) | 專注於 GPT-2 模型的 API 接入，並不具備本地記憶體管理的功能。 | 如果你的專案專注於 GPT-2 的使用，則應選擇它。 | high，因為需要重新設計整個記憶體管理流程。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **OpenChronicle** | **agent-sprite-forge** | **boneyard** | **gpt2api** |
> | --- | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於創建可視化代理，缺乏 OpenChronicle 的記憶體管理功能。 | 主要用於數據存儲和管理，缺乏上下文捕捉功能。 | 專注於 GPT-2 模型的 API 接入，並不具備本地記憶體管理的功能。 |
> | 遷移成本 | - | medium，因為需要重新設計記憶體管理的邏輯。 | low，因為兩者的功能有一定的重疊。 | high，因為需要重新設計整個記憶體管理流程。 |
> | 適用場景 | 主要場景 | 如果你的專案需要可視化的代理而非記憶體管理，則應選擇它。 | 如果你的需求主要集中在數據存儲而非上下文捕捉，則應選擇它。 | 如果你的專案專注於 GPT-2 的使用，則應選擇它。 |
...
- **[HIGH]** 在 macOS 更新後，AX 事件捕捉可能會失效
  - 解法：定期檢查 macOS 更新並更新 OpenChronicle
- [MEDIUM] Markdown 格式的記憶體在某些情況下可能無法正確顯示
  - 解法：檢查 Markdown 語法並使用支持 Markdown 的編輯器
- [MEDIUM] 安裝過程中可能會遇到 Xcode Command Line Tools 的安裝問題
  - 解法：確保已安裝最新版本的 Xcode Command Line Tools

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 能夠有效管理和捕捉開發過程中的上下文信息，提升團隊協作效率。 |
| 大型企業的數據分析團隊 | 不適合 | 目前僅支持 macOS，無法滿足跨平台需求。 |
| 自由職業者的個人項目 | 適合 | 能夠輕鬆捕捉和管理個人工作記錄，提升工作效率。 |
| 學術研究團隊的實驗記錄 | 普通 | 雖然能夠捕捉上下文，但目前功能仍在開發中，穩定性不足。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，3 小時整合，得到一個靈活的本地記憶體系統，值得嘗試。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：本工具在本地運行，不需要高權限，且不存取敏感資料。依賴鏈的信任程度良好，無已知的供應鏈風險。
...
相關概念：[[MCP Protocol]] · [[記憶體管理]] · [[本地優先架構]]

相關專案：[[0x0funky--agent-sprite-forge|0x0funky/agent-sprite-forge]] · [[0xGF--boneyard|0xGF/boneyard]] · [[432539--gpt2api|432539/gpt2api]] · [[AgriciDaniel--claude-obsidian|AgriciDaniel/claude-obsidian]] · [[MemPalace--mempalace|MemPalace/mempalace]] · [[milla-jovovich--mempalace|milla-jovovich/mempalace]] · [[BuilderPulse--BuilderPulse|BuilderPulse/BuilderPulse]] · [[AgentSeal--codeburn|AgentSeal/codeburn]]
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["MCP Protocol","記憶體管理","本地優先架構"];
