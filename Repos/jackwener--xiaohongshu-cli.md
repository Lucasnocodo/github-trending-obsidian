...
star_history: "2026-03-11:389,2026-03-13:786,2026-03-13:791,2026-03-14:921,2026-03-14:923,2026-03-15:1080,2026-03-15:1082,2026-03-16:1177,2026-03-18:1268,2026-03-19:1304,2026-03-20:1331,2026-03-21:1343,2026-03-22:1370,2026-03-24:1402,2026-03-25:1418,2026-04-07:1551"
tags:
  - github
  - "category/cli_工具"
  - "lang/python"
...
**1.6k** stars · **53** stars/天 · 建立 29 天前 · Python · 未標註授權
...
> **適合** 希望在 CLI 中高效管理小紅書內容的社交媒體管理者和開發者。
> **一句話重點** xiaohongshu-cli 透過反向工程的 API 提供了一個強大的 CLI 工具，讓用戶能夠高效地互動小紅書內容。
...
> **結論** 花 5 小時學、2 小時整合，得到高效的社交媒體管理工具，值得投入。

> [!abstract] 核心創新
> 提供了一個反向工程的 API 來支持小紅書的 CLI 互動。

## 專案簡介

xiaohongshu-cli 是一個用於小紅書的命令行工具，能夠進行搜尋、閱讀和互動。使用者可以透過 `xhs login` 指令自動提取瀏覽器的 cookies，或使用 QR 碼進行登錄，這樣的設計能夠簡化身份驗證過程。工具支持多種搜尋功能，如關鍵字搜尋、用戶搜尋和主題搜尋，並且能夠按流行程度或最新進行排序，這使得用戶能快速找到感興趣的內容。除了閱讀筆記，使用者還可以查看評論、用戶資料以及推薦的熱點內容，這些功能都能透過簡單的 CLI 指令實現。這個工具的賣點在於其反向工程的 API，能夠提供穩定的訪問和互動能力，並且在使用過程中具備反檢測機制，降低被平台檢測的風險。

技術上，該工具使用 Python 開發，依賴於 `httpx`、`click` 和 `rich` 等庫，這些選擇使得 CLI 的使用體驗更加豐富和友好。它的依賴相對輕量，適合快速上手和部署。與其他類似工具相比，如 `bilibili-cli` 和 `twitter-cli`，xiaohongshu-cli 提供了更強的社交互動功能，能夠進行點贊、收藏和評論等操作，這些在其他平台的 CLI 工具中可能不那麼全面。



在實際使用中，該工具能夠處理大量的請求，並且具備自動重試和延遲機制，這對於避免被封鎖非常重要。使用者可能會遇到一些常見問題，如在 Windows 環境下的編碼問題，這需要特別注意。社群活躍度不高，解決問題的速度也有待提高，這可能會影響使用者的體驗。

這個專案目前處於 alpha 階段，適合個人或小型團隊進行試用，但不建議在生產環境中使用。預計未來會持續增強功能，特別是在社交互動和反檢測方面。

適合的使用情境包括：需要在 CLI 中快速查找小紅書內容的社交媒體管理者，或是開發者希望自動化小紅書的內容互動；不適合的情境則是對於穩定性要求極高的商業應用。整合成本預估在 5 小時學習和 2 小時整合，得到的效果是能夠快速進行小紅書內容的管理和互動。

**技術棧**：`Python 3.10` · `httpx` · `click` · `rich`

## 重點功能

- 自動登錄 — 支持瀏覽器 cookie 提取和 QR 碼登錄，簡化身份驗證過程。
- 多種搜尋功能 — 支持關鍵字、用戶和主題搜尋，並可按流行程度排序。
- 社交互動 — 能夠進行點贊、收藏、評論和關注等操作，增強用戶互動體驗。
- 反檢測機制 — 包含高級的請求延遲和指紋穩定性設計，降低被檢測風險。
- 結構化輸出 — 支持 YAML 和 JSON 格式的輸出，方便用戶進行數據處理。
...
> [!tip] 爆紅原因
> 建立 29 天內累積 1551 stars（53/天），forks 157（10.1%），顯示出穩定的增長潛力。作者 jackwener 過去有開發多個 CLI 工具的經驗，這使得他能夠針對小紅書的需求提供一個專業的解決方案。這個工具解決了小紅書用戶在 CLI 環境下無法方便互動的痛點，之前用戶只能依賴瀏覽器進行操作，效率低下。社群的反饋和需求推動了這個工具的發展，特別是在反向工程 API 的使用上。這個工具的高 forks/stars 比率（10.1%）顯示出許多人在實際修改和使用，反映出其實用性和潛力。

## 適合誰使用

**目標受眾**：希望在 CLI 中高效管理小紅書內容的社交媒體管理者和開發者。

> [!example] 使用場景
> - 社交媒體管理者用它來自動搜尋小紅書上的熱門話題，因為手動搜尋效率低且容易錯過重要信息。
> - 開發者用它來測試小紅書的 API，因為 CLI 工具能快速進行多次請求，便於調試和驗證。
> - 內容創作者用它來管理自己的筆記和互動，因為能夠快速進行點贊和評論，提升互動效率。

## 架構分析

xiaohongshu-cli 採用 Python 開發，使用了多個輕量級庫來實現其功能。整體架構設計為一個命令行工具，通過反向工程的 API 進行數據交互。資料流方面，使用者的請求通過 HTTP 請求發送到小紅書的 API，並返回相應的數據。

這樣的設計使得工具能夠快速響應用戶的需求，並且能夠在不同的操作系統上運行。選擇 Python 是因為其在 CLI 工具開發中的普遍性和豐富的庫支持，這樣的選擇使得開發和維護變得更加高效。擴展性方面，隨著小紅書 API 的變化，工具的功能可能需要不斷更新，這對於維護者來說是一個挑戰。

## 技術深入分析

xiaohongshu-cli 的核心技術機制在於其反向工程的 API 設計，這使得用戶能夠在命令行中進行豐富的互動。使用了 `httpx` 來處理 HTTP 請求，這提供了非同步請求的能力，能夠提高性能和響應速度。工具的設計考慮到了用戶的使用習慣，提供了簡單的命令行介面，並且支持多種輸出格式，這對於開發者和社交媒體管理者來說非常友好。效能方面，該工具能夠處理大量請求，並且具備自動重試和延遲機制，這對於避免被封鎖非常重要。設計取捨方面，選擇 Python 作為開發語言使得工具易於維護，但可能在性能上不如某些編譯型語言。技術風險方面，隨著小紅書 API 的變化，工具的穩定性可能受到影響，這需要維護者持續關注。整合方面，該工具能夠輕鬆與現有的 CLI 工具鏈結合，並且支持與 CI/CD 流程的整合，這對於開發者來說是一個加分項。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝和使用指導，包含必要的範例。安裝過程相對順暢，但在 Windows 環境下可能會遇到編碼問題。文件目前僅提供英文和中文，對於不同語言的使用者來說可能需要額外的翻譯支持。

## 優缺點分析

> [!success] 優點
> - 功能全面，支持多種社交互動操作。
> - 反檢測機制設計良好，降低被平台檢測的風險。
> - 支持結構化輸出，方便用戶進行數據處理。

> [!danger] 缺點
> - 目前處於 alpha 階段，功能和穩定性可能不夠成熟。
> - 在特定環境下（如 Windows）可能會遇到編碼問題。
> - 社群活躍度不高，問題解決速度較慢。

> [!warning] 注意事項
> - 僅支援 Python 3.10+
> - 在 Windows 環境下可能會遇到編碼問題，特別是使用 QR 碼登錄時。
> - 目前處於 alpha 階段，功能和 API 穩定性可能會變化。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [jackwener/bilibili-cli](https://github.com/jackwener/bilibili-cli) | 專注於 Bilibili 內容的 CLI 工具，提供視頻和用戶互動功能，但缺乏小紅書的社交互動特性。 |
| [jackwener/twitter-cli](https://github.com/jackwener/twitter-cli) | 針對 Twitter 的 CLI 工具，支持時間線和書籤，但在小紅書的特定功能上不具優勢。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [jackwener/bilibili-cli](https://github.com/jackwener/bilibili-cli) | 專注於 Bilibili 內容的 CLI 工具，提供視頻和用戶互動功能，但缺乏小紅書的社交互動特性。 | 如果你的主要需求是管理 Bilibili 內容，且需要視頻支持，則應選擇此工具。 | low，因為兩者都使用 CLI 操作，且有相似的使用模式。 |
| [jackwener/twitter-cli](https://github.com/jackwener/twitter-cli) | 針對 Twitter 的 CLI 工具，支持時間線和書籤，但在小紅書的特定功能上不具優勢。 | 如果你的工作重心在 Twitter 上，且需要時間線管理，則應選擇此工具。 | medium，因為需要重新適應不同的 API 和命令格式。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **xiaohongshu-cli** | **bilibili-cli** | **twitter-cli** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於 Bilibili 內容的 CLI 工具，提供視頻和用戶互動功能，但缺乏小紅書的社交互動特性。 | 針對 Twitter 的 CLI 工具，支持時間線和書籤，但在小紅書的特定功能上不具優勢。 |
> | 遷移成本 | - | low，因為兩者都使用 CLI 操作，且有相似的使用模式。 | medium，因為需要重新適應不同的 API 和命令格式。 |
> | 適用場景 | 主要場景 | 如果你的主要需求是管理 Bilibili 內容，且需要視頻支 | 如果你的工作重心在 Twitter 上，且需要時間線管理，則 |
...
- **[HIGH]** 在 Windows 環境下使用 QR 碼登錄時可能會崩潰，導致無法登錄。
  - 解法：使用其他操作系統或手動登錄。
- [MEDIUM] 某些 API 請求可能會因為反向工程不完全而失敗。
  - 解法：定期更新工具以獲取最新修復。
- [MEDIUM] 使用過程中可能會遇到 cookie 失效的情況。
  - 解法：定期檢查登錄狀態並重新登錄。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 適合 | 能夠快速進行小紅書內容的管理和互動，適合小型團隊使用。 |
| 大型企業的社交媒體管理 | 普通 | 雖然功能全面，但目前處於 alpha 階段，穩定性可能不足。 |
| 個人開發者的社交媒體工具 | 非常適合 | 提供了強大的 CLI 功能，能夠快速實現個人需求。 |
| 需要高穩定性的商業應用 | 不適合 | 目前處於 alpha 階段，功能和穩定性不夠成熟。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學、2 小時整合，得到高效的社交媒體管理工具，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 該工具不需要高權限運行，主要使用瀏覽器的 cookies 進行身份驗證，對敏感資料的存取相對有限。依賴鏈中有多個已知的庫，需定期檢查安全性。
...
> | [#40](https://github.com/jackwener/xiaohongshu-cli/issues/40) | xhs favorites API error `bug` | 0 | 0 |
