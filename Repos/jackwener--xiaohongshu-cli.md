...
use_case: "提供小紅書的 CLI 工具，讓用戶能夠搜尋、閱讀和互動，透過反向工程的 API 實現。"
priority: high
ring: assess
...
star_history: "2026-03-11:389,2026-03-13:786,2026-03-13:791,2026-03-14:921,2026-03-14:923,2026-03-15:1080,2026-03-15:1082,2026-03-16:1177,2026-03-18:1268,2026-03-19:1304,2026-03-20:1331,2026-03-21:1343,2026-03-22:1370,2026-03-24:1402,2026-03-25:1418,2026-04-07:1551,2026-04-08:1561,2026-04-10:1580"
tags:
  - github
  - "category/cli_工具"
  - "lang/python"
aliases:
  ...
  - "提供小紅書的 CLI 工具，讓用戶能夠搜尋、閱讀和互動，透過反向工程的 API 實現。"
---

# xiaohongshu-cli

**1.6k** stars · **49** stars/天 · 建立 32 天前 · Python · 未標註授權
...
> **適合** 需要在命令列環境中高效管理小紅書內容的數據分析師或內容創作者。
> **一句話重點** 這個 CLI 工具不僅提供了小紅書的基本操作，還考量了反檢測和自動化的需求，讓用戶能夠更靈活地管理內容。
...
> [!abstract] 核心創新
> 透過反向工程的 API 提供全面的小紅書命令列操作，支持多種社交互動功能。

## 專案簡介

xiaohongshu-cli 是一個用於小紅書的命令列介面工具，允許用戶透過反向工程的 API 進行搜尋、閱讀和互動。用戶可以使用 `xhs login` 命令自動提取瀏覽器 cookies，或透過 QR 碼登入，並能夠檢查登入狀態。搜尋功能支持關鍵字、用戶和主題的查詢，並可透過 `--sort` 和 `--type` 參數進行篩選。閱讀功能則允許用戶查看筆記詳細內容及其評論，並可通過短索引導航快速訪問最近的結果。該工具還支持社交互動，如關注、收藏、讚和評論，並提供通知功能以跟蹤未讀消息。其設計考量了反檢測機制，使用高斯抖動和自動重試來模擬自然瀏覽行為，減少被檢測的風險。這個 CLI 工具的賣點在於其全面的功能和對小紅書 API 的深入整合，讓用戶能夠高效地進行內容管理和互動。

**技術棧**：`Python 3.10` · `httpx` · `click` · `rich`

## 重點功能

- 自動登入 — 通過提取瀏覽器 cookies 或 QR 碼登入，支持多種瀏覽器。
- 搜尋功能 — 支持關鍵字、用戶和主題的搜尋，並可用 `--sort` 和 `--type` 參數篩選結果。
- 社交互動 — 允許用戶關注、收藏、讚、評論和刪除內容，提升用戶互動性。
- 通知系統 — 提供未讀消息計數和通知，幫助用戶跟蹤互動。
- 反檢測機制 — 使用高斯抖動和自動重試來模擬自然行為，減少被檢測的風險。
...
> [!tip] 爆紅原因
> 建立 32 天內累積 1580 stars（49/天），forks 159（10.1%），顯示出穩定的增長趨勢。作者 jackwener 過去有開發多個 CLI 工具的經驗，這個專案解決了小紅書用戶在使用官方應用時的限制，特別是在自動化和批量操作方面。最近的 commit 活動顯示出活躍的開發進度，並且社群對於 captcha 和 Windows 環境的問題表現出關注，顯示出實際使用中的痛點。

## 適合誰使用

**目標受眾**：需要在命令列環境中高效管理小紅書內容的數據分析師或內容創作者。

> [!example] 使用場景
> - 內容創作者用它來批量管理小紅書筆記，因為可以快速發佈、編輯和刪除內容，節省大量時間。
> - 數據分析師用它來提取小紅書上的熱門話題和用戶互動數據，因為可以透過 API 獲取結構化數據進行分析。
> - 社交媒體經理用它來監控品牌提及和用戶反饋，因為可以快速查詢和回應評論，提升品牌互動率。

## 架構分析

這個專案採用 Python 作為主要開發語言，並使用了多個庫來實現功能，如 httpx 用於 HTTP 請求，click 用於命令列介面設計。資料流方面，使用者的請求會經過身份驗證後發送到小紅書的 API，並根據返回的數據進行處理。選擇 Python 是因為其生態系統豐富且易於快速開發，但這也意味著可能面臨性能瓶頸，特別是在高頻請求的情況下。擴展性方面，這個工具的設計使得未來可以輕鬆添加新功能，但也需要注意 API 的變動可能會影響現有功能的穩定性。

## 技術深入分析

xiaohongshu-cli 的核心技術機制是透過反向工程的 API 來實現與小紅書的互動。使用 httpx 庫進行非同步 HTTP 請求，確保高效能和低延遲。該工具的設計考量了用戶的安全性，實施了多種反檢測措施，如高斯抖動和自動重試機制，這些措施能夠有效降低被檢測的風險。從性能角度來看，該工具能夠處理多個請求，但在高頻請求的情況下可能會遇到瓶頸。選擇 Python 作為開發語言使得開發過程快速，但也可能導致在性能上不如某些編譯語言。設計上，這個工具的依賴樹相對簡單，主要依賴於幾個核心庫，降低了維護的複雜度。對於未來的擴展，這個工具的架構設計使得添加新功能變得容易，但也需注意 API 的變動可能會帶來的影響。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了詳細的安裝和使用指南。安裝過程相對順暢，但在某些環境下可能會遇到 captcha 驗證問題。文件中有多語言支持，但主要以英文為主，中文部分較少。
...
> - 功能全面，支持多種社交互動操作。
> - 設計考量反檢測機制，降低被封鎖的風險。
> - 使用簡單，命令直觀，適合 CLI 使用者。

> [!danger] 缺點
> - 仍在 alpha 階段，穩定性和功能可能會有變動。
> - 在某些環境下可能會遇到 captcha 驗證問題。
> - 依賴於小紅書的 API，若 API 改變可能影響工具的功能。
...
| [jackwener/twitter-cli](https://github.com/jackwener/twitter-cli) | 針對 Twitter 的命令列工具，專注於時間線和書籤管理，與小紅書的功能範圍不同。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於數據提取和分析，提供更強大的數據處理功能，但缺乏社交互動功能。 | 如果需要強大的數據分析功能而非社交互動，則可選擇此工具。 | medium，因為需要重新學習命令和功能。 |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 以同步為主的工具，適合需要簡單操作的用戶，但不支持複雜的社交互動。 | 如果只需要基本的內容管理而不需要社交互動，則可選擇此工具。 | low，因為功能簡單，易於上手。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **xiaohongshu-cli** | **boneyard** | **parsync** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於數據提取和分析，提供更強大的數據處理功能，但缺乏社交互動功能。 | 以同步為主的工具，適合需要簡單操作的用戶，但不支持複雜的社交互動。 |
> | 遷移成本 | - | medium，因為需要重新學習命令和功能。 | low，因為功能簡單，易於上手。 |
> | 適用場景 | 主要場景 | 如果需要強大的數據分析功能而非社交互動，則可選擇此工具。 | 如果只需要基本的內容管理而不需要社交互動，則可選擇此工具。 |
...
- **[HIGH]** 在某些環境下可能會遇到 captcha 驗證問題，導致無法正常登入。
  - 解法：嘗試使用不同的瀏覽器或手動登入。
- [MEDIUM] Windows 環境下可能會出現編碼錯誤，導致程式崩潰。
  - 解法：使用 UTF-8 編碼的終端。
- **[HIGH]** API 變更可能導致某些功能失效。
  - 解法：定期檢查更新和社群討論。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型創業團隊的社交媒體管理 | 非常適合 | 提供全面的社交互動功能，能夠高效管理內容。 |
| 個人用戶希望自動化小紅書內容管理 | 適合 | 命令列操作簡單，適合自動化腳本使用。 |
| 大型企業的社交媒體監控 | 不適合 | 目前仍在 alpha 階段，穩定性不足。 |
...
> [!warning] 安全性快速掃描
> 安全性評估中等，該工具需要存取用戶的瀏覽器 cookies，使用時需注意隱私保護。依賴於小紅書的 API，若 API 發生變更，可能影響工具的正常運作。
