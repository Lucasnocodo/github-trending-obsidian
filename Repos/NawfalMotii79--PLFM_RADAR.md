...
use_case: "開源、低成本的 10.5 GHz PLFM 相控陣雷達系統。"
priority: high
ring: assess
...
aliases:
  ...
  - "開源、低成本的 10.5 GHz PLFM 相控陣雷達系統。"
---

# PLFM_RADAR

**3.9k** stars · **112** stars/天 · 建立 35 天前 · PLSQL · NOASSERTION
...
> **適合** 對相控陣雷達技術有興趣的研究者和開發者，尤其是無人機和信號處理領域的專業人士。
> **一句話重點** AERIS-10 不僅是一個雷達系統，更是一個開放的實驗平台，讓更多人能夠探索雷達技術的潛力。
...
> [!abstract] 核心創新
> AERIS-10 提供了一個完全開源的相控陣雷達系統，讓用戶能夠自由修改和擴展。

## 專案簡介

AERIS-10 是一個開源的 10.5 GHz 相控陣雷達系統，專為研究者和無人機開發者設計，提供 3km 和 20km 的兩個版本。其核心機制是透過脈衝線性頻率調變（LFM）來生成雷達信號，並使用 FPGA 進行信號處理，最終將數據通過 Python GUI 進行可視化。使用者只需安裝 Python 3.8+ 和 FPGA 開發工具（Vivado），即可進行硬體組裝和信號處理的修改。這個專案的賣點在於其完全開源的硬體和軟體設計，讓使用者能夠自由修改和擴展系統功能。技術上，AERIS-10 使用了高效能的 FPGA 來處理脈衝壓縮、Doppler FFT 和目標追蹤等信號處理任務，並且支持 GPS/IMU 整合以進行實時位置和姿態修正。

與傳統雷達系統相比，這個系統的模組化設計使得用戶可以根據需求進行定制和擴展。相較於其他類似工具，如 0xGF/boneyard 和 AlpinDale/parsync，AERIS-10 提供了更為完整的開源硬體設計，並且在信號處理和用戶界面上有更高的可操作性。這使得它特別適合於學術研究和開發原型，尤其是在需要快速迭代和實驗的環境中。使用者在實際操作中可能會遇到一些硬體組裝的挑戰，但社群的活躍度和文檔的完整性能夠有效降低這些障礙。這個專案目前處於穩定階段，適合中小型團隊進行開發和實驗，未來可能會持續增強其功能和社群支持。
...
> [!tip] 爆紅原因
> 建立 35 天內累積 3906 stars（112/天），forks 840（21.5%），顯示出強烈的社群興趣。這個專案由 NawfalMotii79 和其他貢獻者共同開發，解決了傳統雷達系統高成本和封閉性問題，讓更多人能夠接觸和實驗雷達技術。近期的推文和社群討論也促進了其曝光率，尤其是在無人機和信號處理領域。高達 21.5% 的 forks/stars 比率顯示出許多開發者對此專案進行了實際修改和使用，這是其受歡迎的原因之一。

## 適合誰使用

**目標受眾**：對相控陣雷達技術有興趣的研究者和開發者，尤其是無人機和信號處理領域的專業人士。

> [!example] 使用場景
> - 無人機開發者用它來實現自主飛行中的目標追蹤，因為其高精度的信號處理能力能夠提供即時的環境數據。
> - 學術研究者用它來進行雷達信號處理的實驗，因為開源的設計讓他們能夠自由修改和擴展系統功能。
> - 工程師用它來開發新型的雷達應用，因為其模組化設計使得快速迭代和測試成為可能。

## 架構分析

AERIS-10 的架構設計為模組化，主要由電源管理板、頻率合成器板和主板組成。電源管理板確保所有電子元件的電壓供應和過濾，頻率合成器板使用高性能的時鐘生成器來提供相位對齊的時鐘參考。主板上集成了 DAC、微波混頻器和相位移器，這些元件共同實現了雷達信號的生成和處理。

選擇 FPGA 來進行信號處理是因為其高效能和靈活性，能夠快速處理大量數據。這種設計的代價在於需要較高的硬體組裝和配置知識，對於新手來說可能會有一定的學習曲線。整體架構的擴展性良好，能夠根據需求添加更多功能模組。

## 技術深入分析

AERIS-10 的核心技術機制依賴於 FPGA 進行高效的信號處理，使用了先進的脈衝壓縮和 Doppler FFT 演算法。這使得系統能夠在實時環境中捕捉和分析雷達數據。效能方面，FPGA 的並行處理能力使得系統能夠快速處理大量數據，並且支持多種信號處理任務，如自動增益控制和 I/Q 基帶下變換。設計上選擇 FPGA 而非傳統的微處理器，因為 FPGA 在處理速度和靈活性上有明顯優勢，但這也意味著需要更高的技術門檻來進行配置和開發。技術風險方面，隨著使用者數量的增加，可能會面臨硬體組裝和配置的挑戰，特別是對於新手來說。整合方面，AERIS-10 能夠與多種主流開發環境和工具鏈相容，但對於某些特定的 CI/CD 流程可能需要額外的適配。整體而言，AERIS-10 提供了一個強大的平台，適合進行各種雷達技術的實驗和開發。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了詳細的安裝步驟和使用說明，對於新手來說相對友好。安裝過程中需要安裝 Python 和 FPGA 開發工具，這可能是個挑戰。整體上，文件清晰且有範例，能夠幫助使用者快速上手。
...
| 工具 | 差異 |
| --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard 提供的是一個多用途的開源硬體平台，但缺乏專注於雷達信號處理的功能，適合更廣泛的應用場景。 |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | Parsync 專注於數據同步和傳輸，但不具備 AERIS-10 的雷達信號處理能力，適合需要高效數據處理的場景。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard 提供的是一個多用途的開源硬體平台，但缺乏專注於雷達信號處理的功能。 | 如果你需要一個通用的開源硬體平台，而不僅僅是雷達應用。 | medium，因為需要重新設計信號處理部分。 |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | Parsync 專注於數據同步和傳輸，但不具備 AERIS-10 的雷達信號處理能力。 | 如果你的需求是高效的數據處理，而非雷達應用。 | low，因為可以直接使用現有的數據處理流程。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PLFM_RADAR** | **boneyard** | **parsync** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | Boneyard 提供的是一個多用途的開源硬體平台，但缺乏專注於雷達信號處理的功能。 | Parsync 專注於數據同步和傳輸，但不具備 AERIS-10 的雷達信號處理能力。 |
> | 遷移成本 | - | medium，因為需要重新設計信號處理部分。 | low，因為可以直接使用現有的數據處理流程。 |
> | 適用場景 | 主要場景 | 如果你需要一個通用的開源硬體平台，而不僅僅是雷達應用。 | 如果你的需求是高效的數據處理，而非雷達應用。 |
...
- [MEDIUM] FPGA 配置可能需要多次調試才能正常運行。
  - 解法：參考社群提供的配置範例，並進行逐步測試。
- [MEDIUM] Python GUI 在某些環境下可能會出現兼容性問題。
  - 解法：確保使用最新版本的 Python 和相關庫。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 大學研究實驗室進行雷達信號處理研究 | 非常適合 | 提供完整的開源設計，便於修改和實驗。 |
| 小型無人機開發團隊需要集成雷達功能 | 適合 | 模組化設計便於快速迭代和測試。 |
| 大型企業需要穩定的商業雷達解決方案 | 不適合 | 目前仍處於 beta 階段，穩定性和支持可能不足。 |
...
> | [#57](https://github.com/NawfalMotii79/PLFM_RADAR/issues/57) | ه | 0 | 0 |
