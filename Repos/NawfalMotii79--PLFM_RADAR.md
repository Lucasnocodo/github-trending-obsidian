...
**18.7k** stars · **360** stars/天 · 建立 52 天前 · PLSQL · NOASSERTION
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Growing · **熱度** Hot (360 stars/day)
> **授權** NOASSERTION · **維護** Active (最後推送 0 天前) · **貢獻者** 5+ 人 · **參與度** Medium
> **適合** 希望探索相控陣雷達技術的研究人員和開發者，特別是對無人機和信號處理有興趣的人。
...
AERIS-10是一個開源的相控陣雷達系統，工作於10.5 GHz，支持脈衝線性頻率調製（LFM）。用戶可以選擇3公里或20公里的量測範圍，並且系統設計為模組化，便於研究人員和開發者進行實驗。系統的核心功能包括電子波束控制、先進的信號處理和Python GUI介面，能夠實時顯示目標位置。這個雷達系統的設計使用FPGA進行信號處理，並整合了GPS和IMU進行位置和姿態校正。

其架構中，FPGA負責脈衝壓縮、Doppler FFT、MTI和CFAR處理，並且支持多種信號處理功能。與傳統雷達系統相比，AERIS-10的開源特性使得其設計和實作更具透明度和可擴展性，特別適合學術研究和創新開發。使用者可以透過簡單的Python指令來控制雷達操作，並且可以根據需求進行硬體和軟體的自定義。這個系統的賣點在於其低成本和開放性，讓更多人能夠進入雷達技術的領域。

**技術棧**：`Python 3.8+` · `Vivado` · `FPGA`

## 重點功能

- 開源硬體與軟體 — 完整的電路圖、PCB佈局、韌體和軟體皆可獲得。
- 雙版本可選 — AERIS-10N（3公里範圍）和AERIS-10E（20公里範圍）。
- 全電子波束控制 — 在仰角和方位角上±45°的電子控制。
- 先進的信號處理 — FPGA負責脈衝壓縮、Doppler FFT、MTI和CFAR處理。
- Python GUI — 提供友好的用戶介面，並整合地圖功能。
- GPS/IMU整合 — 實時位置和姿態校正。
...
> [!tip] 爆紅原因
> 建立52天內累積18724 stars（360/天），forks達4481（23.9%），顯示出強大的社群參與度。專案的主要貢獻者來自不同背景，提供了多樣的技術支持。這個專案解決了傳統雷達系統高成本和封閉的問題，讓研究者和開發者能夠以低成本進行雷達技術的探索。社群的活躍度和開放的問題討論也促進了專案的快速成長。技術上，這個系統的開源特性使得其設計和實作更具透明度和可擴展性，特別適合學術研究和創新開發。

## 適合誰使用

**目標受眾**：希望探索相控陣雷達技術的研究人員和開發者，特別是對無人機和信號處理有興趣的人。

> [!example] 使用場景
> - 無人機開發者用它來實現自動導航和避障，因為其GPS/IMU整合能提供精確的位置信息。
> - 學術研究人員用它來進行雷達信號處理的實驗，因為開源設計能讓他們自由修改和擴展功能。
> - 電子工程師用它來測試新型雷達技術，因為其模組化設計方便快速迭代和測試不同的硬體組件。

## 架構分析

AERIS-10的架構設計為模組化，分為電源管理、頻率合成和主板等子系統。每個子系統都獨立運作，並通過微控制器進行協調。信號處理主要在FPGA上進行，這樣的設計使得系統能夠靈活應對不同的信號處理需求。

選擇FPGA的原因在於其高效能和可編程性，能夠實現複雜的信號處理演算法。這樣的架構雖然增加了設計的複雜性，但卻能提供更高的性能和靈活性。擴展性方面，模組化設計允許用戶根據需求添加或替換硬體組件，這對於進行實驗和創新至關重要。

## 技術深入分析

AERIS-10的核心技術機制在於其FPGA信號處理，使用VHDL/Verilog語言實現複雜的信號處理演算法，如脈衝壓縮和Doppler FFT。這些演算法能夠在400 MHz的時鐘頻率下高效運行，確保雷達系統能夠實時處理來自ADC的數據。設計上，選擇FPGA而非微控制器的主要原因是其高效能和可編程性，能夠處理更複雜的信號處理任務。這樣的選擇雖然增加了開發的難度，但卻能在性能上獲得顯著提升。系統的擴展性也相當好，模組化設計允許用戶根據需求添加或替換硬體組件，這對於進行實驗和創新至關重要。對於資源需求，該系統在運行時需要穩定的電源供應，並且FPGA的冷啟動時間相對較短，能夠快速進入工作狀態。整合方面，AERIS-10能夠與多種主流開發環境（如Vivado）兼容，並且支持與Python進行良好的互動，這使得開發者能夠快速上手並進行二次開發。

## 新手體驗

> [!info] 上手難度評估
> README文件清晰且詳細，提供了豐富的背景資訊和使用指導。安裝過程相對順暢，但對於初學者來說，硬體組裝可能會有挑戰。文件中包含了良好的入門指南，幫助用戶快速上手。文件目前僅提供英文版本，可能對非英語使用者造成一定困難。

## 優缺點分析

> [!success] 優點
> - 開源設計，透明度高，易於修改和擴展。
> - 低成本，相比傳統雷達系統大幅降低進入門檻。
> - 模組化架構，便於維護和升級。

> [!danger] 缺點
> - 需要專業知識進行硬體組裝和調試。
> - FPGA開發工具的學習曲線較陡，對初學者不友好。
> - 社群支持相對較新，可能遇到問題時缺乏即時解決方案。

> [!warning] 注意事項
> - 需要基本的雷達原理知識。
> - 硬體組裝需要經驗，對於初學者可能有挑戰。
> - FPGA開發工具（Vivado）需額外安裝，且可能需要高性能計算機。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於AI agent的開發，而PLFM_RADAR專注於雷達技術，兩者應用場景不同。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard主要用於數據處理，而PLFM_RADAR則是硬體雷達系統，功能和應用範圍不同。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於AI agent的開發，而PLFM_RADAR專注於雷達技術，兩者應用場景不同。 | 如果你的專案需要AI agent的功能，則agent-sprite-forge更合適。 | low，因為兩者的技術棧差異不大。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard主要用於數據處理，而PLFM_RADAR則是硬體雷達系統，功能和應用範圍不同。 | 如果你的專案主要集中在數據分析而非硬體控制，則boneyard更合適。 | medium，因為需要調整專案的核心邏輯。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PLFM_RADAR** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於AI agent的開發，而PLFM_RADAR專注於雷達技術，兩者應用場景不同。 | Boneyard主要用於數據處理，而PLFM_RADAR則是硬體雷達系統，功能和應用範圍不同。 |
> | 遷移成本 | - | low，因為兩者的技術棧差異不大。 | medium，因為需要調整專案的核心邏輯。 |
> | 適用場景 | 主要場景 | 如果你的專案需要AI agent的功能，則agent-spr | 如果你的專案主要集中在數據分析而非硬體控制，則boneyar |
...
- **[HIGH]** 硬體組裝過程中可能會遇到焊接問題，導致電路不穩定。
  - 解法：確保使用適當的焊接技術和工具。
- [MEDIUM] FPGA配置過程中可能出現時序問題，影響信號處理。
  - 解法：仔細檢查時序設置，並參考官方文檔。
- [MEDIUM] Python GUI在某些系統上可能無法正常啟動。
  - 解法：確保安裝了所有必要的依賴，並檢查Python版本。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10人以下的新創公司後端API | 普通 | 雖然系統開源，但需要專業知識進行硬體組裝和調試。 |
| 大型研究機構的雷達信號處理實驗 | 非常適合 | 開源設計和模組化架構非常符合研究需求。 |
| 個人開發者的無人機導航系統 | 適合 | 系統的GPS/IMU整合能提供精確的位置信息。 |
| 中型企業的商業雷達應用 | 不適合 | 可能需要更穩定的商業支持和服務。 |
...
> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限，且不存取敏感資料。依賴鏈中無已知的供應鏈風險，適合在CI/CD中使用。
...
> | [#129](https://github.com/NawfalMotii79/PLFM_RADAR/issues/129) | ADAR1000Manager: Propagate SPI/ADC Communication Errors to C | 0 | 2 |
