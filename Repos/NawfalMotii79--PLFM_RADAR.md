...
**18.1k** stars · **385** stars/天 · 建立 47 天前 · PLSQL · NOASSERTION
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Growing · **熱度** Hot (385 stars/day)
> **授權** NOASSERTION · **維護** Active (最後推送 1 天前) · **貢獻者** 5+ 人 · **參與度** Medium
> **適合** 對雷達技術有興趣的開發者和研究者，尤其是那些希望在低成本環境下進行實驗的人。
> **一句話重點** AERIS-10不僅是雷達技術的開源實驗平台，更是推動相關技術民主化的重要一步。
...
> **結論** 花20小時學習，30小時整合，能夠獲得一個靈活的雷達系統，值得投入。

> [!abstract] 核心創新
> AERIS-10是首個開源的相控陣雷達系統，提供模組化和可擴展的設計。

## 專案簡介

AERIS-10是一個開源的10.5 GHz相控陣雷達系統，專為研究人員、無人機開發者和SDR愛好者設計。用戶可以透過該系統進行脈衝線性頻率調製（LFM），實現高達20公里的探測範圍。系統的核心在於其模組化設計，允許用戶根據需求進行硬體組裝與信號處理的修改。使用者需要具備基本的雷達原理知識和PCB組裝經驗，並需安裝Python 3.8以上版本來運行GUI。該系統的主要特點包括開源硬體與軟體、全電子波束轉向、先進的信號處理能力（如脈衝壓縮和多普勒處理），以及GPS/IMU集成以實現實時位置校正。使用者可以透過Python GUI進行雷達控制與目標可視化，並且系統支持多種硬體配置，適合不同的應用場景。這個專案的賣點在於其開放性與可擴展性，讓使用者能夠深入探索雷達技術的各個方面。

**技術棧**：`Python 3.8+` · `Vivado` · `FPGA (XC7A50T)` · `STM32F746xx`

## 重點功能

- 開源硬體與軟體 — 完整的電路圖、PCB佈局和韌體可供使用。
- 雙版本可用 — AERIS-10N（3公里範圍）和AERIS-10E（20公里範圍）。
- 全電子波束轉向 — 在仰角和方位角上可達±45°的電子轉向。
- 先進信號處理 — FPGA處理脈衝壓縮、多普勒FFT、MTI和CFAR。
- Python GUI — 提供用戶友好的界面，支持地圖集成和雷達控制。
...
```
2. 下載並安裝FPGA開發工具（Vivado）
```bash
# 參考Vivado官方文檔進行安裝
```
3. 組裝硬體，參考電路圖和PCB佈局
```bash
# 參考/4_Schematics and Boards Layout中的文件
```

## 為什麼值得關注

> [!tip] 爆紅原因
> 建立47天內累積18082 stars（385/天），forks 4313（23.9%），顯示出強烈的社群參與。專案的主要貢獻者包括JJassonn69和NawfalMotii79，這些成員在開源硬體領域有豐富經驗。AERIS-10解決了高成本雷達系統的問題，讓更多人能夠進行雷達技術的研究與開發。專案的開源性與模組化設計吸引了大量對雷達技術感興趣的開發者。社群的活躍度也反映在熱門問題的討論上，像是對於波束校準策略的探討。

## 適合誰使用

**目標受眾**：對雷達技術有興趣的開發者和研究者，尤其是那些希望在低成本環境下進行實驗的人。

> [!example] 使用場景
> - 無人機開發者用它來實現高精度的目標追蹤，因為其先進的信號處理能力能夠提供準確的距離和速度數據。
> - 大學研究生用它來進行雷達技術的實驗，因為開源的設計讓他們能夠自由修改和擴展系統功能。
> - SDR愛好者用它來探索不同的雷達波形和信號處理算法，因為其模組化設計允許用戶輕鬆替換和升級硬體組件。

## 架構分析

AERIS-10的架構設計為模組化，主要由電源管理板、頻率合成板和主板組成。電源管理板確保所有電子元件的電壓供應，頻率合成板使用高性能時鐘生成器提供相位對齊的時鐘參考。主板上搭載FPGA進行雷達信號處理，並與STM32微控制器協同工作以管理系統。這種設計使得系統能夠靈活應對不同的應用需求，但也增加了組裝的複雜性。擴展性方面，模組化設計使得用戶可以根據需求自由增減硬體組件，然而在大規模應用時可能會面臨資源管理的挑戰。

## 技術深入分析

AERIS-10的核心技術機制在於其模組化設計，使用FPGA進行信號處理，並結合STM32微控制器進行系統管理。FPGA負責脈衝壓縮、多普勒處理等複雜的信號處理任務，這使得系統能夠在實時環境中高效運行。系統能夠處理的最大範圍為20公里，並且具備電子波束轉向功能，這在傳統雷達系統中是難以實現的。設計選擇上，使用FPGA而非傳統DSP處理器，能夠在性能與靈活性之間取得良好平衡，但也增加了開發的複雜度。技術風險方面，FPGA的配置與調試可能會在擴大應用時出現瓶頸，尤其是在資源分配與管理上。整合方面，與主流的開發工具鏈（如Vivado）相容性良好，但對於不熟悉FPGA的開發者來說，學習曲線較陡峭。

## 新手體驗

> [!info] 上手難度評估
> README文件清晰，提供了詳細的安裝步驟和硬體組裝指南。安裝過程相對順暢，但對於新手來說，FPGA開發工具的使用可能會有一定的挑戰。文件中有提供Getting Started指南，幫助用戶快速上手。文件目前僅有英文版本，缺乏多語言支持。
...
> [!danger] 缺點
> - 硬體組裝與調試需要一定技術背景。
> - 對新手來說，FPGA開發工具的使用門檻較高。
> - 系統的性能依賴於用戶的組裝與配置質量。

> [!warning] 注意事項
> - 需要對雷達原理有基本了解。
> - 硬體組裝需要一定的技術能力。
> - FPGA開發工具需要額外安裝，可能對新手造成困難。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard專注於RFID技術，而AERIS-10專注於雷達系統，前者不具備相控陣能力。 |
| [NVIDIA/NemoClaw](https://github.com/NVIDIA/NemoClaw) | NemoClaw是針對AI推論的工具，而AERIS-10則是針對雷達信號處理，應用場景完全不同。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard專注於RFID技術，而AERIS-10專注於雷達系統，前者不具備相控陣能力。 | 如果你的專案需要RFID技術的支持，Boneyard會是更合適的選擇。 | medium，因為兩者的技術路線不同，需重新設計部分架構。 |
| [NVIDIA/NemoClaw](https://github.com/NVIDIA/NemoClaw) | NemoClaw是針對AI推論的工具，而AERIS-10則是針對雷達信號處理，應用場景完全不同。 | 如果你的專案主要集中在AI推論而非雷達技術，NemoClaw將更適合。 | high，因為兩者的功能和技術棧差異較大，需進行大幅度重構。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PLFM_RADAR** | **boneyard** | **NemoClaw** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | Boneyard專注於RFID技術，而AERIS-10專注於雷達系統，前者不具備相控陣能力。 | NemoClaw是針對AI推論的工具，而AERIS-10則是針對雷達信號處理，應用場景完全不同。 |
> | 遷移成本 | - | medium，因為兩者的技術路線不同，需重新設計部分架構。 | high，因為兩者的功能和技術棧差異較大，需進行大幅度重構。 |
> | 適用場景 | 主要場景 | 如果你的專案需要RFID技術的支持，Boneyard會是更合 | 如果你的專案主要集中在AI推論而非雷達技術，NemoClaw |
...
- **[HIGH]** 組裝過程中可能會遇到元件不匹配的問題，導致無法正常運行。
  - 解法：仔細檢查BOM清單，確保所有元件正確無誤。
- [MEDIUM] FPGA配置可能會因為時序問題導致系統不穩定。
  - 解法：參考最新的時序報告，調整設計以滿足時序要求。
- [MEDIUM] Python GUI可能在某些環境中無法正常顯示。
  - 解法：確保安裝了所有必要的依賴，並在支持的環境中運行。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 大學研究生進行雷達技術的實驗 | 非常適合 | 開源設計允許自由修改和擴展，適合學術研究。 |
| 無人機開發公司需要集成雷達系統 | 適合 | 提供的模組化設計方便集成，且具備先進的信號處理能力。 |
| 業餘愛好者想要探索雷達技術 | 非常適合 | 低成本且開源，適合個人實驗和學習。 |
| 大型企業需要商業級雷達系統 | 不適合 | 目前仍處於beta階段，尚未達到商業級的穩定性和可靠性。 |
...
> [!tip] 投入 vs 回報
> 花20小時學習，30小時整合，能夠獲得一個靈活的雷達系統，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限運行，且不會存取敏感資料。依賴鏈中未發現已知的供應鏈風險，適合在CI/CD中使用。
...
> | [#21](https://github.com/NawfalMotii79/PLFM_RADAR/issues/21) | Adding LAN interface | 2 | 17 |
> | [#75](https://github.com/NawfalMotii79/PLFM_RADAR/issues/75) | [Question/Discussion] Beamforming Calibration Strategy & Pha | 1 | 9 |
> | [#125](https://github.com/NawfalMotii79/PLFM_RADAR/issues/125) | Boards STEP files | 0 | 3 |
...
相關專案：[[0xGF--boneyard|0xGF/boneyard]] · [[NVIDIA--NemoClaw|NVIDIA/NemoClaw]] · [[432539--gpt2api|432539/gpt2api]] · [[BuilderPulse--BuilderPulse|BuilderPulse/BuilderPulse]] · [[AgentSeal--codeburn|AgentSeal/codeburn]] · [[AgriciDaniel--claude-obsidian|AgriciDaniel/claude-obsidian]] · [[BigBodyCobain--Shadowbroker|BigBodyCobain/Shadowbroker]] · [[AlpinDale--parsync|AlpinDale/parsync]]
