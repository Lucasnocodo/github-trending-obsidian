...
**18.9k** stars · **350** stars/天 · 建立 54 天前 · PLSQL · NOASSERTION
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Growing · **熱度** Hot (350 stars/day)
> **授權** NOASSERTION · **維護** Active (最後推送 1 天前) · **貢獻者** 5+ 人 · **參與度** Medium
> **適合** 對雷達技術感興趣的研究人員和開發者，尤其是需要低成本解決方案的團隊。
> **一句話重點** AERIS-10不僅是一個雷達系統，更是一個開放的實驗平台，讓任何人都能進入這個技術領域。
...
> **結論** 花20小時學習，15小時整合，得到一個靈活的雷達系統，值得投入。

> [!abstract] 核心創新
> 這個專案的核心創新在於提供一個完全開源的相控陣列雷達系統，讓更多人能夠接觸和實驗雷達技術。

## 專案簡介

AERIS-10是一個開源的10.5 GHz相控陣列雷達系統，設計目的是為了讓研究人員和開發者能夠探索雷達技術。用戶可以透過該系統進行脈衝線性頻率調變（LFM）調變，並可選擇3km或20km的量測範圍。系統的核心是FPGA，負責信號處理，包括脈衝壓縮、Doppler FFT和目標追蹤等功能。使用者可以透過Python GUI進行操作，並整合GPS和IMU進行實時位置修正。這樣的設計使得雷達系統不僅具備高效能，還能夠靈活應對不同的應用場景。最關鍵的指令包括使用Vivado進行FPGA開發，並透過Python GUI進行雷達控制。這個工具的賣點在於其開源性和模組化設計，讓使用者能夠根據需求進行自定義和擴展。

**技術棧**：`Python 3.8+` · `Vivado` · `FPGA`

## 重點功能

- 開源硬體與軟體 — 完整的電路圖、PCB佈局和韌體可供下載。
...
> [!tip] 爆紅原因
> 建立54天內累積18893 stars（350/天），forks 4519（23.9%），顯示出強烈的社群參與。主要貢獻者包括JJassonn69和NawfalMotii79，他們在開源硬體和雷達技術方面有豐富的經驗。這個專案解決了以往雷達技術昂貴且難以接觸的問題，讓更多的開發者和研究者能夠進入這一領域。社群的活躍度和開放的問題討論也促進了專案的快速成長。技術生態的變化，如FPGA的普及和開源硬體運動的興起，使得這個專案的實現變得可行。forks/stars比率23.9%顯示出許多人在實際修改和使用這個專案。

## 適合誰使用

**目標受眾**：對雷達技術感興趣的研究人員和開發者，尤其是需要低成本解決方案的團隊。

> [!example] 使用場景
> - 大學研究生用它來進行雷達信號處理實驗，因為開源設計讓他們能夠輕鬆修改和擴展功能。
> - 無人機開發者用它來實現目標追蹤功能，因為其模組化設計能快速整合到現有系統中。
> - 業餘愛好者用它來學習和實驗相控陣列雷達技術，因為低成本和開源特性降低了入門門檻。

## 架構分析

AERIS-10的架構設計為模組化，主要由電源管理板、頻率合成器板和主板組成。電源管理板確保所有電子元件的電壓供應，頻率合成器板則使用高性能的低抖動時鐘生成器，提供相位對齊的時鐘參考。主板上集成了FPGA，負責雷達信號處理，並與微控制器進行通信以管理系統。

這樣的設計使得系統能夠靈活應對不同的應用需求，並且易於維護和擴展。選擇FPGA作為信號處理單元的原因在於其高效能和可編程性，雖然這也增加了開發的複雜度。整體架構的模組化設計使得未來的擴展和維護變得更加簡單。

## 技術深入分析

AERIS-10的核心技術機制在於其模組化設計，使用FPGA進行信號處理，這使得系統能夠高效地處理雷達信號。FPGA的使用讓開發者能夠根據需求進行自定義，而不必依賴固定的硬體設計。系統的架構允許用戶在不同的應用場景中進行調整和擴展，這是傳統雷達系統所無法比擬的。效能方面，FPGA能夠實時處理大量數據，並且在高頻率下穩定運行。設計選擇上，使用CERN-OHL-P授權來保護硬體設計，這樣的選擇使得專案在法律上更具保障。技術風險方面，FPGA的開發和維護需要專業知識，這可能會對新手造成一定的障礙。整合方面，與現有的開發工具鏈（如Vivado）相容性良好，但對於初學者來說，學習曲線可能較陡峭。

## 新手體驗

> [!info] 上手難度評估
> README文件清晰，提供了詳細的安裝步驟和範例。安裝過程相對順暢，但對於新手來說，FPGA開發工具的配置可能會有些挑戰。文件中有連結到詳細的文檔，方便用戶查閱。整體來說，花30分鐘能夠完成基本的設置。
...
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供開源硬體設計，但不專注於雷達系統，功能範圍較窄。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於AI代理的開發，與PLFM_RADAR的雷達技術無直接關聯。 | 如果你的專案重點在於AI代理的開發，而非雷達技術。 | low，因為兩者的技術棧差異不大。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供開源硬體設計，但不專注於雷達系統，功能範圍較窄。 | 如果需要一個更簡單的開源硬體設計，而不是專注於雷達技術。 | medium，因為需要重新適應不同的設計理念。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PLFM_RADAR** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於AI代理的開發，與PLFM_RADAR的雷達技術無直接關聯。 | 提供開源硬體設計，但不專注於雷達系統，功能範圍較窄。 |
> | 遷移成本 | - | low，因為兩者的技術棧差異不大。 | medium，因為需要重新適應不同的設計理念。 |
> | 適用場景 | 主要場景 | 如果你的專案重點在於AI代理的開發，而非雷達技術。 | 如果需要一個更簡單的開源硬體設計，而不是專注於雷達技術。 |
...
- **[HIGH]** FPGA配置過程中可能遇到的兼容性問題，特別是在不同版本的Vivado中。
  - 解法：確保使用與專案相容的Vivado版本。
- [MEDIUM] 在硬體組裝過程中，可能會因為PCB設計不當導致元件無法正常工作。
  - 解法：仔細檢查電路圖和PCB佈局，確保正確組裝。
- [MEDIUM] Python GUI可能在某些環境下無法正常啟動。
  - 解法：檢查Python版本和依賴，確保環境配置正確。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 大學研究生進行雷達信號處理實驗 | 非常適合 | 開源設計和模組化架構讓學生能夠輕鬆修改和實驗。 |
| 小型無人機開發團隊需要雷達系統 | 適合 | 低成本和靈活性使其成為理想選擇。 |
| 業餘愛好者想學習雷達技術 | 普通 | 雖然有學習曲線，但開源特性降低了入門門檻。 |
| 大型商業雷達系統開發 | 不適合 | 目前仍處於beta階段，穩定性不足。 |
...
> [!tip] 投入 vs 回報
> 花20小時學習，15小時整合，得到一個靈活的雷達系統，值得投入。
