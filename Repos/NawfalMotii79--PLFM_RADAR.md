...
use_case: "提供開源、低成本的10.5 GHz相控陣列雷達系統，讓研究者和開發者能夠探索雷達技術。"
priority: high
ring: assess
...
aliases:
  ...
  - "提供開源、低成本的10.5 GHz相控陣列雷達系統，讓研究者和開發者能夠探索雷達技術。"
---

# PLFM_RADAR

**20.3k** stars · **290** stars/天 · 建立 70 天前 · PLSQL · NOASSERTION
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Growing · **熱度** Hot (290 stars/day)
> **授權** NOASSERTION · **維護** Active (最後推送 2 天前) · **貢獻者** 5+ 人 · **參與度** Medium
> **適合** 希望探索和實驗雷達技術的研究者和開發者，特別是對開源硬體有興趣的人。
> **一句話重點** AERIS-10不僅是一個雷達系統，更是一個開放的實驗平台，讓更多人能夠進入雷達技術的世界。
...
AERIS-10是一個開源的10.5 GHz相控陣列雷達系統，旨在為研究者、無人機開發者和高級製造者提供一個可實驗的平台。用戶可以透過Python GUI進行操作，並利用FPGA進行先進的信號處理，支持脈衝壓縮、Doppler處理和目標追蹤。該系統分為兩個版本，AERIS-10N（3公里範圍）和AERIS-10E（20公里範圍），後者使用高功率放大器以擴展範圍。設計上，AERIS-10採用模組化架構，將電源管理、頻率合成和RF板分開，這樣的設計不僅提高了靈活性，還便於維護。

系統的信號處理由FPGA負責，能夠進行高效的脈衝壓縮和Doppler FFT計算，並且具備電子波束轉向功能，這使得雷達能夠在±45°的範圍內進行精確的波束控制。與傳統雷達系統相比，AERIS-10的開源特性使得用戶可以自由修改和擴展系統，這對於學術研究和創新開發尤為重要。使用者需要具備基本的雷達原理知識和PCB組裝經驗，並需使用FPGA開發工具進行信號處理的修改。這個專案的成熟度和活躍度都相當高，社群對於問題的回應速度也很快，適合希望深入了解雷達技術的開發者和研究者。

**技術棧**：`Python 3.8+` · `FPGA (Vivado)` · `C++` · `Verilog`

## 重點功能

- 開源硬體與軟體 — 提供完整的電路圖、PCB佈局、韌體及軟體。
- 雙版本可選 — AERIS-10N（3公里範圍）和AERIS-10E（20公里範圍）。
- 全電子波束轉向 — 在仰角和方位角上可進行±45°的電子轉向。
- 先進的信號處理 — FPGA處理脈衝壓縮、Doppler FFT、MTI和CFAR。
- Python GUI — 友好的用戶界面，支持地圖集成。
- GPS/IMU集成 — 實時位置和姿態修正。
...
> [!tip] 爆紅原因
> 建立70天內累積20281 stars（290/天），forks 4822（23.8%），顯示出強烈的社群參與。專案的主要貢獻者包括多位活躍的開發者，這表明有穩定的維護團隊。AERIS-10解決了傳統雷達系統高成本和封閉性問題，讓更多人能夠進行雷達技術的實驗和開發。這個專案的開源特性和模組化設計吸引了許多對雷達技術感興趣的開發者，並且其在學術界和創新領域的應用潛力巨大。社群的活躍度和開放的問題討論也促進了其快速擴散。

## 適合誰使用

**目標受眾**：希望探索和實驗雷達技術的研究者和開發者，特別是對開源硬體有興趣的人。

> [!example] 使用場景
> - 大學研究人員用它來進行雷達信號處理算法的實驗，因為開源設計讓他們能夠自由修改和擴展系統功能。
> - 無人機開發者用它來測試新型的雷達避障系統，因為其模組化設計允許快速迭代和實驗。
> - 高級製造者用它來開發自定義的雷達應用，因為完整的硬體和軟體文檔使得整合變得容易。

## 架構分析

AERIS-10的架構由多個模組組成，主要包括電源管理板、頻率合成器板和主板。這樣的設計使得系統能夠靈活配置，並且便於維護。信號處理由FPGA負責，能夠實現高效的脈衝壓縮和Doppler處理。

系統的每個模組都能獨立運作，這樣的設計提高了系統的可擴展性。選擇FPGA而非傳統微控制器，能夠在處理速度和並行處理能力上獲得顯著提升，但也增加了設計的複雜性。整體架構的模組化設計使得未來的擴展和功能添加變得更加容易。

## 技術深入分析

AERIS-10的核心技術機制包括使用FPGA進行信號處理，這使得系統能夠實現高效的脈衝壓縮和Doppler FFT計算。系統能夠處理的最大範圍達到20公里，並且具備電子波束轉向功能，這在傳統雷達系統中是難以實現的。設計上，AERIS-10採用模組化架構，這樣的選擇使得系統的維護和升級變得更加方便。使用FPGA而非微控制器的選擇，雖然在設計上增加了複雜性，但在性能上卻能提供更高的處理速度。系統的資源需求相對較高，對於初學者來說，可能需要額外的學習時間來掌握FPGA的開發工具。整合到現有的技術生態中，AERIS-10能夠與多種開發環境和工具鏈相容，這使得其在學術界和業界都有廣泛的應用潛力。

## 新手體驗

> [!info] 上手難度評估
> README文件提供了清晰的安裝步驟和使用指南，對於新手來說相對友好。安裝過程中需要一些硬體組裝的經驗，但整體流程順暢。文件中有詳細的功能介紹和技術規格，幫助用戶快速上手。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 這個專案專注於開發AI代理，而PLFM_RADAR專注於雷達技術的開源實驗，應用場景完全不同。 |
| [432539/gpt2api](https://github.com/432539/gpt2api) | gpt2api是針對NLP的API工具，而PLFM_RADAR則是針對雷達系統的硬體和軟體整合。 |
| [NVIDIA/NemoClaw](https://github.com/NVIDIA/NemoClaw) | NemoClaw專注於AI模型的訓練和推理，而PLFM_RADAR則提供實際的雷達硬體平台，兩者的技術路線和應用領域有明顯差異。 |
...
> | [@NawfalMotii79](https://github.com/NawfalMotii79) | 125 |
...
**社群活躍度**：社群活躍，問題回應速度快，開發者互動良好。
**連結**：[文件](https://NawfalMotii79.github.io/PLFM_RADAR/docs/)

## 開發動態

> [!abstract] 最近 10 次 commit（2026-04-22 ~ 2026-05-15）
> **活躍天數** 9 天 · **最新 commit** 3D Waveguide
...
> | [#144](https://github.com/NawfalMotii79/PLFM_RADAR/issues/144) | Production files | 0 | 1 |
> | [#143](https://github.com/NawfalMotii79/PLFM_RADAR/issues/143) | Impedance control PCBs layer clarification and issues | 0 | 7 |
...
相關專案：[[0x0funky--agent-sprite-forge|0x0funky/agent-sprite-forge]] · [[432539--gpt2api|432539/gpt2api]] · [[NVIDIA--NemoClaw|NVIDIA/NemoClaw]] · [[0xGF--boneyard|0xGF/boneyard]] · [[AlpinDale--parsync|AlpinDale/parsync]] · [[AgriciDaniel--claude-obsidian|AgriciDaniel/claude-obsidian]] · [[BuilderPulse--BuilderPulse|BuilderPulse/BuilderPulse]] · [[AgentSeal--codeburn|AgentSeal/codeburn]]
