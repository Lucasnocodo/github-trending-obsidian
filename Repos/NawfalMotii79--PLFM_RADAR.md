...
**20.2k** stars · **301** stars/天 · 建立 67 天前 · PLSQL · NOASSERTION
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Growing · **熱度** Hot (301 stars/day)
> **授權** NOASSERTION · **維護** Active (最後推送 1 天前) · **貢獻者** 5+ 人 · **參與度** Medium
> **適合** 對雷達技術有深入需求的開發者和研究人員，尤其是在學術和創新領域。
> **一句話重點** AERIS-10 代表了開源硬體在雷達技術領域的創新，讓更多人能夠接觸和實驗這項技術。
...
> **結論** 花 20 小時學習，40 小時整合，能夠獲得一個靈活的開源雷達系統，值得投資。

> [!abstract] 核心創新
> AERIS-10 是一個完全開源的相控陣雷達系統，提供靈活的模組化設計和先進的信號處理能力。

## 專案簡介

AERIS-10 是一個開源的 10.5 GHz 相控陣雷達系統，旨在讓研究人員、無人機開發者和高級製造者能夠探索雷達技術。這個系統支持脈衝線性頻率調變，並提供兩個版本：AERIS-10N（3km 範圍）和 AERIS-10E（20km 範圍）。其核心機制是通過 FPGA 處理信號，實現脈衝壓縮、Doppler FFT 和目標跟踪等功能。用戶可以透過 Python GUI 進行操作，並整合 GPS/IMU 進行實時位置校正。這個系統的賣點在於其完全開源的硬體和軟體，讓用戶能夠自由修改和實驗。

技術上，AERIS-10 使用了高性能的時鐘生成器和多通道相位移器，確保了信號的準確性和穩定性。相較於其他雷達系統，如傳統的商業雷達，AERIS-10 提供了更大的靈活性和可擴展性，特別適合於學術研究和創新實驗。實際使用中，該系統的效能取決於硬體組裝的精確度和信號處理的優化，並且需要一定的 FPGA 開發經驗。這個專案目前處於活躍開發階段，社群回應迅速，問題解決率達 88%。整體來看，AERIS-10 是一個成熟的開源雷達平台，適合對雷達技術有深入需求的開發者和研究人員。

**技術棧**：`Python 3.8+` · `FPGA (Verilog)` · `C/C++`

## 重點功能

- 開源硬體與軟體 — 完整的電路圖、PCB 佈局、韌體和軟體可供使用。
- 雙版本可用性 — AERIS-10N（3km）和 AERIS-10E（20km）提供不同的應用場景。
- 全電子波束控制 — 在仰角和方位角上 ±45° 的電子控制。
- 先進的信號處理 — FPGA 處理脈衝壓縮、Doppler FFT、MTI 和 CFAR。
- Python GUI — 用戶友好的界面，支持地圖整合和實時數據顯示。
- GPS/IMU 整合 — 實時位置和姿態校正，提升雷達數據的準確性。
...
> [!tip] 爆紅原因
> 建立 67 天內累積 20188 stars（301/天），forks 4800（23.8%），顯示出強烈的社群興趣。作者 NawfalMotii79 和團隊專注於開源硬體，過去有多個成功的開源專案。AERIS-10 解決了高成本和封閉系統的痛點，讓更多人能夠接觸和使用雷達技術。社群的活躍討論和問題解決也促進了這個專案的快速成長，並且其開源特性吸引了許多開發者的參與。這個工具的可行性也得益於現今開源硬體的興起，讓更多人能夠參與到雷達技術的探索中。forks/stars 比率高達 23.8%，顯示出許多人在實際修改和使用這個專案。

## 適合誰使用

**目標受眾**：對雷達技術有深入需求的開發者和研究人員，尤其是在學術和創新領域。

> [!example] 使用場景
> - 大學研究生用它來進行雷達信號處理的實驗，因為開源的特性讓他們能夠自由修改和調整系統以符合研究需求。
> - 無人機開發者用它來整合雷達系統於自主飛行任務中，因為其 20km 的範圍和即時數據處理能力能提升無人機的導航精度。
> - 電子工程師用它來學習和實踐相控陣雷達技術，因為系統的模組化設計和詳細文檔使得學習曲線更平緩。

## 架構分析

AERIS-10 的架構採用模組化設計，主要由電源管理板、頻率合成板和主板組成。電源管理板確保所有電子元件的電壓供應，頻率合成板使用高性能的時鐘生成器來提供相位對齊的時鐘參考。主板上集成了 DAC、微波混頻器和 FPGA，負責雷達信號的生成和處理。

這種設計使得系統能夠在不同的應用場景中進行擴展，但也要求使用者具備一定的硬體組裝和信號處理知識。選擇 FPGA 進行信號處理的好處是能夠實現高效的並行計算，然而這也意味著需要較高的開發成本和技術門檻。整體架構的擴展性良好，但在大規模應用時可能會面臨資源分配和處理速度的瓶頸。

## 技術深入分析

AERIS-10 的核心技術機制包括使用 FPGA 進行信號處理，這使得系統能夠實現高效的脈衝壓縮和 Doppler FFT。系統的處理管道從 DAC 生成 LFM 脈衝開始，經過混頻器進行頻率轉換，然後由相位移器控制波束方向。FPGA 負責捕獲原始 ADC 數據並進行 I/Q 基帶下變換、過濾和脈衝壓縮等處理。這種設計使得 AERIS-10 能夠在實時環境中運行，並能夠處理高達 20km 的雷達信號。選擇 FPGA 而非傳統的微處理器的好處在於其並行處理能力，但這也要求開發者具備相應的硬體設計能力。技術風險方面，系統的複雜性可能導致在擴展時出現性能瓶頸，特別是在高負載情況下。與主流框架的整合難度相對較高，特別是在 CI/CD 流程中，使用者需要自行處理硬體和軟體的整合問題。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了詳細的安裝步驟和系統架構說明，對於新手來說相對友好。安裝過程需要一定的硬體組裝知識，可能會遇到一些挑戰。文件中有清晰的 getting started guide，幫助使用者快速上手。文件目前僅提供英文版本，對於非英語使用者可能會有一定的障礙。
...
> [!danger] 缺點
> - 需要一定的電子工程和 FPGA 開發知識。
> - 硬體組裝過程可能會遇到挑戰。
> - 目前僅支援 Linux 環境，對於 Windows 用戶不友好。

> [!warning] 注意事項
> - 需要 FPGA 開發經驗以進行信號處理的修改。
> - 硬體組裝需要一定的電子工程知識。
> - 不支援 Windows 環境，僅適用於 Linux 系統。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 這個專案專注於 AI agent 的開發，而 PLFM_RADAR 專注於雷達技術，兩者的應用場景和技術需求完全不同。 |
| [432539/gpt2api](https://github.com/432539/gpt2api) | gpt2api 是針對語言模型的 API，而 PLFM_RADAR 是硬體和信號處理的開源平台，兩者在技術架構和使用者需求上有明顯差異。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於 AI agent 的開發，與 PLFM_RADAR 的硬體和信號處理特性完全不同。 | 如果你的專案需要 AI agent 的開發而非雷達技術，則應選擇此工具。 | low，因為兩者的技術需求差異大，遷移不會涉及相同的技術棧。 |
| [432539/gpt2api](https://github.com/432539/gpt2api) | 針對語言模型的 API，與 PLFM_RADAR 的硬體設計無關。 | 如果你的需求是處理自然語言而非雷達信號，則應選擇此工具。 | low，因為兩者的應用場景和技術需求完全不同。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PLFM_RADAR** | **agent-sprite-forge** | **gpt2api** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於 AI agent 的開發，與 PLFM_RADAR 的硬體和信號處理特性完全不同。 | 針對語言模型的 API，與 PLFM_RADAR 的硬體設計無關。 |
> | 遷移成本 | - | low，因為兩者的技術需求差異大，遷移不會涉及相同的技術棧。 | low，因為兩者的應用場景和技術需求完全不同。 |
> | 適用場景 | 主要場景 | 如果你的專案需要 AI agent 的開發而非雷達技術，則應 | 如果你的需求是處理自然語言而非雷達信號，則應選擇此工具。 |
...
> [!tip] 採用建議
> 適合學術研究和實驗，但不建議用於生產環境的核心應用。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 硬體組裝過程中可能會遇到元件不匹配的問題，導致系統無法正常運行。
  - 解法：仔細檢查 BOM 和元件規格，確保所有元件相容。
- [MEDIUM] FPGA 開發過程中可能會遇到時序問題，影響信號處理的準確性。
  - 解法：使用 Vivado 的時序分析工具進行優化，確保時序滿足要求。
- [MEDIUM] Python GUI 在某些環境下可能會出現相容性問題。
  - 解法：確保使用 Python 3.8+ 並安裝所有必要的依賴。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 大學研究生的雷達信號處理課程 | 非常適合 | 開源特性和詳細文檔使得學生能夠深入學習。 |
| 小型無人機開發團隊 | 適合 | 提供了可擴展的雷達系統，能夠整合到無人機中。 |
| 大型企業的商業雷達應用 | 不適合 | 目前尚未達到生產環境的穩定性和成熟度。 |
...
> [!tip] 投入 vs 回報
> 花 20 小時學習，40 小時整合，能夠獲得一個靈活的開源雷達系統，值得投資。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限，且不存取敏感資料。依賴鏈中的元件經過審核，無已知的供應鏈風險。
...
> | [#137](https://github.com/NawfalMotii79/PLFM_RADAR/issues/137) | [RFC] Proposal: Add inter-board RF cable assembly specificat | 0 | 2 |
