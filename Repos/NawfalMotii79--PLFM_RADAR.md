...
**20.4k** stars · **276** stars/天 · 建立 74 天前 · PLSQL · NOASSERTION
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Growing · **熱度** Hot (276 stars/day)
> **授權** NOASSERTION · **維護** Active (最後推送 3 天前) · **貢獻者** 5+ 人 · **參與度** Medium
> **適合** 對雷達技術有興趣的研究者和開發者，尤其是希望在開源環境中進行實驗的人。
> **一句話重點** 這個專案不僅提供了一個雷達系統，更是開源硬體設計的典範，讓更多人能夠進入這個領域。
...
> [!abstract] 核心創新
> 提供開源的相控陣列雷達系統，讓任何人都能夠探索和實驗雷達技術。

## 專案簡介

AERIS-10是一個開源的10.5 GHz相控陣列雷達系統，旨在讓研究者、無人機開發者和進階製作者能夠實驗和探索雷達技術。使用者可以透過Python GUI進行操作，並且該系統支持電子波束轉向和先進的信號處理，具體包括脈衝壓縮、Doppler處理和目標追蹤。系統的核心在於FPGA，負責處理信號並進行各種數據處理，並且支持GPS/IMU整合以實現實時位置修正。AERIS-10有兩個版本：AERIS-10N（3km範圍）和AERIS-10E（20km範圍），後者配備了高功率放大器以擴展範圍。這個系統的賣點在於其開源硬體和軟體，讓用戶可以自由修改和擴展，並且所有設計文件均可獲得。使用者只需具備基本的雷達原理知識和PCB組裝經驗，即可開始構建和實驗。這個系統的設計考量了模組化，便於用戶根據需求進行擴展和改進。
...
> [!tip] 爆紅原因
> 建立74天內累積20425 stars（276/天），forks 4854（23.8%），顯示出強烈的社群參與。這個專案由NawfalMotii79主導，解決了開源雷達系統的需求，過去這類技術多由大型企業主導，成本高且難以獲得。隨著DIY電子和無人機技術的興起，這樣的開源雷達系統變得越來越受歡迎。社群的討論和需求反映在熱門Issues上，例如LAN介面的添加和波束成形校準策略的討論，顯示出用戶對於功能擴展的強烈需求。這個專案的開源性質和模組化設計使其在技術生態中具備了良好的適應性。

## 適合誰使用

**目標受眾**：對雷達技術有興趣的研究者和開發者，尤其是希望在開源環境中進行實驗的人。

> [!example] 使用場景
> - 大學研究生用它來進行雷達信號處理實驗，因為開源設計讓他們能夠自由修改和擴展系統功能。
> - 無人機開發者用它來實現目標追蹤功能，因為其高達20km的範圍和先進的信號處理技術能夠滿足商業需求。
> - 電子愛好者用它來學習和實驗雷達技術，因為其模組化設計和完整的文檔使得入門變得容易。

## 架構分析

AERIS-10的架構由多個模組組成，包括電源管理板、頻率合成器板和主板，這些模組協同工作以實現雷達信號的生成和處理。電源管理板確保所有電子元件獲得正確的電壓和過濾，頻率合成器板則提供相位對齊的時鐘參考。主板上包含DAC、微波混合器、相位移器和FPGA，負責生成雷達脈衝和處理接收到的信號。這種模組化設計使得系統易於擴展和維護，但也可能導致整合時的複雜性增加。系統的擴展性主要受限於FPGA的處理能力和模組之間的通訊延遲。

## 技術深入分析

AERIS-10的核心技術機制是基於FPGA的信號處理，使用Verilog語言實現雷達脈衝的生成和數據處理。整個系統能夠處理高達20km的範圍，並且支持電子波束轉向，這使得其在無人機和其他應用中具有廣泛的潛力。系統的設計考量了模組化，這意味著使用者可以根據需求進行擴展和修改。選擇FPGA而非傳統的微控制器，能夠提供更高的處理速度和靈活性，但也增加了設計的複雜性。系統的效能在於其能夠實時處理來自ADC的數據，並進行FFT和脈衝壓縮，這對於雷達應用至關重要。隨著使用者需求的增加，未來可能會增加更多的功能和模組，進一步提升系統的擴展性和應用範圍。

## 新手體驗

> [!info] 上手難度評估
> README文件提供了詳細的安裝步驟和使用指南，對於新手來說相對友好。安裝過程中需要注意PCB組裝的細節，可能會有一些挑戰。文件中有清晰的getting started guide，幫助使用者快速上手。文件目前僅提供英文版本，對於非英語使用者可能有一定的障礙。
...
> [!danger] 缺點
> - 對於新手來說，PCB組裝可能有一定的挑戰。
> - 需要較高的技術背景才能充分利用系統的功能。
> - 目前僅在Linux環境下進行測試，Windows支援有限。

> [!warning] 注意事項
> - 需要具備PCB組裝經驗，對於新手可能有一定的學習曲線。
> - 不支援Windows，主要針對Linux環境進行測試。
> - 高功率放大器僅在AERIS-10E版本中提供，AERIS-10N無法擴展至20km範圍。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於無人機控制和自動化，而PLFM_RADAR專注於雷達信號處理和探測。 |
| [432539/gpt2api](https://github.com/432539/gpt2api) | 主要用於NLP應用，與PLFM_RADAR的硬體設計無關。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於無人機控制和自動化，而PLFM_RADAR則專注於雷達信號處理和探測。 | 如果你的專案主要集中在無人機的自動化和控制上，則可以選擇這個工具。 | low，因為兩者的技術棧有一定的相似性。 |
| [432539/gpt2api](https://github.com/432539/gpt2api) | 主要用於NLP應用，與PLFM_RADAR的硬體設計無關。 | 如果你的需求是進行自然語言處理，則這個工具會更合適。 | high，因為兩者的技術棧和應用場景差異較大。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PLFM_RADAR** | **agent-sprite-forge** | **gpt2api** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於無人機控制和自動化，而PLFM_RADAR則專注於雷達信號處理和探測。 | 主要用於NLP應用，與PLFM_RADAR的硬體設計無關。 |
> | 遷移成本 | - | low，因為兩者的技術棧有一定的相似性。 | high，因為兩者的技術棧和應用場景差異較大。 |
> | 適用場景 | 主要場景 | 如果你的專案主要集中在無人機的自動化和控制上，則可以選擇這個 | 如果你的需求是進行自然語言處理，則這個工具會更合適。 |
...
- **[HIGH]** 在組裝過程中，PCB的焊接可能會導致短路，特別是在高功率放大器附近。
  - 解法：仔細檢查焊接點，並使用多用電表進行測試。
- [MEDIUM] FPGA配置過程中，可能會遇到時序問題，導致信號處理不穩定。
  - 解法：參考官方文檔中的時序設置建議進行調整。
- [MEDIUM] Python GUI在某些Linux發行版上可能無法正常運行。
  - 解法：確保安裝了所有必要的依賴，並使用虛擬環境進行測試。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10人以下的新創公司進行雷達技術研究 | 非常適合 | 開源設計和模組化架構使得小型團隊能夠快速實驗和迭代。 |
| 大型企業需要商業級雷達解決方案 | 不適合 | 目前仍在beta階段，穩定性和支援可能不足。 |
| 學術機構進行雷達信號處理的研究 | 非常適合 | 開源性質和完整的文檔支持學術研究和實驗。 |
| 電子愛好者進行DIY項目 | 非常適合 | 模組化設計和開源硬體使得DIY項目變得可行。 |
...
> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限運行，且不會存取敏感資料。依賴的庫和工具均為開源，無已知的供應鏈風險。
