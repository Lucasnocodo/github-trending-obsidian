...
use_case: "開源、低成本的10.5 GHz PLFM相控陣雷達系統。"
priority: high
ring: assess
...
star_history: "2026-03-15:1703,2026-03-15:1708,2026-03-16:2576,2026-03-17:3042,2026-03-18:3202"
tags:
  - github
  - "category/基礎設施"
  - "lang/c"
aliases:
  ...
  - "開源、低成本的10.5 GHz PLFM相控陣雷達系統。"
---

# PLFM_RADAR

**3.2k** stars · **356** stars/天 · 建立 9 天前 · C · NOASSERTION
...
> **適合** 對雷達技術有興趣的研究人員和開發者，特別是需要開源解決方案的無人機開發者。
> **一句話重點** AERIS-10不僅是一個雷達系統，更是一個開源平台，讓開發者能夠自由探索和實驗雷達技術。
...
>     .where(p => p.subcategory === "無人機技術" && p.file.name !== "NawfalMotii79--PLFM_RADAR" && p.status !== "archived")
...
>       p.ring || "assess"
>     ]));
>   } else { dv.paragraph("_目前 vault 中沒有其他 無人機技術 類工具_"); }
> }
> ```

> [!question] TL;DR — 值得投入嗎？
> **成熟度** Alpha (不穩定) · **安裝** Medium (需設定) · **學習** ~20h · **綁定風險** low
> **結論** 花20小時學習，40小時整合，得到一個靈活的開源雷達系統，值得投入。

> [!abstract] 核心創新
> 這個專案提供了一個完全開源的相控陣雷達系統，讓更多開發者能夠進行雷達技術的實驗和研究。

## 專案簡介

AERIS-10是一個開源的10.5 GHz相控陣雷達系統，專為研究人員和無人機開發者設計，提供兩個版本，分別支持3km和20km的探測範圍。其核心機制是利用脈衝線性頻率調製（LFM）來生成雷達信號，並通過FPGA進行先進的信號處理，包括脈衝壓縮、Doppler FFT和目標跟蹤。用戶可以通過Python GUI進行實時控制和數據可視化，並整合GPS和IMU進行位置和姿態修正。這個系統的賣點在於其完全開源的硬體和軟體設計，讓使用者能夠自由修改和擴展。系統架構上，AERIS-10包含多個子系統，如功率管理、頻率合成和信號處理，並使用高效的時鐘生成器和FPGA來確保信號的準確性和穩定性。

與傳統雷達系統相比，AERIS-10的模組化設計使其更易於維護和升級，並且能夠支持更高的靈活性和擴展性。這個系統適合於學術研究、無人機開發和高級SDR愛好者，特別是對雷達技術感興趣的開發者。使用者需要具備PCB組裝經驗和基本的雷達原理知識，並能夠使用FPGA開發工具進行信號處理的修改。整體來看，AERIS-10是一個具潛力的開源雷達平台，適合於多種應用場景。

**技術棧**：`C` · `Python` · `Verilog` · `C++` · `SMT` · `TypeSpec` · `MATLAB` · `Tcl`

## 重點功能

- 開源硬體與軟體 — 提供完整的電路圖、PCB佈局和韌體，便於修改和擴展。
- 雙版本可選 — AERIS-10N（3km範圍）和AERIS-10E（20km範圍），滿足不同需求。
- 全電子波束轉向 — 在仰角和方位角上可達±45°的電子轉向。
- 先進信號處理 — FPGA負責脈衝壓縮、Doppler FFT、MTI和CFAR等處理。
- Python GUI — 提供友好的用戶界面，支持地圖集成和實時目標繪製。
- GPS/IMU整合 — 實時位置和姿態修正，提升數據準確性。
- 模組化設計 — 獨立的電源管理、頻率合成和RF板，便於維護。

## 快速開始

1. 訂購PCB
```bash
從/4_Schematics and Boards Layout獲取Gerber文件
```
2. 組裝硬體
```bash
遵循/10_docs/assembly_guide.md中的組裝指南
```
3. 安裝Python環境
```bash
...
> [!tip] 爆紅原因
> 建立9天內累積3202 stars（356/天），forks 671（21.0%），這顯示出強勁的社群興趣。作者NawfalMotii79專注於開源硬體，過去的貢獻也集中在無人機和雷達技術上。這個專案解決了高成本雷達系統的痛點，讓更多開發者能夠進行實驗和研究。近期的推廣活動和社群討論也促進了其曝光率，特別是在無人機和SDR社群中。高forks/stars比率顯示出許多開發者在實際修改和使用這個專案，反映出其實用性和潛力。

## 適合誰使用

**目標受眾**：對雷達技術有興趣的研究人員和開發者，特別是需要開源解決方案的無人機開發者。

> [!example] 使用場景
> - 無人機開發者用它來實現自主導航和障礙物檢測，因為其高精度的雷達數據能夠提升飛行安全性。
> - 學術研究人員用它來進行雷達信號處理的實驗，因為開源的特性允許他們自由修改和擴展系統功能。
> - 高級SDR愛好者用它來探索相控陣雷達技術，因為其模組化設計讓他們能夠輕鬆進行硬體和軟體的實驗。

## 架構分析

AERIS-10的架構設計為模組化，主要分為功率管理、頻率合成和信號處理三大子系統。功率管理板確保所有電子元件的電壓供應，頻率合成板使用高性能的低抖動時鐘生成器來提供相位對齊的時鐘參考。信號處理則由FPGA負責，處理流程包括脈衝生成、上/下變頻、波束轉向和信號處理。

這種設計使得系統能夠高效運行，但也增加了對硬體組裝和配置的要求。選擇FPGA進行信號處理的好處是能夠實現高效的並行處理，但代價是需要較高的開發門檻。整體來看，這個架構在靈活性和性能上取得了良好的平衡。

## 技術深入分析

AERIS-10的核心技術機制包括使用FPGA進行信號處理，這使得系統能夠高效地處理雷達信號並實現即時反饋。其信號處理流程涵蓋了從脈衝生成到Doppler處理的多個步驟，這些步驟都在FPGA內部完成，確保了高效能和低延遲。系統的設計使其能夠處理多達20km的探測範圍，並且在信號處理上具有優越的性能。選擇FPGA而非傳統的微控制器來進行信號處理，帶來了更高的計算能力和靈活性，但同時也增加了開發的複雜度。這個系統的依賴樹相對簡單，主要依賴於FPGA和微控制器，降低了潛在的vendor lock-in風險。技術風險方面，隨著使用者數量的增加，可能會出現更多的bug和需求變更，這需要持續的維護和更新。整合方面，AERIS-10可以與多種現有的開發工具和環境兼容，特別是在無人機和SDR領域，這使得它在技術生態中具有良好的適應性。

## 新手體驗

> [!info] 上手難度評估
> README文件清晰，提供了詳細的安裝和組裝指南，但缺乏具體的程式碼範例。安裝過程需要一定的硬體組裝經驗，對新手來說可能有挑戰。文件中提供的getting started guide對於理解系統架構和功能有幫助，但沒有多語言支持。
...
> [!danger] 缺點
> - 對新手來說，組裝和配置可能有一定挑戰。
> - 需要額外的FPGA開發工具，增加學習成本。
> - 法律保護相對較弱，使用時需謹慎。

> [!warning] 注意事項
> - 需要基本的PCB組裝經驗，對新手有一定的學習曲線。
> - FPGA開發工具（如Vivado）需要額外安裝，可能增加配置複雜度。
> - 開源硬體的法律保護較弱，使用時需注意相關法律問題。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 提供類似的開源雷達解決方案，但專注於不同的頻率範圍和應用場景。 |
| [NVIDIA/NemoClaw](https://github.com/NVIDIA/NemoClaw) | 專注於機器學習和自動化的雷達系統，與AERIS-10相比，功能更為專業化。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | 提供不同的信號處理技術，適合於特定的無人機應用，但不如AERIS-10開源友好。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 專注於不同的頻率範圍，提供相似的開源雷達解決方案。 | 如果你的應用需要特定的頻率範圍或不同的信號處理技術。 | medium，因為需要重新調整硬體設置和信號處理流程。 |
| [NVIDIA/NemoClaw](https://github.com/NVIDIA/NemoClaw) | 專注於機器學習和自動化的雷達系統，功能更為專業化。 | 如果你的應用需要結合機器學習技術進行雷達數據分析。 | high，因為需要重新設計整個系統架構。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PLFM_RADAR** | **parsync** | **NemoClaw** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於不同的頻率範圍，提供相似的開源雷達解決方案。 | 專注於機器學習和自動化的雷達系統，功能更為專業化。 |
> | 遷移成本 | - | medium，因為需要重新調整硬體設置和信號處理流程。 | high，因為需要重新設計整個系統架構。 |
> | 適用場景 | 主要場景 | 如果你的應用需要特定的頻率範圍或不同的信號處理技術。 | 如果你的應用需要結合機器學習技術進行雷達數據分析。 |
...
> [!tip] 採用建議
> 適合個人實驗和學術研究，不建議用於商業生產環境。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** FPGA配置過程中可能會遇到兼容性問題，特別是在不同版本的Vivado中。
  - 解法：確保使用與FPGA版本相符的Vivado版本。
- [MEDIUM] 組裝過程中可能會缺少某些元件，特別是PCB上的小型元件。
  - 解法：提前檢查BOM並確保所有元件到位。
- [MEDIUM] 在使用Python GUI時，可能會遇到依賴包版本不兼容的問題。
  - 解法：確保使用Python 3.8+並安裝正確的依賴包版本。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 無人機開發團隊進行雷達數據收集 | 非常適合 | 其高性能和開源特性使得無人機開發者能夠靈活調整系統以滿足需求。 |
| 學術機構進行雷達信號處理研究 | 非常適合 | 開源的設計允許研究人員自由修改和實驗。 |
| 小型企業開發商業雷達產品 | 不適合 | 系統仍在alpha階段，穩定性不足，不建議用於商業用途。 |
| 個人愛好者進行DIY雷達項目 | 適合 | 開源和模組化設計適合個人進行實驗和學習。 |
...
> [!tip] 投入 vs 回報
> 花20小時學習，40小時整合，得到一個靈活的開源雷達系統，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限操作，且不存取敏感資料。依賴鏈的信任程度良好，無已知的供應鏈風險。
...
> | 貢獻者 | Commits |
> | --- | --- |
> | [@NawfalMotii79](https://github.com/NawfalMotii79) | 49 |

## 社群與生態

**社群活躍度**：社群活躍，近期有多個issue和pull request的討論。
**連結**：[文件](https://github.com/NawfalMotii79/PLFM_RADAR)

## 開發動態

> [!abstract] 最近 10 次 commit（2026-03-15 ~ 2026-03-17）
> **活躍天數** 3 天 · **最新 commit** Added all boards stack
...
> | --- | --- | --- | --- |
> | [#22](https://github.com/NawfalMotii79/PLFM_RADAR/issues/22) | FPGA Files | 2 | 2 |
> | [#21](https://github.com/NawfalMotii79/PLFM_RADAR/issues/21) | Adding LAN interface | 2 | 2 |
> | [#10](https://github.com/NawfalMotii79/PLFM_RADAR/issues/10) | Folder 10_docs seems missing | 1 | 2 |
> | [#23](https://github.com/NawfalMotii79/PLFM_RADAR/issues/23) | Missing Gerberfiles and Schematics | 0 | 4 |
> | [#17](https://github.com/NawfalMotii79/PLFM_RADAR/issues/17) | 3D Enclosure | 0 | 1 |
...
相關概念：[[無人機技術]] · [[信號處理]] · [[開源硬體]]

相關專案：[[AlpinDale--parsync|AlpinDale/parsync]] · [[NVIDIA--NemoClaw|NVIDIA/NemoClaw]] · [[BigBodyCobain--Shadowbroker|BigBodyCobain/Shadowbroker]] · [[L42ARO--Mercury-Transforming-Drone|L42ARO/Mercury-Transforming-Drone]] · [[Flowseal--tg-ws-proxy|Flowseal/tg-ws-proxy]] · [[novatic14--MANPADS-System-Launcher-and-Rocket|novatic14/MANPADS-System-Launcher-and-Rocket]]

[GitHub](https://github.com/NawfalMotii79/PLFM_RADAR)

## 相關收錄

> [!note]- 直接競品（同子分類：無人機技術）
> ```dataview
> TABLE stars, stars_per_day AS "Stars/天", install_complexity AS "難度", use_case AS "用途"
> FROM "Repos"
> WHERE subcategory = "無人機技術" AND file.name != "NawfalMotii79--PLFM_RADAR"
> SORT stars DESC
> ```
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["無人機技術","信號處理","開源硬體"];
