...
**19.4k** stars · **329** stars/天 · 建立 59 天前 · PLSQL · NOASSERTION
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Growing · **熱度** Hot (329 stars/day)
> **授權** NOASSERTION · **維護** Active (最後推送 0 天前) · **貢獻者** 5+ 人 · **參與度** Medium
> **適合** 對雷達技術有興趣的開發者，特別是需要低成本解決方案的學術研究者和無人機開發者。
> **一句話重點** AERIS-10 不僅是一個雷達系統，更是一個開放的實驗平台，讓使用者能夠自由探索和創新。
...
> [!abstract] 核心創新
> 這個專案的核心創新在於提供一個完全開源且低成本的相控陣雷達系統，讓更多人能夠探索和實驗雷達技術。

## 專案簡介

AERIS-10 是一個開源的 10.5 GHz 相控陣雷達系統，旨在讓研究人員、無人機開發者和高級製造者能夠實驗和探索雷達技術。用戶可以透過 Python GUI 進行操作，並利用 FPGA 進行信號處理，具體流程包括 DAC 生成 LFM 脈衝、上/下變頻、電子波束控制及信號處理。這個系統的賣點在於其模組化設計，支持多種應用場景，並且提供完整的硬體和軟體開源資料。技術上，系統使用了高性能的時鐘生成器和 FPGA 來處理複雜的信號處理任務，並且具備 GPS/IMU 整合能力，實現實時位置和姿態校正。

與傳統雷達系統相比，AERIS-10 提供了更高的可擴展性和靈活性，特別適合於需要快速原型開發的環境。相較於其他雷達系統，如商業化的高端雷達，AERIS-10 的成本大幅降低，且開源的特性使得用戶可以根據需求進行定制和擴展。使用者在實際操作中可能會遇到硬體組裝的挑戰，特別是對於 PCB 的組裝和調試，但這些都在其文檔中有詳細說明。這個專案目前處於穩定階段，適合小型團隊或個人開發者使用，未來可能會隨著社群的進一步發展而增強功能和擴展性。

**技術棧**：`Python 3.8+` · `FPGA (Vivado)` · `C/C++`

## 重點功能

- 開源硬體與軟體 — 提供完整的電路圖、PCB 佈局和韌體。
...
```bash
sudo apt install python3.8 vivado
```
2. 下載專案並安裝依賴
```bash
git clone https://github.com/NawfalMotii79/PLFM_RADAR.git && cd PLFM_RADAR && pip install -r requirements.txt
```
3. 運行 GUI
```bash
python3 gui.py
...
> [!tip] 爆紅原因
> 建立 59 天就累積 19440 stars（329/天），forks 4638（23.9%），顯示出強大的社群支持。這位作者 Nawfal Motii 之前在開源硬體領域有過多次貢獻，這個專案解決了高成本雷達系統的問題，讓更多人能夠進入這個領域。社群對於開源雷達技術的需求日益增加，尤其是在學術研究和無人機應用上。這個專案的開源性和模組化設計吸引了大量工程師的關注，並且在 GitHub 上的活躍度也顯示出其受歡迎程度。forks/stars 比率高達 23.9%，顯示出許多人在實際修改和使用這個專案。

## 適合誰使用

**目標受眾**：對雷達技術有興趣的開發者，特別是需要低成本解決方案的學術研究者和無人機開發者。

> [!example] 使用場景
> - 學術研究者用它來進行雷達信號處理的實驗，因為其開源特性和完整的硬體設計讓他們能夠自由調整和擴展系統。
> - 無人機開發者用它來整合雷達系統於自家無人機，因為其低成本和模組化設計使得集成變得簡單。
> - 高級製造者用它來測試和驗證新型雷達技術，因為其開放的硬體和軟體資源能夠支持快速原型開發。

## 架構分析

AERIS-10 的架構設計為模組化，主要由電源管理板、頻率合成器板和主板組成。電源管理板負責提供穩定的電壓，頻率合成器板使用高性能的時鐘生成器來供應相位對齊的時鐘參考。主板上集成了 DAC、微波混頻器、相位移器和 FPGA，這些組件共同協作以實現雷達信號的生成和處理。

這種設計使得系統能夠靈活應對不同的應用需求，但也要求使用者具備一定的硬體組裝能力。選擇 FPGA 進行信號處理的好處是能夠快速實現複雜的算法，但這也增加了系統的學習曲線。整體架構的可擴展性使得未來可以輕鬆添加新功能或改進現有功能，這是傳統商業雷達系統所無法比擬的。

## 技術深入分析

AERIS-10 的核心技術機制在於其模組化設計和開源特性，使用 FPGA 進行複雜的信號處理，並透過 Python GUI 提供用戶友好的操作界面。系統能夠處理多達 20km 的雷達範圍，並且具備先進的信號處理能力，如脈衝壓縮和 Doppler FFT。這些功能的實現依賴於高性能的硬體組件，如 DAC 和相位移器，這使得系統在性能上具備競爭力。選擇 FPGA 而非傳統的微控制器進行信號處理，雖然增加了開發的複雜性，但卻能夠提供更高的處理速度和靈活性。技術上，這個專案的依賴關係相對簡單，主要集中在 FPGA 和 Python 環境，這降低了學習曲線。未來在擴展性方面，若要增加新的功能或改進現有功能，使用者需要考慮到硬體的兼容性和可擴展性，這可能會成為一個瓶頸。整合到現有的 CI/CD pipeline 中相對容易，特別是對於有經驗的開發者來說，這使得 AERIS-10 在實驗和開發中具備良好的適應性。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝指引和使用說明，並且有詳細的硬體組裝步驟。安裝過程相對順暢，但對於新手來說，可能需要一些時間來理解硬體組裝的細節。文件中有提供 Getting Started 指南，對於初學者來說非常友好。文件目前僅提供英文版本，對於非英語使用者可能會有一定的障礙。
...
> - 需要一定的電子技術基礎來進行硬體組裝。
> - 信號處理的修改需要 FPGA 開發經驗。
> - 目前僅支援特定頻率範圍，無法擴展至其他應用。

> [!warning] 注意事項
> - 需要 FPGA 開發工具進行信號處理的修改。
> - 硬體組裝需要一定的電子技術基礎。
> - 目前僅支援 10.5 GHz 頻率範圍，無法擴展至其他頻段。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 這個專案專注於 AI agent 的生成，而 AERIS-10 專注於雷達技術的開源實驗。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard 提供硬體設計的開源資源，但不專注於雷達技術，功能範圍較窄。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於 AI agent 的開發，而 AERIS-10 專注於雷達技術的實驗和開發。 | 如果你的需求是開發 AI agent 而非雷達技術，則應選擇此工具。 | medium，因為兩者的技術棧和應用場景不同。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供硬體設計的開源資源，但不專注於雷達技術，功能範圍較窄。 | 如果你的需求是一般的開源硬體設計，而非專注於雷達，則應選擇此工具。 | low，因為兩者的設計理念相似。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PLFM_RADAR** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於 AI agent 的開發，而 AERIS-10 專注於雷達技術的實驗和開發。 | 提供硬體設計的開源資源，但不專注於雷達技術，功能範圍較窄。 |
> | 遷移成本 | - | medium，因為兩者的技術棧和應用場景不同。 | low，因為兩者的設計理念相似。 |
> | 適用場景 | 主要場景 | 如果你的需求是開發 AI agent 而非雷達技術，則應選擇 | 如果你的需求是一般的開源硬體設計，而非專注於雷達，則應選擇此 |
...
> [!tip] 採用建議
> 適合小型團隊或個人開發者使用，能夠在實驗和開發中提供穩定的支持。
...
- **[HIGH]** 硬體組裝過程中可能會遇到焊接問題，導致電路不穩定。
  - 解法：仔細檢查焊點，並確保所有連接正確。
- [MEDIUM] 在信號處理過程中可能會出現延遲，影響實時性能。
  - 解法：優化 FPGA 設計以減少延遲。
- [MEDIUM] 使用者可能對 GUI 操作不熟悉，導致使用困難。
  - 解法：參考官方文檔中的使用指南。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型研究團隊進行雷達技術的探索 | 非常適合 | 開源特性和模組化設計使得研究團隊能夠自由調整系統。 |
| 大型企業開發商用雷達系統 | 不適合 | 商業需求可能需要更高的可靠性和支持。 |
| 無人機開發者整合雷達系統 | 適合 | 低成本和模組化設計使得集成變得簡單。 |
| 個人愛好者進行 DIY 雷達項目 | 非常適合 | 提供完整的開源資料，適合愛好者進行實驗。 |
...
> | [@NawfalMotii79](https://github.com/NawfalMotii79) | 121 |
...
> [!abstract] 最近 10 次 commit（2026-04-19 ~ 2026-05-06）
> **活躍天數** 7 天 · **最新 commit** Add files via upload
...
> | [#134](https://github.com/NawfalMotii79/PLFM_RADAR/issues/134) | Possible Typos / Incorrect Part Numbers in the BOM File | 0 | 5 |
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["開源硬體","信號處理","模組化設計"];
