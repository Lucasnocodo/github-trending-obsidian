...
**3.8k** stars · **129** stars/天 · 建立 29 天前 · PLSQL · NOASSERTION
...
> **適合** 對相控陣列雷達技術有興趣的研究人員和開發者，尤其是希望進行實驗和創新的小型團隊。
> **一句話重點** AERIS-10 不僅是一個雷達系統，更是一個開源社群的實驗平台，讓更多人能夠探索和創新。
...
> **結論** 花 10 小時學習，20 小時整合，得到一個靈活的開源雷達系統，值得投資。

> [!abstract] 核心創新
> AERIS-10 提供了一個完全開源的相控陣列雷達系統，讓更多人能夠接觸和實驗雷達技術。

## 專案簡介

AERIS-10 是一個開源的 10.5 GHz 相控陣列雷達系統，專為研究人員、無人機開發者及 SDR 愛好者設計。該系統支持兩種版本，分別為 3 公里和 20 公里的探測範圍，並利用脈衝線性頻率調變技術進行信號處理。用戶可以透過 Python GUI 進行操作，並享有實時的目標追蹤和地圖整合功能。硬體架構包括電源管理板、頻率合成器板和主板，主板上集成了 FPGA 進行信號處理，並支持電子波束轉向。這種模組化設計使得用戶能夠根據需求進行擴展和修改。

相較於傳統雷達系統，AERIS-10 的開源特性使得其更具可接近性，並能促進技術創新。與其他商業雷達系統相比，AERIS-10 的低成本和開放性使其成為教育和研究的理想選擇。使用者需具備基本的雷達原理知識和 PCB 組裝經驗，並需要 Python 3.8 以上版本來運行 GUI。這個專案的成熟度尚在 beta 階段，適合小型團隊或個人進行實驗和開發，但不建議用於商業產品的核心功能。

**技術棧**：`Python 3.8+` · `FPGA` · `Verilog`

## 重點功能

- 開源硬體與軟體 — 提供完整的電路圖、PCB 布局、韌體和軟體。
- 雙版本可選 — AERIS-10N (3km) 和 AERIS-10E (20km)，滿足不同需求。
- 全電子波束轉向 — 在仰角和方位角上可達 ±45° 的電子轉向。
- 先進的信號處理 — FPGA 處理脈衝壓縮、Doppler FFT、MTI 和 CFAR。
- Python GUI — 提供用戶友好的界面，支持地圖整合和實時目標繪製。
...
> [!tip] 爆紅原因
> 建立 29 天內累積 3752 stars（129/天），forks 794（21.2%），顯示出強烈的社群興趣。專案的主要貢獻者 JJassonn69 和 NawfalMotii79 在開源硬體和軟體領域有豐富經驗，這使得專案具備良好的技術基礎。AERIS-10 解決了高成本雷達系統的問題，讓更多研究者和開發者能夠進行實驗。社群的活躍度高，熱門問題如「增加 LAN 接口」和「缺少文件夾」顯示出用戶對功能擴展和文檔支持的需求。這些因素共同促進了專案的快速成長。

## 適合誰使用

**目標受眾**：對相控陣列雷達技術有興趣的研究人員和開發者，尤其是希望進行實驗和創新的小型團隊。

> [!example] 使用場景
> - 無人機開發者用它來實現高精度的目標追蹤，因為 AERIS-10 提供了電子波束轉向和實時信號處理功能。
> - 學術研究者用它來進行雷達信號處理的實驗，因為其開源特性允許自由修改和擴展。
> - 高級製造者用它來探索相控陣列技術，因為完整的硬體和軟體文檔使得開發過程更為順利。

## 架構分析

AERIS-10 的架構設計為模組化，主要由電源管理板、頻率合成器板和主板組成。電源管理板確保所有電子元件的電壓供應，頻率合成器板使用高性能的低抖動時鐘生成器，為接收和發射頻率合成器提供相位對齊的時鐘參考。主板上集成了 FPGA，負責雷達信號處理，並支持多種信號處理功能，如脈衝壓縮和 Doppler FFT。

這種設計使得系統能夠靈活應對不同的應用需求，並且便於未來的擴展和升級。選擇 FPGA 而非傳統的微處理器，能夠在實時信號處理上提供更高的性能，但也增加了開發的複雜度。整體架構的模組化設計使得各個子系統之間的耦合度降低，便於維護和升級。

## 技術深入分析

AERIS-10 的核心技術機制在於其模組化設計和先進的信號處理能力。使用 FPGA 進行信號處理，能夠實現高效的脈衝壓縮和 Doppler FFT，這對於雷達系統的性能至關重要。該系統能夠處理的最大範圍為 20 公里，並支持電子波束轉向，這使得其在多種應用場景中具備靈活性。設計上的取捨包括選擇 FPGA 而非微處理器，雖然 FPGA 提供了更高的性能，但開發難度也相對增加。該系統的依賴樹相對簡單，主要依賴於 FPGA 和 Python，這降低了潛在的 vendor lock-in 風險。隨著使用者基數的擴大，未來可能會面臨性能瓶頸，特別是在高負載的信號處理情境下。整合到現有的開發環境中相對容易，因為其使用了廣泛的開源工具和標準。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了詳細的安裝步驟和使用指南。安裝過程順暢，但需要用戶具備一定的硬體組裝經驗。文件中有針對 GUI 的使用說明，對於新手來說相對友好。
...
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | 專注於信號處理的開源工具，與 AERIS-10 的硬體設計不同。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 專注於不同的頻率範圍，提供類似的開源雷達系統，但功能和應用場景有所不同。 | 如果需要特定頻率範圍的雷達系統，且不需要 AERIS-10 的模組化設計。 | medium，因為需要重新適應不同的系統架構和功能。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | 專注於信號處理的開源工具，與 AERIS-10 的硬體設計不同，適合需要強大信號處理能力的用戶。 | 如果主要需求是信號處理，而不需要完整的雷達系統。 | low，因為可以直接使用其信號處理功能。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PLFM_RADAR** | **parsync** | **Shadowbroker** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於不同的頻率範圍，提供類似的開源雷達系統，但功能和應用場景有所不同。 | 專注於信號處理的開源工具，與 AERIS-10 的硬體設計不同，適合需要強大信號處理能力的用戶。 |
> | 遷移成本 | - | medium，因為需要重新適應不同的系統架構和功能。 | low，因為可以直接使用其信號處理功能。 |
> | 適用場景 | 主要場景 | 如果需要特定頻率範圍的雷達系統，且不需要 AERIS-10  | 如果主要需求是信號處理，而不需要完整的雷達系統。 |
...
- [MEDIUM] 某些功能仍在開發中，可能不穩定
  - 解法：定期檢查 GitHub issues 獲取最新狀態
- **[HIGH]** 缺少某些文檔，導致使用困難
  - 解法：參考社群討論或直接詢問貢獻者
- [MEDIUM] 硬體組裝過程中可能出現問題
  - 解法：遵循提供的組裝指南，並尋求社群幫助
...
| 大型企業的商業雷達系統 | 不適合 | 尚未成熟，缺乏商業支持和穩定性。 |
| 學術研究機構進行雷達信號處理實驗 | 適合 | 開源和模組化設計便於實驗和修改。 |
| 個人愛好者進行 SDR 項目 | 非常適合 | 提供了豐富的功能和靈活性，適合探索和學習。 |
...
> [!tip] 投入 vs 回報
> 花 10 小時學習，20 小時整合，得到一個靈活的開源雷達系統，值得投資。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限，主要是硬體組裝和信號處理，無需存取敏感資料。依賴鏈相對簡單，沒有已知的供應鏈風險，適合在 CI/CD 中使用。

## 生態系整合

> [!abstract] 如何融入你的工具鏈

AERIS-10 最常與 FPGA 開發工具（如 Vivado）和 Python 環境搭配使用，適合在開發和測試階段進行集成。在使用中，開發者可以通過 Python GUI 控制雷達系統，並實時查看數據。與主流 CI 工具（如 GitHub Actions）相容，並能夠與多種 IDE（如 VS Code）整合。整合過程中，可能需要根據硬體配置進行調整，特別是在信號處理和數據輸出方面。

## 歷史脈絡

> [!info] 這個工具為什麼現在出現？

在 AERIS-10 出現之前，雷達系統通常需要高昂的成本和專業的技術支持，限制了許多開發者和研究者的使用。隨著開源硬體和軟體的興起，AERIS-10 提供了一個全新的解決方案，讓更多人能夠接觸和實驗雷達技術。技術的進步，如 FPGA 的普及和 Python 的廣泛應用，使得這個專案的實現變得可行。

未來，隨著社群的壯大和技術的進步，AERIS-10 可能會成為開源雷達技術的標準。
...
> | [#39](https://github.com/NawfalMotii79/PLFM_RADAR/issues/39) | DC notch, GUI, and cosim not yet adapted for dual 16-pt FFT | 0 | 2 |
