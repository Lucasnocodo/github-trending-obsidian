...
use_case: "提供一個開源、低成本的 10.5 GHz PLFM 相控陣雷達系統。"
priority: high
ring: assess
...
aliases:
  ...
  - "提供一個開源、低成本的 10.5 GHz PLFM 相控陣雷達系統。"
---

# PLFM_RADAR

**19.3k** stars · **333** stars/天 · 建立 58 天前 · PLSQL · NOASSERTION
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Growing · **熱度** Hot (333 stars/day)
> **授權** NOASSERTION · **維護** Active (最後推送 0 天前) · **貢獻者** 5+ 人 · **參與度** Medium
> **適合** 對雷達技術有興趣並希望進行實驗的研究者和開發者。
> **一句話重點** AERIS-10 不僅是雷達系統，更是一個開放的實驗平台，讓使用者能夠自由探索和創新。
...
> **結論** 花 10 小時學習，20 小時整合，得到一個靈活的雷達系統，值得嘗試。

> [!abstract] 核心創新
> AERIS-10 是一個完全開源的相控陣雷達系統，提供模組化設計和靈活的信號處理能力。

## 專案簡介

AERIS-10 專案旨在民主化雷達技術，提供一個完全開源的模組化雷達系統，適合大學研究者、無人機初創公司或進階製作者使用。使用者可以透過該系統進行波束形成、脈衝壓縮、多普勒處理和目標追蹤等實驗。核心機制包括一個 FPGA 負責信號處理，並且提供 ±45° 的電子波束轉向，讓使用者能夠靈活調整雷達的觀測方向。該系統的 GUI 使用 Python 開發，並整合 GPS/IMU 進行實時位置和姿態修正。這個工具的賣點在於其開源硬體與軟體的完整性，並且提供兩個版本，分別支持 3km 和 20km 的探測範圍。

技術實作上，AERIS-10 使用 Verilog 和 C++ 進行 FPGA 和軟體的開發，並且依賴 Vivado 工具進行信號處理的修改。與其他雷達系統相比，AERIS-10 的模組化設計使得硬體組裝和維護更加靈活，並且開源的特性降低了進入門檻。實際使用中，使用者可能會面臨 PCB 組裝的挑戰，並需要具備基本的雷達原理知識。這個專案目前處於穩定版本，適合對雷達技術有興趣的開發者和研究者使用。未來可能會增加更多的功能和改進，特別是在信號處理和用戶界面方面。

**技術棧**：`Python 3.8+` · `Verilog` · `C++` · `Vivado`

## 重點功能

- 開源硬體與軟體 — 提供完整的電路圖、PCB 佈局、韌體和軟體。
- 雙版本可用性 — AERIS-10N (3km) 和 AERIS-10E (20km) 兩種版本。
- 全電子波束轉向 — 在仰角和方位角上 ±45° 的電子轉向。
- 先進的信號處理 — FPGA 負責脈衝壓縮、多普勒 FFT、MTI 和 CFAR。
- Python GUI — 友好的用戶界面，並整合地圖功能。
...
> [!tip] 爆紅原因
> 建立 58 天內累積 19315 stars（333/天），forks 4602（23.8%），顯示出強烈的社群興趣。這個專案由 NawfalMotii79 和其他貢獻者推動，解決了開源雷達技術的需求，之前的解決方案多數是封閉的商業產品，無法滿足研究和開發的靈活性。社群對於開源硬體的需求不斷增加，尤其是在無人機和自動化領域。這個專案的開放性和模組化設計吸引了許多開發者的注意，特別是在雷達技術的應用上。forks/stars 比率高達 23.8%，顯示出許多人在實際修改和使用這個專案。

## 適合誰使用

**目標受眾**：對雷達技術有興趣並希望進行實驗的研究者和開發者。

> [!example] 使用場景
> - 無人機開發者用它來測試和優化雷達系統，因為其開源特性讓他們能夠自由修改和擴展功能。
> - 大學研究者用它來進行雷達信號處理的實驗，因為提供完整的硬體和軟體設計能夠加速研究進程。
> - 進階製作者用它來創建自定義的雷達應用，因為模組化設計使得硬體組裝和功能擴展變得簡單。

## 架構分析

AERIS-10 採用模組化設計，主要由電源管理板、頻率合成板和 FPGA 組成。電源管理板負責為所有電子元件提供必要的電壓和濾波，頻率合成板使用高性能的低抖動時鐘生成器來提供相位對齊的時鐘參考。這樣的設計使得系統能夠在不同的應用場景中靈活調整，並且降低了維護的複雜度。選擇 FPGA 進行信號處理的原因在於其高效能和可編程性，能夠實現複雜的信號處理算法。模組化設計的代價是需要較高的硬體組裝技術，並且在擴展性方面可能會受到某些硬體限制的影響。

## 技術深入分析

AERIS-10 的核心技術機制包括使用 FPGA 進行信號處理，這使得系統能夠實現高效的脈衝壓縮和多普勒處理。該系統的設計考量了模組化和開源的特性，使得使用者能夠根據需求進行擴展和修改。效能方面，FPGA 的使用能夠處理高達 400 MHz 的信號，並且在實時處理上具有優勢。選擇 Python 作為 GUI 開發語言的原因在於其易用性和廣泛的生態系統，這使得使用者能夠快速上手並進行二次開發。

設計上的取捨包括在開源和商業化之間的平衡，這使得專案能夠吸引更多的開發者參與。技術風險方面，隨著使用者數量的增加，可能會面臨更多的維護和支持需求。整合方面，AERIS-10 可以與多種開發工具和平台相容，並且支持 CI/CD 流程的集成，這使得團隊能夠在現有的工作流程中輕鬆採用。整體來看，AERIS-10 提供了一個強大的平台，讓使用者能夠在雷達技術上進行創新和實驗。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的介紹和安裝步驟，但缺乏詳細的範例。安裝過程相對順暢，但對於硬體組裝的部分可能需要額外的指導。文件目前僅提供英文，對於非英語使用者可能會有一定的障礙。
...
> [!danger] 缺點
> - 需要一定的硬體組裝經驗。
> - 對於初學者來說，理解雷達原理可能有一定難度。
> - 目前僅支援特定版本的 Python，限制了使用者的選擇。

> [!warning] 注意事項
> - 需要基本的雷達原理知識。
> - 需要具備 PCB 組裝經驗以進行硬體建置。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於開發 AI agent，而 AERIS-10 專注於雷達技術，功能和應用場景完全不同。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard 是一個開源硬體平台，但不專注於雷達技術，AERIS-10 提供專門的雷達信號處理功能。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於開發 AI agent，而 AERIS-10 專注於雷達技術，功能和應用場景完全不同。 | 如果你的專案需要開發智能代理而非雷達系統，則應選擇此工具。 | medium，因為需要重新設計整個系統架構。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard 是一個開源硬體平台，但不專注於雷達技術，AERIS-10 提供專門的雷達信號處理功能。 | 如果你需要一個通用的開源硬體平台，而不特定於雷達應用，則選擇此工具。 | low，因為兩者的硬體設計可能有相似之處。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PLFM_RADAR** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於開發 AI agent，而 AERIS-10 專注於雷達技術，功能和應用場景完全不同。 | Boneyard 是一個開源硬體平台，但不專注於雷達技術，AERIS-10 提供專門的雷達信號處理功能。 |
> | 遷移成本 | - | medium，因為需要重新設計整個系統架構。 | low，因為兩者的硬體設計可能有相似之處。 |
> | 適用場景 | 主要場景 | 如果你的專案需要開發智能代理而非雷達系統，則應選擇此工具。 | 如果你需要一個通用的開源硬體平台，而不特定於雷達應用，則選擇 |
...
- **[HIGH]** 在某些環境下，FPGA 開發工具可能無法正確識別硬體配置
  - 解法：確保所有硬體連接正確，並重新啟動開發環境
- [MEDIUM] 雷達信號處理過程中可能會出現延遲
  - 解法：優化 FPGA 設計以減少延遲
- [MEDIUM] 使用 Python GUI 時，可能會遇到相依性衝突
  - 解法：使用虛擬環境管理 Python 依賴

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司進行雷達技術開發 | 非常適合 | 開源特性降低了開發成本，並且模組化設計便於快速原型製作。 |
| 大型研究機構進行雷達信號處理研究 | 適合 | 提供完整的硬體和軟體設計，支持高效的信號處理。 |
| 個人開發者進行雷達相關實驗 | 非常適合 | 開源和低成本使得個人開發者能夠輕鬆進行實驗。 |
| 需要商業化的雷達產品開發 | 不適合 | 目前仍處於 beta 階段，穩定性和支持性不足。 |
...
> [!tip] 投入 vs 回報
> 花 10 小時學習，20 小時整合，得到一個靈活的雷達系統，值得嘗試。
...
> | [#134](https://github.com/NawfalMotii79/PLFM_RADAR/issues/134) | Possible Typos / Incorrect Part Numbers in the BOM File | 0 | 3 |
...
相關專案：[[0x0funky--agent-sprite-forge|0x0funky/agent-sprite-forge]] · [[0xGF--boneyard|0xGF/boneyard]] · [[432539--gpt2api|432539/gpt2api]] · [[NVIDIA--NemoClaw|NVIDIA/NemoClaw]] · [[AgriciDaniel--claude-obsidian|AgriciDaniel/claude-obsidian]] · [[BuilderPulse--BuilderPulse|BuilderPulse/BuilderPulse]] · [[AgentSeal--codeburn|AgentSeal/codeburn]] · [[BigBodyCobain--Shadowbroker|BigBodyCobain/Shadowbroker]]
