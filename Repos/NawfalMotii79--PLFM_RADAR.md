...
**3.7k** stars · **133** stars/天 · 建立 28 天前 · PLSQL · NOASSERTION
...
> **適合** 希望在雷達技術上進行實驗的研究人員和開發者，特別是對開源硬體有興趣的人。
> **一句話重點** AERIS-10 不僅是一個雷達系統，它是一個開源社群的實驗平台，讓更多人能夠接觸和學習雷達技術。
...
> **結論** 花 20 小時學習，15 小時整合，得到高性能的雷達系統，值得投入。

> [!abstract] 核心創新
> AERIS-10 提供了一個完全開源的相控陣列雷達系統，讓研究人員和開發者能夠自由探索和實驗。

## 專案簡介

AERIS-10 是一個開源的 10.5 GHz 相控陣列雷達系統，專為研究人員、無人機開發者和 SDR 愛好者設計。用戶可以選擇 3km 或 20km 的版本，並利用其模組化設計進行實驗。該系統支持電子波束轉向，並具備先進的信號處理能力，使用 FPGA 進行脈衝壓縮和多普勒處理。用戶可以透過 Python GUI 進行操作，並整合 GPS/IMU 進行實時位置修正。這個系統的「一句話賣點」是其開源硬體和軟體的完整性，讓使用者能夠深入探索雷達技術。

系統架構包括電源管理、頻率合成和主板，每個子系統都經過精心設計以確保性能和穩定性。相較於其他商業雷達系統，AERIS-10 提供了更低的成本和更高的可定制性，特別適合學術研究和創客實驗。使用者可以在 3km 和 20km 的範圍內進行測試，並且可根據需求調整硬體配置。這個專案的社群活躍度高，開發者之間的互動頻繁，能夠快速解決問題。考慮到其開源特性和模組化設計，這個系統適合於小型團隊和個人開發者，尤其是那些希望在雷達技術上進行創新的人。

**技術棧**：`Python` · `Verilog` · `C++` · `MATLAB` · `Tcl`

## 重點功能

- 開源硬體與軟體 — 提供完整的原理圖、PCB 佈局、韌體和軟體。
...
```plsql
{
  ...
  "指令": "python gui.py",
  "預期輸出": "啟動 AERIS-10 的用戶界面，顯示雷達數據和控制選項。"
}
```
...
> [!tip] 爆紅原因
> 建立 28 天內累積 3714 stars（133/天），forks 789（21.2%），顯示出強烈的社群興趣。作者 NawfalMotii79 和 JJassonn69 在開源硬體領域有豐富經驗，這個專案解決了高成本雷達系統的痛點，讓更多人能夠接觸和使用雷達技術。近期的社群討論和建議（如建立協作群組）也顯示出使用者對於進一步合作的需求，這有助於提升專案的發展潛力。技術上，開源的設計理念和模組化架構使得這個系統能夠快速適應不同的應用場景，並且降低了使用門檻。

## 適合誰使用

**目標受眾**：希望在雷達技術上進行實驗的研究人員和開發者，特別是對開源硬體有興趣的人。

> [!example] 使用場景
> - 無人機開發者用它來測試和驗證自動導航系統，因為其 20km 的範圍和高精度的信號處理能力能夠提供可靠的數據。
> - 學術研究者用它來進行雷達信號處理的實驗，因為開源的設計讓他們能夠自由修改和擴展系統功能。
> - 創客用它來設計和實現自定義的雷達應用，因為模組化的架構使得硬體和軟體的整合變得簡單。

## 架構分析

AERIS-10 的架構設計為模組化，主要由電源管理板、頻率合成板和主板組成。電源管理板確保所有電子元件的電壓穩定，頻率合成板使用高性能時鐘生成器提供相位對齊的時鐘參考。主板則包含 DAC、微波混頻器和 FPGA，負責雷達信號的生成和處理。

這樣的設計使得各個模組可以獨立升級或更換，降低了維護成本。選擇 FPGA 處理信號的好處是能夠實現高效能的即時處理，但代價是需要較高的技術門檻來設置和編程。整體架構的擴展性良好，但在高負載情況下可能會遇到性能瓶頸，特別是在信號處理的實時性方面。

## 技術深入分析

AERIS-10 的核心技術機制包括使用 FPGA 進行信號處理，這使得系統能夠實現高效的脈衝壓縮和多普勒處理。系統能夠處理的數據量取決於 FPGA 的性能，通常能夠在實時中處理多達數百個樣本。設計上選擇 FPGA 而非傳統的微控制器，能夠提供更高的計算能力和靈活性，但也增加了設計的複雜度。系統的依賴關係相對簡單，主要依賴於 FPGA 和高性能的時鐘生成器，這降低了潛在的供應鏈風險。技術風險方面，FPGA 的配置和編程可能在擴展時遇到瓶頸，特別是當需要處理更高頻率或更複雜的信號時。與主流的開發框架如 MATLAB 和 Python 的整合相對容易，因為這些工具都能夠支持 FPGA 的開發和測試流程，並且有豐富的文檔和社群支持。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的介紹和安裝步驟，但缺乏詳細的範例。安裝過程相對順暢，但對於新手來說，可能需要額外的技術支持。沒有專門的快速入門指南，文件主要以英文為主，缺少多語言支持。
...
> [!warning] 注意事項
> - 目前僅支持 10.5 GHz 頻段，無法擴展到其他頻段。
> - 需要專業知識來設置和調整硬體配置。
> - 對於新手來說，可能需要時間來熟悉系統架構和操作。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 提供類似的開源雷達系統，但專注於低功耗設計，適合小型無人機使用。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | 專注於信號處理，但不具備完整的硬體設計，適合已有硬體的開發者。 |
| [NVIDIA/NemoClaw](https://github.com/NVIDIA/NemoClaw) | 提供高性能的雷達系統，但成本較高，適合商業用途。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 專注於低功耗設計，適合小型無人機使用，與 AERIS-10 的高性能設計形成對比。 | 如果你的應用需要長時間運行且對功耗敏感，則應選擇它。 | medium，因為需要調整硬體和軟體設計以適應不同的架構。 |
| [NVIDIA/NemoClaw](https://github.com/NVIDIA/NemoClaw) | 提供高性能的雷達系統，但成本較高，適合商業用途，與 AERIS-10 的開源特性形成對比。 | 如果你的專案需要商業級的性能和支持，則應選擇它。 | high，因為需要重構整個系統以符合商業標準。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PLFM_RADAR** | **parsync** | **NemoClaw** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於低功耗設計，適合小型無人機使用，與 AERIS-10 的高性能設計形成對比。 | 提供高性能的雷達系統，但成本較高，適合商業用途，與 AERIS-10 的開源特性形成對比。 |
> | 遷移成本 | - | medium，因為需要調整硬體和軟體設計以適應不同的架構。 | high，因為需要重構整個系統以符合商業標準。 |
> | 適用場景 | 主要場景 | 如果你的應用需要長時間運行且對功耗敏感，則應選擇它。 | 如果你的專案需要商業級的性能和支持，則應選擇它。 |
...
> [!tip] 採用建議
> 適合個人和小型團隊的實驗性專案，不建議用於生產環境的核心應用。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在高頻操作時，可能會遇到信號干擾問題。
  - 解法：確保所有連接良好並使用適當的屏蔽材料。
- [MEDIUM] 某些元件的供應可能不穩定，影響組裝。
  - 解法：提前規劃元件採購，並考慮替代品。
- **[HIGH]** FPGA 配置不當可能導致性能下降。
  - 解法：參考官方文檔進行配置，並進行充分測試。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 大學研究實驗室的雷達信號處理研究 | 非常適合 | 開源設計和模組化架構能夠支持多樣化的實驗需求。 |
| 小型無人機開發團隊 | 適合 | 提供的 3km 和 20km 版本能夠滿足不同的應用需求。 |
| 大型商業雷達系統開發 | 不適合 | 目前尚未成熟，且缺乏商業支持。 |
...
> [!tip] 投入 vs 回報
> 花 20 小時學習，15 小時整合，得到高性能的雷達系統，值得投入。
...
> | [@NawfalMotii79](https://github.com/NawfalMotii79) | 95 |

## 社群與生態

**社群活躍度**：社群活躍，定期有新的討論和問題解決。
**連結**：[文件](https://github.com/NawfalMotii79/PLFM_RADAR/wiki)

## 開發動態

> [!abstract] 最近 10 次 commit（2026-04-02 ~ 2026-04-05）
...
> | [#23](https://github.com/NawfalMotii79/PLFM_RADAR/issues/23) | Missing Gerberfiles and Schematics | 0 | 8 |
> | [#17](https://github.com/NawfalMotii79/PLFM_RADAR/issues/17) | 3D Enclosure | 0 | 9 |
...
相關專案：[[AlpinDale--parsync|AlpinDale/parsync]] · [[BigBodyCobain--Shadowbroker|BigBodyCobain/Shadowbroker]] · [[NVIDIA--NemoClaw|NVIDIA/NemoClaw]] · [[L42ARO--Mercury-Transforming-Drone|L42ARO/Mercury-Transforming-Drone]] · [[Flowseal--tg-ws-proxy|Flowseal/tg-ws-proxy]] · [[novatic14--MANPADS-System-Launcher-and-Rocket|novatic14/MANPADS-System-Launcher-and-Rocket]] · [[TinyAGI--fractals|TinyAGI/fractals]] · [[aiming-lab--AutoResearchClaw|aiming-lab/AutoResearchClaw]]
