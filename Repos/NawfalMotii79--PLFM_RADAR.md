...
**3.7k** stars · **142** stars/天 · 建立 26 天前 · Verilog · NOASSERTION
...
> **適合** 對相控陣雷達技術有興趣的研究人員和開發者，特別是無人機和 SDR 領域的專業人士。
> **一句話重點** AERIS-10 是一個開源雷達系統，讓更多開發者能夠接觸和實驗先進的雷達技術。
...
> [!abstract] 核心創新
> AERIS-10 提供了一個完全開源的相控陣雷達系統，讓研究者和開發者能夠自由探索和實驗。

## 專案簡介

AERIS-10 是一個開源的 10.5 GHz 相控陣雷達系統，專為研究人員和無人機開發者設計。它提供兩種版本，分別為 3 公里和 20 公里的探測範圍，並支援脈衝線性頻率調變（LFM）調製。使用者可以透過 Python GUI 進行實時目標繪製和雷達控制，並整合 GPS 和 IMU 進行位置和姿態校正。該系統的核心處理由 FPGA 負責，能進行脈衝壓縮、Doppler FFT、MTI 和 CFAR 等信號處理。

AERIS-10 的模組化設計使得各個子系統（如電源管理、頻率合成和 RF 板）可獨立開發和測試，這樣的設計不僅降低了成本，也提高了可擴展性。與其他商業雷達系統相比，AERIS-10 提供了更高的可定制性，適合對雷達技術有深入需求的開發者和研究者。使用者需要具備基本的雷達原理知識及 PCB 組裝經驗，以便進行硬體組裝和調試。整體而言，這是一個適合學術研究和創新開發的雷達平台。
...
- 雙版本可選 — AERIS-10N 提供 3 公里範圍，AERIS-10E 提供 20 公里範圍。
...
> [!tip] 爆紅原因
> 建立 26 天內累積 3681 stars（142/天），forks 776（21.1%），顯示出強烈的社群興趣。專案由 NawfalMotii79 和 JJassonn69 共同開發，解決了傳統雷達系統昂貴且難以接觸的問題，讓更多開發者能夠探索相控陣雷達技術。社群對於開源雷達技術的需求日益增加，特別是在無人機和自動化領域。這個專案的開放性和模組化設計使其成為研究和開發的理想選擇，並且有助於推動相關技術的進步。

## 適合誰使用

**目標受眾**：對相控陣雷達技術有興趣的研究人員和開發者，特別是無人機和 SDR 領域的專業人士。

> [!example] 使用場景
> - 無人機開發者用它來實現高精度的目標追蹤，因為 AERIS-10 提供了先進的信號處理功能和實時數據可視化。
> - 學術研究者用它來進行雷達信號處理的實驗，因為開源的硬體和軟體設計使得修改和擴展變得容易。
> - 電子工程師用它來測試和驗證新的雷達技術，因為其模組化設計允許快速迭代和測試不同的配置。

## 架構分析

AERIS-10 的架構設計為模組化，主要由電源管理板、頻率合成器板和主板組成。電源管理板提供所有電子元件所需的電壓和過濾，頻率合成器板使用高效能的低抖動時鐘生成器來提供相位對齊的時鐘參考。主板包含 DAC、微波混合器、相位移器和 FPGA，負責雷達信號處理。

這樣的設計使得系統能夠靈活應對不同的應用需求，並且便於維護和升級。選擇 FPGA 進行信號處理的原因在於其高效能和可編程性，這使得開發者能夠根據需求調整處理算法。模組化設計的代價在於需要更多的組裝和調試工作，但這也使得系統更具可擴展性。

## 技術深入分析

AERIS-10 的核心技術機制包括使用 FPGA 進行信號處理，這使得系統能夠高效地處理複雜的雷達信號。系統能夠處理的資料量相當龐大，因為它需要實時捕捉和分析來自多個通道的信號。設計上選擇 FPGA 而非傳統的微控制器，因為 FPGA 提供了更高的處理速度和靈活性，能夠應對不同的信號處理需求。這種選擇雖然增加了設計的複雜性，但也讓系統在性能上有了顯著提升。技術風險方面，若未來需求增加，可能會面臨擴展性瓶頸，特別是在硬體資源的配置上。整合方面，與主流開發工具如 Vivado 的相容性良好，但在 CI/CD 流程中可能需要額外的適配工作。整體而言，AERIS-10 提供了一個強大的平台，適合進行雷達技術的研究和開發。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了詳細的安裝和組裝指南，對於新手來說相對友好。安裝過程需要一些 PCB 組裝經驗，但整體流程清晰。文件中包含了多個參考資料和範例，幫助使用者更快上手。整體來說，花 30 分鐘應該能夠完成基本的環境搭建。
