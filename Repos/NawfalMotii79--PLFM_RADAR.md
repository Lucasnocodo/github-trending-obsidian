...
star_history: "2026-03-15:1703,2026-03-15:1708,2026-03-16:2576,2026-03-17:3042,2026-03-18:3202,2026-03-19:3341,2026-03-20:3405,2026-03-21:3448,2026-03-22:3471,2026-03-23:3512,2026-03-24:3539,2026-03-25:3549,2026-03-26:3559,2026-03-27:3573,2026-03-28:3584"
tags:
  - github
  - "category/基礎設施"
  - "lang/verilog"
...
**3.6k** stars · **189** stars/天 · 建立 19 天前 · Verilog · NOASSERTION
...
> **適合** 對雷達技術有興趣的研究人員和開發者，特別是具備 PCB 組裝和 Python 開發能力的團隊。
> **一句話重點** AERIS-10 讓雷達技術變得可接觸，並鼓勵開發者進行創新和實驗。
...
> **結論** 花 20 小時學習，15 小時整合，得到靈活的雷達系統，值得投入。

> [!abstract] 核心創新
> 提供開源、低成本的相控陣列雷達系統，讓更多人能夠進入雷達技術領域。

## 專案簡介

AERIS-10 是一款開源的 10.5 GHz 相控陣列雷達系統，專為研究人員和無人機開發者設計，支持 3km 和 20km 的兩種版本。其核心機制包括使用 DAC 生成脈衝線性頻率調變（LFM）信號，並通過 FPGA 進行先進的信號處理，最終將數據通過 Python GUI 進行可視化。這種設計的賣點在於其開放性和模組化，讓使用者能夠自由探索和實驗雷達技術。系統架構上，AERIS-10 結合了多個硬體模組，如電源管理板、頻率合成器和主板，並使用 STM32 微控制器進行系統管理和 GPS/IMU 整合。這樣的設計選擇使得雷達系統的組裝和維護相對簡單，並且能夠靈活應對不同的應用需求。

與其他雷達系統相比，AERIS-10 的模組化設計使得其在功能擴展和維護上具有優勢，特別適合於學術研究和開發環境。使用者可以通過簡單的步驟進行硬體組裝，並利用提供的 GUI 進行實時數據監控和控制。這個專案的社群活躍度高，開發者可以輕鬆獲得支援和更新。適合有 PCB 組裝經驗和 Python 開發能力的團隊使用，未來可能會進一步增強其功能和擴展性。
...
- 雙版本可用性 — AERIS-10N (3km) 和 AERIS-10E (20km) 兩種版本，滿足不同需求。
- 全電子波束轉向 — 在仰角和方位角上實現 ±45° 的電子轉向。
- 先進信號處理 — FPGA 處理脈衝壓縮、Doppler FFT、MTI 和 CFAR。
- Python GUI — 提供用戶友好的界面，支持地圖整合和實時數據顯示。
- GPS/IMU 整合 — 實時位置和姿態修正，提升數據準確性。
- 模組化設計 — 獨立的電源管理、頻率合成和 RF 板，便於維護和擴展。
...
2. 源材料
```bash
在 /4_Schematics and Boards Layout/4_7_Production Files 獲取材料清單 (BOM)
```
3. 組裝
```bash
...
> [!tip] 爆紅原因
> 建立 19 天內累積 3584 stars（189/天），forks 762（21.3%），顯示出強烈的社群興趣。這個專案的作者 NawfalMotii79 和 JJassonn69 具有豐富的開源經驗，解決了傳統雷達系統高成本和封閉性問題，讓更多開發者能夠進入這一領域。專案的開源特性和模組化設計吸引了許多對雷達技術感興趣的研究者和開發者。社群中的討論和建議（如創建協作群組）也顯示出使用者對於進一步合作的渴望。這些因素共同促進了專案的快速增長。

## 適合誰使用

**目標受眾**：對雷達技術有興趣的研究人員和開發者，特別是具備 PCB 組裝和 Python 開發能力的團隊。

> [!example] 使用場景
> - 無人機開發者用它來測試和優化飛行控制系統，因為 AERIS-10 提供了實時的目標追蹤和位置數據。
> - 學術研究者用它來進行雷達信號處理的實驗，因為其開源設計允許自由修改和擴展功能。
> - 工程師用它來開發新型的雷達應用，因為模組化設計使得硬體組裝和維護變得簡單。

## 架構分析

AERIS-10 的架構設計為模組化，便於使用者根據需求進行擴展和修改。其主要組件包括電源管理板、頻率合成器和主板，這樣的設計使得每個模組都能獨立運作，降低了故障風險。資料流方面，雷達信號的生成、處理和可視化均由 FPGA 和 STM32 微控制器協同完成，確保了高效的數據處理能力。選擇 FPGA 進行信號處理的優勢在於其高效能和靈活性，但相對的也增加了開發的複雜度。整體而言，這種架構設計使得 AERIS-10 在功能上具備擴展性，能夠應對不同的應用需求，但在初期設置和維護上可能需要一定的技術門檻。

## 技術深入分析

AERIS-10 的核心技術機制包括使用 FPGA 進行信號處理，這使得其在脈衝壓縮和 Doppler FFT 等方面具備高效能。系統能夠處理的數據量相當可觀，因為 FPGA 能夠同時進行多個信號處理任務，並且支持實時數據流。設計上選擇 STM32 微控制器進行系統管理，這樣的選擇使得整體系統的功耗較低，並且能夠有效控制各個模組的協作。技術選型上，FPGA 的使用雖然提高了開發的難度，但帶來了更高的靈活性和性能。整體依賴樹相對簡單，主要依賴於 FPGA 開發工具和 Python 環境，這降低了使用者的入門門檻。隨著使用者對於雷達技術的需求增加，AERIS-10 的設計也可能會進一步演進，未來可能會增加更多的功能和擴展性。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，包含詳細的安裝步驟和組裝指南。安裝過程順暢，但對於新手來說，可能需要額外的學習時間來理解 PCB 組裝和 FPGA 開發。文件目前僅提供英文版本，可能對非英語使用者造成一定的障礙。
...
> [!danger] 缺點
> - 對於新手來說，組裝和設置過程可能較為複雜。
> - 需要專業的 FPGA 開發工具和知識。
> - 僅支持特定的頻率範圍，限制了應用場景。

> [!warning] 注意事項
> - 僅支援 10.5 GHz 頻率範圍，無法調整到其他頻段。
> - 需要具備 PCB 組裝經驗，對於新手可能有一定的學習曲線。
> - FPGA 開發需要使用 Vivado 工具，對於不熟悉的使用者可能會有挑戰。
...
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | 專注於信號處理和數據分析，與 AERIS-10 的硬體設計相比，功能較為單一。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 專注於雷達數據的同步處理，提供更高的數據處理效率。 | 如果你的專案需要高效的數據同步處理，而不僅僅是雷達信號生成。 | medium，因為需要重新設計數據處理流程。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | 專注於信號處理和數據分析，功能較為單一。 | 如果你的需求主要集中在信號處理，而不需要硬體組裝的靈活性。 | low，因為功能相似，轉移相對簡單。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PLFM_RADAR** | **parsync** | **Shadowbroker** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於雷達數據的同步處理，提供更高的數據處理效率。 | 專注於信號處理和數據分析，功能較為單一。 |
> | 遷移成本 | - | medium，因為需要重新設計數據處理流程。 | low，因為功能相似，轉移相對簡單。 |
> | 適用場景 | 主要場景 | 如果你的專案需要高效的數據同步處理，而不僅僅是雷達信號生成。 | 如果你的需求主要集中在信號處理，而不需要硬體組裝的靈活性。 |
...
- **[HIGH]** 在組裝過程中，若未正確配置電源管理，可能導致系統無法啟動。
  - 解法：仔細檢查電源管理板的連接和配置。
- [MEDIUM] FPGA 開發工具的版本不匹配可能導致編譯失敗。
  - 解法：確保使用與專案相容的 Vivado 版本。
- [MEDIUM] Python GUI 可能在某些環境下無法正常運行。
  - 解法：確認 Python 版本為 3.8 以上，並安裝所有必要的依賴。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司進行雷達技術開發 | 非常適合 | 開源設計和模組化架構使得開發成本低且靈活。 |
| 大型企業的核心雷達系統 | 不適合 | 目前仍處於 beta 階段，穩定性和支持性不足。 |
| 學術機構進行雷達信號處理研究 | 適合 | 開源特性和先進的信號處理能力非常符合研究需求。 |
| 個人開發者進行無人機雷達應用 | 適合 | 模組化設計和易於組裝的特性適合個人開發者。 |
...
> [!tip] 投入 vs 回報
> 花 20 小時學習，15 小時整合，得到靈活的雷達系統，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限，且不存取敏感資料。依賴鏈相對簡單，無已知的供應鏈風險，適合在 CI/CD 中使用。
...
> | [@NawfalMotii79](https://github.com/NawfalMotii79) | 70 |
...
> [!abstract] 最近 10 次 commit（2026-03-20 ~ 2026-03-27）
> **活躍天數** 4 天 · **最新 commit** Merge pull request #33 from JJassonn69/fix/staggered-prf-dual16-doppler
...
> | [#32](https://github.com/NawfalMotii79/PLFM_RADAR/issues/32) | MCU scan sequence does not match documented 31-elevation / ± | 0 | 4 |
