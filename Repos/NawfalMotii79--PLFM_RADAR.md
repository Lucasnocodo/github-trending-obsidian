...
use_case: "提供開源、低成本的 10.5 GHz PLFM 相控陣雷達系統，讓研究人員和開發者能夠輕鬆實驗雷達技術。"
priority: high
ring: assess
...
aliases:
  ...
  - "提供開源、低成本的 10.5 GHz PLFM 相控陣雷達系統，讓研究人員和開發者能夠輕鬆實驗雷達技術。"
---

# PLFM_RADAR

**20.2k** stars · **293** stars/天 · 建立 69 天前 · PLSQL · NOASSERTION
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Growing · **熱度** Hot (293 stars/day)
> **授權** NOASSERTION · **維護** Active (最後推送 1 天前) · **貢獻者** 5+ 人 · **參與度** Medium
> **適合** 對雷達技術有興趣的研究人員和開發者，特別是希望進行實驗和開發的學生和初創公司。
> **一句話重點** AERIS-10 讓雷達技術變得可接觸，並且其開源特性促進了技術的民主化。
...
> [!abstract] 核心創新
> AERIS-10 提供了一個完全開源的相控陣雷達系統，讓更多人能夠接觸和實驗雷達技術。

## 專案簡介

AERIS-10 是一個開源的相控陣雷達系統，專為研究和開發而設計，支持 10.5 GHz 的脈衝線性頻率調變（PLFM）。用戶可以透過 FPGA 進行信號處理，並使用 Python GUI 進行操作，這樣的設計使得雷達技術的學習和實驗變得更加容易。該系統有兩個版本：AERIS-10N 提供 3 公里的探測範圍，而 AERIS-10E 則可達 20 公里，滿足不同需求的用戶。這個系統的電子波束控制範圍達到 ±45°，並且具備先進的信號處理功能，如脈衝壓縮和多普勒 FFT。使用者需具備基本的雷達原理知識和 PCB 組裝經驗，並需安裝 Python 3.8 以上版本及 FPGA 開發工具（Vivado）。相較於傳統的雷達系統，AERIS-10 的模組化設計和開源特性使得其更具靈活性和可擴展性，適合各種實驗和開發需求。這個專案的社群活躍度高，開發者可以從中獲得支持和資源，並且有助於推動雷達技術的民主化。

**技術棧**：`Python 3.8+` · `Verilog` · `C++` · `MATLAB`

## 重點功能

- 開源硬體與軟體 — 完整的電路圖、PCB 佈局、韌體和軟體均可獲得。
- 雙版本可選 — AERIS-10N 提供 3km 的範圍，AERIS-10E 提供 20km 的範圍。
...
> [!tip] 爆紅原因
> 建立 69 天內累積 20249 stars（293/天），forks 4813（23.8%），顯示出強烈的社群參與。這個專案由多位貢獻者共同開發，解決了傳統雷達系統昂貴且難以接觸的問題，讓更多人能夠進行雷達技術的實驗。社群的活躍討論和問題解決也促進了專案的快速成長。技術上，開源硬體的興起和 FPGA 技術的普及使得這個專案的實現變得可行。高達 23.8% 的 forks/stars 比率顯示出許多開發者在實際修改和使用這個專案，這代表著強烈的實際應用潛力。

## 適合誰使用

**目標受眾**：對雷達技術有興趣的研究人員和開發者，特別是希望進行實驗和開發的學生和初創公司。

> [!example] 使用場景
> - 大學研究生用它來進行雷達信號處理實驗，因為開源的設計讓他們能夠自由修改和擴展功能。
> - 無人機開發者用它來集成雷達系統於其平台，因為其模組化設計使得整合變得簡單且靈活。
> - 電子工程師用它來測試新型的波束成形技術，因為其提供的完整硬體和軟體資源能加速開發流程。

## 架構分析

AERIS-10 的架構由多個模組組成，包括電源管理板、頻率合成器板和 FPGA 信號處理單元。這樣的設計使得每個模組可以獨立開發和升級，降低了整體系統的維護難度。電源管理板確保所有電子元件的電壓供應和過濾，頻率合成器則提供相位對齊的時鐘參考，這對於信號的準確性至關重要。

選擇 FPGA 進行信號處理的原因是其高效能和靈活性，能夠實現複雜的數字信號處理任務。這樣的設計雖然增加了開發的複雜性，但也帶來了更高的性能和可擴展性。整體架構的模組化設計使得未來的擴展和功能添加變得更加容易。

## 技術深入分析

AERIS-10 的核心技術機制是基於 FPGA 的數字信號處理，這使得其能夠實現高效的脈衝壓縮和多普勒分析。系統設計上，使用了高性能的低抖動時鐘生成器來確保信號的準確性，這對於雷達系統至關重要。該系統的效能特性使其能夠處理高達 20 公里的探測範圍，並且在實際應用中能夠保持穩定的性能。選擇 FPGA 而非傳統的微控制器是因為其在處理速度和靈活性上的優勢，這樣的選擇雖然增加了開發的複雜度，但也帶來了更高的性能。技術風險方面，FPGA 的配置和開發需要專業知識，這可能在團隊擴大時成為瓶頸。整合方面，與主流的開發環境如 Vivado 的兼容性良好，但對於不熟悉 FPGA 的開發者來說，可能需要額外的學習成本。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的指導和必要的背景知識，安裝過程相對順暢，但對於新手來說，可能會遇到一些挑戰。缺乏詳細的 getting started guide，可能需要用戶自行探索。文件目前僅提供英文版本。
...
> | [#143](https://github.com/NawfalMotii79/PLFM_RADAR/issues/143) | Impedance control PCBs layer clarification and issues | 0 | 6 |
> | [#141](https://github.com/NawfalMotii79/PLFM_RADAR/issues/141) | Why are all the capacitors and resistors on the circuit boar | 0 | 1 |
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["開源硬體","信號處理","相控陣雷達"];
