...
**7.3k** stars · **197** stars/天 · 建立 37 天前 · PLSQL · NOASSERTION
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Growing · **熱度** Hot (197 stars/day)
> **授權** NOASSERTION · **維護** Active (最後推送 0 天前) · **貢獻者** 3 人 · **參與度** Medium
> **適合** 對雷達技術有興趣的研究人員和開發者，特別是希望進行實驗和開發的 SDR 愛好者。
> **一句話重點** 這個專案不僅是雷達技術的開源實踐，更是對於開發者社群的鼓勵，讓更多人能夠參與到高科技的探索中。
...
> **結論** 花 20 小時學習，40 小時整合，得到一個強大的開源雷達系統，值得投入。

> [!abstract] 核心創新
> 這個專案的核心創新在於提供一個完全開源的相控陣列雷達系統，讓更多開發者能夠進行實驗和研究。

## 專案簡介

AERIS-10 是一個開源的 10.5 GHz 相控陣列雷達系統，專為研究人員、無人機開發者和 SDR 愛好者設計。其核心機制是透過脈衝線性頻率調變（LFM）來生成雷達波形，並利用 FPGA 進行信號處理，最終將數據透過 Python GUI 進行可視化。使用者可以選擇 3km 或 20km 的版本，並且具備電子波束控制和先進的信號處理功能。這個系統的賣點在於其完全開源的設計，讓使用者能夠自由修改和擴展硬體和軟體。系統架構包含多個子系統，如電源管理、頻率合成器和主板，並且使用 STM32 微控制器進行系統管理。關鍵指令包括使用 Vivado 進行 FPGA 開發和 Python 3.8+ 來運行 GUI。這樣的設計使得雷達技術的民主化成為可能，讓更多人能夠探索和實驗這項技術。

**技術棧**：`Python 3.8+` · `Vivado` · `FPGA` · `STM32`

## 重點功能

- 開源硬體與軟體 — 完整的電路圖、PCB 佈局、韌體和軟體皆可獲得。
- 雙版本可用性 — AERIS-10N（3km）和 AERIS-10E（20km）兩種版本。
- 全電子波束控制 — 在仰角和方位角上可進行 ±45° 的電子控制。
- 先進的信號處理 — FPGA 負責脈衝壓縮、Doppler FFT、MTI 和 CFAR 處理。
...
1. 安裝 Python 3.8+ 並安裝必要的依賴
```bash
pip install -r requirements.txt
```
2. 使用 Vivado 開發 FPGA 代碼
```bash
...
```plsql
{
  "前置條件": "安裝 Python 3.8+ 和必要的依賴",
  "指令": "python gui.py",
  "預期輸出": "啟動雷達控制界面，顯示實時數據和地圖。"
}
```
...
> [!tip] 爆紅原因
> 建立 37 天內累積 7305 stars（197/天），forks 1528（20.9%），顯示出強烈的社群參與。作者 NawfalMotii79 和主要貢獻者 JJassonn69 具備相關背景，專案解決了傳統雷達系統高成本和封閉性問題，讓更多開發者能夠進入這個領域。近期的更新和活躍的討論也促進了專案的曝光度，尤其是關於硬體設計的建議和改進。這個專案的開源性和模組化設計使其在現有雷達技術中脫穎而出。

## 適合誰使用

**目標受眾**：對雷達技術有興趣的研究人員和開發者，特別是希望進行實驗和開發的 SDR 愛好者。

> [!example] 使用場景
> - 研究人員用它來進行雷達信號處理實驗，因為其開源設計允許自由修改和擴展功能。
> - 無人機開發者用它來測試新型雷達技術，因為其低成本和模組化設計適合快速迭代。
> - 高級製作人用它來學習和實驗相控陣列雷達技術，因為完整的硬體和軟體文檔提供了良好的學習資源。

## 架構分析

AERIS-10 的架構設計為模組化，主要由電源管理板、頻率合成器板和主板組成。這樣的設計使得系統能夠靈活地進行升級和維護，並且每個子系統都能獨立運作。數據流從 DAC 生成雷達脈衝開始，經過上/下變頻處理，然後由 FPGA 進行信號處理，最後透過 Python GUI 進行可視化。選擇 FPGA 進行信號處理的好處是能夠實現高效能的即時數據處理，但相對的也增加了開發的複雜度。該系統的擴展性良好，能夠支持不同的雷達應用需求。

## 技術深入分析

AERIS-10 的核心技術機制包括使用 FPGA 進行信號處理，這使得系統能夠在實時環境中處理複雜的雷達數據。該系統能夠處理高達 20km 的雷達範圍，並且具備先進的脈衝壓縮和 Doppler FFT 功能。設計上選擇 FPGA 而非傳統的微處理器，主要是為了提高處理速度和效率，但這也增加了開發的難度和學習曲線。系統的資源使用率合理，FPGA 的 LUT 和 Flip-Flop 使用率分別為 31.4% 和 19.7%。在設計取捨方面，選擇開源硬體的模式使得用戶能夠自由修改，但也可能導致硬體的法律責任問題。技術風險方面，隨著使用者數量的增加，可能會出現的問題包括硬體兼容性和社群支持的不足。整合方面，該系統與現有的開發工具鏈（如 Vivado 和 Python）相容性良好，但對於新手來說，學習成本可能較高。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了詳細的安裝步驟和系統架構說明，對於新手來說相對友好。安裝過程需要一些硬體組裝經驗，但整體上是可行的。文件中有清晰的快速入門指南，並且提供了豐富的文檔資源，方便用戶學習和使用。
...
> - 硬體組裝需要基本的 PCB 組裝經驗。
> - 不支援 Windows 系統的某些開發工具。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供類似的開源雷達解決方案，但專注於低功耗設計，適合小型無人機使用。 |
| [AgentSeal/codeburn](https://github.com/AgentSeal/codeburn) | 專注於無人機的雷達應用，提供即時數據處理，但不如 AERIS-10 模組化。 |
| [AgriciDaniel/claude-obsidian](https://github.com/AgriciDaniel/claude-obsidian) | 提供開源的雷達數據分析工具，適合後端處理，但缺乏硬體組件的支持。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供類似的開源雷達解決方案，但專注於低功耗設計，適合小型無人機使用。 | 如果你的專案需要低功耗且小型化的雷達系統，這個工具會是更好的選擇。 | medium，因為需要重新設計硬體架構。 |
| [AgentSeal/codeburn](https://github.com/AgentSeal/codeburn) | 專注於無人機的雷達應用，提供即時數據處理，但不如 AERIS-10 模組化。 | 如果你的專案已經在使用無人機平台，這個工具可能更容易整合。 | low，因為可以直接使用現有的無人機系統。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PLFM_RADAR** | **boneyard** | **codeburn** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 提供類似的開源雷達解決方案，但專注於低功耗設計，適合小型無人機使用。 | 專注於無人機的雷達應用，提供即時數據處理，但不如 AERIS-10 模組化。 |
> | 遷移成本 | - | medium，因為需要重新設計硬體架構。 | low，因為可以直接使用現有的無人機系統。 |
> | 適用場景 | 主要場景 | 如果你的專案需要低功耗且小型化的雷達系統，這個工具會是更好的 | 如果你的專案已經在使用無人機平台，這個工具可能更容易整合。 |
...
- **[HIGH]** 硬體組裝過程中可能會遇到焊接問題，導致功能不正常
  - 解法：確保使用合適的焊接工具和材料，並仔細檢查每個連接
- [MEDIUM] FPGA 開發工具的版本不兼容問題
  - 解法：使用推薦的 Vivado 版本，並參考官方文檔進行設置
- [MEDIUM] Python GUI 在某些環境下可能無法正常啟動
  - 解法：檢查依賴是否正確安裝，並確保 Python 版本符合要求

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司進行雷達技術開發 | 非常適合 | 開源設計和低成本使其成為初創企業的理想選擇。 |
| 大型研究機構進行雷達信號處理研究 | 適合 | 提供了強大的信號處理能力，但可能需要額外的技術支持。 |
| 個人開發者進行雷達技術學習 | 非常適合 | 完整的文檔和開源特性使學習變得容易。 |
| 需要商業化的雷達解決方案的企業 | 不適合 | 目前仍處於 beta 階段，穩定性不足。 |
...
> [!tip] 投入 vs 回報
> 花 20 小時學習，40 小時整合，得到一個強大的開源雷達系統，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 該工具本身不需要高權限，但在使用過程中可能會存取敏感資料（如 GPS 和 IMU 數據）。依賴鏈的信任程度良好，沒有已知的供應鏈風險。
