...
use_case: "提供開源、低成本的10.5 GHz相控陣雷達系統，適合研究者和開發者探索雷達技術。"
priority: high
ring: assess
...
aliases:
  ...
  - "提供開源、低成本的10.5 GHz相控陣雷達系統，適合研究者和開發者探索雷達技術。"
---

# PLFM_RADAR

**17.8k** stars · **388** stars/天 · 建立 46 天前 · PLSQL · NOASSERTION
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Growing · **熱度** Hot (388 stars/day)
> **授權** NOASSERTION · **維護** Active (最後推送 0 天前) · **貢獻者** 5+ 人 · **參與度** Medium
> **適合** 對雷達技術有興趣的研究者和開發者，特別是那些希望進行自訂和實驗的工程師。
> **一句話重點** AERIS-10不僅是一個雷達系統，更是一個開源社群的實驗平台，讓更多人能夠探索和創新雷達技術。
...
> **結論** 花20小時學習，40小時整合，得到一個靈活的雷達系統，值得投入。

> [!abstract] 核心創新
> AERIS-10是首個完全開源的10.5 GHz相控陣雷達系統，旨在讓雷達技術更具可及性。

## 專案簡介

AERIS-10是一個開源的10.5 GHz相控陣雷達系統，專為研究者和開發者設計，旨在讓使用者能夠探索雷達技術。這個系統支持兩種版本，分別為3公里和20公里的探測範圍，並使用脈衝線性頻率調變（LFM）來進行信號調製。使用者可以透過Python GUI進行操作，並整合GPS和IMU進行實時位置修正。系統的核心在於FPGA，負責處理脈衝壓縮、Doppler FFT、目標追蹤等信號處理任務。AERIS-10的賣點在於其開源硬體和軟體，提供完整的電路圖和PCB佈局，讓使用者能夠自由修改和擴展。

與傳統的商業雷達系統相比，這個開源平台的成本大幅降低，並且提供了更高的靈活性和可擴展性。對於需要進行雷達技術實驗的學術機構或創業公司來說，AERIS-10是一個理想的選擇。系統設計的模組化特性使得不同的子系統可以獨立開發和測試，這樣的設計不僅提高了開發效率，也降低了整體的維護成本。使用者在使用過程中可能會遇到一些技術挑戰，例如FPGA配置和PCB組裝，但這些都可以通過社群支持和文檔來解決。整體而言，AERIS-10是一個成熟的開源雷達系統，適合對雷達技術有興趣的開發者和研究者。
...
- 雙版本可選 — AERIS-10N（3公里範圍）和AERIS-10E（20公里範圍）。
- 全電子波束轉向 — 在仰角和方位角上±45°的電子轉向能力。
- 先進的信號處理 — FPGA負責脈衝壓縮、Doppler FFT、MTI和CFAR。
- Python GUI — 提供用戶友好的界面，支持地圖整合和雷達控制。

## 快速開始

1. 安裝Python 3.8+，以便運行GUI
```bash
pip install -r requirements.txt
```
2. 下載並安裝FPGA開發工具Vivado
```bash
從Xilinx官網下載Vivado
```
3. 組裝PCB並連接所有硬體元件
...
> [!tip] 爆紅原因
> 建立46天內累積17829 stars（388/天），forks達4242（23.8%），顯示出強烈的社群參與。這個專案的作者NawfalMotii79在開源硬體領域有豐富經驗，之前的專案也獲得了良好的反響。AERIS-10解決了傳統雷達系統高成本和封閉性問題，提供了一個開放且可自訂的解決方案。社群的活躍度和對於開源硬體的需求增加，促進了這個專案的快速成長。高比例的forks顯示出許多開發者對於進行自訂和改進的興趣。

## 適合誰使用

**目標受眾**：對雷達技術有興趣的研究者和開發者，特別是那些希望進行自訂和實驗的工程師。

> [!example] 使用場景
> - 學術研究者用它來進行雷達信號處理的實驗，因為開源設計讓他們能夠自由修改和擴展系統功能。
> - 無人機開發者用它來實現精確的目標追蹤，因為其模組化設計允許快速集成到現有的無人機系統中。
> - 電子工程師用它來測試和驗證新的雷達技術，因為完整的硬體和軟體文檔使得實驗過程更加順利。

## 架構分析

AERIS-10的架構設計為模組化，主要分為電源管理、頻率合成和信號處理三大板塊。電源管理板確保所有電子元件的電壓供應，頻率合成板則使用高性能的低抖動時鐘生成器提供相位對齊的時鐘參考。信號處理則由FPGA負責，實現複雜的脈衝壓縮和Doppler處理。

這種模組化設計使得不同的子系統可以獨立開發和測試，降低了整體的維護成本。選擇FPGA而非傳統微控制器的原因在於其高效能和可編程性，能夠處理更複雜的信號處理任務。整體架構的設計考量了擴展性，未來可以根據需求增加更多功能模組。

## 技術深入分析

AERIS-10的核心技術在於其模組化設計和FPGA信號處理。使用FPGA使得系統能夠高效地處理複雜的信號，並且支持即時的脈衝壓縮和Doppler FFT。系統的設計考量了擴展性，未來可以根據需求增加更多功能模組。由於使用了高性能的時鐘生成器和頻率合成器，系統在信號精度和穩定性上表現出色。

設計選擇上，FPGA的使用雖然增加了開發的複雜度，但卻能夠在性能上提供顯著的優勢。技術風險方面，FPGA的配置和調試可能會在大規模使用時遇到問題，特別是在信號處理的精度和延遲上。整合方面，AERIS-10與主流開發工具的相容性良好，特別是在使用Python進行GUI開發時，能夠快速實現功能。整體而言，AERIS-10是一個具備高性能和靈活性的開源雷達系統，適合對雷達技術有深入探索需求的開發者和研究者。

## 新手體驗

> [!info] 上手難度評估
> README文件提供了清晰的系統架構和使用說明，但缺乏具體的安裝範例。安裝過程需要一定的硬體組裝經驗，對新手來說可能會有挑戰。整體而言，對於有經驗的開發者來說，花30分鐘應該能夠完成基本的環境設置。
...
| 工具 | 差異 |
| --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard專注於低成本的開源無人機雷達，而AERIS-10則提供更高的靈活性和可擴展性，適合更廣泛的應用場景。 |
| [432539/gpt2api](https://github.com/432539/gpt2api) | GPT-2 API專注於自然語言處理，而AERIS-10則專注於雷達信號處理，兩者在應用領域上有明顯差異。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard專注於低成本的開源無人機雷達，提供簡化的設計和功能。 | 如果你的專案需要一個簡單的雷達解決方案，並且不需要AERIS-10的高性能和靈活性。 | medium，因為兩者在設計和功能上有一定的差異。 |
| [432539/gpt2api](https://github.com/432539/gpt2api) | GPT-2 API專注於自然語言處理，與AERIS-10的雷達技術無直接關聯。 | 如果你的專案需要自然語言處理的功能，則應選擇GPT-2 API而非AERIS-10。 | high，因為兩者的技術路線和應用場景完全不同。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PLFM_RADAR** | **boneyard** | **gpt2api** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | Boneyard專注於低成本的開源無人機雷達，提供簡化的設計和功能。 | GPT-2 API專注於自然語言處理，與AERIS-10的雷達技術無直接關聯。 |
> | 遷移成本 | - | medium，因為兩者在設計和功能上有一定的差異。 | high，因為兩者的技術路線和應用場景完全不同。 |
> | 適用場景 | 主要場景 | 如果你的專案需要一個簡單的雷達解決方案，並且不需要AERIS | 如果你的專案需要自然語言處理的功能，則應選擇GPT-2 AP |
...
> [!tip] 採用建議
> 適合對雷達技術有興趣的開發者進行實驗，但不建議用於生產環境的核心應用。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** FPGA配置過程中可能會遇到延遲問題，影響信號處理精度。
  - 解法：仔細檢查時鐘配置和信號路徑，確保無過度約束。
- [MEDIUM] 組裝過程中可能會出現元件焊接不良的情況。
  - 解法：使用熱風槍進行焊接，並檢查每個連接。
- [MEDIUM] Python GUI在某些系統上可能無法正常啟動。
  - 解法：確保安裝了所有必要的依賴，並使用Python 3.8以上版本。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10人以下的新創公司進行雷達技術開發 | 非常適合 | 開源設計和低成本使其適合小型團隊進行實驗。 |
| 大型學術機構的雷達研究項目 | 適合 | 模組化設計允許進行深入的技術研究和開發。 |
| 需要高精度雷達系統的商業應用 | 不適合 | 目前仍處於beta階段，穩定性和性能尚未達到商業應用標準。 |
...
> [!tip] 投入 vs 回報
> 花20小時學習，40小時整合，得到一個靈活的雷達系統，值得投入。
...
> | [#21](https://github.com/NawfalMotii79/PLFM_RADAR/issues/21) | Adding LAN interface | 2 | 16 |
> | [#75](https://github.com/NawfalMotii79/PLFM_RADAR/issues/75) | [Question/Discussion] Beamforming Calibration Strategy & Pha | 1 | 9 |
> | [#125](https://github.com/NawfalMotii79/PLFM_RADAR/issues/125) | Boards STEP files | 0 | 0 |
> | [#124](https://github.com/NawfalMotii79/PLFM_RADAR/issues/124) | Question about sum/difference beams for monopulse angle meas | 0 | 1 |
...
相關專案：[[0xGF--boneyard|0xGF/boneyard]] · [[432539--gpt2api|432539/gpt2api]] · [[BuilderPulse--BuilderPulse|BuilderPulse/BuilderPulse]] · [[AgentSeal--codeburn|AgentSeal/codeburn]] · [[AgriciDaniel--claude-obsidian|AgriciDaniel/claude-obsidian]] · [[BigBodyCobain--Shadowbroker|BigBodyCobain/Shadowbroker]] · [[AlpinDale--parsync|AlpinDale/parsync]] · [[NVIDIA--NemoClaw|NVIDIA/NemoClaw]]
