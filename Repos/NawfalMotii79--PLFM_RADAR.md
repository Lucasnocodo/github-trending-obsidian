...
**3.7k** stars · **152** stars/天 · 建立 24 天前 · Verilog · NOASSERTION
...
> **適合** 對雷達技術有興趣的研究人員和開發者，尤其是無人機和 SDR 領域的專業人士。
> **一句話重點** AERIS-10 不僅是一個雷達系統，更是一個開源平台，讓開發者能夠自由探索和實驗雷達技術。
...
> **結論** 花 10 小時學習，20 小時整合，得到一個靈活的雷達系統，值得投入。

> [!abstract] 核心創新
> 提供一個完全開源且模組化的相控陣雷達系統，讓更多開發者能夠進行實驗和學習。

## 專案簡介

AERIS-10 是一個開源的 10.5 GHz 相控陣雷達系統，專為研究人員、無人機開發者和 SDR 愛好者設計。該系統提供兩種版本，分別支持 3 公里和 20 公里的探測範圍，並具備全電子波束轉向功能。核心機制包括 DAC 生成雷達脈衝，LT5552 混頻器進行頻率轉換，FPGA 負責信號處理，並通過 Python GUI 提供用戶友好的操作界面。這樣的設計使得用戶能夠輕鬆進行波束成形、脈衝壓縮和目標跟蹤，並且所有硬體和軟體都是開源的，便於修改和擴展。

系統架構上，AERIS-10 包含多個子系統，如電源管理、頻率合成和主板，這些子系統協同工作以實現高效的雷達信號處理。與其他雷達系統相比，AERIS-10 的模組化設計使得用戶能夠根據需求進行自定義，並且開源的特性使得其在學術和開發領域具有廣泛的應用潛力。使用者可以通過簡單的組裝步驟來構建系統，並利用提供的文檔進行操作。這個專案的賣點在於其開源性和可擴展性，讓更多人能夠接觸和實驗雷達技術。
...
- 雙版本可選 — AERIS-10N（3 公里範圍）和 AERIS-10E（20 公里範圍）。
- 全電子波束轉向 — 在仰角和方位角上 ±45° 的電子轉向。
- 先進的信號處理 — FPGA 負責脈衝壓縮、Doppler FFT、MTI 和 CFAR。
- Python GUI — 提供用戶友好的界面，並支持地圖集成。
- GPS/IMU 集成 — 實時位置和姿態修正。
...
> [!tip] 爆紅原因
> 建立 24 天內累積 3653 stars（152/天），forks 772（21.1%），顯示出強烈的社群參與。專案的作者 NawfalMotii79 及其團隊在開源硬體領域有豐富的經驗，這個專案填補了低成本雷達系統的市場空白，讓更多開發者能夠實驗和學習雷達技術。近期的社群互動和建議顯示出對於協作和設計支持的需求，這也促進了專案的活躍度。技術上，開源硬體的趨勢讓這個專案能夠迅速獲得關注，並且其模組化設計使得用戶能夠根據需求進行自定義。forks/stars 比率 21.1% 表示許多人在實際修改和使用這個專案。

## 適合誰使用

**目標受眾**：對雷達技術有興趣的研究人員和開發者，尤其是無人機和 SDR 領域的專業人士。

> [!example] 使用場景
> - 無人機開發者用它來測試和優化雷達系統，因為其開源設計讓他們能夠快速迭代和調整功能。
> - 學術研究者用它來進行雷達信號處理的實驗，因為完整的文檔和開源硬體設計使得重現性高。
> - 高級製造者用它來探索相控陣雷達技術，因為模組化設計讓他們能夠根據需求進行自定義和擴展。

## 架構分析

AERIS-10 的架構設計以模組化為核心，將不同功能劃分為獨立的子系統，如電源管理、頻率合成和信號處理。這樣的設計使得系統在維護和擴展上更為靈活。資料流從 DAC 生成雷達脈衝開始，經過混頻器進行頻率轉換，然後由 FPGA 負責信號處理，最後通過 Python GUI 進行可視化。

選擇 FPGA 進行信號處理的原因在於其高效能和並行處理能力，能夠應對複雜的信號處理任務。模組化設計的代價是需要更多的組裝和配置工作，但這也使得用戶能夠根據需求進行自定義。擴展性方面，系統能夠支持不同的天線配置和信號處理算法，這為未來的功能擴展提供了可能性。

## 技術深入分析

AERIS-10 的核心技術機制包括使用 FPGA 進行信號處理，這使得系統能夠高效地處理複雜的雷達信號。其設計模式採用模組化架構，將不同功能劃分為獨立的子系統，這樣的設計不僅提高了維護性，也使得用戶能夠根據需求進行自定義。系統能夠處理的最大範圍取決於所選擇的天線配置，AERIS-10E 版本的最大範圍可達 20 公里。冷啟動時間和記憶體佔用未具體說明，但由於使用了 FPGA，預期能夠在實時處理中保持較低的延遲。

選擇使用 Verilog 和 C++ 進行開發的原因在於這些語言能夠有效地支持硬體描述和高效的計算，這使得系統在性能上有優勢。依賴樹的複雜度中等，主要依賴於 FPGA 開發工具和 Python 環境。技術風險方面，隨著使用者數量的增加，可能會出現的問題包括硬體組裝的挑戰和信號處理算法的優化需求。整合方面，與主流的開發框架（如 Python 和 FPGA 開發環境）相容性良好，並且能夠輕鬆集成到現有的 CI/CD pipeline 中。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了詳細的安裝步驟和使用說明，對於新手來說相對友好。安裝過程中需要注意硬體組裝的複雜性，可能會對初學者造成挑戰。文件中有清晰的指引和範例，幫助用戶快速上手。整體而言，花 30 分鐘應該能夠完成基本的設置。
...
| 工具 | 差異 |
| --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 專注於高效的數據同步，但不提供雷達信號處理功能，適合需要數據傳輸的場景。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | 主要用於數據收集和分析，與 AERIS-10 的實時信號處理功能不同。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 專注於數據同步，並不涉及雷達信號處理或硬體組裝。 | 如果你的需求主要是數據傳輸而非雷達技術，則應選擇此工具。 | low，因為主要功能不同，無需轉換過多概念。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | 主要用於數據收集和分析，與 AERIS-10 的實時信號處理功能不同。 | 如果你的專案需要強大的數據分析能力，而非雷達信號處理，則選擇此工具。 | medium，因為需要重新設計數據流和處理邏輯。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PLFM_RADAR** | **parsync** | **Shadowbroker** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於數據同步，並不涉及雷達信號處理或硬體組裝。 | 主要用於數據收集和分析，與 AERIS-10 的實時信號處理功能不同。 |
> | 遷移成本 | - | low，因為主要功能不同，無需轉換過多概念。 | medium，因為需要重新設計數據流和處理邏輯。 |
> | 適用場景 | 主要場景 | 如果你的需求主要是數據傳輸而非雷達技術，則應選擇此工具。 | 如果你的專案需要強大的數據分析能力，而非雷達信號處理，則選擇 |
...
- **[HIGH]** 硬體組裝過程中可能會遇到不兼容的元件問題
  - 解法：仔細檢查 BOM 和元件規格，確保兼容性
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司進行雷達技術研究 | 非常適合 | 開源設計和模組化架構使得研究和開發更為靈活。 |
| 大型企業進行商業化雷達產品開發 | 不適合 | 目前仍在開發中，穩定性和支持性不足。 |
| 學術機構進行雷達信號處理的實驗 | 適合 | 提供完整的文檔和開源硬體設計，便於重現性研究。 |
| 業餘愛好者進行無人機雷達項目 | 非常適合 | 開源性和低成本使得愛好者能夠輕鬆入手。 |
...
> [!tip] 投入 vs 回報
> 花 10 小時學習，20 小時整合，得到一個靈活的雷達系統，值得投入。
...
> | [@NawfalMotii79](https://github.com/NawfalMotii79) | 91 |
...
> **活躍天數** 2 天 · **最新 commit** Revise index.html for AERIS-10 documentation site
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["相控陣雷達","信號處理","開源硬體"];
