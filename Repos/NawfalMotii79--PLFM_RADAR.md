...
use_case: "開源、低成本的 10.5 GHz PLFM 相控陣雷達系統，適合研究和開發。"
priority: high
ring: assess
...
star_history: "2026-03-15:1703,2026-03-15:1708,2026-03-16:2576,2026-03-17:3042,2026-03-18:3202,2026-03-19:3341,2026-03-20:3405,2026-03-21:3448,2026-03-22:3471,2026-03-23:3512,2026-03-24:3539,2026-03-25:3549,2026-03-26:3559,2026-03-27:3573,2026-03-28:3584,2026-03-29:3597,2026-03-30:3611"
tags:
  - github
  - "category/基礎設施"
  - "lang/verilog"
aliases:
  ...
  - "開源、低成本的 10.5 GHz PLFM 相控陣雷達系統，適合研究和開發。"
---

# PLFM_RADAR

**3.6k** stars · **172** stars/天 · 建立 21 天前 · Verilog · NOASSERTION
...
> **適合** 需要探索相控陣雷達技術的研究者和開發者，特別是對開源硬體有興趣的團隊。
> **一句話重點** AERIS-10 不僅是一個雷達系統，更是一個開源社群的實驗平台，讓技術愛好者能夠共同探索和創新。
...
> **結論** 花 20 小時學習，30 小時整合，得到一個靈活的雷達系統，值得投資。

> [!abstract] 核心創新
> AERIS-10 提供了一個完全開源和模組化的相控陣雷達系統，讓研究者和開發者能夠自由探索和實驗。

## 專案簡介

AERIS-10 是一個開源的 10.5 GHz 相控陣雷達系統，專為研究者、無人機開發者和 SDR 愛好者設計。用戶可以選擇 3km 或 20km 的版本，並利用其全電子波束轉向和先進信號處理功能。系統的核心是 FPGA，負責脈衝壓縮、Doppler FFT 和目標跟蹤等處理，並通過 Python GUI 提供用戶友好的界面。這個設計的賣點在於其模組化架構，讓用戶可以根據需求進行擴展和修改。硬體方面，系統包含多個子系統，如電源管理、頻率合成和 RF 板，並使用高性能的時鐘生成器來確保信號的相位對齊。

與其他商業雷達系統相比，AERIS-10 的開源特性使其在成本和可定制性上具有優勢，特別適合小型研究團隊和創業公司。使用者可以利用其完整的硬體和軟體文檔進行自定義設計，這在商業解決方案中往往難以實現。這個系統的設計考量了使用者的實驗需求，並提供了豐富的接口和擴展選項，讓用戶能夠進行多樣化的實驗和開發。隨著社群的活躍，使用者可以獲得持續的支持和更新，這對於長期的研究項目尤為重要。

**技術棧**：`Verilog` · `C` · `Python` · `Tcl` · `C++` · `SMT` · `TypeSpec` · `MATLAB` · `Shell` · `Makefile`

## 重點功能

- 開源硬體與軟體 — 完整的電路圖、PCB 佈局、韌體和軟體可供下載。
- 雙版本可選 — AERIS-10N 提供 3km 範圍，AERIS-10E 提供 20km 範圍。
- 全電子波束轉向 — 在仰角和方位角上實現 ±45° 的電子轉向。
- 先進信號處理 — FPGA 負責脈衝壓縮、Doppler FFT、MTI 和 CFAR 等處理。
- Python GUI — 提供友好的界面，支持地圖整合和實時目標顯示。
- GPS/IMU 整合 — 實時位置和姿態修正，提升數據準確性。
...
3. 參考組裝指南
```bash
查看 /10_docs/assembly_guide.md
```

## 為什麼值得關注

> [!tip] 爆紅原因
> 建立 21 天就累積 3611 stars（172/天），forks 765（21.2%），顯示出強烈的社群參與。作者 NawfalMotii79 和 JJassonn69 在開源硬體領域有一定的經驗，這個專案填補了低成本相控陣雷達系統的市場空白，之前的商業解決方案價格高昂且不易修改。社群的活躍度和對於開源硬體的需求推動了這個專案的快速成長。

## 適合誰使用

**目標受眾**：需要探索相控陣雷達技術的研究者和開發者，特別是對開源硬體有興趣的團隊。

> [!example] 使用場景
> - 無人機開發者用它來測試和優化自動導航系統，因為其精確的雷達數據能提高飛行安全性和效率。
> - 學術研究者用它來進行雷達信號處理的實驗，因為開源的特性讓他們可以自由修改和擴展功能。
> - 電子工程師用它來設計和測試新型雷達系統，因為模組化設計使得硬體組裝和測試變得簡單。

## 架構分析

AERIS-10 採用模組化設計，將不同功能分為多個子系統，如電源管理、頻率合成和信號處理。這種設計使得系統易於擴展和維護，但也要求使用者具備一定的硬體組裝能力。數據流從 DAC 生成雷達脈衝，經過混頻器進行頻率轉換，然後由 FPGA 處理信號。選擇 FPGA 進行信號處理的好處在於其高效能和可編程性，但也增加了開發的複雜性。整體架構的擴展性良好，能夠支持未來的功能擴展和升級。

## 技術深入分析

AERIS-10 的核心技術機制在於其模組化設計和 FPGA 信號處理。使用 Verilog 和 C 語言開發的 FPGA 負責雷達信號的生成和處理，這使得系統能夠實現高效的脈衝壓縮和 Doppler FFT。系統能夠處理的數據量相當可觀，支持實時的目標跟蹤和信號分析。選擇 FPGA 而非傳統微控制器的原因在於其在處理速度和並行運算上的優勢，雖然這也意味著需要更高的開發技能。

設計上，AERIS-10 需要一定的硬體資源，特別是在信號處理和數據存儲方面。這使得系統在冷啟動時需要一定的時間，但一旦運行，性能表現相當穩定。對於未來的擴展性，模組化設計提供了良好的支持，使用者可以根據需求添加新功能或改進現有功能。與主流的商業雷達系統相比，AERIS-10 的開源特性使其在成本和靈活性上具有顯著優勢，特別適合小型團隊和研究機構。

整合到現有的開發環境中也相對簡單，能夠與多種開發工具和框架兼容，降低了使用門檻。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且包含必要的範例，安裝過程相對順暢，但對於初學者可能會有一定的挑戰。提供了良好的入門指南，文件主要以英文撰寫，缺乏多語言支持。
...
> - 開源特性讓使用者能自由修改和擴展系統。
> - 模組化設計便於維護和升級，適合長期研究。
> - 先進的信號處理能力，能夠滿足多樣化的應用需求。

> [!danger] 缺點
> - 組裝和配置過程對初學者有一定的挑戰。
> - 需要專業的 FPGA 開發工具，增加了學習成本。
> - 對於不熟悉雷達技術的使用者，理解系統架構可能需要時間。

> [!warning] 注意事項
> - 需要 FPGA 開發工具 (Vivado) 進行信號處理修改。
> - 組裝過程需要基本的 PCB 組裝經驗。
> - 僅支援 Python 3.8 以上版本的 GUI 軟體。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 提供類似的開源雷達系統，但專注於同步和數據傳輸，缺乏 AERIS-10 的模組化設計和擴展性。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | 專注於信號處理的開源工具，但不具備 AERIS-10 的硬體組件和實時應用能力。 |
| [L42ARO/Mercury-Transforming-Drone](https://github.com/L42ARO/Mercury-Transforming-Drone) | 針對無人機的雷達解決方案，但其成本較高且不開源，無法進行自定義修改。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 專注於數據同步和傳輸的開源雷達系統，缺乏 AERIS-10 的模組化設計。 | 如果你的專案需要強調數據同步而非硬體組裝，這是更合適的選擇。 | medium，因為需要重新設計硬體架構。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | 提供信號處理功能，但不具備 AERIS-10 的硬體組件和實時應用能力。 | 如果你的需求主要集中在信號處理而非完整的雷達系統，這是更合適的選擇。 | high，因為需要重新設計整個系統。 |
| [L42ARO/Mercury-Transforming-Drone](https://github.com/L42ARO/Mercury-Transforming-Drone) | 針對無人機的雷達解決方案，成本較高且不開源。 | 如果你的團隊已經在使用商業解決方案，且不需要開源特性，可以考慮這個選擇。 | low，因為可以直接使用現有的商業解決方案。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PLFM_RADAR** | **parsync** | **Shadowbroker** | **Mercury-Transforming-Drone** |
> | --- | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於數據同步和傳輸的開源雷達系統，缺乏 AERIS-10 的模組化設計。 | 提供信號處理功能，但不具備 AERIS-10 的硬體組件和實時應用能力。 | 針對無人機的雷達解決方案，成本較高且不開源。 |
> | 遷移成本 | - | medium，因為需要重新設計硬體架構。 | high，因為需要重新設計整個系統。 | low，因為可以直接使用現有的商業解決方案。 |
> | 適用場景 | 主要場景 | 如果你的專案需要強調數據同步而非硬體組裝，這是更合適的選擇。 | 如果你的需求主要集中在信號處理而非完整的雷達系統，這是更合適 | 如果你的團隊已經在使用商業解決方案，且不需要開源特性，可以考 |
...
> [!tip] 採用建議
> 適合個人或小型團隊的實驗項目，不建議用於生產環境的核心應用。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 組裝過程中可能會遇到元件缺失的問題，特別是在初次訂購時。
  - 解法：提前檢查 BOM，並與供應商確認元件可用性。
- [MEDIUM] FPGA 開發環境配置可能會有兼容性問題。
  - 解法：確保使用官方推薦的版本和配置，並參考社群的解決方案。
- [MEDIUM] Python GUI 可能在某些環境中無法正常運行。
  - 解法：檢查 Python 版本和依賴，必要時重新安裝。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型研究團隊進行雷達技術的探索 | 非常適合 | 開源特性和模組化設計使得研究和實驗變得容易。 |
| 無人機開發公司需要集成雷達系統 | 適合 | 提供了必要的功能和擴展性，適合商業應用。 |
| 大型企業需要穩定的商業雷達系統 | 不適合 | 目前處於 beta 階段，尚未準備好用於生產環境。 |
| 教育機構進行雷達技術的教學 | 非常適合 | 開源和模組化設計使得學生能夠深入理解雷達技術。 |
...
> [!tip] 投入 vs 回報
> 花 20 小時學習，30 小時整合，得到一個靈活的雷達系統，值得投資。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：系統本身不需要高權限，且不存取敏感資料。依賴鏈的信任程度較高，適合在 CI/CD 環境中使用。
...
> | [#32](https://github.com/NawfalMotii79/PLFM_RADAR/issues/32) | MCU scan sequence does not match documented 31-elevation / ± | 0 | 7 |
