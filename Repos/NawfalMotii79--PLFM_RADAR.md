...
**18.8k** stars · **355** stars/天 · 建立 53 天前 · PLSQL · NOASSERTION
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Growing · **熱度** Hot (355 stars/day)
> **授權** NOASSERTION · **維護** Active (最後推送 0 天前) · **貢獻者** 5+ 人 · **參與度** Medium
> **適合** 對雷達技術有興趣的研究人員和開發者，特別是那些希望進行實驗和開發的創客。
> **一句話重點** 這個專案不僅提供了雷達技術的開源平台，還促進了相關技術的學習和實驗。
...
> **結論** 花 20 小時學習，30 小時整合，得到一個可擴展的雷達系統，值得嘗試。

> [!abstract] 核心創新
> 這個專案的創新點在於其開源的雷達系統設計，讓更多人能夠接觸和實驗雷達技術。

## 專案簡介

AERIS-10 是一個開源的 10.5 GHz 相控陣列雷達系統，旨在讓研究人員和開發者能夠探索雷達技術。用戶可以透過 Python GUI 進行操作，並可選擇 3km 或 20km 的量測範圍。系統的核心是 FPGA，負責信號處理，包括脈衝壓縮和多普勒處理。這個設計的賣點在於其模組化架構，讓用戶可以根據需求進行擴展和修改。AERIS-10 的硬體設計包括多個子系統，如電源管理、頻率合成和 RF 板，這些都可以獨立開發和測試。

與其他商業雷達系統相比，AERIS-10 提供了更低的進入門檻和更高的可定制性，適合學術研究和創客使用。使用者需要具備基本的雷達原理知識和 PCB 組裝經驗，並且需要使用 Vivado 進行 FPGA 開發。這個系統的效能在於其先進的信號處理能力和電子波束控制，能夠實現 ±45° 的電子掃描。儘管如此，使用者在組裝和調試過程中可能會遇到一些挑戰，特別是在硬體組裝和軟體整合方面。整體來說，這是一個適合需要深入了解雷達技術的開發者和研究者的專案。
...
- 雙版本可選 — AERIS-10N (3km) 和 AERIS-10E (20km) 版本。
- 全電子波束控制 — 在仰角和方位角上 ±45° 的電子掃描。
- 先進的信號處理 — FPGA 負責脈衝壓縮、多普勒 FFT、MTI 和 CFAR。
- Python GUI — 提供友好的使用者介面，並整合地圖功能。
- GPS/IMU 整合 — 實時位置和姿態修正。
- 模組化設計 — 獨立的電源管理、頻率合成和 RF 板。

## 快速開始

1. 安裝 Python 3.8+ 和所需的開發工具
```bash
...
> [!tip] 爆紅原因
> 建立 53 天內累積 18823 stars（355/天），forks 4505（23.9%），顯示出強烈的社群興趣。作者 NawfalMotii79 和團隊在開源硬體領域有豐富經驗，這個專案解決了傳統雷達系統高成本和封閉性問題，讓更多人能夠接觸和實驗雷達技術。社群的活躍度和開放的問題討論區也促進了使用者的參與。這個專案的技術背景和開源精神吸引了許多對雷達技術感興趣的開發者。

## 適合誰使用

**目標受眾**：對雷達技術有興趣的研究人員和開發者，特別是那些希望進行實驗和開發的創客。

> [!example] 使用場景
> - 研究人員用它來進行雷達信號處理的實驗，因為其開源設計讓他們能夠自由修改和擴展功能。
> - 無人機開發者用它來測試不同的雷達波束形成技術，因為其模組化架構允許快速迭代和實驗。
> - 高級創客用它來學習和實踐雷達技術，因為其低成本和開源特性使得學習門檻大幅降低。

## 架構分析

AERIS-10 的架構設計為模組化，主要分為電源管理、頻率合成、信號處理和用戶介面四個部分。每個子系統都可以獨立開發和測試，這樣的設計使得整體系統更具靈活性和可擴展性。信號處理主要由 FPGA 負責，這樣可以實現高效的脈衝壓縮和多普勒處理。

選擇 FPGA 而非傳統的微控制器是因為其在處理速度和並行處理能力上的優勢。這樣的設計雖然增加了開發的複雜度，但卻能在性能上獲得顯著提升。整體架構的擴展性良好，能夠支持未來的功能擴展和升級。

## 技術深入分析

AERIS-10 的核心技術機制在於其模組化的設計，將雷達系統的各個部分分開，使得開發和維護變得更為簡單。FPGA 負責信號處理，這使得系統能夠高效地進行脈衝壓縮和多普勒處理，並且能夠實現快速的數據處理。這個系統能夠處理的最大範圍為 20km，並且在信號處理過程中，FPGA 的性能表現出色，能夠在實時環境中運行。選擇 FPGA 而非微控制器的原因在於其優越的並行處理能力，這對於雷達信號的即時處理至關重要。整體架構的依賴關係較為簡單，但仍需注意在擴展時可能出現的性能瓶頸。安全性方面，系統的設計考量了高功率 RF 的風險，並且遵循開源硬體的標準，這樣的設計使得使用者能夠在合法的範圍內進行修改和使用。整合方面，與現有的開發工具鏈相容性良好，能夠方便地進行 CI/CD 流程的整合，並且支持多種 IDE 的使用。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且詳細，提供了足夠的範例和指導。安裝過程相對順暢，但對於新手來說，可能會遇到一些硬體組裝的挑戰。文件提供了良好的入門指南，並且有多語言支持，這對於不同背景的使用者非常友好。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於 AI agent 的開發，而 AERIS-10 專注於雷達技術，功能範圍不同。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供開源硬體設計，但不專注於雷達系統，適用場景不同。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於 AI agent 的開發，而 AERIS-10 專注於雷達技術，功能範圍不同。 | 如果你的專案主要集中在 AI agent 的開發上，這個工具會更合適。 | medium，因為需要重新設計系統架構。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供開源硬體設計，但不專注於雷達系統，適用場景不同。 | 如果需要一個更通用的開源硬體設計，這個工具會更合適。 | low，因為大部分設計理念可以直接應用。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PLFM_RADAR** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於 AI agent 的開發，而 AERIS-10 專注於雷達技術，功能範圍不同。 | 提供開源硬體設計，但不專注於雷達系統，適用場景不同。 |
> | 遷移成本 | - | medium，因為需要重新設計系統架構。 | low，因為大部分設計理念可以直接應用。 |
> | 適用場景 | 主要場景 | 如果你的專案主要集中在 AI agent 的開發上，這個工具 | 如果需要一個更通用的開源硬體設計，這個工具會更合適。 |
...
> [!tip] 採用建議
> 適合個人或小型團隊進行實驗，但不建議用於生產環境的核心應用。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在硬體組裝過程中，可能會遇到元件不匹配的問題。
  - 解法：仔細檢查 BOM 和元件清單，確保所有元件正確。
- [MEDIUM] FPGA 開發過程中，可能會遇到時序問題。
  - 解法：檢查時序約束，並進行適當的調整。
- [MEDIUM] 在使用 GUI 時，可能會遇到界面反應慢的情況。
  - 解法：確保使用的計算機性能足夠，並關閉不必要的應用程式。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 大學研究團隊進行雷達技術的實驗 | 非常適合 | 提供開源平台，便於修改和擴展。 |
| 小型創業公司開發無人機雷達系統 | 適合 | 低成本且模組化設計，便於快速迭代。 |
| 個人開發者學習雷達技術 | 普通 | 需要一定的技術背景，對新手來說有挑戰。 |
| 大型企業的核心產品開發 | 不適合 | 目前仍處於 beta 階段，穩定性不足。 |
...
> [!tip] 投入 vs 回報
> 花 20 小時學習，30 小時整合，得到一個可擴展的雷達系統，值得嘗試。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：系統本身不需要高權限，且不存取敏感資料。依賴鏈的信任程度良好，無已知的供應鏈風險。
