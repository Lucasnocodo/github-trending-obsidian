...
use_case: "提供開源、低成本的 10.5 GHz 相控陣雷達系統，適合研究者和開發者探索雷達技術。"
priority: high
ring: assess
...
aliases:
  ...
  - "提供開源、低成本的 10.5 GHz 相控陣雷達系統，適合研究者和開發者探索雷達技術。"
---

# PLFM_RADAR

**17.4k** stars · **395** stars/天 · 建立 44 天前 · PLSQL · NOASSERTION
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Growing · **熱度** Hot (395 stars/day)
> **授權** NOASSERTION · **維護** Active (最後推送 0 天前) · **貢獻者** 5+ 人 · **參與度** Medium
> **適合** 對雷達技術有興趣的研究者和開發者，尤其是小型團隊和個人開發者。
> **一句話重點** AERIS-10 不僅是一個雷達系統，更是一個開源平台，讓更多人能夠探索和實驗雷達技術。
...
> **結論** 花 10 小時學習，20 小時整合，能夠快速掌握雷達技術，值得投入。

> [!abstract] 核心創新
> 這個專案的創新點在於其開源的設計和模組化架構，使得雷達技術的研究和應用變得更加可及。

## 專案簡介

AERIS-10 是一個開源的 10.5 GHz 相控陣雷達系統，專為研究者和開發者設計，支持 Pulse Linear Frequency Modulated (LFM) 調變。用戶可以選擇 3km 或 20km 的版本，並利用 Python GUI 進行操作。其核心機制包括 DAC 生成 LFM 脈衝，FPGA 負責信號處理，並通過 STM32 控制系統管理。這個設計的賣點在於其模組化架構，允許用戶根據需求進行硬體和軟體的調整。系統使用高效的 FPGA 進行脈衝壓縮和 Doppler FFT，並且支持 GPS/IMU 整合，實現實時位置和姿態修正。

這樣的設計不僅降低了雷達技術的進入門檻，還促進了創新和實驗。相較於其他雷達系統，AERIS-10 的開源特性和低成本使其更具吸引力，特別是對於小型團隊和個人開發者。使用者可以透過 GitHub 獲得完整的硬體和軟體設計，並且社群活躍，能夠快速解決問題。這個專案的成熟度較高，適合有 PCB 組裝經驗的開發者使用。未來 6 個月內，預計會有更多的功能和改進，特別是在信號處理和用戶界面方面。

**技術棧**：`Python 3.8+` · `FPGA (Vivado)` · `C/C++` · `Verilog` · `MATLAB`
...
> [!tip] 爆紅原因
> 建立 44 天內累積 17367 stars（395/天），forks 4095（23.6%），顯示出強烈的社群興趣。這個專案的作者 NawfalMotii79 和其他貢獻者在開源硬體領域有豐富經驗，解決了傳統雷達系統高成本和封閉性問題。社群的活躍討論和建議，像是增加 LAN 接口和協作設計，進一步推動了專案的發展。技術上，開源硬體的趨勢使得這類專案變得可行，並且吸引了許多對雷達技術感興趣的開發者。高達 23.6% 的 forks/stars 比率顯示出許多用戶在實際修改和使用這個專案。
...
> - 無人機開發者用它來整合雷達系統於其飛行器，因為低成本和模組化設計使得集成變得簡單。
> - 電子工程師用它來學習相控陣雷達技術，因為完整的硬體和軟體文檔提供了良好的學習資源。

## 架構分析

AERIS-10 採用模組化的硬體架構，主要由電源管理、頻率合成和主板組成。每個模組獨立運作，確保了系統的靈活性和可擴展性。信號處理主要由 FPGA 負責，這樣的設計能夠快速處理雷達信號並進行實時分析。選擇 FPGA 而非傳統微控制器是因為其在處理速度和並行計算上的優勢，但這也增加了設計的複雜度。系統的擴展性良好，但在高功率 RF 環境下，對硬體的要求也相對較高。

## 技術深入分析

AERIS-10 的核心技術在於其模組化設計和 FPGA 驅動的信號處理。使用 Verilog 和 C/C++ 開發的 FPGA 程式碼，能夠實現高效的脈衝壓縮和 Doppler FFT。這樣的設計使得系統能夠在實時環境中處理大量數據，並且支持多種信號處理演算法。系統的效能特性表現在其能夠支持 3km 和 20km 的雷達範圍，並且在高功率下仍能保持穩定運作。選擇 FPGA 而非傳統微控制器的原因在於其能夠進行並行處理，這對於雷達信號的即時分析至關重要。技術上，這個專案的依賴關係相對簡單，主要集中在 FPGA 和 Python 環境，降低了使用門檻。隨著社群的發展，未來可能會有更多的功能和擴展，特別是在信號處理和用戶界面方面。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的指導，並包含詳細的安裝步驟和硬體組裝指南。安裝過程中可能會遇到一些挑戰，特別是在 PCB 組裝方面，但整體上是可行的。文件中有良好的範例和連結，幫助新手快速上手。
...
> - 目前僅提供 3km 和 20km 兩個版本，無法擴展至其他範圍。
> - 硬體組裝過程中可能需要額外的工具和材料。
...
| 工具 | 差異 |
| --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供類似的開源硬體設計，但專注於不同的應用場景，如無人機和自動駕駛。 |
| [BuilderPulse/BuilderPulse](https://github.com/BuilderPulse/BuilderPulse) | 專注於建築和施工領域的開源雷達技術，與 AERIS-10 的應用場景不同。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於無人機和自動駕駛的開源雷達系統，提供不同的應用場景。 | 如果你的專案主要集中在無人機技術上，這個工具可能更適合。 | medium，因為需要調整硬體和信號處理邏輯。 |
| [BuilderPulse/BuilderPulse](https://github.com/BuilderPulse/BuilderPulse) | 專注於建築和施工領域的雷達技術，與 AERIS-10 的應用場景不同。 | 如果你的需求主要是建築檢測，這個工具會更合適。 | low，因為其設計理念相似，轉換成本較低。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PLFM_RADAR** | **boneyard** | **BuilderPulse** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於無人機和自動駕駛的開源雷達系統，提供不同的應用場景。 | 專注於建築和施工領域的雷達技術，與 AERIS-10 的應用場景不同。 |
> | 遷移成本 | - | medium，因為需要調整硬體和信號處理邏輯。 | low，因為其設計理念相似，轉換成本較低。 |
> | 適用場景 | 主要場景 | 如果你的專案主要集中在無人機技術上，這個工具可能更適合。 | 如果你的需求主要是建築檢測，這個工具會更合適。 |
...
- **[HIGH]** 在高功率運行時，可能會出現過熱問題。
  - 解法：確保良好的散熱設計，並定期檢查溫度。
- [MEDIUM] 硬體組裝過程中，可能會因為元件錯誤導致功能失效。
  - 解法：仔細核對 BOM 和組裝指南，確保元件正確安裝。
- **[HIGH]** FPGA 配置不當可能導致信號處理失敗。
  - 解法：遵循官方文檔中的配置步驟，並進行充分測試。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型創業團隊開發無人機雷達系統 | 非常適合 | 模組化設計和低成本使得開發變得可行。 |
| 大學研究機構進行雷達技術研究 | 適合 | 開源特性和豐富的文檔支持研究需求。 |
| 大型企業需要商業化雷達解決方案 | 不適合 | 目前仍處於 beta 階段，穩定性不足。 |
| 個人開發者學習雷達技術 | 非常適合 | 完整的文檔和社群支持使得學習過程順利。 |
...
> [!tip] 投入 vs 回報
> 花 10 小時學習，20 小時整合，能夠快速掌握雷達技術，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限運行，且不存取敏感資料。依賴鏈的信任程度高，無已知的供應鏈風險。
...
> | [@NawfalMotii79](https://github.com/NawfalMotii79) | 116 |
...
**社群活躍度**：社群活躍，有定期的更新和問題解決。
**連結**：[文件](https://NawfalMotii79.github.io/PLFM_RADAR/docs/)

## 開發動態

> [!abstract] 最近 10 次 commit（2026-04-18 ~ 2026-04-22）
> **活躍天數** 4 天 · **最新 commit** Added BOM and Gerbers 
...
> | --- | --- | --- | --- |
> | [#29](https://github.com/NawfalMotii79/PLFM_RADAR/issues/29) | Suggestion: Create Chat Group for Collaboration + Enclosure  | 4 | 16 |
...
相關概念：[[開源硬體]] · [[信號處理]] · [[雷達技術]] · [[模組化設計]] · [[FPGA]]
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["開源硬體","信號處理","雷達技術","模組化設計","FPGA"];
