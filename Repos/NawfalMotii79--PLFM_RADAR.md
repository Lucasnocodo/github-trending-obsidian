...
use_case: "提供開源、低成本的10.5 GHz PLFM相控陣雷達系統，讓研究者和開發者能夠探索雷達技術。"
priority: high
ring: assess
...
star_history: "2026-03-15:1703,2026-03-15:1708,2026-03-16:2576,2026-03-17:3042,2026-03-18:3202,2026-03-19:3341,2026-03-20:3405,2026-03-21:3448,2026-03-22:3471,2026-03-23:3512,2026-03-24:3539,2026-03-25:3549,2026-03-26:3559,2026-03-27:3573,2026-03-28:3584,2026-03-29:3597"
tags:
  - github
  - "category/基礎設施"
  - "lang/verilog"
aliases:
  ...
  - "提供開源、低成本的10.5 GHz PLFM相控陣雷達系統，讓研究者和開發者能夠探索雷達技術。"
---

# PLFM_RADAR

**3.6k** stars · **180** stars/天 · 建立 20 天前 · Verilog · NOASSERTION
...
> **適合** 對雷達技術有興趣的研究者和開發者，尤其是希望在低成本環境中進行實驗的人。
> **一句話重點** 這個專案不僅提供了雷達技術的開源實現，還鼓勵社群參與和合作，讓更多人能夠接觸和學習這項技術。
...
> **結論** 花10小時學習，20小時整合，得到一個靈活的雷達系統，值得投入。

> [!abstract] 核心創新
> AERIS-10提供了一個完全開源的雷達系統，讓用戶能夠自由修改和擴展其功能。

## 專案簡介

AERIS-10是一個開源的10.5 GHz相控陣雷達系統，專為研究者、無人機開發者及SDR愛好者設計。它提供了兩個版本，分別適用於3公里和20公里的探測範圍，並支持電子波束控制和先進的信號處理。用戶可以透過Python GUI進行實時操作，並整合GPS/IMU進行位置校正。該系統的架構包括多個子系統，如電源管理、頻率合成器和FPGA信號處理，確保了高效的信號處理和控制。

與其他雷達系統相比，AERIS-10的開源特性使其更具可擴展性和可定制性，適合不同的實驗需求。這個系統的設計考慮到了模組化，讓用戶可以根據需求進行組裝和修改。雖然目前仍在開發中，但其開源的特性和活躍的社群使其在未來有潛力成為一個成熟的雷達解決方案。對於需要進行雷達技術研究或開發的團隊，AERIS-10提供了一個低成本且靈活的平台。
...
- 雙版本可用性 — AERIS-10N（3公里）和AERIS-10E（20公里）版本。
- 全電子波束轉向 — 提供±45°的電子轉向能力。
- 先進的信號處理 — FPGA處理脈衝壓縮、Doppler FFT、MTI和CFAR。
- Python GUI — 提供用戶友好的介面和地圖整合功能。
- GPS/IMU整合 — 實時位置和姿態校正。
...
> [!tip] 爆紅原因
> 建立20天內累積3597 stars（每天180），forks 765（21.3%），顯示出強烈的社群參與度。這個專案由NawfalMotii79和JJassonn69主導，解決了傳統雷達系統高成本和封閉性問題，讓更多開發者能夠接觸和實驗雷達技術。社群的活躍度和開放的合作建議（如#29）顯示出對於這個專案的廣泛興趣。技術上，開源硬體和軟體的結合讓這個專案在雷達技術上具備了更高的靈活性和可擴展性。

## 適合誰使用

**目標受眾**：對雷達技術有興趣的研究者和開發者，尤其是希望在低成本環境中進行實驗的人。

> [!example] 使用場景
> - 無人機開發者用它來測試和優化自動導航系統，因為其高精度的雷達數據能提供實時環境感知。
> - 學術研究者用它來進行雷達信號處理的實驗，因為開源的特性允許他們自由修改和擴展功能。
> - 電子工程師用它來設計和測試新型雷達元件，因為其模組化設計使得硬體組裝和測試變得簡單。
> - 愛好者用它來學習雷達技術和信號處理，因為其完整的開源文檔和社群支持降低了入門門檻。

## 架構分析

AERIS-10的架構採用模組化設計，將不同的功能分散到獨立的子系統中，如電源管理、頻率合成和信號處理。這樣的設計使得系統更易於維護和擴展。數據流從DAC生成的LFM脈衝開始，經過上/下變頻處理，再由FPGA進行信號處理，最終通過Python GUI進行可視化。

選擇FPGA進行信號處理是因為其高效能和靈活性，能夠實現複雜的演算法。這種設計的代價是需要更高的開發和測試成本，特別是在FPGA開發上。整體架構的擴展性良好，但在高功率RF應用中可能面臨散熱和電源管理的挑戰。

## 技術深入分析

AERIS-10的核心技術機制包括使用FPGA進行信號處理，這使得系統能夠在實時環境中進行複雜的數據處理。系統的設計考慮到了模組化，將電源管理、頻率合成和信號處理分開，這樣可以簡化維護和升級的過程。效能方面，FPGA能夠快速處理大量數據，但需要高效的電源管理來支持其運行。選擇使用Python作為GUI開發語言，這使得用戶界面更加友好，並且易於擴展。技術風險方面，FPGA的開發需要專業知識，對於新手來說可能會造成困難。整合到現有的開發環境中，特別是與CI/CD流程的兼容性，可能需要額外的適配工作。

## 新手體驗

> [!info] 上手難度評估
> README文件提供了清晰的指導和範例，安裝過程相對順暢，但對於硬體組裝的要求可能對新手造成挑戰。整體上，對於有一定基礎的用戶來說，30分鐘內可以完成基本設置。
...
| 工具 | 差異 |
| --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 提供類似的雷達數據處理功能，但不具備開源硬體設計，適合商業用途。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | 專注於信號處理，但缺乏相控陣雷達的硬體支持，適合純軟體開發者。 |
| [NVIDIA/NemoClaw](https://github.com/NVIDIA/NemoClaw) | 針對AI和機器學習的應用，與本專案的硬體導向不同，適合需要高性能計算的場景。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 專注於數據處理和分析，但缺乏硬體設計，適合純軟體開發者。 | 如果你的團隊已經在使用數據分析工具，且不需要硬體支持。 | medium，因為需要重新設計數據流和處理邏輯。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | 提供信號處理功能，但不具備相控陣雷達的硬體支持，適合商業用途。 | 如果需要商業級別的信號處理而不關心硬體。 | low，因為主要是軟體層面的替代。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PLFM_RADAR** | **parsync** | **Shadowbroker** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於數據處理和分析，但缺乏硬體設計，適合純軟體開發者。 | 提供信號處理功能，但不具備相控陣雷達的硬體支持，適合商業用途。 |
> | 遷移成本 | - | medium，因為需要重新設計數據流和處理邏輯。 | low，因為主要是軟體層面的替代。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用數據分析工具，且不需要硬體支持。 | 如果需要商業級別的信號處理而不關心硬體。 |
...
> [!tip] 採用建議
> 適合有興趣的開發者進行實驗，不建議用於生產環境的核心應用。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 硬體組裝過程中可能會遇到元件缺失的問題
  - 解法：檢查BOM清單，確保所有元件都已訂購
- [MEDIUM] FPGA開發環境配置不當可能導致編譯失敗
  - 解法：參考官方文檔進行環境設置
- [MEDIUM] Python GUI在某些環境下可能無法正常啟動
  - 解法：確保安裝了正確版本的Python和依賴包

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 大學研究團隊進行雷達技術研究 | 非常適合 | 開源特性和模組化設計使其易於修改和擴展。 |
| 小型無人機開發公司需要低成本雷達解決方案 | 適合 | 提供了低成本的雷達系統，適合小型專案。 |
| 個人愛好者想學習雷達技術 | 非常適合 | 開源文檔和社群支持降低了學習門檻。 |
| 大型商業雷達系統開發 | 不適合 | 目前仍在開發中，穩定性不足。 |
...
> [!tip] 投入 vs 回報
> 花10小時學習，20小時整合，得到一個靈活的雷達系統，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限，且不存取敏感資料。依賴鏈的信任程度良好，適合在CI/CD中使用。
...
> | [#32](https://github.com/NawfalMotii79/PLFM_RADAR/issues/32) | MCU scan sequence does not match documented 31-elevation / ± | 0 | 6 |
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["自動化測試","深度學習","機器學習"];
