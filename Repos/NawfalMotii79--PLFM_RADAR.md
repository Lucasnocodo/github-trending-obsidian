...
use_case: "提供開源、低成本的10.5 GHz相控陣雷達系統，適合研究和開發。"
priority: high
ring: assess
...
aliases:
  ...
  - "提供開源、低成本的10.5 GHz相控陣雷達系統，適合研究和開發。"
---

# PLFM_RADAR

**18.6k** stars · **365** stars/天 · 建立 51 天前 · PLSQL · NOASSERTION
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Growing · **熱度** Hot (365 stars/day)
> **授權** NOASSERTION · **維護** Active (最後推送 1 天前) · **貢獻者** 5+ 人 · **參與度** Medium
> **適合** 對相控陣雷達技術感興趣的研究者和開發者，特別是無人機和SDR愛好者。
...
> **結論** 花10小時學習，15小時整合，得到一個靈活的雷達系統，值得投入。

> [!abstract] 核心創新
> AERIS-10是首個完全開源的低成本相控陣雷達系統，旨在讓更多人能夠接觸和實驗雷達技術。

## 專案簡介

AERIS-10是一個開源的10.5 GHz相控陣雷達系統，專為研究者和無人機開發者設計，支持脈衝線性頻率調製（LFM）。用戶可以透過Python GUI進行操作，並利用FPGA進行信號處理，實現目標追蹤和波束形成。這個系統提供兩種版本，分別為3公里和20公里的探測範圍，並具備電子波束轉向功能，能在±45°範圍內進行調整。系統架構上，AERIS-10採用模組化設計，將電源管理、頻率合成和射頻板分開，這樣的設計使得維護和擴展變得更加方便。關鍵的信號處理功能由FPGA負責，包括脈衝壓縮、Doppler FFT和目標檢測，這些功能使得雷達系統在處理複雜信號時表現出色。與傳統雷達系統相比，這個開源平台不僅降低了成本，還鼓勵用戶進行實驗和創新。使用者可以在GitHub上獲得完整的硬體和軟體設計，並且社群活躍，能夠快速獲得支援和解決方案。
...
- 雙版本可用 — AERIS-10N（3km範圍）和AERIS-10E（20km範圍）。
- 全電子波束轉向 — 在仰角和方位角上可達±45°的電子轉向。
- 先進的信號處理 — FPGA處理脈衝壓縮、Doppler FFT、MTI和CFAR。
- Python GUI — 友好的用戶介面，支持地圖集成和實時目標繪製。
...
> [!tip] 爆紅原因
> 建立51天內累積18597 stars（365/天），forks 4447（23.9%），顯示出極高的社群參與度。這個專案的作者NawfalMotii79及其團隊在開源硬體領域有豐富的經驗，之前的專案也受到廣泛關注。AERIS-10解決了高成本雷達系統的痛點，讓更多人能夠接觸和使用雷達技術。最近的社群討論和提議，如建立合作群組和增加LAN介面，進一步促進了其熱度。隨著開源硬體的興起，這個專案的可行性和實用性也得到了技術生態的支持，讓更多開發者能夠參與進來。forks/stars比率高達23.9%，顯示出許多人在實際修改和使用此工具。

## 適合誰使用

**目標受眾**：對相控陣雷達技術感興趣的研究者和開發者，特別是無人機和SDR愛好者。

> [!example] 使用場景
> - 研究人員用它來進行雷達信號處理實驗，因為其開源設計允許自由修改和擴展功能。
> - 無人機開發者用它來實現高精度的目標追蹤，因為其電子波束轉向功能能夠快速調整雷達方向。
> - 高級製造者用它來探索新型雷達技術，因為完整的硬體和軟體設計文件使得實驗變得更加容易。

## 架構分析

AERIS-10採用模組化設計，將電源管理、頻率合成和射頻板分開，這樣的設計使得維護和擴展變得更加方便。信號處理由FPGA負責，這樣可以實現高效的脈衝壓縮和Doppler處理。系統的每個子系統都有明確的功能分工，並且通過STM32微控制器進行協調，這樣的設計使得系統在處理複雜信號時能夠保持穩定性。選擇FPGA而非傳統微處理器是因為FPGA在處理高速數據流時的優勢，能夠實現更低的延遲和更高的吞吐量。整體架構的擴展性良好，能夠根據需求添加新的功能模塊。

## 技術深入分析

AERIS-10的核心技術機制在於其模組化設計和FPGA信號處理。FPGA負責脈衝壓縮、Doppler FFT等關鍵任務，這使得系統在處理複雜信號時能夠保持高效能。系統的最大範圍分為AERIS-10N和AERIS-10E，分別支持3公里和20公里的探測，這樣的設計考慮到不同用戶的需求。整體架構的設計選擇了高性能的時鐘生成器和頻率合成器，以確保信號的穩定性和準確性。由於使用了開源硬體設計，這個專案的依賴樹相對簡單，降低了維護成本。技術風險方面，FPGA的使用可能在未來的擴展中帶來一定的挑戰，尤其是在處理更高頻率或更複雜的信號時。整合方面，與現有的開發工具鏈相容性良好，能夠方便地與其他開源工具結合使用。

## 新手體驗

> [!info] 上手難度評估
> README文件清晰，包含詳細的安裝步驟和範例；安裝過程順暢，沒有明顯的坑；提供了良好的入門指南，文件有多語言支持。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於AI代理的開發，而AERIS-10專注於雷達技術，兩者的應用場景和技術需求不同。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 主要用於數據存儲和管理，而AERIS-10則專注於實時信號處理和雷達應用。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於AI代理的開發，而AERIS-10專注於雷達技術，兩者的應用場景和技術需求不同。 | 如果你的專案需要AI代理的功能，而不是雷達技術。 | medium，因為需要重新設計整個系統架構。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 主要用於數據存儲和管理，而AERIS-10則專注於實時信號處理和雷達應用。 | 如果你的需求主要是數據管理而非實時信號處理。 | low，因為可以直接使用現有的數據管理工具。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PLFM_RADAR** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於AI代理的開發，而AERIS-10專注於雷達技術，兩者的應用場景和技術需求不同。 | 主要用於數據存儲和管理，而AERIS-10則專注於實時信號處理和雷達應用。 |
> | 遷移成本 | - | medium，因為需要重新設計整個系統架構。 | low，因為可以直接使用現有的數據管理工具。 |
> | 適用場景 | 主要場景 | 如果你的專案需要AI代理的功能，而不是雷達技術。 | 如果你的需求主要是數據管理而非實時信號處理。 |
...
- [MEDIUM] FPGA配置可能需要多次調整才能達到最佳性能。
  - 解法：參考社群提供的最佳實踐和調整建議。
- [MEDIUM] 在不同環境下，雷達性能可能會受到干擾影響。
  - 解法：在安靜的環境中進行測試，並調整參數以適應環境變化。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10人以下的新創公司進行雷達技術研究 | 非常適合 | 開源設計降低了成本，並且社群支持活躍。 |
| 大型企業的商業雷達應用 | 不適合 | 目前仍在beta階段，穩定性和支持性不足。 |
| 學術機構進行雷達信號處理的實驗 | 非常適合 | 提供完整的硬體和軟體設計，便於實驗和研究。 |
| 個人開發者探索無人機雷達應用 | 適合 | 模組化設計使得個人開發者能夠輕鬆進行修改和擴展。 |
...
> [!tip] 投入 vs 回報
> 花10小時學習，15小時整合，得到一個靈活的雷達系統，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限，且不存取敏感資料。依賴鏈中沒有已知的供應鏈風險，適合在CI/CD中使用。
...
> | [#129](https://github.com/NawfalMotii79/PLFM_RADAR/issues/129) | ADAR1000Manager: Propagate SPI/ADC Communication Errors to C | 0 | 1 |
