...
use_case: "提供一個開源、低成本的 10.5 GHz 相控陣列雷達系統，適合研究和開發用途。"
priority: high
ring: assess
...
aliases:
  ...
  - "提供一個開源、低成本的 10.5 GHz 相控陣列雷達系統，適合研究和開發用途。"
---

# PLFM_RADAR

**3.8k** stars · **112** stars/天 · 建立 34 天前 · PLSQL · NOASSERTION
...
> **適合** 對雷達技術有興趣的研究人員和開發者，特別是希望探索開源硬體的使用者。
> **一句話重點** AERIS-10 不僅是一個雷達系統，更是一個開源硬體社群的實驗平台。
...
> [!abstract] 核心創新
> AERIS-10 是一個完全開源的相控陣列雷達系統，旨在降低雷達技術的進入門檻。

## 專案簡介

AERIS-10 是一個開源的 10.5 GHz 相控陣列雷達系統，專為研究人員、無人機開發者和 SDR 愛好者設計。其核心機制是使用脈衝線性頻率調製（LFM）技術，能夠在 3 公里和 20 公里的範圍內進行探測。用戶可以透過 Python GUI 進行操作，並且系統支持 GPS/IMU 整合以實現實時位置和姿態修正。這個系統的賣點在於其完全開源的硬體和軟體設計，讓使用者能夠自由修改和擴展功能。技術上，AERIS-10 使用了 XC7A50T FPGA 來處理信號，並且具備先進的信號處理能力，如脈衝壓縮和多普勒處理。與其他雷達系統相比，AERIS-10 提供了電子波束轉向功能，並且在硬體設計上採用了模組化架構，便於維護和升級。

這種設計使得使用者可以根據需求選擇不同的天線陣列和功率放大器，從而達到最佳的性能。實際使用中，AERIS-10 需要一定的硬體組裝經驗，並且對於 FPGA 開發工具（如 Vivado）有基本的要求。社群活躍度良好，開放問題解決率達到 73%。這個專案目前正處於穩定階段，適合對雷達技術有興趣的研究團隊或開發者。未來可望進一步擴展功能，並增加社群合作的機會。

**技術棧**：`Vivado` · `Python 3.8+` · `XC7A50T FPGA` · `STM32`
...
- 先進的信號處理 — FPGA 負責脈衝壓縮、多普勒 FFT、MTI 和 CFAR 等處理。
- Python GUI — 提供用戶友好的界面，並整合地圖功能以顯示目標。
- GPS/IMU 整合 — 實時位置和姿態修正，增強雷達性能。
...
> [!tip] 爆紅原因
> 建立 34 天內累積 3810 stars（112/天），forks 809（21.2%），顯示出強烈的社群興趣。專案的主要貢獻者包括 JJassonn69 和 NawfalMotii79，他們在開源硬體領域有一定的經驗。AERIS-10 解決了傳統雷達系統高成本和封閉性問題，讓更多開發者能夠進入這個領域。此專案的開放性和模組化設計吸引了許多對雷達技術感興趣的使用者，並且有潛在的市場需求。社群的活躍討論和建議也促進了專案的快速迭代和改進。

## 適合誰使用

**目標受眾**：對雷達技術有興趣的研究人員和開發者，特別是希望探索開源硬體的使用者。

> [!example] 使用場景
> - 無人機開發者用它來測試和驗證雷達技術，因為它提供了低成本的實驗平台，能夠進行多種雷達功能的實驗。
> - 學術研究者用它來進行相控陣列雷達的研究，因為其開源特性允許他們自由修改和擴展系統。
> - 電子工程師用它來學習和實踐 FPGA 設計，因為系統中使用了 XC7A50T FPGA，並且提供了豐富的信號處理功能。

## 架構分析

AERIS-10 的架構設計為模組化，主要由電源管理板、頻率合成板和主板組成。電源管理板確保所有電子元件的電壓供應，頻率合成板提供相位對齊的時鐘參考。主板上集成了 DAC、微波混頻器、相位移器和 FPGA，負責雷達信號的生成和處理。

這種設計使得系統能夠靈活應對不同的應用需求，並且便於維護和升級。選擇 FPGA 作為信號處理核心，能夠實現高效的數據處理和即時反應，然而這也增加了系統的複雜性和學習曲線。整體架構的模組化設計使得未來擴展和功能增強變得更加容易，但也可能導致整合時的摩擦點，特別是在硬體組裝和配置方面。

## 技術深入分析

AERIS-10 的核心技術機制基於相控陣列雷達原理，使用 LFM 技術進行信號調製。系統的信號處理由 XC7A50T FPGA 負責，這使得其在處理速度和效率上具有優勢。整個系統能夠處理的數據量相當可觀，並且具備即時反應能力。設計上選擇 FPGA 而非傳統的微控制器，能夠實現更複雜的信號處理算法，但也增加了開發的難度和學習曲線。系統的模組化設計使得不同子系統之間的整合變得靈活，但在實際應用中，可能會遇到硬體兼容性和配置的挑戰。對於未來的擴展，AERIS-10 的設計考慮到了可升級性，這意味著隨著技術的進步，使用者可以輕鬆地添加新功能或改進現有功能。整體而言，這個專案在開源硬體領域具有重要的實驗意義，並且在技術上具備相當的前瞻性。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的概述和安裝步驟，但缺乏詳細的組裝指南。安裝過程需要用戶具備一定的硬體和軟體基礎，對於新手來說可能會有一定的挑戰。文件中提供的範例和指令能夠幫助用戶快速上手，但仍需依賴社群的支持來解決具體問題。
...
> - FPGA 開發工具（如 Vivado）需要安裝以進行信號處理修改。
> - 目前沒有專門的硬體組裝指南，使用者需依賴電路圖進行組裝。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供類似的開源雷達技術，但主要集中在不同的頻率範圍和應用場景。 |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 專注於雷達數據的同步處理，而 AERIS-10 提供完整的雷達系統解決方案。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | 主要用於信號處理和數據分析，與 AERIS-10 的硬體整合性較低。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供類似的開源雷達技術，但主要集中在不同的頻率範圍和應用場景。 | 如果你的需求是針對特定頻率範圍的雷達應用，則可以考慮這個選項。 | medium，因為需要重新適應不同的設計和架構。 |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 專注於雷達數據的同步處理，而 AERIS-10 提供完整的雷達系統解決方案。 | 如果你的專案主要集中在數據處理而非硬體設計，則這個工具可能更合適。 | low，因為主要是數據處理的工具，與 AERIS-10 的整合相對簡單。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PLFM_RADAR** | **boneyard** | **parsync** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 提供類似的開源雷達技術，但主要集中在不同的頻率範圍和應用場景。 | 專注於雷達數據的同步處理，而 AERIS-10 提供完整的雷達系統解決方案。 |
> | 遷移成本 | - | medium，因為需要重新適應不同的設計和架構。 | low，因為主要是數據處理的工具，與 AERIS-10 的整合相對簡單。 |
> | 適用場景 | 主要場景 | 如果你的需求是針對特定頻率範圍的雷達應用，則可以考慮這個選項 | 如果你的專案主要集中在數據處理而非硬體設計，則這個工具可能更 |
...
- **[HIGH]** 硬體組裝過程中可能出現的兼容性問題，特別是在不同模組之間的連接上。
  - 解法：仔細檢查電路圖和連接，必要時尋求社群幫助。
- [MEDIUM] FPGA 開發環境的配置可能會對新手造成困難。
  - 解法：參考官方文檔和社群資源，逐步學習 FPGA 開發。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司進行雷達技術開發 | 非常適合 | 開源特性和低成本使其成為新創公司的理想選擇。 |
| 大型企業的核心雷達系統 | 不適合 | 目前仍在 beta 階段，穩定性和支持性不足。 |
| 學術研究機構進行雷達技術研究 | 適合 | 開放的設計和模組化架構適合進行深入研究。 |
| 個人開發者探索雷達技術 | 非常適合 | 提供了良好的學習平台和實驗機會。 |
...
> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限運行，且不存取敏感資料。依賴的開源庫經過社群審查，安全性相對可靠。
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["相控陣列","開源硬體","信號處理","雷達技術","FPGA"];
