...
use_case: "提供開源、低成本的10.5 GHz相控陣列雷達系統，適合研究和實驗。"
priority: high
ring: assess
...
aliases:
  ...
  - "提供開源、低成本的10.5 GHz相控陣列雷達系統，適合研究和實驗。"
---

# PLFM_RADAR

**14.6k** stars · **356** stars/天 · 建立 41 天前 · PLSQL · NOASSERTION
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Growing · **熱度** Hot (356 stars/day)
> **授權** NOASSERTION · **維護** Active (最後推送 0 天前) · **貢獻者** 5+ 人 · **參與度** Medium
> **適合** 對相控陣列雷達技術有興趣的研究人員和開發者，尤其是希望進行實驗和開發的學術界和創客。
> **一句話重點** AERIS-10不僅是雷達系統，更是一個開放的實驗平台，讓技術愛好者能夠深入探索雷達技術。
...
> **結論** 花20小時學習、30小時整合，得到一個可實驗的雷達系統，值得投入。

> [!abstract] 核心創新
> AERIS-10提供了一個完全開源的相控陣列雷達系統，讓用戶能夠自由修改和實驗。

## 專案簡介

AERIS-10是一個開源的10.5 GHz相控陣列雷達系統，旨在讓研究人員和開發者能夠探索和實驗雷達技術。使用者需要具備基本的雷達原理知識和PCB組裝經驗，並需安裝Python 3.8以上版本來運行GUI軟體。該系統的主要賣點在於其模組化設計，允許用戶進行電子波束控制、脈衝壓縮和目標追蹤等功能。系統架構包含多個硬體子系統，如電源管理、頻率合成和FPGA信號處理，並且支持GPS和IMU整合以實現實時位置修正。這個專案的技術實作上，使用了FPGA來處理脈衝壓縮和多普勒FFT，並且提供了Python GUI來進行可視化操作。

與其他雷達系統相比，AERIS-10的開源特性和低成本使其成為學術界和創客的理想選擇。相較於商業雷達系統，AERIS-10的可擴展性和可修改性讓用戶能夠根據需求進行調整。實際使用中，該系統能夠處理高達20公里的範圍，並且在信號處理上具備先進的功能，如MTI和CFAR檢測。社群活躍度高，開放的問題解決率達83%，顯示出良好的維護狀況。這個專案目前處於穩定階段，適合中小型團隊進行實驗和開發，未來可能會持續增強其功能和社群支持。

**技術棧**：`Python 3.8+` · `FPGA (XC7A50T)` · `Verilog`

## 重點功能

- 開源硬體與軟體 — 提供完整的電路圖、PCB佈局、韌體和軟體。
- 雙版本可用性 — AERIS-10N (3km範圍) 和 AERIS-10E (20km範圍)。
- 全電子波束控制 — 在仰角和方位角上可達±45°的電子控制。
- 先進信號處理 — FPGA負責脈衝壓縮、Doppler FFT、MTI和CFAR。
- Python GUI — 提供友好的用戶介面，支持地圖整合。
...
> [!tip] 爆紅原因
> 建立41天內累積14604 stars（356/天），forks數量3160（21.6%），顯示出強大的社群參與度。主要貢獻者包括JJassonn69和joyshmitz等，這些貢獻者在開源硬體和雷達技術方面有豐富經驗。這個專案解決了傳統雷達系統高成本和封閉性問題，讓更多人能夠接觸和實驗雷達技術。社群的活躍討論和建議進一步促進了專案的發展。技術上，開源硬體的興起使得這樣的系統變得可行，並且有助於推動相關技術的進步。forks/stars比率21.6%顯示出許多用戶在積極修改和使用這個專案。

## 適合誰使用

**目標受眾**：對相控陣列雷達技術有興趣的研究人員和開發者，尤其是希望進行實驗和開發的學術界和創客。

> [!example] 使用場景
> - 大學研究生用它來進行相控陣列雷達的實驗，因為其開源特性讓他們能夠自由修改設計以符合研究需求。
> - 無人機開發者用它來整合雷達功能於自家產品中，因為其低成本和模組化設計使得集成變得簡單。
> - 電子工程師用它來測試和驗證新的信號處理算法，因為其先進的信號處理能力和開放的硬體設計提供了良好的測試平台。

## 架構分析

AERIS-10的架構設計為模組化，主要由電源管理板、頻率合成板和主板組成。電源管理板確保所有電子元件的電壓供應，頻率合成板使用高性能時鐘生成器提供相位對齊的時鐘參考。主板上集成了DAC、微波混頻器、相位移器和FPGA，負責生成雷達脈衝和信號處理。

選擇FPGA進行信號處理的原因在於其高效能和可編程性，能夠實現複雜的信號處理算法。這樣的設計雖然增加了開發的複雜度，但也提供了更大的靈活性和擴展性。整體架構的瓶頸在於FPGA的性能，隨著處理需求的增加，可能需要更高效的硬體來支援未來的擴展。

整合到現有的開發環境中，特別是與FPGA開發工具的兼容性，是一個需要注意的摩擦點。

## 技術深入分析

AERIS-10的核心技術機制是其相控陣列雷達設計，使用FPGA進行信號處理，這使得系統能夠實現高效的脈衝壓縮和多普勒FFT處理。該系統能夠處理的最大範圍為20公里，並且在信號處理上具備先進的功能，如MTI和CFAR檢測。設計上選擇FPGA而非其他處理器，因為FPGA能提供更高的並行處理能力，適合於實時信號處理的需求。依賴樹的複雜度適中，主要依賴於FPGA開發工具和Python環境。技術風險方面，隨著使用者需求的增加，FPGA的性能可能成為瓶頸，未來可能需要進一步的硬體升級。整合到現有的開發環境中，特別是與FPGA開發工具的兼容性，是一個需要注意的摩擦點。整體來說，這個專案在開源硬體領域提供了一個有價值的實驗平台，並且對於學術研究和創客文化有著重要的推動作用。

## 新手體驗

> [!info] 上手難度評估
> README文件清晰且詳細，提供了安裝和使用的具體步驟。安裝過程相對順暢，但需要一定的硬體組裝經驗。文件中有良好的入門指南，並且提供了多語言支持，方便不同背景的使用者理解。
...
| 工具 | 差異 |
| --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard專注於無人機的飛行控制，而AERIS-10則專注於雷達信號處理，適用於不同的應用場景。 |
| [AgentSeal/codeburn](https://github.com/AgentSeal/codeburn) | Codeburn主要用於數據處理和分析，而AERIS-10則提供硬體層面的雷達系統，兩者的技術路線和應用範圍不同。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard專注於無人機的飛行控制，而AERIS-10則專注於雷達信號處理，適用於不同的應用場景。 | 如果你的專案需要無人機的飛行控制功能，Boneyard會是更適合的選擇。 | medium，因為兩者的架構和功能差異較大。 |
| [AgentSeal/codeburn](https://github.com/AgentSeal/codeburn) | Codeburn主要用於數據處理和分析，而AERIS-10則提供硬體層面的雷達系統，兩者的技術路線和應用範圍不同。 | 如果你的需求主要集中在數據分析而非硬體實現，Codeburn會更合適。 | high，因為需要重新考慮整個系統架構。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PLFM_RADAR** | **boneyard** | **codeburn** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | Boneyard專注於無人機的飛行控制，而AERIS-10則專注於雷達信號處理，適用於不同的應用場景。 | Codeburn主要用於數據處理和分析，而AERIS-10則提供硬體層面的雷達系統，兩者的技術路線和應用範圍不同。 |
> | 遷移成本 | - | medium，因為兩者的架構和功能差異較大。 | high，因為需要重新考慮整個系統架構。 |
> | 適用場景 | 主要場景 | 如果你的專案需要無人機的飛行控制功能，Boneyard會是更 | 如果你的需求主要集中在數據分析而非硬體實現，Codeburn |
...
- **[HIGH]** 硬體組裝過程中可能會遇到元件不匹配的問題
  - 解法：仔細檢查BOM清單，確保所有元件正確無誤
- [MEDIUM] FPGA開發工具的版本兼容性問題
  - 解法：使用推薦的Vivado版本進行開發
- [MEDIUM] 信號處理過程中可能出現延遲
  - 解法：優化FPGA設計以減少延遲
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10人以下的新創公司進行雷達技術開發 | 非常適合 | 開源特性和低成本使得技術實驗變得可行。 |
| 大型企業的核心雷達系統 | 不適合 | 可能缺乏商業支持和穩定性。 |
| 學術研究機構進行雷達技術的探索 | 非常適合 | 開放的設計和社群支持促進了研究的進行。 |
| 愛好者進行DIY雷達項目 | 非常適合 | 模組化設計和開源特性使得實驗變得簡單。 |
...
> [!tip] 投入 vs 回報
> 花20小時學習、30小時整合，得到一個可實驗的雷達系統，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限操作，且不存取敏感資料。依賴鏈的信任程度較高，沒有已知的供應鏈風險，適合在CI/CD中使用。

## 生態系整合

> [!abstract] 如何融入你的工具鏈

AERIS-10最常與FPGA開發工具和Python環境搭配使用，適合在開發和測試階段進行整合。在一個使用Vivado的FPGA專案中，可以用AERIS-10來進行信號處理，具體做法是將FPGA設計與AERIS-10的硬體架構結合。支援GitHub Actions等CI工具，並且可以與VS Code進行整合。整合過程中，最常見的問題是FPGA開發環境的版本兼容性，建議使用官方推薦的版本。
...
> | [@NawfalMotii79](https://github.com/NawfalMotii79) | 112 |
> | [@joyshmitz](https://github.com/joyshmitz) | 10 |
> | [@3aLaee](https://github.com/3aLaee) | 3 |
> | [@Copilot](https://github.com/Copilot) | 1 |

**最新版本**：v2.0.0-fft2048 — v2.0.0-fft2048 — AERIS-10 2048-pt FFT + Full Timing Closure (2026-04-16)
...
> **活躍天數** 3 天 · **最新 commit** Merge pull request #109 from NawfalMotii79/develop
...
> | [#112](https://github.com/NawfalMotii79/PLFM_RADAR/issues/112) | UX improvement? | 0 | 0 |
...
相關專案：[[0xGF--boneyard|0xGF/boneyard]] · [[AgentSeal--codeburn|AgentSeal/codeburn]] · [[BuilderPulse--BuilderPulse|BuilderPulse/BuilderPulse]] · [[AgriciDaniel--claude-obsidian|AgriciDaniel/claude-obsidian]] · [[BigBodyCobain--Shadowbroker|BigBodyCobain/Shadowbroker]] · [[AlpinDale--parsync|AlpinDale/parsync]] · [[NVIDIA--NemoClaw|NVIDIA/NemoClaw]] · [[L42ARO--Mercury-Transforming-Drone|L42ARO/Mercury-Transforming-Drone]]
