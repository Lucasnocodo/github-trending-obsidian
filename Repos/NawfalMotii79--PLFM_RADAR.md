...
**20.4k** stars · **279** stars/天 · 建立 73 天前 · PLSQL · NOASSERTION
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Growing · **熱度** Hot (279 stars/day)
> **授權** NOASSERTION · **維護** Active (最後推送 2 天前) · **貢獻者** 5+ 人 · **參與度** Medium
> **適合** 對雷達技術有興趣的研究者和開發者，特別是需要低成本解決方案的團隊。
> **一句話重點** AERIS-10不僅是雷達技術的開源實驗平台，更是推動開源硬體發展的重要一步。
...
> [!abstract] 核心創新
> 這個專案的核心創新在於其開源的相控陣列雷達系統，讓更多人能夠接觸和實驗雷達技術。

## 專案簡介

AERIS-10是一個開源的10.5 GHz相控陣列雷達系統，旨在讓研究者和開發者能夠輕鬆實驗雷達技術。用戶可以透過Python GUI進行操作，並利用FPGA進行高效的信號處理。該系統支持脈衝線性頻率調變（LFM），並提供兩個版本，分別為3km和20km的探測範圍，適合不同需求的用戶。核心賣點在於其模組化設計，使用者可以根據需求進行硬體組裝和軟體修改。系統架構包括電源管理、頻率合成、信號處理等多個子系統，並且具備GPS/IMU集成以實現實時定位。使用者可以透過命令行工具進行操作，並利用FPGA進行脈衝壓縮、Doppler處理等高級信號處理。這樣的設計使得AERIS-10在低成本的同時，依然能提供高性能的雷達功能。

**技術棧**：`Python 3.8+` · `FPGA (Vivado)` · `C++` · `Verilog`

## 重點功能

- 開源硬體與軟體 — 完整的原理圖、PCB佈局和韌體可供使用者修改。
- 雙版本可選 — AERIS-10N（3km範圍）和AERIS-10E（20km範圍），滿足不同需求。
- 全電子波束轉向 — 在仰角和方位角上可進行±45°的電子轉向。
- 先進的信號處理 — FPGA進行脈衝壓縮、Doppler FFT、MTI和CFAR處理。
- Python GUI — 提供用戶友好的界面，並集成地圖功能。
- GPS/IMU集成 — 實時位置和姿態修正，增強雷達數據的準確性。
...
3. 組裝硬體
```bash
根據/4_Schematics and Boards Layout中的文件進行組裝
```

## 為什麼值得關注

> [!tip] 爆紅原因
> 建立73天內累積20363 stars（279/天），forks 4841（23.8%），顯示出強烈的社群興趣。作者NawfalMotii79及其團隊致力於開源硬體，並在雷達技術上填補了市場空白，讓更多人能夠接觸和使用雷達技術。這個專案的出現正好符合了對開源、可修改硬體的需求，特別是在教育和研究領域。社群的活躍度也反映在熱門問題上，顯示出使用者對於功能擴展和性能優化的需求。
...
> [!example] 使用場景
> - 大學研究生用它來進行雷達信號處理實驗，因為其開源特性讓他們能夠自由修改和擴展功能。
> - 無人機開發者用它來測試和優化飛行中的雷達探測，因為其低成本和模組化設計適合快速迭代。
> - 電子工程師用它來學習和實驗相控陣列雷達技術，因為完整的硬體和軟體文檔提供了良好的學習資源。

## 架構分析

AERIS-10的架構設計為模組化，主要由電源管理、頻率合成、信號處理等子系統組成。每個子系統都專注於特定功能，確保系統的靈活性和可擴展性。信號處理部分使用FPGA來進行高效的數據處理，這樣的設計使得系統能夠在實時環境中運行。

選擇FPGA而非傳統微控制器的原因在於其能夠同時處理大量數據，並且支持複雜的信號處理算法。這樣的架構設計雖然提高了開發的複雜性，但也為用戶提供了更大的自由度和擴展性。系統的擴展性瓶頸主要在於FPGA的資源限制，隨著功能的增加，可能需要更高性能的FPGA來支持。

## 技術深入分析

AERIS-10的核心技術機制在於其使用FPGA進行信號處理，這使得系統能夠實現複雜的雷達信號處理任務，如脈衝壓縮和Doppler FFT。這些處理任務需要高效的數據流和計算能力，FPGA的並行處理能力正好滿足了這一需求。系統能夠處理的數據量取決於FPGA的資源配置和設計，通常能夠支持多達16個通道的同時處理。設計選擇上，FPGA的使用雖然增加了開發的複雜性，但也提供了更高的性能和靈活性。這個選擇的代價在於需要使用者具備FPGA開發的知識，並且需要相應的開發工具。技術風險方面，隨著用戶需求的增加，FPGA的資源可能成為瓶頸，影響系統的擴展性。整合方面，AERIS-10與主流開發環境的兼容性良好，特別是在Linux系統下，能夠輕鬆整合到現有的開發流程中。

## 新手體驗

> [!info] 上手難度評估
> README文件提供了清晰的安裝指導和使用說明，但缺乏具體的範例。安裝過程相對順暢，但對於初學者來說，可能會遇到一些技術挑戰。整體來說，花30分鐘能夠完成基本的安裝和配置，但深入使用仍需時間學習。
...
> - 對於硬體組裝有一定的技術要求，需具備PCB組裝經驗。
> - 不支援Windows系統，需在Linux或macOS環境下運行。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於智能代理的開發，而本專案專注於雷達技術的開源實驗。 |
| [432539/gpt2api](https://github.com/432539/gpt2api) | 主要用於自然語言處理，而本專案則是針對雷達信號處理的硬體解決方案。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於智能代理的開發，使用Python進行高層次的抽象，而本專案則是針對低層次的硬體設計和信號處理。 | 如果你的團隊專注於AI和自動化代理的開發，則選擇agent-sprite-forge會更合適。 | medium，因為需要重新學習不同的技術棧。 |
| [432539/gpt2api](https://github.com/432539/gpt2api) | 主要用於自然語言處理，提供API接口，而本專案則是針對實體硬體的開發和實驗。 | 如果你的專案重點在於文本生成和自然語言處理，則選擇gpt2api會更合適。 | high，因為需要完全不同的技術背景和開發環境。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PLFM_RADAR** | **agent-sprite-forge** | **gpt2api** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於智能代理的開發，使用Python進行高層次的抽象，而本專案則是針對低層次的硬體設計和信號處理。 | 主要用於自然語言處理，提供API接口，而本專案則是針對實體硬體的開發和實驗。 |
> | 遷移成本 | - | medium，因為需要重新學習不同的技術棧。 | high，因為需要完全不同的技術背景和開發環境。 |
> | 適用場景 | 主要場景 | 如果你的團隊專注於AI和自動化代理的開發，則選擇agent- | 如果你的專案重點在於文本生成和自然語言處理，則選擇gpt2a |
...
> [!tip] 採用建議
> 適合進行實驗和學術研究，但不建議用於生產環境的核心應用。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** FPGA配置不當可能導致信號處理失敗
  - 解法：確保遵循官方文檔中的配置步驟
- [MEDIUM] 硬體組裝過程中可能出現焊接問題
  - 解法：建議使用專業的PCB組裝服務
- [MEDIUM] Python GUI在某些Linux版本上可能不兼容
  - 解法：檢查相依性並更新至最新版本
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 大學研究生進行雷達信號處理實驗 | 非常適合 | 開源設計和完整的文檔支持學術研究。 |
| 無人機開發團隊需要集成雷達系統 | 適合 | 模組化設計便於集成和擴展。 |
| 小型創業公司開發商業雷達產品 | 普通 | 雖然成本低，但可能需要較多的技術支持。 |
| 業界大型企業進行雷達技術商業化 | 不適合 | 目前仍處於beta階段，穩定性不足。 |
...
> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限操作，且不存取敏感資料，適合在CI/CD環境中使用。
