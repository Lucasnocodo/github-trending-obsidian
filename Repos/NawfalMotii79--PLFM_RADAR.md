...
**18.5k** stars · **369** stars/天 · 建立 50 天前 · PLSQL · NOASSERTION
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Growing · **熱度** Hot (369 stars/day)
> **授權** NOASSERTION · **維護** Active (最後推送 0 天前) · **貢獻者** 5+ 人 · **參與度** Medium
> **適合** 對雷達技術有興趣的研究者和開發者，特別是需要低成本解決方案的團隊。
> **一句話重點** AERIS-10 不僅是一個雷達系統，更是一個開放的實驗平台，讓使用者能夠深入探索雷達技術。
...
> [!abstract] 核心創新
> AERIS-10 提供一個完全開源且模組化的相控陣列雷達系統，讓使用者能夠自由探索和實驗。

## 專案簡介

AERIS-10 是一個開源的 10.5 GHz 相控陣列雷達系統，專為研究人員、無人機開發者和 SDR 愛好者設計。使用者可以透過該系統探索波束形成、脈衝壓縮、杜普勒處理和目標追蹤等技術。其核心機制包括 DAC 生成 LFM 脈衝，經過 LTC5552 混頻器進行頻率轉換，然後由 FPGA 負責信號處理，最終透過 Python GUI 進行可視化。這個系統的賣點在於其開源硬體和軟體，讓使用者可以自由修改和擴展功能。技術上，AERIS-10 使用了高性能的 FPGA 進行信號處理，並且支援 GPS/IMU 整合，提供實時位置和姿態修正。

與其他雷達系統相比，AERIS-10 的模組化設計使其更易於維護和升級，並且提供兩個版本以滿足不同範圍需求（3km 和 20km）。在同類產品中，像是 0x0funky/agent-sprite-forge 和 0xGF/boneyard 主要集中於特定應用，而 AERIS-10 則提供更廣泛的應用場景和開發靈活性。實際使用中，該系統能夠處理複雜的信號處理任務，但對於硬體組裝和 FPGA 開發有一定的技術門檻。這個專案目前處於穩定階段，適合對雷達技術有興趣的開發者和研究團隊。未來可能會增加更多的功能和改進，特別是在信號處理和用戶界面方面。

**技術棧**：`Vivado` · `Python 3.8+` · `FPGA (XC7A50T)`
...
- 先進的信號處理 — FPGA 負責脈衝壓縮、杜普勒 FFT、MTI 和 CFAR。
- Python GUI — 提供用戶友好的界面，並整合地圖功能。
- GPS/IMU 整合 — 實時位置和姿態修正，增強雷達數據的準確性。
...
> [!tip] 爆紅原因
> 建立 50 天就累積 18452 stars（369/天），forks 4414（23.9%），顯示出強烈的社群參與。專案的主要貢獻者包括多位活躍的開發者，且其開源性質吸引了大量對雷達技術感興趣的研究者。AERIS-10 解決了高成本雷達系統的痛點，讓更多人能夠進行相關研究和開發。社群的活躍度和開發者的回應速度都相對良好，顯示出這個專案的健康狀態。技術上，開源硬體的興起和對無人機技術的需求增加，使得這個工具的可行性大幅提升。
...
> [!example] 使用場景
> - 無人機開發者用它來實現高精度的目標追蹤，因為其先進的信號處理和波束形成技術能夠提供更準確的數據。
> - 學術研究者用它來進行雷達信號處理的實驗，因為開源的特性讓他們能夠自由修改和擴展系統功能。
> - 電子工程師用它來測試和驗證新開發的雷達算法，因為其模組化設計使得硬體和軟體的整合變得簡單。

## 架構分析

AERIS-10 的架構由多個子系統組成，包括電源管理板、頻率合成器板和主板。電源管理板確保所有電子元件獲得所需的電壓，頻率合成器板使用高性能的低抖動時鐘生成器來提供相位對齊的時鐘參考。主板則包含 DAC、混頻器、相位移器和 FPGA，負責雷達信號處理。

選擇 FPGA 進行信號處理的原因在於其高效能和靈活性，能夠處理複雜的信號處理任務。這種架構的代價在於需要較高的技術門檻和硬體組裝經驗。擴展性方面，模組化設計使得未來的功能擴展和維護變得更容易。

## 技術深入分析

AERIS-10 的核心技術機制包括使用 FPGA 進行信號處理，這使其能夠高效地處理複雜的雷達信號。該系統能夠處理高達 20km 的距離，並具備電子波束控制能力，這在傳統雷達系統中往往需要昂貴的硬體。設計上選擇了模組化架構，使得不同子系統可以獨立開發和升級，這樣的選擇雖然增加了初期的組裝複雜度，但長期來看卻能降低維護成本。技術風險方面，FPGA 的使用需要專業知識，可能在擴展時遇到瓶頸。與主流開發框架的整合方面，AERIS-10 提供了 Python GUI，這使得與現有的開發工具鏈相容性良好，並且能夠簡化使用者的操作流程。整體而言，AERIS-10 提供了一個強大的平台，適合對雷達技術有興趣的開發者進行深入研究和實驗。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且詳細，提供了安裝和使用的指導。安裝過程中需要注意硬體組裝的細節，對於新手可能有一定挑戰。文件中包含了良好的入門指南，並且有多語言支持，方便不同背景的使用者理解。
...
> [!danger] 缺點
> - 對於新手來說，硬體組裝和 FPGA 開發有一定的技術門檻。
> - 目前僅支援特定的雷達應用，擴展性可能有限。
> - 需要額外的硬體元件，可能增加成本。

> [!warning] 注意事項
> - 需要一定的硬體組裝經驗，對於新手可能有挑戰。
> - FPGA 開發需要專業知識，可能不適合所有開發者。
> - 目前僅支援特定的雷達應用，對於其他用途的擴展可能有限。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 主要針對特定應用的雷達系統，而 AERIS-10 提供更廣泛的應用和開發靈活性。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於特定的信號處理技術，AERIS-10 則提供完整的雷達系統解決方案。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於特定應用的雷達系統，提供簡化的功能集。 | 如果你的需求僅限於簡單的雷達應用，且不需要擴展性。 | low，因為功能簡單且易於上手。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 主要針對特定的信號處理技術，功能較為專一。 | 如果你的專案需要針對特定信號處理進行深度優化。 | medium，因為需要重新調整設計以符合其架構。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PLFM_RADAR** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於特定應用的雷達系統，提供簡化的功能集。 | 主要針對特定的信號處理技術，功能較為專一。 |
> | 遷移成本 | - | low，因為功能簡單且易於上手。 | medium，因為需要重新調整設計以符合其架構。 |
> | 適用場景 | 主要場景 | 如果你的需求僅限於簡單的雷達應用，且不需要擴展性。 | 如果你的專案需要針對特定信號處理進行深度優化。 |
...
- **[HIGH]** 硬體組裝過程中可能會遇到元件缺失的問題
  - 解法：仔細檢查 BOM 文件，確保所有元件都已準備好
- [MEDIUM] FPGA 開發工具的版本不兼容可能導致編譯失敗
  - 解法：遵循 README 中的版本要求，使用推薦的工具版本
- [MEDIUM] Python GUI 可能在某些環境下無法正常啟動
  - 解法：確保安裝了所有必要的依賴，並檢查 Python 版本

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司進行雷達技術開發 | 非常適合 | 開源性質和低成本使其成為理想選擇。 |
| 大型企業的核心雷達系統 | 不適合 | 目前仍在 beta 階段，穩定性不足。 |
| 學術機構進行雷達信號處理研究 | 適合 | 提供開放的實驗平台，便於研究和開發。 |
| 業餘愛好者進行無人機雷達應用 | 非常適合 | 模組化設計和開源特性使其易於使用和修改。 |
...
> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限，且不存取敏感資料。依賴鏈中未發現已知的供應鏈風險，適合在 CI/CD 中使用。
