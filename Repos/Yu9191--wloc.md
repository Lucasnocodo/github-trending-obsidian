...
use_case: "修改 Apple 网络定位返回坐标，支持多种代理工具和快捷指令一键设置。"
priority: high
ring: assess
...
star_history: "2026-06-27:457,2026-06-27:462,2026-06-28:644,2026-06-28:647,2026-06-29:1092,2026-06-29:1098,2026-06-30:1508,2026-06-30:1515,2026-07-01:1965,2026-07-01:1968,2026-07-02:2357,2026-07-03:2584,2026-07-04:2845,2026-07-05:2957,2026-07-06:3094,2026-07-07:3167,2026-07-08:3299,2026-07-09:3489"
tags:
  - github
  - "category/開發工具"
  - "lang/javascript"
...
**3.5k** stars · **249** stars/天 · 建立 14 天前 · JavaScript · 未標註授權
...
> [!info] 速覽
> **安裝難度** Easy · **專案狀態** Recent · **熱度** Hot (249 stars/day)
> **授權** 未標註授權 (風險較高) · **維護** Moderate (最後推送 11 天前) · **貢獻者** Solo (bus factor 風險) · **參與度** Medium
> **適合** 需要在 iOS 設備上虛擬定位的用戶，尤其是開發者和自由職業者。
> **一句話重點** WLOC 的設計不僅解決了虛擬定位的需求，還考慮到了用戶的操作便利性，這是它成功的關鍵。
...
> **結論** 花 2 小時學習，1 小時整合，得到高效的虛擬定位功能，值得使用。

> [!abstract] 核心創新
> 提供了一個簡單的方式來修改 Apple 網絡定位服務返回的坐標，支持多種代理工具和快捷指令。

## 專案簡介

WLOC 是一個用於修改 Apple 網絡定位服務（gs-loc）的工具，能夠虛擬化 iOS 設備的地理位置。用戶可以通過選擇地圖上的位置，然後使用快捷指令來設置或恢復定位，這樣就不需要手動輸入經緯度。它支持 Surge、Quantumult X、Loon 和 Stash 等多種代理工具，並且提供了簡單的 API 解析功能，能夠將 GCJ-02 坐標轉換為 WGS84 格式。這種設計使得用戶在使用地圖應用時，能夠輕鬆切換虛擬位置，避免了繁瑣的手動設置。WLOC 的核心賣點在於其便捷性，特別是對於需要頻繁更改位置的用戶來說，這大大提高了效率。

技術上，WLOC 使用 JavaScript 實現，並依賴於各種代理工具的模組化設計，這使得其擴展性和靈活性都非常高。與其他類似工具相比，WLOC 提供了更簡單的使用流程和更高的兼容性，特別是在 iOS 26 及以上版本中，能夠有效應對系統對定位緩存的強化。使用者在使用過程中可能會遇到定位緩存問題，這需要重啟設備來清除緩存。總體來說，WLOC 是一個成熟且實用的工具，適合需要頻繁改變位置的用戶使用。
...
- 快捷指令一鍵設置 — 用戶可通過快捷指令快速設置或恢復位置，無需手動填寫經緯度。
- 在線選點頁面 — 提供簡單的界面讓用戶選擇地理位置，並自動生成設置指令。
- GCJ-02 轉 WGS84 解析 — 自動處理中國大陸的坐標轉換，確保定位準確性。
- 支持收藏位置 — 用戶可以收藏多個虛擬位置，方便快速切換。
...
2. 選擇位置並儲存
```bash
在地圖 App 中選擇位置，然後分享並選擇「wloc 设置地理位置」
```
3. 清除位置
```bash
使用快捷指令「wloc 清理恢复位置」
...
> [!tip] 爆紅原因
> 建立 14 天內累積 3489 stars（249/天），forks 521（14.9%），顯示出強勁的增長勢頭。作者 Yu9191 是一位活躍的開發者，專注於網絡定位和代理工具的開發。WLOC 解決了 iOS 設備在定位方面的痛點，特別是對於需要虛擬定位的用戶。這個工具的出現正值 iOS 定位機制變化的時期，許多用戶需要新的解決方案來應對定位緩存問題。高 forks/stars 比率（14.9%）顯示出社群對這個工具的實際修改和使用需求。

## 適合誰使用

**目標受眾**：需要在 iOS 設備上虛擬定位的用戶，尤其是開發者和自由職業者。

> [!example] 使用場景
> - 自由職業者用它來在不同城市的地圖應用中虛擬定位，因為這樣可以避免因為實際位置而受到的限制。
> - 開發者用它來測試地圖應用的定位功能，因為可以快速切換不同的虛擬位置，節省了大量的時間。
> - 旅行者用它來在不出門的情況下查看不同地點的資訊，因為這樣可以在計劃行程時獲得更準確的地理數據。

## 架構分析

WLOC 的架構設計基於模組化的代理工具，這樣的設計使得用戶可以根據自己的需求選擇不同的模組。資料流方面，使用者選擇位置後，通過快捷指令發送請求到 gs-loc.apple.com，然後由代理模組攔截並修改返回的坐標。這種設計的優勢在於靈活性，使用者可以根據不同的需求選擇合適的模組，但缺點是需要用戶具備一定的技術基礎來配置代理工具。擴展性方面，WLOC 可以輕鬆添加新的模組以支持更多的代理工具，這使得它在未來的發展中具有良好的潛力。

## 技術深入分析

WLOC 的核心技術機制是通過攔截 Apple 網絡定位服務的請求來修改返回的坐標，這一過程使用了 JavaScript 實現的代理模組。這些模組能夠根據用戶的需求靈活調整，並且支持多種不同的代理工具。效能方面，WLOC 能夠快速響應用戶的請求，並且在大多數情況下能夠即時生效。設計上，WLOC 依賴於現有的代理工具生態，這使得它的依賴樹相對簡單，降低了使用門檻。然而，這也意味著用戶需要對代理工具有一定的了解，才能充分發揮 WLOC 的功能。技術風險方面，WLOC 可能會受到 iOS 系統更新的影響，特別是在定位緩存方面的變化，這可能會導致用戶在使用過程中遇到問題。整合方面，WLOC 可以與主流的代理工具無縫對接，並且在 CI/CD pipeline 中也能夠輕鬆集成，這使得它在開發過程中非常友好。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了詳細的使用說明和範例。安裝過程相對順暢，但需要用戶具備一定的技術基礎。文件目前僅提供英文版本，對於非英語使用者可能會有一定的學習曲線。
...
> [!danger] 缺點
> - 僅限於 iOS 設備，無法跨平台使用。
> - 需要用戶具備一定的技術基礎來配置代理工具。
> - 在高版本 iOS 中可能需要重啟設備以清除定位緩存。

> [!warning] 注意事項
> - 僅支持 iOS 設備，無法在 Android 上使用。
> - 需要在代理模式下運行，否則無法正常工作。
> - iOS 26 及以上版本需要重啟設備以清除定位緩存。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供類似的虛擬定位功能，但主要針對遊戲環境，使用場景較為狹窄。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於網絡流量的修改，並不專門針對定位功能，適用範圍更廣。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於遊戲虛擬定位，使用 Unity 實現，對於遊戲開發者來說更具針對性。 | 如果你的主要需求是在遊戲環境中進行虛擬定位，則這個工具更合適。 | medium，因為需要重新學習新的 API 和模組配置。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供更廣泛的網絡流量修改功能，使用 Python 實現，適合需要全面流量控制的用戶。 | 如果你需要的不僅僅是定位，還需要對網絡流量進行全面控制，則這個工具更合適。 | high，因為需要重新設置整個流量控制環境。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **wloc** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於遊戲虛擬定位，使用 Unity 實現，對於遊戲開發者來說更具針對性。 | 提供更廣泛的網絡流量修改功能，使用 Python 實現，適合需要全面流量控制的用戶。 |
> | 遷移成本 | - | medium，因為需要重新學習新的 API 和模組配置。 | high，因為需要重新設置整個流量控制環境。 |
> | 適用場景 | 主要場景 | 如果你的主要需求是在遊戲環境中進行虛擬定位，則這個工具更合適 | 如果你需要的不僅僅是定位，還需要對網絡流量進行全面控制，則這 |
...
- **[HIGH]** 在 iOS 26+ 中，定位緩存可能導致修改後的坐標無法立即生效。
  - 解法：重啟設備以清除定位緩存。
- [MEDIUM] 需要確保代理工具已正確配置，否則無法正常工作。
  - 解法：檢查代理設置並確保信任 gs-loc.apple.com。
- [MEDIUM] 在高版本 iOS 中，可能需要額外的步驟來清除定位緩存。
  - 解法：參考 README 中的高版本操作流程。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 需要在 iOS 設備上虛擬定位的開發者 | 非常適合 | 提供了簡單的設置方式和多種代理工具支持。 |
| 需要在 Android 設備上虛擬定位的用戶 | 不適合 | 僅支持 iOS 設備。 |
| 需要頻繁切換虛擬位置的自由職業者 | 非常適合 | 快捷指令的設計大大提高了操作效率。 |
| 對定位準確性要求極高的專業用戶 | 普通 | 可能需要手動處理 GCJ-02 到 WGS84 的轉換。 |
...
> [!tip] 投入 vs 回報
> 花 2 小時學習，1 小時整合，得到高效的虛擬定位功能，值得使用。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限，並且不存取敏感資料。依賴的模組和 API 也經過開源社群的驗證，安全性較高。
...
> | --- | --- | --- | --- |
> | [#39](https://github.com/Yu9191/wloc/issues/39) | Shadowrocket+快捷指令（秒成功）操作流程图文记录 | 12 | 15 |
> | [#35](https://github.com/Yu9191/wloc/issues/35) | Shadowrocket设置使用教程 | 9 | 25 |
> | [#46](https://github.com/Yu9191/wloc/issues/46) | Shadowrocket完整教程请看这里-小白保姆级教程 | 0 | 0 |
> | [#45](https://github.com/Yu9191/wloc/issues/45) | ios27 stash也失败了 | 0 | 0 |
> | [#44](https://github.com/Yu9191/wloc/issues/44) | ios27 stash也失败了 | 0 | 0 |
