...
star_history: "2026-06-27:457,2026-06-27:462,2026-06-28:644,2026-06-28:647,2026-06-29:1092,2026-06-29:1098,2026-06-30:1508,2026-06-30:1515,2026-07-01:1965,2026-07-01:1968,2026-07-02:2357,2026-07-03:2584,2026-07-04:2845,2026-07-05:2957,2026-07-06:3094,2026-07-07:3167,2026-07-08:3299"
tags:
  - github
  - "category/其他"
  - "lang/javascript"
...
**3.3k** stars · **254** stars/天 · 建立 13 天前 · JavaScript · 未標註授權
...
> [!info] 速覽
> **安裝難度** Easy · **專案狀態** Recent · **熱度** Hot (254 stars/day)
> **授權** 未標註授權 (風險較高) · **維護** Moderate (最後推送 10 天前) · **貢獻者** Solo (bus factor 風險) · **參與度** Medium
> **適合** 需要在 iOS 設備上進行虛擬定位的開發者和測試人員。
> **一句話重點** WLOC 的設計讓虛擬定位變得前所未有的簡單，特別適合需要頻繁切換地理位置的用戶。
...
> **結論** 花 2 小時學習，1 小時整合，得到便捷的虛擬定位功能，值得嘗試。
...
WLOC 是一個用於修改 Apple 網絡定位服務（gs-loc.apple.com）返回坐標的工具，讓用戶能夠在 iOS 設備上進行虛擬定位。用戶只需在地圖應用中選擇位置，然後通過快捷指令或代理工具設定即可生效。其核心機制是通過代理攔截定位請求，並返回用戶所選的虛擬坐標，這樣用戶無需手動填寫經緯度。主要的 CLI 指令包括 `wloc 设置地理位置` 和 `wloc 清理恢复位置`，這些指令能夠快速切換或恢復真實定位。此工具支持 Surge、Quantumult X、Loon 和 Stash 等多種代理工具，並提供相應的配置文件，方便用戶直接使用。技術上，它使用了 JavaScript 進行網絡請求的攔截和坐標的解析，並且設計上考慮了隱私問題，所有的解析過程不會存儲任何數據。

與其他類似工具相比，如 Shadowrocket，WLOC 提供了更簡單的快捷指令操作，並且能夠自動處理 GCJ-02 到 WGS84 的坐標轉換。使用者在 iOS 26 及以上版本時，可能會遇到定位緩存的問題，這時需要重啟設備以清除緩存。這個工具的設計非常適合需要頻繁切換定位的用戶，特別是開發者和測試人員。未來可能會進一步增強對不同地圖服務的支持，並優化用戶體驗。對於小型團隊或個人開發者來說，這是一個值得嘗試的工具，尤其是在需要虛擬定位的場景下。

**技術棧**：`JavaScript`

## 重點功能

- 支持多種代理工具 — 包括 Surge、Quantumult X、Loon 和 Stash，並提供相應的配置文件。
- 快捷指令操作 — 用戶可以通過快捷指令快速設置和恢復位置，無需手動填寫經緯度。
- 自動坐標轉換 — 自動處理 GCJ-02 到 WGS84 的坐標轉換，確保定位準確性。
- 隱私保護 — 所有解析過程不會存儲任何數據，確保用戶隱私。
- 開源部署 — 用戶可選擇自行部署 worker，進一步控制數據處理過程。
...
> [!tip] 爆紅原因
> 建立 13 天就累積 3299 stars（254/天），forks 455（13.8%），顯示出強烈的社群興趣。作者 Yu9191 之前在 GitHub 上有其他開源項目，這使得他在開發者社群中有一定的影響力。這個工具解決了在 iOS 設備上進行虛擬定位的痛點，之前的方案往往需要繁瑣的手動設置，並且不夠靈活。近期的 iOS 更新可能使得許多用戶需要這樣的工具來繞過定位限制，這也促進了其使用率的上升。forks/stars 比率為 13.8%，顯示出許多用戶在實際修改和使用這個工具。

## 適合誰使用

**目標受眾**：需要在 iOS 設備上進行虛擬定位的開發者和測試人員。

> [!example] 使用場景
> - 開發者用它來在測試環境中模擬不同地理位置，因為手動設置經緯度繁瑣且容易出錯。
> - 測試人員用它來驗證應用在不同地理位置的行為，因為這樣可以快速切換位置而不需要重啟設備。
> - 普通用戶用它來隱藏真實位置，因為這樣可以在使用某些服務時避免地理限制。

## 架構分析

WLOC 的架構基於 JavaScript，主要通過代理模塊攔截定位請求並返回虛擬坐標。其設計選擇了輕量級的 worker 模式，這樣可以避免對用戶設備造成過多負擔。資料流方面，當用戶在選點頁面選擇位置時，系統會發送請求到 gs-loc.apple.com，然後通過代理模塊攔截並修改返回的 protobuf 響應。

這樣的設計使得用戶可以無縫地切換定位，而不需要手動干預。選擇 JavaScript 作為開發語言，因為其在網絡請求處理上具有良好的性能和靈活性。這種架構的擴展性較好，但在高流量情況下可能會遇到性能瓶頸，特別是當多個用戶同時請求時。

## 技術深入分析

WLOC 的核心技術機制是通過 JavaScript 實現的網絡請求攔截，並使用 protobuf 解析來修改定位數據。這種設計使得用戶可以在不干預系統的情況下，實現虛擬定位。效能方面，該工具能夠快速響應用戶請求，並在高流量情況下仍能保持穩定。設計上選擇了輕量級的 worker 模式，這樣可以減少對用戶設備的資源消耗。

選擇 JavaScript 作為開發語言，因為其在網絡請求處理上具有良好的性能和靈活性。技術風險方面，若用戶同時有大量請求，可能會導致性能瓶頸。整合方面，WLOC 可以與主流的代理工具無縫搭配，並且在 CI/CD pipeline 中的使用也相對簡單。整體來看，WLOC 是一個高效且易於使用的虛擬定位工具，特別適合需要頻繁切換定位的開發者和測試人員。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且提供了詳細的使用範例，讓新手能夠快速上手。安裝過程相對順暢，但需要用戶具備一定的技術背景。文件目前僅提供英文版本，對於非英語用戶可能會有一定的學習曲線。

## 優缺點分析

> [!success] 優點
> - 使用簡單，通過快捷指令即可快速設置和恢復位置。
> - 支持多種代理工具，方便用戶選擇適合的方案。
> - 隱私保護良好，不會存儲用戶的位置信息。

> [!danger] 缺點
> - 僅限於 iOS 設備，對 Android 用戶不適用。
> - 在某些 iOS 版本中可能需要重啟設備以清除緩存。
> - 需要用戶具備一定的技術背景來配置代理工具。

> [!warning] 注意事項
> - 僅支援 iOS 設備，無法在 Android 上使用。
> - 在 iOS 26 及以上版本中，可能需要重啟設備以清除定位緩存。
> - 使用過程中需確保代理工具正常運行，否則無法生效。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [BigPizzaV3/Shadowbroker](https://github.com/BigPizzaV3/Shadowbroker) | Shadowbroker 提供了類似的虛擬定位功能，但主要針對 Android 設備，對於 iOS 用戶來說不適用。 |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | Agent Sprite Forge 提供了更為複雜的定位模擬功能，但設置過程較為繁瑣，WLOC 則提供了更簡單的快捷指令操作。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [BigPizzaV3/Shadowbroker](https://github.com/BigPizzaV3/Shadowbroker) | Shadowbroker 提供了類似的虛擬定位功能，但主要針對 Android 設備，對於 iOS 用戶來說不適用。 | 如果你的團隊主要在 Android 平台上開發，且需要虛擬定位功能。 | medium，因為需要重新配置 Android 環境。 |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | Agent Sprite Forge 提供了更為複雜的定位模擬功能，但設置過程較為繁瑣，WLOC 則提供了更簡單的快捷指令操作。 | 如果你需要更高級的定位模擬功能，並且不介意繁瑣的設置過程。 | high，因為需要重新學習和配置整個工具。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **wloc** | **Shadowbroker** | **agent-sprite-forge** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | Shadowbroker 提供了類似的虛擬定位功能，但主要針對 Android 設備，對於 iOS 用戶來說不適用。 | Agent Sprite Forge 提供了更為複雜的定位模擬功能，但設置過程較為繁瑣，WLOC 則提供了更簡單的快捷指令操作。 |
> | 遷移成本 | - | medium，因為需要重新配置 Android 環境。 | high，因為需要重新學習和配置整個工具。 |
> | 適用場景 | 主要場景 | 如果你的團隊主要在 Android 平台上開發，且需要虛擬定 | 如果你需要更高級的定位模擬功能，並且不介意繁瑣的設置過程。 |
...
- **[HIGH]** 在 iOS 26 中，定位緩存可能導致修改後的坐標無法立即生效。
  - 解法：需要重啟設備以清除緩存。
- [MEDIUM] 某些用戶在使用 Shadowrocket 時，需重啟才能生效。
  - 解法：確保在設置後重啟設備。
- [MEDIUM] 在使用高德地圖時，短鏈解析可能出現問題。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 需要頻繁切換定位的開發者 | 非常適合 | 提供了快捷指令操作，能夠快速切換地理位置。 |
| 普通用戶希望隱藏真實位置 | 適合 | 能夠簡單地設置虛擬定位，避免地理限制。 |
| Android 用戶需要虛擬定位 | 不適合 | 僅支持 iOS 設備，無法在 Android 上使用。 |
| 大型團隊需要穩定的虛擬定位解決方案 | 普通 | 目前仍在 beta 階段，可能存在不穩定性。 |
...
> [!tip] 投入 vs 回報
> 花 2 小時學習，1 小時整合，得到便捷的虛擬定位功能，值得嘗試。
...
> | --- | --- | --- | --- |
> | [#39](https://github.com/Yu9191/wloc/issues/39) | Shadowrocket+快捷指令（秒成功）操作流程图文记录 | 10 | 15 |
> | [#35](https://github.com/Yu9191/wloc/issues/35) | Shadowrocket设置使用教程 | 9 | 25 |
> | [#42](https://github.com/Yu9191/wloc/issues/42) | iOS 27 beta不行了？ | 0 | 2 |
> | [#41](https://github.com/Yu9191/wloc/issues/41) | iPhone 8，iOS 16.7.15，用的shadowrocket，每次设置完之后，必须重启才能生效，请问这是怎么回 | 0 | 0 |
> | [#40](https://github.com/Yu9191/wloc/issues/40) | ios26用loon存储位置文件失败，报错404 | 0 | 10 |
