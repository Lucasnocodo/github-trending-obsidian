...
star_history: "2026-06-27:457,2026-06-27:462,2026-06-28:644,2026-06-28:647,2026-06-29:1092,2026-06-29:1098,2026-06-30:1508,2026-06-30:1515,2026-07-01:1965,2026-07-01:1968,2026-07-02:2357"
tags:
  - github
  - "category/其他"
  - "lang/javascript"
...
**2.4k** stars · **337** stars/天 · 建立 7 天前 · JavaScript · 未標註授權
...
> **適合** 需要在 iOS 設備上靈活修改地理位置的用戶，特別是開發者和隱私保護意識強的普通用戶。
> **一句話重點** WLOC 讓虛擬定位變得前所未有的簡單，特別是對於需要頻繁更改位置的用戶。
...
> **結論** 花 3 小時學習，2 小時整合，得到靈活的虛擬定位功能，值得一試。

> [!abstract] 核心創新
> 提供了一個便捷的方式來修改 Apple 網絡定位，支持多種代理工具和快捷指令。

## 專案簡介

WLOC 是一個用於修改 Apple 網絡定位的工具，通過代理模塊來改變設備的地理位置。用戶可以在地圖應用中選擇位置，然後通過快捷指令將其設置為當前位置，這樣就能實現虛擬定位。這個工具支持 Surge、Quantumult X、Loon 和 Stash 等多種代理工具，並且提供了簡單的快捷指令來快速設置和清除位置。其核心機制是通過攔截 WLOC 請求，修改返回的坐標，並且支持 GCJ-02 到 WGS84 的坐標轉換，這在中國大陸的定位中尤為重要。

WLOC 的設計考慮到了用戶隱私，所有的解析請求都不會被存儲或記錄，這使得用戶可以放心使用。這個工具的賣點在於其便捷性和高效性，能夠在不需要手動輸入經緯度的情況下，快速實現虛擬定位。使用者只需簡單的幾個步驟就能完成設置，並且可以通過收藏位置功能來方便地切換不同的虛擬位置。與其他類似工具相比，WLOC 提供了更為靈活的使用方式，並且在使用上更為直觀，特別適合需要頻繁更改位置的用戶。
...
- 快捷指令一鍵設置 — 用戶可通過快捷指令快速設置和恢復地理位置，提升使用效率。
- 在線選點功能 — 用戶可以在選點頁面選擇位置，無需手動輸入經緯度。
- GCJ-02 到 WGS84 轉換 — 自動處理中國大陸的坐標偏差，確保定位準確。
- 收藏位置功能 — 支持用戶收藏多個位置，方便快速切換。
...
2. 選擇位置
```bash
在地圖應用中選擇位置並共享到 'wloc 設置地理位置'
```
3. 清理位置
```bash
使用 'wloc 清理恢復位置' 快捷指令恢復真實定位
```

## 為什麼值得關注

> [!tip] 爆紅原因
> 建立 7 天內累積 2357 stars（337/天），forks 316（13.4%），顯示出強勁的增長勢頭。作者 Yu9191 之前在定位修改方面有過相關的開發經驗，這個工具解決了用戶在使用 Apple 地圖時無法方便修改位置的痛點。社群中對於如何使用 Shadowrocket 的問題也引發了較多的討論，顯示出用戶對於這個工具的需求和興趣。技術上，這個工具的設計使得用戶能夠在不影響 GPS 定位的情況下，靈活地修改網絡定位，這在當前的技術環境中是非常有價值的。forks/stars 比率 13.4% 表示有相當一部分用戶在實際修改和使用這個工具。

## 適合誰使用

**目標受眾**：需要在 iOS 設備上靈活修改地理位置的用戶，特別是開發者和隱私保護意識強的普通用戶。

> [!example] 使用場景
> - iOS 開發者用它來測試地理位置功能，因為可以快速切換位置而不需要實際移動設備。
> - 普通用戶用它來隱藏真實位置，因為可以在家中使用虛擬定位來訪問地區限制的內容。
> - 旅行者用它來模擬在其他國家的位置，因為可以在不出門的情況下體驗當地的服務和應用。

## 架構分析

WLOC 的架構基於 JavaScript，通過代理模塊攔截和修改 WLOC 請求。其設計選擇了 MITM 代理方式，這樣可以在不影響 GPS 定位的情況下，靈活地修改網絡定位。資料流上，使用者選擇位置後，通過快捷指令發送請求，代理模塊攔截請求並修改返回的坐標。這樣的設計使得用戶可以快速設置虛擬位置，但也增加了對代理設置的依賴。擴展性方面，這種架構可以方便地添加對其他代理工具的支持，但在使用上對新手來說可能存在一定的學習曲線。

## 技術深入分析

WLOC 的核心技術機制是通過 JavaScript 實現的 MITM 代理，攔截 Apple 的 WLOC 請求並修改返回的坐標。這樣的設計使得用戶可以在不需要手動輸入經緯度的情況下，快速實現虛擬定位。效能上，這個工具能夠在大多數情況下即時修改定位，但在 iOS 26 以上版本中，由於系統的定位緩存機制，可能需要重啟設備才能生效。設計上，選擇 JavaScript 作為開發語言，這使得工具的擴展性和可維護性較高，但也要求用戶具備一定的技術背景。技術風險方面，對於不熟悉 MITM 的用戶來說，初始設置可能會造成困難，並且在使用過程中可能會遇到與其他代理工具的兼容性問題。整體來看，WLOC 在現有的代理工具生態中提供了一個靈活且高效的解決方案，特別適合需要頻繁修改位置的用戶。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了詳細的使用說明和範例。安裝過程需要設置 MITM 代理，對於新手來說可能有一定的難度，但整體流程還算順暢。文件目前僅提供英文版本，可能對非英語用戶造成障礙。
...
> [!danger] 缺點
> - 需要一定的技術背景來設置代理。
> - 在 iOS 26 以上版本中可能需要重啟設備才能生效。
> - 對於不熟悉 MITM 的用戶來說，初始設置可能較為複雜。

> [!warning] 注意事項
> - 僅支持 iOS 設備，無法在 Android 上使用。
> - 需要 MITM 代理設置，對於新手可能有一定的學習曲線。
> - 在 iOS 26 及以上版本中，定位緩存問題可能導致修改後需要重啟設備才能生效。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 這個工具專注於代理和虛擬定位，但不支持快捷指令的便捷設置，使用上相對繁瑣。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供類似的虛擬定位功能，但缺乏 WLOC 的多代理支持和在線選點功能。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 這個工具使用 Rust 實作，記憶體用量少，但功能相對單一，缺乏 WLOC 的多代理支持。 | 如果你的團隊已經在使用 Rust 並希望減少記憶體佔用，這個工具會是一個不錯的選擇。 | medium，因為需要重新學習 Rust 的相關知識。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供類似的虛擬定位功能，但不支持快捷指令的便捷設置，使用上相對繁瑣。 | 如果你需要一個簡單的虛擬定位工具，並且不需要快捷指令的支持，可以考慮使用這個工具。 | low，因為功能相似，學習成本不高。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **wloc** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 這個工具使用 Rust 實作，記憶體用量少，但功能相對單一，缺乏 WLOC 的多代理支持。 | 提供類似的虛擬定位功能，但不支持快捷指令的便捷設置，使用上相對繁瑣。 |
> | 遷移成本 | - | medium，因為需要重新學習 Rust 的相關知識。 | low，因為功能相似，學習成本不高。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用 Rust 並希望減少記憶體佔用，這個 | 如果你需要一個簡單的虛擬定位工具，並且不需要快捷指令的支持， |
...
- **[HIGH]** 在 iOS 26 及以上版本中，定位緩存問題可能導致修改後需要重啟設備才能生效
  - 解法：重啟設備以清除定位緩存
- [MEDIUM] 使用快捷指令時，可能會遇到權限問題，導致無法設置位置
  - 解法：確保已授予快捷指令訪問位置信息的權限
- [MEDIUM] 在某些情況下，選點頁面可能無法正常加載
  - 解法：檢查代理設置是否正確，並確保網絡連接正常

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 需要在開發環境中測試地理位置功能的開發者 | 非常適合 | 提供了便捷的虛擬定位設置，能快速測試不同位置的功能。 |
| 普通用戶希望隱藏真實位置以訪問地區限制內容 | 適合 | 能夠方便地修改位置，並且不影響 GPS 定位。 |
| 需要在公共 WiFi 環境中使用定位的用戶 | 普通 | 在 WiFi 定位為主的場景下效果最佳，但在 GPS 信號強的情況下可能會被忽略。 |
| 對於不熟悉技術的普通用戶 | 不適合 | 需要一定的技術背景來設置代理，對新手來說可能有困難。 |
...
> [!tip] 投入 vs 回報
> 花 3 小時學習，2 小時整合，得到靈活的虛擬定位功能，值得一試。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限，並且不存取敏感資料，使用過程中不會記錄用戶數據，適合在 CI/CD 中使用。
...
> | --- | --- | --- | --- |
> | [#35](https://github.com/Yu9191/wloc/issues/35) | Shadowrocket设置使用教程 | 6 | 18 |
> | [#37](https://github.com/Yu9191/wloc/issues/37) | 用小火箭没弄成功 | 0 | 0 |
> | [#36](https://github.com/Yu9191/wloc/issues/36) | 偏差距離問題 | 0 | 3 |
> | [#34](https://github.com/Yu9191/wloc/issues/34) | 按照我的方法百分之100成功（系统26.3） | 0 | 37 |
> | [#31](https://github.com/Yu9191/wloc/issues/31) | 为什么打开快捷指令弹PESPOND | 0 | 1 |
