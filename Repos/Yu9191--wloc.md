...
use_case: "修改 Apple 网络定位返回坐标，支持多种代理工具，方便用户进行虚拟定位。"
priority: high
ring: assess
...
aliases:
  - "wloc"
  - "Yu9191/wloc"
  - "修改 Apple 网络定位返回坐标，支持多种代理工具，方便用户进行虚拟定位。"
---

# wloc

**5.4k** stars · **235** stars/天 · 建立 23 天前 · JavaScript · 未標註授權
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Recent · **熱度** Hot (235 stars/day)
> **授權** 未標註授權 (風險較高) · **維護** Moderate (最後推送 20 天前) · **貢獻者** Solo (bus factor 風險) · **參與度** Medium
> **適合** 需要在 iOS 設備上進行虛擬定位的用戶，特別是隱私意識強烈的學生和開發者。
> **一句話重點** WLOC 讓 iOS 用戶能夠輕鬆實現虛擬定位，特別適合需要隱私保護的場景。
...
> **結論** 花 3 小時學習，2 小時整合，得到靈活的虛擬定位功能，值得採用。

> [!abstract] 核心創新
> 透過 MITM 技術修改 Apple 網絡定位服務返回的坐標，實現虛擬定位。

## 專案簡介

WLOC 主要功能是修改 Apple 网络定位服务返回的坐标，讓使用者能夠在 iOS 設備上進行虛擬定位。使用者可以透過在線選點頁面選擇位置，然後透過快捷指令將該位置設置為當前定位。這個工具的賣點在於其簡單易用的快捷指令，使用者只需在地圖 App 中選擇位置並分享，即可快速切換定位。該工具支持 Surge、Quantumult X、Loon 和 Stash 等多種代理工具，並且提供了相應的模組配置。技術上，WLOC 使用 JavaScript 實現，並透過 MITM 技術攔截和修改定位請求，這樣可以避免 GPS 硬體定位的限制，尤其在 WiFi 定位為主的室內場景中效果最佳。

與其他虛擬定位工具相比，WLOC 的優勢在於其對 Apple 地圖和高德地圖的支持，以及對 GCJ-02 和 WGS84 坐標系的自動轉換。使用者在使用過程中可能會遇到 iOS 26 及以上版本的定位緩存問題，這需要重啟設備來清除緩存。這個專案目前處於穩定狀態，值得在個人項目中使用，但在生產環境中需謹慎考慮其合法性和安全性。對於需要頻繁切換位置的用戶，WLOC 提供了便捷的收藏位置功能，能夠快速管理多個虛擬定位。整體而言，WLOC 是一個功能強大且易於使用的虛擬定位工具，特別適合需要在 iOS 設備上進行位置模擬的用戶。
...
- 快捷指令一鍵設置 — 使用者可透過快捷指令快速切換和清除定位。
- 在線選點頁面 — 提供用戶方便的選擇界面，支持 Apple 地圖和高德地圖。
- GCJ-02 轉 WGS84 自動轉換 — 確保定位的準確性，特別是在中國大陸使用時。
- 收藏位置功能 — 支持用戶管理多個虛擬位置，方便快速切換。
...
3. 選擇位置並儲存
```bash
在地圖選擇位置後，分享並選擇 'wloc 設置地理位置'
```

## 為什麼值得關注

> [!tip] 爆紅原因
> 建立 23 天就累積 5412 stars（235/天），forks 1030（19.0%），顯示出強烈的社群興趣。作者 Yu9191 在 GitHub 上有其他開源專案，這個專案解決了 Apple 定位服務的局限性，讓用戶能夠自由選擇虛擬位置。這個工具的出現正好滿足了用戶對於隱私和位置控制的需求，特別是在需要使用地理位置的應用中。社群中有許多使用者分享了操作流程和使用心得，進一步促進了這個專案的流行。這些因素共同推動了 WLOC 的快速增長。

## 適合誰使用

**目標受眾**：需要在 iOS 設備上進行虛擬定位的用戶，特別是隱私意識強烈的學生和開發者。

> [!example] 使用場景
> - 學生用它來在校園內隨意設置虛擬位置，因為這樣可以避免地理位置限制，方便使用各種基於位置的應用。
> - 開發者用它來測試地理位置相關的功能，因為能夠快速切換位置而不需要實際移動，節省了大量時間。
> - 隱私意識強的用戶用它來隱藏真實位置，因為這樣可以保護個人隱私，避免被追蹤。

## 架構分析

WLOC 的架構基於 MITM 代理技術，通過攔截和修改網絡請求來實現虛擬定位。具體來說，當用戶在選點頁面選擇位置後，系統會發送請求到 gs-loc.apple.com，WLOC 代理會攔截該請求，並根據用戶的設置返回修改後的坐標。這樣的設計使得用戶無需手動填寫經緯度，簡化了操作流程。選擇 MITM 技術的代價是需要用戶信任代理工具，並且在某些情況下可能會影響網絡性能。擴展性方面，WLOC 可以支持多種代理工具，但在不同工具間的兼容性可能會帶來整合上的挑戰。

## 技術深入分析

WLOC 的核心技術機制是基於 MITM 代理技術，通過攔截 Apple 網絡定位服務的請求來修改返回的坐標。這樣的設計使得用戶能夠在不需要手動輸入經緯度的情況下，輕鬆設置虛擬位置。效能方面，WLOC 主要依賴用戶的網絡環境，理論上可以處理大量的請求，但在高流量時可能會遇到延遲問題。選擇 JavaScript 作為開發語言的好處在於其跨平台特性，但也可能導致在某些環境下的性能瓶頸。技術風險方面，WLOC 依賴於 Apple 的定位服務，若未來 Apple 改變其 API 或安全策略，可能會影響 WLOC 的功能。整合方面，WLOC 與主流的代理工具（如 Surge 和 Quantumult X）有良好的兼容性，但在不同版本間可能會出現不一致的行為。整體而言，WLOC 是一個功能強大且易於使用的虛擬定位工具，適合需要在 iOS 設備上進行位置模擬的用戶。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且提供了詳細的使用說明和範例，安裝過程相對順暢，但需要用戶具備一定的 MITM 知識。文件目前僅提供英文，可能對非英語使用者造成一定困難。整體而言，花 30 分鐘應該能夠成功運行。 
...
> [!danger] 缺點
> - 需要 MITM 代理工具，對於新手用戶有一定的學習曲線。
> - 在 iOS 26 及以上版本中可能會遇到定位緩存問題。
> - 僅限於 iOS 平台，無法在其他系統上使用。

> [!warning] 注意事項
> - 僅支持 iOS 設備，其他平台無法使用。
> - 需要 MITM 代理工具，對於不熟悉的用戶可能有一定的學習曲線。
> - 在 iOS 26 及以上版本中，定位緩存可能導致設置後無法立即生效。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供類似的虛擬定位功能，但主要針對 Android 平台，無法直接應用於 iOS。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 雖然也提供位置修改功能，但不支持快捷指令，使用上不如 WLOC 方便。 |
| [0xSero/codex-shim](https://github.com/0xSero/codex-shim) | 專注於 API 代理，無法提供虛擬定位的功能，適用場景不同。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於 Android 平台的虛擬定位，無法直接應用於 iOS。 | 如果你的團隊主要開發 Android 應用，且需要虛擬定位功能。 | medium，因為需要重新適配 Android 環境。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供位置修改功能，但不支持快捷指令，使用上不如 WLOC 方便。 | 如果你的需求不需要快捷指令，且希望使用更簡單的工具。 | low，因為功能相似，轉換成本低。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **wloc** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於 Android 平台的虛擬定位，無法直接應用於 iOS。 | 提供位置修改功能，但不支持快捷指令，使用上不如 WLOC 方便。 |
> | 遷移成本 | - | medium，因為需要重新適配 Android 環境。 | low，因為功能相似，轉換成本低。 |
> | 適用場景 | 主要場景 | 如果你的團隊主要開發 Android 應用，且需要虛擬定位功 | 如果你的需求不需要快捷指令，且希望使用更簡單的工具。 |
...
> [!tip] 採用建議
> 適合個人項目或小型團隊使用，但在生產環境中需謹慎考慮其合法性。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在 iOS 26 及以上版本中，定位緩存可能導致設置後無法立即生效。
  - 解法：需要重啟設備來清除定位緩存。
- [MEDIUM] 如果用戶手動修改了經緯度，清除持久化數據後仍會使用手動設置的坐標。
  - 解法：保持默認參數不變，清除持久化數據即可進入透傳模式。
- [MEDIUM] 在某些網絡環境下，可能會遇到延遲問題。
  - 解法：確保網絡穩定，必要時更換代理服務器。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 需要在校園內隨意設置虛擬位置的學生 | 非常適合 | 簡單易用的快捷指令讓學生能快速切換位置。 |
| 開發者需要測試地理位置相關的功能 | 非常適合 | 能夠快速切換位置而不需要實際移動，節省了大量時間。 |
| 隱私意識強的用戶需要隱藏真實位置 | 適合 | 能夠有效保護個人隱私，避免被追蹤。 |
| 需要在 Android 設備上進行虛擬定位的用戶 | 不適合 | 本專案僅支持 iOS 平台，無法在 Android 上使用。 |
| 不熟悉 MITM 的新手用戶 | 普通 | 需要一定的學習曲線，對於新手用戶可能會有挑戰。 |
...
> [!tip] 投入 vs 回報
> 花 3 小時學習，2 小時整合，得到靈活的虛擬定位功能，值得採用。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：不需要高權限，僅修改網絡定位，不存取敏感資料。依賴於用戶的 MITM 代理工具的安全性，需確保使用可信的代理。
...
**社群活躍度**：社群活躍，有多個熱門 Issues 和使用者分享的教程。
...
> | [#54](https://github.com/Yu9191/wloc/issues/54) | 借个楼，正在开发iOS端和Mac端，网页端可在线体验 | 0 | 0 |
> | [#53](https://github.com/Yu9191/wloc/issues/53) | 怎么给Mac虚拟定位 | 0 | 2 |
...
相關概念：[[虛擬定位]] · [[MITM]] · [[GCJ-02]] · [[WGS84]] · [[快捷指令]]
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["虛擬定位","MITM","GCJ-02","WGS84","快捷指令"];
