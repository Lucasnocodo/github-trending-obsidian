...
star_history: "2026-06-27:457,2026-06-27:462,2026-06-28:644,2026-06-28:647,2026-06-29:1092,2026-06-29:1098,2026-06-30:1508,2026-06-30:1515,2026-07-01:1965,2026-07-01:1968,2026-07-02:2357,2026-07-03:2584,2026-07-04:2845,2026-07-05:2957,2026-07-06:3094,2026-07-07:3167,2026-07-08:3299,2026-07-09:3489,2026-07-10:3676,2026-07-11:3806,2026-07-12:3932"
tags:
  - github
  - "category/其他"
  - "lang/javascript"
...
**3.9k** stars · **231** stars/天 · 建立 17 天前 · JavaScript · 未標註授權
...
> **適合** 需要在 iOS 設備上虛擬定位的普通用戶和開發者。
> **一句話重點** WLOC 讓虛擬定位變得簡單，特別適合需要頻繁修改位置的用戶。
...
> [!abstract] 核心創新
> 提供簡單的快捷指令集成，讓用戶能夠快速修改 Apple 網絡定位。

## 專案簡介

WLOC 是一個用於修改 Apple 網絡定位服務的工具，能夠虛擬化 iOS 設備的地理位置。用戶可以透過選擇地圖上的位置，然後使用快捷指令來設置或恢復定位，這樣就無需手動輸入經緯度。其核心機制是通過代理模組攔截定位請求，並在返回的數據中修改坐標，支持 Surge、Quantumult X、Loon 和 Stash 等多種工具。這種設計的好處在於用戶不需要進行繁瑣的配置，只需簡單的幾個步驟即可完成定位修改。技術上，WLOC 使用 JavaScript 實現，並依賴於 MITM（中間人攻擊）技術來攔截和修改網絡請求，這使得它能夠在不影響 GPS 硬件定位的情況下，專注於 WiFi 和基站定位。

與其他定位修改工具相比，WLOC 的優勢在於其簡單易用的快捷指令集成，並且支持多種主流代理工具。實際使用中，WLOC 能夠有效處理高德和蘋果地圖的短鏈解析，並且在中國大陸的 GCJ-02 坐標系統中進行轉換。需要注意的是，iOS 26 及以上版本的定位緩存機制可能會影響定位修改的即時性，建議用戶在修改後重啟設備以清除緩存。整體來看，WLOC 是一個成熟且穩定的工具，適合需要頻繁修改定位的用戶，特別是在室內環境中使用 WiFi 定位的場景。

**技術棧**：`JavaScript`

## 重點功能

- 快捷指令支持 — 提供一鍵設置和清理位置的快捷指令，方便用戶快速操作。
- 多種代理工具支持 — 支持 Surge、Quantumult X、Loon 和 Stash，滿足不同用戶需求。
- 自定義位置設置 — 用戶可在選點頁面選擇位置，並自動生成經緯度。
- GCJ-02 到 WGS84 轉換 — 自動處理中國大陸的坐標系轉換，確保定位準確。
- 無需手動填寫經緯度 — 透過簡單的分享操作即可完成定位設置。
...
> [!tip] 爆紅原因
> 建立 17 天內累積 3932 stars（231/天），forks 697（17.7%），顯示出強勁的增長趨勢。作者 Yu9191 在這個領域有一定的經驗，提供了一個簡單有效的解決方案來應對 Apple 定位的限制。之前用戶通常需要手動修改經緯度或使用複雜的工具，這個專案簡化了整個過程。社群的反饋也顯示出對於這個工具的需求，尤其是在熱門 Issues 中，有多個針對 Shadowrocket 的使用教程，顯示出用戶對於這個工具的興趣和需求。

## 適合誰使用

**目標受眾**：需要在 iOS 設備上虛擬定位的普通用戶和開發者。

> [!example] 使用場景
> - iOS 開發者用它來測試應用的地理位置功能，因為可以快速修改定位而不需要實際移動設備。
> - 普通用戶用它來隱藏真實位置，享受地理限制的內容，因為只需幾個步驟即可完成設置。
> - 遊戲玩家用它來在地理位置限制的遊戲中虛擬定位，因為能夠輕鬆切換到不同的地點，提升遊戲體驗。

## 架構分析

WLOC 的架構基於 JavaScript，主要通過 MITM 技術攔截和修改網絡請求。用戶在選點頁面選擇位置後，系統會發送請求到 gs-loc.apple.com，並由代理模組攔截該請求，修改返回的坐標。這種設計使得用戶無需手動輸入經緯度，降低了使用門檻。選擇 JavaScript 作為開發語言的好處在於其跨平台性和易於集成的特性，代價則是對於性能的要求較高。整體架構的擴展性良好，但在高並發情況下可能會出現性能瓶頸，特別是當大量用戶同時請求定位時。

## 技術深入分析

WLOC 的核心技術在於其使用 JavaScript 進行網絡請求的攔截和修改。通過 MITM 技術，WLOC 能夠在用戶的設備和 Apple 的定位服務之間進行數據處理，這樣用戶只需在選點頁面選擇位置，系統便會自動修改返回的經緯度。這種設計不僅提高了用戶的便利性，還能夠在不影響 GPS 硬件定位的情況下，專注於 WiFi 和基站定位。效能方面，WLOC 在處理請求時的延遲相對較低，能夠快速響應用戶的需求，但在高並發的情況下可能會出現性能瓶頸。選擇 JavaScript 作為開發語言的原因在於其跨平台性和易於集成的特性，這使得 WLOC 能夠在多種環境中運行。技術風險方面，WLOC 依賴於網絡請求的攔截和修改，這可能會受到網絡環境的影響，特別是在不穩定的網絡情況下，定位的準確性可能會受到影響。整體而言，WLOC 在現有的定位工具中提供了一個簡單而有效的解決方案，適合需要頻繁修改定位的用戶。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，包含了詳細的使用說明和範例。安裝過程相對順暢，但對於不熟悉代理工具的用戶可能需要一些額外的學習。文件目前僅提供英文，可能對某些用戶造成障礙。

## 優缺點分析

> [!success] 優點
> - 操作簡單，使用快捷指令即可快速設置和清理位置。
> - 支持多種主流代理工具，靈活性高。
> - 自動處理坐標系轉換，適應中國大陸的定位需求。

> [!danger] 缺點
> - 僅支持 WiFi 定位，無法影響 GPS 硬件定位。
> - 在 iOS 26 及以上版本中，定位修改後需要重啟設備才能生效。
> - 對於不熟悉代理工具的用戶，初期設置可能會有一定難度。

> [!warning] 注意事項
> - 僅支持 WiFi 定位，無法修改 GPS 硬件定位。
> - iOS 26 及以上版本需要重啟設備以清除定位緩存。
> - 使用過程中需確保代理工具已啟用，否則無法生效。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供類似的虛擬定位功能，但主要針對遊戲玩家，使用場景較為狹窄。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 雖然也提供定位修改功能，但不支持快捷指令，使用上較為繁瑣。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於遊戲玩家的虛擬定位工具，使用 C++ 實作，性能優於 WLOC，但使用場景較為狹窄。 | 如果你的主要需求是遊戲虛擬定位，且需要更高的性能，則應選擇此工具。 | medium，因為需要重新學習新的操作流程和配置。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供定位修改功能，但不支持快捷指令，使用上較為繁瑣，主要用於開發者。 | 如果你需要更高的自定義能力，並且不介意較為複雜的設置過程，可以考慮此工具。 | high，因為需要重新配置和學習新的操作方式。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **wloc** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於遊戲玩家的虛擬定位工具，使用 C++ 實作，性能優於 WLOC，但使用場景較為狹窄。 | 提供定位修改功能，但不支持快捷指令，使用上較為繁瑣，主要用於開發者。 |
> | 遷移成本 | - | medium，因為需要重新學習新的操作流程和配置。 | high，因為需要重新配置和學習新的操作方式。 |
> | 適用場景 | 主要場景 | 如果你的主要需求是遊戲虛擬定位，且需要更高的性能，則應選擇此 | 如果你需要更高的自定義能力，並且不介意較為複雜的設置過程，可 |
...
> [!tip] 採用建議
> 適合個人使用和小型專案，但不建議在生產環境的核心路徑上使用。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在 iOS 26 及以上版本中，定位修改後需要重啟設備才能生效。
  - 解法：重啟設備以清除定位緩存。
- [MEDIUM] 如果用戶在模組參數中手動修改了經緯度，清除持久化數據後仍會使用模組參數中的坐標。
  - 解法：保持默認參數不變，清除持久化數據後進入透傳模式。
- [MEDIUM] 使用過程中需確保代理工具已啟用，否則無法生效。
  - 解法：確認代理工具已啟用並運行。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 需要在 iOS 設備上虛擬定位的普通用戶 | 非常適合 | 操作簡單，快捷指令支持使得使用體驗良好。 |
| 開發者需要測試應用的地理位置功能 | 適合 | 能快速修改定位，方便進行測試。 |
| 遊戲玩家需要虛擬定位來參加地理限制的遊戲 | 非常適合 | 支持快速切換位置，提升遊戲體驗。 |
| 企業需要在生產環境中使用虛擬定位 | 不適合 | 目前仍在 beta 階段，穩定性不足。 |
...
> [!warning] 安全性快速掃描
> 低風險：不需要高權限，僅修改網絡定位，不存取敏感資料。依賴鏈的信任程度高，無已知的供應鏈風險。
...
**社群活躍度**：社群活躍，熱門 Issues 顯示出用戶對於使用的需求和反饋。
...
> | [#46](https://github.com/Yu9191/wloc/issues/46) | Shadowrocket完整教程请看这里-小白保姆级教程 | 3 | 3 |
> | [#47](https://github.com/Yu9191/wloc/issues/47) | karing这款APP好像没教程。是不是不支持啊。 | 0 | 0 |
