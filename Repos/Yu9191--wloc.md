...
use_case: "修改 Apple 网络定位返回的坐标，支持多种代理工具的快速设置与恢复。"
priority: high
ring: assess
...
star_history: "2026-06-27:457,2026-06-27:462,2026-06-28:644,2026-06-28:647,2026-06-29:1092,2026-06-29:1098,2026-06-30:1508,2026-06-30:1515,2026-07-01:1965,2026-07-01:1968,2026-07-02:2357,2026-07-03:2584,2026-07-04:2845,2026-07-05:2957,2026-07-06:3094"
tags:
  - github
  - "category/其他"
  - "lang/javascript"
aliases:
  - "wloc"
  - "Yu9191/wloc"
  - "修改 Apple 网络定位返回的坐标，支持多种代理工具的快速设置与恢复。"
---

# wloc

**3.1k** stars · **281** stars/天 · 建立 11 天前 · JavaScript · 未標註授權
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Recent · **熱度** Hot (281 stars/day)
> **授權** 未標註授權 (風險較高) · **維護** Moderate (最後推送 8 天前) · **貢獻者** Solo (bus factor 風險) · **參與度** Medium
> **適合** 需要靈活調整 Apple 設備定位的用戶，特別是旅行者和開發者。
> **一句話重點** WLOC 讓虛擬定位變得簡單易用，尤其適合需要頻繁調整位置的用戶。
...
> [!abstract] 核心創新
> 提供一個無需手動填寫經緯度的虛擬定位解決方案，並支持多種代理工具。

## 專案簡介

WLOC 是一個用於修改 Apple 網絡定位服務返回坐標的工具，旨在實現 iOS 設備的虛擬定位。用戶可以通過選擇地圖上的位置並使用快捷指令來設置或清除地理位置，這樣可以避免手動輸入經緯度的繁瑣。該工具支持 Surge、Quantumult X、Loon 和 Stash 等多種代理工具，並提供了相應的模塊配置。其核心機制是通過代理攔截 Apple 的定位請求，並在後端進行坐標的轉換和返回，這樣用戶可以在不改變 GPS 硬件定位的情況下，實現網絡定位的變更。使用者只需在選點頁面選擇位置，然後透過快捷指令輕鬆設置，無需進行複雜的配置。

這種設計使得 WLOC 在使用上非常便捷，特別適合需要頻繁更改位置的用戶。該工具的設計考量了隱私問題，所有請求均不進行存儲或記錄，確保用戶數據的安全性。使用者可選擇自部署 Worker 以避免公共請求限制，這對於高頻使用者來說尤為重要。WLOC 的一大賣點是其簡單易用的快捷指令，這使得即使是技術小白也能輕鬆上手。與其他定位修改工具相比，WLOC 提供了更為直觀的用戶體驗和更高的靈活性。

**技術棧**：`JavaScript`

## 重點功能

- 快捷指令支持 — 用戶可通過快捷指令快速設置或清除虛擬位置，無需手動輸入經緯度。
- 多種代理工具支持 — 提供 Surge、Quantumult X、Loon 和 Stash 的模塊配置，方便用戶選擇。
- 自部署 Worker — 用戶可選擇自部署 Worker 以避免公共請求限制，提升使用靈活性。
- 隱私保護 — 所有請求不進行存儲或記錄，確保用戶數據安全。
- 支持多地圖服務 — 能夠處理 Apple 地圖和高德地圖的定位請求，並進行坐標轉換。
...
> [!tip] 爆紅原因
> 建立 11 天內累積 3094 stars（281/天），forks 421（13.6%），顯示出強勁的增長潛力。這位開發者 Yu9191 之前在網絡定位方面有過相關的開發經驗，解決了用戶在使用 Apple 定位時無法靈活調整位置的痛點。這個工具的出現使得用戶能夠更方便地進行虛擬定位，特別是在需要頻繁變更位置的場景中。社群對於這個工具的需求逐漸增加，並且有不少使用者在 GitHub 上分享了他們的使用經驗和教程，進一步促進了工具的擴散。

## 適合誰使用

**目標受眾**：需要靈活調整 Apple 設備定位的用戶，特別是旅行者和開發者。

> [!example] 使用場景
> - 旅行者用它來在地圖上設置虛擬位置，因為這樣可以避免在不同地點之間手動調整 GPS 定位。
> - 開發者用它來測試地理位置相關的應用功能，因為能夠快速切換位置而不需要實際移動。
> - 隱私意識強的用戶用它來隱藏真實位置，因為它不會記錄用戶的任何定位數據，保障了使用者的隱私。

## 架構分析

WLOC 的架構基於代理攔截和請求轉發。用戶在選點頁面選擇位置後，系統會發送請求到 gs-loc.apple.com，代理模塊攔截該請求並進行坐標的修改。這種設計使得用戶無需手動輸入經緯度，直接從地圖選擇位置即可。

選擇使用 JavaScript 作為開發語言，因為它在網絡請求處理上具有良好的性能和靈活性。這種架構的代價是需要用戶信任代理工具，並確保其配置正確。擴展性方面，若用戶需要更高的請求頻率，建議自部署 Worker 以避免公共請求的限制。

## 技術深入分析

WLOC 的核心技術機制是通過代理攔截 Apple 的定位請求，並在後端進行坐標的轉換。其使用了 JavaScript 來處理網絡請求，這使得整個過程高效且靈活。該工具能夠處理的資料量取決於用戶的請求頻率，公共選點頁面有請求上限，因此建議高頻使用者自部署 Worker。設計上選擇了不存儲任何用戶數據的方式，這樣可以有效保護用戶隱私，但同時也意味著無法提供歷史定位數據。這一選擇的代價是用戶需要自行管理其位置設置。技術風險方面，若用戶在配置上出現問題，可能會導致定位失效。整合方面，WLOC 與主流的代理工具相容性良好，並且能夠輕鬆集成到現有的工作流中，降低了使用門檻。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的使用說明和範例，安裝過程相對順暢，但需要用戶具備一定的代理工具使用經驗。整體上，花 30 分鐘能夠順利運行起來。
...
> [!danger] 缺點
> - 對於 GPS 硬件定位無法修改。
> - 在 iOS 26+ 版本中，定位變更需要重啟設備才能生效。
> - 公共選點頁面存在請求上限，需自部署以提升使用體驗。

> [!warning] 注意事項
> - 僅支持 WiFi 定位，對 GPS 硬件定位無影響。
> - iOS 26+ 版本中，定位緩存機制可能導致位置變更不立即生效，需要重啟設備。
> - 公共選點頁面有請求上限，建議自部署以避免限制。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供類似的虛擬定位功能，但主要針對遊戲場景，適合需要在遊戲中隱藏位置的用戶。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於隱私保護的虛擬定位工具，提供更強的數據加密，但使用上較為複雜。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於遊戲虛擬定位，使用 C++ 實作，性能優於 JavaScript，但整合難度較高。 | 如果你的主要需求是遊戲中的虛擬定位，且對性能有較高要求。 | medium，因為需要重新配置環境和學習新的 API。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供更強的隱私保護功能，使用 Rust 實作，記憶體使用效率高，但學習曲線較陡。 | 如果你的主要需求是隱私保護，且願意投入時間學習新工具。 | high，因為需要全面重構現有的定位邏輯。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **wloc** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於遊戲虛擬定位，使用 C++ 實作，性能優於 JavaScript，但整合難度較高。 | 提供更強的隱私保護功能，使用 Rust 實作，記憶體使用效率高，但學習曲線較陡。 |
> | 遷移成本 | - | medium，因為需要重新配置環境和學習新的 API。 | high，因為需要全面重構現有的定位邏輯。 |
> | 適用場景 | 主要場景 | 如果你的主要需求是遊戲中的虛擬定位，且對性能有較高要求。 | 如果你的主要需求是隱私保護，且願意投入時間學習新工具。 |
...
- **[HIGH]** 在 iOS 26+ 上，定位變更後需要重啟設備才能生效
  - 解法：重啟設備以清除定位緩存
- [MEDIUM] 公共選點頁面請求上限，可能導致無法使用
  - 解法：建議自部署 Worker 以避免限制
- [MEDIUM] 如果在模塊參數中手動修改經緯度，清除持久化數據後仍會使用修改的坐標
  - 解法：保持默認參數不變，僅清除持久化數據以進入透傳模式

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 需要頻繁變更位置的旅行者 | 非常適合 | 提供簡單的快捷指令和易用的界面，能快速切換位置。 |
| 開發者需要測試地理位置功能的應用 | 適合 | 能夠快速設置虛擬位置，方便進行測試。 |
| 對隱私有高要求的用戶 | 普通 | 雖然不存儲數據，但仍需信任代理工具的配置。 |
| 不熟悉技術的普通用戶 | 不適合 | 需要一定的代理工具使用經驗，對於完全不熟悉的用戶來說上手難度較高。 |
...
> [!warning] 安全性快速掃描
> 低風險：不需要高權限，並且不存取敏感資料，適合在 CI/CD pipeline 中使用。
...
> | --- | --- | --- | --- |
> | [#39](https://github.com/Yu9191/wloc/issues/39) | Shadowrocket+快捷指令（秒成功）操作流程图文记录 | 9 | 12 |
