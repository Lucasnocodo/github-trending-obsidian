...
star_history: "2026-06-27:457,2026-06-27:462,2026-06-28:644,2026-06-28:647,2026-06-29:1092,2026-06-29:1098,2026-06-30:1508,2026-06-30:1515,2026-07-01:1965,2026-07-01:1968,2026-07-02:2357,2026-07-03:2584,2026-07-04:2845,2026-07-05:2957,2026-07-06:3094,2026-07-07:3167"
tags:
  - github
  - "category/開發工具"
  - "lang/javascript"
...
**3.2k** stars · **264** stars/天 · 建立 12 天前 · JavaScript · 未標註授權
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Recent · **熱度** Hot (264 stars/day)
> **授權** 未標註授權 (風險較高) · **維護** Moderate (最後推送 9 天前) · **貢獻者** Solo (bus factor 風險) · **參與度** Medium
> **適合** 需要在 iOS 上虛擬定位但不想手動填寫經緯度的用戶。
> **一句話重點** WLOC 讓 iOS 用戶能夠靈活控制虛擬定位，特別是在隱私和位置控制方面提供了新的解決方案。
...
> **結論** 花 3 小時學習，2 小時整合，得到靈活的虛擬定位功能，值得投入。

> [!abstract] 核心創新
> 提供了一种通过代理模块修改 Apple 网络定位返回坐标的全新方式。

## 專案簡介

WLOC 主要功能是修改 Apple 的网络定位服务返回的坐标，允许用户在 iOS 设备上实现虚拟定位。使用者可以通过在线选点页面选择目标位置，系统会自动将选定的经纬度通过代理模块拦截并修改 WLOC 响应。设置位置的指令非常简单，用户只需在地图 App 中选定位置，分享后选择 'wloc 设置地理位置' 即可生效。此工具支持 Surge、Quantumult X、Loon 和 Stash 等多种代理工具，用户可以直接订阅相应的模块来启用功能。WLOC 还提供了清理恢复位置的快捷指令，用户可以一键恢复真实定位，极大提高了使用便利性。

其核心技术是通过解析 Apple 和高德地图的链接，获取真实坐标并进行 GCJ-02 到 WGS84 的转换，确保在中国大陆的定位准确性。与其他定位修改工具相比，WLOC 的优势在于其支持多种代理工具，并且提供了在线选点和快捷指令的便捷操作。使用 WLOC 时，用户需注意 iOS 26 及以上版本可能会因系统缓存导致定位不生效，需重启设备以清除缓存。该工具适合需要频繁切换位置的用户，尤其是在室内环境中依赖 WiFi 定位的场景。若用户希望自行部署，WLOC 的 Worker 源码完全开源，方便用户根据需求进行定制。

**技術棧**：`JavaScript`

## 重點功能

- 多代理支持 — 支持 Surge、Quantumult X、Loon 和 Stash 等多種代理工具的模块。
- 快捷指令 — 提供一键设置和恢复位置的快捷指令，使用方便。
- 在线选点 — 用户可以通过在线页面快速选择目标位置，避免手动输入经纬度。
- 隐私保护 — 解析过程不存储、不记日志，确保用户隐私。
- 自部署 Worker — 提供开源源码，用户可自行部署 Worker 以满足特定需求。
...
2. 打开在线选点页面
```bash
访问 https://wloc-pages.pages.dev/
```
3. 选择位置并保存
```bash
在地图中选择位置，分享并选择 'wloc 设置地理位置'
```

## 為什麼值得關注

> [!tip] 爆紅原因
> 建立 12 天就累積 3167 stars（264/天），forks 442（14.0%），顯示出強烈的社群需求。作者 Yu9191 在這個領域有一定的經驗，解決了 iOS 定位精度不足的痛點，讓用戶能夠自由設置虛擬位置。此專案的流行可能與用戶對於隱私和位置控制的需求增長有關，並且社群中對於如何使用的討論也促進了其擴散。

## 適合誰使用

**目標受眾**：需要在 iOS 上虛擬定位但不想手動填寫經緯度的用戶。

> [!example] 使用場景
> - 上班族用它來在地圖上虛擬定位，因為這樣可以避免因為地理位置而影響的工作安排。
> - 開發者用它來測試地理位置相關的應用，因為可以快速切換位置而不需要實際移動。
> - 旅行者用它來在不同城市的地圖上設置虛擬位置，因為這樣可以提前規劃行程而不必實際到達目的地。

## 架構分析

WLOC 的架构采用了模块化设计，用户通过代理工具订阅不同的模块来实现定位修改。数据流从用户在地图上选择位置开始，经过在线选点页面的解析，最终通过代理模块拦截并修改 WLOC 响应。选择这种设计的原因在于提高了灵活性和可扩展性，用户可以根据自己的需求选择不同的代理工具。此架构的代价是需要用户具备一定的代理工具使用经验，且在 iOS 26 及以上版本中，系统的定位缓存机制可能导致定位不生效。整体来看，该架构适合需要频繁修改位置的用户，但在高版本 iOS 上可能面临缓存问题。

## 技術深入分析

WLOC 的核心技術在於通過代理模塊修改 Apple 的 WLOC 响应，這涉及到對網絡請求的攔截和數據的解析。使用 JavaScript 實現的模塊化設計使得用戶可以根據需求選擇合適的代理工具。這種設計不僅提高了靈活性，還能夠在不同的場景下進行調整。效能方面，WLOC 主要依賴於網絡請求的速度，理論上可以處理大量的請求，但在高版本 iOS 中，定位緩存的問題可能會影響使用體驗。選擇 JavaScript 作為開發語言的原因在於其廣泛的兼容性和易於部署的特性，這使得 WLOC 能夠快速適應不同的環境。技術風險方面，WLOC 依賴於 Apple 的定位服務，若未來 Apple 進行重大改動，可能會影響到 WLOC 的功能。整合方面，WLOC 可以與主流的代理工具無縫對接，但對於不熟悉這些工具的用戶，仍需一定的學習成本。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的使用說明和範例，安裝過程相對順暢，但對於新手來說，理解代理工具的使用可能會有一定的難度。整體來說，花 30 分鐘應該能夠順利上手並運行起來。
...
> [!danger] 缺點
> - 仅支持 iOS，Android 用户无法使用。
> - 需要重启设备清除定位缓存，使用不便。
> - 对用户的代理工具使用经验有一定要求。

> [!warning] 注意事項
> - 仅支持 iOS 设备，Android 用户无法使用。
> - iOS 26 及以上版本可能需要重启设备以清除定位缓存。
> - 不支持 GPS 硬件定位，仅修改网络定位结果。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供类似的虚拟定位功能，但主要针对游戏场景，适用性较窄。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 专注于隐私保护的定位工具，功能上不如 WLOC 灵活。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 使用 Rust 實作，記憶體用量少，但功能上較為單一。 | 如果你的團隊已經在使用 Rust 並希望減少記憶體使用，這個工具會是更好的選擇。 | medium，因為需要重新學習 Rust 的生態和工具鏈。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於隱私保護，功能上不如 WLOC 靈活。 | 如果你的主要需求是隱私保護而不是靈活的定位修改，這個工具會更合適。 | low，因為功能上相似，轉換成本較低。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **wloc** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 使用 Rust 實作，記憶體用量少，但功能上較為單一。 | 專注於隱私保護，功能上不如 WLOC 靈活。 |
> | 遷移成本 | - | medium，因為需要重新學習 Rust 的生態和工具鏈。 | low，因為功能上相似，轉換成本較低。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用 Rust 並希望減少記憶體使用，這個 | 如果你的主要需求是隱私保護而不是靈活的定位修改，這個工具會更 |
...
- **[HIGH]** 在 iOS 26 及以上版本中，定位可能因緩存問題而不生效。
  - 解法：重啟設備以清除緩存。
- [MEDIUM] 使用高德地圖時，短鏈解析可能會失敗。
  - 解法：確保代理工具正確配置，並檢查網絡連接。
- [MEDIUM] 如果手動修改了模塊參數，清除持久化數據後仍會使用舊坐標。
  - 解法：確保保持默認參數不變，然後清除持久化數據。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 需要在辦公室內虛擬定位的上班族 | 非常適合 | WLOC 提供了靈活的定位修改功能，適合頻繁切換位置的需求。 |
| 開發地理位置相關應用的開發者 | 適合 | 能快速測試不同地理位置的應用功能，提升開發效率。 |
| 希望在旅行中提前規劃行程的旅行者 | 非常適合 | 能夠在不實際到達的情況下設置虛擬位置，方便行程規劃。 |
| 不熟悉代理工具的普通用戶 | 不適合 | 需要一定的技術背景來配置和使用代理工具。 |
...
> [!tip] 投入 vs 回報
> 花 3 小時學習，2 小時整合，得到靈活的虛擬定位功能，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> WLOC 本身不需要高權限，但需信任代理工具的安全性。它不存取敏感資料，且依賴鏈中無已知的供應鏈風險，使用於 CI/CD 中相對安全。

## 生態系整合

> [!abstract] 如何融入你的工具鏈

WLOC 通常與 Surge、Quantumult X 等代理工具搭配使用，主要在開發和測試階段進行定位修改。在一個使用 Surge 的專案中，使用者可以通過添加 WLOC 模塊來實現虛擬定位，具體做法是訂閱模塊後，配置 MITM 設置。與主流工具鏈的相容性良好，但對於不熟悉的用戶，可能需要額外的學習時間。常見的整合問題包括代理工具配置不當，這可能導致定位功能失效。
...
> | --- | --- | --- | --- |
> | [#39](https://github.com/Yu9191/wloc/issues/39) | Shadowrocket+快捷指令（秒成功）操作流程图文记录 | 9 | 14 |
> | [#35](https://github.com/Yu9191/wloc/issues/35) | Shadowrocket设置使用教程 | 8 | 24 |
> | [#42](https://github.com/Yu9191/wloc/issues/42) | iOS 27 beta不行了？ | 0 | 1 |
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["自動化測試","隱私保護","虛擬定位"];
