...
use_case: "修改 Apple 网络定位，支持 Surge、Quantumult X、Loon 和 Stash 的虚拟定位工具。"
priority: high
ring: assess
...
star_history: "2026-06-27:457,2026-06-27:462,2026-06-28:644,2026-06-28:647,2026-06-29:1092,2026-06-29:1098,2026-06-30:1508,2026-06-30:1515,2026-07-01:1965,2026-07-01:1968,2026-07-02:2357,2026-07-03:2584,2026-07-04:2845,2026-07-05:2957,2026-07-06:3094,2026-07-07:3167,2026-07-08:3299,2026-07-09:3489,2026-07-10:3676"
tags:
  - github
  - "category/其他"
  - "lang/javascript"
...
aliases:
  - "wloc"
  - "Yu9191/wloc"
  - "修改 Apple 网络定位，支持 Surge、Quantumult X、Loon 和 Stash 的虚拟定位工具。"
---

# wloc

**3.7k** stars · **245** stars/天 · 建立 15 天前 · JavaScript · 未標註授權
...
> [!info] 速覽
> **安裝難度** Easy · **專案狀態** Recent · **熱度** Hot (245 stars/day)
> **授權** 未標註授權 (風險較高) · **維護** Moderate (最後推送 12 天前) · **貢獻者** Solo (bus factor 風險) · **參與度** Medium
> **適合** 需要在 iOS 上靈活調整地理位置但不想手動輸入經緯度的普通用戶。
> **一句話重點** WLOC 的最大優勢在於其便捷的使用方式和對多種代理工具的支持，讓虛擬定位變得更加容易。
...
> [!abstract] 核心創新
> 提供了一個簡單易用的方式來修改 Apple 網絡定位服務的返回坐標。

## 專案簡介

WLOC 是一個用於修改 Apple 網絡定位服務的工具，能夠讓用戶在 iOS 上虛擬定位。用戶可以透過選擇地圖上的位置，並使用快捷指令來設置或清除虛擬定位。具體流程是：選擇位置 → 共享 → 使用「wloc 設置地理位置」指令來生效。這個工具的賣點在於其簡便性和快速反應，無需手動填寫經緯度。技術上，WLOC 使用 JavaScript 實現，並依賴於代理模塊來攔截和修改 WLOC 請求，確保定位數據的準確性。它支持多種代理工具，如 Surge 和 Quantumult X，並提供了清晰的配置文件和模塊。

與其他虛擬定位工具相比，WLOC 的優勢在於其對 Apple 地圖和高德地圖的兼容性，以及簡化的使用流程。使用者在 iOS 26 及以上版本時，需注意定位緩存問題，可能需要重啟設備以清除舊的定位數據。這個工具適合需要在室內環境中使用 WiFi 定位的用戶，特別是對於需要頻繁切換位置的場景。由於其開源特性，用戶也可以自行部署 Worker 以避免公共服務的請求限制。整體而言，WLOC 是一個成熟且實用的虛擬定位解決方案，適合個人和小型團隊使用。

**技術棧**：`JavaScript`

## 重點功能

- 快捷指令支持 — 透過快捷指令快速設置或清除虛擬定位，無需手動操作。
- 多平台支持 — 支持 Surge、Quantumult X、Loon 和 Stash 等多種代理工具。
- 自部署 Worker — 提供開源 Worker 代碼，允許用戶自行部署以避免公共服務的請求限制。
- 坐標轉換 — 自動將 GCJ-02 坐標轉換為 WGS84，確保定位數據的準確性。
- 收藏位置功能 — 允許用戶收藏多個位置，方便快速切換。
...
2. 設置地理位置
```bash
在地圖 App 選擇位置 → 共享 → 選擇「wloc 設置地理位置」
```
3. 清理恢復位置
```bash
點擊「wloc 清理恢復位置」
...
> [!tip] 爆紅原因
> 建立 15 天內累積 3676 stars（245/天），forks 605（16.5%），顯示出強烈的社群興趣。作者 Yu9191 之前有相關的開源經驗，這個工具解決了 iOS 用戶在使用 Apple 地圖時無法靈活調整定位的痛點。社群中對於如何使用 Shadowrocket 和快捷指令的討論熱烈，顯示出實際使用中的需求。技術上，這個工具的出現是因為 Apple 的定位機制需要更靈活的解決方案，尤其是在中國大陸的 GCJ-02 坐標系統下。高達 16.5% 的 forks/stars 比率顯示許多開發者在實際修改和使用這個工具。

## 適合誰使用

**目標受眾**：需要在 iOS 上靈活調整地理位置但不想手動輸入經緯度的普通用戶。

> [!example] 使用場景
> - 普通用戶用它來在 Apple 地圖上虛擬定位，因為手動填寫經緯度繁瑣，使用快捷指令可快速切換位置。
> - 開發者用它來測試地理位置相關的應用，因為可以輕鬆模擬不同的地理環境，提升測試效率。
> - 旅行者用它來在不同城市中隱藏真實位置，因為能夠在不改變實際位置的情況下使用基於位置的服務。

## 架構分析

WLOC 的架構基於 JavaScript，主要通過代理模塊來攔截和修改定位請求。用戶首先需要訂閱相應的模塊，然後通過快捷指令來設置或清理虛擬定位。資料流方面，當用戶選擇位置後，系統會發送請求到 gs-loc.apple.com，代理模塊會攔截這個請求，並根據用戶設置的坐標來修改返回的數據。這種設計的好處是簡化了用戶的操作流程，但也帶來了對於代理服務的依賴。擴展性方面，這個工具可以通過自部署 Worker 來減少對公共服務的依賴，但需要用戶具備一定的技術能力來進行部署。

## 技術深入分析

WLOC 的核心技術機制是使用 JavaScript 來攔截和修改 Apple 的 WLOC 請求，這使得用戶能夠在不手動輸入經緯度的情況下，快速設置虛擬定位。這個工具的效能表現良好，因為它依賴於代理服務來處理請求，並且能夠快速返回修改後的定位數據。設計上，WLOC 選擇了 JavaScript 作為實現語言，這使得它在多平台上有良好的兼容性，但也帶來了對於代理服務的依賴。技術風險方面，WLOC 在高版本 iOS 中可能會遇到定位緩存的問題，這需要用戶重啟設備來清除舊的定位數據。整合方面，這個工具可以輕鬆與現有的代理工具鏈結合，但對於不熟悉代理配置的用戶來說，可能需要一些學習成本。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了詳細的使用說明和快捷指令的連結，安裝過程相對順暢，但對於不熟悉代理工具的用戶可能會有一定的學習曲線。整體而言，花 30 分鐘應該能夠順利運行起來。
...
> [!danger] 缺點
> - 需要在代理模式下使用，對於不熟悉的用戶可能有一定的學習曲線。
> - 在高版本 iOS 中，定位緩存問題可能導致需要重啟設備。
> - 僅支持 iOS，對於 Android 用戶無法使用。

> [!warning] 注意事項
> - 僅支持 iOS 系統，對於 Android 用戶無法使用。
> - 需要在代理模式下使用，否則無法正常拦截請求。
...
| 工具 | 差異 |
| --- | --- |
| proxypin-wloc-spoofer | 這個工具提供類似的虛擬定位功能，但可能不支持多種代理工具的配置。 |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 這個工具專注於代理和虛擬定位，但使用的技術路線與 WLOC 不同，可能需要更多的手動配置。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| proxypin-wloc-spoofer | 這個工具使用類似的虛擬定位技術，但主要針對特定的代理工具，可能不如 WLOC 那麼靈活。 | 如果你的需求僅限於特定的代理工具，且不需要多平台支持，可以考慮使用它。 | low，因為兩者的使用方式相似，切換過程中不需要太多調整。 |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 這個工具專注於代理和虛擬定位，但使用的技術路線與 WLOC 不同，可能需要更多的手動配置。 | 如果你的團隊已經在使用這個工具，且需要更高的自定義能力，可以考慮它。 | medium，因為可能需要重新配置和調整現有的使用流程。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **wloc** | **proxypin-wloc-spoofer** | **agent-sprite-forge** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 這個工具使用類似的虛擬定位技術，但主要針對特定的代理工具，可能不如 WLOC 那麼靈活。 | 這個工具專注於代理和虛擬定位，但使用的技術路線與 WLOC 不同，可能需要更多的手動配置。 |
> | 遷移成本 | - | low，因為兩者的使用方式相似，切換過程中不需要太多調整。 | medium，因為可能需要重新配置和調整現有的使用流程。 |
> | 適用場景 | 主要場景 | 如果你的需求僅限於特定的代理工具，且不需要多平台支持，可以考 | 如果你的團隊已經在使用這個工具，且需要更高的自定義能力，可以 |
...
- **[HIGH]** 在 iOS 26 以上版本中，定位緩存可能導致需要重啟設備才能生效。
  - 解法：重啟設備以清除定位緩存。
- [MEDIUM] 使用快捷指令時，可能需要確認權限設定。
  - 解法：確保在首次使用時選擇允許訪問位置信息。
- [MEDIUM] 在某些地區，可能無法正常使用 Apple 地圖的虛擬定位功能。
  - 解法：嘗試使用其他地圖服務，如高德地圖。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 需要在室內環境中使用 WiFi 定位的普通用戶 | 非常適合 | 這個工具專為虛擬定位設計，能夠快速切換位置。 |
| 開發地理位置相關應用的開發者 | 適合 | 能夠輕鬆模擬不同的地理環境，提升測試效率。 |
| 需要在戶外使用 GPS 定位的用戶 | 不適合 | 這個工具僅修改網絡定位，對於 GPS 定位無法提供支持。 |
...
> [!warning] 安全性快速掃描
> 低風險：這個工具不需要高權限，且不存取敏感資料。依賴的第三方服務需信任，但整體設計考量了隱私問題。
...
**社群活躍度**：社群活躍，有不少使用者在討論使用技巧和問題解決方案。
...
> | [#35](https://github.com/Yu9191/wloc/issues/35) | Shadowrocket设置使用教程 | 10 | 25 |
> | [#46](https://github.com/Yu9191/wloc/issues/46) | Shadowrocket完整教程请看这里-小白保姆级教程 | 2 | 0 |
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["虛擬定位","API 設計","自動化"];
