...
use_case: "修改 Apple 网络定位返回坐标，支持多种代理工具的定位虚拟化。"
priority: high
ring: assess
...
aliases:
  - "wloc"
  - "Yu9191/wloc"
  - "修改 Apple 网络定位返回坐标，支持多种代理工具的定位虚拟化。"
---

# wloc

**5.6k** stars · **234** stars/天 · 建立 24 天前 · JavaScript · 未標註授權
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Recent · **熱度** Hot (234 stars/day)
> **授權** 未標註授權 (風險較高) · **維護** Moderate (最後推送 21 天前) · **貢獻者** Solo (bus factor 風險) · **參與度** Medium
> **適合** 需要在 iOS 上快速修改地理位置的開發者和普通用戶。
> **一句話重點** WLOC 的設計讓虛擬定位變得簡單，特別是對於需要頻繁變更位置的用戶來說，這是一個非常實用的工具。
...
> **結論** 花 2 小時學習，1 小時整合，得到便捷的虛擬定位功能，值得一試。

> [!abstract] 核心創新
> WLOC 提供了一個簡單的方式來修改 Apple 網絡定位返回的坐標，支持多種代理工具和快捷指令。

## 專案簡介

WLOC 是一個用於修改 Apple 網絡定位服務返回坐標的工具，支持 Surge、Quantumult X、Loon 和 Stash 等多種代理工具。使用者可以透過快捷指令快速設置或恢復地理位置，無需手動輸入經緯度。具體流程是用戶在地圖應用中選擇位置，然後通過共享功能選擇「wloc 設置地理位置」，系統會自動將選定的坐標發送給 WLOC 服務進行處理。這樣的設計使得虛擬定位變得簡單且高效，特別適合需要頻繁變更位置的用戶。技術上，WLOC 使用了純轉發解析的方式，通過 worker 來解析 Apple 地圖和高德地圖的短鏈，並進行 GCJ-02 到 WGS84 的坐標轉換，這樣可以避免因為地理位置偏移而導致的定位不準確問題。

與其他類似工具相比，WLOC 的優勢在於其支持多種代理工具，並且提供了便捷的快捷指令，使用者可以輕鬆上手。實際使用中，WLOC 需要用戶開啟代理並信任 gs-loc.apple.com，並且在 iOS 26 及以上版本中，可能需要重啟設備以清除定位緩存。這使得 WLOC 在高版本系統中的使用體驗略有複雜性，但整體上仍然是一個非常實用的工具。對於需要在不同地點進行測試或使用特定地理位置的開發者來說，WLOC 提供了一個高效的解決方案。

**技術棧**：`JavaScript`

## 重點功能

- 多種代理支持 — 支持 Surge、Quantumult X、Loon 和 Stash，方便用戶選擇。
- 快捷指令集成 — 提供快捷指令來快速設置和清理位置，無需手動操作。
- 自動坐標轉換 — 自動將 GCJ-02 坐標轉換為 WGS84，確保定位準確性。
- 純轉發解析 — 使用 worker 進行坐標解析，無存儲和日誌記錄，保護用戶隱私。
- 在線選點頁面 — 提供用戶友好的界面來選擇地理位置，簡化操作流程。

## 快速開始

1. 訂閱並啟用代理模塊
```bash
在 Surge/Quantumult X/Loon/Stash 中訂閱對應模塊
```
2. 選擇地理位置
```bash
在地圖應用中選擇位置，然後共享選擇 'wloc 設置地理位置'
```
3. 清理位置
```bash
使用快捷指令 'wloc 清理恢復位置' 來恢復真實定位
```
...
# Surge 模塊示例
[Script]
wloc = type=global,script-path=modules/wloc.js

# Quantumult X 模塊示例
[rewrite_local]
^https://gs-loc.apple.com/(.*) url script-request-body modules/wloc.js

# 預期輸出
# 代理會成功修改 Apple 定位返回的坐標。
```

## 為什麼值得關注

> [!tip] 爆紅原因
> 建立 24 天內累積 5621 stars（234/天），forks 1090（19.4%），這顯示出強烈的社群興趣。作者 Yu9191 之前在定位相關工具上有過貢獻，這次的 WLOC 解決了以往需要手動設定經緯度的繁瑣問題，讓用戶能夠快速而輕鬆地修改定位。近期的社群討論和教程需求也顯示出使用者對於這個工具的需求和興趣。這個工具的可行性得益於 Apple 地圖和高德地圖的 API 變化，讓定位虛擬化變得更為簡單。forks/stars 比率為 19.4%，顯示出許多人在實際修改和使用這個工具。

## 適合誰使用

**目標受眾**：需要在 iOS 上快速修改地理位置的開發者和普通用戶。

> [!example] 使用場景
> - 移動開發者用它來在不同地點測試應用，因為可以快速切換地理位置，避免了手動輸入經緯度的繁瑣。
> - 測試工程師用它來模擬用戶在不同城市的行為，因為 WLOC 支持多種地圖應用，能夠輕鬆設置虛擬位置。
> - 普通用戶用它來隱藏真實位置，因為只需簡單的快捷指令即可完成設置，操作直觀且方便。

## 架構分析

WLOC 的架構基於 JavaScript，主要通過代理模塊來攔截和修改 Apple 網絡定位的請求。使用者需要訂閱相應的模塊並啟用 MITM 來進行定位修改。資料流方面，當用戶在選點頁面選擇位置後，系統會發送請求到 gs-loc.apple.com，代理模塊攔截該請求並修改返回的坐標。

這樣的設計使得定位虛擬化過程中不需要用戶手動輸入經緯度，提升了使用體驗。選擇 JavaScript 作為主要語言，因為其在網絡環境中的高效性和靈活性，然而這也意味著對於不熟悉 JavaScript 的用戶來說，學習曲線會稍微陡峭。擴展性方面，WLOC 可以支持多種代理工具，但在 iOS 26 及以上版本中，定位緩存機制可能會影響使用效果，這是需要注意的技術風險。

## 技術深入分析

WLOC 的核心技術機制是通過攔截 Apple 網絡定位請求，並修改返回的坐標來實現虛擬定位。使用了 JavaScript 作為開發語言，這使得其在網絡環境中能夠高效運行。效能上，WLOC 能夠快速響應用戶的請求，並且支持多種地圖應用的坐標解析，這在實際使用中表現良好。設計上，選擇代理模塊的方式可以讓用戶在不改動系統設置的情況下進行定位修改，這樣的設計取捨使得使用者能夠輕鬆上手，但也意味著對於不熟悉代理設置的用戶來說，可能會有一定的學習成本。技術風險方面，WLOC 在 iOS 26 及以上版本中可能會受到定位緩存機制的影響，這需要用戶手動重啟設備來清除緩存，這是一個潛在的使用障礙。整合方面，WLOC 可以與多種主流代理工具無縫對接，並且提供了良好的使用文檔，這使得其在開發者社群中受到廣泛關注和使用。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了詳細的使用說明和快捷指令的連結。安裝過程相對順暢，但對於不熟悉代理設置的用戶來說，可能會有一定的學習曲線。文件中沒有多語言支持，主要以中文為主，這對於非中文用戶可能會造成障礙。
...
> [!danger] 缺點
> - 僅限於 iOS 平台，無法在其他系統使用。
> - 需要重啟設備以清除定位緩存，使用上不便。
> - 對於不熟悉代理設置的用戶，學習曲線較陡。

> [!warning] 注意事項
> - 僅支援 iOS 26 及以上版本的定位虛擬化。
> - 需要用戶手動重啟設備以清除定位緩存，否則可能無法生效。
> - 不支援 Windows 平台的使用，僅限於 iOS 設備。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供類似的定位虛擬化功能，但主要針對 Android 系統，無法在 iOS 上使用。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於網絡代理的定位修改，功能較為單一，缺乏 WLOC 的多樣化支持。 |
| [0xSero/codex-shim](https://github.com/0xSero/codex-shim) | 提供 API 層級的定位修改，但不支持快捷指令的便捷操作，使用上不如 WLOC 直觀。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 使用 Rust 實作，記憶體用量較少，但插件生態不如 JavaScript 豐富。 | 如果你的團隊已經在使用 Rust 進行開發，且需要一個高效的定位修改方案。 | medium，因為需要重新適應 Rust 的開發環境。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於網絡代理的定位修改，功能較為單一，缺乏 WLOC 的多樣化支持。 | 如果你只需要一個簡單的定位修改工具，而不需要多種代理支持。 | low，因為功能簡單，易於上手。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **wloc** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 使用 Rust 實作，記憶體用量較少，但插件生態不如 JavaScript 豐富。 | 專注於網絡代理的定位修改，功能較為單一，缺乏 WLOC 的多樣化支持。 |
> | 遷移成本 | - | medium，因為需要重新適應 Rust 的開發環境。 | low，因為功能簡單，易於上手。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用 Rust 進行開發，且需要一個高效的 | 如果你只需要一個簡單的定位修改工具，而不需要多種代理支持。 |
...
- **[HIGH]** 在 iOS 26 及以上版本中，定位緩存可能導致修改後的坐標不生效。
  - 解法：需要重啟設備以清除緩存。
- [MEDIUM] 對於不熟悉代理設置的用戶，可能會遇到設置困難。
  - 解法：參考官方文檔或社群教程。
- [MEDIUM] 在某些地區，地圖服務可能無法正常工作。
  - 解法：嘗試使用其他地圖服務或檢查網絡連接。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 需要頻繁修改定位的開發者 | 非常適合 | WLOC 提供了便捷的快捷指令和多種代理支持。 |
| 普通用戶想隱藏真實位置 | 適合 | 簡單的操作流程和快捷指令使得使用方便。 |
| 不熟悉代理設置的用戶 | 普通 | 可能需要一定的學習曲線來掌握使用方法。 |
| 需要在生產環境中使用的企業 | 不適合 | 目前處於 beta 階段，穩定性和安全性尚未得到充分驗證。 |
...
> [!tip] 投入 vs 回報
> 花 2 小時學習，1 小時整合，得到便捷的虛擬定位功能，值得一試。

## 安全性評估

> [!warning] 安全性快速掃描
> WLOC 本身不需要高權限，並且不會存取敏感資料。依賴鏈的信任程度較高，但用戶應注意在公共網絡環境中使用時的安全性。
...
> | [#54](https://github.com/Yu9191/wloc/issues/54) | 借个楼，正在开发iOS端和Mac端，网页端可在线体验 | 1 | 0 |
> | [#56](https://github.com/Yu9191/wloc/issues/56) | 一键部署，不生效 | 0 | 0 |
