...
**4.5k** stars · **236** stars/天 · 建立 19 天前 · JavaScript · 未標註授權
...
> **適合** 需要在 Apple 地圖上靈活修改定位的用戶，特別是遊戲玩家和開發者。
> **一句話重點** 這個工具讓用戶能夠輕鬆修改 Apple 地圖的定位，解決了許多用戶在使用過程中的痛點。
...
> **結論** 花 2 小時學習，1 小時整合，得到靈活的定位修改功能，值得使用。

> [!abstract] 核心創新
> 提供了一種無需手動填寫經緯度即可快速修改 Apple 網絡定位的解決方案。

## 專案簡介

WLOC 是一個用於修改 Apple 網絡定位服務的工具，主要功能是通過代理模組來返回虛擬坐標，讓用戶能夠在不改變實際位置的情況下進行定位。用戶可以通過選擇地圖上的位置，然後使用快捷指令來設置或清除定位，這樣的設計使得操作非常便捷。具體來說，用戶在地圖應用中選擇一個點，然後共享並選擇「wloc 設置地理位置」，系統會通過代理模組攔截並修改返回的坐標。這個工具支持 Surge、Quantumult X、Loon 和 Stash 等多種代理工具，並且提供了完整的模組和配置文件。

技術上，WLOC 使用了純轉發解析的方式來處理坐標，並不會存儲或記錄用戶的任何數據，這在隱私上提供了額外的保障。與其他類似工具相比，如 Shadowrocket，WLOC 提供了更簡單的快捷指令操作，並且支持多種地圖服務的解析。實際使用中，用戶可能會遇到 iOS 26 及以上版本的定位緩存問題，這需要重啟設備來清除緩存。這個專案目前處於穩定狀態，並且有活躍的社群支持，適合需要經常修改定位的用戶使用。

**技術棧**：`JavaScript`

## 重點功能

- 虛擬定位 — 透過代理模組修改 Apple 網絡定位返回的坐標。
- 支持多種代理工具 — 包括 Surge、Quantumult X、Loon 和 Stash，提供相應的模組配置。
- 快捷指令操作 — 用戶可通過快捷指令快速設置或清除定位，無需手動輸入坐標。
- 隱私保護 — 解析過程不存儲任何用戶數據，確保用戶隱私。
- 地圖服務支持 — 支持 Apple 地圖和高德地圖的坐標解析，並能自動處理 GCJ-02 到 WGS84 的轉換。
...
3. 選擇位置並儲存
```bash
在地圖上選擇位置後，選擇共享並選擇 'wloc 設置地理位置'
```

## 為什麼值得關注

> [!tip] 爆紅原因
> 建立 19 天內累積 4482 stars（236/天），forks 836（18.7%），顯示出強烈的社群需求。作者 Yu9191 是一位活躍的開發者，專注於網絡定位相關的工具開發。這個專案解決了用戶在使用 Apple 地圖時無法靈活修改定位的痛點，之前的方案往往需要繁瑣的手動操作。最近的社群討論和教程也顯示出對於這個工具的需求和關注，尤其是在 Shadowrocket 用戶中。技術上，隨著 iOS 系統對定位服務的強化，這個工具的需求也隨之增加，因為它提供了更靈活的解決方案。高達 18.7% 的 forks/stars 比率顯示出許多用戶正在積極修改和使用這個工具。

## 適合誰使用

**目標受眾**：需要在 Apple 地圖上靈活修改定位的用戶，特別是遊戲玩家和開發者。

> [!example] 使用場景
> - 遊戲玩家用它來在地圖上虛擬定位，因為這樣可以在不移動的情況下獲得遊戲中的特定位置獎勵。
> - 開發者用它來測試地理位置相關的應用，因為可以快速切換位置而不需要實際移動到不同的地點。
> - 需要在特定地點打卡的上班族用它來修改定位，因為這樣可以避免因為距離問題而無法打卡的困擾。

## 架構分析

WLOC 的架構主要基於 JavaScript，通過代理模組攔截 Apple 網絡定位的請求，並修改返回的坐標。這樣的設計使得用戶能夠在地圖應用中輕鬆選擇位置，並通過快捷指令進行設置。資料流中，選點頁面發送請求至 gs-loc.apple.com，代理模組則會攔截並修改該請求的返回值。

這樣的設計使得整體架構相對簡單，但也依賴於用戶的代理配置是否正確。由於使用了純轉發解析的方式，這意味著不會對用戶的數據進行存儲或記錄，這在隱私上提供了保障。擴展性方面，若未來需要支持更多的地圖服務，可能需要增加額外的解析邏輯。

## 技術深入分析

WLOC 的核心技術機制是通過 JavaScript 編寫的代理模組來攔截和修改 Apple 網絡定位的請求。具體來說，當用戶在地圖上選擇位置時，系統會發送請求至 gs-loc.apple.com，代理模組會攔截該請求並修改返回的坐標。這樣的設計使得用戶無需手動輸入經緯度，操作更加便捷。效能方面，WLOC 能夠快速處理請求，並且由於不涉及存儲數據，對資源的需求相對較低。

設計上選擇了純轉發解析的方式，這樣可以有效保護用戶隱私，但也意味著在某些情況下可能需要用戶手動重啟設備以清除緩存。這個選擇的代價是需要用戶具備一定的技術背景來配置代理工具。技術風險方面，若未來 Apple 改變了其定位服務的接口，可能會影響到 WLOC 的正常運作。整合方面，WLOC 可以輕鬆與主流的代理工具進行整合，但在某些情況下可能需要用戶進行額外的配置。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了詳細的使用說明和範例。安裝過程相對順暢，但需要用戶具備一定的代理工具使用經驗。文件目前僅提供英文，可能對某些用戶造成障礙。
...
> - 僅支援特定版本的 iOS，對於舊版用戶不友好。
> - 需要用戶具備一定的代理工具使用經驗。
> - 不支持 Windows 系統，限制了使用範圍。

> [!warning] 注意事項
> - 僅支援 iOS 15 及以上版本。
> - 在 iOS 26 及以上版本中，定位緩存可能導致修改後的坐標不立即生效。
> - 需要啟用代理服務才能正常工作。
> - 不支持 Windows 系統的定位修改。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供類似的虛擬定位功能，但主要針對不同的代理環境，使用上可能較為複雜。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於特定的定位服務，功能較為單一，無法支持多種代理工具的靈活性。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| Shadowrocket | 使用類似的代理技術來修改定位，但操作上較為繁瑣，需要手動配置多個參數。 | 如果你的團隊已經在使用 Shadowrocket 且熟悉其配置流程，則可以選擇它。 | medium，因為需要重新配置定位模組。 |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供了虛擬定位的功能，但主要針對特定的代理環境，使用上可能較為複雜。 | 如果你需要針對特定環境進行定位修改，這個工具可能更合適。 | high，因為需要重新學習和配置新的工具。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **wloc** | **Shadowrocket** | **agent-sprite-forge** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 使用類似的代理技術來修改定位，但操作上較為繁瑣，需要手動配置多個參數。 | 提供了虛擬定位的功能，但主要針對特定的代理環境，使用上可能較為複雜。 |
> | 遷移成本 | - | medium，因為需要重新配置定位模組。 | high，因為需要重新學習和配置新的工具。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用 Shadowrocket 且熟悉其配 | 如果你需要針對特定環境進行定位修改，這個工具可能更合適。 |
...
> [!tip] 採用建議
> 適合個人使用和小型團隊的定位需求，不建議用於大型商業應用。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在 iOS 26 及以上版本中，定位緩存可能導致修改後的坐標不立即生效
  - 解法：需要重啟設備以清除緩存
- [MEDIUM] 某些地圖服務的短鏈解析可能會失敗
  - 解法：使用完整的地圖鏈接而非短鏈
- [MEDIUM] 用戶需確保代理工具已正確配置
  - 解法：參考官方文檔進行配置
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 遊戲玩家需要在特定位置虛擬定位 | 非常適合 | 能夠快速修改定位，提升遊戲體驗。 |
| 開發者需要測試地理位置相關的應用 | 適合 | 提供靈活的定位修改功能。 |
| 需要在特定地點打卡的上班族 | 非常適合 | 能夠避免因距離問題無法打卡的困擾。 |
| 不熟悉代理工具的普通用戶 | 不適合 | 需要一定的技術背景來配置和使用。 |
...
> [!tip] 投入 vs 回報
> 花 2 小時學習，1 小時整合，得到靈活的定位修改功能，值得使用。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限，並且不存取敏感資料，依賴鏈的信任程度較高，適合在 CI/CD 中使用。
...
> | [#46](https://github.com/Yu9191/wloc/issues/46) | Shadowrocket完整教程请看这里-小白保姆级教程 | 6 | 5 |
> | [#52](https://github.com/Yu9191/wloc/issues/52) | 小火箭图文版教程 | 0 | 1 |
> | [#51](https://github.com/Yu9191/wloc/issues/51) | 请问钉钉打卡可以用吗 | 0 | 1 |
