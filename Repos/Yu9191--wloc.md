...
**6.5k** stars · **202** stars/天 · 建立 32 天前 · JavaScript · 未標註授權
...
> [!info] 速覽
> **安裝難度** Easy · **專案狀態** Growing · **熱度** Hot (202 stars/day)
> **授權** 未標註授權 (風險較高) · **維護** Moderate (最後推送 29 天前) · **貢獻者** Solo (bus factor 風險) · **參與度** Medium
> **適合** 需要在 iOS 設備上虛擬定位但不想手動填寫經緯度的普通用戶和開發者。
> **一句話重點** 這個專案的成功在於其簡單易用的設計，讓虛擬定位變得輕鬆可行。
...
> [!abstract] 核心創新
> 提供了一個簡單易用的虛擬定位解決方案，無需手動填寫經緯度。

## 專案簡介

這個專案的核心功能是修改 Apple 網絡定位服務的返回坐標，讓用戶能夠在 iOS 設備上虛擬定位。用戶可以透過選擇地圖上的位置，然後使用快捷指令來設置或清除定位，簡化了傳統上需要手動填寫經緯度的繁瑣過程。具體來說，當用戶在地圖應用中選擇一個點並分享時，透過 `wloc 设置地理位置` 的快捷指令，系統會自動將選定的經緯度發送到 `gs-loc.apple.com`，並由代理模組攔截並修改返回的數據。這樣的設計讓用戶不需要進入複雜的設定界面，直接透過簡單的操作即可完成定位。此專案使用 JavaScript 開發，並且支持 Surge、Quantumult X、Loon 和 Stash 等多個代理工具，這使得它在不同的環境中都能靈活運用。

相較於其他虛擬定位工具，這個專案的優勢在於其簡單易用的快捷指令和對多種代理的支持，讓用戶能夠快速上手。使用者需要注意的是，iOS 26 及以上版本對定位緩存的強化，可能會導致定位變更不立即生效，這需要重啟設備來清除緩存。整體而言，這個專案適合需要經常變更定位的用戶，特別是那些使用 Apple 生態系統的開發者或普通用戶。未來可能會持續增強對其他平台的支持，並改善用戶體驗。

**技術棧**：`JavaScript`

## 重點功能

- 虛擬定位 — 透過快捷指令快速設置或清除定位，支持 Apple 地圖和高德地圖。
- 多代理支持 — 兼容 Surge、Quantumult X、Loon 和 Stash，方便用戶在不同環境中使用。
- 在線選點 — 提供選點頁面，讓用戶可以輕鬆選擇想要的地理位置。
- 隱私保護 — 解析過程不記錄任何用戶數據，確保用戶隱私安全。
- 開源部署 — 提供源碼，允許用戶自行部署解析服務，增加靈活性。
...
> [!tip] 爆紅原因
> 建立 32 天內累積 6473 stars（202 stars/天），forks 1287（19.9%），顯示出強烈的社群參與。作者 Yu9191 以其在定位工具開發方面的專業知識而聞名，這個專案解決了用戶在使用 Apple 地圖時的定位限制，特別是在中國大陸的 GCJ-02 偏移問題。近期的社群討論和教程需求（如 #39 和 #35）顯示出用戶對於如何使用此工具的興趣和需求，這進一步推動了專案的曝光和使用。這個工具的出現正好填補了市場上對於簡單易用的虛擬定位工具的需求，尤其是在移動設備上。

## 適合誰使用

**目標受眾**：需要在 iOS 設備上虛擬定位但不想手動填寫經緯度的普通用戶和開發者。

> [!example] 使用場景
> - iOS 開發者用它來測試地理位置功能，因為能快速切換定位而不需要實際移動到不同地點。
> - 普通用戶用它來在 Apple 地圖上虛擬定位，因為這樣可以在不離開家中的情況下使用地理位置相關的應用。
> - 隱私意識強的用戶用它來隱藏真實位置，因為它提供了一個簡單的方式來修改定位而不需要複雜的設置。

## 架構分析

這個專案採用了模組化的設計，讓用戶可以根據自己的需求選擇不同的代理工具進行配置。核心的定位修改功能是通過攔截 Apple 網絡定位服務的請求來實現的，這樣的設計使得用戶可以在不改變原有應用的情況下，靈活地修改定位。資料流方面，使用者的選點請求會經過代理模組的攔截，然後進行坐標的修改，最後返回給用戶。這樣的架構設計讓系統能夠快速響應用戶的請求，但在高版本的 iOS 中，定位緩存的問題可能會影響即時性，這需要用戶手動重啟設備來清除緩存。整體而言，這個架構在靈活性和易用性之間取得了良好的平衡。

## 技術深入分析

這個專案的核心技術在於攔截和修改 Apple 的網絡定位請求，使用 JavaScript 實現了對定位數據的動態修改。透過設計的 worker，專案能夠解析來自 Apple 地圖和高德的請求，並將 GCJ-02 坐標轉換為 WGS84，這對於在中國大陸使用定位服務至關重要。效能方面，這個工具能夠快速響應用戶的請求，但在 iOS 26 及以上版本中，定位緩存的問題可能會影響即時性，這意味著用戶需要重啟設備來清除緩存。這個專案的選擇依賴於 JavaScript，這使得它在開發和維護上相對簡單，但也可能面臨性能瓶頸，特別是在大量用戶同時請求時。技術風險方面，隨著 Apple 對定位服務的更新，未來可能會出現兼容性問題，這需要開發者持續關注和更新。此外，這個專案的整合難度相對較低，能夠與主流的代理工具鏈無縫對接，對於開發者來說，學習成本也相對較低。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且包含了詳細的使用說明和範例，讓新手能夠快速上手。安裝過程相對順暢，但需要用戶了解代理工具的基本操作。文件目前僅提供英文，可能對非英語使用者造成一定障礙。
...
> - 僅支持 iOS 設備，無法跨平台使用。
> - 對於新手用戶來說，初次設置可能仍有一定的學習曲線。

> [!warning] 注意事項
> - 僅支持 iOS 設備，無法在 Android 上使用。
> - 需要重啟設備以清除定位緩存，特別是在 iOS 26 及以上版本。
> - 使用時必須保持代理工具開啟，否則無法生效。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供更高級的定位修改功能，但設置較為複雜，適合技術用戶。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於隱私保護的定位工具，功能較為單一，適合隱私需求強的用戶。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 使用 Rust 實作，記憶體用量少，但學習曲線較陡峭。 | 如果你的團隊需要更高效的性能和更低的資源消耗，這個工具可能更合適。 | medium，因為需要重新學習其配置和使用方式。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於隱私保護，功能較為單一，但易於使用。 | 如果你的需求主要是隱私保護，且不需要複雜的定位功能，這個工具會更合適。 | low，因為其設置相對簡單。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **wloc** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 使用 Rust 實作，記憶體用量少，但學習曲線較陡峭。 | 專注於隱私保護，功能較為單一，但易於使用。 |
> | 遷移成本 | - | medium，因為需要重新學習其配置和使用方式。 | low，因為其設置相對簡單。 |
> | 適用場景 | 主要場景 | 如果你的團隊需要更高效的性能和更低的資源消耗，這個工具可能更 | 如果你的需求主要是隱私保護，且不需要複雜的定位功能，這個工具 |
...
> [!tip] 採用建議
> 適合個人使用或小型專案試用，但不建議用於生產環境的核心功能。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在 iOS 26 及以上版本中，定位變更後需要重啟設備才能生效
  - 解法：重啟設備以清除定位緩存
- [MEDIUM] 使用時必須保持代理工具開啟，否則無法生效
  - 解法：確保代理工具持續運行
- [MEDIUM] 首次設置時可能會遇到學習曲線
  - 解法：參考官方文檔和社群教程
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 需要經常變更定位的開發者 | 非常適合 | 提供了簡單的快捷指令和多種代理支持，方便開發測試。 |
| 普通用戶希望隱藏真實位置 | 適合 | 能夠快速設置虛擬定位，滿足日常需求。 |
| 不熟悉技術的用戶 | 普通 | 雖然有簡單的快捷指令，但仍需了解基本的代理設置。 |
| 需要在 Android 設備上使用虛擬定位的用戶 | 不適合 | 目前僅支持 iOS 設備，無法跨平台使用。 |
...
> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限，且不存取敏感資料，使用過程中不會記錄用戶行為，適合在 CI/CD 中安全使用。

## 生態系整合

> [!abstract] 如何融入你的工具鏈

這個工具最常與 Surge、Quantumult X、Loon 和 Stash 等代理工具搭配使用，通常在用戶的定位請求過程中進行攔截和修改。在一個使用 Surge 的環境中，用戶可以簡單地將模組地址添加到配置中，然後透過快捷指令來設置虛擬位置。與主流 CI/CD 工具的整合難度較低，能夠快速上手，且不會對現有的工作流造成太大影響。最常見的整合問題是需要確保代理工具的持續運行，否則定位修改將無法生效。

## 歷史脈絡

> [!info] 這個工具為什麼現在出現？

在這個工具出現之前，虛擬定位主要依賴於複雜的手動設置和不夠友好的用戶介面，這使得普通用戶難以使用。隨著移動設備的普及和隱私意識的提升，對於簡單易用的虛擬定位工具的需求日益增加。這個專案的出現正好填補了市場的空白，提供了一個簡單的解決方案，並且隨著 Apple 對定位服務的持續更新，未來可能會進一步優化其功能和用戶體驗。
...
> [!tip] 導入策略
> 第一週：在個人項目中試用。第二週：在小型內部工具中導入。第三週：編寫使用文檔和最佳實踐。第四週：在主產品中開始使用。

**成功指標**：定位功能的使用率提高 50%。

> [!warning] 退出計畫
> 所有設定均以標準格式存儲，方便用戶在需要時輕鬆轉移到其他工具或方案。
...
> | --- | --- | --- | --- |
> | [#39](https://github.com/Yu9191/wloc/issues/39) | Shadowrocket+快捷指令（秒成功）操作流程图文记录 | 15 | 21 |
...
> | [#72](https://github.com/Yu9191/wloc/issues/72) | 关于Mac OS上的使用问题 | 0 | 3 |
