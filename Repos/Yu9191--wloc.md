...
use_case: "修改 Apple 网络定位返回坐标，支持多种代理工具，快速设置和恢复定位。"
priority: high
ring: assess
...
star_history: "2026-06-27:457,2026-06-27:462,2026-06-28:644,2026-06-28:647,2026-06-29:1092,2026-06-29:1098,2026-06-30:1508,2026-06-30:1515,2026-07-01:1965,2026-07-01:1968,2026-07-02:2357,2026-07-03:2584"
tags:
  - github
  - "category/其他"
  - "lang/javascript"
aliases:
  - "wloc"
  - "Yu9191/wloc"
  - "修改 Apple 网络定位返回坐标，支持多种代理工具，快速设置和恢复定位。"
---

# wloc

**2.6k** stars · **323** stars/天 · 建立 8 天前 · JavaScript · 未標註授權
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Recent · **熱度** Hot (323 stars/day)
> **授權** 未標註授權 (風險較高) · **維護** Active (最後推送 5 天前) · **貢獻者** Solo (bus factor 風險) · **參與度** Medium
> **適合** 需要在 iOS 設備上靈活修改定位的開發者和普通用戶。
> **一句話重點** WLOC 讓 iOS 用戶能夠輕鬆修改定位，並提供了強大的快捷指令支持，這在同類工具中相當罕見。
...
> [!abstract] 核心創新
> WLOC 提供了簡便的快捷指令來快速設置和恢復虛擬定位，並支持多種代理工具。

## 專案簡介

WLOC 是一個用於修改 Apple 網絡定位服務的工具，透過代理模塊來改變返回的坐標。用戶可以在地圖應用中選擇位置，然後通過快捷指令一鍵設置虛擬定位，這樣就不需要手動輸入經緯度。設置完成後，系統會自動將定位結果發送到指定的代理服務，並且支持多種代理工具如 Surge、Quantumult X、Loon 和 Stash。這個工具的賣點在於其簡便的使用方式和高效的定位修改能力。技術上，WLOC 使用了 JavaScript 編寫，並且依賴於 MITM 代理技術來攔截和修改網絡請求。

這種設計使得用戶可以在不改變設備設置的情況下，靈活地控制定位信息。與其他定位修改工具相比，WLOC 支持多種代理工具，並且提供了方便的快捷指令來簡化操作流程。這使得它在需要頻繁切換位置的場景下特別有用，例如開發者測試地理位置相關的應用。使用者可能會遇到定位維持時間短的問題，這是由於 iOS 系統的定位緩存機制所致，重啟設備可以解決這個問題。整體來看，WLOC 是一個穩定的工具，適合需要頻繁修改定位的用戶，並且社群活躍度高，能夠及時解決使用中的問題。

**技術棧**：`JavaScript`

## 重點功能

- 快捷指令支持 — 用戶可通過快捷指令一鍵設置或清除虛擬定位。
- 多種代理工具支持 — 同時支持 Surge、Quantumult X、Loon 和 Stash，滿足不同用戶需求。
- 在線選點功能 — 用戶可在地圖上選擇位置，無需手動輸入經緯度。
- 自部署 Worker — 提供開源的 Worker 代碼，允許用戶自行部署以避免公共請求限制。
- 隱私保護 — 所有解析請求不記錄任何日誌，確保用戶隱私。
...
> [!tip] 爆紅原因
> 建立 8 天就累積 2584 stars（323/天），forks 345（13.4%），這顯示出其快速增長的潛力。作者 Yu9191 之前在定位修改領域有過相關經驗，這個工具解決了用戶在使用 Apple 定位服務時無法靈活修改位置的痛點。這之前的解決方案往往需要繁瑣的手動操作，WLOC 則通過快捷指令簡化了這一過程。社群中對於如何使用 Shadowrocket 的討論也引發了更多的關注，這進一步推動了專案的流行。技術上，WLOC 的設計利用了現有的代理生態系統，讓用戶能夠輕鬆集成到自己的工作流中。

## 適合誰使用

**目標受眾**：需要在 iOS 設備上靈活修改定位的開發者和普通用戶。

> [!example] 使用場景
> - 開發者用它來在測試環境中快速切換地理位置，因為這樣可以更方便地驗證應用的地理位置功能。
> - 普通用戶用它來在社交媒體上隱藏真實位置，因為這樣可以保護隱私並避免不必要的曝光。
> - 遊戲玩家用它來在遊戲中虛擬定位，因為這樣可以獲得更好的遊戲體驗，無需實際移動。

## 架構分析

WLOC 的架構基於 MITM 代理技術，通過攔截和修改網絡請求來實現虛擬定位。用戶在地圖應用中選擇位置後，通過快捷指令將選擇的坐標發送到代理服務。代理服務會攔截對 Apple 網絡定位服務的請求，並修改返回的坐標。

這種設計的優勢在於用戶無需手動輸入經緯度，且能夠靈活切換位置。缺點是需要用戶信任代理服務，並且在 iOS 26 及以上版本中，定位緩存機制可能影響效果。整體而言，這種架構在功能上提供了高度的靈活性，但在安全性上則需要用戶謹慎考量。

## 技術深入分析

WLOC 的核心技術機制是基於 MITM 代理技術，通過攔截 Apple 網絡定位服務的請求來修改返回的坐標。使用 JavaScript 編寫的代理模塊能夠靈活地處理用戶的請求，並將其轉發至 Apple 的服務。這種設計使得用戶可以在不改變設備設置的情況下，動態修改定位信息。效能方面，WLOC 能夠快速響應用戶的請求，並且在大多數情況下能夠即時生效，但在 iOS 26 及以上版本中，由於定位緩存機制，可能需要重啟設備才能清除舊的定位數據。

選擇 JavaScript 作為開發語言的原因在於其在網絡環境中的高效性和靈活性，並且能夠與現有的代理工具無縫集成。這個選擇使得 WLOC 在功能上具備了高度的可擴展性，並且能夠快速適應用戶的需求。技術風險方面，依賴於外部 API 的穩定性可能會影響工具的長期運行，特別是如果 Apple 改變了其定位服務的接口。整合方面，WLOC 與主流的代理工具如 Surge 和 Quantumult X 的兼容性良好，能夠輕鬆集成到現有的工作流中，並且支持 CI/CD 的自動化部署。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的使用說明和範例，安裝過程相對順暢，但對於新手來說，MITM 代理的設置可能會有一些困難。文件中包含了快捷指令的使用方法，這對於快速上手非常有幫助。整體而言，花 30 分鐘應該能夠順利運行。文件目前僅提供英文，可能對非英語用戶造成一定障礙。
...
> [!danger] 缺點
> - 需要依賴 MITM 代理，對於新手用戶可能有一定的學習曲線。
> - 在 iOS 26 以上版本中，定位變更可能不即時生效。
> - 僅限於 iOS 設備，無法在其他平台使用。

> [!warning] 注意事項
> - 僅支持 iOS 設備，無法在 Android 上使用。
> - 需要啟用 MITM 代理，否則無法正常工作。
> - 在 iOS 26 及以上版本中，定位緩存機制可能導致定位變更不即時生效。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 這個工具也提供虛擬定位功能，但主要針對 Android 設備，無法直接應用於 iOS。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 這是一個針對 iOS 的定位修改工具，但功能相對簡單，缺乏 WLOC 的多樣性和快捷指令支持。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 這個工具主要針對 Android 設備，使用原生 API 來修改定位，與 WLOC 的代理模式不同，這使得它在 Android 環境中表現更好。 | 如果你的團隊主要使用 Android 設備，並且需要更深層次的系統集成，則應選擇這個工具。 | medium，因為需要重新適應 Android 的開發環境和 API。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 這是一個針對 iOS 的定位修改工具，但功能相對簡單，缺乏 WLOC 的多樣性和快捷指令支持。 | 如果你只需要基本的定位修改功能，且不需要快捷指令的支持，則可以考慮這個工具。 | low，因為其設置和使用相對簡單。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **wloc** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 這個工具主要針對 Android 設備，使用原生 API 來修改定位，與 WLOC 的代理模式不同，這使得它在 Android 環境中表現更好。 | 這是一個針對 iOS 的定位修改工具，但功能相對簡單，缺乏 WLOC 的多樣性和快捷指令支持。 |
> | 遷移成本 | - | medium，因為需要重新適應 Android 的開發環境和 API。 | low，因為其設置和使用相對簡單。 |
> | 適用場景 | 主要場景 | 如果你的團隊主要使用 Android 設備，並且需要更深層次 | 如果你只需要基本的定位修改功能，且不需要快捷指令的支持，則可 |
...
- **[HIGH]** 在 iOS 26 以上版本中，定位變更可能不即時生效，需要重啟設備。
  - 解法：重啟設備以清除定位緩存。
- [MEDIUM] 使用 Shadowrocket 時可能無法成功設置定位。
  - 解法：檢查代理設置和模塊是否正確啟用。
- [MEDIUM] 在高德地圖中，短鏈的解析可能會失敗。
  - 解法：確保使用最新的 WLOC 模塊，並檢查網絡連接。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 需要頻繁切換位置的開發者 | 非常適合 | 因為其快捷指令支持能夠快速設置和恢復定位。 |
| 普通用戶想要隱藏真實位置 | 適合 | 因為它提供了簡便的虛擬定位功能。 |
| 遊戲玩家需要虛擬定位 | 非常適合 | 因為它能夠快速切換位置而不需要實際移動。 |
| 不熟悉代理設置的新手用戶 | 不適合 | 因為需要一定的技術背景來配置 MITM 代理。 |
...
> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限運行，並且不存取敏感資料，依賴的 API 也相對穩定，適合在 CI/CD 中使用。
...
> | --- | --- | --- | --- |
> | [#35](https://github.com/Yu9191/wloc/issues/35) | Shadowrocket设置使用教程 | 7 | 22 |
> | [#39](https://github.com/Yu9191/wloc/issues/39) | Shadowrocket+快捷指令（秒成功）操作流程图文记录 | 3 | 5 |
...
> | [#36](https://github.com/Yu9191/wloc/issues/36) | 偏差距離問題 | 0 | 3 |
