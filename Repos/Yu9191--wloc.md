...
use_case: "修改 Apple 网络定位服务，支持多种代理工具的一键定位设置与恢复。"
priority: high
ring: assess
...
star_history: "2026-06-27:457,2026-06-27:462,2026-06-28:644,2026-06-28:647,2026-06-29:1092,2026-06-29:1098,2026-06-30:1508,2026-06-30:1515,2026-07-01:1965,2026-07-01:1968,2026-07-02:2357,2026-07-03:2584,2026-07-04:2845,2026-07-05:2957,2026-07-06:3094,2026-07-07:3167,2026-07-08:3299,2026-07-09:3489,2026-07-10:3676,2026-07-11:3806"
tags:
  - github
  - "category/其他"
  - "lang/javascript"
aliases:
  - "wloc"
  - "Yu9191/wloc"
  - "修改 Apple 网络定位服务，支持多种代理工具的一键定位设置与恢复。"
---

# wloc

**3.8k** stars · **238** stars/天 · 建立 16 天前 · JavaScript · 未標註授權
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Recent · **熱度** Hot (238 stars/day)
> **授權** 未標註授權 (風險較高) · **維護** Moderate (最後推送 13 天前) · **貢獻者** Solo (bus factor 風險) · **參與度** Medium
> **適合** 需要在 iOS 上虛擬定位的用戶，特別是使用代理工具的技術愛好者。
> **一句話重點** WLOC 的設計讓虛擬定位變得前所未有的簡單，特別是對於需要頻繁修改位置的用戶。
...
> **結論** 花 2 小時學習，1 小時整合，得到靈活的虛擬定位功能，值得嘗試。

> [!abstract] 核心創新
> WLOC 提供了一個簡單的方式來修改 Apple 網絡定位，支持多種代理工具和快捷指令。

## 專案簡介

WLOC 是一個用於修改 Apple 網絡定位服務的工具，通過代理模塊來虛擬定位。用戶可以在地圖應用中選擇位置，然後通過快捷指令一鍵設置或恢復定位。具體流程是：選擇位置 → 共享至 WLOC → 代理模塊攔截請求 → 修改返回的坐標。這樣的設計使得用戶不需要手動填寫經緯度，簡化了操作。WLOC 支持 Surge、Quantumult X、Loon 和 Stash 四種代理工具，並且提供了簡單的快捷指令來快速設置和清除位置。

其核心功能是通過解析 Apple 地圖和高德地圖的返回數據來實現定位的修改，並且支持 GCJ-02 到 WGS84 的坐標轉換。這種設計的好處在於用戶可以在不同的地圖平台上無縫切換，並且不需要擔心坐標系的差異。與其他定位修改工具相比，WLOC 的優勢在於其簡單的使用方式和對多種代理工具的支持，特別適合需要頻繁修改定位的用戶。使用 WLOC 時，需注意 iOS 26 及以上版本的定位緩存問題，可能需要重啟設備以清除舊的定位數據。整體來看，WLOC 是一個針對需要虛擬定位的用戶設計的高效工具，特別適合於需要在室內環境中使用 WiFi 定位的場景。
...
- 快捷指令一鍵設置 — 用戶可通過快捷指令快速設置和清除定位，提升使用效率。
- 坐標解析功能 — 透過 worker 解析 Apple 地圖和高德的返回數據，實現精確定位。
- 支持 GCJ-02 到 WGS84 的坐標轉換 — 確保不同地圖平台之間的兼容性。
...
> [!tip] 爆紅原因
> 建立 16 天就累積 3806 stars（238/天），forks 668（17.6%），顯示出強烈的社群興趣。作者 Yu9191 之前在 GitHub 上有其他開源項目，這次專案解決了 Apple 定位服務的痛點，讓用戶能夠更方便地修改定位。這個工具的實用性和簡單性吸引了大量用戶，並且在社群中引發了熱烈的討論和分享。特別是針對 Shadowrocket 的使用教程，讓新手更容易上手，這促進了其快速增長。

## 適合誰使用

**目標受眾**：需要在 iOS 上虛擬定位的用戶，特別是使用代理工具的技術愛好者。

> [!example] 使用場景
> - 學生用它來在校園內隨意修改定位，因為可以快速切換到不同的地點進行社交活動。
> - 遊戲玩家用它來在 AR 遊戲中虛擬定位，因為這樣可以在不出門的情況下獲得遊戲中的獎勵。
> - 開發者用它來測試地理位置相關的應用，因為能夠快速設置和恢復定位，節省了大量的測試時間。

## 架構分析

WLOC 的架構設計為一個模塊化的代理工具，主要由不同的代理模塊組成，這些模塊負責攔截和修改網絡請求。用戶通過快捷指令與這些模塊交互，選擇地點並設置虛擬定位。這樣的設計使得用戶能夠靈活地選擇不同的代理工具，並且通過簡單的操作來達成定位修改。

每個模塊的功能獨立，這樣的分離使得維護和擴展變得更加容易。WLOC 的資料流主要是通過用戶的請求 → 代理模塊攔截 → 修改返回的坐標，這樣的流程設計簡潔明瞭，易於理解和使用。整體架構的選擇使得 WLOC 能夠快速適應不同的使用場景，並且能夠在未來進行功能擴展。

## 技術深入分析

WLOC 的核心技術機制是通過代理模塊攔截 Apple 網絡定位服務的請求，並修改返回的坐標。這些模塊使用 JavaScript 實現，並且通過與快捷指令的整合，實現了用戶友好的操作介面。效能上，WLOC 能夠快速響應用戶的請求，並且在大多數情況下能夠即時更新定位數據。設計上，WLOC 選擇了模塊化的架構，使得不同的代理工具可以獨立運作，這樣的選擇帶來了良好的擴展性和維護性。技術風險方面，WLOC 依賴於 Apple 的網絡定位服務，任何服務的變更都可能影響到 WLOC 的功能。整合方面，WLOC 與主流的代理工具相容性良好，能夠輕鬆融入現有的工作流中，並且支持在 CI/CD pipeline 中使用。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了詳細的使用說明和範例；安裝過程相對順暢，但需要注意重啟設備以清除定位緩存；有提供快捷指令的鏈接，方便新手使用；目前僅有英文文檔，缺乏多語言支持。

## 優缺點分析

> [!success] 優點
> - 使用簡單，通過快捷指令一鍵設置和清除定位。
> - 支持多種代理工具，適應性強。
> - 提供坐標解析功能，確保不同地圖平台的兼容性。

> [!danger] 缺點
> - 僅限於 iOS 系統，無法在 Android 上使用。
> - 需要重啟設備以清除定位緩存，增加了使用的複雜性。
> - 不支持 GPS 硬件定位的修改，限制了使用場景。

> [!warning] 注意事項
> - 僅支持 iOS 系統，對於 Android 用戶無法使用。
> - 需要重啟設備以清除定位緩存，特別是在 iOS 26 及以上版本。
> - 不支持 GPS 硬件定位的修改，僅限於網絡定位。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| proxypin-wloc-spoofer | 這個工具也專注於修改 Apple 定位，但 WLOC 提供了更簡單的使用流程和快捷指令支持。 |
| [FFF686868/proxypin-wloc-spoofer](https://github.com/FFF686868/proxypin-wloc-spoofer) | 這個工具的定位修改方法與 WLOC 相似，但 WLOC 提供了更好的用戶界面和多種代理支持。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| proxypin-wloc-spoofer | 這個工具也專注於修改 Apple 定位，但 WLOC 提供了更簡單的使用流程和快捷指令支持。 | 如果你需要一個更簡單的用戶界面和快捷指令支持，WLOC 是更好的選擇。 | low，因為兩者的功能相似，轉換相對容易。 |
| [FFF686868/proxypin-wloc-spoofer](https://github.com/FFF686868/proxypin-wloc-spoofer) | 這個工具的定位修改方法與 WLOC 相似，但 WLOC 提供了更好的用戶界面和多種代理支持。 | 如果你需要更高的自定義性和控制，FFF686868 的工具可能更合適。 | medium，因為需要重新配置一些設置。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **wloc** | **proxypin-wloc-spoofer** | **proxypin-wloc-spoofer** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 這個工具也專注於修改 Apple 定位，但 WLOC 提供了更簡單的使用流程和快捷指令支持。 | 這個工具的定位修改方法與 WLOC 相似，但 WLOC 提供了更好的用戶界面和多種代理支持。 |
> | 遷移成本 | - | low，因為兩者的功能相似，轉換相對容易。 | medium，因為需要重新配置一些設置。 |
> | 適用場景 | 主要場景 | 如果你需要一個更簡單的用戶界面和快捷指令支持，WLOC 是更 | 如果你需要更高的自定義性和控制，FFF686868 的工具可 |
...
- **[HIGH]** 在 iOS 26+ 上，定位緩存可能導致修改後的坐標不生效
  - 解法：重啟設備以清除緩存
- [MEDIUM] 如果用戶手動修改了模塊參數，清除持久化數據後仍會使用舊坐標
  - 解法：保持默認參數不變，僅清除持久化數據
- [MEDIUM] 在某些地區，Apple 可能會限制定位服務的使用
  - 解法：使用 VPN 或其他代理工具來繞過限制

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 需要在室內環境中使用 WiFi 定位的用戶 | 非常適合 | WLOC 專為網絡定位設計，效果最佳。 |
| 開發者需要測試地理位置相關的應用 | 適合 | 能夠快速設置和恢復定位，節省測試時間。 |
| 需要在戶外使用 GPS 定位的用戶 | 不適合 | WLOC 僅修改網絡定位，無法影響 GPS 硬件定位。 |
...
> [!tip] 投入 vs 回報
> 花 2 小時學習，1 小時整合，得到靈活的虛擬定位功能，值得嘗試。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：WLOC 本身不需要高權限，僅修改網絡定位，不存取敏感資料。依賴鏈的信任程度高，無已知的供應鏈風險。
...
**社群活躍度**：社群活躍，存在多個熱門問題和解答，使用者互動良好。
...
> | [#46](https://github.com/Yu9191/wloc/issues/46) | Shadowrocket完整教程请看这里-小白保姆级教程 | 3 | 2 |
