...
use_case: "修改 Apple 网络定位返回坐标，支持多种代理工具，方便快捷地实现虚拟定位。"
priority: high
ring: assess
...
aliases:
  - "wloc"
  - "Yu9191/wloc"
  - "修改 Apple 网络定位返回坐标，支持多种代理工具，方便快捷地实现虚拟定位。"
---

# wloc

**6.4k** stars · **206** stars/天 · 建立 31 天前 · JavaScript · 未標註授權
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Growing · **熱度** Hot (206 stars/day)
> **授權** 未標註授權 (風險較高) · **維護** Moderate (最後推送 28 天前) · **貢獻者** Solo (bus factor 風險) · **參與度** Medium
> **適合** 需要在 iOS 上快速修改定位但不想進行繁瑣配置的普通用戶。
> **一句話重點** WLOC 讓虛擬定位變得前所未有的簡單，特別適合需要隱私保護的用戶。
...
> **結論** 花 2 小時學習，1 小時整合，得到便捷的虛擬定位功能，值得使用。

> [!abstract] 核心創新
> 提供一鍵設置虛擬定位的功能，並支持多種代理工具，簡化了用戶操作流程。

## 專案簡介

WLOC 是一個用於修改 Apple 網絡定位服務的工具，允許用戶透過代理工具（如 Surge、Quantumult X、Loon 和 Stash）來虛擬定位。用戶可以在地圖應用中選擇位置，然後透過快捷指令輕鬆設置或恢復定位。核心流程為：用戶選擇位置 → 透過快捷指令發送請求 → 代理工具攔截並修改返回的坐標。這種設計使得用戶無需手動填寫經緯度，簡化了操作流程。WLOC 的一句話賣點是「一鍵設置虛擬定位，無需手動操作」。技術上，WLOC 使用 JavaScript 實現，並依賴於 MITM（中間人攻擊）技術來攔截和修改網絡請求，這樣可以在不儲存任何數據的情況下進行坐標轉換，確保用戶隱私。

與其他類似工具相比，WLOC 的優勢在於其支持多種流行的代理工具，並且提供了便捷的快捷指令來簡化操作。對比 Shadowrocket 和其他工具，WLOC 的使用門檻更低，因為它不需要複雜的配置，只需簡單的選擇和分享操作。實際使用中，WLOC 能夠有效處理 Apple 地圖和高德地圖的定位需求，但在 iOS 26 及以上版本中，系統的定位緩存機制可能會導致定位不即時更新，這需要用戶重啟設備來清除緩存。整體來看，WLOC 是一個成熟的工具，適合需要頻繁修改定位的用戶，尤其是在需要隱私保護的場景下。未來可能會進一步擴展到更多平台和功能，提升用戶體驗。

**技術棧**：`JavaScript`

## 重點功能

- 一鍵設置虛擬定位 — 用戶可透過快捷指令快速設置或恢復地理位置。
- 支持多種代理工具 — 包括 Surge、Quantumult X、Loon 和 Stash，滿足不同用戶需求。
- 在線選點頁面 — 提供便捷的界面讓用戶選擇地點，並自動生成定位請求。
- 隱私保護 — 所有解析請求不會儲存任何數據，確保用戶隱私安全。
- 自部署 Worker — 用戶可自行部署 Worker 以避免公共請求限制，滿足高頻使用需求。
...
> [!tip] 爆紅原因
> 建立 31 天內累積 6400 stars（206/天），forks 1272（19.9%），顯示出強勁的增長潛力。作者 Yu9191 在開源社區中活躍，解決了以往虛擬定位工具配置繁瑣的痛點，讓用戶能夠快速、方便地修改定位。這個工具的出現正好契合了用戶對隱私和便利性的需求，並且在社交媒體上引發了不少討論，進一步推動了它的流行。高達 19.9% 的 forks/stars 比率顯示出許多用戶對此工具的實際修改和使用，這反映了其在開源社區中的活躍度。

## 適合誰使用

**目標受眾**：需要在 iOS 上快速修改定位但不想進行繁瑣配置的普通用戶。

> [!example] 使用場景
> - 自由職業者用它來在不同城市的地圖應用中設置虛擬定位，因為這樣可以避免因地理位置限制而無法使用某些服務。
> - 開發者用它來測試應用的地理位置功能，因為可以快速切換到不同的地理位置進行測試，而不需要實際移動。
> - 隱私意識強的用戶用它來隱藏真實位置，因為這樣可以避免被追蹤或定位，保護個人隱私。

## 架構分析

WLOC 的架構設計基於 MITM 技術，通過攔截網絡請求來修改 Apple 網絡定位服務的返回數據。具體來說，當用戶在地圖應用中選擇位置後，系統會發送請求到 `gs-loc.apple.com`，WLOC 透過代理工具攔截該請求，並根據用戶選擇的坐標來修改返回的 protobuf 數據。這種設計的好處是能夠實現即時的定位修改，而不需要用戶手動輸入經緯度。缺點是依賴於代理工具的配置，對於不熟悉的用戶來說可能會有一定的學習成本。擴展性方面，WLOC 可以通過自部署 Worker 來應對高頻請求的需求，這樣用戶可以避免公共請求的限制。

## 技術深入分析

WLOC 的核心技術機制是通過攔截和修改網絡請求來實現虛擬定位。它使用 JavaScript 編寫，並依賴於 MITM 技術來攔截 Apple 的定位請求，這樣用戶可以在不手動輸入經緯度的情況下，快速設置虛擬位置。效能方面，WLOC 能夠即時響應用戶的定位請求，並且不會存儲任何用戶數據，這對於隱私保護至關重要。設計上，WLOC 的依賴樹相對簡單，主要依賴於代理工具的配置，這使得它的學習曲線相對較低。技術風險方面，WLOC 依賴於 Apple 的網絡定位服務，若該服務發生變更，可能會影響 WLOC 的功能。整合方面，WLOC 可以與主流的代理工具無縫整合，並且支持快捷指令的使用，這使得它的使用體驗更加流暢。整體來看，WLOC 是一個設計精良的虛擬定位工具，適合需要隱私保護的用戶使用。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了詳細的使用說明和範例；安裝過程相對順暢，但對於不熟悉代理工具的用戶可能有一定的學習曲線；有提供快捷指令的使用指南，方便上手；文件目前僅提供中文，可能對非中文用戶造成一定障礙。
...
> - 僅支持 iOS，無法在其他平台使用。
> - 需要一定的技術背景來配置代理工具。
> - 在高版本 iOS 中可能需要重啟設備來清除定位緩存。

> [!warning] 注意事項
> - 僅支持 iOS 平台，無法在 Android 上使用。
> - 需要 MITM 代理工具的支持，對於不熟悉的用戶可能存在學習曲線。
> - 在 iOS 26 及以上版本中，定位緩存可能導致修改後的定位不即時生效，需重啟設備。
...
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 雖然也支持定位修改，但配置過程較為繁瑣，不如 WLOC 直觀。 |
| [0xSero/codex-shim](https://github.com/0xSero/codex-shim) | 主要用於 AI 代理的定位需求，功能較為專一，無法廣泛應用於其他場景。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 主要針對遊戲定位需求，使用的技術路線與 WLOC 相似，但功能較為專一。 | 如果你的主要需求是遊戲中的虛擬定位，而不是一般應用的定位需求。 | medium，因為需要重新配置和適應不同的使用場景。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供虛擬定位功能，但配置過程較為繁瑣，使用上不如 WLOC 直觀。 | 如果你需要更高的自定義配置，而不介意複雜的設置過程。 | high，因為需要重新學習和配置。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **wloc** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 主要針對遊戲定位需求，使用的技術路線與 WLOC 相似，但功能較為專一。 | 提供虛擬定位功能，但配置過程較為繁瑣，使用上不如 WLOC 直觀。 |
> | 遷移成本 | - | medium，因為需要重新配置和適應不同的使用場景。 | high，因為需要重新學習和配置。 |
> | 適用場景 | 主要場景 | 如果你的主要需求是遊戲中的虛擬定位，而不是一般應用的定位需求 | 如果你需要更高的自定義配置，而不介意複雜的設置過程。 |
...
- **[HIGH]** 在 iOS 26+ 中，定位緩存可能導致修改後的定位不即時生效
  - 解法：重啟設備以清除緩存
- [MEDIUM] 使用快捷指令時，需確保代理已開啟，否則無法生效
  - 解法：確認代理工具已啟用
- [MEDIUM] 清除持久化數據後，若手動修改過經緯度，仍會使用舊的坐標
  - 解法：保持默認參數不變，清除持久化數據後自動進入透傳模式

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 需要在不同城市測試應用的開發者 | 非常適合 | 提供快速的虛擬定位功能，方便測試。 |
| 隱私意識強的用戶需要隱藏真實位置 | 非常適合 | 能夠有效保護用戶隱私。 |
| 普通用戶希望簡單修改定位 | 適合 | 操作簡單，易於上手。 |
| 需要在 Android 上使用虛擬定位的用戶 | 不適合 | 僅支持 iOS 平台。 |
...
> [!tip] 投入 vs 回報
> 花 2 小時學習，1 小時整合，得到便捷的虛擬定位功能，值得使用。
...
> | [#72](https://github.com/Yu9191/wloc/issues/72) | 关于Mac OS上的使用问题 | 0 | 2 |
