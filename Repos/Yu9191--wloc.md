...
**457** stars · **229** stars/天 · 建立 2 天前 · JavaScript · 未標註授權
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Brand New · **熱度** Hot (229 stars/day)
> **授權** 未標註授權 (風險較高) · **維護** Active (最後推送 0 天前) · **貢獻者** Solo (bus factor 風險) · **參與度** Medium
> **適合** 需要在 iOS 設備上虛擬定位但不想手動輸入經緯度的普通用戶。
> **一句話重點** WLOC 的設計讓虛擬定位變得前所未有的簡單，特別適合需要隱私保護的用戶。
...
> **結論** 花 2 小時學習，1 小時整合，得到隱私保護和虛擬定位的效果，值得一試。

> [!abstract] 核心創新
> WLOC 提供了一個簡單的方式來修改 Apple 網絡定位服務的返回坐標，無需手動填寫經緯度。

## 專案簡介

WLOC 是一個用於修改 Apple 網絡定位服務的工具，能夠虛擬化 iOS 設備的地理位置。用戶可以透過選擇地圖上的位置，並使用快捷指令來設置或恢復定位。具體來說，使用者在地圖 App 中選擇位置後，透過共享功能調用 `wloc 设置地理位置` 快捷指令，系統會將選定的經緯度發送至代理服務，然後由代理模組攔截並修改 WLOC 的返回數據。這樣，系統就能夠使用虛擬的地理位置，而非實際的 GPS 定位。這個工具的最大賣點在於其簡便性和高效性，無需手動輸入經緯度，並且支持多種代理工具如 Surge、Quantumult X、Loon 和 Stash。

技術上，WLOC 使用 JavaScript 實現，並依賴於 MITM（中間人攻擊）技術來攔截和修改網絡請求。這意味著用戶需要信任 `gs-loc.apple.com`，並確保代理工具已啟用。

效能方面，該工具能夠快速響應用戶的定位請求，並且不會存儲任何用戶數據，保證隱私安全。

與其他類似工具相比，WLOC 在操作上更為簡單，因為它不需要用戶手動配置複雜的參數，並且提供了便捷的快捷指令支持。相比之下，像 Shadowrocket 這類工具則需要更多的手動設置，可能對於新手不太友好。此外，WLOC 也支持高德地圖的短鏈解析，這在其他工具中並不常見。

不過，使用者需要注意的是，iOS 26 及以上版本對定位緩存的強化可能導致定位結果不即時更新，這需要重啟設備來清除緩存。整體而言，WLOC 是一個針對需要虛擬定位的用戶設計的高效工具，適合個人用戶和小型團隊使用。

**技術棧**：`JavaScript`

## 重點功能

- 快捷指令支持 — 透過快捷指令一鍵設置或恢復地理位置，無需手動輸入經緯度。
- 多平台支持 — 支持 Surge、Quantumult X、Loon 和 Stash 等多種代理工具，方便用戶選擇。
- 在線選點功能 — 提供在線選點頁面，讓用戶可以輕鬆選擇需要的虛擬位置。
- 隱私保護 — 所有請求均不存儲用戶數據，確保用戶隱私安全。
- 自部署選項 — 用戶可以選擇自行部署 Worker，避免公共服務的請求限制。
...
2. 設置位置
```bash
在地圖 App 中選擇位置後，使用快捷指令 'wloc 设置地理位置'
```
3. 清理位置
```bash
使用快捷指令 'wloc 清理恢复位置'
...
> [!tip] 爆紅原因
> 建立 2 天內累積 457 stars（229/天），forks 71（15.5%），顯示出強勁的增長潛力。作者 Yu9191 是一位活躍的開發者，專注於網絡定位技術，這個專案解決了 iOS 用戶在定位方面的痛點，特別是對於需要隱私保護的用戶。近期的推廣和社群討論也促進了其曝光率，特別是在 Telegram 和相關論壇上。這個工具的出現正好填補了市場上對於簡單易用的虛擬定位工具的需求，並且其開源特性也吸引了許多開發者的關注。

## 適合誰使用

**目標受眾**：需要在 iOS 設備上虛擬定位但不想手動輸入經緯度的普通用戶。

> [!example] 使用場景
> - 獨立開發者用它來在測試環境中虛擬定位，因為這樣可以避免實際位置暴露，並且能快速切換測試場景。
> - 小型團隊的測試工程師用它來模擬不同地理位置的用戶行為，因為這樣能夠有效驗證應用在不同地區的功能。
> - 隱私意識強的用戶用它來隱藏真實位置，因為這樣可以在使用地圖服務時保護個人隱私。

## 架構分析

WLOC 的架構基於 JavaScript，使用 MITM 技術來攔截和修改網絡請求。其核心流程包括：用戶在地圖 App 中選擇位置，透過快捷指令將經緯度發送至代理服務，代理服務攔截 WLOC 的請求並修改返回數據。這種設計使得用戶無需手動配置複雜的參數，簡化了使用流程。選擇 MITM 技術的代價是需要用戶信任代理服務，並且對於不熟悉的用戶可能存在設置上的挑戰。擴展性方面，由於使用了開源的 Worker，使用者可以自行部署以避免公共服務的請求限制，這為需要高頻使用的用戶提供了靈活性。

## 技術深入分析

WLOC 的核心技術機制基於 JavaScript，利用 MITM 技術來攔截和修改網絡請求。當用戶選擇地圖上的位置時，系統會將經緯度發送至代理服務，然後由代理模組攔截 WLOC 的返回數據，實現虛擬定位。這種設計使得用戶無需手動輸入經緯度，並且支持多種代理工具，增加了靈活性。效能方面，WLOC 能夠快速響應用戶的定位請求，並且不會存儲任何用戶數據，這對於隱私保護至關重要。選擇 MITM 技術的代價是需要用戶信任代理服務，並且對於不熟悉的用戶可能存在設置上的挑戰。技術風險方面，iOS 26 及以上版本對定位緩存的強化可能導致定位結果不即時更新，這需要重啟設備來清除緩存。整合方面，WLOC 可以與主流的代理工具（如 Surge 和 Quantumult X）無縫集成，並且支持快捷指令，這使得其在用戶體驗上更為友好。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了詳細的使用說明和快捷指令的設置，安裝過程相對順暢，但對於不熟悉 MITM 的用戶可能會有一定的學習曲線。整體而言，花 30 分鐘內應該能夠順利運行。
...
> - 需要一定的技術背景來設置代理工具。
> - 在 iOS 26 及以上版本中需要重啟設備來清除定位緩存。

> [!warning] 注意事項
> - 僅支持 iOS 設備，無法在 Android 或其他平台使用。
> - 需要 MITM 代理工具的支持，對於不熟悉的用戶可能存在設置難度。
> - iOS 26 及以上版本需要重啟設備才能清除定位緩存，這可能影響使用體驗。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供虛擬定位功能，但需要更複雜的配置和設置，對新手不太友好。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 雖然也支持虛擬定位，但主要針對開發者，缺乏 WLOC 的便捷性和用戶友好性。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 使用 Rust 實作，記憶體用量少，但功能設置較為複雜。 | 如果你的團隊已經在使用 Rust 並需要高效能的虛擬定位工具。 | medium，因為需要重新學習 Rust 的配置方式。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 基於 Python 實作，易於擴展，但性能相對較低。 | 如果你的團隊對 Python 更為熟悉，並且需要更多的擴展性。 | low，因為 Python 的學習曲線相對較平緩。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **wloc** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 使用 Rust 實作，記憶體用量少，但功能設置較為複雜。 | 基於 Python 實作，易於擴展，但性能相對較低。 |
> | 遷移成本 | - | medium，因為需要重新學習 Rust 的配置方式。 | low，因為 Python 的學習曲線相對較平緩。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用 Rust 並需要高效能的虛擬定位工具 | 如果你的團隊對 Python 更為熟悉，並且需要更多的擴展性 |
...
- **[HIGH]** iOS 26 及以上版本需要重啟設備才能清除定位緩存
  - 解法：重啟設備以清除緩存，無法通過其他方式清除。
- [MEDIUM] 使用高德地圖時，短鏈解析可能失敗
  - 解法：確保代理工具已啟用，並檢查網絡連接。
- [MEDIUM] 在某些情況下，快捷指令可能無法正常執行
  - 解法：檢查快捷指令設置，確保已授權訪問定位服務。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 需要在測試環境中虛擬定位的開發者 | 非常適合 | 因為其簡便的設置流程和快捷指令支持。 |
| 隱私意識強的用戶 | 適合 | 因為其不存儲用戶數據的設計。 |
| 大型企業的生產環境 | 不適合 | 因為目前仍處於 alpha 階段，穩定性不足。 |
...
> [!tip] 投入 vs 回報
> 花 2 小時學習，1 小時整合，得到隱私保護和虛擬定位的效果，值得一試。
...
相關專案：[[0x0funky--agent-sprite-forge|0x0funky/agent-sprite-forge]] · [[0xGF--boneyard|0xGF/boneyard]] · [[loc567--loc567|loc567/loc567]] · [[BigBodyCobain--Shadowbroker|BigBodyCobain/Shadowbroker]] · [[BohemiaInteractive--CWR|BohemiaInteractive/CWR]]
