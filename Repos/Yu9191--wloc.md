...
**5.3k** stars · **239** stars/天 · 建立 22 天前 · JavaScript · 未標註授權
...
> **適合** 需要在 iOS 設備上進行虛擬定位的用戶，特別是遊戲玩家和開發者。
> **一句話重點** WLOC 讓虛擬定位變得簡單易用，特別適合需要頻繁切換位置的用戶。
...
> **結論** 花 2 小時學習，3 小時整合，得到靈活的虛擬定位功能，值得考慮。

> [!abstract] 核心創新
> WLOC 提供了一種簡單的方式來修改 Apple 網路定位返回的坐標，並支持多種代理工具。

## 專案簡介

WLOC 透過修改 Apple 的網路定位服務（gs-loc）返回的坐標，實現 iOS 設備的虛擬定位。用戶可以在地圖應用中選擇位置，透過快捷指令或代理工具輕鬆設置虛擬位置，無需手動填寫經緯度。主要指令包括 `wloc 设置地理位置` 和 `wloc 清理恢复位置`，這樣用戶能快速切換或恢復真實位置。技術上，WLOC 使用 JavaScript 實現，並依賴於代理工具（如 Surge、Quantumult X、Loon 和 Stash）來攔截和修改 WLOC 請求，將用戶選擇的虛擬坐標寫入持久化存儲。這種設計使得用戶能夠在不改變 GPS 硡體定位的情況下，靈活地進行網路定位。

與其他工具相比，WLOC 的優勢在於其支持多種代理工具，並且提供了簡單的快捷指令來進行操作，這使得它在使用上更加方便。實際使用中，WLOC 需要用戶信任特定的網路請求，並且在 iOS 26 及以上版本中，可能需要重啟設備以清除定位緩存，這是使用時的一個注意事項。整體來看，WLOC 是一個穩定的解決方案，適合需要靈活定位的用戶，尤其是在需要頻繁切換位置的場景。未來可能會進一步增強對更多地圖服務的支持，並優化用戶體驗。

**技術棧**：`JavaScript`

## 重點功能

- 支持多種代理工具 — 包括 Surge、Quantumult X、Loon 和 Stash，方便用戶根據需求選擇。
- 快捷指令一鍵設置 — 用戶可以通過快捷指令快速設置和清理虛擬位置，提升使用效率。
- 在線選點頁面 — 提供便捷的選點功能，支持用戶直接選擇地圖位置。
- 支持坐標轉換 — 自動將 GCJ-02 坐標轉換為 WGS84，確保定位準確性。
- 隱私保護 — 解析過程不存儲任何用戶數據，確保用戶隱私安全。
...
> [!tip] 爆紅原因
> 建立 22 天內累積 5267 stars（239/天），forks 992（18.8%），顯示出強烈的社群參與。作者 Yu9191 之前在 GitHub 上有其他開源項目，這次的 WLOC 解決了 iOS 用戶在定位上的痛點，特別是需要虛擬定位的需求。近期的社群討論和教程也促進了使用者的關注，特別是針對 Shadowrocket 的使用指南。這些因素共同推動了 WLOC 的快速增長。

## 適合誰使用

**目標受眾**：需要在 iOS 設備上進行虛擬定位的用戶，特別是遊戲玩家和開發者。

> [!example] 使用場景
> - 獨立開發者用它來在測試環境中虛擬定位，因為這樣可以避免在真實位置上進行測試，節省時間和資源。
> - 遊戲玩家用它來在 AR 遊戲中虛擬定位，因為這樣可以在不出門的情況下體驗遊戲的不同地點。
> - 市場調查員用它來模擬不同地理位置的用戶行為，因為這樣可以更有效地收集數據，提升調查的準確性。

## 架構分析

WLOC 的架構基於 JavaScript，通過代理工具攔截網路請求，並在用戶選擇位置後將其寫入持久化存儲。使用者透過快捷指令或在線選點頁面進行操作，這樣的設計使得用戶能夠快速設置虛擬位置。資料流從用戶選擇位置開始，經過代理工具的攔截，最終返回修改後的坐標。這種設計的代價是需要用戶信任特定的網路請求，並且在某些 iOS 版本中可能需要重啟設備來清除緩存。整體來看，這種架構能夠靈活應對不同的使用場景，但在高版本 iOS 中可能會面臨定位緩存的問題。

## 技術深入分析

WLOC 的核心技術機制是通過 JavaScript 攔截 Apple 的網路定位請求，並修改返回的坐標。這種方式利用了代理工具的能力，能夠在不改變 GPS 硬體定位的情況下，靈活地進行網路定位。效能上，WLOC 能夠快速響應用戶的請求，並且支持多種地圖服務的坐標轉換。設計上，選擇 JavaScript 是因為其在網路請求處理上的靈活性和廣泛支持。

依賴樹相對簡單，主要依賴於代理工具的功能，這使得 WLOC 的整體架構較為輕量。技術風險方面，WLOC 可能在高版本 iOS 中面臨定位緩存的問題，這需要用戶重啟設備來解決。此外，對於外部 API 的依賴程度較高，這可能會影響到未來的穩定性。整合分析方面，WLOC 能夠與主流的代理工具良好整合，並且在 CI/CD pipeline 中的使用也相對簡單，適合快速部署和測試。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了詳細的使用說明和範例；安裝過程相對順暢，但需要用戶具備一定的代理工具使用經驗；有良好的 getting started guide，適合新手快速上手；文件目前僅提供中文，可能對非中文用戶造成一定困難。
...
> - 僅支持 iOS 設備，對於 Android 用戶不適用。

> [!warning] 注意事項
> - 僅支持 iOS 設備，無法在 Android 上使用。
> - iOS 26 及以上版本需重啟設備以清除定位緩存。
> - 需要信任特定的網路請求，否則無法正常運作。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供虛擬定位功能，但主要針對不同的代理環境，使用上較為複雜。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於網路請求的攔截和修改，功能較為廣泛，但不專注於虛擬定位。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 使用 Rust 實作，記憶體使用量較低，但生態系統不如 JavaScript 豐富。 | 如果你的團隊已經在使用 Rust 並需要更高效的資源管理。 | medium，因為需要重新學習 Rust 的語法和生態。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於網路請求的攔截和修改，功能較為廣泛，但不專注於虛擬定位。 | 如果需要更全面的網路請求管理功能，而不僅僅是虛擬定位。 | high，因為需要重新設計整個請求處理流程。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **wloc** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 使用 Rust 實作，記憶體使用量較低，但生態系統不如 JavaScript 豐富。 | 專注於網路請求的攔截和修改，功能較為廣泛，但不專注於虛擬定位。 |
> | 遷移成本 | - | medium，因為需要重新學習 Rust 的語法和生態。 | high，因為需要重新設計整個請求處理流程。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用 Rust 並需要更高效的資源管理。 | 如果需要更全面的網路請求管理功能，而不僅僅是虛擬定位。 |
...
> [!tip] 採用建議
> 適合個人使用或小型項目，但不建議用於生產環境的核心功能。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在 iOS 26 及以上版本中，定位緩存可能導致修改後的坐標無法立即生效
  - 解法：重啟設備以清除緩存
- [MEDIUM] 需要信任 gs-loc.apple.com，否則無法正常運作
  - 解法：在代理工具中設置 MITM 以信任該請求
- [MEDIUM] 如果在模塊參數中手動修改經緯度，清除持久化數據後仍會使用手動設置的坐標
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 需要在測試環境中虛擬定位的開發者 | 非常適合 | 簡單的設置和清理功能，能夠快速進行測試。 |
| 遊戲玩家需要在 AR 遊戲中虛擬定位 | 適合 | 支持多種地圖服務，能夠靈活切換位置。 |
| 市場調查員需要模擬不同地理位置的用戶行為 | 普通 | 雖然能夠提供虛擬定位，但可能需要考慮緩存問題。 |
...
> [!tip] 投入 vs 回報
> 花 2 小時學習，3 小時整合，得到靈活的虛擬定位功能，值得考慮。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：WLOC 本身不需要高權限，且不存取敏感資料。依賴的網路請求需信任，確保安全性。
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["虛擬定位","網路請求攔截","地理位置服務"];
