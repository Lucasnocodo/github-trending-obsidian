...
star_history: "2026-06-27:457,2026-06-27:462,2026-06-28:644,2026-06-28:647,2026-06-29:1092,2026-06-29:1098,2026-06-30:1508,2026-06-30:1515,2026-07-01:1965,2026-07-01:1968,2026-07-02:2357,2026-07-03:2584,2026-07-04:2845"
tags:
  - github
  - "category/其他"
  - "lang/javascript"
...
**2.8k** stars · **316** stars/天 · 建立 9 天前 · JavaScript · 未標註授權
...
> **一句話重點** WLOC 讓 iOS 用戶能夠輕鬆修改定位，特別適合需要隱私保護的場景。
...
> **結論** 花 3 小時學習，2 小時整合，能夠靈活修改定位，值得一試。

> [!abstract] 核心創新
> 提供了一個簡單的方式來修改 Apple 網絡定位，支持多種代理工具和快捷指令。

## 專案簡介

WLOC 是一個用於修改 Apple 網絡定位服務的工具，通過代理模組來實現虛擬定位。用戶可以在地圖應用中選擇位置，然後通過快捷指令一鍵設置該位置，無需手動輸入經緯度。此工具支持 Surge、Quantumult X、Loon 和 Stash 等多種代理工具，並提供了簡單的 API 解析服務來處理地圖鏈接。WLOC 的核心賣點在於其便捷性和高效性，能夠快速修改定位而不影響 GPS 硬件定位。技術上，它使用 JavaScript 實現，並通過 MITM 技術來攔截和修改網絡請求，這樣可以直接在用戶的設備上進行定位修改。

與其他定位工具相比，WLOC 的優勢在於其支持多種代理工具，並且能夠在不同的地圖服務（如 Apple 地圖和高德地圖）中統一處理定位數據。使用者在使用過程中可能會遇到定位維持時間短的問題，這是由於 iOS 的定位緩存機制所致，重啟設備可解決此問題。這個專案目前處於穩定階段，適合需要經常修改定位的用戶，特別是開發者和測試人員。未來可能會增加更多的功能來支持不同的定位需求。

**技術棧**：`JavaScript`

## 重點功能

- 多代理支持 — 支持 Surge、Quantumult X、Loon 和 Stash 等多種代理工具。
- 快捷指令集成 — 提供一鍵設置和清理位置的快捷指令，簡化操作流程。
- 在線選點 — 用戶可通過在線選點頁面選擇位置，無需手動輸入經緯度。
- API 解析服務 — 提供鏈接解析服務，支持 Apple 地圖和高德地圖的坐標轉換。
- 隱私保護 — 所有請求均不記錄，確保用戶隱私安全。

## 快速開始

1. 訂閱代理模組
```bash
curl -O https://raw.githubusercontent.com/Yu9191/wloc/refs/heads/main/modules/wloc.sgmodule
```
2. 啟用 MITM
```bash
在代理工具中設置 MITM 以信任 gs-loc.apple.com
```
3. 使用快捷指令設置位置
```bash
...
{
  "前置條件": "已安裝代理工具並啟用相應模組",
  "指令": "curl -X GET 'https://gs-loc.apple.com/wloc-settings/save?lon=113.94114&lat=22.544577'",
  "預期輸出": "返回 JSON 格式的修改結果，顯示坐標已成功設置。"
}
```
...
> [!tip] 爆紅原因
> 建立 9 天就累積 2845 stars（316/天），forks 400（14.1%），這顯示出其在用戶中的高需求。作者 Yu9191 之前有其他開源項目，這次專案解決了 iOS 用戶在使用地圖時的定位限制，特別是在需要隱私或測試的情境下。社群對於如何使用 Shadowrocket 進行設定的討論也引發了廣泛關注，顯示出實際使用中的需求和痛點。技術上，這個工具的出現得益於網絡代理技術的成熟，讓用戶能夠輕鬆地修改定位信息。forks/stars 比率為 14.1%，顯示出許多人在實際修改和使用這個工具。
...
> [!example] 使用場景
> - 測試工程師用它來在不同地點模擬應用行為，因為這樣可以快速驗證地理位置相關功能的正確性。
> - 開發者用它來測試應用在特定地理位置的表現，因為手動修改定位繁瑣且容易出錯。
> - 隱私意識強的用戶用它來隱藏真實位置，因為這樣可以在使用地圖服務時保護個人隱私。

## 架構分析

WLOC 的架構基於 JavaScript，利用 MITM 技術攔截網絡請求來修改定位信息。當用戶在地圖應用中選擇位置後，該位置的經緯度會通過 API 解析服務轉換為 WGS84 坐標，並寫入持久化存儲。這種設計使得用戶無需手動輸入經緯度，簡化了操作流程。選擇 MITM 技術的好處在於能夠實時修改請求，代價是需要用戶信任代理工具的安全性。擴展性方面，該工具可以輕鬆集成到不同的代理工具中，但在高流量時可能會面臨性能瓶頸。

## 技術深入分析

WLOC 使用 JavaScript 實現，並利用 MITM 技術來攔截和修改網絡請求。其核心機制是通過在線選點頁面來獲取用戶選擇的經緯度，然後將這些數據寫入持久化存儲，當系統發起定位請求時，WLOC 會讀取這些數據並修改返回的坐標。這樣的設計使得用戶可以無需手動輸入經緯度，簡化了操作流程。性能方面，該工具在正常使用情況下能夠快速響應用戶請求，但在高流量時可能會面臨性能瓶頸。選擇 JavaScript 作為開發語言的好處在於其易於集成和擴展，但也可能導致較高的內存使用率。技術風險方面，若未來 Apple 改變定位服務的接口，可能會導致工具失效。整合方面，該工具能夠與主流的代理工具無縫對接，並且支持快捷指令的使用，降低了用戶的學習成本。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且提供了詳細的使用說明，安裝過程相對順暢，但需要用戶理解 MITM 的基本概念。提供的快捷指令大大簡化了使用流程，適合新手快速上手。文件目前僅提供中文版本，可能對非中文用戶造成一定障礙。
...
> - 僅支持 iOS 設備，無法在其他平台使用。
> - 對於 GPS 信號強的情況，可能會影響定位效果。

> [!warning] 注意事項
> - 僅支持 iOS 設備，無法在 Android 上使用。
> - 需要重啟設備以清除定位緩存，否則可能無法立即生效。
> - 對於 GPS 信號強的情況，可能會忽略網絡定位結果。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供類似的虛擬定位功能，但主要針對遊戲環境，並不支持多種代理工具。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於隱私保護的定位工具，功能較為單一，無法提供在線選點功能。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於遊戲環境的虛擬定位，使用不同的技術來實現定位修改。 | 如果你的主要需求是針對遊戲進行定位修改，而非日常應用。 | medium，因為需要重新配置不同的代理和設定。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於隱私保護的定位工具，功能較為單一。 | 如果你的主要需求是隱私保護，且不需要複雜的定位修改功能。 | low，因為功能較為簡單，易於上手。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **wloc** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於遊戲環境的虛擬定位，使用不同的技術來實現定位修改。 | 專注於隱私保護的定位工具，功能較為單一。 |
> | 遷移成本 | - | medium，因為需要重新配置不同的代理和設定。 | low，因為功能較為簡單，易於上手。 |
> | 適用場景 | 主要場景 | 如果你的主要需求是針對遊戲進行定位修改，而非日常應用。 | 如果你的主要需求是隱私保護，且不需要複雜的定位修改功能。 |
...
> [!tip] 採用建議
> 適合個人使用或小型團隊的測試需求，不建議用於生產環境的核心路徑上。
...
- [MEDIUM] 使用高德地圖時，短鏈解析可能會失敗
  - 解法：確保代理工具正常運行，並檢查 MITM 設置
- [MEDIUM] 若手動修改經緯度，清除持久化數據後仍會使用舊坐標
  - 解法：保持默認參數不變，然後清除持久化數據

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 需要經常修改定位的測試團隊 | 非常適合 | 工具支持多種代理，並且操作簡單。 |
| 開發者需要在不同地點測試應用 | 適合 | 能夠快速設置和清理位置，節省時間。 |
| 普通用戶希望隱藏真實位置 | 普通 | 雖然功能強大，但需要理解 MITM 的設置。 |
| 不熟悉代理工具的用戶 | 不適合 | 需要一定的技術背景來配置和使用。 |
...
> [!tip] 投入 vs 回報
> 花 3 小時學習，2 小時整合，能夠靈活修改定位，值得一試。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限，並且不存取敏感資料，但用戶需信任代理工具的安全性。
...
> | --- | --- | --- | --- |
> | [#35](https://github.com/Yu9191/wloc/issues/35) | Shadowrocket设置使用教程 | 8 | 22 |
> | [#39](https://github.com/Yu9191/wloc/issues/39) | Shadowrocket+快捷指令（秒成功）操作流程图文记录 | 4 | 10 |
> | [#40](https://github.com/Yu9191/wloc/issues/40) | ios26用loon存储位置文件失败，报错404 | 0 | 1 |
