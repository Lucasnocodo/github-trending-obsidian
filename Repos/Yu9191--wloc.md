...
**6.3k** stars · **211** stars/天 · 建立 30 天前 · JavaScript · 未標註授權
...
> **適合** 需要在 Apple 服務中靈活修改地理位置的普通用戶和開發者。
> **一句話重點** WLOC 讓虛擬定位變得前所未有的簡單，特別是對於普通用戶而言。
...
> [!abstract] 核心創新
> 提供了一個簡單的快捷指令來自動化 Apple 網絡定位的修改過程。

## 專案簡介

WLOC 是一個用於修改 Apple 網絡定位服務返回坐標的工具，支持 Surge、Quantumult X、Loon 和 Stash 四種代理工具。用戶可以通過選擇地圖上的位置來設置虛擬定位，並且可以使用快捷指令輕鬆切換或清除定位。這個工具的核心機制是通過代理模組攔截 WLOC 請求，並在響應中替換坐標，從而實現虛擬定位。使用者只需在地圖應用中選擇位置，然後分享給 WLOC，即可完成設置。這樣的設計使得用戶無需手動輸入經緯度，簡化了操作流程。

WLOC 依賴於 JavaScript，並且是輕量級的，沒有過多的依賴項，便於快速部署。與其他類似工具相比，WLOC 提供了更為直觀的使用體驗，特別是在設定和清除定位方面，使用快捷指令的方式大大降低了使用門檻。相比於 Shadowrocket 和其他工具，WLOC 的設計更注重用戶的便捷性，並且支持多種地圖服務的解析，這使得它在多種場景下都能有效運行。使用者在高版本 iOS 上可能會遇到定位緩存的問題，這需要重啟設備來清除緩存。整體來看，WLOC 是一個穩定且易於使用的虛擬定位工具，適合需要頻繁更改地理位置的用戶。

**技術棧**：`JavaScript`

## 重點功能

- 虛擬定位 — 通過代理模組修改 Apple 網絡定位返回的坐標。
- 快捷指令支持 — 提供一鍵設置和清除位置的快捷指令，簡化操作流程。
- 支持多種代理工具 — 兼容 Surge、Quantumult X、Loon 和 Stash，靈活應用。
- 在線選點頁面 — 用戶可通過網頁選擇位置，並自動生成定位設置。
- 隱私保護 — 全程不記錄用戶數據，確保用戶隱私。
...
> [!tip] 爆紅原因
> 建立 30 天內累積 6316 stars（211/天），forks 1253（19.8%），顯示出強烈的用戶需求和活躍的開發社群。作者 Yu9191 過去在類似工具上有豐富的開發經驗，這個專案解決了用戶在 Apple 服務中無法靈活修改定位的痛點。之前的解決方案往往需要繁瑣的手動操作，而 WLOC 提供了一個簡單的快捷指令來自動化這一過程。社群中對於如何使用 Shadowrocket 和 WLOC 的討論熱烈，顯示出這個工具的實用性和受歡迎程度。

## 適合誰使用

**目標受眾**：需要在 Apple 服務中靈活修改地理位置的普通用戶和開發者。

> [!example] 使用場景
> - 普通用戶用它來在 Apple 地圖上快速設置虛擬地理位置，因為手動輸入經緯度繁瑣且容易出錯。
> - 開發者用它來測試基於地理位置的應用，因為可以快速切換不同的定位而不需要重啟應用。
> - 測試人員用它來模擬不同地理位置的用戶行為，因為這樣可以在不離開辦公室的情況下進行多地點測試。

## 架構分析

WLOC 的架構設計以輕量級的 JavaScript 模組為基礎，主要通過代理工具攔截網絡請求來實現定位修改。當用戶在地圖上選擇位置時，系統會將該位置的經緯度信息傳遞給代理模組，然後通過 WLOC 的設定腳本將這些信息寫入持久化存儲。這樣的設計使得用戶可以在不手動輸入經緯度的情況下快速修改定位。

選擇 JavaScript 作為開發語言是因為其在網絡環境中的廣泛應用，並且易於與各種代理工具集成。這種架構的代價是需要用戶在代理工具中進行一定的配置，對於不熟悉技術的用戶來說可能會有一定的學習曲線。整體來看，WLOC 的架構設計在保持輕量的同時，提供了靈活的擴展性，能夠支持多種不同的地圖服務和定位需求。

## 技術深入分析

WLOC 的核心技術機制是通過代理模組攔截 Apple 的定位請求，並在響應中修改返回的坐標。這一過程涉及到對 protobuf 格式的解析和修改，確保用戶能夠獲得正確的虛擬定位。這個工具的設計使得用戶無需手動輸入經緯度，僅需在地圖上選擇位置即可。效能方面，由於 WLOC 是輕量級的 JavaScript 應用，對系統資源的需求相對較低，能夠在大多數設備上流暢運行。

設計上，選擇 JavaScript 作為開發語言使得其能夠輕鬆與各種代理工具集成，但這也意味著需要用戶具備一定的技術背景來進行配置。技術風險方面，WLOC 依賴於 Apple 的網絡定位服務，如果 Apple 改變了其 API 或數據格式，可能會導致工具失效。此外，對於高版本 iOS 的用戶，定位緩存問題可能會影響使用體驗，這需要用戶重啟設備來解決。整體來看，WLOC 在設計上兼顧了易用性和功能性，適合需要靈活定位的用戶。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了詳細的使用說明和範例；安裝過程相對順暢，但需要用戶具備一定的技術背景；有提供快捷指令的使用指南，方便新手上手；文件目前僅提供中文，對於非中文用戶可能不太友好。

## 優缺點分析

> [!success] 優點
> - 操作簡單，通過快捷指令即可快速設置和清除定位。
> - 支持多種代理工具，靈活性高。
> - 隱私保護良好，無需記錄用戶數據。

> [!danger] 缺點
> - 需要一定的技術背景來配置代理工具。
> - 在高版本 iOS 上可能需要重啟設備以清除緩存。
> - 僅支持特定的地圖服務，限制了使用範圍。

> [!warning] 注意事項
> - 僅支持 Apple 地圖和高德地圖的虛擬定位。
> - 在 iOS 26 及以上版本中，可能需要重啟設備才能清除定位緩存。
> - 不支持 GPS 硬件定位的修改，僅修改網絡定位結果。
> - 需要信任 MITM 證書以正常運行。
...
| 工具 | 差異 |
| --- | --- |
| [FFF686868/proxypin-wloc-spoofer](https://github.com/FFF686868/proxypin-wloc-spoofer) | 提供類似的定位修改功能，但可能不如 WLOC 直觀易用。 |
| [Shadowrocket/Shadowrocket](https://github.com/Shadowrocket/Shadowrocket) | 雖然功能相似，但 WLOC 提供了更簡單的快捷指令支持，適合普通用戶。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [FFF686868/proxypin-wloc-spoofer](https://github.com/FFF686868/proxypin-wloc-spoofer) | 提供類似的定位修改功能，但可能不如 WLOC 直觀易用。 | 如果你需要一個功能類似但不需要快捷指令的工具，可以考慮這個選項。 | medium，因為需要重新配置代理模組。 |
| [Shadowrocket/Shadowrocket](https://github.com/Shadowrocket/Shadowrocket) | 雖然功能相似，但 WLOC 提供了更簡單的快捷指令支持，適合普通用戶。 | 如果你的團隊已經在使用 Shadowrocket 且熟悉其配置，則可以考慮繼續使用。 | low，因為兩者的配置方式相似。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **wloc** | **proxypin-wloc-spoofer** | **Shadowrocket** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 提供類似的定位修改功能，但可能不如 WLOC 直觀易用。 | 雖然功能相似，但 WLOC 提供了更簡單的快捷指令支持，適合普通用戶。 |
> | 遷移成本 | - | medium，因為需要重新配置代理模組。 | low，因為兩者的配置方式相似。 |
> | 適用場景 | 主要場景 | 如果你需要一個功能類似但不需要快捷指令的工具，可以考慮這個選 | 如果你的團隊已經在使用 Shadowrocket 且熟悉其配 |
...
- **[HIGH]** 在 iOS 26 及以上版本中，定位緩存可能導致修改後的坐標不生效
  - 解法：需要重啟設備以清除緩存
- [MEDIUM] 如果在模組參數中手動修改經緯度，清除持久化數據後仍會使用模組參數中的坐標
  - 解法：保持模組參數不變，僅清除持久化數據
- [MEDIUM] 需要信任 MITM 證書才能正常運行
  - 解法：確保已經信任相關證書
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 需要頻繁更改地理位置的普通用戶 | 非常適合 | 操作簡單，支持快捷指令，無需手動輸入經緯度。 |
| 開發者需要在測試中模擬不同的地理位置 | 適合 | 能快速切換定位，方便測試。 |
| 不熟悉技術的用戶 | 普通 | 需要一定的技術背景來配置代理工具。 |
| 需要長期使用虛擬定位的商業用戶 | 不適合 | 目前仍處於 beta 階段，穩定性不足。 |
...
> | [#71](https://github.com/Yu9191/wloc/issues/71) | 关于使用中的一个问题 | 0 | 0 |
