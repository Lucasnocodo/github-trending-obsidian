...
star_history: "2026-06-27:457,2026-06-27:462,2026-06-28:644,2026-06-28:647,2026-06-29:1092,2026-06-29:1098,2026-06-30:1508,2026-06-30:1515,2026-07-01:1965,2026-07-01:1968"
tags:
  - github
  - "category/其他"
  - "lang/javascript"
...
**2.0k** stars · **328** stars/天 · 建立 6 天前 · JavaScript · 未標註授權
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Brand New · **熱度** Hot (328 stars/day)
> **授權** 未標註授權 (風險較高) · **維護** Active (最後推送 3 天前) · **貢獻者** Solo (bus factor 風險) · **參與度** Medium
> **適合** 需要在 iOS 設備上虛擬定位但不想手動調整的普通用戶和開發者。
> **一句話重點** WLOC 讓虛擬定位變得簡單，特別是對於需要頻繁改變位置的 iOS 用戶。
...
> [!abstract] 核心創新
> WLOC 提供了一個簡單的解決方案來修改 Apple 網絡定位的返回坐標，支持多種代理工具和快捷指令。

## 專案簡介

WLOC 主要功能是修改 Apple 网络定位服务（gs-loc.apple.com）返回的坐标，實現 iOS 的虛擬定位。用戶可以通過在線選點頁面選擇位置，然後使用快捷指令快速設置或恢復定位。具體流程是：用戶在地圖應用中選擇位置，然後共享至 WLOC 進行設置，系統會通過代理模塊攔截並修改 WLOC 的響應，返回用戶指定的經緯度。這樣的設計使得用戶無需手動填寫經緯度，提升了便利性。核心賣點在於支持多種代理工具（如 Surge、Quantumult X、Loon 和 Stash），並提供快捷指令以簡化操作。

技術上，WLOC 使用 JavaScript 實現，並依賴於 MITM 代理來攔截和修改網絡請求，這樣的設計使得其能夠在不影響 GPS 硬件定位的情況下，專注於 WiFi 和基站的定位。與其他虛擬定位工具相比，WLOC 的優勢在於其簡單的設置流程和對多種工具的支持，特別適合需要頻繁變更位置的用戶。使用者在高版本 iOS 上可能會遇到定位緩存問題，需重啟設備來清除緩存。這個專案目前處於穩定階段，適合個人使用或小型團隊進行測試和實驗。

**技術棧**：`JavaScript`

## 重點功能

- 多種代理支持 — 兼容 Surge、Quantumult X、Loon 和 Stash，滿足不同用戶需求。
- 快捷指令一鍵設置 — 透過快捷指令快速設置和清理位置，提升使用便利性。
- 在線選點頁面 — 用戶可在網頁上選擇位置，無需手動輸入經緯度。
- 支持 GCJ-02 到 WGS84 轉換 — 自動處理中國大陸的坐標偏差問題，確保準確性。
- 純轉發解析 — 使用 worker 進行坐標解析，不記錄用戶數據，保護隱私。

## 快速開始

1. 訂閱代理模塊
```bash
在 Surge 中添加模塊地址 https://raw.githubusercontent.com/Yu9191/wloc/refs/heads/main/modules/wloc.sgmodule
```
2. 設置位置
```bash
...
> [!tip] 爆紅原因
> 建立 6 天就累積 1968 stars（328/天），forks 264（13.4%），顯示出用戶對於虛擬定位需求的強烈反響。作者 Yu9191 是一位活躍的開發者，專注於網絡定位和代理工具的開發。這個專案解決了在 iOS 系統中，使用者需要頻繁更改位置卻缺乏簡單有效工具的痛點。之前的解決方案往往需要手動操作，且不夠直觀。最近的社群討論和使用者反饋也促進了這個專案的曝光和使用。技術上，隨著 MITM 代理技術的普及，使得這種定位修改變得可行，並且能夠在多種環境下運行。forks/stars 比率為 13.4%，顯示出相對較高的實際使用和修改需求。

## 適合誰使用

**目標受眾**：需要在 iOS 設備上虛擬定位但不想手動調整的普通用戶和開發者。

> [!example] 使用場景
> - iOS 開發者用它來測試應用的地理位置功能，因為能快速切換虛擬位置，節省了手動調整的時間。
> - 旅行者用它來在不同地區使用地圖應用，因為能在不實際移動的情況下設置當前地理位置，避免漫遊費用。
> - 測試人員用它來模擬不同地理位置的使用情境，因為能夠在多個位置之間快速切換，提升測試效率。

## 架構分析

WLOC 的架構基於 MITM 代理技術，通過攔截和修改網絡請求來實現虛擬定位。用戶首先在地圖應用中選擇位置，然後通過 WLOC 的在線選點頁面發送請求。代理模塊攔截這些請求，並根據用戶設置的經緯度修改 WLOC 的響應。

這種設計使得用戶無需手動輸入坐標，提升了使用的便利性。選擇 MITM 代理的代價是需要用戶信任代理證書，並且在高版本 iOS 上可能會遇到定位緩存的問題。擴展性方面，WLOC 可以支持多種代理工具，但在不同工具間的兼容性可能會影響使用體驗。

## 技術深入分析

WLOC 的核心技術機制依賴於 MITM 代理技術，通過攔截和修改網絡請求來實現虛擬定位。具體而言，當用戶在地圖應用中選擇位置時，系統會發送請求至 gs-loc.apple.com，WLOC 的代理模塊會攔截這個請求，並根據用戶的設置返回修改後的經緯度。這樣的設計使得用戶無需手動輸入坐標，提升了便利性。效能方面，WLOC 在處理請求時的延遲相對較低，因為其依賴於快速的網絡請求轉發。設計取捨方面，選擇 MITM 代理的好處是能夠靈活地修改請求，但這也要求用戶信任代理證書，並且可能在高版本 iOS 上遇到定位緩存的問題。技術風險方面，若用戶不清楚如何配置代理，可能會導致定位失敗。整合方面，WLOC 與主流的代理工具（如 Surge 和 Quantumult X）相容性良好，但在不同工具間的兼容性可能會影響使用體驗。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了詳細的使用說明和範例。安裝過程相對順暢，但需要用戶具備一定的技術背景來設置代理。文檔中有快捷指令的鏈接，方便用戶快速上手。整體而言，花 30 分鐘能夠順利運行。
...
> [!danger] 缺點
> - 需要用戶信任 MITM 代理證書，存在安全風險。
> - 在 iOS 26+ 版本中可能需要重啟設備來清除定位緩存。
> - 對於不熟悉技術的用戶，初次設置可能有一定難度。

> [!warning] 注意事項
> - 僅支持 WiFi 和基站定位，不影響 GPS 硬件定位。
> - 在 iOS 26 及以上版本中，定位緩存可能導致修改無法立即生效，需重啟設備。
> - 需要 MITM 代理證書信任 gs-loc.apple.com 和 gs-loc-cn.apple.com。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供類似的虛擬定位功能，但主要針對遊戲環境，可能不支持多種代理工具。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於網絡請求的攔截和修改，但不專門針對定位功能，使用場景較窄。 |
...
> | --- | --- | --- | --- |
> | [#35](https://github.com/Yu9191/wloc/issues/35) | Shadowrocket设置使用教程 | 3 | 10 |
> | [#37](https://github.com/Yu9191/wloc/issues/37) | 用小火箭没弄成功 | 0 | 0 |
> | [#36](https://github.com/Yu9191/wloc/issues/36) | 偏差距離問題 | 0 | 0 |
> | [#34](https://github.com/Yu9191/wloc/issues/34) | 按照我的方法百分之100成功（系统26.3） | 0 | 35 |
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["虛擬定位","MITM 代理","坐標轉換"];
