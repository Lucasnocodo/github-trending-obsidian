...
use_case: "修改 Apple 网络定位返回坐标，支持多种代理工具，方便快捷地设置和恢复定位。"
priority: high
ring: assess
...
star_history: "2026-06-27:457,2026-06-27:462,2026-06-28:644,2026-06-28:647,2026-06-29:1092,2026-06-29:1098,2026-06-30:1508,2026-06-30:1515,2026-07-01:1965"
tags:
  - github
  - "category/開發工具"
  - "lang/javascript"
...
**1.5k** stars · **303** stars/天 · 建立 5 天前 · JavaScript · 未標註授權
...
> **適合** 需要在 iOS 上修改網絡定位但不想手動填寫經緯度的開發者和普通用戶。
> **一句話重點** WLOC 的設計不僅解決了定位的靈活性問題，還考慮了用戶的隱私，這在同類工具中是相對少見的。
...
> **結論** 花 2 小時學習，3 小時整合，得到靈活的虛擬定位功能，值得投入。

> [!abstract] 核心創新
> 支持多種代理工具的虛擬定位，並提供一鍵設置和恢復功能。

## 專案簡介

WLOC 是一個用於修改 Apple 網絡定位服務返回的座標的工具，主要針對 iOS 設計。用戶可以通過在線選點頁面選擇位置，然後使用快捷指令或代理工具（如 Surge、Quantumult X、Loon 和 Stash）來設置虛擬定位。這個工具的賣點在於其簡便性，使用者只需在地圖應用中選擇位置並分享，即可透過快捷指令設置新的地理位置。技術上，WLOC 透過攔截特定網絡請求來修改返回的座標，並使用 Worker 進行坐標解析，支持 GCJ-02 到 WGS84 的轉換。這意味著用戶不需要手動填寫經緯度，並且能夠在多個地圖服務中使用。

與其他類似工具相比，WLOC 提供了更簡單的用法和更高的靈活性，特別是在 iOS 26 及以上版本中，這些版本強化了定位緩存機制，WLOC 仍能有效運作。使用者需要注意的是，iOS 26 及以上版本可能需要重啟設備以清除舊的定位緩存。這個工具的設計考慮了用戶隱私，所有解析過程不會存儲任何數據，並且開源可自部署。對於需要在特定地點進行測試或使用特定服務的開發者來說，這是一個非常實用的工具。

**技術棧**：`JavaScript`

## 重點功能

- 虛擬定位 — 透過在線選點頁面選擇位置，無需手動填寫經緯度。
- 支持多種代理工具 — 包括 Surge、Quantumult X、Loon 和 Stash，方便用戶根據需求選擇。
- 快捷指令集成 — 提供一鍵設置和恢復位置的快捷指令，提升使用便捷性。
- 支持 GCJ-02 到 WGS84 的坐標轉換 — 確保在中國大陸使用的準確性。
- 開源自部署 — 用戶可自行部署 Worker，保障隱私和數據安全。
...
> [!tip] 爆紅原因
> 建立 5 天內累積 1515 stars（303/天），forks 197（13.0%），顯示出強烈的用戶需求和活躍度。作者 Yu9191 是一位活躍的開發者，專注於網絡定位和代理工具的開發，這個專案解決了 iOS 用戶在使用定位服務時的靈活性問題，特別是在需要隱藏真實位置的場景下。社群中對於如何在不同工具中使用的討論顯示出用戶在實際操作中的困惑，這也促進了專案的活躍度。

## 適合誰使用

**目標受眾**：需要在 iOS 上修改網絡定位但不想手動填寫經緯度的開發者和普通用戶。

> [!example] 使用場景
> - 開發者用它來在測試環境中模擬不同地理位置，因為這樣可以快速驗證應用在不同地點的行為。
> - 普通用戶用它來隱藏真實位置，方便使用地理限制的服務，因為這樣能夠安全地訪問受限內容。
> - 遊戲玩家用它來在 AR 遊戲中虛擬定位，因為這樣可以在不出門的情況下體驗不同的遊戲場景。

## 架構分析

WLOC 的架構設計主要基於 JavaScript，通過攔截網絡請求來修改定位數據。使用 Worker 進行坐標解析，這樣的設計使得用戶能夠靈活地選擇位置而不需要手動輸入經緯度。資料流從用戶選擇位置開始，經過 Worker 解析後返回修改的座標，這樣的設計考慮了用戶的隱私，因為不會存儲任何數據。選擇 JavaScript 作為開發語言使得部署和維護變得簡單，但也可能面臨性能瓶頸，特別是在高並發請求時。整體來說，這個架構在靈活性和隱私保護之間取得了良好的平衡。

## 技術深入分析

WLOC 的核心技術機制是通過攔截 Apple 的網絡定位請求來修改返回的座標，這樣的設計使得用戶能夠在不手動輸入經緯度的情況下輕鬆設置虛擬位置。使用 Worker 進行坐標解析，這樣的設計不僅提高了效率，還能確保用戶的隱私，因為所有的請求都是純轉發，不會存儲任何數據。從性能上來看，這個工具能夠在高並發的情況下運行，但在高版本的 iOS 中，定位緩存的問題可能會影響用戶體驗。選擇 JavaScript 作為開發語言使得這個工具易於維護和部署，但在性能要求較高的場景下可能會出現瓶頸。整體來說，這個工具在靈活性和隱私保護之間取得了良好的平衡，適合需要虛擬定位的用戶使用。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且提供了多個使用範例，安裝過程相對順暢，但需要用戶具備一定的技術背景來設置 MITM 代理。整體來說，花 30 分鐘能夠順利運行。
...
> [!danger] 缺點
> - 僅支持 iOS，對 Android 用戶無法使用。
> - 需要一定的技術背景來設置 MITM 代理。
> - 在高版本 iOS 中可能需要重啟設備以清除緩存。

> [!warning] 注意事項
> - 僅支持 iOS 系統，對於 Android 用戶無法使用。
> - 需要 MITM 代理設置，否則無法正常工作。
> - iOS 26+ 版本需要重啟設備以清除定位緩存。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 這個工具專注於代理和虛擬定位，但不支持多種代理工具的整合，適合需要單一解決方案的用戶。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 這是一個針對特定應用的定位修改工具，功能較為單一，適合對特定場景有需求的用戶。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 這個工具專注於代理和虛擬定位，但不支持多種代理工具的整合，適合需要單一解決方案的用戶。 | 如果你的團隊已經在使用特定的代理工具，且不需要多種工具的支持。 | medium，因為需要重新配置代理設置。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 這是一個針對特定應用的定位修改工具，功能較為單一，適合對特定場景有需求的用戶。 | 如果你的需求非常專一，且不需要通用的虛擬定位功能。 | low，因為功能簡單，容易上手。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **wloc** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 這個工具專注於代理和虛擬定位，但不支持多種代理工具的整合，適合需要單一解決方案的用戶。 | 這是一個針對特定應用的定位修改工具，功能較為單一，適合對特定場景有需求的用戶。 |
> | 遷移成本 | - | medium，因為需要重新配置代理設置。 | low，因為功能簡單，容易上手。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用特定的代理工具，且不需要多種工具的支持 | 如果你的需求非常專一，且不需要通用的虛擬定位功能。 |
...
- **[HIGH]** 在 iOS 26 及以上版本中，定位緩存可能導致修改後的坐標不生效。
  - 解法：需要重啟設備以清除緩存。
- [MEDIUM] 用戶在小火箭中不清楚如何使用，導致操作困難。
  - 解法：參考官方文檔或社群討論。
- [MEDIUM] 時區問題可能導致顯示的時間不準確。
  - 解法：檢查設備的時區設置，確保與實際位置一致。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 需要在 iOS 上進行虛擬定位的開發者 | 非常適合 | 因為這個工具提供了簡便的設置和恢復功能，能夠快速驗證應用行為。 |
| 普通用戶希望隱藏真實位置 | 適合 | 因為這個工具能夠方便地設置虛擬位置，提升隱私。 |
| 對定位精度要求高的專業用戶 | 普通 | 因為在高版本 iOS 中可能需要重啟設備，影響使用體驗。 |
| 需要在 Android 上使用虛擬定位的用戶 | 不適合 | 因為這個工具僅支持 iOS 系統。 |
...
> [!tip] 投入 vs 回報
> 花 2 小時學習，3 小時整合，得到靈活的虛擬定位功能，值得投入。
...
> | --- | --- | --- | --- |
> | [#26](https://github.com/Yu9191/wloc/issues/26) | ios27好像用不了（还是说我操作有误） | 0 | 0 |
> | [#25](https://github.com/Yu9191/wloc/issues/25) | 没看明白在小火箭里面怎么使用 | 0 | 0 |
> | [#24](https://github.com/Yu9191/wloc/issues/24) | 時區問題 | 0 | 1 |
> | [#23](https://github.com/Yu9191/wloc/issues/23) | issue | 0 | 7 |
