...
**5.9k** stars · **225** stars/天 · 建立 26 天前 · JavaScript · 未標註授權
...
> [!info] 速覽
> **安裝難度** Easy · **專案狀態** Recent · **熱度** Hot (225 stars/day)
> **授權** 未標註授權 (風險較高) · **維護** Moderate (最後推送 23 天前) · **貢獻者** Solo (bus factor 風險) · **參與度** Medium
> **適合** 需要在不同地理位置進行測試的開發者和希望隱藏真實位置的普通用戶。
> **一句話重點** WLOC 不僅提供虛擬定位的功能，更是針對中國大陸的特殊需求進行了優化，讓用戶能夠在隱私和便利性之間取得平衡。
...
> [!abstract] 核心創新
> WLOC 提供了一個簡單易用的解決方案來修改 Apple 網絡定位服務的返回坐標，特別針對中國大陸的坐標偏移問題。

## 專案簡介

WLOC 是一個用於修改 Apple 網絡定位服務的工具，能夠將用戶的實際位置虛擬化。使用者只需在地圖應用中選擇位置，然後透過快捷指令來設置新的地理位置，無需手動輸入經緯度。這個工具的核心賣點在於其簡便性和高效性，能夠快速切換位置並恢復真實定位，適合需要頻繁更改位置的用戶。技術上，WLOC 使用 JavaScript 編寫，並透過 Surge、Quantumult X、Loon 和 Stash 等代理工具進行位置修改，這使得它能夠在不同的環境中靈活運用。

其設計考量了用戶隱私，所有解析過程均不存儲任何數據，確保了用戶的安全。與其他虛擬定位工具相比，WLOC 的優勢在於其支持多種代理工具和快捷指令，並且能夠自動處理 GCJ-02 到 WGS84 的坐標轉換，這在中國大陸的定位服務中尤為重要。使用者在高版本的 iOS 系統上可能會遇到定位緩存問題，需重啟設備以清除舊的定位數據，這是使用 WLOC 時需要注意的地方。整體來看，WLOC 是一個穩定且功能強大的虛擬定位工具，適合需要在不同地點進行測試或使用特定地理位置服務的開發者和用戶。
...
- 快捷指令一鍵設置 — 用戶可透過快捷指令快速設置和恢復位置，提升使用便利性。
- 自動坐標轉換 — 內建 GCJ-02 到 WGS84 的轉換功能，確保在中國大陸的準確性。
- 隱私保護 — 所有解析過程不存儲數據，確保用戶隱私安全。
- 開源可自部署 — 用戶可根據需求自行部署 Worker，進一步增強自定義能力。
...
3. 選擇位置並儲存到設備
```bash
在地圖應用中選擇位置後，使用共享功能選擇 'wloc 设置地理位置'
```

## 為什麼值得關注

> [!tip] 爆紅原因
> 建立 26 天內累積 5858 stars（225/天），forks 1156（19.7%），顯示出強烈的社群關注。作者 Yu9191 在 GitHub 上活躍，專注於網絡定位和代理工具的開發，這個專案解決了許多用戶在使用 Apple 地圖時的定位限制，特別是在中國大陸的坐標偏移問題。此專案的推出正值用戶對於隱私和位置控制需求上升的時期，並且提供了簡單易用的解決方案。社群中對於使用流程的討論和教程也顯示了其實際應用的需求。

## 適合誰使用

**目標受眾**：需要在不同地理位置進行測試的開發者和希望隱藏真實位置的普通用戶。

> [!example] 使用場景
> - 開發者用它來在測試環境中模擬不同地理位置，因為這樣可以快速驗證地理位置相關功能的正確性。
> - 普通用戶用它來在 Apple 地圖上虛擬定位，因為這樣可以隱藏真實位置，保護隱私。
> - 數據分析師用它來收集不同地區的數據，因為這樣可以方便地獲取特定地理位置的數據而不需實地前往。

## 架構分析

WLOC 的架構基於 JavaScript，通過代理模塊攔截 Apple 網絡定位的請求，並修改返回的坐標。用戶在選點頁面選擇位置後，系統會發送請求到 gs-loc.apple.com，然後通過代理模塊進行處理。這種設計使得用戶能夠在不手動輸入坐標的情況下，輕鬆設置虛擬位置。

由於所有的解析過程都是即時的，這樣的架構能夠有效減少延遲，提升用戶體驗。選擇 JavaScript 作為開發語言，能夠快速迭代和部署，並且易於與現有的網絡技術生態相容。整體來說，這種架構設計在功能上提供了靈活性，但在高流量的情況下，可能會面臨性能瓶頸，尤其是在解析請求的速度上。

## 技術深入分析

WLOC 的核心技術機制是通過代理模塊攔截 Apple 網絡定位的請求，並修改返回的坐標。這一過程中，使用了 JavaScript 的 fetch API 來發送請求，並通過解析返回的 protobuf 數據來獲取和修改坐標。這種設計使得用戶能夠在不需要手動輸入坐標的情況下，輕鬆設置虛擬位置。效能方面，WLOC 能夠快速處理用戶的請求，並在大多數情況下能夠即時返回修改後的坐標。

然而，在高流量的情況下，可能會面臨性能瓶頸，特別是在解析請求的速度上。選擇 JavaScript 作為開發語言，能夠快速迭代和部署，並且易於與現有的網絡技術生態相容。這種選擇雖然帶來了開發的靈活性，但也可能在某些情況下導致性能下降。技術風險方面，WLOC 依賴於 Apple 的網絡定位服務，若其服務發生變化，可能會影響 WLOC 的功能。

整合方面，WLOC 能夠與多種代理工具無縫對接，並且支持快捷指令的使用，這使得它在現有的技術生態中具有良好的兼容性。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了詳細的使用說明和快捷指令的連結。安裝過程相對簡單，使用者只需訂閱模塊並啟用 MITM。文件中有針對新手的使用指南，能夠幫助用戶快速上手。
...
> [!danger] 缺點
> - 在高版本 iOS 上需要重啟設備以清除定位緩存。
> - 僅限於特定的地理位置服務，可能無法全面應用。
> - 對於新手用戶，初次設置可能需要一些學習成本。

> [!warning] 注意事項
> - 僅支援 iOS 26 及以上版本，舊版本可能不兼容。
> - 需要重啟設備以清除定位緩存，否則可能無法生效。
> - 某些地理位置服務可能仍會受到其他限制，無法完全虛擬化。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供類似的虛擬定位功能，但主要針對遊戲環境，缺乏對 Apple 地圖的支持。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於代理工具的優化，並不專門針對虛擬定位，適合需要更高效能的用戶。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於遊戲虛擬定位，使用 C++ 實作，性能更優但功能較單一。 | 如果你的主要需求是針對遊戲環境的虛擬定位，這個工具會更合適。 | medium，因為需要重新配置和適應新的環境。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供更高效能的代理工具，使用 Rust 實作，適合需要高性能的用戶。 | 如果你的應用場景需要高效能的網絡請求處理，這個工具會更適合。 | high，因為需要重新學習和適應新的技術棧。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **wloc** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於遊戲虛擬定位，使用 C++ 實作，性能更優但功能較單一。 | 提供更高效能的代理工具，使用 Rust 實作，適合需要高性能的用戶。 |
> | 遷移成本 | - | medium，因為需要重新配置和適應新的環境。 | high，因為需要重新學習和適應新的技術棧。 |
> | 適用場景 | 主要場景 | 如果你的主要需求是針對遊戲環境的虛擬定位，這個工具會更合適。 | 如果你的應用場景需要高效能的網絡請求處理，這個工具會更適合。 |
...
- **[HIGH]** 在高版本 iOS 上，定位緩存可能導致修改後的坐標不生效。
  - 解法：需要重啟設備以清除緩存。
- [MEDIUM] 某些地理位置服務可能不支持，導致虛擬定位失敗。
  - 解法：確認服務支持後再使用。
- [low] 對於新手用戶，初次設置可能需要一些學習成本。
  - 解法：參考社群提供的教程和使用案例。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 需要在不同地理位置進行測試的開發者 | 非常適合 | 提供了快速且靈活的虛擬定位解決方案。 |
| 普通用戶希望隱藏真實位置 | 適合 | 能夠簡單地設置虛擬位置，保護隱私。 |
| 數據分析師需要收集不同地區的數據 | 普通 | 雖然能夠提供虛擬定位，但可能面臨服務支持的限制。 |
...
> | --- | --- | --- | --- |
> | [#39](https://github.com/Yu9191/wloc/issues/39) | Shadowrocket+快捷指令（秒成功）操作流程图文记录 | 13 | 19 |
...
> | [#61](https://github.com/Yu9191/wloc/issues/61) | 问题有点多呀这个 | 0 | 0 |
