...
**1.6k** stars · **37** stars/天 · 建立 43 天前 · Python · MIT
...
> **授權** MIT (商業友好) · **維護** Slow (最後推送 42 天前) · **貢獻者** 2 人 · **參與度** Low
> **適合** 需要在 NVIDIA GPU 上進行高效能 LLM 計算的 AI 開發者。
> **一句話重點** TileKernels 讓開發者能夠以簡單的方式構建高效能的 GPU 核心，特別適合需要快速迭代的 AI 研究。
...
> **結論** 花 5 小時學習，10 小時整合，得到高效能的 GPU 核心，值得嘗試。

> [!abstract] 核心創新
> 提供了一個基於 TileLang 的高效能 GPU 核心庫，專為 LLM 操作優化。

## 專案簡介

TileKernels 是一個專為 LLM 操作優化的 GPU 核心庫，使用 TileLang 這個專門語言來表達高效能的 GPU 核心。使用者可以透過簡單的 Python 語法來進行高效能計算，並且享受自動優化的好處。這個庫支持多種功能，包括 Top-k 專家選擇、量化、批量轉置等，這些功能能夠顯著提升模型的運行效率。核心的設計理念是讓開發者能夠在不深入底層細節的情況下，輕鬆地構建和優化 GPU 核心，這在傳統的 CUDA 開發中是相對困難的。使用者只需安裝 TileKernels，然後便可利用 `torch.autograd.Function` 將低層核心組合成可訓練的層。

這個工具的賣點在於其簡單易用的 API 和強大的性能，特別適合需要高效能計算的 AI 開發者。這個庫的依賴相對簡單，僅需 Python 3.10 以上、PyTorch 2.10 以上和 TileLang 0.1.9 以上，並要求 NVIDIA SM90 或 SM100 架構的 GPU。與其他 GPU 核心庫相比，如 NVIDIA 的 cuDNN，TileKernels 提供了更高層次的抽象，讓開發者能夠更快地開發和測試新算法。儘管目前仍在 alpha 階段，但其潛力巨大，特別是在需要快速迭代和實驗的研究環境中。

**技術棧**：`TileLang` · `PyTorch 2.10` · `CUDA Toolkit 13.1`

## 重點功能

- Gating — 支持 Top-k 專家選擇和評分，提升模型的計算效率。
- MoE Routing — 提供令牌到專家的映射，融合擴展/縮減和權重正規化。
- Quantization — 支持 FP8/FP4/E5M6 的量化，提升運算速度和減少記憶體使用。
- Transpose — 支持批量轉置操作，適合大規模數據處理。
- Engram — 提供融合 RMSNorm 的 Engram gating 核心，支持前向和反向傳播。
- Manifold HyperConnection — 包含 Sinkhorn 正規化的超連接核心，支持混合分割和應用。
- Modeling — 提供高層次的 `torch.autograd.Function` 包裝，方便組合低層核心。
...
```python
{
  "前置條件": "需要安裝 TileKernels 和 PyTorch。",
  "指令": "pip install tile-kernels",
  "預期輸出": "安裝完成後，您可以使用 TileKernels 提供的 GPU 核心進行計算。"
}
```
...
> [!tip] 爆紅原因
> 建立 43 天就累積 1575 stars（37/天），forks 135（8.6%），顯示出相對穩定的增長。這個專案的主要貢獻者來自 DeepSeek 團隊，他們在 AI 和 GPU 計算領域有豐富的經驗。TileKernels 解決了傳統 GPU 核心開發的複雜性，讓開發者能夠更容易地構建高效能的 LLM 操作。這個專案的出現正好滿足了對高效能計算需求日益增長的市場需求，尤其是在 AI 模型訓練和推論的場景中。forks/stars 比率為 8.6%，顯示出有相當一部分用戶在積極修改和使用這個專案。

## 適合誰使用

**目標受眾**：需要在 NVIDIA GPU 上進行高效能 LLM 計算的 AI 開發者。

> [!example] 使用場景
> - AI 研究人員用它來快速實現和測試新的 LLM 算法，因為 TileKernels 提供了高效能的 GPU 核心，能夠顯著縮短訓練時間。
> - 後端工程師用它來優化現有的模型推論速度，因為其支持的量化和專家路由技術能夠提升性能。
> - 開發者用它來構建新的 AI 應用，因為 TileLang 的簡單語法讓他們能夠專注於算法而非底層實現。

## 架構分析

TileKernels 採用模組化設計，將不同的 GPU 核心功能分為多個子模組，如量化、轉置和專家路由。這樣的設計使得開發者可以根據需求選擇性地使用特定功能，降低了整體的複雜度。每個模組都專注於特定的計算任務，並通過 TileLang 進行優化，這樣可以在不影響其他模組的情況下進行獨立開發和測試。這種模組化也使得未來的擴展變得更加容易，開發者可以在不影響現有功能的情況下添加新特性。

## 技術深入分析

TileKernels 的核心技術機制在於使用 TileLang 這個專門為 GPU 核心設計的語言，這使得開發者能夠用更高層次的抽象來進行計算。這個庫的設計模式是模組化的，將不同的計算任務分為多個子模組，如量化、轉置和專家路由，這樣的設計使得功能之間的耦合度降低，便於維護和擴展。效能方面，這些核心能夠充分利用 NVIDIA GPU 的計算能力，特別是在處理大規模數據時，能夠顯著提升運算速度。由於使用了 TileLang，開發者可以在不深入底層細節的情況下，快速構建和測試新的算法，這在傳統的 CUDA 開發中是相對困難的。選擇 TileLang 而非其他語言的原因在於其專門為 GPU 核心設計的特性，這使得開發者能夠更快地實現高效能計算。技術風險方面，由於目前仍在 alpha 階段，API 可能會有變動，這對於長期使用者來說可能會造成困擾。整合方面，TileKernels 可以輕鬆與 PyTorch 整合，開發者只需使用 `torch.autograd.Function` 來將低層核心組合成可訓練的層，這降低了與主流框架的整合難度。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝指引和使用範例，對於新手來說相對友好。安裝過程順暢，無需複雜的配置。文件中包含測試指令，幫助用戶快速驗證安裝是否成功。整體而言，花 30 分鐘應該能夠順利運行起來。

## 優缺點分析

> [!success] 優點
> - 高效能：針對 LLM 操作進行了優化，能夠充分利用 GPU 的計算能力。
> - 簡單易用：使用 TileLang 進行開發，降低了學習曲線。
> - 模組化設計：便於擴展和維護，開發者可以根據需求選擇性使用功能。

> [!danger] 缺點
> - 仍在 alpha 階段，可能存在不穩定性和 API 變更的風險。
> - 對於不熟悉 TileLang 的開發者來說，可能需要額外的學習時間。
> - 目前的文檔和範例仍需改進，可能影響使用體驗。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供類似的 GPU 核心功能，但主要針對多代理環境，適合需要多任務處理的場景。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於模型壓縮和加速，適合需要在資源受限環境中運行的應用。 |
| [2aronS/Duel-Agents](https://github.com/2aronS/Duel-Agents) | 針對對抗學習的 GPU 核心，適合需要進行強化學習的場景。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於多代理環境的 GPU 核心，提供了針對多任務處理的優化。 | 如果你的應用需要同時處理多個代理的計算，則 agent-sprite-forge 會是更合適的選擇。 | medium，因為需要重新調整代碼以適應不同的 API 和設計模式。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於模型壓縮和加速，提供了不同的優化策略。 | 如果你的應用需要在資源受限環境中運行，boneyard 會是更好的選擇。 | high，因為需要重新設計模型架構以適應壓縮策略。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **TileKernels** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於多代理環境的 GPU 核心，提供了針對多任務處理的優化。 | 專注於模型壓縮和加速，提供了不同的優化策略。 |
> | 遷移成本 | - | medium，因為需要重新調整代碼以適應不同的 API 和設計模式。 | high，因為需要重新設計模型架構以適應壓縮策略。 |
> | 適用場景 | 主要場景 | 如果你的應用需要同時處理多個代理的計算，則 agent-sp | 如果你的應用需要在資源受限環境中運行，boneyard 會是 |
...
- [MEDIUM] 在某些 NVIDIA GPU 上，性能表現可能不如預期，尤其是舊款 GPU。
  - 解法：確保使用最新的 CUDA 驅動和工具包。
- **[HIGH]** 文檔不夠完善，可能導致使用者在某些功能上無法快速上手。
  - 解法：參考 GitHub Issues 中的討論尋找解決方案。
- [MEDIUM] 在高負載情況下，測試可能會出現不穩定性。
  - 解法：在測試時減少並行工作數量。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司進行 AI 模型開發 | 非常適合 | 提供高效能的 GPU 核心，能夠加速模型訓練和推論。 |
| 大型企業的核心生產環境 | 不適合 | 仍在 alpha 階段，API 穩定性不足。 |
| 學術研究團隊進行新算法的實驗 | 適合 | 支持快速迭代和實驗，能夠提升研究效率。 |
| 需要在資源受限的環境中運行的應用 | 普通 | 可能需要考慮其他專注於模型壓縮的工具。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，10 小時整合，得到高效能的 GPU 核心，值得嘗試。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：不需要高權限，僅需安裝在支持的 NVIDIA GPU 上，並不存取敏感資料。
