...
star_history: "2026-04-25:1072,2026-04-26:1160,2026-04-27:1219,2026-04-28:1281,2026-04-28:1282,2026-04-29:1320,2026-04-29:1320,2026-04-30:1351,2026-05-03:1414,2026-05-04:1418,2026-05-05:1431,2026-05-06:1443,2026-05-07:1462,2026-05-09:1487"
tags:
  - github
  - "category/ai_ml"
  - "lang/python"
...
**1.5k** stars · **93** stars/天 · 建立 16 天前 · Python · MIT
...
> **授權** MIT (商業友好) · **維護** Moderate (最後推送 15 天前) · **貢獻者** 2 人 · **參與度** Low
> **適合** 需要在 NVIDIA GPU 上優化 LLM 操作的機器學習工程師。
...
> **結論** 花 8 小時學、12 小時整合，得到高效能 GPU 核心的優勢，值得在有相應硬體的情況下採用。

> [!abstract] 核心創新
> 使用 TileLang 提供高效能 GPU 核心的開發和優化。

## 專案簡介

TileKernels 是一個專為 LLM 操作優化的 GPU 核心庫，使用 TileLang 這個專用語言來表達高效能的 GPU 核心。使用者可以透過簡單的 Python 語法，快速開發和優化 GPU 核心，並且支援多種高效能操作，如 Top-k 專家選擇、量化和轉置等。這些核心已在內部訓練和推論中使用，雖然目前代碼質量和文檔仍在改進中。核心功能包括 Gating、MoE Routing、Quantization 和 Engram 等，這些功能能夠有效提升模型的計算效率和記憶體帶寬的使用。專案依賴於 PyTorch 和 TileLang，並要求 NVIDIA SM90 或 SM100 架構的 GPU，這使得它在高效能計算上具有一定的硬體需求。

與其他類似工具相比，TileKernels 提供了更高層次的抽象，讓開發者能夠更方便地將低層次的核心組合成可訓練的層，這在使用 PyTorch 的生態系統中是非常有價值的。這個專案目前仍處於 alpha 階段，開發者需要注意其穩定性和文檔的不足。對於需要高效能 GPU 計算的機器學習專案，TileKernels 提供了一個強大的解決方案，但在使用時需考慮到其依賴的硬體和軟體環境。適合有經驗的開發者在具備相應硬體的情況下使用，並且能夠承擔一定的學習成本和整合時間。

**技術棧**：`TileLang` · `PyTorch 2.10` · `CUDA Toolkit 13.1`

## 重點功能

- Gating — 提供 Top-k 專家選擇和打分功能，簡化 Mixture of Experts 的路由。
- MoE Routing — 支援令牌到專家的映射，並融合擴展/縮減和權重正規化。
- Quantization — 支援 FP8/FP4/E5M6 的量化，並提供融合的 SwiGLU + 量化操作。
- Transpose — 提供批量轉置操作，優化矩陣運算性能。
- Engram — 支援融合 RMSNorm 的 Engram gating 核心，包含前向/反向傳播和權重梯度縮減。
...
> [!tip] 爆紅原因
> 建立 16 天就累積 1487 stars（93/天），forks 120（8.1%），顯示出不錯的社群關注度。這個專案的主要貢獻者來自 DeepSeek 團隊，專注於高效能計算領域，解決了 LLM 操作中 GPU 核心優化的需求。之前的解決方案往往缺乏針對特定硬體的優化，TileKernels 透過 TileLang 提供了一個更高效的開發方式。社群活躍度高，且有持續的更新和改進，這使得它在開源社群中迅速獲得關注。

## 適合誰使用

**目標受眾**：需要在 NVIDIA GPU 上優化 LLM 操作的機器學習工程師。

> [!example] 使用場景
> - 機器學習工程師用它來優化 LLM 的推論速度，因為它提供了針對 GPU 的高效能核心，能顯著降低計算延遲。
> - 研究人員用它來開發新的量化技術，因為 TileKernels 支援多種量化方法，能夠在保持準確度的同時減少模型大小。
> - 數據科學家用它來實現 Mixture of Experts 模型，因為其內建的 MoE Routing 功能能夠簡化專家選擇的實作過程。

## 架構分析

TileKernels 的架構基於 TileLang，這是一種專為高效能 GPU 核心設計的領域特定語言。這種設計使得開發者能夠以較高的抽象層次來編寫核心，降低了開發難度。資料流方面，核心功能如 Gating 和 MoE Routing 都是通過 TileLang 的語法來實現，這樣可以充分利用 GPU 的計算能力。

選擇 TileLang 而非傳統的 CUDA 或 C++，使得開發過程更為靈活，但可能在性能上有所妥協。隨著專案的發展，擴展性可能成為一個瓶頸，特別是在需要支持更多硬體架構時。整體而言，這種架構設計使得專案能夠快速迭代和優化，但也需要持續關注性能的提升。

## 技術深入分析

TileKernels 的核心技術機制是基於 TileLang，這使得開發者能夠以 Python 語法編寫高效能的 GPU 核心。這些核心專為 LLM 操作設計，並支援多種高效能運算，如 Gating 和 MoE Routing。效能方面，這些核心能夠充分利用 NVIDIA GPU 的計算能力，但具體的效能數據在 README 中並未提供。選擇 TileLang 而非傳統的 CUDA 或 C++，使得開發過程更為靈活，並降低了學習曲線。

依賴的庫如 PyTorch 和 TileLang 提供了良好的生態支持，但也可能帶來一定的依賴風險。隨著專案的發展，如何平衡性能和開發效率將成為一個挑戰。整合到現有的 PyTorch 環境中相對簡單，但在大規模應用時，可能會遇到性能瓶頸。整體而言，TileKernels 提供了一個強大的工具，但使用者需要有一定的技術背景來充分發揮其潛力。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了安裝和測試的詳細步驟；安裝過程相對順暢，但需要注意硬體要求；有提供測試範例，幫助新手快速上手；目前文件僅提供英文版本。 

## 優缺點分析

> [!success] 優點
> - 高效能的 GPU 核心，適合 LLM 操作。
> - 使用 TileLang 提供的高層次抽象，降低開發難度。
> - 支援多種先進的量化技術，提升模型效率。

> [!danger] 缺點
> - 目前仍在 alpha 階段，穩定性不足。
> - 需要特定的 NVIDIA 硬體支持，限制了使用範圍。
> - 文檔和代碼質量尚需改進。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供了一套針對多種 AI 模型的核心，但不專注於 LLM 的 GPU 優化。 |
| [NVIDIA/NemoClaw](https://github.com/NVIDIA/NemoClaw) | 專注於 NVIDIA 硬體的深度學習優化，但缺乏 TileLang 的高層次抽象。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 使用 C++ 實作 GPU 核心，記憶體用量較少，但開發難度較高。 | 如果你的團隊需要更細緻的控制和優化，且不介意較高的開發成本。 | medium，因為需要將現有的 Python 代碼轉換為 C++。 |
| [NVIDIA/NemoClaw](https://github.com/NVIDIA/NemoClaw) | 專注於 NVIDIA 硬體的深度學習優化，使用 CUDA 實現，性能優越但學習曲線陡峭。 | 如果你的團隊已經熟悉 CUDA 且需要極致的性能優化。 | high，因為需要重寫大量代碼以適應 CUDA 的開發模式。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **TileKernels** | **boneyard** | **NemoClaw** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 使用 C++ 實作 GPU 核心，記憶體用量較少，但開發難度較高。 | 專注於 NVIDIA 硬體的深度學習優化，使用 CUDA 實現，性能優越但學習曲線陡峭。 |
> | 遷移成本 | - | medium，因為需要將現有的 Python 代碼轉換為 C++。 | high，因為需要重寫大量代碼以適應 CUDA 的開發模式。 |
> | 適用場景 | 主要場景 | 如果你的團隊需要更細緻的控制和優化，且不介意較高的開發成本。 | 如果你的團隊已經熟悉 CUDA 且需要極致的性能優化。 |
...
> [!tip] 採用建議
> 適合有經驗的開發者在非生產環境中試用，尚不建議用於關鍵應用。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- [MEDIUM] 測試時可能會遇到隨機種子未能正確設置的問題
  - 解法：手動設置隨機種子以確保測試一致性
- **[HIGH]** 在某些 GPU 上可能無法達到預期的性能
  - 解法：檢查硬體兼容性，並根據需求調整核心設置
- [MEDIUM] 文檔不完整，可能導致使用者無法快速上手
  - 解法：參考社群討論或直接聯繫開發者以獲取幫助

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 大型 AI 模型的訓練和推論 | 非常適合 | 提供針對 LLM 的高效能 GPU 核心，能顯著提升計算效率。 |
| 小型專案或個人實驗 | 普通 | 雖然功能強大，但可能對硬體要求較高，且穩定性不足。 |
| 需要高效能量化技術的研究 | 適合 | 支援多種先進的量化技術，能夠有效減少模型大小。 |
| 不熟悉 GPU 開發的團隊 | 不適合 | 需要一定的技術背景和硬體支持，對新手不友好。 |
...
> [!tip] 投入 vs 回報
> 花 8 小時學、12 小時整合，得到高效能 GPU 核心的優勢，值得在有相應硬體的情況下採用。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：不需要高權限操作，但需注意依賴的庫是否存在安全漏洞。整體來說，使用於 CI/CD pipeline 是安全的。
...
相關概念：[[GPU 加速]] · [[量化技術]] · [[Mixture of Experts]]
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["GPU 加速","量化技術","Mixture of Experts"];
