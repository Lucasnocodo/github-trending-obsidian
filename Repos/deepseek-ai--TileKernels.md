...
**1.6k** stars · **38** stars/天 · 建立 41 天前 · Python · MIT
...
> **授權** MIT (商業友好) · **維護** Slow (最後推送 40 天前) · **貢獻者** 2 人 · **參與度** Low
> **適合** 需要在高效能 GPU 上進行 LLM 訓練和推論的深度學習開發者。
> **一句話重點** TileKernels 將 GPU 核心優化與高效能計算結合，為 LLM 訓練提供了強大的支持。
...
> **結論** 花 10 小時學、5 小時整合，得到高效能的 LLM 訓練支持，值得投入。

> [!abstract] 核心創新
> 提供專為 LLM 訓練優化的 GPU 核心，利用 TileLang 實現高效能計算。

## 專案簡介

TileKernels 是一個專為 LLM 操作設計的 GPU 核心庫，使用 TileLang 進行開發，旨在提供高效能的計算和記憶體帶寬利用。這個庫的核心機制包括多種專為 Mixture of Experts (MoE) 設計的功能，如 Top-k 專家選擇、token 到專家的映射以及量化操作，這些都能顯著提升模型的運算效率。使用者只需安裝 Python 3.10 以上和 PyTorch 2.10 以上，即可透過 `pip install tile-kernels` 進行安裝，並利用提供的測試指令進行功能驗證。這個工具的賣點在於其能夠在 GPU 上實現高效的計算，並且支援自動優化，讓開發者能夠專注於模型的設計而非底層的性能調整。該專案的技術實作基於 TileLang，這是一種專為表達高效能 GPU 核心而設計的領域特定語言，這使得開發者能夠輕鬆地進行代碼遷移和敏捷開發。

這個庫的依賴關係相對簡單，主要依賴於 PyTorch 和 TileLang，這使得它在資源需求上較為輕量。與其他類似工具相比，如 0x0funky/agent-sprite-forge 和 2aronS/Duel-Agents，TileKernels 在 MoE 路由和量化方面提供了更深入的功能，尤其是在高效能計算的需求上，這使得它在大型模型訓練中更具優勢。實際使用中，這個庫的效能表現良好，但需要 NVIDIA SM90 或 SM100 架構的 GPU，並且對 CUDA 的版本有一定要求，這可能限制了某些使用者的使用範圍。該專案目前仍處於 Alpha 階段，建議在非關鍵的開發環境中使用，並持續關注其更新和改進。對於需要進行大規模 LLM 訓練的團隊，這個工具的整合成本相對較低，學習曲線也不算陡峭，適合在實驗和開發階段使用。

**技術棧**：`TileLang` · `PyTorch 2.10` · `Python 3.10`

## 重點功能

- Gating — 提供 Top-k 專家選擇和評分功能，支援 Mixture of Experts 路由。
- MoE Routing — 包含 token 到專家的映射、融合擴展/縮減和權重正規化。
- Quantization — 支援每個 token、每個區塊和每個通道的 FP8/FP4/E5M6 量化操作。
- Transpose — 支援批量轉置操作，提升計算效率。
- Engram — 提供融合 RMSNorm 的 Engram gating 核心，支援前向/反向傳遞和權重梯度縮減。
...
> [!tip] 爆紅原因
> 建立 41 天就累積 1567 stars（38/天），forks 134（8.6%），顯示出不錯的社群關注度。這個專案的主要貢獻者來自 DeepSeek 團隊，專注於 AI 和深度學習領域，過去有多個相關專案的經驗。TileKernels 解決了在 LLM 訓練中對高效能 GPU 核心的需求，之前的解決方案往往在性能上無法達到最佳化，這使得開發者在訓練大型模型時面臨挑戰。該專案的推出正好填補了這一空白，並且在社群中引起了討論和關注。技術上，TileLang 的出現使得這個工具的實現變得可行，因為它提供了專為 GPU 優化的語法和結構。forks/stars 比率為 8.6%，顯示出有相當比例的使用者在實際修改和使用這個專案，這意味著社群對這個工具的實際應用有一定的投入。

## 適合誰使用

**目標受眾**：需要在高效能 GPU 上進行 LLM 訓練和推論的深度學習開發者。

> [!example] 使用場景
> - 深度學習研究員用它來優化 LLM 的訓練過程，因為其提供的 MoE 路由和量化功能能顯著提升訓練效率。
> - AI 工程師用它來在大型模型中實現高效的 GPU 核心運算，因為 TileLang 使得開發過程更為簡便和靈活。
> - 數據科學家用它來進行模型推論，因為其優化的核心能夠在實際應用中提供更快的反應時間。

## 架構分析

TileKernels 採用模組化架構，將不同的 GPU 核心功能分為多個子模組，如 MoE、量化、轉置等，這樣的設計使得開發者可以根據需求選擇性使用。每個模組都專注於特定的計算任務，並且能夠在不影響其他模組的情況下進行優化和更新。這種設計的代價在於可能需要更多的初始學習時間來理解各個模組的功能和接口。

資料流方面，TileLang 提供的語法使得開發者能夠輕鬆地將高層次的計算邏輯轉換為低層次的 GPU 核心，這樣的轉換過程能夠自動進行優化。這種設計使得整體架構在性能上具備優勢，但在擴展性上可能會受到特定硬體架構的限制，特別是在需要支援多種 GPU 架構的情況下。整體而言，這個架構在 LLM 訓練的特定場景下表現良好，但對於其他計算任務的適用性可能有限。

## 技術深入分析

TileKernels 的核心技術在於其使用 TileLang 進行 GPU 核心的開發，這使得開發者能夠以高層次的語法來表達複雜的計算邏輯。這種設計不僅提高了開發效率，還能自動進行性能優化，從而達到更高的計算效能。該專案能夠處理大規模的 LLM 訓練，具體的性能數據尚未公開，但其設計目標是接近硬體性能的極限。選擇 TileLang 而非其他語言的原因在於其專為 GPU 計算優化的特性，這使得開發者能夠更高效地利用硬體資源。依賴關係方面，該專案主要依賴於 PyTorch 和 TileLang，這使得整體架構相對輕量，降低了維護成本。技術風險方面，隨著使用者數量的增加，可能會出現 API 穩定性問題，特別是在 Alpha 階段的情況下。整合方面，TileKernels 與 PyTorch 的整合相對簡單，開發者可以輕鬆地將其應用於現有的深度學習框架中，並且支持 CI/CD 流程的整合，這對於團隊協作非常友好。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝指南和使用範例，對於新手來說相對友好。安裝過程順暢，沒有明顯的坑。測試部分也提供了詳細的指令，方便使用者進行驗證。文件目前僅提供英文版本，可能對非英語使用者造成一定困難。

## 優缺點分析

> [!success] 優點
> - 專為 LLM 訓練優化，提供高效能的 GPU 核心。
> - 模組化設計，便於擴展和維護。
> - 支援先進的量化技術，提升計算效率。

> [!danger] 缺點
> - 僅支援特定的 NVIDIA GPU 架構，限制使用者範圍。
> - 目前仍在 Alpha 階段，API 穩定性不足。
> - 對於新手來說，學習曲線較陡峭。

> [!warning] 注意事項
> - 僅支援 NVIDIA SM90 或 SM100 架構的 GPU。
> - 需要 CUDA Toolkit 13.1 或更高版本。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於多代理系統的開發，TileKernels 更專注於 LLM 的 GPU 核心優化。 |
| [2aronS/Duel-Agents](https://github.com/2aronS/Duel-Agents) | 提供對抗性訓練的框架，TileKernels 則專注於高效能計算和核心優化。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於多代理系統的開發，TileKernels 更專注於 LLM 的 GPU 核心優化。 | 如果你的專案需要多代理協作，且不僅限於 LLM 訓練，則可以考慮這個工具。 | medium，因為兩者的架構和功能有一定差異。 |
| [2aronS/Duel-Agents](https://github.com/2aronS/Duel-Agents) | 提供對抗性訓練的框架，TileKernels 則專注於高效能計算和核心優化。 | 如果你的應用場景涉及對抗性訓練，則這個工具會更合適。 | high，因為需要重構部分計算邏輯以適應不同的框架。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **TileKernels** | **agent-sprite-forge** | **Duel-Agents** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於多代理系統的開發，TileKernels 更專注於 LLM 的 GPU 核心優化。 | 提供對抗性訓練的框架，TileKernels 則專注於高效能計算和核心優化。 |
> | 遷移成本 | - | medium，因為兩者的架構和功能有一定差異。 | high，因為需要重構部分計算邏輯以適應不同的框架。 |
> | 適用場景 | 主要場景 | 如果你的專案需要多代理協作，且不僅限於 LLM 訓練，則可以 | 如果你的應用場景涉及對抗性訓練，則這個工具會更合適。 |
...
- **[HIGH]** 在某些 NVIDIA GPU 上可能會遇到兼容性問題，特別是較舊的型號。
  - 解法：確保使用最新的驅動程式和 CUDA Toolkit。
- [MEDIUM] 在進行量化操作時，可能會導致數據精度損失。
  - 解法：在量化前進行充分的測試和驗證。
- **[HIGH]** API 可能會隨著更新而變動，導致現有代碼不兼容。
  - 解法：定期檢查更新日誌，並及時調整代碼。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 大型 AI 團隊進行 LLM 訓練 | 非常適合 | 提供高效能的 GPU 核心，能顯著提升訓練速度。 |
| 小型開發團隊進行深度學習實驗 | 適合 | 模組化設計使得功能擴展方便，適合快速迭代。 |
| 個人開發者進行小型專案 | 普通 | 雖然功能強大，但對於小型專案可能過於複雜。 |
| 需要穩定 API 的生產環境 | 不適合 | 目前處於 Alpha 階段，API 穩定性不足。 |
...
> [!tip] 投入 vs 回報
> 花 10 小時學、5 小時整合，得到高效能的 LLM 訓練支持，值得投入。
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["GPU 加速","量化技術","深度學習優化"];
