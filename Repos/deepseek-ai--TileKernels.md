...
use_case: "提供高效能的 GPU 核心，專為 LLM 操作設計，使用 TileLang 實現。"
priority: high
ring: assess
...
star_history: "2026-04-25:1072,2026-04-26:1160,2026-04-27:1219,2026-04-28:1281,2026-04-28:1282,2026-04-29:1320,2026-04-29:1320,2026-04-30:1351,2026-05-03:1414,2026-05-04:1418,2026-05-05:1431,2026-05-06:1443,2026-05-07:1462,2026-05-09:1487,2026-05-10:1491"
tags:
  - github
  - "category/ai_ml"
  - "lang/python"
...
aliases:
  ...
  - "提供高效能的 GPU 核心，專為 LLM 操作設計，使用 TileLang 實現。"
---

# TileKernels

**1.5k** stars · **88** stars/天 · 建立 17 天前 · Python · MIT
...
> [!info] 速覽
> **安裝難度** Easy · **專案狀態** Recent · **熱度** Growing (88 stars/day)
> **授權** MIT (商業友好) · **維護** Moderate (最後推送 16 天前) · **貢獻者** 2 人 · **參與度** Low
> **適合** 需要高效能 GPU 核心以優化 LLM 訓練和推論的深度學習研究者和工程師。
> **一句話重點** TileKernels 透過 TileLang 的使用，提供了一個專為 LLM 操作優化的 GPU 核心解決方案，顯著提升了開發效率和運算性能。
...
> **結論** 花 5 小時學、8 小時整合，得到高效能的 GPU 核心，值得投入。

> [!abstract] 核心創新
> 這個專案的核心創新在於使用 TileLang 提供高效能的 GPU 核心，專為 LLM 操作設計。

## 專案簡介

TileKernels 是一個針對大型語言模型（LLM）操作優化的 GPU 核心庫，使用 TileLang 這個專門語言來表達高效能的 GPU 核心。使用者可以透過簡單的 Python 語法來進行高效能計算，並且支援多種先進的功能，如 Top-k 專家選擇、量化和批次轉置等。這些核心已經在內部訓練和推論中使用，雖然目前仍在改進中，但已經接近硬體性能的極限。核心功能包括 Gating、MoE Routing、Quantization 和 Engram 等，這些功能能夠有效提升模型的運算效率。

使用者需要 Python 3.10 以上、PyTorch 2.10 以上及 NVIDIA SM90 或 SM100 架構的 GPU，並且需要 CUDA Toolkit 13.1 以上。安裝過程簡單，使用 pip install -e '.[dev]' 即可進行開發版本安裝，或使用 pip install tile-kernels 安裝正式版本。這個專案的設計理念是利用 TileLang 的特性來實現自動優化和敏捷開發，這樣的設計使得開發者能夠快速迭代和實現高效能的計算。與其他 GPU 核心庫相比，TileKernels 的優勢在於其針對 LLM 的專業化設計，並且提供了更高層次的抽象來簡化開發過程。

**技術棧**：`Python 3.10+` · `PyTorch 2.10+` · `TileLang 0.1.9+` · `CUDA Toolkit 13.1+`

## 重點功能

- Gating — 提供 Top-k 專家選擇和評分功能，適用於 Mixture of Experts 路由。
- MoE Routing — 支援 Token-to-expert 映射，融合擴展/縮減和權重正規化。
- Quantization — 支援 FP8/FP4/E5M6 的每個 token、每個區塊和每個通道的量化。
- Transpose — 提供批次轉置操作，提升運算效率。
- Engram — 提供融合 RMSNorm 的 Engram gating 核心，支援前向/反向傳遞和權重梯度減少。
...
[
  "# 安裝開發版本",
  "pip install -e '.[dev]'",
  "# 預期輸出：安裝成功，能夠使用 TileKernels 的所有功能。"
]
```
...
> [!tip] 爆紅原因
> 建立 17 天內累積 1491 stars（88/天），forks 121（8.1%），顯示出穩定的增長趨勢。主要貢獻者包括 Chenhao Xu 和 Xiangwen Wang，他們在 GPU 計算和深度學習領域有豐富的經驗。這個專案解決了在 LLM 訓練和推論過程中對高效能 GPU 核心的需求，之前的解決方案往往無法充分利用硬體性能。TileLang 的出現使得開發者能夠更方便地編寫和優化 GPU 核心，這在技術上是一次重要的進步。最近的社群討論和推廣活動也促進了這個專案的曝光率。forks/stars 比率為 8.1%，顯示出使用者對於這個專案的實際修改和使用意圖較高。

## 適合誰使用

**目標受眾**：需要高效能 GPU 核心以優化 LLM 訓練和推論的深度學習研究者和工程師。

> [!example] 使用場景
> - 深度學習工程師用它來優化 LLM 訓練過程，因為其專為 GPU 設計的核心能顯著提升計算效率。
> - 研究人員用它來進行高效能的模型推論，因為 TileKernels 提供的量化功能能減少記憶體使用。
> - 開發者用它來快速實現和測試新的 LLM 架構，因為 TileLang 的簡單語法讓開發過程更為敏捷。

## 架構分析

TileKernels 的架構基於 TileLang，這是一種專為高效能 GPU 核心設計的領域特定語言。這種設計使得開發者能夠在 Python 環境中輕鬆編寫和優化 GPU 核心，並自動化優化過程。資料流方面，核心功能如 Gating 和 MoE Routing 都是通過高效的演算法實現，並且支援批次處理以提升性能。

選擇 TileLang 而非傳統的 CUDA C++，使得開發過程更為靈活，但也可能在性能上略有妥協。擴展性方面，這個架構能夠支持多種 GPU 硬體，但對於非 NVIDIA 硬體的支援則有限。整體而言，這個架構的設計旨在簡化開發流程並提升性能，特別是在 LLM 的應用場景中。

## 技術深入分析

TileKernels 的核心技術機制是基於 TileLang，這是一種專為高效能 GPU 核心設計的語言，能夠簡化開發過程並自動化優化。這個專案的主要功能如 Gating 和 MoE Routing，都是透過高效的演算法實現，並支援批次處理以提升性能。效能方面，這些核心能夠充分利用 NVIDIA SM90 和 SM100 架構的 GPU，並在內部訓練和推論中表現良好。設計取捨方面，選擇 TileLang 而非傳統的 CUDA C++，使得開發過程更為靈活，但在性能上可能略有妥協。技術風險方面，隨著專案的發展，API 可能會有變動，對於使用者來說需要持續關注更新。整合分析方面，這個專案能夠與 PyTorch 等主流深度學習框架良好整合，並且支援 CI/CD 流程，降低了團隊的學習成本。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝和使用說明，並附有範例。安裝過程相對順暢，但需要注意硬體要求。文件中有針對測試的詳細說明，對於新手來說能夠快速上手。整體而言，花 30 分鐘能夠順利跑起來。

## 優缺點分析

> [!success] 優點
> - 專為 LLM 設計的高效能 GPU 核心，能顯著提升運算效率。
> - 使用 TileLang 簡化開發流程，便於快速迭代和優化。
> - 支援多種先進功能，如量化和批次轉置，滿足不同需求。

> [!danger] 缺點
> - 目前仍在 alpha 階段，穩定性和 API 可能會有變動。
> - 對於非 NVIDIA 硬體的支援有限，可能影響使用範圍。
> - 需要較高的硬體要求，對於小型專案可能不太友好。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供類似的 GPU 核心功能，但主要針對即時通訊和 agent 系統，並不專注於 LLM。 |
| [NVIDIA/NemoClaw](https://github.com/NVIDIA/NemoClaw) | 專注於 NVIDIA 硬體的深度學習框架，提供更全面的生態系統支持，但可能不如 TileKernels 專注於 LLM 的性能優化。 |
| [432539/gpt2api](https://github.com/432539/gpt2api) | 專注於 GPT-2 模型的 API 實現，並不提供底層的 GPU 核心優化功能。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [NVIDIA/NemoClaw](https://github.com/NVIDIA/NemoClaw) | 專注於 NVIDIA 硬體的深度學習框架，提供更全面的生態系統支持，但可能不如 TileKernels 專注於 LLM 的性能優化。 | 如果你的團隊已經在使用 NVIDIA 硬體並需要更全面的生態系統支持。 | medium，因為需要重新適應不同的 API 和開發流程。 |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供類似的 GPU 核心功能，但主要針對即時通訊和 agent 系統，並不專注於 LLM。 | 如果你的專案主要集中在即時通訊或 agent 系統，而非 LLM 的訓練和推論。 | low，因為其 API 設計較為相似，遷移成本較低。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **TileKernels** | **NemoClaw** | **agent-sprite-forge** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於 NVIDIA 硬體的深度學習框架，提供更全面的生態系統支持，但可能不如 TileKernels 專注於 LLM 的性能優化。 | 提供類似的 GPU 核心功能，但主要針對即時通訊和 agent 系統，並不專注於 LLM。 |
> | 遷移成本 | - | medium，因為需要重新適應不同的 API 和開發流程。 | low，因為其 API 設計較為相似，遷移成本較低。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用 NVIDIA 硬體並需要更全面的生態 | 如果你的專案主要集中在即時通訊或 agent 系統，而非 L |
...
- [MEDIUM] 在使用 pytest 測試時，--seed 參數未能正確設置 Python 的 random 模組。
  - 解法：手動設置隨機種子以確保測試的一致性。
- **[HIGH]** 對於非 NVIDIA 硬體的支援有限，可能導致無法運行。
  - 解法：確保使用 NVIDIA GPU，或考慮其他替代方案。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 提供高效能的 GPU 核心，能夠顯著提升運算效率。 |
| 大型企業的 LLM 訓練系統 | 適合 | 能夠滿足高效能計算的需求，但需注意硬體要求。 |
| 小型專案或個人開發者 | 普通 | 雖然功能強大，但對硬體要求較高，可能不太友好。 |
| 對 GPU 性能要求不高的應用 | 不適合 | 專案設計主要針對高效能 GPU 核心，對於低效能需求的應用不合適。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學、8 小時整合，得到高效能的 GPU 核心，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：專案本身不需要高權限，且不存取敏感資料，但需注意依賴的安全性和穩定性。

## 生態系整合

> [!abstract] 如何融入你的工具鏈

TileKernels 通常與 PyTorch 等深度學習框架搭配使用，主要在模型訓練和推論的環節中發揮作用。在一個使用 PyTorch 的專案中，你可以用 TileKernels 來優化 LLM 的運算，具體做法是將 TileKernels 的核心功能包裝成 PyTorch 的自定義層。這個專案支援 GitHub Actions 等 CI 工具，並且能夠與 VS Code 等 IDE 整合，降低了開發的摩擦點。整合過程中，最常見的問題是硬體要求不符，使用者需確保擁有合適的 NVIDIA GPU。
