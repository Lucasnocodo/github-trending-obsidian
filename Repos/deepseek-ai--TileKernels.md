...
use_case: "提供針對 LLM 操作的優化 GPU 核心，使用 TileLang 編寫。"
priority: high
ring: assess
...
star_history: "2026-04-25:1072,2026-04-26:1160,2026-04-27:1219,2026-04-28:1281,2026-04-28:1282,2026-04-29:1320"
tags:
  - github
  - "category/ai_ml"
  - "lang/python"
...
aliases:
  ...
  - "提供針對 LLM 操作的優化 GPU 核心，使用 TileLang 編寫。"
---

# TileKernels

**1.3k** stars · **256** stars/天 · 建立 5 天前 · Python · MIT
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Brand New · **熱度** Hot (256 stars/day)
> **授權** MIT (商業友好) · **維護** Active (最後推送 4 天前) · **貢獻者** 2 人 · **參與度** Low
> **適合** 需要在高效能 GPU 上運行 LLM 的 AI 開發者。
> **一句話重點** TileKernels 的設計讓 GPU 核心的開發變得更加簡單和高效，特別適合需要高效能計算的 AI 開發者。
...
> **結論** 花 10 小時學習，5 小時整合，得到高效能的 GPU 核心，值得探索。

> [!abstract] 核心創新
> 使用 TileLang 提供高效能 GPU 核心的自動化優化能力。

## 專案簡介

TileKernels 是一個專為 LLM 操作優化的 GPU 核心庫，使用 TileLang 這個專用語言來表達高效能的 GPU 核心。用戶可以透過簡單的 Python 語法來定義和優化計算密集型的 GPU 操作，這樣的設計使得開發者能夠快速迭代和調整核心，並自動化優化過程。核心功能包括 Top-k 專家選擇、量化、批次轉置等，這些功能能有效提升大型模型的運行效率。安裝時只需執行 `pip install tile-kernels`，即可獲得最新版本，並可選擇開發版本進行本地開發。這個庫的賣點在於其專注於高效能計算和簡化的開發流程，特別適合需要高效 GPU 運算的 AI 開發者。使用者需要 NVIDIA SM90 或 SM100 架構的 GPU 以及 CUDA Toolkit 13.1 以上版本，這意味著它主要針對高效能計算環境進行優化。

**技術棧**：`TileLang` · `PyTorch 2.10` · `CUDA Toolkit 13.1`

## 重點功能

- Gating — 支持 Top-k 專家選擇和評分，簡化 Mixture of Experts 路由。
- MoE Routing — 提供令牌到專家的映射，融合擴展/縮減和權重正規化。
- Quantization — 支持每個令牌、每個區塊和每個通道的 FP8/FP4/E5M6 量化操作。
...
- Modeling — 高層次的 `torch.autograd.Function` 包裝器，將低層次核心組合成可訓練層。
...
```python
{
  "前置條件": "安裝 TileKernels 和必要的依賴",
  "指令": "pytest tests/transpose/test_transpose.py -n 4 # 正確性測試",
  "預期輸出": "測試結果顯示所有測試通過"
}
```
...
> [!tip] 爆紅原因
> 建立 5 天內累積 1282 stars（256/天），forks 105（8.2%），顯示出良好的社群反應。這個專案由深具經驗的開發者團隊維護，包含多位在 AI 領域有實力的貢獻者。它解決了在 GPU 上執行 LLM 操作時的性能瓶頸，之前的解決方案往往無法達到最佳效能，這使得開發者在實現高效能計算時面臨挑戰。最近的推文和討論也引起了關注，進一步促進了其流行。高達 8.2% 的 forks/stars 比率顯示出許多開發者對這個專案的實際使用和修改意願。

## 適合誰使用

**目標受眾**：需要在高效能 GPU 上運行 LLM 的 AI 開發者。

> [!example] 使用場景
> - AI 研究員用它來優化大型語言模型的訓練過程，因為它提供了針對 GPU 的高效能核心，能顯著減少訓練時間。
> - 後端工程師用它來實現 Mixture of Experts 模型，因為其內建的專家路由功能能簡化複雜的模型架構。
> - 數據科學家用它來進行大規模數據的批次處理，因為其批次轉置功能能提升數據處理效率。

## 架構分析

TileKernels 採用模組化設計，將不同的核心功能分為多個子模組，如 MoE、量化和轉置等，這樣的設計使得每個模組可以獨立開發和優化。資料流方面，使用 TileLang 來定義 GPU 核心，並透過 PyTorch 的 autograd 系統將低層次的核心組合成高層次的可訓練層。這樣的設計使得開發者能夠快速迭代和測試不同的核心實現。選擇 TileLang 而非傳統的 CUDA C++，使得開發者能夠用 Python 更簡單地進行開發，但可能在性能上略有損失。擴展性方面，隨著功能的增加，可能會面臨模組間的依賴管理挑戰。

## 技術深入分析

TileKernels 的核心技術在於使用 TileLang 來定義 GPU 核心，這使得開發者能夠用 Python 語法來表達複雜的計算邏輯。這種設計模式使得開發者可以快速迭代和優化核心，並且能夠自動化優化過程。效能方面，這些核心能夠接近硬體性能的極限，特別是在計算密集型的操作中。選擇 TileLang 而非傳統的 CUDA C++，使得開發者能夠更快上手，但可能在性能上略有損失。技術風險方面，隨著功能的增加，可能會面臨模組間的依賴管理挑戰，特別是在擴展性方面。整合方面，TileKernels 可以與 PyTorch 無縫結合，並且支持多種測試框架，這使得它在現有的 AI 開發流程中容易被採用。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了安裝和使用的詳細步驟。安裝過程相對順暢，但需注意依賴的硬體要求。文件中包含測試範例，幫助使用者快速上手。多語言支持尚未提供。

## 優缺點分析

> [!success] 優點
> - 提供高效能的 GPU 核心，專為 LLM 操作設計。
> - 模組化設計，便於擴展和維護。
> - 支持自動化優化，減少手動調整的需求。

> [!danger] 缺點
> - 目前處於 Alpha 階段，可能存在不穩定的 API。
> - 依賴於特定的硬體架構，限制了使用範圍。
> - 文檔和測試仍在改進中，可能影響使用者體驗。

> [!warning] 注意事項
> - 僅支援 Python 3.10 以上版本
> - 需要 NVIDIA SM90 或 SM100 架構的 GPU
> - 不支援非 NVIDIA 硬體的 CUDA 環境
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供類似的 GPU 加速功能，但專注於不同的 AI 應用場景，適合需要多樣化模型的開發。 |
| [NVIDIA/NemoClaw](https://github.com/NVIDIA/NemoClaw) | 專注於語音處理的 GPU 核心，與 TileKernels 在應用領域上有所不同，適合特定的語音 AI 開發。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於深度學習模型的訓練和推理，但不如 TileKernels 在 GPU 核心的優化上具體。 | 如果你的專案主要集中在深度學習模型的訓練而非 GPU 核心的優化，則可選擇此工具。 | medium，因為需要重新調整模型架構。 |
| [NVIDIA/NemoClaw](https://github.com/NVIDIA/NemoClaw) | 專注於語音處理的 GPU 核心，提供針對語音任務的優化，但不如 TileKernels 通用。 | 如果你的專案專注於語音處理，則 NemoClaw 會是更合適的選擇。 | high，因為需要重構整個處理流程。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **TileKernels** | **boneyard** | **NemoClaw** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於深度學習模型的訓練和推理，但不如 TileKernels 在 GPU 核心的優化上具體。 | 專注於語音處理的 GPU 核心，提供針對語音任務的優化，但不如 TileKernels 通用。 |
> | 遷移成本 | - | medium，因為需要重新調整模型架構。 | high，因為需要重構整個處理流程。 |
> | 適用場景 | 主要場景 | 如果你的專案主要集中在深度學習模型的訓練而非 GPU 核心的 | 如果你的專案專注於語音處理，則 NemoClaw 會是更合適 |
...
- [MEDIUM] 在測試中，--seed 參數未能正確設置 Python 的隨機模組
  - 解法：手動設置隨機種子以確保測試的可重複性
- **[HIGH]** 某些核心在特定硬體上性能不佳
  - 解法：在不同的 GPU 上進行測試以找到最佳配置
- [MEDIUM] 文檔不夠完善，可能導致使用困難
  - 解法：參考社群討論和範例來補充學習
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 大型 AI 團隊的模型訓練 | 非常適合 | 提供高效能的 GPU 核心，能顯著提升訓練效率。 |
| 小型專案的快速原型開發 | 適合 | 模組化設計便於快速迭代和測試。 |
| 需要高效能計算的商業應用 | 普通 | 雖然性能優越，但目前仍在 Alpha 階段，穩定性不足。 |
| 教育用途的 AI 課程 | 不適合 | 文檔和支援尚不完善，可能影響學習效果。 |
...
> [!tip] 投入 vs 回報
> 花 10 小時學習，5 小時整合，得到高效能的 GPU 核心，值得探索。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：不需要高權限，且不存取敏感資料。依賴的庫和框架均為開源，信任度高。
...
相關概念：[[GPU 加速]] · [[量化]] · [[Mixture of Experts]] · [[高效能計算]] · [[自動化優化]]
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["GPU 加速","量化","Mixture of Experts","高效能計算","自動化優化"];
