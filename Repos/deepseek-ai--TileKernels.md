...
**1.6k** stars · **28** stars/天 · 建立 57 天前 · Python · MIT
...
> **授權** MIT (商業友好) · **維護** Slow (最後推送 56 天前) · **貢獻者** 2 人 · **參與度** Low
> **適合** 需要高效能 GPU 核心以支持 LLM 操作的研究團隊和開發者。
> **一句話重點** TileKernels 的核心價值在於其能夠簡化 GPU 核心的開發，並提供高效能的解決方案，特別適合 LLM 的需求。
...
> [!abstract] 核心創新
> 使用 TileLang 提供高效能的 GPU 核心，簡化了開發流程並自動進行性能優化。

## 專案簡介

TileKernels 是一個專為 LLM（大型語言模型）操作設計的 GPU 核心庫，使用 TileLang 這個專用語言來表達高效能的 GPU 核心。用戶可以透過簡單的 Python 語法來定義複雜的 GPU 操作，並且自動優化性能。這個庫的核心功能包括 Gating（專家選擇）、MoE Routing（令牌到專家的映射）、量化（FP8/FP4/E5M6 轉換）、轉置操作以及 Engram 核心等，這些功能都針對計算密集型和記憶體帶寬的極限進行了優化。使用者只需安裝 Python 3.10 以上版本及 PyTorch 2.10 以上版本，即可輕鬆集成這些功能。這個工具的「一句話賣點」在於其能夠在不妥協性能的情況下，提供靈活且高效的 GPU 核心設計。技術上，TileKernels 依賴於 TileLang，這是一種專為 GPU 核心設計的語言，能夠簡化開發流程並提高執行效率。

與其他 GPU 核心庫相比，如 NVIDIA 的 cuDNN，TileKernels 提供了更高的靈活性和可擴展性，特別是在處理複雜的 LLM 任務時。這使得它在需要快速迭代和實驗的研究環境中表現出色。實際使用中，TileKernels 需要 NVIDIA SM90 或 SM100 架構的 GPU，並且對 CUDA 工具包有一定的要求，這限制了它的使用範圍。這個專案目前處於 alpha 階段，開發者們正在積極改進代碼質量和文檔。對於需要高效能 GPU 核心的研究團隊，TileKernels 是一個值得考慮的選擇，但對於小型專案或不需要高性能的應用，可能會顯得過於複雜。
...
- MoE Routing — 整合令牌到專家的映射，並進行擴展/縮減和權重正規化。
- Quantization — 支持每個令牌、每個區塊和每個通道的 FP8/FP4/E5M6 轉換，並融合 SwiGLU 和量化操作。
...
- Modeling — 提供高階的 torch.autograd.Function 包裝器，將低階核心組合成可訓練的層。
...
> [!tip] 爆紅原因
> 建立 57 天內累積 1595 stars（28/天），forks 140（8.8%），這顯示出相對穩定的關注度。這個專案的主要貢獻者包括多位來自 DeepSeek 的開發者，他們在 AI 和 GPU 編程方面有豐富的經驗。TileKernels 解決了在高性能計算中，如何簡化 GPU 核心開發的痛點，之前的解決方案往往需要較高的技術門檻和繁瑣的配置。這個專案的推出正好填補了這一需求，並且有可能因為其在 LLM 領域的應用而受到更多關注。作者的背景和過去的經驗也為這個專案的信譽增添了不少分數。

## 適合誰使用

**目標受眾**：需要高效能 GPU 核心以支持 LLM 操作的研究團隊和開發者。

> [!example] 使用場景
> - AI 研究員用它來優化 LLM 的訓練過程，因為其提供的高效 GPU 核心能顯著提高訓練速度。
> - 資料科學家用它來實現複雜的模型推論，因為 TileLang 的簡單語法使得開發和調試過程變得更為高效。
> - 工程師用它來快速原型化新的 LLM 應用，因為其靈活的設計允許快速迭代和測試不同的核心功能。

## 架構分析

TileKernels 採用 TileLang 作為核心架構，這使得開發者能夠以簡單的 Python 語法來定義複雜的 GPU 核心。這種設計的優勢在於能夠快速迭代和優化性能，而代價則是需要依賴特定的硬體架構（如 NVIDIA SM90 或 SM100）。資料流方面，使用者可以通過高階 API 來調用底層的 GPU 核心，這些核心在 TileLang 中被編譯為高效的 CUDA 代碼。

選擇 TileLang 而非傳統的 CUDA 編程，帶來了更高的開發效率，但也意味著開發者需要學習新的語言和工具。擴展性方面，這個架構能夠支持多種 LLM 操作，但在面對更複雜的模型時，可能會遇到性能瓶頸。整體而言，TileKernels 的架構設計旨在平衡性能和開發效率，適合需要快速開發和測試的研究環境。

## 技術深入分析

TileKernels 的核心技術在於使用 TileLang 來表達 GPU 核心，這是一種專為高效能計算設計的語言。這種設計使得開發者能夠以更高層次的抽象來編寫代碼，並且 TileLang 會自動優化生成的 CUDA 代碼。這樣的架構能夠處理大規模的數據，並且在性能上接近硬體的極限。對於資源需求，TileKernels 需要 NVIDIA SM90 或 SM100 的 GPU，這限制了其在一般開發環境中的使用。選擇 TileLang 而非傳統的 CUDA 編程，雖然提升了開發效率，但也要求開發者學習新的語言和工具，這可能會增加學習曲線。技術風險方面，隨著專案的發展，API 可能會變動，這對於依賴於穩定 API 的生產環境來說是一個潛在的風險。整合方面，TileKernels 可以與 PyTorch 無縫整合，這使得它在現有的 AI 開發流程中非常友好，但對於不熟悉 PyTorch 的開發者來說，可能需要額外的學習時間。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，包含了基本的安裝和使用說明，但缺乏詳細的範例。安裝過程相對順暢，只需使用 pip 安裝即可。沒有提供多語言支持，對於非英語使用者可能會有一定的障礙。

## 優缺點分析

> [!success] 優點
> - 提供高效能的 GPU 核心，適合 LLM 操作。
> - 使用 TileLang 簡化開發流程，降低技術門檻。
> - 支持多種先進的 GPU 操作，如量化和 Gating。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於多代理系統的開發，而 TileKernels 更專注於 GPU 核心的優化。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供了不同的 AI 模型訓練工具，但不如 TileKernels 在 GPU 性能優化方面具備針對性。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於多代理系統的開發，並不專注於 GPU 核心的優化。 | 如果你的專案需要多代理系統的協同工作，而不僅僅是 GPU 性能的優化。 | medium，因為需要重新設計系統架構以適應多代理的模式。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供了不同的 AI 模型訓練工具，但不如 TileKernels 在 GPU 性能優化方面具備針對性。 | 如果你的團隊已經在使用其他 AI 訓練工具，並且不需要專門的 GPU 核心優化。 | low，因為可以直接使用現有的工具鏈。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **TileKernels** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於多代理系統的開發，並不專注於 GPU 核心的優化。 | 提供了不同的 AI 模型訓練工具，但不如 TileKernels 在 GPU 性能優化方面具備針對性。 |
> | 遷移成本 | - | medium，因為需要重新設計系統架構以適應多代理的模式。 | low，因為可以直接使用現有的工具鏈。 |
> | 適用場景 | 主要場景 | 如果你的專案需要多代理系統的協同工作，而不僅僅是 GPU 性 | 如果你的團隊已經在使用其他 AI 訓練工具，並且不需要專門的 |
...
- **[HIGH]** 在某些 GPU 上，性能可能無法達到預期，特別是非 SM90 或 SM100 架構的 GPU。
  - 解法：確保使用支持的 GPU 進行測試。
- [MEDIUM] 文檔不夠完善，對於新手來說可能會造成困惑。
  - 解法：參考社群討論或尋求幫助。
- **[HIGH]** 某些核心功能可能尚未經過充分測試，導致不穩定。
  - 解法：在非關鍵環境中測試新功能。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的 AI 研究團隊 | 非常適合 | 能夠快速開發和測試高效能的 GPU 核心，符合小型團隊的需求。 |
| 大型企業的生產環境 | 不適合 | 目前仍在 alpha 階段，穩定性不足。 |
| 需要快速原型化的研究項目 | 適合 | TileLang 的簡單語法能夠加速開發流程。 |
| 對 GPU 性能要求不高的專案 | 普通 | 可能會顯得過於複雜，且不必要。 |
...
> [!warning] 安全性快速掃描
> 低風險：這個工具本身不需要高權限，且不會存取敏感資料，但依賴的庫和工具需定期檢查安全性。
