...
**1.6k** stars · **36** stars/天 · 建立 44 天前 · Python · MIT
...
> **授權** MIT (商業友好) · **維護** Slow (最後推送 43 天前) · **貢獻者** 2 人 · **參與度** Low
> **適合** 需要在高效能 GPU 上運行 LLM 的研究團隊或開發者。
> **一句話重點** TileKernels 的創新在於使用 TileLang 專為 LLM 操作優化 GPU 核心，這在現有工具中尚屬首創。
...
> [!abstract] 核心創新
> 專為 LLM 操作優化的 GPU 核心，使用 TileLang 提供高效能計算。

## 專案簡介

TileKernels 是一個針對大型語言模型（LLM）操作的 GPU 核心庫，使用 TileLang 這個專門為高效能 GPU 核心設計的語言來實現。用戶可以透過簡單的 Python 語法來構建和優化這些核心，這使得開發過程更加靈活且高效。核心功能包括 Gating、MoE Routing、Quantization 等，這些功能能夠顯著提升模型的計算效率和性能。使用者只需安裝 Python 3.10 以上以及 PyTorch 2.10 以上，即可開始使用這個庫。這個工具的賣點在於其針對 GPU 性能的優化，能夠接近硬體性能的極限，並且已經在內部訓練和推理場景中得到應用。技術上，TileKernels 依賴於 NVIDIA SM90 或 SM100 架構的 GPU，並使用 CUDA Toolkit 13.1 進行加速，這使得它在高性能計算上具有優勢。

與其他類似工具相比，如 0x0funky/agent-sprite-forge 和 0xGF/boneyard，TileKernels 提供了更細緻的核心功能，特別是在量化和專家選擇方面的優化。這使得它在需要高效能計算的場景下表現更佳，尤其是在大型模型的訓練和推理中。使用者可能會遇到的問題包括訓練時間過長，這在熱門 Issues 中已經被提及。這個專案目前處於 Alpha 階段，文檔和代碼質量仍在改進中，因此在生產環境中使用時需謹慎考量。適合需要高效能 GPU 計算的研究團隊或開發者，尤其是那些專注於 LLM 的應用。未來六個月內，預期會有更多的功能和優化釋出。

**技術棧**：`TileLang` · `PyTorch 2.10` · `CUDA Toolkit 13.1`

## 重點功能

- Gating — 提供 Top-k 專家選擇和評分的功能，支援 Mixture of Experts 路由。
- MoE Routing — 包括令牌到專家的映射，融合擴展/縮減及權重正規化。
- Quantization — 支援每個令牌、每個區塊及每個通道的 FP8/FP4/E5M6 轉換，並融合 SwiGLU + 量化操作。
- Transpose — 支援批次轉置操作，提升數據處理效率。
- Engram — 提供融合 RMSNorm 的 Engram gating 核心，支援前向/反向傳播及權重梯度縮減。
- Manifold HyperConnection — 包括 Sinkhorn 正規化及混合分割/應用的超連接核心。
...
# 基於文件推測
# 使用 Gating 功能
result = tile_kernels.gating(input_data)
```
...
```python
[
  "# 前置條件：已安裝 TileKernels 和相關依賴\n",
  "import tile_kernels\n",
  "# 使用 Gating 功能\n",
  "result = tile_kernels.gating(input_data)\n",
  "# 預期輸出：返回經過 Gating 處理的結果"
]
```
...
> [!tip] 爆紅原因
> 建立 44 天就累積 1576 stars（36/天），forks 135（8.6%），這顯示出一定的社群關注度。主要貢獻者包括 Chenhao Xu 和 Xiangwen Wang，他們在 AI 和 GPU 計算領域有一定的背景。這個專案解決了在 LLM 訓練中對高效能 GPU 核心的需求，之前的解決方案往往缺乏針對性和優化。最近的推廣活動和社群討論可能也促進了它的曝光率。技術上，TileLang 的出現使得這種優化成為可能，因為它專注於 GPU 性能的提升。forks/stars 比率在 8.6%，顯示出有相當一部分用戶在實際修改和使用這個工具。

## 適合誰使用

**目標受眾**：需要在高效能 GPU 上運行 LLM 的研究團隊或開發者。

> [!example] 使用場景
> - 機器學習工程師用它來優化 LLM 的訓練過程，因為其專為 GPU 性能調整的核心能顯著提升訓練速度。
> - 研究人員用它來實現高效的專家模型路由，因為 Gating 和 MoE Routing 功能能夠有效管理計算資源。
> - 開發者用它來構建可擴展的 AI 應用，因為其提供的高級 API 使得模型集成變得簡單且高效。

## 架構分析

TileKernels 的架構基於 TileLang，這是一種專為高效能 GPU 核心設計的語言，能夠簡化 GPU 核心的開發過程。資料流方面，使用者可以透過 Python 語法調用各種核心功能，這些功能在底層使用 CUDA 進行加速。選擇 TileLang 而非其他通用語言的原因在於其能夠提供針對 GPU 性能的優化，這對於 LLM 訓練至關重要。這樣的設計使得開發者能夠快速實現和測試新算法，但也可能導致對於不熟悉 TileLang 的開發者有一定的學習曲線。隨著使用者數量的增加，擴展性可能成為一個問題，特別是在核心功能的穩定性和性能上。

## 技術深入分析

TileKernels 的核心技術機制依賴於 TileLang，這是一種專為高效能 GPU 核心設計的語言，能夠簡化開發過程並提高性能。使用者可以透過 Python 語法調用各種核心功能，這些功能在底層使用 CUDA 進行加速，這使得它能夠接近硬體性能的極限。這個工具能夠處理大規模的數據集，並且在內部訓練和推理中已經得到應用。選擇 TileLang 而非其他通用語言的原因在於其針對 GPU 性能的優化，這對於 LLM 訓練至關重要。這樣的設計使得開發者能夠快速實現和測試新算法，但也可能導致對於不熟悉 TileLang 的開發者有一定的學習曲線。隨著使用者數量的增加，擴展性可能成為一個問題，特別是在核心功能的穩定性和性能上。整合方面，TileKernels 可以與 PyTorch 無縫協作，這使得它在現有的 AI 生態系中具有良好的兼容性，並且能夠輕鬆融入 CI/CD 流程中。

## 優缺點分析

> [!success] 優點
> - 針對 LLM 操作進行了專門優化，能夠顯著提升性能。
> - 使用 TileLang 簡化了 GPU 核心的開發過程，降低了入門門檻。
> - 提供多種高級功能，如 Gating 和 MoE Routing，適合複雜的模型需求。

> [!danger] 缺點
> - 目前仍處於 Alpha 階段，穩定性和文檔質量有待提升。
> - 僅支援特定的 NVIDIA GPU 架構，限制了使用範圍。
> - 對於不熟悉 TileLang 的開發者，學習曲線可能較陡峭。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供類似的 GPU 核心功能，但不專注於 LLM 的特定優化，適合一般的 GPU 計算需求。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 主要針對量化和模型壓縮，與 TileKernels 的專家路由功能相比，功能較為單一。 |
| [2aronS/Duel-Agents](https://github.com/2aronS/Duel-Agents) | 專注於多代理系統的優化，而 TileKernels 更加關注於 LLM 的計算性能。 |
...
**社群活躍度**：社群活躍度中等，最近有合併請求和問題回應，但解決率較低。
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["GPU 加速","量化","大型語言模型"];
