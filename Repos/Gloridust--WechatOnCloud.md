...
**3.3k** stars · **112** stars/天 · 建立 29 天前 · TypeScript · 未標註授權
...
> **適合** 需要在自己的伺服器上運行多個微信實例並實現跨設備同步的企業和開發者。
> **一句話重點** 這個專案不僅提供了微信的雲端解決方案，還擴展到其他社交媒體平台，實現了真正的多端共享。
...
> **結論** 花 5 小時學習，2 小時整合，得到靈活的多端社交媒體管理，值得一試。

> [!abstract] 核心創新
> 支持在多個設備上共享同一微信會話，並且不需要修改微信客戶端。

## 專案簡介

WechatOnCloud 是一個基於 Docker 的平台，允許用戶在自己的 NAS 或伺服器上運行服務端微信和 Chromium 瀏覽器。用戶可以通過一個管理面板創建多個獨立的實例，每個實例都是一個獨立的容器，支持微信會話和各種社交媒體的網頁版。這樣的架構設計使得用戶能夠在不同設備上共享同一會話，無需修改微信客戶端。核心功能包括多實例管理、子賬號權限控制、文件傳輸和文本剪貼板等，所有操作都在一個統一的面板內完成。用戶只需使用 Docker 部署，並通過簡單的命令行指令來啟動服務，這大大降低了使用門檻。與傳統的微信桌面客戶端相比，這個方案提供了更靈活的多端使用體驗，且支持多種應用場景。技術上，它使用 KasmVNC 進行畫面串流，並且在安全性上也做了優化，確保面板不會直接暴露於公網，降低了潛在的安全風險。

**技術棧**：`TypeScript` · `Docker` · `KasmVNC`

## 重點功能

- 多實例管理 — 一個面板可管理多個獨立的微信或 Chromium 實例，每個實例互不干擾。
- 多端共享 — 允許多個用戶通過瀏覽器訪問同一會話，實現跨設備同步。
- 子賬號管理 — 允許管理員為不同用戶分配訪問權限，實現精細化管理。
- 文件傳輸 — 支持拖拽上傳和下載文件，方便用戶在實例間傳輸資料。
- 安全性優化 — 面板為唯一入口，並對 docker.sock 的訪問進行限制，確保安全性。
...
> [!tip] 爆紅原因
> 建立 29 天內累積 3255 stars（112/天），forks 748（23.0%），顯示出強烈的社群需求。作者 Gloridust 之前在開源社群活躍，這個專案解決了用戶在多設備上使用微信的痛點，特別是在需要長時間保持登入狀態的情境下。社群的反饋和需求驅動了這個專案的快速成長，並且在 Telegram 和 Twitter 上的討論也進一步推動了它的知名度。技術上，Docker 的普及使得這種雲端解決方案變得可行，且用戶對於跨平台的需求日益增加。

## 適合誰使用

**目標受眾**：需要在自己的伺服器上運行多個微信實例並實現跨設備同步的企業和開發者。

> [!example] 使用場景
> - 小型企業的 IT 管理員用它來在公司伺服器上運行多個微信實例，因為這樣可以集中管理和共享資訊，減少了人力成本。
> - 自由工作者用它來在不同設備上隨時訪問社交媒體帳號，因為這樣可以提高工作效率，隨時保持聯繫。
> - 開發者用它來測試多個社交媒體平台的網頁版，因為這樣可以在同一環境中快速切換，節省了測試時間。

## 架構分析

WechatOnCloud 的架構基於 Docker 容器化技術，每個實例都運行在獨立的容器中，以便於管理和資源隔離。面板作為唯一的入口，通過 docker.sock 來創建和管理實例，這樣的設計使得用戶能夠輕鬆地啟動、停止或重啟實例。使用 KasmVNC 進行畫面串流，這樣可以在瀏覽器中實現對實例的遠程操作。選擇 Docker 作為基礎架構的好處在於其可移植性和擴展性，然而，這也意味著用戶需要具備一定的 Docker 知識來進行部署和維護。整體而言，這個架構能夠支持高效的多端使用，但在資源管理上需要謹慎，特別是在高負載情況下可能會遇到性能瓶頸。

## 技術深入分析

WechatOnCloud 的核心技術機制是基於 Docker 容器化技術，使用 KasmVNC 進行畫面串流，這使得用戶可以在瀏覽器中遠程操作微信和其他社交媒體的網頁版。每個實例都是獨立的容器，這樣可以有效地隔離資源，避免不同實例之間的干擾。實測顯示，單個實例在空閒狀態下的 CPU 使用率約為 0.1-0.2 核，內存使用量約為 0.6 GiB，而在活躍狀態下，CPU 使用率可達 1 核，內存使用量在 1-1.5 GiB 之間。這些數據顯示出該系統在資源管理上的有效性，但也指出了內存是主要瓶頸。選擇 Docker 作為基礎架構的好處在於其可移植性和擴展性，然而，這也意味著用戶需要具備一定的 Docker 知識來進行部署和維護。技術風險方面，若用戶未妥善管理伺服器的安全性，可能會導致敏感資料洩露。整合方面，該工具與主流的 Docker 生態系統相容，並且能夠輕鬆地與 CI/CD 流程結合，這使得團隊在部署和維護上能夠更加高效。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，包含詳細的安裝步驟和使用指南。安裝過程相對順暢，但對於不熟悉 Docker 的用戶可能會有一定的學習曲線。文件目前僅提供英文版本，缺乏多語言支持。整體而言，花 30 分鐘應該能夠成功部署並運行起來。

## 優缺點分析

> [!success] 優點
> - 支持多種社交媒體平台的網頁版，提供靈活的使用場景。
> - 通過 Docker 部署，降低了安裝和配置的複雜性。
> - 提供了子賬號管理功能，便於企業用戶進行權限控制。

> [!danger] 缺點
> - 需要用戶自行管理伺服器的安全性，存在一定的風險。
> - 對於高頻使用的情境，可能需要更多的硬體資源。
> - 使用 Docker 需要一定的技術門檻，對新手不太友好。

> [!warning] 注意事項
> - 僅支援 x86_64 和 arm64 架構的 Docker 環境。
> - 需要用戶自行管理伺服器的安全性，避免面板暴露於公網。
> - 對於高頻使用的情境，可能需要更高的硬體資源以支持多個實例。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供了類似的多實例管理功能，但專注於代理和自動化任務，並不支持微信的直接運行。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 這是一個針對社交媒體的自動化工具，雖然也支持多平台，但不具備實時共享會話的能力。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供了多實例管理功能，但專注於代理和自動化任務，並不支持微信的直接運行。 | 如果你的需求主要是自動化任務而非社交媒體的實時互動，則可以選擇這個工具。 | low，因為功能上有一定的相似性，轉移過程相對簡單。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 這是一個針對社交媒體的自動化工具，雖然也支持多平台，但不具備實時共享會話的能力。 | 如果你的團隊已經在使用其他社交媒體自動化工具，並且不需要實時共享功能，則可以考慮這個選擇。 | medium，因為需要重新配置和適應不同的工作流。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **WechatOnCloud** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 提供了多實例管理功能，但專注於代理和自動化任務，並不支持微信的直接運行。 | 這是一個針對社交媒體的自動化工具，雖然也支持多平台，但不具備實時共享會話的能力。 |
> | 遷移成本 | - | low，因為功能上有一定的相似性，轉移過程相對簡單。 | medium，因為需要重新配置和適應不同的工作流。 |
> | 適用場景 | 主要場景 | 如果你的需求主要是自動化任務而非社交媒體的實時互動，則可以選 | 如果你的團隊已經在使用其他社交媒體自動化工具，並且不需要實時 |
...
- **[HIGH]** 首次新建實例若卡住報 `創建容器失敗`，可能是 Docker 守護進程拉取實例鏡像超時。
  - 解法：手動拉取所需的 Docker 鏡像以避免超時。
- **[HIGH]** 面板暴露於公網可能導致安全風險，特別是已登入的社交媒體帳號。
  - 解法：務必使用 VPN 或內網穿透技術來保護面板的安全。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 能夠集中管理多個社交媒體帳號，方便團隊協作。 |
| 大型企業的 IT 部門 | 適合 | 提供了精細化的權限控制和多實例管理功能。 |
| 個人開發者的社交媒體測試 | 普通 | 雖然功能強大，但對於單一用戶來說可能過於複雜。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，2 小時整合，得到靈活的多端社交媒體管理，值得一試。

## 安全性評估

> [!warning] 安全性快速掃描
> 中等風險：面板需要管理 Docker 的權限，可能會接觸敏感資料。建議在內網環境中運行，並確保強密碼和 HTTPS 配置。
...
> | --- | --- | --- | --- |
> | [#97](https://github.com/Gloridust/WechatOnCloud/issues/97) | 新版并远程访问可以打开面板登录界面，但不能登录 | 0 | 0 |
> | [#96](https://github.com/Gloridust/WechatOnCloud/issues/96) | 剪贴板内容字体颜色异常（白底白字无法显示） | 0 | 0 |
