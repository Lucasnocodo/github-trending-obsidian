...
use_case: "在自己的 NAS 或伺服器上運行服務端微信，實現多端共享與跨設備同步。"
priority: high
ring: assess
...
star_history: "2026-06-03:1468,2026-06-03:1480,2026-06-04:1941,2026-06-04:1941,2026-06-05:2054,2026-06-05:2054,2026-06-06:2116,2026-06-07:2144,2026-06-08:2176,2026-06-09:2260,2026-06-10:2286,2026-06-11:2305,2026-06-12:2318,2026-06-13:2335,2026-06-14:2531,2026-06-15:2694,2026-06-16:2784,2026-06-17:2829,2026-06-18:2883,2026-06-19:2937,2026-06-20:2980"
tags:
  - github
  - "category/其他"
  - "lang/typescript"
aliases:
  ...
  - "在自己的 NAS 或伺服器上運行服務端微信，實現多端共享與跨設備同步。"
---

# WechatOnCloud

**3.0k** stars · **142** stars/天 · 建立 21 天前 · TypeScript · 未標註授權
...
> **適合** 需要在 NAS 或伺服器上運行多個社交媒體帳號的中小型企業或自由職業者。
> **一句話重點** 這個專案讓用戶能夠在自己的伺服器上輕鬆運行多個社交媒體帳號，實現跨設備同步，特別適合需要長時間保持登錄狀態的用戶。
...
> **結論** 花 5 小時學、3 小時整合，得到高效的社交媒體管理，值得一試。

> [!abstract] 核心創新
> 提供一個基於 Docker 的多實例管理平台，支持微信和其他社交媒體的同時運行。

## 專案簡介

WechatOnCloud 是一個基於 Docker 的平台，允許用戶在自己的 NAS 或伺服器上運行服務端微信，並支持多個瀏覽器實例共享同一會話。用戶可以透過面板管理多個獨立的容器，每個容器可以運行微信或 Chromium 瀏覽器，並通過 KasmVNC 將畫面串流到瀏覽器。這樣的設計使得用戶無需修改微信客戶端即可實現跨設備同步，並且支持多個用戶同時訪問同一會話。核心指令包括 `docker compose up -d` 來啟動服務，並可透過面板一鍵更新實例。這個工具的賣點在於其靈活性和易用性，特別適合需要同時管理多個社交媒體帳號的用戶。技術上，WechatOnCloud 使用了 Docker 容器技術，並且支持多架構（amd64 和 arm64），這使得它能夠在各種硬體平台上運行。

相比於其他類似工具，如 `0x0funky/agent-sprite-forge`，WechatOnCloud 提供了更直觀的用戶界面和更強的多實例管理能力。使用者可以在面板內輕鬆創建和管理實例，而不需要深入了解 Docker 的底層操作。實際使用中，內存是主要瓶頸，建議至少配置 4 核 / 8 GiB 的宿主機來運行多個實例。這個專案目前處於穩定階段，適合中小型團隊使用，預計未來會增加更多的安全性功能。對於需要長時間保持社交媒體登錄狀態的用戶，WechatOnCloud 是一個理想的選擇。使用者應避免將面板暴露於公網，並確保修改預設密碼以增強安全性。

**技術棧**：`TypeScript` · `Docker` · `KasmVNC`

## 重點功能

- 多實例管理 — 一個面板可管理多個獨立容器，支持同時運行微信和 Chromium 瀏覽器。
- 跨設備同步 — 多個用戶可通過瀏覽器訪問同一會話，實現即時消息接收和發送。
- 一鍵更新 — 面板內可直接拉取新鏡像並自動重建，無需手動操作。
- 安全性設計 — 面板為唯一入口，並且 KasmVNC 憑據不會下發前端，增強安全性。
- 支持中文輸入 — 本地輸入法可直接在容器內使用，無需安裝額外的 IME。
...
> [!tip] 爆紅原因
> 建立 21 天內累積 2980 stars（142/天），forks 715（24.0%），這顯示出強烈的社群參與度。作者 Gloridust 之前在 Docker 和社交媒體整合方面有豐富經驗，這個專案解決了多帳號管理的痛點，特別是在需要跨設備同步的場景下。社群的活躍度和快速的問題解決率（88%）也顯示出這個專案的潛力和穩定性。

## 適合誰使用

**目標受眾**：需要在 NAS 或伺服器上運行多個社交媒體帳號的中小型企業或自由職業者。

> [!example] 使用場景
> - 小型企業的 IT 管理員用它來在內部伺服器上運行多個社交媒體帳號，因為這樣可以減少管理多個設備的麻煩，並且所有帳號都能在同一平台上訪問。
> - 自由職業者用它來在雲端運行微信和其他社交媒體，因為這樣可以隨時隨地訪問自己的帳號，並保證登錄狀態不會丟失。
> - 開發者用它來測試社交媒體 API，因為可以在多個實例中同時運行不同的帳號，方便進行功能測試和調試。

## 架構分析

WechatOnCloud 採用 Docker 容器架構，每個實例運行在獨立的容器中，並通過 KasmVNC 將畫面串流到瀏覽器。這種設計使得用戶可以在同一平台上管理多個社交媒體帳號，並且每個帳號的數據都被隔離，避免了互相干擾。面板作為唯一的入口，通過 Docker API 來創建和管理實例，這樣的設計提高了安全性，但也增加了對 Docker 環境的依賴。整體架構的設計使得用戶可以輕鬆地擴展實例數量，但在資源使用上，特別是內存，則需要謹慎規劃，因為每個實例都會消耗一定的內存資源。

## 技術深入分析

WechatOnCloud 的核心技術機制是基於 Docker 的容器化架構，每個實例都在獨立的容器中運行，這樣的設計不僅提高了安全性，還使得資源的隔離變得更加簡單。使用 KasmVNC 進行畫面串流，這樣用戶可以通過瀏覽器訪問實例，實現多端共享。效能方面，單實例在空閒時的內存使用約為 0.6 GiB，而活躍時可達 1.5 GiB，這意味著在高負載情況下，內存使用會顯著增加。設計上選擇 Docker 主要是為了簡化部署和管理，但這也帶來了對 Docker 環境的依賴，對於不熟悉 Docker 的用戶來說，可能會增加上手難度。技術風險方面，隨著實例數量的增加，內存成為主要瓶頸，這可能會影響到整體的性能。整合方面，WechatOnCloud 能夠與現有的 Docker 環境無縫對接，但對於 CI/CD pipeline 的整合則需要額外的配置。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了詳細的安裝步驟和使用指南。安裝過程相對順暢，但對於不熟悉 Docker 的用戶可能會有一些挑戰。文件目前僅提供英文版本，缺乏多語言支持。

## 優缺點分析

> [!success] 優點
> - 支持多個社交媒體帳號的同時運行，方便用戶管理。
> - 界面友好，易於操作，適合不熟悉 Docker 的用戶。
> - 強大的安全性設計，減少了數據洩露的風險。

> [!danger] 缺點
> - 需要 Docker 環境，對於新手可能有一定的學習曲線。
> - 內存使用較高，運行多個實例時需謹慎規劃資源。
> - 不支持在公網上直接訪問，需額外配置安全措施。

> [!warning] 注意事項
> - 僅支援 x86_64 和 arm64 架構的宿主機。
> - 需要 Docker 環境，對於不熟悉 Docker 的用戶可能有一定的學習曲線。
> - 面板不應該暴露於公網，需確保安全性設置。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 這個工具主要針對 AI agent 的管理，而 WechatOnCloud 專注於社交媒體的多實例管理，提供更直觀的用戶界面和多應用支持。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard 是一個針對 Docker 的管理工具，但不支持社交媒體的多帳號管理，WechatOnCloud 提供了更專業的社交媒體管理功能。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 主要針對 AI agent 的管理，WechatOnCloud 專注於社交媒體的多實例管理，提供更直觀的用戶界面和多應用支持。 | 如果你的需求是管理 AI agent 而非社交媒體帳號，則可以選擇這個工具。 | medium，因為需要重新設計整個管理流程。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard 是一個針對 Docker 的管理工具，但不支持社交媒體的多帳號管理，WechatOnCloud 提供了更專業的社交媒體管理功能。 | 如果你只需要一個通用的 Docker 管理工具，而不需要社交媒體的功能，則可以考慮這個選擇。 | low，因為這是一個通用工具，容易上手。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **WechatOnCloud** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 主要針對 AI agent 的管理，WechatOnCloud 專注於社交媒體的多實例管理，提供更直觀的用戶界面和多應用支持。 | Boneyard 是一個針對 Docker 的管理工具，但不支持社交媒體的多帳號管理，WechatOnCloud 提供了更專業的社交媒體管理功能。 |
> | 遷移成本 | - | medium，因為需要重新設計整個管理流程。 | low，因為這是一個通用工具，容易上手。 |
> | 適用場景 | 主要場景 | 如果你的需求是管理 AI agent 而非社交媒體帳號，則可 | 如果你只需要一個通用的 Docker 管理工具，而不需要社交 |
...
- **[HIGH]** 首次新建實例若卡住報 `创建容器失败：… registry-1.docker.io … timeout`，可能是 Docker 守護進程拉取實例鏡像超時。
  - 解法：先在 NAS 的 Docker 界面手動拉取一次鏡像，然後再新建實例。
- **[HIGH]** 面板暴露於公網可能導致安全風險。
  - 解法：務必在內網使用，或通過 VPN 訪問。
- [MEDIUM] 默認密碼未修改可能導致未授權訪問。
  - 解法：安裝後立即修改默認密碼。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型企業的 IT 管理員 | 非常適合 | 能夠有效管理多個社交媒體帳號，減少管理負擔。 |
| 自由職業者 | 適合 | 能隨時隨地訪問社交媒體帳號，保持登錄狀態。 |
| 開發者 | 普通 | 雖然可以用來測試社交媒體 API，但可能不如專業的測試工具方便。 |
| 大型企業的社交媒體團隊 | 不適合 | 可能需要更高的安全性和擴展性。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學、3 小時整合，得到高效的社交媒體管理，值得一試。

## 安全性評估

> [!warning] 安全性快速掃描
> 中等風險：面板需要 Docker 的 root 訪問權限，並且暴露了已登錄的社交媒體帳號，必須確保不在公網上使用，並修改預設密碼。
...
> | 貢獻者 | Commits |
> | --- | --- |
> | [@Gloridust](https://github.com/Gloridust) | 87 |
> | [@web-flow](https://github.com/web-flow) | 1 |
> | [@huglemon](https://github.com/huglemon) | 1 |
> | [@mkblbj](https://github.com/mkblbj) | 1 |

**最新版本**：v1.2.3 — v1.2.3 · 远程桌面崩溃自愈 + 深色模式 + 一键更新面板 (2026-06-18)

> [!info]- Release Notes
> ## 远程桌面稳定性
> - **修复长时间空闲后偶发的 KasmVNC 致命崩溃**（`Cannot read properties of undefined (reading 'lastActiveAt')`，桌面卡死/弹错）：现自动检测其致命错误浮层并**干净重连一次**（同「重新连接」的安全路径，5 分钟限 4 次防死循环），无需手动刷新。
> 
> ## 深色模式
> - 面板支持深色：**跟随系统 / 亮色 / 深色**，主页顶栏右上角图标一键切换（浏览器本地保存）。
> - **浏览器(Chromium)实例**可深色：管理员用该主题开关开启后，浏览器实例**重启后**以深色启动。
> - 微信：其内核不跟随容器桌面信号，**微信深色请在微信「设置 → 通用 → 深色模式」里选**（随数据卷持久化）。
> 
> ## 面板自更新
> - 新增 **「一键更新面板」**：面板内直接拉新镜像并自重建（健康检查 + 失败自动回滚），不必再去 docker / 飞牛手动 `pull`。各实例镜像可在「管理 → 升级」单独更新。
> 
> ## 构建 / 其它
> - 修复：`panel/.dockerignore` 防止宿主 node_modules 混入镜像；本地构建产出单一镜像，避免标签解析到旧镜像。
> 
> > 升级方式：面板「关于」点「一键更新面板」更新面板；「管理」对各实例点「升级」更新实例镜像。
...
> [!abstract] 最近 10 次 commit（2026-06-16 ~ 2026-06-18）
> **活躍天數** 3 天 · **最新 commit** fix(dark): 撤掉不稳定的容器 portal 方案，浏览器深色回到 --force-dark-mode
...
> | --- | --- | --- | --- |
> | [#73](https://github.com/Gloridust/WechatOnCloud/issues/73) | 微信code | 0 | 0 |
> | [#72](https://github.com/Gloridust/WechatOnCloud/issues/72) | 测试的数据导出，我这里会导致所有容器被停止消失 | 0 | 1 |
> | [#71](https://github.com/Gloridust/WechatOnCloud/issues/71) | 添加超链接 | 0 | 1 |
> | [#64](https://github.com/Gloridust/WechatOnCloud/issues/64) | 能否增加自定义数据存储位置的设置 | 0 | 2 |
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["Docker","多模態","社交媒體管理"];
