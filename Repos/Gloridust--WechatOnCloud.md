...
**3.4k** stars · **69** stars/天 · 建立 50 天前 · TypeScript · 未標註授權
...
> **適合** 需要在私有伺服器上運行微信並希望跨設備同步的普通用戶與開發者。
> **一句話重點** 這個專案讓用戶能夠在自己的伺服器上運行微信，實現了私有化的社交媒體管理。
...
> **結論** 花 5 小時學習，3 小時整合，能夠在自己的伺服器上運行多個微信實例，值得嘗試。

> [!abstract] 核心創新
> 支持在私有伺服器上運行多個微信實例，實現跨設備共享與同步。

## 專案簡介

WechatOnCloud 是一個基於 Docker 的平台，允許用戶在自己的 NAS 或伺服器上運行服務端微信及其他網頁應用。用戶可以通過面板管理多個獨立的實例，每個實例都是一個獨立的容器，支持微信和 Chromium 瀏覽器。這樣的設計使得用戶可以在多個設備上共享同一會話，並且不需要修改微信客戶端。每個實例運行在 Xvfb 虛擬顯示環境中，並通過 KasmVNC 將畫面串流到瀏覽器，實現了高效的多端使用體驗。

核心指令包括 `docker compose up -d` 來啟動服務，並可通過面板一鍵更新和管理實例。這個工具的賣點在於其靈活性和多應用支持，讓用戶能夠在同一平台上運行多種社交媒體應用，並且支持文件傳輸和剪貼板共享等功能。使用者只需在面板中選擇應用類型，便可輕鬆創建新的實例，並進行管理。整體架構設計上，面板是唯一的入口，並且採用 RBAC 進行權限管理，確保安全性。
...
- 多端共享 — 允許多個設備共享同一會話，支持文件傳輸和剪貼板功能。
- 安全性設計 — 面板為唯一入口，並採用 RBAC 進行權限控制。
- 按需下載微信 — 微信不打包在鏡像中，實例創建時可一鍵下載。
- 多架構支持 — 提供 amd64 和 arm64 的預構建鏡像，適用於不同的硬體平台。

## 快速開始

1. 克隆專案並構建本地鏡像
```bash
git clone https://github.com/Gloridust/WechatOnCloud.git WechatOnCloud && cd WechatOnCloud && cp .env.example .env && ./scripts/build-local.sh && docker compose up -d
```
2. 拉取官方鏡像
```bash
...
3. 訪問面板並創建實例
```bash
在瀏覽器中訪問 http://<your-server-ip>:36080，使用 .env 中的管理員賬號登錄。
```
...
> [!tip] 爆紅原因
> 建立 50 天內累積 3437 stars（69/天），forks 775（22.5%），顯示出強勁的社群支持。作者 Gloridust 先前在開源社群中有不錯的聲譽，這個專案解決了在私有伺服器上運行微信的需求，特別是在中國的用戶面臨的網絡限制問題。這個工具的推出正好填補了市場上缺乏類似解決方案的空白，並且在社交媒體的討論中引起了廣泛關注，尤其是在 Telegram 群組中。高 forks/stars 比率（22.5%）表明許多人在實際使用並進行修改，顯示出這個專案的實用性。

## 適合誰使用

**目標受眾**：需要在私有伺服器上運行微信並希望跨設備同步的普通用戶與開發者。

> [!example] 使用場景
> - 系統管理員用它來在自己的 NAS 上運行多個微信實例，因為這樣可以在不同設備間共享會話，提升工作效率。
> - 開發者用它來測試微信的 API，因為可以在獨立的容器中運行多個實例，避免環境干擾。
> - 普通用戶用它來在家中伺服器上運行微信，因為這樣可以在不依賴第三方服務的情況下保持隱私。

## 架構分析

WechatOnCloud 的架構基於 Docker，使用容器化技術來管理多個獨立的實例。每個實例運行在 Xvfb 虛擬顯示環境中，並通過 KasmVNC 將畫面串流到瀏覽器。這樣的設計使得用戶可以在不修改微信客戶端的情況下，實現多端共享。

面板作為唯一的入口，通過 docker.sock 與 Docker 引擎交互，實現實例的創建與管理。這種設計的代價在於需要較高的內存資源，尤其是在運行多個實例時。整體架構的擴展性良好，但對宿主機的硬體要求較高，特別是在內存和 CPU 資源的配置上。

## 技術深入分析

WechatOnCloud 使用 Docker 作為核心技術，將微信和 Chromium 實例運行在獨立的容器中。這樣的設計使得每個實例都能夠獨立運行，並且不會互相干擾。使用 KasmVNC 進行畫面串流，確保了用戶能夠在瀏覽器中流暢地使用這些應用。性能方面，單個實例在空閒狀態下約消耗 0.6 GiB 的內存，活躍時可達 1.5 GiB，這意味著在運行多個實例時，內存成為主要的瓶頸。選擇 Docker 作為平台的好處在於其輕量級和可擴展性，但也帶來了對宿主機資源的依賴。設計上，面板僅作為唯一的入口，並通過 RBAC 進行權限管理，這樣的設計能有效降低安全風險。與主流的 Docker 工具鏈相容性良好，能夠方便地整合到現有的 CI/CD pipeline 中。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了詳細的安裝步驟和使用說明。安裝過程中需要注意 Docker 的配置，對於不熟悉 Docker 的用戶可能會有一定的學習曲線。整體而言，花 30 分鐘能夠順利跑起來。
...
> [!danger] 缺點
> - 需要較高的硬體配置，特別是內存。
> - 安裝過程對 Docker 環境要求較高，對新手不友好。
> - 不建議在公網上使用，安全風險較大。

> [!warning] 注意事項
> - 僅支援 Docker 環境，需安裝 Docker 和 Docker Compose。
> - 不建議將面板暴露在公網上，需使用 VPN 或內網訪問。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 這個工具專注於 AI agent 的創建，而 WechatOnCloud 主要針對社交媒體的多端管理，功能範圍不同。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard 提供的是一個開放的社交媒體管理平台，而 WechatOnCloud 更加專注於微信的運行與管理，適用場景有所不同。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於 AI agent 的創建，使用的是 Python，而 WechatOnCloud 使用 TypeScript 和 Docker，針對社交媒體的多端管理。 | 如果你的團隊需要開發 AI agent，且希望使用 Python 進行開發，則可以選擇這個工具。 | medium，因為需要重新適應不同的技術棧和架構。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供開放的社交媒體管理平台，使用 Node.js，而 WechatOnCloud 更專注於微信的運行與管理。 | 如果需要一個更通用的社交媒體管理工具，且團隊熟悉 Node.js，則可以選擇這個工具。 | high，因為需要重寫大部分的業務邏輯。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **WechatOnCloud** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於 AI agent 的創建，使用的是 Python，而 WechatOnCloud 使用 TypeScript 和 Docker，針對社交媒體的多端管理。 | 提供開放的社交媒體管理平台，使用 Node.js，而 WechatOnCloud 更專注於微信的運行與管理。 |
> | 遷移成本 | - | medium，因為需要重新適應不同的技術棧和架構。 | high，因為需要重寫大部分的業務邏輯。 |
> | 適用場景 | 主要場景 | 如果你的團隊需要開發 AI agent，且希望使用 Pyth | 如果需要一個更通用的社交媒體管理工具，且團隊熟悉 Node. |
...
- **[HIGH]** 在某些 NAS 環境下，Docker 拉取鏡像時可能會超時。
  - 解法：手動拉取鏡像，或設置 Docker 加速器。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 能夠在私有伺服器上運行多個實例，方便團隊協作。 |
| 大型企業的社交媒體管理 | 不適合 | 對於大規模的社交媒體管理需求，可能需要更專業的解決方案。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，3 小時整合，能夠在自己的伺服器上運行多個微信實例，值得嘗試。

## 安全性評估

> [!warning] 安全性快速掃描
> 中等風險：面板需要訪問宿主的 docker.sock，可能存在安全隱患；建議在內網中使用，並設置強密碼。
...
> | 貢獻者 | Commits |
> | --- | --- |
> | [@Gloridust](https://github.com/Gloridust) | 126 |
...
**最新版本**：v1.4.7 — v1.4.7 — 修复「永久卡在旧版本、更新无效」+ 受限网络误判 (2026-07-18)

> [!info]- Release Notes
> > 修复两个会让面板「更新了却没变化 / 实例永远停在老镜像」的问题，尤其影响国内 NAS 用户。**建议升级。**
> 
> ## 🔴 修复：更新面板后仍是旧版本、反复更新无效（#107）
> 少数用户「一键更新面板」后，版本号仍停在很旧的值（如 v1.2.7），再点更新也没用。根因是自更新重建时错误地把旧容器里的版本号继承了下来，且会**自我延续**（每次更新都再次保留旧版本号），只能手动删容器才能逃出。已修复：版本号永远取新镜像自带的值。
> 
> > **已卡住的用户无需手删容器**——升级到本版后，下次点「一键更新面板」即可自动恢复正常。
> 
> ## 🟠 修复：受限网络下误判「镜像不存在」、实例卡在老镜像（#114）
> 国内 NAS 常见：`docker pull` 走加速镜像能拉到，但面板直连镜像仓库的校验请求不通。旧逻辑把「连不上」误当成「该版本不存在」，于是回退到本地可能几周前的旧镜像——表现为面板是新版、实例却一直连不上桌面/停在老镜像，还会显示误导性的「该版本可能未成功发布」。已修复：只有仓库**明确回答不存在**才回退；连不上时保持版本并交给 `docker pull`（走你的加速镜像）去拉，真拉不到才用本地镜像兜底并如实说明。
> 
> ---
...
> [!abstract] 最近 10 次 commit（2026-07-07 ~ 2026-07-16）
> **活躍天數** 3 天 · **最新 commit** fix(image): 区分「连不上仓库」与「版本不存在」，别再回退到过期镜像 (issue #114)
