...
use_case: "在自己的伺服器上運行多端共享的服務端微信，支持多種社交媒體應用。"
priority: high
ring: assess
...
aliases:
  ...
  - "在自己的伺服器上運行多端共享的服務端微信，支持多種社交媒體應用。"
---

# WechatOnCloud

**3.2k** stars · **119** stars/天 · 建立 27 天前 · TypeScript · 未標註授權
...
> **適合** 需要在自己的伺服器上運行多端共享社交媒體應用的個人或小型團隊。
> **一句話重點** 這個專案讓用戶能夠在自己的伺服器上運行多端共享的社交媒體應用，提升了使用的靈活性和安全性。
...
> **結論** 花 4 小時學習，2 小時整合，得到靈活的多端社交媒體管理，值得一試。

> [!abstract] 核心創新
> 支持在同一伺服器上運行多個社交媒體應用實例，並實現多端共享。

## 專案簡介

WechatOnCloud 是一個基於 Docker 的平台，允許用戶在自己的伺服器上運行服務端微信，並可同時管理多個獨立的應用實例。用戶可以透過面板創建和管理微信或 Chromium 瀏覽器的實例，實現多端共享同一會話。每個實例都是一個獨立的 Docker 容器，內部運行 Xvfb 虛擬顯示和應用，並透過 KasmVNC 將畫面串流到瀏覽器。這種設計使得用戶能夠在不同設備上無縫使用同一個應用，並且不需要修改微信客戶端。核心功能包括多實例管理、權限分配、文件傳輸和文本剪貼板等，這些功能都可以通過面板一鍵操作。使用者只需在 Docker 上運行面板，然後透過簡單的命令創建實例，便可開始使用。

這個工具的賣點在於其靈活性和易用性，特別適合需要在多個設備上使用社交媒體的用戶。技術上，該專案使用 TypeScript 和 Docker 技術，並支持多架構部署，適合各種硬體環境。與其他類似工具相比，如直接使用微信的桌面版，WechatOnCloud 提供了更高的靈活性和多應用支持，並且不需要依賴特定的操作系統或硬體配置。使用者可以在自己的伺服器上完全控制數據，避免了隱私風險。這個專案的社群活躍度高，問題解決率達到 89%，顯示出良好的維護狀況。適合小型團隊或個人用戶，尤其是那些需要在多個設備上使用社交媒體的場景。
...
- 權限分配 — 子帳號系統可根據需要分配訪問權限，實現 RBAC。
- 文件傳輸 — 支持拖拽上傳和下載文件，方便用戶之間的資料共享。
- 安全設計 — 面板為唯一入口，並且不直接暴露 Docker API，確保安全性。
...
> [!tip] 爆紅原因
> 建立 27 天就累積 3200 stars（119/天），forks 744（23.3%），這顯示出該專案在社群中的強烈需求。作者 Gloridust 在開源社群中有一定的影響力，且該專案解決了用戶在多設備使用微信時的痛點，之前的解決方案往往需要依賴特定的硬體或操作系統。最近的更新和修復也引起了使用者的關注，特別是針對黑屏問題的修復，這使得更多用戶願意嘗試這個工具。技術上，Docker 的普及使得這類工具的部署變得更加簡單，進一步促進了其流行。forks/stars 比率 23.3% 表示許多用戶不僅在觀望，還在積極修改和使用這個專案。

## 適合誰使用

**目標受眾**：需要在自己的伺服器上運行多端共享社交媒體應用的個人或小型團隊。

> [!example] 使用場景
> - 後端工程師用它來在自己的伺服器上運行服務端微信，因為這樣可以在多個設備上共享同一會話，提升工作效率。
> - 社交媒體經營者用它來同時管理多個社交媒體帳號，因為可以在同一平台上輕鬆切換不同的應用實例，節省時間。
> - 小型團隊用它來協作處理客戶的微信訊息，因為可以設置不同的子帳號，並根據需要分配訪問權限。

## 架構分析

WechatOnCloud 採用 Docker 容器化架構，將每個應用實例獨立運行在不同的容器中，這樣的設計使得資源管理更加靈活。面板作為唯一的入口，通過 Docker API 管理實例的生命週期，並使用 KasmVNC 將畫面串流到瀏覽器。這種設計使得用戶能夠在多個設備上共享同一會話，並且不需要修改應用本身。

選擇 Docker 作為基礎架構的原因在於其便捷的部署和管理能力，而使用 KasmVNC 則是為了提供高效的畫面串流服務。這樣的架構雖然增加了 Docker 的依賴，但卻大幅提升了用戶的使用體驗。擴展性方面，隨著實例數量的增加，內存佔用會隨之增加，因此在資源配置上需要謹慎考量。

## 技術深入分析

WechatOnCloud 的核心技術機制是基於 Docker 的容器化架構，每個應用實例獨立運行，這樣的設計使得用戶可以在同一伺服器上同時運行多個社交媒體應用。使用 KasmVNC 進行畫面串流，確保了高效的用戶體驗。效能方面，單個實例在空閒狀態下約佔用 0.6 GiB 的內存，活躍時可達 1.5 GiB，這意味著在資源配置上需要謹慎考量。選擇 Docker 作為基礎架構的原因在於其便捷的部署和管理能力，而 KasmVNC 的使用則是為了提供高效的畫面串流服務。這樣的選擇雖然增加了 Docker 的依賴，但卻大幅提升了用戶的使用體驗。技術風險方面，隨著實例數量的增加，內存佔用會隨之增加，這可能成為擴展的瓶頸。在整合方面，該專案與主流的 Docker 工具鏈相容性良好，並且支持多種硬體架構，便於部署和擴展。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝步驟和使用說明，並附有範例。安裝過程相對順暢，但需要用戶具備基本的 Docker 知識。文件沒有提供多語言版本，主要以中文為主，這對於非中文使用者可能造成一定的障礙。
...
> [!danger] 缺點
> - 需要一定的 Docker 知識才能部署。
> - 性能依賴於宿主機的資源配置，內存是主要瓶頸。
> - 不建議直接暴露於公網，需額外配置安全措施。

> [!warning] 注意事項
> - 僅支援 Docker 環境，需安裝 Docker 和 Docker Compose。
> - 不建議將面板暴露於公網，需使用 VPN 或內網穿透。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供類似的多應用管理功能，但主要針對遊戲和虛擬代理，適用場景不同。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於社交媒體自動化，但不支持多實例管理，功能範圍較窄。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於遊戲和虛擬代理的多應用管理，但不支持社交媒體的直接運行。 | 如果你的需求主要是針對遊戲或虛擬代理的管理，而不是社交媒體的使用。 | medium，因為需要重新配置和適應不同的架構。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供社交媒體自動化，但不支持多實例管理，功能範圍較窄。 | 如果你的需求主要是針對社交媒體的自動化，而不需要多實例的靈活性。 | low，因為功能相似，遷移成本較低。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **WechatOnCloud** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於遊戲和虛擬代理的多應用管理，但不支持社交媒體的直接運行。 | 提供社交媒體自動化，但不支持多實例管理，功能範圍較窄。 |
> | 遷移成本 | - | medium，因為需要重新配置和適應不同的架構。 | low，因為功能相似，遷移成本較低。 |
> | 適用場景 | 主要場景 | 如果你的需求主要是針對遊戲或虛擬代理的管理，而不是社交媒體的 | 如果你的需求主要是針對社交媒體的自動化，而不需要多實例的靈活 |
...
- **[HIGH]** 面板暴露於公網可能導致安全風險
  - 解法：建議使用 VPN 或內網穿透進行訪問

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 能夠靈活管理多個社交媒體帳號，提升團隊協作效率。 |
| 大型企業的社交媒體管理 | 普通 | 雖然支持多實例，但可能需要更強大的資源配置。 |
| 個人用戶需要在多設備上使用微信 | 非常適合 | 提供了便捷的多端共享功能，提升使用體驗。 |
| 需要高安全性的金融行業應用 | 不適合 | 面板暴露於公網可能導致安全風險。 |
...
> [!tip] 投入 vs 回報
> 花 4 小時學習，2 小時整合，得到靈活的多端社交媒體管理，值得一試。

## 安全性評估

> [!warning] 安全性快速掃描
> 中等風險：面板需要訪問 Docker API，存在一定的安全風險。建議在內網環境中使用，並設置強密碼以保護管理界面。
...
> | 貢獻者 | Commits |
> | --- | --- |
> | [@Gloridust](https://github.com/Gloridust) | 93 |
> | [@web-flow](https://github.com/web-flow) | 1 |
> | [@huglemon](https://github.com/huglemon) | 1 |
> | [@mkblbj](https://github.com/mkblbj) | 1 |

**最新版本**：v1.2.6 — v1.2.6 · 修复一键更新后远程桌面黑屏（关键） (2026-06-25)

> [!info]- Release Notes
> **关键修复**:此前用面板「一键更新面板」升级的用户,远程桌面会黑屏报「桌面服务暂时不可用」;直接 `docker compose pull` 的用户不受影响。本版修复其根因。强烈建议所有用过一键更新的部署升级到本版。
> 
> ## 根因
> 自更新重建面板容器时,错误地复刻了旧容器的 hostname(=旧容器短 ID)。新面板据此认为自己叫旧 ID,而旧容器已被删除 → 面板探测自身所在 Docker 网络失败 → 之后**新建/重启/升级/内存自愈软重启**的实例都落到了默认 `bridge` 网络(而非面板所在网络)→ 反代按容器名访问不到 → 502 黑屏。
> 
> ## 修复
> - 自更新重建面板时不再复刻旧 hostname(用新容器自身 ID)。
> - 网络探测增加按容器名(`woc-panel`)兜底,彻底不依赖 hostname 与容器 ID 一致。
> 
> ## 已中招用户的恢复步骤
> 1. 升级面板到 v1.2.6(「关于 → 一键更新面板」即可,本版的更新流程已修正;或 `docker compose pull && docker compose up -d`)。
> 2. 升级后,对**仍黑屏的实例点「管理 → 重启」**一次,它会重新接入正确网络即恢复(也可等内存自愈自动重启)。
> 
> 实例镜像无变化,无需「升级实例」。

## 社群與生態

**社群活躍度**：社群活躍，問題解決率達到 89%。
**連結**：[文件](https://github.com/Gloridust/WechatOnCloud)

## 開發動態

> [!abstract] 最近 10 次 commit（2026-06-18 ~ 2026-06-25）
> **活躍天數** 4 天 · **最新 commit** fix(self-update): 修一键更新后实例落错网络致黑屏（502）的根因
...
> | --- | --- | --- | --- |
> | [#89](https://github.com/Gloridust/WechatOnCloud/issues/89) | 许愿新增一个修改用户名的功能。万分感谢。 | 0 | 1 |
> | [#87](https://github.com/Gloridust/WechatOnCloud/issues/87) | 希望能支持微信的导出导入功能 | 0 | 0 |
> | [#86](https://github.com/Gloridust/WechatOnCloud/issues/86) | 有没有打算出一个微信聊天记录备份的项目 | 0 | 1 |
...
相關專案：[[0x0funky--agent-sprite-forge|0x0funky/agent-sprite-forge]] · [[0xGF--boneyard|0xGF/boneyard]] · [[2aronS--Duel-Agents|2aronS/Duel-Agents]] · [[Gloridust--WechatOnCloud|Gloridust/WechatOnCloud]] · [[louislva--claude-peers-mcp|louislva/claude-peers-mcp]] · [[op7418--Claude-to-IM-skill|op7418/Claude-to-IM-skill]] · [[BigBodyCobain--Shadowbroker|BigBodyCobain/Shadowbroker]] · [[0xSero--codex-shim|0xSero/codex-shim]]
