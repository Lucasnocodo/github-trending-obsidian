...
**3.4k** stars · **86** stars/天 · 建立 39 天前 · TypeScript · 未標註授權
...
> **適合** 需要在 NAS 或伺服器上運行微信和其他網頁應用的開發者和技術愛好者。
> **一句話重點** 這個專案讓用戶能夠在自己的伺服器上運行多個社交媒體應用，實現真正的多端同步。
...
> **結論** 花 4 小時學習，2 小時整合，得到靈活的多端社交媒體使用體驗，值得嘗試。

> [!abstract] 核心創新
> 支持在多端共享同一會話的服務端微信運行環境。

## 專案簡介

WechatOnCloud 是一個基於 Docker 的平台，允許用戶在自己的 NAS 或伺服器上運行服務端微信及其他網頁應用。用戶可以通過一個管理面板創建多個獨立的實例，每個實例都是一個獨立的容器，支持微信和 Chromium 瀏覽器等應用。這樣的設計使得多端共享同一會話成為可能，並且不需要修改微信客戶端。核心機制是每個實例運行在 Xvfb 虛擬顯示環境中，並通過 KasmVNC 將畫面串流到瀏覽器，這樣用戶可以在多個設備上訪問同一個會話。最關鍵的 CLI 指令是 `docker compose up -d`，用於啟動面板和實例。這個工具的賣點在於它的多應用支持和靈活的實例管理，能夠有效地解決用戶在多設備使用時的同步問題。

**技術棧**：`TypeScript` · `Docker` · `JavaScript` · `CSS` · `Shell`

## 重點功能

- 多實例管理 — 一個面板管理多個獨立實例，每個實例都是獨立的容器和數據卷。
- 多應用支持 — 可選擇運行微信或 Chromium 瀏覽器，支持多種網頁應用。
- 多端共享 — 多設備可同時訪問同一會話，支持子賬號管理和權限分配。
- 實例生命周期管理 — 一鍵啟動、停止、重啟和升級實例，方便管理。
- 安全優先 — 面板為唯一入口，並且對 docker.sock 的訪問進行嚴格控制。
...
[
  ...
  "docker compose up -d",
  "# 預期輸出：啟動面板和實例容器，並在瀏覽器中訪問 http://localhost:36080"
]
```
...
> [!tip] 爆紅原因
> 建立 39 天內累積 3362 stars（86/天），forks 761（22.6%），顯示出強烈的社群興趣。作者 Gloridust 及其團隊在開源社群中活躍，之前的項目也獲得了良好的反響。這個工具解決了在多設備上使用微信的痛點，之前用戶通常需要依賴手機或模擬器，無法實現真正的多端同步。最近的推廣活動和社群互動也促進了其快速增長。多架構支持（amd64 和 arm64）使得它能夠在更廣泛的硬體上運行，這也提升了其吸引力。forks/stars 比率為 22.6%，顯示出許多人在實際修改和使用這個工具。

## 適合誰使用

**目標受眾**：需要在 NAS 或伺服器上運行微信和其他網頁應用的開發者和技術愛好者。

> [!example] 使用場景
> - 後端工程師用它來在自己的伺服器上運行微信，因為這樣可以實現多端同步，無需依賴手機。
> - 產品經理用它來測試微信的各種功能，因為可以同時在多個設備上共享同一會話，方便進行協作。
> - 開發者用它來運行 Chromium 瀏覽器實例，因為可以在雲端持久保持登錄狀態，隨時訪問社交媒體和其他網頁應用。

## 架構分析

WechatOnCloud 採用 Docker 容器化架構，允許用戶在自己的伺服器上運行多個獨立的應用實例。每個實例運行在 Xvfb 虛擬顯示環境中，並通過 KasmVNC 將畫面串流到瀏覽器。這樣的設計使得用戶可以在多個設備上共享同一會話，並且不需要修改微信客戶端。面板作為唯一的入口，通過 docker.sock 控制實例的創建和銷毀，這樣的設計提高了安全性，但也要求用戶對 Docker 有一定的了解。由於容器不設硬性 CPU/內存上限，資源的使用效率較高，但在高負載情況下可能會遇到性能瓶頸。

## 技術深入分析

WechatOnCloud 的核心技術在於其容器化架構，利用 Docker 來管理多個應用實例。每個實例運行在獨立的容器中，並使用 Xvfb 虛擬顯示來支持圖形界面，這使得用戶可以在瀏覽器中訪問這些應用。KasmVNC 提供了畫面串流功能，確保用戶能夠流暢地使用應用。這種設計使得多端共享成為可能，並且不需要對微信客戶端進行任何修改。性能方面，實測顯示單個實例在空閒時的內存使用量約為 0.6 GiB，活躍時可達 1.5 GiB，這對於大多數現代伺服器來說是可接受的。選擇 Docker 作為主要技術棧的好處在於其輕量和易於擴展，但也帶來了對於 Docker 環境的依賴。整體來看，這個專案在技術上是成熟的，並且能夠滿足大多數用戶的需求。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且詳細，包含了安裝和使用的步驟。安裝過程相對順暢，但需要用戶對 Docker 有基本了解。提供了良好的快速開始指南，讓新手能夠在短時間內上手。文件沒有多語言支持，僅有英文版本。
...
> - 不建議將面板暴露於公網，需透過 VPN 或內網訪問。
> - 對於高負載情況，可能需要更多的硬體資源來支持多個實例。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於代理和自動化，而 WechatOnCloud 更專注於多端同步和社交媒體的使用。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供類似的雲端應用支持，但不具備多實例管理的功能。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於代理和自動化，而 WechatOnCloud 更專注於多端同步和社交媒體的使用。 | 如果你的需求主要是自動化和代理功能，而不是社交媒體的多端使用。 | medium，因為需要重新配置和適應不同的使用場景。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供類似的雲端應用支持，但不具備多實例管理的功能。 | 如果你只需要一個簡單的雲端應用，而不需要多實例的管理功能。 | low，因為功能較為簡單，易於上手。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **WechatOnCloud** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於代理和自動化，而 WechatOnCloud 更專注於多端同步和社交媒體的使用。 | 提供類似的雲端應用支持，但不具備多實例管理的功能。 |
> | 遷移成本 | - | medium，因為需要重新配置和適應不同的使用場景。 | low，因為功能較為簡單，易於上手。 |
> | 適用場景 | 主要場景 | 如果你的需求主要是自動化和代理功能，而不是社交媒體的多端使用 | 如果你只需要一個簡單的雲端應用，而不需要多實例的管理功能。 |
...
- **[HIGH]** 首次新建實例若卡住，可能是 Docker 拉取鏡像超時
  - 解法：手動拉取鏡像以避免超時問題
- **[HIGH]** 默認密碼未改變可能導致安全風險
  - 解法：首次登錄後立即修改默認密碼
- [MEDIUM] 多實例同時操作時可能會出現衝突
  - 解法：使用軟鎖功能來避免衝突
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型團隊需要在內部伺服器上運行微信 | 非常適合 | 支持多實例和多端共享，能夠滿足團隊需求。 |
| 個人用戶希望在雲端使用社交媒體 | 適合 | 提供了方便的雲端訪問和多端同步功能。 |
| 大型企業需要高可用性和擴展性 | 不適合 | 目前仍處於 beta 階段，穩定性和性能可能不足以支持大型應用。  |
...
> [!tip] 投入 vs 回報
> 花 4 小時學習，2 小時整合，得到靈活的多端社交媒體使用體驗，值得嘗試。

## 安全性評估

> [!warning] 安全性快速掃描
> 這個工具需要對 Docker 有一定的了解，並且在使用時必須注意安全性，避免將面板暴露於公網。面板會存取宿主的 docker.sock，這意味著有高權限的操作風險。
...
> | 貢獻者 | Commits |
> | --- | --- |
> | [@Gloridust](https://github.com/Gloridust) | 122 |
...
**最新版本**：v1.4.4 — v1.4.4 — 实例镜像恢复正常发布（原生 arm64 构建）+ v1.4.3 全部改进 (2026-07-07)

> [!info]- Release Notes
> > v1.4.1/v1.4.3 的实例镜像因 CI 的 arm64 构建（QEMU 模拟）内存溢出而未能发布。本版**改用原生 arm64 构建**彻底解决，实例镜像恢复正常发布。之前只更新了面板、实例升级不到新版的用户，现在可以正常升级了。
> 
> ## 本版要点
...
>   - **设计系统文档** `doc/dev/design.md`。
> 
> ## 升级（两步，都在面板里）
> 1. 设置 → 关于 →「一键更新面板」→ 确认版本 **v1.4.4**。
> 2. 管理 →「一键升级全部实例」→ 实例卡角标应显示 **镜像 v1.4.4**。
> 
...
> **活躍天數** 2 天 · **最新 commit** docs(p0): 记录 QEMU OOM 的最终解决——改用原生 arm64 runner（v1.4.4 闭环）
