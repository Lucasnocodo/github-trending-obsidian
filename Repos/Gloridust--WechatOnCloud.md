...
use_case: "在自己的 NAS 或伺服器上運行服務端微信，實現多端共享和消息同步。"
priority: high
ring: assess
...
star_history: "2026-06-03:1468,2026-06-03:1480,2026-06-04:1941,2026-06-04:1941,2026-06-05:2054,2026-06-05:2054,2026-06-06:2116,2026-06-07:2144,2026-06-08:2176,2026-06-09:2260"
tags:
  - github
  - "category/其他"
  - "lang/typescript"
aliases:
  ...
  - "在自己的 NAS 或伺服器上運行服務端微信，實現多端共享和消息同步。"
---

# WechatOnCloud

**2.3k** stars · **226** stars/天 · 建立 10 天前 · TypeScript · 未標註授權
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Recent · **熱度** Hot (226 stars/day)
> **授權** 未標註授權 (風險較高) · **維護** Active (最後推送 3 天前) · **貢獻者** 3 人 · **參與度** Medium
> **適合** 需要在 NAS 或伺服器上運行多個微信實例的企業或個人用戶。
> **一句話重點** 這個專案讓用戶能夠在自己的伺服器上安全地運行多個微信實例，實現消息的跨設備同步。
...
> **結論** 花 4 小時學習，2 小時整合，得到靈活的多實例管理，值得一試。

> [!abstract] 核心創新
> 每個微信實例運行在獨立的 Docker 容器中，實現多端共享和消息同步。

## 專案簡介

WechatOnCloud 是一個在 NAS 或伺服器上運行的服務端微信解決方案，允許用戶管理多個獨立的微信實例。每個實例運行在 Docker 容器中，並通過 KasmVNC 將畫面串流到瀏覽器，實現多端共享同一微信會話。用戶只需在面板上點擊幾下即可創建或刪除實例，並可通過拖拽方式進行文件傳輸。這種設計的賣點在於無需修改微信客戶端，並且支持多用戶同時訪問同一會話。

技術上，該專案使用 TypeScript 和 Docker，並且提供了多架構的預構建鏡像，適用於不同的 CPU 架構。相比於其他類似工具，如 0x0funky/agent-sprite-forge 和 0xGF/boneyard，WechatOnCloud 提供了更簡單的多實例管理和權限控制，並且不需要修改微信客戶端的原始碼。使用者在實際操作中可能會遇到如登錄問題或聊天記錄遷移等常見問題，但社群的活躍度和問題解決率相對較高。這個專案目前處於穩定階段，適合需要在多設備上共享微信會話的團隊或個人使用。

**技術棧**：`TypeScript` · `Docker` · `KasmVNC`

## 重點功能

- 多實例管理 — 一個面板管理多個獨立的微信實例，每個實例都有自己的數據卷，互不干擾。
- 多端共享 — 允許多個用戶通過不同設備共享同一微信會話，支持權限分配。
- 微信本體運行時下載 — 面板一鍵下載和安裝官方微信，無需將其打包進鏡像。
- 文件傳輸 — 支持拖拽上傳和下載文件，並可通過剪貼板傳送文本。
- 安全優先 — 面板為唯一入口，並且 KasmVNC 憑據不會下發給前端，確保安全性。
...
```ts
{
  ...
  "預期輸出": "啟動面板容器，並在瀏覽器中訪問 http://<你的IP>:36080 進行登錄。"
}
```
...
> [!tip] 爆紅原因
> 建立 10 天內累積 2260 stars（226/天），forks 612（27.1%），顯示出強烈的社群需求。作者 Gloridust 之前有開發相關的開源專案，這次專案解決了在多設備上使用微信的痛點，特別是對於需要在企業環境中共享微信的用戶。社群活躍度高，問題解決率達到 56%。

## 適合誰使用

**目標受眾**：需要在 NAS 或伺服器上運行多個微信實例的企業或個人用戶。

> [!example] 使用場景
> - 企業 IT 管理員用它來在公司伺服器上運行多個微信實例，因為這樣可以讓不同部門的員工共享同一個微信會話，提升工作效率。
> - 自由職業者用它來在自己的 NAS 上運行微信，因為這樣可以在不同設備上同步消息，避免漏接客戶訊息。
> - 開發者用它來測試微信的多用戶功能，因為這樣可以快速創建和刪除實例，並且不需要修改客戶端。

## 架構分析

WechatOnCloud 採用 Docker 容器化架構，每個微信實例都運行在獨立的容器中，這樣的設計使得每個實例的數據和運行環境互不干擾。面板作為唯一的入口，通過 KasmVNC 將畫面串流到瀏覽器，這樣用戶可以在不同設備上共享同一會話。這種架構的優勢在於靈活性和可擴展性，但也帶來了對資源的需求，特別是在內存方面。每個實例的運行需要一定的內存和 CPU 資源，這可能在實例數量增加時成為瓶頸。整體架構設計考慮了安全性，面板不直接暴露給外部網絡，並且使用了多層安全措施來保護用戶數據。

## 技術深入分析

WechatOnCloud 的核心技術機制是基於 Docker 容器化架構，這使得每個微信實例都能在獨立的環境中運行，避免了數據之間的干擾。使用 KasmVNC 將畫面串流到瀏覽器，這樣的設計不僅提高了用戶的操作靈活性，還能支持多用戶同時訪問同一會話。從性能上看，每個實例在空閒時的 CPU 使用率約為 0.1–0.2 核，活躍時可達 0.5–1 核，這意味著在資源配置上需要謹慎考量。選擇 Docker 作為技術棧的原因在於其便於管理和擴展，但也帶來了對內存的需求，特別是在多實例運行時，內存使用可能成為瓶頸。設計上，面板的安全性考量非常周全，所有操作都需要經過授權，並且不會將敏感信息暴露給前端。這樣的設計使得 WechatOnCloud 在安全性和靈活性之間取得了良好的平衡。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了詳細的安裝步驟和使用說明。安裝過程相對順暢，但需要用戶具備一定的 Docker 知識。文件中有針對新手的指南，幫助用戶快速上手。文件目前僅提供中文，對於非中文用戶可能會有一定的障礙。

## 優缺點分析

> [!success] 優點
> - 支持多實例管理，便於用戶在不同設備間共享微信會話。
> - 安全性高，面板為唯一入口，並且不會將敏感憑據下發給前端。
> - 使用 Docker 容器化技術，便於部署和管理。

> [!danger] 缺點
> - 不保證不被封，使用風險需自行承擔。
> - 需要一定的技術背景來配置和管理 Docker 環境。
> - 在內存使用上可能會成為瓶頸，特別是實例數量較多時。

> [!warning] 注意事項
> - 不保證不被封，風控會持續加新維度。
> - 需要在內網環境中使用，避免直接暴露於公網。
> - 在使用過程中可能會遇到登錄問題或聊天記錄遷移的困難。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供類似的多實例管理，但不支持微信的直接運行，需額外配置。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於即時通訊的其他平台，功能上不如 WechatOnCloud 針對微信的優化。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 使用 Rust 實作，記憶體用量較少，但功能上不如 WechatOnCloud 專注於微信的多實例管理。 | 如果你的團隊需要一個輕量級的解決方案，並且不需要專門針對微信的功能。 | medium，因為需要重新配置和適應不同的使用流程。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於即時通訊的其他平台，功能上不如 WechatOnCloud 針對微信的優化。 | 如果你的團隊已經在使用其他即時通訊工具，並希望整合到現有的工作流中。 | high，因為需要重新設計整個通訊流程。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **WechatOnCloud** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 使用 Rust 實作，記憶體用量較少，但功能上不如 WechatOnCloud 專注於微信的多實例管理。 | 專注於即時通訊的其他平台，功能上不如 WechatOnCloud 針對微信的優化。 |
> | 遷移成本 | - | medium，因為需要重新配置和適應不同的使用流程。 | high，因為需要重新設計整個通訊流程。 |
> | 適用場景 | 主要場景 | 如果你的團隊需要一個輕量級的解決方案，並且不需要專門針對微信 | 如果你的團隊已經在使用其他即時通訊工具，並希望整合到現有的工 |
...
- **[HIGH]** 登錄後自動彈出重新登錄，可能是因為 session 管理問題。
  - 解法：檢查面板的 session 設置，確保不會過早過期。
- [MEDIUM] 聊天記錄遷移可能會遇到文件大小不對的問題。
  - 解法：在遷移前確認文件大小，必要時手動調整。
- [MEDIUM] 使用過程中可能會遇到 KasmVNC 錯誤。
  - 解法：檢查容器日誌，確保 KasmVNC 正常運行。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 能夠靈活地管理多個微信實例，方便團隊協作。 |
| 大型企業需要穩定的即時通訊解決方案 | 普通 | 雖然功能強大，但可能需要額外的安全措施來保護數據。 |
| 個人使用者希望在多設備上同步微信 | 非常適合 | 提供了簡單的多端共享功能，方便日常使用。 |
...
> [!tip] 投入 vs 回報
> 花 4 小時學習，2 小時整合，得到靈活的多實例管理，值得一試。

## 安全性評估

> [!warning] 安全性快速掃描
> 中等風險：面板需要訪問 Docker 的權限，使用時需確保安全配置，避免公開暴露。
...
> | --- | --- | --- | --- |
> | [#42](https://github.com/Gloridust/WechatOnCloud/issues/42) | 聊天记录迁移 | 0 | 0 |
...
> | [#38](https://github.com/Gloridust/WechatOnCloud/issues/38) | 登陆后自动弹出，重新登陆，反复弹出重新登陆，怎么解决？ | 0 | 5 |
