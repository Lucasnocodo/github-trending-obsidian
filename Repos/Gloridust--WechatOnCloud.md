...
use_case: "在自己的 NAS 或伺服器上運行服務端微信，實現多端共享和跨設備同步。"
priority: high
ring: assess
...
aliases:
  ...
  - "在自己的 NAS 或伺服器上運行服務端微信，實現多端共享和跨設備同步。"
---

# WechatOnCloud

**3.5k** stars · **58** stars/天 · 建立 61 天前 · TypeScript · 未標註授權
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Growing · **熱度** Growing (58 stars/day)
> **授權** 未標註授權 (風險較高) · **維護** Active (最後推送 1 天前) · **貢獻者** 5+ 人 · **參與度** Medium
> **適合** 需要在自己的伺服器上運行微信並實現多端共享的用戶，尤其是企業或社交媒體經營者。
> **一句話重點** 這個專案的設計不僅解決了多設備使用微信的問題，還提供了靈活的多應用支持，讓用戶能夠在同一環境中管理多個社交媒體帳號。
...
> **結論** 花 5 小時學習，3 小時整合，得到靈活的多實例管理，值得考慮。

> [!abstract] 核心創新
> 支持多應用平台，允許用戶在同一面板中運行微信和多個社交媒體的網頁版。

## 專案簡介

WechatOnCloud 是一個基於 Docker 的服務端微信解決方案，允許用戶在自己的 NAS 或伺服器上運行多個獨立的微信實例。用戶可以通過瀏覽器訪問這些實例，實現跨設備的消息同步和共享。每個實例都是一個獨立的容器，內部運行 Xvfb 虛擬顯示和官方微信或 Chromium 瀏覽器，並通過 KasmVNC 將畫面串流到瀏覽器。這種設計使得用戶無需修改微信客戶端即可實現多端共享，並且支持多種社交媒體平台的網頁版。核心指令包括 `docker-compose up -d` 來啟動服務，並可一鍵下載和安裝微信。

這個工具的賣點在於其靈活性和易用性，適合需要在多個設備上共享同一微信會話的用戶。其技術架構基於 Docker，支持多架構（amd64 / arm64），並且提供了完善的安全性設計，確保用戶數據的安全性。相較於其他類似工具，WechatOnCloud 提供了更為直觀的多實例管理界面和更高的安全性，特別是在處理敏感數據時。使用者可以在面板中輕鬆管理實例的生命周期，包括啟動、停止和升級，並且支持文件傳輸和文本剪貼板功能，增強了用戶的操作體驗。這個專案在 61 天內累積了 3519 顆星（每天約 58 顆），顯示出其在開發者社群中的受歡迎程度。

**技術棧**：`TypeScript` · `Docker` · `KasmVNC`

## 重點功能

- 多實例管理 — 一個面板管理多個獨立實例，每個實例都是獨立容器和數據卷。
- 多應用支持 — 可選擇運行微信或 Chromium 瀏覽器，支持多種社交媒體平台。
- 文件傳輸 — 支持拖拽上傳和下載文件，並可通過剪貼板中轉文本。
- 安全性設計 — 面板為唯一入口，並且 KasmVNC 憑據不下發給前端。
- 多端共享 — 多設備可共享同一會話，支持子賬號管理和權限分配。
...
> [!tip] 爆紅原因
> 建立 61 天內累積 3519 stars（每天約 58），forks 776（22.1%），顯示出強烈的社群參與度。作者 Gloridust 及其團隊在開源社群中活躍，解決了用戶在多設備使用微信時的痛點，之前的解決方案往往依賴於不穩定的第三方工具或服務。這個工具的出現正好滿足了用戶對於安全和穩定性的需求，並且其設計考量了多種使用場景，讓用戶能夠在自己的環境中運行。社群的活躍度和問題解決率（78%）也顯示出這個專案的成熟度和可靠性。

## 適合誰使用

**目標受眾**：需要在自己的伺服器上運行微信並實現多端共享的用戶，尤其是企業或社交媒體經營者。

> [!example] 使用場景
> - 後端工程師用它來在自己的伺服器上運行多個微信實例，因為這樣可以實現跨設備的消息同步，避免了多設備間的數據丟失。
> - 社交媒體經營者用它來同時管理多個社交媒體帳號，因為它支持在同一界面中運行 Telegram、X 和 Instagram 的網頁版，提升了工作效率。
> - 小型企業用它來集中管理客戶的微信聯絡，因為可以在一個面板中管理多個實例，並且支持多用戶協作。

## 架構分析

WechatOnCloud 採用 Docker 容器化架構，允許用戶在自己的伺服器上運行多個獨立的微信實例。每個實例都是一個獨立的容器，內部運行 Xvfb 虛擬顯示和應用程序，並通過 KasmVNC 將畫面串流到瀏覽器。這種設計使得用戶能夠在不修改微信客戶端的情況下實現多端共享。

面板作為唯一的入口，確保了安全性，並通過 docker.sock 進行實例的動態創建和管理。這種架構的代價在於需要一定的伺服器資源，並且對於不熟悉 Docker 的用戶來說，初始設置可能會有一定的學習曲線。整體來說，這種設計提供了靈活性和擴展性，適合需要在多設備上運行微信的用戶。

## 技術深入分析

WechatOnCloud 的核心技術機制是基於 Docker 的容器化架構，允許用戶在自己的伺服器上運行多個獨立的微信實例。每個實例內部運行 Xvfb 虛擬顯示和應用程序，並通過 KasmVNC 將畫面串流到瀏覽器。這種設計使得用戶能夠在不修改微信客戶端的情況下實現多端共享。效能方面，單個實例在空閒狀態下約佔用 0.6 GiB 內存，活躍時可達 1.5 GiB，這意味著在資源有限的環境中可能會遇到瓶頸。設計選擇上，使用 Docker 作為基礎架構提供了良好的擴展性，但也帶來了對伺服器資源的需求。技術風險方面，面板的安全性設計至關重要，因為它直接影響到用戶的數據安全。整合方面，與現有的 Docker 生態系統相容性良好，但對於不熟悉 Docker 的用戶來說，初始學習曲線可能較陡峭。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且詳細，提供了快速開始的指南和範例。安裝過程相對順暢，但對於不熟悉 Docker 的用戶來說，可能會遇到一些困難。文件目前僅提供英文版本，缺乏多語言支持。
...
> - 靈活的多實例管理，支持多種社交媒體平台。
> - 高安全性設計，避免數據洩露。
> - 支持文件傳輸和文本剪貼板功能，增強用戶體驗。

> [!danger] 缺點
> - 需要一定的伺服器資源，對於小型伺服器可能不夠友好。
> - 初始設置需要一定的 Docker 知識，對新手不夠友好。
> - 面板需在內網環境中使用，限制了使用場景。

> [!warning] 注意事項
> - 僅支援 Docker 環境，需安裝 Docker 和 Docker Compose。
> - 需要一定的伺服器資源，建議至少 2 核 / 2 GiB 內存來運行一個實例。
> - 面板需在內網環境中使用，避免暴露於公網。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供類似的多實例管理功能，但主要針對 AI agent 的運行，並不專注於社交媒體的使用。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於數據管理和備份，並不提供即時的社交媒體交互功能。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於 AI agent 的運行，提供類似的多實例管理功能，但不專注於社交媒體的使用。 | 如果你的需求主要是運行 AI agent，而不是社交媒體的管理，則可以考慮這個工具。 | medium，因為需要重新配置和適應不同的使用場景。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於數據管理和備份，並不提供即時的社交媒體交互功能。 | 如果你的需求主要是數據管理而非社交媒體交互，則可以考慮這個工具。 | low，因為數據管理的概念相對簡單，遷移成本較低。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **WechatOnCloud** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於 AI agent 的運行，提供類似的多實例管理功能，但不專注於社交媒體的使用。 | 專注於數據管理和備份，並不提供即時的社交媒體交互功能。 |
> | 遷移成本 | - | medium，因為需要重新配置和適應不同的使用場景。 | low，因為數據管理的概念相對簡單，遷移成本較低。 |
> | 適用場景 | 主要場景 | 如果你的需求主要是運行 AI agent，而不是社交媒體的管 | 如果你的需求主要是數據管理而非社交媒體交互，則可以考慮這個工 |
...
- **[HIGH]** 首次新建實例若卡住報 `創建容器失敗`，可能是 Docker 守護進程拉取鏡像超時。
  - 解法：手動拉取鏡像以避免超時問題。
- **[HIGH]** 面板暴露於公網可能導致數據洩露。
  - 解法：確保面板僅在內網訪問，或使用 VPN 進行安全訪問。
- [MEDIUM] 在低資源伺服器上運行多個實例可能導致性能問題。
  - 解法：根據實際需求調整運行的實例數量。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型企業需要集中管理客戶的微信聯絡 | 非常適合 | 支持多實例管理和跨設備共享，適合企業需求。 |
| 社交媒體經營者需要同時管理多個社交媒體帳號 | 非常適合 | 支持多應用平台，能夠在同一界面中管理多個帳號。 |
| 個人用戶希望在多設備上使用微信 | 適合 | 提供了靈活的多端共享功能，能夠滿足個人需求。 |
| 資源有限的小型伺服器運行多個實例 | 不適合 | 對伺服器資源的需求較高，可能會導致性能問題。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，3 小時整合，得到靈活的多實例管理，值得考慮。

## 安全性評估

> [!warning] 安全性快速掃描
> 面板需要高權限訪問 Docker，可能存在安全風險。使用時需確保不暴露於公網，並改變默認密碼以增強安全性。
...
> | 貢獻者 | Commits |
> | --- | --- |
> | [@Gloridust](https://github.com/Gloridust) | 133 |
...
**最新版本**：v1.4.9 — v1.4.9 — 桌面连不上可诊断 + Tailscale/内网 IPv6 放行 + 输入体验改进 (2026-07-28)

> [!info]- Release Notes
> ## 桌面连不上，终于能查了（issue #124）
> 
> 有用户反馈「一直卡在桌面正在启动 / 重连中…」，但导出诊断里**什么线索都没有**。查下来发现是我们自己的锅：面板处理 WebSocket 连接时有三道校验（Host 白名单 / URL 解析 / 会话权限），**任何一道不通过都是静默断开，不留任何日志**。于是用户只看到「重连中」，我们也无从下手。
> 
...
> - **内网 IPv6**：`fc00::/7`（ULA，含 Tailscale 的 `fd7a::/48`）与 `fe80::/10`（链路本地）—— 此前只放行 `::1`，纯 IPv6 内网用户完全进不来
> 
...
> ## 输入体验（issue #125）
> 
> - **自动回车开关**：转发输入条右侧新增 `↵` 开关。关掉后只把文字**填进**应用输入框、不代发，方便先编辑再自己发（按钮文案随之变「填入」）。默认保持原有行为。
...
**社群活躍度**：社群活躍，問題解決率達 78%。
**連結**：[文件](https://github.com/Gloridust/WechatOnCloud)

## 開發動態

> [!abstract] 最近 10 次 commit（2026-07-08 ~ 2026-07-28）
> **活躍天數** 4 天 · **最新 commit** docs(tg-bot): 更正私聊窗口为 5 分钟——轮询模式下验证码送不达
...
> | --- | --- | --- | --- |
> | [#128](https://github.com/Gloridust/WechatOnCloud/issues/128) | 界面有点糊 | 0 | 0 |
> | [#127](https://github.com/Gloridust/WechatOnCloud/issues/127) | 微信实例经常自动重启导致容器挂载的聊天记录文件丢失需要重新挂载 | 0 | 0 |
> | [#126](https://github.com/Gloridust/WechatOnCloud/issues/126) | 希望可以适配一下手机浏览器 | 0 | 1 |
> | [#125](https://github.com/Gloridust/WechatOnCloud/issues/125) | 关于【文本转发】和【图标/文字】的一些小问题 | 0 | 3 |
...
相關專案：[[0x0funky--agent-sprite-forge|0x0funky/agent-sprite-forge]] · [[0xGF--boneyard|0xGF/boneyard]] · [[HKUDS--OpenHarness|HKUDS/OpenHarness]] · [[2aronS--Duel-Agents|2aronS/Duel-Agents]] · [[BigBodyCobain--Shadowbroker|BigBodyCobain/Shadowbroker]] · [[AlexandrosGounis--pdfx|AlexandrosGounis/pdfx]] · [[Gloridust--WechatOnCloud|Gloridust/WechatOnCloud]] · [[louislva--claude-peers-mcp|louislva/claude-peers-mcp]]
