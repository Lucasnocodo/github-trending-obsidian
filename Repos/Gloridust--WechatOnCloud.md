...
star_history: "2026-06-03:1468,2026-06-03:1480,2026-06-04:1941,2026-06-04:1941,2026-06-05:2054,2026-06-05:2054"
tags:
  - github
  - "category/其他"
  - "lang/typescript"
...
**2.1k** stars · **342** stars/天 · 建立 6 天前 · TypeScript · 未標註授權
...
> **適合** 需要在本地伺服器上運行多個微信實例以支持團隊協作的中小型企業。
> **一句話重點** 這個專案的設計讓多用戶可以安全地共享同一個微信會話，解決了傳統微信使用中的多設備同步問題。
...
> **結論** 花 5 小時學習，3 小時整合，得到靈活的多用戶微信管理，值得考慮。

> [!abstract] 核心創新
> 支持多用戶同時訪問同一微信實例，並提供用戶權限控制。

## 專案簡介

WechatOnCloud（WOC）是一個在本地伺服器上運行的微信服務，允許用戶管理多個獨立的微信實例，並通過瀏覽器共享同一會話。每個微信實例運行在 Docker 容器中，使用 Xvfb 虛擬顯示和 KasmVNC 將畫面串流到瀏覽器，讓多個用戶能夠同時訪問同一個微信會話。這樣的設計不僅保證了數據的隔離性，還能夠靈活地管理用戶訪問權限。最關鍵的指令是 `docker compose up -d`，這會啟動面板和微信實例，並且用戶可以通過瀏覽器訪問指定端口來進行操作。技術上，這個專案使用了 TypeScript 和 Docker，並且支持多種 CPU 架構（amd64 和 arm64），這使得它能夠在多種硬體上運行。

與其他類似工具（如 lazycat 的懶貓微服）相比，WOC 提供了更強的多實例管理和用戶權限控制功能，讓用戶能夠更靈活地使用微信。實際使用中，WOC 的資源占用相對較低，單個實例在空閒狀態下約佔用 0.6 GiB 的內存，這使得它能夠在資源有限的環境中運行。這個專案目前處於穩定階段，適合中小型團隊使用，未來可能會增加更多安全功能，如 TLS 支持和設備驗證。對於需要在內網環境中運行微信的企業或開發者來說，WOC 是一個值得考慮的選擇。

**技術棧**：`TypeScript` · `Docker` · `KasmVNC`

## 重點功能

- 多實例管理 — 一個面板管理多個獨立的微信會話，每個實例都有獨立的數據卷。
- 用戶權限控制 — 支持子賬號管理，根據賬號分配可訪問的實例，實現 RBAC。
- 微信本體運行時下載 — 面板一鍵下載和安裝微信，支持自動更新。
- 文件和文本傳輸 — 支持拖拽上傳和下載文件，文本可經剪貼板中轉送進微信。
- 安全優先 — 面板為唯一入口，並且不暴露 docker.sock 給前端，確保安全性。
...
2. 複製環境變數範本並修改密碼
```bash
cp .env.example .env && # 修改 WOC_PASSWORD
```
3. 啟動 Docker Compose
```bash
docker compose up -d
...
> [!tip] 爆紅原因
> 建立 6 天就累積 2054 stars（342/天），forks 552（26.9%），顯示出強烈的社群參與。作者 Gloridust 之前有其他開源專案的經驗，這個專案解決了在本地運行微信的需求，特別是在多用戶環境下的消息同步問題。近期的安全補丁和功能更新也吸引了不少關注。社群對於多實例管理和安全性的需求促進了這個專案的快速成長。

## 適合誰使用

**目標受眾**：需要在本地伺服器上運行多個微信實例以支持團隊協作的中小型企業。

> [!example] 使用場景
> - 後端工程師用它來在內部伺服器上運行多個微信實例，因為這樣可以實現跨設備消息同步，提升團隊協作效率。
> - IT 管理員用它來管理公司內部的微信通訊，因為它支持用戶權限控制，能夠確保數據安全。
> - 開發者用它來測試微信相關的應用，因為可以快速創建和刪除微信實例，方便進行功能測試。

## 架構分析

WechatOnCloud 採用 Docker 容器化架構，每個微信實例運行在獨立的容器中，這樣的設計使得每個實例之間的數據完全隔離，避免了相互干擾。面板作為唯一的入口，通過 Docker API 管理實例的創建和刪除，並使用 KasmVNC 將畫面串流到瀏覽器。這種設計的好處是能夠靈活地管理多個實例，但代價是需要用戶具備一定的 Docker 知識。擴展性方面，由於資源主要集中在內存上，當實例數量增加時，內存使用量會線性增長，這可能成為瓶頸。

## 技術深入分析

WechatOnCloud 的核心技術機制是基於 Docker 的容器化架構，每個微信實例都運行在獨立的容器中，這樣的設計使得數據隔離和資源管理變得更加靈活。使用 KasmVNC 將畫面串流到瀏覽器，這樣用戶可以在多個設備上訪問同一個微信會話。效能方面，單個實例在空閒狀態下約佔用 0.6 GiB 的內存，活躍時可達 1.5 GiB，這使得它在資源有限的環境中運行良好。選擇 Docker 作為基礎架構的原因是其便於管理和擴展，但這也帶來了對用戶 Docker 知識的要求。技術風險方面，內存使用量隨著實例數量的增加而增加，可能成為性能瓶頸。整合方面，與主流的 CI/CD 工具鏈相容性良好，能夠輕鬆集成到現有的開發流程中。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且詳細，包含了快速開始的指引和必要的安裝步驟。安裝過程相對順暢，但對於不熟悉 Docker 的用戶可能會有一定的挑戰。文件目前僅提供英文版本，未來可考慮增加多語言支持。
...
> - 內存使用量隨實例數量增加而增加，可能成為瓶頸。
> - 不建議暴露於公網，需額外的安全措施。

> [!warning] 注意事項
> - 僅支援 x86_64 和 arm64 架構的伺服器。
> - 需要 Docker 環境，對於不熟悉 Docker 的用戶可能有學習曲線。
> - 不建議將面板暴露於公網，需透過 VPN 或內網穿透訪問。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [lazycat/wechat-on-cloud](https://github.com/lazycat/wechat-on-cloud) | 提供類似的多實例管理功能，但缺乏用戶權限控制和安全性設計，適合小型個人使用。 |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於 AI agent 的管理，與 WOC 的即時通訊功能無法直接比較，但在多用戶環境下有其優勢。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [lazycat/wechat-on-cloud](https://github.com/lazycat/wechat-on-cloud) | 專注於簡單的微信實例運行，但缺乏多用戶管理和權限控制功能。 | 如果只需要單一用戶使用微信，且不需要複雜的權限管理，則可以選擇這個工具。 | low，因為功能簡單，遷移成本低。 |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於 AI agent 的管理，與 WOC 的即時通訊功能無法直接比較。 | 如果需要管理多個 AI agent，則可以選擇這個工具。 | medium，因為需要重新設計工作流以適應不同的功能。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **WechatOnCloud** | **wechat-on-cloud** | **agent-sprite-forge** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於簡單的微信實例運行，但缺乏多用戶管理和權限控制功能。 | 專注於 AI agent 的管理，與 WOC 的即時通訊功能無法直接比較。 |
> | 遷移成本 | - | low，因為功能簡單，遷移成本低。 | medium，因為需要重新設計工作流以適應不同的功能。 |
> | 適用場景 | 主要場景 | 如果只需要單一用戶使用微信，且不需要複雜的權限管理，則可以選 | 如果需要管理多個 AI agent，則可以選擇這個工具。 |
...
- **[HIGH]** 在 Ubuntu 26.04 上安裝時可能會卡在下載步驟。
  - 解法：檢查網路連接，或嘗試手動下載並安裝依賴。
- [MEDIUM] 登錄後可能會出現重複登錄的問題。
  - 解法：檢查用戶會話管理設置，確保不重複創建會話。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 支持多用戶管理和數據隔離，適合小型團隊使用。 |
| 大型企業的內部通訊系統 | 適合 | 能夠管理多個微信實例，並提供用戶權限控制。 |
| 個人使用的微信客戶端 | 不適合 | 對於單一用戶，這個工具的功能過於複雜。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，3 小時整合，得到靈活的多用戶微信管理，值得考慮。

## 安全性評估

> [!warning] 安全性快速掃描
> 中等風險：需要對面板進行安全配置，避免暴露於公網，並確保使用強密碼和 HTTPS 反代。
...
> | 貢獻者 | Commits |
> | --- | --- |
> | [@Gloridust](https://github.com/Gloridust) | 27 |
> | [@web-flow](https://github.com/web-flow) | 1 |
> | [@huglemon](https://github.com/huglemon) | 1 |

**最新版本**：v1.1.5 — v1.1.5 · Security: DNS rebinding 防护 (2026-06-04)
...
> ## 修复
> 
> ### Host 头白名单：阻断 DNS rebinding 攻击（[#13](https://github.com/Gloridust/WechatOnCloud/pull/13)，感谢 @aaronjmars）
> 
> **漏洞**：Fastify 默认不校验 Host 头。结合默认凭据（`admin` / `wechat`）和挂载的 `docker.sock`，攻击链：操作员浏览恶意页 → 攻击者用 DNS 短 TTL 把 `woc.attacker.example` 重绑到面板 LAN/loopback IP → 浏览器视为同源、向面板发 `POST /api/auth/login` → 面板把 cookie 签给攻击者域 → 后续 same-site fetch 全部带 cookie，相当于在受害者浏览器里完整接管面板，并通过 `docker.sock` 拿到宿主 root。`sameSite=lax` 不挡此攻击。CWE-346。
> 
> **修复**：`onRequest` hook + WebSocket upgrade gate 双路把守：
> - 默认放行：`localhost` / `127.0.0.1` / `::1` + RFC1918 私网（`10/8`、`172.16-31/12`、`192.168/16`、`169.254/16`）→ **直连 NAS / 局域网零配置生效**
> - 反代部署：`.env` 加 `PANEL_ALLOWED_HOSTS=woc.example.com`，**支持多个域名逗号分隔**：`PANEL_ALLOWED_HOSTS=a.com,b.com,c.lan`
> - 公网 IP / 未列入域名 → 400 `Host header not allowed`，响应里**回显被拒的 host + 反代修复 hint**（便于反代调试）
> 
> ## ⚠️ 升级注意（反代用户）
> 
> 如果你用 **Cloudflare Tunnel / frp / 某些反代** 把客户端 Host 改写成了内网名,升级后访问会 400。修复路径二选一:
> 1. 修反代让 Host 头**透传客户端原始 Host**(nginx 默认 `proxy_set_header Host \$host`、Caddy v2 默认透传)
> 2. 把反代后端实际看到的 Host 加进 `.env`:`PANEL_ALLOWED_HOSTS=...`
> 
> 400 响应里现在会**直接告诉你反代后端传过来的 Host 是什么**,不用猜。
> 
> ## 镜像
> 
> - \`ghcr.io/gloridust/woc-panel:1.1.5\` / \`latest\`
> - \`ghcr.io/gloridust/wechat-on-cloud:1.1.5\` / \`latest\`(与 1.1.4 相同,随发布同步推 latest)
> 
> ## 升级
> 
> \`\`\`bash
...
**連結**：[文件](https://github.com/Gloridust/WechatOnCloud#文档)

## 開發動態

> [!abstract] 最近 10 次 commit（2026-06-02 ~ 2026-06-04）
> **活躍天數** 2 天 · **最新 commit** ci: optional Docker Hub mirror in release workflow
...
> | --- | --- | --- | --- |
> | [#27](https://github.com/Gloridust/WechatOnCloud/issues/27) | 登录不上了，登录就重新登录 | 3 | 4 |
> | [#28](https://github.com/Gloridust/WechatOnCloud/issues/28) | ubuntu26.04 desktop 一直卡在下载安装这一步 | 0 | 1 |
> | [#26](https://github.com/Gloridust/WechatOnCloud/issues/26) | 公众号文章好像打不开 | 0 | 0 |
> | [#25](https://github.com/Gloridust/WechatOnCloud/issues/25) | Xvnc (KasmVNC) 存在严重的内存泄漏！ | 0 | 6 |
> | [#24](https://github.com/Gloridust/WechatOnCloud/issues/24) | 导入导出聊天记录不在同一网络 | 0 | 1 |
