...
**3.5k** stars · **64** stars/天 · 建立 55 天前 · TypeScript · 未標註授權
...
> **適合** 需要在自己的伺服器上運行多個微信帳號的商業用戶或開發者。
> **一句話重點** 這個專案讓在多設備上使用微信變得簡單而高效，特別適合需要同時管理多個帳號的用戶。
...
> **結論** 花 5 小時學習，2 小時整合，得到靈活的多帳號管理，值得投資。

> [!abstract] 核心創新
> 支持多實例和多應用的雲端即時通訊解決方案。

## 專案簡介

WechatOnCloud 是一個基於 Docker 的服務端微信解決方案，允許用戶在自己的 NAS 或伺服器上運行多個獨立的微信實例。每個實例都是一個獨立的容器，並且可以通過瀏覽器共享同一會話。用戶只需使用 `docker-compose` 部署，並可選擇安裝微信或 Chromium 瀏覽器，實現多端同步。這樣的設計不僅提高了用戶的靈活性，還避免了對微信客戶端的修改，確保了使用的穩定性。技術上，該專案使用了 Xvfb 虛擬顯示和 KasmVNC 來實現畫面串流，並通過自研面板進行管理。

這種架構使得用戶能夠在不同設備上無縫切換，並且支持文件傳輸和文本剪貼板功能。相較於傳統的微信客戶端，這種服務端解決方案能夠更好地滿足多用戶和多設備的需求，特別適合需要同時管理多個帳號的商業用戶。使用者可以通過面板一鍵下載和安裝微信，並且支持多架構的 Docker 映像，方便不同硬體環境的部署。這樣的設計讓用戶能夠快速上手，並且在遇到問題時能夠通過詳細的日誌和診斷工具進行排查。

**技術棧**：`TypeScript` · `Docker` · `KasmVNC`

## 重點功能

- 多實例管理 — 一個面板管理多個獨立的微信或 Chromium 實例，互不干擾。
- 文件傳輸 — 支持拖拽上傳和下載文件，直接到實例桌面。
- 文本剪貼板 — 支持在實例和本地之間傳送文本，方便交流。
- 安全優先 — 面板為唯一入口，並且支持多層安全防護。
- 多架構支持 — 提供 amd64 和 arm64 的預構建映像，方便不同硬體環境使用。
...
> [!tip] 爆紅原因
> 建立 55 天就累積 3504 stars（64/天），forks 777（22.2%），顯示出強勁的增長勢頭。Gloridust 是該專案的主要貢獻者，過去在開源社群中有著良好的聲譽。這個專案解決了在多設備上使用微信的痛點，特別是對於需要同時管理多個帳號的用戶，之前的解決方案往往不夠靈活或穩定。社群的活躍度和問題解決率也顯示出這個專案的實用性和穩定性，吸引了大量用戶關注和使用。

## 適合誰使用

**目標受眾**：需要在自己的伺服器上運行多個微信帳號的商業用戶或開發者。

> [!example] 使用場景
> - 企業用戶用它來在內部伺服器上運行多個微信實例，因為這樣可以同時管理多個帳號，提升工作效率。
> - 開發者用它來測試微信相關的應用，因為可以快速創建和刪除實例，並且不需要修改客戶端。
> - 家庭用戶用它來在不同設備上共享同一微信會話，因為這樣可以隨時隨地接收消息，保持聯繫。

## 架構分析

WechatOnCloud 採用 Docker 容器化架構，每個微信實例運行在獨立的容器中，這樣的設計使得用戶可以靈活地管理多個實例。每個實例使用 Xvfb 虛擬顯示來運行應用，並通過 KasmVNC 將畫面串流到瀏覽器。這種設計的好處在於用戶可以在不同設備上共享同一會話，而不需要修改微信客戶端。代價是需要一定的硬體資源，特別是內存，因為每個實例都需要分配一定的資源來運行。擴展性方面，隨著實例數量的增加，內存使用量會顯著增加，這需要用戶根據實際需求進行配置。

## 技術深入分析

WechatOnCloud 的核心技術機制在於使用 Docker 容器化技術來運行多個獨立的微信實例。每個實例都運行在獨立的容器中，這樣的設計不僅提高了靈活性，還能確保每個實例之間的數據隔離。使用 KasmVNC 進行畫面串流，這使得用戶能夠在瀏覽器中無縫訪問這些實例。效能方面，實測顯示單個實例在空閒狀態下 CPU 使用率約為 0.1-0.2 核，內存使用約為 0.6 GiB，這對於大多數現代伺服器來說是可接受的。設計上選擇 Docker 作為基礎架構，這樣可以利用其強大的隔離性和可擴展性，但同時也帶來了對 Docker 環境的依賴。技術風險方面，隨著實例數量的增加，內存使用量會顯著增加，這可能會成為瓶頸。整合方面，該專案與現有的 Docker 生態系統兼容，並且可以輕鬆地與 CI/CD 流程集成。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且詳細，提供了完整的安裝步驟和使用說明。安裝過程相對順暢，但對於不熟悉 Docker 的用戶來說可能會有一定的學習曲線。文件中提供了快速開始指南，並且有多語言支持，對新手友好。

## 優缺點分析

> [!success] 優點
> - 支持多個微信實例，方便管理。
> - 提供文件傳輸和文本剪貼板功能，提升使用體驗。
> - 安全性高，面板為唯一入口，避免未授權訪問。
...
> [!warning] 注意事項
> - 需要 Docker 環境，對於不熟悉 Docker 的用戶來說有一定的學習曲線。
> - 不支持直接在公網上使用，必須通過內網或 VPN 訪問。
> - 實例數量過多可能會導致內存占用過高，需根據實際情況調整硬體配置。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 這個工具專注於代理和自動化，而 WechatOnCloud 主要針對微信的多實例管理。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard 提供了類似的即時通訊功能，但不支持多實例和多應用的管理。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於代理和自動化，而 WechatOnCloud 主要針對微信的多實例管理。 | 如果需要一個專注於自動化的解決方案，而不需要多實例管理的功能。 | medium，由於功能差異，可能需要重新設計部分工作流。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供即時通訊功能，但不支持多實例和多應用的管理。 | 如果只需要一個簡單的即時通訊解決方案，而不需要多帳號支持。 | low，因為功能較為簡單，遷移成本低。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **WechatOnCloud** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於代理和自動化，而 WechatOnCloud 主要針對微信的多實例管理。 | 提供即時通訊功能，但不支持多實例和多應用的管理。 |
> | 遷移成本 | - | medium，由於功能差異，可能需要重新設計部分工作流。 | low，因為功能較為簡單，遷移成本低。 |
> | 適用場景 | 主要場景 | 如果需要一個專注於自動化的解決方案，而不需要多實例管理的功能 | 如果只需要一個簡單的即時通訊解決方案，而不需要多帳號支持。 |
...
- **[HIGH]** 首次新建實例若卡住報錯，可能是 Docker 拉取超時。
  - 解法：手動拉取鏡像以避免超時。
- **[HIGH]** 在公網上使用時可能面臨安全風險。
  - 解法：務必使用內網或 VPN 訪問。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 能夠靈活管理多個微信帳號，提升工作效率。 |
| 大型企業需要同時管理多個微信帳號 | 適合 | 支持多實例和多用戶共享，符合需求。 |
| 個人用戶只需一個微信帳號 | 普通 | 雖然可以使用，但功能過於複雜。 |
| 不熟悉 Docker 的用戶 | 不適合 | 需要一定的技術門檻，對新手不友好。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，2 小時整合，得到靈活的多帳號管理，值得投資。

## 安全性評估

> [!warning] 安全性快速掃描
> 該工具需要高權限來訪問 Docker 環境，並且會存取敏感資料如微信聊天記錄。建議在內網環境中使用，並且務必改變預設密碼以提高安全性。
...
> | [#124](https://github.com/Gloridust/WechatOnCloud/issues/124) | 群晖配置无法正常使用 | 0 | 3 |
