...
**3.2k** stars · **122** stars/天 · 建立 26 天前 · TypeScript · 未標註授權
...
> **適合** 需要在內網環境中運行微信並實現多端共享的開發者和團隊。
> **一句話重點** 這個專案讓在 NAS 上運行微信變得簡單且安全，特別適合需要多端同步的用戶。
...
> **結論** 花 5 小時學習，2 小時整合，得到安全的多端微信使用體驗，值得投入。

> [!abstract] 核心創新
> 支持多應用的雲端服務端微信解決方案，實現多端共享與同步。

## 專案簡介

WechatOnCloud 是一個在 NAS 或伺服器上運行的服務端微信解決方案，允許多個用戶通過瀏覽器共享同一微信會話。每個實例都是獨立的 Docker 容器，運行 Xvfb 虛擬顯示和應用程序，並通過 KasmVNC 將畫面串流到瀏覽器。用戶只需在面板中創建實例，選擇應用類型（微信或 Chromium 瀏覽器），便可輕鬆使用。這種設計讓用戶無需修改微信客戶端，便能在多個設備上實現即時同步和共享。核心賣點在於其多實例管理和安全性，面板是唯一的訪問入口，並且支持多用戶權限管理。

技術上，該專案使用 Docker 來管理容器，並且支持多架構（amd64 和 arm64）。與其他類似工具相比，如 `0x0funky/agent-sprite-forge` 和 `2aronS/Duel-Agents`，WechatOnCloud 提供了更強的多應用支持和用戶管理功能，能夠同時運行微信和多個瀏覽器實例，並且在資源管理上更為靈活。實際使用中，內存是主要瓶頸，每個微信實例大約需要 1.5 GiB 的內存，適合中小型團隊使用。該專案目前處於穩定階段，值得在非關鍵環境中試用，未來可能會進一步增強安全性和擴展性。對於需要在內網環境中運行微信的開發者，這是一個非常合適的選擇。
...
- 多應用支持 — 可選擇運行微信或 Chromium 瀏覽器，支持多種社交媒體的登入。
- 多端共享 — 多個設備可共享同一會話，支持子帳號權限管理。
- 文件傳輸 — 支持拖拽上傳和下載文件，並可通過剪貼板傳送文本。
- 安全設計 — 面板是唯一入口，並且支持 KasmVNC 的安全性設計，避免敏感信息泄露。
...
```ts
{
  ...
  "預期輸出": "啟動面板容器，並在瀏覽器中訪問 http://localhost:36080 進行管理。"
}
```
...
> [!tip] 爆紅原因
> 建立 26 天就累積 3162 stars（122/天），forks 741（23.4%），顯示出強烈的社群關注。作者 Gloridust 之前有其他開源專案經驗，這個專案解決了在多設備上使用微信的痛點，特別是對於需要長期保持登入狀態的用戶。社群的反饋和需求驅動了這個工具的快速發展，並且有活躍的交流渠道。這些因素共同促進了專案的快速增長。

## 適合誰使用

**目標受眾**：需要在內網環境中運行微信並實現多端共享的開發者和團隊。

> [!example] 使用場景
> - 後端工程師用它來在 NAS 上運行服務端微信，因為這樣可以在多個設備上共享同一會話，提升工作效率。
> - 社群管理者用它來管理多個社交媒體帳號，因為可以通過一個面板同時登入 Telegram 和 Instagram，方便集中管理。
> - 小型團隊用它來在內網環境中運行微信，因為它提供了安全的多用戶訪問和實例管理功能。

## 架構分析

WechatOnCloud 採用 Docker 容器化架構，將每個微信或 Chromium 實例作為獨立容器運行。這樣的設計使得每個實例之間互不干擾，並且能夠靈活管理資源。面板作為唯一的入口，通過 docker.sock 控制容器的創建和銷毀，確保安全性。

這種設計的代價是需要用戶具備一定的 Docker 知識來進行初始設置。由於內存是主要瓶頸，實例數量的增加會直接影響到系統的性能，特別是在資源有限的情況下。整體架構的擴展性良好，但在高負載情況下可能需要進一步的優化。

使用 KasmVNC 進行畫面串流，確保了用戶的操作流暢性和安全性。

## 技術深入分析

WechatOnCloud 的核心技術在於其容器化架構，使用 Docker 來管理每個微信和 Chromium 實例，這樣的設計使得每個實例都能獨立運行，互不干擾。通過 KasmVNC 將畫面串流到瀏覽器，確保了用戶操作的流暢性。實際測試顯示，單個實例在空閒狀態下的內存使用約為 0.6 GiB，而在活躍狀態下可達 1.5 GiB，這使得內存成為主要的性能瓶頸。選擇 Docker 作為技術棧的原因在於其靈活性和可擴展性，能夠支持多架構（amd64 和 arm64），但這也意味著用戶需要具備一定的 Docker 知識。設計上，面板作為唯一入口，確保了安全性，避免了敏感信息的泄露。未來在高負載情況下，可能需要進一步的優化來提升性能，特別是在內存資源有限的情況下。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了詳細的安裝步驟和使用說明。安裝過程相對順暢，但對於不熟悉 Docker 的用戶可能會有一定的學習曲線。文件中有多種語言的支持，方便不同用戶使用。整體來說，花 30 分鐘能夠順利運行起來。

## 優缺點分析

> [!success] 優點
> - 支持多應用，除了微信還能運行 Chromium 瀏覽器。
> - 多實例管理，方便用戶在同一面板下管理多個會話。
> - 安全性高，面板是唯一入口，避免敏感信息泄露。

> [!danger] 缺點
> - 需要較高的內存資源，單個實例約需 1.5 GiB。
> - 初始設置需要一定的 Docker 知識，對新手不友好。
> - 不支持在公網環境中運行，限制了使用場景。

> [!warning] 注意事項
> - 僅支援在內網環境中運行，暴露於公網存在安全風險。
> - 需要一定的 Docker 知識才能進行初始設置和故障排除。
> - 內存需求較高，單個微信實例約需 1.5 GiB 的內存。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 這個工具主要專注於自動化代理，而 WechatOnCloud 提供了多應用支持和更強的用戶管理功能。 |
| [2aronS/Duel-Agents](https://github.com/2aronS/Duel-Agents) | Duel-Agents 主要針對 AI 代理，而 WechatOnCloud 則專注於社交媒體的多端同步。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 這個工具主要專注於自動化代理，而 WechatOnCloud 提供了多應用支持和更強的用戶管理功能。 | 如果你的團隊需要一個自動化的代理解決方案，而不僅僅是社交媒體的管理。 | medium，因為需要重新設計用戶管理和實例管理的流程。 |
| [2aronS/Duel-Agents](https://github.com/2aronS/Duel-Agents) | Duel-Agents 主要針對 AI 代理，而 WechatOnCloud 則專注於社交媒體的多端同步。 | 如果你的需求是針對 AI 代理的管理，而不是社交媒體的多端使用。 | high，因為需要完全重構應用架構和用戶交互方式。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **WechatOnCloud** | **agent-sprite-forge** | **Duel-Agents** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 這個工具主要專注於自動化代理，而 WechatOnCloud 提供了多應用支持和更強的用戶管理功能。 | Duel-Agents 主要針對 AI 代理，而 WechatOnCloud 則專注於社交媒體的多端同步。 |
> | 遷移成本 | - | medium，因為需要重新設計用戶管理和實例管理的流程。 | high，因為需要完全重構應用架構和用戶交互方式。 |
> | 適用場景 | 主要場景 | 如果你的團隊需要一個自動化的代理解決方案，而不僅僅是社交媒體 | 如果你的需求是針對 AI 代理的管理，而不是社交媒體的多端使 |
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的小型團隊需要運行微信 | 非常適合 | 提供了多用戶管理和實例共享功能，適合小型團隊使用。 |
| 需要在公網環境中使用微信 | 不適合 | 存在安全風險，建議僅在內網中使用。 |
| 需要長期保持社交媒體登入狀態的用戶 | 非常適合 | 支持多應用的常駐登入，方便用戶管理多個社交媒體帳號。  |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，2 小時整合，得到安全的多端微信使用體驗，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 中等風險：面板需要管理 Docker 的權限，可能會接觸到敏感資料，建議在內網環境中運行，並設置強密碼。
...
> | 貢獻者 | Commits |
> | --- | --- |
> | [@Gloridust](https://github.com/Gloridust) | 92 |
> | [@web-flow](https://github.com/web-flow) | 1 |
> | [@huglemon](https://github.com/huglemon) | 1 |
> | [@mkblbj](https://github.com/mkblbj) | 1 |

**最新版本**：v1.2.5 — v1.2.5 · 桌面「暂不可用」改自动重连页 + 诊断包补宿主信号 (2026-06-24)

> [!info]- Release Notes
> 面板修复版（实例镜像与 v1.2.3/1.2.4 相同，**无需升级实例**，只需更新面板）。
> 
> ## 远程桌面「桌面服务暂不可用」黑屏
> - 实例在 **创建初始化 / 升级 / 重启 / 内存自愈软重启**，以及 **面板自更新**（代理短暂中断）时会短暂连不上，旧版直接黑屏显示死的「桌面服务暂时不可用」、且不再自动重试。
> - 现改为**带 spinner、自动重连**的页面:实例一就绪即自动连上,无需手动刷新;约 30s 仍不行才转手动「重试」。**绝大多数"黑屏"会变成"短暂转圈后自动恢复"。**
> 
> ## 诊断包增强（便于定位宿主问题）
> - 很多报障用户在 Ubuntu server 等宿主上,问题常是**宿主级资源/限制**(如小内存被 OOM)。诊断包现补充:
>   - system.txt:内存&Swap 限制支持、cgroup 版本/驱动、存储驱动、SecurityOptions(apparmor/userns/seccomp);面板实例配置(SHM、是否设 docker 硬内存上限、GPU);内存自愈 soft/hard 阈值。
>   - 每个实例:**当前内存占用 MiB**(配合宿主总内存/OOMKilled 一眼判断是否内存不足)。
> - 导出方式不变:面板「诊断与日志 → 导出诊断包」。
> 
> > 更新:面板「关于」→「一键更新面板」即可;实例无需升级。
...
> **活躍天數** 3 天 · **最新 commit** feat(diag): 诊断包补宿主级信号——内存/cgroup/安全选项/实例实时内存
...
> | [#86](https://github.com/Gloridust/WechatOnCloud/issues/86) | 有没有打算出一个微信聊天记录备份的项目 | 0 | 0 |
