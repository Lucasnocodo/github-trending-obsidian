...
use_case: "在自己的伺服器上運行多個微信和瀏覽器實例，實現跨設備同步。"
priority: high
ring: assess
...
aliases:
  ...
  - "在自己的伺服器上運行多個微信和瀏覽器實例，實現跨設備同步。"
---

# WechatOnCloud

**3.3k** stars · **90** stars/天 · 建立 37 天前 · TypeScript · 未標註授權
...
> **適合** 需要在自己的伺服器上運行多個社交媒體帳號的用戶，特別是中小型企業或自由職業者。
> **一句話重點** 這個專案讓用戶能夠在自己的伺服器上靈活管理多個社交媒體帳號，特別適合需要長期保持登入狀態的用戶。
...
>     .where(p => p.subcategory === "雲端服務" && p.file.name !== "Gloridust--WechatOnCloud" && p.status !== "archived")
...
>       p.ring || "assess"
>     ]));
>   } else { dv.paragraph("_目前 vault 中沒有其他 雲端服務 類工具_"); }
> }
> ```

> [!question] TL;DR — 值得投入嗎？
> **成熟度** Beta (可試用) · **安裝** Medium (需設定) · **學習** ~3h · **綁定風險** low
> **結論** 花 3 小時學習、2 小時整合，得到靈活的社交媒體管理工具，值得。

> [!abstract] 核心創新
> 支持多個獨立實例的雲端微信管理平台，實現跨設備同步。

## 專案簡介

WechatOnCloud 是一個基於 Docker 的平台，允許用戶在自己的 NAS 或伺服器上運行多個獨立的微信和 Chromium 瀏覽器實例。每個實例都是一個獨立的容器，透過 KasmVNC 將畫面串流到瀏覽器，實現多端共享同一會話。用戶可以通過面板管理實例，並在不同設備上無縫切換，無需修改微信客戶端。這個設計的賣點在於其靈活性和便捷性，特別適合需要同時管理多個社交媒體帳號的用戶。核心技術上，該專案使用了 Xvfb 虛擬顯示和 Docker 容器化技術，確保了資源的有效利用和實例的獨立性。

用戶可以選擇安裝微信或直接使用已內建的 Chromium 瀏覽器，這樣的設計使得用戶在使用過程中無需等待下載和安裝。與其他類似工具相比，如 WeChat Web 或其他雲端桌面解決方案，WechatOnCloud 提供了更高的靈活性和可擴展性，特別是在多用戶環境下的權限管理和實例共享方面。實際使用中，系統對資源的需求相對較低，單個實例在空閒時僅需 0.1-0.2 核 CPU 和 0.6 GiB 記憶體，這使得它能在普通的伺服器上運行良好。這個專案目前處於穩定階段，並且有活躍的社群支持，適合需要在多設備上管理社交帳號的用戶，特別是中小型團隊或個人使用者。未來可能會增加更多的安全性功能和用戶體驗改進，值得關注。

**技術棧**：`TypeScript` · `Docker` · `KasmVNC`

## 重點功能

- 多實例管理 — 一個面板可以管理多個獨立的微信和瀏覽器實例，每個實例互不干擾。
- 即時更新 — 面板可以一鍵更新實例鏡像，確保用戶使用最新版本的應用。
- 多端共享 — 支持多設備同時訪問同一實例，實現無縫的跨設備體驗。
- 文件傳輸 — 支持拖拽上傳和下載文件，方便用戶在實例間傳輸資料。
- 安全優先 — 面板是唯一的入口，並且提供了多層安全措施，確保用戶數據安全。
...
> [!tip] 爆紅原因
> 建立 37 天就累積 3337 stars（90/天），forks 759（22.7%），顯示出強勁的社群興趣。作者 Gloridust 之前在開源社群活躍，這個專案解決了用戶在多設備環境下管理微信和其他社交媒體的痛點，特別是對於需要長期保持登入狀態的用戶。社群的反饋和問題解決速度也顯示出這個專案的健康度，許多用戶對於多實例管理的需求促進了其快速成長。

## 適合誰使用

**目標受眾**：需要在自己的伺服器上運行多個社交媒體帳號的用戶，特別是中小型企業或自由職業者。

> [!example] 使用場景
> - 社交媒體經理用它來在雲端管理多個微信帳號，因為可以在不同設備上隨時訪問同一會話，提升工作效率。
> - 開發者用它來測試不同版本的微信，因為可以快速創建和刪除實例，並保持數據的獨立性。
> - 小型團隊用它來協作處理客戶的微信訊息，因為可以設置不同的子帳號來管理不同的客戶。

## 架構分析

WechatOnCloud 採用 Docker 容器化架構，將每個微信或瀏覽器實例封裝在獨立的容器中，這樣的設計使得每個實例之間互不干擾，並且可以根據需求動態創建或銷毀。使用 KasmVNC 將虛擬顯示的畫面串流到用戶的瀏覽器，確保了用戶體驗的流暢性。這種架構的優勢在於資源的有效利用，尤其是在多用戶環境下，每個實例的資源需求相對較低。選擇 Docker 作為基礎架構的原因在於其輕量級和可擴展性，能夠快速部署和管理多個實例。這樣的設計也使得系統的擴展性良好，未來可以根據需求增加更多功能或支持更多應用。

## 技術深入分析

WechatOnCloud 的核心技術機制是基於 Docker 容器化架構，使用 KasmVNC 將虛擬顯示的畫面串流到用戶的瀏覽器。這樣的設計使得每個實例都能獨立運行，並且可以根據需求動態創建或銷毀。效能方面，單個實例在空閒時僅需 0.1-0.2 核 CPU 和 0.6 GiB 記憶體，這使得它能在普通的伺服器上運行良好。設計取捨方面，選擇 Docker 作為基礎架構的原因在於其輕量級和可擴展性，能夠快速部署和管理多個實例。這樣的選擇也使得系統的擴展性良好，未來可以根據需求增加更多功能或支持更多應用。技術風險方面，隨著用戶數量的增加，可能會出現資源競爭的情況，特別是在內存方面。整合分析方面，與主流的 Docker 工具鏈相容性良好，能夠輕鬆整合到現有的 CI/CD pipeline 中，對於團隊的學習成本相對較低。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了詳細的安裝步驟和使用指南，對於新手來說相對友好。安裝過程順暢，沒有明顯的坑。文檔有多語言支持，但主要以中文為主。

## 優缺點分析

> [!success] 優點
> - 支持多個獨立實例，適合需要同時管理多個社交媒體帳號的用戶。
> - 使用 Docker 容器化技術，確保資源的有效利用和實例的獨立性。
> - 提供了一鍵更新和安裝功能，提升用戶體驗。

> [!danger] 缺點
> - 需要一定的 Docker 知識，對新手來說有學習曲線。
> - 面板需在內網環境中運行，限制了使用場景。
> - 目前僅支持微信和 Chromium，對於其他應用的支持有限。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供類似的雲端桌面服務，但不專注於社交媒體帳號的管理，適合需要更廣泛應用的用戶。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 雖然也提供雲端服務，但主要針對開發環境，對於社交媒體的支持較弱。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供雲端桌面服務，但不專注於社交媒體帳號的管理，適合需要更廣泛應用的用戶。 | 如果需要更廣泛的應用支持，而不僅限於社交媒體帳號的管理。 | medium，因為需要重新配置和適應不同的使用方式。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 雖然也提供雲端服務，但主要針對開發環境，對於社交媒體的支持較弱。 | 如果主要需求是開發環境的雲端支持，而非社交媒體帳號的管理。 | high，因為需要完全重新配置和適應新的環境。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **WechatOnCloud** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 提供雲端桌面服務，但不專注於社交媒體帳號的管理，適合需要更廣泛應用的用戶。 | 雖然也提供雲端服務，但主要針對開發環境，對於社交媒體的支持較弱。 |
> | 遷移成本 | - | medium，因為需要重新配置和適應不同的使用方式。 | high，因為需要完全重新配置和適應新的環境。 |
> | 適用場景 | 主要場景 | 如果需要更廣泛的應用支持，而不僅限於社交媒體帳號的管理。 | 如果主要需求是開發環境的雲端支持，而非社交媒體帳號的管理。 |
...
- **[HIGH]** 首次新建實例若卡住報 `创建容器失败`，可能是 Docker 拉取超時。
  - 解法：手動拉取鏡像，確保本地有可用的鏡像。
- **[HIGH]** 面板需在內網環境中運行，否則可能暴露用戶數據。
  - 解法：使用 VPN 或內網穿透工具。
- [MEDIUM] 需要改變默認密碼，否則存在安全風險。
  - 解法：在部署前修改 `.env` 文件中的密碼。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的小型團隊管理社交媒體帳號 | 非常適合 | 支持多用戶共享同一會話，方便團隊協作。 |
| 需要在雲端長期運行的社交媒體帳號 | 非常適合 | 支持多實例管理，方便用戶隨時訪問。 |
| 大型企業的社交媒體管理 | 不適合 | 可能需要更強的安全性和管理功能。 |
| 開發者測試不同版本的微信 | 適合 | 能快速創建和刪除實例，保持數據獨立。 |
...
> [!tip] 投入 vs 回報
> 花 3 小時學習、2 小時整合，得到靈活的社交媒體管理工具，值得。

## 安全性評估

> [!warning] 安全性快速掃描
> 這個工具需要高權限來運行 Docker，並且會存取用戶的社交媒體帳號，因此必須在內網環境中運行，避免暴露於公網。
...
> | 貢獻者 | Commits |
> | --- | --- |
> | [@Gloridust](https://github.com/Gloridust) | 107 |
...
**最新版本**：v1.3.1 — v1.3.1 — 修复升级实例后 Chromium 黑屏（严重回归） (2026-07-05)

> [!info]- Release Notes
> ## 🔴 严重修复：升级实例后浏览器黑屏崩溃循环
> 
> **谁受影响：** 用「Chromium 浏览器」类型实例、且升级过实例镜像（或新建于 v1.3.0）的用户。表现为进入桌面后**只有鼠标、无任何窗口**（微信实例不受影响）。
> 
> **根因：** v1.3.0 重建实例镜像时，Debian 源把 Chromium 拉到了 149。较新 Chromium 在 KasmVNC 尚无客户端连接、分辨率/DPI 为退化值时，device-scale-factor 读成 0 → 显示变换矩阵不可逆 → GPU/viz 进程连崩 → 浏览器退出 → autostart 每 2s 重启 → 死循环黑屏。旧版 Chromium 能容忍，149 变严格才暴露（非某个改动的代码 bug，是打包版本漂移）。
> 
> **修复：** Chromium 启动强制 `--force-device-scale-factor=1`，使变换矩阵可逆，从根上消除崩溃。已在最坏情形（无客户端连接、分辨率退化）下验证：30s 零崩溃、窗口正常。
> 
> **如何拿到修复：** 到面板「管理」对受影响实例点**「升级实例」**（或用下面的「一键升级全部实例」）。
> 
> ## ✨ 更新体验改进
> - **实例镜像落后检测 + 一键升级全部**：面板检测到实例镜像落后会在「管理」页顶部提示，一键升级全部落后实例。每张实例卡也标「可升级」。
> - **关于页提示**：更新面板后，若实例镜像也有新版会提示——**更新面板 ≠ 更新实例**（二者是不同镜像）。
> - **壁纸/字体友好报错**：实例镜像过旧（缺 xwallpaper）时直接提示「请先升级实例」，不再是看懵的退出码 127。
> 
> **升级面板：** 面板「设置 → 关于」点「一键更新面板」，或 `docker compose pull && docker compose up -d`。

## 社群與生態

**社群活躍度**：社群活躍，問題解決率高，經常有更新和改進。
**連結**：[文件](https://github.com/Gloridust/WechatOnCloud)

## 開發動態

> [!abstract] 最近 10 次 commit（2026-07-04 ~ 2026-07-05）
> **活躍天數** 2 天 · **最新 commit** Update .gitignore
...
> | --- | --- | --- | --- |
> | [#107](https://github.com/Gloridust/WechatOnCloud/issues/107) | 桌面长时间未就绪 实例可能在重启或未运行。可继续重试，或用左上角菜单返回主页让管理员检查。 重试 | 0 | 1 |
...
> FROM "Repos"
> WHERE subcategory = "雲端服務" AND file.name != "Gloridust--WechatOnCloud"
> SORT stars DESC
> ```
