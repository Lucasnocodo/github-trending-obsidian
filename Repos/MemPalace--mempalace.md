...
use_case: "提供一個本地優先的 AI 記憶系統，無需 API 金鑰，能夠有效地儲存和檢索對話歷史。"
priority: high
ring: assess
...
aliases:
  - "mempalace"
  - "MemPalace/mempalace"
  - "提供一個本地優先的 AI 記憶系統，無需 API 金鑰，能夠有效地儲存和檢索對話歷史。"
---

# mempalace

**51.5k** stars · **1.6k** stars/天 · 建立 33 天前 · Python · MIT
...
> [!info] 速覽
> **安裝難度** Easy · **專案狀態** Growing · **熱度** Viral (1.6k stars/day)
> **授權** MIT (商業友好) · **維護** Active (最後推送 0 天前) · **貢獻者** 5+ 人 · **參與度** Medium
> **適合** 需要在本地環境中管理大量對話和專案資料的開發者和研究人員。
> **一句話重點** MemPalace 的本地優先設計讓用戶在管理 AI 記憶時擁有更高的隱私和控制權。
...
> **結論** 花 5 小時學習，3 小時整合，得到高隱私性和靈活性的記憶管理系統，值得投入。

> [!abstract] 核心創新
> 提供一個完全本地化的 AI 記憶系統，無需 API 金鑰，並且支持高效的語義檢索。

## 專案簡介

MemPalace 是一個本地優先的 AI 記憶系統，專注於以逐字文本儲存對話歷史並透過語義搜索進行檢索。用戶可以透過 `mempalace mine` 指令將專案文件或對話內容挖掘進入記憶宮殿，並使用 `mempalace search` 指令進行查詢，這樣的設計讓用戶不必依賴雲端服務或 API 金鑰，提升了隱私性和安全性。系統的索引結構化，將人員和專案視為「翼」，主題視為「房間」，這樣的設計使得搜索可以更具針對性，而不是對一個平坦的語料庫進行檢索。MemPalace 的檢索層是可插拔的，預設使用 ChromaDB，這意味著用戶可以根據需求更換後端而不影響整體系統。這種靈活性使得它在不同的使用場景中都能表現良好。

與其他工具相比，MemPalace 的優勢在於其高達 96.6% 的檢索回憶率，且不需要任何 LLM 的支持，這在處理大量對話數據時特別有用。相較於其他 AI 記憶系統，如 Mem0 和 Hindsight，MemPalace 提供了更高的隱私保護和更靈活的架構選擇。使用者在實際操作中可能會遇到一些問題，例如在現代 Linux 系統上可能會遇到兼容性問題，但這些問題通常可以透過社群的支持來解決。總體來說，MemPalace 是一個值得考慮的選擇，特別是對於需要高隱私性和靈活性的開發者。未來幾個月內，預計會有更多的功能和改進推出，進一步提升其使用體驗。

**技術棧**：`Python 3.9+` · `ChromaDB`

## 重點功能

- 逐字儲存 — 將對話歷史逐字儲存，無需摘要或提煉。
- 可插拔後端 — 預設使用 ChromaDB，支持更換其他後端而不影響系統運作。
- 高檢索回憶率 — 在 LongMemEval 測試中達到 96.6% 的檢索回憶率，無需 LLM 支持。
- 本地優先設計 — 完全在本地運行，無需 API 金鑰或雲端服務。
- 跨專案連結 — 自動連結不同專案的相似主題，提升資料整合性。
...
> [!tip] 爆紅原因
> 建立 33 天就累積 51469 stars（1560/天），forks 6774（13.2%），顯示出強勁的增長勢頭。這個專案的主要貢獻者包括 igorls 和 bensig，他們在開源社群中有著良好的聲譽。MemPalace 解決了以往 AI 記憶系統需要依賴雲端服務的痛點，提供了一個完全本地化的解決方案，這在隱私日益受到重視的當下尤為重要。此專案的推出恰逢對於本地 AI 記憶系統需求上升的時期，並且在社群中引發了廣泛的討論和關注。forks/stars 比率為 13.2%，顯示出許多人對這個專案的實際修改和使用。

## 適合誰使用

**目標受眾**：需要在本地環境中管理大量對話和專案資料的開發者和研究人員。

> [!example] 使用場景
> - 後端工程師用它來在本地儲存和檢索專案對話歷史，因為這樣可以避免雲端服務帶來的隱私風險。
> - 產品經理用它來分析不同專案間的關聯，因為它能夠自動連結跨專案的主題，提升工作效率。
> - 研究人員用它來挖掘和整理研究資料，因為其高效的檢索能力能快速找到所需信息。

## 架構分析

MemPalace 採用本地優先的架構設計，所有數據儲存在用戶的機器上，這樣的設計不僅提升了隱私性，還減少了對網路連接的依賴。系統的索引結構化，將對話和專案資料分為不同的翼和房間，這樣的設計使得檢索更加高效且具針對性。檢索層是可插拔的，預設使用 ChromaDB，這意味著用戶可以根據需求選擇不同的後端，這樣的靈活性使得系統能夠適應不同的使用場景。選擇本地運行的代價是用戶需要自行管理數據的安全性和備份，但這也使得用戶對數據擁有更高的控制權。整體來說，這樣的架構設計使得 MemPalace 在隱私和靈活性上有著明顯的優勢。

## 技術深入分析

MemPalace 的核心技術機制在於其本地優先的設計，所有數據都儲存在用戶的機器上，這樣的設計不僅提升了隱私性，還減少了對網路連接的依賴。系統的索引結構化，將對話和專案資料分為不同的翼和房間，這樣的設計使得檢索更加高效且具針對性。檢索層是可插拔的，預設使用 ChromaDB，這意味著用戶可以根據需求選擇不同的儲存後端。這樣的靈活性使得系統能夠適應不同的使用場景。

效能方面，MemPalace 在 LongMemEval 測試中達到 96.6% 的檢索回憶率，這在不依賴 LLM 的情況下實現，顯示出其強大的檢索能力。設計取捨方面，選擇本地運行的代價是用戶需要自行管理數據的安全性和備份，但這也使得用戶對數據擁有更高的控制權。技術風險方面，隨著使用者數量的增加，系統的性能可能會受到影響，特別是在數據量大時。整合分析方面，MemPalace 與主流的開發工具鏈相容性良好，能夠輕鬆集成到現有的工作流程中，並且支持 CLI 操作，這對於開發者來說非常友好。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且包含安裝和使用範例，安裝過程相對簡單，沒有明顯的坑。提供了良好的入門指南，並且文件有多語言支持，適合新手快速上手。

## 優缺點分析

> [!success] 優點
> - 高隱私性 — 所有數據都儲存在本地，無需擔心雲端數據洩漏。
> - 靈活的後端選擇 — 用戶可以根據需求選擇不同的儲存後端。
> - 高效的檢索能力 — 在 LongMemEval 測試中達到 96.6% 的檢索回憶率。

> [!danger] 缺點
> - 需要一定的技術知識來設置和維護本地環境。
> - 在某些 Linux 環境下可能會遇到兼容性問題。
> - 缺乏雲端功能，對於需要跨設備同步的用戶可能不太方便。

> [!warning] 注意事項
> - 僅支援 Python 3.9 以上版本。
> - 需要安裝 ChromaDB 作為後端儲存。
> - 在某些 Linux 環境下可能會遇到兼容性問題。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 這個工具專注於創建 AI 代理，而 MemPalace 更加專注於記憶管理和檢索，適合需要記錄和檢索對話的場景。 |
| Hindsight | Hindsight 提供雲端服務，而 MemPalace 完全在本地運行，這對於需要高隱私性的用戶來說是更好的選擇。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| Hindsight | Hindsight 提供雲端服務，而 MemPalace 完全在本地運行，這對於需要高隱私性的用戶來說是更好的選擇。 | 如果你的團隊需要雲端功能並且不介意數據存儲在外部服務上。 | medium — 需要將數據從本地遷移到雲端，並重新設置環境。 |
| Mem0 | Mem0 提供類似的記憶管理功能，但缺乏 MemPalace 的本地優先設計，這使得用戶的數據安全性較低。 | 如果你的團隊不需要本地存儲，並且希望使用更簡單的雲端解決方案。 | low — 直接使用雲端服務，無需特別的遷移步驟。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **mempalace** | **Hindsight** | **Mem0** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | Hindsight 提供雲端服務，而 MemPalace 完全在本地運行，這對於需要高隱私性的用戶來說是更好的選擇。 | Mem0 提供類似的記憶管理功能，但缺乏 MemPalace 的本地優先設計，這使得用戶的數據安全性較低。 |
> | 遷移成本 | - | medium — 需要將數據從本地遷移到雲端，並重新設置環境。 | low — 直接使用雲端服務，無需特別的遷移步驟。 |
> | 適用場景 | 主要場景 | 如果你的團隊需要雲端功能並且不介意數據存儲在外部服務上。 | 如果你的團隊不需要本地存儲，並且希望使用更簡單的雲端解決方案 |
...
- **[HIGH]** 在某些 Linux 環境下可能會遇到兼容性問題，導致安裝失敗。
  - 解法：可以嘗試使用虛擬環境或 Docker 來隔離安裝環境。
- [MEDIUM] Claude Code 會在 30 天後過期，若未設置自動保存鉤子將導致數據丟失。
  - 解法：確保設置自動保存鉤子以防止數據丟失。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 本地優先設計能夠保護敏感數據，並且易於管理。 |
| 需要跨設備同步的團隊 | 不適合 | MemPalace 完全在本地運行，無法實現跨設備的數據同步。 |
| 需要高隱私性的個人項目 | 非常適合 | 所有數據都儲存在本地，無需擔心雲端數據洩漏。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，3 小時整合，得到高隱私性和靈活性的記憶管理系統，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：MemPalace 完全在本地運行，無需高權限操作，且不會存取敏感資料。
...
> | 貢獻者 | Commits |
> | --- | --- |
> | [@igorls](https://github.com/igorls) | 348 |
...
> **活躍天數** 1 天 · **最新 commit** Merge pull request #1406 from MemPalace/fix/925-diary-content-hash
