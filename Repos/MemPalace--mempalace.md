...
use_case: "提供一個本地的 AI 記憶系統，讓用戶能夠以語義搜尋方式檢索對話歷史。"
priority: high
ring: assess
...
star_history: "2026-04-12:41802,2026-04-12:41809,2026-04-13:43509,2026-04-14:44893,2026-04-15:45915,2026-04-16:46607,2026-04-17:47148,2026-04-18:47576,2026-04-19:47861,2026-04-20:48135,2026-04-21:48498,2026-04-22:48797"
tags:
  - github
  - "category/ai_ml"
  - "lang/python"
...
aliases:
  - "mempalace"
  - "MemPalace/mempalace"
  - "提供一個本地的 AI 記憶系統，讓用戶能夠以語義搜尋方式檢索對話歷史。"
---

# mempalace

**48.8k** stars · **2.9k** stars/天 · 建立 17 天前 · Python · MIT
...
> [!info] 速覽
> **安裝難度** Easy · **專案狀態** Recent · **熱度** Viral (2.9k stars/day)
> **授權** MIT (商業友好) · **維護** Active (最後推送 0 天前) · **貢獻者** 5+ 人 · **參與度** Medium
> **適合** 需要一個本地化的 AI 記憶系統以管理對話數據的開發者和團隊。
> **一句話重點** MemPalace 的設計不僅強調隱私性，還提供了一個靈活且高效的本地記憶系統。
...
> [!abstract] 核心創新
> 提供一個完全本地的 AI 記憶系統，無需 API 鍵。

## 專案簡介

MemPalace 是一個本地優先的 AI 記憶系統，專注於將對話歷史以逐字文本形式儲存並進行語義搜尋。用戶可以透過 `mempalace mine` 指令將專案檔案或對話內容挖掘進入記憶系統，並使用 `mempalace search` 指令進行搜尋，這樣的設計讓用戶不需要依賴 API，所有資料都保留在本地，增強了隱私性。系統的索引結構化，將人員和專案視為「翼」，主題視為「房間」，而原始內容則存放在「抽屜」中，這樣的設計使得搜尋可以更具針對性，而不是對一個平坦的資料庫進行搜尋。MemPalace 的檢索層是可插拔的，預設使用 ChromaDB 作為後端，這意味著用戶可以根據需求更換後端而不影響整體系統。根據基準測試，MemPalace 在 LongMemEval 上的檢索回憶率達到 96.6%，這在不需要任何 API 鍵的情況下實現，這是其最大的賣點之一。

與其他類似工具相比，如 Hindsight 和 Zep，MemPalace 提供了更高的隱私性和可擴展性，因為所有數據都存儲在本地，並且不需要依賴外部服務。使用者在使用過程中可能會遇到一些常見問題，如與現代 Linux 的相容性問題，這些問題在 GitHub Issues 中有提到。整體而言，MemPalace 是一個適合需要高隱私性和本地存儲的開發者使用的工具，特別是在處理大量對話數據時。未來六個月內，隨著更多功能的加入和社群的活躍，預期會有更好的使用體驗和性能改進。
...
- 可插拔後端 — 預設使用 ChromaDB，支持更換其他後端而不影響系統。
- 高隱私性 — 所有數據均保留在本地，無需 API 鍵或雲端服務。
- 結構化索引 — 將人員、專案和主題進行結構化，便於針對性搜尋。
- 高效檢索 — 在 LongMemEval 上達到 96.6% 的檢索回憶率，無需 LLM 支持。
...
> [!tip] 爆紅原因
> 建立 17 天就累積 48797 stars（2870/天），forks 6388（13.1%），這顯示出強烈的社群關注。這個專案的主要貢獻者有多位活躍的開發者，且其解決了以往 AI 記憶系統需要依賴雲端服務的痛點，提供了一個完全本地的解決方案。近期的社群討論和問題反饋也顯示出用戶對於隱私和數據控制的需求日益增加，這使得 MemPalace 的出現恰逢其時。這個專案的設計也符合當前對於本地化 AI 解決方案的需求，並且在技術上具備可擴展性，讓用戶能夠根據需求調整後端存儲。

## 適合誰使用

**目標受眾**：需要一個本地化的 AI 記憶系統以管理對話數據的開發者和團隊。

> [!example] 使用場景
> - 後端工程師用它來將專案的對話歷史儲存並檢索，因為這樣可以在不依賴雲端的情況下保護敏感資訊。
> - 產品經理用它來記錄會議內容並快速搜尋過去的決策，因為這樣能夠提高工作效率，避免重複討論。
> - AI 開發者用它來測試不同的對話模型，因為能夠快速挖掘和檢索過去的對話數據，提升模型的訓練效果。

## 架構分析

MemPalace 採用本地優先的架構，所有數據均存儲在用戶的設備上，這樣的設計旨在提高隱私性和數據控制。系統的索引結構化，將對話歷史分為翼、房間和抽屜，這使得檢索過程更具針對性。檢索層是可插拔的，預設使用 ChromaDB，這意味著用戶可以根據需求選擇不同的後端而不影響整體系統。這樣的設計選擇使得系統在性能上具備靈活性，但也要求用戶具備一定的技術能力來進行後端的替換和配置。整體而言，這種架構設計在提高隱私性的同時，也可能在某些情況下增加使用的複雜度。

## 技術深入分析

MemPalace 的核心技術機制在於其結構化的索引系統，這使得用戶能夠以更具針對性的方式檢索對話歷史。系統使用 ChromaDB 作為預設的向量存儲後端，這樣的選擇使得檢索速度更快且效率更高。根據基準測試，MemPalace 在 LongMemEval 上的檢索回憶率達到 96.6%，這表明其在無需 LLM 支持的情況下也能提供高效的檢索能力。設計上，MemPalace 選擇了 Python 作為主要開發語言，這使得其在開發和維護上具備良好的可讀性和擴展性。依賴樹相對簡單，主要依賴 ChromaDB 和 PyYAML，這使得整體系統的安裝和維護成本較低。隨著用戶數據量的增加，系統的性能可能會受到影響，特別是在檢索速度上，這需要在設計上考慮到擴展性。整體而言，MemPalace 在技術上具備良好的可擴展性，並且能夠與主流的開發框架進行整合，這使得其在實際應用中具備較高的靈活性。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝和使用指導，並包含了範例指令。安裝過程相對順暢，但對於不熟悉命令行的用戶來說，可能會有一定的學習曲線。文件目前僅提供英文版本，對於非英語使用者可能會造成一定的障礙。
...
> [!warning] 注意事項
> - 僅支援 Python 3.9 以上版本。
> - 需要約 300 MB 的磁碟空間來存儲預設的嵌入模型。
> - 在某些現代 Linux 環境下可能會遇到相容性問題。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard 提供了一個類似的記憶系統，但主要依賴於雲端存儲，這使得隱私性較低。 |
| [Hindsight/hindsight](https://github.com/Hindsight/hindsight) | Hindsight 提供了記憶功能，但其設計上更依賴於外部 API，使用上較為繁瑣。 |
| [Zep/zep](https://github.com/Zep/zep) | Zep 也提供記憶功能，但其性能和檢索效率不及 MemPalace 的高效檢索能力。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [Hindsight/hindsight](https://github.com/Hindsight/hindsight) | Hindsight 依賴於雲端服務來存儲和檢索數據，而 MemPalace 則完全在本地運行，這樣的設計使得 MemPalace 在隱私性上更具優勢。 | 如果你的團隊需要一個快速上手的雲端解決方案，Hindsight 可能更適合。 | medium，因為需要將數據從雲端轉移到本地系統。 |
| [Zep/zep](https://github.com/Zep/zep) | Zep 提供了類似的記憶功能，但其性能和檢索效率不及 MemPalace 的高效檢索能力。 | 如果你的應用需要更高的檢索效率和本地存儲，MemPalace 是更好的選擇。 | low，因為兩者的數據結構相似，轉移相對簡單。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **mempalace** | **hindsight** | **zep** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | Hindsight 依賴於雲端服務來存儲和檢索數據，而 MemPalace 則完全在本地運行，這樣的設計使得 MemPalace 在隱私性上更具優勢。 | Zep 提供了類似的記憶功能，但其性能和檢索效率不及 MemPalace 的高效檢索能力。 |
> | 遷移成本 | - | medium，因為需要將數據從雲端轉移到本地系統。 | low，因為兩者的數據結構相似，轉移相對簡單。 |
> | 適用場景 | 主要場景 | 如果你的團隊需要一個快速上手的雲端解決方案，Hindsigh | 如果你的應用需要更高的檢索效率和本地存儲，MemPalace |
...
- **[HIGH]** 在某些現代 Linux 環境下可能會遇到相容性問題，特別是與 Python 3 的相容性。
  - 解法：檢查 Python 版本並確保使用正確的環境配置。
- [MEDIUM] 使用 `mempalace mine` 指令時，可能會遇到權限問題。
  - 解法：確保以適當的權限執行命令。
- **[HIGH]** 在特定情況下，可能會出現資料損壞的情況。
  - 解法：定期備份資料以防止損失。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 因為其本地存儲和高隱私性非常符合小型團隊的需求。 |
| 大型企業的數據分析部門 | 普通 | 雖然功能強大，但在數據量大時可能會遇到性能瓶頸。 |
| 個人開發者的 AI 專案 | 非常適合 | 能夠快速整合並提供高效的記憶功能。 |
| 需要高可擴展性的雲端應用 | 不適合 | 因為其設計是以本地優先為主，不適合雲端環境。 |
...
> [!warning] 安全性快速掃描
> 低風險：本工具不需要高權限，所有數據均保留在本地，並且不會存取敏感資料。依賴鏈的信任程度高，未發現已知的供應鏈風險。
...
> | 貢獻者 | Commits |
> | --- | --- |
> | [@igorls](https://github.com/igorls) | 161 |
> | [@bensig](https://github.com/bensig) | 107 |
> | [@milla-jovovich](https://github.com/milla-jovovich) | 29 |
> | [@tmuskal](https://github.com/tmuskal) | 26 |
> | [@jphein](https://github.com/jphein) | 25 |

**最新版本**：v3.3.2 (2026-04-21)
...
> **活躍天數** 4 天 · **最新 commit** Merge pull request #661 from jphein/perf/graph-cache
