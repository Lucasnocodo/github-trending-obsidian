...
use_case: "提供一個本地的 AI 記憶系統，讓使用者能夠將對話歷史儲存為可搜尋的文本。"
priority: high
ring: assess
...
star_history: "2026-04-12:41802,2026-04-12:41809,2026-04-13:43509,2026-04-14:44893,2026-04-15:45915,2026-04-16:46607,2026-04-17:47148,2026-04-18:47576,2026-04-19:47861,2026-04-20:48135,2026-04-21:48498,2026-04-22:48797,2026-04-23:49029,2026-04-24:49281,2026-04-25:49482,2026-04-26:49660"
tags:
  - github
  - "category/ai_ml"
  - "lang/python"
...
aliases:
  - "mempalace"
  - "MemPalace/mempalace"
  - "提供一個本地的 AI 記憶系統，讓使用者能夠將對話歷史儲存為可搜尋的文本。"
---

# mempalace

**49.7k** stars · **2.4k** stars/天 · 建立 21 天前 · Python · MIT
...
> [!info] 速覽
> **安裝難度** Easy · **專案狀態** Recent · **熱度** Viral (2.4k stars/day)
> **授權** MIT (商業友好) · **維護** Active (最後推送 0 天前) · **貢獻者** 5+ 人 · **參與度** Medium
> **適合** 需要在本地環境中儲存和檢索對話歷史的開發者和產品經理。
> **一句話重點** MemPalace 的設計不僅強調資料的安全性，還提供高效的檢索能力，這在當前對隱私要求日益提高的環境中尤為重要。
...
> [!abstract] 核心創新
> MemPalace 提供了一個本地優先的 AI 記憶系統，無需 API 金鑰，並且支持高效的語義搜尋。

## 專案簡介

MemPalace 是一個本地優先的 AI 記憶系統，能夠將對話歷史以逐字文本形式儲存，並透過語義搜尋進行檢索。使用者可以透過 `mempalace mine` 指令將項目文件或對話內容挖掘進入記憶宮殿，並用 `mempalace search` 指令進行搜尋，這種設計使得使用者能夠在不依賴雲端或 API 的情況下，快速獲取過去的對話內容。系統的索引結構化，將人員和項目視為「翼」，主題視為「房間」，原始內容則存放在「抽屜」中，這樣的設計使得搜尋可以更具針對性，而不是在平坦的資料庫中進行。MemPalace 的檢索層是可插拔的，預設使用 ChromaDB，並允許使用者根據需求替換後端。這種靈活性使得系統能夠適應不同的使用情境，並且不會將資料上傳至雲端，確保了使用者的隱私。其核心賣點是高達 96.6% 的檢索回憶率，且無需 API 金鑰或雲端服務，這使得它在本地環境中表現出色。
...
- 可插拔後端 — 預設使用 ChromaDB，並可根據需求替換為其他後端。
- 高檢索回憶率 — 在 LongMemEval 基準測試中達到 96.6% 的 R@5，無需 API 金鑰。
- 結構化索引 — 將資料組織為翼、房間和抽屜，提升搜尋效率。
- 本地運行 — 所有資料儲存在本地，確保使用者隱私。
...
> [!tip] 爆紅原因
> 建立 21 天內累積 49660 stars（2365/天），forks 6508（13.1%），顯示出強烈的社群關注。這個專案由多位貢獻者共同開發，解決了許多 AI 記憶系統在隱私和資料存取上的痛點，特別是在不需要依賴外部 API 的情況下。近期的討論和問題反映出使用者對於系統性能和功能的高度關注，尤其是在基準測試結果的透明度上。技術上，這個工具的可插拔設計和本地運行的特性，使其在當前對數據隱私要求日益提高的環境中，顯得尤為重要。

## 適合誰使用

**目標受眾**：需要在本地環境中儲存和檢索對話歷史的開發者和產品經理。

> [!example] 使用場景
> - 後端工程師用它來將專案文件內容挖掘進記憶宮殿，因為這樣可以快速檢索過去的設計決策，避免重複工作。
> - 產品經理用它來儲存會議紀錄和對話歷史，因為這樣能夠在需要時迅速回顧過去的討論，提升決策效率。
> - AI 開發者用它來測試和調整 AI 模型的回應，因為能夠在本地環境中進行高效的資料檢索，無需擔心資料外洩。

## 架構分析

MemPalace 採用本地優先的架構設計，所有資料儲存在使用者的機器上，這樣的設計確保了隱私和安全性。系統的資料流分為數個層級：首先，使用者通過 CLI 指令將資料挖掘進入記憶宮殿，然後這些資料會被結構化存儲，最後透過語義搜尋進行檢索。這種設計的優勢在於使用者不需要依賴外部 API，並且能夠快速獲取過去的對話內容。選擇 ChromaDB 作為預設後端，因為它在性能和可擴展性上表現優異，但這也意味著使用者需要自行整合其他後端時可能面臨挑戰。整體架構的可擴展性高，但在資料量極大時可能會遇到性能瓶頸。

## 技術深入分析

MemPalace 的核心技術機制是將對話歷史以逐字文本形式儲存，並透過語義搜尋進行檢索。這種設計使得使用者能夠在不依賴外部 API 的情況下，快速獲取過去的對話內容。系統使用 ChromaDB 作為預設的向量存儲後端，這使得檢索速度和準確性都得到了保障。根據基準測試，MemPalace 在 LongMemEval 中的檢索回憶率達到 96.6%，這顯示出其強大的性能。選擇 Python 作為開發語言，使得專案易於維護和擴展，但也可能導致在性能要求極高的場景下出現瓶頸。技術風險方面，若使用者資料量過大，可能會影響檢索效率，這是未來需要優化的方向。整合方面，MemPalace 可以輕鬆與現有的開發流程結合，特別是在使用 CLI 工具時，能夠無縫接入現有的工作流。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且詳細，提供了安裝和使用的範例。安裝過程相對順暢，但需要注意 Python 版本要求。文件中有良好的入門指南，並且有多語言支持，這對於新手來說非常友好。

## 優缺點分析

> [!success] 優點
> - 高效的語義搜尋能力，檢索回憶率高達 96.6%。
> - 所有資料儲存在本地，確保使用者隱私。
> - 可插拔的後端設計，靈活性高。

> [!danger] 缺點
> - 目前僅支援 ChromaDB，其他後端整合需額外工作。
> - 對於大型資料集的性能可能會有瓶頸。
> - 需要一定的技術背景來進行安裝和配置。

> [!warning] 注意事項
> - 僅支援 Python 3.9 以上版本
> - 需要大約 300 MB 磁碟空間來儲存預設的嵌入模型
> - 目前僅支援 ChromaDB 作為向量存儲後端，其他後端需自行整合

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard 提供類似的記憶功能，但主要依賴雲端服務，對於隱私的考量不如 MemPalace。 |
| [AgriciDaniel/claude-obsidian](https://github.com/AgriciDaniel/claude-obsidian) | Claude-Obsidian 專注於筆記整理，而 MemPalace 更加強調對話歷史的逐字儲存和檢索。 |
| [Zep/zep](https://github.com/Zep/zep) | Zep 提供即時的筆記和協作功能，但不具備 MemPalace 的高效能檢索能力和本地運行特性。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [Zep/zep](https://github.com/Zep/zep) | Zep 提供即時的筆記和協作功能，但它依賴雲端服務，對於隱私的考量不如 MemPalace。 | 如果你的團隊需要即時協作和雲端存儲的功能，Zep 會是更好的選擇。 | medium，因為需要將資料從本地轉移到雲端。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard 提供類似的記憶功能，但主要依賴雲端服務，對於隱私的考量不如 MemPalace。 | 如果你需要一個雲端解決方案，並且不太關心資料隱私，Boneyard 會是合適的選擇。 | high，因為需要重構整個資料存儲架構。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **mempalace** | **zep** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | Zep 提供即時的筆記和協作功能，但它依賴雲端服務，對於隱私的考量不如 MemPalace。 | Boneyard 提供類似的記憶功能，但主要依賴雲端服務，對於隱私的考量不如 MemPalace。 |
> | 遷移成本 | - | medium，因為需要將資料從本地轉移到雲端。 | high，因為需要重構整個資料存儲架構。 |
> | 適用場景 | 主要場景 | 如果你的團隊需要即時協作和雲端存儲的功能，Zep 會是更好的 | 如果你需要一個雲端解決方案，並且不太關心資料隱私，Boney |
...
- **[HIGH]** 在 Linux 上安裝時可能會遇到 PEP 668 的相容性問題
  - 解法：使用虛擬環境來隔離安裝。
- [MEDIUM] 在大型資料集上進行搜尋時性能可能下降
  - 解法：考慮將資料分割成多個小型宮殿。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 能夠在本地儲存和檢索對話歷史，確保資料隱私。 |
| 大型企業的資料分析團隊 | 普通 | 雖然能夠處理資料，但在性能上可能會遇到瓶頸。 |
| 個人開發者的 AI 專案 | 非常適合 | 提供高效的記憶功能，且無需依賴外部服務。 |
| 需要即時協作的團隊 | 不適合 | 缺乏雲端協作功能，無法滿足即時需求。 |
...
> [!warning] 安全性快速掃描
> 低風險：MemPalace 本身不需要高權限運行，所有資料儲存在本地，並且不會存取敏感資料。依賴鏈的信任程度高，無已知的供應鏈風險，適合在 CI/CD 中使用。
...
> | 貢獻者 | Commits |
> | --- | --- |
> | [@igorls](https://github.com/igorls) | 221 |
...
> [!abstract] 最近 10 次 commit（2026-04-17 ~ 2026-04-25）
> **活躍天數** 2 天 · **最新 commit** Merge pull request #935 from shaun0927/fix/repair-crash-safety
