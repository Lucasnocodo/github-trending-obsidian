...
homepage: "https://mempalace.github.io/mempalace/"
stars: 43509
stars_per_day: 5439
forks: 5566
open_issues: 358
...
use_case: "提供一個高效能的 AI 記憶系統，讓你能夠隨時檢索過去的對話和決策。"
priority: high
ring: assess
...
star_history: "2026-04-12:41802,2026-04-12:41809,2026-04-13:43509"
tags:
  - github
  - "category/ai_ml"
  - "lang/python"
...
aliases:
  - "mempalace"
  - "MemPalace/mempalace"
  - "提供一個高效能的 AI 記憶系統，讓你能夠隨時檢索過去的對話和決策。"
---

# mempalace

**43.5k** stars · **5.4k** stars/天 · 建立 8 天前 · Python · MIT
...
> [!info] 速覽
> **安裝難度** Easy · **專案狀態** Recent · **熱度** Viral (5.4k stars/day)
> **授權** MIT (商業友好) · **維護** Active (最後推送 0 天前) · **貢獻者** 5+ 人 · **參與度** Medium
> **適合** 需要一個本地運行的 AI 記憶系統，能夠在不依賴雲端服務的情況下進行資料存儲和檢索的開發者。
> **一句話重點** MemPalace 的創新在於其不依賴 AI 決定記憶的重要性，而是將所有對話原文存儲並結構化，讓用戶能夠自由檢索。
...
> [!abstract] 核心創新
> MemPalace 是目前唯一一個在 LongMemEval 測試中達到 96.6% 的高分且完全免費的 AI 記憶系統。

## 專案簡介

MemPalace 是一個 AI 記憶系統，旨在解決每次與 AI 互動後記憶消失的問題。用戶可以透過 `mempalace init` 指令進行初始化，然後使用 `mempalace mine` 指令來提取和儲存對話或項目資料。這個系統的核心在於將所有對話原文存儲在 ChromaDB 中，並透過結構化的方式（如 wings 和 rooms）來提升檢索效率，這樣用戶可以隨時查詢過去的決策和討論。MemPalace 的設計理念是讓用戶掌控記憶，而不是讓 AI 決定什麼重要，這樣的設計使得用戶能夠在需要時快速找到所需信息。技術上，MemPalace 使用 Python 開發，並依賴 ChromaDB 作為資料庫，這使得它在處理大量資料時能夠保持高效。

這個系統的表現經過多項基準測試，顯示出在 LongMemEval 測試中達到 96.6% 的高分，這是目前已知的最高分數，且不需要任何 API 認證。與其他記憶系統相比，MemPalace 不依賴於外部服務，所有資料均保留在本地，這樣的設計不僅提高了安全性，也減少了使用成本。使用者可以在本地運行，並根據個人需求進行調整，這使得它非常適合需要私密性和控制權的開發者和團隊。這個專案目前處於 beta 階段，社群活躍且持續修正問題，值得關注。

**技術棧**：`Python 3.9+` · `ChromaDB`

## 重點功能

- 全原文儲存 — 將所有對話原文存儲在 ChromaDB 中，無需摘要或提取。
- 結構化檢索 — 使用 wings 和 rooms 組織記憶，提升檢索效率達 34%。
- 本地運行 — 完全在用戶的機器上運行，無需外部 API，保障資料隱私。
- 多種挖掘模式 — 支持對項目和對話的多種挖掘模式，滿足不同需求。
- 高效能基準 — 在 LongMemEval 測試中達到 96.6% 的高分，為目前最高記錄。
...
> [!tip] 爆紅原因
> 建立 8 天內累積 43509 stars（5439/天），forks 5566（12.8%），顯示出強勁的增長勢頭。這個專案由 Milla Jovovich 和 Ben Sigman 等人開發，解決了過去 AI 記憶系統無法有效存儲和檢索對話的痛點。之前的解決方案如 Supermemory 和 Mastra 都需要 API 認證且有額外費用，而 MemPalace 則完全免費且不依賴雲端服務。社群的反饋和修正速度也顯示出這個專案的活力，尤其是在推出後迅速修正 README 中的錯誤。這些因素共同促成了它的快速成長。

## 適合誰使用

**目標受眾**：需要一個本地運行的 AI 記憶系統，能夠在不依賴雲端服務的情況下進行資料存儲和檢索的開發者。

> [!example] 使用場景
> - 獨立開發者用它來儲存和檢索過去的項目決策，因為這樣可以避免重複工作並提高效率。
> - 團隊領導用它來管理團隊成員的對話記錄，因為這樣可以在需要時快速回顧過去的討論和決策。
> - 資料科學家用它來整理和查詢過去的實驗結果，因為這樣可以更方便地追蹤和分析數據變化。

## 架構分析

MemPalace 採用本地運行的架構，所有資料均儲存在用戶的機器上，這樣的設計避免了對外部 API 的依賴。資料流從用戶的對話和項目資料進入系統，經過初始化和挖掘後，存儲在 ChromaDB 中。檢索時，系統根據用戶的查詢在結構化的 wings 和 rooms 中進行搜尋，這樣的設計使得用戶能夠快速找到所需的記憶。

選擇本地運行的架構雖然提高了資料安全性，但也意味著用戶需要自行管理資料的備份和安全。擴展性方面，隨著資料量的增加，可能需要進一步優化性能以保持高效能。整體而言，這樣的架構設計使得 MemPalace 成為一個靈活且強大的工具，特別適合需要私密性和控制權的用戶。

## 技術深入分析

MemPalace 的核心技術機制是將用戶的對話和項目資料原文存儲在 ChromaDB 中，這樣的設計使得用戶能夠隨時檢索過去的記憶。系統使用了結構化的 wings 和 rooms 來組織資料，這樣不僅提升了檢索效率，還讓用戶能夠更直觀地管理記憶。效能方面，MemPalace 在 LongMemEval 測試中達到 96.6% 的高分，顯示出其在資料檢索上的優越性。選擇 Python 作為開發語言，讓開發者能夠快速迭代和修正問題，但也意味著需要用戶具備一定的 Python 知識。技術風險方面，隨著資料量的增加，可能會出現性能瓶頸，這需要開發團隊持續優化系統。整合方面，MemPalace 可以與多種 AI 工具搭配使用，這使得它在現有的技術生態中具有良好的兼容性。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且提供了詳細的安裝和使用指導，安裝過程相對順暢，沒有明顯的坑。文件中有多個範例，幫助用戶快速上手。整體而言，新手體驗良好，能夠在 30 分鐘內完成設置並運行。

## 優缺點分析

> [!success] 優點
> - 完全免費且無需外部 API，降低使用成本。
> - 高效能的檢索系統，能夠快速找到過去的對話和決策。
> - 本地運行，保障資料隱私和安全。

> [!danger] 缺點
> - 目前僅支援 Python 3.9 以上版本，限制了使用者的範圍。
> - 在 Windows 環境下可能會遇到 Unicode 錯誤，影響使用體驗。
> - 對於大型資料集，可能需要進一步的性能優化。

> [!warning] 注意事項
> - 目前僅支援 Python 3.9 以上版本。
> - 在 Windows 環境下可能會遇到 Unicode 錯誤。
> - 對於大型資料集，可能需要進一步的性能優化。
...
| 工具 | 差異 |
| --- | --- |
| [Supermemory/supermemory](https://github.com/Supermemory/supermemory) | 需要 API 認證且有額外費用，MemPalace 則完全免費且不依賴雲端服務。 |
| [Mastra/mastra](https://github.com/Mastra/mastra) | 同樣需要 API 認證且有額外費用，MemPalace 提供本地運行的選擇。 |
...
| Forks | 5.6k |
| Open Issues | 358 |
| Issue 解決率 | 28% (136 closed) |
| 最後推送 | 2026-04-13 |
| 建立日期 | 2026-04-05 |
| 官方網站 | [Link](https://mempalace.github.io/mempalace/) |
| Repo 大小 | 1.9 MB |
...
**社群活躍度**：社群活躍，持續修正問題並回應使用者反饋。
**連結**：[文件](https://mempalace.github.io/mempalace/)

## 開發動態

> [!abstract] 最近 10 次 commit（2026-04-12 ~ 2026-04-12）
> **活躍天數** 1 天 · **最新 commit** style: ruff format convo_miner.py (#741)
...
> | --- | --- | --- | --- |
> | [#27](https://github.com/MemPalace/mempalace/issues/27) | Multiple issues between README claims and codebase `bug` | 310 | 36 |
...
> | [#408](https://github.com/MemPalace/mempalace/issues/408) | Plugin assumes python3 -m mempalace — breaks on modern Linux | 6 | 11 |
...
[GitHub](https://github.com/MemPalace/mempalace) · [官方網站](https://mempalace.github.io/mempalace/)
