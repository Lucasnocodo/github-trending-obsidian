...
use_case: "提供一個本地化的 AI 記憶系統，無需 API 金鑰，讓你能夠搜尋和管理對話歷史。"
priority: high
ring: assess
...
aliases:
  - "mempalace"
  - "MemPalace/mempalace"
  - "提供一個本地化的 AI 記憶系統，無需 API 金鑰，讓你能夠搜尋和管理對話歷史。"
---

# mempalace

**50.7k** stars · **1.9k** stars/天 · 建立 27 天前 · Python · MIT
...
> **適合** 需要在本地環境中管理 AI 記憶的開發者和團隊，特別是對數據隱私有高要求的使用者。
> **一句話重點** MemPalace 的設計讓用戶能夠完全掌控自己的 AI 記憶，並且不必擔心數據隱私問題。
...
> **結論** 花 5 小時學習、3 小時整合，得到高效的本地 AI 記憶系統，值得一試。

> [!abstract] 核心創新
> MemPalace 提供了一個完全本地化的 AI 記憶系統，無需 API 金鑰，並且實現了高效的檢索回憶率。

## 專案簡介

MemPalace 是一個本地優先的 AI 記憶系統，能夠將對話歷史以逐字文本的形式存儲，並透過語意搜尋進行檢索。使用者可以透過 `mempalace mine` 指令將專案文件或對話內容挖掘進入記憶宮殿，並使用 `mempalace search` 指令來搜尋特定內容。這個設計的賣點在於完全不需要 API 金鑰或雲端服務，所有數據都保留在本地，確保隱私和安全。MemPalace 的檢索層是可插拔的，默認使用 ChromaDB，這意味著用戶可以根據需求替換後端，而不影響整體系統的運作。根據官方基準測試，MemPalace 在 LongMemEval 的檢索回憶率達到 96.6%，而不需要任何 LLM，這在同類工具中表現相當出色。

與其他工具相比，如 Mem0 和 Zep，MemPalace 提供了更高的本地化和隱私保護，並且不依賴於外部 API，這使得它在處理敏感數據時更具優勢。實際使用中，MemPalace 需要 Python 3.9 以上版本，並且對硬碟空間的需求約為 300MB。社群活躍度相對較高，但仍有一定數量的開放問題需要解決，這可能影響新用戶的體驗。整體來看，MemPalace 是一個值得探索的工具，特別適合需要本地化 AI 記憶系統的開發者和團隊。

**技術棧**：`Python 3.9+` · `ChromaDB` · `SQLite`

## 重點功能

- 本地優先設計 — 所有數據存儲在本地，無需 API 金鑰，確保數據隱私。
- 逐字存儲與檢索 — 將對話歷史以逐字文本形式存儲，並透過語意搜尋進行檢索。
- 可插拔的檢索層 — 默認使用 ChromaDB，支持替換後端而不影響系統運作。
- 高效的檢索回憶率 — 在 LongMemEval 測試中達到 96.6% 的檢索回憶率，無需 LLM。
- 跨專案主題連結 — 當不同專案涉及相同主題時，自動建立連結，方便用戶探索相關內容。
...
> [!tip] 爆紅原因
> 建立 27 天內累積 50674 stars（1877/天），forks 6655（13.1%），顯示出強大的增長潛力。作者 Igorls 和其他貢獻者在開源社群中有良好的聲譽，解決了以往 AI 記憶系統需要依賴雲端服務的痛點，讓用戶能夠完全控制自己的數據。最近的推廣活動和社群討論也吸引了更多的注意。技術上，隨著本地 LLM 的普及，這種無需 API 的設計變得越來越可行，讓更多開發者能夠輕鬆上手。高達 13.1% 的 forks/stars 比率顯示出許多人在實際修改和使用這個工具。

## 適合誰使用

**目標受眾**：需要在本地環境中管理 AI 記憶的開發者和團隊，特別是對數據隱私有高要求的使用者。

> [!example] 使用場景
> - 後端開發者用它來管理和搜尋專案中的對話歷史，因為這樣可以快速找到過去的決策和討論，避免重複工作。
> - AI 研究人員用它來挖掘和分析不同專案中的相似主題，因為這樣可以更有效地整合跨專案的知識。
> - 產品經理用它來追蹤與團隊的討論歷史，因為這樣可以清晰地了解產品需求的演變和決策過程。

## 架構分析

MemPalace 採用本地優先的架構設計，所有數據存儲在用戶的機器上，這樣可以確保數據的隱私和安全。系統的檢索層是可插拔的，默認使用 ChromaDB，這意味著用戶可以根據需求輕鬆替換後端，而不影響其他功能。資料流方面，對話歷史以逐字文本的形式存儲，並透過語意搜尋進行檢索，這樣的設計使得用戶能夠快速找到所需的資訊。選擇本地存儲而非雲端服務的代價是需要更多的硬碟空間，但這樣的設計能夠提供更高的數據安全性。擴展性方面，由於系統架構的靈活性，未來可以輕鬆加入新的檢索後端或功能模組。

## 技術深入分析

MemPalace 的核心技術機制在於其本地優先的存儲設計，所有對話歷史以逐字文本形式存儲，並透過語意搜尋進行檢索。這樣的設計不僅提高了數據的安全性，也使得檢索過程更加高效。根據基準測試，MemPalace 在 LongMemEval 測試中達到 96.6% 的檢索回憶率，這顯示出其在無需依賴 LLM 的情況下仍能提供高效的檢索能力。選擇使用 ChromaDB 作為默認後端，這使得系統在性能上能夠保持較高的效率，並且支持未來的擴展。技術上，MemPalace 的設計避免了對外部 API 的依賴，這降低了潛在的安全風險。未來，隨著用戶需求的增加，可能會出現更多的檢索後端和功能模組，這將進一步提升系統的靈活性和擴展性。整體來看，MemPalace 在記憶系統的設計上提供了一個創新的解決方案，特別適合對數據隱私有高要求的用戶。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝和使用指導，並包含範例。安裝過程相對順暢，但對於某些 Linux 環境可能會遇到相容性問題。文件中有詳細的 getting started guide，對於新手來說友好度較高。

## 優缺點分析

> [!success] 優點
> - 完全本地化設計，無需 API 金鑰，確保數據隱私。
> - 高效的檢索回憶率，無需依賴外部 LLM。
> - 可插拔的檢索層，靈活性高，易於擴展。
> - 支持跨專案主題連結，方便用戶探索相關內容。

> [!danger] 缺點
> - 僅支援 Python 3.9 以上版本，對環境要求較高。
> - 需要一定的硬碟空間來存儲嵌入模型。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | Agent Sprite Forge 提供了一個更為輕量級的記憶系統，但缺乏 MemPalace 的本地優先設計和高效檢索能力。 |
| [Mastra/memory](https://github.com/Mastra/memory) | Mastra 提供了雲端記憶服務，雖然功能強大，但對數據隱私的保護不如 MemPalace。 |
| [Zep/zep](https://github.com/Zep/zep) | Zep 提供了類似的記憶功能，但需要依賴外部 API，這在隱私方面存在風險。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [Mastra/memory](https://github.com/Mastra/memory) | Mastra 提供了雲端記憶服務，雖然功能強大，但對數據隱私的保護不如 MemPalace。 | 如果你的團隊已經在使用雲端服務，並且不太擔心數據隱私問題，Mastra 可能是一個不錯的選擇。 | medium，因為需要將數據從本地轉移到雲端。 |
| [Zep/zep](https://github.com/Zep/zep) | Zep 提供了類似的記憶功能，但需要依賴外部 API，這在隱私方面存在風險。 | 如果你的應用需要與其他雲端服務整合，Zep 可能更適合，但需注意數據隱私。 | high，因為需要重新設計數據流和 API 整合。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **mempalace** | **memory** | **zep** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | Mastra 提供了雲端記憶服務，雖然功能強大，但對數據隱私的保護不如 MemPalace。 | Zep 提供了類似的記憶功能，但需要依賴外部 API，這在隱私方面存在風險。 |
> | 遷移成本 | - | medium，因為需要將數據從本地轉移到雲端。 | high，因為需要重新設計數據流和 API 整合。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用雲端服務，並且不太擔心數據隱私問題，M | 如果你的應用需要與其他雲端服務整合，Zep 可能更適合，但需 |
...
- **[HIGH]** 在某些 Linux 環境下可能會遇到相容性問題，特別是與 Python 版本相關的問題。
  - 解法：確保使用 Python 3.9 以上版本，並檢查相關依賴。
- [MEDIUM] 在使用 `mempalace mine` 指令時，可能會因為文件路徑問題導致挖掘失敗。
  - 解法：確認文件路徑正確，並使用絕對路徑。
- [low] 社群中有許多開放問題未解決，可能影響使用體驗。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 本地優先設計能夠確保數據隱私，適合小型團隊使用。 |
| 需要處理敏感數據的企業應用 | 適合 | 完全本地化的設計能夠提供更高的數據安全性。 |
| 大型企業的核心系統 | 不適合 | 目前仍在 beta 階段，可能不夠穩定。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習、3 小時整合，得到高效的本地 AI 記憶系統，值得一試。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：MemPalace 不需要高權限，所有數據存取均在本地進行，且不依賴外部 API，降低了潛在的安全風險。

## 生態系整合

> [!abstract] 如何融入你的工具鏈

MemPalace 最常與 Python 環境搭配使用，特別是在開發和測試階段。用戶可以在本地環境中輕鬆集成 MemPalace，並使用 CLI 工具進行操作。與主流 IDE（如 VS Code）相容良好，支持通過終端執行指令。常見的整合問題包括依賴版本不匹配，建議使用虛擬環境來管理依賴。
...
> **活躍天數** 1 天 · **最新 commit** Merge pull request #1303 from MemPalace/fix/mcp-server-missing-embedding-function
