...
star_history: "2026-07-06:536,2026-07-06:541,2026-07-07:617,2026-07-07:619,2026-07-08:663,2026-07-09:700,2026-07-10:723,2026-07-11:738,2026-07-12:749,2026-07-13:757,2026-07-14:769"
tags:
  - github
  - "category/開發工具"
  - "lang/typescript"
...
**769** stars · **70** stars/天 · 建立 11 天前 · TypeScript · MIT
...
> **適合** 擔心在使用 Claude 時被標記為中國用戶的網頁開發者和數據隱私專家。
> **一句話重點** Fuck Claude 提供了一個簡單有效的方式來檢測用戶是否會被標記為中國用戶，並且完全保護了用戶的隱私。
...
> **結論** 花 2 小時學習，1 小時整合，得到快速的風險評估工具，值得使用。

> [!abstract] 核心創新
> 這個專案的創新點在於其完全本地化的風險評估機制，無需數據上傳。

## 專案簡介

Fuck Claude 是一個輕量級的單頁應用，旨在檢測用戶的瀏覽器環境，判斷是否會被 Claude Code 標記為中國用戶。用戶只需點擊一次，即可啟動一個動畫掃描，並獲得風險評估結果，所有操作均在本地完成，無需網絡請求或數據上傳。這個工具使用了 Claude Fable 5，並且支持中英文界面，適合不同語言的用戶。其核心機制是通過檢測系統時區、瀏覽器語言、已安裝的中文字體等信號來評估風險，這些信號的權重總和為 100。使用者可以透過一鍵分享功能將結果分享至社交平台，並且還提供了 API 接口供開發者使用。

這樣的設計不僅提升了用戶體驗，也讓開發者能夠方便地集成該功能。這個工具的賣點在於其完全本地化的運行方式，避免了數據隱私問題。使用者可以通過 `curl` 命令調用 API 獲取結果，並且支持根據請求頭自動選擇語言。這樣的設計使得該工具在隱私保護上表現出色，且操作簡便。整體來看，Fuck Claude 是一個針對特定需求的實用工具，尤其適合那些擔心被標記的用戶。

**技術棧**：`Astro 7.x` · `@astrojs/vercel`

## 重點功能

- 本地掃描 — 所有檢測在用戶瀏覽器中完成，無需數據上傳。
- 風險評估 — 根據系統時區、瀏覽器語言等信號計算風險分數，範圍從 0 到 100。
- 一鍵分享 — 用戶可以將結果和生成的圖片通過社交媒體分享。
- API 接口 — 提供 `/api/check` 端點，支持 curl 調用以獲取風險評估。
...
```ts
[
  "# 使用 curl 調用 API 獲取風險評估結果",
  "curl https://fuck-claude.vercel.app/api/check",
  "# 預期輸出：返回 JSON 格式的風險評估結果"
]
```
...
> [!tip] 爆紅原因
> 建立 11 天就累積 769 stars（70/天），forks 75（9.8%），顯示出不錯的增長潛力。作者 LinXiaoTao 之前的專案或許有一定的影響力，這個工具解決了用戶在使用 Claude 時可能面臨的風險評估問題。特別是在中國用戶面臨的封號風險上，這個工具提供了一個簡單的解決方案。社交媒體上的分享和討論也可能促進了這個工具的流行，尤其是抖音上的演示視頻獲得了超過 20 萬的播放量。這些因素共同驅動了這個專案的快速增長。

## 適合誰使用

**目標受眾**：擔心在使用 Claude 時被標記為中國用戶的網頁開發者和數據隱私專家。

> [!example] 使用場景
> - 網頁開發者用它來檢查用戶是否會被 Claude 標記為中國用戶，因為這可以幫助他們調整網站內容以避免封號風險。
> - 數據隱私專家用它來評估不同用戶環境的風險，因為這樣可以提供更安全的使用建議。
> - 社交媒體管理員用它來檢測用戶的風險評估，因為這可以幫助他們制定更有效的內容策略，避免不必要的封號。

## 架構分析

Fuck Claude 採用單頁應用架構，所有功能均在用戶瀏覽器中運行，這樣的設計可以確保用戶數據的隱私和安全。核心的掃描邏輯是用 TypeScript 實現，並且通過 Astro 框架進行構建，這使得應用在性能和 SEO 上都有良好的表現。資料流方面，從用戶的瀏覽器環境中提取信號，然後計算風險分數，並將結果反饋給用戶。這樣的設計避免了對後端的依賴，降低了延遲，但也意味著無法處理某些需要伺服器支持的功能。整體上，這個架構在隱私和性能之間取得了良好的平衡。

## 技術深入分析

Fuck Claude 的核心技術機制是基於對用戶瀏覽器環境的分析，通過檢測系統時區、瀏覽器語言和已安裝的字體等信號來評估風險。這些信號的權重設計使得最關鍵的指標（如系統時區）能夠對最終結果產生最大的影響。效能方面，因為所有操作都在用戶端進行，所以對伺服器的依賴性非常低，這樣的設計使得應用的響應速度非常快。選擇使用 Astro 框架的原因在於其靜態生成的特性，這樣可以確保 SEO 效果，同時也降低了伺服器的負擔。技術風險方面，這個工具在面對不同用戶環境時，可能會出現評估不準確的情況，特別是在一些特殊的瀏覽器或設備上。整合方面，這個工具可以輕鬆地與現有的前端技術棧結合，並且對於 CI/CD 流程的友好度也相對較高，因為其依賴的技術棧相對輕量。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的使用說明和安裝步驟，並且有範例指令。安裝過程相對順暢，沒有明顯的坑。整體來說，花 30 分鐘內應該能夠順利運行起來。

## 優缺點分析

> [!success] 優點
> - 完全本地化運行，無需數據上傳，保護用戶隱私。
> - 支持多語言界面，適合不同語言的用戶。
> - 提供 API 接口，方便開發者集成。

> [!danger] 缺點
> - 僅支持現代瀏覽器，對於舊版瀏覽器支持不足。
> - API 功能僅限於 Vercel 部署，無法在其他靜態主機上使用。
> - 對某些特殊環境的風險評估可能不準確。

> [!warning] 注意事項
> - 僅支持現代瀏覽器，舊版瀏覽器可能無法正確運行。
> - API 端點僅在 Vercel 部署下可用，其他靜態主機無法使用此功能。
> - 對於某些特殊用戶環境，可能無法準確評估風險。
...
- **[HIGH]** 在某些舊版瀏覽器中無法正確運行，可能導致掃描失敗。
  - 解法：建議使用最新版本的 Chrome 或 Firefox 瀏覽器。
- [MEDIUM] API 端點在非 Vercel 部署下無法使用，限制了功能。
  - 解法：考慮在 Vercel 部署以使用完整功能。
- [MEDIUM] 對於某些特殊用戶環境，風險評估可能不準確。
  - 解法：用戶需自行判斷結果，並對照其他工具。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 中小型開發團隊希望評估用戶風險 | 非常適合 | 因為其簡單易用且無需後端支持，適合快速集成. |
| 大型企業需要高準確度的風險評估工具 | 不適合 | 因為在某些特殊環境下可能無法準確評估風險. |
| 個人開發者想要快速檢測風險 | 適合 | 因為這個工具簡單易用，適合個人使用. |
...
> [!tip] 投入 vs 回報
> 花 2 小時學習，1 小時整合，得到快速的風險評估工具，值得使用。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具在本地運行，不需要高權限，且不存取敏感資料。
...
> | [#14](https://github.com/LinXiaoTao/FuckClaude/issues/14) | 已安装中文字体和Emoji 渲染风格这个怎么隐藏 | 0 | 0 |
