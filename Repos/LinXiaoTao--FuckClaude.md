...
star_history: "2026-07-06:536,2026-07-06:541,2026-07-07:617,2026-07-07:619,2026-07-08:663,2026-07-09:700,2026-07-10:723,2026-07-11:738,2026-07-12:749,2026-07-13:757,2026-07-14:769,2026-07-15:777,2026-07-16:779,2026-07-17:783"
tags:
  - github
  - "category/開發工具"
  - "lang/typescript"
...
**783** stars · **56** stars/天 · 建立 14 天前 · TypeScript · MIT
...
> **適合** 需要檢查其瀏覽器環境是否會被標記為中國用戶的開發者或隱私意識強的用戶。
> **一句話重點** 這個工具的獨特之處在於其完全本地運行的隱私檢查機制，讓用戶能夠安心檢查其環境。
...
> [!abstract] 核心創新
> 這個專案的核心創新在於其完全本地運行的隱私檢查機制。

## 專案簡介

Fuck Claude 是一個輕量級的單頁應用，專門用來檢查用戶的瀏覽器環境是否會被 Claude Code 標記為中國用戶。用戶只需點擊一次，應用會運行動畫掃描，顯示各項信號的風險累積，最後給出判斷結果和命中的信號列表。這一切都在本地完成，無需網絡請求或數據上傳，確保了用戶的隱私。這個工具使用了 Claude Fable 5 技術，並且支持中英文界面，讓不同語言的用戶都能輕鬆使用。其核心機制是通過分析系統時區、瀏覽器語言、已安裝的中文字體等多個信號來計算風險分數，並給出 0-100 的評分。最關鍵的 CLI 指令是 `curl https://fuck-claude.vercel.app/api/check`，用於獲取文本報告。這個工具的賣點在於其完全本地運行的特性，避免了數據洩露的風險。

技術上，它使用了 Astro 框架來構建靜態頁面，並且集成了 Vercel 的 API 功能來處理後端請求。這樣的設計使得應用在性能上非常輕量，且易於部署。與其他類似工具相比，如 `0x0funky/agent-sprite-forge`，Fuck Claude 更加專注於用戶隱私和本地運行，避免了依賴外部服務的風險。使用者在使用過程中可能會遇到的問題包括對於某些信號的誤判，這可能會影響最終的評分。這個專案目前處於活躍開發中，未來可能會增加更多的檢測信號和功能。適合需要檢查其網絡環境的開發者或對隱私有高要求的用戶。整體來說，這是一個值得一試的工具，尤其對於在中國或與中國相關的開發者來說。

**技術棧**：`Astro 7.x` · `@astrojs/vercel` · `pnpm`

## 重點功能

- 本地運行 — 所有檢查在用戶的瀏覽器中完成，無需網絡請求。
- 多語言支持 — 提供中英文界面，適合不同語言的用戶使用。
- 一鍵分享 — 結果可以通過 Web Share API 一鍵分享至社交媒體。
- API 接口 — 提供 `/api/check` 接口，支持終端和腳本調用。
- 信號檢測 — 分析系統時區、瀏覽器語言、已安裝字體等多個信號來評估風險。
...
```ts
{
  "前置條件": "使用 curl 進行 API 請求",
  "指令": "curl https://fuck-claude.vercel.app/api/check",
  "預期輸出": "返回一個文本報告，顯示用戶的風險評分和命中的信號。"
}
```
...
> [!tip] 爆紅原因
> 建立 14 天內累積 783 stars（56/天），forks 78（10.0%），這顯示出相對穩定的增長。作者 LinXiaoTao 是一位活躍的開發者，過去的專案也集中在隱私和網絡安全領域。這個工具解決了用戶在使用 Claude Code 時可能面臨的隱私風險，特別是針對中國用戶的標記問題。該工具的出現正好填補了市場上對於用戶隱私檢查的需求，並且其本地運行的特性使得用戶不必擔心數據洩露。社群的反饋良好，且目前沒有開放的 Issues，顯示出開發者對於用戶反饋的重視。這些因素共同促成了其快速的增長。

## 適合誰使用

**目標受眾**：需要檢查其瀏覽器環境是否會被標記為中國用戶的開發者或隱私意識強的用戶。

> [!example] 使用場景
> - 前端開發者用它來檢查其應用在中國用戶的可用性，因為這樣可以提前發現可能的風險信號，避免未來的合規問題。
> - 網絡安全專家用它來評估其客戶的瀏覽器環境，因為這能幫助他們提供更具針對性的建議，提升客戶的安全性。
> - 隱私意識強的用戶用它來確認自己的瀏覽器環境不會被標記，因為這樣可以更安心地使用網絡服務。

## 架構分析

Fuck Claude 採用單頁應用架構，主要由 Astro 框架構建，這使得應用在性能上非常輕量。所有檢查邏輯都封裝在一個 TypeScript 模組中，並且使用 Vercel 的 API 功能來處理後端請求。這樣的設計使得應用能夠快速響應用戶的請求，同時保持高效的資源使用。由於所有檢查都在客戶端進行，這樣的架構也避免了對外部服務的依賴，降低了數據洩露的風險。整體架構的選擇使得應用在部署上非常靈活，可以在多種靜態主機上運行，僅需 Vercel 的 API 功能來支持部分後端邏輯。

## 技術深入分析

Fuck Claude 的核心技術機制是通過分析用戶的瀏覽器環境來檢測是否會被 Claude Code 標記。這包括檢查系統時區、瀏覽器語言、已安裝字體等多個信號。這些信號的權重是根據其對於判斷中國用戶的影響程度來設置的，例如系統時區佔 26 分，瀏覽器語言佔 20 分。這樣的設計使得應用能夠準確評估用戶的風險分數。

整體應用的性能表現良好，因為所有檢查都是在客戶端進行，避免了對伺服器的依賴。這樣的架構設計使得應用在部署上非常靈活，可以在多種靜態主機上運行。選擇使用 Astro 框架的原因在於其靜態輸出特性，能夠提高加載速度和 SEO 表現。這個選擇的代價是需要依賴 Vercel 的 API 功能來支持部分後端邏輯，這可能會在未來造成一定的擴展性瓶頸。

整體來看，這個工具在技術上具有良好的可擴展性和靈活性，適合需要檢查其環境的用戶使用。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了詳細的使用說明和安裝步驟。安裝過程順暢，沒有明顯的坑。文件中包含中英文版本，方便不同語言的用戶理解。整體來說，花 30 分鐘能夠順利運行起來。
...
> [!danger] 缺點
> - 僅支持現代瀏覽器，舊版瀏覽器可能無法使用。
> - 對於某些信號的檢測可能存在誤判。
> - 未來可能需要定期更新以支持新信號。

> [!warning] 注意事項
> - 僅支持現代瀏覽器，舊版瀏覽器可能無法正確運行。
> - 對於某些信號的檢測可能存在誤判，影響最終評分。
> - 未來可能會增加更多檢測信號，需定期更新。
...
**社群活躍度**：社群活躍，所有 Issues 已解決，顯示出開發者對用戶反饋的重視。
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["隱私保護","網絡安全","瀏覽器指紋"];
