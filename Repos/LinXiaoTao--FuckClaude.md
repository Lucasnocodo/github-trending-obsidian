...
use_case: "檢查你的瀏覽器環境是否會被 Claude 標記為中國用戶。"
priority: medium
ring: assess
...
star_history: "2026-07-06:536,2026-07-06:541,2026-07-07:617,2026-07-07:619,2026-07-08:663,2026-07-09:700"
tags:
  - github
  - "category/開發工具"
  - "lang/typescript"
...
aliases:
  ...
  - "檢查你的瀏覽器環境是否會被 Claude 標記為中國用戶。"
---

# FuckClaude

**700** stars · **117** stars/天 · 建立 6 天前 · TypeScript · MIT
...
> **適合** 需要檢查自己是否會被 Claude 標記為中國用戶的網頁開發者和自由職業者。
> **一句話重點** Fuck Claude 的設計不僅解決了用戶隱私的問題，還提供了即時的風險評估，讓用戶能夠更好地掌握自己的網絡身份。
...
>     .where(p => p.subcategory === "API 工具" && p.file.name !== "LinXiaoTao--FuckClaude" && p.status !== "archived")
...
> **結論** 花 2 小時學習、1 小時整合，得到即時的風險評估，值得嘗試。

> [!abstract] 核心創新
> 這個專案提供了一個完全本地化的解決方案來檢查用戶是否會被標記為中國用戶，並且不需要任何網絡請求。

## 專案簡介

Fuck Claude 是一個輕量級的單頁應用，旨在檢查用戶的瀏覽器環境，判斷其是否會被 Claude Code 標記為中國用戶。用戶只需點擊一次，即可啟動掃描，並通過動畫顯示風險累加，最終給出評分和命中的信號列表。這個工具完全在本地運行，不會發起任何網絡請求或數據上傳，確保用戶隱私。核心機制依賴於瀏覽器的 `Intl` API 來讀取系統時區，這是 Claude 判定用戶地理位置的關鍵指標。除了時區，還檢查瀏覽器語言、安裝的中文字體及其他環境指紋，並將這些信號轉化為 0-100 的風險分數。

使用者可以透過 API 獲取服務端估算，該估算基於請求的 geo headers 和 User-Agent，這使得它在終端和腳本中也能運行。這個工具的賣點在於其簡單易用和完全本地化的設計，適合任何需要檢查自己是否被標記的用戶。這樣的設計避免了數據洩露的風險，並提供了即時的反饋。相較於其他工具，Fuck Claude 的獨特之處在於它不需要網絡連接，並且提供了多語言支持，讓全球用戶都能使用。

**技術棧**：`Astro 7.x` · `@astrojs/vercel`

## 重點功能

- 本地掃描 — 所有檢查在用戶的瀏覽器中完成，不發送數據到伺服器。
- 風險評分 — 根據多個信號（如時區、語言、字體）計算 0-100 的風險分數。
- 多語言支持 — 提供英語和中文界面，方便全球用戶使用。
- 一鍵分享 — 用戶可以輕鬆分享掃描結果及圖片到社交媒體。
- API 接口 — 提供 `/api/check` 端點，支持終端和腳本調用，返回服務端估算結果。
...
```ts
{
  "前置條件": "需要安裝 pnpm 和 Node.js。",
  "指令": "curl https://fuck-claude.vercel.app/api/check",
  "預期輸出": "返回用戶的風險評分和命中的信號列表。"
}
```
...
> [!tip] 爆紅原因
> 建立 6 天內累積 700 stars（117/天），forks 68（9.7%），顯示出強勁的用戶興趣。作者 LinXiaoTao 針對 Claude 的中國用戶檢測需求，提供了一個簡單的解決方案，這在過去並沒有好方案。這個工具的出現正好填補了用戶對隱私保護的需求，尤其是在中國用戶面臨的風險識別問題上。社交媒體上的分享和討論也推動了其曝光率，特別是短視頻平台的推廣。這些因素共同促進了其快速增長。

## 適合誰使用

**目標受眾**：需要檢查自己是否會被 Claude 標記為中國用戶的網頁開發者和自由職業者。

> [!example] 使用場景
> - 網頁開發者用它來檢查自己的網站是否會被 Claude 標記，因為這樣可以避免不必要的封號風險。
> - 自由職業者用它來確保在使用 Claude 進行項目時不會被誤判為中國用戶，因為這樣可以保護其帳號安全。
> - 數據分析師用它來測試不同環境下的風險評估，因為這樣能幫助他們更好地理解用戶行為。

## 架構分析

Fuck Claude 的架構基於單頁應用模式，使用 Astro 框架來生成靜態頁面，並利用 Vercel 的 serverless 功能來處理 API 請求。這樣的設計使得應用的每個頁面都能快速加載，並且 API 端點僅在需要時運行，減少了資源消耗。資料流方面，掃描過程中用戶的瀏覽器環境信號會被收集並評分，最終結果在本地生成，這樣避免了數據洩露的風險。選擇 Astro 使得這個專案能夠輕鬆地支持多語言和 SEO，這對於希望吸引全球用戶的應用來說是至關重要的。這種架構的代價在於，API 功能依賴於 Vercel 的部署，若轉移至其他平台，可能需要重新設計 API 部分。

## 技術深入分析

Fuck Claude 利用現代瀏覽器的 API 來檢查用戶的環境，這包括 `Intl.DateTimeFormat` 和 `navigator.language` 等功能，這些都是用來判斷用戶是否會被標記為中國用戶的關鍵信號。這個工具的效能主要依賴於用戶的設備性能和網頁加載速度，因為所有檢查都是在本地進行的。設計上選擇了 Astro 框架，這使得應用能夠快速生成靜態頁面，並且支持 SEO，這對於希望吸引全球用戶的應用來說是至關重要的。選擇 Vercel 作為部署平台，雖然提供了 serverless 功能，但也帶來了對平台的依賴。技術風險方面，若未來 Claude 的檢測機制發生變化，這個工具可能需要隨之調整，否則將無法提供準確的評估。整合方面，這個工具可以輕鬆地與現有的開發流程結合，特別是在需要進行用戶身份驗證或隱私保護的場景中。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且提供了詳細的安裝和使用說明，包含了實際的指令範例。安裝過程相對順暢，沒有明顯的坑。文件目前僅有英語和中文版本，對於多語言用戶來說是友好的。

## 優缺點分析

> [!success] 優點
> - 完全本地化運行，無需網絡請求，保護用戶隱私。
> - 簡單易用的界面，適合各類用戶。
> - 支持多語言，擴大了使用範圍。

> [!danger] 缺點
> - 僅限於現代瀏覽器，舊版瀏覽器不兼容。
> - API 端點依賴於 Vercel，轉移到其他平台可能會遇到問題。
> - 移動端支持有限，可能影響用戶體驗。

> [!warning] 注意事項
> - 僅支持現代瀏覽器，舊版瀏覽器可能無法正確運行。
> - 不支持移動端的某些功能，可能影響用戶體驗。
> - API 端點僅在 Vercel 部署上可用，其他靜態主機無法使用該功能。
...
- **[HIGH]** 在某些舊版瀏覽器上可能無法正確運行
  - 解法：建議使用最新版本的瀏覽器
- [MEDIUM] API 端點在非 Vercel 環境下無法使用
  - 解法：考慮使用 Vercel 部署以獲取完整功能
- [MEDIUM] 移動端的某些功能可能不完整
  - 解法：在桌面環境中使用以獲得最佳體驗
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 需要檢查是否會被標記的開發者 | 非常適合 | 因為這個工具提供了即時的風險評估，能幫助開發者避免封號。 |
| 需要在移動端使用的用戶 | 普通 | 雖然支持移動端，但某些功能可能不完整。 |
| 希望在 CI/CD 流程中集成的團隊 | 不適合 | 目前的 API 端點依賴於 Vercel，無法在所有環境中使用。  |
...
> [!tip] 投入 vs 回報
> 花 2 小時學習、1 小時整合，得到即時的風險評估，值得嘗試。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限運行，且不會存取敏感資料，所有檢查在本地完成，安全性較高。

## 生態系整合

> [!abstract] 如何融入你的工具鏈

Fuck Claude 可以與現有的開發環境輕鬆整合，特別是在需要用戶身份驗證的場景中。它可以與 Vercel 部署的應用結合，通過 API 端點提供額外的風險評估功能。在使用 GitHub Actions 進行 CI/CD 時，可以將此工具作為檢查步驟的一部分，確保在部署前用戶不會被標記。整合的摩擦點主要在於 API 端點的依賴，若轉移到其他平台，可能需要重新設計。
...
**連結**：[文件](https://github.com/LinXiaoTao/FuckClaude)
...
> FROM "Repos"
> WHERE subcategory = "API 工具" AND file.name != "LinXiaoTao--FuckClaude"
> SORT stars DESC
> ```
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["隱私保護","API 設計","自動化測試"];
