...
star_history: "2026-07-06:536,2026-07-06:541,2026-07-07:617,2026-07-07:619,2026-07-08:663"
tags:
  - github
  - "category/開發工具"
  - "lang/typescript"
...
**663** stars · **133** stars/天 · 建立 5 天前 · TypeScript · MIT
...
> **適合** 需要檢查其瀏覽器環境以避免被 Claude 標記的開發者和用戶。
> **一句話重點** 這個工具的設計充分考慮了用戶隱私，並提供了一個簡單的方式來檢查是否會被標記為中國用戶。
...
> [!abstract] 核心創新
> 這個專案提供了一個完全本地化的檢測工具，無需上傳數據即可判斷用戶是否會被標記為中國用戶。

## 專案簡介

Fuck Claude 是一個輕量級的單頁應用，旨在檢查用戶的瀏覽器環境是否會被 Claude Code 標記為中國用戶。用戶只需點擊一次，應用便會運行一系列的信號檢查，並根據檢測結果給出風險評分。所有檢測均在本地進行，無需網絡請求，確保了用戶隱私。這個工具的賣點在於其簡單易用和完全本地化的運行方式，讓用戶能夠快速獲得結果。它使用了多種信號，如系統時區、瀏覽器語言和已安裝的中文字體來進行評分，這些信號的權重總和為100。具體的檢測信號包括系統時區（26分）、瀏覽器語言（20分）等，這些信號的分數會根據其權重進行加總，最終得出一個0-100的風險評分。用戶還可以通過 API 獲取服務器端的估算結果，這對於終端和腳本使用者來說非常方便。

**技術棧**：`Astro 7.x` · `@astrojs/vercel`

## 重點功能

- 本地檢測 — 所有檢測在用戶瀏覽器中完成，無需上傳數據。
- 多語言支持 — 提供中英文界面，方便不同語言用戶使用。
- 即時分享 — 檢測結果可通過 Web Share API 一鍵分享至社交媒體。
- API 接口 — 提供 `/api/check` 端點，允許終端和腳本使用。
- 信號檢測 — 根據多個信號（如時區、語言、字體）進行風險評分。
...
> [!tip] 爆紅原因
> 建立 5 天內累積 663 stars（133/天），forks 66（10.0%），顯示出相對活躍的開發和使用社群。LinXiaoTao 是這個專案的主要貢獻者，過去可能有相關的開發經驗。這個工具解決了用戶在使用 Claude 時可能被誤判為中國用戶的問題，特別是在當前對中國用戶的限制日益嚴格的背景下。社交媒體上對這個工具的討論也可能促進了它的快速增長。這個工具的存在填補了市場上對於檢查用戶環境的需求，特別是在對於隱私和安全的考量日益重要的今天。

## 適合誰使用

**目標受眾**：需要檢查其瀏覽器環境以避免被 Claude 標記的開發者和用戶。

> [!example] 使用場景
> - 網頁開發者用它來檢查用戶的瀏覽器環境是否會被 Claude 標記，因為這能幫助他們優化用戶體驗，避免不必要的風險。
> - 數據科學家用它來分析不同地區用戶的行為，因為他們需要確保數據的準確性和可靠性，特別是在涉及中國用戶時。
> - 市場營銷專員用它來測試廣告投放效果，因為他們希望確保廣告不會被誤判為來自中國的流量，影響轉化率。

## 架構分析

這個專案採用單頁應用架構，使用 Astro 框架來構建靜態頁面，並通過 Vercel 部署。所有檢測邏輯都在客戶端運行，這樣的設計確保了用戶的隱私，因為不需要發送任何數據到伺服器。應用的主要功能是根據用戶的瀏覽器環境進行風險評估，這是通過檢測多個信號來實現的。選擇 Astro 框架的原因是其靜態生成的特性，這樣可以提高性能並減少伺服器負擔。這種架構在擴展性上有一定的限制，特別是在需要處理大量用戶請求時，可能會面臨性能瓶頸。

## 技術深入分析

Fuck Claude 的核心技術機制是基於用戶的瀏覽器環境進行風險評估，使用了多個信號來判斷用戶是否會被標記為中國用戶。這些信號包括系統時區、瀏覽器語言和已安裝的字體等。這些信號的權重總和為100，並且每個信號的得分會根據其權重進行加總，最終得出一個0-100的風險評分。這種設計使得應用能夠在不發送任何數據的情況下，提供準確的評估結果。

效能方面，由於所有檢測都在客戶端進行，因此對伺服器的負擔非常小，並且能夠快速響應用戶請求。選擇 Astro 框架的原因在於其靜態生成的特性，這樣可以提高性能並減少伺服器負擔。這種架構在擴展性上有一定的限制，特別是在需要處理大量用戶請求時，可能會面臨性能瓶頸。整體來說，這個工具在設計上充分考慮了用戶的隱私和安全性，並提供了一個簡單易用的解決方案。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了詳細的使用說明和範例。安裝過程順暢，沒有明顯的坑。文件目前僅提供英文和中文版本，對於雙語用戶友好。
...
> - 無法檢測非瀏覽器環境的用戶，如使用 CLI 的用戶。
> - 對於某些信號的檢測可能會受到用戶隱私設置的影響。
