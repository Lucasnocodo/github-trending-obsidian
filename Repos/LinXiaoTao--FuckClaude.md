...
**536** stars · **179** stars/天 · 建立 3 天前 · TypeScript · MIT
...
> **適合** 希望檢查自己瀏覽器環境是否會被 Claude 標記的普通用戶和開發者。
> **一句話重點** 這個專案的創新在於提供了一個無需上傳數據的隱私檢測工具，讓用戶能夠自我評估風險。
...
> [!abstract] 核心創新
> 這個專案提供了一個完全本地化的檢測工具，無需上傳數據即可評估用戶的風險。

## 專案簡介

Fuck Claude 是一個輕量級的單頁應用，旨在檢測用戶的瀏覽器環境，並告訴他們是否會被 Claude Code 標記為中國用戶。用戶只需點擊一次，應用會運行一系列的檢測信號，並根據結果給出風險評分及詳細信號列表。所有檢測均在本地完成，不會發起任何網絡請求，這樣能保護用戶隱私。這個工具使用了 Astro 框架，並且支持中英文界面，適合全球用戶。

關鍵指令包括 `curl https://fuck-claude.vercel.app/api/check`，這樣可以獲得基於 IP 的估算結果。其設計理念是基於 Claude Code 的檢測機制，通過分析用戶的時區、語言和字體等信號來判斷是否為中國用戶。這樣的設計使得用戶能夠快速了解自己的風險狀況，並做出相應的應對。這個工具的賣點在於其完全本地化的運行方式，讓用戶無需擔心數據外洩。

**技術棧**：`Astro 7.x` · `@astrojs/vercel`

## 重點功能

- 本地檢測 — 所有檢測在用戶瀏覽器中執行，無需上傳數據。
- 中英文支持 — 提供雙語界面，適合全球用戶。
- 即時風險評估 — 一鍵運行檢測，快速獲得風險評分。
- API 接口 — 提供 `/api/check` 接口，方便終端和腳本使用。
- 分享功能 — 檢測結果可通過社交媒體分享，並生成結果圖片。
...
> [!tip] 爆紅原因
> 建立 3 天內累積 536 stars（179/天），forks 50（9.3%），顯示出良好的增長潛力。作者 LinXiaoTao 針對 Claude Code 的中國用戶識別問題提出了這個工具，解決了用戶在使用 Claude 時可能面臨的風險判定過高的問題。這個工具的出現正好填補了用戶對於隱私保護的需求，並且在社交媒體上引起了討論，特別是針對中國用戶的風險評估。其技術架構的選擇使得這個工具在性能和易用性上都有不錯的表現。

## 適合誰使用

**目標受眾**：希望檢查自己瀏覽器環境是否會被 Claude 標記的普通用戶和開發者。

> [!example] 使用場景
> - 網頁開發者用它來檢測用戶是否會被 Claude 標記為中國用戶，因為這樣可以幫助他們優化用戶體驗，避免潛在的風險。
> - 數據隱私專家用它來評估不同用戶的風險，因為這能幫助他們提供更好的建議，降低用戶的風險。
> - 普通用戶用它來了解自己的瀏覽器環境是否安全，因為這樣可以幫助他們在使用 Claude 服務時保護自己的隱私。

## 架構分析

Fuck Claude 採用單頁應用架構，利用 Astro 框架進行靜態頁面生成，所有檢測邏輯封裝在 TypeScript 模組中。這樣的設計使得應用能夠在客戶端快速運行，並且減少了伺服器負擔。檢測信號的計算完全依賴於用戶的瀏覽器環境，這樣能夠確保用戶的隱私不被侵犯。選擇 Astro 的原因是其靜態生成的特性，這樣可以提高加載速度和 SEO 表現。整體架構的輕量化設計使得應用能夠在各種環境中快速部署，並且易於維護。

## 技術深入分析

Fuck Claude 的核心技術在於其檢測信號的設計，通過分析用戶的時區、語言和字體等信息來判斷是否為中國用戶。這些信號的權重設計使得最重要的信號（如系統時區）能夠對最終評分產生最大的影響。該工具的性能表現良好，因為所有檢測都在客戶端進行，避免了伺服器的負擔。這種設計選擇使得用戶的隱私得到保障，並且提高了用戶體驗。使用 Astro 框架的好處在於其靜態生成的特性，這樣能夠提高加載速度並且對 SEO 友好。整體架構的輕量化設計使得應用能夠快速部署，並且易於維護。未來可能需要關注的風險包括對於新興瀏覽器的兼容性問題，以及如何在不影響性能的情況下增加更多檢測信號。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，包含了安裝步驟和使用範例。安裝過程順暢，無明顯坑。文件沒有多語言支持，但提供了中英文版本。整體上，花 30 分鐘能夠順利運行起來。
...
> [!danger] 缺點
> - 僅支持現代瀏覽器，兼容性有限。
> - API 接口僅在特定平台可用，限制了使用場景。
> - 對於非瀏覽器應用無法提供支持。

> [!warning] 注意事項
> - 僅支持現代瀏覽器，舊版瀏覽器可能無法正確檢測。
> - 依賴於用戶的瀏覽器環境，無法檢測非瀏覽器的應用。
> - API 接口僅在 Vercel 部署上可用，其他靜態主機無法使用。
...
相關專案：[[BigBodyCobain--Shadowbroker|BigBodyCobain/Shadowbroker]] · [[BohemiaInteractive--CWR|BohemiaInteractive/CWR]] · [[BuilderPulse--BuilderPulse|BuilderPulse/BuilderPulse]] · [[Yu9191--wloc|Yu9191/wloc]]
