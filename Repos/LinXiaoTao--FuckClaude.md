...
star_history: "2026-07-06:536,2026-07-06:541,2026-07-07:617,2026-07-07:619,2026-07-08:663,2026-07-09:700,2026-07-10:723,2026-07-11:738,2026-07-12:749,2026-07-13:757,2026-07-14:769,2026-07-15:777,2026-07-16:779"
tags:
  - github
  - "category/開發工具"
  - "lang/typescript"
...
**779** stars · **60** stars/天 · 建立 13 天前 · TypeScript · MIT
...
> **適合** 希望檢測自己瀏覽器環境是否會被 AI 標記的隱私意識強的用戶。
> **一句話重點** 這個專案不僅提供了風險評估，還強調了用戶隱私的重要性。
...
> [!abstract] 核心創新
> 這個專案的核心創新在於完全本地運行的風險評估，無需上傳任何數據。

## 專案簡介

Fuck Claude 是一個輕量級的單頁應用，能夠掃描用戶的瀏覽器環境，並告訴用戶是否會被 Claude Code 標記為中國用戶。用戶只需點擊一次，應用便會運行動畫掃描，隨著風險的增加，顯示一個評分。所有檢測都在本地進行，無需網絡請求或數據上傳，這是它的主要賣點。該工具使用 Astro 框架構建，並利用 Vercel 部署，確保靜態頁面快速加載。它支持中英雙語，並能根據用戶的 Accept-Language 標頭自動返回相應語言的結果。

這個工具的技術實現依賴於多個信號的評分，包括系統時區、瀏覽器語言和已安裝的中文字體等，這些信號的權重總和為100。與其他類似工具相比，Fuck Claude 的優勢在於它的本地運行和無需網絡請求的設計，這使得用戶的隱私得到更好的保護。實際使用中，這個工具能夠快速提供結果，並且支持一鍵分享功能，方便用戶將結果分享至社交媒體。這個專案目前處於穩定階段，沒有開放的問題，顯示出良好的社群活躍度。
...
- 中英雙語支持 — 自動根據用戶語言返回結果。
- 一鍵分享功能 — 可將掃描結果和圖片通過社交媒體分享。
- API 接口 — 提供 `/api/check` 端點，支持終端和腳本使用。
- 信號評估 — 根據多個信號（如時區、語言、字體）進行風險評估。
...
> [!tip] 爆紅原因
> 建立 13 天內累積 779 stars（60/天），forks 77（9.9%），這顯示出不錯的增長潛力。作者 LinXiaoTao 之前在開源社區有過其他貢獻，這使得該專案在社群中獲得了一定的信任。這個工具解決了用戶在使用 Claude Code 時可能面臨的隱私問題，特別是中國用戶的標記問題，這在當前的技術環境中是個重要的痛點。社群的反饋和需求促使這個工具的快速發展，並且其簡單易用的特性也吸引了不少使用者。

## 適合誰使用

**目標受眾**：希望檢測自己瀏覽器環境是否會被 AI 標記的隱私意識強的用戶。

> [!example] 使用場景
> - 前端工程師用它來檢測自己的瀏覽器環境是否會被 Claude 標記，因為這能幫助他們在開發過程中避免潛在的風險。
> - 數據隱私專家用它來測試不同環境下的風險評估，因為這能讓他們更好地理解用戶在使用 AI 工具時的隱私問題。
> - 社交媒體用戶用它來分享自己的掃描結果，因為一鍵分享功能讓這個過程變得簡單且方便。

## 架構分析

Fuck Claude 採用單頁應用架構，使用 Astro 框架來構建靜態頁面，並在 Vercel 部署。所有的掃描和評分邏輯都在用戶的瀏覽器中運行，這樣設計的目的是為了保護用戶隱私，避免數據上傳的風險。資料流方面，應用通過 JavaScript 讀取用戶的瀏覽器環境，並根據定義的信號進行評分。

這樣的設計使得應用能夠快速響應用戶的操作，並提供即時的結果。選擇 Astro 作為框架是因為它能夠有效地生成靜態頁面，並且支持現代的前端開發流程。這樣的架構在擴展性上有一定的限制，因為所有的邏輯都依賴於用戶的瀏覽器環境，無法進行伺服器端的處理。

## 技術深入分析

Fuck Claude 的核心技術機制在於它能夠通過用戶的瀏覽器環境來評估是否會被 Claude Code 標記。它使用了多個信號來進行評分，包括系統時區、瀏覽器語言和已安裝的中文字體等。這些信號的權重總和為100，確保了評分的準確性。效能方面，因為所有的檢測都在本地進行，所以不會受到網絡延遲的影響，能夠快速返回結果。

設計上選擇了 Astro 框架，這使得應用能夠生成靜態頁面，並且支持現代的前端開發流程。這樣的選擇帶來了快速的加載時間，但在擴展性上可能會有所限制。技術風險方面，這個工具依賴於用戶的瀏覽器環境，若用戶使用不支持的瀏覽器，則無法正常運行。整合方面，這個工具可以輕鬆地與現有的前端應用進行集成，並且支持多種社交媒體的分享。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且有範例，安裝過程順暢，沒有明顯的坑。提供了良好的入門指南，對於新手來說，花 30 分鐘能夠順利運行。文件目前僅提供英文和中文版本。
...
> [!danger] 缺點
> - 僅支持現代瀏覽器，對於舊版瀏覽器的支持不足。
> - API 僅限於 Vercel 部署，其他靜態主機可能不支持。
> - 功能相對單一，主要集中於風險評估。

> [!warning] 注意事項
> - 僅支持現代瀏覽器，舊版瀏覽器可能無法正常運行。
> - 不支持在非網頁環境下使用，僅限於瀏覽器。
> - API 接口僅適用於 Vercel 部署，其他靜態主機可能不支持。
