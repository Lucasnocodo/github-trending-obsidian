...
star_history: "2026-03-10:950,2026-03-11:953,2026-03-13:1077,2026-03-13:1078,2026-03-14:1125,2026-03-14:1125,2026-03-15:1165,2026-03-15:1165,2026-03-16:1208,2026-03-17:1285,2026-03-18:1340,2026-03-19:1394,2026-03-20:1439,2026-03-21:1469,2026-03-22:1503,2026-03-23:1523,2026-03-24:1561,2026-03-25:1607,2026-03-26:1640,2026-03-27:1670,2026-03-28:1704,2026-03-29:1731,2026-03-30:1747,2026-03-31:1773"
tags:
  - github
  - "category/ai_ml"
  - "lang/python"
...
**1.8k** stars · **81** stars/天 · 建立 22 天前 · Python · 未標註授權
...
> **一句話重點** 這個專案不僅提供技能，還能讓 AI 助手具備醫療專業知識，顯著提升其在醫學研究中的應用潛力。
...
> [!abstract] 核心創新
> 這個專案整合了來自 12+ 開源技能庫的醫療 AI 技能，提供了全面的醫學研究能力。

## 專案簡介

OpenClaw Medical Skills 是一個包含 869 個醫療和生物醫學研究技能的開源庫，旨在提升 OpenClaw 和 NanoClaw AI 助手的專業能力。使用者可以透過簡單的命令將這些技能安裝到自己的工作空間或全局環境中，並且不需要重啟系統。這些技能涵蓋臨床報告、基因組學、藥物發現等多個領域，並能連接到實際的數據庫和 API，提供結構化的醫學或科學相關輸出。這樣的設計使得 AI 助手能夠提供比一般 AI 更具專業性的回應，像是查詢 PubMed 或 ClinicalTrials.gov 的資料。技術上，這個庫使用 Python 和多種工具（如 Docker 和 Git LFS）來管理和安裝技能，並且支持多種資料格式和工作流。

與其他醫療 AI 工具相比，OpenClaw Medical Skills 提供了更全面的技能集合，能夠應對更複雜的醫學查詢和數據分析需求。這使得它特別適合需要進行深入醫學研究的團隊或個人。使用者在安裝時需注意某些技能可能包含大型數據文件，這可能會影響安裝速度和存儲需求。整體來說，這個專案對於希望提升 AI 助手醫學能力的開發者來說，是一個值得考慮的選擇。

**技術棧**：`Python` · `Jupyter Notebook` · `Docker`

## 重點功能

- 869 個專業技能 — 覆蓋臨床、基因組學、藥物發現等多個醫療領域。
- 即時數據查詢 — 連接 PubMed 和 ClinicalTrials.gov，提供真實的醫學資訊。
- 靈活的安裝方式 — 支援全局安裝或工作區域安裝，並可選擇性安裝特定技能。
- 支援大型數據集 — 可透過 Git LFS 管理大型數據文件，避免下載不必要的資料。
- 自動技能加載 — 安裝後無需重啟，技能會在下次會話中自動加載。
...
> [!tip] 爆紅原因
> 建立 22 天內累積 1773 stars（81/天），forks 230（13.0%），顯示出強烈的社群興趣。這個專案的主要貢獻者來自醫療和 AI 領域，解決了傳統醫療 AI 工具無法提供專業技能的痛點。之前的解決方案往往缺乏針對性和深度，而這個庫整合了多個開源資源，讓 AI 助手具備更強的專業能力。社群的活躍度也顯示出使用者對於技能提交和改進的積極參與，這對於未來的發展是個好兆頭。
...
> [!example] 使用場景
> - 醫療研究人員用它來快速查詢臨床試驗資料，因為能夠直接連接到 ClinicalTrials.gov，節省了大量手動搜尋的時間。
> - 生物資訊學家用它來進行基因組數據分析，因為提供了多種基因組學技能，能夠快速執行 RNA-seq 和變異分析。
> - 藥物研發團隊用它來獲取藥物相互作用資訊，因為整合了 ChEMBL 和 DrugBank 的數據，能夠提供即時的藥物安全性評估。

## 架構分析

OpenClaw Medical Skills 的架構設計以模組化為核心，將各種醫療技能作為獨立的模組進行管理。每個技能都包含一個 SKILL.md 文件，定義了該技能的功能和使用方式。這樣的設計使得使用者可以靈活地選擇和安裝所需的技能，而不必下載整個庫。

資料流方面，技能會自動從指定的路徑加載，並且可以連接到外部數據庫和 API，提供即時的查詢和分析能力。選擇這種模組化架構的好處是能夠快速擴展和更新技能，而代價則是需要使用者具備一定的技術能力來進行安裝和配置。整體來說，這種設計能夠有效地支持多樣化的醫療研究需求。

## 技術深入分析

OpenClaw Medical Skills 的核心技術機制在於其模組化設計，允許使用者根據需求選擇和安裝特定的醫療技能。每個技能都能連接到實際的數據庫和 API，這使得 AI 助手能夠提供即時且具體的醫學資訊。這種設計不僅提升了使用者的查詢效率，也使得 AI 助手的應用範圍大幅擴展。從效能上看，這個庫能夠處理多種醫學查詢，並且能夠支持大規模的數據分析。

選擇 Python 作為主要開發語言，使得這個專案能夠利用大量現有的科學計算和數據處理庫，降低了開發成本。這種選擇雖然增加了對 Python 環境的依賴，但也使得整體架構更加靈活。技術風險方面，未來在擴展技能庫時，可能會面臨資料一致性和更新的挑戰。整合方面，這個專案與其他主流 AI 框架的兼容性良好，能夠輕鬆融入現有的開發流程。

整體來說，OpenClaw Medical Skills 在醫療 AI 領域展現出強大的潛力，未來的發展值得期待。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了詳細的安裝步驟和技能概述，對於新手來說相對友好。安裝過程中需要注意 Git LFS 的使用，這可能會對某些使用者造成困難。整體來說，文件清晰且有助於快速上手，適合希望在醫療領域進行 AI 應用的開發者。

## 優缺點分析

> [!success] 優點
> - 提供全面的醫療 AI 技能，能夠滿足多種研究需求。
> - 靈活的安裝選項，使用者可以根據需要選擇安裝特定技能。
> - 與實際數據庫連接，提供即時的醫學資訊查詢。

> [!danger] 缺點
> - 安裝過程對新手來說可能有一定的難度。
> - 某些技能可能需要大量存儲空間。
> - 依賴於 OpenClaw 或 NanoClaw 的運行環境，限制了使用範圍。

> [!warning] 注意事項
> - 某些技能可能包含大型數據文件，影響安裝時間。
> - 需要 Git LFS 來管理大型數據集。
> - 依賴於 OpenClaw 或 NanoClaw 的運行環境。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [FreedomIntelligence/OpenClaw](https://github.com/FreedomIntelligence/OpenClaw) | OpenClaw 是一個更通用的 AI 助手框架，而 OpenClaw Medical Skills 專注於醫療和生物醫學領域的技能。 |
| [NVIDIA/NemoClaw](https://github.com/NVIDIA/NemoClaw) | NemoClaw 提供了針對 AI 模型的訓練和推論，而 OpenClaw Medical Skills 提供了具體的醫療技能和數據查詢能力。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [NVIDIA/NemoClaw](https://github.com/NVIDIA/NemoClaw) | NemoClaw 專注於 AI 模型的訓練和推論，而 OpenClaw Medical Skills 提供具體的醫療技能和數據查詢能力。 | 如果你的重點在於 AI 模型的訓練和推論，而非具體的醫療應用，則可以選擇 NemoClaw。 | medium，因為需要重新設計應用架構以適應不同的工具。 |
| [GAIR-NLP/daVinci-MagiHuman](https://github.com/GAIR-NLP/daVinci-MagiHuman) | MagiHuman 提供了針對自然語言處理的醫療應用，而 OpenClaw Medical Skills 提供更全面的醫療技能。 | 如果你的專案主要集中在自然語言處理方面，則 MagiHuman 可能更合適。 | low，因為兩者在某些功能上有相似性，轉換相對容易。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **OpenClaw-Medical-Skills** | **NemoClaw** | **daVinci-MagiHuman** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | NemoClaw 專注於 AI 模型的訓練和推論，而 OpenClaw Medical Skills 提供具體的醫療技能和數據查詢能力。 | MagiHuman 提供了針對自然語言處理的醫療應用，而 OpenClaw Medical Skills 提供更全面的醫療技能。 |
> | 遷移成本 | - | medium，因為需要重新設計應用架構以適應不同的工具。 | low，因為兩者在某些功能上有相似性，轉換相對容易。 |
> | 適用場景 | 主要場景 | 如果你的重點在於 AI 模型的訓練和推論，而非具體的醫療應用 | 如果你的專案主要集中在自然語言處理方面，則 MagiHuma |
...
- **[HIGH]** 安裝過程中可能會遇到 Git LFS 的配置問題
  - 解法：確保在安裝前正確安裝 Git LFS
- [MEDIUM] 某些技能可能需要較大的存儲空間，影響安裝速度
  - 解法：選擇性安裝所需技能以減少存儲需求
- [MEDIUM] 在某些環境下可能無法正確加載技能
  - 解法：檢查 OpenClaw 的配置文件，確保路徑正確

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 醫療研究機構的 AI 助手 | 非常適合 | 提供全面的醫療技能，能夠滿足多樣化的研究需求。 |
| 小型醫療團隊的日常查詢工具 | 適合 | 能夠快速提供醫學資訊，但可能需要額外的配置。 |
| 個人開發者的醫療應用原型 | 普通 | 雖然功能強大，但安裝和配置過程可能對新手造成挑戰。 |
| 大型醫療系統的核心系統 | 不適合 | 目前仍處於 beta 階段，穩定性不足。 |
...
> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限，並且不會存取敏感資料。依賴的外部 API 和數據庫需自行評估安全性。
...
> | --- | --- | --- | --- |
> | [#18](https://github.com/FreedomIntelligence/OpenClaw-Medical-Skills/issues/18) | [Skill Submission] MediWise Health Suite - Family Health Man | 0 | 0 |
...
相關概念：[[機器學習]] · [[生物資訊學]] · [[自然語言處理]]

相關專案：[[NVIDIA--NemoClaw|NVIDIA/NemoClaw]] · [[AlpinDale--parsync|AlpinDale/parsync]] · [[GAIR-NLP--daVinci-MagiHuman|GAIR-NLP/daVinci-MagiHuman]] · [[MiniMax-AI--skills|MiniMax-AI/skills]] · [[BigBodyCobain--Shadowbroker|BigBodyCobain/Shadowbroker]] · [[Lightricks--LTX-Desktop|Lightricks/LTX-Desktop]] · [[RunanywhereAI--RCLI|RunanywhereAI/RCLI]] · [[binance--binance-skills-hub|binance/binance-skills-hub]]
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["機器學習","生物資訊學","自然語言處理"];
