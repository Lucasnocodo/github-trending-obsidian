...
use_case: "提供多種自動化交易策略，幫助用戶在 Polymarket 上進行交易。"
priority: medium
ring: assess
...
> **適合** 希望在 Polymarket 上進行自動化交易的活躍交易者和風險管理專家。
> **一句話重點** PolyEdge 提供了一個完整的自動化交易解決方案，專為 Polymarket 的市場機會設計，並強調風險控制和透明度。
...
> **結論** 花 5 小時學習，3 小時整合，得到高效的自動化交易工具，值得投資。

> [!abstract] 核心創新
> 提供四種自動化交易策略，專為 Polymarket 設計，並支持模擬交易。

## 專案簡介

PolyEdge 是一個針對 Polymarket 的自動化交易平台，提供四種主要策略：複製交易、套利、對沖和市場做市。用戶可以透過 API 路由進行身份驗證、配置機器人、啟動和停止交易，並查看即時狀態和績效。這些策略旨在利用 Polymarket 的市場低效性，讓用戶在不冒險的情況下進行交易。每個策略都在模擬模式下運行，確保用戶在投入資本前能驗證其有效性。技術上，PolyEdge 使用 Next.js 和 PostgreSQL，並依賴於多個 Web3 和交易 API 來實現其功能。

這個平台的設計考慮到用戶的風險控制，並提供詳細的交易透明度。與其他交易工具相比，PolyEdge 的複製交易功能允許用戶即時跟隨成功的交易者，而套利策略則能在市場價格不一致時獲利。使用者可以在模擬模式下測試策略，然後再轉換到實際交易，這樣可以降低風險。整體而言，PolyEdge 提供了一個完整的工具集，適合希望在 Polymarket 上進行系統化交易的用戶。

**技術棧**：`Next.js 14` · `PostgreSQL` · `TypeScript`

## 重點功能

- 複製交易 — 即時跟隨成功交易者的交易，並根據用戶設定的大小進行複製。
- 套利機器人 — 在二元市場中捕捉價格不一致的機會，實現無風險套利。
- 對沖策略 — 通過採取相反的頭寸來鎖定未實現的利潤或限制損失。
- 市場做市 — 在雙方報價中被動報價以捕捉價差和流動性獎勵。
- 模擬模式 — 在投入資本前，先在模擬環境中驗證交易策略的有效性。
...
```bash
{
  "前置條件": "需要 Node.js 20+ 和 PostgreSQL 14+。",
  "指令": "npm install",
  "預期輸出": "安裝所有必要的依賴包。"
}
```
...
> [!tip] 爆紅原因
> 建立 3 天內累積 378 stars（126/天），forks 710（187.8%），這顯示出極高的用戶興趣。作者 MstKail 針對 Polymarket 的交易需求，提供了一個多功能的自動化交易解決方案，填補了市場上對於高效交易工具的需求。這個專案的推出正值預測市場逐漸受歡迎的時期，讓用戶能夠更有效地利用市場機會。高達 187.8% 的 forks/stars 比率顯示出用戶對於修改和擴展此工具的強烈興趣，這可能意味著該專案的實用性和靈活性受到廣泛認可。

## 適合誰使用

**目標受眾**：希望在 Polymarket 上進行自動化交易的活躍交易者和風險管理專家。

> [!example] 使用場景
> - 交易者用它來自動跟隨成功的 Polymarket 交易者，因為這樣可以在不需手動操作的情況下快速複製他們的交易。
> - 風險管理專家用它來對沖現有的市場風險，因為這樣可以在市場波動時保護資本。
> - 套利交易者用它來捕捉市場價格不一致的機會，因為這樣可以在低風險的情況下獲利。

## 架構分析

PolyEdge 的架構分為兩個主要部分：Web 進程和機器人工作進程。Web 進程負責用戶界面和 API 路由，並與 PostgreSQL 數據庫進行交互。機器人工作進程專注於執行交易策略，並通過 API 與 Polymarket 的 CLOB 進行通信。

這種分離設計確保了兩者不會重複執行訂單，降低了錯誤風險。整體架構的設計旨在提供高效的交易執行和用戶友好的界面，並且在交易過程中保持透明度和可追蹤性。此架構的代價在於需要額外的配置和管理，但能夠提供更高的穩定性和可擴展性。

## 技術深入分析

PolyEdge 的核心技術機制基於多種交易策略，利用 Polymarket 的市場低效性進行套利和對沖。它使用 Next.js 作為框架，並搭配 PostgreSQL 作為數據存儲，確保高效的數據處理和用戶界面。這個平台能夠處理大量的即時交易數據，並在模擬模式下運行，確保用戶在投入資本前能夠驗證其策略的有效性。設計上，PolyEdge 強調用戶友好性和透明度，並提供詳細的交易報告和績效追蹤。選擇 Next.js 和 PostgreSQL 的好處在於它們的成熟度和社群支持，然而，這也意味著需要一定的學習曲線來掌握這些技術。隨著市場需求的增長，PolyEdge 可能會面臨擴展性挑戰，特別是在處理更高交易量時。整合到現有的交易流程中相對簡單，但用戶需要了解如何配置 API 和管理交易策略。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且包含必要的安裝指令，安裝過程相對順暢。提供了良好的入門指南，但缺乏多語言支持。用戶在 30 分鐘內應能順利運行起來。

## 優缺點分析

> [!success] 優點
> - 提供多種交易策略，滿足不同用戶需求。
> - 模擬模式降低了風險，讓用戶能在無資本風險下測試策略。
> - 透明的交易過程，讓用戶能清楚了解每筆交易的狀態。

> [!danger] 缺點
> - 需要一定的技術背景來配置和運行。
> - 僅限於 Polymarket 平台，無法擴展到其他市場。
> - 依賴於外部 API，可能會受到服務中斷影響。

> [!warning] 注意事項
> - 僅支援 Node.js 20+ 和 PostgreSQL 14+。
> - 需要用戶提供 Polymarket API 憑證以進行實際交易。
> - 目前僅支援 Polymarket 平台，無法擴展到其他交易所。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供類似的自動化交易功能，但專注於不同的市場和策略。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 針對特定的交易策略進行優化，可能不具備 PolyEdge 的多樣性。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於特定的交易策略，可能不如 PolyEdge 的多樣性。 | 如果你的需求僅限於特定的交易策略，且不需要多樣化的功能。 | medium，因為需要重新配置和調整策略。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供類似的自動化交易功能，但專注於不同的市場和策略。 | 如果你的團隊已經在使用該工具，並希望保持一致性。 | low，因為功能相似，轉移成本較低。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **polymarket-trading-bot-services-polyedge365** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於特定的交易策略，可能不如 PolyEdge 的多樣性。 | 提供類似的自動化交易功能，但專注於不同的市場和策略。 |
> | 遷移成本 | - | medium，因為需要重新配置和調整策略。 | low，因為功能相似，轉移成本較低。 |
> | 適用場景 | 主要場景 | 如果你的需求僅限於特定的交易策略，且不需要多樣化的功能。 | 如果你的團隊已經在使用該工具，並希望保持一致性。 |
...
> [!tip] 採用建議
> 適合個人試用和探索，但不建議用於生產環境的核心交易。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- [MEDIUM] 在模擬模式下，可能會遇到與實際市場不一致的情況。
  - 解法：在轉換到實際交易前，仔細檢查模擬結果。
- **[HIGH]** 配置 API 憑證時，可能會出現格式錯誤。
  - 解法：確保按照文檔中的格式進行配置。
- [MEDIUM] 需要定期更新依賴包，否則可能會影響功能。
  - 解法：定期檢查和更新依賴包版本。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 活躍的 Polymarket 交易者 | 非常適合 | 提供多種交易策略，能夠滿足不同的交易需求。 |
| 風險管理專家 | 適合 | 能夠有效對沖市場風險，保護資本。 |
| 初學者交易者 | 普通 | 雖然功能強大，但需要一定的技術背景來配置。 |
| 不活躍的交易者 | 不適合 | 如果不經常交易，則無需使用這樣的自動化工具。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，3 小時整合，得到高效的自動化交易工具，值得投資。

## 安全性評估

> [!warning] 安全性快速掃描
> 中等風險：需要用戶提供 API 憑證，並存取敏感資料。依賴於外部 API，需注意供應鏈風險。
...
相關專案：[[0x0funky--agent-sprite-forge|0x0funky/agent-sprite-forge]] · [[0xGF--boneyard|0xGF/boneyard]] · [[ConardLi--garden-skills|ConardLi/garden-skills]] · [[DietrichGebert--ponytail|DietrichGebert/ponytail]] · [[Fokkyp--SoftwareCopyright-Skill|Fokkyp/SoftwareCopyright-Skill]]
