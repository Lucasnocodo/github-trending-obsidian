...
use_case: "傻瓜式生成注册机和协议分析文档，支持多种应用和浏览器的流量捕获与AI分析。"
priority: high
ring: assess
...
**798** stars · **266** stars/天 · 建立 3 天前 · TypeScript · 未標註授權
...
> **適合** 需要快速捕獲和分析多種應用流量的開發者和安全研究人員。
> **一句話重點** 這個專案的最大亮點在於其全場景抓包能力和 AI 自動分析，能夠大幅提升開發者的工作效率。
...
>     .where(p => p.subcategory === "抓包工具" && p.file.name !== "Mouseww--anything-analyzer" && p.status !== "archived")
...
>       p.ring || "assess"
>     ]));
>   } else { dv.paragraph("_目前 vault 中沒有其他 抓包工具 類工具_"); }
> }
> ```

> [!question] TL;DR — 值得投入嗎？
> **成熟度** Beta (可試用) · **安裝** Medium (需設定) · **學習** ~4h · **綁定風險** medium
> **結論** 花 4 小時學習、2 小時整合，得到強大的抓包與分析能力，值得投入。

> [!abstract] 核心創新
> 提供全场景抓包与 AI 自动分析的能力，突破传统工具的局限。

## 專案簡介

Anything Analyzer 是一款全场景的网络协议分析工具，能够通过浏览器内嵌和 MITM 代理捕获 HTTP(S) 流量。用户只需设置 CA 证书并启用代理，便可自动捕获来自浏览器、桌面应用、终端命令、Python 脚本及移动应用的请求。所有捕获的数据都会统一汇入同一会话，随后通过 AI 进行自动分析，生成协议逆向、安全审计和加密分析报告。其一键 CA 证书管理和系统代理集成功能，降低了用户的配置复杂度。核心技术使用了 Chrome DevTools Protocol (CDP) 和内置的 MITM 代理，支持高效的 TLS 解密和请求标记。

与传统工具相比，Anything Analyzer 提供了更为全面的抓包能力，能够同时处理多种来源的数据，避免了手动分析的繁琐。此工具特别适合需要快速逆向分析 API 或进行安全审计的开发者和安全研究人员。使用时可能会遇到 macOS 下的兼容性问题，但整体功能强大，适合中小型团队使用。未来可能会进一步扩展 AI 分析能力，提升用户体验。

**技術棧**：`Electron 35` · `React 19` · `TypeScript 5.7.0` · `better-sqlite3`

## 重點功能

- 全場景抓包 — 支持捕获浏览器、桌面应用、终端命令、Python 脚本及移动应用的 HTTP(S) 流量。
- 双通道捕获 — 同时使用 CDP 和 MITM 代理，所有请求汇入同一会话，便于统一分析。
- 一键 CA 证书管理 — 自动生成和管理 CA 证书，支持跨平台操作，简化配置过程。
- AI 智能分析 — 提供多种分析模式，包括 API 逆向、安全审计和性能分析，自动生成报告。
- 域名过滤功能 — 请求列表支持按域名分组过滤，便于快速查找和分析特定请求。
...
3. 启用代理并开始捕获
```bash
设置为系统代理或手动指定 http://127.0.0.1:8888
```
...
> [!tip] 爆紅原因
> 建立 3 天就累積 798 stars（266/天），forks 212（26.6%），顯示出強烈的社群興趣。作者 Mouseww 以往在開源社群活躍，這個專案解決了傳統抓包工具的局限性，提供全場景的流量捕獲與 AI 分析，讓開發者能更快速地進行協議逆向和安全分析。近期的推文和討論也引發了更多關注，顯示出其潛在的市場需求。

## 適合誰使用

**目標受眾**：需要快速捕獲和分析多種應用流量的開發者和安全研究人員。

> [!example] 使用場景
> - 前端工程師用它來逆向網站 API，因為能快速生成 API 文檔和 Python 复现代码，節省手動分析時間。
> - 安全研究員用它來進行應用的安全審計，因為可以自動識別 Token 泄露和 CSRF 漏洞，提升審計效率。
> - 後端開發者用它來調試微服務，因為能夠清晰地追蹤服務間的調用鏈路，快速定位問題。

## 架構分析

Anything Analyzer 採用 Electron 框架，整合了 React 和 TypeScript，提供跨平台的桌面應用。其架構分為主進程和渲染進程，主進程負責抓包引擎和 AI 分析，渲染進程則提供用戶界面。數據流從 CDP 和 MITM 代理進入統一會話，經過 AI 分析後生成報告。選擇 Electron 使得應用能夠在多個平台上運行，但也帶來了較大的包體積和資源消耗。整體架構設計旨在簡化用戶操作，降低使用門檻。

## 技術深入分析

Anything Analyzer 的核心技術機制是基於 Electron 框架，使用 Chrome DevTools Protocol (CDP) 和內置的 MITM 代理來捕獲 HTTP(S) 流量。其抓包引擎能夠同時處理來自瀏覽器和外部應用的請求，並將所有數據統一匯入同一會話進行分析。AI 分析部分則分為兩個階段，首先過濾噪聲請求，然後進行深度分析，支持多種分析模式，如 API 逆向和安全審計。效能上，該工具能夠在多種環境下運行，並且對於中小型數據集的處理能力良好，但在處理大文件時會受到限制。選擇 Electron 使得應用能夠跨平台運行，但也帶來了較大的包體積和資源消耗。技術風險方面，MITM 代理的使用可能面臨安全性考量，特別是在處理敏感數據時。此外，對於依賴於外部 API 的功能，可能會增加系統的複雜性和維護成本。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了詳細的使用說明和範例；安裝過程相對順暢，但在 macOS 下可能遇到兼容性問題；有良好的快速開始指南，幫助新手快速上手；目前僅提供英文文檔，缺乏多語言支持。

## 優缺點分析

> [!success] 優點
> - 全場景捕获，支持多种流量来源，极大提升了分析效率。
> - AI 自动分析，减少了手动分析的繁琐，快速生成报告。
> - 一键 CA 证书管理，简化了用户配置过程，适合新手使用。

> [!danger] 缺點
> - 在 macOS 下可能存在兼容性问题，影响用户体验。
> - MITM 代理为只读捕获，无法修改请求，限制了一些高级用例。
> - 对大文件的支持有限，单个 body 上限 1MB，可能影响某些场景。

> [!warning] 注意事項
> - 不支持 Windows 以外的某些特定功能，可能在 macOS 下遇到兼容性问题。
> - MITM 代理为只读捕获，不修改请求/响应内容，可能无法处理某些复杂场景。
> - 单个 body 上限 1MB，二进制内容自动跳过，可能影响某些大文件的捕获。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供类似的抓包功能，但主要聚焦于安全审计，适合需要深入安全分析的用户。 |
| [AgentSeal/codeburn](https://github.com/AgentSeal/codeburn) | 专注于 API 流量的捕获与分析，适合需要高频次 API 调试的开发者。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| Fiddler | 專注於 HTTP(S) 流量的捕獲和分析，但不支持多種流量來源的統一分析。 | 如果你的主要需求是針對 HTTP(S) 流量進行深入分析，且不需要捕獲其他應用的流量。 | medium，因為需要重新學習 Fiddler 的操作界面和功能。 |
| Charles | 提供類似的 MITM 代理功能，但不具備 AI 自動分析能力。 | 如果你的團隊已經在使用 Charles 且不需要 AI 分析，則可以考慮繼續使用。 | medium，因為需要重新配置代理和抓包設置。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **anything-analyzer** | **Fiddler** | **Charles** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於 HTTP(S) 流量的捕獲和分析，但不支持多種流量來源的統一分析。 | 提供類似的 MITM 代理功能，但不具備 AI 自動分析能力。 |
> | 遷移成本 | - | medium，因為需要重新學習 Fiddler 的操作界面和功能。 | medium，因為需要重新配置代理和抓包設置。 |
> | 適用場景 | 主要場景 | 如果你的主要需求是針對 HTTP(S) 流量進行深入分析，且 | 如果你的團隊已經在使用 Charles 且不需要 AI 分析 |
...
- **[HIGH]** macOS 環境下安裝後可能無法正常啟動
  - 解法：嘗試重新安裝或檢查 CA 證書配置
- [MEDIUM] 某些外部應用可能無法正確捕獲流量
  - 解法：檢查代理設置，確保應用配置正確
- [MEDIUM] 在高流量情況下可能導致性能下降
  - 解法：減少同時捕獲的請求數量，分批進行分析

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型開發團隊進行 API 逆向分析 | 非常適合 | 提供全場景抓包和 AI 分析，能快速生成文檔。 |
| 大型企業的安全審計 | 適合 | 能夠識別潛在的安全漏洞，提升審計效率。 |
| 個人開發者進行簡單的抓包分析 | 普通 | 功能強大但可能對新手有一定學習曲線。 |
| 需要處理高流量的商業應用 | 不適合 | 在高流量情況下可能導致性能下降。 |
...
> [!tip] 投入 vs 回報
> 花 4 小時學習、2 小時整合，得到強大的抓包與分析能力，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具本身不需要高權限，僅需安裝 CA 證書；不會存取敏感資料，但使用時需注意流量的安全性；依賴於外部 API 的功能可能增加安全風險。
...
> | --- | --- | --- | --- |
> | [#10](https://github.com/Mouseww/anything-analyzer/issues/10) | 功能请求:过反爬虫 | 0 | 0 |
> | [#9](https://github.com/Mouseww/anything-analyzer/issues/9) | 功能请求：在点击pause后，这个按钮可以切换为绿色的start按钮，启动抓包 | 0 | 1 |
> | [#7](https://github.com/Mouseww/anything-analyzer/issues/7) | mac环境下正常退出弹意外退出 | 0 | 2 |
> | [#4](https://github.com/Mouseww/anything-analyzer/issues/4) | mac 下dmg后安装无法打开 | 0 | 2 |
...
> FROM "Repos"
> WHERE subcategory = "抓包工具" AND file.name != "Mouseww--anything-analyzer"
> SORT stars DESC
> ```
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["MITM 代理","API 逆向","安全审计"];
