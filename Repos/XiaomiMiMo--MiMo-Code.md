...
use_case: "提供一個終端原生的 AI 編碼助手，能夠讀寫程式碼、執行命令、管理 Git，並透過持久記憶系統持續改進。"
priority: high
ring: assess
...
star_history: "2026-06-11:681,2026-06-11:794,2026-06-12:4740,2026-06-12:4782,2026-06-13:6887,2026-06-13:6905,2026-06-14:7947,2026-06-14:7958,2026-06-15:8469,2026-06-15:8478,2026-06-16:9041,2026-06-16:9048,2026-06-17:9378,2026-06-17:9380,2026-06-18:9596,2026-06-19:9794,2026-06-20:9948"
tags:
  - github
  - "category/開發工具"
  - "lang/typescript"
...
aliases:
  ...
  - "提供一個終端原生的 AI 編碼助手，能夠讀寫程式碼、執行命令、管理 Git，並透過持久記憶系統持續改進。"
---

# MiMo-Code

**9.9k** stars · **1.1k** stars/天 · 建立 9 天前 · TypeScript · MIT
...
> **適合** 需要高效編碼和專案管理的開發者，特別是長期專案的維護者。
...
> **結論** 花 5 小時學、3 小時整合，得到高效的編碼助手，值得一試。

> [!abstract] 核心創新
> MiMoCode 提供了持久記憶和智能上下文管理功能，這在現有的 AI 編碼助手中是獨一無二的。

## 專案簡介

MiMoCode 是一個終端原生的 AI 編碼助手，能夠讀寫程式碼、執行命令、管理 Git，並使用持久記憶系統來保持對專案的深刻理解。使用者可以透過簡單的安裝指令 `curl -fsSL https://mimo.xiaomi.com/install | bash` 或 `npm install -g @mimo-ai/cli` 開始使用。核心機制包括多個代理（如 `build`、`plan` 和 `compose`），這些代理能夠根據需要創建子代理，並在會話間保留上下文，讓使用者不必重複學習專案背景。MiMoCode 的持久記憶系統使用 SQLite FTS5 來管理專案知識，並自動保存會話狀態，這樣即使在不同會話中也能保持連貫性。

該工具還支持語音輸入，並能夠與主流 LLM 提供者 API 連接，這使得開發過程更加靈活和高效。相較於其他工具，MiMoCode 在上下文管理和任務追蹤方面具有明顯優勢，特別是在需要長期跟蹤專案進度的情況下。使用者可以透過 `/goal` 命令設置會話的停止條件，並利用 `/dream` 和 `/distill` 命令進行知識提取和技能打包，這些功能使得 MiMoCode 在自我改進上具有獨特的能力。整體而言，MiMoCode 是一個強大的開發工具，適合需要高效編碼和專案管理的開發者。

**技術棧**：`TypeScript` · `Astro` · `Docker` · `SQLite`

## 重點功能

- 多代理系統 — 包含 `build`、`plan` 和 `compose` 代理，能根據需求創建子代理。
- 持久記憶 — 使用 SQLite FTS5 來管理專案知識，保持會話間的上下文連貫性。
- 智能上下文管理 — 自動保存會話狀態，並在需要時重建上下文。
- 任務追蹤系統 — 支持樹狀任務結構，自動與檢查點系統整合。
- 語音輸入 — 實時語音輸入功能，支持多種語音提供者，提升編碼效率。
...
> [!tip] 爆紅原因
> 建立 9 天內累積 9948 stars（1105/天），forks 916（9.2%），顯示出強勁的增長潛力。這個專案由小米團隊開發，旨在解決現有 AI 編碼助手在持久記憶和上下文管理上的不足。之前的工具如 OpenCode 雖然功能強大，但缺乏有效的記憶系統和上下文管理，這使得開發者在長期專案中容易失去上下文。小米的推廣和社群的活躍也促進了這個專案的快速成長。

## 適合誰使用

**目標受眾**：需要高效編碼和專案管理的開發者，特別是長期專案的維護者。

> [!example] 使用場景
> - 後端工程師用它來自動化管理 Git 和執行命令，因為這樣能夠節省大量手動操作時間，提升開發效率。
> - 全端開發者用它來進行多任務的專案管理，因為其持久記憶系統能夠有效跟蹤專案進度，避免重複工作。
> - AI 研究人員用它來快速測試和調整模型，因為語音輸入功能能夠讓他們在編碼時保持手部自由，專注於思考。

## 架構分析

MiMoCode 採用模組化架構，核心功能由多個代理組成，這樣的設計使得功能擴展變得靈活。每個代理負責特定任務，並能夠根據需要創建子代理，這樣的設計使得任務可以並行處理。持久記憶系統使用 SQLite 來管理專案知識，這樣可以快速檢索和更新記憶。整體架構的設計考量了開發者的使用習慣，提供了簡單的 CLI 和直觀的配置方式，降低了使用門檻。這種模組化的設計也使得未來的功能擴展變得容易，能夠快速適應不斷變化的開發需求。

## 技術深入分析

MiMoCode 的核心技術機制包括多代理系統和持久記憶管理。每個代理負責特定的任務，如開發、計劃和組合，這樣的設計使得任務可以靈活分配。持久記憶系統使用 SQLite FTS5 來管理專案知識，這不僅提高了檢索速度，也能夠在不同會話中保持上下文連貫性。該系統能夠自動保存會話狀態，並在需要時重建上下文，這對於長期專案的維護至關重要。設計上，MiMoCode 旨在降低使用門檻，提供簡單的 CLI 和直觀的配置方式，這使得新手也能快速上手。未來，隨著功能的擴展，可能會出現更多的集成選項，進一步提升開發者的使用體驗。整體而言，MiMoCode 在 AI 編碼助手領域提供了一個創新的解決方案，特別是在持久記憶和上下文管理方面。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝指導和範例，安裝過程相對順暢。首次啟動時會自動引導配置，降低了新手的上手難度。文件目前僅提供英文，可能對非英語使用者造成一定障礙。

## 優缺點分析

> [!success] 優點
> - 具備持久記憶系統，能夠在不同會話中保持上下文。
> - 多代理系統支持靈活的任務管理和並行處理。
> - 語音輸入功能提升了編碼的便利性和效率。
...
> - 需要持續的網路連接以使用 LLM 提供者的 API。
> - 在 WSL 環境下可能會遇到剪貼板問題，需要額外安裝 `xsel`。
...
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於代碼生成，但缺乏上下文管理和任務追蹤功能。 |
| [0xSero/codex-shim](https://github.com/0xSero/codex-shim) | 提供 LLM 接口，但不支持多代理系統和語音輸入功能。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供類似的 AI 助手功能，但不具備持久記憶系統，適合短期專案。 | 如果你的專案不需要長期的上下文管理，且希望快速上手。 | low，因為功能相似，轉換成本不高。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於代碼生成，但缺乏上下文管理和任務追蹤功能。 | 如果你的需求主要是生成代碼，而不需要持久的上下文。 | medium，因為需要重新適應不同的工作流。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **MiMo-Code** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 提供類似的 AI 助手功能，但不具備持久記憶系統，適合短期專案。 | 專注於代碼生成，但缺乏上下文管理和任務追蹤功能。 |
> | 遷移成本 | - | low，因為功能相似，轉換成本不高。 | medium，因為需要重新適應不同的工作流。 |
> | 適用場景 | 主要場景 | 如果你的專案不需要長期的上下文管理，且希望快速上手。 | 如果你的需求主要是生成代碼，而不需要持久的上下文。 |
...
- **[HIGH]** 在 WSL 環境下可能會遇到剪貼板問題，導致文本亂碼
  - 解法：安裝 `xsel` 解決問題
- [MEDIUM] 首次使用時可能需要較長的配置時間
  - 解法：參考官方文檔快速配置
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 持久記憶系統能夠有效跟蹤專案進度，避免重複工作。 |
| 大型企業的長期專案維護 | 適合 | 多代理系統能夠靈活分配任務，提升效率。 |
| 短期專案或快速原型開發 | 普通 | 雖然功能強大，但可能過於複雜。 |
| 需要快速生成代碼的開發者 | 不適合 | 專注於代碼生成的工具可能更合適。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學、3 小時整合，得到高效的編碼助手，值得一試。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具本身不需要高權限，並且不會存取敏感資料。依賴鏈的信任程度良好，適合在 CI/CD 中使用。
...
> | [#1118](https://github.com/XiaomiMiMo/MiMo-Code/issues/1118) | wechat-mimocode — MiMoCode 链接微信的实践与反馈 | 5 | 0 |
...
相關概念：[[AI 助手]] · [[持久記憶]] · [[上下文工程]] · [[語音輸入]] · [[任務追蹤]]
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["AI 助手","持久記憶","上下文工程","語音輸入","任務追蹤"];
