...
use_case: "一個終端原生的 AI 編程助手，能夠讀寫代碼、執行命令並管理 Git。"
priority: high
ring: assess
...
star_history: "2026-06-11:681,2026-06-11:794,2026-06-12:4740,2026-06-12:4782,2026-06-13:6887,2026-06-13:6905,2026-06-14:7947,2026-06-14:7958,2026-06-15:8469,2026-06-15:8478,2026-06-16:9041,2026-06-16:9048,2026-06-17:9378,2026-06-17:9380,2026-06-18:9596"
tags:
  - github
  - "category/開發工具"
  - "lang/typescript"
...
aliases:
  ...
  - "一個終端原生的 AI 編程助手，能夠讀寫代碼、執行命令並管理 Git。"
---

# MiMo-Code

**9.6k** stars · **1.4k** stars/天 · 建立 7 天前 · TypeScript · MIT
...
> **適合** 希望提升開發效率並需要多任務處理能力的開發者，特別是小型團隊或個人開發者。
> **一句話重點** MiMo Code 的核心創新在於將 AI 代理與持久記憶系統結合，讓開發者能夠在多任務環境中高效工作。
...
> **結論** 花 3 小時學習，2 小時整合，得到高效的開發助手，值得一試。

> [!abstract] 核心創新
> 提供了一個終端原生的 AI 編程助手，結合持久記憶和智能上下文管理。

## 專案簡介

MiMo Code 是一個終端原生的 AI 編程助手，旨在提升開發效率。用戶可以透過簡單的命令如 `mimo` 來啟動，並在首次啟動時自動引導配置。其核心功能包括多個代理模式（如 `build`、`plan` 和 `compose`），這些代理可以根據需求切換，並在開發過程中提供不同的功能。MiMo Code 的持久記憶系統使用 SQLite FTS5 進行全文本搜索，能夠在不同會話之間保持對項目的深刻理解，並自動保存會話狀態。這種設計使得開發者無需重複學習項目上下文，從而提高了工作效率。

與其他工具相比，MiMo Code 支持與主流 LLM 提供者 API 連接，並且提供了無需配置的免費通道，這使得新手開發者也能輕鬆上手。相較於傳統的 IDE，MiMo Code 的智能上下文管理和任務追蹤系統能夠更好地適應開發者的需求，特別是在多任務環境下。這些功能的整合使得它在處理複雜項目時，能夠提供更高的靈活性和效率。實際使用中，開發者可能會遇到 WSL 的剪貼簿問題，但可以透過安裝 `xsel` 來解決。整體來看，MiMo Code 目前處於 beta 階段，適合小型團隊或個人開發者使用，未來可能會進一步增強其功能和穩定性。

**技術棧**：`TypeScript` · `Astro` · `Docker`

## 重點功能

- 多代理系統 — 提供 `build`、`plan` 和 `compose` 三種模式，根據需求切換。
- 持久記憶 — 使用 SQLite FTS5 進行全文本搜索，保持項目上下文。
- 智能上下文管理 — 自動決定何時保存會話狀態，並重建上下文。
- 任務追蹤系統 — 樹狀任務系統，支持任務進度的持久化。
- 子代理系統 — 主代理可根據需求創建子代理，支持並行處理。
...
> [!tip] 爆紅原因
> 建立 7 天內累積 9596 stars（1371/天），forks 863（9.0%），這顯示出強勁的增長潛力。主要貢獻者來自小米，這是一個有實力的團隊，過去在 AI 及開發工具領域有豐富經驗。MiMo Code 解決了開發者在多任務和上下文管理上的痛點，這在傳統 IDE 中往往無法高效實現。近期的推廣活動和社群討論也為其帶來了關注。這個工具的設計考量了現代開發者的需求，尤其是在快速迭代的環境中，能夠提供即時的上下文支持和任務追蹤。

## 適合誰使用

**目標受眾**：希望提升開發效率並需要多任務處理能力的開發者，特別是小型團隊或個人開發者。

> [!example] 使用場景
> - 後端工程師用它來管理 Git 和執行命令，因為它的持久記憶系統能夠保持項目上下文，減少重複學習的時間。
> - 全端開發者用它來協作開發多個功能模組，因為其多代理系統能夠同時處理不同的任務，提升工作效率。
> - 新手開發者用它來快速上手編程，因為它提供了免費的無需配置通道，降低了入門門檻。

## 架構分析

MiMo Code 採用模組化架構，主要由多個代理組成，這些代理根據用戶需求切換。持久記憶系統使用 SQLite FTS5 來管理項目上下文，這樣的設計使得開發者無需重複學習項目細節，提升了開發效率。任務追蹤系統則以樹狀結構管理任務，並與持久記憶系統自動整合。

這種設計使得在多任務環境中，開發者能夠輕鬆跟蹤進度和狀態。選擇 SQLite 作為持久記憶的數據庫，因為其輕量和高效，適合快速查詢和更新。整體架構的擴展性良好，但在處理極大規模的項目時，可能會面臨性能瓶頸。

## 技術深入分析

MiMo Code 的核心技術機制在於其多代理系統，這些代理根據開發者的需求進行切換，並能夠在不同任務間共享上下文。持久記憶系統使用 SQLite FTS5 進行全文本搜索，這使得開發者能夠快速查詢和更新項目知識。這種設計使得開發者在不同會話之間能夠保持對項目的深刻理解，從而提升工作效率。在效能方面，MiMo Code 能夠處理中小型項目的開發需求，但在面對大型項目時，持久記憶的性能可能會受到影響。選擇 TypeScript 和 Astro 作為技術棧，這不僅提高了開發效率，還能夠在不同平台上保持一致性。整體架構的設計考量了擴展性，但在處理極大規模的項目時，可能會面臨性能瓶頸。對於外部 API 的依賴程度較低，這使得其在不同環境中都有良好的適應性。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了安裝和使用的詳細指導。安裝過程相對順暢，但在 WSL 環境下可能會遇到剪貼簿問題。文件中有多語言支持，適合不同語言的開發者。整體來說，花 30 分鐘能夠順利運行起來。
...
> - 多代理系統能夠靈活應對不同開發需求。
> - 持久記憶系統減少了開發者的重複學習時間。
> - 智能上下文管理提升了開發效率，特別是在多任務環境下。

> [!danger] 缺點
> - 目前仍在 beta 階段，穩定性有待加強。
> - 對於大型項目，持久記憶的性能可能會受到影響。
> - 在 WSL 環境下可能會遇到剪貼簿問題。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供類似的 AI 代理功能，但更專注於遊戲開發場景。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於代碼的版本控制和管理，缺乏 MiMo Code 的智能上下文管理功能。 |
| [2aronS/Duel-Agents](https://github.com/2aronS/Duel-Agents) | 提供多代理系統，但不支持持久記憶和上下文重建。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於遊戲開發的 AI 代理，提供不同的功能模組，但不如 MiMo Code 的多代理系統靈活。 | 如果你的開發重點是遊戲相關的 AI 代理，則可以選擇此工具。 | medium，因為需要重新適應不同的開發環境和工具鏈。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於代碼的版本控制和管理，缺乏 MiMo Code 的智能上下文管理功能。 | 如果你的主要需求是版本控制而非上下文管理，則可以考慮此工具。 | low，因為大多數開發者對版本控制工具有一定的熟悉度。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **MiMo-Code** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於遊戲開發的 AI 代理，提供不同的功能模組，但不如 MiMo Code 的多代理系統靈活。 | 專注於代碼的版本控制和管理，缺乏 MiMo Code 的智能上下文管理功能。 |
> | 遷移成本 | - | medium，因為需要重新適應不同的開發環境和工具鏈。 | low，因為大多數開發者對版本控制工具有一定的熟悉度。 |
> | 適用場景 | 主要場景 | 如果你的開發重點是遊戲相關的 AI 代理，則可以選擇此工具。 | 如果你的主要需求是版本控制而非上下文管理，則可以考慮此工具。 |
...
- **[HIGH]** 在 WSL 環境下剪貼簿可能出現問題，導致文本複製失敗。
  - 解法：安裝 xsel 來解決此問題。
- [MEDIUM] 使用過程中可能會遇到性能瓶頸，特別是在大型項目中。
  - 解法：考慮將項目拆分為小型模組來減少負擔。
- [MEDIUM] 持久記憶的數據可能會隨著時間增長而影響查詢性能。
  - 解法：定期清理不必要的記錄以保持性能。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 5 人以下的新創團隊開發小型應用 | 非常適合 | 其持久記憶和多代理系統能夠有效支持小型團隊的需求。 |
| 大型企業的核心系統開發 | 不適合 | 目前仍在 beta 階段，穩定性不足。 |
| 個人開發者進行快速原型開發 | 適合 | 提供了無需配置的免費通道，降低了入門門檻。 |
...
> [!tip] 投入 vs 回報
> 花 3 小時學習，2 小時整合，得到高效的開發助手，值得一試。
...
> | 貢獻者 | Commits |
> | --- | --- |
> | [@qiaozongming](https://github.com/qiaozongming) | 8 |
> | [@yanyihan-xiaomi](https://github.com/yanyihan-xiaomi) | 2 |
...
**社群活躍度**：社群活躍度中等，開放問題數量較多，顯示出使用者的需求和反饋。
**連結**：[文件](https://mimo.xiaomi.com/coder)

## 開發動態

> [!abstract] 最近 10 次 commit（2026-06-11 ~ 2026-06-17）
> **活躍天數** 3 天 · **最新 commit** feat(voice): make ASR/control provider configurable via voice config (#900)
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["Agent 框架","任務管理","持久記憶"];
