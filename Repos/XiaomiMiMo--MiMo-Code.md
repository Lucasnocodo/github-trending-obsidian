...
use_case: "提供一個智能的終端 AI 編碼助手，能夠理解並持續改進你的專案。"
priority: high
ring: assess
...
star_history: "2026-06-11:681,2026-06-11:794,2026-06-12:4740,2026-06-12:4782,2026-06-13:6887,2026-06-13:6905,2026-06-14:7947,2026-06-14:7958,2026-06-15:8469,2026-06-15:8478,2026-06-16:9041,2026-06-16:9048,2026-06-17:9378,2026-06-17:9380,2026-06-18:9596,2026-06-19:9794,2026-06-20:9948,2026-06-21:10087,2026-06-22:10194,2026-06-23:10358,2026-06-24:10497,2026-06-25:10624"
tags:
  - github
  - "category/開發工具"
  - "lang/typescript"
...
aliases:
  ...
  - "提供一個智能的終端 AI 編碼助手，能夠理解並持續改進你的專案。"
---

# MiMo-Code

**10.6k** stars · **759** stars/天 · 建立 14 天前 · TypeScript · MIT
...
> [!info] 速覽
> **安裝難度** Easy · **專案狀態** Recent · **熱度** Hot (759 stars/day)
> **授權** MIT (商業友好) · **維護** Active (最後推送 0 天前) · **貢獻者** 5+ 人 · **參與度** Low
> **適合** 需要在終端環境中高效編碼並管理專案的開發者，特別是那些希望利用 AI 提升工作效率的人。
> **一句話重點** MiMoCode 的持久記憶和智能上下文管理功能，讓開發者能夠在多次會話中高效工作，這是其他工具所不具備的優勢。
...
> **結論** 花 3 小時學習，2 小時整合，得到高效的編碼助手，值得投資。

> [!abstract] 核心創新
> MiMoCode 引入了持久記憶和智能上下文管理，讓 AI 代理能夠在多次會話中持續學習和改進。

## 專案簡介

MiMoCode 是一個終端原生的 AI 編碼助手，能夠讀取和寫入代碼、執行命令、管理 Git，並利用持久記憶系統在多次會話中保持對專案的深刻理解。用戶可以透過簡單的安裝指令 `curl -fsSL https://mimo.xiaomi.com/install | bash` 開始使用，並在首次啟動時自動引導配置。這個工具的賣點在於其持久記憶功能，使用 SQLite FTS5 進行全文本搜索，能夠保存專案知識、會話檢查點和任務進度，讓用戶在不同會話中無縫接續工作。技術上，MiMoCode 利用 TypeScript 和 Bun 作為包管理器，並且支持多種 LLM 提供者的 API，這使得它在功能上具備高度的靈活性和擴展性。

與其他類似工具相比，如 OpenCode，MiMoCode 增加了智能上下文管理和子代理系統，這讓它在處理複雜任務時更為高效。實際使用中，MiMoCode 能夠自動保存會話狀態，並在上下文接近限制時重建上下文，這樣的設計避免了重複學習的成本。這個工具適合需要高效編碼和項目管理的開發者，特別是在快速迭代的環境中。未來六個月內，預期會有更多功能更新和社群支持，進一步提升其穩定性和使用體驗。

**技術棧**：`TypeScript` · `Bun` · `SQLite` · `Docker`

## 重點功能

- 持久記憶 — 使用 SQLite FTS5 保存專案知識和會話檢查點，減少重複學習。
- 多代理系統 — 支持多種工作模式（如開發、計劃和組合），用戶可按需切換。
- 智能上下文管理 — 自動保存會話狀態，並在上下文接近限制時重建上下文。
- 任務追蹤 — 樹狀任務系統自動與檢查點系統整合，保持任務進度。
- 語音輸入 — 實時流式語音輸入，支持多平台配置，提升使用便利性。
...
> [!tip] 爆紅原因
> 建立 14 天內累積 10624 stars（759/天），forks 998（9.4%），顯示出強勁的增長勢頭。主要貢獻者來自小米，這是一個具有豐富背景的團隊，致力於開發 AI 相關工具。這個專案解決了開發者在多會話環境中持續學習和記憶的痛點，之前的工具往往無法有效管理專案上下文。最近的推廣活動和社群互動也促進了其曝光率，讓更多開發者注意到這個工具。高 forks/stars 比率顯示出使用者對於這個工具的實際修改和需求，代表著活躍的社群參與。

## 適合誰使用

**目標受眾**：需要在終端環境中高效編碼並管理專案的開發者，特別是那些希望利用 AI 提升工作效率的人。

> [!example] 使用場景
> - 全端工程師用它來管理大型專案的代碼和文檔，因為其持久記憶功能能夠減少重複學習的時間。
> - DevOps 工程師用它來自動化部署流程，因為它能夠與 Git 進行無縫整合，並追蹤任務進度。
> - AI 研究員用它來快速實驗和迭代模型，因為它的多代理系統能夠同時處理多個任務，提高工作效率。

## 架構分析

MiMoCode 採用模組化架構，主要由多個代理組成，這些代理能夠根據需求創建子代理以處理不同任務。持久記憶系統使用 SQLite FTS5 進行資料存儲，這使得快速檢索和更新成為可能。上下文管理的智能化設計讓代理能夠根據當前任務自動調整記憶內容，這樣的設計提高了系統的靈活性和效率。

選擇使用 TypeScript 和 Bun 使得開發過程中能夠享受靜態類型檢查的優勢，並且 Bun 的高效性能也有助於提升整體運行效率。這樣的設計雖然增加了系統的複雜性，但也帶來了更強的功能擴展性。擴展性方面，MiMoCode 的模組化設計使得未來可以輕鬆添加新功能或整合其他服務。

## 技術深入分析

MiMoCode 的核心技術機制包括多代理架構和持久記憶系統，這使得它能夠在不同任務間靈活切換並保持上下文。使用 SQLite FTS5 進行記憶管理，能夠快速檢索和更新專案知識，這在大型專案中尤為重要。性能方面，MiMoCode 在處理多任務時能夠保持良好的響應速度，但在記憶內容過多的情況下可能會出現性能瓶頸。選擇 TypeScript 和 Bun 作為開發語言和環境，讓開發者能夠享受靜態類型檢查的優勢，並且 Bun 的高效性能也有助於提升整體運行效率。這樣的選擇雖然增加了系統的複雜性，但也帶來了更強的功能擴展性。技術風險方面，持久記憶的管理可能在未來的擴展中出現挑戰，特別是在資料量增大時。整合方面，MiMoCode 能夠與主流的 LLM 提供者無縫連接，這使得它在實際應用中具備高度的靈活性。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且提供了安裝和使用的範例，讓新手能夠快速上手。安裝過程相對順暢，但 Windows 用戶需注意 WSL 的配置問題。該專案的文檔支持多語言，對於不同語言的用戶友好。
...
> [!warning] 注意事項
> - 目前僅支援 Linux 和 macOS，Windows 用戶需透過 WSL 使用。
> - 持久記憶功能可能在大型專案中導致性能下降，需適當管理記憶內容。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於創建和管理 AI 代理，提供更強的自定義能力，但缺乏持久記憶功能。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供代碼生成和自動化功能，但在上下文管理和多代理系統上不如 MiMoCode 靈活。 |
| [0xSero/codex-shim](https://github.com/0xSero/codex-shim) | 專注於與 OpenAI Codex 的集成，但缺少 MiMoCode 的持久記憶和任務追蹤功能。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於創建和管理 AI 代理，提供更強的自定義能力，但缺乏持久記憶功能。 | 如果需要更高的自定義能力和靈活性，且不需要持久記憶功能。 | medium - 需要重新配置代理和任務管理系統。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供代碼生成和自動化功能，但在上下文管理和多代理系統上不如 MiMoCode 靈活。 | 如果主要需求是代碼生成而非上下文管理，且希望使用簡單的自動化工具。 | low - 主要是代碼生成的替代，遷移成本較低。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **MiMo-Code** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於創建和管理 AI 代理，提供更強的自定義能力，但缺乏持久記憶功能。 | 提供代碼生成和自動化功能，但在上下文管理和多代理系統上不如 MiMoCode 靈活。 |
> | 遷移成本 | - | medium - 需要重新配置代理和任務管理系統。 | low - 主要是代碼生成的替代，遷移成本較低。 |
> | 適用場景 | 主要場景 | 如果需要更高的自定義能力和靈活性，且不需要持久記憶功能。 | 如果主要需求是代碼生成而非上下文管理，且希望使用簡單的自動化 |
...
- **[HIGH]** 在 WSL 中使用時，複製的文本可能出現亂碼問題。
  - 解法：安裝 xsel 解決剪貼板問題。
- [MEDIUM] 大型專案中，持久記憶可能導致性能下降。
  - 解法：定期清理不必要的記憶內容。
- [low] 某些功能需要額外安裝依賴，增加了使用的複雜性。
  - 解法：參考文檔安裝必要的依賴。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 持久記憶功能能夠提高開發效率，減少重複學習的時間。 |
| 大型企業的多團隊協作專案 | 普通 | 雖然支持多代理，但在大型專案中可能出現性能瓶頸。 |
| 對 AI 編碼助手有興趣的個人開發者 | 非常適合 | 簡單的安裝和強大的功能使其成為個人開發者的理想選擇。 |
...
> [!tip] 投入 vs 回報
> 花 3 小時學習，2 小時整合，得到高效的編碼助手，值得投資。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：不需要高權限運行，且不存取敏感資料。依賴的庫和模組經過審計，整體安全性良好。
...
> | [@JinyuXiang-Mimo](https://github.com/JinyuXiang-Mimo) | 28 |
> | [@yanyihan-xiaomi](https://github.com/yanyihan-xiaomi) | 22 |
...
> **活躍天數** 1 天 · **最新 commit** chore: update gen.ts file in sdk (#1303)
...
> | [#1228](https://github.com/XiaomiMiMo/MiMo-Code/issues/1228) | 提示✗ Request blocked by risk control status 400 · non-retryab | 6 | 14 |
...
相關概念：[[Agent 框架]] · [[持久記憶]] · [[上下文工程]] · [[任務追蹤]] · [[語音識別]]
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["Agent 框架","持久記憶","上下文工程","任務追蹤","語音識別"];
