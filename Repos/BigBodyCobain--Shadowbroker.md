...
use_case: "提供全球即時開源情報，整合多個數據源於一個界面，讓用戶能夠追蹤各種公共信號。"
priority: medium
ring: assess
...
star_history: "2026-03-10:1455,2026-03-11:1464,2026-03-13:2127,2026-03-14:2662,2026-03-15:2852,2026-03-16:3053,2026-03-17:3274,2026-03-18:3512,2026-03-19:3617,2026-03-20:3892,2026-03-21:4221,2026-03-22:4471,2026-03-23:4737,2026-03-24:4878,2026-03-25:4964,2026-03-26:5049,2026-03-27:5096,2026-03-28:5128"
tags:
  - github
  - "category/其他"
  - "lang/typescript"
...
aliases:
  ...
  - "提供全球即時開源情報，整合多個數據源於一個界面，讓用戶能夠追蹤各種公共信號。"
---

# Shadowbroker

**5.1k** stars · **233** stars/天 · 建立 22 天前 · TypeScript · AGPL-3.0
...
> **適合** 需要即時追蹤全球公共信號的安全分析師和研究人員。
> **一句話重點** 這個專案的強大之處在於其即時性和多樣性，能夠將多個公共信號整合於一個平台，讓用戶能夠全面了解全球動態。
...
> **結論** 花 4 小時學習，2 小時整合，得到即時的全球監控能力，值得一試。

> [!abstract] 核心創新
> 將多個即時情報來源整合於單一界面，提供即時的全球監控能力。

## 專案簡介

ShadowBroker 是一個即時的多領域開源情報 (OSINT) 儀表板，將 60 多個即時情報來源融合於單一的黑色操作地圖介面。用戶可以追蹤飛機、船隻、衛星、衝突區域、CCTV 網絡等，所有數據實時更新。這個工具的設計理念是讓分析師、研究人員和無論是誰都能在同一地圖上看到全球的公共信號。使用者只需透過 Docker 部署，執行 `docker compose up -d` 即可啟動儀表板，無需收集或傳輸用戶數據，完全在本地運行。這樣的設計不僅提升了用戶的隱私，也簡化了部署流程。技術上，ShadowBroker 使用 Next.js 和 FastAPI，確保前端和後端的模組化架構，便於擴展和維護。

這個專案的賣點在於其即時性和多樣性，能夠整合來自不同來源的數據，並提供豐富的視覺化選項。與其他 OSINT 工具相比，如 OpenSky Network 和 Shodan，ShadowBroker 提供了更全面的數據覆蓋範圍，並且支持多種數據層的切換。使用者可以透過右鍵點擊地圖上的任意位置，獲取該國的資料和最新的衛星照片，這在其他工具中並不常見。儘管如此，這個工具仍然依賴於外部 API，使用者需要準備相應的 API 金鑰來獲取某些數據。整體而言，ShadowBroker 是一個功能強大且靈活的 OSINT 解決方案，適合需要即時情報的用戶。

**技術棧**：`Next.js` · `FastAPI` · `Python` · `Docker`

## 重點功能

- 即時數據整合 — 融合 60+ 個即時情報來源，提供全面的全球監控。
- 多層次視覺化 — 提供 35+ 可切換的數據層，支持不同的視覺模式（如衛星、夜光等）。
- 模組化架構 — 前端和後端可獨立擴展，便於維護和升級。
- 安全性設計 — 以限制 UID 運行容器，提升安全性。
- 無需用戶數據 — 儀表板完全在本地運行，無需收集或傳輸用戶數據。
...
```ts
{
  ...
  "預期輸出": "儀表板在 http://localhost:3000 可用"
}
```
...
> [!tip] 爆紅原因
> 建立 22 天就累積 5128 stars（233/天），forks 768（15%），顯示出強烈的社群興趣。作者 BigBodyCobain 和其他貢獻者在開源社群中有一定的影響力，並且這個專案解決了以往 OSINT 工具分散、難以整合的痛點。之前的方案如 OpenSky Network 雖然提供飛行數據，但缺乏全面的地理信息整合。這個專案的推出引起了關注，可能是因為其獨特的即時數據整合能力和用戶友好的界面設計。forks/stars 比率為 15%，顯示出許多用戶在實際修改和使用這個工具。

## 適合誰使用

**目標受眾**：需要即時追蹤全球公共信號的安全分析師和研究人員。

> [!example] 使用場景
> - 安全分析師用它來追蹤全球的飛行器動態，因為它能夠即時顯示商業和軍事飛行的數據，並且提供多層次的視覺化選項。
> - 研究人員利用它來分析地震活動和衝突區域的變化，因為它整合了多個數據來源，讓他們能在一個平台上獲取所需的所有信息。
> - 無線電操作員使用它來監控即時的無線電頻道和設備，因為它支持實時數據流，能夠快速反應於變化的情況。

## 架構分析

ShadowBroker 採用模組化架構，前端使用 Next.js 提供用戶界面，後端使用 FastAPI 處理數據請求。這樣的設計使得前後端可以獨立擴展，便於維護。數據流從多個外部 API 獲取，經過 FastAPI 處理後傳遞給前端顯示。

選擇 Docker 作為部署方式，能夠簡化環境配置和依賴管理，但也意味著需要用戶具備 Docker 的基本知識。這種架構的優勢在於能快速部署和更新，但在處理大量數據時可能會面臨性能瓶頸，特別是當多個數據源同時更新時。整體而言，這樣的設計使得 ShadowBroker 能夠靈活應對不同的使用場景。

## 技術深入分析

ShadowBroker 的核心技術機制在於其即時數據整合能力，使用了多個外部 API 來獲取不同類型的數據，如飛行器、船隻和地震活動等。這些數據通過 FastAPI 處理，並在 Next.js 前端展示。效能方面，儀表板能夠同時處理多個數據流，但在高負載情況下可能會出現延遲。選擇 Docker 作為部署方式，能夠簡化環境配置，但也需要用戶具備一定的技術背景。這種設計的取捨在於提高了部署的靈活性，但也可能增加了學習成本。技術風險方面，依賴於外部 API 的穩定性可能會成為未來擴展的瓶頸，特別是在數據源更新頻繁的情況下。整合方面，與主流的 CI/CD 工具如 GitHub Actions 和 Docker Hub 的兼容性良好，便於團隊的持續集成和部署。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且提供了詳細的安裝步驟和範例，讓新手能夠快速上手。安裝過程順暢，Docker 的使用使得環境配置變得簡單。文件中有針對開發者的設置指南，對於希望修改代碼的用戶非常友好。

## 優缺點分析

> [!success] 優點
> - 即時數據更新，讓用戶能夠快速獲取最新情報。
> - 多樣的視覺化選項，提升用戶的數據分析能力。
> - 模組化設計，便於擴展和維護。

> [!danger] 缺點
> - 需要有效的 API 金鑰來獲取某些數據，增加了使用門檻。
> - 依賴於外部服務的穩定性，可能影響數據的可靠性。
> - 對系統資源的需求較高，可能不適合低配置的環境。

> [!warning] 注意事項
> - 需要有效的 API 金鑰來獲取某些數據源的數據。
> - 依賴於外部服務的穩定性，若服務中斷可能影響數據更新。
> - 對於大型數據集的處理可能需要較高的系統資源。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| OpenSky Network | 專注於飛行數據，無法提供全面的地理信息整合，適合只需飛行數據的用戶。 |
| Shodan | 主要針對互聯網連接設備的搜索，無法提供即時的地理數據視覺化，適合需要設備搜索的用戶。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| OpenSky Network | 專注於飛行數據，無法提供全面的地理信息整合。 | 如果你的需求僅限於飛行數據的追蹤，OpenSky Network 可能更簡單易用。 | low，因為主要是專注於飛行數據，功能較為單一。 |
| Shodan | 主要針對互聯網連接設備的搜索，無法提供即時的地理數據視覺化。 | 如果你需要針對設備的詳細搜索，Shodan 是更合適的選擇。 | medium，因為需要重新適應不同的數據來源和使用方式。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **Shadowbroker** | **OpenSky Network** | **Shodan** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於飛行數據，無法提供全面的地理信息整合。 | 主要針對互聯網連接設備的搜索，無法提供即時的地理數據視覺化。 |
> | 遷移成本 | - | low，因為主要是專注於飛行數據，功能較為單一。 | medium，因為需要重新適應不同的數據來源和使用方式。 |
> | 適用場景 | 主要場景 | 如果你的需求僅限於飛行數據的追蹤，OpenSky Netwo | 如果你需要針對設備的詳細搜索，Shodan 是更合適的選擇。 |
...
> [!tip] 採用建議
> 適合個人和小型團隊的試用，但不建議在生產環境的核心路徑上使用。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 某些 API 金鑰未正確配置會導致數據無法獲取
  - 解法：確保在 .env 文件中正確設置所有 API 金鑰
- [MEDIUM] Docker 映像更新後可能需要重新啟動容器
  - 解法：使用 `docker compose down` 和 `docker compose up -d` 來重啟容器
- [MEDIUM] 在高負載下，數據更新可能會延遲
  - 解法：考慮增加系統資源或優化數據流
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司需要即時監控全球動態 | 非常適合 | 即時數據整合和多樣的視覺化選項能夠滿足小型團隊的需求。 |
| 大型企業需要處理大量數據流 | 普通 | 雖然功能強大，但對系統資源的需求較高，可能需要額外的硬體支持。 |
| 學術研究機構需要分析地震和衝突數據 | 適合 | 整合多個數據來源的能力能夠支持深入的研究分析。 |
| 個人用戶希望追蹤特定的公共信號 | 適合 | 用戶友好的界面和即時更新能夠滿足個人需求。 |
...
> [!tip] 投入 vs 回報
> 花 4 小時學習，2 小時整合，得到即時的全球監控能力，值得一試。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：儀表板在本地運行，無需高權限操作，且不存取敏感資料。依賴的外部 API 需謹慎配置，確保不洩露 API 金鑰。
...
> | 貢獻者 | Commits |
> | --- | --- |
> | [@anoracleofra-code](https://github.com/anoracleofra-code) | 111 |
> | [@BigBodyCobain](https://github.com/BigBodyCobain) | 72 |
...
> **活躍天數** 1 天 · **最新 commit** fix: add version to health endpoint + warn users with stale compose files
...
> | [NASA GIBS](https://gibs.earthdata.nasa.gov) | MODIS T
