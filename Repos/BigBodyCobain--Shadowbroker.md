...
use_case: "提供全球開源情報的即時地理空間智能平台，整合多個數據來源於一個界面中。"
priority: medium
ring: assess
...
star_history: "2026-03-10:1455,2026-03-11:1464,2026-03-13:2127,2026-03-14:2662,2026-03-15:2852,2026-03-16:3053,2026-03-17:3274,2026-03-18:3512,2026-03-19:3617,2026-03-20:3892,2026-03-21:4221,2026-03-22:4471,2026-03-23:4737,2026-03-24:4878,2026-03-25:4964,2026-03-26:5049,2026-03-27:5096,2026-03-28:5128,2026-04-04:5151"
tags:
  - github
  - "category/其他"
  - "lang/typescript"
...
aliases:
  ...
  - "提供全球開源情報的即時地理空間智能平台，整合多個數據來源於一個界面中。"
---

# Shadowbroker

**5.2k** stars · **178** stars/天 · 建立 29 天前 · TypeScript · AGPL-3.0
...
> **適合** 需要即時追蹤全球事件並整合多個情報來源的安全分析師和研究者。
> **一句話重點** 這個專案最厲害的地方在於它將多個即時情報來源整合在一起，讓用戶能夠在一個界面上獲得全面的全球視圖。
...
> **結論** 花 5 小時學習，3 小時整合，得到即時情報的全面視圖，值得嘗試。

> [!abstract] 核心創新
> 將 60+ 個即時情報來源整合到一個黑色操作地圖介面中，提供無與倫比的即時數據視圖。

## 專案簡介

ShadowBroker 是一個即時的多領域開源情報(OSINT)儀表板，將 60 多個即時情報來源融合到一個黑色操作地圖介面中。用戶可以追蹤飛機、船隻、衛星、衝突區域、CCTV 網絡等，所有數據都在一個屏幕上實時更新。這個工具的賣點在於其模組化架構，允許用戶根據需求獨立擴展情報後端和前端界面。使用者只需在 Docker 中運行，無需用戶數據收集，完全在本地運行。關鍵指令如 `docker compose up -d` 可以快速啟動儀表板。技術上，這個專案使用了 Next.js、MapLibre GL 和 FastAPI，確保了前後端的高效協作。

這樣的設計使得用戶能夠靈活地選擇所需的數據層，並且能夠在地圖上右鍵點擊獲取特定區域的詳細信息。與其他類似工具相比，如 AlpinDale/parsync 和其他 OSINT 工具，ShadowBroker 提供了更全面的數據整合和即時更新能力，特別適合分析師和研究者使用。實際使用中，儀表板能夠處理大量的數據流，並且支援多種數據來源的即時更新，這對於需要快速反應的情境尤為重要。這個專案目前處於穩定階段，值得在非關鍵環境中使用，並且未來可能會進一步擴展其數據來源和功能。對於需要即時情報的團隊，這是一個非常適合的選擇，但在需要高可用性和擴展性的情況下，可能需要考慮其他解決方案。

**技術棧**：`Next.js` · `MapLibre GL` · `FastAPI` · `Python`

## 重點功能

- 即時數據整合 — 整合 60+ 個即時情報來源，提供全面的地理空間視圖。
- 模組化架構 — 獨立擴展情報後端和前端，根據需求調整。
- 無需用戶數據 — 儀表板完全在本地運行，無需收集用戶數據。
- 多層數據切換 — 提供 35+ 可切換的數據層，靈活滿足用戶需求。
- Docker 支援 — 透過 Docker 快速啟動，簡化安裝過程。
...
```ts
{
  ...
  "預期輸出": "儀表板將在 http://localhost:3000 上運行"
}
```
...
> [!tip] 爆紅原因
> 建立 29 天內累積 5151 stars（178/天），forks 773（15.0%），這顯示出強烈的社群興趣。這個專案的主要貢獻者來自於多個背景，並且解決了過去開源情報工具無法有效整合多個數據來源的痛點。技術上，這個專案的即時更新能力和模組化設計使其在同類工具中脫穎而出。社群的活躍度和開放的問題回應率（95%）也顯示出其良好的維護狀態。

## 適合誰使用

**目標受眾**：需要即時追蹤全球事件並整合多個情報來源的安全分析師和研究者。

> [!example] 使用場景
> - 情報分析師用它來追蹤全球軍事活動，因為它能即時整合多個數據來源，提供全面的情境分析。
> - 研究人員用它來分析氣候變化影響，因為它提供了全球地震和氣象數據的即時更新。
> - 無線電操作員用它來監控即時通訊，因為它能顯示多個無線電頻道的實時數據，便於快速反應。

## 架構分析

ShadowBroker 採用模組化架構，前端使用 Next.js 和 MapLibre GL，後端使用 FastAPI 和 Python，這樣的設計使得前後端能夠獨立擴展。數據流通過多個即時來源進行更新，並在用戶界面上實時顯示。這種設計的優勢在於能夠快速適應不同的數據需求，但可能在處理大量數據時面臨性能瓶頸。整體架構的輕量化使得部署和維護變得簡單，但在高負載情況下可能需要考慮擴展方案。

## 技術深入分析

ShadowBroker 的核心技術機制是將多個即時數據來源整合到一個儀表板中，使用了 Next.js 和 FastAPI 來實現前後端的高效協作。這種設計使得用戶能夠靈活選擇所需的數據層，並且能夠在地圖上右鍵點擊獲取特定區域的詳細信息。效能方面，儀表板能夠處理大量的數據流，並且支援多種數據來源的即時更新，這對於需要快速反應的情境尤為重要。選擇使用 Docker 來簡化部署過程，這樣的選擇使得用戶能夠快速啟動和更新，但在高負載情況下可能需要考慮擴展方案。技術風險方面，隨著數據來源的增加，可能會面臨數據一致性和更新延遲的問題。整合分析方面，這個專案與主流框架的整合難度較低，並且能夠輕鬆融入現有的 CI/CD 流程中。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且詳細，提供了完整的安裝和使用指導。安裝過程順暢，Docker 設置簡單。提供了良好的入門指南，讓新手能夠快速上手。文件目前僅提供英文版本。
...
| 工具 | 差異 |
| --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 提供類似的數據同步功能，但缺乏即時更新和多數據來源整合的能力。 |
| [ChinaSiro/claude-code-sourcemap](https://github.com/ChinaSiro/claude-code-sourcemap) | 專注於代碼分析，無法提供地理空間情報的即時視圖。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 使用 Python 實作，專注於數據同步，但缺乏即時更新能力。 | 如果你的團隊需要一個專注於數據同步的工具，而不是即時情報視圖。 | medium，因為需要重新設計數據流和更新機制。 |
| [ChinaSiro/claude-code-sourcemap](https://github.com/ChinaSiro/claude-code-sourcemap) | 專注於代碼分析，無法提供地理空間情報的即時視圖。 | 如果你的需求主要集中在代碼分析而非地理情報。 | low，因為代碼分析工具通常不需要改變現有的工作流。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **Shadowbroker** | **parsync** | **claude-code-sourcemap** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 使用 Python 實作，專注於數據同步，但缺乏即時更新能力。 | 專注於代碼分析，無法提供地理空間情報的即時視圖。 |
> | 遷移成本 | - | medium，因為需要重新設計數據流和更新機制。 | low，因為代碼分析工具通常不需要改變現有的工作流。 |
> | 適用場景 | 主要場景 | 如果你的團隊需要一個專注於數據同步的工具，而不是即時情報視圖 | 如果你的需求主要集中在代碼分析而非地理情報。 |
...
- **[HIGH]** 在 Docker 中啟動前端時可能會遇到問題
  - 解法：檢查 Docker 配置和依賴版本
- [MEDIUM] 某些數據來源的 API 金鑰可能需要額外配置
  - 解法：參考文檔中的 API 配置部分
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的小型安全分析團隊 | 非常適合 | 模組化架構和即時數據更新非常符合小型團隊的需求。 |
| 大型企業的安全部門 | 適合 | 能夠處理大量數據，但需要考慮擴展性。 |
| 學術研究機構 | 普通 | 提供的數據來源可能不夠專業化，需根據需求調整。 |
| 需要即時反應的應急管理團隊 | 不適合 | 可能在高負載情況下面臨性能瓶頸。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，3 小時整合，得到即時情報的全面視圖，值得嘗試。

## 安全性評估

> [!warning] 安全性快速掃描
> 中等風險：儀表板本身不需要高權限，但需要處理多個外部 API 的金鑰，需確保這些金鑰的安全性。依賴鏈的信任程度需進一步評估，特別是在生產環境中使用時。
...
> | [@BigBodyCobain](https://github.com/BigBodyCobain) | 75 |
...
> | --- | --- | --- | --- |
> | [#127](https://github.com/BigBodyCobain/Shadowbroker/issues/127) | Install from readme.md, Front end will not start on docker. | 0 | 2 |
> | [#126](https://github.com/BigBodyCobain/Shadowbroker/issues/126) | attempting to just use the package as detailed in the README | 0 | 3 |
...
> | [NASA GIBS](https://gibs.earthdata.nasa.gov) | MODIS Terra daily satellite imagery | Daily (24-48h delay) |

## 延伸閱讀

相關概念：[[OSINT]] · [[地理空間分析]] · [[即時數據流]]

相關專案：[[AlpinDale--parsync|AlpinDale/parsync]] · [[ChinaSiro--claude-code-sourcemap|ChinaSiro/claude-code-sourcemap]] · [[FreedomIntelligence--OpenClaw-Medical-Skills|FreedomIntelligence/OpenClaw-Medical-Skills]] · [[Flowseal--tg-ws-proxy|Flowseal/tg-ws-proxy]] · [[L42ARO--Mercury-Transforming-Drone|L42ARO/Mercury-Transforming-Drone]] · [[Minecraft-Community-Edition--client|Minecraft-Community-Edition/client]] · [[joeseesun--qiaomu-mondo-poster-design|joeseesun/qiaomu-mondo-poster-design]] · [[HKUDS--CLI-Anything|HKUDS/CLI-Anything]]
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["OSINT","地理空間分析","即時數據流"];
