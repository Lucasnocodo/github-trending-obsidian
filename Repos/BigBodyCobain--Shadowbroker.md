...
use_case: "提供全球即時開源情報，整合多種數據來源於單一介面，讓使用者能追蹤各種公共信號。"
priority: medium
ring: assess
...
aliases:
  ...
  - "提供全球即時開源情報，整合多種數據來源於單一介面，讓使用者能追蹤各種公共信號。"
---

# Shadowbroker

**5.2k** stars · **136** stars/天 · 建立 38 天前 · TypeScript · AGPL-3.0
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Growing · **熱度** Hot (136 stars/day)
> **授權** AGPL-3.0 (Copyleft，商用需注意) · **維護** Moderate (最後推送 8 天前) · **貢獻者** 5+ 人 · **參與度** Medium
> **適合** 需要即時追蹤公共信號並整合多種數據來源的分析師和研究人員。
> **一句話重點** ShadowBroker 的強大在於其即時整合多種公共信號，讓使用者能在一個介面上獲得全面的情報。
...
> **結論** 花 5 小時學習，3 小時整合，能夠獲得即時的開源情報，值得考慮。

> [!abstract] 核心創新
> ShadowBroker 是一個即時多域的 OSINT 儀表板，將多個實時情報來源融合到一個黑暗操作地圖介面中。

## 專案簡介

ShadowBroker 是一個即時多域的開源情報平台，將超過 60 條實時情報來源融合到一個黑暗操作地圖介面中。使用者可以追蹤飛機、船隻、衛星、衝突區域、CCTV 網絡、GPS 干擾、互聯網設備、警用掃描器等，所有數據都在一個螢幕上實時更新。這個工具的賣點在於其模組化架構，前端和後端可以獨立擴展，並且完全在用戶的瀏覽器中運行，無需收集或傳輸用戶數據。使用者只需執行 `docker compose up -d` 即可啟動儀表板，並通過 `http://localhost:3000` 訪問。

技術上，這個專案使用了 Next.js、MapLibre GL 和 FastAPI，並且在安全性上採取了限制 UID 的運行方式，確保容器的安全性。與其他 OSINT 工具相比，ShadowBroker 提供了更為豐富的數據層和即時更新功能，特別適合需要即時情報的分析師和研究人員。這個工具的社群活躍度高，開發者對於問題的回應速度快，並且有著 98% 的問題解決率。對於需要即時追蹤公共信號的團隊來說，這是一個值得考慮的選擇。

**技術棧**：`Next.js` · `MapLibre GL` · `FastAPI` · `Python`

## 重點功能

- 即時數據更新 — 整合超過 60 條實時情報來源，所有數據在一個介面上即時更新。
- 模組化架構 — 前端和後端可以獨立擴展，便於根據需求調整。
- 安全性設計 — 運行在限制 UID 的環境中，增強容器的安全性。
- 無需用戶數據 — 儀表板完全在用戶的瀏覽器中運行，無需收集或傳輸用戶數據。
- 多層數據切換 — 提供 35+ 可切換的數據層，使用者可根據需求選擇顯示的數據。
...
> [!tip] 爆紅原因
> 建立 38 天就累積 5180 stars（136/天），forks 776（15.0%），顯示出強烈的社群興趣。作者 BigBodyCobain 和其團隊在 OSINT 領域有著豐富的經驗，解決了傳統 OSINT 工具無法整合多種數據來源的痛點。這個工具的出現正值全球對於即時情報需求增加的背景下，特別是在地緣政治緊張的情況下。高達 15% 的 forks/stars 比率顯示出許多開發者正在實際修改和使用這個工具，代表著其實用性和潛在的擴展性。

## 適合誰使用

**目標受眾**：需要即時追蹤公共信號並整合多種數據來源的分析師和研究人員。

> [!example] 使用場景
> - 情報分析師用它來實時追蹤全球的飛機和船隻動態，因為它整合了多個數據來源，讓分析師能快速獲得全面的情況。
> - 研究人員用它來觀察地震事件和其他自然災害的即時數據，因為其提供的即時更新功能能幫助他們快速做出反應。
> - 安全專家用它來監控 CCTV 網絡和警用掃描器的活動，因為其界面友好且數據層豐富，能有效提高監控效率。

## 架構分析

ShadowBroker 採用模組化架構，前端使用 Next.js 和 MapLibre GL，後端則使用 FastAPI 和 Python。這種設計使得前端和後端可以獨立擴展，滿足不同規模的需求。數據流從多個實時情報來源進入後端，經過處理後再發送到前端顯示。

這樣的設計使得系統能夠快速響應用戶的請求，並且在數據量增加時仍能保持性能。選擇使用 FastAPI 是因為其高效的異步處理能力，這對於即時數據更新至關重要。這種架構的擴展性良好，但在處理超大規模數據時可能會遇到性能瓶頸，特別是在數據來源過多的情況下。

## 技術深入分析

ShadowBroker 的核心技術機制在於其即時數據更新能力，通過整合 60 多個實時情報來源，使用者能夠在一個界面上獲得豐富的情報。這個系統能夠處理來自飛機、船隻、衛星等多種來源的數據，並且在用戶操作時提供即時反饋。由於使用了 FastAPI，系統能夠高效地處理請求，並且支持異步操作，這對於即時數據更新至關重要。設計上，選擇使用 Next.js 和 MapLibre GL 來構建前端，這使得用戶界面更加友好且易於操作。這種架構的優勢在於其擴展性，但在數據量過大時可能會出現性能瓶頸。技術風險方面，隨著數據來源的增加，系統可能會面臨更高的維護成本和性能挑戰。整合方面，與主流 CI/CD 工具的兼容性良好，能夠輕鬆融入現有的開發流程。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且提供了詳細的安裝步驟，安裝過程相對順暢，但需要用戶熟悉 Docker。提供了良好的快速入門指南，讓新手能夠快速上手。文件目前僅提供英文版本，可能對非英語使用者造成一定困難。
...
> [!danger] 缺點
> - 需要 Docker 環境，對於不熟悉 Docker 的使用者有學習曲線
> - 不支援 Windows 系統的直接啟動
> - 對於大型數據集的性能可能不佳

> [!warning] 注意事項
> - 需要 Docker 環境才能運行
> - 不支援 Windows 系統的直接啟動，需使用 Docker
> - 對於大型數據集的處理可能會有性能瓶頸
...
| 工具 | 差異 |
| --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供類似的 OSINT 功能，但主要集中在網絡流量和數據包分析，適合需要深入網絡分析的使用者。 |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 專注於文件同步和傳輸，與 ShadowBroker 的即時數據更新功能有明顯區別。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於網絡流量和數據包分析，提供的功能更偏向於深度網絡分析。 | 如果你的需求主要集中在網絡流量的監控和分析上，則應選擇這個工具。 | medium，因為需要重新適應不同的數據來源和分析方式。 |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 專注於文件同步和傳輸，與 ShadowBroker 的即時數據更新功能有明顯區別。 | 如果你的主要需求是文件同步而非即時情報，則應選擇這個工具。 | low，因為功能差異不大，容易上手。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **Shadowbroker** | **boneyard** | **parsync** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於網絡流量和數據包分析，提供的功能更偏向於深度網絡分析。 | 專注於文件同步和傳輸，與 ShadowBroker 的即時數據更新功能有明顯區別。 |
> | 遷移成本 | - | medium，因為需要重新適應不同的數據來源和分析方式。 | low，因為功能差異不大，容易上手。 |
> | 適用場景 | 主要場景 | 如果你的需求主要集中在網絡流量的監控和分析上，則應選擇這個工 | 如果你的主要需求是文件同步而非即時情報，則應選擇這個工具。 |
...
- **[HIGH]** 在高負載情況下，可能會導致性能下降
  - 解法：考慮擴展後端服務以分擔負載
- [MEDIUM] Docker 環境配置不當可能導致啟動失敗
  - 解法：確保 Docker 和 Docker Compose 正確安裝和配置
- [MEDIUM] 某些數據源可能會因為 API 限制而無法獲取
  - 解法：定期檢查數據源的可用性和限制
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司進行市場調查 | 非常適合 | 即時數據更新和多層數據切換能幫助快速獲得市場情報。 |
| 大型企業的安全監控中心 | 普通 | 雖然功能豐富，但在處理超大規模數據時可能性能不足。 |
| 學術研究機構進行地緣政治分析 | 非常適合 | 提供即時的地緣政治數據，對於研究分析非常有幫助。 |
| 個人使用者進行日常新聞追蹤 | 適合 | 能夠快速獲取各類公共信號，適合日常使用。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，3 小時整合，能夠獲得即時的開源情報，值得考慮。
...
相關專案：[[0xGF--boneyard|0xGF/boneyard]] · [[AlpinDale--parsync|AlpinDale/parsync]] · [[ChinaSiro--claude-code-sourcemap|ChinaSiro/claude-code-sourcemap]] · [[FreedomIntelligence--OpenClaw-Medical-Skills|FreedomIntelligence/OpenClaw-Medical-Skills]] · [[Flowseal--tg-ws-proxy|Flowseal/tg-ws-proxy]] · [[L42ARO--Mercury-Transforming-Drone|L42ARO/Mercury-Transforming-Drone]] · [[Minecraft-Community-Edition--client|Minecraft-Community-Edition/client]] · [[joeseesun--qiaomu-mondo-poster-design|joeseesun/qiaomu-mondo-poster-design]]
