...
use_case: "一個輕量級的 Cloudflare IP 掃描器，專為不穩定的網絡設計。"
priority: high
ring: assess
...
aliases:
  ...
  - "一個輕量級的 Cloudflare IP 掃描器，專為不穩定的網絡設計。"
---

# SenPaiScanner

**905** stars · **181** stars/天 · 建立 5 天前 · Go · MIT
...
> **適合** 需要在不穩定網絡環境中尋找可用 Cloudflare IP 的網路工程師。
> **一句話重點** 這個工具的設計讓尋找可用的 Cloudflare IP 變得簡單直觀，特別適合在不穩定的網絡環境中使用。
...
> **結論** 花 2 小時學習，1 小時整合，得到快速找到可用 Cloudflare IP 的效果，值得投入。

> [!abstract] 核心創新
> 提供了一個簡單的終端介面來掃描和驗證 Cloudflare IP，無需記憶命令。

## 專案簡介

SenPai Scanner 是一個專為不穩定網絡環境設計的 Cloudflare IP 掃描工具，使用者可以透過簡單的終端介面來尋找可用的 Cloudflare IP。使用者只需粘貼 VLESS 或 Trojan 配置，選擇設置後，工具會自動找到可用的 IP。該工具的核心機制分為兩個階段：第一階段進行連通性掃描，使用者可以選擇隨機 IP 或從文件中讀取 IP，並設置並行探測數量、超時時間及端口；第二階段則是通過嵌入的 xray 實例進行端到端的驗證，測試最佳的 Phase 1 結果。這樣的設計讓使用者能夠在不需要記憶命令的情況下，快速找到有效的 IP，並且即時查看結果。

**技術棧**：`Go 1.26.3` · `Bubble Tea` · `Charmbracelet`

## 重點功能

- 兩階段掃描 — 第一階段進行連通性掃描，第二階段使用 xray 進行端到端驗證。
- 可配置的掃描參數 — 使用者可以自定義 IP 數量、並行工作者、超時時間及端口。
- 即時結果更新 — 每次掃描結果會即時寫入 `SenPaiScannerResult-YYYYMMDD-HHMMSS.txt` 文件。
- 支持從文件讀取 IP — 使用者可以將 IP 列表放在 `ips.txt` 文件中進行掃描。
- 簡單的終端介面 — 使用者可以透過鍵盤操作輕鬆導航，無需記憶命令。
...
```bash
在 TUI 中選擇 'Find Working IPs' 並粘貼配置
```
...
> [!tip] 爆紅原因
> 建立 5 天內累積 905 stars（181/天），forks 59（6.5%），顯示出不錯的增長潛力。這個專案的作者 MatinSenPai 之前有開發過類似的工具，解決了在不穩定網絡環境中尋找可用 Cloudflare IP 的痛點，這在現有的工具中並不多見。社群的反饋也顯示出對於掃描過程中的穩定性和即時結果的需求，這使得這個工具受到了關注。

## 適合誰使用

**目標受眾**：需要在不穩定網絡環境中尋找可用 Cloudflare IP 的網路工程師。

> [!example] 使用場景
> - 網路工程師用它來快速找到可用的 Cloudflare IP，因為手動測試 IP 既耗時又容易出錯。
> - DevOps 工程師用它來在 CI/CD 流程中自動化 IP 掃描，因為這樣可以減少人工干預，提高部署效率。
> - 研究人員用它來收集 Cloudflare IP 數據，因為它提供了即時的結果和可視化界面，方便分析。

## 架構分析

SenPai Scanner 採用 Go 語言編寫，並使用了 Bubble Tea 框架來實現終端用戶介面。這樣的設計使得工具在不同平台上都能保持一致的使用體驗。資料流方面，掃描過程分為兩個階段，第一階段使用 HTTP probe 進行連通性檢查，第二階段則是通過 xray 進行詳細的驗證。這種設計能夠有效地檢查 IP 的可用性，並且能夠即時反饋結果。選擇 Go 語言的原因在於其高效的並行處理能力，能夠在不穩定的網絡環境中保持良好的性能。

## 技術深入分析

SenPai Scanner 的核心技術機制是基於 Go 語言的並行處理能力，使用 Bubble Tea 框架來實現終端用戶介面。掃描過程中，第一階段使用 HTTP probe 檢查 IP 的連通性，這是因為 Cloudflare 的邊緣 IP 會丟棄 ICMP 封包，傳統的 ping 命令無法有效檢查可用性。第二階段則是通過嵌入的 xray 實例進行端到端的驗證，這樣的設計能夠更接近實際使用情況，提供更準確的結果。效能方面，該工具能夠在不穩定的網絡環境中保持良好的性能，並且能夠處理大量的 IP 掃描。設計選擇上，Go 語言的高效性和並行處理能力使得這個工具在性能上有很大的優勢，依賴樹相對簡單，主要依賴於 Bubble Tea 和其他輕量級庫。技術風險方面，隨著使用者數量的增加，可能會面臨性能瓶頸，特別是在高並發的情況下。整合方面，該工具能夠輕鬆與現有的網絡工具鏈相容，並且支持在 CI/CD 流程中使用。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了詳細的安裝和使用說明，並包含範例指令。安裝過程順暢，無明顯坑點。文件目前僅提供英文版本，對於非英語使用者可能有一定的學習曲線。
...
> [!danger] 缺點
> - 僅支援 Linux 和 macOS，Windows 用戶需額外步驟。
> - 對於大量端口的掃描，可能會導致速度變慢。
> - 需要手動配置 `ips.txt` 文件，對新手不太友好。

> [!warning] 注意事項
> - 僅支援 Linux 和 macOS 平台，Windows 用戶需使用 PowerShell 下載。
> - 對於大量端口的掃描，可能會導致掃描速度變慢。
> - 需要手動配置 `ips.txt` 文件以從文件中讀取 IP。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 這個工具專注於代理服務器的管理，而 SenPai Scanner 專注於 Cloudflare IP 的掃描和驗證。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard 提供了更全面的網絡測試功能，但使用上較為複雜，SenPai Scanner 則以簡單易用為主。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 這個工具專注於代理服務器的管理，而 SenPai Scanner 專注於 Cloudflare IP 的掃描和驗證。 | 如果你需要一個全面的代理管理工具，而不僅僅是 IP 掃描，則可以選擇這個工具。 | medium，因為需要重新學習新的操作流程。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard 提供了更全面的網絡測試功能，但使用上較為複雜，SenPai Scanner 則以簡單易用為主。 | 如果你需要進行更複雜的網絡測試，Boneyard 可能更合適。 | high，因為功能和操作方式差異較大。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **SenPaiScanner** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 這個工具專注於代理服務器的管理，而 SenPai Scanner 專注於 Cloudflare IP 的掃描和驗證。 | Boneyard 提供了更全面的網絡測試功能，但使用上較為複雜，SenPai Scanner 則以簡單易用為主。 |
> | 遷移成本 | - | medium，因為需要重新學習新的操作流程。 | high，因為功能和操作方式差異較大。 |
> | 適用場景 | 主要場景 | 如果你需要一個全面的代理管理工具，而不僅僅是 IP 掃描，則 | 如果你需要進行更複雜的網絡測試，Boneyard 可能更合適 |
...
- **[HIGH]** 在使用 From File 功能時，若未將 `ips.txt` 放在正確位置會導致掃描失敗。
  - 解法：確保 `ips.txt` 文件與可執行文件在同一目錄下。
- [MEDIUM] 掃描速度在選擇多個端口時會顯著下降。
  - 解法：減少選擇的端口數量以提高速度。
- [MEDIUM] 某些 IP 可能會因為防火牆規則而無法通過掃描。
  - 解法：嘗試使用不同的端口或 IP 範圍。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 該工具能快速找到可用的 Cloudflare IP，適合小型團隊的需求。 |
| 大型企業的網絡安全測試 | 普通 | 雖然功能強大，但在高並發情況下可能會遇到性能瓶頸。 |
| 個人開發者的網絡工具 | 非常適合 | 輕量級且易於使用，適合個人開發者進行快速測試。 |
| 需要穩定性和高效能的商業應用 | 不適合 | 目前仍處於 beta 階段，尚未完全穩定。 |
...
> [!tip] 投入 vs 回報
> 花 2 小時學習，1 小時整合，得到快速找到可用 Cloudflare IP 的效果，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限，且不存取敏感資料。依賴的庫也相對安全，無已知的供應鏈風險。
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["CLI/TUI","網路掃描","代理服務"];
