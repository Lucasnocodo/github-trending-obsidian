...
use_case: "輕量級的 Cloudflare IP 掃描器，使用 Golang 實作。"
priority: high
ring: assess
...
star_history: "2026-05-31:595,2026-05-31:595,2026-06-01:802,2026-06-01:802,2026-06-02:857,2026-06-02:857,2026-06-03:905,2026-06-03:905,2026-06-04:1032,2026-06-04:1032,2026-06-05:1162,2026-06-06:1254"
tags:
  - github
  - "category/開發工具"
  - "lang/go"
...
aliases:
  ...
  - "輕量級的 Cloudflare IP 掃描器，使用 Golang 實作。"
---

# SenPaiScanner

**1.3k** stars · **157** stars/天 · 建立 8 天前 · Go · MIT
...
> **適合** 需要在不穩定網路環境中快速找到可用 Cloudflare IP 的網路工程師。
> **一句話重點** 這個工具的設計讓 IP 掃描變得前所未有的簡單，特別是在不穩定的網路環境中。
...
> **結論** 花 2 小時學習，1 小時整合，得到快速掃描可用 IP 的效果，值得使用。

> [!abstract] 核心創新
> 提供即時的掃描結果和簡單的配置選項，特別適合不穩定網路環境。

## 專案簡介

SenPai Scanner 是一個專為 Cloudflare IP 掃描設計的工具，提供簡單的終端用戶介面。用戶可以選擇隨機生成 IP 或從檔案中讀取，並設定掃描的 IP 數量、工作線程、超時時間及端口。掃描過程分為兩個階段：第一階段進行連通性檢查，第二階段則可選擇進行更深入的驗證，利用內嵌的 xray 實例來測試 IP 的有效性。這樣的設計使得用戶能夠快速獲得可用的 Cloudflare IP，並且透過簡單的配置來適應不同的網路環境。關鍵指令如 `senpaiscanner` 可以直接啟動掃描，並且不需要記憶任何複雜的命令。這個工具的賣點在於其簡潔的使用方式與即時的結果反饋，特別適合需要快速驗證 IP 可用性的使用者。

**技術棧**：`Go 1.26.1` · `xray-core`

## 重點功能

- 多種掃描來源 — 支持隨機生成或從檔案中讀取 IP。
- 可調整的掃描參數 — 包括 IP 數量、工作線程、超時時間及端口設定。
- 兩階段掃描 — 第一階段進行連通性檢查，第二階段可選擇進行 xray 驗證。
- 即時結果反饋 — 掃描過程中可隨時查看結果，並可將有效 IP 複製到剪貼簿。
- 跨平台支持 — 提供 Linux、macOS 和 Windows 的預編譯二進制檔案。
...
> [!tip] 爆紅原因
> 建立 8 天就累積 1254 stars（157/天），forks 75（6.0%），顯示出穩定的增長。作者 MatinSenPai 之前有開發多個相關工具，這個專案解決了用戶在不穩定網路環境下快速找到可用 Cloudflare IP 的需求。此工具的推出正值許多開發者需要穩定的 IP 來源以支持各種應用，並且在社群中引發了討論。高 forks/stars 比率顯示許多人在積極修改和使用此工具，而非僅僅觀望。

## 適合誰使用

**目標受眾**：需要在不穩定網路環境中快速找到可用 Cloudflare IP 的網路工程師。

> [!example] 使用場景
> - 網路工程師用它來快速掃描可用的 Cloudflare IP，因為手動檢查 IP 的效率低且耗時。
> - DevOps 工程師用它來在 CI/CD 流程中自動檢測可用的 Cloudflare IP，因為這樣可以減少部署過程中的網路問題。
> - 安全研究人員用它來驗證特定 IP 是否可用，因為這樣可以快速獲得有效的測試環境。

## 架構分析

SenPai Scanner 採用 Go 語言實作，設計為輕量級的掃描工具，主要透過終端用戶介面進行操作。架構上，掃描過程分為兩個階段，第一階段使用 HTTP probe 檢查 IP 的可達性，第二階段則透過內嵌的 xray 實例進行更深入的驗證。這樣的設計使得用戶能夠靈活選擇掃描模式，並且快速獲得結果。選擇 Go 語言的原因在於其高效能和簡單的佈署特性，這使得工具能夠在多種平台上運行而不需要複雜的依賴管理。整體架構的輕量化設計也使得其在資源有限的環境中表現良好。

## 技術深入分析

SenPai Scanner 的核心技術在於其使用 Go 語言的高效能和簡潔性，並且透過 TUI 提供直觀的使用體驗。掃描過程中，第一階段的連通性檢查使用了標準的 HTTP probe，這樣的設計使得用戶可以快速獲得可用的 IP。第二階段的 xray 驗證則是通過內嵌的 xray 實例來進行，這樣的設計不僅提高了驗證的準確性，也減少了用戶的操作負擔。從效能上來看，這個工具能夠在多達 200 個工作線程下運行，這使得其在大規模掃描時仍然保持高效。選擇 Go 語言的原因在於其原生支持並發，這使得工具能夠在多核處理器上充分發揮效能。技術風險方面，隨著使用者數量的增加，可能會出現性能瓶頸，特別是在高流量的網路環境中。整合方面，這個工具可以輕鬆與現有的 CI/CD 流程結合，並且支持多種平台，降低了整合的難度。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且包含詳細的安裝步驟，安裝過程順暢且無明顯坑。提供了良好的入門指南，讓新手能夠快速上手。文件目前僅提供英文版本，可能對非英語使用者造成一定困難。
...
> [!danger] 缺點
> - 僅限於 Cloudflare IP，功能範圍有限。
> - 在高流量環境下可能影響掃描效率。
> - 對於新手用戶可能需要一些學習曲線。

> [!warning] 注意事項
> - 僅支持 Cloudflare IP 的掃描，對其他 IP 範圍無法使用。
> - 需要穩定的網路連接以獲得準確的掃描結果。
> - 在高流量環境下可能會受到限制，建議調整工作線程數量。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 這個工具專注於代理伺服器的管理，而 SenPai Scanner 專注於 Cloudflare IP 的掃描，適用場景不同。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard 提供更全面的網路掃描功能，但配置較為複雜，SenPai Scanner 提供更簡單的使用體驗。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於代理伺服器的管理，提供更全面的網路掃描功能。 | 如果需要管理多個代理伺服器並進行複雜的網路配置，則應選擇此工具。 | medium，因為需要重新學習工具的使用方式。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供更複雜的網路掃描功能，但配置較為繁瑣。 | 如果需要進行全面的網路掃描並不介意複雜的配置，則應選擇此工具。 | high，因為需要適應全新的操作流程。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **SenPaiScanner** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於代理伺服器的管理，提供更全面的網路掃描功能。 | 提供更複雜的網路掃描功能，但配置較為繁瑣。 |
> | 遷移成本 | - | medium，因為需要重新學習工具的使用方式。 | high，因為需要適應全新的操作流程。 |
> | 適用場景 | 主要場景 | 如果需要管理多個代理伺服器並進行複雜的網路配置，則應選擇此工 | 如果需要進行全面的網路掃描並不介意複雜的配置，則應選擇此工具 |
...
- **[HIGH]** 在高流量環境下可能導致掃描效率降低
  - 解法：調整工作線程數量以適應網路狀況
- [MEDIUM] 某些 IP 可能因為防火牆設定而無法掃描
  - 解法：嘗試使用不同的端口或來源進行掃描
- [MEDIUM] 配置 URL 格式不正確會導致掃描失敗
  - 解法：確保 URL 格式正確，並檢查是否有額外的空格或錯誤字符

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型團隊的網路測試專案 | 非常適合 | 簡單易用，能快速獲得可用 IP。 |
| 大型企業的網路監控系統 | 普通 | 雖然功能強大，但可能不夠全面。 |
| 個人開發者的測試環境 | 非常適合 | 輕量級且易於配置，適合快速驗證。 |
| 需長期運行的生產環境 | 不適合 | 目前仍在 beta 階段，穩定性不足。 |
...
> [!tip] 投入 vs 回報
> 花 2 小時學習，1 小時整合，得到快速掃描可用 IP 的效果，值得使用。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限，僅需網路存取權限，並不存取敏感資料。依賴鏈中無已知的供應鏈風險，適合在 CI/CD pipeline 中使用。
...
> | 貢獻者 | Commits |
> | --- | --- |
> | [@MatinSenPai](https://github.com/MatinSenPai) | 31 |
...
> [!abstract] 最近 10 次 commit（2026-06-04 ~ 2026-06-05）
> **活躍天數** 2 天 · **最新 commit** feat: enhance probe functionality with fallback SNI and improved trace handling
...
> | [#75](https://github.com/MatinSenPai/SenPaiScanner/issues/75) | گیر کردن اسکنر | 1 | 1 |
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["網路掃描","IP 探測","Cloudflare"];
