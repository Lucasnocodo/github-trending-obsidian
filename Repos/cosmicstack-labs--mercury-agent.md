...
star_history: "2026-04-26:1117,2026-04-26:1124,2026-04-27:1416,2026-04-27:1422,2026-04-28:1549,2026-04-29:1649,2026-04-30:1774,2026-05-01:1837,2026-05-02:1880,2026-05-03:1910,2026-05-04:1946,2026-05-05:1962,2026-05-06:1974,2026-05-07:2008,2026-05-08:2020,2026-05-09:2034"
tags:
  - github
  - "category/ai_ml"
  - "lang/typescript"
...
**2.0k** stars · **113** stars/天 · 建立 18 天前 · TypeScript · MIT
...
> **適合** 需要一個能夠持續運行並具備權限管理的 AI 助手的開發者和專業人士。
> **一句話重點** Mercury 的設計強調用戶控制和持續運行，這在現有的 AI 助手中是相對少見的。
...
> **結論** 花 2 小時學習，3 小時整合，得到一個靈活的 AI 助手，值得一試。

> [!abstract] 核心創新
> Mercury 提供了靈魂驅動的 AI 助手，具備權限管理和持續運行的獨特設計。

## 專案簡介

Mercury 是一個以「靈魂驅動」的 AI 助手，能夠在 CLI 或 Telegram 上持續運行，並具備多種內建工具和擴展技能。用戶可以通過簡單的指令如 `npx @cosmicstack/mercury-agent` 來啟動，首次運行會觸發設置精靈，並要求用戶選擇權限模式（如 `Ask Me` 或 `Allow All`）。這個設計使得 Mercury 在執行任務時不會隨意行動，而是會詢問用戶的許可，這樣可以避免意外操作。技術上，Mercury 使用 SQLite 作為其「第二大腦」的記憶系統，支持多種記憶類型和自動提取功能，這使得它能夠學習用戶的偏好和習慣。

與其他 AI 助手相比，如 OpenAI 的 ChatGPT，Mercury 提供了更強的權限管理和記憶功能，並且能夠在多個通道（CLI 和 Telegram）上運行。這使得它在需要長時間運行的任務中表現優異，並且能夠在崩潰後自動重啟。使用者可以透過 `mercury up` 指令將其設置為系統服務，實現自動啟動和背景運行。這種設計使得 Mercury 成為一個理想的長期助手，特別適合需要持續監控和執行任務的場景。

**技術棧**：`TypeScript` · `SQLite` · `Ink`

## 重點功能

- 權限管理 — 使用者可選擇 `Ask Me` 或 `Allow All` 模式，確保每次操作都經過確認。
- 第二大腦記憶系統 — 使用 SQLite 存儲和管理記憶，支持多種記憶類型和自動提取功能。
- 多通道支持 — 同時在 CLI 和 Telegram 上運行，提供靈活的訪問方式。
- 持續運行 — 可設置為系統服務，自動啟動並在崩潰後自動重啟。
- 內建工具 — 提供多達 31 種內建工具，支持文件操作、命令執行和任務調度等功能。

## 快速開始

1. 使用 npx 安裝並啟動 Mercury
```bash
...
> [!tip] 爆紅原因
> 建立 18 天內累積 2034 stars（113/天），forks 212（10.4%），顯示出強勁的增長潛力。作者 Cosmic Stack 團隊在 AI 和開源工具方面有豐富的經驗，這個專案解決了現有 AI 助手在權限管理和持續運行方面的不足。特別是，Mercury 的設計讓用戶能夠在執行任務時保持控制，這在許多 AI 應用中是缺失的。社群的反饋也顯示出對於多通道支持的需求，這可能是其快速增長的原因之一。

## 適合誰使用

**目標受眾**：需要一個能夠持續運行並具備權限管理的 AI 助手的開發者和專業人士。

> [!example] 使用場景
> - 開發者用它來在 CLI 中自動化重複性任務，因為 Mercury 的權限管理讓他們可以安全地執行命令而不必擔心意外操作。
> - 產品經理用它來管理日常任務和會議記錄，因為它的第二大腦功能能夠持續記錄和提取重要資訊。
> - 系統管理員用它來監控伺服器狀態，因為它能夠在背景運行並自動重啟，確保服務的持續可用性。

## 架構分析

Mercury 採用模組化設計，核心功能使用 TypeScript 實現，並依賴 SQLite 作為記憶存儲。這種設計使得 Mercury 能夠在多平台上運行，並能夠輕鬆擴展新功能。資料流方面，所有的用戶數據和配置存儲在 `~/.mercury/` 目錄下，這樣可以保持用戶的數據獨立於專案。選擇 SQLite 作為存儲解決方案的好處在於其輕量和易於管理，但在處理大規模數據時可能會出現性能瓶頸。整體來看，這種設計使得 Mercury 在功能上靈活且易於維護，但在高負載情況下可能需要進一步的優化。

## 技術深入分析

Mercury 的核心技術機制是基於 TypeScript 和 SQLite，這使得它在開發和維護上都相對簡單。使用 SQLite 作為記憶系統的選擇，讓它能夠高效地存儲和檢索用戶數據，並支持全文本搜索。效能方面，Mercury 能夠處理日常任務和長期記憶，但在高並發的情況下可能會遇到性能瓶頸。設計上，選擇了模組化的架構，使得新功能的添加變得容易，但這也可能導致依賴關係的複雜性增加。技術風險方面，Mercury 依賴於外部 API 進行許多操作，這可能會在 API 變更時導致問題。整合方面，Mercury 可以輕鬆與現有的 CLI 工具鏈結合，但在與大型系統的整合上可能需要額外的適配層。整體而言，Mercury 提供了一個靈活且強大的 AI 助手，但在某些情況下可能需要進一步的優化和調整。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了詳細的安裝和使用指導。安裝過程相對順暢，沒有明顯的坑。文件中有提供多語言選擇，但中文部分尚需改進。
...
> [!danger] 缺點
> - 對於中文輸入的支持尚未完善。
> - 需要用戶手動設置 Telegram 訪問權限，增加了初期配置的複雜度。
> - 在處理大量數據時可能會遇到性能瓶頸。

> [!warning] 注意事項
> - 僅支援 Node.js 20.0.0 以上版本。
> - 需要用戶手動設置 Telegram 訪問權限。
> - CLI 模式下對於中文輸入的支持尚不完善。
...
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於任務調度，但不具備 Mercury 的多通道支持和持續運行能力。 |
| [432539/gpt2api](https://github.com/432539/gpt2api) | 提供 API 接口，但不支持長時間運行的背景任務。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 使用 Rust 實作，記憶體用量較少，但功能上不如 Mercury 完善。 | 如果你的團隊需要一個高效能的 AI 助手，且已經熟悉 Rust 生態。 | medium，因為需要重寫部分功能和適配新的環境。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於任務調度，功能較為單一，且不支持多通道。 | 如果你的需求主要集中在任務調度而非多通道交互。 | low，因為功能相似，遷移成本較低。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **mercury-agent** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 使用 Rust 實作，記憶體用量較少，但功能上不如 Mercury 完善。 | 專注於任務調度，功能較為單一，且不支持多通道。 |
> | 遷移成本 | - | medium，因為需要重寫部分功能和適配新的環境。 | low，因為功能相似，遷移成本較低。 |
> | 適用場景 | 主要場景 | 如果你的團隊需要一個高效能的 AI 助手，且已經熟悉 Rus | 如果你的需求主要集中在任務調度而非多通道交互。 |
...
- **[HIGH]** CLI 模式下對於中文輸入的支持不佳，可能導致輸入錯誤。
  - 解法：使用英文輸入或在 Telegram 中進行交互。
- [MEDIUM] 在高負載情況下，SQLite 可能會導致性能瓶頸。
  - 解法：考慮使用更強大的資料庫系統。
- [MEDIUM] 初次設置過程中，Telegram 訪問權限的配置可能會讓新手感到困惑。
  - 解法：參考官方文檔進行設置。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 具備持續運行和權限管理，適合小型團隊的需求。 |
| 大型企業的任務調度系統 | 不適合 | 在高負載情況下性能可能不佳。 |
| 需要長期監控的系統管理 | 適合 | 具備自動重啟和背景運行的能力。 |
| 個人開發者的日常助手 | 非常適合 | 靈活的使用方式和強大的記憶功能。 |
...
> [!tip] 投入 vs 回報
> 花 2 小時學習，3 小時整合，得到一個靈活的 AI 助手，值得一試。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：Mercury 不需要高權限運行，且不會存取敏感資料。依賴的外部 API 需定期檢查安全性。
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["AI 助手","持續運行","權限管理"];
