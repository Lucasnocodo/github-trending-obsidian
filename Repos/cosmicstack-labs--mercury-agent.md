...
use_case: "提供一個具備權限管理的 AI 助手，能夠持續運行並記住重要資訊。"
priority: high
ring: assess
...
star_history: "2026-04-26:1117,2026-04-26:1124,2026-04-27:1416,2026-04-27:1422,2026-04-28:1549"
tags:
  - github
  - "category/ai_ml"
  - "lang/typescript"
...
aliases:
  ...
  - "提供一個具備權限管理的 AI 助手，能夠持續運行並記住重要資訊。"
---

# mercury-agent

**1.5k** stars · **221** stars/天 · 建立 7 天前 · TypeScript · MIT
...
> **適合** 需要一個具備安全性與記憶功能的 AI 助手的開發者與產品經理。
> **一句話重點** Mercury Agent 的設計讓用戶在使用 AI 助手時擁有更高的安全性與控制權。
...
> **結論** 花 3 小時學習，2 小時整合，得到一個安全且靈活的 AI 助手，值得考慮。

> [!abstract] 核心創新
> Mercury Agent 的獨特之處在於其權限管理與第二大腦記憶功能，能夠在執行命令前詢問用戶意圖。

## 專案簡介

Mercury Agent 是一個靈活的 AI 助手，設計上強調用戶的控制權與安全性。用戶可以透過 CLI 或 Telegram 進行互動，並且具備 31 種內建工具，支持多種任務。其核心機制是透過 SQLite 實現的第二大腦記憶，能夠持久保存用戶的偏好與行為，並在需要時主動詢問用戶的意圖。這樣的設計使得 Mercury 不僅能執行命令，還能記住用戶的需求，避免不必要的錯誤。用戶可以透過 `mercury up` 命令啟動背景守護進程，並自動重啟，確保服務持續運行。

技術上，Mercury 使用 TypeScript 和多個 AI SDK，並整合了 DeepSeek、OpenAI 和 Anthropic 等多個 LLM 提供者，這使得它在處理請求時具備彈性與擴展性。與其他 AI 助手相比，如 0x0funky/agent-sprite-forge 和 432539/gpt2api，Mercury 的獨特之處在於其權限管理與記憶功能，能夠在執行命令前詢問用戶，並且在多用戶環境中提供安全的訪問控制。使用上，Mercury 需要 Node.js 20 以上的環境，並且能夠在多平台上運行，適合需要長時間運行的任務或需要持續監控的場景。社群活躍度良好，開發者對於問題的回應速度也相對迅速，顯示出這個專案的健康度。總體來看，Mercury 是一個值得考慮的 AI 助手，特別適合需要高安全性與持久記憶的用戶。

**技術棧**：`TypeScript` · `Node.js >=20.0.0` · `SQLite`

## 重點功能

- 權限管理 — Mercury 在執行命令前會詢問用戶，避免意外操作。
- 第二大腦記憶 — 使用 SQLite 保存用戶的偏好與行為，支持多種記憶類型。
- 多通道訪問 — 支援 CLI 與 Telegram 互動，並能在多用戶環境中運行。
- 自動重啟 — 背景守護進程能夠在崩潰後自動重啟，確保服務持續運行。
- 可擴展性 — 支援安裝社群技能，並能夠根據需求進行擴展。
...
> [!tip] 爆紅原因
> 建立 7 天內累積 1549 stars（221/天），forks 164（10.6%），顯示出強勁的增長潛力。開發者 Cosmic Stack 以其在 AI 領域的經驗，針對用戶在使用 AI 助手時的安全性與記憶需求進行了深入研究，填補了市場上對於權限管理與個性化記憶的空白。近期的社群討論與需求反饋也促進了這個專案的快速發展，特別是對於 Telegram 支援的需求。這些因素共同推動了 Mercury Agent 的快速擴散。

## 適合誰使用

**目標受眾**：需要一個具備安全性與記憶功能的 AI 助手的開發者與產品經理。

> [!example] 使用場景
> - 產品經理用它來自動化日常任務，因為 Mercury 能夠記住過去的偏好並主動詢問，提升工作效率。
> - 開發者用它來管理多個 API 密鑰，因為其權限管理功能能夠確保安全性，避免意外執行危險命令。
> - 團隊成員用它來協作處理任務，因為 Mercury 支援多用戶訪問，並能夠根據角色進行權限控制。

## 架構分析

Mercury Agent 採用模組化架構，核心是基於 TypeScript 的 CLI 工具，並使用 SQLite 作為記憶存儲。這樣的設計使得用戶能夠在多種環境中輕鬆部署。資料流方面，所有用戶的交互都通過 CLI 或 Telegram 進行，並且在後端進行處理。選擇 SQLite 作為存儲解決方案，帶來了快速查詢與持久化的優勢，但也限制了在高並發場景下的擴展性。整體來看，這種設計使得 Mercury Agent 能夠在安全性與易用性之間取得良好平衡。

## 技術深入分析

Mercury Agent 的核心技術機制基於 TypeScript，並使用 SQLite 作為記憶存儲。這使得它能夠快速查詢用戶的偏好與行為，並在需要時進行記憶的自動提取。效能上，Mercury 能夠在多用戶環境中運行，並且支持多種 LLM 提供者，這使得它在處理請求時具備彈性。選擇 SQLite 作為存儲解決方案，雖然帶來了快速查詢的優勢，但在高並發場景下可能會成為瓶頸。技術上，Mercury 的設計考慮到了安全性，所有命令在執行前都需要用戶確認，這樣的設計能夠有效避免意外操作。整合方面，Mercury 能夠與 Telegram 和 CLI 無縫協作，並且支持多用戶訪問，這使得它在團隊協作中表現出色。整體來看，Mercury Agent 提供了一個安全且靈活的 AI 助手解決方案，適合需要長時間運行的任務或需要持續監控的場景。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝與使用說明，並包含了基本的範例。安裝過程相對順暢，但對於新手用戶來說，Telegram 的設置可能會有些挑戰。整體而言，花 30 分鐘應該能夠順利運行起來。

## 優缺點分析

> [!success] 優點
> - 具備權限管理，能夠避免意外執行危險命令。
> - 記憶功能強大，能夠持久保存用戶的偏好與行為。
> - 多通道支持，適合不同使用場景的需求。

> [!danger] 缺點
> - 需要 Node.js 20 以上的環境，對於某些用戶來說可能有門檻。
> - 在高負載情況下可能會出現延遲，影響使用體驗。
> - Telegram 設置需要額外的配置，對新手用戶不太友好。

> [!warning] 注意事項
> - 僅支援 Node.js 20 以上版本
> - 需要適當配置 Telegram 機器人以進行多用戶訪問
> - 在高負載情況下，可能會出現延遲
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 雖然也提供 AI 助手功能，但缺乏 Mercury 的權限管理與記憶功能，適合較簡單的任務。 |
| [432539/gpt2api](https://github.com/432539/gpt2api) | 主要專注於 GPT-2 的 API 接入，而 Mercury 提供多種 LLM 支援與多通道訪問，功能更為全面。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 使用 Python 實作，專注於簡單的任務自動化，記憶功能較弱。 | 如果你的需求較為簡單，且團隊熟悉 Python，則可以考慮這個工具。 | low，因為語言相似且功能簡單。 |
| [432539/gpt2api](https://github.com/432539/gpt2api) | 專注於 GPT-2 的 API 接入，功能較為單一。 | 如果你的應用主要依賴於 GPT-2 的功能，這個工具會更簡單直接。 | medium，因為需要調整 API 調用的方式。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **mercury-agent** | **agent-sprite-forge** | **gpt2api** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 使用 Python 實作，專注於簡單的任務自動化，記憶功能較弱。 | 專注於 GPT-2 的 API 接入，功能較為單一。 |
> | 遷移成本 | - | low，因為語言相似且功能簡單。 | medium，因為需要調整 API 調用的方式。 |
> | 適用場景 | 主要場景 | 如果你的需求較為簡單，且團隊熟悉 Python，則可以考慮這 | 如果你的應用主要依賴於 GPT-2 的功能，這個工具會更簡單 |
...
- [MEDIUM] 在高負載情況下，可能會出現延遲，影響使用體驗
  - 解法：減少同時請求數量，或考慮升級伺服器資源
- **[HIGH]** Telegram 設置過程中，可能會遇到權限問題
  - 解法：確保 Telegram 機器人已正確配置並授予必要權限
- [MEDIUM] 在某些環境下，可能需要額外的 Node.js 配置
  - 解法：參考官方文檔進行配置
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 具備安全性與記憶功能，能夠有效管理多個 API 密鑰。 |
| 大型企業的 AI 助手 | 普通 | 雖然具備多用戶支持，但在高負載情況下可能會出現延遲。 |
| 個人開發者的日常任務自動化 | 非常適合 | 能夠持續運行並記住用戶的偏好，提升效率。 |
| 需要高並發的實時應用 | 不適合 | 在高負載情況下可能會出現性能瓶頸。 |
...
> [!tip] 投入 vs 回報
> 花 3 小時學習，2 小時整合，得到一個安全且靈活的 AI 助手，值得考慮。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：Mercury Agent 不需要高權限運行，且所有敏感資料都經過加密處理。對於 CI/CD 的整合也相對安全，因為它不會存取任何敏感環境變數。
...
> | 貢獻者 | Commits |
> | --- | --- |
> | [@hotheadhacker](https://github.com/hotheadhacker) | 111 |
...
> **活躍天數** 3 天 · **最新 commit** chore: bump version to 1.1.2
...
> | [#23](https://github.com/cosmicstack-labs/mercury-agent/issues/23) | Telegram inline keyboard (Continue/Stop) buttons persist aft | 0 | 0 |
...
相關專案：[[0x0funky--agent-sprite-forge|0x0funky/agent-sprite-forge]] · [[432539--gpt2api|432539/gpt2api]] · [[0xGF--boneyard|0xGF/boneyard]] · [[AgentSeal--codeburn|AgentSeal/codeburn]] · [[AgriciDaniel--claude-obsidian|AgriciDaniel/claude-obsidian]] · [[Einsia--OpenChronicle|Einsia/OpenChronicle]] · [[VoltAgent--awesome-claude-design|VoltAgent/awesome-claude-design]] · [[HKUDS--OpenHarness|HKUDS/OpenHarness]]
