...
use_case: "提供一個具備記憶功能的 AI 助手，能夠在多種平台上運行並進行自我管理。"
priority: high
ring: assess
...
aliases:
  ...
  - "提供一個具備記憶功能的 AI 助手，能夠在多種平台上運行並進行自我管理。"
---

# mercury-agent

**2.3k** stars · **81** stars/天 · 建立 29 天前 · TypeScript · MIT
...
> **適合** 需要一個能夠持續運行並具備自我管理能力的 AI 助手的專業人士。
> **一句話重點** Mercury Agent 的獨特之處在於其靈魂驅動的設計理念，讓 AI 助手不僅僅是工具，而是能夠理解和學習用戶的需求。
...
> **結論** 花 5 小時學習，10 小時整合，得到一個靈活的 AI 助手，值得一試。

> [!abstract] 核心創新
> Mercury 提供了一個靈魂驅動的 AI 助手，強調用戶的控制權和透明度。

## 專案簡介

Mercury Agent 是一個以「靈魂驅動」為理念的 AI 助手，具備多通道訪問能力，能在 CLI、Telegram 或 Web 上持續運行。用戶可以透過簡單的 CLI 指令 `npx @cosmicstack/mercury-agent` 開始使用，首次運行會啟動設置向導，並要求用戶選擇權限模式（如「詢問我」或「允許所有」）。這個工具的核心在於其「第二大腦」記憶系統，利用 SQLite 儲存用戶的偏好和習慣，並能自動提取和整合資訊。Mercury 的設計理念強調用戶的控制權，所有操作都需經過用戶同意，這樣的設計有效避免了許多 AI 工具常見的隱私問題。

其內建的 31 種工具涵蓋了文件系統操作、Shell 命令執行、消息發送、Git 操作等，並且支持通過 `/budget` 指令來管理每日的 API 使用預算。這樣的設計使得 Mercury 能夠在多種場景中靈活應用，並且不會超出用戶的預算限制。其 Web Dashboard 提供了直觀的用戶界面，支持 Kanban 板和工作區 IDE，讓用戶能夠更方便地管理任務和記憶。這些功能使得 Mercury 成為一個強大的個人助理，特別適合需要高效管理時間和任務的用戶。

**技術棧**：`TypeScript` · `React` · `SQLite`

## 重點功能

- 靈魂驅動的個性 — 透過 markdown 文件定義 AI 的個性和行為。
- 持久的第二大腦 — 使用 SQLite 儲存用戶的偏好和記憶，支持全文本搜索。
- 權限管理 — 每次操作都需用戶確認，避免未經授權的行為。
- 多通道支持 — 同時在 CLI、Telegram 和 Web 上運行，提供靈活的使用場景。
- 實時流媒體 — CLI 和 Telegram 上的實時消息流，支持編輯和即時反饋。
...
> [!tip] 爆紅原因
> 建立 29 天內累積 2336 stars（81/天），forks 240（10.3%），顯示出強勁的增長潛力。這個專案的主要貢獻者來自 Cosmic Stack 團隊，他們在 AI 和開源領域有豐富的經驗。Mercury 解決了許多 AI 助手在用戶隱私和操作透明度上的痛點，提供了一個「詢問後行動」的操作模式，這在市場上是相對少見的。近期的版本更新引入了 Web Dashboard，進一步提升了用戶體驗。社群的活躍度也顯示出用戶對這個工具的需求和反饋，特別是在處理多通道和權限管理方面。

## 適合誰使用

**目標受眾**：需要一個能夠持續運行並具備自我管理能力的 AI 助手的專業人士。

> [!example] 使用場景
> - 獨立開發者用它來自動化日常任務，因為 Mercury 的 Kanban 板和自動化功能能夠有效管理多個項目進度。
> - 產品經理用它來追蹤團隊任務和進度，因為其內建的 Kanban 系統可以讓他們輕鬆管理和調整任務優先級。
> - 系統管理員用它來監控和管理伺服器任務，因為 Mercury 的守護進程和自動重啟功能確保了系統的穩定性和持續運行。

## 架構分析

Mercury Agent 採用模組化架構，核心功能由 TypeScript 實現，並使用 SQLite 作為持久化存儲。用戶的所有數據和配置都存放在 `~/.mercury/` 目錄下，這樣的設計使得用戶可以輕鬆管理和備份。資料流從用戶輸入經過權限檢查後，通過內建的工具執行相應的操作，並將結果存儲到記憶系統中。選擇 SQLite 作為存儲解決方案的好處在於其輕量級和高效能，但在處理大規模數據時可能會遇到性能瓶頸。整體架構設計考慮到了擴展性，支持用戶安裝社群技能，這使得 Mercury 能夠隨著需求不斷增長而擴展功能。

## 技術深入分析

Mercury Agent 的核心技術基於 TypeScript，這使得代碼可讀性和維護性都得到了提升。其記憶系統使用 SQLite 作為後端，支持多種記憶類型的存儲，並利用 FTS5 實現高效的全文搜索。這樣的設計不僅提高了資料檢索的速度，也讓用戶能夠方便地管理和查詢自己的記憶。整體架構中，Mercury 的多通道支持是其一大亮點，能夠在 CLI、Telegram 和 Web 上無縫運行，這使得用戶可以根據自己的需求選擇最適合的操作方式。選擇使用 Node.js 作為執行環境，讓 Mercury 能夠在多平台上運行，但也帶來了對 Node.js 版本的依賴。隨著用戶基數的增長，如何保持系統的穩定性和性能將是未來的一大挑戰，特別是在高並發的情況下。整合方面，Mercury 能夠與主流的 CI/CD 工具鏈相容，並且支持通過 API 擴展功能，這為開發者提供了更大的靈活性。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了詳細的安裝和使用說明。安裝過程相對順暢，但對於低資源環境可能會遇到困難。文檔目前僅提供英文和簡體中文版本，對於非英語用戶可能會有一定的學習曲線。

## 優缺點分析

> [!success] 優點
> - 強調用戶隱私和控制權，所有操作需用戶確認。
> - 持久的記憶系統，能夠自動學習用戶偏好。
> - 多通道支持，靈活應用於不同平台。

> [!danger] 缺點
> - 安裝包相對較大，對於低資源環境可能不友好。
> - CLI 模式下的中文輸入兼容性問題，影響部分用戶體驗。
> - 社群支持和文檔仍在完善中，可能會遇到一些使用上的困難。

> [!warning] 注意事項
> - 僅支援 Node.js 20 以上版本
> - 對於中文輸入在 CLI 模式下存在兼容性問題
> - 目前的依賴樹相對較重，安裝包大小約 87 MB
...
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於任務自動化，但不具備 Mercury 的多通道支持和個性化設定。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於創建 AI 代理的框架，提供更靈活的擴展性，但缺乏 Mercury 的權限管理和記憶系統。 | 如果你的團隊需要一個高度可定制的 AI 代理框架，而不在意權限管理的複雜性。 | medium，因為需要重新設計權限和記憶管理邏輯。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於任務自動化，提供基本的 AI 功能，但不具備 Mercury 的多通道支持和個性化設定。 | 如果你的需求主要集中在任務自動化，而不需要多通道的靈活性。 | low，因為功能相似，遷移成本較低。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **mercury-agent** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於創建 AI 代理的框架，提供更靈活的擴展性，但缺乏 Mercury 的權限管理和記憶系統。 | 專注於任務自動化，提供基本的 AI 功能，但不具備 Mercury 的多通道支持和個性化設定。 |
> | 遷移成本 | - | medium，因為需要重新設計權限和記憶管理邏輯。 | low，因為功能相似，遷移成本較低。 |
> | 適用場景 | 主要場景 | 如果你的團隊需要一個高度可定制的 AI 代理框架，而不在意權 | 如果你的需求主要集中在任務自動化，而不需要多通道的靈活性。 |
...
> [!tip] 採用建議
> 適合個人項目和小型團隊試用，但不建議用於生產環境的核心任務。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** CLI 模式下中文輸入不兼容，可能導致無法正常使用
  - 解法：使用英文輸入或在 Web 界面中操作
- [MEDIUM] 安裝過程中可能遇到依賴問題，導致安裝失敗
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的創業團隊需要一個自動化助手 | 非常適合 | Mercury 的多通道支持和任務管理功能能夠有效提升團隊效率。 |
| 大型企業需要一個高效的任務管理系統 | 普通 | 雖然 Mercury 提供了 Kanban 板，但在高並發情況下可能會遇到性能瓶頸。 |
| 需要一個具備自我學習能力的 AI 助手的個人用戶 | 非常適合 | Mercury 的第二大腦記憶系統能夠幫助用戶管理個人知識。 |
| 對安全性要求極高的金融行業 | 不適合 | 雖然 Mercury 強調用戶控制，但仍然存在潛在的安全風險。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，10 小時整合，得到一個靈活的 AI 助手，值得一試。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：Mercury 不需要高權限運行，並且所有敏感資料都存儲在用戶的本地環境中。依賴的庫經過審計，整體安全性較高。
...
> [!abstract] 最近 10 次 commit（2026-05-06 ~ 2026-05-19）
> **活躍天數** 3 天 · **最新 commit** release: v1.1.9 (renumber from unpublished 1.1.8, dep cleanup, graph UX)
