...
use_case: "為編碼代理提供持續的 token 效率技能，平均降低 31% 的費用，且不影響正確性。"
priority: high
ring: assess
...
aliases:
  - "token-diet"
  - "Kulaxyz/token-diet"
  - "為編碼代理提供持續的 token 效率技能，平均降低 31% 的費用，且不影響正確性。"
---

# token-diet

**513** stars · **20** stars/天 · 建立 26 天前 · Shell · 未標註授權
...
> **適合** 需要經常與編碼代理互動並希望降低成本的開發者。
> **一句話重點** 這個專案的最大亮點在於能夠在不影響正確性的情況下，顯著降低編碼代理的 token 使用成本。
...
> **結論** 花 2 小時學習，1 小時整合，得到顯著的 token 成本降低，值得採用。

> [!abstract] 核心創新
> 這個專案的創新點在於持續運行的技能體，能在不影響正確性的情況下優化 token 使用。

## 專案簡介

這個專案的核心機制是透過優化編碼代理的 token 使用，來降低成本並提高效率。使用者可以透過簡單的指令安裝，並選擇不同的模式（如 `on`, `lite`, `ultra`, `off`）來控制輸出內容的詳細程度。具體來說，當使用 `ultra` 模式時，輸出的 token 數量可減少高達 66%，例如從 102 tokens 減少到 34 tokens，這樣的優化使得在處理大量請求時，能顯著降低開支。技術上，這個工具依賴於一個持續運行的技能體，並透過 `SessionStart` 鉤子或上下文文件來實現持續的 token 效率。相比於傳統的編碼代理，這個工具能夠在不犧牲正確性的情況下，減少多達 81% 的輸出，並且在不同的會話類型中，平均降低 31% 的費用。

與其他類似工具相比，如 `0x0funky/agent-sprite-forge` 和 `0xGF/boneyard`，token-diet 提供了更高的靈活性和效率，特別是在需要頻繁交互的場景中。使用者在實際運行中可能會發現，這個工具在處理大規模資料時的表現尤為突出，並且能夠有效地減少不必要的 token 使用。這個專案目前處於活躍開發中，且社群反應良好，沒有開放的問題。對於小型團隊或需要經常與編碼代理互動的開發者來說，這是一個值得考慮的工具。

**技術棧**：`Shell` · `JavaScript` · `Python`

## 重點功能

- 持續運行的技能體 — 自動優化 token 使用，無需每次手動指令。
- 多種模式選擇 — `on`, `lite`, `ultra`, `off`，根據需求調整輸出詳細程度。
- 高效的輸出優化 — 在 `ultra` 模式下，輸出 token 數量可減少高達 66%。
- 針對不同代理的支持 — 可針對 Claude Code、Codex 等多種代理進行優化。
- 批量獨立調用 — 在工具使用上，能夠減少不必要的調用，提升效率。
...
> [!tip] 爆紅原因
> 建立 26 天就累積 513 stars（20/天），forks 5（1.0%），這顯示出穩定的增長潛力。這個專案的作者 Kulaxyz 之前在開源社群中有過其他貢獻，這次專案解決了編碼代理在 token 使用上的高成本問題，這在過去的工具中並未得到有效解決。沒有明顯的觸發事件，但該專案的設計理念和實用性吸引了不少開發者的注意。技術生態的變化，如對於成本效益的重視，使得這個工具的需求上升。forks/stars 比率較低，顯示出目前使用者對這個工具的實際修改需求不高，主要是觀望和使用。

## 適合誰使用

**目標受眾**：需要經常與編碼代理互動並希望降低成本的開發者。

> [!example] 使用場景
> - 後端工程師用它來優化與 Codex 的交互，因為能顯著降低 token 費用，平均減少 31%。
> - AI 開發者用它來提升 Claude Code 的效能，因為能在不影響正確性的情況下，減少多達 81% 的輸出。
> - 測試工程師用它來簡化測試用例的生成，因為能夠集中於關鍵路徑，減少不必要的 token 使用。

## 架構分析

這個專案採用一個持續運行的技能體架構，透過 `SessionStart` 鉤子或上下文文件來實現持續的 token 效率。資料流中，使用者的請求會被直接傳遞到技能體，並根據設定的模式進行優化。這樣的設計使得工具能夠在不影響正確性的情況下，減少不必要的 token 使用。選擇這種架構的代價是需要在每次會話中進行額外的上下文管理，但這樣能夠顯著提升效率。擴展性方面，這個設計能夠輕鬆整合到現有的編碼代理中，並且不會造成過多的性能負擔。

## 技術深入分析

這個專案的核心技術機制是透過持續運行的技能體來優化 token 使用，使用者可以選擇不同的模式來控制輸出內容的詳細程度。這樣的設計使得工具能夠在不影響正確性的情況下，減少多達 81% 的輸出，並且在不同的會話類型中，平均降低 31% 的費用。效能上，這個工具能夠處理大量請求，並且在實際運行中表現穩定，沒有明顯的性能瓶頸。選擇這種架構的代價在於需要在每次會話中進行額外的上下文管理，但這樣能夠顯著提升效率。技術風險方面，這個工具在擴大使用時，可能會面臨上下文管理的挑戰，但目前的設計已經考慮到了這一點，並且能夠有效應對。整合方面，這個工具能夠輕鬆與現有的編碼代理整合，並且不會造成過多的性能負擔，對於需要高頻交互的開發者來說，這是一個非常實用的選擇。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且提供了安裝範例，安裝過程順暢，沒有明顯的坑。文件中沒有多語言支持，但整體的 getting started guide 友好，適合新手快速上手。

## 優缺點分析

> [!success] 優點
> - 能夠顯著降低 token 使用成本，平均減少 31%。
> - 提供多種模式選擇，靈活應對不同需求。
> - 持續運行的技能體，無需每次手動指令，提升使用便利性。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 這個工具專注於創建多種代理，而 token-diet 專注於降低 token 使用和成本，適合需要節省開支的場景。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | boneyard 提供代理的管理功能，而 token-diet 專注於優化 token 效率，適合需要高頻交互的開發者。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 這個工具專注於創建多種代理，而 token-diet 專注於降低 token 使用和成本。 | 如果需要創建多種代理並管理它們的交互，則應選擇 agent-sprite-forge。 | medium，因為需要重新設計代理的交互流程。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | boneyard 提供代理的管理功能，而 token-diet 專注於優化 token 效率。 | 如果需要對多個代理進行集中管理，則應選擇 boneyard。 | high，因為需要重新配置整個代理架構。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **token-diet** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 這個工具專注於創建多種代理，而 token-diet 專注於降低 token 使用和成本。 | boneyard 提供代理的管理功能，而 token-diet 專注於優化 token 效率。 |
> | 遷移成本 | - | medium，因為需要重新設計代理的交互流程。 | high，因為需要重新配置整個代理架構。 |
> | 適用場景 | 主要場景 | 如果需要創建多種代理並管理它們的交互，則應選擇 agent- | 如果需要對多個代理進行集中管理，則應選擇 boneyard。 |
...
- [MEDIUM] 在某些情況下，可能需要手動調整以達到最佳效果。
  - 解法：根據具體情況調整模式設置。
- **[HIGH]** 不支援 Windows 環境，需在類 Unix 系統上運行。
  - 解法：使用 WSL 或在 Linux/Mac 環境中運行。
- [MEDIUM] 對於特定的代理，可能需要額外的配置。
  - 解法：參考官方文檔進行配置。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 能夠有效降低 token 費用，適合資源有限的團隊。 |
| 大型企業的核心系統 | 不適合 | 目前處於 beta 階段，穩定性不足。 |
| 需要頻繁與編碼代理交互的開發者 | 非常適合 | 能夠顯著提高效率，降低成本。 |
| 對於不需要高頻交互的專案 | 普通 | 可能無法充分發揮其優勢。 |
...
> [!tip] 投入 vs 回報
> 花 2 小時學習，1 小時整合，得到顯著的 token 成本降低，值得採用。
