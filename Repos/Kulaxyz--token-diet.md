...
star_history: "2026-07-06:590,2026-07-06:590,2026-07-07:596"
tags:
  - github
  - "category/開發工具"
  - "lang/shell"
...
**596** stars · **199** stars/天 · 建立 3 天前 · Shell · 未標註授權
...
> [!info] 速覽
> **安裝難度** Easy · **專案狀態** Brand New · **熱度** Hot (199 stars/day)
> **授權** 未標註授權 (風險較高) · **維護** Active (最後推送 2 天前) · **貢獻者** Solo (bus factor 風險) · **參與度** Low
> **適合** 需要長時間運行編碼代理並希望降低成本的獨立開發者或小型團隊。
> **一句話重點** token-diet 不僅能降低成本，還能提高編碼代理的效率，這在當前的開發環境中非常重要。
...
> **結論** 花 2 小時學、1 小時整合，得到顯著的成本降低，值得採用。

> [!abstract] 核心創新
> 通過持續的令牌效率技能，顯著降低編碼代理的運行成本。

## 專案簡介

token-diet 是一個專為編碼代理設計的工具，旨在減少會話中的令牌使用量，從而降低使用成本。用戶只需執行一條命令即可安裝，並可選擇不同的運行模式（如 `on`、`lite`、`ultra` 和 `off`），以調整輸出內容的詳細程度。該工具的核心機制是通過優化回答方式、文件讀取和工具使用來減少不必要的文字，從而達到降低令牌消耗的目的。具體來說，當用戶請求時，token-diet 會直接給出答案，省略多餘的前言和後語，並且在測試中只關注關鍵路徑，這樣可以大幅減少輸出的令牌數量。根據實際測試，這個工具在不同的會話類型中，平均可以減少 31% 的費用，具體數據顯示在輸出密集型的情況下，甚至可以達到 81% 的減少。

技術上，它使用了 Shell 和 Python，並且依賴於一個簡單的安裝腳本，這使得它的部署非常輕量。與其他工具相比，如 0x0funky/agent-sprite-forge 和 0xGF/boneyard，token-diet 更加專注於令牌的高效使用，而不是功能的擴展。實際使用中，token-diet 的效果顯著，尤其是在需要大量輸出的場景中，能夠有效降低成本。這使得它特別適合需要長時間運行的編碼代理，並且能夠在不影響正確性的情況下，持續提供高效的服務。對於小型團隊或個人開發者來說，這是一個值得考慮的工具，因為它能顯著減少開支並提高工作效率。

**技術棧**：`Shell` · `JavaScript` · `Python`

## 重點功能

- 自動安裝 — 使用一行命令安裝，支持多種編碼代理。
- 多種運行模式 — 可選擇 `on`、`lite`、`ultra` 或 `off` 來調整輸出內容的詳細程度。
- 減少不必要的文字 — 省略前言和後語，直接給出關鍵答案。
- 優化測試用例 — 僅關注關鍵路徑，減少測試中的令牌使用。
- 批量獨立調用工具 — 在迭代過程中，根據需要進行批量調用。
...
> [!tip] 爆紅原因
> 建立 3 天內累積 596 stars（199/天），forks 僅 2（0.3%），顯示出強烈的初期關注。作者 Kulaxyz 似乎專注於提升編碼代理的效率，這在當前的開發環境中是一個未被充分解決的痛點。之前的解決方案往往無法有效控制令牌的使用，導致開發成本上升。這個工具的出現正好填補了這一空白，並且其簡單的安裝方式和即時的效果吸引了不少用戶的注意。由於目前沒有類似的解決方案能夠達到如此高的效率，這使得 token-diet 在短時間內迅速獲得關注。

## 適合誰使用

**目標受眾**：需要長時間運行編碼代理並希望降低成本的獨立開發者或小型團隊。

> [!example] 使用場景
> - 後端工程師用它來優化與 Codex 的交互，因為能夠顯著降低令牌費用，平均節省 31%。
> - 數據科學家用它來減少模型推理的成本，因為在輸出密集型的任務中，能夠達到 81% 的令牌減少。
> - 開發者用它來提升與 Claude Code 的對話效率，因為它能夠在不損失正確性的情況下，減少多餘的文字輸出。

## 架構分析

token-diet 的架構設計旨在提供一個輕量級的解決方案，能夠在多種編碼代理中運行。它使用一個簡單的安裝腳本來自動檢測和配置目標代理，並通過 `SessionStart` hook 或上下文文件來持續應用優化。這樣的設計使得用戶無需手動干預，能夠在每次會話中自動獲得令牌效率的提升。這種方法的代價是需要依賴於特定的代理架構，可能會限制其在其他環境中的應用。整體而言，這種設計使得 token-diet 能夠在不影響正確性的情況下，持續提供高效的服務。

## 技術深入分析

token-diet 的核心技術機制是通過優化輸出內容來減少令牌的消耗。它使用了精簡的回答方式，省略不必要的前言和後語，並且在測試中只關注關鍵路徑，這樣可以有效降低令牌使用。根據實際測試，這個工具在不同的會話類型中，平均可以減少 31% 的費用，具體數據顯示在輸出密集型的情況下，甚至可以達到 81% 的減少。這種設計使得 token-diet 能夠在不影響正確性的情況下，持續提供高效的服務。

技術上，它使用了 Shell 和 Python，並且依賴於一個簡單的安裝腳本，這使得它的部署非常輕量。與其他工具相比，如 0x0funky/agent-sprite-forge 和 0xGF/boneyard，token-diet 更加專注於令牌的高效使用，而不是功能的擴展。實際使用中，token-diet 的效果顯著，尤其是在需要大量輸出的場景中，能夠有效降低成本。對於小型團隊或個人開發者來說，這是一個值得考慮的工具，因為它能顯著減少開支並提高工作效率。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且提供了安裝和使用的範例，讓新手能夠快速上手。安裝過程非常順暢，僅需一行命令即可完成。文件中沒有多語言支持，但英文內容足夠詳細，能夠幫助用戶理解。整體而言，花 30 分鐘內能夠順利運行。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於多種代理的整合，而 token-diet 更專注於令牌效率的提升。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供更廣泛的功能擴展，而 token-diet 專注於減少令牌使用和成本。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於多種代理的整合，而 token-diet 更專注於令牌效率的提升。 | 如果需要一個多功能的代理整合方案，而不僅僅是令牌優化。 | medium，因為需要調整現有的代理配置。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供更廣泛的功能擴展，而 token-diet 專注於減少令牌使用和成本。 | 如果需要更多的功能擴展和自定義選項，而不僅僅是令牌優化。 | high，因為需要重構現有的代碼以適應新的功能。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **token-diet** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於多種代理的整合，而 token-diet 更專注於令牌效率的提升。 | 提供更廣泛的功能擴展，而 token-diet 專注於減少令牌使用和成本。 |
> | 遷移成本 | - | medium，因為需要調整現有的代理配置。 | high，因為需要重構現有的代碼以適應新的功能。 |
> | 適用場景 | 主要場景 | 如果需要一個多功能的代理整合方案，而不僅僅是令牌優化。 | 如果需要更多的功能擴展和自定義選項，而不僅僅是令牌優化。 |
...
- [MEDIUM] 在某些情況下，過度簡化可能導致信息遺失
  - 解法：根據需要調整運行模式以獲得更詳細的輸出
- **[HIGH]** 僅支援特定的編碼代理，無法廣泛應用
  - 解法：確認使用的代理是否在支持列表中
- [MEDIUM] 需要持續的網絡連接以便進行安裝和更新
  - 解法：確保在穩定的網絡環境中進行安裝
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 能夠顯著降低開支並提高開發效率。 |
| 大型企業的核心系統 | 不適合 | 可能無法滿足對正確性和功能的高要求。 |
| 個人開發者的實驗性項目 | 非常適合 | 簡單的安裝和高效的令牌使用能夠降低成本。 |
| 需要長時間運行的編碼代理 | 非常適合 | 能夠在不影響正確性的情況下持續提供高效的服務。 |
...
token-diet 最常與 Claude Code 和 Codex 搭配使用，通常在開發和測試階段進行整合。實際整合範例是在使用 Codex 進行代碼生成時，通過 token-diet 來優化輸出，具體做法是使用安裝腳本進行配置。該工具支援與 GitHub Actions 等 CI 工具的整合，並且可以在 VS Code 等 IDE 中使用。整合的摩擦點主要是需要確認代理的兼容性，否則可能會遇到功能限制。

## 歷史脈絡

> [!info] 這個工具為什麼現在出現？

在 token-diet 出現之前，開發者通常依賴於各種編碼代理，但這些工具在令牌使用上並不高效，導致成本上升。隨著開發需求的增加，對於降低開支的需求也越來越迫切。token-diet 的出現正好填補了這一空白，並且其簡單的安裝方式和即時的效果吸引了不少用戶的注意。

這個工具代表了一種新的技術趨勢，即在保持功能的同時，優化資源的使用，未來可能會有更多類似的工具出現。
