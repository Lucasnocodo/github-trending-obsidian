...
use_case: "為編碼代理提供持續的令牌效率技能，平均節省 31% 的費用，且不損失正確性。"
priority: high
ring: assess
...
star_history: "2026-07-06:590,2026-07-06:590,2026-07-07:596,2026-07-08:611,2026-07-09:566,2026-07-10:555,2026-07-11:557,2026-07-12:560,2026-07-13:561,2026-07-14:549,2026-07-15:544,2026-07-16:530,2026-07-17:531,2026-07-18:527,2026-07-19:531,2026-07-20:533,2026-07-21:525,2026-07-22:524"
tags:
  - github
  - "category/開發工具"
  - "lang/shell"
...
aliases:
  - "token-diet"
  - "Kulaxyz/token-diet"
  - "為編碼代理提供持續的令牌效率技能，平均節省 31% 的費用，且不損失正確性。"
---

# token-diet

**524** stars · **29** stars/天 · 建立 18 天前 · Shell · 未標註授權
...
> **適合** 需要在多個編碼代理中優化令牌使用的開發者和團隊。
> **一句話重點** 這個專案的真正價值在於它能夠在不影響正確性的情況下，顯著降低與大型模型交互的成本。
...
> **結論** 花 2 小時學習，1 小時整合，得到顯著的成本節省，值得採用。

> [!abstract] 核心創新
> 這個專案的創新點在於提供一個持續的令牌效率技能，能夠在不損失正確性的情況下顯著降低成本。

## 專案簡介

這個專案的核心機制是透過持續的令牌效率技能來優化編碼代理的輸出，減少不必要的文字，從而降低成本。使用者可以通過簡單的命令安裝，像是 `curl -fsSL https://raw.githubusercontent.com/Kulaxyz/token-diet/main/install.sh | bash`，並可選擇不同的模式（如 `--ultra`）來調整輸出精簡程度。這個工具的賣點在於能夠在不影響正確性的情況下，將輸出減少 30% 到 81%，並且在實際運行中，平均能降低 31% 的費用。技術上，它使用了 SessionStart hook 或持續載入的上下文文件來確保技能始終啟用，這樣的設計使得用戶無需每次都手動啟動。

與其他類似工具相比，如 `0x0funky/agent-sprite-forge` 和 `0xGF/boneyard`，token-diet 更加專注於持續性和正確性，並且提供了更精細的控制選項。實際使用中，這個工具在輸出密集型的會話中能達到 81% 的節省，這對於需要大量交互的開發者來說，無疑是一個巨大的成本優勢。雖然它的功能強大，但仍需注意在某些情境下可能會導致信息過於簡化，影響溝通的完整性。整體來看，這是一個適合需要高效能和成本控制的開發團隊使用的工具，特別是在處理大量代碼和文檔時。

**技術棧**：`Shell` · `JavaScript` · `Python`

## 重點功能

- 持續的令牌效率 — 在整個會話中自動減少不必要的文字，降低成本。
- 多種模式選擇 — 提供 `on`, `lite`, `ultra`, `off` 四種模式來調整輸出精簡程度。
- 針對不同代理的支持 — 支持 Claude Code, Codex, Cursor, Windsurf, Cline 等多種編碼代理。
- 批量處理工具調用 — 能夠在迭代過程中進行批量獨立調用，減少不必要的請求。
- 精簡的測試生成 — 減少測試用例的數量，僅保留關鍵路徑，最多 10 個每次會話。
...
> [!tip] 爆紅原因
> 建立 18 天內累積 524 stars（29/天），forks 4（0.8%），這顯示出穩定的增長。作者 Kulaxyz 以其在編碼代理領域的專業知識，提供了一個針對高成本問題的解決方案，特別是在使用大型模型時，令牌的消耗成為一大痛點。這個工具的出現正好填補了市場上對於高效能和低成本的需求，並且在社群中引起了討論。技術上，隨著 AI 模型的普及，對於令牌的管理和優化變得越來越重要，這使得 token-diet 的需求上升。forks/stars 比率較低，顯示出使用者對此工具的興趣尚在觀望階段。

## 適合誰使用

**目標受眾**：需要在多個編碼代理中優化令牌使用的開發者和團隊。

> [!example] 使用場景
> - 後端工程師用它來優化與 Codex 的交互，減少令牌消耗，因為這樣可以在長期內節省開支，平均節省 31%。
> - AI 開發者用它來在測試階段減少不必要的輸出，因為這樣能提高測試效率，並保持高正確性。
> - 產品經理用它來生成簡潔的文檔和報告，因為這樣能更快地傳達關鍵信息，避免冗長的敘述。

## 架構分析

這個專案採用了一個持續運行的技能架構，通過安裝腳本將指令注入到每個會話中。這樣的設計確保了技能的持續性，無需用戶每次手動啟動。資料流中，使用者的請求會經過這個技能進行處理，並在輸出時自動減少不必要的文字。選擇這種架構的代價是需要用戶在初始安裝時進行配置，但這樣可以在長期使用中獲得更高的效率。擴展性方面，這個工具能夠支持多種編碼代理，但仍需注意不同代理的配置需求。

## 技術深入分析

token-diet 的核心技術機制在於其持續運行的技能架構，這意味著它能夠在每次會話中自動優化輸出。使用者可以通過簡單的命令安裝，並在運行中選擇不同的模式來調整輸出精簡程度。這種設計使得用戶無需每次都手動啟動，提升了使用的便利性。效能方面，這個工具在實際運行中能夠達到高達 81% 的輸出節省，對於需要大量交互的開發者來說，無疑是一個巨大的成本優勢。設計取捨方面，選擇這種持續運行的架構意味著需要用戶在初始安裝時進行配置，但長期使用中能夠獲得更高的效率。技術風險評估中，這個工具在擴展性上表現良好，但仍需注意不同代理的配置需求可能導致的使用障礙。整合方面，這個工具能夠與多種編碼代理配合使用，但在與現有工具鏈的整合上可能需要額外的適配工作。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且提供了安裝指令和使用範例，安裝過程順暢且無明顯坑。文件中沒有多語言支持，但整體上易於理解，適合新手快速上手。
...
> [!danger] 缺點
> - 在某些情況下可能會影響溝通的完整性。
> - 需要對不同代理進行適當的配置，否則效果不佳。
> - 不支持所有編碼代理，限制了使用範圍。

> [!warning] 注意事項
> - 在某些情況下，過度簡化可能會影響溝通的完整性。
> - 需要對不同代理進行適當的配置，否則可能無法達到最佳效果。
> - 不支持所有編碼代理，僅限於列舉的幾個。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於多代理的協作，但不如 token-diet 在令牌效率上具體化。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供類似的功能，但缺乏 token-diet 的持續性和精細控制選項。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於多代理的協作，但不如 token-diet 在令牌效率上具體化。 | 如果需要多代理的協作功能，而不僅僅是令牌優化。 | medium，因為需要重新配置多個代理的協作方式。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供類似的功能，但缺乏 token-diet 的持續性和精細控制選項。 | 如果需要一個簡單的令牌優化工具，而不需要持續運行的功能。 | low，因為功能上相似，遷移成本較低。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **token-diet** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於多代理的協作，但不如 token-diet 在令牌效率上具體化。 | 提供類似的功能，但缺乏 token-diet 的持續性和精細控制選項。 |
> | 遷移成本 | - | medium，因為需要重新配置多個代理的協作方式。 | low，因為功能上相似，遷移成本較低。 |
> | 適用場景 | 主要場景 | 如果需要多代理的協作功能，而不僅僅是令牌優化。 | 如果需要一個簡單的令牌優化工具，而不需要持續運行的功能。 |
...
- [MEDIUM] 在某些情況下，過度簡化可能會影響溝通的完整性。
  - 解法：根據具體情況調整使用模式。
- **[HIGH]** 需要對不同代理進行適當的配置，否則可能無法達到最佳效果。
  - 解法：仔細閱讀文檔，確保正確配置。
- [MEDIUM] 不支持所有編碼代理，限制了使用範圍。
  - 解法：確認所用代理是否在支持列表中。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 能夠顯著降低令牌消耗，節省開支。 |
| 大型團隊進行多代理協作 | 普通 | 雖然支持多代理，但配置需求較高。 |
| 需要高效能的測試環境 | 適合 | 能夠減少不必要的輸出，提升測試效率。 |
| 單一代理的簡單應用 | 不適合 | 功能過於複雜，對於簡單應用來說可能不必要。 |
...
> [!tip] 投入 vs 回報
> 花 2 小時學習，1 小時整合，得到顯著的成本節省，值得採用。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：此工具不需要高權限，且不存取敏感資料，適合在 CI/CD pipeline 中使用。安全性考量良好，無已知的供應鏈風險。

## 生態系整合

> [!abstract] 如何融入你的工具鏈

token-diet 最常與各種編碼代理搭配使用，特別是在開發和測試階段。實際整合範例：在使用 Codex 的專案中，可以通過安裝 token-diet 來優化與 Codex 的交互，具體做法是使用安裝指令並選擇 `--ultra` 模式。這個工具與主流 CI 工具（如 GitHub Actions）相容，能夠輕鬆整合到現有的開發流程中。整合的摩擦點主要在於需要根據不同代理進行配置，這可能會導致初期的使用障礙。
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["自動化","機器學習","API 設計"];
