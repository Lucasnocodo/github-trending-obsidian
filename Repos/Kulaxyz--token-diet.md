...
star_history: "2026-07-06:590,2026-07-06:590,2026-07-07:596,2026-07-08:611,2026-07-09:566,2026-07-10:555,2026-07-11:557,2026-07-12:560,2026-07-13:561,2026-07-14:549,2026-07-15:544,2026-07-16:530"
tags:
  - github
  - "category/開發工具"
  - "lang/shell"
...
**530** stars · **44** stars/天 · 建立 12 天前 · Shell · 未標註授權
...
> **適合** 需要在使用 AI 編碼助手時降低令牌消耗的開發團隊。
> **一句話重點** 這個專案的核心在於通過簡化對話內容來實現令牌的高效使用，對於經常使用 AI 編碼助手的開發者來說，這是一個值得考慮的工具。
...
> **結論** 花 2 小時學、1 小時整合，得到 31% 的成本節省，值得考慮。

> [!abstract] 核心創新
> 通過持續的令牌效率技能，平均降低 31% 的費用，且不損失正確性。

## 專案簡介

這個專案旨在提高編碼代理（如 Claude Code、Codex 等）的令牌使用效率，透過優化對話內容來減少不必要的文字輸出。使用者只需執行一個簡單的安裝指令，便可啟用該功能，並可透過 `/token-diet [on|lite|ultra|off]` 指令隨時調整模式。其核心機制是通過減少冗餘的語言來降低令牌消耗，例如在回覆中省略多餘的引言和結尾，並且在文件讀取時只關注必要的行。該專案的「一句話賣點」是「以 31% 的平均成本節省，保持正確性」。技術上，它利用了 session hooks 來確保功能在每次會話中持續運作，這樣的設計避免了每次都需手動啟用的麻煩。

與同類工具相比，如 0x0funky/agent-sprite-forge 和 0xGF/boneyard，這個專案專注於令牌的節省，而不是單純的功能擴展，這使得它在需要高效能的場景中更具優勢。實際使用中，該工具在不同的會話類型中實現了高達 81% 的輸出節省，並且在保持正確性的同時，顯著降低了費用。社群活躍度良好，無開放問題，顯示出維護者對專案的重視。這個專案目前處於早期階段，適合需要優化令牌使用的開發團隊，尤其是那些經常使用 AI 編碼助手的團隊。

**技術棧**：`Shell` · `JavaScript` · `Python`

## 重點功能

- 持續運作 — 安裝後自動啟用，無需每次手動設置。
- 多層次模式 — 支援 `on`、`lite`、`ultra` 和 `off`，可根據需求調整輸出精簡程度。
- 減少冗餘 — 在回覆中省略多餘的引言和結尾，直接給出關鍵信息。
- 優化測試 — 僅針對關鍵和邊緣路徑進行測試，最多 10 個測試案例。
- 批量處理 — 針對工具的獨立調用進行批量處理，減少不必要的請求。
- 子代理委派 — 將廣泛的搜索委派給更便宜的模型，保持正確性。
- 高效的上下文處理 — 只讀取必要的行，避免重複讀取已編輯的文件。
...
> [!tip] 爆紅原因
> 建立 12 天內累積 530 stars（44/天），forks 4（0.8%），這顯示出穩定的增長。作者 Kulaxyz 之前的專案經驗可能為此專案的成功奠定了基礎。這個工具解決了編碼代理在高頻使用時的費用問題，過去使用者常常面臨高額的 API 費用。近期的社群討論和需求也促進了這個工具的曝光。技術上，令牌效率的提升使得這個工具在當前的 AI 生態中變得尤為重要。低於 5% 的 forks/stars 比率顯示出使用者對此工具的實際應用仍在觀望階段。

## 適合誰使用

**目標受眾**：需要在使用 AI 編碼助手時降低令牌消耗的開發團隊。

> [!example] 使用場景
> - 後端工程師用它來優化與 Codex 的交互，因為可以降低 API 費用，平均節省 31%。
> - 資料科學家用它來減少與 AI 模型的對話令牌消耗，因為能夠在保持正確性的同時，顯著減少輸出內容。
> - 開發團隊用它來提高開發效率，因為能夠在測試和文檔生成中減少不必要的文字，從而節省時間和資源。

## 架構分析

該專案採用簡單的安裝腳本來配置和啟用令牌效率技能，這樣的設計使得用戶可以輕鬆上手。資料流方面，安裝過程會自動檢測並設定相應的編碼代理，並在每次會話開始時注入必要的指令。這樣的設計避免了用戶每次都需手動啟用的麻煩，提升了使用體驗。

選擇使用 Shell 腳本而非其他語言，因為這樣可以保證在多種環境下的兼容性。這種設計的代價是可能無法提供更複雜的功能擴展，但對於專注於令牌效率的需求來說，這樣的簡單性是非常合適的。擴展性方面，由於專案目前只針對特定的編碼代理，未來若要支持更多代理，可能需要進一步的開發和測試。

## 技術深入分析

token-diet 的核心技術機制在於通過減少冗餘的語言來降低令牌消耗，這涉及到對回覆內容的精簡處理。使用者可以選擇不同的模式來調整輸出內容的精簡程度，這樣的設計使得用戶能夠根據需求靈活調整。該工具在不同的會話類型中實現了高達 81% 的輸出節省，這表明其在處理大量文本時的高效性。由於專案主要依賴 Shell 腳本，這使得其安裝和使用都非常方便，但也限制了其功能的擴展性。未來若要支持更多的編碼代理，可能需要進一步的開發和測試。設計上，選擇使用簡單的安裝腳本而非複雜的框架，這樣的選擇使得專案在多種環境下都能良好運作，但也可能在功能上有所妥協。對於使用者來說，這個工具在實際應用中能夠有效降低成本，並且保持了相對的正確性，這是其最大的優勢。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，包含安裝步驟和使用範例。安裝過程相對順暢，沒有明顯的坑。文件中未提供多語言支持，但基本的使用說明已經足夠讓新手快速上手。
...
> [!warning] 注意事項
> - 僅支援特定的編碼代理（Claude Code、Codex、Cursor 等）。
> - 在某些情況下，過度簡化可能導致信息遺失。
> - 需要用戶理解不同模式的效果，以便選擇合適的設置。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於多代理的協作，而 token-diet 專注於單一代理的令牌效率。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供更廣泛的功能擴展，但不專注於令牌消耗的優化。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於多代理的協作，而 token-diet 專注於單一代理的令牌效率。 | 如果你的團隊需要多個代理的協作而非單一代理的效率優化。 | medium，因為需要重新配置多個代理的設置。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供更廣泛的功能擴展，但不專注於令牌消耗的優化。 | 如果你需要更多功能擴展而不僅僅是令牌效率的提升。 | high，因為需要重新設計整個工作流程以適應新的功能。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **token-diet** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於多代理的協作，而 token-diet 專注於單一代理的令牌效率。 | 提供更廣泛的功能擴展，但不專注於令牌消耗的優化。 |
> | 遷移成本 | - | medium，因為需要重新配置多個代理的設置。 | high，因為需要重新設計整個工作流程以適應新的功能。 |
> | 適用場景 | 主要場景 | 如果你的團隊需要多個代理的協作而非單一代理的效率優化。 | 如果你需要更多功能擴展而不僅僅是令牌效率的提升。 |
...
- [MEDIUM] 在某些情況下，過度簡化可能導致信息遺失，影響溝通效果。
  - 解法：使用 `lite` 或 `on` 模式以獲取更完整的回覆。
- **[HIGH]** 僅支援特定的編碼代理，限制了使用範圍。
  - 解法：確保你的環境中已安裝支持的代理。
- [MEDIUM] 需要用戶對不同模式的效果有一定理解，否則可能無法達到預期效果。
  - 解法：參考 README 中的模式說明，選擇合適的模式。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 能有效降低 API 使用成本，提升開發效率。 |
| 大型企業的多代理系統 | 不適合 | 目前僅支援特定的編碼代理，無法滿足多代理需求。 |
| 需要經常與 AI 編碼助手互動的開發者 | 非常適合 | 能顯著降低令牌消耗，節省開支。 |
| 對於需要高精度回覆的專案 | 普通 | 過度簡化可能導致信息遺失，影響溝通效果。 |
...
> [!tip] 投入 vs 回報
> 花 2 小時學、1 小時整合，得到 31% 的成本節省，值得考慮。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限，且不存取敏感資料，依賴鏈中的風險較低，適合在 CI/CD 中使用。
...
**社群活躍度**：社群活躍度良好，無開放問題，顯示出維護者對專案的重視。
