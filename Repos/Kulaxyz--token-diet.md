...
use_case: "為編碼代理提供持續的 token 效率優化，平均降低 31% 的費用，且不損失正確性。"
priority: high
ring: assess
...
aliases:
  - "token-diet"
  - "Kulaxyz/token-diet"
  - "為編碼代理提供持續的 token 效率優化，平均降低 31% 的費用，且不損失正確性。"
---

# token-diet

**508** stars · **25** stars/天 · 建立 20 天前 · Shell · 未標註授權
...
> **授權** 未標註授權 (風險較高) · **維護** Moderate (最後推送 19 天前) · **貢獻者** Solo (bus factor 風險) · **參與度** Low
> **適合** 需要與編碼代理頻繁互動並希望降低成本的開發者。
> **一句話重點** 這個專案的價值在於它能在不影響正確性的情況下，顯著降低與編碼代理互動的成本。
...
> **結論** 花 2 小時學、1 小時整合，得到顯著的 token 成本降低，值得採用。

> [!abstract] 核心創新
> 這個專案的創新在於提供了一個持續的 token 效率優化技能，能在不影響正確性的情況下顯著降低成本。

## 專案簡介

這個專案的核心功能是透過持續的 token 效率優化，幫助編碼代理（如 Claude Code、Codex 等）在整個會話中減少不必要的 token 使用，從而降低成本。使用者可以透過簡單的安裝指令 `curl -fsSL https://raw.githubusercontent.com/Kulaxyz/token-diet/main/install.sh | bash` 來安裝，並選擇不同的模式（如 `--ultra`）來調整輸出精簡程度。這樣的設計讓使用者能夠在不影響正確性的情況下，獲得更高效的輸出，具體表現在對話、文件、測試和代碼的處理上。技術上，這個工具利用了 session hook 來持續應用優化，並且在多種情境下進行測試，顯示出在不同類型的會話中都能達到顯著的 token 減少效果。與其他工具相比，token-diet 不僅能減少輸出 token，還能保持高正確性，這是許多類似工具所無法做到的。

實際使用中，這個工具在處理大量代碼變更和測試時，能夠有效降低成本，特別是在輸出密集的情境下，平均可降低 31% 的費用。社群活躍度高，且目前無開放問題，顯示出良好的維護狀態。對於需要經常與編碼代理互動的開發者來說，這是一個值得考慮的工具，特別是當他們希望控制成本時。未來，隨著更多人使用這個工具，可能會進一步優化其性能和功能。

**技術棧**：`Shell` · `JavaScript` · `Python`

## 重點功能

- 持續的 token 效率優化 — 在整個會話中減少不必要的 token 使用，平均降低 31% 的費用。
- 多種模式選擇 — 支持 `on`, `lite`, `ultra`, `off` 模式，根據需求調整輸出精簡程度。
- 自動安裝 — 使用簡單的 curl 命令安裝，並能自動檢測支持的代理。
- 針對性測試 — 只測試關鍵路徑，確保不會漏掉重要的測試。
- 上下文管理 — 只讀取必要的行，減少不必要的文件讀取。
...
> [!tip] 爆紅原因
> 建立 20 天內累積 508 stars（25/天），forks 4（0.8%），顯示出穩定的增長。作者 Kulaxyz 在這個領域有一定的經驗，解決了以往編碼代理在 token 使用上的高成本問題，這在過去的工具中並未得到有效解決。這個工具的出現，正好填補了這一空白，並且在社群中引起了討論。技術上，這個工具的設計使其能夠持續運作，這在其他同類工具中並不常見。forks/stars 比率偏低，顯示大多數使用者仍在觀望。

## 適合誰使用

**目標受眾**：需要與編碼代理頻繁互動並希望降低成本的開發者。

> [!example] 使用場景
> - 後端工程師用它來優化與 Codex 的互動，因為能有效降低 token 使用，節省開支。
> - 資料科學家用它來減少在生成文檔時的成本，因為能保持輸出正確性且減少多餘的文字。
> - DevOps 工程師用它來在 CI/CD 流程中與多個代理進行互動，因為能在不影響準確性的情況下，減少每次請求的 token 數量。

## 架構分析

token-diet 採用了一種持續運作的架構，透過 session hook 來自動應用優化，這樣的設計使得使用者無需每次都手動啟用。資料流方面，當用戶與編碼代理互動時，token-diet 會在每次請求中自動減少不必要的 token 使用，這樣能夠有效降低成本。選擇這種架構的代價是需要依賴特定的代理環境，可能會限制其適用性。擴展性方面，這種設計在多代理環境中表現良好，但在單一代理的情境下可能未必能發揮最大效能。

## 技術深入分析

token-diet 的核心技術機制在於其持續的 token 效率優化，透過 session hook 的方式自動應用於每次與編碼代理的互動。這樣的設計使得使用者無需手動啟用，提升了使用的便利性。效能方面，這個工具在多種情境下都能保持高效，特別是在輸出密集的場景中，能有效降低 31% 的費用。設計上選擇了 Shell、JavaScript 和 Python 作為技術棧，這使得工具的安裝和運行都相對簡單，但也限制了其在 Windows 環境下的使用。技術風險方面，這個工具的依賴性相對較低，但在特定的編碼代理環境中可能會遇到兼容性問題。整合方面，token-diet 能夠輕鬆融入現有的開發流程，特別是在使用 CI/CD 工具時，能夠自動化優化過程，減少手動干預的需求。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且提供了安裝範例，安裝過程順暢，無明顯坑。文件中有簡單的使用指南，但缺乏多語言支持，可能對非英語使用者造成障礙。

## 優缺點分析

> [!success] 優點
> - 能有效降低 token 使用，平均節省 31% 的費用。
> - 持續運作，無需手動啟用，提升使用便利性。
> - 支持多種模式，能根據需求調整輸出精簡程度。

> [!danger] 缺點
> - 僅支援特定的編碼代理，限制了使用範圍。
> - 在某些情境下可能需要手動調整以達到最佳效果。
> - 不支援 Windows 環境，需在類 Unix 系統上運行。

> [!warning] 注意事項
> - 僅支援特定的編碼代理（Claude Code、Codex 等）。
> - 在某些情境下可能需要手動調整模式以獲得最佳效果。
> - 不支援 Windows 環境，需在類 Unix 系統上運行。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 這個工具專注於創建多種代理的協作，而 token-diet 專注於降低 token 使用成本，適合需要多代理協作的情境。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | boneyard 提供了多種代理的功能，但不專注於 token 優化，適合需要多功能的開發者。 |
| [0xSero/codex-shim](https://github.com/0xSero/codex-shim) | codex-shim 提供了對 Codex 的擴展功能，但不具備 token 效率優化的特性，適合需要擴展 Codex 功能的使用者。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於多代理協作，而 token-diet 專注於降低 token 使用成本，適合需要多代理協作的情境。 | 如果你的專案需要多個代理之間的協作，且不太關心 token 使用的優化。 | medium，因為需要重新調整整個代理的協作邏輯。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供多種功能的代理，但不專注於 token 優化，適合需要多功能的開發者。 | 如果你的專案需要多種功能的支持，而不僅僅是 token 優化。 | medium，因為需要重新評估功能需求。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **token-diet** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於多代理協作，而 token-diet 專注於降低 token 使用成本，適合需要多代理協作的情境。 | 提供多種功能的代理，但不專注於 token 優化，適合需要多功能的開發者。 |
> | 遷移成本 | - | medium，因為需要重新調整整個代理的協作邏輯。 | medium，因為需要重新評估功能需求。 |
> | 適用場景 | 主要場景 | 如果你的專案需要多個代理之間的協作，且不太關心 token  | 如果你的專案需要多種功能的支持，而不僅僅是 token 優化 |
...
- [MEDIUM] 在某些情境下，可能需要手動調整模式以達到最佳效果。
  - 解法：根據具體情境調整模式設置。
- **[HIGH]** 不支援 Windows 環境，可能會導致無法使用。
  - 解法：在類 Unix 系統上運行。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 能有效降低與編碼代理的互動成本，適合小型團隊的預算。 |
| 大型企業的多代理系統 | 普通 | 雖然能降低成本，但在多代理協作中可能需要更多的調整。 |
| 個人開發者的 side project | 非常適合 | 能夠輕鬆安裝並降低成本，適合個人開發者的需求。  |
...
> [!tip] 投入 vs 回報
> 花 2 小時學、1 小時整合，得到顯著的 token 成本降低，值得採用。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：這個工具不需要高權限運行，且不會存取敏感資料，適合在 CI/CD pipeline 中安全使用。

## 生態系整合

> [!abstract] 如何融入你的工具鏈

token-diet 最常與編碼代理搭配使用，特別是在開發和測試階段。實際整合範例中，開發者可以在使用 Codex 進行代碼生成時，透過 token-diet 來優化輸出，具體做法是安裝後直接在開發環境中啟用。這個工具支援與 GitHub Actions 等 CI 工具的整合，能夠自動化優化過程。與主流 IDE（如 VS Code）相容良好，無需額外的插件或擴展。整合的摩擦點主要在於需要確保代理環境的兼容性，可能會遇到版本不一致的問題。
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["自動化測試","API 設計","微服務"];
