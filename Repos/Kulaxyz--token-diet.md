...
star_history: "2026-07-06:590,2026-07-06:590,2026-07-07:596,2026-07-08:611,2026-07-09:566,2026-07-10:555,2026-07-11:557,2026-07-12:560,2026-07-13:561,2026-07-14:549"
tags:
  - github
  - "category/開發工具"
  - "lang/shell"
...
**549** stars · **55** stars/天 · 建立 10 天前 · Shell · 未標註授權
...
> **適合** 希望降低編碼代理運行成本的開發者，特別是在大型專案中。
> **一句話重點** 這個專案不僅能降低成本，還能提升編碼代理的效率，特別適合需要長時間運行的場景。
...
> **結論** 花 2 小時學、1 小時整合，得到顯著的運行成本降低，值得採用。

> [!abstract] 核心創新
> 這個專案的創新在於提供一個持續運行的令牌效率技能，專為多種編碼代理設計。

## 專案簡介

這個專案的核心機制是提供一個持續運行的令牌效率技能，專為多種編碼代理（如 Claude Code、Codex 等）設計。使用者只需執行一條簡單的安裝指令，便能自動偵測並安裝到目標代理中。它會在整個會話中修剪不必要的令牌，從而在不損失正確性的情況下，顯著降低費用，平均可達 31% 的節省，並在某些情況下輸出減少高達 81%。這樣的設計不僅能減少開支，還能提高效率，因為它會優化回應的結構，去除多餘的前言和後語，並專注於關鍵信息。技術上，它利用了 SessionStart 鉤子或持久的上下文文件來確保技能始終啟用，這樣的設計使得用戶無需每次都手動啟用。

與其他工具相比，如 0x0funky/agent-sprite-forge 和 0xGF/boneyard，這個專案的獨特之處在於它的持續運行能力和對多種代理的支持，這使得它在需要高效能和低成本的場景中表現出色。使用者可能會發現，在處理大量輸出或代碼變更時，這個工具能顯著減少開支，尤其是在大型專案中。儘管如此，對於小型專案或不需要高頻率互動的情況，可能無法充分發揮其優勢。整體來看，這是一個針對需要長時間運行的編碼代理的高效工具，適合希望降低運行成本的開發者。

**技術棧**：`Shell` · `JavaScript` · `Python`

## 重點功能

- 持續運行的令牌效率 — 自動修剪會話中的不必要令牌，無需手動啟用。
- 多種代理支持 — 兼容 Claude Code、Codex、Cursor 等多個編碼代理。
- 多級模式選擇 — 提供 `on`、`lite`、`ultra` 和 `off` 模式，滿足不同需求。
- 精簡回應內容 — 去除多餘的前言和後語，直接提供關鍵信息。
- 批量處理工具調用 — 在迭代時，能夠針對性地進行測試運行，減少不必要的調用。
...
> [!tip] 爆紅原因
> 建立 10 天內累積 549 stars（55/天），forks 3（0.5%），這顯示出一定的關注度。作者 Kulaxyz 之前在這個領域有過其他貢獻，這個專案解決了長期以來編碼代理的令牌費用過高的問題，之前的解決方案往往無法兼顧效率和正確性。這個工具的出現正好填補了這一空白，並且在社交媒體上引起了一些討論，進一步推動了它的流行。技術上，這個工具的設計使得它能夠在多種編碼代理中運行，這在目前的生態系中相當罕見，這也可能是它受到關注的原因之一。

## 適合誰使用

**目標受眾**：希望降低編碼代理運行成本的開發者，特別是在大型專案中。

> [!example] 使用場景
> - 後端工程師用它來優化與 Claude Code 的互動，因為它能減少 31% 的運行費用，讓預算更有效。
> - 資料科學家用它來快速生成測試用例，因為它能在保持正確性的同時，減少輸出內容，提升效率。
> - 全端開發者用它來與 Codex 進行代碼生成，因為它能在不影響代碼質量的情況下，減少令牌消耗。

## 架構分析

這個專案採用了一個簡單的安裝腳本，通過 curl 命令下載並執行安裝。這樣的設計使得用戶可以快速上手，無需繁瑣的配置。資料流方面，安裝腳本會自動偵測目標代理並進行配置，這樣的設計降低了使用門檻。

選擇使用 Shell 腳本的原因是其跨平台的兼容性，能夠在多種環境中運行。這樣的架構雖然簡單，但可能在高度自定義的環境中遇到挑戰，因為它依賴於標準的 Shell 環境。整體來看，這個架構在易用性和靈活性之間取得了良好的平衡。

## 技術深入分析

這個專案的核心技術機制是通過修剪不必要的令牌來提高編碼代理的效率。它使用了一個持續運行的技能，這意味著用戶無需每次都手動啟用，這在長期使用中能顯著減少操作負擔。效能方面，根據實際測試，這個工具在不同會話類型中能夠達到高達 81% 的輸出節省，並且在費用上平均降低 31%。這樣的設計選擇使得它能夠在多種代理中運行，這在目前的生態系中相當罕見。

技術上，它依賴於 Shell 腳本的簡單性，這使得它在安裝和使用上都相對輕量。這個選擇雖然降低了使用門檻，但在高度自定義的環境中可能會遇到挑戰。安全性方面，這個工具不需要高權限運行，並且不會存取敏感資料，這使得它在 CI/CD 環境中使用時相對安全。整合方面，這個工具能夠與多種編碼代理無縫配合，這使得它在現有的技術生態中具有良好的兼容性。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了詳細的安裝指令和使用範例。安裝過程相對順暢，沒有明顯的坑。文件中缺乏多語言支持，但整體上能夠讓新手在 30 分鐘內上手。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 這個工具專注於創建和管理多個代理，而 token-diet 更注重於令牌效率和成本控制。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | boneyard 提供了更廣泛的功能集，但在令牌效率方面不如 token-diet 專注。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 這個工具專注於創建和管理多個代理，而 token-diet 更注重於令牌效率和成本控制。 | 如果你的團隊需要管理多個代理並且希望有更高的靈活性，則應選擇 agent-sprite-forge。 | medium，因為需要重新配置代理管理流程。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | boneyard 提供了更廣泛的功能集，但在令牌效率方面不如 token-diet 專注。 | 如果你的專案需要多樣的功能而不僅僅是令牌效率，則應選擇 boneyard。 | high，因為需要重新設計整個工作流。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **token-diet** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 這個工具專注於創建和管理多個代理，而 token-diet 更注重於令牌效率和成本控制。 | boneyard 提供了更廣泛的功能集，但在令牌效率方面不如 token-diet 專注。 |
> | 遷移成本 | - | medium，因為需要重新配置代理管理流程。 | high，因為需要重新設計整個工作流。 |
> | 適用場景 | 主要場景 | 如果你的團隊需要管理多個代理並且希望有更高的靈活性，則應選擇 | 如果你的專案需要多樣的功能而不僅僅是令牌效率，則應選擇 bo |
...
- [MEDIUM] 在特定環境中安裝可能失敗，特別是缺少依賴的情況下。
  - 解法：確保所有必要的依賴已安裝，或手動配置環境。
- [low] 在小型專案中可能無法充分發揮其優勢。
  - 解法：在小型專案中考慮使用其他工具。
- [MEDIUM] 對於不熟悉 Shell 腳本的用戶，可能需要額外的學習成本。
  - 解法：參考 Shell 腳本的基本知識。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 能夠顯著降低運行成本，提升效率。 |
| 大型企業的多代理系統 | 適合 | 支持多種編碼代理，靈活性高。 |
| 小型個人專案 | 普通 | 在小型專案中可能無法充分發揮其優勢。 |
| 需要高頻率互動的應用 | 非常適合 | 能夠持續運行，減少手動干預。 |
...
> [!tip] 投入 vs 回報
> 花 2 小時學、1 小時整合，得到顯著的運行成本降低，值得採用。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：這個工具不需要高權限運行，也不會存取敏感資料，適合在 CI/CD 環境中使用。
...
**社群活躍度**：社群活躍度中等，開放問題數量為 0，顯示出良好的維護狀態。
