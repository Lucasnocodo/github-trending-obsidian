...
use_case: "為編碼代理提供持續的代幣效率技能，平均降低約31%的費用，且不影響正確性。"
priority: high
ring: assess
...
star_history: "2026-07-06:590,2026-07-06:590,2026-07-07:596,2026-07-08:611,2026-07-09:566,2026-07-10:555,2026-07-11:557,2026-07-12:560,2026-07-13:561,2026-07-14:549,2026-07-15:544,2026-07-16:530,2026-07-17:531,2026-07-18:527,2026-07-19:531,2026-07-20:533,2026-07-21:525,2026-07-22:524,2026-07-23:519"
tags:
  - github
  - "category/開發工具"
  - "lang/shell"
...
aliases:
  - "token-diet"
  - "Kulaxyz/token-diet"
  - "為編碼代理提供持續的代幣效率技能，平均降低約31%的費用，且不影響正確性。"
---

# token-diet

**519** stars · **27** stars/天 · 建立 19 天前 · Shell · 未標註授權
...
> **適合** 需要在多次交互中降低代幣成本的開發者和產品經理。
> **一句話重點** 這個專案的核心價值在於它能夠在不影響正確性的情況下，顯著降低代幣消耗，對於長時間使用編碼代理的開發者來說，這是一個不可或缺的工具。
...
> **結論** 花 2 小時學、1 小時整合，得到顯著的代幣節省，值得採用。

> [!abstract] 核心創新
> 通過減少冗餘內容，實現代幣使用的顯著降低，同時保持正確性。

## 專案簡介

token-diet 是一個專為編碼代理（如 Claude Code、Codex 等）設計的工具，旨在優化代幣使用效率。它通過減少不必要的文字和上下文來降低代幣消耗，從而在整個會話中實現約31%的費用節省。使用者可以透過簡單的安裝指令 `curl -fsSL https://raw.githubusercontent.com/Kulaxyz/token-diet/main/install.sh | bash` 來安裝，並可選擇不同的模式（如 `--ultra`）來調整輸出精簡程度。這個工具的核心在於它能夠在不損失正確性的情況下，減少回覆中的冗餘內容，並且在測試和代碼生成時僅聚焦於關鍵部分。技術上，它利用了會話開始時的鉤子來持續應用這些優化，確保每次會話都能自動生效。

與其他類似工具相比，如 0x0funky/agent-sprite-forge，token-diet 更加專注於代幣消耗的直接減少，而不是僅僅提高生成的內容質量。這使得它在需要高頻率交互的場景中，特別是對於大型專案的開發，能夠顯著降低成本。使用者在實際運行中可能會發現，雖然輸出量減少，但正確性和關鍵測試覆蓋率仍然保持在高水平。這個工具適合需要長時間與編碼代理互動的開發者，尤其是在資源有限的情況下。未來幾個月，隨著使用者反饋的積累，可能會進一步優化其功能和性能。

**技術棧**：`Shell` · `JavaScript` · `Python`

## 重點功能

- 自動化安裝 — 使用 `curl` 指令快速安裝，支持多種代理。
- 多種模式選擇 — 提供 `on`, `lite`, `ultra`, `off` 等模式以調整輸出精簡程度。
- 減少冗餘內容 — 回覆中去除不必要的前言和後語，僅保留關鍵信息。
- 批量處理工具調用 — 支持獨立調用的批量處理，減少不必要的請求。
- 持續運行 — 透過會話開始鉤子確保每次會話都能自動應用優化。
...
> [!tip] 爆紅原因
> 建立 19 天內累積 519 stars（27/天），forks 僅 4（0.8%），顯示出相對較低的實際修改需求。作者 Kulaxyz 之前在開源社區活躍，這個專案解決了代幣使用效率低下的問題，特別是在高頻率的編碼代理交互中。這個工具的出現正好滿足了對於成本控制的需求，尤其是在使用 AI 進行開發的背景下。由於其獨特的功能，吸引了不少開發者的注意，並且在社群中引發了討論。forks/stars 比率低，顯示出使用者主要是觀望而非積極修改，可能是因為這個工具的使用相對簡單，且功能明確。

## 適合誰使用

**目標受眾**：需要在多次交互中降低代幣成本的開發者和產品經理。

> [!example] 使用場景
> - 後端工程師用它來優化與 Codex 的互動，減少代幣消耗，因為在大型專案中，代幣成本會迅速累積。
> - AI 開發者用它來在長時間的測試中降低費用，因為它能夠在保持正確性的同時，顯著減少輸出內容。
> - 產品經理用它來與 Claude Code 進行高效的需求討論，因為它能夠快速提供關鍵資訊而不浪費代幣。

## 架構分析

token-diet 的架構設計上，採用簡單的 shell 腳本來進行安裝和配置，這使得用戶能夠快速上手。它的核心功能是通過會話開始的鉤子來持續應用代幣優化，這樣的設計能夠確保每次會話都能自動生效，減少了用戶的操作負擔。資料流方面，使用者的請求會經過優化處理，僅保留必要的內容，這樣可以有效降低代幣消耗。選擇這種設計的代價是可能在某些情況下影響上下文的完整性，但整體上能夠顯著提高效率。擴展性方面，這個工具能夠輕鬆整合到現有的開發流程中，並且不會對系統性能造成太大影響。

## 技術深入分析

token-diet 的核心技術機制在於通過減少冗餘的文字輸出來達到代幣的節省。它利用了會話開始時的鉤子來持續應用這些優化，這樣的設計使得每次會話都能自動生效，從而減少了用戶的操作負擔。在效能方面，這個工具能夠在多種場景下實現顯著的代幣節省，例如在輸出密集的會話中，能夠達到高達81%的節省。設計上選擇使用 shell 腳本進行安裝和配置，這樣的選擇使得用戶能夠快速上手，但也可能在某些情況下影響上下文的完整性。技術風險方面，過度精簡可能會導致某些關鍵信息的遺失，這在實際使用中需要特別注意。整合方面，這個工具能夠輕鬆融入現有的開發流程，並且不會對系統性能造成太大影響，對於需要長時間與編碼代理互動的開發者來說，這是一個非常實用的工具。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且提供了安裝指令，讓新手能夠快速上手。安裝過程順暢，沒有明顯的坑。文件中有提供使用範例，能夠幫助使用者理解如何啟用功能。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於生成代理的能力提升，而非直接降低代幣消耗，適合需要高質量輸出的場景。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供更廣泛的功能集，但在代幣效率上不如 token-diet，適合多樣化需求的開發者。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於生成代理的能力提升，而非直接降低代幣消耗，適合需要高質量輸出的場景。 | 如果你的專案需要高質量的生成內容，而不太在意代幣消耗。 | medium，因為需要調整使用方式和集成流程。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供更廣泛的功能集，但在代幣效率上不如 token-diet，適合多樣化需求的開發者。 | 如果你的需求不僅限於代幣效率，還需要其他功能。 | low，因為功能上有一定的相似性。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **token-diet** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於生成代理的能力提升，而非直接降低代幣消耗，適合需要高質量輸出的場景。 | 提供更廣泛的功能集，但在代幣效率上不如 token-diet，適合多樣化需求的開發者。 |
> | 遷移成本 | - | medium，因為需要調整使用方式和集成流程。 | low，因為功能上有一定的相似性。 |
> | 適用場景 | 主要場景 | 如果你的專案需要高質量的生成內容，而不太在意代幣消耗。 | 如果你的需求不僅限於代幣效率，還需要其他功能。 |
...
- [MEDIUM] 在某些情況下，過度精簡可能會導致關鍵信息的遺失。
  - 解法：使用 `lite` 或 `on` 模式以保留更多上下文。
- **[HIGH]** 安裝過程中可能會遇到網路連接問題。
  - 解法：確保網路穩定，或考慮手動下載安裝。
- [low] 不支援某些舊版的編碼代理。
  - 解法：檢查代理版本，確保使用最新版本。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 能夠有效降低代幣成本，適合資源有限的團隊。 |
| 大型企業的長期開發專案 | 適合 | 在高頻率的交互中能夠顯著降低費用。 |
| 個人開發者的 side project | 非常適合 | 簡單的安裝和使用，能快速上手。 |
| 需要高質量生成內容的專案 | 不適合 | 過度精簡可能影響內容的完整性。 |
