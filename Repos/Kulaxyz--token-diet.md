...
star_history: "2026-07-06:590,2026-07-06:590,2026-07-07:596,2026-07-08:611,2026-07-09:566,2026-07-10:555,2026-07-11:557"
tags:
  - github
  - "category/開發工具"
  - "lang/shell"
...
**557** stars · **80** stars/天 · 建立 7 天前 · Shell · 未標註授權
...
> [!info] 速覽
> **安裝難度** Easy · **專案狀態** Brand New · **熱度** Growing (80 stars/day)
> **授權** 未標註授權 (風險較高) · **維護** Active (最後推送 6 天前) · **貢獻者** Solo (bus factor 風險) · **參與度** Low
> **適合** 需要長時間運行編碼代理並希望降低成本的開發者。
> **一句話重點** 這個工具的真正價值在於它能在不影響正確性的情況下，幫助開發者節省大量的令牌費用。
...
> [!abstract] 核心創新
> 這個專案的創新點在於能在不損失正確性的情況下，顯著降低編碼代理的令牌消耗。

## 專案簡介

這個工具的核心機制是透過優化對話過程中的令牌使用，從而降低成本。使用者可以透過簡單的安裝指令，例如 `curl -fsSL https://raw.githubusercontent.com/Kulaxyz/token-diet/main/install.sh | bash`，輕鬆將其整合到現有的編碼代理中。它的設計理念是「修剪多餘，保留精華」，確保在回應、文檔、測試和代碼中都能保持高效能。具體來說，這個工具會在回應中省略不必要的前言和後語，並且在測試中只關注關鍵路徑，從而減少令牌的使用量。這樣的設計不僅能降低費用，還能在不影響正確性的情況下提升效率。

技術上，它支持多種編碼代理，如 Claude Code 和 Codex，並提供不同的運行模式（如 `on`、`lite`、`ultra` 和 `off`），讓使用者可以根據需求調整令牌的使用策略。與其他工具相比，如 0x0funky/agent-sprite-forge 和 0xGF/boneyard，token-diet 更加專注於令牌的精簡和成本控制，特別適合需要長時間運行的編碼代理。實際使用中，這個工具能在多種場景下有效降低令牌消耗，並且不會影響到生成的代碼質量或正確性。對於需要經常使用編碼代理的開發者來說，這是一個值得考慮的選擇。

**技術棧**：`Shell` · `JavaScript` · `Python`

## 重點功能

- 令牌優化 — 在整個會話中修剪多餘的令牌，平均降低約31%的費用。
- 多模式支持 — 提供 `on`、`lite`、`ultra` 和 `off` 模式，讓使用者根據需求調整令牌使用策略。
- 自動安裝 — 使用簡單的 `curl` 指令即可快速安裝，支持多種編碼代理。
- 針對性測試 — 測試中只關注關鍵路徑，確保不會漏掉重要的測試。
- 即時控制 — 使用 `/token-diet [on|lite|ultra|off]` 指令隨時調整運行狀態。
...
> [!tip] 爆紅原因
> 建立 7 天內累積 557 stars（79.57/天），forks 3（0.5%），顯示出一定的關注度。作者 Kulaxyz 之前在開源社群中活躍，這個工具解決了編碼代理在長時間運行時令牌消耗過高的問題，之前的方案往往無法有效控制成本。這個工具的推出正好填補了這一空白，並且在社群中引起了討論。技術上，隨著 AI 編碼代理的普及，對於令牌效率的需求也在上升，這使得 token-diet 的出現變得更加合時宜。

## 適合誰使用

**目標受眾**：需要長時間運行編碼代理並希望降低成本的開發者。

> [!example] 使用場景
> - 後端工程師用它來優化與 Codex 的互動，因為能有效降低令牌費用，平均節省約31%。
> - 資料科學家用它來進行長時間的模型訓練，因為它能減少不必要的回應，提升輸出效率。
> - 開發團隊用它來整合到 CI/CD 流程中，因為它能在不影響正確性的情況下降低成本。

## 架構分析

這個專案採用簡單的 Shell 腳本作為安裝和運行的核心，因為這樣可以確保輕量和易於部署。資料流方面，安裝過程會自動檢測環境中的編碼代理，並根據用戶的需求進行配置。選擇 Shell 腳本的代價是可能在某些環境中缺乏靈活性，但這樣的設計使得使用者能夠快速上手。擴展性方面，這個工具可以隨著新的編碼代理的出現而進行更新，保持其競爭力。

## 技術深入分析

這個專案的核心技術機制是透過優化令牌的使用來降低成本，具體包括省略不必要的前言和後語，並在測試中只關注關鍵路徑。它的設計模式是基於命令行的交互，這使得使用者能夠靈活地控制功能開啟與關閉。效能方面，這個工具能在多種場景下有效降低令牌消耗，並且在實際測試中證明了其有效性。選擇 Shell 腳本作為實現語言的好處在於其輕量和易於部署，但也可能在某些環境中導致兼容性問題。技術風險方面，過度優化可能會導致信息缺失，這在某些情況下可能影響到生成的代碼質量。整合方面，這個工具能夠與現有的編碼代理無縫對接，並且不會對現有的工作流造成太大影響。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且提供了安裝範例，安裝過程順暢且無明顯坑點。文件中有詳細的使用說明，能夠幫助新手快速上手。整體而言，花30分鐘應該能夠順利運行起來。

## 優缺點分析

> [!success] 優點
> - 簡單易用的安裝過程，快速集成到現有的編碼代理中。
> - 能顯著降低令牌消耗，幫助開發者節省成本。
> - 提供多種運行模式，靈活適應不同的使用需求。

> [!danger] 缺點
> - 目前僅支持特定的編碼代理，限制了使用範圍。
> - 過度優化可能導致信息缺失，影響回應質量。
> - 需要在特定環境中運行，可能不適合所有開發者。

> [!warning] 注意事項
> - 目前僅支持指定的編碼代理，如 Claude Code 和 Codex。
> - 在某些情況下，過度優化可能導致信息缺失。
> - 需要在支持的環境中運行，可能不適用於所有開發環境。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於生成代理的多樣性，而 token-diet 更專注於令牌的效率和成本控制。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供多種代理的整合，但不如 token-diet 在令牌優化方面有效。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於生成代理的多樣性，而 token-diet 更專注於令牌的效率和成本控制。 | 如果需要多樣化的代理生成而不僅僅是令牌優化，則可以選擇此工具。 | medium，因為需要重新調整工作流程以適應不同的代理生成邏輯。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供多種代理的整合，但不如 token-diet 在令牌優化方面有效。 | 如果需要多種代理的整合而不僅僅是令牌效率，則可以考慮這個工具。 | low，因為可以輕鬆整合到現有的工作流中。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **token-diet** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於生成代理的多樣性，而 token-diet 更專注於令牌的效率和成本控制。 | 提供多種代理的整合，但不如 token-diet 在令牌優化方面有效。 |
> | 遷移成本 | - | medium，因為需要重新調整工作流程以適應不同的代理生成邏輯。 | low，因為可以輕鬆整合到現有的工作流中。 |
> | 適用場景 | 主要場景 | 如果需要多樣化的代理生成而不僅僅是令牌優化，則可以選擇此工具 | 如果需要多種代理的整合而不僅僅是令牌效率，則可以考慮這個工具 |
...
- [MEDIUM] 在某些環境中可能無法正確安裝，特別是缺少必要的依賴時。
  - 解法：確保環境中已安裝所有必要的依賴。
- **[HIGH]** 過度優化可能導致信息缺失，影響回應質量。
  - 解法：根據需要調整運行模式，避免使用過於激進的優化。
- [MEDIUM] 在某些編碼代理中可能無法完全發揮效果。
  - 解法：測試不同的代理，選擇最適合的配置。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型開發團隊使用 Codex 進行項目開發 | 非常適合 | 能有效降低令牌消耗，節省開發成本。 |
| 大型企業需要長時間運行的編碼代理 | 適合 | 能在不影響正確性的情況下降低成本。 |
| 獨立開發者進行個人項目 | 普通 | 雖然能降低成本，但對於小型項目來說可能不必要。 |
| 需要多樣化代理生成的開發環境 | 不適合 | 不如其他工具在代理生成方面有效。 |
...
> [!warning] 安全性快速掃描
> 低風險：這個工具不需要高權限運行，且不會存取敏感資料。依賴鏈的信任程度較高，適合在 CI/CD 中安全使用。

## 生態系整合

> [!abstract] 如何融入你的工具鏈

這個工具最常與各種編碼代理搭配使用，特別是在開發和測試階段。實際整合範例包括在使用 Codex 的項目中，通過安裝 token-diet 來優化令牌使用，具體做法是使用 `curl` 指令進行安裝。它與主流 CI 工具（如 GitHub Actions）相容，並且可以輕鬆集成到現有的開發流程中。整合的摩擦點主要在於需要確保環境中有正確的依賴，否則可能會影響安裝過程。
...
**社群活躍度**：社群活躍度中等，開發者在最近幾天有持續的更新和維護。
