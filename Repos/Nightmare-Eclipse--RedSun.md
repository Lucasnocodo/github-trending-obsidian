...
star_history: "2026-04-17:785,2026-04-17:793,2026-04-18:1354,2026-04-18:1358,2026-04-19:1500,2026-04-19:1500,2026-04-20:1573,2026-04-20:1573,2026-04-21:1642,2026-04-21:1643,2026-04-22:1685,2026-04-22:1685,2026-04-23:1716"
tags:
  - github
  - "category/安全"
  - "lang/c++"
...
**1.7k** stars · **245** stars/天 · 建立 7 天前 · C++ · MIT
...
> [!abstract] 核心創新
> RedSun 利用 Windows Defender 的特定行為來提升系統權限，展示了防毒軟體設計的幽默與矛盾。

## 專案簡介

RedSun 是一個針對 Windows Defender 的漏洞庫，利用其對於標記為惡意的檔案的處理方式來提升系統權限。具體來說，當 Windows Defender 偵測到一個有雲標籤的檔案時，它會將該檔案重寫回原始位置，而這個行為被 RedSun 利用來覆寫系統檔案，從而獲得管理員權限。這個 PoC 的核心賣點在於其幽默的設計，讓人不禁思考防毒軟體的邏輯。使用者只需執行提供的程式碼，即可觸發這一漏洞。這樣的設計不僅展示了漏洞的利用方式，還引發了對防毒軟體設計的質疑。RedSun 的技術實作是基於 C++，並且在效能上並未提供具體數據，但其設計的輕量性使得它能夠快速運行。與其他類似工具相比，如 0xGF/boneyard 和 AgentSeal/codeburn，RedSun 的獨特之處在於它專注於 Windows Defender 的特定行為，而非一般的漏洞利用。

這使得它在針對特定環境時更具針對性和有效性。使用者在實際操作中可能會遇到 Windows Defender 的誤報，這可能會影響其使用體驗。維護者的活躍度相對較高，且社群對於漏洞的討論也顯示出一定的興趣。這個專案目前處於 alpha 階段，適合對安全研究有興趣的開發者進行探索和實驗。預測未來幾個月內，隨著更多人關注這一漏洞，可能會有更多的討論和改進。對於想要深入了解 Windows 安全性和漏洞利用的開發者來說，RedSun 是一個值得一試的工具。

**技術棧**：`C++`

## 重點功能

- PoC 代碼 — 提供針對 Windows Defender 的漏洞利用範例，讓使用者能夠直接測試。
- 幽默的設計 — 透過有趣的方式展示漏洞，增加學習的趣味性。
- 高效能 — 基於 C++ 實作，能夠快速運行並觸發漏洞。
- 社群互動 — 討論區中有關於漏洞的熱烈討論，提供使用者更多的背景資訊。
- 開源 — 以 MIT 授權釋出，使用者可以自由修改和分享。
...
> [!tip] 爆紅原因
> 建立 7 天內累積 1716 stars（245/天），forks 376（21.9%），顯示出強烈的社群關注。作者 Nightmare-Eclipse 之前並未有太多知名專案，但這個專案解決了 Windows Defender 在處理惡意檔案時的邏輯漏洞，這在安全研究中是一個有趣的切入點。社群中對於這個漏洞的討論熱烈，尤其是關於其幽默性和實用性的反思，進一步促進了其流行。技術生態中，隨著對安全性越來越重視，這類漏洞的研究和利用工具自然會受到關注。forks/stars 比率為 21.9%，顯示出許多使用者對於這個專案的實際修改和使用。
...
> [!example] 使用場景
> - 安全研究人員用它來測試 Windows Defender 的漏洞，因為這能幫助他們了解防毒軟體的弱點和改進方向。
> - 滲透測試工程師用它來模擬攻擊，因為這能有效展示防毒軟體在特定情境下的反應。
> - 開發者用它來學習漏洞利用技術，因為這個專案提供了實用的 PoC 代碼和幽默的背景故事。

## 架構分析

RedSun 的架構設計是基於 C++，專注於利用 Windows Defender 的行為。其資料流主要是透過執行 PoC 代碼來觸發漏洞，這樣的設計使得使用者能夠快速驗證漏洞的存在。選擇 C++ 的原因在於其效能和對系統層級操作的支持，這使得漏洞利用更加高效。這種設計的代價在於需要使用者具備一定的 C++ 編譯和執行環境。擴展性方面，由於專注於特定漏洞，未來可能需要根據 Windows Defender 的更新進行調整。
...
| 工具 | 差異 |
| --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | boneyard 專注於多種漏洞的利用，而 RedSun 專注於 Windows Defender 的特定行為，適合針對特定環境進行測試。 |
| [AgentSeal/codeburn](https://github.com/AgentSeal/codeburn) | codeburn 提供更廣泛的漏洞利用工具，而 RedSun 則專注於一個具體的漏洞，適合深入研究。 |
...
**社群活躍度**：社群對於漏洞的討論熱烈，特別是在熱門 Issues 中。
