...
**1.9k** stars · **93** stars/天 · 建立 20 天前 · C++ · MIT
...
> **適合** 對安全研究和漏洞利用有興趣的開發者。
> **一句話重點** RedSun 利用 Windows Defender 的漏洞，揭示了反病毒軟體在特定情況下的邏輯錯誤。
...
> **結論** 花 5 小時學習，3 小時整合，能獲得對 Windows Defender 漏洞的深入理解，值得投入。

> [!abstract] 核心創新
> 利用 Windows Defender 的邏輯錯誤來提升系統權限。

## 專案簡介

RedSun 是一個專注於利用 Windows Defender 的漏洞來獲取系統管理權限的工具。它的核心機制是透過一個 PoC（Proof of Concept）代碼，當 Windows Defender 偵測到一個帶有雲標籤的惡意檔案時，反而會將該檔案重寫回原位置，這使得攻擊者能夠覆蓋系統檔案並獲得管理權限。這種行為的設計背後，反映了反病毒軟體在處理某些情況下的邏輯錯誤，這使得 RedSun 能夠在不被檢測的情況下執行其攻擊。使用者只需執行 `./RedSun`，便能啟動攻擊流程，這使得這個工具的使用門檻相對較低。技術上，這個工具是用 C++ 實作的，依賴於 Windows API 來進行檔案操作，這使得它能夠直接與作業系統進行互動。

相較於其他類似工具，如 0x0funky/agent-sprite-forge，RedSun 的優勢在於其針對 Windows Defender 的特定漏洞進行了深度優化，能夠在不被檢測的情況下執行攻擊，這在實際應用中可能會帶來更高的成功率。使用者在實際操作中可能會遇到 Windows Defender 立即檢測到檔案的問題，這需要用戶進行一些額外的配置來避開防護。這個工具目前處於 alpha 階段，社群活躍度中等，適合對安全研究和漏洞利用有興趣的開發者。未來幾個月內，隨著社群的回饋，可能會有更多的功能和修正釋出。

**技術棧**：`C++`

## 重點功能

- PoC 漏洞利用 — 利用 Windows Defender 的邏輯錯誤來提升權限。
- C++ 實作 — 直接與 Windows API 互動，進行檔案操作。
- 簡單執行 — 只需執行 `./RedSun` 即可啟動攻擊流程。
- 社群支持 — 有活躍的討論和問題解決。
- MIT 授權 — 允許自由使用和修改。
...
> [!tip] 爆紅原因
> 建立 20 天就累積 1866 stars（93/天），forks 405（21.7%），這顯示出強烈的社群關注。作者 Nightmare-Eclipse 以其在安全研究領域的經驗而聞名，這個專案解決了反病毒軟體在特定情況下的邏輯錯誤，這在過去的工具中並未得到充分利用。近期的社群討論和反應也顯示出對這一漏洞的興趣，尤其是對於如何利用此漏洞的探討。這樣的情況使得該專案在短時間內迅速獲得關注。

## 適合誰使用

**目標受眾**：對安全研究和漏洞利用有興趣的開發者。

> [!example] 使用場景
> - 安全研究人員用它來測試 Windows Defender 的漏洞，因為這能幫助他們了解防護系統的弱點。
> - 滲透測試工程師用它來模擬攻擊，因為這能提供真實世界中反病毒軟體的行為分析。
> - 開發者用它來學習如何利用系統漏洞，因為這能幫助他們提升對安全性問題的認識。

## 架構分析

RedSun 採用 C++ 實作，設計上選擇直接與 Windows API 互動，這樣能夠高效地進行檔案操作。資料流方面，當使用者執行工具時，會觸發一系列系統調用來覆蓋檔案。這種設計使得工具能夠在不被檢測的情況下運行，但也帶來了被防病毒軟體即時檢測的風險。選擇 C++ 而非其他語言，主要是因為其對系統資源的控制能力更強，能夠直接操作底層系統資源。擴展性方面，若未來需要支援其他作業系統，則需要重寫大量代碼。

## 技術深入分析

RedSun 的核心技術在於利用 Windows Defender 的行為邏輯，當檔案被標記為惡意時，反而會被重寫回原位置。這種設計使得攻擊者能夠覆蓋系統檔案，獲得管理權限。效能上，這個工具能夠快速執行，但在被防病毒軟體檢測時，可能會導致執行失敗。選擇 C++ 作為開發語言，主要是因為其對系統資源的高效控制能力，這使得工具能夠直接操作底層系統資源。這也帶來了依賴於 Windows API 的風險，若未來 Windows 更新改變了 API，可能會導致工具失效。技術風險方面，這種利用方式在未來可能會受到更嚴格的防護措施影響，導致其有效性下降。整合方面，這個工具與現有的滲透測試流程相容性良好，但在使用時需要考慮防病毒軟體的影響。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的介紹和使用範例，安裝過程相對順暢，但需要用戶具備一定的 C++ 編譯經驗。文件目前僅提供英文版本，對於非英語使用者可能會有一定的門檻。整體而言，花 30 分鐘內能夠成功運行工具。

## 優缺點分析

> [!success] 優點
> - 直接利用 Windows Defender 的漏洞，成功率高。
> - 簡單易用，適合快速測試。
> - 社群活躍，能獲得即時的支持和反饋。

> [!danger] 缺點
> - 僅限於 Windows 環境，無法跨平台使用。
> - 可能會被防病毒軟體即時檢測，影響使用體驗。
> - 目前仍在 alpha 階段，功能和穩定性可能不足。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於不同的漏洞利用技術，RedSun 針對 Windows Defender 的特定行為進行了優化。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供不同的攻擊向量，RedSun 的方法更具針對性和直接性。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於不同的漏洞利用技術，RedSun 針對 Windows Defender 的特定行為進行了優化。 | 如果需要更廣泛的漏洞利用技術，而不僅限於 Windows Defender 的行為。 | medium，因為需要重新學習不同的漏洞利用技術。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供不同的攻擊向量，RedSun 的方法更具針對性和直接性。 | 如果需要針對其他反病毒軟體進行測試，而不僅限於 Windows Defender。 | medium，因為需要適應不同的攻擊向量和技術。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **RedSun** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於不同的漏洞利用技術，RedSun 針對 Windows Defender 的特定行為進行了優化。 | 提供不同的攻擊向量，RedSun 的方法更具針對性和直接性。 |
> | 遷移成本 | - | medium，因為需要重新學習不同的漏洞利用技術。 | medium，因為需要適應不同的攻擊向量和技術。 |
> | 適用場景 | 主要場景 | 如果需要更廣泛的漏洞利用技術，而不僅限於 Windows D | 如果需要針對其他反病毒軟體進行測試，而不僅限於 Window |
...
- **[HIGH]** Windows Defender 可能會立即檢測到檔案，導致執行失敗。
  - 解法：需要在安全環境中進行測試，或關閉防病毒軟體。
- [MEDIUM] 在某些系統上可能無法正常執行，導致權限提升失敗。
  - 解法：確保使用者擁有管理員權限，並在兼容的 Windows 版本上運行。
- [MEDIUM] 使用過程中可能會遇到編譯錯誤，特別是在不同的環境中。
  - 解法：檢查依賴項和編譯環境，確保符合要求。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 安全研究團隊進行漏洞測試 | 非常適合 | 專注於 Windows Defender 的漏洞利用，能提供真實的測試結果。 |
| 小型企業的安全測試 | 普通 | 雖然有效，但可能會因防病毒軟體的干擾而影響測試結果。 |
| 個人學習安全漏洞利用 | 非常適合 | 提供簡單的使用方式，適合初學者學習。 |
| 大型企業的安全防護測試 | 不適合 | 可能會引發防病毒軟體的警報，影響正常業務運作。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，3 小時整合，能獲得對 Windows Defender 漏洞的深入理解，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 中等風險：該工具需要管理員權限來執行，並可能會存取系統檔案。使用時需謹慎，特別是在生產環境中。
