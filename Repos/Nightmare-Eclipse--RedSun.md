...
**1.8k** stars · **140** stars/天 · 建立 13 天前 · C++ · MIT
...
> **適合** 對 Windows 安全性有興趣的安全研究者和滲透測試工程師。
> **一句話重點** 這個專案揭示了防毒軟體在特定情況下的反直覺行為，並提供了一個有趣的測試工具。
...
> **結論** 花 5 小時學習，3 小時整合，得到對防毒軟體行為的深入理解，值得嘗試。

> [!abstract] 核心創新
> 利用防毒軟體的錯誤行為來獲取系統管理權限的創新方法。

## 專案簡介

RedSun 是一個針對 Windows Defender 的漏洞利用庫，透過一個有趣的 PoC（Proof of Concept）來展示如何利用防毒軟體的錯誤行為來獲取系統管理權限。具體來說，當 Windows Defender 偵測到一個帶有雲標籤的惡意檔案時，它竟然選擇將該檔案重寫回原位置，而不是刪除它。這個行為使得攻擊者可以利用此漏洞來覆蓋系統檔案並獲取管理權限。這個工具的賣點在於它揭示了防毒軟體在某些情況下的反直覺行為，並且提供了一個簡單的方式來測試這個漏洞。使用者只需編譯 PoC 代碼並執行，即可觸發漏洞。

這個工具的技術實作基於 C++，並且沒有重依賴，這使得它在使用上相對輕量。與其他漏洞利用工具相比，如 0x0funky/agent-sprite-forge 和 0xGF/boneyard，RedSun 的獨特之處在於它專注於防毒軟體的錯誤行為，而不是傳統的系統漏洞。實際使用中，這個工具可能會被 Windows Defender 立即檢測並阻擋，因此使用者需考慮如何在安全的環境中進行測試。這個專案目前處於早期階段，社群活躍度中等，對於想要了解防毒軟體行為的安全研究者來說，這是一個有趣的實驗工具。未來可能會有更多的功能擴展，特別是在漏洞利用方面的進一步開發。

**技術棧**：`C++`

## 重點功能

- 漏洞利用 PoC — 利用 Windows Defender 的錯誤行為來獲取系統管理權限。
- C++ 實作 — 輕量且無重依賴，方便編譯和執行。
- 即時檢測 — 能夠在測試過程中觀察到防毒軟體的反應。
- 社群互動 — 有活躍的討論和反饋，幫助使用者理解漏洞的影響。
- 開源 — 允許使用者自由修改和擴展功能。
...
> [!tip] 爆紅原因
> 建立 13 天就累積 1817 stars（140/天），forks 397（21.8%），這顯示出強烈的社群興趣。作者 Nightmare-Eclipse 在安全領域有一定的知名度，這個專案解決了防毒軟體在特定情況下的反直覺行為，之前的解決方案往往無法有效利用這種漏洞。最近的討論和反應也顯示出社群對這個漏洞的好奇心和實驗興趣，特別是對於如何在安全環境中測試這個 PoC 的探討。這個專案的成功也反映了當前安全研究中對於防毒軟體行為的關注。

## 適合誰使用

**目標受眾**：對 Windows 安全性有興趣的安全研究者和滲透測試工程師。

> [!example] 使用場景
> - 安全研究員用它來測試 Windows Defender 的行為，因為這可以揭示防毒軟體在特定情況下的漏洞。
> - 滲透測試工程師用它來模擬攻擊，因為這可以幫助他們評估系統的安全性。
> - 開發者用它來了解防毒軟體的運作原理，因為這有助於在開發過程中避免誤報。

## 架構分析

RedSun 的架構設計簡單明瞭，主要由一個 C++ 應用程式組成，專注於利用 Windows Defender 的行為。這個設計選擇使得專案輕量且易於編譯，適合快速測試。資料流方面，應用程式會觸發 Windows Defender 的檢測機制，並利用其錯誤行為來覆蓋系統檔案。

這樣的設計使得使用者能夠直接觀察到防毒軟體的反應，並進行相應的調整。選擇 C++ 作為開發語言的好處在於其高效能和對系統資源的直接控制，但代價是需要使用者具備一定的編譯和執行環境。擴展性方面，由於專案目前仍在早期階段，未來可能會加入更多的功能和擴展，但目前的設計已經能夠有效展示其核心功能。

## 技術深入分析

RedSun 的核心技術機制在於利用 Windows Defender 對於帶有雲標籤檔案的錯誤處理。這個 PoC 透過 C++ 編寫，能夠直接與系統檔案進行互動。效能方面，由於其設計輕量，對系統資源的需求相對較低，適合在各種環境中進行測試。設計取捨方面，選擇 C++ 使得專案在效能上有優勢，但也要求使用者具備一定的技術背景。技術風險方面，這種利用方式可能在未來的 Windows 更新中被修補，導致專案失去效用。整合分析中，這個工具在 CI/CD pipeline 中的應用尚未明確，但其實驗性質使得它在安全測試中具有一定的潛力。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的介紹和使用指導，但缺乏詳細的範例。安裝過程相對順暢，但使用者需具備 C++ 編譯環境。沒有提供多語言的支持，可能對非英語使用者造成困難。

## 優缺點分析

> [!success] 優點
> - 輕量級的 C++ 實作，易於編譯和執行。
> - 能夠直接觀察防毒軟體的行為，提供實驗性質的測試。
> - 開源社群活躍，能夠獲得即時的反饋和支持。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於多平台的漏洞利用，而 RedSun 專注於 Windows Defender 的特定行為。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供更廣泛的漏洞利用工具，而 RedSun 是針對特定防毒行為的 PoC。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於多平台的漏洞利用，提供更廣泛的功能。 | 如果需要跨平台的漏洞利用工具，則應選擇此工具。 | medium，因為需要重新適應不同的 API 和功能。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供更全面的漏洞利用工具，涵蓋多種攻擊向量。 | 如果需要一個功能更全面的漏洞利用工具，這是更好的選擇。 | high，因為需要重新學習和適應新的工具架構。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **RedSun** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於多平台的漏洞利用，提供更廣泛的功能。 | 提供更全面的漏洞利用工具，涵蓋多種攻擊向量。 |
> | 遷移成本 | - | medium，因為需要重新適應不同的 API 和功能。 | high，因為需要重新學習和適應新的工具架構。 |
> | 適用場景 | 主要場景 | 如果需要跨平台的漏洞利用工具，則應選擇此工具。 | 如果需要一個功能更全面的漏洞利用工具，這是更好的選擇。 |
...
- **[HIGH]** Windows Defender 可能會立即檢測到並阻擋執行檔案
  - 解法：在安全環境中進行測試，或使用虛擬機器。
- [MEDIUM] 需要 C++ 編譯環境，對新手不友好
  - 解法：尋找 C++ 編譯的相關資源或請教有經驗的開發者。
- [MEDIUM] 可能會因為防毒軟體的更新而失去效用
  - 解法：定期檢查 Windows 更新和防毒軟體的行為變化。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 安全研究團隊進行防毒軟體行為測試 | 非常適合 | 專案專注於防毒軟體的漏洞利用，能夠提供實驗數據。 |
| 個人開發者想了解系統安全 | 適合 | 能夠提供對防毒軟體行為的深入理解。 |
| 企業環境中進行安全測試 | 不適合 | 可能會被防毒軟體檢測並阻擋，影響測試結果。  |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，3 小時整合，得到對防毒軟體行為的深入理解，值得嘗試。

## 安全性評估

> [!warning] 安全性快速掃描
> 中等風險：需要在安全環境中執行，並且可能會被防毒軟體檢測。對於敏感資料的存取風險較低，但仍需謹慎處理。
...
> | [#12](https://github.com/Nightmare-Eclipse/RedSun/issues/12) | windows defender immediately detects the file lol | 1 | 3 |
