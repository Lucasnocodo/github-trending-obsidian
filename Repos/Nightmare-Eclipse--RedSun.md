...
**793** stars · **793** stars/天 · 建立 1 天前 · C++ · MIT
...
> **結論** 花 2 小時學習，1 小時整合，能快速驗證防毒軟體的漏洞，值得使用。

> [!abstract] 核心創新
> 利用 Windows Defender 的反直覺行為來實現系統檔案覆寫。

## 專案簡介

RedSun 是一個針對 Windows Defender 的漏洞利用庫，核心機制在於利用該防毒軟體對帶有雲標籤的惡意檔案的處理方式。當 Windows Defender 偵測到這類檔案時，竟然會將其重寫回原位置，這使得攻擊者能夠覆寫系統檔案並獲得管理權限。這個 PoC（Proof of Concept）展示了防毒軟體在某些情況下的反直覺行為，並且強調了安全產品在設計上的缺陷。使用者只需執行提供的程式碼，即可觸發這一行為，這是其最大的賣點。技術上，這個專案使用 C++ 編寫，依賴於 Windows API 來進行檔案操作，並且不需要額外的庫，這使得其部署相對簡單。

與其他漏洞利用工具相比，如 0xGF/boneyard 和 AgentSeal/codeburn，RedSun 的特點在於其針對特定防毒軟體的精準攻擊，且不需要複雜的環境配置。實際使用中，這個工具的效果取決於目標系統的防護措施，對於未更新的系統可能會有較高的成功率。由於這是一個相對新穎的專案，社群活躍度尚可，開發者在未來可能會針對其他防毒產品進行擴展。總體來看，這個專案適合安全研究人員和漏洞分析師使用，但不建議在生產環境中使用，因為其本質上是針對安全防護的攻擊工具。

**技術棧**：`C++`

## 重點功能

- PoC 漏洞利用 — 利用 Windows Defender 的特定行為覆寫系統檔案。
- 簡單部署 — 僅需 C++ 編譯環境，無需額外依賴。
- 針對特定防毒產品 — 專注於 Windows Defender 的漏洞，提供精準攻擊。
- 活躍的社群討論 — 有多個熱門 Issues，顯示出使用者的興趣和反饋。
- 幽默的設計理念 — 對於安全研究的輕鬆態度，吸引了不少關注。
...
> [!tip] 爆紅原因
> 建立 1 天就累積 793 stars（793/天），forks 151（19.0%），這顯示出強烈的興趣。作者 Nightmare-Eclipse 之前在安全領域有過多個相關專案，這次的工具解決了 Windows Defender 在處理特定檔案時的漏洞，這在業界引起了廣泛的討論。特別是其 PoC 的幽默性和實用性吸引了不少安全研究者的注意。社群中對於這個專案的反應熱烈，尤其是對於其可能的應用場景和擴展性有著積極的討論。
...
> [!example] 使用場景
> - 安全研究員用它來測試 Windows Defender 的防護能力，因為能夠快速驗證防毒軟體的漏洞是否存在。
> - 滲透測試工程師用它來模擬攻擊，評估企業系統的安全性，因為這能幫助他們發現潛在的安全風險。
> - 漏洞分析師用它來分析防毒軟體的行為，因為這能提供對於防護機制的深入理解，並促進漏洞修補。

## 架構分析

RedSun 的架構設計相對簡單，主要依賴於 Windows API 進行檔案操作。這樣的設計使得程式碼能夠直接與作業系統互動，降低了對外部庫的依賴，從而簡化了部署過程。資料流方面，程式碼首先檢測檔案的雲標籤，然後利用 Windows Defender 的重寫行為進行系統檔案的覆寫。這樣的設計選擇使得攻擊者能夠在不被察覺的情況下獲取管理權限，但也可能導致在某些環境中被防毒軟體即時阻擋。整體來看，這個專案的擴展性有限，因為它專注於特定的防毒產品，未來可能需要針對其他防毒軟體進行調整。

## 技術深入分析

RedSun 的核心技術機制在於利用 Windows Defender 對於帶有雲標籤檔案的處理方式，這是一種反直覺的行為，讓攻擊者能夠透過覆寫系統檔案來獲取管理權限。這個專案的效能特性取決於目標系統的防護措施，對於未更新的系統可能會有較高的成功率。由於專案使用 C++ 編寫，這使得其在效能上相對優越，但也意味著需要一定的編譯環境。設計上，選擇直接與 Windows API 互動的方式，雖然簡化了依賴，但也可能在未來的擴展中遇到挑戰。技術風險方面，這個專案的設計決策可能在面對更強的防護措施時失效，並且需要持續關注防毒軟體的更新。整合方面，這個工具的使用相對獨立，與主流開發框架的整合難度較低，但在 CI/CD pipeline 中的應用可能會受到限制。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了簡單的使用說明和範例。安裝過程順暢，無明顯坑點。文件未提供多語言支持，但內容簡潔易懂。
...
| 工具 | 差異 |
| --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於多種漏洞利用，但不針對特定防毒產品。 |
| [AgentSeal/codeburn](https://github.com/AgentSeal/codeburn) | 提供更廣泛的漏洞利用工具，但相對複雜，需額外依賴。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 使用多種漏洞利用技術，提供更廣泛的攻擊面。 | 如果需要針對多種防護措施進行測試，而不僅限於 Windows Defender。 | medium，因為需要重新學習不同的漏洞利用技術。 |
| [AgentSeal/codeburn](https://github.com/AgentSeal/codeburn) | 提供更複雜的漏洞利用工具，需額外依賴。 | 如果需要更高級的漏洞利用功能，且團隊有相應的技術能力。 | high，因為需要重新配置環境和學習新工具。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **RedSun** | **boneyard** | **codeburn** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 使用多種漏洞利用技術，提供更廣泛的攻擊面。 | 提供更複雜的漏洞利用工具，需額外依賴。 |
> | 遷移成本 | - | medium，因為需要重新學習不同的漏洞利用技術。 | high，因為需要重新配置環境和學習新工具。 |
> | 適用場景 | 主要場景 | 如果需要針對多種防護措施進行測試，而不僅限於 Windows | 如果需要更高級的漏洞利用功能，且團隊有相應的技術能力。 |
...
- **[HIGH]** 在某些系統上可能會被防毒軟體即時阻擋，導致無法執行。
  - 解法：在測試環境中禁用防毒軟體。
- [MEDIUM] 執行後可能會導致系統不穩定，需謹慎使用。
  - 解法：在虛擬機中測試，避免影響主系統。
- **[HIGH]** 需要管理員權限才能完全執行，普通用戶無法使用。
  - 解法：確保以管理員身份運行程式。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 大型企業的安全測試團隊 | 非常適合 | 能夠有效測試防毒軟體的漏洞，提升安全性。 |
| 個人開發者進行安全研究 | 適合 | 提供簡單的漏洞利用示範，易於學習。 |
| 小型企業的生產環境 | 不適合 | 存在安全風險，不建議在生產環境中使用。 |
...
> [!tip] 投入 vs 回報
> 花 2 小時學習，1 小時整合，能快速驗證防毒軟體的漏洞，值得使用。

## 安全性評估

> [!warning] 安全性快速掃描
> 中等風險：需要管理員權限，可能會存取敏感系統檔案，使用時需謹慎。
...
**社群活躍度**：社群活躍度尚可，熱門 Issues 顯示出使用者的興趣。
