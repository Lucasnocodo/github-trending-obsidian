...
**1.8k** stars · **116** stars/天 · 建立 16 天前 · C++ · MIT
...
> **適合** 對 Windows 安全性有興趣的研究者和開發者，特別是那些想要了解防毒軟體行為的人。
> **一句話重點** RedSun 讓我們看到防毒軟體在某些情況下的反直覺行為，並利用這一點來獲取權限。
...
> **結論** 花 5 小時學習，3 小時整合，得到對 Windows Defender 行為的深入理解，值得安全研究者使用。
...
RedSun 是一個針對 Windows Defender 漏洞的 PoC（Proof of Concept）代碼庫，利用了防毒軟體在檢測到有害文件時的奇怪行為。當 Windows Defender 偵測到一個帶有雲標籤的惡意文件時，它會將該文件重寫回原位置，這個行為被 RedSun 利用來覆蓋系統文件並獲取管理權限。這個工具的賣點在於它揭示了防毒軟體在某些情況下的反直覺行為，並且提供了一個有趣的視角來看待安全漏洞的利用。技術上，這個專案是用 C++ 實作，並且專注於利用 Windows 環境中的特定行為。與其他安全工具相比，RedSun 直接針對防毒軟體的漏洞，而不是通用的攻擊手法，這使得它在特定場景下的有效性更高。

與其他類似工具如 0x0funky/agent-sprite-forge 和 0xGF/boneyard 相比，RedSun 的獨特之處在於它專注於 Windows Defender 的行為，而不是一般的漏洞利用。使用這個工具的風險在於它可能會被防毒軟體檢測到，並且可能導致系統不穩定或安全問題。這個專案目前處於活躍開發階段，社群也相對活躍，開放的問題數量顯示出使用者對於漏洞的興趣和討論。對於安全研究者來說，這是一個值得關注的專案，但不建議在生產環境中使用。適合對 Windows 安全性有興趣的開發者和研究者，特別是那些想要了解防毒軟體行為的人。

**技術棧**：`C++`

## 重點功能

- 漏洞利用 — 利用 Windows Defender 的行為來獲取系統管理權限。
- PoC 代碼 — 提供了具體的代碼示例，方便研究和學習。
- 社群支持 — 開放的問題和討論，促進使用者之間的交流。
- 易於修改 — 高 fork 比率顯示出許多人在實際修改和使用這個工具。
- 活躍開發 — 定期更新和維護，保持工具的有效性。
...
> [!tip] 爆紅原因
> 建立 16 天內累積 1848 stars（115.5/天），forks 405（21.9%），這顯示出社群對這個漏洞的高度關注。作者 Nightmare-Eclipse 似乎專注於安全研究，這個專案解決了之前沒有好方案的痛點，即如何利用防毒軟體的反直覺行為來獲取權限。這種利用方式在安全界引起了廣泛的討論，特別是在社交媒體上引發了反響。技術上，Windows Defender 的行為使這個工具變得可行，並且其高 fork 比率顯示出許多人在實際修改和使用這個工具。

## 適合誰使用

**目標受眾**：對 Windows 安全性有興趣的研究者和開發者，特別是那些想要了解防毒軟體行為的人。

> [!example] 使用場景
> - 安全研究員用它來測試 Windows Defender 的行為，因為這能幫助他們理解防毒軟體的漏洞和弱點。
> - 滲透測試工程師用它來模擬攻擊，因為這能提供一個真實的環境來評估系統的安全性。
> - 開發者用它來學習如何利用防毒軟體的行為，因為這能幫助他們設計更安全的應用程式。

## 架構分析

RedSun 採用 C++ 實作，設計上專注於利用 Windows Defender 的特定行為。這種選擇使得工具能夠直接操作系統文件，並且在 Windows 環境中表現良好。資料流方面，工具會檢測到防毒軟體的行為，然後利用該行為來覆蓋系統文件。

這樣的設計使得工具在特定情境下能夠有效，但也帶來了被檢測的風險。選擇 C++ 而非其他語言，能夠更好地控制系統資源，但也增加了開發的複雜度。擴展性方面，這個工具的設計並不容易擴展到其他平台，因為它專注於 Windows 的特定行為。

## 技術深入分析

RedSun 的核心技術機制在於利用 Windows Defender 的行為，這涉及到對系統文件的直接操作。這個工具能夠在特定情境下覆蓋系統文件，從而獲取管理權限。效能上，這個工具在 Windows 環境中表現良好，但在其他平台上無法運行。設計上選擇 C++ 是因為它能夠更好地控制系統資源，然而這也增加了開發的複雜度。這個選擇使得工具在性能上有優勢，但也可能帶來更高的維護成本。技術風險方面，這個工具的設計可能在面對新的防毒軟體更新時出現問題，因為防毒軟體的行為可能會隨著更新而改變。整合方面，這個工具的使用需要一定的 C++ 開發經驗，對於不熟悉這個語言的開發者來說，學習成本較高。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了具體的使用步驟和範例。安裝過程相對順暢，但需要一定的 C++ 環境配置。文件沒有多語言支持，但內容簡單易懂。整體來說，花 30 分鐘能夠順利運行起來。

## 優缺點分析

> [!success] 優點
> - 能夠利用防毒軟體的反直覺行為來獲取權限。
> - 提供了具體的 PoC 代碼，方便研究和學習。
> - 社群活躍，問題解決率相對較高。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於利用特定的攻擊手法，而不是針對防毒軟體的行為。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供更廣泛的漏洞利用工具，而 RedSun 專注於 Windows Defender 的特定行為。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於利用特定的攻擊手法，而不是針對防毒軟體的行為。 | 如果你的團隊需要針對特定漏洞進行研究，而不僅僅是防毒軟體的行為。 | medium，因為需要重新學習不同的攻擊手法。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供更廣泛的漏洞利用工具，而 RedSun 專注於 Windows Defender 的特定行為。 | 如果需要一個多功能的漏洞利用工具，而不僅僅是針對防毒軟體的行為。 | high，因為需要重新適應不同的工具和工作流程。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **RedSun** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於利用特定的攻擊手法，而不是針對防毒軟體的行為。 | 提供更廣泛的漏洞利用工具，而 RedSun 專注於 Windows Defender 的特定行為。 |
> | 遷移成本 | - | medium，因為需要重新學習不同的攻擊手法。 | high，因為需要重新適應不同的工具和工作流程。 |
> | 適用場景 | 主要場景 | 如果你的團隊需要針對特定漏洞進行研究，而不僅僅是防毒軟體的行 | 如果需要一個多功能的漏洞利用工具，而不僅僅是針對防毒軟體的行 |
...
- **[HIGH]** 運行時可能會被防毒軟體檢測到，導致程序無法啟動。
  - 解法：在安全環境中運行，或暫時禁用防毒軟體。
- [MEDIUM] 需要管理員權限運行，否則無法覆蓋系統文件。
  - 解法：確保以管理員身份運行程序。
- [MEDIUM] 可能會因為系統更新而失效。
  - 解法：定期檢查防毒軟體的更新和行為變化。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 安全研究團隊進行 Windows 環境的漏洞測試 | 非常適合 | 專注於 Windows Defender 的行為，能提供具體的測試案例。 |
| 小型開發團隊需要快速測試防毒軟體的行為 | 適合 | 提供了易於使用的 PoC 代碼，能快速上手。 |
| 大型企業的生產環境 | 不適合 | 可能會被防毒軟體檢測到，導致不穩定性。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，3 小時整合，得到對 Windows Defender 行為的深入理解，值得安全研究者使用。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：不需要高權限運行，但可能會存取系統文件。依賴鏈的信任程度較高，因為主要是自包含的 C++ 代碼。
