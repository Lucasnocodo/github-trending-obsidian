...
**1.9k** stars · **109** stars/天 · 建立 17 天前 · C++ · MIT
...
> **適合** 對安全漏洞研究有興趣的安全專業人士或滲透測試工程師。
> **一句話重點** RedSun 不僅是一個漏洞利用工具，更是一個引發對防毒軟體行為反思的案例。
...
> **結論** 花 5 小時學習、3 小時整合，得到對防毒軟體行為的深入理解，值得安全研究者使用。
...
RedSun 是一個針對 Windows Defender 的漏洞庫，主要利用其對帶有雲標籤的惡意檔案的處理方式來提升系統權限。當 Windows Defender 偵測到這類檔案時，反而會將其重寫回原位置，這一行為被 RedSun 利用來覆寫系統檔案。這個 PoC（概念驗證）展示了防毒軟體在某些情況下的反直覺行為，並且揭示了安全產品在處理惡意檔案時可能出現的漏洞。使用者只需執行相應的 C++ 程式碼，即可觸發這一行為，進而獲得管理員權限。這個工具的賣點在於其簡單的使用方式和對 Windows Defender 的直接挑戰。技術上，RedSun 主要依賴 C++ 來實現其功能，並且在效能上並不需要過多的資源，因為它主要依賴於操作系統的漏洞。

與其他安全工具相比，RedSun 的獨特之處在於它專注於利用防毒軟體的錯誤，而不是單純的惡意程式碼。這使得它在特定的安全測試環境中非常有用，但在實際應用中可能會引發法律和倫理問題。使用者在使用時需謹慎，因為這樣的工具可能會被誤用。這個專案目前仍在 alpha 階段，並且社群活躍度一般，開發者的回應速度尚可，適合對安全漏洞有興趣的研究者和安全專業人士，但不建議在生產環境中使用。未來可能會有更多針對其他防毒軟體的類似工具出現。

**技術棧**：`C++`

## 重點功能

- 利用 Windows Defender 的漏洞 — 透過重寫檔案來獲得管理員權限。
- PoC 代碼簡單易用 — 只需執行 C++ 程式碼即可觸發漏洞。
- 針對特定情況的防護測試 — 測試防毒軟體在面對帶有雲標籤檔案時的行為。
- 社群活躍 — 有許多討論和反饋，幫助使用者理解漏洞的影響。
- 開源且免費 — 任何人都可以使用和修改這個工具。
...
[
  "# 前置條件：需要 C++ 編譯器",
  "g++ -o RedSun main.cpp",
  "# 預期輸出：程式將觸發 Windows Defender 的行為"
]
```
...
> [!tip] 爆紅原因
> 建立 17 天就累積 1853 stars（109/天），forks 405（21.9%），這顯示出社群對於這個漏洞的高度關注。作者 Nightmare-Eclipse 似乎專注於安全研究，這個專案解決了傳統防毒軟體在處理特定情況下的漏洞，之前的解決方案往往無法有效應對這類問題。最近的社群討論和反應也顯示出對於這個漏洞的興趣，尤其是 Windows Defender 的反應引發了不少笑聲。這樣的情況讓這個專案在安全研究領域中迅速獲得了關注。

## 適合誰使用

**目標受眾**：對安全漏洞研究有興趣的安全專業人士或滲透測試工程師。

> [!example] 使用場景
> - 安全研究人員用它來測試 Windows Defender 的漏洞，因為這能揭示防毒軟體在特定情況下的反直覺行為。
> - 滲透測試工程師用它來模擬攻擊，因為這樣的工具能幫助他們評估系統的防護能力。
> - 開發者用它來學習如何利用系統漏洞，因為這提供了一個實際的案例來理解安全漏洞的運作方式。

## 架構分析

RedSun 的架構基於 C++，專注於利用 Windows Defender 的特定行為。這個工具的設計理念是利用防毒軟體在處理帶有雲標籤的檔案時的反直覺行為。資料流從檔案的偵測開始，然後透過程式碼覆寫系統檔案，最終實現管理員權限的獲取。這樣的設計選擇使得 RedSun 在特定情況下能夠有效運作，但也帶來了法律和倫理上的風險。擴展性方面，這個工具的使用場景相對狹窄，主要針對特定的防毒軟體，未來可能需要針對其他防毒軟體進行類似的開發。

## 技術深入分析

RedSun 利用 Windows Defender 的特定行為來提升系統權限，核心技術在於 C++ 的檔案操作和系統調用。這個工具的效能主要依賴於操作系統的漏洞，並不需要大量資源，因為它主要是利用防毒軟體的錯誤行為。選擇 C++ 是因為其對系統層級操作的強大支持，這使得 RedSun 能夠直接與操作系統交互。這樣的選擇雖然帶來了高效能，但也增加了使用的複雜度。技術風險方面，RedSun 可能在未來面臨防毒軟體的更新和修補，這將影響其有效性。整合方面，這個工具的使用場景相對狹窄，主要針對特定的防毒軟體，與主流開發框架的整合性較低，但在安全測試環境中仍然具有一定的價值。

## 新手體驗

> [!info] 上手難度評估
> README 文件簡潔明瞭，提供了基本的使用說明和範例。安裝過程相對順暢，但需要一定的 C++ 知識。文件目前僅提供英文版本，對於非英語使用者可能會有一定的學習曲線。

## 優缺點分析

> [!success] 優點
> - 直接利用防毒軟體的漏洞，展示其反直覺行為。
> - 簡單易用，適合安全研究和滲透測試。
> - 社群活躍，能獲得及時的反饋和支持。

> [!danger] 缺點
> - 僅針對 Windows 系統，無法跨平台使用。
> - 可能會被防毒軟體檢測到，影響使用體驗。
> - 法律和倫理風險較高，使用時需謹慎。

> [!warning] 注意事項
> - 僅針對 Windows 系統有效，無法在其他操作系統上運行。
> - 可能會被防毒軟體檢測到並阻擋執行。
> - 需要一定的 C++ 知識來編譯和運行程式碼。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於利用其他防毒軟體的漏洞，但不如 RedSun 直接針對 Windows Defender 的行為。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供了多種漏洞利用工具，但缺乏 RedSun 專注於特定防毒軟體的深度。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於利用其他防毒軟體的漏洞，但不如 RedSun 直接針對 Windows Defender 的行為。 | 如果你需要針對多種防毒軟體進行測試，這個工具會更合適。 | medium，因為需要學習新的 API 和使用方式。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供了多種漏洞利用工具，但缺乏 RedSun 專注於特定防毒軟體的深度。 | 如果你需要一個多功能的漏洞利用工具，這個選擇會更靈活。 | high，因為需要重新適應不同的工具和流程。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **RedSun** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於利用其他防毒軟體的漏洞，但不如 RedSun 直接針對 Windows Defender 的行為。 | 提供了多種漏洞利用工具，但缺乏 RedSun 專注於特定防毒軟體的深度。 |
> | 遷移成本 | - | medium，因為需要學習新的 API 和使用方式。 | high，因為需要重新適應不同的工具和流程。 |
> | 適用場景 | 主要場景 | 如果你需要針對多種防毒軟體進行測試，這個工具會更合適。 | 如果你需要一個多功能的漏洞利用工具，這個選擇會更靈活。 |
...
- **[HIGH]** 執行時可能會被防毒軟體阻擋，無法正常運行。
  - 解法：在無防毒軟體的環境中測試。
- **[HIGH]** 使用過程中可能會引發法律問題。
  - 解法：在合法的測試環境中使用。
- [MEDIUM] 需要 C++ 編譯環境，對於不熟悉的人來說可能有學習曲線。
  - 解法：尋找相關的 C++ 教學資源。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 滲透測試團隊進行內部系統安全評估 | 非常適合 | 直接針對防毒軟體的漏洞，能有效測試系統防護能力。 |
| 個人開發者學習安全漏洞利用 | 適合 | 提供了實際的案例來理解安全漏洞的運作方式。 |
| 大型企業的生產環境 | 不適合 | 法律和倫理風險較高，不建議在生產環境中使用。  |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習、3 小時整合，得到對防毒軟體行為的深入理解，值得安全研究者使用。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具本身不需要高權限，但使用過程中可能會存取系統檔案。依賴鏈的信任程度尚可，無已知的供應鏈風險。
...
> | [#12](https://github.com/Nightmare-Eclipse/RedSun/issues/12) | windows defender immediately detects the file lol | 3 | 3 |
