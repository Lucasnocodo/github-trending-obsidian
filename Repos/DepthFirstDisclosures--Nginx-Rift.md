...
star_history: "2026-05-18:674,2026-05-18:674,2026-05-19:711,2026-05-20:732,2026-05-21:753,2026-05-22:782,2026-05-23:792,2026-05-24:795"
tags:
  - github
  - "category/安全"
  - "lang/python"
...
**795** stars · **72** stars/天 · 建立 11 天前 · Python · 未標註授權
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Recent · **熱度** Growing (72 stars/day)
> **授權** 未標註授權 (風險較高) · **維護** Moderate (最後推送 10 天前) · **貢獻者** Solo (bus factor 風險) · **參與度** Medium
> **適合** 需要測試 NGINX 伺服器安全性並具備基本漏洞利用知識的安全研究人員。
> **一句話重點** 這個工具展示了如何利用 NGINX 中的內存管理漏洞，並提供了簡單的利用方式，適合安全研究和測試。
...
> **結論** 花 4 小時學習，2 小時整合，得到快速測試 NGINX 漏洞的能力，值得在安全研究中使用。

> [!abstract] 核心創新
> 該工具針對 NGINX 的特定漏洞提供了簡單易用的遠端代碼執行利用方式。

## 專案簡介

Nginx-Rift 是一個針對 CVE-2026-42945 的遠端代碼執行漏洞利用工具，這個漏洞源於 NGINX 的 `ngx_http_rewrite_module` 中的堆緩衝區溢出問題。該漏洞允許未經身份驗證的攻擊者利用 `rewrite` 和 `set` 指令來執行任意代碼。具體來說，該漏洞利用了 NGINX 的兩階段處理過程：首先計算所需的緩衝區大小，然後將數據複製到該緩衝區。由於在計算長度時使用的子引擎被重置，導致長度計算錯誤，最終在複製過程中溢出緩衝區。這樣的設計使得攻擊者能夠控制 URI 數據，並通過跨請求的堆操作來破壞相鄰的內存結構。使用者只需執行 `python3 poc.py --shell` 即可觸發漏洞，獲得 shell 訪問權限。這個工具的賣點在於其簡單的使用方式和針對特定漏洞的專注，讓安全研究人員能夠快速測試 NGINX 的脆弱性。



技術上，該工具使用 Python 和 Shell 進行實作，並在 Ubuntu 24.04.3 LTS 上進行測試。其依賴於 Docker 來啟動一個易受攻擊的 NGINX 伺服器，並且其設計考慮了快速部署和易於使用的需求。與其他類似工具相比，如 0x0funky/agent-sprite-forge 和 0xGF/boneyard，Nginx-Rift 專注於特定的 CVE，並提供了直接的利用方式，而不是僅僅是漏洞掃描或報告。

在實際使用中，該工具的效果取決於 NGINX 的版本，且存在一些已知的問題，例如在某些版本上無法正常工作。社群活躍度不高，開放的問題未解決，顯示出使用者可能會遇到的挑戰。整體來看，這是一個針對特定漏洞的實用工具，但在生產環境中使用需謹慎，因為其穩定性和社群支持尚未成熟。

**技術棧**：`Python` · `Shell` · `Docker`

## 重點功能

- 針對 CVE-2026-42945 的漏洞利用 — 允許未經身份驗證的遠端代碼執行。
- 簡單的使用流程 — 只需執行 `python3 poc.py --shell` 即可獲得 shell 訪問權限。
- Docker 支持 — 提供了易於部署的 Docker 環境來測試漏洞。
- 針對特定 NGINX 版本的兼容性 — 針對受影響的 NGINX 版本進行測試。
- 詳細的技術說明 — 提供了漏洞的技術背景和利用方式的詳細說明。
...
> [!tip] 爆紅原因
> 建立 11 天就累積 795 stars（72/天），forks 144（18.1%），顯示出相對高的實際使用和修改需求。這個工具由 Markakd 開發，專注於一個具體的安全漏洞，解決了 NGINX 在處理 URI 時的內存管理問題，這在之前的工具中並未得到有效解決。社群的反饋和問題顯示出使用者對於工具的實際應用情況有著高度的關注，特別是在不同版本的 NGINX 上的兼容性問題。這些因素共同促進了該工具的快速增長。

## 適合誰使用

**目標受眾**：需要測試 NGINX 伺服器安全性並具備基本漏洞利用知識的安全研究人員。

> [!example] 使用場景
> - 安全研究員用它來測試 NGINX 伺服器的脆弱性，因為它能快速觸發 CVE-2026-42945 的漏洞，並獲得 shell 訪問權限。
> - 滲透測試工程師用它來驗證客戶的 NGINX 配置安全性，因為它提供了針對特定漏洞的直接利用方式，能夠有效評估風險。
> - 開發者用它來學習和理解 NGINX 的內存管理問題，因為它清楚地展示了如何利用堆緩衝區溢出來進行攻擊。

## 架構分析

Nginx-Rift 的架構基於一個簡單的 Docker 環境，這樣的設計使得使用者能夠快速搭建一個易受攻擊的 NGINX 伺服器進行測試。數據流從 Docker 容器啟動開始，然後通過執行漏洞利用腳本來觸發 CVE-2026-42945。這種設計的優勢在於快速部署和易於使用，但代價是需要使用者具備基本的 Docker 知識。整體架構的擴展性有限，因為它主要針對特定漏洞，無法輕易擴展到其他漏洞的利用。最常見的瓶頸在於 NGINX 版本的兼容性，使用者需要根據自己的環境進行調整。

## 技術深入分析

Nginx-Rift 的核心技術機制是利用 NGINX 的堆緩衝區溢出漏洞，這一漏洞源於其 `ngx_http_rewrite_module` 的內部實現。具體來說，該工具利用了 NGINX 的兩階段處理過程，首先計算所需的緩衝區大小，然後將數據複製到該緩衝區。這樣的設計使得攻擊者能夠控制 URI 數據，並最終導致內存溢出。該工具在處理多大資料方面的效能取決於 NGINX 的版本，且在某些情況下可能會遇到性能瓶頸。

設計上選擇了 Python 和 Shell 作為主要語言，這使得工具易於使用和修改，但也可能導致性能上的限制。整體依賴樹相對簡單，主要依賴於 Docker 環境。技術風險方面，這個工具的設計可能在面對不同版本的 NGINX 時出現問題，特別是在版本兼容性上。整合到現有的 CI/CD pipeline 中也可能存在挑戰，因為需要確保環境的一致性和穩定性。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的使用說明和必要的步驟，安裝過程相對順暢，但需要使用者具備 Docker 知識。沒有提供多語言支持，可能對某些使用者造成困難。整體來說，花 30 分鐘能夠成功運行起來。

## 優缺點分析

> [!success] 優點
> - 針對特定漏洞的直接利用，易於使用。
> - 提供了完整的技術背景和使用說明，適合學習。
> - Docker 環境的支持使得部署快速且方便。
...
> [!warning] 注意事項
> - 僅支援特定版本的 NGINX，可能在其他版本上無法正常工作。
> - 需要 Docker 環境來運行測試，對於不熟悉 Docker 的使用者可能有學習曲線。
> - 存在未解決的問題，社群支持有限，可能需要自行解決兼容性問題。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於多種漏洞的利用，而 Nginx-Rift 專注於特定的 CVE-2026-42945，提供更具針對性的測試。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供更廣泛的漏洞掃描功能，而 Nginx-Rift 則提供直接的漏洞利用腳本，適合需要快速測試的場景。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於多種漏洞的利用，提供更廣泛的測試功能，而 Nginx-Rift 專注於特定的 CVE-2026-42945。 | 如果需要針對多個漏洞進行測試，並且希望有更廣泛的功能支持。 | medium，因為需要重新學習使用方法和配置。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供漏洞掃描和報告功能，而 Nginx-Rift 專注於直接的漏洞利用。 | 如果需要進行全面的安全評估，而不僅僅是針對特定漏洞的測試。 | low，因為兩者的使用方式相似。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **Nginx-Rift** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於多種漏洞的利用，提供更廣泛的測試功能，而 Nginx-Rift 專注於特定的 CVE-2026-42945。 | 提供漏洞掃描和報告功能，而 Nginx-Rift 專注於直接的漏洞利用。 |
> | 遷移成本 | - | medium，因為需要重新學習使用方法和配置。 | low，因為兩者的使用方式相似。 |
> | 適用場景 | 主要場景 | 如果需要針對多個漏洞進行測試，並且希望有更廣泛的功能支持。 | 如果需要進行全面的安全評估，而不僅僅是針對特定漏洞的測試。 |
...
- **[HIGH]** 在某些 NGINX 版本上無法正常工作，導致無法觸發漏洞。
  - 解法：檢查 NGINX 版本並確保使用受影響的版本。
- [MEDIUM] Docker 環境配置不當可能導致啟動失敗。
  - 解法：確保 Docker 和 Docker Compose 正確安裝並配置。
- [MEDIUM] 社群支持有限，許多問題未解決。
  - 解法：需要自行查找解決方案或參考其他資源。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 大型企業的安全測試團隊 | 適合 | 能夠快速測試 NGINX 的特定漏洞，評估安全性。 |
| 小型開發團隊 | 普通 | 雖然能提供一些學習機會，但可能需要額外的支持來解決問題。 |
| 個人安全研究者 | 非常適合 | 提供了簡單的利用方式，適合進行漏洞研究和學習。 |
| 生產環境的 NGINX 伺服器 | 不適合 | 不建議在生產環境中使用，存在穩定性和安全性風險。 |
...
> [!tip] 投入 vs 回報
> 花 4 小時學習，2 小時整合，得到快速測試 NGINX 漏洞的能力，值得在安全研究中使用。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具本身不需要高權限運行，但使用時需注意 NGINX 伺服器的安全性。依賴於 Docker，需確保環境的安全性和穩定性。
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["安全漏洞","遠端代碼執行","堆緩衝區溢出"];
