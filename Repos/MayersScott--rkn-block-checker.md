...
star_history: "2026-05-09:796,2026-05-09:800,2026-05-10:969,2026-05-10:971,2026-05-11:1062,2026-05-12:1100,2026-05-13:1149,2026-05-14:1175"
tags:
  - github
  - "category/cli_工具"
  - "lang/python"
...
**1.2k** stars · **118** stars/天 · 建立 10 天前 · Python · MIT
...
> **適合** 需要了解網路封鎖情況的技術專業人員或開發者。
> **一句話重點** 這個工具不僅告訴你網站是否可訪問，還告訴你為什麼無法訪問，這對於解決網路問題至關重要。
...
> **結論** 花 2 小時學習，1 小時整合，得到詳細的網路封鎖分析，值得使用。

> [!abstract] 核心創新
> 這個專案提供了逐層檢查網路封鎖的能力，能夠具體識別封鎖類型。

## 專案簡介

RKN Block Checker 是一個 CLI 工具，旨在幫助用戶確定其連接是否位於 RKN/TSPU 封鎖區域，並且具體分析封鎖的類型。用戶只需執行 `rkn-check`，該工具會自動探測一組預設網站，並根據每一層的檢查結果報告封鎖的原因。這樣的設計能夠提供比單純的「網站無法訪問」更具體的資訊，讓用戶了解問題出在哪一層。該工具使用 Python 3.10+ 開發，依賴於 `requests` 庫，並且可以輕鬆安裝。其核心機制是逐層檢查 DNS、TCP、TLS 和 HTTP，並在第一個失敗的地方停止，這樣能夠快速定位問題。

這種方法比起傳統的檢查方式更有效，因為它能夠識別不同的封鎖機制，如 DNS 中毒、TCP 重置、TLS DPI 和 HTTP stub 頁面。與其他類似工具相比，RKN Block Checker 更加專注於層級分析，提供更詳細的診斷結果，特別適合需要深入了解網路封鎖情況的用戶。使用者可以透過命令行選項自定義檢查的網站列表，並且支持 JSON 輸出，方便進一步分析。這使得它在需要持續監控或自動化檢查的場景中表現出色。

**技術棧**：`Python 3.10` · `requests >= 2.31`

## 重點功能

- 逐層檢查 — 檢查 DNS、TCP、TLS 和 HTTP，並報告哪一層出現問題。
- 自動探測 — 內建網站列表，無需配置即可開始檢查。
- JSON 輸出 — 支持將結果輸出為 JSON 格式，方便進一步分析。
- 自定義網站列表 — 允許用戶使用自定義的白名單和黑名單進行檢查。
- 多線程檢查 — 支持設置檢查的線程數量，以提高檢查效率。
...
> [!tip] 爆紅原因
> 建立 10 天內累積 1175 stars（117.5/天），forks 55（4.7%），顯示出穩定的增長趨勢。作者 MayersScott 之前有過相關的開發經驗，這個工具解決了用戶在面對網路封鎖時缺乏具體診斷工具的痛點。之前的解決方案往往只能告訴用戶「無法訪問」，而無法深入分析問題所在。社群的反饋也表明，這個工具的實用性和易用性吸引了不少用戶。這個工具的出現正好填補了這一需求，並且其功能的獨特性使其在同類工具中脫穎而出。

## 適合誰使用

**目標受眾**：需要了解網路封鎖情況的技術專業人員或開發者。

> [!example] 使用場景
> - 網路管理員用它來檢查公司網路是否受到 RKN 封鎖，因為這樣能夠快速定位問題並採取行動。
> - 開發者用它來測試自己開發的應用在不同地區的可達性，因為能夠獲得詳細的封鎖類型報告。
> - 研究人員用它來分析特定網站在不同 ISP 下的可訪問性，因為這樣能夠獲得更具體的數據支持其研究結論。

## 架構分析

RKN Block Checker 的架構設計為一個 CLI 工具，主要由 Python 編寫，並使用 requests 庫進行網路請求。其設計目的是為了簡化用戶的使用體驗，無需複雜的配置。資料流方面，工具依次檢查 DNS、TCP、TLS 和 HTTP，並在第一層失敗時停止，這樣能夠快速定位問題。

這種逐層檢查的方式能夠有效區分不同的封鎖類型，並且提供清晰的報告。選擇 Python 作為開發語言使得工具易於維護和擴展，但也可能在性能上受到限制，特別是在高併發檢查時。整體而言，這個設計在用戶友好性和功能性之間取得了良好的平衡。

## 技術深入分析

RKN Block Checker 的核心技術機制是逐層檢查網路連接，使用 DNS、TCP、TLS 和 HTTP 四個層級進行分析。每一層的檢查都使用了不同的技術手段，例如使用系統 DNS 和 Cloudflare 的 DoH 進行比較，以識別 DNS 中毒的情況。效能方面，工具在正常網路環境下能夠快速完成檢查，但在高流量的情況下可能會受到影響。選擇 Python 作為開發語言使得工具易於維護，但在性能上可能不如用 C/C++ 實現的工具。這個選擇使得開發速度更快，但在處理大量請求時可能會出現瓶頸。技術風險方面，隨著使用者數量的增加，可能會面臨伺服器負載過重的問題。整合方面，這個工具可以輕鬆地與現有的 CI/CD 流程結合，並且支持 JSON 輸出，便於進一步的數據分析。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且提供了安裝和使用範例，安裝過程順暢，無明顯坑點。文件中有基本的使用指南，但缺乏多語言支持，對於非英語用戶可能不太友好。

## 優缺點分析

> [!success] 優點
> - 提供詳細的封鎖類型分析，幫助用戶理解問題所在。
> - 易於安裝和使用，無需複雜配置。
> - 支持自定義檢查網站列表，靈活性高。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 大型企業的網路管理團隊 | 非常適合 | 能夠詳細分析網路封鎖情況，幫助制定對策。 |
| 個人開發者測試應用可達性 | 適合 | 提供簡單的 CLI 工具，易於集成到開發流程中。 |
| 學術研究者進行網路封鎖研究 | 普通 | 雖然提供了詳細的分析，但可能需要更高的精度和穩定性。 |
| 小型企業的日常網路檢查 | 適合 | 簡單易用，能夠快速獲得網路狀況。 |
...
> [!tip] 投入 vs 回報
> 花 2 小時學習，1 小時整合，得到詳細的網路封鎖分析，值得使用。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限運行，且不存取敏感資料。使用過程中不會對系統造成安全威脅。

## 生態系整合

> [!abstract] 如何融入你的工具鏈

RKN Block Checker 最常與網路監控工具搭配使用，通常在開發和測試階段進行連接檢查。在一個使用 Docker 部署的環境中，可以將此工具作為一個獨立的服務運行，通過命令行進行調用。它支持與 CI/CD 工具如 GitHub Actions 整合，能夠在每次部署前自動檢查網路連接。最常見的整合問題是需要確保 Docker 環境中已安裝 Python 和相關依賴，這可能需要額外的配置。
