...
use_case: "診斷 RKN/TSPU 網路封鎖，逐層分析 DNS、TCP、TLS、HTTP。"
priority: medium
ring: assess
...
star_history: "2026-05-09:796,2026-05-09:800,2026-05-10:969,2026-05-10:971,2026-05-11:1062,2026-05-12:1100,2026-05-13:1149,2026-05-14:1175,2026-05-15:1192"
tags:
  - github
  - "category/開發工具"
  - "lang/python"
...
aliases:
  ...
  - "診斷 RKN/TSPU 網路封鎖，逐層分析 DNS、TCP、TLS、HTTP。"
---

# rkn-block-checker

**1.2k** stars · **108** stars/天 · 建立 11 天前 · Python · MIT
...
> **授權** MIT (商業友好) · **維護** Active (最後推送 5 天前) · **貢獻者** 3 人 · **參與度** Low
> **適合** 需要診斷 RKN/TSPU 網路封鎖的網路工程師和研究人員。
> **一句話重點** 這個工具不僅告訴你網站是否可訪問，還能深入分析封鎖的具體原因，讓使用者能夠更有效地應對網路封鎖。
...
> **結論** 花 2 小時學習，1 小時整合，得到詳細的封鎖診斷效果，值得嘗試。

> [!abstract] 核心創新
> 逐層分析網路封鎖的具體類型，提供比一般檢查工具更深入的診斷。

## 專案簡介

這個 CLI 工具的核心功能是檢查當前網路連接是否受到 RKN/TSPU 的封鎖，並具體指出封鎖的類型，像是 DNS 中毒、TCP 重置、TLS DPI 或 ISP 的 stub 頁面。使用者只需執行 `rkn-check`，工具會自動探測一組預設網站，並根據每一層的檢查結果報告封鎖情況，這樣的設計讓使用者能夠快速了解問題出在哪一層，而不是僅僅知道某個網站無法訪問。技術上，這個工具使用 Python 3.10+，依賴 `requests` 庫來進行網路請求，並且可以通過 Docker 進行容器化部署。與其他工具相比，這個工具的獨特之處在於它能夠逐層分析網路堆疊，並且提供詳細的診斷報告，這在面對複雜的網路封鎖時特別有用。

對比其他診斷工具，如 `curl` 或 `ping`，這個工具提供了更高層次的封鎖分析，能夠識別出封鎖的具體類型，這在選擇合適的繞過工具或報告問題時非常關鍵。使用者可以自定義檢查的網站名單，並且支持 JSON 輸出，方便進行後續的數據處理。儘管目前的開發活動相對活躍，但仍然有一些開放的問題需要解決，例如對 FakeIP 的支持。整體而言，這是一個針對特定需求的專案，適合需要深入了解網路封鎖情況的使用者。

**技術棧**：`Python 3.10` · `requests>=2.31`

## 重點功能

- 逐層分析 — 檢查 DNS、TCP、TLS 和 HTTP 層的封鎖情況，提供詳細的診斷報告。
- 自定義目標列表 — 支持用戶自定義檢查的網站名單，方便針對特定需求進行檢查。
- JSON 輸出 — 提供機器可讀的 JSON 格式輸出，便於後續數據處理。
- 多線程檢查 — 使用 `--workers` 參數設置並行檢查的線程數，默認為 10。
- 靈活的命令行選項 — 支持多種命令行參數，如 `--white` 和 `--black`，以便針對不同的檢查需求。
...
> [!tip] 爆紅原因
> 建立 11 天內累積 1192 stars（108/天），forks 54（4.5%），顯示出相對穩定的增長。作者 MayersScott 之前的專案涉及網路診斷，這個工具填補了對於 RKN/TSPU 封鎖診斷的需求。之前的工具往往無法提供這麼詳細的層級分析，這使得使用者在面對封鎖時無法有效地選擇解決方案。近期的社交媒體討論和相關論壇的關注也推動了這個專案的曝光率。由於網路封鎖問題日益嚴重，這個工具的實用性和針對性使其迅速受到關注。forks/stars 比率為 4.5%，顯示出使用者對這個工具的實際修改和使用意圖。

## 適合誰使用

**目標受眾**：需要診斷 RKN/TSPU 網路封鎖的網路工程師和研究人員。

> [!example] 使用場景
> - 網路工程師用它來檢查公司網路是否受到 RKN 封鎖，因為這能幫助他們快速定位問題並採取行動。
> - 研究人員用它來分析特定網站在不同地區的可訪問性，因為這能提供更具體的封鎖類型數據。
> - 開發者用它來測試自己的應用在 RKN 封鎖環境下的表現，因為這能幫助他們優化應用的可用性。

## 架構分析

這個工具的架構基於 Python，使用 requests 庫進行網路請求，並且設計為 CLI 工具以便於快速使用。資料流方面，工具逐層檢查 DNS、TCP、TLS 和 HTTP，並在遇到第一個失敗時停止，這樣的設計能有效減少不必要的檢查，節省時間。選擇 Python 作為開發語言使得工具易於擴展和維護，但也可能在性能上受到限制，特別是在高並發的情況下。整體而言，這個設計使得工具在使用上非常靈活，但在處理大量請求時可能會遇到瓶頸。

## 技術深入分析

這個工具的核心技術機制在於逐層檢查網路堆疊，使用 DNS、TCP、TLS 和 HTTP 的檢查流程來確定封鎖類型。每一層的檢查都使用了不同的技術手段，例如 DNS 檢查使用系統解析器與 Cloudflare 的 DoH 進行比較，以識別 DNS 中毒的情況。效能方面，這個工具能夠快速檢查多個網站，但在高並發情況下可能會遇到性能瓶頸，特別是當使用者設置較高的 `--workers` 參數時。設計上選擇 Python 使得開發和維護變得簡單，但在性能上可能不如其他編譯型語言。這個工具的依賴關係相對簡單，主要依賴 requests 庫，這降低了使用門檻。技術風險方面，隨著 RKN/TSPU 封鎖技術的演進，這個工具可能需要不斷更新以應對新的封鎖手段。整合方面，這個工具可以輕鬆地與現有的網路診斷工具鏈結合，並且可以通過 cron 作業進行定期檢查，這使得它在實際應用中非常靈活。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且提供了詳細的使用範例，安裝過程也非常順暢。使用者只需執行簡單的命令即可開始檢查，沒有明顯的坑。文件目前僅提供英文版本，可能對非英語使用者造成一定障礙。

## 優缺點分析

> [!success] 優點
> - 提供詳細的封鎖類型分析，幫助使用者理解網路問題。
> - 支持自定義檢查名單，增加靈活性。
> - 使用簡單，無需複雜配置即可快速上手。

> [!danger] 缺點
> - 目前僅支援 Python 3.10+，限制了使用者群體。
> - 對於不在預設名單中的網站需要手動指定，增加了使用成本。
> - 在高並發情況下性能可能受到影響。

> [!warning] 注意事項
> - 僅支援 Python 3.10+。
> - 需要網路連接以進行檢查。
> - 目前僅支持預設的網站名單，對於不在名單中的網站需要手動指定。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 這個工具專注於網路流量的代理和隱私保護，而 rkn-block-checker 專注於封鎖診斷。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | boneyard 提供了更廣泛的網路診斷功能，但不專注於 RKN/TSPU 封鎖的具體分析。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| curl | curl 是一個通用的網路請求工具，能夠檢查網站的可訪問性，但無法提供具體的封鎖類型分析。 | 如果你只需要檢查網站是否可訪問，而不需要深入分析封鎖類型，curl 會是更簡單的選擇。 | low，因為 curl 的基本用法非常簡單，容易上手。 |
| ping | ping 用於檢查網路連接的基本可用性，但無法提供任何關於封鎖類型的詳細信息。 | 如果你只需要快速檢查網路連接狀態，ping 是一個輕量級的選擇。 | low，因為 ping 是一個非常基本的命令，幾乎所有使用者都能輕鬆使用。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **rkn-block-checker** | **curl** | **ping** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | curl 是一個通用的網路請求工具，能夠檢查網站的可訪問性，但無法提供具體的封鎖類型分析。 | ping 用於檢查網路連接的基本可用性，但無法提供任何關於封鎖類型的詳細信息。 |
> | 遷移成本 | - | low，因為 curl 的基本用法非常簡單，容易上手。 | low，因為 ping 是一個非常基本的命令，幾乎所有使用者都能輕鬆使用。 |
> | 適用場景 | 主要場景 | 如果你只需要檢查網站是否可訪問，而不需要深入分析封鎖類型，c | 如果你只需要快速檢查網路連接狀態，ping 是一個輕量級的選 |
...
- [MEDIUM] 在某些 ISP 環境中，TCP 重置可能會被誤判為封鎖，導致誤報。
  - 解法：在多個環境中進行測試以確認結果。
- **[HIGH]** 使用自定義網站名單時，格式不正確可能導致檢查失敗。
  - 解法：確保使用正確的 JSON 或文本格式。
- [MEDIUM] 在高並發情況下，可能會遇到性能瓶頸。
  - 解法：適當調整 --workers 參數以平衡性能。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 需要檢查 RKN 封鎖的網路環境 | 非常適合 | 工具專注於 RKN/TSPU 封鎖的診斷，提供詳細分析。 |
| 需要快速檢查網站可訪問性 | 普通 | 雖然可以使用，但其他工具如 curl 可能更簡單。 |
| 需要自定義檢查名單的情境 | 非常適合 | 支持自定義名單，靈活性高。 |
| 需要在生產環境中使用的情境 | 不適合 | 目前仍在 beta 階段，穩定性不足。 |
...
> [!tip] 投入 vs 回報
> 花 2 小時學習，1 小時整合，得到詳細的封鎖診斷效果，值得嘗試。
...
> FROM "Repos"
> WHERE category = "開發工具" AND file.name != "MayersScott--rkn-block-checker"
> SORT stars DESC
> LIMIT 8
> ```
