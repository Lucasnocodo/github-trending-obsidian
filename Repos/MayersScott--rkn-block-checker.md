...
use_case: "診斷 RKN/TSPU 網路封鎖，逐層分析封鎖類型（DNS、TCP、TLS、HTTP）。"
priority: medium
ring: assess
...
star_history: "2026-05-09:796,2026-05-09:800,2026-05-10:969,2026-05-10:971,2026-05-11:1062,2026-05-12:1100"
tags:
  - github
  - "category/cli_工具"
  - "lang/python"
...
aliases:
  ...
  - "診斷 RKN/TSPU 網路封鎖，逐層分析封鎖類型（DNS、TCP、TLS、HTTP）。"
---

# rkn-block-checker

**1.1k** stars · **138** stars/天 · 建立 8 天前 · Python · MIT
...
> **適合** 需要深入了解 RKN/TSPU 網路封鎖情況的技術用戶或開發者。
> **一句話重點** 這個工具不僅能告訴你網站是否可訪問，還能具體指出是哪一層出現了問題，這對於解決網路封鎖問題至關重要。
...
> **結論** 花 2 小時學習，1 小時整合，得到高效的網路封鎖檢測效果，值得使用。

> [!abstract] 核心創新
> 這個專案的創新在於能夠逐層分析網路封鎖的具體類型，提供高信心的診斷結果。

## 專案簡介

這個 CLI 工具的核心功能是檢測當前連線是否位於 RKN/TSPU 封鎖區域，並且具體指出是哪一層出現問題。使用者只需輸入 `rkn-check`，工具會自動探測一系列預設網站，並根據每一層的檢查結果報告封鎖類型，如 DNS 中毒、TCP 重置、TLS DPI 或 ISP stub 頁面。這種逐層檢查的設計使得使用者能夠更清楚地了解網路問題的根源，而不僅僅是「這個網站無法訪問」的模糊訊息。技術上，工具使用 Python 3.10+ 開發，並依賴 `requests` 庫進行 HTTP 請求，整體架構輕量且易於安裝，僅需一行指令即可完成安裝。

與其他封鎖檢測工具相比，如 `ping` 或 `traceroute`，這個工具提供了更具體的封鎖類型識別，幫助使用者選擇合適的繞過工具或進行有效的問題報告。使用者可以自定義檢查的網站列表，並且支持 JSON 輸出，方便進行後續數據處理。該工具的設計考慮到了不同封鎖機制的特徵，能夠提供高信心的診斷結果，並且在封鎖類型的識別上具有明確的信心標籤，讓使用者能夠快速理解當前的網路狀況。整體而言，這個工具非常適合需要深入了解網路封鎖情況的技術用戶或開發者。
...
- 自動探測 — 使用內建的網站列表進行自動化檢測，無需額外配置。
- JSON 輸出 — 支持以 JSON 格式輸出結果，方便進行後續數據處理。
- 自定義目標 — 允許用戶使用自定義的白名單和黑名單文件進行檢測。
- 高信心診斷 — 提供封鎖類型的信心標籤，幫助用戶理解問題的嚴重性。
...
> [!tip] 爆紅原因
> 建立 8 天內累積 1100 stars（138/天），forks 50（4.5%），顯示出強勁的增長潛力。作者 MayersScott 具備網路診斷和封鎖技術的背景，這個工具解決了以往缺乏針對性封鎖檢測的痛點，使用者能夠獲得更具體的封鎖類型識別。近期的社群反應和需求，尤其是對代理和 FakeIP 支持的需求，進一步推動了這個工具的受歡迎程度。技術上，隨著網路封鎖手段的多樣化，這個工具的實用性愈加凸顯，尤其在當前的網路環境中，對於需要繞過封鎖的用戶來說，這是一個極具價值的工具。

## 適合誰使用

**目標受眾**：需要深入了解 RKN/TSPU 網路封鎖情況的技術用戶或開發者。

> [!example] 使用場景
> - 網路工程師用它來診斷公司內部網路的封鎖情況，因為可以逐層分析問題來源，從而選擇合適的解決方案。
> - 開發者用它來測試自己的應用在 RKN 封鎖環境下的表現，因為能夠清楚知道哪些請求被封鎖，從而進行調整。
> - 隱私保護倡導者用它來檢查特定網站的可訪問性，因為能夠提供具體的封鎖類型，幫助他們進行有效的報告和行動。

## 架構分析

這個工具採用簡單的 CLI 架構，主要依賴 Python 的 requests 庫進行網路請求。資料流從 DNS 檢查開始，接著是 TCP、TLS 和 HTTP 層，依次檢查每一層的連接狀態。這種設計使得工具能夠快速定位問題所在，並且在每一層失敗時即停止檢查，避免不必要的資源浪費。選擇 Python 作為開發語言的好處在於其廣泛的庫支持和易於維護的特性，但代價是可能在高併發場景下性能不如某些編譯語言。整體而言，這個工具的架構設計簡潔明瞭，適合快速部署和使用。

## 技術深入分析

這個工具的核心技術機制是逐層檢查網路連接的各個層面，使用 DNS、TCP、TLS 和 HTTP 的檢查方式來確定封鎖類型。每一層的檢查都有其特定的意義，DNS 檢查主要用於識別是否存在 DNS 中毒，TCP 檢查則用於確認是否有 IP 層的黑洞現象。TLS 檢查則是針對現代的 DPI 封鎖技術，通過檢查 SNI 來判斷是否被封鎖。HTTP 檢查則用於識別 ISP 提供的 stub 頁面。

這種設計使得工具能夠提供高信心的診斷結果，並且在每一層失敗時即停止檢查，避免不必要的資源浪費。效能上，該工具能夠快速檢查多個網站，並且支持並行檢查，這對於需要同時檢查多個目標的用戶來說非常方便。設計上，選擇 Python 作為開發語言的好處在於其廣泛的庫支持和易於維護的特性，但代價是可能在高併發場景下性能不如某些編譯語言。整體而言，這個工具的架構設計簡潔明瞭，適合快速部署和使用，並且能夠與其他工具鏈良好整合。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且提供了詳細的使用範例，安裝過程順暢，僅需一行指令即可完成。文件中有針對常見使用情境的說明，對於新手來說非常友好。整體而言，花 30 分鐘能夠順利運行起來。
...
> - 僅支援 Python 3.10+。
> - 需要網路連接以進行檢查。
> - 對於某些封鎖類型，可能需要額外的配置或調整。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [MayersScott/rkn-block-checker](https://github.com/MayersScott/rkn-block-checker) | 這個工具專注於逐層檢查封鎖類型，提供更詳細的診斷，而其他工具如 ping 主要檢查連通性，無法提供具體的封鎖原因。 |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 雖然都是網路診斷工具，但 agent-sprite-forge 更偏向於代理和隱私保護，而 rkn-block-checker 專注於 RKN 封鎖的具體分析。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| ping | 使用 ICMP 封包檢查連通性，無法提供具體的封鎖類型。 | 如果只需要檢查基本的連通性，而不需要深入分析封鎖類型。 | low，因為 ping 是基本的網路工具，使用簡單。 |
| traceroute | 檢查路由路徑，無法提供具體的封鎖類型，主要用於定位網路問題。 | 如果需要檢查數據包的路由路徑，而不僅僅是封鎖情況。 | low，因為 traceroute 也是基本的網路工具，使用簡單。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **rkn-block-checker** | **ping** | **traceroute** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 使用 ICMP 封包檢查連通性，無法提供具體的封鎖類型。 | 檢查路由路徑，無法提供具體的封鎖類型，主要用於定位網路問題。 |
> | 遷移成本 | - | low，因為 ping 是基本的網路工具，使用簡單。 | low，因為 traceroute 也是基本的網路工具，使用簡單。 |
> | 適用場景 | 主要場景 | 如果只需要檢查基本的連通性，而不需要深入分析封鎖類型。 | 如果需要檢查數據包的路由路徑，而不僅僅是封鎖情況。 |
...
- [MEDIUM] 在某些網路環境下，檢查結果可能受到防火牆影響，導致誤報
  - 解法：在不同的網路環境下進行多次檢查以確認結果
- **[HIGH]** 自定義檢查列表格式不正確會導致檢查失敗
  - 解法：確保使用正確的 JSON 或文本格式
- [MEDIUM] 在高延遲網路環境下，檢查時間可能過長
  - 解法：調整 `--timeout` 參數以縮短檢查時間

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 能夠快速檢查 API 的可訪問性，並提供具體的封鎖類型。 |
| 大型企業的網路安全團隊 | 適合 | 能夠深入分析網路封鎖情況，幫助制定對策。 |
| 個人用戶希望繞過網路封鎖 | 普通 | 雖然能提供封鎖類型，但對於非技術用戶來說可能有些複雜。 |
| 需要高頻率檢查的自動化系統 | 不適合 | 在高併發場景下性能可能不如其他編譯語言實作的工具。 |
...
> [!tip] 投入 vs 回報
> 花 2 小時學習，1 小時整合，得到高效的網路封鎖檢測效果，值得使用。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限，僅需存取網路資源，且不會存取敏感資料。
...
相關概念：[[網路封鎖]] · [[DNS 中毒]] · [[TLS DPI]] · [[HTTP stub 頁面]]
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["網路封鎖","DNS 中毒","TLS DPI","HTTP stub 頁面"];
