...
star_history: "2026-05-09:796,2026-05-09:800,2026-05-10:969,2026-05-10:971,2026-05-11:1062,2026-05-12:1100,2026-05-13:1149,2026-05-14:1175,2026-05-15:1192,2026-05-16:1223,2026-05-17:1235,2026-05-18:1248,2026-05-19:1262,2026-05-20:1275,2026-05-21:1285,2026-05-22:1289"
tags:
  - github
  - "category/cli_工具"
  - "lang/python"
...
**1.3k** stars · **72** stars/天 · 建立 18 天前 · Python · MIT
...
> **適合** 需要在 RKN/TSPU 環境中進行網路診斷的系統管理員和開發者。
> **一句話重點** 這個工具不僅告訴你網站無法訪問，還能告訴你具體的封鎖類型，讓你能夠針對性地解決問題。
...
> **結論** 花 2 小時學習，1 小時整合，得到快速的網路診斷能力，值得投入。

> [!abstract] 核心創新
> 這個專案提供了逐層檢查網路封鎖的能力，能夠清晰地識別出封鎖的具體類型。

## 專案簡介

這個工具的核心機制是逐層檢查網路連接，從 DNS 開始，然後是 TCP、TLS，最後是 HTTP，並報告哪一層出現了問題。使用者只需執行 `rkn-check`，工具會自動探測一組預設網站，並根據每一層的檢查結果給出封鎖類型的診斷。這樣的設計讓使用者能夠更清楚地了解網路問題的根源，而不僅僅是知道某個網站無法訪問。這種逐層檢查的方式比一般的網頁無法加載提示更具信息量，因為它能夠識別出具體的封鎖類型，例如 DNS 污染、TCP 重置或 TLS DPI。技術上，這個工具使用 Python 開發，依賴於 `requests` 庫進行網路請求，並且可以在 Python 3.10 以上的環境中運行。

相較於其他類似工具，如 `dnsmasq` 或 `curl`，這個工具專注於多層次的封鎖檢測，能夠提供更詳細的診斷信息，特別是在面對複雜的網路封鎖時。使用者可以通過自定義白名單和黑名單來擴展檢測範圍，這在需要針對特定網站進行檢查時非常有用。實際使用中，這個工具能夠快速提供封鎖的類型和可能的原因，並且支持 JSON 格式輸出，方便進一步的數據處理。這使得它成為網路管理員和技術支持人員的有力工具，尤其是在需要快速識別網路問題的情況下。整體來看，這個專案的成熟度高，值得在需要進行網路診斷的環境中使用。

**技術棧**：`Python 3.10+` · `requests>=2.31`

## 重點功能

- 逐層檢查 — 檢查 DNS、TCP、TLS 和 HTTP 四層，並報告具體的封鎖類型。
- 自定義黑白名單 — 支援用戶自定義檢查的網站列表，方便針對特定網站進行診斷。
- JSON 輸出 — 支援以 JSON 格式輸出結果，便於進一步的數據處理和分析。
- 多線程檢查 — 可設定檢查的線程數量，提高檢查效率。
- 詳細的診斷報告 — 提供每個檢查的詳細結果，包括延遲時間和狀態碼。
...
[
  ...
  "rkn-check",
  "# 預期輸出：逐層檢查結果，顯示哪些網站可用，哪些被封鎖，及其封鎖類型。"
]
```
...
> [!tip] 爆紅原因
> 建立 18 天就累積 1289 stars（72/天），forks 56（4.3%），這顯示出穩定的增長趨勢。作者 MayersScott 之前有其他網路相關專案，這次專案解決了在 RKN/TSPU 封鎖環境中，無法清楚識別封鎖類型的痛點。之前的工具多數只能告訴使用者網站無法訪問，但無法提供具體的封鎖原因，這使得使用者無法有效地採取行動。這個專案的出現正好填補了這一空白，並且在社群中引起了廣泛的關注。forks/stars 比率為 4.3%，顯示出有一定比例的使用者對此專案進行了修改或擴展，這反映了社群的活躍度。

## 適合誰使用

**目標受眾**：需要在 RKN/TSPU 環境中進行網路診斷的系統管理員和開發者。

> [!example] 使用場景
> - 網路管理員用它來檢查公司網路是否受到 RKN 封鎖，因為能夠快速識別封鎖類型，從而採取相應措施。
> - 開發者用它來測試應用程式在不同網路環境下的可用性，因為它能提供詳細的封鎖診斷，幫助優化應用。
> - 技術支持人員用它來幫助用戶解決無法訪問特定網站的問題，因為它能提供具體的封鎖原因，便於進行故障排除。

## 架構分析

這個工具採用 Python 開發，主要由 CLI 組成，使用 `requests` 庫進行網路請求。架構上，工具分為檢查模組和報告模組，檢查模組負責逐層檢查網路連接，報告模組則負責生成診斷報告。這樣的設計使得工具能夠快速響應用戶的檢查請求，並且能夠靈活地擴展檢查的網站列表。選擇 Python 作為開發語言，因為其擁有豐富的網路庫和良好的可讀性，適合快速開發和迭代。這個工具的擴展性良好，能夠支持用戶自定義檢查列表，但在處理極大規模的檢查時，可能會面臨性能瓶頸。

## 技術深入分析

這個工具的核心技術機制是逐層檢查網路連接，使用 DNS、TCP、TLS 和 HTTP 四層來診斷封鎖。每一層的檢查都是獨立進行的，並且根據不同的失敗情況給出不同的診斷結果。效能上，這個工具能夠快速檢查多個網站，並且支持多線程檢查以提高效率。設計上選擇 Python 是因為其簡單易用和豐富的庫支持，這使得開發和維護變得更加高效。這個工具的依賴關係相對簡單，主要依賴 `requests` 庫，這使得整體安裝和使用都非常方便。技術風險方面，對於某些複雜的封鎖情況，可能會出現誤判，這需要用戶根據具體情況進行判斷。整合方面，這個工具可以很容易地與現有的 CI/CD 流程結合，並且能夠通過 JSON 輸出與其他工具進行數據交互。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且提供了詳細的使用範例，安裝過程順暢，沒有明顯的坑。文件中有針對常見場景的使用說明，對於新手來說，能夠在 30 分鐘內順利運行。文件目前僅提供英文版本，可能對某些使用者造成障礙。
...
> - 需要穩定的網路連接，否則檢查結果可能不準確。
> - 對於某些複雜的封鎖情況，可能無法提供明確的診斷。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 這個工具專注於網路代理的使用，而 rkn-block-checker 專注於檢測封鎖類型，適用場景不同。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | boneyard 提供了更廣泛的網路監控功能，但缺乏 rkn-block-checker 的逐層封鎖檢測能力。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 這個工具專注於網路代理的使用，而 rkn-block-checker 專注於檢測封鎖類型，適用場景不同。 | 如果你的需求是使用代理來繞過封鎖，而不是診斷封鎖類型，則應選擇此工具。 | medium，因為需要重新設計網路請求的方式。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | boneyard 提供了更廣泛的網路監控功能，但缺乏 rkn-block-checker 的逐層封鎖檢測能力。 | 如果需要一個全面的網路監控解決方案，而不僅僅是封鎖檢測，則應選擇此工具。 | high，因為需要重新調整整體的監控策略。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **rkn-block-checker** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 這個工具專注於網路代理的使用，而 rkn-block-checker 專注於檢測封鎖類型，適用場景不同。 | boneyard 提供了更廣泛的網路監控功能，但缺乏 rkn-block-checker 的逐層封鎖檢測能力。 |
> | 遷移成本 | - | medium，因為需要重新設計網路請求的方式。 | high，因為需要重新調整整體的監控策略。 |
> | 適用場景 | 主要場景 | 如果你的需求是使用代理來繞過封鎖，而不是診斷封鎖類型，則應選 | 如果需要一個全面的網路監控解決方案，而不僅僅是封鎖檢測，則應 |
...
> [!tip] 採用建議
> 適合用於個人或小型團隊的網路診斷，但不建議用於生產環境的核心路徑上。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- [MEDIUM] 在某些網路環境下，檢查結果可能受到 ISP 的影響，導致誤判。
  - 解法：在不同的網路環境中進行多次檢查以確認結果。
- [MEDIUM] 對於某些網站，可能會因為伺服器問題而無法正確檢測。
  - 解法：使用自定義的檢查列表來排除這些網站。
- **[HIGH]** 在高延遲的網路環境下，檢查可能會超時。
  - 解法：調整 `--timeout` 參數以增加超時時間。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 能夠快速檢查 API 是否受到 RKN 封鎖，並提供具體的封鎖類型。 |
| 大型企業的網路管理 | 適合 | 能夠提供詳細的網路診斷，但可能需要額外的整合工作。 |
| 個人用戶的日常網路檢查 | 非常適合 | 使用簡單，能夠快速了解當前網路狀況。 |
| 需要高頻率檢查的網路服務提供商 | 不適合 | 在高頻率檢查下可能會面臨性能瓶頸。 |
...
> [!tip] 投入 vs 回報
> 花 2 小時學習，1 小時整合，得到快速的網路診斷能力，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：這個工具不需要高權限運行，且不會存取敏感資料。依賴的庫也都是廣泛使用且經過驗證的，整體安全性良好。

## 生態系整合

> [!abstract] 如何融入你的工具鏈

這個工具最常與其他網路診斷工具搭配使用，處於網路檢查的開發階段。在一個使用 Python 的專案中，可以通過 `pip install` 直接整合進來，並在 CI/CD 流程中使用。支援 GitHub Actions 進行自動化檢查，與 VS Code 的整合也相對簡單。最常見的整合問題是需要確保 Python 環境的正確配置，這可能會對新手造成困難。

## 歷史脈絡

> [!info] 這個工具為什麼現在出現？

在這個工具出現之前，使用者通常依賴於單一的網路檢查工具，無法提供多層次的封鎖診斷。這使得在 RKN/TSPU 封鎖環境中，使用者無法有效地識別問題。隨著網路封鎖技術的進步，這個工具的出現正好填補了這一空白，並且在技術上能夠提供更精確的診斷。

這個工具代表了網路診斷技術的一個重要趨勢，未來可能會有更多類似的工具出現，進一步提升網路透明度和可用性。
...
> [!tip] 導入策略
> 第一週：在個人專案中試用。第二週：在內部工具中導入，收集反饋。第三週：撰寫使用文檔，分享最佳實踐。

**成功指標**：能夠快速識別網路封鎖的類型，並減少用戶報告的問題數量。

> [!warning] 退出計畫
> 如果要退出這個工具，所有設定都可以導出為 JSON 格式，方便轉換為其他工具。
...
相關概念：[[網路封鎖]] · [[DNS 污染]] · [[TLS DPI]]

相關專案：[[0x0funky--agent-sprite-forge|0x0funky/agent-sprite-forge]] · [[0xGF--boneyard|0xGF/boneyard]] · [[MayersScott--rkn-block-checker|MayersScott/rkn-block-checker]] · [[432539--gpt2api|432539/gpt2api]] · [[Narcooo--inkos|Narcooo/inkos]] · [[ahmadawais--chartli|ahmadawais/chartli]] · [[holysheep123--holysheep-cli|holysheep123/holysheep-cli]] · [[jackwener--bilibili-cli|jackwener/bilibili-cli]]
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["網路封鎖","DNS 污染","TLS DPI"];
