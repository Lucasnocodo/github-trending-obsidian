...
star_history: "2026-05-09:796,2026-05-09:800,2026-05-10:969,2026-05-10:971,2026-05-11:1062,2026-05-12:1100,2026-05-13:1149,2026-05-14:1175,2026-05-15:1192,2026-05-16:1223,2026-05-17:1235,2026-05-18:1248"
tags:
  - github
  - "category/cli_工具"
  - "lang/python"
...
> **授權** MIT (商業友好) · **維護** Moderate (最後推送 8 天前) · **貢獻者** 3 人 · **參與度** Low
> **適合** 需要了解自己網路連線狀態的技術人員和普通用戶。
> **一句話重點** 這個工具不僅告訴你網站是否可達，還告訴你為什麼無法訪問，這對於解決網路問題至關重要。
...
> **結論** 花 2 小時學習，1 小時整合，得到精確的網路診斷效果，值得投入。

> [!abstract] 核心創新
> 逐層分析網路封鎖的具體類型，提供更精確的診斷結果。

## 專案簡介

RKN Block Checker 是一個 CLI 工具，專門用來診斷當前網路連線是否受到 RKN/TSPU 封鎖，並且能夠具體指出封鎖的類型，包括 DNS 中毒、TCP 重置、TLS DPI 以及 HTTP Stub 頁面。使用者只需執行 `rkn-check`，工具會自動探測內建的網站列表，並根據每一層的檢查結果報告問題所在。這種逐層檢查的方式比單純的「網站無法訪問」更具洞察力，因為它能告訴你是在哪一層出現了問題。技術上，該工具使用 Python 3.10+ 開發，依賴 `requests` 套件，並且支持 Docker 部署，這使得它在不同環境下的使用變得簡單。與其他工具相比，如 `ping` 或 `traceroute`，RKN Block Checker 更加專注於網路封鎖的具體原因，而不僅僅是連通性問題。

它的設計考量了不同的封鎖機制，並提供了詳細的診斷報告，這對於需要繞過封鎖或進行故障排除的用戶來說非常有用。使用者可以自定義檢查的網站列表，並且可以選擇輸出 JSON 格式的結果以便進一步處理。這樣的靈活性使得 RKN Block Checker 在面對複雜的網路環境時，能夠提供更精確的診斷。整體來看，這個工具對於希望深入了解網路封鎖情況的用戶來說，是一個非常實用的選擇。
...
- 內建網站列表 — 自動探測多個網站，無需額外配置。
- 自定義檢查 — 支持用戶自定義白名單和黑名單網站列表。
- JSON 輸出 — 可選擇以 JSON 格式輸出結果，方便進一步處理。
- 多線程檢查 — 支持設置檢查的線程數量以提高效率。
...
```python
{
  ...
  "指令": "rkn-check",
  "預期輸出": "RKN Block Checker 的檢查結果，包括每個網站的檢查狀態和詳細報告。"
}
```
...
> [!tip] 爆紅原因
> 建立 14 天內累積 1248 stars（89/天），forks 55（4.4%），這顯示出其在特定社群中的快速增長。作者 MayersScott 之前有其他開源項目，這個工具解決了在 RKN/TSPU 封鎖下，使用者無法確定問題來源的痛點。這個問題在當前的網路環境中非常重要，因為許多用戶面對封鎖時無法有效地進行故障排除。社群的反應也顯示出對這個工具的需求，尤其是在需要進行網路診斷的情況下。這個工具的出現恰好填補了這一需求，並且其簡單易用的特性使得它迅速獲得了關注。

## 適合誰使用

**目標受眾**：需要了解自己網路連線狀態的技術人員和普通用戶。

> [!example] 使用場景
> - 網路管理員用它來檢查公司網路是否受到 RKN/TSPU 封鎖，因為能夠快速定位問題來源，節省了故障排除的時間。
> - 開發者用它來測試自己的應用是否在特定地區被封鎖，因為這能幫助他們更好地調整應用的可用性。
> - 普通用戶用它來了解自己所在的網路環境是否受到封鎖，因為這能讓他們選擇合適的翻牆工具。

## 架構分析

RKN Block Checker 的架構基於 Python，使用 CLI 工具模式設計，旨在提供簡單易用的網路診斷功能。資料流從 DNS 檢查開始，接著是 TCP 和 TLS，最後是 HTTP，這樣的設計使得使用者能夠快速定位問題所在。選擇 Python 作為開發語言使得工具的可移植性和擴展性較高，但也可能帶來執行效率上的挑戰。整體架構輕量，無需複雜的依賴，適合快速部署和使用。由於使用了多線程來提高檢查效率，這在高流量的網路環境中能夠顯著提升性能。

## 技術深入分析

RKN Block Checker 的核心技術機制是逐層檢查網路連線的各個層級，使用 DNS、TCP、TLS 和 HTTP 的檢查來確定封鎖的具體類型。每一層的檢查都是獨立進行的，這樣的設計能夠快速定位問題所在，並提供詳細的診斷報告。效能方面，該工具能夠在合理的時間內完成檢查，具體取決於網路狀況和檢查的網站數量。設計上選擇 Python 作為開發語言使得工具的可擴展性和可維護性較高，但在高負載情況下可能會面臨性能瓶頸。技術風險方面，對於某些複雜的封鎖機制，該工具可能無法提供準確的診斷，這需要使用者具備一定的網路知識來解讀結果。整合到現有的 CI/CD pipeline 中相對簡單，因為它的 CLI 特性使得可以輕鬆地與其他工具鏈結合使用。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了詳細的使用範例和安裝步驟。安裝過程順暢，無明顯坑點。文件中有針對常見使用情境的說明，適合新手快速上手。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於自動化代理設置，而本專案專注於封鎖診斷，適用場景不同。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供多種網路測試工具，但不專注於 RKN/TSPU 封鎖的具體分析。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於自動化代理設置，而本專案專注於封鎖診斷，適用場景不同。 | 如果你的需求是自動化代理設置而非封鎖診斷，則應選擇此工具。 | medium，因為需要重新學習不同的操作流程。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供多種網路測試工具，但不專注於 RKN/TSPU 封鎖的具體分析。 | 如果需要多功能的網路測試工具，而不僅僅是封鎖診斷，則應選擇此工具。 | low，因為使用者可以輕鬆轉換到其他網路測試工具。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **rkn-block-checker** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於自動化代理設置，而本專案專注於封鎖診斷，適用場景不同。 | 提供多種網路測試工具，但不專注於 RKN/TSPU 封鎖的具體分析。 |
> | 遷移成本 | - | medium，因為需要重新學習不同的操作流程。 | low，因為使用者可以輕鬆轉換到其他網路測試工具。 |
> | 適用場景 | 主要場景 | 如果你的需求是自動化代理設置而非封鎖診斷，則應選擇此工具。 | 如果需要多功能的網路測試工具，而不僅僅是封鎖診斷，則應選擇此 |
...
- [MEDIUM] 在某些網路環境下，檢查結果可能受到 ISP 的影響，導致誤報。
  - 解法：使用不同的網路環境進行測試以確認結果。
- [MEDIUM] 對於某些網站，可能因為伺服器問題而無法提供準確的診斷。
  - 解法：嘗試多次檢查或使用其他工具進行交叉驗證。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 網路管理員檢查公司網路 | 非常適合 | 能夠快速定位網路封鎖問題，提升故障排除效率。 |
| 開發者測試應用可用性 | 適合 | 提供詳細的封鎖診斷，幫助調整應用設計。 |
| 普通用戶了解網路狀態 | 普通 | 雖然能提供資訊，但可能需要一定的技術背景來解讀。 |
...
> [!tip] 投入 vs 回報
> 花 2 小時學習，1 小時整合，得到精確的網路診斷效果，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限，僅需訪問網路資源，且不存取敏感資料。
...
相關概念：[[網路診斷]] · [[DNS 中毒]] · [[TLS DPI]]

相關專案：[[0x0funky--agent-sprite-forge|0x0funky/agent-sprite-forge]] · [[0xGF--boneyard|0xGF/boneyard]] · [[432539--gpt2api|432539/gpt2api]] · [[MayersScott--rkn-block-checker|MayersScott/rkn-block-checker]] · [[Narcooo--inkos|Narcooo/inkos]] · [[ahmadawais--chartli|ahmadawais/chartli]] · [[holysheep123--holysheep-cli|holysheep123/holysheep-cli]] · [[jackwener--bilibili-cli|jackwener/bilibili-cli]]
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["網路診斷","DNS 中毒","TLS DPI"];
