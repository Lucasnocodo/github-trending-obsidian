...
use_case: "提供一個開放的代理架構，讓個人 AI 助手 Ohmo 能夠長時間為用戶工作。"
priority: high
ring: assess
...
aliases:
  - "OpenHarness"
  - "HKUDS/OpenHarness"
  - "提供一個開放的代理架構，讓個人 AI 助手 Ohmo 能夠長時間為用戶工作。"
---

# OpenHarness

**11.4k** stars · **439** stars/天 · 建立 26 天前 · Python · MIT
...
> [!info] 速覽
> **安裝難度** Easy · **專案狀態** Recent · **熱度** Hot (439 stars/day)
> **授權** MIT (商業友好) · **維護** Active (最後推送 2 天前) · **貢獻者** 5+ 人 · **參與度** Medium
> **適合** 需要長期運行 AI 助手並希望整合多種 API 的開發者和研究人員。
> **一句話重點** OpenHarness 的設計不僅僅是提供一個 AI 助手，而是建立一個靈活的代理架構，讓開發者能夠根據需求自定義和擴展功能。
...
> **結論** 花 5 小時學、10 小時整合，得到靈活的 AI 助手功能，值得。

> [!abstract] 核心創新
> OpenHarness 提供了一個開放的代理架構，讓個人 AI 助手能夠在多平台上長時間運行並保持上下文。

## 專案簡介

OpenHarness 是一個開源的代理架構，旨在提供工具使用、技能、記憶和多代理協調的核心基礎設施。用戶可以透過 Ohmo 這個個人 AI 助手，與其在 Feishu、Slack、Telegram 和 Discord 等平台上進行互動。Ohmo 能夠自動分支、編寫代碼、運行測試並開啟 PR，並且不需要額外的 API 金鑰，因為它可以運行在現有的 Claude Code 或 Codex 訂閱上。安裝過程簡單，只需執行 `curl -fsSL https://raw.githubusercontent.com/HKUDS/OpenHarness/main/scripts/install.sh | bash` 即可。技術上，OpenHarness 使用 Python 實現，並依賴多個庫如 `anthropic` 和 `openai` 來支持不同的 API。

這個工具的賣點在於其靈活性和擴展性，支持多種 API 和插件生態系統，並且能夠在長時間的會話中保持上下文和記憶。與其他類似工具相比，OpenHarness 的優勢在於其多代理協調能力和對多種 API 的兼容性，這使得它在需要長期運行的 AI 助手場景中表現出色。使用者可以利用 `ohmo init` 和 `ohmo config` 來快速設置個人助手，並且支持非互動模式的管道和腳本使用。這個工具的社群活躍度高，解決率達到 77%，顯示出良好的維護狀態。整體來看，OpenHarness 是一個值得關注的開源項目，特別適合需要長期運行 AI 助手的開發者和研究人員。
...
- 靈活的 API 支持 — 兼容多種 API，包括 Claude 和 OpenAI，無需額外的 API 金鑰。
- 持久記憶功能 — 能夠在多次會話中保持上下文，支持長期任務。
- 插件生態系統 — 支持自定義插件和技能擴展，滿足特定需求。
- 非互動模式支持 — 允許通過管道和腳本進行自動化操作，方便集成到現有工作流中。
...
> [!tip] 爆紅原因
> 建立 26 天內累積 11412 stars（439/天），forks 1926（16.9%），顯示出強勁的增長潛力。開發者包括多位活躍於 AI 領域的貢獻者，這些人過去在相關技術上有豐富的經驗。這個工具解決了長期運行 AI 助手的需求，並且提供了一個靈活的架構來支持多種 API，這在市場上是相對少見的。社群的活躍討論和問題解決也顯示出使用者對這個工具的需求和期待。這些因素共同促成了其快速的受歡迎程度。

## 適合誰使用

**目標受眾**：需要長期運行 AI 助手並希望整合多種 API 的開發者和研究人員。

> [!example] 使用場景
> - 後端工程師用它來自動化 CI/CD 流程，因為 Ohmo 能夠在長時間內持續運行並自動處理多個任務。
> - 產品經理用它來管理團隊的任務分配，因為它的多代理協調能力可以有效地分配任務給不同的子代理。
> - 開發者用它來在 Slack 中進行代碼審查，因為 Ohmo 能夠自動生成 PR 並進行測試，節省了大量的手動工作。

## 架構分析

OpenHarness 採用模組化設計，核心架構由多個獨立的模組組成，這樣的設計使得擴展和維護變得更加容易。資料流方面，使用者的請求會經過 Ohmo 進行處理，然後根據請求的類型調用相應的工具或 API。這樣的設計使得系統能夠靈活應對不同的請求，並且能夠在多個代理之間協調任務。

選擇 Python 作為主要語言的原因在於其豐富的生態系統和社群支持，這對於快速開發和迭代非常重要。擴展性方面，系統能夠支持多種 API 和插件，但這也可能導致在某些情況下的性能瓶頸，特別是在高負載的情況下。整體來看，這種模組化的架構設計使得 OpenHarness 在靈活性和可擴展性上具有優勢，但也需要注意在實際運行中的性能調整。

## 技術深入分析

OpenHarness 的核心技術機制是其模組化設計，這使得不同的工具和 API 能夠靈活集成。使用者的請求會經過 Ohmo 進行處理，然後根據請求的類型調用相應的工具或 API，這樣的設計使得系統能夠靈活應對不同的請求，並且能夠在多個代理之間協調任務。效能方面，系統能夠支持多達 43 種工具的並行執行，但在高負載的情況下可能會出現性能瓶頸。選擇 Python 作為主要語言的原因在於其豐富的生態系統和社群支持，這對於快速開發和迭代非常重要。依賴樹的複雜度相對較低，主要依賴於幾個核心庫，這降低了維護成本。技術風險方面，隨著使用者數量的增加，系統的性能和穩定性可能會受到影響，特別是在高並發的情況下。整合方面，OpenHarness 能夠與主流的 CI/CD 工具鏈無縫集成，並且支持多種 IDE 的插件，這使得開發者能夠在熟悉的環境中使用這個工具。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且包含詳細的安裝步驟和範例，讓新手能夠快速上手。安裝過程相對順暢，但對於新手來說，可能需要一些時間來理解配置過程。文件提供了多語言支持，這對於不同語言的使用者非常友好。

## 優缺點分析

> [!success] 優點
> - 靈活的 API 支持，能夠整合多種 AI 服務。
> - 持久記憶功能，適合長期運行的任務。
> - 開放的插件生態系統，便於擴展和自定義。

> [!danger] 缺點
> - 對於大型專案，可能需要額外的資源來維持性能。
> - 某些功能在 Windows 環境下可能會有兼容性問題。
> - 安裝和配置過程對於新手來說可能有一定的學習曲線。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於創建和管理 AI 代理，但缺乏 OpenHarness 的多代理協調能力和長期記憶功能。 |
| [AgentSeal/codeburn](https://github.com/AgentSeal/codeburn) | 提供類似的自動化功能，但不支持多種 API 的靈活集成。 |
| [HKUDS/OpenClaw-Medical-Skills](https://github.com/HKUDS/OpenClaw-Medical-Skills) | 專注於醫療領域的技能，但不具備 OpenHarness 的廣泛應用範圍和插件生態系統。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於創建和管理 AI 代理，但缺乏 OpenHarness 的多代理協調能力和長期記憶功能。 | 如果你的需求主要是創建單一代理而不是多代理協調，這個工具可能更合適。 | medium，因為需要重新設計工作流以適應不同的架構。 |
| [AgentSeal/codeburn](https://github.com/AgentSeal/codeburn) | 提供類似的自動化功能，但不支持多種 API 的靈活集成。 | 如果你的專案已經使用了 AgentSeal，轉換成本會較低。 | low，因為功能上有相似之處。 |
| [HKUDS/OpenClaw-Medical-Skills](https://github.com/HKUDS/OpenClaw-Medical-Skills) | 專注於醫療領域的技能，但不具備 OpenHarness 的廣泛應用範圍和插件生態系統。 | 如果你的專案專注於醫療領域，這個工具可能更適合。 | high，因為需要重新設計整個系統以適應不同的 API 和功能。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **OpenHarness** | **agent-sprite-forge** | **codeburn** | **OpenClaw-Medical-Skills** |
> | --- | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於創建和管理 AI 代理，但缺乏 OpenHarness 的多代理協調能力和長期記憶功能。 | 提供類似的自動化功能，但不支持多種 API 的靈活集成。 | 專注於醫療領域的技能，但不具備 OpenHarness 的廣泛應用範圍和插件生態系統。 |
> | 遷移成本 | - | medium，因為需要重新設計工作流以適應不同的架構。 | low，因為功能上有相似之處。 | high，因為需要重新設計整個系統以適應不同的 API 和功能。 |
> | 適用場景 | 主要場景 | 如果你的需求主要是創建單一代理而不是多代理協調，這個工具可能 | 如果你的專案已經使用了 AgentSeal，轉換成本會較低。 | 如果你的專案專注於醫療領域，這個工具可能更適合。 |
...
- **[HIGH]** 在 Windows 環境下，某些命令可能會因為 PowerShell 的限制而無法正常運行。
  - 解法：使用 WSL 或者在 Linux 環境中運行。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 靈活的 API 支持和持久記憶功能能夠滿足小型團隊的需求。 |
| 大型企業的多代理協調系統 | 適合 | 雖然支持多代理，但在高負載下可能需要額外的資源。 |
| 個人開發者的 AI 助手 | 非常適合 | 簡單的安裝和配置過程使得個人開發者能夠快速上手。 |
| 需要長期運行的 AI 代理系統 | 普通 | 雖然支持長期運行，但在高並發情況下可能會出現性能問題。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學、10 小時整合，得到靈活的 AI 助手功能，值得。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限運行，且不存取敏感資料。依賴鏈中有些庫可能存在已知漏洞，但整體風險可控。在 CI/CD 中使用是安全的。
...
> | [#206](https://github.com/HKUDS/OpenHarness/issues/206) | [Feature]: Support Authorization Bearer like x-api-key `enhancement` | 0 | 0 |
> | [#204](https://github.com/HKUDS/OpenHarness/issues/204) | [Bug] Subprocess stderr=PIPE deadlocks background tasks in g | 0 | 0 |
...
相關專案：[[0x0funky--agent-sprite-forge|0x0funky/agent-sprite-forge]] · [[AgentSeal--codeburn|AgentSeal/codeburn]] · [[0xGF--boneyard|0xGF/boneyard]] · [[Einsia--OpenChronicle|Einsia/OpenChronicle]] · [[AgriciDaniel--claude-obsidian|AgriciDaniel/claude-obsidian]] · [[FreedomIntelligence--OpenClaw-Medical-Skills|FreedomIntelligence/OpenClaw-Medical-Skills]] · [[HKUDS--ClawTeam|HKUDS/ClawTeam]] · [[HKUDS--CLI-Anything|HKUDS/CLI-Anything]]
