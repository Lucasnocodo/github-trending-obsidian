...
use_case: "提供一個開放的 AI 代理架構，讓用戶能夠與個人 AI 代理進行互動，並自動化多項任務。"
priority: high
ring: assess
...
star_history: "2026-04-08:7142,2026-04-08:7164,2026-04-09:7764,2026-04-10:8224,2026-04-11:8613,2026-04-12:8807,2026-04-13:9045,2026-04-14:9341,2026-04-15:9616,2026-04-16:9857,2026-04-17:10038"
tags:
  - github
  - "category/ai_ml"
  - "lang/python"
...
aliases:
  - "OpenHarness"
  - "HKUDS/OpenHarness"
  - "提供一個開放的 AI 代理架構，讓用戶能夠與個人 AI 代理進行互動，並自動化多項任務。"
---

# OpenHarness

**10.0k** stars · **669** stars/天 · 建立 15 天前 · Python · MIT
...
> [!info] 速覽
> **安裝難度** Easy · **專案狀態** Recent · **熱度** Hot (669 stars/day)
> **授權** MIT (商業友好) · **維護** Active (最後推送 0 天前) · **貢獻者** 5+ 人 · **參與度** Medium
> **適合** 需要在多平台上自動化 AI 代理互動的開發者和研究者。
> **一句話重點** OpenHarness 不僅是個 AI 代理，更是一個開放的架構，讓開發者能夠自由地擴展和自定義功能。
...
> **結論** 花 5 小時學習，10 小時整合，得到高效的 AI 代理功能，值得投入。

> [!abstract] 核心創新
> OpenHarness 提供了一個開放的 AI 代理架構，讓用戶能夠與個人 AI 代理進行互動，並自動化多項任務。

## 專案簡介

OpenHarness 是一個開源的 AI 代理架構，旨在提供一個輕量級的基礎設施，讓用戶能夠與個人 AI 代理 ohmo 進行互動。用戶可以透過 Feishu、Slack、Telegram 和 Discord 與 ohmo 聊天，ohmo 能夠自動分支、撰寫程式碼、執行測試並開啟 PR，這些功能無需額外的 API 金鑰，因為它可以直接使用現有的 Claude Code 或 Codex 訂閱。使用者只需執行 `pip install openharness-ai`，然後透過 `oh setup` 進行配置即可啟動。這個架構的設計重點在於提供多代理協調、上下文記憶及工具使用的功能，並支援多種 API 提供者，這使得它在開發過程中極具靈活性和擴展性。

核心技術包括使用 Python 和 TypeScript，並整合多種依賴來實現其功能。與其他工具相比，OpenHarness 提供了更高的自動化程度和多樣的 API 支援，例如支援 OpenAI 和 Anthropic 的 API，這使得它在多種場景下都能靈活應用。使用者在實際操作中可能會遇到一些問題，例如需要正確配置 API 提供者，這在初次使用時可能會造成困擾。這個專案目前處於 v0.1.6 版本，功能仍在持續增強，適合對 AI 代理有興趣的開發者和研究者使用。

**技術棧**：`Python 3.10` · `TypeScript` · `Docker`

## 重點功能

- 多代理協調 — 支援多個代理同時運行並協同工作，提升任務處理效率。
- 上下文記憶 — 自動壓縮上下文以節省資源，並支援長時間會話。
- 工具使用 — 提供 43 種工具的支援，包括文件、Shell 和 Web 查詢等。
- API 兼容性 — 支援多種 API 提供者，如 OpenAI 和 Anthropic，無需額外金鑰。
- 自動化任務 — ohmo 能夠自動撰寫程式碼、執行測試並開啟 PR，提升開發效率。
...
> [!tip] 爆紅原因
> 建立 15 天內累積 10038 stars（669/天），forks 1706（17.0%），顯示出強勁的增長潛力。主要貢獻者包括多位活躍的開發者，這表明社群參與度高。這個專案解決了在多平台上自動化與 AI 代理互動的需求，之前的解決方案往往缺乏靈活性和擴展性。最近的推特討論和社群反饋也促進了其曝光率。技術上，隨著 AI 代理技術的進步，這個工具的可行性大幅提升。高達 17% 的 forks/stars 比率顯示出許多開發者正在實際修改和使用這個專案。

## 適合誰使用

**目標受眾**：需要在多平台上自動化 AI 代理互動的開發者和研究者。

> [!example] 使用場景
> - 後端工程師用它來自動化測試和部署流程，因為 ohmo 能夠自動撰寫測試和開啟 PR，節省了大量手動操作時間。
> - 產品經理用它來快速生成需求文檔，因為 ohmo 可以根據簡單的指令生成結構化的文檔，提升了工作效率。
> - 開發者用它來在 Slack 中進行即時的程式碼審查，因為 ohmo 能夠即時回應查詢並提供建議，改善了團隊協作。

## 架構分析

OpenHarness 採用模組化的設計架構，將核心功能分為多個獨立的模組，這樣的設計使得擴展和維護變得更加容易。資料流方面，使用者的請求會通過 ohmo 代理進行處理，然後根據不同的 API 提供者進行調用。這樣的架構使得用戶能夠輕鬆切換不同的 API 提供者，而不需要改動核心邏輯。

選擇 Python 和 TypeScript 作為主要開發語言，這樣的選擇提供了良好的生態系統支持和開發效率。這種模組化設計的代價是可能會增加初次配置的複雜性，尤其是在多代理協調的情境下。擴展性方面，這種架構能夠支持未來的功能擴展，但在高負載情況下可能會出現性能瓶頸，特別是在多代理同時運行時。

## 技術深入分析

OpenHarness 的核心技術機制是基於 Python 和 TypeScript 的模組化設計，這使得其架構能夠靈活應對不同的使用場景。使用者的請求會通過 ohmo 代理進行處理，並根據不同的 API 提供者進行調用，這樣的設計使得用戶能夠輕鬆切換不同的 API 提供者。效能方面，該架構能夠支持多達 43 種工具的使用，並且在多代理協調的情況下，能夠有效地管理任務和上下文。設計取捨方面，選擇 Python 和 TypeScript 作為主要開發語言，這樣的選擇提供了良好的生態系統支持和開發效率，但也可能導致在高負載情況下出現性能瓶頸。技術風險方面，隨著使用者數量的增加，可能會出現系統資源的競爭，這需要在架構上進行優化以應對未來的擴展需求。整合方面，OpenHarness 能夠與主流的 CI/CD 工具鏈相容，並且提供了良好的文檔支持，降低了整合的摩擦點。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且提供了詳細的安裝步驟和範例，讓新手能夠快速上手。安裝過程相對順暢，但在配置 API 提供者時可能會遇到一些挑戰。文件目前僅提供英文，缺乏多語言支持，這對於非英語使用者來說可能會造成困難。

## 優缺點分析

> [!success] 優點
> - 開源且活躍的社群支持，便於獲取幫助和貢獻。
> - 多平台兼容性，能夠在多種聊天應用中運行。
> - 自動化程度高，能夠顯著提高開發效率。

> [!danger] 缺點
> - 需要一定的配置和學習成本，尤其是對於新手來說。
> - 在某些平台上可能會遇到兼容性問題。
> - 功能仍在持續開發中，可能會有不穩定的情況。

> [!warning] 注意事項
> - 僅支援 Python 3.10 以上版本。
> - 需要正確配置 API 提供者，否則可能無法正常運行。
> - 在某些平台上可能會遇到兼容性問題，特別是在 Windows 環境中。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供類似的 AI 代理功能，但主要集中在特定任務的自動化，而 OpenHarness 提供更廣泛的多代理協作功能。 |
| [AgentSeal/codeburn](https://github.com/AgentSeal/codeburn) | 專注於安全性和隱私保護，而 OpenHarness 更加強調靈活性和擴展性。 |
| [FreedomIntelligence/OpenClaw-Medical-Skills](https://github.com/FreedomIntelligence/OpenClaw-Medical-Skills) | 主要針對醫療領域的 AI 代理，而 OpenHarness 則是通用的 AI 代理框架，適用於多種場景。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [AgentSeal/codeburn](https://github.com/AgentSeal/codeburn) | 專注於安全性和隱私保護，而 OpenHarness 更加強調靈活性和擴展性。 | 如果你的應用需要強調安全性和隱私保護，AgentSeal 會是更好的選擇。 | medium - 由於架構和功能的不同，遷移可能需要重新設計部分邏輯。 |
| [FreedomIntelligence/OpenClaw-Medical-Skills](https://github.com/FreedomIntelligence/OpenClaw-Medical-Skills) | 專注於醫療領域的 AI 代理，而 OpenHarness 則是通用的 AI 代理框架。 | 如果你的專案是針對醫療應用，OpenClaw 提供了更專業的功能和工具。 | high - 由於專案的專業性，遷移可能需要大量的調整和測試。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **OpenHarness** | **codeburn** | **OpenClaw-Medical-Skills** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於安全性和隱私保護，而 OpenHarness 更加強調靈活性和擴展性。 | 專注於醫療領域的 AI 代理，而 OpenHarness 則是通用的 AI 代理框架。 |
> | 遷移成本 | - | medium - 由於架構和功能的不同，遷移可能需要重新設計部分邏輯。 | high - 由於專案的專業性，遷移可能需要大量的調整和測試。 |
> | 適用場景 | 主要場景 | 如果你的應用需要強調安全性和隱私保護，AgentSeal 會 | 如果你的專案是針對醫療應用，OpenClaw 提供了更專業的 |
...
- **[HIGH]** 在 Windows 環境中，某些指令可能無法正常執行，導致安裝失敗。
  - 解法：確保使用 PowerShell 的最新版本，並遵循官方文檔進行安裝。
- [MEDIUM] API 提供者配置不當可能導致 ohmo 無法正常運行。
  - 解法：仔細檢查配置步驟，確保所有必要的憑證和設定都已正確填寫。
- [MEDIUM] 在高負載情況下，多代理協調可能會導致性能下降。
  - 解法：考慮使用更高效的資源管理策略，或限制同時運行的代理數量。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | OpenHarness 提供了靈活的多代理協作功能，能夠有效支持小型團隊的開發需求。 |
| 大型企業的生產環境 | 不適合 | 目前仍在 beta 階段，穩定性和性能可能無法滿足生產環境的需求。 |
| AI 研究團隊進行實驗 | 適合 | 開源的特性和靈活的架構使其成為研究和實驗的理想選擇。 |
| 需要強調安全性的應用 | 普通 | 雖然提供了一些安全功能，但相較於專注於安全的工具，可能不夠完善。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，10 小時整合，得到高效的 AI 代理功能，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：OpenHarness 本身不需要高權限，且不會存取敏感資料。依賴鏈的信任程度高，無已知的供應鏈風險，適合在 CI/CD 中使用。
...
> | 貢獻者 | Commits |
> | --- | --- |
> | [@tjb-tech](https://github.com/tjb-tech) | 177 |
...
> **活躍天數** 2 天 · **最新 commit** fix(dashboard): rename to OpenHarness, simplify kanban to 4 columns, richer pipeline animation
...
> | [#153](https://github.com/HKUDS/OpenHarness/issues/153) | [Bug] Explore and claude-code-guide agents fail on non-Anthr | 0 | 0 |
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["Agent 框架","多模態","CLI/TUI"];
