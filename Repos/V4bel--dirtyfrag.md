...
use_case: "透過鏈接兩個漏洞來獲取 Linux 系統的 root 權限。"
priority: high
ring: assess
...
star_history: "2026-05-09:2889,2026-05-09:2898,2026-05-10:3645,2026-05-10:3648,2026-05-11:4025,2026-05-11:4026,2026-05-12:4201,2026-05-12:4203,2026-05-13:4331,2026-05-13:4331,2026-05-14:4421,2026-05-14:4421,2026-05-15:4485,2026-05-16:4544,2026-05-17:4583,2026-05-18:4613,2026-05-19:4641"
tags:
  - github
  - "category/安全"
  - "lang/c"
...
**4.6k** stars · **422** stars/天 · 建立 11 天前 · C · 未標註授權
...
> [!info] 速覽
> **安裝難度** Easy · **專案狀態** Recent · **熱度** Hot (422 stars/day)
> **授權** 未標註授權 (風險較高) · **維護** Moderate (最後推送 8 天前) · **貢獻者** 2 人 · **參與度** Medium
> **適合** 需要測試 Linux 系統安全性的安全研究人員和系統管理員。
> **一句話重點** Dirty Frag 不僅是一個漏洞利用工具，更是對 Linux 系統安全性的一次深刻反思。
...
> [!abstract] 核心創新
> Dirty Frag 是一種新的漏洞利用工具，能夠通過鏈接兩個已知漏洞來獲取 root 權限，具有高成功率和廣泛的兼容性。

## 專案簡介

Dirty Frag 是一個針對 Linux 系統的漏洞利用工具，通過鏈接 `xfrm-ESP Page-Cache Write (CVE-2026-43284)` 和 `RxRPC Page-Cache Write (CVE-2026-43500)` 兩個漏洞來獲取 root 權限。用戶只需執行一行命令 `git clone https://github.com/V4bel/dirtyfrag.git && cd dirtyfrag && gcc -O0 -Wall -o exp exp.c -lutil && ./exp` 即可啟動漏洞利用。這個工具的賣點在於其高成功率，因為它不依賴於時間窗口，也不會因為失敗而導致內核崩潰。該工具的設計考慮到了大多數 Linux 發行版的兼容性，並且能夠在多個主要發行版上運行。技術上，這個工具使用了 C 語言，並且依賴於 Linux 的內核特性，這使得它在執行時的效率非常高。

與其他漏洞利用工具相比，Dirty Frag 的優勢在於其不需要特權來創建命名空間，這使得它在某些受限環境下仍然能夠運行。使用者在運行該工具後必須清理頁面緩存，以確保系統穩定，這一點在 README 中有明確的指示。這個工具的社群活躍度高，開發者積極回應問題，並且有多個熱門的討論主題，顯示出用戶對這個工具的關注和需求。整體來看，Dirty Frag 是一個針對 Linux 系統的強大漏洞利用工具，適合安全研究人員和系統管理員使用。

**技術棧**：`C`

## 重點功能

- 高成功率 — 由於不依賴於時間窗口，成功率非常高。
- 多發行版兼容性 — 能在多個主要 Linux 發行版上運行，包括 Ubuntu、RHEL 和 Fedora。
- 簡單的使用方式 — 只需一行命令即可啟動漏洞利用。
- 清理頁面緩存的指令 — 提供清理污染頁面緩存的指令以確保系統穩定。
- 活躍的社群 — 有多個熱門討論和問題，顯示出用戶的關注和需求。
...
> [!tip] 爆紅原因
> 建立 11 天就累積 4641 stars（422/天），forks 732（15.8%），這顯示出極高的關注度。作者 V4bel 是一位活躍的安全研究者，之前的研究成果也引起了廣泛關注。這個工具解決了 Linux 系統中存在的根權限獲取問題，之前的解決方案往往需要特權或特定環境，限制了其使用範圍。最近的安全事件和漏洞披露使得這個工具的需求激增。高 forks/stars 比率表明許多開發者正在積極修改和使用這個工具，顯示出其實用性和潛在的應用場景。

## 適合誰使用

**目標受眾**：需要測試 Linux 系統安全性的安全研究人員和系統管理員。

> [!example] 使用場景
> - 安全研究人員用它來測試 Linux 系統的安全性，因為它能快速驗證系統是否受到 Dirty Frag 漏洞影響。
> - 系統管理員用它來檢查和修補 Linux 發行版中的安全漏洞，因為這能幫助他們提前發現潛在的安全風險。
> - 開發者用它來進行漏洞利用測試，因為它提供了一個簡單的 PoC 來展示漏洞的存在。

## 架構分析

Dirty Frag 的架構基於兩個漏洞的鏈接，利用了 Linux 的內核特性來實現權限提升。這種設計使得它不依賴於時間窗口，從而提高了成功率。資料流方面，工具首先檢查系統的兼容性，然後利用漏洞進行權限提升，最後提供清理頁面緩存的指令。選擇這種設計的代價是需要用戶在執行後手動清理系統。這樣的設計使得工具在多個發行版上都能有效運行，但也可能導致在某些環境下的兼容性問題。

## 技術深入分析

Dirty Frag 利用兩個漏洞來實現權限提升，這兩個漏洞分別是 `xfrm-ESP Page-Cache Write` 和 `RxRPC Page-Cache Write`。這些漏洞的利用不需要特權來創建命名空間，這使得它在某些受限環境下仍然能夠運行。該工具的設計使得它在執行時的效率非常高，並且能夠在多個主要發行版上運行。使用者在運行該工具後必須清理頁面緩存，以確保系統穩定，這一點在 README 中有明確的指示。這種設計考慮到了大多數 Linux 發行版的兼容性，並且能夠在多個主要發行版上運行。整體來看，Dirty Frag 是一個針對 Linux 系統的強大漏洞利用工具，適合安全研究人員和系統管理員使用。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的指導，並包含了範例指令。安裝過程相對順暢，沒有明顯的坑。文件中有關於如何清理頁面緩存的說明，對新手友好。
...
> - 高成功率，因為不依賴於時間窗口。
> - 簡單易用，只需一行命令即可啟動。
> - 廣泛的發行版兼容性，能在多個主要 Linux 發行版上運行。
> - 活躍的社群支持，能快速獲得幫助和更新。

> [!danger] 缺點
> - 僅支援特定的 Linux 發行版，限制了使用範圍。
> - 需要在授權的系統上運行，否則可能違反法律。
> - 執行後需要清理頁面緩存，增加了操作步驟。
> - 存在一定的安全風險，使用不當可能導致系統不穩定。
