...
use_case: "透過鏈接兩個漏洞實現 Linux 系統的本地提權。"
priority: high
ring: assess
...
star_history: "2026-05-09:2889,2026-05-09:2898,2026-05-10:3645,2026-05-10:3648,2026-05-11:4025,2026-05-11:4026,2026-05-12:4201,2026-05-12:4203,2026-05-13:4331,2026-05-13:4331,2026-05-14:4421,2026-05-14:4421,2026-05-15:4485,2026-05-16:4544,2026-05-17:4583"
tags:
  - github
  - "category/安全"
  - "lang/c"
...
aliases:
  - "dirtyfrag"
  - "V4bel/dirtyfrag"
  - "透過鏈接兩個漏洞實現 Linux 系統的本地提權。"
---

# dirtyfrag

**4.6k** stars · **509** stars/天 · 建立 9 天前 · C · 未標註授權
...
> [!info] 速覽
> **安裝難度** Easy · **專案狀態** Recent · **熱度** Hot (509 stars/day)
> **授權** 未標註授權 (風險較高) · **維護** Active (最後推送 6 天前) · **貢獻者** 2 人 · **參與度** Medium
> **適合** 需要檢測 Linux 系統安全性並尋找本地提權漏洞的安全研究人員。
> **一句話重點** Dirty Frag 的出現不僅揭示了 Linux 系統中的安全漏洞，也強調了在多發行版環境中進行安全測試的重要性。
...
>     .where(p => p.subcategory === "漏洞測試" && p.file.name !== "V4bel--dirtyfrag" && p.status !== "archived")
...
> **結論** 花 2 小時學習，1 小時整合，能夠有效檢測 Linux 系統的安全性，值得採用。

> [!abstract] 核心創新
> Dirty Frag 是一個新發現的漏洞類別，能夠鏈接兩個已知漏洞以實現本地提權。

## 專案簡介

Dirty Frag 是一個針對主要 Linux 發行版的本地提權漏洞，通過鏈接 `xfrm-ESP Page-Cache Write (CVE-2026-43284)` 和 `RxRPC Page-Cache Write (CVE-2026-43500)` 來獲取 root 權限。這個漏洞的核心在於它不依賴於競爭條件，且在失敗時不會導致內核崩潰，成功率極高。使用者只需執行一行命令 `git clone https://github.com/V4bel/dirtyfrag.git && cd dirtyfrag && gcc -O0 -Wall -o exp exp.c -lutil && ./exp` 即可進行測試。該工具的設計考量了不同發行版的安全策略，通過鏈接兩個漏洞來覆蓋彼此的盲點，從而在所有主要發行版上獲得 root 權限。這種設計使得即使在某些環境中無法觸發一個漏洞，仍然可以通過另一個漏洞來實現攻擊。

該漏洞的有效期約為 9 年，並已在多個發行版上進行測試，包括 Ubuntu、RHEL 和 Fedora 等。使用者在運行該漏洞後需清理頁面快取，確保系統穩定，這可以通過執行 `echo 3 > /proc/sys/vm/drop_caches` 或重啟系統來完成。此工具的社群活躍度高，熱門問題顯示出使用者對漏洞的關注和討論，尤其是對於如何修復和緩解的討論。整體來看，Dirty Frag 是一個針對 Linux 系統安全的重要研究工具，適合安全研究人員和系統管理員使用。

**技術棧**：`C`

## 重點功能

- 本地提權漏洞鏈接 — 通過鏈接兩個漏洞來獲取 root 權限，無需競爭條件。
- 高成功率 — 由於不依賴於時間窗口，成功率極高。
- 多發行版支持 — 已在多個主要 Linux 發行版上進行測試，包括 Ubuntu、RHEL 和 Fedora。
- 簡單的使用方式 — 只需一行命令即可運行測試。
- 清理頁面快取 — 提供清理頁面快取的命令以確保系統穩定。
...
```c
[
  "# 前置條件：需要在 Linux 環境中運行；",
  "git clone https://github.com/V4bel/dirtyfrag.git && cd dirtyfrag && gcc -O0 -Wall -o exp exp.c -lutil && ./exp",
  "# 預期輸出：成功獲取 root 權限（具體輸出依賴於環境設定）。"
]
```
...
> [!tip] 爆紅原因
> 建立 9 天內累積 4583 stars（509/天），forks 716（15.6%），這顯示出強烈的社群關注。作者 V4bel 之前在安全領域有過多次貢獻，這次的 Dirty Frag 漏洞解決了 Linux 系統中本地提權的痛點，特別是在多發行版環境下的安全性。這一漏洞的發現引起了廣泛的討論，尤其是在安全社群中，並且有許多使用者對其潛在影響表示擔憂。這個工具的出現正好填補了之前缺乏有效解決方案的空白，並且隨著 Linux 安全性需求的上升，這個工具的實用性也愈加凸顯。

## 適合誰使用

**目標受眾**：需要檢測 Linux 系統安全性並尋找本地提權漏洞的安全研究人員。

> [!example] 使用場景
> - 安全研究人員用它來測試 Linux 系統的安全性，因為它能有效地揭示系統中的本地提權漏洞。
> - 系統管理員用它來檢查自家伺服器的安全性，因為能夠快速驗證是否受到 Dirty Frag 漏洞的影響。
> - 開發者用它來進行漏洞測試，因為這能幫助他們在開發過程中提前發現潛在的安全問題。

## 架構分析

Dirty Frag 的架構設計基於鏈接兩個漏洞，這種設計選擇使得攻擊者能夠在多個發行版上獲得 root 權限。資料流中，首先通過 `xfrm-ESP Page-Cache Write` 獲取寫入權限，然後利用 `RxRPC Page-Cache Write` 進一步提升權限。這種設計的優勢在於不依賴於時間窗口，降低了攻擊失敗的風險，但代價是需要在特定的環境中運行。整體來看，這種架構在多發行版的兼容性上表現良好，但在某些環境中仍可能受到 AppArmor 等安全策略的限制。

## 技術深入分析

Dirty Frag 利用兩個漏洞進行鏈接，這種設計使得攻擊者能夠在多個發行版上獲得 root 權限。核心技術在於 `xfrm-ESP Page-Cache Write` 和 `RxRPC Page-Cache Write` 的結合，這使得攻擊者能夠在不依賴於時間窗口的情況下成功執行攻擊。效能上，該工具在多個發行版上經過測試，顯示出良好的穩定性和高成功率。設計上選擇 C 語言實現，這使得其在性能上有優勢，但也可能導致較高的安全風險。技術風險方面，這種設計在面對未來的安全更新時可能會遇到挑戰，特別是當發行版開始修補這些漏洞時。整合上，該工具與現有的 Linux 環境相容性良好，但在某些情況下可能需要額外的配置。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了詳細的使用說明和範例。安裝過程相對簡單，僅需一行命令即可運行。文件中缺乏多語言支持，但內容足夠詳細，適合有一定技術背景的使用者。整體來說，花 30 分鐘能夠成功運行。
...
> - 高成功率，無需競爭條件。
> - 簡單易用，僅需一行命令即可測試。
> - 支持多個主要 Linux 發行版，適用範圍廣泛。

> [!danger] 缺點
> - 僅適用於特定的 Linux 發行版，其他發行版可能不受影響。
> - 運行後需清理頁面快取以避免系統不穩定。
> - 需要在具有相應權限的環境中運行，否則無法測試。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於不同類型的漏洞測試，Dirty Frag 專注於特定的本地提權漏洞。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供更廣泛的漏洞測試工具，而 Dirty Frag 專注於鏈接特定漏洞。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於不同類型的漏洞測試，提供多樣化的漏洞檢測工具。 | 如果需要更廣泛的漏洞測試工具，而不僅僅是針對本地提權的測試。 | low，因為兩者的使用方式相似。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供更廣泛的漏洞測試工具，涵蓋多種漏洞類型。 | 如果需要針對多種漏洞進行測試，而不僅限於本地提權。 | medium，因為可能需要調整測試策略。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **dirtyfrag** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於不同類型的漏洞測試，提供多樣化的漏洞檢測工具。 | 提供更廣泛的漏洞測試工具，涵蓋多種漏洞類型。 |
> | 遷移成本 | - | low，因為兩者的使用方式相似。 | medium，因為可能需要調整測試策略。 |
> | 適用場景 | 主要場景 | 如果需要更廣泛的漏洞測試工具，而不僅僅是針對本地提權的測試。 | 如果需要針對多種漏洞進行測試，而不僅限於本地提權。 |
...
> [!tip] 採用建議
> 適合安全研究人員進行漏洞測試，但不建議在生產環境中使用。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在某些 Linux 發行版上可能無法觸發漏洞，特別是受到 AppArmor 限制的環境。
  - 解法：檢查系統安全策略，必要時調整配置。
- [MEDIUM] 運行後系統可能不穩定，需要清理頁面快取。
  - 解法：執行 `echo 3 > /proc/sys/vm/drop_caches` 來清理。
- **[HIGH]** 在未經授權的系統上運行可能違反法律。
  - 解法：僅在授權的測試環境中運行。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 大型企業的 Linux 伺服器安全檢測 | 非常適合 | 能夠有效檢測本地提權漏洞，保障伺服器安全。 |
| 個人開發者的安全測試工具 | 適合 | 簡單易用，適合個人開發者進行安全測試。 |
| 小型企業的系統管理 | 普通 | 雖然有用，但可能需要更多的安全防護措施。 |
| 不熟悉 Linux 系統的使用者 | 不適合 | 需要一定的技術背景，否則難以理解和使用。 |
...
> [!tip] 投入 vs 回報
> 花 2 小時學習，1 小時整合，能夠有效檢測 Linux 系統的安全性，值得採用。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具本身不需要高權限，但在測試過程中可能會影響系統穩定性。使用時需確保在授權的環境中進行測試。
...
**社群活躍度**：社群活躍，熱門問題顯示出使用者對漏洞的關注和討論。
...
> FROM "Repos"
> WHERE subcategory = "漏洞測試" AND file.name != "V4bel--dirtyfrag"
> SORT stars DESC
> ```
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["安全漏洞","本地提權","Linux 安全"];
