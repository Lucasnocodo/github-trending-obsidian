...
use_case: "透過鏈結兩個 Linux 漏洞，實現獲取 root 權限的攻擊手法。"
priority: high
ring: assess
...
> **適合** 對 Linux 安全性有興趣的安全研究員和系統管理員。
> **一句話重點** Dirty Frag 不僅是一個新的漏洞利用工具，更是對 Linux 核心安全性的一次深刻挑戰。
...
> **結論** 花 2 小時學習、1 小時整合，得到對 Linux 安全性的深入理解，值得嘗試。

> [!abstract] 核心創新
> Dirty Frag 是一種新的本地特權提升漏洞，通過鏈結兩個已知漏洞來實現。這種方法不依賴於競爭條件，成功率高。

## 專案簡介

Dirty Frag 是一種新的 Linux 本地特權提升漏洞，通過鏈結 `xfrm-ESP Page-Cache Write` 和 `RxRPC Page-Cache Write` 漏洞來獲取 root 權限。這種漏洞不依賴於競爭條件，成功率高，且不會導致內核崩潰。用戶只需執行一行命令：`git clone https://github.com/V4bel/dirtyfrag.git && cd dirtyfrag && gcc -O0 -Wall -o exp exp.c -lutil && ./exp`，即可測試該漏洞。這個工具的賣點在於其簡單的使用方式和高效的攻擊成功率。技術上，這個漏洞利用了 Linux 核心中的特定邏輯錯誤，並且已在多個主要發行版上進行了測試，包括 Ubuntu 和 RHEL。

與其他漏洞相比，Dirty Frag 的特點在於它不需要時間窗口，這使得它在實際攻擊中更具優勢。使用者在運行該漏洞後，必須清理被污染的頁面快取，這可以通過執行 `echo 3 > /proc/sys/vm/drop_caches` 來完成。這個工具的社群活躍度高，開放問題的解決率為 13%。目前該漏洞的修補尚未普遍推出，這使得它在安全研究中具有重要意義。對於希望了解 Linux 安全性和漏洞利用的開發者，Dirty Frag 提供了一個實用的學習資源。

**技術棧**：`C`

## 重點功能

- 鏈結漏洞利用 — 通過鏈結兩個漏洞獲取 root 權限，成功率高。
- 簡單的使用方式 — 只需一行命令即可運行漏洞利用程式。
- 高效的攻擊成功率 — 不依賴於時間窗口，避免了競爭條件的問題。
- 污染頁面快取的清理 — 提供清理污染頁面快取的命令，確保系統穩定性。
- 廣泛的發行版支持 — 在多個主要 Linux 發行版上進行測試，確保其普遍適用性。
...
> [!tip] 爆紅原因
> 建立 1 天就累積 2889 stars（2889/天），forks 446（15.4%），顯示出強烈的社群興趣。這位作者 V4bel 之前已經發現過其他類似的漏洞，這使得他在安全研究領域有一定的聲譽。Dirty Frag 解決了之前在 Linux 系統中缺乏有效特權提升漏洞的痛點，特別是在不依賴時間窗口的情況下。這些特性使得它在安全研究者和攻擊者中都引起了廣泛關注。社群討論熱烈，尤其是關於漏洞的利用和修補的問題，顯示出這個專案的實際影響力。

## 適合誰使用

**目標受眾**：對 Linux 安全性有興趣的安全研究員和系統管理員。

> [!example] 使用場景
> - 安全研究員用它來測試 Linux 系統的安全性，因為它提供了一個簡單的方式來驗證系統的漏洞。
> - 系統管理員用它來檢查和加固伺服器的安全性，因為了解這種漏洞有助於提前防範潛在的攻擊。
> - 開發者用它來學習 Linux 核心漏洞的利用技術，因為這是一個實用的案例來理解漏洞鏈的運作。

## 架構分析

Dirty Frag 的架構基於兩個漏洞的鏈結，這使得其利用過程相對簡單。使用者只需編譯並運行一個 C 程式，便可觸發漏洞。這種設計的好處在於其易用性和高效性，然而，這也意味著其在不同環境中的適用性可能受到限制。由於不依賴於時間窗口，這使得漏洞的利用更加穩定，但也可能導致系統在遭受攻擊後需要手動清理頁面快取，這是一個潛在的維護負擔。整體而言，這個工具的架構設計旨在簡化漏洞利用的過程，並提高成功率。

## 技術深入分析

Dirty Frag 利用的核心技術是將兩個漏洞鏈結在一起，這使得攻擊者能夠在不依賴競爭條件的情況下獲取 root 權限。這種設計的優勢在於其高成功率和穩定性，因為它不會因為時間窗口的問題而失敗。該工具的效能在於其簡單的使用方式，使用者只需執行一行命令即可完成漏洞利用。從技術選型上看，使用 C 語言編寫使得其在性能上表現優異，且能夠直接與 Linux 核心進行交互。

這種選擇雖然提高了性能，但也增加了使用者對於 C 語言的要求，可能會對新手造成一定的學習負擔。設計上，這個工具的依賴性相對較低，主要依賴於 Linux 核心的特定版本，這使得其在不同的系統上具有較好的兼容性。然而，這也意味著在某些環境中可能會出現不兼容的情況，這是使用者需要注意的風險。整體而言，Dirty Frag 提供了一個高效且易於使用的漏洞利用方案，對於安全研究和實際攻擊都具有重要的參考價值。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的漏洞描述和使用指令，安裝過程相對順暢。雖然沒有詳細的入門指南，但核心命令簡單明瞭，對於有一定基礎的使用者來說，能夠在短時間內上手。文件目前僅提供英文版本，可能對非英語使用者造成一定的障礙。
...
> [!warning] 注意事項
> - 目前尚未有普遍的修補方案可用於所有發行版。
> - 需要在受測系統上擁有適當的權限來運行漏洞利用程式。
> - 在某些環境中，可能會受到 AppArmor 等安全措施的限制。
...
- **[HIGH]** 運行漏洞利用後，頁面快取會被污染，需手動清理。
  - 解法：執行 `echo 3 > /proc/sys/vm/drop_caches` 清理頁面快取。
- [MEDIUM] 在某些 Linux 發行版中，可能會受到 AppArmor 等安全措施的限制。
  - 解法：確認系統安全策略，必要時進行調整。
- **[HIGH]** 目前尚無普遍的修補方案，可能導致系統長期暴露於風險中。
  - 解法：定期檢查發行版的更新，及時應用安全補丁。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 大型企業的 Linux 伺服器安全測試 | 非常適合 | 能夠有效測試伺服器的安全性，發現潛在漏洞。 |
| 個人開發者的安全研究項目 | 適合 | 提供了一個簡單的方式來學習和實驗漏洞利用技術。 |
| 生產環境的核心系統 | 不適合 | 由於尚無修補方案，存在安全風險。 |
...
> [!tip] 投入 vs 回報
> 花 2 小時學習、1 小時整合，得到對 Linux 安全性的深入理解，值得嘗試。

## 安全性評估

> [!warning] 安全性快速掃描
> 中等風險：該工具需要在具有管理權限的環境中運行，並可能會存取系統的敏感資料。使用時需謹慎，避免在未授權的系統上進行測試。
...
相關概念：[[安全漏洞]] · [[Linux 核心]] · [[漏洞利用]]

相關專案：[[Nightmare-Eclipse--RedSun|Nightmare-Eclipse/RedSun]] · [[hicode002--qualcomm_gbl_exploit_poc|hicode002/qualcomm_gbl_exploit_poc]] · [[opa334--darksword-kexploit|opa334/darksword-kexploit]]
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["安全漏洞","Linux 核心","漏洞利用"];
