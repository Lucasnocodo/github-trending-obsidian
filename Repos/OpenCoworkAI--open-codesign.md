...
use_case: "提供開源的 Claude 設計替代方案，讓用戶能夠快速從提示生成原型、簡報或 PDF。"
priority: high
ring: assess
...
star_history: "2026-04-23:1261,2026-04-23:1276,2026-04-24:1902,2026-04-24:1911,2026-04-25:2186,2026-04-25:2189,2026-04-26:2396,2026-04-27:2734,2026-04-28:3010,2026-04-29:3211,2026-04-30:3553,2026-05-01:3798,2026-05-02:3962,2026-05-03:4155,2026-05-04:4400"
tags:
  - github
  - "category/開發工具"
  - "lang/typescript"
...
aliases:
  ...
  - "提供開源的 Claude 設計替代方案，讓用戶能夠快速從提示生成原型、簡報或 PDF。"
---

# open-codesign

**4.4k** stars · **293** stars/天 · 建立 15 天前 · TypeScript · MIT
...
> **適合** 希望在本地環境中使用 AI 工具進行設計的獨立開發者或小型團隊。
> **一句話重點** Open CoDesign 提供了一個靈活的本地設計解決方案，讓用戶可以自由選擇 AI 模型，並且不受雲端限制。
...
> [!abstract] 核心創新
> Open CoDesign 是一個開源的本地運行設計工具，支持多種 AI 模型，並且允許用戶自帶 API 金鑰。

## 專案簡介

Open CoDesign 是一款開源的 AI 設計工具，旨在將用戶的提示轉換為精緻的原型、簡報或行銷資產，並且完全在本地運行。用戶只需提供一個 API 金鑰，便可快速上手，從提示生成設計，並支持多種 AI 模型（如 Claude、GPT、Gemini 等）。這個工具的賣點在於其本地運行的特性，避免了雲端依賴和訂閱鎖定，並且提供了即時的設計反饋和版本歷史保存。用戶可以通過 CLIProxyAPI 一鍵導入現有的 API 設定，並在設計過程中生成圖像資產，這些功能使得設計過程更加流暢和高效。技術上，Open CoDesign 使用 Electron 框架構建，並依賴 TypeScript 和 Vue，這使得其在桌面應用的性能和用戶體驗上有良好的表現。

相較於同類工具如 Claude Design 和 Figma AI，Open CoDesign 提供了更大的靈活性，支持多種模型和本地運行，並且不會將用戶的數據上傳到雲端。使用者在實際操作中可能會遇到一些 API 設定的問題，但這些問題通常可以通過其社群支持或文檔獲得解決。這個專案目前處於 v0.1.4 版本，雖然仍在開發中，但已經展現出穩定的功能和良好的使用體驗。對於需要快速生成設計原型而不想依賴雲端服務的團隊來說，這是一個非常合適的選擇。未來幾個月，預計將推出更多功能，如 Agentic Design，進一步提升設計的智能化水平。

**技術棧**：`TypeScript` · `JavaScript` · `Vue` · `Electron`

## 重點功能

- 多模型支持 — 支援 Claude、GPT、Gemini、Ollama 等多種 AI 模型。
- 本地運行 — 完全在用戶的設備上運行，無需雲端依賴。
- 一鍵導入 API 金鑰 — 用戶可以輕鬆導入現有的 Claude Code 或 Codex API 金鑰。
- 即時設計反饋 — 在生成過程中可見的工具調用和進度更新。
- AI 生成圖像 — 支持生成各類設計所需的圖像資產。
- 版本歷史保存 — 本地保存設計的每個版本，方便用戶隨時回溯。
- 多種輸出格式 — 支援導出為 HTML、PDF、PPTX 等多種格式。
...
> [!tip] 爆紅原因
> 建立 15 天就累積 4400 stars（293/天），forks 443（10.1%），顯示出強勁的增長潛力。這個專案由 OpenCoworkAI 團隊開發，專注於提供開源的設計解決方案，解決了許多設計師對於雲端依賴和訂閱模式的不滿。隨著 AI 設計工具的普及，開源且本地運行的選擇變得愈加重要，這使得 Open CoDesign 的出現恰逢其時。社群的活躍度和開發者的回應速度也為其增添了吸引力，特別是在 GitHub 上的討論和問題追蹤中，開發者對於用戶反饋的重視使得專案更具生命力。

## 適合誰使用

**目標受眾**：希望在本地環境中使用 AI 工具進行設計的獨立開發者或小型團隊。

> [!example] 使用場景
> - UI 設計師用它來快速生成行銷簡報，因為它支持多種模型並能即時反饋設計效果。
> - 產品經理用它來創建原型，因為它能夠在本地運行，避免了雲端數據安全的顧慮。
> - 開發者用它來生成 HTML 元件，因為它支持將設計直接轉換為可用的代碼，提升開發效率。

## 架構分析

Open CoDesign 採用 Electron 框架構建，這使得其能夠跨平台運行。應用的核心是基於 TypeScript 和 Vue，提供了現代化的用戶界面和良好的性能。資料流方面，使用者的提示會被發送到選定的 AI 模型進行處理，然後生成的設計結果會在本地顯示。

這種設計選擇使得應用能夠在不依賴雲端的情況下運行，並且能夠快速響應用戶的操作。選擇 Electron 使得開發者能夠利用網頁技術快速構建桌面應用，但也帶來了較大的應用體積和資源消耗。擴展性方面，由於使用了 SQLite 保存版本歷史，未來可以考慮增加更多的數據分析功能。

## 技術深入分析

Open CoDesign 的核心技術機制是將用戶的提示轉換為設計原型，這一過程依賴於多種 AI 模型的支持。使用者可以選擇不同的模型來生成不同風格的設計，這使得工具在設計上具有高度的靈活性。效能方面，因為是本地運行，對於網絡延遲的依賴較小，能夠快速響應用戶的操作。設計選擇上，使用 Electron 框架使得開發者能夠利用現有的網頁技術快速構建桌面應用，但這也可能導致應用的資源消耗較高。技術風險方面，隨著用戶數量的增加，可能會面臨性能瓶頸，特別是在處理大型設計項目時。整合方面，因為是基於現代的 JavaScript 生態系統，與其他主流框架（如 React、Vue）整合相對容易，並且在 CI/CD pipeline 中的使用也相對友好。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且包含範例，安裝過程大致順暢，但在 macOS 上可能需要手動處理安全設置。提供了良好的快速入門指導，文件目前僅有英文版本，中文版本在 README 中有鏈接。
...
> - 安裝過程中可能需要手動處理某些安全設置，特別是在 macOS 上。
> - 某些功能仍在開發中，可能會出現不穩定的情況。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [OpenAI/figma-ai](https://github.com/OpenAI/figma-ai) | Figma AI 是一個雲端工具，無法本地運行，且依賴於 OpenAI 的 API，限制了用戶的靈活性。 |
| [Anthropic/claude-design](https://github.com/Anthropic/claude-design) | Claude Design 僅支持 Claude 模型，而 Open CoDesign 支持多種模型，提供更大的靈活性和選擇。 |
| [Vercel/v0](https://github.com/Vercel/v0) | v0 是一個雲端解決方案，無法在本地運行，且對於數據隱私的控制較弱。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [OpenAI/figma-ai](https://github.com/OpenAI/figma-ai) | Figma AI 是一個雲端工具，無法本地運行，且依賴於 OpenAI 的 API，限制了用戶的靈活性。 | 如果你的團隊已經在使用 Figma 並希望利用其強大的協作功能。 | medium - 需要將設計流程從本地轉移到雲端，並適應新的工具生態。 |
| [Anthropic/claude-design](https://github.com/Anthropic/claude-design) | Claude Design 僅支持 Claude 模型，而 Open CoDesign 支持多種模型，提供更大的靈活性和選擇。 | 如果你的團隊已經在使用 Claude 並且不需要其他模型的支持。 | low - 只需適應 Claude Design 的使用方式。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **open-codesign** | **figma-ai** | **claude-design** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | Figma AI 是一個雲端工具，無法本地運行，且依賴於 OpenAI 的 API，限制了用戶的靈活性。 | Claude Design 僅支持 Claude 模型，而 Open CoDesign 支持多種模型，提供更大的靈活性和選擇。 |
> | 遷移成本 | - | medium - 需要將設計流程從本地轉移到雲端，並適應新的工具生態。 | low - 只需適應 Claude Design 的使用方式。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用 Figma 並希望利用其強大的協作功 | 如果你的團隊已經在使用 Claude 並且不需要其他模型的支 |
...
- **[HIGH]** 在 macOS 上安裝後可能需要手動處理安全設置，否則無法正常啟動。
  - 解法：右鍵點擊應用，選擇 '打開' 以繞過安全限制。
- [MEDIUM] 某些 API 設定可能會導致生成失敗，特別是在使用不兼容的模型時。
  - 解法：確保使用的模型與 Open CoDesign 兼容，並檢查 API 金鑰的正確性。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型設計團隊需要快速生成原型 | 非常適合 | 本地運行且支持多種模型，能夠快速響應設計需求。 |
| 大型企業需要穩定的設計工具 | 不適合 | 目前仍在開發中，穩定性和功能完整性尚未達到生產環境的要求。 |
| 獨立開發者希望自由選擇 AI 模型 | 非常適合 | 支持多種模型且無需雲端依賴，滿足個人需求。 |
...
> [!warning] 安全性快速掃描
> 低風險：本地運行的應用不需要高權限，且不會存取敏感資料，依賴鏈的信任程度較高。
...
> | 貢獻者 | Commits |
> | --- | --- |
> | [@hqhq1025](https://github.com/hqhq1025) | 502 |
> | [@Sun-sunshine06](https://github.com/Sun-sunshine06) | 46 |
...
> **活躍天數** 1 天 · **最新 commit** fix(desktop): quiet missing update channel checks
...
> | [#284](https://github.com/OpenCoworkAI/open-codesign/issues/284) | [Bug]: 导出功能存在多格式内容/样式丢失问题 | 0 | 2 |
