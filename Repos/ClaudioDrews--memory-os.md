...
use_case: "提供持久記憶的操作系統，讓 Hermes Agent 能夠更有效地記錄和回憶上下文。"
priority: high
ring: assess
...
aliases:
  ...
  - "提供持久記憶的操作系統，讓 Hermes Agent 能夠更有效地記錄和回憶上下文。"
---

# memory-os

**1.3k** stars · **28** stars/天 · 建立 45 天前 · Python · MIT
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Growing · **熱度** Growing (28 stars/day)
> **授權** MIT (商業友好) · **維護** Slow (最後推送 35 天前) · **貢獻者** 4+ 人 · **參與度** Low
> **適合** 需要在本地運行持久記憶系統的開發者，特別是對數據隱私有高要求的團隊。
> **一句話重點** 這個專案的最大優勢在於它提供了本地化的持久記憶解決方案，讓用戶能夠完全掌控自己的數據。
...
> **結論** 花 10 小時學習，20 小時整合，得到本地化的持久記憶系統，值得考慮。

> [!abstract] 核心創新
> 提供本地化的持久記憶基礎設施，無需雲端服務。

## 專案簡介

Memory OS 是一個針對 Hermes Agent 設計的七層記憶操作系統，旨在解決代理人記憶不持久的問題。它的核心機制是將記憶分層管理，並透過 Qdrant 和 Redis 提供本地化的記憶基礎設施，支持任何 LLM 提供者。使用者可以透過 API 直接與記憶系統互動，實現持久化記憶，並在需要時自動注入上下文。最關鍵的 Layer 7 確保了注入的記憶能被有效利用，避免了重複查詢和資源浪費。這樣的設計使得 Memory OS 能夠在本地運行，無需依賴雲端服務，避免了數據外洩的風險，並且不會被特定供應商鎖定。

技術上，Memory OS 結合了 Qdrant 的向量搜索和 Redis 的高效存儲，並且使用了多種算法來確保記憶的準確性和可靠性。與傳統的 Hermes 系統相比，Memory OS 提供了更強大的跨會話回憶、結構化事實存儲和自我整理的維基功能，這些都是標準 Hermes 所不具備的。實際使用中，這個系統能夠處理大量的對話數據，並且提供快速的查詢響應。這使得它特別適合需要長期記憶和上下文管理的應用場景，例如個人助理或專案管理工具。對於小型團隊或個人開發者來說，這是一個值得考慮的選擇，尤其是在需要保護敏感數據的情況下。

**技術棧**：`Python 3.10` · `Docker` · `Redis 5.0` · `Qdrant 1.17`

## 重點功能

- 七層記憶架構 — 提供結構化的記憶管理，確保記憶的捕獲、存儲和注入。
- 本地化記憶基礎設施 — 使用 Qdrant 和 Redis，無需雲端服務，保護數據隱私。
- 智能上下文注入 — 自動將相關記憶注入對話中，提升交互效率。
- 自我整理的維基 — 自動生成和維護知識庫，支持跨會話的記憶回憶。
- 高效的向量搜索 — 結合 Qdrant 的向量檢索，提供快速的查詢響應。
...
> [!tip] 爆紅原因
> 建立 45 天內累積 1271 stars（28/天），forks 119（9.4%），顯示出穩定的增長。作者 ClaudioDrews 之前有開發相關的 AI 工具，這次專案解決了現有記憶系統的雲端依賴問題，提供了本地化的解決方案。社群對於本地記憶基礎設施的需求日益增加，尤其是在隱私保護方面。這個專案的設計理念和功能填補了市場上對於私有記憶系統的空白，並且在 GitHub 上引起了廣泛的討論。forks/stars 比率為 9.4%，顯示出不少用戶在實際修改和使用這個專案。

## 適合誰使用

**目標受眾**：需要在本地運行持久記憶系統的開發者，特別是對數據隱私有高要求的團隊。

> [!example] 使用場景
> - AI 研究者用它來管理多個實驗的上下文和結果，因為能夠持久記憶和快速檢索歷史資料，提升了研究效率。
> - 自由職業者用它來記錄客戶需求和會議紀錄，因為能夠自動注入相關上下文，減少了重複查詢的時間。
> - 小型團隊用它來協作開發專案，因為提供了結構化的知識庫和跨會話的記憶回憶，增強了團隊的協作效率。

## 架構分析

Memory OS 採用七層架構，將記憶的捕獲、存儲和注入分開處理。每一層都有其特定的功能，確保記憶的有效性和準確性。Qdrant 負責向量搜索，而 Redis 則用於存儲和快速檢索。

這樣的設計使得系統可以在本地運行，避免了雲端依賴的風險。選擇本地化架構的代價是需要用戶自行管理基礎設施，但這樣也提供了更高的數據安全性。擴展性方面，系統的性能在處理大量數據時可能會受到影響，特別是在高並發查詢的情況下。

整體而言，這種架構適合需要高安全性和靈活性的應用場景。

## 技術深入分析

Memory OS 的核心技術機制是基於七層架構的記憶管理，這使得記憶的捕獲、存儲和注入過程能夠分開處理。使用 Qdrant 進行向量搜索，能夠快速檢索到相關的記憶，而 Redis 則提供高效的數據存儲和檢索功能。這樣的設計使得系統在本地運行時，能夠避免雲端依賴的風險。性能方面，系統能夠處理大量的對話數據，並且在查詢響應上表現良好，但在高並發情況下可能會出現瓶頸。選擇 Python 作為開發語言，能夠利用其豐富的生態系統，但也可能帶來一定的性能開銷。整體而言，這個系統在設計上考慮到了數據安全性和用戶靈活性，但在擴展性上可能需要進一步優化。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了詳細的安裝步驟和範例。安裝過程需要 Docker 環境，對於新手來說可能會有一定的學習曲線。文件中有針對快速入門的指南，能夠幫助用戶快速上手。文件目前僅提供英文版本，對於非英文使用者可能會有障礙。

## 優缺點分析

> [!success] 優點
> - 提供本地化的記憶解決方案，保護用戶數據隱私。
> - 智能上下文注入功能，提升交互的效率。
> - 結構化的知識庫，支持跨會話的記憶回憶。

> [!danger] 缺點
> - 安裝和配置過程相對複雜，需要 Docker 環境。
> - 對於大型數據集的性能可能不佳。
> - 目前僅支援特定的 LLM 提供者，靈活性較低。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供雲端記憶解決方案，但不支持本地化運行，對數據隱私的保護較弱。 |
| Zep | 雲端記憶系統，缺乏本地化功能，對於需要保護敏感數據的用戶不太適合。 |
| mem0 | 雲端優先的記憶系統，無法提供本地運行的靈活性和數據控制。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| Zep | 雲端記憶系統，無法提供本地化運行的靈活性。 | 如果你的團隊已經在使用雲端解決方案，並且不需要本地數據控制。 | medium，因為需要重新設計數據流和存儲方式。 |
| mem0 | 雲端優先的記憶系統，無法提供本地運行的靈活性。 | 如果你的應用場景不需要高安全性，並且可以接受雲端存儲。 | high，因為需要重構整個記憶管理系統。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **Zep** | **mem0** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 雲端記憶系統，無法提供本地化運行的靈活性。 | 雲端優先的記憶系統，無法提供本地運行的靈活性。 |
> | 遷移成本 | - | medium，因為需要重新設計數據流和存儲方式。 | high，因為需要重構整個記憶管理系統。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用雲端解決方案，並且不需要本地數據控制。 | 如果你的應用場景不需要高安全性，並且可以接受雲端存儲。 |
...
- [MEDIUM] 在高並發查詢下性能下降
  - 解法：考慮使用更強大的硬體或優化查詢邏輯
- [MEDIUM] 對於大型數據集的處理速度較慢
  - 解法：考慮分批處理數據或優化數據結構
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 能夠提供本地化的記憶解決方案，保護數據隱私。 |
| 大型企業的核心系統 | 不適合 | 目前性能和擴展性可能無法滿足需求。 |
| 個人助理應用 | 非常適合 | 能夠持久記憶用戶的需求和上下文。 |
| 需要高並發的即時通訊系統 | 普通 | 在高並發下性能可能會下降。 |
...
> [!tip] 投入 vs 回報
> 花 10 小時學習，20 小時整合，得到本地化的持久記憶系統，值得考慮。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：本地運行，不需要高權限，數據不會外洩。依賴的庫有一定的信任度，但仍需注意供應鏈風險。
...
**連結**：[文件](https://github.com/ClaudioDrews/memory-os/blob/main/README.md)
