...
**1.3k** stars · **28** stars/天 · 建立 46 天前 · Python · MIT
...
> **授權** MIT (商業友好) · **維護** Slow (最後推送 36 天前) · **貢獻者** 4+ 人 · **參與度** Low
> **適合** 需要在本地運行持久記憶系統的專業用戶，如研究人員和開發者。
> **一句話重點** Memory OS 不僅僅是插件，而是一個完整的記憶操作系統，能夠真正解決長期記憶的問題。
...
> **結論** 花 5 小時學習，8 小時整合，得到高效的本地記憶管理系統，值得投入。

> [!abstract] 核心創新
> 提供一個完整的七層記憶操作系統，並且具備智能上下文注入功能。

## 專案簡介

Memory OS 是一個針對 Hermes Agent 的七層記憶操作系統，旨在解決代理在多次對話中記憶遺失的問題。它的核心機制是將記憶分為七層，從工作區到結構化事實，並透過智能上下文注入來提升記憶的使用效率。用戶可以透過 `curl -sSL https://raw.githubusercontent.com/ClaudioDrews/memory-os/main/setup.sh | bash` 指令快速安裝整個系統，無需雲端訂閱，並且支持多種 LLM 提供者。這個系統的賣點在於其本地運行的記憶基礎設施，避免了數據外洩的風險。技術上，Memory OS 使用了 Qdrant 作為向量數據庫，並結合 Redis 和 ARQ 來管理記憶的存取，這樣的設計使得記憶的檢索和存儲都能高效運行。

與其他記憶解決方案相比，如 mem0 和 Zep，Memory OS 提供了更高的靈活性和私密性，因為它不依賴雲端服務。實際使用中，這個系統能夠支持大量的對話記錄和結構化事實，並且具備自我整理的維基功能，讓用戶能夠隨時檢索過去的對話和決策。這使得它非常適合需要長期記憶的專業用戶，如研究人員或開發者。雖然目前仍在 beta 階段，但其社群活躍，問題解決率達 67%。如果你需要一個能夠在本地運行的記憶系統，Memory OS 是一個值得考慮的選擇。
...
- 智能上下文注入 — 自動將相關記憶注入對話中，提升效率。
- 本地運行 — 完全在用戶機器上運行，無需雲端服務，保障隱私。
- 支持多種 LLM 提供者 — 包括 OpenRouter、OpenAI、Anthropic 等。
- 自我整理的維基功能 — 自動整理和維護知識庫，便於檢索。
...
> [!tip] 爆紅原因
> 建立 46 天內累積 1276 stars（28/天），forks 119（9.3%），顯示出穩定增長。作者 ClaudioDrews 之前在記憶管理領域有豐富經驗，這個專案解決了現有記憶解決方案的雲端依賴和隱私問題。沒有其他本地記憶解決方案能提供相同的靈活性和功能，特別是在不需要雲端訂閱的情況下。社群的活躍度和問題解決率也顯示出其潛在的實用性。

## 適合誰使用

**目標受眾**：需要在本地運行持久記憶系統的專業用戶，如研究人員和開發者。

> [!example] 使用場景
> - 研究人員用它來記錄和回顧長期研究項目的進展，因為它能夠持久保存每次會議的上下文和決策。
> - 開發者用它來管理多個專案的歷史和結構化事實，因為它能夠自動整理和檢索關鍵資訊，節省時間。
> - 自由職業者用它來跟蹤客戶需求和會議記錄，因為它能夠提供智能的上下文注入，避免重複溝通。

## 架構分析

Memory OS 採用七層架構，從工作區到向量數據庫，確保記憶的捕捉、存儲和注入。每層都有其特定功能，並透過 Qdrant 和 Redis 進行高效的數據存取。這樣的設計使得系統能夠在本地運行，避免了雲端依賴的風險。選擇本地存儲的代價是需要用戶管理自己的數據和基礎設施，但這也帶來了更高的隱私保障。系統的擴展性受到硬體限制，尤其是在處理大量數據時，可能需要更高的計算資源。

## 技術深入分析

Memory OS 的核心在於其七層架構，這使得記憶的捕捉、存儲和注入變得高效且有序。每一層都有其特定的功能，例如工作區層負責基本的記憶存儲，而向量數據庫層則使用 Qdrant 進行高效的向量檢索。這樣的設計不僅提升了記憶的效率，也減少了不必要的資源浪費。選擇本地運行的架構意味著用戶需要自行管理數據和基礎設施，這在隱私上提供了保障，但也增加了維護的複雜度。系統的效能在處理大量數據時可能會受到硬體限制，特別是在高並發的情況下。對於依賴外部 API 的部分，使用者需要注意可能的連接問題和延遲。整體而言，Memory OS 的設計考量了用戶的需求，並在功能和隱私之間取得了良好的平衡。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，包含安裝指令和使用範例。安裝過程相對順暢，但需要 Docker 環境，對新手可能有挑戰。文件目前僅提供英文版本，缺乏多語言支持。
...
| 工具 | 差異 |
| --- | --- |
| mem0 | mem0 是雲端解決方案，無法提供本地運行的隱私保障，適合需要即時存取的用戶。 |
| Zep | Zep 主要針對企業用戶，缺乏個人用戶所需的靈活性和私密性。 |
| Letta | Letta 也依賴雲端服務，無法提供本地記憶管理的選項。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| mem0 | mem0 是基於雲端的記憶解決方案，無法提供本地運行的選項。 | 如果需要即時存取和雲端協作的功能，mem0 可能更合適。 | medium，因為需要將數據從本地轉移到雲端。 |
| Zep | Zep 主要針對企業用戶，提供雲端記憶管理，但缺乏個人用戶所需的靈活性。 | 如果你的團隊已經在使用 Zep 進行企業級記憶管理，則選擇 Zep 可能更方便。 | high，因為需要重新設計整個記憶管理流程。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **mem0** | **Zep** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | mem0 是基於雲端的記憶解決方案，無法提供本地運行的選項。 | Zep 主要針對企業用戶，提供雲端記憶管理，但缺乏個人用戶所需的靈活性。 |
> | 遷移成本 | - | medium，因為需要將數據從本地轉移到雲端。 | high，因為需要重新設計整個記憶管理流程。 |
> | 適用場景 | 主要場景 | 如果需要即時存取和雲端協作的功能，mem0 可能更合適。 | 如果你的團隊已經在使用 Zep 進行企業級記憶管理，則選擇  |
...
- **[HIGH]** 在某些環境下，Docker 啟動可能會遇到權限問題。
  - 解法：確保 Docker 服務以正確的用戶權限運行。
- [MEDIUM] 在高並發情況下，記憶檢索速度可能會變慢。
  - 解法：考慮升級硬體配置以提高效能。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 本地運行的特性和靈活性非常符合小型團隊的需求。 |
| 需要長期記憶的研究項目 | 適合 | 能夠持久保存和檢索關鍵資訊，提升研究效率。 |
| 大型企業的記憶管理系統 | 不適合 | 目前仍在 beta 階段，穩定性不足。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，8 小時整合，得到高效的本地記憶管理系統，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：本地運行，不需要高權限，且不存取敏感資料。依賴的第三方庫需定期檢查安全性。
