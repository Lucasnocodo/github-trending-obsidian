...
use_case: "為 Hermes Agent 提供持久記憶的操作系統，實現智能上下文注入和自動化維基管理。"
priority: high
ring: assess
...
aliases:
  ...
  - "為 Hermes Agent 提供持久記憶的操作系統，實現智能上下文注入和自動化維基管理。"
---

# memory-os

**1.3k** stars · **23** stars/天 · 建立 56 天前 · Python · MIT
...
> **授權** MIT (商業友好) · **維護** Slow (最後推送 46 天前) · **貢獻者** 4+ 人 · **參與度** Low
> **適合** 需要在本地運行記憶系統以管理長期對話的 AI 研究者和開發者。
> **一句話重點** 這個專案不僅提供記憶功能，更是設計成一個完整的操作系統，讓 AI 代理能夠真正記住並利用過去的對話。
...
> **結論** 花 5 小時學習，10 小時整合，得到高效的記憶管理，值得投入。

> [!abstract] 核心創新
> 提供了一個完整的本地記憶操作系統，具備七層架構和智能上下文注入功能。

## 專案簡介

Memory OS 是一個為 Hermes Agent 設計的七層記憶操作系統，旨在解決 AI 代理在長期對話中遺忘上下文的問題。它的核心流程是將用戶的記憶、會話歷史和結構化事實注入到代理中，並通過 Qdrant 和 Redis 提供高效的檢索和存儲。用戶只需運行 `curl -sSL https://raw.githubusercontent.com/ClaudioDrews/memory-os/main/setup.sh | bash`，即可一鍵安裝整個系統，無需雲端訂閱，確保數據的私密性和靈活性。這個系統的賣點在於其智能的上下文注入和自我管理的維基，能夠在每次會話中提供準確的背景信息，讓代理如同一位長期合作的同事。技術上，Memory OS 使用了 Python 和 Docker，並依賴於 Qdrant 進行向量搜索和 Redis 進行數據存儲，這樣的設計使得系統能夠在本地運行，避免了雲端依賴的問題。

與其他記憶解決方案相比，Memory OS 提供了更高的靈活性和私密性，特別是在需要本地運行的場景下。使用者可以在任何 LLM 提供者上運行此系統，並且支持多種數據格式和檢索方式，這使得它在資料處理上具有優越性。社群活躍度良好，開發者對於問題的回應速度也相對較快，這對於使用者來說是一個加分項。整體來看，這是一個成熟且值得信賴的記憶系統，適合需要長期記憶和上下文管理的專業用戶。

**技術棧**：`Python 3.11` · `Docker` · `Redis` · `Qdrant`

## 重點功能

- 七層記憶架構 — 提供從工作空間到向量數據庫的完整記憶管理。
- 智能上下文注入 — 自動將相關記憶注入到每次會話中，提升代理的連貫性。
- 自動化維基管理 — 持續更新和整理知識，減少手動維護的需求。
- 本地運行 — 完全在用戶本地環境中運行，無需雲端依賴。
- 多種 LLM 支持 — 兼容 OpenRouter、OpenAI、Anthropic、Ollama 等多個提供者。
...
> [!tip] 爆紅原因
> 建立 56 天內累積 1299 stars（每日約 23 stars），forks 數量為 121（9.3%），顯示出良好的社群參與度。開發者 ClaudioDrews 之前在 AI 記憶領域有過多次嘗試，這個專案解決了現有記憶系統的雲端依賴和數據私密性問題，提供了一個本地運行的解決方案。該專案的推出引發了社群的廣泛討論，特別是在 StackMap 上的曝光，進一步提高了其知名度。技術上，隨著本地運行能力的提升和對多種 LLM 提供者的支持，這個工具的可行性大幅提升。forks/stars 比率為 9.3%，顯示出許多人在實際使用和修改此工具。

## 適合誰使用

**目標受眾**：需要在本地運行記憶系統以管理長期對話的 AI 研究者和開發者。

> [!example] 使用場景
> - AI 研究員用它來管理長期項目的記憶，因為它能夠自動記錄和回顧過去的決策和討論，避免重複工作。
> - 產品經理用它來追蹤多個產品的開發進度，因為它能夠提供每個會話的上下文，讓團隊更有效地協作。
> - 開發者用它來整合不同來源的技術文檔，因為它的自動化維基功能能夠持續更新和整理資訊，減少手動維護的負擔。

## 架構分析

Memory OS 的架構由七層組成，每一層都有其特定的功能和責任。第一層處理工作空間的基本記憶，第二層負責會話記錄，第三層則管理結構化事實。這樣的設計使得每一層可以專注於特定的記憶類型，並通過 Qdrant 和 Redis 進行高效的數據存取。

選擇這種分層架構的原因在於能夠清晰地管理不同類型的記憶，並在需要時進行快速檢索。這樣的架構雖然增加了系統的複雜性，但卻能有效提升記憶的準確性和效率。整體架構的擴展性良好，能夠根據用戶需求進行調整和擴展，未來可考慮加入更多的記憶層或功能模組。

## 技術深入分析

Memory OS 的核心技術機制在於其七層記憶架構，這使得系統能夠有效地管理和檢索不同類型的記憶。每一層都專注於特定的功能，例如第一層處理工作空間記憶，第二層則專注於會話記錄，這樣的分層設計使得系統在處理複雜的記憶時能夠保持高效。效能方面，系統的設計考慮到了實際使用中的需求，能夠在本地環境中快速響應用戶的請求，並且在數據量增大時仍能保持良好的性能。選擇 Python 和 Docker 作為技術棧的原因在於它們的普遍性和靈活性，這使得開發者能夠輕鬆地進行擴展和維護。依賴樹的複雜度相對較低，主要依賴於幾個核心庫，這降低了維護的難度。技術風險方面，隨著數據量的增長，系統可能會面臨性能瓶頸，特別是在記憶檢索的過程中。整合方面，這個工具能夠與多種 LLM 提供者無縫對接，並且在 CI/CD pipeline 中的友好度也相對較高，這使得團隊能夠快速上手並進行實際應用。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且詳細，提供了安裝和使用的具體指導。安裝過程相對順暢，但需要用戶具備 Docker 的基本知識。文件中包含了良好的入門指南，幫助新手快速上手。文件目前僅提供英文版本，對於非英語使用者可能會有一定的障礙。
...
> - 在大型數據集上可能會面臨性能瓶頸，特別是在記憶檢索時。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| mem0 | mem0 是一個雲端記憶解決方案，無法提供本地運行的靈活性，適合需要快速部署的場景，但不適合對數據隱私有高要求的用戶。 |
| Zep | Zep 提供了類似的記憶功能，但主要依賴雲端基礎設施，缺乏本地存儲的選項，對於需要私密性的用戶來說不夠理想。 |
| Letta | Letta 也提供記憶功能，但缺乏智能上下文注入和自動維基管理，對於需要長期記憶的專業用戶來說功能較為單薄。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| mem0 | mem0 是一個雲端記憶解決方案，無法提供本地運行的靈活性。 | 如果你的團隊需要快速部署且不在意數據隱私，mem0 可能是更好的選擇。 | medium，因為需要將數據從本地轉移到雲端。 |
| Zep | Zep 主要依賴雲端基礎設施，缺乏本地存儲的選項。 | 如果你的團隊已經在使用 Zep 的生態系統，則選擇 Zep 會更方便。 | high，因為需要重新設計數據流和存儲方式。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **mem0** | **Zep** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | mem0 是一個雲端記憶解決方案，無法提供本地運行的靈活性。 | Zep 主要依賴雲端基礎設施，缺乏本地存儲的選項。 |
> | 遷移成本 | - | medium，因為需要將數據從本地轉移到雲端。 | high，因為需要重新設計數據流和存儲方式。 |
...
- **[HIGH]** 在某些環境下，Docker 啟動可能會遇到權限問題，導致服務無法正常運行。
  - 解法：確保 Docker 具有足夠的權限，或使用 sudo 啟動。
- [MEDIUM] 在大型數據集上進行檢索時可能會出現性能下降。
  - 解法：定期清理不必要的數據，並優化查詢。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 能夠有效管理長期記憶，提升團隊協作效率。 |
| 大型企業的 AI 代理系統 | 普通 | 在數據量大時可能會面臨性能瓶頸。 |
| 個人開發者的 AI 助手 | 非常適合 | 本地運行的特性能夠保護用戶數據隱私。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，10 小時整合，得到高效的記憶管理，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具在本地運行，不需要高權限，且不會存取敏感資料，依賴鏈的信任程度良好，適合在 CI/CD 中使用。
