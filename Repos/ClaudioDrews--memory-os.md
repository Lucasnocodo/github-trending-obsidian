...
**1.2k** stars · **43** stars/天 · 建立 28 天前 · Python · MIT
...
> **授權** MIT (商業友好) · **維護** Moderate (最後推送 18 天前) · **貢獻者** 4+ 人 · **參與度** Low
> **適合** 需要在本地運行持久記憶系統以支持 Hermes Agent 的開發者。
> **一句話重點** Memory OS 讓 Hermes Agent 擁有持久記憶，真正成為長期合作的夥伴。
...
> **結論** 花 5 小時學、10 小時整合，得到靈活的本地化記憶系統，值得投入。

> [!abstract] 核心創新
> Memory OS 提供了一個完整的七層記憶架構，並引入了 Ground Truth Hierarchy 確保記憶的有效使用。

## 專案簡介

Memory OS 是一個針對 Hermes Agent 設計的七層記憶操作系統，旨在解決 AI 在長期合作中忘記上下文的問題。使用者可以透過這個系統將過去的對話、決策和結構化事實持久化，並在需要時自動注入相關上下文。核心機制包括將記憶分為七層，從工作區到向量資料庫，並透過 Qdrant 實現高效的語義搜索。使用者只需執行 `curl -sSL https://raw.githubusercontent.com/ClaudioDrews/memory-os/main/setup.sh | bash` 即可快速安裝整個堆疊，無需雲端訂閱，支持多種 LLM 提供者。這個系統的賣點在於其「Ground Truth Hierarchy」，確保注入的記憶被正確使用，避免重複查詢造成的資源浪費。

技術上，Memory OS 結合了 Redis、Qdrant 和 ARQ Worker，並使用 SQLite 作為資料儲存，提供了高效的資料檢索和管理。與其他記憶解決方案相比，Memory OS 提供了真正的本地化記憶基礎設施，無需依賴雲端，並且支持多種 LLM 提供者，這使得其在隱私和靈活性上具有優勢。使用者可能會遇到的挑戰包括需要適當配置 Docker 環境，以及確保與 Hermes Agent 的兼容性。這個專案目前處於活躍開發階段，社群也在持續貢獻改善，未來可能會進一步擴展功能和優化性能。

**技術棧**：`Python 3.11` · `Docker` · `Redis` · `Qdrant` · `SQLite`

## 重點功能

- 七層記憶架構 — 包含工作區、會話、結構化事實、跨會話記憶等層次，確保記憶的完整性和可用性。
- Ground Truth Hierarchy — 確保注入的記憶被正確使用，避免重複查詢造成的資源浪費。
- 本地化記憶基礎設施 — 完全在用戶本地運行，無需雲端訂閱，保護用戶隱私。
- 支持多種 LLM 提供者 — 包括 OpenRouter、OpenAI、Anthropic 等，靈活性高。
- 自動化維護和去重 — 提供智能的記憶清理和去重機制，保持資料庫的整潔。
...
> [!tip] 爆紅原因
> 建立 28 天內累積 1205 stars（43/天），forks 112（9.3%），顯示出穩定的增長。作者 ClaudioDrews 在 AI 記憶領域有豐富經驗，這個專案解決了許多現有記憶解決方案無法提供本地化記憶的痛點。此專案的出現正好滿足了對隱私和靈活性需求日益增長的市場。社群的活躍度也反映在熱門 Issues 上，顯示出使用者對於功能擴展和安裝支持的需求。這些因素共同推動了專案的快速成長。

## 適合誰使用

**目標受眾**：需要在本地運行持久記憶系統以支持 Hermes Agent 的開發者。

> [!example] 使用場景
> - AI 研究員用它來持久記錄實驗結果和參數，因為這樣能夠隨時回顧過去的決策和調整。
> - 開發者用它來管理多個專案的上下文，因為能夠快速檢索相關資訊，提升工作效率。
> - 產品經理用它來追蹤會議紀錄和決策過程，因為這樣能夠避免重複討論，節省時間。

## 架構分析

Memory OS 的架構由七層組成，從工作區到向量資料庫，設計上旨在提供全面的記憶管理。每一層都有其特定的功能，例如工作區層負責基本的記憶存儲，而向量資料庫層則使用 Qdrant 進行高效的語義檢索。這種分層設計使得系統能夠靈活地處理不同類型的記憶需求。選擇使用 Docker 來容器化服務，便於部署和管理，但這也增加了對使用者環境的依賴。整體架構的可擴展性良好，但在高負載情況下，Redis 和 Qdrant 的性能可能成為瓶頸。

## 技術深入分析

Memory OS 的核心技術機制是其七層架構，這種設計使得記憶的捕捉、儲存和注入變得高效且有序。每一層都有其特定的功能，從基本的工作區到複雜的向量資料庫，這樣的分層設計使得系統能夠靈活地應對不同的記憶需求。效能方面，使用 Qdrant 進行語義檢索，能夠快速處理大量資料，但在高負載下可能會出現性能瓶頸。選擇 Docker 作為容器化解決方案，雖然方便部署，但也增加了對使用者環境的依賴。設計上，Memory OS 旨在提供一個無需雲端的本地化解決方案，這在隱私和數據安全上具有明顯優勢。然而，這也意味著使用者需要自行管理基礎設施，這對於一些用戶來說可能是一個挑戰。整體而言，這個專案在技術上是成熟的，但仍需注意在高負載情況下的性能表現。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝指引和使用範例，讓新手能夠快速上手。安裝過程相對順暢，但對於不熟悉 Docker 的用戶可能會有一些挑戰。整體來說，文件質量良好，能夠幫助使用者在短時間內完成設置。
...
> - 本地化運行，無需依賴雲端，保護用戶隱私。
> - 支持多種 LLM 提供者，靈活性高。
> - 七層記憶架構提供全面的記憶管理，適合複雜的應用場景。

> [!danger] 缺點
> - 需要 Docker 環境，對於不熟悉的用戶有學習曲線。
> - 安裝和配置過程可能較為繁瑣，特別是在不同環境下。
> - 對於小型專案來說，可能顯得過於複雜。

> [!warning] 注意事項
> - 需要 Docker 環境運行，對於不熟悉 Docker 的用戶可能有學習曲線。
> - 目前僅支持 Python 3.11+，舊版本不兼容。
> - 在某些情況下，可能需要手動配置環境變數以確保正確運行。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| mem0 | mem0 是一個雲端記憶解決方案，而 Memory OS 提供本地化的記憶基礎設施，適合對隱私有高要求的用戶。 |
| Zep | Zep 主要針對雲端環境設計，無法提供本地運行的選項，對於需要本地部署的用戶不合適。 |
| Letta | Letta 提供的記憶功能較為淺顯，無法實現 Memory OS 的多層次記憶管理和智能注入。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| mem0 | mem0 是一個雲端記憶解決方案，而 Memory OS 提供本地化的記憶基礎設施，適合對隱私有高要求的用戶。 | 如果你的團隊已經在使用雲端解決方案，且不需要本地化記憶。 | medium，因為需要重新設計記憶管理流程。 |
| Zep | Zep 主要針對雲端環境設計，無法提供本地運行的選項，對於需要本地部署的用戶不合適。 | 如果你的應用場景完全依賴雲端，且不需要本地化記憶。 | high，因為需要大幅度調整架構。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **mem0** | **Zep** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | mem0 是一個雲端記憶解決方案，而 Memory OS 提供本地化的記憶基礎設施，適合對隱私有高要求的用戶。 | Zep 主要針對雲端環境設計，無法提供本地運行的選項，對於需要本地部署的用戶不合適。 |
> | 遷移成本 | - | medium，因為需要重新設計記憶管理流程。 | high，因為需要大幅度調整架構。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用雲端解決方案，且不需要本地化記憶。 | 如果你的應用場景完全依賴雲端，且不需要本地化記憶。 |
...
- [MEDIUM] 在某些環境下，Docker 容器啟動時間較長，可能導致使用者誤以為安裝失敗。
  - 解法：耐心等待，並檢查 Docker 狀態。
- **[HIGH]** 某些配置選項可能需要手動調整，特別是在不同的作業系統上。
  - 解法：參考官方文檔進行配置。
- **[HIGH]** 在高負載情況下，Redis 和 Qdrant 的性能可能會下降。
  - 解法：考慮增加硬體資源或優化查詢。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 提供本地化記憶，能夠有效管理團隊的知識和決策。 |
| 大型企業的 AI 服務 | 普通 | 雖然功能強大，但可能需要更多的資源來支持高負載。 |
| 個人開發者的 AI 專案 | 非常適合 | 能夠在本地運行，無需依賴雲端，保護個人隱私。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學、10 小時整合，得到靈活的本地化記憶系統，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：本工具在本地運行，無需高權限，並不存取敏感資料。依賴鏈中無已知的供應鏈風險，適合在 CI/CD 中安全使用。

## 生態系整合

> [!abstract] 如何融入你的工具鏈

Memory OS 可以與多種 LLM 提供者搭配使用，並且能夠輕鬆整合進現有的開發流程中。在一個使用 Docker 和 Python 的專案中，你可以將 Memory OS 作為本地記憶解決方案，具體做法是通過 Docker Compose 啟動所有服務。此工具支援與 GitHub Actions 等 CI 工具的整合，並且可以在 VS Code 等 IDE 中進行開發。整合過程中，最常見的問題是 Docker 配置不當，這可能需要用戶進行額外的調整。

## 歷史脈絡

> [!info] 這個工具為什麼現在出現？

在 Memory OS 出現之前，許多記憶解決方案都依賴雲端，這使得用戶的數據面臨隱私風險。傳統的記憶系統往往無法提供持久的上下文，導致 AI 在長期使用中無法有效記憶。隨著對數據隱私和本地化解決方案需求的增加，Memory OS 應運而生，代表了在 AI 記憶領域的一次重要進步。

未來，這類工具可能會進一步發展，提供更智能的記憶管理和上下文注入功能。
