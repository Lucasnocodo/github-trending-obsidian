...
star_history: "2026-06-04:756,2026-06-04:757,2026-06-05:828,2026-06-05:828,2026-06-06:882,2026-06-06:882,2026-06-07:951,2026-06-07:951,2026-06-08:1011,2026-06-09:1037,2026-06-10:1050,2026-06-11:1069,2026-06-12:1086,2026-06-13:1093,2026-06-14:1098,2026-06-15:1104"
tags:
  - github
  - "category/ai_ml"
  - "lang/python"
...
**1.1k** stars · **79** stars/天 · 建立 14 天前 · Python · MIT
...
> **適合** 需要在本地運行 AI 記憶系統以保護數據隱私的開發者和研究人員。
> **一句話重點** Memory OS 的創新在於其七層架構和真實性層級，這使得 AI 能夠有效地記住並使用過去的對話和決策。
...
> **結論** 花 5 小時學習，10 小時整合，得到一個靈活且私密的 AI 記憶系統，值得投入。

> [!abstract] 核心創新
> Memory OS 提供了一個完整的七層記憶架構，並引入了真實性層級來確保注入的記憶被有效使用。

## 專案簡介

Memory OS 是一個七層記憶操作系統，專為 Hermes Agent 設計，旨在解決 AI 忘記過去對話和決策的問題。該系統的核心在於持久記憶，運行在本地，並支持多種 LLM 提供者。用戶可以透過 `curl -sSL https://raw.githubusercontent.com/ClaudioDrews/memory-os/main/setup.sh | bash` 一鍵安裝整個堆疊，包括 Docker 服務和 SQLite 數據庫。系統的七層架構從工作區、會話記憶到結構化事實，並最終透過第七層的「真實性層級」確保注入的記憶被有效使用。這樣的設計避免了重複的上下文檢索，提升了令牌使用效率。技術上，Memory OS 使用 Qdrant 作為向量數據庫，並結合 Redis 和 ARQ Worker，這樣的架構使得整體系統能夠在本地運行，避免了雲端依賴和數據洩露的風險。與其他記憶解決方案相比，Memory OS 提供了更高的私密性和靈活性，特別適合需要長期記憶的 AI 應用。

**技術棧**：`Python 3.11+` · `Docker` · `Redis` · `Qdrant`

## 重點功能

- 七層記憶架構 — 包括工作區、會話、結構化事實等，確保全面的記憶管理。
- 本地運行 — 完全在用戶的機器上運行，無需雲端依賴，保護用戶數據隱私。
- 多供應商支持 — 兼容多種 LLM 提供者，如 OpenAI、Anthropic 和本地模型。
- 智能上下文注入 — 自動將相關記憶注入到對話中，提升交互效率。
- 真實性層級 — 確保注入的記憶被有效使用，避免重複檢索浪費資源。
...
> [!tip] 爆紅原因
> 建立 14 天內累積 1104 stars（79/天），forks 103（9.3%），顯示出穩定的增長趨勢。作者 Claudio Drews 之前在 AI 記憶和代理領域有豐富的經驗，這個專案解決了現有記憶解決方案的雲端依賴和數據隱私問題。這個工具的出現正好滿足了對於本地記憶解決方案的需求，並且在社群中引起了討論，尤其是關於性能基準的問題。隨著越來越多的用戶尋求更私密的 AI 解決方案，Memory OS 的需求也隨之上升。

## 適合誰使用

**目標受眾**：需要在本地運行 AI 記憶系統以保護數據隱私的開發者和研究人員。

> [!example] 使用場景
> - AI 研究員用它來持久記錄實驗結果和參考資料，因為這樣可以避免重複輸入並提高工作效率。
> - 開發者用它來管理多個專案的上下文和決策，因為能夠快速檢索過去的對話和結論，減少了尋找資料的時間。
> - 產品經理用它來追蹤會議記錄和決策過程，因為這樣可以確保團隊成員在每次會議中都能獲得正確的背景資訊。

## 架構分析

Memory OS 的架構由七層組成，從工作區到向量數據庫，並最終透過真實性層級確保注入的記憶被有效使用。這樣的設計使得系統能夠在本地運行，避免了雲端依賴。每一層都有其特定的功能，如使用 Qdrant 作為向量數據庫，並結合 Redis 進行數據管理。這種架構的選擇使得系統在性能和靈活性上達到平衡，但也可能在資源有限的情況下出現瓶頸。整體而言，這種設計使得用戶能夠在本地環境中構建一個強大的記憶系統，並且能夠隨著需求的增長進行擴展。

## 技術深入分析

Memory OS 的核心技術機制在於其七層記憶架構，這包括從工作區到向量數據庫的多層設計。每一層都有其特定的功能，如使用 Qdrant 作為向量數據庫，並結合 Redis 進行數據管理。這樣的設計不僅提高了記憶的效率，還能確保用戶的數據隱私。效能上，系統能夠處理中小型數據集，並在本地環境中運行，避免了雲端依賴的風險。選擇 Python 和 Docker 作為技術棧的原因在於其廣泛的社群支持和靈活性，但這也意味著需要一定的技術背景來設置和運行。設計上的取捨在於性能和靈活性之間的平衡，尤其是在資源有限的情況下，可能會出現性能瓶頸。整合方面，Memory OS 能夠與多種 LLM 提供者兼容，這使得它在不同的應用場景中都能發揮作用。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且包含安裝指令，安裝過程相對順暢，沒有明顯的坑。提供了一鍵安裝的腳本，對於新手來說非常友好。文件目前僅提供英文版本，可能對非英語使用者造成一定的障礙。

## 優缺點分析

> [!success] 優點
> - 提供完整的本地記憶解決方案，無需雲端依賴。
> - 智能上下文注入提升了交互的效率。
> - 七層架構確保了記憶的全面性和可靠性。
...
> - 需要一定的技術背景來設置和運行。
> - 對於大型數據集的性能可能會受到限制。
> - 尚在活躍開發中，可能會有不穩定的 API 變更。

> [!warning] 注意事項
> - 目前僅支持 Python 3.11+。
> - 需要 Docker 環境來運行 Qdrant 和 Redis。
> - 對於大型數據集的性能可能會受到限制，特別是在資源有限的機器上。
> - 尚處於活躍開發中，可能會有不穩定的 API 變更。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| mem0 | mem0 是一個雲端記憶解決方案，而 Memory OS 提供本地運行的選項，保護用戶數據隱私。 |
| Zep | Zep 主要針對雲端環境，而 Memory OS 則專注於本地部署，提供更高的靈活性和控制權。 |
| Letta | Letta 也依賴雲端服務，而 Memory OS 提供無需訂閱的本地解決方案，適合對數據隱私有高要求的用戶。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| mem0 | mem0 是一個雲端記憶解決方案，而 Memory OS 提供本地運行的選項，保護用戶數據隱私。 | 如果你的團隊已經在使用雲端解決方案且不需要本地部署的話，mem0 可能更適合。 | medium - 需要將數據從本地遷移到雲端，並重新配置環境。 |
| Zep | Zep 主要針對雲端環境，而 Memory OS 則專注於本地部署，提供更高的靈活性和控制權。 | 如果你的應用場景需要雲端擴展，Zep 可能更合適。 | high - 需要重新設計架構以適應雲端環境。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **mem0** | **Zep** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | mem0 是一個雲端記憶解決方案，而 Memory OS 提供本地運行的選項，保護用戶數據隱私。 | Zep 主要針對雲端環境，而 Memory OS 則專注於本地部署，提供更高的靈活性和控制權。 |
> | 遷移成本 | - | medium - 需要將數據從本地遷移到雲端，並重新配置環境。 | high - 需要重新設計架構以適應雲端環境。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用雲端解決方案且不需要本地部署的話，me | 如果你的應用場景需要雲端擴展，Zep 可能更合適。 |
...
> [!tip] 採用建議
> 適合個人或小型團隊的實驗性項目，但不建議在生產環境中使用。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在某些環境中，Docker 啟動可能會遇到性能問題
  - 解法：確保 Docker 配置正確，並在高性能機器上運行。
- [MEDIUM] 在使用過程中可能會遇到記憶檢索不準確的情況
  - 解法：定期檢查和更新記憶庫的內容。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 AI 系統 | 非常適合 | 本地運行的記憶系統能夠保護敏感數據，並且靈活性高。 |
| 大型企業的雲端 AI 解決方案 | 不適合 | 需要雲端擴展和高可用性，Memory OS 更適合本地部署。 |
| AI 研究機構需要長期記憶的實驗 | 適合 | 能夠持久記錄實驗過程和結果，提升研究效率。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，10 小時整合，得到一個靈活且私密的 AI 記憶系統，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具在本地運行，不需要高權限，並且不存取敏感資料。依賴鏈的信任程度高，無已知的供應鏈風險。
