...
star_history: "2026-06-04:756,2026-06-04:757,2026-06-05:828,2026-06-05:828,2026-06-06:882,2026-06-06:882,2026-06-07:951,2026-06-07:951,2026-06-08:1011,2026-06-09:1037,2026-06-10:1050,2026-06-11:1069,2026-06-12:1086,2026-06-13:1093"
tags:
  - github
  - "category/ai_ml"
  - "lang/python"
...
**1.1k** stars · **91** stars/天 · 建立 12 天前 · Python · MIT
...
> **適合** 需要在本地運行持久記憶系統的開發者和團隊。
> **一句話重點** Memory OS 的設計不僅解決了記憶丟失的問題，還提供了本地運行的靈活性，讓用戶能夠完全掌控自己的數據。
...
> **結論** 花 5 小時學習，10 小時整合，得到一個靈活的本地記憶系統，值得一試。

> [!abstract] 核心創新
> Memory OS 提供了一個完整的七層記憶架構，並且具備地面真相層級，確保注入的記憶能被有效利用。

## 專案簡介

Memory OS 是一個七層的記憶操作系統，專為 Hermes Agent 設計，旨在提供持久的記憶基礎設施。用戶可以透過這個系統將記憶捕獲、存儲並注入到代理中，進而提升其長期合作的能力。核心機制包括使用 Qdrant 作為向量數據庫，並結合 Redis 進行記憶的管理，最終實現智能的上下文注入。使用者只需執行一條指令 `curl -sSL https://raw.githubusercontent.com/ClaudioDrews/memory-os/main/setup.sh | bash` 即可完成整個安裝，這大大簡化了部署過程。這個系統的賣點在於它的本地運行能力，避免了雲端記憶的依賴，並且支持多種 LLM 提供者，讓用戶可以自由選擇。

技術上，Memory OS 採用了 SQLite 和 Qdrant 的組合，並且在結構化事實和信任評分上有獨特的設計，這使得它在記憶的準確性和效率上優於其他方案。與傳統的 Hermes 相比，Memory OS 提供了更豐富的功能，如全文本搜索和自動化的知識管道。這些功能的實現依賴於其七層架構，尤其是第七層的地面真相層級，確保了注入的記憶能夠被有效利用。使用者在實際操作中可能會遇到的問題包括與非 Hermes 環境的整合挑戰，以及性能基準的測試需求。整體而言，這個專案的成熟度尚在 beta 階段，但已經具備了實用性，適合需要本地記憶解決方案的開發者和團隊。
...
> [!tip] 爆紅原因
> 建立 12 天就累積 1093 stars（91/天），forks 100（9.1%），這顯示出相對穩定的增長趨勢。作者 ClaudioDrews 之前在記憶系統方面有過深入的研究，這個專案解決了現有記憶解決方案的雲端依賴問題，提供了一個本地運行的選擇。這個工具的推出正好滿足了對於私有記憶基礎設施的需求，並且在社群中引起了討論，尤其是關於性能基準的問題。技術上，隨著本地運算能力的提升，這種解決方案變得越來越可行。forks/stars 比率為 9.1%，顯示出有相當比例的用戶在積極修改和使用這個專案。

## 適合誰使用

**目標受眾**：需要在本地運行持久記憶系統的開發者和團隊。

> [!example] 使用場景
> - AI 研究員用它來在本地環境中持久化對話記憶，因為這樣可以避免雲端存儲的隱私風險。
> - 開發者用它來整合多個 LLM 提供者，因為它支持任意 LLM，讓開發過程更靈活。
> - 企業團隊用它來構建內部知識庫，因為自動化的知識管道能夠持續更新和維護資訊。

## 架構分析

Memory OS 採用七層架構設計，旨在提供全面的記憶管理。每一層都有其特定功能，從工作區到會話，再到結構化事實和向量數據庫。這樣的設計使得系統能夠高效地捕獲和利用記憶。

具體來說，Layer 1 到 Layer 6 負責記憶的捕獲和存儲，而 Layer 7 則確保這些記憶能被有效利用。這種設計的代價在於需要較高的資源消耗，特別是在使用 Qdrant 和 Redis 的情況下。擴展性方面，隨著資料量的增加，系統的性能可能會受到影響，特別是在查詢效率上。

## 技術深入分析

Memory OS 的核心技術機制在於其七層架構，這使得記憶的捕獲、存儲和注入能夠高效運行。每一層都有其特定的功能，Layer 1 到 Layer 6 主要負責記憶的管理，而 Layer 7 則確保這些記憶能夠被有效利用。系統使用 Qdrant 作為向量數據庫，並結合 Redis 進行記憶的管理，這樣的設計使得記憶的檢索效率大幅提升。效能上，系統能夠處理大量的資料，但在高並發的情況下可能會出現瓶頸。設計上選擇 Python 和 Docker，這使得系統的部署變得簡單，但也帶來了對資源的需求。技術風險方面，隨著資料量的增加，系統的性能可能會受到影響，特別是在查詢效率上。整合方面，Memory OS 可以與多種 LLM 提供者兼容，但在非 Hermes 環境中的整合可能會遇到挑戰。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且提供了安裝範例，安裝過程相對順暢，但需要 Docker 環境。沒有專門的入門指南，但安裝腳本簡化了部署過程。文件目前僅提供英文版本。
...
> - 智能的上下文注入，提升代理的長期合作能力。

> [!danger] 缺點
> - 安裝過程需要 Docker 環境，對新手可能有一定難度。
> - 對於非 Hermes 環境的整合可能存在挑戰。
...
| 工具 | 差異 |
| --- | --- |
| [mem0/mem0](https://github.com/mem0/mem0) | mem0 是雲端解決方案，無法提供本地運行的選擇，對於需要私有記憶的用戶不合適。 |
| [Zep/zep](https://github.com/Zep/zep) | Zep 主要針對雲端記憶，缺乏本地基礎設施的支持，無法滿足對隱私的需求。 |
| [Letta/let](https://github.com/Letta/let) | Letta 提供的功能較為淺顯，無法實現 Memory OS 的深度結構化和智能注入。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [mem0/mem0](https://github.com/mem0/mem0) | mem0 是雲端解決方案，無法提供本地運行的選擇。 | 如果你的團隊已經在使用雲端記憶解決方案且不需要本地部署。 | medium，因為需要重新配置雲端環境。 |
| [Zep/zep](https://github.com/Zep/zep) | Zep 主要針對雲端記憶，缺乏本地基礎設施的支持。 | 如果你的應用場景不需要本地數據存儲，且可以接受雲端解決方案。 | medium，因為需要將數據遷移到雲端。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **mem0** | **zep** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | mem0 是雲端解決方案，無法提供本地運行的選擇。 | Zep 主要針對雲端記憶，缺乏本地基礎設施的支持。 |
> | 遷移成本 | - | medium，因為需要重新配置雲端環境。 | medium，因為需要將數據遷移到雲端。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用雲端記憶解決方案且不需要本地部署。 | 如果你的應用場景不需要本地數據存儲，且可以接受雲端解決方案。 |
...
- **[HIGH]** 在非 Hermes 環境中整合時，可能會遇到依賴問題。
  - 解法：確保所有依賴都正確安裝，並參考文檔進行配置。
- [MEDIUM] 性能基準測試結果可能不如預期。
  - 解法：在不同的硬體環境中進行測試，並調整配置。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 本地運行的特性和靈活的 LLM 支持非常符合小型團隊的需求。 |
| 大型企業的數據分析平台 | 不適合 | 在高並發情況下可能會出現性能瓶頸。 |
| AI 研究機構的長期項目 | 適合 | 能夠有效管理和持久化記憶，支持多種 LLM 提供者。 |
| 需要即時數據處理的應用 | 普通 | 雖然支持本地運行，但在高頻率的數據更新下可能會有延遲。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，10 小時整合，得到一個靈活的本地記憶系統，值得一試。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：本地運行，無需高權限，且不存取敏感資料，依賴鏈的信任程度較高。
