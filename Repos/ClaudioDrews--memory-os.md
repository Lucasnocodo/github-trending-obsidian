...
**1.2k** stars · **48** stars/天 · 建立 25 天前 · Python · MIT
...
> **授權** MIT (商業友好) · **維護** Moderate (最後推送 15 天前) · **貢獻者** 4+ 人 · **參與度** Low
> **適合** 需要在本地運行持久記憶系統以支持長期項目管理的專業用戶。
> **一句話重點** Memory OS 的最大優勢在於其本地化的記憶管理能力，讓用戶能夠完全掌控自己的數據。
...
Memory OS 是一個為 Hermes Agent 設計的七層記憶操作系統，旨在解決代理在多次對話中遺忘上下文的問題。它的核心機制是將記憶分為七層，從工作區到結構化事實，再到向量數據庫，並實現智能的上下文注入。使用者只需運行一個命令 `curl -sSL https://raw.githubusercontent.com/ClaudioDrews/memory-os/main/setup.sh | bash` 即可安裝整個系統，無需雲端記憶訂閱，支持多種 LLM 提供者。技術上，Memory OS 結合了 Qdrant 作為向量數據庫，並使用 Redis 和 ARQ Worker 來管理記憶的存取和維護。這種設計使得記憶能夠在本地運行，避免了資料外洩的風險。

與傳統的 Hermes 相比，Memory OS 提供了更強大的跨會話回憶能力和結構化事實存儲，並且具備自我整理的維基功能，這些都是標準 Hermes 所不具備的。使用者可以在任何支持的 LLM 上運行此系統，並享受無需訂閱的持久記憶。這使得 Memory OS 特別適合需要長期記憶的專業用戶，如研究人員或開發者。由於其架構的複雜性，使用者在初期可能需要一些時間來熟悉系統，但一旦掌握，將能夠顯著提高工作效率。

**技術棧**：`Python 3.11` · `Docker` · `Redis` · `Qdrant`

## 重點功能

- 七層記憶架構 — 包括工作區、會話、結構化事實、跨會話記憶、向量數據庫、自我整理的維基和真實性層。
- 智能上下文注入 — 在每次 LLM 調用前自動從多個來源回憶相關上下文，減少重複查詢。
- 本地運行 — 完全在用戶的機器上運行，無需雲端服務，保障數據隱私。
- 支持多種 LLM 提供者 — 包括 OpenRouter、OpenAI、Anthropic 等，無供應商鎖定。
- 自我整理的維基 — 自動生成和維護知識庫，持續更新和優化信息。
...
> [!tip] 爆紅原因
> 建立 25 天內累積 1195 stars（48/天），forks 113（9.5%），顯示出強勁的增長潛力。作者 ClaudioDrews 之前在記憶和 AI 領域有多個貢獻，這個專案解決了現有記憶解決方案的雲端依賴問題，提供了本地化的記憶基礎設施。社群對於這個專案的反應熱烈，尤其是對於如何與 Hermes Agent 整合的討論。技術上，這個專案的出現正好迎合了對於數據隱私和本地運算需求的增加，特別是在 AI 應用中。forks/stars 比率為 9.5%，顯示出許多使用者正在積極修改和使用這個工具。

## 適合誰使用

**目標受眾**：需要在本地運行持久記憶系統以支持長期項目管理的專業用戶。

> [!example] 使用場景
> - 研究人員用它來記錄和回顧過去的研究決策，因為它能夠持久保存對話和上下文，避免重複工作。
> - 開發者用它來管理多個專案的歷史和決策，因為它提供了結構化的事實存儲和智能上下文注入。
> - 產品經理用它來跟踪會議記錄和客戶反饋，因為它能夠自動整理和回顧過去的對話內容。

## 架構分析

Memory OS 的架構由七層組成，這些層次從工作區到結構化事實，再到向量數據庫，形成了一個完整的記憶管理系統。每一層都有其特定的功能，例如，Layer 1 專注於工作區的記錄，Layer 2 則使用 SQLite 和 FTS5 進行全文本搜索，Layer 3 提供結構化事實存儲，並透過信任評分進行管理。這種設計使得用戶能夠在本地運行所有記憶基礎設施，避免了雲端依賴的風險。

選擇使用 Qdrant 作為向量數據庫，因為它能夠提供高效的相似度搜索，並支持多種檢索策略。這樣的架構雖然複雜，但能夠提供強大的記憶管理能力，特別是在需要長期記憶的應用場景中。擴展性方面，由於所有組件都是本地運行，使用者可以根據需求調整資源配置，這在雲端解決方案中往往無法實現。

## 技術深入分析

Memory OS 的核心技術機制是其七層架構，這使得記憶的捕獲、存儲和注入變得高效且有序。每一層都有其特定的功能，Layer 1 提供工作區的記錄，Layer 2 使用 SQLite 和 FTS5 進行全文本搜索，Layer 3 則專注於結構化事實的存儲和管理。這些層次的設計使得記憶能夠在本地運行，避免了雲端依賴的風險。效能方面，使用 Qdrant 作為向量數據庫能夠提供高效的相似度搜索，並支持多種檢索策略。

這樣的設計雖然複雜，但能夠提供強大的記憶管理能力，特別是在需要長期記憶的應用場景中。設計取捨方面，選擇本地運行的架構雖然增加了安裝和配置的複雜性，但卻能夠保障數據的隱私和安全。技術風險方面，隨著資料量的增加，可能會出現性能瓶頸，因此需要定期進行性能調整和優化。整合方面，Memory OS 能夠與多種 LLM 提供者兼容，這使得其在不同的應用場景中都能夠發揮作用。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且詳細，提供了安裝步驟和使用範例。安裝過程順暢，使用者只需執行一條命令即可完成安裝。文件中包含了良好的入門指南，對於新手來說非常友好。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供類似的記憶管理功能，但主要依賴雲端基礎設施，無法實現本地運行。 |
| Zep | 雖然也提供記憶功能，但缺乏結構化事實存儲和自我整理的維基功能。 |
| mem0 | 專注於雲端記憶解決方案，無法提供本地化的記憶基礎設施。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| Zep | Zep 提供雲端記憶解決方案，無法實現本地運行，且缺乏結構化事實存儲功能。 | 如果你的團隊已經在使用 Zep 並且不需要本地化的記憶管理。 | medium，因為需要重新設計數據存儲和管理流程。 |
| mem0 | mem0 也專注於雲端記憶，但無法提供本地化的記憶基礎設施，且功能相對簡單。 | 如果你的團隊已經在使用 mem0 並且不需要複雜的記憶管理功能。 | high，因為需要大幅度改變現有的記憶管理策略。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **Zep** | **mem0** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | Zep 提供雲端記憶解決方案，無法實現本地運行，且缺乏結構化事實存儲功能。 | mem0 也專注於雲端記憶，但無法提供本地化的記憶基礎設施，且功能相對簡單。 |
> | 遷移成本 | - | medium，因為需要重新設計數據存儲和管理流程。 | high，因為需要大幅度改變現有的記憶管理策略。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用 Zep 並且不需要本地化的記憶管理。 | 如果你的團隊已經在使用 mem0 並且不需要複雜的記憶管理功 |
...
- **[HIGH]** 在大型資料集上可能會出現性能瓶頸
  - 解法：定期優化性能參數以適應資料量變化
- [MEDIUM] Docker 環境配置不當可能導致安裝失敗
  - 解法：確保 Docker 和相關依賴正確安裝

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 能夠提供本地化的記憶管理，適合小型團隊的需求。 |
| 大型企業的記憶管理系統 | 普通 | 雖然功能強大，但安裝和配置過程可能過於複雜。 |
| 需要長期記憶的研究項目 | 非常適合 | 能夠有效管理和回顧過去的研究決策。 |
