...
use_case: "為 Hermes Agent 提供持久記憶的操作系統，讓代理能夠記住過去的對話與決策。"
priority: high
ring: assess
...
aliases:
  ...
  - "為 Hermes Agent 提供持久記憶的操作系統，讓代理能夠記住過去的對話與決策。"
---

# memory-os

**1.3k** stars · **31** stars/天 · 建立 40 天前 · Python · MIT
...
> **授權** MIT (商業友好) · **維護** Moderate (最後推送 30 天前) · **貢獻者** 4+ 人 · **參與度** Low
> **適合** 需要在本地運行持久記憶系統的開發者或團隊，尤其是與 Hermes Agent 相關的用戶。
> **一句話重點** Memory OS 的設計不僅是為了解決記憶問題，更是為了提供一個靈活且安全的本地記憶解決方案。
...
> **結論** 花 5 小時學習，10 小時整合，得到靈活的本地記憶解決方案，值得考慮。

> [!abstract] 核心創新
> 提供完整的本地記憶基礎設施，避免了雲端鎖定和訂閱費用。

## 專案簡介

Memory OS 是一個為 Hermes Agent 設計的七層記憶操作系統，旨在解決代理在多次會話中忘記上下文的問題。用戶可以透過簡單的指令 `curl -sSL https://raw.githubusercontent.com/ClaudioDrews/memory-os/main/setup.sh | bash` 來安裝整個系統，這個系統不僅能夠在本地運行，還能與任何 LLM 提供者兼容。其核心機制包括結構化事實存儲、智能上下文注入以及一個自我策展的維基系統，這些都能有效提升代理的記憶能力。特別是第七層的「真實性層次」，確保注入的記憶被代理視為權威，避免了重複查詢和浪費資源的情況。技術上，Memory OS 使用了 Qdrant 作為向量數據庫，並結合 Redis 和 ARQ Worker，這樣的架構設計使得整體系統具備高效的性能和靈活性。

與其他記憶解決方案相比，如 mem0 和 Zep，Memory OS 提供了完全的本地記憶基礎設施，避免了雲端鎖定和訂閱費用。實際使用中，這個系統能夠處理大量的對話數據，並且支持全文本搜索和智能去重，從而提升用戶的工作效率。儘管目前仍在 beta 階段，但其社群活躍度和持續的更新顯示出良好的發展潛力。對於需要長期記憶的開發者或團隊，Memory OS 是一個值得考慮的選擇。
...
- 智能上下文注入 — 確保每次會話都能夠使用最新的記憶。
- 自我策展維基 — 自動整理和更新知識庫，提升信息的可用性。
- 真實性層次 — 確保注入的記憶被代理視為權威，避免重複查詢。
- 全文本搜索 — 使用 SQLite 和 FTS5 進行快速查詢，提升查找效率。
- 向量數據庫支持 — 利用 Qdrant 提供高效的向量檢索能力。
- 無雲端鎖定 — 完全本地運行，無需訂閱費用，保護用戶數據隱私。
...
```python
{
  ...
  "預期輸出": "安裝完成後，系統應該能夠正常運行，並且所有服務都已啟動。"
}
```
...
> [!tip] 爆紅原因
> 建立 40 天內累積 1254 stars（31/天），forks 119（9.5%），顯示出穩定的增長潛力。這個專案由 ClaudioDrews 等人開發，他們在 AI 記憶和上下文注入方面有豐富的經驗。Memory OS 解決了現有記憶解決方案的痛點，特別是對於需要本地運行的用戶，這在以往的雲端解決方案中難以實現。最近的推廣活動和社群參與也促進了其曝光率，讓更多開發者注意到這個工具。整體來看，這個專案的增長主要來自於其獨特的本地記憶基礎設施和靈活的提供者兼容性。

## 適合誰使用

**目標受眾**：需要在本地運行持久記憶系統的開發者或團隊，尤其是與 Hermes Agent 相關的用戶。

> [!example] 使用場景
> - AI 研究員用它來持久記錄實驗結果，因為這樣可以避免重複進行相同的實驗，節省時間和資源。
> - 產品經理用它來追蹤會議記錄和決策過程，因為這樣能夠在未來的會議中快速回顧過去的討論。
> - 開發者用它來管理代碼變更的歷史和上下文，因為這樣能夠更清楚地了解每次提交的背景和理由。

## 架構分析

Memory OS 的架構由七層組成，每一層都有其特定的功能和責任。第一層處理工作區的記憶，第二層負責會話記錄，第三層則專注於結構化事實的存儲。這樣的分層設計使得系統能夠有效地管理和檢索記憶。

使用 Qdrant 作為向量數據庫，能夠提供高效的檢索性能，而 Redis 則用於快速存取會話數據。這種設計的代價是需要一定的硬體資源來支持整個系統的運行，特別是在面對大量數據時。整體架構的擴展性良好，但在資料量極大時，可能會出現性能瓶頸。

每一層的設計都考慮到了用戶的使用場景，並且提供了靈活的接口來進行擴展和自定義。

## 技術深入分析

Memory OS 的核心技術機制是其七層架構，每一層都專注於特定的記憶管理任務。第一層的工作區記憶使用平面文件來存儲用戶的基本信息；第二層的會話記憶則利用 SQLite 和 FTS5 提供全文本搜索功能，這使得用戶能夠快速查找過去的對話。第三層的結構化事實則通過信任評分和自動反饋循環來持久化存儲。效能方面，Memory OS 能夠處理大量的對話數據，並且在查詢時能夠保持低延遲。

選擇 Python 和 Docker 作為主要技術棧，這使得系統能夠在多種環境中靈活運行。依賴樹相對簡單，主要依賴於幾個核心庫，這降低了維護的複雜性。設計上，Memory OS 針對本地運行進行了優化，這樣的選擇使得用戶不必擔心雲端鎖定的風險。隨著用戶數據的增長，可能會出現性能瓶頸，特別是在高負載的情況下。

整合方面，Memory OS 能夠與主流的 LLM 提供者無縫對接，這使得用戶能夠根據需求選擇最適合的模型。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且詳細，提供了安裝和使用的具體步驟。安裝過程相對順暢，但需要用戶具備基本的 Docker 知識。文件中有針對新手的指南，幫助用戶快速上手。整體而言，花 30 分鐘應該能夠順利運行起來。

## 優缺點分析

> [!success] 優點
> - 本地運行，無需擔心數據隱私問題。
> - 提供結構化事實和智能上下文注入，提升記憶的有效性。
> - 安裝簡單，使用單一指令即可完成設置。

> [!danger] 缺點
> - 仍在 beta 階段，可能存在不穩定的情況。
> - 需要 Docker 環境，對於某些用戶來說可能增加了複雜性。
> - 對於大型數據集，可能需要較高的硬體資源來維持性能。
...
| 工具 | 差異 |
| --- | --- |
| mem0 | mem0 是一個雲端記憶解決方案，無法提供本地運行的選項，對於需要保護數據隱私的用戶不適合。 |
| Zep | Zep 主要聚焦於雲端記憶，而 Memory OS 提供了完整的本地基礎設施，避免了雲端鎖定。 |
| Letta | Letta 也無法提供本地記憶解決方案，且缺乏結構化事實和智能上下文注入的功能。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| mem0 | mem0 是一個雲端記憶解決方案，無法提供本地運行的選項。 | 如果需要一個簡單的雲端解決方案，並且不在意數據隱私問題。 | medium，因為需要重新設計數據存儲和檢索邏輯。 |
| Zep | Zep 主要聚焦於雲端記憶，無法提供完整的本地基礎設施。 | 如果團隊已經在使用 Zep 並且不需要本地運行的功能。 | high，因為需要大量的數據遷移和系統重構。 |
| Letta | Letta 也無法提供本地記憶解決方案，且缺乏結構化事實和智能上下文注入的功能。 | 如果需要一個簡單的記憶解決方案，但不需要高級功能。 | medium，因為需要重新設計數據結構。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **mem0** | **Zep** | **Letta** |
> | --- | --- | --- | --- | --- |
> | 技術路線 | 本專案 | mem0 是一個雲端記憶解決方案，無法提供本地運行的選項。 | Zep 主要聚焦於雲端記憶，無法提供完整的本地基礎設施。 | Letta 也無法提供本地記憶解決方案，且缺乏結構化事實和智能上下文注入的功能。 |
> | 遷移成本 | - | medium，因為需要重新設計數據存儲和檢索邏輯。 | high，因為需要大量的數據遷移和系統重構。 | medium，因為需要重新設計數據結構。 |
> | 適用場景 | 主要場景 | 如果需要一個簡單的雲端解決方案，並且不在意數據隱私問題。 | 如果團隊已經在使用 Zep 並且不需要本地運行的功能。 | 如果需要一個簡單的記憶解決方案，但不需要高級功能。 |
...
- **[HIGH]** 在某些環境下，Docker 可能無法正確啟動所有服務。
  - 解法：確保 Docker 版本為最新，並檢查系統資源配置。
- [MEDIUM] 在大量數據下，檢索速度可能會變慢。
  - 解法：考慮優化數據結構或升級硬體資源。
- [MEDIUM] 某些功能在特定 LLM 提供者上可能不完全兼容。
  - 解法：查看官方文檔以獲取兼容性資訊。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 能夠提供本地記憶，避免雲端鎖定，適合小型團隊的需求。 |
| 需要長期記憶的 AI 研究項目 | 適合 | 具備結構化事實和智能上下文注入的功能，能夠有效管理記憶。 |
| 大型企業的核心業務系統 | 不適合 | 目前仍在 beta 階段，穩定性和性能可能無法滿足企業需求。 |
| 需要快速原型開發的個人項目 | 非常適合 | 安裝簡單，能夠快速上手並進行測試。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，10 小時整合，得到靈活的本地記憶解決方案，值得考慮。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：本地運行，不需要高權限，且不存取敏感資料。依賴鏈相對簡單，無已知的供應鏈風險，適合在 CI/CD 中使用。
