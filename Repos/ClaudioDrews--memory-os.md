...
**1.2k** stars · **45** stars/天 · 建立 27 天前 · Python · MIT
...
> **授權** MIT (商業友好) · **維護** Moderate (最後推送 17 天前) · **貢獻者** 4+ 人 · **參與度** Low
> **適合** 希望在本地環境中運行持久記憶系統的開發者和團隊。
> **一句話重點** Memory OS 不僅僅是一個插件，而是一個完整的記憶操作系統，能夠有效解決 Hermes Agent 的記憶問題。
...
> **結論** 花 5 小時學、10 小時整合，得到一個靈活的本地記憶系統，值得嘗試。

> [!abstract] 核心創新
> 提供了一個完整的七層記憶操作系統，支持智能上下文注入和自我修正的維基管理。

## 專案簡介

Memory OS 是一個針對 Hermes Agent 設計的七層記憶操作系統，旨在解決代理在多次會話中無法持久記憶的問題。它通過將記憶分為七層，從工作區到結構化事實，並利用 Qdrant 和 Redis 等技術來實現高效的記憶存取。用戶可以通過一個簡單的命令安裝整個系統，並且支持多種 LLM 提供者，這樣用戶就不必擔心供應商鎖定的問題。核心賣點在於其智能的上下文注入和自動化的維基管理，讓代理能夠在每次對話中準確回憶過去的決策和項目。技術上，Memory OS 使用了 SQLite 和 Qdrant 來支持高效的全文本搜索和向量檢索，並且具備自我修正和去重的能力。

與傳統的 Hermes 系統相比，它在記憶的結構化和跨會話回憶上有顯著提升，並且提供了更高的令牌效率。這使得它在需要長期記憶的應用場景中表現優異。使用者可能會遇到的問題包括 Docker 環境的配置和依賴管理，但整體社群活躍度高，問題解決率達到67%。這個專案目前處於穩定的 beta 階段，適合需要持久記憶的開發者和團隊使用。
...
- 智能上下文注入 — 自動將相關記憶注入每次對話中，提升交互效率。
- 自我修正維基 — 自動整理和更新知識庫，保持信息的準確性和時效性。
- Qdrant 向量檢索 — 支持高效的向量檢索和全文本搜索，提升查詢速度。
- 本地運行 — 完全在本地環境中運行，無需雲端依賴，保護用戶隱私。
...
> [!tip] 爆紅原因
> 建立 27 天內累積 1205 stars（每天 45），forks 112（9.3%），顯示出穩定的增長。作者 Claudio Drews 在記憶系統方面有深入的研究背景，這個專案解決了現有記憶解決方案的雲端依賴問題，提供了本地運行的選擇。這樣的設計讓用戶能夠完全掌控自己的數據，並且不需要訂閱服務。近期的社群討論和問題反饋也顯示出對這個工具的需求和興趣。

## 適合誰使用

**目標受眾**：希望在本地環境中運行持久記憶系統的開發者和團隊。

> [!example] 使用場景
> - AI 研究員用它來記錄和回顧過去的實驗結果，因為它能夠自動整理和檢索重要的數據和決策。
> - 產品經理用它來追蹤項目進度和團隊決策，因為它提供了持久的上下文記憶，避免重複討論。
> - 開發者用它來在 CI/CD 流程中自動化測試和回顧過去的 bug 修復，因為它能夠快速檢索相關的歷史記錄。

## 架構分析

Memory OS 的架構由七層組成，每一層都專注於特定的記憶功能。第一層是工作區，負責記錄用戶的基本信息；第二層是會話記憶，利用 SQLite 和 FTS5 進行全文本搜索；第三層是結構化事實，結合信任評分來提升記憶的準確性；第四層是跨會話的記憶提取，使用 Icarus 插件進行多來源的注入；第五層是向量數據庫 Qdrant，提供高效的檢索能力；第六層是自動化維基，持續更新知識庫；第七層是關鍵的真實層級，確保注入的記憶被代理正確使用。這樣的設計使得記憶的存取更加高效且智能，並且避免了重複的上下文檢索。整體架構的選擇使得 Memory OS 能夠在本地運行，避免了雲端依賴的風險，但也增加了對 Docker 和其他依賴的需求。

## 技術深入分析

Memory OS 的核心技術機制是其七層架構，這使得記憶的捕獲、存儲和注入變得高效且智能。每一層都有其特定的功能，從基本的工作區到複雜的向量數據庫，這樣的設計不僅提升了記憶的準確性，也確保了用戶能夠快速檢索所需的信息。效能方面，使用 Qdrant 和 Redis 的組合，能夠在大規模數據下保持良好的響應速度，並且支持 O(1) 的路徑查找，這對於需要快速回應的應用場景至關重要。選擇 Python 作為主要開發語言，因為其在數據處理和機器學習領域的廣泛應用，並且有著豐富的生態系統。依賴樹相對簡單，主要依賴於幾個關鍵庫，這降低了維護的複雜度。技術風險方面，隨著用戶數量的增加，可能會出現性能瓶頸，特別是在高並發的情況下，這需要進一步的優化。整合方面，Memory OS 可以與主流的 CI/CD 工具鏈無縫對接，並且支持多種 IDE 的插件，這使得它在開發流程中非常友好。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝指導和範例，安裝過程相對順暢，但對於不熟悉 Docker 的用戶可能會有一些挑戰。整體來說，文檔質量良好，能夠幫助新手快速上手。
...
> - 在某些低配置機器上，Docker 啟動時間較長。

> [!warning] 注意事項
> - 需要 Docker 環境支持，對於不熟悉 Docker 的用戶可能有學習曲線。
> - 目前僅支持 Python 3.11 以上版本，舊版本用戶需升級。
> - 在某些低配置機器上，Docker 的啟動時間可能較長。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| mem0 | mem0 是雲端解決方案，無法提供本地運行的選擇，且需要訂閱服務。 |
| Zep | Zep 主要針對雲端環境設計，缺乏本地數據存儲的能力。 |
| Letta | Letta 提供的功能較為淺顯，無法達到 Memory OS 的深度和靈活性。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| mem0 | mem0 是一個雲端解決方案，無法提供本地運行的選擇，且需要訂閱服務。 | 如果你的團隊已經在使用雲端服務且不介意數據存儲在外部。 | medium，因為需要重新配置和適應雲端環境。 |
| Zep | Zep 主要針對雲端環境設計，缺乏本地數據存儲的能力。 | 如果你的團隊需要一個簡單的雲端解決方案，且不需要複雜的記憶功能。 | medium，因為需要重新設計數據存儲和檢索邏輯。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **mem0** | **Zep** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | mem0 是一個雲端解決方案，無法提供本地運行的選擇，且需要訂閱服務。 | Zep 主要針對雲端環境設計，缺乏本地數據存儲的能力。 |
> | 遷移成本 | - | medium，因為需要重新配置和適應雲端環境。 | medium，因為需要重新設計數據存儲和檢索邏輯。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用雲端服務且不介意數據存儲在外部。 | 如果你的團隊需要一個簡單的雲端解決方案，且不需要複雜的記憶功 |
...
- [MEDIUM] 在某些低配置機器上，Docker 啟動時間較長，可能導致用戶不耐煩。
  - 解法：建議使用較高配置的機器或提前做好環境配置。
- **[HIGH]** 對於不熟悉 Docker 的用戶，安裝過程可能會遇到困難。
  - 解法：建議參考官方文檔，並尋求社群幫助。
- [MEDIUM] 某些依賴版本不兼容，可能導致安裝失敗。
  - 解法：確保所有依賴都符合要求，並查看 GitHub 上的 issue 以獲取解決方案。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 提供本地運行的記憶系統，能夠有效管理團隊的知識和決策。 |
| 大型企業的數據分析團隊 | 普通 | 雖然功能強大，但可能需要更多的資源來支持。 |
| 個人開發者的 AI 項目 | 非常適合 | 能夠快速集成並提供持久的記憶功能。 |
| 需要高可用性的商業應用 | 不適合 | 目前仍處於 beta 階段，穩定性尚未達到商業應用的要求。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學、10 小時整合，得到一個靈活的本地記憶系統，值得嘗試。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：本地運行的系統不需要高權限，且不會存取敏感資料，依賴鏈相對簡單，無已知的供應鏈風險。

## 生態系整合

> [!abstract] 如何融入你的工具鏈

Memory OS 可以與多種 LLM 提供者無縫整合，並且在開發流程中支持主流的 CI/CD 工具鏈。具體來說，在使用 Next.js 和 Vercel 部署的專案中，可以通過 Docker 來快速設置和運行 Memory OS，並利用其記憶功能來提升應用的智能化程度。整合時，最常見的問題是 Docker 環境的配置，建議提前準備好相關的環境變數和依賴。
