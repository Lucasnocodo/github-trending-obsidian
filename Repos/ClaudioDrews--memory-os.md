...
use_case: "為 Hermes Agent 提供持久記憶的七層記憶操作系統，支持本地運行和多種 LLM 提供者。"
priority: high
ring: assess
...
aliases:
  ...
  - "為 Hermes Agent 提供持久記憶的七層記憶操作系統，支持本地運行和多種 LLM 提供者。"
---

# memory-os

**1.3k** stars · **24** stars/天 · 建立 55 天前 · Python · MIT
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Growing · **熱度** Growing (24 stars/day)
> **授權** MIT (商業友好) · **維護** Slow (最後推送 45 天前) · **貢獻者** 4+ 人 · **參與度** Low
> **適合** 需要在本地運行 AI 代理並希望其具備長期記憶功能的開發者。
> **一句話重點** Memory OS 的創新在於其七層架構和真實性層，讓 AI 代理能夠真正記住並有效利用過去的對話和決策。
...
> **結論** 花 5 小時學習，10 小時整合，得到高效的記憶管理，值得投資。

> [!abstract] 核心創新
> Memory OS 提供了一個完整的七層記憶架構，並引入了真實性層來確保代理能夠有效利用注入的記憶。

## 專案簡介

Memory OS 是一個為 Hermes Agent 設計的七層記憶操作系統，旨在解決傳統 AI 代理在記憶上的不足。它的核心機制是將用戶的對話和決策記錄在本地，並透過 Qdrant 進行結構化存儲，讓代理能夠在需要時自動調用這些記憶。用戶只需透過 `curl -sSL https://raw.githubusercontent.com/ClaudioDrews/memory-os/main/setup.sh | bash` 指令即可快速安裝整個系統，無需雲端訂閱，避免了數據外洩的風險。這個系統的賣點在於其智能的上下文注入和自我整理的維基功能，能夠在每次對話中提供準確的上下文信息。技術上，Memory OS 使用 Python 和 Docker，並依賴 Qdrant 作為向量數據庫，支持多種 LLM 提供者，這使得它在功能上比傳統的 Hermes 更加強大。

與其他記憶解決方案相比，如 mem0 和 Zep，Memory OS 提供了本地化的記憶基礎設施，並且無需雲端訂閱，這對於重視隱私的用戶來說是一大優勢。實際使用中，這個系統能夠有效處理大量對話數據，並且提供了智能的去重和歸檔功能，確保記憶的準確性和可靠性。這個專案目前處於活躍開發中，社群也在持續增長，適合需要長期記憶的 AI 代理應用。使用者在整合時需注意 Docker 環境的配置，並確保符合 Python 3.11 以上的要求。
...
- 本地運行 — 完全在用戶機器上運行，無需雲端訂閱，保護用戶隱私。
- 智能上下文注入 — 根據用戶的對話歷史自動注入相關上下文，提升對話的連貫性。
- 結構化事實存儲 — 使用 SQLite 和 Qdrant 管理事實，並提供信任評分和自動反饋迴路。
- 自我整理的維基 — 自動整理和更新知識庫，方便用戶隨時查閱過去的對話和決策。
...
> [!tip] 爆紅原因
> 建立 55 天內累積 1297 stars（24/天），forks 121（9.3%），顯示出穩定的增長潛力。作者 ClaudioDrews 之前在 AI 記憶領域有多項貢獻，這個專案解決了現有記憶解決方案的雲端依賴問題，讓用戶能在本地運行，並提供靈活的 LLM 支持。近期的社群互動和問題回應顯示出活躍的開發和使用者基礎，這對於未來的發展非常重要。

## 適合誰使用

**目標受眾**：需要在本地運行 AI 代理並希望其具備長期記憶功能的開發者。

> [!example] 使用場景
> - AI 開發者用它來為 Hermes Agent 提供持久記憶，因為這樣可以避免每次對話都重複上下文，提升工作效率。
> - 數據科學家用它來管理和檢索過去的實驗數據，因為 Memory OS 的結構化事實存儲能夠快速找到關鍵信息。
> - 產品經理用它來追蹤項目進度和決策歷史，因為自動整理的維基功能能夠隨時回顧過去的討論和決策。

## 架構分析

Memory OS 採用七層架構設計，從工作區到向量數據庫，每一層都專注於特定的記憶功能。這種設計使得記憶的捕捉、存儲和注入變得高效且有序。使用 Qdrant 作為向量數據庫，能夠支持高效的相似度檢索，並通過多層次的回退機制確保資料的完整性。

選擇本地化的架構設計，避免了雲端依賴，但也使得使用者需要自行管理基礎設施。這種設計的代價在於需要用戶具備一定的技術能力來維護系統。整體架構的擴展性良好，但在大規模資料處理時可能會面臨性能瓶頸，特別是在資料量極大的情況下。

## 技術深入分析

Memory OS 的核心技術機制在於其七層記憶架構，這包括工作區、會話、結構化事實、跨會話記憶、向量數據庫、LLM 維基和真實性層。這種設計模式使得記憶的捕捉和存儲變得高效，並且能夠在需要時自動注入相關上下文。效能方面，使用 Qdrant 作為向量數據庫，能夠支持高效的相似度檢索，並且在資料量增大時，通過多層次的回退機制來確保資料的完整性。選擇 Python 和 Docker 作為技術棧，帶來了良好的開發效率，但也要求使用者具備一定的技術能力來維護系統。技術風險方面，隨著資料量的增長，可能會出現性能瓶頸，特別是在資料量極大的情況下。整合方面，Memory OS 能夠與多種 LLM 提供者兼容，但對於不在支持範圍內的提供者，可能需要額外的適配工作。整體而言，這個專案在技術上具有創新性，並且能夠有效解決現有記憶解決方案的不足。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且提供了安裝指令，安裝過程順暢，沒有明顯的坑。文件中包含了快速入門指南，對於新手友好，但目前僅提供英文文檔，缺乏多語言支持。
...
> - 僅支援 Python 3.11 以上版本，限制了使用者的選擇。
> - 目前對於 LLM 提供者的支持有限，可能不適用於所有用例。

> [!warning] 注意事項
> - 僅支援 Python 3.11 以上版本。
> - 需要 Docker 環境，對於不熟悉 Docker 的用戶可能有學習曲線。
> - 目前僅支持特定的 LLM 提供者，對於其他提供者的兼容性可能有限。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| mem0 | mem0 是一個雲端記憶解決方案，而 Memory OS 提供本地化的記憶基礎設施，適合重視隱私的用戶。 |
| Zep | Zep 主要依賴雲端服務，而 Memory OS 則允許用戶在本地運行，無需雲端訂閱。 |
| Letta | Letta 不支持結構化事實和自我整理的維基功能，而 Memory OS 提供這些高級功能以提升記憶的有效性。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| mem0 | mem0 是一個雲端記憶解決方案，而 Memory OS 提供本地化的記憶基礎設施，適合重視隱私的用戶。 | 如果你的團隊需要一個雲端解決方案並且不介意數據存儲在外部，mem0 可能更合適。 | medium，因為需要將數據從本地轉移到雲端。 |
| Zep | Zep 主要依賴雲端服務，而 Memory OS 則允許用戶在本地運行，無需雲端訂閱。 | 如果你的團隊已經在使用 Zep 並且需要雲端功能，則 Zep 會更方便。 | high，因為需要重新設計記憶架構以適應 Zep 的雲端模式。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **mem0** | **Zep** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | mem0 是一個雲端記憶解決方案，而 Memory OS 提供本地化的記憶基礎設施，適合重視隱私的用戶。 | Zep 主要依賴雲端服務，而 Memory OS 則允許用戶在本地運行，無需雲端訂閱。 |
> | 遷移成本 | - | medium，因為需要將數據從本地轉移到雲端。 | high，因為需要重新設計記憶架構以適應 Zep 的雲端模式。 |
> | 適用場景 | 主要場景 | 如果你的團隊需要一個雲端解決方案並且不介意數據存儲在外部，m | 如果你的團隊已經在使用 Zep 並且需要雲端功能，則 Zep |
...
- **[HIGH]** 在某些環境下，Docker 容器啟動可能會失敗，特別是資源不足的情況下。
  - 解法：確保 Docker 環境有足夠的資源，或在更高效的機器上運行。
- [MEDIUM] 某些 LLM 提供者的兼容性問題，可能導致記憶無法正常使用。
  - 解法：檢查支持的 LLM 列表，並選擇兼容的提供者。
- [MEDIUM] 在高負載情況下，記憶檢索速度可能會變慢。
  - 解法：優化 Qdrant 的配置或增加資源。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的小型團隊使用 Hermes Agent 進行項目管理 | 非常適合 | Memory OS 提供的本地化記憶解決方案能夠有效管理小型團隊的知識和決策。 |
| 大型企業的 AI 代理應用 | 不適合 | 在大規模環境下，可能會面臨性能瓶頸和資源管理挑戰。 |
| 需要長期記憶的個人開發者 | 適合 | Memory OS 能夠提供持久的記憶功能，幫助開發者記錄和回顧過去的工作。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，10 小時整合，得到高效的記憶管理，值得投資。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：Memory OS 在本地運行，不需要高權限，並且不會存取敏感資料。依賴鏈的信任程度良好，無已知的供應鏈風險。
