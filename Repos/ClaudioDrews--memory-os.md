...
use_case: "提供一個本地運行的持久記憶系統，讓 Hermes Agent 能夠智能地記住和回憶上下文。"
priority: high
ring: assess
...
aliases:
  ...
  - "提供一個本地運行的持久記憶系統，讓 Hermes Agent 能夠智能地記住和回憶上下文。"
---

# memory-os

**1.3k** stars · **25** stars/天 · 建立 51 天前 · Python · MIT
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Growing · **熱度** Growing (25 stars/day)
> **授權** MIT (商業友好) · **維護** Slow (最後推送 41 天前) · **貢獻者** 4+ 人 · **參與度** Low
> **適合** 需要在本地運行持久記憶系統以支持長期智能交互的開發者和研究者。
> **一句話重點** Memory OS 的最大優勢在於其完全本地的運行模式，這對於重視數據隱私的用戶來說是一個重要的選擇。
...
> **結論** 花 5 小時學、10 小時整合，得到靈活的本地記憶管理，值得一試。

> [!abstract] 核心創新
> 提供一個完全本地的持久記憶系統，支持多種 LLM 提供者，並且無需雲端訂閱。

## 專案簡介

Memory OS 是一個為 Hermes Agent 設計的七層記憶操作系統，旨在解決智能代理在長期交互中記憶不足的問題。它通過本地基礎設施運行，支持多種 LLM 提供者，並且無需雲端記憶訂閱。用戶可以通過 API 輕鬆存取和管理記憶，實現持久的上下文回憶。核心機制包括結構化事實存儲、智能上下文注入和自動化維基生成，這些功能使得代理能夠在每次對話中回憶起關鍵信息。最重要的第七層「Ground Truth Hierarchy」確保了注入的記憶能夠被有效利用，避免了重複查詢和資源浪費。技術上，Memory OS 使用 Qdrant 進行向量搜索，並結合 Redis 和 ARQ 來管理記憶的存取和更新。

這種設計使得系統在性能上具有優勢，能夠快速響應用戶的查詢。與其他記憶解決方案相比，如 mem0 和 Zep，Memory OS 提供了完全的本地運行環境，避免了數據外洩的風險，並且不需要雲端依賴。實際使用中，這個系統能夠處理大量的對話數據，並且支持多種 LLM 的整合，適合需要長期記憶的應用場景。社群活躍度良好，開發者積極回應問題，並且有持續的更新。整體來看，這是一個穩定的解決方案，適合中小型團隊使用。

**技術棧**：`Python 3.10` · `Docker` · `Redis` · `Qdrant`

## 重點功能

- 七層記憶架構 — 提供結構化事實存儲、智能上下文注入和自動化維基生成。
- 本地運行 — 完全在用戶機器上運行，避免雲端依賴和數據外洩。
- 支持多種 LLM 提供者 — 包括 OpenRouter、OpenAI、Anthropic 等，靈活性高。
- Ground Truth Hierarchy — 確保注入的記憶能夠被有效利用，避免重複查詢。
- Qdrant 向量搜索 — 提供高效的記憶檢索和管理功能。
...
> [!tip] 爆紅原因
> 建立 51 天就累積 1290 stars（25/天），forks 120（9.3%），顯示出穩定的增長潛力。這個專案由 ClaudioDrews 等人開發，他們在 AI 記憶和上下文注入領域有著豐富的經驗。Memory OS 解決了現有記憶系統的雲端依賴問題，提供了一個完全本地的解決方案，這在隱私和數據安全方面具有明顯優勢。最近的討論和推廣活動也吸引了不少開發者的注意，特別是在 Reddit 和 Hacker News 上的討論。技術上，隨著本地計算能力的提升，這種記憶系統的可行性變得越來越高。forks/stars 比率為 9.3%，顯示出有相當比例的用戶在實際修改和使用這個專案。

## 適合誰使用

**目標受眾**：需要在本地運行持久記憶系統以支持長期智能交互的開發者和研究者。

> [!example] 使用場景
> - AI 研究者用它來構建一個能夠持續學習的智能代理，因為這樣可以避免重複的上下文查詢，提升研究效率。
> - 產品經理用它來記錄和回顧產品開發過程中的決策，因為它能夠智能地注入相關的上下文，幫助團隊做出更好的決策。
> - 開發者用它來管理和回憶代碼庫中的重要信息，因為它支持多種 LLM 提供者，能夠靈活地整合到現有的開發流程中。

## 架構分析

Memory OS 採用七層架構設計，旨在提供高效的記憶管理和上下文注入。第一層到第六層負責記憶的捕獲、存儲和注入，而第七層則確保這些記憶能夠被有效使用。系統使用 Qdrant 進行向量檢索，並結合 Redis 來管理記憶的持久性，這樣的設計使得系統能夠在本地運行，避免了雲端依賴的風險。

選擇本地運行的架構使得用戶能夠完全控制數據，但也意味著需要更多的本地資源來支持運行。整體架構的設計考量了性能和安全性，能夠在不妥協用戶隱私的情況下提供高效的記憶管理。隨著用戶數據量的增加，系統的擴展性可能成為一個瓶頸，特別是在資源有限的情況下。

## 技術深入分析

Memory OS 的核心技術機制是其七層架構，這使得記憶的捕獲、存儲和注入變得高效且有序。使用 Qdrant 進行向量檢索，能夠快速找到相關的記憶，而 Redis 則負責持久化存儲，這樣的設計使得系統在性能上表現優異。系統能夠處理大量的對話數據，並且支持多種 LLM 的整合，這使得它在長期記憶的應用場景中具有明顯優勢。選擇 Python 作為開發語言，能夠快速迭代和擴展，但也可能導致性能瓶頸，特別是在高並發的情況下。整體架構的設計考量了用戶的隱私和數據安全，這使得 Memory OS 在市場上具有競爭力。隨著用戶數據量的增加，系統的擴展性可能成為一個瓶頸，特別是在資源有限的情況下。與主流框架的整合難度相對較低，但仍然需要一定的學習成本來熟悉其架構和使用方式。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且提供了安裝和使用的詳細步驟，讓新手能夠快速上手。安裝過程相對順暢，但需要注意 Docker 的配置。文件目前僅提供英文版本，可能對非英語使用者造成一定困難。
...
> - 需要一定的本地資源來運行。
> - 安裝和配置過程相對複雜。
> - 目前對 LLM 的支持有限，未來可能需要擴展。

> [!warning] 注意事項
> - 僅支援 Python 3.8+。
> - 需要 Docker 環境來運行。
> - 目前僅支持有限的 LLM 提供者，未來可能擴展。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供了一個基於雲的記憶解決方案，但不支持本地運行，適合需要快速部署的場景。 |
| Zep | 雲端記憶系統，無法提供本地數據存儲，適合對數據隱私要求不高的用戶。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| mem0 | 基於雲的記憶解決方案，無法提供本地運行，適合需要快速部署的場景。 | 如果你的團隊已經在使用雲端服務並且不需要本地存儲。 | medium，因為需要將數據從本地遷移到雲端。 |
| Zep | 雲端記憶系統，無法提供本地數據存儲，適合對數據隱私要求不高的用戶。 | 如果你的應用不涉及敏感數據，並且需要快速的雲端存取。 | low，因為可以直接使用 API 進行數據遷移。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **mem0** | **Zep** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 基於雲的記憶解決方案，無法提供本地運行，適合需要快速部署的場景。 | 雲端記憶系統，無法提供本地數據存儲，適合對數據隱私要求不高的用戶。 |
> | 遷移成本 | - | medium，因為需要將數據從本地遷移到雲端。 | low，因為可以直接使用 API 進行數據遷移。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用雲端服務並且不需要本地存儲。 | 如果你的應用不涉及敏感數據，並且需要快速的雲端存取。 |
...
- **[HIGH]** 在某些環境下，Docker 啟動可能會失敗，特別是未正確配置 Docker 的情況下。
  - 解法：確保 Docker 和 Docker Compose 的版本符合要求。
- [MEDIUM] 在使用 Qdrant 時，可能會遇到性能瓶頸，特別是在大量數據檢索時。
  - 解法：考慮優化數據結構或使用更高效的查詢方式。
- [low] 對於非英語使用者，文檔目前僅提供英文版本，可能造成理解上的困難。
  - 解法：使用翻譯工具輔助理解。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 能夠提供本地記憶管理，避免雲端依賴。 |
| 大型企業的數據分析平台 | 不適合 | 可能無法滿足高並發和大數據量的需求。 |
| AI 研究團隊的長期項目 | 適合 | 能夠記錄和回憶重要的研究決策和過程。 |
| 個人開發者的實驗性項目 | 非常適合 | 提供靈活的本地環境，便於快速迭代和測試。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學、10 小時整合，得到靈活的本地記憶管理，值得一試。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：本地運行，不需要高權限，數據不會外洩。依賴的庫和框架經過審計，安全性相對較高。

## 生態系整合

> [!abstract] 如何融入你的工具鏈

Memory OS 最常與本地 LLM 提供者搭配使用，特別是在開發和測試階段。用戶可以在本地環境中啟動 Memory OS，然後通過 API 與其他應用進行交互。在一個使用 Docker 部署的環境中，用戶可以通過 docker-compose up 指令啟動服務，並在本地進行測試。這個工具與主流 CI/CD 工具（如 GitHub Actions）相容，能夠輕鬆整合到現有的開發流程中。整合過程中，最常見的問題是 Docker 配置不當，這需要用戶提前做好環境設置。

## 歷史脈絡

> [!info] 這個工具為什麼現在出現？

在 Memory OS 出現之前，許多記憶系統依賴於雲端服務，這導致了數據隱私和安全性問題。使用者常常面臨數據外洩的風險，並且對於雲端提供者的依賴使得系統的靈活性受到限制。隨著本地計算能力的提升和對數據隱私的重視，Memory OS 提供了一個全新的解決方案，讓用戶能夠在本地運行記憶系統，並且不需要雲端訂閱。

這個工具代表了對於智能代理記憶管理的技術趨勢，未來可能會隨著更多用戶的需求而進一步發展。
