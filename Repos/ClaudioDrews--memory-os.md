...
use_case: "為 Hermes Agent 提供持久記憶的操作系統，讓其能夠記住過去的對話與決策。"
priority: high
ring: assess
...
star_history: "2026-06-04:756,2026-06-04:757,2026-06-05:828,2026-06-05:828,2026-06-06:882,2026-06-06:882,2026-06-07:951,2026-06-07:951,2026-06-08:1011,2026-06-09:1037,2026-06-10:1050,2026-06-11:1069,2026-06-12:1086"
tags:
  - github
  - "category/ai_ml"
  - "lang/python"
...
aliases:
  ...
  - "為 Hermes Agent 提供持久記憶的操作系統，讓其能夠記住過去的對話與決策。"
---

# memory-os

**1.1k** stars · **99** stars/天 · 建立 11 天前 · Python · MIT
...
> **一句話重點** Memory OS 不僅僅是記憶插件，而是一個完整的記憶操作系統，能夠在本地運行並提供智能上下文注入。
...
> **結論** 花 4 小時學、6 小時整合，得到本地化的記憶系統，值得一試。

> [!abstract] 核心創新
> Memory OS 提供了一個完整的記憶操作系統，具備七層架構和智能上下文注入，能夠在本地運行並支持多種 LLM 提供者。

## 專案簡介

Memory OS 是一個七層的記憶操作系統，專為 Hermes Agent 設計，能夠在本地運行並與任何 LLM 提供者兼容。其核心機制是通過七層結構來捕捉、存儲和注入記憶，最終以 Ground Truth 層確保注入的記憶被實際使用。用戶可以透過 `curl -sSL https://raw.githubusercontent.com/ClaudioDrews/memory-os/main/setup.sh | bash` 指令一鍵安裝整個系統，無需雲端訂閱，避免了供應商鎖定的問題。技術上，Memory OS 使用了 Qdrant 作為向量數據庫，並結合 Redis 和 ARQ Worker，這樣的架構設計使得記憶的檢索和存儲都能高效運行。

與傳統的 Hermes 相比，Memory OS 提供了結構化事實、跨會話回憶和智能注入等功能，這些都是 stock Hermes 所不具備的。相較於其他記憶解決方案如 mem0 和 Zep，Memory OS 的本地運行特性和無需訂閱的優勢，使其在隱私和靈活性上具有明顯的優勢。使用者在實際應用中可能會遇到的問題包括性能基準測試和與非 Hermes 環境的兼容性，這些都是目前社群關注的熱門議題。整體來看，這個專案的成熟度為 beta，適合中小型團隊使用，未來可能會進一步擴展功能和社群支持。

**技術棧**：`Python 3.11` · `Docker` · `Redis` · `Qdrant`

## 重點功能

- 七層記憶架構 — 包含工作區、會話、結構化事實、跨會話記憶、向量數據庫、LLM Wiki 和 Ground Truth 層。
- 本地運行 — 完全在用戶的機器上運行，無需雲端訂閱，保護用戶隱私。
- 智能上下文注入 — 根據過去的對話自動注入相關記憶，提升交互的連貫性。
- 結構化事實存儲 — 提供事實的信任評分和自動反饋循環，確保記憶的準確性。
- 多來源注入 — 支持從多個來源（如 Qdrant 和會話歷史）進行記憶檢索，提升靈活性。
...
> [!tip] 爆紅原因
> 建立 11 天就累積 1086 stars（99/天），forks 100（9.2%），這顯示出強勁的增長潛力。作者 ClaudioDrews 之前在 AI 記憶領域有過多次實驗，這個專案解決了現有記憶解決方案的雲端依賴和隱私問題。社群對這個專案的反應熱烈，尤其是在 GitHub 上的討論和問題反饋中，顯示出使用者對於記憶性能和兼容性的關注。這個專案的出現正好填補了本地記憶基礎設施的需求，並且隨著 AI 應用的普及，這種需求只會增長。forks/stars 比率為 9.2%，顯示出許多人對此專案有實際的修改和使用需求。
...
> [!example] 使用場景
> - AI 研究人員用它來持久記錄實驗結果和參數，因為這樣可以避免每次啟動時重複設定，節省時間。
> - 開發者用它來管理多個專案的上下文，因為它能夠自動注入相關的歷史記憶，提升工作效率。
> - 產品經理用它來追蹤會議記錄和決策過程，因為它能夠提供跨會話的回憶，避免重複討論。

## 架構分析

Memory OS 採用七層架構設計，從工作區到向量數據庫，層層相互協作以實現高效的記憶管理。每一層都有其特定的功能，如層 1 負責工作區的記錄，層 5 使用 Qdrant 進行向量檢索。這樣的設計使得記憶的捕捉和注入都能夠高效進行，並且每層都有其獨特的優化策略。選擇本地運行的架構使得用戶不必擔心數據隱私問題，但也意味著需要用戶自行管理基礎設施。整體架構的擴展性良好，能夠根據用戶需求進行調整和優化。

## 技術深入分析

Memory OS 的核心技術機制是基於七層架構，這些層次分別負責不同的記憶管理任務。層 1 到層 6 負責記憶的捕捉和存儲，而層 7 的 Ground Truth 層則確保這些記憶被實際使用。這種設計模式使得記憶的檢索和注入都能夠高效進行，並且每層都有其獨特的優化策略。效能方面，這個系統能夠處理大量的對話數據，並且在記憶的檢索上具有較低的延遲。選擇 Python 和 Docker 作為技術棧，能夠方便地進行部署和擴展，但也意味著需要用戶具備一定的技術能力。技術風險方面，隨著使用者數量的增加，系統的性能可能會受到影響，特別是在高並發的情況下。整合分析方面，這個系統能夠與主流的 LLM 提供者進行良好的整合，但在 CI/CD pipeline 的友善度上仍需進一步提升。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且提供了安裝指令，對於新手來說相對友好。安裝過程順暢，且提供了一鍵安裝的選項，降低了入門門檻。文件中有詳細的使用說明和範例，能夠幫助新手快速上手。

## 優缺點分析

> [!success] 優點
> - 本地運行，無需雲端訂閱，保護用戶隱私。
> - 七層架構提供全面的記憶管理功能，提升了上下文的連貫性。
> - 支持多種 LLM 提供者，靈活性高。

> [!danger] 缺點
> - 需要用戶具備一定的 Docker 知識，對新手不太友好。
> - 在性能基準測試上仍需進一步優化。
> - 目前僅支援 Python 3.11+，限制了使用者的選擇。

> [!warning] 注意事項
> - 目前僅支援 Python 3.11+。
> - 需要 Docker 環境運行，對於不熟悉 Docker 的用戶可能有學習曲線。
> - 在性能基準測試上仍需進一步優化，特別是在大規模數據處理時。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| mem0 | mem0 是一個雲端記憶解決方案，無法提供本地運行的隱私保障，且需要訂閱費用。 |
| Zep | Zep 主要針對雲端環境設計，無法支持本地化的記憶存儲和管理。 |
| Letta | Letta 也無法提供本地運行的能力，且缺乏結構化事實存儲的功能。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| mem0 | mem0 是一個雲端記憶解決方案，無法提供本地運行的隱私保障，且需要訂閱費用。 | 如果你的團隊已經在使用雲端服務並希望快速部署記憶系統，mem0 可能更合適。 | medium，因為需要將數據從本地遷移到雲端。 |
| Zep | Zep 主要針對雲端環境設計，無法支持本地化的記憶存儲和管理。 | 如果你的應用場景主要在雲端，且不需要本地記憶，Zep 可能更合適。 | high，因為需要重構整個記憶架構。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **mem0** | **Zep** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | mem0 是一個雲端記憶解決方案，無法提供本地運行的隱私保障，且需要訂閱費用。 | Zep 主要針對雲端環境設計，無法支持本地化的記憶存儲和管理。 |
> | 遷移成本 | - | medium，因為需要將數據從本地遷移到雲端。 | high，因為需要重構整個記憶架構。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用雲端服務並希望快速部署記憶系統，mem | 如果你的應用場景主要在雲端，且不需要本地記憶，Zep 可能更 |
...
- [MEDIUM] 在某些環境下，Docker 容器啟動可能會遇到延遲
  - 解法：確保 Docker 服務正常運行，並檢查系統資源
- **[HIGH]** 與非 Hermes 環境的兼容性問題
  - 解法：使用官方文檔中的指導進行配置
...
| 大型企業的核心應用 | 不適合 | 目前仍在 beta 階段，穩定性不足。 |
| AI 研究團隊進行實驗 | 適合 | 能夠持久記錄實驗數據和上下文。 |
...
> [!tip] 投入 vs 回報
> 花 4 小時學、6 小時整合，得到本地化的記憶系統，值得一試。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：本地運行的系統不需要高權限，且不會存取敏感資料。依賴鏈的信任程度高，沒有已知的供應鏈風險。
...
**社群活躍度**：社群活躍，已經有多個貢獻者參與，並且有定期的更新和問題回應。
