...
**1.2k** stars · **50** stars/天 · 建立 24 天前 · Python · MIT
...
> **授權** MIT (商業友好) · **維護** Moderate (最後推送 14 天前) · **貢獻者** 4+ 人 · **參與度** Low
> **適合** 需要在本地環境中運行 AI 記憶系統的開發者和團隊。
> **一句話重點** Memory OS 的創新在於其七層記憶架構，讓 AI 能夠在本地環境中持久記憶，避免了雲端依賴的風險。
...
> **結論** 花 5 小時學習，10 小時整合，得到一個強大的本地化記憶系統，值得投入。

> [!abstract] 核心創新
> 提供一個完整的七層記憶操作系統，具備智能上下文注入和自動化維基功能，讓 AI 能夠持久記憶。

## 專案簡介

Memory OS 是一個為 Hermes Agent 設計的七層記憶操作系統，旨在解決 AI 在長期合作中遺忘上下文的問題。它的核心機制是將過去的對話、決策和結構化事實持久保存，並在每次交互中自動注入相關記憶，讓 AI 像同事一樣記住所有重要信息。使用者只需安裝 Docker 和 Hermes Agent，便可透過簡單的指令 `curl -sSL https://raw.githubusercontent.com/ClaudioDrews/memory-os/main/setup.sh | bash` 一鍵設置整個系統。技術上，Memory OS 結合了 Qdrant 作為向量資料庫，並利用 Redis 和 ARQ Worker 來管理記憶的存取與處理，這樣的設計使得記憶的檢索和注入過程高效且靈活。

與傳統的 Hermes 系統相比，Memory OS 提供了結構化事實、信任評分和自動化的維基管道，這些功能讓記憶的管理更加智能化。相較於其他雲端記憶解決方案如 mem0 和 Zep，Memory OS 的最大優勢在於其完全本地化的記憶基礎設施，無需訂閱服務，並且支持多種 LLM 提供者，避免了數據外洩的風險。使用者在實際應用中可能會遇到的挑戰包括需要配置 Docker 環境和確保與 Hermes Agent 的兼容性，但這些都是可以通過社群支持和文檔來解決的。總體而言，這是一個成熟且值得信賴的解決方案，適合需要長期記憶管理的開發者和團隊。

**技術棧**：`Python 3.11` · `Docker` · `Qdrant` · `Redis` · `ARQ Worker`

## 重點功能

- 七層記憶架構 — 包括工作空間、會話、結構化事實、跨會話回憶等，提供全面的記憶管理。
- 本地化記憶基礎設施 — 完全在用戶的機器上運行，無需雲端服務，保護數據隱私。
- 智能上下文注入 — 自動將相關記憶注入到每次交互中，提升 AI 的上下文理解能力。
- 結構化事實與信任評分 — 提供持久的事實存儲，並根據用戶的反饋動態調整信任度。
- 自動化維基管道 — 持續整理和更新知識庫，幫助用戶快速查找信息。
...
> [!tip] 爆紅原因
> 建立 24 天內累積 1193 stars（50/天），forks 113（9.5%），顯示出穩定的增長趨勢。這個專案由 ClaudioDrews 等人主導，解決了現有記憶解決方案的雲端依賴問題，提供了一個完全本地化的記憶基礎設施。之前的解決方案如 mem0 和 Zep 都無法滿足用戶對私密性和靈活性的需求，這讓 Memory OS 成為一個理想的選擇。社群的活躍度也反映在熱門 Issues 上，顯示出用戶對於功能擴展和安裝支持的需求。這些因素共同推動了其快速增長。

## 適合誰使用

**目標受眾**：需要在本地環境中運行 AI 記憶系統的開發者和團隊。

> [!example] 使用場景
> - AI 開發者用它來在本地環境中運行 Hermes Agent，因為它提供了持久的記憶功能，避免了每次啟動時重複配置的麻煩。
> - 企業團隊用它來管理長期項目記錄，因為它的自動化維基功能能夠有效整理和回顧過去的決策。
> - 研究人員用它來記錄實驗過程和結果，因為它能夠持久保存結構化事實，並在需要時快速檢索。

## 架構分析

Memory OS 的架構由七層組成，每一層都有其特定的功能，從工作空間到向量資料庫，確保記憶的捕捉、存儲和使用。第一層是工作空間，通過 MEMORY.md 和 USER.md 文件來管理用戶的基本信息；第二層是會話記憶，使用 SQLite 和 FTS5 進行全文搜索；第三層是結構化事實，結合信任評分來管理持久的事實存儲；第四層是跨會話的記憶提取，通過 Icarus 插件進行多來源注入；第五層是 Qdrant 向量資料庫，提供多層次的檢索機制；第六層是自動化維基，持續更新知識庫；第七層是關鍵的 Ground Truth 層，確保注入的記憶被正確使用。這種設計使得記憶的管理更加高效，但也增加了系統的複雜性，對於新手來說可能會有一定的學習曲線。

## 技術深入分析

Memory OS 的核心技術機制是其七層記憶架構，這包括了工作空間、會話記憶、結構化事實、跨會話回憶、向量資料庫、自動化維基和 Ground Truth 層。這種設計使得記憶的捕捉和存儲不僅高效，還能夠根據用戶的需求進行智能化的上下文注入。效能方面，系統能夠處理大量的對話數據，並且通過 Qdrant 的向量檢索機制實現快速的記憶檢索。設計取捨方面，選擇使用 Python 和 Docker 使得系統能夠在多平台上運行，但也帶來了對 Docker 環境的依賴。技術風險方面，隨著數據量的增加，可能會面臨性能瓶頸，特別是在記憶檢索和存儲的效率上。整合方面，Memory OS 可以與多種 LLM 提供者兼容，並且能夠輕鬆整合到現有的開發環境中，這使得它在使用上非常靈活。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了詳細的安裝指引和使用說明，對於新手來說相對友好。安裝過程需要 Docker 環境，對於不熟悉的用戶可能會有一定的挑戰，但整體安裝流程是順暢的。文檔中有清晰的範例和指令，幫助用戶快速上手。

## 優缺點分析

> [!success] 優點
> - 完全本地化的記憶基礎設施，保護用戶數據隱私。
> - 智能上下文注入功能，提升 AI 的上下文理解能力。
> - 結構化事實與信任評分，讓記憶管理更具智能化。
...
> - 目前僅支持 Python 3.11 以上版本，舊版本的 Python 可能無法運行。
> - 對於大型數據集的處理性能可能受到硬體限制，建議使用較新型的機器。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| mem0 | mem0 是一個雲端記憶解決方案，無法提供本地化的記憶基礎設施，適合需要快速部署的場景。 |
| Zep | Zep 主要針對雲端用戶設計，缺乏本地運行的靈活性，適合不在意數據隱私的用戶。 |
| Letta | Letta 也屬於雲端解決方案，無法提供結構化事實和信任評分功能，適合簡單的記憶需求。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| mem0 | mem0 是一個雲端記憶解決方案，無法提供本地化的記憶基礎設施。 | 如果你的團隊需要快速部署且不在意數據隱私，mem0 可能是更合適的選擇。 | medium，因為需要將數據從本地環境遷移到雲端。 |
| Zep | Zep 主要針對雲端用戶設計，缺乏本地運行的靈活性。 | 如果你的團隊已經在使用 Zep 的生態系統，則選擇 Zep 會更方便。 | high，因為需要重構應用架構以適應雲端環境。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **mem0** | **Zep** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | mem0 是一個雲端記憶解決方案，無法提供本地化的記憶基礎設施。 | Zep 主要針對雲端用戶設計，缺乏本地運行的靈活性。 |
> | 遷移成本 | - | medium，因為需要將數據從本地環境遷移到雲端。 | high，因為需要重構應用架構以適應雲端環境。 |
> | 適用場景 | 主要場景 | 如果你的團隊需要快速部署且不在意數據隱私，mem0 可能是更 | 如果你的團隊已經在使用 Zep 的生態系統，則選擇 Zep  |
...
- **[HIGH]** Docker 環境配置不當可能導致安裝失敗
  - 解法：確保 Docker 已正確安裝並運行
- [MEDIUM] 在大型數據集上性能可能下降
  - 解法：考慮使用更高效的硬體配置
- [MEDIUM] 與某些 LLM 提供者的兼容性問題
  - 解法：查閱文檔以獲取最新的兼容性列表
...
| 需要長期記憶的 AI 開發者 | 非常適合 | 具備持久記憶功能，適合長期項目管理。 |
| 大型企業的 AI 部門 | 普通 | 雖然功能強大，但可能需要更多的硬體資源來支持。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，10 小時整合，得到一個強大的本地化記憶系統，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：本地化運行，不需要高權限，且不存取敏感資料，適合在 CI/CD pipeline 中使用。
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["RAG","向量資料庫","自動化","持久記憶","上下文注入"];
