...
use_case: "提供持久記憶的本地操作系統，讓 Hermes Agent 能夠記住過去的對話和決策。"
priority: high
ring: assess
...
star_history: "2026-06-04:756,2026-06-04:757,2026-06-05:828,2026-06-05:828,2026-06-06:882,2026-06-06:882,2026-06-07:951,2026-06-07:951,2026-06-08:1011"
tags:
  - github
  - "category/ai_ml"
  - "lang/python"
...
aliases:
  ...
  - "提供持久記憶的本地操作系統，讓 Hermes Agent 能夠記住過去的對話和決策。"
---

# memory-os

**1.0k** stars · **144** stars/天 · 建立 7 天前 · Python · MIT
...
> **適合** 需要在本地運行長期記憶系統並希望保持上下文一致性的專業用戶。
> **一句話重點** Memory OS 不僅是一個插件，而是一個完整的記憶操作系統，能夠讓 Hermes Agent 成為真正的長期合作夥伴。
...
> **結論** 花 5 小時學習，3 小時整合，得到高效的長期記憶管理，值得投入。

> [!abstract] 核心創新
> 提供一個完整的七層記憶操作系統，確保代理人能夠有效使用注入的記憶。

## 專案簡介

Memory OS 是一個針對 Hermes Agent 設計的七層記憶操作系統，旨在解決代理人經常忘記過去對話和決策的問題。用戶可以在本地運行這個系統，無需雲端服務，並支持多種 LLM 提供者。核心機制包括將記憶分為七層，從工作區到結構化事實，再到向量數據庫，每層都有其特定的功能。特別是第七層的 Ground Truth 層，確保注入的記憶被代理人視為權威，避免了重複查詢和資源浪費。安裝過程簡單，只需一條命令即可設置整個堆棧，並且支持 Docker 環境。

這個系統的設計使得記憶的捕捉、存儲和注入變得高效，並且能夠自動生成和維護一個知識庫。與傳統的 Hermes 系統相比，Memory OS 提供了更高效的跨會話回憶和結構化事實存儲，並且具備智能的上下文注入功能。這使得用戶在與代理人互動時，能夠獲得更連貫的對話體驗。這個工具特別適合需要長期記憶的專業用戶，如研究人員或開發者，他們需要在多次會話中保持上下文一致性。

**技術棧**：`Python 3.11` · `Docker` · `Redis` · `Qdrant`

## 重點功能

- 七層記憶架構 — 包含工作區、會話、結構化事實、跨會話回憶、向量數據庫、自動維護的知識庫和權威記憶層。
- 本地運行 — 完全在用戶的機器上運行，無需雲端服務，保護用戶數據隱私。
- 智能上下文注入 — 能夠自動從多個來源提取相關上下文，並在每次對話中注入。
- 結構化事實存儲 — 提供事實的信任評分和自動反饋循環，確保記憶的準確性。
- 一鍵安裝 — 使用簡單的命令即可設置整個系統，降低了使用門檻。
...
> [!tip] 爆紅原因
> 建立 7 天內累積 1011 stars（144/天），forks 95（9.4%），顯示出強勁的增長潛力。作者 ClaudioDrews 之前在 AI 記憶領域有多項貢獻，這次專案解決了許多用戶在使用 Hermes 時遇到的記憶遺失問題，特別是對於需要長期記憶的應用場景。社群的活躍度和問題解決率也顯示出該專案的穩定性和可用性。此外，簡化的安裝過程和本地運行的特性使得它在當前的 AI 記憶解決方案中脫穎而出。

## 適合誰使用

**目標受眾**：需要在本地運行長期記憶系統並希望保持上下文一致性的專業用戶。

> [!example] 使用場景
> - 研究人員用它來持續記錄和回顧實驗數據，因為它能夠自動整理和回憶過去的討論和決策。
> - 開發者用它來管理多個專案的上下文，因為它的智能上下文注入功能能夠減少重複工作。
> - 產品經理用它來記錄用戶反饋和需求變更，因為它能夠提供結構化的事實存儲和回憶功能。

## 架構分析

Memory OS 的架構由七層組成，每層專注於不同的記憶功能。第一層為工作區，存儲用戶的基本信息；第二層為會話記憶，使用 SQLite 和 FTS5 實現全文搜索；第三層為結構化事實，提供信任評分和自動反饋；第四層為跨會話的 Fabric 層，支持多來源的注入；第五層為向量數據庫，使用 Qdrant 進行高效檢索；第六層為自動維護的知識庫；第七層的 Ground Truth 層確保注入的記憶被視為權威。這種設計使得用戶能夠在每次對話中獲得準確的上下文，避免了重複查詢的資源浪費。整體架構的選擇使得系統能夠在本地運行，並且具備良好的擴展性。

## 技術深入分析

Memory OS 的核心技術機制是其七層記憶架構，這使得記憶的捕捉、存儲和注入變得高效。每一層都有其特定的功能，從基本的工作區到複雜的向量數據庫，這種設計使得用戶能夠在多次會話中保持上下文一致性。效能方面，系統能夠處理大量的會話記錄，並且使用 Qdrant 進行高效的向量檢索。設計上選擇了 SQLite 和 Redis 作為數據存儲解決方案，這使得系統在本地運行時具備良好的性能和擴展性。技術風險方面，對於大規模使用者，可能會面臨資源管理和性能瓶頸的挑戰。整合方面，與現有的 LLM 提供者的兼容性良好，但對於新用戶來說，理解整個架構可能需要一些時間。整體而言，Memory OS 提供了一個創新的解決方案，讓用戶能夠在本地環境中實現高效的長期記憶管理。

## 新手體驗

> [!info] 上手難度評估
> README 文件詳細說明了安裝步驟和系統架構，並提供了範例命令。安裝過程相對順暢，但需要用戶具備 Docker 的基本知識。文件目前僅提供英文版本，可能對非英語用戶造成一定障礙。
...
> - 本地運行，保護用戶數據隱私。
> - 智能上下文注入，提升對話的連貫性。
> - 結構化事實存儲，確保記憶的準確性和可靠性。

> [!danger] 缺點
> - 僅限於 Hermes Agent，其他代理人無法使用。
> - 需要較高的系統資源來運行 Docker 環境。
> - 對於新手用戶，可能需要一些學習曲線來理解七層架構。

> [!warning] 注意事項
> - 僅支援 Python 3.11+。
> - 需要 Docker 環境來運行 Qdrant 和 Redis。
> - 目前僅針對 Hermes Agent 設計，其他代理人可能無法直接使用。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於多代理協作，而 Memory OS 專注於單一代理的長期記憶管理。 |
| [mem0/mem0](https://github.com/mem0/mem0) | mem0 是雲端解決方案，而 Memory OS 提供本地運行的選擇，適合對隱私有更高要求的用戶。 |
| [Zep/zep](https://github.com/Zep/zep) | Zep 提供的記憶功能較為淺顯，無法實現 Memory OS 的智能上下文注入和結構化事實存儲。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [mem0/mem0](https://github.com/mem0/mem0) | mem0 是一個雲端解決方案，專注於提供記憶功能，但不支持本地運行。 | 如果你的團隊已經在使用雲端服務並希望利用其記憶功能，可以考慮 mem0。 | medium，因為需要重新設計數據流和存儲架構。 |
| [Zep/zep](https://github.com/Zep/zep) | Zep 提供的記憶功能較為淺顯，無法實現 Memory OS 的智能上下文注入和結構化事實存儲。 | 如果你的需求較為簡單，只需基本的記憶功能，Zep 可能更適合。 | low，因為其架構較為簡單，易於上手。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **mem0** | **zep** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | mem0 是一個雲端解決方案，專注於提供記憶功能，但不支持本地運行。 | Zep 提供的記憶功能較為淺顯，無法實現 Memory OS 的智能上下文注入和結構化事實存儲。 |
> | 遷移成本 | - | medium，因為需要重新設計數據流和存儲架構。 | low，因為其架構較為簡單，易於上手。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用雲端服務並希望利用其記憶功能，可以考慮 | 如果你的需求較為簡單，只需基本的記憶功能，Zep 可能更適合 |
...
- **[HIGH]** 在某些環境下，Docker 啟動可能會遇到資源不足的問題。
  - 解法：確保系統有足夠的內存和 CPU 資源。
- [MEDIUM] 某些 LLM 提供者的兼容性問題，可能導致功能不全。
  - 解法：檢查官方文檔以獲取最新的兼容性列表。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 能夠在本地運行，保護數據隱私，並提供長期記憶功能。 |
| 大型企業的多團隊協作 | 普通 | 雖然具備強大的記憶功能，但可能面臨資源管理挑戰。 |
| 需要快速原型開發的個人開發者 | 非常適合 | 簡單的安裝過程和本地運行特性使其易於上手。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，3 小時整合，得到高效的長期記憶管理，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：本地運行的系統不需要高權限，且不會存取敏感資料，依賴鏈的信任程度良好。
