...
use_case: "為 Hermes Agent 提供持久記憶的七層記憶操作系統，支持本地運行並兼容多種 LLM 提供者。"
priority: high
ring: assess
...
aliases:
  ...
  - "為 Hermes Agent 提供持久記憶的七層記憶操作系統，支持本地運行並兼容多種 LLM 提供者。"
---

# memory-os

**1.2k** stars · **54** stars/天 · 建立 22 天前 · Python · MIT
...
> **授權** MIT (商業友好) · **維護** Moderate (最後推送 12 天前) · **貢獻者** 4+ 人 · **參與度** Low
> **適合** 需要在本地環境中運行長期記憶系統的開發者，並希望避免雲端依賴的團隊。
> **一句話重點** Memory OS 的創新在於其七層架構和真實性層級，這使得記憶的管理和使用達到前所未有的靈活性和效率。
...
> [!abstract] 核心創新
> Memory OS 提供了一個完整的七層記憶架構，並引入了明確的真實性層級，確保代理能夠有效利用注入的記憶。

## 專案簡介

Memory OS 是一個專為 Hermes Agent 設計的七層記憶操作系統，旨在解決代理在會話中遺忘上下文的問題。它的核心機制是將記憶分為七層，從工作區到結構化事實，並且能夠智能地注入上下文。用戶可以透過 `curl -sSL https://raw.githubusercontent.com/ClaudioDrews/memory-os/main/setup.sh | bash` 一鍵安裝整個堆疊，這樣的設計讓使用者無需擔心複雜的安裝過程。技術上，Memory OS 使用了 Qdrant 作為向量數據庫，並結合 Redis 和 ARQ Worker 來實現高效的記憶存取。這種架構不僅支持本地運行，還能與任何 LLM 提供者兼容，避免了雲端記憶的依賴。

與其他記憶解決方案相比，Memory OS 提供了結構化事實和信任評分，並且具備自我整理的維基功能，這使得用戶能夠在多次會話中持續追蹤和回憶重要信息。這些特性讓它在處理長期記憶時比其他工具如 mem0 和 Zep 更具優勢，特別是在需要私有化和本地化的情境下。使用者可能會遇到的問題包括 Docker 安裝的兼容性和性能瓶頸，但社群的活躍度和持續的更新能夠幫助解決這些問題。總體來看，這是一個適合需要長期記憶的開發者的成熟工具，未來可能會進一步增強其社群支持和功能擴展。

**技術棧**：`Python 3.11` · `Docker` · `Redis` · `Qdrant`

## 重點功能

- 七層記憶架構 — 包含工作區、會話、結構化事實等，支持多層次的記憶管理。
- 智能上下文注入 — 能夠根據會話歷史自動注入相關上下文，提升交互效率。
- 本地運行 — 完全在用戶本地機器上運行，無需雲端記憶訂閱。
- 兼容多種 LLM 提供者 — 支持 OpenRouter、OpenAI、Anthropic 等多種模型。
- 自我整理的維基功能 — 自動整理和更新知識庫，提升信息檢索效率。
...
> [!tip] 爆紅原因
> 建立 22 天就累積 1187 stars（54/天），forks 112（9.4%），這顯示出穩定的增長。作者 ClaudioDrews 之前在記憶解決方案方面有豐富的經驗，這個專案解決了許多現有工具的痛點，例如缺乏本地存儲和靈活性。雖然沒有明確的觸發事件，但其設計理念和功能組合吸引了許多開發者的注意。技術上，隨著本地運行和多提供者兼容性的需求上升，這個工具的可行性大幅提升。forks/stars 比率為 9.4%，顯示出有相當比例的用戶在積極修改和使用這個專案。

## 適合誰使用

**目標受眾**：需要在本地環境中運行長期記憶系統的開發者，並希望避免雲端依賴的團隊。

> [!example] 使用場景
> - AI 研究員用它來持久化實驗結果和上下文，因為這樣能夠在不同實驗之間保持一致性，避免重複工作。
> - 產品經理用它來記錄會議決策和項目進度，因為這樣能夠隨時回顧過去的討論，提升團隊協作效率。
> - 開發者用它來管理代碼變更和技術文檔，因為這樣能夠在開發過程中快速查找相關資訊，減少查詢時間。

## 架構分析

Memory OS 的架構由七層組成，每一層都專注於不同的記憶管理任務。第一層處理工作區的基本記錄，第二層使用 SQLite 和 FTS5 進行會話的全文檢索，第三層則是結構化事實的存儲，並引入信任評分機制。這種設計使得記憶的存取和使用效率大幅提升。

特別是第七層的真實性層級，確保了注入的記憶被代理視為權威，這是其他工具所缺乏的。整體架構的設計考量了本地運行的需求，避免了雲端依賴，並且支持多種 LLM 提供者。這樣的設計雖然在初期需要一定的資源配置，但長期來看能夠提供更高的靈活性和安全性。

## 技術深入分析

Memory OS 的核心技術機制是其七層記憶架構，這包括了從基本的工作區到結構化事實的多層次管理。每一層都使用不同的技術來優化記憶的捕捉和使用，例如使用 SQLite 和 FTS5 來實現高效的會話檢索。效能方面，這個系統能夠處理大量的對話記錄，並且通過 Qdrant 的向量檢索技術來加速信息的查找。設計上，選擇 Python 和 Docker 作為核心技術棧，這樣的選擇使得系統能夠在多種環境中靈活運行，並且降低了維護的複雜度。然而，這也帶來了對 Docker 環境的依賴，可能會對某些用戶造成困擾。技術風險方面，隨著用戶數量的增加，系統的性能可能會受到挑戰，特別是在高並發的情況下。整合方面，Memory OS 能夠與主流的 LLM 提供者無縫對接，並且在 CI/CD pipeline 中的整合也相對簡單，這使得它在開發流程中具有良好的適應性。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且詳細，提供了安裝和使用的步驟，並且有一鍵安裝的選項。安裝過程相對順暢，但對於不熟悉 Docker 的用戶可能會有一些挑戰。文件中沒有多語言支持，但整體的指導性強，適合新手快速上手。

## 優缺點分析

> [!success] 優點
> - 提供完整的本地記憶解決方案，無需依賴雲端服務。
> - 支持多種 LLM 提供者，靈活性高。
> - 智能上下文注入和自我整理的維基功能提升了使用體驗。
...
| 工具 | 差異 |
| --- | --- |
| mem0 | mem0 是一個雲端記憶解決方案，無法提供本地運行的選項，且需要訂閱費用。 |
| Zep | Zep 提供類似的記憶功能，但缺乏自我整理的維基和結構化事實的支持。 |
| Letta | Letta 主要集中於雲端解決方案，無法滿足需要本地存儲的用戶需求。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| mem0 | mem0 是一個雲端記憶解決方案，無法提供本地運行的選項，且需要訂閱費用。 | 如果你的團隊已經在使用雲端解決方案，並且不需要本地存儲的話，可以考慮 mem0。 | medium，因為需要重新設計記憶管理流程。 |
| Zep | Zep 提供類似的記憶功能，但缺乏自我整理的維基和結構化事實的支持。 | 如果你需要一個簡單的記憶解決方案，且不需要複雜的結構化功能，可以選擇 Zep。 | low，因為功能相似，遷移成本較低。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **mem0** | **Zep** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | mem0 是一個雲端記憶解決方案，無法提供本地運行的選項，且需要訂閱費用。 | Zep 提供類似的記憶功能，但缺乏自我整理的維基和結構化事實的支持。 |
> | 遷移成本 | - | medium，因為需要重新設計記憶管理流程。 | low，因為功能相似，遷移成本較低。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用雲端解決方案，並且不需要本地存儲的話， | 如果你需要一個簡單的記憶解決方案，且不需要複雜的結構化功能， |
...
- [MEDIUM] 在某些低配置機器上，Docker 啟動時間較長，影響使用體驗。
  - 解法：使用更高配置的機器或優化 Docker 設定。
- **[HIGH]** 某些 LLM 提供者的兼容性問題，可能導致功能不全。
  - 解法：檢查官方文檔以確認支持的 LLM 列表。
...
| 大型企業的長期記憶管理 | 普通 | 雖然功能強大，但可能需要更高的資源配置。 |
| 個人開發者的 AI 實驗 | 非常適合 | 能夠快速設置並提供靈活的記憶管理。 |
...
> [!warning] 安全性快速掃描
> 低風險：本地運行，不存取敏感資料，無需高權限操作，依賴鏈相對簡單，適合在 CI/CD pipeline 中使用。
...
**社群活躍度**：社群活躍，持續更新和修復問題，已經有外部貢獻者參與。
