...
use_case: "為 Hermes Agent 提供持久記憶的 7 層記憶作業系統，支持本地運行和多種 LLM 提供者。"
priority: high
ring: assess
...
aliases:
  ...
  - "為 Hermes Agent 提供持久記憶的 7 層記憶作業系統，支持本地運行和多種 LLM 提供者。"
---

# memory-os

**1.3k** stars · **26** stars/天 · 建立 50 天前 · Python · MIT
...
> **授權** MIT (商業友好) · **維護** Slow (最後推送 40 天前) · **貢獻者** 4+ 人 · **參與度** Low
> **適合** 希望在本地運行持久記憶系統的 AI 開發者和研究人員。
> **一句話重點** Memory OS 讓 AI 代理擁有持久記憶，能夠在本地運行，避免了雲端依賴。
...
> **結論** 花 8 小時學習，4 小時整合，得到靈活的本地記憶系統，值得嘗試。

> [!abstract] 核心創新
> 提供本地運行的持久記憶系統，無需雲端依賴。

## 專案簡介

Memory OS 是一個針對 Hermes Agent 的記憶作業系統，旨在解決 AI 代理的記憶問題。它通過七層記憶架構來捕捉、存儲和注入記憶，最終實現智能的上下文注入。用戶可以在本地運行這個系統，並與多種 LLM 提供者（如 OpenAI、Anthropic 等）兼容，避免了雲端記憶的依賴。關鍵指令如 `docker-compose up` 可以快速啟動整個系統，讓用戶無需擔心記憶訂閱或供應商鎖定。這個系統的賣點在於其持久性和靈活性，能夠在本地構建完整的記憶基礎設施。技術上，它使用 Qdrant 作為向量資料庫，並結合 Redis 來管理記憶的存取，這樣的設計使得記憶的檢索和更新都能高效進行。

與傳統的 Hermes 系統相比，Memory OS 提供了更為強大的跨會話回憶和結構化事實的支持，並且具備自我整理的維基功能。這些特性使得它在需要長期記憶的應用場景中表現更佳，特別是對於需要持續記錄和回顧的項目管理和決策支持。使用者可能會遇到的問題包括 Docker 環境的配置和依賴管理，但整體來說，這個系統的設計能夠有效減少記憶的重複檢索和資源浪費。這個專案目前處於活躍開發中，未來可能會進一步增強其功能和穩定性，特別是在社群的支持和貢獻方面。對於希望在本地構建 AI 記憶系統的開發者來說，Memory OS 是一個值得考慮的選擇。

**技術棧**：`Python` · `Docker` · `Redis` · `Qdrant`

## 重點功能

- 七層記憶架構 — 提供全面的記憶捕捉、存儲和注入功能。
- 本地運行 — 完全在用戶機器上運行，無需雲端依賴。
- 與多種 LLM 兼容 — 支持 OpenAI、Anthropic 等多種模型。
- 智能上下文注入 — 自動將相關記憶注入對話中，提升交互質量。
- 自我整理維基 — 自動生成和維護知識庫，便於查詢和回顧。
- Qdrant 向量資料庫 — 支持高效的向量檢索和多層次回憶。
- 信任評分系統 — 針對結構化事實提供信任評分，提升信息的可靠性。
...
> [!tip] 爆紅原因
> 建立 50 天內累積 1289 stars（約 26 stars/天），forks 數量為 120（9.3%），顯示出良好的社群參與度。作者 ClaudioDrews 在 AI 記憶領域有豐富經驗，此專案解決了現有雲端記憶方案的隱私和靈活性問題，讓用戶能夠在本地運行記憶系統。沒有明確的觸發事件，但隨著 AI 應用的普及，對於本地記憶解決方案的需求日益增加。這個工具的 forks/stars 比率顯示出使用者對其進行修改和擴展的興趣，這通常代表著活躍的開發社群。

## 適合誰使用

**目標受眾**：希望在本地運行持久記憶系統的 AI 開發者和研究人員。

> [!example] 使用場景
> - AI 研究人員用它來持久記錄實驗結果，因為這樣可以避免重複實驗並提高效率。
> - 產品經理用它來追蹤會議記錄和決策過程，因為它能夠自動整理和回顧歷史資訊。
> - 開發者用它來管理代碼變更和版本歷史，因為它提供了結構化的事實存儲和回憶功能。

## 架構分析

Memory OS 採用七層架構設計，確保記憶的捕捉、存儲和注入。每一層都有明確的功能，例如使用 Qdrant 作為向量資料庫來支持高效的檢索，而 Redis 則用於管理記憶的狀態和存取。這樣的設計使得整個系統可以在本地運行，避免了雲端的依賴，並且提供了靈活的擴展性。

選擇本地運行的架構雖然增加了初期的配置複雜度，但卻能保障用戶的數據隱私和安全。擴展性方面，系統的性能在處理大量資料時可能會受到硬體資源的限制，尤其是在高並發的情況下。整體而言，這種架構設計使得 Memory OS 在記憶管理上具備了強大的功能和靈活性。

## 技術深入分析

Memory OS 的核心技術機制在於其七層記憶架構，這使得記憶的捕捉、存儲和注入變得高效且智能。系統使用 Qdrant 作為向量資料庫，這不僅支持高效的檢索，還能夠處理複雜的查詢。整個系統的性能在於其本地運行的特性，這意味著用戶可以在不依賴雲端的情況下，獲得持久的記憶支持。設計上，選擇使用 Redis 來管理記憶的狀態，這樣可以快速存取和更新記憶，並且支持高並發的請求。

這種設計雖然增加了初期的配置複雜度，但卻能夠在長期使用中提供更好的性能和安全性。技術風險方面，隨著資料量的增加，系統可能會面臨性能瓶頸，特別是在高並發的情況下。整合方面，Memory OS 可以輕鬆與主流的 LLM 提供者整合，並且支持 CI/CD 流程的友好整合，使得開發者能夠快速部署和測試。整體而言，這個系統在記憶管理上具備了強大的功能和靈活性，適合需要長期記憶的應用場景。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且詳細，提供了良好的使用範例。安裝過程中需要配置 Docker，對新手來說可能有一定的挑戰。文件中有 getting started guide，幫助用戶快速上手。文件目前僅提供英文，對於非英語使用者可能會造成障礙。
...
> - 需要 Docker 環境支持。
> - 對於大型資料集，可能需要較高的系統資源。
> - 目前僅支持 Python 3.8 以上版本。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供類似的記憶管理功能，但主要依賴雲端架構，無法實現本地運行。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於簡單的記憶存儲，但缺乏智能上下文注入和自我整理功能。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| mem0 | mem0 主要依賴雲端架構，無法實現本地運行，這使得其在數據隱私方面存在風險。 | 如果你的團隊已經在使用雲端解決方案，並且不需要本地運行的靈活性。 | medium，因為需要重新配置和適應雲端環境。 |
| Zep | Zep 提供類似的記憶功能，但主要針對特定的雲端平台，缺乏本地運行的選項。 | 如果你的應用場景已經完全依賴於某個雲端平台，並且不需要本地存儲。 | high，因為需要重新設計整個記憶架構以適應雲端。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **mem0** | **Zep** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | mem0 主要依賴雲端架構，無法實現本地運行，這使得其在數據隱私方面存在風險。 | Zep 提供類似的記憶功能，但主要針對特定的雲端平台，缺乏本地運行的選項。 |
> | 遷移成本 | - | medium，因為需要重新配置和適應雲端環境。 | high，因為需要重新設計整個記憶架構以適應雲端。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用雲端解決方案，並且不需要本地運行的靈活 | 如果你的應用場景已經完全依賴於某個雲端平台，並且不需要本地存 |
...
- **[HIGH]** Docker 環境配置不當可能導致啟動失敗。
  - 解法：確保 Docker 和 Docker Compose 正確安裝，並檢查配置文件。
- [MEDIUM] 在高並發情況下，系統性能可能下降。
  - 解法：考慮擴展硬體資源或優化記憶檢索邏輯。
- [low] 文檔中缺乏中文支持，對非英語使用者不友好。
  - 解法：尋找社群翻譯或使用翻譯工具。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 AI 系統 | 非常適合 | 本地運行的特性能夠保障數據隱私，並且支持靈活的 LLM 整合。 |
| 大型企業的雲端 AI 解決方案 | 不適合 | 缺乏雲端支持，無法滿足大型企業的需求。 |
| AI 研究團隊需要長期記憶的實驗記錄 | 適合 | 提供持久記憶和智能上下文注入，能夠有效支持研究工作。 |
| 個人開發者的 AI 助手 | 適合 | 本地運行的特性和靈活性使其成為個人項目的理想選擇。 |
...
> [!tip] 投入 vs 回報
> 花 8 小時學習，4 小時整合，得到靈活的本地記憶系統，值得嘗試。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：本地運行的系統不需要高權限，並且不會存取敏感資料，依賴鏈的信任程度良好。
