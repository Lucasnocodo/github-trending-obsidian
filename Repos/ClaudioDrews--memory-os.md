...
use_case: "為 Hermes Agent 提供持久記憶的操作系統，讓它不再忘記重要資訊。"
priority: high
ring: assess
...
aliases:
  ...
  - "為 Hermes Agent 提供持久記憶的操作系統，讓它不再忘記重要資訊。"
---

# memory-os

**1.3k** stars · **32** stars/天 · 建立 39 天前 · Python · MIT
...
> **授權** MIT (商業友好) · **維護** Moderate (最後推送 29 天前) · **貢獻者** 4+ 人 · **參與度** Low
> **適合** 需要在本地運行記憶系統以支持長期合作的開發者和團隊。
> **一句話重點** Memory OS 不僅是一個記憶插件，而是一個完整的記憶操作系統，解決了現有解決方案的多個痛點。
...
> **結論** 花 10 小時學、5 小時整合，得到高效的本地記憶系統，值得投入。

> [!abstract] 核心創新
> Memory OS 提供了一個完整的七層記憶架構，並且通過真實性層確保注入的記憶被有效利用。

## 專案簡介

Memory OS 是一個為 Hermes Agent 設計的七層記憶操作系統，旨在解決代理在會話間遺忘重要資訊的問題。它的核心機制是將記憶分為七層，從工作區到結構化事實，再到跨會話的回憶，並最終通過一個明確的真實性層來確保注入的記憶被有效利用。用戶可以透過簡單的命令 `curl -sSL https://raw.githubusercontent.com/ClaudioDrews/memory-os/main/setup.sh | bash` 安裝整個系統，這樣的設計讓使用者無需擔心複雜的配置。技術上，Memory OS 使用 Qdrant 作為向量數據庫，並結合 Redis 來管理會話和事實存儲，這樣的架構使得記憶的檢索和存儲效率極高，並且支持多種 LLM 提供者。

與其他記憶解決方案相比，如 mem0 和 Zep，Memory OS 提供本地化的記憶基礎設施，避免了雲端依賴和訂閱費用。實際使用中，這個系統能夠處理大量的對話記錄，並且支持自動化的知識庫更新，這對於需要長期記憶的應用場景非常有用。儘管如此，使用者需要注意的是，這個系統的設計仍然需要一定的技術背景來進行有效的配置和維護。整體來看，Memory OS 是一個穩定的解決方案，適合需要長期記憶的開發者和團隊使用。

**技術棧**：`Python 3.11` · `Docker` · `Redis` · `Qdrant`

## 重點功能

- 七層記憶架構 — 包含工作區、會話、結構化事實等層級，確保記憶的有效捕捉和使用。
- 本地化記憶基礎設施 — 無需雲端服務，所有數據都保存在本地，保障隱私。
- 智能上下文注入 — 自動將相關記憶注入到會話中，提升交互的連貫性。
- 自動化知識庫 — 持續更新的自我策展維基，方便用戶檢索和管理知識。
- 支持多種 LLM 提供者 — 兼容 OpenRouter、OpenAI、Anthropic 等多個模型，靈活性高。
...
> [!tip] 爆紅原因
> 建立 39 天內累積 1250 stars（32/天），forks 119（9.5%），顯示出強烈的社群興趣。作者 ClaudioDrews 之前在 AI 記憶領域有多個貢獻，這個專案解決了現有記憶解決方案的雲端依賴和隱私問題，提供了本地化的解決方案。近期的推廣活動和社群互動也促進了其快速增長。這個工具的出現正好滿足了對於私有記憶基礎設施的需求，尤其是在數據隱私日益受到重視的情況下。forks/stars 比率為 9.5%，顯示出許多用戶對於修改和擴展的興趣。

## 適合誰使用

**目標受眾**：需要在本地運行記憶系統以支持長期合作的開發者和團隊。

> [!example] 使用場景
> - AI 開發者用它來構建一個能夠持續記憶用戶偏好的智能助手，因為它能在本地運行並避免雲端依賴。
> - 產品經理用它來記錄和回顧會議記錄，因為它能夠自動整理和檢索過去的對話。
> - 研究人員用它來管理和回顧實驗數據，因為它的結構化事實存儲能夠提高數據檢索效率。

## 架構分析

Memory OS 的架構由七層組成，從工作區到結構化事實，並最終通過真實性層來確保注入的記憶被有效使用。這樣的設計使得系統能夠在本地運行，避免了雲端依賴的問題。每一層都有其特定的功能，例如使用 Qdrant 作為向量數據庫來進行高效的記憶檢索，並利用 Redis 管理會話和事實存儲。這樣的架構設計使得記憶的檢索和存儲效率極高，並且支持多種 LLM 提供者。選擇本地化架構的代價是需要用戶具備一定的技術背景來進行有效的配置和維護，這可能會對某些用戶造成困難。

## 技術深入分析

Memory OS 的核心技術機制在於其七層記憶架構，這使得不同類型的記憶可以被有效捕捉和使用。每一層都有其特定的功能，從工作區的基本記憶到結構化事實的存儲，再到跨會話的回憶，最終通過真實性層來確保注入的記憶被有效利用。這樣的設計使得系統能夠在本地運行，避免了雲端依賴的問題。效能方面，Memory OS 能夠處理大量的對話記錄，並且支持自動化的知識庫更新，這對於需要長期記憶的應用場景非常有用。選擇本地化架構的代價是需要用戶具備一定的技術背景來進行有效的配置和維護，這可能會對某些用戶造成困難。整體來看，這個系統在設計上考慮到了多種使用場景，並且提供了靈活的配置選項，這使得它在市場上具有競爭力。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了安裝步驟和範例。安裝過程相對順暢，但對於不熟悉 Docker 的用戶可能會有一定的學習曲線。文件目前僅提供英文版本，缺乏多語言支持。
...
> - 提供本地化記憶基礎設施，保障用戶隱私。
> - 智能上下文注入功能提升了交互的連貫性。
> - 自動化知識庫更新，方便用戶管理和檢索知識。

> [!danger] 缺點
> - 需要一定的技術背景來進行配置和維護。
> - 對於大型數據集的處理可能需要較高的硬體配置。
> - 安裝過程中可能遇到 Docker 的相關問題。

> [!warning] 注意事項
> - 僅支援 Python 3.11+
> - 需要 Docker 環境運行，對於不熟悉 Docker 的用戶可能有一定學習曲線。
> - 對於大型數據集的處理可能需要較高的硬體配置。
...
| 工具 | 差異 |
| --- | --- |
| mem0 | mem0 提供雲端記憶解決方案，但缺乏本地化支持，對於需要隱私的用戶不適合。 |
| Zep | Zep 主要針對雲端環境，無法提供本地記憶基礎設施，對於需要本地運行的用戶來說不合適。 |
| Letta | Letta 提供類似的功能，但不具備自動化知識庫更新的能力，使用上較為繁瑣。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| mem0 | mem0 提供雲端記憶解決方案，但缺乏本地化支持，對於需要隱私的用戶不適合。 | 如果你的團隊已經在使用雲端服務並且不擔心數據隱私問題，可以考慮 mem0。 | medium，因為需要重新配置記憶存儲和檢索邏輯。 |
| Zep | Zep 主要針對雲端環境，無法提供本地記憶基礎設施，對於需要本地運行的用戶來說不合適。 | 如果你的應用場景需要雲端協作，Zep 可能是合適的選擇。 | high，因為需要重構整個記憶管理系統。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **mem0** | **Zep** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | mem0 提供雲端記憶解決方案，但缺乏本地化支持，對於需要隱私的用戶不適合。 | Zep 主要針對雲端環境，無法提供本地記憶基礎設施，對於需要本地運行的用戶來說不合適。 |
> | 遷移成本 | - | medium，因為需要重新配置記憶存儲和檢索邏輯。 | high，因為需要重構整個記憶管理系統。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用雲端服務並且不擔心數據隱私問題，可以考 | 如果你的應用場景需要雲端協作，Zep 可能是合適的選擇。 |
...
- **[HIGH]** 安裝過程中可能遇到 Docker 的相關問題，特別是在不同作業系統上。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 本地化記憶基礎設施能夠有效支持小型團隊的需求。 |
| 需要長期記憶的 AI 開發者 | 非常適合 | 提供了智能上下文注入和自動化知識庫更新功能。 |
| 大型企業的記憶管理系統 | 不適合 | 對於大型數據集的處理可能需要更高的硬體配置。 |
...
> [!tip] 投入 vs 回報
> 花 10 小時學、5 小時整合，得到高效的本地記憶系統，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：本地運行的系統不需要高權限，且所有數據保存在本地，保障用戶隱私。
