...
**1.3k** stars · **24** stars/天 · 建立 54 天前 · Python · MIT
...
> **授權** MIT (商業友好) · **維護** Slow (最後推送 44 天前) · **貢獻者** 4+ 人 · **參與度** Low
> **適合** 需要在本地構建持久記憶系統的 AI 開發者和研究人員。
> **一句話重點** Memory OS 的設計不僅解決了記憶的持久性問題，還提供了智能的上下文管理，讓 AI 更像一位長期的合作夥伴。
...
> **結論** 花 5 小時學、10 小時整合，得到強大的本地記憶系統，值得投入。

> [!abstract] 核心創新
> 提供了完整的本地記憶基礎設施，並具備智能上下文注入和自動整理維基的功能。

## 專案簡介

Memory OS 是一個七層的記憶操作系統，專為 Hermes Agent 設計，能夠在本地運行並支持多種 LLM 提供者。使用者可以將其視為一個完整的記憶基礎設施，能夠捕捉、存儲並智能地注入上下文。核心流程是將每次對話的記憶層層疊加，並在需要時進行精確的上下文回憶，這樣使用者就不必在每次會話中重複背景資訊。最關鍵的指令是 `curl -sSL https://raw.githubusercontent.com/ClaudioDrews/memory-os/main/setup.sh | bash`，這一行能夠快速設置整個記憶系統。這個系統的設計考量了用戶對於隱私的需求，完全在本地運行，避免了雲端記憶的限制和訂閱費用。

相較於傳統的 Hermes，Memory OS 提供了結構化事實、跨會話回憶和自動整理的維基功能，這些都是其他解決方案無法提供的。這種設計使得使用者能夠在多次會話中持續獲得相關上下文，從而提升了 AI 的協作能力。整體架構中，Ground Truth 層確保了注入的記憶被有效利用，這一點是其他記憶解決方案所缺乏的。使用者可以期待在每次會話中獲得準確且有用的上下文，這樣的設計使得 AI 更像是一位長期的合作夥伴。

**技術棧**：`Python 3.11` · `Docker` · `Qdrant` · `Redis`

## 重點功能

- 七層記憶架構 — 包含工作區、會話、結構化事實、跨會話回憶、向量數據庫、LLM 維基和 Ground Truth 層。
- 本地運行 — 完全在本地環境中運行，無需雲端存儲，保護用戶數據隱私。
- 智能上下文注入 — 自動將相關上下文注入到每次對話中，提升 AI 的協作能力。
- 結構化事實存儲 — 提供事實存儲和信任評分功能，幫助用戶管理信息。
- 自動整理維基 — 具備自我整理的維基功能，持續更新和整理知識庫。
...
> [!tip] 爆紅原因
> 建立 54 天內累積 1294 stars（24/天），forks 120（9.3%），顯示出穩定的增長趨勢。作者 ClaudioDrews 之前在 AI 記憶領域有過多次實驗，這個專案解決了現有記憶解決方案的雲端依賴問題，提供了本地運行的選擇。這一需求在開發者社群中引起了廣泛的關注，特別是在對數據隱私日益重視的背景下。社群的活躍度也顯示出使用者對於這個工具的興趣，並且有持續的貢獻和問題反饋。forks/stars 比率為 9.3%，顯示出有相當一部分使用者在進行實際修改和使用。

## 適合誰使用

**目標受眾**：需要在本地構建持久記憶系統的 AI 開發者和研究人員。

> [!example] 使用場景
> - AI 開發者用它來構建本地的記憶系統，因為這樣可以避免雲端依賴並保護用戶數據隱私。
> - 產品經理用它來記錄和回顧過去的會議決策，因為它能夠自動整理和回憶相關上下文，提升工作效率。
> - 研究人員用它來管理實驗數據和結果，因為它的結構化事實和智能注入功能能夠幫助他們快速找到所需信息。

## 架構分析

Memory OS 採用七層架構設計，這樣的設計使得每一層都能專注於特定的記憶功能。第一層到第六層負責記憶的捕捉、存儲和注入，而第七層則確保這些記憶被有效利用。這樣的設計使得系統能夠在本地運行，避免了雲端依賴的問題。

資料流從用戶的對話進入系統，經過多層處理後，最終以結構化的形式存儲在本地數據庫中。選擇使用 Qdrant 作為向量數據庫，因為它能夠提供高效的相似度檢索，而 Redis 則用於快速存取和管理會話狀態。這樣的架構設計使得系統在性能上具備優勢，能夠快速響應用戶請求。

整體架構的擴展性良好，但在處理大量資料時可能會遇到性能瓶頸，特別是在記憶的清理和去重過程中。

## 技術深入分析

Memory OS 的核心技術機制在於其七層架構，這使得每一層都能專注於特定的記憶功能。使用 Qdrant 作為向量數據庫，能夠提供高效的相似度檢索，並利用 Redis 管理會話狀態。這樣的設計使得系統能夠在本地運行，避免了雲端依賴的問題。效能方面，系統能夠快速響應用戶請求，但在處理大量資料時可能會遇到性能瓶頸。

選擇 Python 作為主要開發語言，因為其生態系統豐富且易於擴展，這樣的選擇也降低了開發的複雜度。依賴樹相對簡單，主要依賴於幾個關鍵庫，這使得整體系統的維護成本較低。技術風險方面，隨著使用者數量的增加，系統的性能可能會受到影響，特別是在記憶的清理和去重過程中。整合方面，與主流框架的兼容性良好，能夠輕鬆融入現有的開發流程。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝指引和範例，安裝過程相對順暢，但對於新手來說可能會有一定的學習曲線。文件中有詳細的步驟說明，並且有針對常見問題的解答。整體而言，花 30 分鐘應該能夠成功運行起來。
...
> - 對於大型資料集的處理可能需要較高的系統資源。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供類似的本地記憶功能，但缺乏自動整理維基的能力，適合需要簡單記憶的使用者。 |
| mem0 | 雲端解決方案，無法提供本地運行的選擇，適合不介意數據隱私的使用者。 |
| Zep | 雲端記憶系統，功能較淺，無法提供深層的上下文回憶能力，適合小型專案。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| mem0 | mem0 是一個雲端記憶解決方案，無法提供本地運行的選擇，這使得它不適合對數據隱私有高要求的使用者。 | 如果你的團隊已經在使用雲端服務，並且不介意數據隱私問題，可以考慮使用 mem0。 | medium，因為需要重新設計數據流和存儲架構。 |
| Zep | Zep 主要針對簡單的記憶需求，功能較淺，無法提供深層的上下文回憶能力。 | 如果你的專案需求較簡單，且不需要複雜的上下文管理，可以考慮 Zep。 | low，因為其架構較為簡單，遷移成本低。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **mem0** | **Zep** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | mem0 是一個雲端記憶解決方案，無法提供本地運行的選擇，這使得它不適合對數據隱私有高要求的使用者。 | Zep 主要針對簡單的記憶需求，功能較淺，無法提供深層的上下文回憶能力。 |
> | 遷移成本 | - | medium，因為需要重新設計數據流和存儲架構。 | low，因為其架構較為簡單，遷移成本低。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用雲端服務，並且不介意數據隱私問題，可以 | 如果你的專案需求較簡單，且不需要複雜的上下文管理，可以考慮  |
...
- **[HIGH]** 在某些系統上安裝過程可能會遇到 Docker 版本不兼容的問題。
  - 解法：確保 Docker 更新到最新版本。
- [MEDIUM] 記憶清理過程中可能會導致性能下降。
  - 解法：定期監控系統性能，必要時調整清理策略。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 AI 系統 | 非常適合 | 本地運行的特性能夠保護數據隱私，且記憶功能能提升團隊協作。 |
| 大型企業的 AI 記憶系統 | 普通 | 雖然功能強大，但在處理大量資料時可能會遇到性能瓶頸。 |
| 個人開發者的 AI 助手 | 非常適合 | 本地運行且無需雲端訂閱，適合個人項目。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學、10 小時整合，得到強大的本地記憶系統，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：本地運行的系統不需要高權限，且不會存取敏感資料，依賴鏈中的庫也相對成熟，無已知的供應鏈風險。
