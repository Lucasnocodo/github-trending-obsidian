...
use_case: "為 Hermes Agent 提供持久記憶的操作系統，讓 AI 能夠記住過去的對話與決策。"
priority: high
ring: assess
...
star_history: "2026-06-04:756,2026-06-04:757,2026-06-05:828,2026-06-05:828,2026-06-06:882,2026-06-06:882,2026-06-07:951,2026-06-07:951,2026-06-08:1011,2026-06-09:1037"
tags:
  - github
  - "category/ai_ml"
  - "lang/python"
...
aliases:
  ...
  - "為 Hermes Agent 提供持久記憶的操作系統，讓 AI 能夠記住過去的對話與決策。"
---
...
> **適合** 需要在本地運行持久記憶系統以支持 AI 助手的獨立開發者或小型團隊。
> **一句話重點** Memory OS 的設計不僅解決了記憶管理的問題，還提供了用戶完全掌控數據的能力，這在當前的 AI 生態中是非常重要的。
...
> **結論** 花 5 小時學習，10 小時整合，得到一個靈活的本地記憶系統，值得嘗試。

> [!abstract] 核心創新
> Memory OS 提供了一個完整的七層記憶操作系統，能夠在本地運行並支持多種 LLM 提供者。

## 專案簡介

Memory OS 是一個為 Hermes Agent 設計的七層記憶操作系統，旨在解決 AI 在長期合作中忘記重要信息的問題。它的核心機制是通過將記憶分層管理，從工作區到結構化事實，再到跨會話的回憶，最終確保注入的記憶被有效使用。用戶可以透過 `curl -sSL https://raw.githubusercontent.com/ClaudioDrews/memory-os/main/setup.sh | bash` 指令一鍵安裝整個系統，無需雲端訂閱，並支持多種 LLM 提供者。技術上，Memory OS 結合了 Qdrant 和 Redis，利用全文本搜索和信任評分來管理結構化事實，並且提供自動化的維基知識管道。

這樣的設計使得用戶能夠在每次會話中獲得準確的上下文，避免重複的資訊搜尋，從而節省 token 和時間。相比於傳統的 Hermes，Memory OS 提供了更強大的記憶管理功能，包括智能的上下文注入和持久的記憶結構。與其他雲端記憶解決方案相比，Memory OS 的本地化設計使得用戶能夠完全掌控自己的數據，避免了資料外洩的風險。這個系統的成熟度已經相對穩定，適合小型團隊或個人開發者使用，但對於需要大規模部署的企業用戶，可能還需進一步的測試和優化。

**技術棧**：`Python 3.11` · `Docker` · `Qdrant` · `Redis`

## 重點功能

- 七層記憶架構 — 包含工作區、會話、結構化事實、跨會話回憶、向量數據庫、LLM 維基和地面真相層。
- 本地化記憶基礎架構 — 完全在用戶本地運行，無需雲端訂閱，支持多種 LLM 提供者。
- 智能上下文注入 — 自動從多個來源提取相關上下文，並確保其在每次會話中被有效使用。
- 結構化事實存儲 — 利用 Qdrant 和 Redis 管理事實，並提供信任評分和自動反饋迴路。
- 自動化維基知識管道 — 持續整理和更新知識庫，提升資訊的可用性和準確性。
...
> [!tip] 爆紅原因
> 建立 8 天就累積 1037 stars（130/天），forks 98（9.5%），顯示出強勁的增長潛力。作者 ClaudioDrews 在 AI 記憶領域有豐富的經驗，這個專案解決了現有記憶系統的雲端依賴問題，提供本地化的記憶解決方案，讓用戶能夠完全掌控自己的數據。近期的推廣活動和社群討論也為專案帶來了關注，尤其是針對記憶性能的基準測試和與非 Hermes 系統的兼容性問題。這些因素共同推動了專案的快速成長。

## 適合誰使用

**目標受眾**：需要在本地運行持久記憶系統以支持 AI 助手的獨立開發者或小型團隊。

> [!example] 使用場景
> - AI 開發者用它來構建能夠持續記憶用戶對話的智能助手，因為這樣可以減少重複的上下文輸入，提升用戶體驗。
> - 產品經理用它來記錄會議決策和項目進展，因為它能夠自動整理和回顧過去的會議記錄，避免信息遺失。
> - 研究人員用它來管理實驗數據和結果，因為它提供結構化的事實存儲和智能檢索功能，提升研究效率。

## 架構分析

Memory OS 的架構由七層組成，從工作區到結構化事實，再到跨會話的回憶，最上層的地面真相層確保注入的記憶被有效使用。這種設計使得用戶能夠在每次會話中獲得準確的上下文，避免重複的資訊搜尋。每一層都有其特定的功能，例如使用 Qdrant 進行向量搜索，利用 Redis 管理會話狀態，並通過自動化維基知識管道來整理和更新知識。這樣的架構設計使得系統在性能上具備高效能，能夠快速檢索和處理大量資料，但也需要一定的資源來支持 Docker 和相關服務的運行。整體而言，這種層次化的設計使得系統具備良好的擴展性和靈活性。

## 技術深入分析

Memory OS 的核心技術機制是其七層架構，這種設計使得記憶的捕獲、存儲和注入能夠高效運行。每一層都有其特定的功能，例如工作區層負責基本的用戶信息存儲，而結構化事實層則利用信任評分來管理事實的準確性。效能方面，系統能夠處理大量的會話數據，並且通過 Qdrant 的向量搜索技術，能夠快速檢索相關信息。設計上選擇了 Docker 作為容器化技術，這樣的選擇使得部署變得簡單，但也帶來了對資源的需求。技術風險方面，隨著用戶數量的增加，系統的性能可能會受到影響，特別是在高並發情況下。整合方面，Memory OS 與主流的 LLM 提供者兼容，但在與其他 AI 系統的整合上可能需要額外的適配工作。整體而言，這個系統在記憶管理和數據隱私方面提供了一個創新的解決方案。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了詳細的安裝步驟和使用範例。安裝過程相對順暢，但對於不熟悉 Docker 的用戶可能會有一些挑戰。文件目前僅提供英文版本，未來若能增加多語言支持將更有利於擴展用戶群。

## 優缺點分析

> [!success] 優點
> - 本地化記憶系統，無需雲端依賴，保障數據隱私。
> - 智能上下文注入，提升會話的連貫性和效率。
> - 結構化事實存儲，支持信任評分和自動反饋迴路。

> [!danger] 缺點
> - 需要 Docker 環境，對新手有一定的學習曲線。
> - 目前僅針對 Hermes Agent 設計，與其他系統的兼容性有限。
> - 安裝過程可能會遇到一些配置問題，需進行手動調整。

> [!warning] 注意事項
> - 僅支援 Python 3.11+。
> - 需要 Docker 環境運行，對於不熟悉 Docker 的用戶可能有學習曲線。
> - 目前僅針對 Hermes Agent 設計，與其他 AI 系統的兼容性可能有限。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [mem0/mem0](https://github.com/mem0/mem0) | mem0 提供雲端記憶解決方案，但缺乏本地化支持，無法滿足對數據隱私的需求。 |
| [Zep/zep](https://github.com/Zep/zep) | Zep 主要針對雲端環境設計，無法提供本地存儲和無訂閱的靈活性。 |
| [Letta/let](https://github.com/Letta/let) | Letta 也偏向雲端解決方案，缺少結構化事實存儲和智能上下文注入功能。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [mem0/mem0](https://github.com/mem0/mem0) | mem0 提供雲端記憶解決方案，但缺乏本地化支持，無法滿足對數據隱私的需求。 | 如果你的團隊已經在使用雲端解決方案，並且不需要本地存儲的靈活性。 | medium，因為需要重新設計數據流和存儲方式。 |
| [Zep/zep](https://github.com/Zep/zep) | Zep 主要針對雲端環境設計，無法提供本地存儲和無訂閱的靈活性。 | 如果你的團隊已經在使用 Zep，並且需要與其生態系統的整合。 | high，因為需要全面轉換到新的記憶系統。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **mem0** | **zep** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | mem0 提供雲端記憶解決方案，但缺乏本地化支持，無法滿足對數據隱私的需求。 | Zep 主要針對雲端環境設計，無法提供本地存儲和無訂閱的靈活性。 |
> | 遷移成本 | - | medium，因為需要重新設計數據流和存儲方式。 | high，因為需要全面轉換到新的記憶系統。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用雲端解決方案，並且不需要本地存儲的靈活 | 如果你的團隊已經在使用 Zep，並且需要與其生態系統的整合。 |
...
- **[HIGH]** 安裝過程中可能會遇到 Docker 配置問題，導致服務無法啟動。
  - 解法：檢查 Docker 版本和配置，確保符合要求。
- [MEDIUM] 在高並發情況下，系統性能可能下降。
  - 解法：考慮增加硬體資源或優化數據流。
- [MEDIUM] 與非 Hermes 系統的兼容性問題，可能需要額外的適配工作。
  - 解法：提前進行兼容性測試，並準備相應的適配方案。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 5 人以下的創業團隊需要記錄會議和決策 | 非常適合 | 本地化設計能夠保障數據隱私，並且支持智能上下文注入。 |
| 大型企業需要高效的記憶管理系統 | 不適合 | 目前仍處於 beta 階段，可能不夠穩定。 |
| 個人開發者希望構建智能助手 | 適合 | 提供靈活的本地記憶解決方案，支持多種 LLM 提供者。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，10 小時整合，得到一個靈活的本地記憶系統，值得嘗試。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：本系統在本地運行，不需要高權限，並且不存取敏感資料。依賴鏈的信任程度較高，適合在 CI/CD pipeline 中使用。
...
> | --- | --- | --- | --- |
> | [#27](https://github.com/ClaudioDrews/memory-os/issues/27) | Conversation with Hermes Agent regarding this memory project `bug` | 0 | 0 |
