...
use_case: "提供一個本地化的持久記憶系統，讓 AI 代理能夠記住過去的對話和決策。"
priority: high
ring: assess
...
aliases:
  ...
  - "提供一個本地化的持久記憶系統，讓 AI 代理能夠記住過去的對話和決策。"
---

# memory-os

**1.3k** stars · **25** stars/天 · 建立 52 天前 · Python · MIT
...
> **授權** MIT (商業友好) · **維護** Slow (最後推送 42 天前) · **貢獻者** 4+ 人 · **參與度** Low
> **適合** 希望在本地環境中運行 AI 記憶系統的開發者，並且不想依賴雲端服務。
> **一句話重點** 這個專案提供了一個靈活且私密的本地記憶解決方案，讓 AI 代理能夠真正記住過去的對話和決策。
...
> **結論** 花 5 小時學習，10 小時整合，得到一個靈活且私密的記憶系統，值得投資。

> [!abstract] 核心創新
> 提供一個完全本地化的持久記憶系統，無需雲端服務，並且支持多種 LLM 提供者。

## 專案簡介

Memory OS 是一個七層記憶操作系統，旨在為 Hermes Agent 提供持久的記憶基礎架構。使用者可以將過去的對話和決策以結構化的方式儲存，並在需要時自動注入相關的上下文。這個系統的核心在於其獨特的 Ground Truth 層，確保注入的記憶能夠被有效利用，避免了重複查詢和不必要的 token 消耗。使用者可以透過簡單的 API 與任何 LLM 提供者進行整合，無需擔心雲端記憶訂閱或供應商鎖定。具體來說，使用者可以透過 `docker-compose` 快速啟動本地環境，並使用 Qdrant 進行向量搜尋，支持多種 LLM 模型。這個系統的賣點在於其完全本地化的架構，讓用戶能夠擁有完整的數據控制權，並且具備自動化的維護和更新機制。相較於其他雲端解決方案，Memory OS 提供了更高的隱私性和靈活性，特別適合需要長期記憶的 AI 應用場景。

**技術棧**：`Python 3.9` · `Docker` · `Qdrant`

## 重點功能

- 七層記憶架構 — 提供多層次的記憶儲存和檢索機制。
- 本地化運行 — 完全在用戶的機器上運行，無需雲端服務。
- 自動化上下文注入 — 根據過去的對話自動提供相關上下文。
- Qdrant 向量搜尋 — 支持高效的向量搜尋和檢索。
- 信任評分系統 — 對儲存的事實進行信任評分，提升資訊的可靠性。
...
> [!tip] 爆紅原因
> 建立 52 天就累積 1294 stars（25/天），forks 120（9.3%），顯示出穩定的增長趨勢。作者 ClaudioDrews 之前在 AI 記憶領域有過深入的研究，這個專案解決了許多現有雲端記憶解決方案的隱私和依賴問題。特別是針對本地化記憶基礎架構的需求，Memory OS 提供了一個靈活且無需訂閱的解決方案。近期的社群討論和反饋也顯示出用戶對於這個工具的興趣和需求，進一步推動了其流行。

## 適合誰使用

**目標受眾**：希望在本地環境中運行 AI 記憶系統的開發者，並且不想依賴雲端服務。

> [!example] 使用場景
> - AI 開發者用它來構建能夠持久記憶的智能助手，因為這樣的助手能夠在多次對話中保持上下文，提升用戶體驗。
> - 產品經理用它來記錄會議決策和項目進度，因為這樣可以避免重複討論，並能隨時回顧過去的會議內容。
> - 研究人員用它來管理實驗數據和結果，因為這樣可以確保所有關鍵資訊都能被妥善保存和檢索。

## 架構分析

Memory OS 採用七層架構設計，確保記憶的捕捉、儲存和注入。每一層都有其特定功能，從基本的記憶儲存到高效的上下文注入。資料流從用戶的輸入開始，經過 Qdrant 進行向量搜尋，最終將相關的記憶注入到 Hermes Agent 中。

這種設計的優勢在於能夠有效管理記憶的使用，避免重複查詢和 token 浪費。選擇本地化架構的代價是需要用戶自行管理基礎設施，但這也帶來了更高的數據隱私和控制權。擴展性方面，隨著資料量的增加，可能會遇到效能瓶頸，特別是在查詢速度上。

整體而言，這種架構適合需要長期記憶和高效檢索的應用場景。

## 技術深入分析

Memory OS 的核心技術在於其七層記憶架構，這使得記憶的捕捉和注入變得更加高效。每一層都有其特定的功能，從基本的記憶儲存到高效的上下文注入。使用 Qdrant 進行向量搜尋，能夠快速定位相關記憶，並且透過信任評分系統來確保資訊的可靠性。這種設計的效能在於能夠減少重複查詢的需求，從而節省 token 和時間。

選擇 Python 作為開發語言，讓這個專案能夠輕鬆整合到現有的 Python 生態系統中，並且依賴於 Docker 來簡化部署過程。這樣的選擇雖然增加了初期的學習成本，但長期來看能夠提供更高的靈活性和可擴展性。隨著使用者數量的增加，可能會面臨效能瓶頸，尤其是在處理大量資料時。整合到 CI/CD pipeline 的友善度高，能夠快速部署和測試新功能。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且包含必要的安裝步驟，讓新手能夠快速上手。安裝過程相對順暢，但需要確保 Docker 環境的正確配置。文件中提供了基本的使用範例，對於初學者來說非常友好。
...
> - 僅支援 Python 3.9+
> - 需要 Docker 環境才能運行
> - 對於大規模資料集的處理效能尚未經過充分測試

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供了一個基於雲端的 AI 記憶解決方案，適合需要快速擴展的團隊，但缺乏本地化的數據控制。 |
| mem0 | 專注於雲端記憶管理，無法提供本地化的記憶基礎設施，對於隱私要求高的用戶不太適合。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| Zep | 雲端記憶管理，無法提供本地化的數據控制，適合快速擴展的團隊。 | 如果你的團隊需要快速部署並且不介意將數據存儲在雲端。 | medium，因為需要重新設計數據存取方式。 |
| mem0 | 雲端解決方案，無法提供本地化的記憶基礎設施，對於隱私要求高的用戶不太適合。 | 如果你的團隊已經在使用雲端服務且不需要本地化的數據控制。 | high，因為需要全面遷移數據和應用邏輯。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **Zep** | **mem0** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 雲端記憶管理，無法提供本地化的數據控制，適合快速擴展的團隊。 | 雲端解決方案，無法提供本地化的記憶基礎設施，對於隱私要求高的用戶不太適合。 |
> | 遷移成本 | - | medium，因為需要重新設計數據存取方式。 | high，因為需要全面遷移數據和應用邏輯。 |
> | 適用場景 | 主要場景 | 如果你的團隊需要快速部署並且不介意將數據存儲在雲端。 | 如果你的團隊已經在使用雲端服務且不需要本地化的數據控制。 |
...
- [MEDIUM] 在大規模資料集上查詢速度可能變慢
  - 解法：考慮優化資料結構或使用更強大的硬體
- [MEDIUM] 需要手動管理記憶的清理和去重
  - 解法：定期運行清理腳本以維持效能
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 AI 系統 | 非常適合 | 提供本地化的記憶解決方案，適合小型團隊的需求。 |
| 大型企業的 AI 記憶管理 | 不適合 | 可能面臨效能瓶頸，且需要更強的基礎設施支持。 |
| 需要長期記憶的個人專案 | 適合 | 能夠有效管理和檢索過去的對話和決策。 |
| 需要快速擴展的雲端服務 | 不適合 | 本地化架構不支持快速擴展的需求。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，10 小時整合，得到一個靈活且私密的記憶系統，值得投資。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：本地化運行，不需要高權限，且不存取敏感資料。依賴的庫和框架經過審計，整體安全性良好。

## 生態系整合

> [!abstract] 如何融入你的工具鏈

Memory OS 最常與 Python 生態系統中的其他工具搭配使用，特別是在開發和測試階段。用戶可以在本地環境中運行 Memory OS，並與其他 Python 應用進行整合，例如使用 Flask 或 FastAPI 來構建前端介面。這個工具支援與 GitHub Actions 等 CI 工具的整合，能夠輕鬆部署和測試新功能。常見的整合問題包括 Docker 配置不當，這可能導致啟動失敗，建議用戶在部署前仔細檢查環境設定。

## 歷史脈絡

> [!info] 這個工具為什麼現在出現？

在 Memory OS 出現之前，許多記憶管理工具都是基於雲端的，這導致了數據隱私和控制權的問題。隨著對數據隱私的重視增加，開發者開始尋求本地化的解決方案。Memory OS 的出現正好滿足了這一需求，提供了一個靈活且私密的記憶系統，讓用戶能夠完全控制自己的數據。

這個工具代表了向本地化記憶管理的趨勢，未來可能會有更多類似的解決方案出現。
