...
use_case: "為 Hermes Agent 提供持久記憶的七層記憶操作系統，解決了代理人忘記上下文的問題。"
priority: high
ring: assess
...
star_history: "2026-06-04:756,2026-06-04:757,2026-06-05:828,2026-06-05:828,2026-06-06:882"
tags:
  - github
  - "category/開發工具"
  - "lang/python"
aliases:
  ...
  - "為 Hermes Agent 提供持久記憶的七層記憶操作系統，解決了代理人忘記上下文的問題。"
---

# memory-os

**828** stars · **207** stars/天 · 建立 4 天前 · Python · MIT
...
> **一句話重點** Memory OS 透過本地化的記憶管理，讓 Hermes Agent 變得更智能，避免了重複的上下文問題。
...
> **結論** 花 5 小時學習，10 小時整合，得到高效的本地化記憶管理，值得投資。

> [!abstract] 核心創新
> 提供一個完整的本地化記憶操作系統，具備七層記憶架構和智能上下文注入功能。

## 專案簡介

Memory OS 是一個針對 Hermes Agent 設計的七層記憶操作系統，旨在解決代理人在每次會話中忘記上下文的問題。它的核心機制是將用戶的記憶持久化，並能在每次交互中智能地注入相關上下文。使用者只需運行 Docker，並安裝 Hermes Agent，就能利用這個系統來增強代理人的記憶能力，避免重複上下文或丟失重要決策。關鍵指令包括使用 Docker 部署 Qdrant 和 Redis，並透過 Python 3.11+ 來運行系統。這個工具的賣點在於它提供了一個完全本地化的記憶基礎設施，無需依賴雲服務，並且支持多種 LLM 提供者。

技術上，Memory OS 利用 SQLite 和 Qdrant 來實現結構化事實的存儲和檢索，並且具備智能的上下文注入機制，確保每次交互都能利用過去的知識。與傳統的 Hermes 相比，Memory OS 提供了更高效的記憶管理，並且能夠自動清理和去重，這在長期使用中能顯著提高代理人的表現。這個系統的設計考慮到了用戶的隱私，所有數據都保存在本地，避免了雲端記憶解決方案的限制。使用者在實際操作中可能會遇到安裝文檔不足的問題，但整體上，這個系統非常適合需要長期記憶的用戶。未來六個月內，這個專案可能會持續增強其功能，特別是在記憶管理和上下文注入方面。

**技術棧**：`Python 3.11` · `Docker` · `Qdrant` · `Redis` · `SQLite`

## 重點功能

- 七層記憶架構 — 包含工作區、會話、結構化事實、跨會話回憶等功能。
- 本地化記憶基礎設施 — 無需雲端服務，所有數據保存在本地。
- 智能上下文注入 — 在每次交互中自動注入相關上下文，提升代理人效率。
- 結構化事實存儲 — 使用 SQLite 和 Qdrant 來管理持久化的知識。
- 自動清理與去重 — 定期掃描和去除重複的上下文，節省資源。

## 快速開始

1. 克隆專案並進入目錄
```bash
git clone https://github.com/ClaudioDrews/memory-os.git && cd memory-os
```
2. 建置 Docker 環境
```bash
docker-compose up -d
...
> [!tip] 爆紅原因
> 建立 4 天就累積 828 stars（207/天），forks 82（9.9%），顯示出強烈的社群興趣。作者 ClaudioDrews 過去專注於開發記憶相關的解決方案，這個專案解決了許多使用者面對 Hermes Agent 時的記憶問題，特別是對於長期使用者來說，這是一個迫切的需求。這個工具的推出恰逢使用者對於本地化記憶解決方案的需求上升，因為許多現有方案都依賴於雲端服務，導致隱私問題。這些因素共同促成了這個專案的快速增長。
...
> [!example] 使用場景
> - AI 研究員用它來持久化實驗記錄，因為這樣可以避免每次啟動代理時重複設定環境。
> - 產品經理用它來記錄會議決策，因為這樣能夠在未來的會議中快速回顧過去的討論。
> - 開發者用它來追蹤代碼變更歷史，因為這樣能夠在不同版本之間保持一致的上下文。
> - 自由工作者用它來管理客戶需求，因為這樣能夠在每次與客戶交流時提供更精準的回應。

## 架構分析

Memory OS 的架構由七層組成，這樣的設計使得每一層都能專注於特定的記憶功能。第一層是工作區，負責用戶的基本記憶；第二層是會話，利用 SQLite 實現全文本搜索；第三層是結構化事實，結合信任評分來管理持久化知識；第四層是跨會話的回憶，通過 Icarus 插件實現多來源的上下文注入；第五層是向量數據庫 Qdrant，提供高效的檢索能力；第六層是自動策劃的維基系統，持續更新知識；第七層則是關鍵的真實性層級，確保注入的記憶被代理人認可。這樣的設計讓系統能夠高效地捕捉、存儲和使用記憶，並且避免了重複和不必要的資源浪費。選擇本地化的架構使得用戶能夠完全控制自己的數據，這在隱私方面是一個重要的考量。

## 技術深入分析

Memory OS 的核心技術機制是其七層架構，這使得每一層都能專注於特定的記憶功能。使用 SQLite 和 Qdrant 來實現結構化事實的存儲和檢索，這樣的設計提供了高效的數據處理能力。系統能夠處理大量的會話數據，並且具備全文本搜索功能，這對於長期使用者來說至關重要。選擇 Python 作為主要開發語言，因為它擁有豐富的庫和社群支持，這使得開發過程更加高效。依賴樹相對輕量，主要依賴於幾個核心庫，這降低了維護成本。技術風險方面，隨著使用者數據的增長，性能可能會受到影響，特別是在記憶的清理和去重過程中。整合到現有的 CI/CD pipeline 中相對簡單，因為它的設計考慮到了可擴展性，並且與主流框架的兼容性良好。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝步驟和系統架構說明，但安裝過程中可能會遇到文檔不足的問題。整體上，安裝過程相對順暢，對於熟悉 Docker 的用戶來說不會有太大困難。文件目前僅提供英文版本，缺乏多語言支持。

## 優缺點分析

> [!success] 優點
> - 提供完整的本地化記憶解決方案，無需依賴雲端服務。
> - 智能上下文注入機制，提升代理人的交互效率。
> - 結構化事實存儲，支持持久化知識的管理。
> - 自動清理和去重功能，節省資源和提高性能。
...
> [!warning] 注意事項
> - 安裝文檔尚不完善，可能需要額外的配置幫助。
> - 目前僅支持 Python 3.11+，舊版本不兼容。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供類似的記憶管理功能，但主要依賴雲端服務，無法提供本地化的解決方案。 |
| Zep | 雖然也提供記憶功能，但缺乏自動清理和去重的能力，可能導致性能下降。 |
| Letta | Letta 專注於簡單的記憶管理，但不支持複雜的上下文注入，對於長期使用者來說效果有限。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| mem0 | mem0 主要依賴雲端服務來提供記憶管理，而 Memory OS 則提供本地化的解決方案，這樣用戶能夠完全控制自己的數據。 | 如果你的團隊已經在使用雲端記憶解決方案，並且不需要本地化的數據管理。 | medium，因為需要重新設計數據流和存儲結構。 |
| Zep | Zep 提供簡單的記憶管理功能，但缺乏自動清理和去重的能力，這可能導致性能下降。 | 如果你的需求只是基本的記憶管理，且不需要複雜的上下文注入。 | low，因為功能相似，遷移過程相對簡單。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **mem0** | **Zep** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | mem0 主要依賴雲端服務來提供記憶管理，而 Memory OS 則提供本地化的解決方案，這樣用戶能夠完全控制自己的數據。 | Zep 提供簡單的記憶管理功能，但缺乏自動清理和去重的能力，這可能導致性能下降。 |
> | 遷移成本 | - | medium，因為需要重新設計數據流和存儲結構。 | low，因為功能相似，遷移過程相對簡單。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用雲端記憶解決方案，並且不需要本地化的數 | 如果你的需求只是基本的記憶管理，且不需要複雜的上下文注入。 |
...
- [MEDIUM] 安裝過程中可能會遇到文檔不足的問題，特別是對於 Docker 的配置。
  - 解法：查閱社群論壇或 GitHub Issues 獲取幫助。
- **[HIGH]** 對於大型數據集的性能可能會下降，特別是在清理和去重過程中。
  - 解法：定期檢查和手動清理數據。
- **[HIGH]** 需要確保 Python 版本為 3.11+，否則可能會導致相容性問題。
  - 解法：升級 Python 至最新版本。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 本地化的記憶管理解決方案能夠有效支持小型團隊的需求。 |
| 大型企業的長期項目管理 | 適合 | 提供的結構化事實和智能上下文注入能夠提升項目管理效率。 |
| 對數據隱私有高要求的用戶 | 非常適合 | 所有數據保存在本地，無需擔心雲端數據洩露。 |
| 需要快速原型開發的開發者 | 普通 | 雖然功能強大，但安裝和配置可能需要額外時間。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，10 小時整合，得到高效的本地化記憶管理，值得投資。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：系統運行在本地，不需要高權限，且不存取敏感資料。依賴鏈信任程度高，沒有已知的供應鏈風險，適合在 CI/CD pipeline 中使用。
...
> | 貢獻者 | Commits |
> | --- | --- |
> | [@ClaudioDrews](https://github.com/ClaudioDrews) | 6 |
> | [@Davidsoff](https://github.com/Davidsoff) | 2 |
> | [@brian-doherty](https://github.com/brian-doherty) | 2 |

## 社群與生態

**社群活躍度**：社群活躍度中等，最近有持續的提交和問題回覆。
**連結**：[文件](https://github.com/ClaudioDrews/memory-os/wiki)

## 開發動態

> [!abstract] 最近 10 次 commit（2026-06-03 ~ 2026-06-03）
> **活躍天數** 1 天 · **最新 commit** refactor: extract shared parse_entry into icarus/parsing.py (fixes D01, D09)
...
> | --- | --- | --- | --- |
> | [#17](https://github.com/ClaudioDrews/memory-os/issues/17) | Using with non Hermes harnesses | 0 | 0 |
> | [#16](https://github.com/ClaudioDrews/memory-os/issues/16) | Benchmark performance | 0 | 0 |
> | [#15](https://github.com/ClaudioDrews/memory-os/issues/15) | Installation DOC | 0 | 1 |
> | [#14](https://github.com/ClaudioDrews/memory-os/issues/14) | Feature request: Support local embedding models (Ollama) wit | 0 | 1 |
...
>   The upstream [esaradev/icarus-plugin](https://github.com/esaradev/icarus-plugin) is the base, but this fork is not upstream-compatible. Key additions: LLM-powered session extraction (replaces `text[:500]` truncation), multi-source injection (Qdrant + sessions +

## 延伸閱讀

相關概念：[[記憶管理]] · [[本地化存儲]] · [[智能上下文注入]]
...
> FROM "Repos"
> WHERE category = "開發工具" AND file.name != "ClaudioDrews--memory-os"
> SORT stars DESC
> LIMIT 8
> ```
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["記憶管理","本地化存儲","智能上下文注入"];
