...
use_case: "提供持久記憶的操作系統，讓 Hermes Agent 能夠記住過去的對話與決策。"
priority: high
ring: assess
...
aliases:
  ...
  - "提供持久記憶的操作系統，讓 Hermes Agent 能夠記住過去的對話與決策。"
---

# memory-os

**1.3k** stars · **27** stars/天 · 建立 47 天前 · Python · MIT
...
> **授權** MIT (商業友好) · **維護** Slow (最後推送 37 天前) · **貢獻者** 4+ 人 · **參與度** Low
> **適合** 需要在本地環境中運行長期記憶的開發者，並希望避免雲端依賴的專業人士。
> **一句話重點** Memory OS 的創新在於它提供了一個完整的本地記憶解決方案，解決了現有雲端方案的隱私和靈活性問題。
...
> **結論** 花 5 小時學、3 小時整合，得到靈活的本地記憶解決方案，值得投資。

> [!abstract] 核心創新
> Memory OS 提供了一個完整的七層記憶架構，並引入了地面真相層級，確保注入的記憶能被有效使用。

## 專案簡介

Memory OS 是一個七層的記憶操作系統，專為 Hermes Agent 設計，旨在解決記憶遺失的問題。它的核心機制包括將過去的對話、決策和結構化事實存儲在本地，並透過智能上下文注入來提高交互的連貫性。用戶只需運行一個命令 `curl -sSL https://raw.githubusercontent.com/ClaudioDrews/memory-os/main/setup.sh | bash` 即可安裝整個堆疊，無需雲端訂閱，並支持多種 LLM 提供者。這種設計使得用戶能夠在本地環境中擁有完全的數據控制，避免了雲端解決方案的限制。技術上，Memory OS 使用 Qdrant 作為向量數據庫，結合 Redis 和 ARQ 進行高效的數據檢索和存儲。

這種架構不僅提高了記憶的持久性，還能通過自動化的知識管道來持續更新和優化記憶內容。與傳統的 Hermes 相比，Memory OS 提供了結構化事實、跨會話回憶和智能去重等功能，顯著提升了用戶體驗。使用者在實際操作中可能會遇到 Docker 配置的挑戰，但社群已經提供了詳細的安裝指導和問題解決方案。這個專案目前處於穩定階段，對於需要長期記憶的開發者來說，值得考慮在生產環境中使用。

**技術棧**：`Python 3.11` · `Docker` · `Redis` · `Qdrant`

## 重點功能

- 七層記憶架構 — 包括工作區、會話、結構化事實、跨會話記憶等，全面提升記憶能力。
- 本地運行 — 完全在用戶機器上運行，無需雲端訂閱，保護用戶隱私。
- 智能上下文注入 — 自動將相關記憶注入對話中，提升交互的連貫性。
- Qdrant 向量數據庫 — 支持高效的向量檢索，並提供四級回退機制以提高查詢效率。
- 自動化知識維護 — 自我策展的維基系統，持續更新和優化記憶內容。
...
> [!tip] 爆紅原因
> 建立 47 天就累積 1279 stars（27/天），forks 120（9.4%），顯示出良好的社群反響。作者 ClaudioDrews 之前在記憶解決方案方面有豐富的經驗，這個專案解決了現有雲端記憶方案的隱私和靈活性問題。這個工具的推出引起了社群的廣泛討論，特別是在對話記憶的需求上。技術上，隨著本地計算能力的提升，這個工具的可行性得到了增強。高達 9.4% 的 forks/stars 比率表明許多人在實際修改和使用這個專案。

## 適合誰使用

**目標受眾**：需要在本地環境中運行長期記憶的開發者，並希望避免雲端依賴的專業人士。

> [!example] 使用場景
> - AI 研究員用它來持久化對話記錄和實驗結果，因為這樣可以避免每次會議重複說明背景，提升效率。
> - 產品經理用它來追蹤產品決策和反饋，因為能夠快速回顧過去的討論，避免重複錯誤。
> - 開發者用它來管理代碼審查和版本歷史，因為這樣可以確保每次提交都有清晰的上下文，減少誤解。

## 架構分析

Memory OS 的架構由七層組成，從工作區到向量數據庫，層層相互聯繫。這種設計使得記憶的捕獲、存儲和注入變得高效且智能。每一層都有其特定的功能，例如，層 5 使用 Qdrant 進行向量檢索，層 7 則確保了注入的記憶被視為權威資料。

這樣的設計使得用戶能夠在每次交互中獲得準確的上下文，而不必重複 rediscovery 的過程。選擇本地化架構的代價是需要用戶提供更多的硬體資源，但這樣的選擇也帶來了數據隱私的保障。整體來看，這個架構在性能和安全性之間取得了良好的平衡。

## 技術深入分析

Memory OS 的核心技術機制是其七層記憶架構，這使得記憶的捕獲、存儲和注入變得高效且智能。層 1 到層 6 負責記憶的管理，而層 7 則確保這些記憶被視為權威資料。這種設計模式不僅提高了記憶的持久性，還能通過自動化的知識管道來持續更新和優化記憶內容。效能上，這個系統能夠處理大量的數據，並且在本地運行時不會受到網路延遲的影響。選擇使用 Python 和 Docker 作為技術棧，帶來了靈活性和可擴展性，但也增加了對用戶硬體的要求。這個架構在性能和安全性之間取得了良好的平衡，並且能夠與多種 LLM 提供者兼容，這使得它在實際應用中非常靈活。整合到現有的技術生態中相對簡單，尤其是對於已經使用 Docker 的團隊來說，學習成本較低。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝指導和使用範例，安裝過程相對順暢，但對於不熟悉 Docker 的用戶可能會有一些挑戰。文件中沒有多語言支持，主要以英文為主。整體來說，花 30 分鐘能夠成功運行起來。
...
> - 本地運行，無需雲端依賴，保護用戶隱私。
> - 提供智能上下文注入，提升交互的流暢性。
> - 結構化事實和自我策展的維基系統，持續優化記憶內容。

> [!danger] 缺點
> - 需要 Docker 環境，對新手有一定的學習曲線。
> - 對硬體資源有較高要求，可能不適合低配置機器。
> - 目前僅支持 Python 3.11 以上版本，限制了某些用戶。

> [!warning] 注意事項
> - 需要 Docker 環境，對於不熟悉 Docker 的用戶可能有學習曲線。
> - 目前僅支持 Python 3.11 以上版本，舊版本用戶需升級。
> - 對於大型數據集，可能需要較高的硬體資源以確保性能。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| mem0 | mem0 是雲端解決方案，無法提供本地存儲和隱私保護，而 Memory OS 完全在本地運行。 |
| Zep | Zep 主要針對雲端環境設計，缺乏 Memory OS 的本地化和自我策展功能。 |
| Letta | Letta 也依賴雲端服務，無法提供 Memory OS 的結構化事實和智能上下文注入。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| mem0 | mem0 是一個雲端解決方案，無法提供本地存儲和隱私保護，而 Memory OS 完全在本地運行。 | 如果你的團隊已經在使用雲端記憶解決方案，並且不需要本地存儲的隱私保障。 | medium，因為需要將數據從雲端遷移到本地環境。 |
| Zep | Zep 主要針對雲端環境設計，缺乏 Memory OS 的本地化和自我策展功能。 | 如果你的團隊已經在使用 Zep 並且不需要本地化的記憶解決方案。 | high，因為需要重構整個記憶系統。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **mem0** | **Zep** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | mem0 是一個雲端解決方案，無法提供本地存儲和隱私保護，而 Memory OS 完全在本地運行。 | Zep 主要針對雲端環境設計，缺乏 Memory OS 的本地化和自我策展功能。 |
> | 遷移成本 | - | medium，因為需要將數據從雲端遷移到本地環境。 | high，因為需要重構整個記憶系統。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用雲端記憶解決方案，並且不需要本地存儲的 | 如果你的團隊已經在使用 Zep 並且不需要本地化的記憶解決方 |
...
- **[HIGH]** Docker 配置可能會遇到問題，特別是在不同操作系統上，導致服務無法啟動。
  - 解法：檢查 Docker 的版本和配置，確保符合要求。
- [MEDIUM] 在大數據集上運行時，可能會出現性能瓶頸。
  - 解法：考慮升級硬體或優化數據結構。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 能夠提供本地化的記憶解決方案，避免雲端依賴。 |
| 大型企業的數據分析團隊 | 普通 | 雖然能夠處理大量數據，但可能需要較高的硬體資源。 |
| 個人開發者的 AI 實驗 | 非常適合 | 提供靈活的本地環境，適合快速迭代和測試。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學、3 小時整合，得到靈活的本地記憶解決方案，值得投資。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：本工具在本地運行，不需要高權限，並且不會存取敏感資料。依賴鏈的信任程度高，無已知的供應鏈風險。
