...
use_case: "為 Hermes Agent 提供持久記憶的七層記憶操作系統，實現自動化的上下文注入和自我策展的維基系統。"
priority: high
ring: assess
...
aliases:
  ...
  - "為 Hermes Agent 提供持久記憶的七層記憶操作系統，實現自動化的上下文注入和自我策展的維基系統。"
---

# memory-os

**1.3k** stars · **23** stars/天 · 建立 58 天前 · Python · MIT
...
> [!info] 速覽
> **安裝難度** Medium · **專案狀態** Growing · **熱度** Growing (23 stars/day)
> **授權** MIT (商業友好) · **維護** Slow (最後推送 48 天前) · **貢獻者** 4+ 人 · **參與度** Low
> **適合** 希望在本地運行 LLM 並需要持久記憶的開發者或團隊。
> **一句話重點** Memory OS 的設計不僅解決了記憶丟失的問題，還提供了一個完整的本地記憶基礎設施，讓使用者擁有更高的控制權和隱私。
...
>     .where(p => p.subcategory === "RAG" && p.file.name !== "ClaudioDrews--memory-os" && p.status !== "archived")
...
>       p.ring || "assess"
>     ]));
>   } else { dv.paragraph("_目前 vault 中沒有其他 RAG 類工具_"); }
> }
> ```

> [!question] TL;DR — 值得投入嗎？
> **成熟度** Beta (可試用) · **安裝** Medium (需設定) · **學習** ~8h · **綁定風險** low
> **結論** 花 8 小時學習，12 小時整合，得到高效的本地記憶管理，值得投資。

> [!abstract] 核心創新
> 提供一個完整的本地記憶操作系統，具備七層架構和智能上下文注入功能。

## 專案簡介

Memory OS 是一個針對 Hermes Agent 設計的七層記憶操作系統，旨在解決使用者在多次會話中記憶丟失的問題。它的核心流程是：使用者的記憶被捕獲並存儲在本地，然後通過智能上下文注入來強化對話的連貫性，最終在每次交互中提供準確的上下文。關鍵指令如 `curl -sSL https://raw.githubusercontent.com/ClaudioDrews/memory-os/main/setup.sh | bash` 可一鍵安裝整個系統，簡化了部署過程。這個系統的技術架構包括 Qdrant 作為向量數據庫，Redis 作為記憶存儲，並使用 FastAPI 進行 API 通訊，這樣的設計使得系統能夠在本地運行，避免了雲端依賴。與其他記憶解決方案相比，Memory OS 提供了本地記憶基礎設施，無需訂閱服務，並且支持多種 LLM 提供者，這使得它在私密性和靈活性上具有優勢。

此系統的效能特點包括 O(1) 的路徑查找和全文本搜索，能夠高效處理大量對話數據。使用者可能會遇到的問題包括 Docker 環境配置的複雜性，但社群已經提供了多個解決方案。這個專案目前處於 beta 階段，適合希望在本地環境中構建長期記憶的開發者。預計未來會持續增強社群支持和功能擴展。對於需要在本地運行 LLM 並希望擁有完整控制權的團隊，Memory OS 是一個理想的選擇。

**技術棧**：`Python 3.11` · `Docker` · `Redis` · `Qdrant`

## 重點功能

- 七層記憶架構 — 包括工作區、會話、結構化事實、跨會話回憶、向量數據庫、LLM 維基和真實性層。
- 智能上下文注入 — 根據使用者的對話歷史自動提供相關上下文，減少重複查詢。
- 本地運行 — 完全在本地環境中運行，無需雲端服務，保障數據隱私。
- 多提供者支持 — 兼容多種 LLM 提供者，如 OpenAI、Anthropic 和本地模型。
- 自我策展維基 — 自動整理和更新知識庫，便於快速查詢和回顧。
...
> [!tip] 爆紅原因
> 建立 58 天內累積 1307 stars（23/天），forks 122（9.3%），顯示出穩定的增長。這個專案的主要貢獻者 Claudio Drews 之前在 AI 記憶領域有豐富的經驗，解決了現有記憶解決方案普遍存在的雲端依賴和隱私問題。使用者對於記憶丟失的痛點反映在熱門 Issues 中，顯示出對此工具的需求和興趣。社群活躍度高，且有多位貢獻者參與，這表明專案的可持續性和發展潛力。

## 適合誰使用

**目標受眾**：希望在本地運行 LLM 並需要持久記憶的開發者或團隊。

> [!example] 使用場景
> - AI 研究員用它來保存和回顧實驗結果，因為這樣可以避免重複工作並提高效率。
> - 產品經理用它來追蹤會議記錄和決策過程，因為這樣能夠更好地管理項目進度和團隊協作。
> - 開發者用它來記錄代碼變更和技術決策，因為這樣可以在未來的開發中快速回顧過去的選擇和理由。

## 架構分析

Memory OS 的架構由七層組成，從工作區到向量數據庫，確保記憶的捕獲、存儲和注入。這樣的設計使得系統能夠在本地運行，避免了雲端依賴，並且支持多種 LLM 提供者。每一層都有其特定的功能，如層 1 處理工作區記憶，層 5 使用 Qdrant 作為向量數據庫進行高效查詢。選擇這種架構的代價是需要較多的資源來維護多層系統，但這樣的設計使得系統在處理大量數據時能夠保持高效。擴展性方面，系統能夠支持多種 LLM 提供者，這使得未來的功能擴展變得更加靈活。

## 技術深入分析

Memory OS 的核心技術機制是其七層架構，這使得記憶的捕獲、存儲和注入變得高效且有序。每一層都專注於特定的功能，例如層 2 使用 SQLite 和 FTS5 實現全文本搜索，層 5 則利用 Qdrant 進行向量查詢。這樣的設計使得系統能夠在本地環境中運行，避免了雲端依賴，並且支持多種 LLM 提供者。效能方面，系統能夠高效處理大量對話數據，並且具備 O(1) 的查詢性能。設計取捨上，選擇使用 Python 和 Docker 使得開發和部署變得靈活，但也帶來了對於 Docker 環境的依賴。技術風險方面，未來在擴展時可能會遇到性能瓶頸，特別是在資源有限的環境中。整合方面，與主流框架的兼容性良好，能夠輕鬆融入現有的開發流程。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且包含安裝範例，安裝過程相對順暢，但對於不熟悉 Docker 的使用者可能會有一定的學習曲線。文件目前僅提供英文版本，可能對非英語使用者造成障礙。
...
> - 在資源有限的機器上運行時，性能可能會受到影響。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供類似的記憶功能，但主要依賴雲端服務，缺乏本地運行的選項。 |
| Zep | 雖然也提供記憶功能，但不支持本地基礎設施，並且缺乏自我策展的維基系統。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| mem0 | mem0 主要依賴雲端服務，而 Memory OS 提供本地運行的選項，這對於需要數據隱私的使用者更具吸引力。 | 如果你的團隊已經在使用雲端記憶解決方案，且不需要本地運行的靈活性，則可以考慮 mem0。 | medium，因為需要重新配置記憶存儲和上下文注入的邏輯。 |
| Zep | Zep 提供類似的記憶功能，但不支持本地基礎設施，並且缺乏自我策展的維基系統。 | 如果你的團隊需要一個簡單的雲端解決方案，且不需要本地控制，Zep 可能是更合適的選擇。 | high，因為需要重構整個記憶管理流程。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **mem0** | **Zep** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | mem0 主要依賴雲端服務，而 Memory OS 提供本地運行的選項，這對於需要數據隱私的使用者更具吸引力。 | Zep 提供類似的記憶功能，但不支持本地基礎設施，並且缺乏自我策展的維基系統。 |
> | 遷移成本 | - | medium，因為需要重新配置記憶存儲和上下文注入的邏輯。 | high，因為需要重構整個記憶管理流程。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用雲端記憶解決方案，且不需要本地運行的靈 | 如果你的團隊需要一個簡單的雲端解決方案，且不需要本地控制，Z |
...
- **[HIGH]** Docker 環境配置可能會導致安裝失敗，特別是在資源有限的機器上。
  - 解法：確保 Docker 版本符合要求，並參考社群提供的解決方案。
- [MEDIUM] 在使用過程中可能會遇到性能瓶頸，特別是在大量數據處理時。
  - 解法：優化數據存儲和查詢邏輯，並考慮升級硬體。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 5 人以下的初創團隊需要長期記憶管理 | 非常適合 | 本地運行的特性和靈活性使其非常適合小型團隊。 |
| 大型企業需要雲端記憶解決方案 | 不適合 | Memory OS 主要針對本地運行，無法滿足雲端需求。 |
| AI 研究團隊需要保存實驗數據 | 適合 | 能夠有效管理和回顧大量對話和實驗數據。 |
| 需要快速原型開發的開發者 | 普通 | 雖然功能強大，但安裝和配置過程可能會影響開發速度。 |
...
> [!tip] 投入 vs 回報
> 花 8 小時學習，12 小時整合，得到高效的本地記憶管理，值得投資。
...
> FROM "Repos"
> WHERE subcategory = "RAG" AND file.name != "ClaudioDrews--memory-os"
> SORT stars DESC
> ```
