...
use_case: "為 Hermes Agent 提供持久記憶的操作系統，解決記憶丟失問題。"
priority: high
ring: assess
...
**756** stars · **252** stars/天 · 建立 3 天前 · Python · MIT
...
> **適合** 希望在本地運行 AI 代理並持久記憶的開發者和研究人員。
> **一句話重點** Memory OS 讓 Hermes Agent 擁有持久記憶，徹底改變了用戶與 AI 代理的互動方式。
...
> [!abstract] 核心創新
> 提供七層記憶架構，並且確保注入的記憶被代理正確使用。

## 專案簡介

Memory OS 是一個為 Hermes Agent 設計的七層記憶操作系統，旨在解決使用者在多次會話中記憶丟失的問題。它通過將用戶的記憶結構化，並在每次交互中自動注入相關上下文，來確保代理能夠持續記住過去的決策和項目。使用者只需運行 Docker 環境，並搭配 Qdrant 和 Redis，便可在本地運行，無需依賴雲端服務。這種設計不僅避免了供應商鎖定，還能確保用戶的數據隱私。

核心功能包括全會話的全文檢索、結構化事實存儲、跨會話的記憶回憶以及自動生成的維基知識庫，這些都能有效提高記憶的準確性和可用性。與傳統的 Hermes 代理相比，Memory OS 提供了更為高效的記憶管理，並且通過第七層的「真實性層級」確保注入的記憶被代理正確使用，避免了重複查詢的資源浪費。這使得用戶在與代理互動時，能夠獲得更連貫的體驗。整體架構的設計考量了用戶的實際需求，並針對現有的記憶解決方案的不足進行了優化，特別是在本地運行的靈活性和數據安全性方面。

**技術棧**：`Python 3.11` · `Docker` · `Redis` · `Qdrant`

## 重點功能

- 七層記憶架構 — 包括工作區、會話、結構化事實、跨會話回憶、向量數據庫等。
- 自動化上下文注入 — 在每次交互中自動注入相關的記憶上下文，提升代理的連貫性。
- 結構化事實存儲 — 使用 SQLite 和信任評分系統，確保記憶的準確性和可靠性。
- 本地運行 — 完全在用戶的機器上運行，無需雲端服務，保護數據隱私。
- 多供應商支持 — 兼容多種 LLM 提供者，如 OpenAI、Anthropic 等，無供應商鎖定。
...
> [!tip] 爆紅原因
> 建立 3 天內累積 756 stars（252/天），forks 74（9.8%），這顯示出強烈的社群興趣。作者 ClaudioDrews 是一位專注於 AI 代理的開發者，過去的經驗讓他認識到現有記憶解決方案的不足，尤其是雲端依賴和記憶丟失問題。這個專案的推出正好填補了這一空白，提供了本地運行的記憶解決方案，並且不需要訂閱費用。社群的反饋也顯示出對這種解決方案的需求，特別是在對數據隱私的重視上。

## 適合誰使用

**目標受眾**：希望在本地運行 AI 代理並持久記憶的開發者和研究人員。

> [!example] 使用場景
> - AI 研究人員用它來持續跟蹤實驗結果和參數調整，因為它能夠記住每次實驗的細節，避免重複設定。
> - 產品經理用它來管理多個項目的進度和決策，因為它能夠在會話中自動注入相關的歷史資料，提升工作效率。
> - 開發者用它來記錄代碼變更和問題解決過程，因為它能夠結構化保存這些信息，方便未來查詢和回顧。

## 架構分析

Memory OS 的架構由七層組成，每一層都專注於不同的記憶管理功能。第一層是工作區，將用戶的記憶注入到系統提示中；第二層是會話記憶，使用 SQLite 進行全文檢索；第三層是結構化事實存儲，結合信任評分系統進行持久化存儲。第四層是跨會話的記憶回憶，利用 Icarus 插件進行多來源注入；第五層是向量數據庫 Qdrant，提供高效的相似度檢索；第六層是自動生成的維基知識庫，持續更新並存儲概念和實體；第七層則是確保注入記憶被代理正確使用的真實性層級。這種設計的代價在於需要較多的資源來運行多個服務，但相對於雲端解決方案，提供了更高的數據安全性和靈活性。

## 技術深入分析

Memory OS 的核心技術機制是其七層記憶架構，這使得記憶的捕獲、存儲和注入變得高效且有序。每一層都專注於特定的功能，例如使用 SQLite 實現的全文檢索和 Qdrant 提供的向量檢索，這些都能有效提高記憶的準確性。整體架構的設計考量了用戶的需求，並針對現有的記憶解決方案的不足進行了優化，特別是在本地運行的靈活性和數據安全性方面。設計選擇上，選擇了 Python 作為開發語言，因為其生態系統豐富且易於擴展，這在實現複雜的記憶管理功能時提供了便利。技術風險方面，隨著用戶數據量的增加，性能可能會受到影響，特別是在記憶的檢索和更新上。整合方面，Memory OS 與主流的 LLM 提供者兼容，並且能夠輕鬆融入現有的開發流程中，這降低了學習成本和整合難度。

## 新手體驗

> [!info] 上手難度評估
> README 文件詳細且清晰，提供了完整的安裝和使用指南。安裝過程中需要 Docker，對新手來說可能有一定的挑戰。文件中包含了設定範例，幫助用戶快速上手，但目前僅提供英文文檔，缺乏多語言支持。
...
| Zep | Zep 主要針對雲端記憶解決方案，而 Memory OS 提供本地運行的選擇，並且無需訂閱費用。 |
| mem0 | mem0 提供基本的記憶功能，但缺乏 Memory OS 的結構化事實和自動化上下文注入特性。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| mem0 | mem0 提供基本的記憶功能，但主要依賴雲端服務，缺乏本地運行的靈活性。 | 如果你的團隊已經在使用雲端解決方案，並且不介意數據存儲在外部。 | medium，因為需要重新配置記憶管理的方式。 |
| Zep | Zep 主要針對雲端記憶解決方案，而 Memory OS 提供本地運行的選擇，並且無需訂閱費用。 | 如果你的團隊已經在使用 Zep 並希望繼續使用雲端服務。 | high，因為需要重新設計記憶管理架構。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **mem0** | **Zep** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | mem0 提供基本的記憶功能，但主要依賴雲端服務，缺乏本地運行的靈活性。 | Zep 主要針對雲端記憶解決方案，而 Memory OS 提供本地運行的選擇，並且無需訂閱費用。 |
> | 遷移成本 | - | medium，因為需要重新配置記憶管理的方式。 | high，因為需要重新設計記憶管理架構。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用雲端解決方案，並且不介意數據存儲在外部 | 如果你的團隊已經在使用 Zep 並希望繼續使用雲端服務。 |
...
- [MEDIUM] 在高負載情況下，記憶檢索速度可能變慢
  - 解法：優化 Qdrant 的配置，或考慮擴展硬體資源

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 5 人以下的初創團隊使用 Hermes 進行項目管理 | 非常適合 | 能夠有效記錄和管理團隊的決策和進度。 |
| 大型企業的 AI 研究部門 | 適合 | 需要強大的記憶管理功能來支持多個項目的進行。 |
| 個人開發者進行 AI 代理的實驗 | 普通 | 雖然功能強大，但可能對新手有一定的學習曲線。 |
...
> [!warning] 安全性快速掃描
> 低風險：本地運行，不需要高權限，且不存取敏感資料。依賴的庫需定期更新以避免安全漏洞。
...
相關專案：[[0x0funky--agent-sprite-forge|0x0funky/agent-sprite-forge]] · [[MemPalace--mempalace|MemPalace/mempalace]] · [[milla-jovovich--mempalace|milla-jovovich/mempalace]] · [[EvoLinkAI--awesome-gpt-image-2-prompts|EvoLinkAI/awesome-gpt-image-2-prompts]] · [[nashsu--llm_wiki|nashsu/llm_wiki]]
