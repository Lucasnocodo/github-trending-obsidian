...
use_case: "為 Hermes Agent 提供持久記憶的操作系統，解決了記憶丟失的問題。"
priority: high
ring: assess
...
aliases:
  ...
  - "為 Hermes Agent 提供持久記憶的操作系統，解決了記憶丟失的問題。"
---

# memory-os

**1.2k** stars · **38** stars/天 · 建立 32 天前 · Python · MIT
...
> **授權** MIT (商業友好) · **維護** Moderate (最後推送 22 天前) · **貢獻者** 4+ 人 · **參與度** Low
> **適合** 需要在本地運行 AI 代理並希望其能記住上下文的開發者。
> **一句話重點** Memory OS 不僅是記憶的存儲，更是記憶的智能管理與使用。
...
>     .where(p => p.subcategory === "記憶操作系統" && p.file.name !== "ClaudioDrews--memory-os" && p.status !== "archived")
...
>       p.ring || "assess"
>     ]));
>   } else { dv.paragraph("_目前 vault 中沒有其他 記憶操作系統 類工具_"); }
> }
> ```

> [!question] TL;DR — 值得投入嗎？
> **成熟度** Beta (可試用) · **安裝** Easy (一行搞定) · **學習** ~5h · **綁定風險** low
> **結論** 花 5 小時學習，3 小時整合，得到持久記憶的效果，值得投資。

> [!abstract] 核心創新
> Memory OS 提供了一個完整的七層記憶架構，並引入了 Ground Truth 層來確保注入的記憶被有效使用。

## 專案簡介

Memory OS 是一個針對 Hermes Agent 設計的七層記憶操作系統，旨在解決 AI 代理在會話中忘記上下文的問題。它的核心機制是將記憶分為七層，從工作區到結構化事實，並透過智能上下文注入來確保每次會話都能使用正確的記憶。用戶只需運行 `curl -sSL https://raw.githubusercontent.com/ClaudioDrews/memory-os/main/setup.sh | bash` 即可快速安裝整個系統，無需雲端訂閱，完全在本地運行。技術上，Memory OS 使用了 Qdrant 作為向量資料庫，並結合 Redis 和 ARQ Worker 來實現高效能的資料存取與管理。這種設計使得記憶的注入不僅僅是存儲，而是能夠在需要時被有效地調用，避免了重複查詢造成的資源浪費。

與傳統的 Hermes 相比，Memory OS 提供了結構化事實、跨會話回憶和自動清理等功能，這些在其他記憶解決方案中往往缺失。這使得它在需要長期記憶的應用場景中，特別是在小型團隊或個人開發者中，表現出色。這個專案的成熟度還在 beta 階段，但已經有穩定的社群支持和活躍的開發進度。對於需要在本地運行 AI 代理並希望其能記住上下文的開發者來說，Memory OS 是一個值得考慮的選擇。

**技術棧**：`Python 3.11` · `Docker` · `Redis` · `Qdrant`

## 重點功能

- 七層記憶架構 — 包含工作區、會話、結構化事實等，確保記憶的有效捕捉與使用。
- 智能上下文注入 — 自動將相關記憶注入到每次會話中，減少重複查詢。
- 本地運行 — 完全在用戶的機器上運行，無需雲端依賴，保護用戶數據隱私。
- 支持多種 LLM 提供者 — 包括 OpenRouter、OpenAI、Anthropic 等，靈活性高。
- 自動清理與去重 — 透過 decay scanner 和語義去重，保持記憶的準確性與效率。
...
> [!tip] 爆紅原因
> 建立 32 天內累積 1219 stars（38/天），forks 116（9.5%），顯示出穩定的增長潛力。作者 ClaudioDrews 之前專注於 AI 記憶和上下文注入的相關專案，這次的 Memory OS 解決了現有記憶解決方案的雲端依賴問題，讓用戶能在本地運行，避免了數據外洩的風險。這個專案的推出引起了社群的關注，尤其是對於希望在本地環境中運行 AI 的開發者來說，這是一個全新的選擇。forks/stars 比率為 9.5%，顯示出有相當多的用戶在實際修改使用，這意味著它的實用性和需求都在上升。

## 適合誰使用

**目標受眾**：需要在本地運行 AI 代理並希望其能記住上下文的開發者。

> [!example] 使用場景
> - AI 研究者用它來在本地環境中運行 Hermes Agent，因為它提供了持久記憶功能，避免了每次會話都要重複上下文的麻煩。
> - 小型開發團隊用它來管理專案記憶，因為它能夠自動整理和回憶重要決策，提升團隊協作效率。
> - 獨立開發者用它來構建個人助理，因為它支持多種 LLM 提供者，並且無需雲端訂閱，降低了成本。

## 架構分析

Memory OS 採用七層架構，從工作區到向量資料庫，確保記憶的有效捕捉與使用。每一層都有其特定功能，例如 Layer 1 處理基本的工作區記憶，而 Layer 5 則使用 Qdrant 進行向量搜索。這種設計使得記憶的注入不僅是存儲，而是能夠在需要時被有效地調用。選擇本地運行的架構避免了雲端依賴的風險，但也可能在資源有限的情況下遇到性能瓶頸。整體而言，這種架構的設計考量了用戶的數據隱私與操作的靈活性。

## 技術深入分析

Memory OS 的核心在於其七層架構，這使得記憶的捕捉、存儲和使用都能高效運行。每一層都有其特定的功能，Layer 1 處理基本的工作區記憶，Layer 2 則使用 SQLite 進行會話的全文本搜索，Layer 5 則利用 Qdrant 進行向量資料的高效檢索。這種設計不僅提升了記憶的使用效率，還降低了重複查詢的成本。選擇本地運行的架構避免了雲端依賴的風險，但也可能在資源有限的情況下遇到性能瓶頸。技術上，Memory OS 的設計考量了用戶的數據隱私與操作的靈活性，並且在社群的支持下持續進行優化與改進。整體而言，這個專案的設計思路是為了讓 AI 代理能夠真正記住用戶的上下文，成為長期的合作夥伴。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了安裝指令和使用範例；安裝過程相對順暢，使用一行指令即可完成；有良好的入門指南，並且文件支持多語言，方便新手上手。

## 優缺點分析

> [!success] 優點
> - 本地運行，無需雲端訂閱，保護用戶數據隱私。
> - 提供智能上下文注入，減少重複查詢的成本。
> - 支持多種 LLM 提供者，靈活性高。

> [!danger] 缺點
> - 目前僅支持 Python 3.11+，對於舊版本的兼容性差。
> - 需要 Docker 環境，對於不熟悉 Docker 的用戶有學習曲線。
> - 在資源有限的情況下可能會遇到性能瓶頸。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供了類似的記憶功能，但主要依賴雲端服務，無法實現本地運行。 |
| Zep | 雖然也提供記憶功能，但缺乏自動清理和去重的能力，對於長期使用的效率較低。 |
| Letta | Letta 提供的記憶功能較為淺薄，無法實現深度的上下文注入。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| mem0 | mem0 主要依賴雲端服務，無法實現本地運行，這使得用戶的數據安全性受到威脅。 | 如果你的團隊已經在使用雲端服務並且不在意數據隱私，mem0 可能是更方便的選擇。 | medium，因為需要重新配置雲端環境和數據遷移。 |
| Zep | Zep 提供的記憶功能較為淺薄，無法實現深度的上下文注入，且主要依賴雲端。 | 如果你的需求不需要深度記憶功能，Zep 可能會是個簡單的選擇。 | low，因為 Zep 的設置相對簡單。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **mem0** | **Zep** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | mem0 主要依賴雲端服務，無法實現本地運行，這使得用戶的數據安全性受到威脅。 | Zep 提供的記憶功能較為淺薄，無法實現深度的上下文注入，且主要依賴雲端。 |
> | 遷移成本 | - | medium，因為需要重新配置雲端環境和數據遷移。 | low，因為 Zep 的設置相對簡單。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用雲端服務並且不在意數據隱私，mem0  | 如果你的需求不需要深度記憶功能，Zep 可能會是個簡單的選擇 |
...
- **[HIGH]** 在某些低效能機器上運行時可能會遇到性能瓶頸，導致系統反應緩慢。
  - 解法：建議在較高效能的機器上運行，或優化 Docker 配置以提升性能。
- [MEDIUM] 安裝過程中可能會遇到 Docker 相關的問題，特別是在 Windows 環境下。
  - 解法：建議使用 WSL 或在 Linux 環境中安裝。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 5 人以下的初創團隊需要長期記憶的 AI 代理 | 非常適合 | 提供本地運行的記憶解決方案，無需擔心數據隱私問題。 |
| 個人開發者希望構建一個智能助理 | 適合 | 能夠提供持久記憶，並支持多種 LLM 提供者。 |
| 大型企業需要雲端記憶解決方案 | 不適合 | 本專案專注於本地運行，無法滿足雲端需求。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，3 小時整合，得到持久記憶的效果，值得投資。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：本專案在本地運行，不需要高權限，且不存取敏感資料，對於 CI/CD 的使用也相對安全。

## 生態系整合

> [!abstract] 如何融入你的工具鏈

Memory OS 最常與 Docker 和 Python 環境搭配使用，通常在開發和測試階段進行整合。在使用中，可以透過 Docker Compose 來啟動所有服務，並且與現有的 LLM 提供者進行整合。由於其本地運行的特性，與其他雲端服務的整合相對較少，主要集中在本地環境的優化上。整合的摩擦點主要在於 Docker 的配置，對於不熟悉 Docker 的用戶可能需要額外的學習時間。
...
> [!note]- 直接競品（同子分類：記憶操作系統）
> ```dataview
> TABLE stars, stars_per_day AS "Stars/天", install_complexity AS "難度", use_case AS "用途"
> FROM "Repos"
> WHERE subcategory = "記憶操作系統" AND file.name != "ClaudioDrews--memory-os"
> SORT stars DESC
> ```
