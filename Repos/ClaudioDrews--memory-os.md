...
use_case: "提供一個本地運行的記憶體作業系統，讓 Hermes Agent 能夠持久記憶和智能上下文注入。"
priority: high
ring: assess
...
star_history: "2026-06-04:756,2026-06-04:757,2026-06-05:828,2026-06-05:828,2026-06-06:882,2026-06-06:882,2026-06-07:951,2026-06-07:951,2026-06-08:1011,2026-06-09:1037,2026-06-10:1050,2026-06-11:1069,2026-06-12:1086,2026-06-13:1093,2026-06-14:1098,2026-06-15:1104,2026-06-16:1119,2026-06-17:1126"
tags:
  - github
  - "category/ai_ml"
  - "lang/python"
...
aliases:
  ...
  - "提供一個本地運行的記憶體作業系統，讓 Hermes Agent 能夠持久記憶和智能上下文注入。"
---

# memory-os

**1.1k** stars · **70** stars/天 · 建立 16 天前 · Python · MIT
...
> **適合** 希望在本地運行記憶系統並管理智能代理的開發者和團隊。
> **一句話重點** Memory OS 讓智能代理能夠真正記住過去的對話和決策，提升了其長期合作的能力。
...
> **結論** 花 5 小時學習，8 小時整合，得到本地化的記憶系統，值得一試。

> [!abstract] 核心創新
> 提供一個完全本地化的記憶系統，讓智能代理能夠持久記憶並智能注入上下文。

## 專案簡介

Memory OS 是一個為 Hermes Agent 設計的七層記憶體作業系統，旨在解決智能代理的記憶問題。它通過將記憶捕獲、存儲和注入的過程自動化，讓代理能夠在每次交互中回憶起過去的對話和決策。使用者可以透過簡單的 API 調用來存取這些記憶，並且支持多種 LLM 提供者，如 OpenAI 和 Anthropic。這個系統的賣點在於它的本地運行特性，避免了雲端記憶的訂閱費用和數據外洩的風險。

技術上，Memory OS 使用 Qdrant 作為向量數據庫，並結合 Redis 和 ARQ 來構建本地記憶基礎設施。這種設計使得系統能夠實現高效的全文本搜索和多來源注入，並且提供了信任評分和反饋循環來確保記憶的準確性。

與傳統的 Hermes 系統相比，Memory OS 提供了更豐富的功能，如自我策展的維基系統和語義搜索。

與其他記憶解決方案如 mem0 和 Zep 相比，Memory OS 的最大優勢在於其本地化的記憶基礎設施，這使得使用者不必依賴雲服務，並且可以完全控制自己的數據。這對於需要高隱私保護的用戶來說，無疑是一個重要的考量。

然而，這個系統仍然需要一定的技術背景來設置和維護，對於不熟悉 Docker 和 Python 的用戶來說，可能會面臨一定的學習曲線。整體而言，Memory OS 是一個適合希望在本地運行智能代理的開發者和團隊的解決方案。

**技術棧**：`Python 3.9` · `Docker` · `Redis` · `Qdrant`

## 重點功能

- 七層記憶架構 — 包括捕獲、存儲、注入和使用記憶的完整流程。
- 本地運行 — 完全在用戶的機器上運行，無需雲端服務，保障數據隱私。
- 智能上下文注入 — 自動將相關記憶注入到對話中，提高代理的上下文理解能力。
- Qdrant 向量搜索 — 提供高效的語義搜索功能，支持多層次的回溯和查詢。
- 自我策展維基 — 自動生成和更新的知識庫，幫助用戶整理和回顧信息。
...
```python
{
  ...
  "預期輸出": "Memory OS 服務啟動，並可通過指定端口訪問。"
}
```
...
> [!tip] 爆紅原因
> 建立 16 天內累積 1126 stars（70/天），forks 108（9.6%），顯示出穩定的增長潛力。這個專案的作者 ClaudioDrews 之前在 AI 和記憶系統方面有豐富的經驗，解決了現有記憶解決方案過於依賴雲端的痛點，讓用戶能夠在本地運行記憶系統。近期的社群討論和反饋也顯示出對於本地化記憶的需求日益增加，尤其是在隱私保護方面的考量。這個工具的成功也反映了對於開源和自我主權數據管理的重視，吸引了許多開發者的關注。

## 適合誰使用

**目標受眾**：希望在本地運行記憶系統並管理智能代理的開發者和團隊。

> [!example] 使用場景
> - AI 研究員用它來記錄實驗過程和結果，因為這樣可以隨時回顧過去的決策和數據，避免重複錯誤。
> - 產品經理用它來管理多個項目的需求和進度，因為能夠快速檢索過去的會議記錄和決策依據，提高工作效率。
> - 開發者用它來在本地環境中測試和調整智能代理，因為這樣可以完全控制數據流和記憶管理，減少外部依賴。

## 架構分析

Memory OS 的架構設計為七層，確保記憶的捕獲、存儲、注入和使用。第一層至第六層負責記憶的管理，而第七層則確保這些記憶能被有效利用。使用 Qdrant 作為向量數據庫，支持高效的語義搜索，並利用 Redis 作為快取和存儲層。

這種設計使得系統能夠在本地運行，避免了雲端依賴，並且提供了靈活的擴展性。選擇這種架構的代價在於需要用戶具備一定的技術背景來設置和維護系統，特別是在 Docker 環境下的運行。整體而言，這種設計使得 Memory OS 在數據隱私和控制上具有明顯優勢，但也可能在性能上面臨挑戰，特別是在處理大量數據時。

## 技術深入分析

Memory OS 的核心技術機制在於其七層架構，這不僅確保了記憶的捕獲和存儲，還強調了記憶的有效使用。系統使用 Qdrant 作為向量數據庫，這使得語義搜索變得高效，並且支持多層次的回溯和查詢。整體效能方面，系統在本地運行，對於小型和中型數據集的處理表現良好，但在面對大規模數據時可能會遇到性能瓶頸。選擇 Python 和 Docker 作為主要技術棧，這使得系統的部署相對簡單，但也要求用戶具備一定的技術背景。這種選擇的代價在於可能需要額外的資源來優化性能，特別是在高負載情況下。技術風險方面，隨著系統的擴展，記憶的管理和存取可能會變得更加複雜，這需要持續的維護和優化。與主流框架的整合方面，Memory OS 可以與多種 LLM 提供者兼容，但在實際整合過程中可能會遇到一些摩擦點，特別是在配置和依賴管理上。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝指導和使用範例，對於新手來說相對友好。安裝過程需要 Docker 環境，對於不熟悉的用戶可能會有一些挑戰。整體來說，花 30 分鐘應該能夠成功運行起來，但需要一定的技術背景來解決可能出現的問題。
...
> - 需要一定的技術背景來設置和維護。
> - 對於大規模數據處理可能會遇到性能瓶頸。
> - 目前功能仍在持續開發中，穩定性可能不如成熟產品。

> [!warning] 注意事項
> - 需要 Docker 環境來運行，對於不熟悉 Docker 的用戶可能有學習曲線。
> - 目前僅支持 Python 3.7 以上版本，舊版本可能無法正常運行。
> - 對於大規模數據處理可能會遇到性能瓶頸，需進行優化。
...
| [mem0/mem0](https://github.com/mem0/mem0) | 雖然也提供記憶功能，但缺乏自我策展的維基系統，無法實現智能上下文注入。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [mem0/mem0](https://github.com/mem0/mem0) | mem0 主要依賴雲端記憶解決方案，而 Memory OS 則提供本地化的運行環境。 | 如果你的團隊已經在使用雲端服務並希望快速部署，mem0 可能是更好的選擇。 | medium，因為需要將數據從雲端轉移到本地環境。 |
| [Zep/zep](https://github.com/Zep/zep) | Zep 提供類似的記憶功能，但主要依賴於雲端架構，缺乏本地化支持。 | 如果你的應用場景不需要本地化記憶，Zep 可能會更方便。 | low，因為可以直接使用現有的雲端服務。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **mem0** | **zep** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | mem0 主要依賴雲端記憶解決方案，而 Memory OS 則提供本地化的運行環境。 | Zep 提供類似的記憶功能，但主要依賴於雲端架構，缺乏本地化支持。 |
> | 遷移成本 | - | medium，因為需要將數據從雲端轉移到本地環境。 | low，因為可以直接使用現有的雲端服務。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用雲端服務並希望快速部署，mem0 可能 | 如果你的應用場景不需要本地化記憶，Zep 可能會更方便。 |
...
- **[HIGH]** Docker 容器啟動時可能會遇到端口衝突問題
  - 解法：檢查並確保指定的端口未被其他服務佔用
- [MEDIUM] 在高負載情況下，系統可能會出現性能下降
  - 解法：考慮優化數據結構或增加硬體資源
- [MEDIUM] 記憶的管理和存取可能變得複雜
  - 解法：定期進行維護和優化，保持系統的流暢運行

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 本地化運行和智能上下文注入能夠提高團隊的協作效率。 |
| 大型企業的數據分析部門 | 普通 | 雖然能夠處理一定量的數據，但在面對大規模數據時可能會遇到性能瓶頸。 |
| 個人開發者的 AI 專案 | 非常適合 | 本地化的記憶系統能夠讓開發者自由控制數據，並提升代理的智能。 |
| 需要高頻率記憶更新的實時應用 | 不適合 | 系統在高負載下可能會出現性能下降，無法滿足實時需求。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，8 小時整合，得到本地化的記憶系統，值得一試。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：本地運行的系統不需要高權限，且不會存取敏感資料。依賴鏈中的庫相對穩定，無已知的供應鏈風險。
