...
use_case: "提供一個本地化的持久記憶系統，讓 Hermes Agent 能夠有效記住和調用過去的對話和決策。"
priority: high
ring: assess
...
star_history: "2026-06-04:756,2026-06-04:757,2026-06-05:828,2026-06-05:828,2026-06-06:882,2026-06-06:882,2026-06-07:951,2026-06-07:951"
tags:
  - github
  - "category/ai_ml"
  - "lang/python"
...
aliases:
  ...
  - "提供一個本地化的持久記憶系統，讓 Hermes Agent 能夠有效記住和調用過去的對話和決策。"
---

# memory-os

**951** stars · **159** stars/天 · 建立 6 天前 · Python · MIT
...
> **適合** 需要在本地環境中運行持久記憶系統以支持 Hermes Agent 的開發者。
> **一句話重點** Memory OS 的設計讓用戶能夠在本地環境中擁有完整的記憶管理系統，這在當前的雲端解決方案中是無法實現的。
...
> **結論** 花 5 小時學習和 3 小時整合，能夠獲得高效的本地記憶管理，值得投資。

> [!abstract] 核心創新
> Memory OS 提供了一個完整的本地化記憶操作系統，具備七層架構和智能上下文注入，解決了現有記憶解決方案的不足。

## 專案簡介

Memory OS 是一個七層的記憶操作系統，專為 Hermes Agent 設計，旨在解決記憶丟失的問題。用戶可以透過這個系統將過去的對話、決策和結構化事實持久化，並在需要時自動調用。系統的核心在於其第七層的「真實性層級」，確保注入的記憶被有效利用，避免了重複查找和不必要的 token 消耗。安裝過程簡單，只需執行一條命令 `curl -sSL https://raw.githubusercontent.com/ClaudioDrews/memory-os/main/setup.sh | bash` 即可完成整個堆疊的設置。

技術上，Memory OS 結合了 Qdrant 作為向量數據庫，並使用 Redis 和 ARQ Worker 進行本地記憶管理，這使得它在性能和靈活性上優於許多雲端解決方案。與其他記憶解決方案相比，Memory OS 提供了完整的本地基礎設施，無需訂閱費用，並且支持多種 LLM 提供者，這使得用戶能夠自由選擇。這個系統特別適合需要長期記憶和上下文注入的開發者，尤其是在處理複雜項目時。使用者可以期待更高的記憶效率和更少的重複工作，從而提升整體生產力。

**技術棧**：`Python 3.11` · `Docker` · `Redis` · `Qdrant`

## 重點功能

- 七層記憶架構 — 包含工作區、會話、結構化事實、交叉會話、向量數據庫、LLM Wiki 和真實性層級。
- 本地記憶基礎設施 — 完全在用戶機器上運行，無需雲端服務。
- 智能上下文注入 — 自動將相關的過去記憶注入到當前會話中，提升對話的連貫性。
- 結構化事實存儲 — 提供信任評分和自動反饋循環，確保記憶的準確性和可靠性。
- 支持多種 LLM 提供者 — 包括 OpenRouter、OpenAI、Anthropic 和本地模型，無供應商鎖定。
...
> [!tip] 爆紅原因
> 建立 6 天就累積 951 stars（158.5/天），forks 92（9.7%），顯示出穩定的增長潛力。作者 Claudio Drews 在記憶系統的開發上有豐富經驗，這個專案解決了現有記憶解決方案的雲端依賴問題，提供了一個完全本地化的選擇。這種設計使得用戶能夠完全控制自己的數據，避免了雲端存儲的隱私風險。社群的活躍度也顯示出對這個專案的需求，尤其是在開源和自我托管的環境中。這些因素共同推動了專案的快速成長。

## 適合誰使用

**目標受眾**：需要在本地環境中運行持久記憶系統以支持 Hermes Agent 的開發者。

> [!example] 使用場景
> - AI 研究者用它來持久化對話歷史，因為這樣可以在每次會話中快速調用之前的決策，避免重複工作。
> - 開發者用它來記錄和管理專案進度，因為它能夠自動整理和回顧過去的關鍵決策，提升團隊協作效率。
> - 產品經理用它來跟蹤用戶反饋和需求變更，因為它提供了結構化的事實存儲和智能上下文注入，讓決策過程更加透明。

## 架構分析

Memory OS 的架構由七層組成，每一層都專注於特定的記憶功能。第一層至第六層負責記憶的捕獲、存儲和注入，而第七層則確保這些記憶被有效利用。這樣的設計使得用戶能夠在本地環境中管理記憶，避免了雲端依賴。

每一層之間的資料流動是通過明確的接口進行，確保了系統的靈活性和可擴展性。選擇使用 Qdrant 作為向量數據庫，因為其支持高效的相似性搜索，並且能夠處理大量數據。這種架構的代價在於需要更多的本地資源來運行，但這也意味著用戶擁有更高的數據隱私和控制權。

整體來看，這種設計適合需要長期記憶和上下文管理的應用場景。

## 技術深入分析

Memory OS 的核心技術機制在於其七層架構，這使得記憶的捕獲、存儲和注入變得更加高效。每一層都專注於特定的功能，例如第一層的工作區記憶和第五層的向量數據庫 Qdrant，這樣的設計使得系統能夠處理大量的對話歷史和結構化事實。效能方面，系統能夠在本地環境中高效運行，並且支持 O(1) 的路徑查找，這對於需要快速檢索的應用場景非常重要。選擇 Python 作為主要語言，因為其在數據處理和機器學習領域的廣泛應用，這使得開發者能夠輕鬆擴展功能。技術風險方面，隨著用戶數量的增加，系統的性能可能會受到影響，特別是在記憶數據量大時。整合方面，與主流框架的相容性良好，但對於新手來說，學習曲線可能會稍陡峭。整體而言，Memory OS 提供了一個強大的本地記憶管理解決方案，適合需要長期記憶的開發者。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝指導和範例，安裝過程相對順暢，但對於新手來說可能需要一些時間來理解整個架構。文件中沒有提供中文或多語言支持，這可能對非英語使用者造成一定障礙。整體而言，花 30 分鐘能夠完成基本的安裝和配置，但深入理解系統的運作可能需要更長的時間。

## 優缺點分析

> [!success] 優點
> - 提供完整的本地記憶基礎設施，無需雲端服務。
> - 智能上下文注入提高了對話的連貫性和效率。
> - 支持多種 LLM 提供者，避免供應商鎖定。

> [!danger] 缺點
> - 需要較高的本地資源來運行 Docker 和相關服務。
> - 對於非 Hermes Agent 的用戶，適用性有限。
> - 安裝和配置過程對新手來說可能有一定挑戰。

> [!warning] 注意事項
> - 僅支援 Python 3.11+。
> - 需要 Docker 環境來運行 Qdrant 和 Redis。
> - 目前僅針對 Hermes Agent 設計，對其他代理不一定適用。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| mem0 | mem0 是一個雲端記憶解決方案，無法提供本地化的記憶基礎設施，適合需要雲端存儲的用戶。 |
| Zep | Zep 主要是針對雲端環境設計，缺乏本地運行的選項，對於需要本地存儲的用戶不夠理想。 |
| Letta | Letta 也依賴雲端服務，無法提供完全的本地控制，對於隱私要求高的用戶不適合。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| mem0 | mem0 是一個雲端記憶解決方案，無法提供本地化的記憶基礎設施，適合需要雲端存儲的用戶。 | 如果你的團隊已經在使用雲端記憶解決方案且不需要本地存儲。 | medium，因為需要重新設計數據存儲和訪問方式。 |
| Zep | Zep 主要是針對雲端環境設計，缺乏本地運行的選項，對於需要本地存儲的用戶不夠理想。 | 如果你的應用場景完全依賴雲端服務，且不需要本地數據控制。 | medium，因為需要調整 API 調用和數據流。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **mem0** | **Zep** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | mem0 是一個雲端記憶解決方案，無法提供本地化的記憶基礎設施，適合需要雲端存儲的用戶。 | Zep 主要是針對雲端環境設計，缺乏本地運行的選項，對於需要本地存儲的用戶不夠理想。 |
> | 遷移成本 | - | medium，因為需要重新設計數據存儲和訪問方式。 | medium，因為需要調整 API 調用和數據流。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用雲端記憶解決方案且不需要本地存儲。 | 如果你的應用場景完全依賴雲端服務，且不需要本地數據控制。 |
...
> [!tip] 採用建議
> 適合個人測試和小型專案，但不建議在生產環境的核心路徑上使用。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在某些環境中安裝過程可能會出現 Docker 兼容性問題
  - 解法：確保 Docker 版本符合要求，並檢查系統資源
- [MEDIUM] 使用過程中可能會遇到記憶重複的問題
  - 解法：調整上下文注入的閾值以減少重複
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 能夠提供本地化的記憶管理，適合小型團隊使用。 |
| 大型企業的 AI 研究部門 | 普通 | 雖然功能強大，但可能需要更多的資源來運行。 |
| 個人開發者的 AI 助手 | 非常適合 | 能夠在本地環境中持久化記憶，提升使用體驗。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習和 3 小時整合，能夠獲得高效的本地記憶管理，值得投資。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：本地運行的系統不需要高權限，且不存取敏感資料，依賴鏈的信任程度高，適合在 CI/CD 中使用。
...
> | --- | --- | --- | --- |
> | [#20](https://github.com/ClaudioDrews/memory-os/issues/20) | fabric_curate cannot find entries: YAML quote mismatch in st | 0 | 1 |
> | [#19](https://github.com/ClaudioDrews/memory-os/issues/19) | Recurring leak of internal mechanics into user-facing conver | 0 | 0 |
