...
**1.2k** stars · **36** stars/天 · 建立 34 天前 · Python · MIT
...
> **授權** MIT (商業友好) · **維護** Moderate (最後推送 24 天前) · **貢獻者** 4+ 人 · **參與度** Low
> **適合** 需要本地運行 AI 記憶系統並希望避免雲端依賴的開發者和研究者。
> **一句話重點** Memory OS 不僅是一個記憶插件，而是一個完整的記憶操作系統，能夠有效地管理和利用 AI 的記憶。
...
> **結論** 花 5 小時學習，10 小時整合，得到一個靈活的本地 AI 記憶系統，值得嘗試。

> [!abstract] 核心創新
> 提供一個完整的七層記憶操作系統，確保 AI 能夠有效利用注入的記憶。

## 專案簡介

Memory OS 是一個為 Hermes Agent 設計的七層記憶操作系統，旨在解決 AI 在長期合作中忘記過去對話和決策的問題。它支持本地運行，無需雲端記憶訂閱，並且能與任何 LLM 提供者兼容。核心流程包括將過去的對話和結構化事實注入到 AI 的上下文中，並通過 Qdrant 和 Redis 進行高效的記憶檢索。使用者可以透過簡單的命令安裝整個系統，例如 `curl -sSL https://raw.githubusercontent.com/ClaudioDrews/memory-os/main/setup.sh | bash`，這樣就能快速搭建起來。Memory OS 的一大賣點是其獨特的 Ground Truth 層，確保注入的記憶被 AI 正確使用，而不是重複檢索，從而節省了計算資源和時間。這個系統的設計考慮到了用戶的隱私，所有數據都保存在本地，並提供了智能的上下文注入和自我策展的維基功能。與其他記憶解決方案相比，Memory OS 不依賴雲端，並且提供了更高的靈活性和控制權，特別適合需要長期記憶的 AI 應用場景。

**技術棧**：`Python 3.11` · `Docker` · `Redis` · `Qdrant`

## 重點功能

- 七層記憶架構 — 包含工作區、會話、結構化事實、跨會話回憶、向量數據庫等。
- 智能上下文注入 — 自動將相關記憶注入 AI 的上下文中，提升對話的連貫性。
- Ground Truth 層 — 確保注入的記憶被 AI 正確使用，避免重複檢索。
- 本地運行 — 無需雲端服務，所有數據保存在本地，保障用戶隱私。
- 自我策展的維基功能 — 自動整理和更新知識庫，方便用戶查詢。
...
> [!tip] 爆紅原因
> 建立 34 天內累積 1224 stars（36/天），forks 116（9.5%），顯示出強烈的社群關注。這個專案由 ClaudioDrews 等人開發，他們在 AI 記憶系統方面有豐富的經驗。Memory OS 解決了現有記憶方案的痛點，特別是雲端依賴和記憶持久性不足的問題，讓用戶能夠在本地運行記憶系統。最近的推特討論和社群反饋也促進了其快速增長，顯示出對本地化記憶解決方案的需求上升。forks/stars 比率為 9.5%，顯示出許多用戶對於修改和擴展此專案的興趣。

## 適合誰使用

**目標受眾**：需要本地運行 AI 記憶系統並希望避免雲端依賴的開發者和研究者。

> [!example] 使用場景
> - AI 研究員用它來持久化對話歷史，因為這樣可以避免重複教導 AI 相同的知識，節省時間和精力。
> - 產品經理用它來記錄會議決策，因為它能自動整理和回顧過去的會議記錄，提升團隊協作效率。
> - 開發者用它來管理多個專案的上下文，因為它能夠在不同專案間快速切換並保持記憶，避免信息遺失。

## 架構分析

Memory OS 的架構由七層組成，從工作區到向量數據庫，確保記憶的捕獲、存儲和注入。每一層都專注於特定的功能，例如工作區層負責基本的記憶存儲，而向量數據庫層則提供高效的檢索能力。這種設計使得記憶的使用更加靈活和高效，特別是 Ground Truth 層的引入，確保了注入的記憶能被 AI 正確使用，避免了重複檢索的浪費。這樣的架構設計使得 Memory OS 能夠在本地環境中運行，並且能夠與多種 LLM 提供者兼容，提供了更高的靈活性和控制權。整體架構的設計考慮到了用戶的隱私和數據安全，所有數據都保存在本地，避免了雲端依賴的風險。

## 技術深入分析

Memory OS 的核心技術機制是其七層記憶架構，這些層級包括工作區、會話、結構化事實、跨會話回憶、向量數據庫等。這樣的設計使得記憶的捕獲、存儲和檢索變得更加高效。特別是使用 Qdrant 作為向量數據庫，支持 4096 維的餘弦相似度檢索，並結合 FTS5 提供全文本搜索，能夠快速找到相關的記憶。設計上選擇本地運行的架構，避免了雲端依賴，這對於需要保護隱私的用戶來說是個重要優勢。這個系統的依賴樹相對簡單，主要依賴於 Python 和 Docker，降低了使用的複雜度。技術風險方面，隨著用戶數量的增加，可能會面臨性能瓶頸，特別是在記憶數據量大時。整合方面，Memory OS 能夠與多種 LLM 提供者兼容，但在實際使用中，可能需要進行一些配置來確保最佳性能。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且提供了安裝指令，但對於新手來說，Docker 的使用可能會造成一些困難。安裝過程相對順暢，但需要注意 Docker 的配置。文件沒有提供多語言支持，僅有英文版本。

## 優缺點分析

> [!success] 優點
> - 提供本地運行的記憶基礎設施，保障用戶隱私。
> - 智能上下文注入功能，提升對話的連貫性和效率。
> - 支持多種 LLM 提供者，靈活性高。

> [!danger] 缺點
> - 需要 Docker 環境，對於不熟悉的用戶有學習曲線。
> - 目前僅支援 Python 3.11+，限制了使用者範圍。
> - 安裝過程可能對新手不夠友好，需要一定的技術背景。

> [!warning] 注意事項
> - 僅支援 Python 3.11+。
> - 需要 Docker 環境運行，對於不熟悉 Docker 的用戶可能有學習曲線。
> - 目前僅支援特定的 LLM 提供者，雖然是開放的，但整合可能需要額外的配置。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [mem0/mem0](https://github.com/mem0/mem0) | mem0 提供雲端記憶解決方案，而 Memory OS 則專注於本地運行，無需雲端依賴。 |
| [Zep/zep](https://github.com/Zep/zep) | Zep 主要是雲端服務，無法提供本地化的記憶基礎設施，而 Memory OS 提供完整的本地解決方案。 |
| [Letta/let](https://github.com/Letta/let) | Letta 也依賴雲端，並且缺乏 Memory OS 的智能上下文注入和自我策展功能。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [mem0/mem0](https://github.com/mem0/mem0) | mem0 提供雲端記憶解決方案，無法提供本地化的記憶基礎設施，這使得 Memory OS 在隱私和控制上更具優勢。 | 如果你的團隊已經在使用雲端解決方案並且不需要本地化的記憶管理，mem0 可能更適合。 | medium，因為需要將數據從雲端遷移到本地。 |
| [Zep/zep](https://github.com/Zep/zep) | Zep 主要是雲端服務，缺乏 Memory OS 的智能上下文注入和自我策展功能。 | 如果你的團隊需要快速部署雲端解決方案，Zep 可能更方便。 | high，因為需要重新設計記憶管理流程。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **mem0** | **zep** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | mem0 提供雲端記憶解決方案，無法提供本地化的記憶基礎設施，這使得 Memory OS 在隱私和控制上更具優勢。 | Zep 主要是雲端服務，缺乏 Memory OS 的智能上下文注入和自我策展功能。 |
> | 遷移成本 | - | medium，因為需要將數據從雲端遷移到本地。 | high，因為需要重新設計記憶管理流程。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用雲端解決方案並且不需要本地化的記憶管理 | 如果你的團隊需要快速部署雲端解決方案，Zep 可能更方便。 |
...
> [!tip] 採用建議
> 適合個人或小型團隊的實驗性項目，不建議在生產環境的核心路徑上使用。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在某些環境下，Docker 啟動可能會遇到問題，特別是資源不足的情況。
  - 解法：確保 Docker 有足夠的資源配置，並檢查系統日誌以獲取詳細錯誤信息。
- [MEDIUM] 某些 LLM 提供者的整合可能需要額外的配置，導致使用者困惑。
  - 解法：參考官方文檔中的整合指南，確保正確配置。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 AI 系統 | 非常適合 | 本地運行的特性和持久記憶功能能夠滿足小型團隊的需求。 |
| 大型企業的 AI 記憶管理 | 不適合 | 可能面臨性能瓶頸，且不具備雲端擴展能力。 |
| 個人開發者的 AI 實驗項目 | 非常適合 | 提供靈活的本地環境，適合快速實驗和迭代。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，10 小時整合，得到一個靈活的本地 AI 記憶系統，值得嘗試。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：本地運行的系統不需要高權限，所有數據保存在本地，避免了雲端數據洩露的風險。
