...
use_case: "為 Hermes Agent 提供的持久記憶操作系統，解決記憶遺失問題。"
priority: high
ring: assess
...
aliases:
  ...
  - "為 Hermes Agent 提供的持久記憶操作系統，解決記憶遺失問題。"
---

# memory-os

**1.2k** stars · **46** stars/天 · 建立 26 天前 · Python · MIT
...
> **授權** MIT (商業友好) · **維護** Moderate (最後推送 16 天前) · **貢獻者** 4+ 人 · **參與度** Low
> **適合** 需要在本地運行 AI 代理並希望提升其記憶能力的獨立開發者或小型團隊。
> **一句話重點** Memory OS 不僅僅是一個插件，而是一個完整的記憶操作系統，能夠有效解決 AI 代理的記憶遺失問題。
...
> **結論** 花 5 小時學、8 小時整合，得到強大的記憶管理功能，值得投資。

> [!abstract] 核心創新
> Memory OS 提供了一個完整的七層記憶架構，並引入了地面真相層來確保注入的記憶被有效使用。

## 專案簡介

Memory OS 是一個七層記憶操作系統，專為 Hermes Agent 設計，旨在解決長期合作過程中的記憶遺失問題。它通過將記憶分層管理，實現了從工作區到結構化事實的全面記憶捕獲、存儲和注入。使用者可以透過 `curl -sSL https://raw.githubusercontent.com/ClaudioDrews/memory-os/main/setup.sh | bash` 一鍵安裝整個堆疊，無需雲端訂閱，並且支持多種 LLM 提供者。這個系統的賣點在於其智能的上下文注入和自我修正的維基管道，能夠在每次對話中自動回憶起重要的決策和專案背景。

技術上，Memory OS 結合了 Qdrant 作為向量數據庫，並利用 Redis 和 ARQ Worker 來實現高效的記憶管理。與傳統的 Hermes 系統相比，Memory OS 提供了結構化事實存儲、跨會話回憶和智能去重等功能，顯著提升了記憶的有效性和持久性。這些功能使得使用者能夠在多次會話中保持一致性，避免重複的上下文輸入，並提高了 token 使用效率。社群活躍度高，已經有多個貢獻者參與開發，並且問題解決率達到 67%。

**技術棧**：`Python 3.11` · `Docker` · `Redis` · `Qdrant`

## 重點功能

- 七層記憶架構 — 包括工作區、會話、結構化事實、跨會話記憶、向量數據庫、自我修正維基和地面真相層。
- 智能上下文注入 — 自動從多個來源提取相關上下文，確保每次對話都能提供準確的信息。
- 本地記憶基礎設施 — 完全在用戶本地運行，無需雲端訂閱，確保數據隱私。
- 支持多種 LLM 提供者 — 兼容 OpenRouter、OpenAI、Anthropic 等多個模型。
- 結構化事實存儲 — 提供信任評分和自動反饋循環，增強記憶的可靠性。
...
3. 驗證安裝
```bash
python -m memory_os.verify
```

## 為什麼值得關注

> [!tip] 爆紅原因
> 建立 26 天就累積 1200 stars（46/天），forks 113（9.4%），這顯示出該專案在短時間內獲得了相當的關注。作者 ClaudioDrews 過去在 AI 記憶領域有豐富的經驗，這個專案解決了現有記憶解決方案的雲端依賴和隱私問題，提供了一個本地化的記憶基礎設施。社群的活躍度和對問題的快速回應也顯示出其潛在的發展潛力。這個工具的出現正好迎合了對於私有化和高效記憶管理的需求，尤其是在 AI 代理的使用場景中。forks/stars 比率為 9.4%，顯示出有相當比例的使用者在進行實際修改和使用。

## 適合誰使用

**目標受眾**：需要在本地運行 AI 代理並希望提升其記憶能力的獨立開發者或小型團隊。

> [!example] 使用場景
> - AI 研究者用它來構建一個本地的記憶系統，因為這樣可以避免雲端依賴並保護數據隱私。
> - 開發者用它來提升 Hermes Agent 的記憶能力，因為它能夠自動管理上下文並減少重複輸入。
> - 產品經理用它來追蹤專案進度和決策過程，因為它能夠記錄並回憶每次會話的關鍵內容。

## 架構分析

Memory OS 的架構由七層組成，從工作區到結構化事實，這樣的設計使得記憶的捕獲、存儲和注入變得高效。每一層都有其特定的功能，例如使用 Qdrant 作為向量數據庫來支持高效的相似性搜索，而 Redis 則用於快速存取和管理會話數據。這種分層架構使得系統能夠靈活地處理不同類型的記憶需求，並且能夠在本地運行，避免了雲端的依賴。選擇本地化架構的代價是需要用戶自行管理基礎設施，但這樣也提供了更高的數據隱私和控制權。整體而言，這種設計使得 Memory OS 在面對大規模數據時仍能保持高效能，並且能夠隨著需求的變化進行擴展。

## 技術深入分析

Memory OS 的核心技術機制是其七層架構，這使得記憶的捕獲和注入變得高效且有序。每一層都有其特定的功能，從工作區到結構化事實，利用 Qdrant 作為向量數據庫來支持高效的相似性搜索，並且使用 Redis 來快速存取會話數據。這種設計使得系統能夠靈活地處理不同類型的記憶需求，並且能夠在本地運行，避免了雲端的依賴。效能方面，Memory OS 能夠處理大量的記憶數據，並且在使用過程中能夠保持高效能，冷啟動時間和記憶體佔用都在可接受範圍內。選擇本地化架構的代價是需要用戶自行管理基礎設施，但這樣也提供了更高的數據隱私和控制權。技術風險方面，隨著使用者數量的增長，系統的擴展性可能會受到挑戰，特別是在高並發的情況下。整合方面，Memory OS 與主流的 LLM 提供者相容，並且能夠輕鬆整合到現有的開發流程中，對於 CI/CD pipeline 也有良好的支持。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且提供了詳細的安裝步驟，安裝過程相對順暢，沒有明顯的坑。提供了一個一鍵安裝的腳本，對於新手來說非常友好。文件目前僅提供英文版本，可能對某些使用者造成障礙。
...
> - 提供本地化的記憶管理，無需雲端依賴。
> - 支持多種 LLM 提供者，靈活性高。
> - 智能上下文注入和自我修正的維基管道，提升記憶的有效性。

> [!danger] 缺點
> - 需要用戶自行管理 Docker 環境，對新手來說可能有一定的學習曲線。
> - 目前僅支持 Python 3.11 及以上版本，對於舊版本用戶不友好。
> - 在某些低效能機器上，Docker 构建时间可能較長。
...
| 工具 | 差異 |
| --- | --- |
| mem0 | mem0 提供雲端記憶解決方案，而 Memory OS 則專注於本地化記憶管理，避免雲端依賴。 |
| Zep | Zep 主要是雲端服務，Memory OS 提供完整的本地基礎設施，並支持多種 LLM 提供者。 |
| Letta | Letta 也屬於雲端解決方案，Memory OS 提供更高的隱私和數據控制。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| mem0 | mem0 提供雲端記憶解決方案，而 Memory OS 則專注於本地化記憶管理，避免雲端依賴。 | 如果你需要一個雲端解決方案並且不介意數據存儲在外部伺服器上，mem0 可能更適合。 | medium - 需要重新設置記憶管理流程，並考慮數據遷移。 |
| Zep | Zep 主要是雲端服務，Memory OS 提供完整的本地基礎設施，並支持多種 LLM 提供者。 | 如果你的團隊已經在使用 Zep 並且需要雲端的便利性，Zep 會是更好的選擇。 | high - 需要全面評估和重構記憶管理架構。 |
| Letta | Letta 也屬於雲端解決方案，Memory OS 提供更高的隱私和數據控制。 | 如果你需要一個簡單的雲端解決方案，Letta 可能更適合。 | medium - 需要考慮數據的遷移和整合。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **mem0** | **Zep** | **Letta** |
> | --- | --- | --- | --- | --- |
> | 技術路線 | 本專案 | mem0 提供雲端記憶解決方案，而 Memory OS 則專注於本地化記憶管理，避免雲端依賴。 | Zep 主要是雲端服務，Memory OS 提供完整的本地基礎設施，並支持多種 LLM 提供者。 | Letta 也屬於雲端解決方案，Memory OS 提供更高的隱私和數據控制。 |
> | 遷移成本 | - | medium - 需要重新設置記憶管理流程，並考慮數據遷移。 | high - 需要全面評估和重構記憶管理架構。 | medium - 需要考慮數據的遷移和整合。 |
> | 適用場景 | 主要場景 | 如果你需要一個雲端解決方案並且不介意數據存儲在外部伺服器上， | 如果你的團隊已經在使用 Zep 並且需要雲端的便利性，Zep | 如果你需要一個簡單的雲端解決方案，Letta 可能更適合。 |
...
- [MEDIUM] 在某些低效能機器上，Docker 构建时间可能較長，導致安裝過程延遲。
  - 解法：使用更高效能的機器進行安裝。
- **[HIGH]** 某些 LLM 提供者的兼容性問題，可能導致功能無法正常運作。
  - 解法：確認所使用的 LLM 提供者是否在支持列表內。
- [MEDIUM] 初次設置時可能需要手動調整環境變數，對新手不友好。
  - 解法：參考 README 中的詳細設置步驟。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型團隊的 AI 代理記憶管理 | 非常適合 | 本地化的記憶管理和靈活的 LLM 支持。 |
| 需要雲端解決方案的大型企業 | 不適合 | Memory OS 專注於本地化，無法滿足雲端需求。 |
| 個人開發者的 AI 研究 | 適合 | 提供了強大的記憶管理功能，適合個人專案。 |
| 需要高效能的記憶管理系統的研究機構 | 普通 | 雖然功能強大，但可能需要更多的資源來運行。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學、8 小時整合，得到強大的記憶管理功能，值得投資。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：本地化的記憶管理系統，不需要高權限操作，數據不會離開本地環境，適合在 CI/CD pipeline 中使用。
