...
**1.3k** stars · **23** stars/天 · 建立 57 天前 · Python · MIT
...
> **授權** MIT (商業友好) · **維護** Slow (最後推送 47 天前) · **貢獻者** 4+ 人 · **參與度** Low
> **適合** 需要在本地環境中運行長期記憶 AI 的開發者和團隊。
> **一句話重點** Memory OS 的創新在於其完整的七層記憶架構，確保 AI 能夠有效使用過去的記憶。
...
> **結論** 花 10 小時學、5 小時整合，得到靈活的本地記憶解決方案，值得投入。

> [!abstract] 核心創新
> Memory OS 提供了一個完整的七層記憶操作系統，確保 AI 能夠有效使用注入的記憶。

## 專案簡介

Memory OS 是一個針對 Hermes Agent 設計的七層記憶操作系統，旨在解決 AI 在長期交互中記憶喪失的問題。它通過將記憶分層管理，從工作區到會話、結構化事實、跨會話回憶，最終到向量數據庫 Qdrant，實現了智能的上下文注入。用戶只需運行一條命令 `curl -sSL https://raw.githubusercontent.com/ClaudioDrews/memory-os/main/setup.sh | bash` 即可快速安裝整個堆棧，無需雲端訂閱，並支持多種 LLM 提供商。這個系統的賣點在於其「地面真相層級」，確保注入的記憶被 AI 正確使用，避免了重複查詢的資源浪費。技術上，Memory OS 使用了 Python、Docker 和 Redis，並依賴於 Qdrant 進行向量搜索，確保高效的記憶檢索。

與其他記憶解決方案相比，如 mem0 和 Zep，Memory OS 提供了本地運行的記憶基礎設施，避免了雲端鎖定和數據外洩的風險。使用者可以在本地環境中完全控制自己的數據，並享受無需訂閱的靈活性。這個系統特別適合需要長期記憶的 AI 應用場景，如個人助理或專業協作工具。由於其設計的複雜性，對於初學者來說，學習曲線可能較陡峭，但其強大的功能和靈活性使其值得投入時間學習。

**技術棧**：`Python 3.11` · `Docker` · `Redis` · `Qdrant`

## 重點功能

- 七層記憶架構 — 包含工作區、會話、結構化事實、跨會話回憶等層級，提供全面的記憶管理。
- 地面真相層級 — 確保注入的記憶被 AI 正確使用，避免重複查詢。
- 支持多種 LLM 提供商 — 包括 OpenRouter、OpenAI、Anthropic 等，無需雲端訂閱。
- 自動化維基功能 — 持續更新和整理知識庫，支持快速檢索。
- 高效的向量搜索 — 使用 Qdrant 進行多層次的記憶檢索，提供更好的性能和準確性。
...
> [!tip] 爆紅原因
> 建立 57 天內累積 1303 stars（約23 stars/天），forks 122（9.4%），顯示出穩定的增長趨勢。作者 ClaudioDrews 之前在 AI 記憶領域有豐富經驗，這個專案解決了現有記憶解決方案的雲端鎖定問題，提供了本地運行的替代方案。近期在社群中引起討論，尤其是針對如何與 Hermes Agent 整合的問題。技術上，隨著本地計算能力的提升，這種解決方案變得越來越可行，並且 forks/stars 比率為 9.4%，顯示出使用者對此專案的積極參與。

## 適合誰使用

**目標受眾**：需要在本地環境中運行長期記憶 AI 的開發者和團隊。

> [!example] 使用場景
> - AI 開發者用它來構建長期記憶的虛擬助手，因為它能夠持久保存對話歷史，避免重複教學。
> - 產品經理用它來記錄會議決策和產品路線圖，因為它的結構化事實存儲能夠快速檢索過去的決策。
> - 研究人員用它來整理實驗數據和結果，因為自動化的維基功能可以持續更新和整理知識庫。

## 架構分析

Memory OS 的架構設計為七層，從工作區到向量數據庫，確保記憶的捕獲、存儲和注入。每一層都有其特定的功能，如使用 SQLite 進行會話管理，利用 Qdrant 進行向量搜索。這種設計使得記憶的檢索和使用更加高效，並且通過地面真相層級來確保注入的記憶被 AI 正確使用。選擇本地運行的架構使得用戶能夠完全控制數據，避免了雲端鎖定的風險。這樣的設計雖然在初始設置上可能較為複雜，但長期來看能夠提供更高的靈活性和安全性。

## 技術深入分析

Memory OS 的核心技術機制在於其七層記憶架構，這些層級分別處理不同類型的記憶，如工作區、會話和結構化事實。這些層級的設計使得記憶的捕獲和檢索更加高效，特別是使用 Qdrant 進行向量搜索時，能夠提供多層次的回退機制以確保準確性。這個系統的效能特性使其能夠處理大量的對話數據，並且在本地環境中運行，避免了雲端的延遲和數據外洩風險。選擇 Python 和 Docker 作為技術棧使得開發和部署變得靈活，但也帶來了對 Docker 環境的依賴。技術風險方面，隨著使用者數量的增加，如何保持系統的穩定性和效能將是未來的挑戰。整合方面，Memory OS 可以與現有的開發工具鏈良好結合，特別是在 CI/CD 流程中，能夠提供自動化的記憶管理功能。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝指導和範例，安裝過程相對順暢，但對於不熟悉 Docker 的用戶可能會有一些挑戰。整體來說，文件的質量較高，能夠幫助新手快速上手。

## 優缺點分析

> [!success] 優點
> - 提供本地運行的記憶基礎設施，避免雲端鎖定。
> - 支持多種 LLM 提供商，靈活性高。
> - 自動化維基功能能夠持續更新知識庫，提升使用效率。

> [!danger] 缺點
> - 需要 Docker 環境，對於不熟悉的用戶有學習曲線。
> - 目前僅支援特定的 LLM 提供商，對於其他提供商的支持可能有限。
> - 安裝和配置過程可能對新手不夠友好。
...
| 工具 | 差異 |
| --- | --- |
| mem0 | mem0 是一個雲端記憶解決方案，無法提供本地運行的選項，適合需要雲端存取的場景。 |
| Zep | Zep 主要針對雲端使用，缺乏本地數據控制，適合不介意數據外洩的用戶。 |
| Letta | Letta 也為雲端解決方案，無法提供持久記憶的本地基礎設施，適合快速開發的場景。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| mem0 | mem0 是一個雲端記憶解決方案，無法提供本地運行的選項，適合需要雲端存取的場景。 | 如果你的團隊已經在使用雲端解決方案且不需要本地數據控制。 | medium，因為需要重新設計數據存儲和檢索邏輯。 |
| Zep | Zep 主要針對雲端使用，缺乏本地數據控制，適合不介意數據外洩的用戶。 | 如果你的應用場景不需要長期記憶，且可以接受雲端存取。 | medium，因為需要適應其雲端架構和API。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **mem0** | **Zep** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | mem0 是一個雲端記憶解決方案，無法提供本地運行的選項，適合需要雲端存取的場景。 | Zep 主要針對雲端使用，缺乏本地數據控制，適合不介意數據外洩的用戶。 |
> | 遷移成本 | - | medium，因為需要重新設計數據存儲和檢索邏輯。 | medium，因為需要適應其雲端架構和API。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用雲端解決方案且不需要本地數據控制。 | 如果你的應用場景不需要長期記憶，且可以接受雲端存取。 |
...
- **[HIGH]** 在某些環境下，Docker 啟動可能會遇到性能問題
  - 解法：確保 Docker 設置符合官方建議，並在高效能環境中運行。
- [MEDIUM] 初次設置時可能會遇到依賴問題
  - 解法：仔細檢查 requirements.txt，確保所有依賴都已安裝。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 能夠提供本地運行的記憶基礎設施，避免雲端鎖定。 |
| 需要長期記憶的個人助理應用 | 非常適合 | 支持多種 LLM 提供商，靈活性高。 |
| 大型企業的雲端記憶解決方案 | 不適合 | 缺乏對雲端數據的支持，無法滿足大型企業的需求。 |
...
> [!tip] 投入 vs 回報
> 花 10 小時學、5 小時整合，得到靈活的本地記憶解決方案，值得投入。
