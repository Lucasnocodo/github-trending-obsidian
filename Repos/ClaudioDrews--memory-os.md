...
use_case: "提供一個本地的持久記憶系統，讓 Hermes Agent 能夠記住過去的對話和決策。"
priority: high
ring: assess
...
aliases:
  ...
  - "提供一個本地的持久記憶系統，讓 Hermes Agent 能夠記住過去的對話和決策。"
---

# memory-os

**1.3k** stars · **24** stars/天 · 建立 53 天前 · Python · MIT
...
> **授權** MIT (商業友好) · **維護** Slow (最後推送 43 天前) · **貢獻者** 4+ 人 · **參與度** Low
> **適合** 需要在本地運行 AI 記憶系統以保護數據隱私的開發者和研究者。
> **一句話重點** Memory OS 不僅是一個記憶插件，而是一個完整的記憶操作系統，解決了 AI 交互中的記憶問題。
...
> **結論** 花 5 小時學習，10 小時整合，得到一個靈活且安全的記憶系統，值得採用。

> [!abstract] 核心創新
> Memory OS 提供了一個完整的七層記憶架構，並且具備真實性層以確保注入的記憶被有效使用。

## 專案簡介

Memory OS 是一個七層的記憶操作系統，專為 Hermes Agent 設計，旨在解決 AI 在長期交互中遺忘關鍵信息的問題。它通過將記憶分為七層來確保信息的捕獲、存儲和注入，最終層的 Ground Truth 層確保了注入的記憶被有效使用。用戶可以通過簡單的指令 `curl -sSL https://raw.githubusercontent.com/ClaudioDrews/memory-os/main/setup.sh | bash` 一鍵安裝整個系統，無需雲端訂閱，且支持多種 LLM 提供者。技術上，Memory OS 結合了 Qdrant 作為向量數據庫，使用 Redis 和 ARQ 進行數據存儲和處理，並且採用 SQLite 進行會話記錄的全文本搜索。

這種架構設計的優勢在於能夠在本地運行，避免了數據外洩的風險，並且提供了靈活的供應商選擇。與其他記憶解決方案相比，Memory OS 提供了更深層次的結構化事實和信任評分系統，這在其他解決方案中並不常見。使用者可能會遇到的問題包括 Docker 環境的配置和依賴管理，但社群的活躍度和開發者的回應速度相對良好，能夠提供支持。這個專案目前處於 beta 階段，適合需要在本地運行 AI 記憶系統的開發者，未來可能會進一步增強其功能和社群支持。

**技術棧**：`Python 3.11` · `Docker` · `Redis` · `Qdrant` · `SQLite`

## 重點功能

- 七層記憶架構 — 包括工作區、會話、結構化事實、跨會話記憶、向量數據庫、LLM 維基和真實性層。
- 本地運行 — 完全在用戶本地機器上運行，無需雲端服務。
- 支持多種 LLM 提供者 — 包括 OpenRouter、OpenAI、Anthropic 和 Ollama。
- 智能上下文注入 — 自動注入相關的記憶和上下文，提升交互的連貫性。
- 自我整理的維基系統 — 自動整理和更新知識庫，便於查詢和使用。
...
> [!tip] 爆紅原因
> 建立 53 天就累積 1294 stars（24/天），forks 120（9.3%），顯示出穩定的增長。作者 ClaudioDrews 之前在 AI 記憶和 Hermes Agent 領域有豐富經驗，這個專案解決了現有記憶系統多數依賴雲端的痛點，提供了一個本地化的解決方案。最近的推廣和社群討論也促進了它的曝光度，特別是在 StackMap 上的展示。這個工具的出現正好滿足了對於私有化記憶系統的需求，並且在技術上具備了多樣的選擇性，讓使用者能夠靈活應對不同的 LLM 提供者。forks/stars 比率為 9.3%，顯示出有相當比例的使用者在實際修改和使用這個專案。

## 適合誰使用

**目標受眾**：需要在本地運行 AI 記憶系統以保護數據隱私的開發者和研究者。

> [!example] 使用場景
> - AI 研究者用它來在本地環境中持久化記憶，因為這樣可以避免數據外洩並確保隱私。
> - 開發者用它來整合多個 LLM 提供者的記憶功能，因為它支持多種 API 並且不鎖定於特定供應商。
> - 產品經理用它來追蹤項目進度和決策歷史，因為它能夠自動整理和回顧過去的對話和決策。

## 架構分析

Memory OS 的架構由七層組成，第一層是工作區，負責存儲用戶的基本信息和創意；第二層是會話層，使用 SQLite 和 FTS5 進行全文本搜索；第三層是結構化事實層，確保記憶的持久性和信任評分；第四層是跨會話的記憶層，使用 Icarus 插件進行多源注入；第五層是向量數據庫層，使用 Qdrant 進行高效的向量檢索；第六層是自動整理的維基層，持續更新知識庫；第七層是關鍵的真實性層，確保注入的記憶被代理有效使用。這種設計的好處在於能夠在本地運行，避免了雲端依賴的風險，但代價是需要用戶具備一定的技術能力來配置和維護系統。整體架構的擴展性良好，但在高負載情況下可能會遇到性能瓶頸，特別是在記憶數據量大時。

## 技術深入分析

Memory OS 的核心技術機制是其七層架構，從工作區到向量數據庫，每一層都有明確的功能分工。第一層的工作區存儲基本的用戶信息，第二層的會話層使用 SQLite 和 FTS5 進行全文本搜索，這使得用戶能夠快速檢索過去的對話。第三層的結構化事實層則確保了記憶的持久性，並通過信任評分系統來提升記憶的可靠性。第四層的跨會話記憶層使用 Icarus 插件進行多源注入，這樣可以從不同的會話中提取信息。

第五層的向量數據庫層使用 Qdrant 進行高效的向量檢索，並且設計了四級回退機制以確保檢索的準確性。第六層的自動整理維基層則持續更新知識庫，這樣用戶可以隨時查詢最新的信息。第七層的真實性層則確保了注入的記憶被代理有效使用，這是其他記憶系統所缺乏的。整體架構的設計考慮了性能和擴展性，但在高負載情況下可能會遇到瓶頸，特別是在記憶數據量大時。

對於依賴外部 API 的程度，Memory OS 的設計使其能夠在本地運行，降低了對雲端服務的依賴，這在數據隱私上是一個重要的考量。整合到現有的技術生態中，Memory OS 可以與多種 LLM 提供者兼容，並且在 CI/CD pipeline 中也能夠順利運行，這使得它在開發過程中非常友好。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且詳細，提供了安裝和使用的具體指導。安裝過程相對順暢，使用者只需執行一條命令即可完成安裝。文件中包含了良好的入門指南，對於新手來說，花 30 分鐘就能成功運行起來。
...
> - 本地化運行，保護用戶數據隱私。
> - 支持多種 LLM 提供者，靈活性高。
> - 提供深層次的結構化事實和信任評分系統，提升記憶的有效性。

> [!danger] 缺點
> - 需要 Docker 環境，對新手有一定的學習曲線。
> - 目前僅支持 Python 3.11+，限制了使用者的選擇。
> - 在某些硬體上，Docker 啟動時間較長，影響使用體驗。

> [!warning] 注意事項
> - 需要 Docker 環境，對於不熟悉 Docker 的用戶可能有學習曲線。
> - 目前僅支持 Python 3.11+，舊版本的 Python 可能不兼容。
> - 在某些硬體上，Docker 的啟動時間可能較長，影響用戶體驗。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| mem0 | mem0 是一個雲端記憶解決方案，而 Memory OS 提供本地化的記憶基礎設施，避免了數據外洩的風險。 |
| Zep | Zep 主要針對雲端環境，而 Memory OS 支持本地運行，並且提供更深層的結構化事實和信任評分系統。 |
| Letta | Letta 也依賴雲端服務，而 Memory OS 則完全在用戶本地運行，提供更大的靈活性和隱私保障。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| mem0 | mem0 是一個雲端記憶解決方案，而 Memory OS 提供本地化的記憶基礎設施，避免了數據外洩的風險。 | 如果你的團隊已經在使用雲端記憶解決方案並且不擔心數據隱私，mem0 可能更方便。 | medium，因為需要重新配置記憶系統的架構。 |
| Zep | Zep 主要針對雲端環境，而 Memory OS 支持本地運行，並且提供更深層的結構化事實和信任評分系統。 | 如果你的團隊需要雲端協作功能，Zep 可能更適合。 | high，因為需要重新設計記憶系統的整體架構。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **mem0** | **Zep** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | mem0 是一個雲端記憶解決方案，而 Memory OS 提供本地化的記憶基礎設施，避免了數據外洩的風險。 | Zep 主要針對雲端環境，而 Memory OS 支持本地運行，並且提供更深層的結構化事實和信任評分系統。 |
> | 遷移成本 | - | medium，因為需要重新配置記憶系統的架構。 | high，因為需要重新設計記憶系統的整體架構。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用雲端記憶解決方案並且不擔心數據隱私，m | 如果你的團隊需要雲端協作功能，Zep 可能更適合。 |
...
- [MEDIUM] 在某些環境下，Docker 啟動時間可能較長，影響用戶體驗。
  - 解法：確保使用較新版本的 Docker，並在性能較好的機器上運行。
- **[HIGH]** 依賴的庫版本不兼容可能導致安裝失敗。
  - 解法：檢查 requirements.txt 中的庫版本，確保與本地環境相符。
- [MEDIUM] 在使用過程中，可能會遇到記憶重複的情況。
  - 解法：使用內建的去重功能，並定期檢查記憶庫。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 AI 系統 | 非常適合 | 能夠在本地運行，保護數據隱私，且支持多種 LLM 提供者。 |
| 大型企業的雲端 AI 記憶系統 | 不適合 | 主要設計為本地化解決方案，不支持雲端協作。 |
| 需要快速原型開發的個人開發者 | 適合 | 安裝簡單且能快速搭建記憶系統。 |
| 對數據隱私有高要求的研究團隊 | 非常適合 | 完全在本地運行，無需擔心數據外洩。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，10 小時整合，得到一個靈活且安全的記憶系統，值得採用。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：本地運行的系統不需要高權限，且不存取敏感資料，依賴鏈的信任程度較高，適合在 CI/CD 中使用。
