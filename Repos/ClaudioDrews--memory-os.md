...
use_case: "為 Hermes Agent 提供持久記憶的操作系統，讓其能夠記住過去的對話和決策。"
priority: high
ring: assess
...
aliases:
  ...
  - "為 Hermes Agent 提供持久記憶的操作系統，讓其能夠記住過去的對話和決策。"
---

# memory-os

**1.2k** stars · **52** stars/天 · 建立 23 天前 · Python · MIT
...
> **授權** MIT (商業友好) · **維護** Moderate (最後推送 13 天前) · **貢獻者** 4+ 人 · **參與度** Low
> **適合** 需要在本地運行記憶系統並且希望避免雲端依賴的開發者和團隊。
> **一句話重點** Memory OS 不僅是一個插件，而是一個完整的記憶操作系統，能夠有效提升 Hermes Agent 的長期記憶能力。
...
> **結論** 花 5 小時學習，10 小時整合，得到靈活的本地記憶管理系統，值得投入。

> [!abstract] 核心創新
> Memory OS 提供了一個完整的本地記憶操作系統，具備七層記憶架構和智能上下文注入功能。

## 專案簡介

Memory OS 是一個為 Hermes Agent 設計的七層記憶操作系統，能夠在本地運行，並與各種 LLM 提供者兼容。它的核心機制是將過去的對話和決策結構化存儲，並能夠在需要時自動注入相關上下文。使用者只需執行一條命令 `curl -sSL https://raw.githubusercontent.com/ClaudioDrews/memory-os/main/setup.sh | bash` 即可完成整個系統的安裝，這大大簡化了部署過程。這個系統的賣點在於其提供的本地記憶基礎設施，避免了雲端記憶的訂閱費用和數據外洩的風險。

技術上，Memory OS 結合了 Qdrant 作為向量數據庫，並使用 Redis 來管理會話數據，這樣的設計使得系統能夠進行高效的全文本搜索和結構化事實存儲。與其他記憶解決方案相比，Memory OS 提供了更高的靈活性和私密性，因為它不依賴於雲端服務。



在與其他工具的比較中，像 mem0 和 Zep 等解決方案主要是雲端優先，無法提供本地運行的選項，這使得 Memory OS 在需要本地記憶基礎設施的情境下更具優勢。實際使用中，Memory OS 能夠處理大量的會話數據，並且具備自動清理和去重的功能，這在長期使用中能有效減少資源浪費。

目前的使用者反映出一些常見問題，例如在非 Hermes 環境下的兼容性問題，這可能會影響某些用戶的使用體驗。從成熟度來看，這個專案仍在 beta 階段，但其活躍的開發和社群支持使其具備了不錯的未來發展潛力。

對於需要在本地運行記憶系統的開發者，特別是那些已經在使用 Hermes Agent 的團隊，Memory OS 是一個值得考慮的選擇。

**技術棧**：`Python 3.11` · `Docker` · `Redis` · `Qdrant`

## 重點功能

- 七層記憶架構 — 包含工作區、會話、結構化事實、跨會話記憶等功能。
- 本地運行 — 完全在本地機器上運行，無需雲端服務。
- 智能上下文注入 — 自動將過去的對話和決策注入到當前會話中。
- 結構化事實存儲 — 提供事實存儲和信任評分機制。
- 一鍵安裝 — 使用 `curl` 命令即可快速安裝整個系統。
...
> [!tip] 爆紅原因
> 建立 23 天內累積 1189 stars（52/天），forks 112（9.4%），顯示出強烈的社群興趣。這個專案由 ClaudioDrews 主導，他在 AI 記憶領域有豐富的經驗，之前的解決方案多數是雲端優先，無法滿足用戶對隱私和靈活性的需求。這個專案的出現正好填補了這一空白，並且其一鍵安裝的特性降低了使用門檻。社群對於本地記憶解決方案的需求不斷增長，這也促進了專案的快速增長。

## 適合誰使用

**目標受眾**：需要在本地運行記憶系統並且希望避免雲端依賴的開發者和團隊。

> [!example] 使用場景
> - AI 研究員用它來在本地運行 Hermes Agent，因為這樣可以保護敏感數據並避免雲端依賴。
> - 開發者用它來記錄和回顧過去的對話，因為它能夠自動注入上下文，提升工作效率。
> - 產品經理用它來追蹤團隊的決策歷程，因為它提供了結構化的事實存儲和回顧功能。

## 架構分析

Memory OS 採用七層架構，從工作區到向量數據庫，每一層都專注於特定的記憶功能。這樣的設計使得系統能夠高效地捕獲、存儲和注入記憶。每一層的資料流都經過精心設計，以確保上下文的準確性和有效性。

選擇本地運行的架構，雖然增加了初始設置的複雜度，但卻提供了更高的數據隱私和安全性。這樣的設計也使得系統能夠在沒有網絡連接的情況下運行，這對於某些用戶來說是至關重要的。整體架構的擴展性良好，但在高負載的情況下可能會遇到性能瓶頸，特別是在多用戶同時訪問時。

## 技術深入分析

Memory OS 的核心在於其七層架構，這使得記憶的捕獲、存儲和注入變得高效且有序。每一層都專注於特定的功能，例如工作區層負責基本的記憶存儲，而向量數據庫層則利用 Qdrant 進行高效的相似度搜索。這樣的設計不僅提高了系統的性能，還能夠在多種場景下靈活應用。與傳統的雲端解決方案相比，Memory OS 的本地運行特性使得用戶能夠完全控制自己的數據，這對於需要高隱私保護的用戶來說至關重要。隨著用戶數量的增加，系統的性能可能會受到影響，特別是在高負載情況下，這需要進一步的優化和擴展。整體而言，Memory OS 提供了一個創新的解決方案，能夠滿足現代用戶對記憶管理的需求。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且詳細，提供了安裝和使用的具體步驟。安裝過程相對順暢，但需要確保 Docker 環境已正確配置。文件中有良好的入門指南，適合新手快速上手。

## 優缺點分析

> [!success] 優點
> - 提供本地運行的記憶解決方案，無需雲端依賴。
> - 結構化事實存儲和智能上下文注入提高了工作效率。
> - 一鍵安裝簡化了部署過程，降低了使用門檻。

> [!danger] 缺點
> - 目前僅支持 Python 3.11 以上版本，限制了兼容性。
> - 需要 Docker 環境，增加了系統要求。
> - 在非 Hermes 環境下的兼容性問題可能影響使用體驗。

> [!warning] 注意事項
> - 目前僅支持 Python 3.11 以上版本。
> - 需要 Docker 環境來運行 Qdrant 和 Redis。
> - 在非 Hermes 環境下的兼容性可能存在問題。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| mem0 | mem0 是雲端優先的記憶解決方案，無法提供本地運行的選項，適合需要快速部署的場景，但不適合對隱私有高要求的用戶。 |
| Zep | Zep 也屬於雲端解決方案，缺乏本地記憶基礎設施，適合小型專案，但無法滿足大型企業的需求。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| mem0 | mem0 是雲端優先的記憶解決方案，無法提供本地運行的選項，這使得它在隱私保護方面不如 Memory OS。 | 如果你的團隊需要快速部署並且不在意數據隱私，mem0 可能是一個選擇。 | medium，因為需要重新設計數據存儲和管理流程。 |
| Zep | Zep 同樣是雲端解決方案，缺乏本地記憶基礎設施，適合小型專案，但無法滿足大型企業的需求。 | 如果你的專案需要快速的雲端解決方案，且不需要本地運行，Zep 是一個不錯的選擇。 | high，因為需要完全重構記憶管理系統。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **mem0** | **Zep** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | mem0 是雲端優先的記憶解決方案，無法提供本地運行的選項，這使得它在隱私保護方面不如 Memory OS。 | Zep 同樣是雲端解決方案，缺乏本地記憶基礎設施，適合小型專案，但無法滿足大型企業的需求。 |
> | 遷移成本 | - | medium，因為需要重新設計數據存儲和管理流程。 | high，因為需要完全重構記憶管理系統。 |
> | 適用場景 | 主要場景 | 如果你的團隊需要快速部署並且不在意數據隱私，mem0 可能是 | 如果你的專案需要快速的雲端解決方案，且不需要本地運行，Zep |
...
- **[HIGH]** 在非 Hermes 環境下可能會遇到兼容性問題，導致功能無法正常運作。
  - 解法：建議在 Hermes 環境中運行以獲得最佳效果。
- [MEDIUM] Docker 環境配置不當可能導致安裝失敗。
  - 解法：確保 Docker 已正確安裝並運行。
- [MEDIUM] 在高負載情況下，系統性能可能下降。
  - 解法：考慮擴展硬體資源以應對高並發需求。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 能夠在本地運行，保護敏感數據，並提供靈活的記憶管理。 |
| 大型企業的數據分析平台 | 不適合 | 目前在高負載情況下性能可能不佳，需進一步優化。 |
| AI 研究團隊進行長期項目追蹤 | 適合 | 提供結構化的記憶存儲和自動上下文注入功能，提升效率。 |
| 個人開發者的實驗性專案 | 非常適合 | 簡單的安裝過程和本地運行特性使其成為理想選擇。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，10 小時整合，得到靈活的本地記憶管理系統，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：本工具在本地運行，不需要高權限，且不會存取敏感資料。依賴鏈中無已知的供應鏈風險，適合在 CI/CD 中使用。
