...
**1.2k** stars · **33** stars/天 · 建立 37 天前 · Python · MIT
...
> **授權** MIT (商業友好) · **維護** Moderate (最後推送 27 天前) · **貢獻者** 4+ 人 · **參與度** Low
> **適合** 希望在本地環境中運行 AI 記憶系統的開發者和研究者。
> **一句話重點** Memory OS 的設計理念在於提供一個完整的本地記憶解決方案，讓 AI 能夠真正記住用戶的過去。
...
> **結論** 花 5 小時學習，10 小時整合，得到一個高效的本地 AI 記憶系統，值得投入。

> [!abstract] 核心創新
> Memory OS 提供了一個完整的七層記憶架構，並且具備智能上下文注入功能，讓 AI 能夠有效利用注入的記憶。

## 專案簡介

Memory OS 是一個專為 Hermes Agent 設計的七層記憶操作系統，旨在解決 AI 在對話中遺忘上下文的問題。用戶可以透過簡單的指令 `curl -sSL https://raw.githubusercontent.com/ClaudioDrews/memory-os/main/setup.sh | bash` 一鍵安裝整個系統，並運行在本地環境中。這個系統的核心在於其七層架構，從工作區、會話記憶到結構化事實，最終通過一個稱為「Ground Truth Hierarchy」的層級來確保注入的記憶被有效使用。這樣的設計不僅提高了記憶的持久性，還能減少重複查詢的 token 消耗，讓 AI 更加高效。技術上，Memory OS 使用了 Qdrant 作為向量數據庫，並結合 Redis 和 ARQ Worker 來處理記憶的存取和管理。

與其他記憶解決方案相比，Memory OS 提供了本地運行的選擇，避免了雲端依賴，並且是供應商無關的，這意味著用戶可以自由選擇 LLM 提供者。這使得它特別適合需要高隱私和控制的用戶。使用者在實際操作中可能會遇到的問題包括 Docker 安裝的兼容性，以及如何有效利用其自動化的維護功能。整體來看，這是一個穩定的解決方案，適合希望提升 AI 記憶能力的開發者和研究者。

**技術棧**：`Python 3.11` · `Docker` · `Redis` · `Qdrant`

## 重點功能

- 七層記憶架構 — 包含工作區、會話、結構化事實等層級，確保記憶的有效存取和使用。
- 本地運行 — 完全在用戶的機器上運行，無需雲端依賴，保護用戶隱私。
- 智能上下文注入 — 透過 Ground Truth Hierarchy 確保注入的記憶被 AI 有效使用。
- 支持多種 LLM 提供者 — 包括 OpenRouter、OpenAI、Anthropic 等，靈活性高。
- 自動維護和清理 — 提供智能的記憶清理和去重功能，保持數據的整潔。
...
> [!tip] 爆紅原因
> 建立 37 天內累積 1232 stars（33/天），forks 117（9.5%），顯示出強勁的增長勢頭。這個專案的作者 Claudio Drews 之前在 AI 記憶領域有過多次探索，解決了許多現有記憶方案的痛點，特別是關於私有化和本地化的需求。這個專案的推出正好填補了市場上對於本地記憶基礎設施的需求，並且在社群中引起了廣泛的討論。技術上，這個工具的設計理念與當前的 AI 記憶技術趨勢相符，特別是對於持久性和上下文注入的需求。

## 適合誰使用

**目標受眾**：希望在本地環境中運行 AI 記憶系統的開發者和研究者。

> [!example] 使用場景
> - AI 研究者用它來在本地環境中測試和優化 AI 記憶能力，因為它提供了完整的記憶架構，無需依賴雲端服務。
> - 開發者用它來構建能夠記住用戶偏好的智能助手，因為其結構化事實和智能注入功能能夠提升用戶互動的流暢性。
> - 產品經理用它來追蹤團隊的決策歷程，因為它能夠自動整理和回顧過去的會議記錄和決策過程。

## 架構分析

Memory OS 的架構由七層組成，第一層是工作區，負責存儲用戶的基本信息；第二層是會話記憶，使用 SQLite 和 FTS5 實現全文本搜索；第三層是結構化事實，結合信任評分來管理持久化的知識；第四層是跨會話的記憶提取，通過 Icarus 插件實現多來源的注入；第五層是向量數據庫 Qdrant，提供多層次的檢索機制；第六層是自動化維基，持續整理和更新知識；第七層是關鍵的 Ground Truth 層級，確保注入的記憶被 AI 認可和使用。這樣的設計使得記憶的存取更加高效，並且減少了重複查詢的 token 消耗。選擇本地運行的架構是為了提高隱私性，但這也意味著需要用戶自行管理基礎設施，可能會增加維護成本。

## 技術深入分析

Memory OS 的核心技術機制在於其七層架構，這使得記憶的捕捉、存儲和使用變得更加高效。每一層都有其特定的功能，例如使用 SQLite 和 FTS5 來實現會話記憶的全文本搜索，並結合 Qdrant 作為向量數據庫來支持高效的檢索。這樣的設計使得系統能夠在本地運行，避免了雲端依賴，並且能夠靈活地支持多種 LLM 提供者。性能上，系統的設計考慮到了記憶的持久性和上下文的智能注入，這意味著用戶的每一次交互都能夠被記錄並有效利用。選擇 Python 和 Docker 作為技術棧的原因在於其廣泛的社群支持和成熟的生態系統，這使得開發和維護變得更加方便。然而，這也帶來了對 Docker 環境的依賴，可能會對新手用戶造成一定的學習障礙。在技術風險方面，隨著使用者數量的增加，如何保持系統的性能和穩定性將是未來需要關注的重點。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了詳細的安裝指引和範例；安裝過程順暢，無明顯坑；有良好的入門指南，適合新手使用。

## 優缺點分析

> [!success] 優點
> - 提供本地運行的記憶基礎設施，保護用戶隱私。
> - 具備智能上下文注入功能，提升 AI 的記憶效率。
> - 支持多種 LLM 提供者，靈活性高。

> [!danger] 缺點
> - 需要 Docker 環境，對新手可能有一定的學習曲線。
> - 對於大型資料集的性能可能會有瓶頸。
> - 目前僅支援 Python 3.11 以上版本，限制了使用範圍。
...
| 工具 | 差異 |
| --- | --- |
| mem0 | mem0 是一個雲端記憶解決方案，而 Memory OS 提供本地運行的選擇，適合對隱私有高要求的用戶。 |
| Zep | Zep 主要依賴雲端服務，而 Memory OS 則完全在本地運行，避免了雲端依賴。 |
| Letta | Letta 提供的記憶功能較為淺顯，無法提供像 Memory OS 一樣的結構化事實和智能注入。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| mem0 | mem0 是一個雲端記憶解決方案，而 Memory OS 提供本地運行的選擇，適合對隱私有高要求的用戶。 | 如果你的團隊已經在使用雲端記憶解決方案，並且不需要本地運行的需求。 | medium，因為需要重新配置記憶架構。 |
| Zep | Zep 主要依賴雲端服務，而 Memory OS 則完全在本地運行，避免了雲端依賴。 | 如果你的團隊已經習慣使用 Zep 的界面和功能，並且不需要本地化的需求。 | medium，因為需要將數據從雲端遷移到本地。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **mem0** | **Zep** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | mem0 是一個雲端記憶解決方案，而 Memory OS 提供本地運行的選擇，適合對隱私有高要求的用戶。 | Zep 主要依賴雲端服務，而 Memory OS 則完全在本地運行，避免了雲端依賴。 |
> | 遷移成本 | - | medium，因為需要重新配置記憶架構。 | medium，因為需要將數據從雲端遷移到本地。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用雲端記憶解決方案，並且不需要本地運行的 | 如果你的團隊已經習慣使用 Zep 的界面和功能，並且不需要本 |
...
- **[HIGH]** Docker 安裝過程中可能會遇到兼容性問題，特別是在不同操作系統上。
  - 解法：確保 Docker 版本符合要求，並參考官方文檔進行配置。
- [MEDIUM] 在大型資料集上運行時，性能可能會下降。
  - 解法：考慮對資料集進行分片或優化查詢。
- [MEDIUM] 某些功能在特定 LLM 提供者上可能不完全支持。
  - 解法：檢查官方文檔以獲取支持的 LLM 列表。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 AI 系統 | 非常適合 | 本地運行的特性能夠保護敏感數據，並且易於管理。 |
| 大型企業的 AI 記憶系統 | 普通 | 雖然功能強大，但可能需要更多的維護和資源。 |
| 個人開發者的 AI 助手 | 非常適合 | 簡單的安裝和本地運行的特性非常符合個人開發者的需求。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，10 小時整合，得到一個高效的本地 AI 記憶系統，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：本地運行的系統不需要高權限，並且不會存取敏感資料，依賴鏈的信任程度高，適合在 CI/CD 中使用。
