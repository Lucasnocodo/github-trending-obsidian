...
use_case: "提供持久記憶的操作系統，讓 Hermes Agent 能夠記住過去的對話和決策。"
priority: high
ring: assess
...
aliases:
  ...
  - "提供持久記憶的操作系統，讓 Hermes Agent 能夠記住過去的對話和決策。"
---

# memory-os

**1.2k** stars · **35** stars/天 · 建立 35 天前 · Python · MIT
...
> **授權** MIT (商業友好) · **維護** Moderate (最後推送 25 天前) · **貢獻者** 4+ 人 · **參與度** Low
> **適合** 需要在本地運行持久記憶系統的開發者和研究者，特別是對數據隱私有高要求的人。
> **一句話重點** Memory OS 不僅是一個記憶插件，而是一個完整的記憶操作系統，能夠有效解決智能代理的記憶問題。
...
> **結論** 花 5 小時學習，3 小時整合，得到一個高效的本地記憶系統，值得投資。

> [!abstract] 核心創新
> Memory OS 提供了一個完整的七層記憶架構，並引入了 Ground Truth Hierarchy，確保注入的記憶被實際使用。

## 專案簡介

Memory OS 是一個針對 Hermes Agent 的七層記憶操作系統，旨在解決智能代理在多次會話中失去上下文的問題。用戶可以透過這個系統將過去的對話、決策和結構化事實持久化，並在需要時自動注入相關上下文。其核心賣點在於 Layer 7 的 Ground Truth Hierarchy，這一層確保了注入的記憶被代理實際使用，避免了重複查詢帶來的資源浪費。系統架構包括 Qdrant 作為向量數據庫，並結合 Redis 和 SQLite 來管理會話和結構化事實，這樣的設計使得記憶的檢索效率大幅提升。

與傳統的 Hermes 方案相比，Memory OS 提供了更高效的記憶管理和更靈活的上下文注入，特別是在需要跨會話回憶的情境下。這個系統不依賴雲端服務，完全在本地運行，避免了數據外洩的風險。它支持多種 LLM 提供者，並且安裝過程簡單，只需一條命令即可完成。這使得它特別適合需要私密記憶基礎設施的用戶，尤其是那些對數據隱私有高要求的開發者和研究者。

**技術棧**：`Python 3.11` · `Docker` · `Redis` · `Qdrant`

## 重點功能

- 七層記憶架構 — 包含工作區、會話、結構化事實等，提供全面的記憶管理。
- 智能上下文注入 — 自動將相關記憶注入到對話中，提升交互效率。
- 本地運行 — 完全在用戶的機器上運行，避免雲端依賴和數據外洩風險。
- 支持多種 LLM 提供者 — 包括 OpenRouter、OpenAI 和本地模型，靈活性高。
- Ground Truth Hierarchy — 確保注入的記憶被代理實際使用，避免重複查詢浪費資源。
...
> [!tip] 爆紅原因
> 建立 35 天就累積 1226 stars（35/天），forks 117（9.5%），顯示出穩定的增長趨勢。作者 ClaudioDrews 之前在開源社區有豐富的經驗，這個專案解決了現有記憶解決方案普遍存在的雲端依賴問題，讓用戶能夠在本地運行持久記憶系統。這一點在使用者中引起了廣泛的共鳴，特別是在對數據隱私有高要求的開發者中。社群的活躍度也反映在熱門 Issues 上，顯示出使用者對於功能的需求和改進的期待。

## 適合誰使用

**目標受眾**：需要在本地運行持久記憶系統的開發者和研究者，特別是對數據隱私有高要求的人。

> [!example] 使用場景
> - AI 研究者用它來持久化實驗結果和參數設定，因為這樣可以避免每次重新配置的時間浪費。
> - 產品經理用它來記錄會議決策和項目進展，因為這樣能夠隨時回顧過去的討論，提升團隊協作效率。
> - 開發者用它來管理代碼變更和功能需求，因為這樣可以確保在多次迭代中不會遺漏重要的需求變更。

## 架構分析

Memory OS 的架構由七層組成，設計目的是為了提供全面的記憶管理。第一層是工作區，負責記錄用戶的基本信息；第二層是會話，利用 SQLite 和 FTS5 進行全文搜索；第三層是結構化事實，使用信任評分來管理持久化的事實；第四層是跨會話的 Fabric 層，提供多來源的注入；第五層是向量數據庫 Qdrant，支持多層次的檢索；第六層是自動化維基，持續整理和更新知識；第七層是 Ground Truth Hierarchy，確保注入的記憶被代理使用。這樣的設計使得記憶的檢索和使用效率大幅提升，並且避免了重複查詢帶來的資源浪費。整體架構的選擇使得系統能夠在本地運行，避免了雲端依賴的風險。

## 技術深入分析

Memory OS 的核心技術機制在於其七層記憶架構，這使得系統能夠有效地捕捉、存儲和注入記憶。第一層到第六層分別負責不同的記憶管理任務，而第七層的 Ground Truth Hierarchy 確保了注入的記憶被代理實際使用。這樣的設計不僅提升了記憶的檢索效率，還避免了重複查詢帶來的資源浪費。系統的效能特性在於能夠處理大量的會話數據，並且在本地運行時不會受到網絡延遲的影響。選擇 Python 和 Docker 作為核心技術棧，這使得系統的安裝和運行相對簡單，但也帶來了對 Docker 環境的依賴。技術風險方面，隨著系統的擴展，可能會面臨性能瓶頸，特別是在高並發的情境下。整合方面，Memory OS 與主流的 LLM 提供者兼容，並且能夠輕鬆地融入現有的開發流程中。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且包含安裝指令，讓新手能夠快速上手。安裝過程相對順暢，僅需一條命令即可完成設置。文件中提供了詳細的功能介紹和使用範例，對於新手友好。
...
| 工具 | 差異 |
| --- | --- |
| mem0 | mem0 是雲端解決方案，而 Memory OS 完全在本地運行，提供更高的數據隱私保障。 |
| Zep | Zep 主要針對雲端記憶管理，而 Memory OS 支持本地運行，並提供更靈活的 LLM 提供者選擇。 |
| Letta | Letta 依賴雲端基礎設施，而 Memory OS 提供完全的本地記憶解決方案，避免了雲端依賴。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| mem0 | mem0 是一個雲端記憶解決方案，與 Memory OS 的本地運行特性形成鮮明對比。 | 如果你的團隊已經在使用雲端服務，並且不需要本地數據存儲，mem0 可能更適合。 | medium，因為需要重新設計數據流和存儲方式。 |
| Zep | Zep 主要針對雲端記憶管理，缺乏本地運行的靈活性。 | 如果你的應用已經完全依賴雲端架構，Zep 是一個不錯的選擇。 | high，因為需要整合雲端 API 和數據流。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **mem0** | **Zep** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | mem0 是一個雲端記憶解決方案，與 Memory OS 的本地運行特性形成鮮明對比。 | Zep 主要針對雲端記憶管理，缺乏本地運行的靈活性。 |
> | 遷移成本 | - | medium，因為需要重新設計數據流和存儲方式。 | high，因為需要整合雲端 API 和數據流。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用雲端服務，並且不需要本地數據存儲，me | 如果你的應用已經完全依賴雲端架構，Zep 是一個不錯的選擇。 |
...
- **[HIGH]** 在某些低配置機器上運行時可能會遇到性能瓶頸，特別是在高並發情境下。
  - 解法：考慮升級硬體或減少同時運行的實例數量。
- [MEDIUM] Docker 安裝過程中可能會遇到環境變數配置問題。
  - 解法：仔細檢查 setup.sh 中的配置，確保環境變數正確設置。
- **[HIGH]** 在使用過程中，可能會出現記憶未被正確注入的情況。
  - 解法：檢查 Ground Truth Hierarchy 的設置，確保其正確配置。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 需要在本地運行的 AI 研究團隊 | 非常適合 | 提供本地記憶解決方案，保障數據隱私。 |
| 小型創業團隊進行產品開發 | 適合 | 能夠有效管理會議記錄和決策過程。 |
| 大型企業的數據科學部門 | 普通 | 可能需要更高的擴展性和性能。 |
| 個人開發者的 AI 實驗 | 非常適合 | 安裝簡單，能快速上手。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，3 小時整合，得到一個高效的本地記憶系統，值得投資。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：Memory OS 在本地運行，不需要高權限，且不存取敏感資料。依賴鏈的信任程度高，沒有已知的供應鏈風險。
