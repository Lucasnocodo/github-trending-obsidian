...
use_case: "為 Hermes Agent 提供持久記憶的 7 層記憶操作系統，支持本地運行與多種 LLM 提供者。"
priority: high
ring: assess
...
star_history: "2026-06-04:756,2026-06-04:757,2026-06-05:828,2026-06-05:828,2026-06-06:882,2026-06-06:882,2026-06-07:951,2026-06-07:951,2026-06-08:1011,2026-06-09:1037,2026-06-10:1050,2026-06-11:1069,2026-06-12:1086,2026-06-13:1093,2026-06-14:1098"
tags:
  - github
  - "category/ai_ml"
  - "lang/python"
...
aliases:
  ...
  - "為 Hermes Agent 提供持久記憶的 7 層記憶操作系統，支持本地運行與多種 LLM 提供者。"
---

# memory-os

**1.1k** stars · **84** stars/天 · 建立 13 天前 · Python · MIT
...
> **適合** 需要在本地運行持久記憶系統以支持 Hermes Agent 的開發者和團隊。
> **一句話重點** Memory OS 不僅是記憶管理工具，更是一個完整的本地記憶操作系統，能夠有效解決長期記憶的問題。
...
> **結論** 花 5 小時學、10 小時整合，得到高效的本地記憶管理系統，值得投資。

> [!abstract] 核心創新
> 提供了一個完整的本地記憶操作系統，具備七層記憶架構和智能上下文注入功能。

## 專案簡介

Memory OS 是一個針對 Hermes Agent 設計的 7 層記憶操作系統，旨在解決智能代理在長期交互中忘記上下文的問題。它的核心機制是將記憶分為七個層級，從工作空間到結構化事實，並透過智能上下文注入來確保記憶的有效使用。使用者可以透過 `curl -sSL https://raw.githubusercontent.com/ClaudioDrews/memory-os/main/setup.sh | bash` 指令一鍵安裝整個系統，這樣的設計使得用戶無需擔心繁瑣的配置過程。技術上，Memory OS 結合了 Qdrant 作為向量數據庫，並利用 Redis 和 ARQ Worker 提供本地記憶基礎設施，這樣的架構選擇避免了雲端依賴，確保了數據的私密性和安全性。與傳統的 Hermes 相比，Memory OS 提供了更高效的記憶管理，能夠進行多源注入和全會話回憶，並且具備自動清理和去重的功能。

這使得用戶在處理大量資料時，能夠保持高效且不浪費資源。相較於其他記憶解決方案如 mem0 和 Zep，Memory OS 的本地運行特性和無需訂閱的優勢，使其在隱私和靈活性上更具競爭力。使用者在實際運用中可能會遇到的問題包括對於 Layer 7 的理解和配置，因為這一層是確保注入記憶被有效使用的關鍵。總體而言，Memory OS 是一個成熟的解決方案，適合需要長期記憶管理的開發者和團隊，未來可能會持續增強其社群支持和功能擴展。

**技術棧**：`Python 3.11` · `Docker` · `Redis` · `Qdrant`

## 重點功能

- 七層記憶架構 — 包括工作空間、會話、結構化事實、跨會話記憶等，提供全面的記憶管理。
- 智能上下文注入 — 根據用戶的需求自動注入相關記憶，減少重複查詢的開銷。
- 本地運行 — 完全在用戶的機器上運行，無需雲端依賴，確保數據隱私。
- 支持多種 LLM 提供者 — 包括 OpenRouter、OpenAI、Anthropic 等，靈活性高。
- 自動清理和去重 — 透過 decay scanner 和語義去重，保持記憶的整潔和有效性。
...
```python
{
  ...
  "預期輸出": "系統將自動設置記憶基礎設施，包括 Docker 服務和 SQLite 數據庫。"
}
```
...
> [!tip] 爆紅原因
> 建立 13 天內累積 1098 stars（84/天），forks 101（9.2%），顯示出強勁的增長潛力。作者 ClaudioDrews 之前在記憶解決方案方面有豐富的經驗，這個專案解決了現有記憶系統的雲端依賴和數據隱私問題，提供了一個完全本地的替代方案。最近的推文和社群討論也引發了對這個專案的關注，尤其是在長期記憶管理的需求上。技術上，隨著本地計算能力的提升，這種記憶系統的可行性大大增加。forks/stars 比率為 9.2%，顯示出有一定比例的使用者在實際修改和使用這個專案。

## 適合誰使用

**目標受眾**：需要在本地運行持久記憶系統以支持 Hermes Agent 的開發者和團隊。

> [!example] 使用場景
> - AI 研究者用它來記錄和回顧實驗結果，因為它能夠持久保存上下文，避免每次實驗都重複設定。
> - 產品經理用它來追蹤產品開發中的決策和變更，因為它提供的結構化事實和智能注入能夠提高團隊的協作效率。
> - 開發者用它來在本地環境中測試和調整 LLM 模型，因為它支持多種 LLM 提供者，並且無需擔心數據隱私問題。

## 架構分析

Memory OS 的架構由七層記憶組成，從工作空間到結構化事實，每一層都有其特定功能。這樣的設計使得記憶的捕捉、存儲和注入變得高效且有序。使用 Qdrant 作為向量數據庫，能夠實現快速的相似度搜索，而 Redis 則用於存儲會話狀態和結構化事實。

這樣的架構選擇使得整個系統能夠在本地運行，避免了雲端依賴的風險。選擇本地運行的代價在於需要用戶自行管理基礎設施，但這同時也提供了更高的數據安全性。擴展性方面，系統能夠輕鬆整合各種 LLM 提供者，這使得它在不同場景下都能靈活應用。

## 技術深入分析

Memory OS 的核心技術機制是將記憶分為七個層級，每一層都有特定的功能，從而實現高效的記憶捕捉和注入。使用 Qdrant 作為向量數據庫，能夠支持高效的相似度搜索，並且結合 Redis 來管理會話狀態，這樣的設計使得系統能夠在本地運行，避免了雲端依賴的風險。效能方面，系統能夠處理大量的記憶資料，並且透過智能上下文注入來提高效率，減少重複查詢的開銷。設計上選擇了 Python 和 Docker 作為主要技術棧，這使得系統的安裝和使用相對簡單，但也可能對不熟悉這些技術的用戶造成挑戰。技術風險方面，隨著用戶數量的增加，系統的性能可能會受到影響，特別是在高並發的情況下。整合方面，Memory OS 能夠與多種 LLM 提供者無縫對接，這使得它在不同的應用場景中都能靈活使用。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝指導和使用說明，但缺乏詳細的範例。安裝過程相對順暢，但對於不熟悉 Docker 的用戶可能會有一些挑戰。整體而言，對於有一定技術背景的用戶，花 30 分鐘應該能夠成功運行起來。
...
> - 目前僅支持 Hermes Agent，對其他代理支持有限。
> - 仍在活躍開發中，未來可能會有重大變更。
> - 安裝過程可能對新手不夠友好，缺乏詳細的指導。

> [!warning] 注意事項
> - 僅支援 Python 3.11+
> - 需要 Docker 環境運行，對於不熟悉 Docker 的用戶可能有學習曲線。
> - 目前僅支持 Hermes Agent，對於其他 AI 代理可能需要額外調整。
> - 仍在活躍開發中，未來可能會有重大變更。
...
| 工具 | 差異 |
| --- | --- |
| mem0 | mem0 是一個雲端記憶解決方案，無法提供本地運行的靈活性和數據隱私。 |
| Zep | Zep 提供類似的記憶功能，但缺乏智能上下文注入和自動清理功能。 |
| Letta | Letta 主要是雲端解決方案，無法實現本地記憶管理，且功能相對淺顯。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| mem0 | mem0 是一個雲端記憶解決方案，無法提供本地運行的靈活性和數據隱私。 | 如果你的團隊已經在使用雲端解決方案並且不需要本地數據存儲，可以考慮使用 mem0。 | medium，因為需要將數據從本地轉移到雲端。 |
| Zep | Zep 提供類似的記憶功能，但缺乏智能上下文注入和自動清理功能。 | 如果你的團隊需要一個簡單的雲端記憶解決方案，Zep 可能是更合適的選擇。 | low，因為 Zep 提供了簡單的 API 來導入數據。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **mem0** | **Zep** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | mem0 是一個雲端記憶解決方案，無法提供本地運行的靈活性和數據隱私。 | Zep 提供類似的記憶功能，但缺乏智能上下文注入和自動清理功能。 |
> | 遷移成本 | - | medium，因為需要將數據從本地轉移到雲端。 | low，因為 Zep 提供了簡單的 API 來導入數據。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用雲端解決方案並且不需要本地數據存儲，可 | 如果你的團隊需要一個簡單的雲端記憶解決方案，Zep 可能是更 |
...
- **[HIGH]** 在某些系統上安裝過程可能會遇到 Docker 相關的問題
  - 解法：確保 Docker 版本符合要求，並參考官方文檔進行配置
- [MEDIUM] Layer 7 的配置可能不夠直觀，導致記憶未被有效使用
  - 解法：仔細閱讀 README 中的 Layer 7 相關部分，並進行必要的調整

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 本地運行的特性和靈活性使其能夠滿足小型團隊的需求。 |
| 大型企業的記憶管理系統 | 不適合 | 目前仍在 beta 階段，可能不夠穩定。 |
| AI 研究團隊需要長期記憶管理 | 適合 | 具備智能上下文注入和結構化事實的功能，能夠有效支持研究需求。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學、10 小時整合，得到高效的本地記憶管理系統，值得投資。
