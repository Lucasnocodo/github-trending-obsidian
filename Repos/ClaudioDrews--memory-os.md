...
use_case: "為 Hermes Agent 提供持久記憶的操作系統，解決重複上下文和記憶丟失的問題。"
priority: high
ring: assess
...
star_history: "2026-06-04:756,2026-06-04:757,2026-06-05:828"
tags:
  - github
  - "category/ai_ml"
  - "lang/python"
aliases:
  ...
  - "為 Hermes Agent 提供持久記憶的操作系統，解決重複上下文和記憶丟失的問題。"
---

# memory-os

**757** stars · **252** stars/天 · 建立 3 天前 · Python · MIT
...
> **適合** 希望在本地運行持久記憶系統的 Hermes Agent 用戶，尤其是重視數據隱私的開發者和研究者。
> **一句話重點** Memory OS 的設計不僅解決了記憶丟失的問題，還提供了一個靈活且安全的本地運行環境。
...
> **結論** 花 5 小時學習，10 小時整合，得到一個靈活且安全的本地記憶系統，值得投資。

> [!abstract] 核心創新
> 提供了七層記憶架構，並引入了地面真相層以確保記憶的有效使用。

## 專案簡介

Memory OS 是一個為 Hermes Agent 設計的七層記憶操作系統，旨在解決用戶在多次會話中遇到的記憶丟失問題。它通過將用戶的偏好、決策和上下文記錄在本地，並使用 Qdrant 進行向量搜索，實現了持久記憶。用戶只需安裝 Docker 和 Python 3.11+，便可運行此系統，無需依賴雲端服務。其核心賣點在於提供了智能的上下文注入，讓代理能夠在每次會話中自動調用相關記憶，避免重複說明。

系統的架構包括工作區、會話記錄、結構化事實、跨會話回憶、向量數據庫、LLM 維基和地面真相層，確保記憶的有效性和準確性。這樣的設計使得 Memory OS 在記憶的持久性和智能性上超越了傳統的 Hermes 系統，並且沒有任何供應商鎖定的風險。與其他記憶解決方案相比，Memory OS 提供了本地運行的靈活性和安全性，並且不需要任何記憶訂閱服務。使用者可以在本地環境中自由選擇 LLM 提供者，這使得它在隱私和數據安全上有明顯的優勢。

**技術棧**：`Python 3.11` · `Docker` · `Qdrant` · `Redis`

## 重點功能

- 七層記憶架構 — 包括工作區、會話、結構化事實、跨會話回憶等，提供全面的記憶管理。
- 智能上下文注入 — 根據用戶的歷史會話自動注入相關上下文，減少重複說明。
- 本地運行 — 完全在用戶的機器上運行，無需依賴雲端服務，保障數據隱私。
- 支持多種 LLM 提供者 — 兼容 OpenRouter、OpenAI、Anthropic 等，靈活性高。
- 結構化事實存儲 — 使用 SQLite 和信任評分系統，確保記憶的準確性和持久性。

## 快速開始

1. 安裝 Docker 和 Python 3.11+
```bash
sudo apt install docker python3.11
```
2. 克隆專案並進入目錄
```bash
...
> [!tip] 爆紅原因
> 建立 3 天內累積 757 stars（252/天），forks 74（9.8%），顯示出強勁的增長潛力。這個專案的創建者 Claudio Drews 之前在記憶系統方面有深入的研究，解決了許多用戶在使用 Hermes 時遇到的記憶丟失問題。相較於其他雲端記憶解決方案，Memory OS 提供了本地化的記憶基礎設施，這在當前對隱私和數據安全日益重視的背景下，顯得尤為重要。最近的推文和社群討論也引發了對這個工具的關注，顯示出其在技術生態中的重要性。forks/stars 比率為 9.8%，表明有相當比例的用戶在實際修改和使用這個工具。

## 適合誰使用

**目標受眾**：希望在本地運行持久記憶系統的 Hermes Agent 用戶，尤其是重視數據隱私的開發者和研究者。

> [!example] 使用場景
> - AI 研究員用它來記錄和回顧每次實驗的結果，因為這樣可以避免重複設定實驗條件，節省時間。
> - 產品經理用它來跟蹤產品開發過程中的決策和變更，因為這樣能夠在會議中快速回顧過去的討論，提升效率。
> - 開發者用它來管理代碼變更和版本控制，因為這樣可以在不同的開發階段保持一致的上下文，減少錯誤。

## 架構分析

Memory OS 的架構由七層組成，設計目的是為了提供一個全面的記憶解決方案。每一層都有其特定的功能，從工作區到結構化事實，最終到地面真相層，確保注入的記憶能被有效使用。這種設計使得系統能夠在本地運行，避免了雲端依賴的風險。

資料流從用戶的輸入開始，經過各層的處理，最終形成可用的記憶。選擇使用 SQLite 和 Qdrant 進行數據存儲和檢索，這樣的選擇提供了靈活性和高效性，但在處理大型數據集時可能會遇到性能瓶頸。整體架構的設計考量了用戶的隱私需求，並且能夠與多種 LLM 提供者兼容。

## 技術深入分析

Memory OS 的核心技術機制在於其七層架構，這使得記憶的捕獲、存儲和注入變得高效且有序。每一層都有其專門的功能，例如使用 Qdrant 進行向量搜索，並結合 SQLite 提供的全文本搜索能力，確保用戶能夠快速檢索到所需的記憶。系統的設計考量了性能和擴展性，雖然在處理大型數據集時可能會遇到瓶頸，但整體架構仍然保持了高效性。選擇 Python 作為開發語言，結合 Docker 的使用，使得部署和維護變得相對簡單。技術風險方面，隨著用戶數量的增加，系統可能會面臨性能下降的風險，特別是在記憶數據量大時。整合方面，Memory OS 能夠與多種 LLM 提供者兼容，這使得它在不同的應用場景中都能發揮作用。整體來說，這個工具在設計上充分考慮了用戶的需求，並提供了靈活的解決方案。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且詳細，提供了完整的安裝步驟和使用說明。安裝過程需要 Docker 環境，對新手來說可能會有一定的學習曲線。文件中包含了設置指南，幫助用戶快速上手，但目前僅提供英文版本。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供了類似的記憶功能，但主要依賴雲端服務，缺乏本地運行的靈活性。 |
| Zep | 雖然也提供記憶功能，但不支持本地運行，且缺乏結構化事實存儲的能力。 |
| mem0 | mem0 提供記憶功能，但不具備智能上下文注入的能力，使用體驗較差。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| Zep | Zep 提供雲端記憶解決方案，而 Memory OS 則專注於本地運行，這使得 Memory OS 在數據隱私上更具優勢。 | 如果你的團隊已經在使用雲端解決方案，且不需要本地運行的靈活性，則 Zep 可能是更好的選擇。 | medium，因為需要將數據從雲端遷移到本地環境。 |
| mem0 | mem0 提供基本的記憶功能，但缺乏智能上下文注入，這使得 Memory OS 在使用體驗上更為優越。 | 如果你的需求較為簡單，且不需要複雜的記憶管理，mem0 可能足夠。 | low，因為 mem0 的設置相對簡單。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **Zep** | **mem0** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | Zep 提供雲端記憶解決方案，而 Memory OS 則專注於本地運行，這使得 Memory OS 在數據隱私上更具優勢。 | mem0 提供基本的記憶功能，但缺乏智能上下文注入，這使得 Memory OS 在使用體驗上更為優越。 |
> | 遷移成本 | - | medium，因為需要將數據從雲端遷移到本地環境。 | low，因為 mem0 的設置相對簡單。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用雲端解決方案，且不需要本地運行的靈活性 | 如果你的需求較為簡單，且不需要複雜的記憶管理，mem0 可能 |
...
> [!tip] 採用建議
> 目前適合個人試用和小型專案，不建議在生產環境中使用。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在大型數據集上運行時可能會遇到性能瓶頸
  - 解法：定期清理不必要的記憶數據以減少負擔
- [MEDIUM] Docker 配置不當可能導致啟動失敗
  - 解法：確保 Docker 版本符合要求，並檢查配置文件
- [MEDIUM] 某些 LLM 提供者的兼容性問題
  - 解法：查看官方文檔以獲取最新的兼容性列表

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型開發團隊需要持久記憶系統 | 非常適合 | 本地運行和靈活性使得它能夠滿足團隊的需求。 |
| 大型企業需要雲端記憶解決方案 | 不適合 | Memory OS 專注於本地運行，無法滿足雲端需求。 |
| AI 研究者需要記錄實驗過程 | 適合 | 能夠有效管理和回顧實驗記憶。 |
| 個人開發者希望快速上手的記憶系統 | 普通 | 雖然功能強大，但安裝過程可能對新手有挑戰。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，10 小時整合，得到一個靈活且安全的本地記憶系統，值得投資。
