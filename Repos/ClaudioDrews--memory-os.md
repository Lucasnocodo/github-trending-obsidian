...
use_case: "讓 Hermes Agent 擁有持久記憶，並能智能地注入上下文，提升對話的連貫性。"
priority: high
ring: assess
...
aliases:
  ...
  - "讓 Hermes Agent 擁有持久記憶，並能智能地注入上下文，提升對話的連貫性。"
---

# memory-os

**1.3k** stars · **31** stars/天 · 建立 41 天前 · Python · MIT
...
> **授權** MIT (商業友好) · **維護** Slow (最後推送 31 天前) · **貢獻者** 4+ 人 · **參與度** Low
> **適合** 需要在本地運行持久記憶系統以支持長期 AI 互動的開發者。
> **一句話重點** 這個專案的創新在於提供了一個完整的本地記憶系統，讓用戶能夠在不依賴雲端的情況下，實現持久的記憶管理。
...
> **結論** 花 5 小時學、8 小時整合，得到高效的本地記憶管理，值得投入。

> [!abstract] 核心創新
> 提供了完整的七層記憶架構，並確保注入的記憶被有效利用。

## 專案簡介

Memory OS 是一個七層的記憶操作系統，專為 Hermes Agent 設計，旨在解決其記憶不足的問題。用戶可以在本地運行這個系統，無需依賴雲端服務，並能與任何 LLM 提供者兼容。系統的核心在於其七層架構，從工作區記憶到結構化事實，再到跨會話的回憶，最終通過第七層的真實性層確保注入的記憶被有效利用。這樣的設計不僅減少了重複的上下文輸入，還能在每次對話中提供準確的背景信息。關鍵指令如 `curl -sSL https://raw.githubusercontent.com/ClaudioDrews/memory-os/main/setup.sh | bash` 可以快速安裝整個系統，顯示出其易用性和便捷性。

技術上，Memory OS 使用了 Qdrant 作為向量數據庫，並結合 Redis 和 ARQ Worker，確保了高效的數據處理和檢索。與其他記憶解決方案相比，Memory OS 提供了本地化的記憶基礎設施，避免了雲端鎖定的問題，並且支持結構化事實和信任評分的功能，這在其他工具中往往缺失。使用者可以期待更高的記憶效率和更低的上下文重複率，這對於需要長期合作的用戶來說是至關重要的。這個系統的成熟度尚在 beta 階段，但已經具備了穩定運行的基礎，適合對記憶有高需求的開發者和團隊使用。
...
- 智能上下文注入 — 自動將相關記憶注入對話中，減少重複輸入的需要。
- 結構化事實存儲 — 提供信任評分和反饋循環，確保記憶的準確性和可靠性。
...
> [!tip] 爆紅原因
> 建立 41 天內累積 1259 stars（31/天），forks 119（9.5%），顯示出穩定的增長趨勢。作者 ClaudioDrews 之前在 AI 記憶領域有多個貢獻，這個專案解決了現有記憶解決方案的雲端鎖定問題，提供了本地運行的選擇。近期的推廣活動和社群的關注也促進了這個專案的曝光度。這個工具的出現正好填補了市場上對於本地化記憶系統的需求，並且其設計理念與當前的開源趨勢相符。forks/stars 比率為 9.5%，顯示出有相當比例的用戶在實際修改和使用這個專案。

## 適合誰使用

**目標受眾**：需要在本地運行持久記憶系統以支持長期 AI 互動的開發者。

> [!example] 使用場景
> - AI 研究人員用它來持久化對話記憶，因為這樣可以避免每次對話都重複上下文，提升工作效率。
> - 開發者用它來管理多個項目的記憶，因為它能夠自動整理和回顧過去的決策，減少重複性工作。
> - 產品經理用它來追蹤用戶反饋和需求變化，因為其結構化事實和信任評分功能能夠提供準確的數據支持。

## 架構分析

Memory OS 的架構由七層組成，從工作區到結構化事實，再到向量數據庫，最終通過真實性層確保注入記憶的有效使用。這樣的設計使得記憶的捕獲、存儲和注入過程都能高效運行。每一層都有其特定的功能，例如使用 Qdrant 作為向量數據庫，並結合 Redis 進行數據管理。這種設計的代價在於需要一定的資源來運行 Docker 環境，但相對於雲端解決方案，使用者能夠獲得更高的數據隱私和控制權。整體架構的擴展性良好，能夠支持多種 LLM 提供者，並且能夠隨著需求的增加而進行優化。

## 技術深入分析

Memory OS 的核心技術機制在於其七層架構，這使得記憶的捕獲、存儲和注入都能高效運行。每一層都有其特定的功能，例如第一層的工作區記憶和第二層的會話記憶，利用 SQLite 和 FTS5 進行全文本搜索，確保用戶能夠快速檢索過去的對話。整體系統的性能特性在於能夠處理大量的對話數據，並且通過 Qdrant 的向量檢索技術，實現了高效的數據檢索。設計取捨方面，選擇本地運行而非雲端解決方案，雖然在資源需求上有所增加，但卻提供了更高的數據隱私和控制權。技術風險方面，隨著數據量的增加，可能會面臨性能瓶頸，這需要在未來進行優化。整合方面，與現有的 LLM 提供者的整合相對簡單，並且支持多種 CI/CD 流程，對於團隊的採用成本較低。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且詳細，包含了安裝指令和使用範例。安裝過程順暢，沒有明顯的坑。文件中提供了良好的入門指南，對於新手來說，花 30 分鐘能夠順利運行起來。
...
> - 對於大型數據集，性能可能會受到影響，需要進行優化。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| mem0 | mem0 提供雲端記憶解決方案，但不支持本地運行，對於需要數據隱私的用戶不適合。 |
| Zep | Zep 主要針對雲端記憶，缺乏結構化事實和信任評分功能，對於需要精確記憶的用戶不夠理想。 |
| Letta | Letta 也不支持本地運行，且功能較為淺顯，無法滿足高需求的記憶管理。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| mem0 | mem0 提供雲端記憶解決方案，但不支持本地運行，對於需要數據隱私的用戶不適合。 | 如果你的團隊已經在使用雲端記憶解決方案，並且不在意數據隱私問題。 | medium，因為需要將數據從本地遷移到雲端。 |
| Zep | Zep 主要針對雲端記憶，缺乏結構化事實和信任評分功能，對於需要精確記憶的用戶不夠理想。 | 如果你的團隊需要一個簡單的雲端記憶解決方案，並且不需要複雜的功能。 | low，因為是雲端解決方案，數據遷移相對簡單。 |
| Letta | Letta 也不支持本地運行，且功能較為淺顯，無法滿足高需求的記憶管理。 | 如果你的團隊需要一個輕量級的記憶解決方案，並且不需要太多功能。 | low，因為是雲端解決方案，數據遷移相對簡單。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **memory-os** | **mem0** | **Zep** | **Letta** |
> | --- | --- | --- | --- | --- |
> | 技術路線 | 本專案 | mem0 提供雲端記憶解決方案，但不支持本地運行，對於需要數據隱私的用戶不適合。 | Zep 主要針對雲端記憶，缺乏結構化事實和信任評分功能，對於需要精確記憶的用戶不夠理想。 | Letta 也不支持本地運行，且功能較為淺顯，無法滿足高需求的記憶管理。 |
> | 遷移成本 | - | medium，因為需要將數據從本地遷移到雲端。 | low，因為是雲端解決方案，數據遷移相對簡單。 | low，因為是雲端解決方案，數據遷移相對簡單。 |
> | 適用場景 | 主要場景 | 如果你的團隊已經在使用雲端記憶解決方案，並且不在意數據隱私問 | 如果你的團隊需要一個簡單的雲端記憶解決方案，並且不需要複雜的 | 如果你的團隊需要一個輕量級的記憶解決方案，並且不需要太多功能 |
...
- **[HIGH]** 在某些環境下，Docker 服務啟動可能會失敗，特別是資源不足的情況下。
  - 解法：確保有足夠的系統資源，或在更高效的機器上運行。
- [MEDIUM] 使用過程中，可能會遇到記憶重複的情況。
  - 解法：定期檢查和清理記憶庫，使用內建的去重功能。
- [MEDIUM] 某些 LLM 提供者的兼容性問題，可能導致記憶無法正確注入。
  - 解法：參考官方文檔，確保使用的 LLM 提供者支持 Memory OS。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 能夠有效管理和持久化記憶，提升團隊的工作效率。 |
| 大型企業的 AI 記憶系統 | 普通 | 雖然功能強大，但可能需要更多的資源和管理。 |
| 個人開發者的 AI 助手 | 非常適合 | 本地運行的特性能夠保護個人數據隱私。 |
| 需要雲端記憶解決方案的團隊 | 不適合 | Memory OS 專注於本地運行，無法滿足雲端需求。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學、8 小時整合，得到高效的本地記憶管理，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：本地運行不需要高權限，且不存取敏感資料，依賴鏈的信任程度高，適合在 CI/CD 中使用。
