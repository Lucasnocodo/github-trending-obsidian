...
star_history: "2026-06-17:831,2026-06-17:834,2026-06-18:1012,2026-06-18:1015,2026-06-19:1072,2026-06-19:1072,2026-06-20:1117,2026-06-20:1117,2026-06-21:1139,2026-06-22:1160,2026-06-23:1218,2026-06-24:1277,2026-06-25:1314,2026-06-26:1359,2026-06-27:1369,2026-06-28:1385"
tags:
  - github
  - "category/開發工具"
  - "lang/swift"
...
**1.4k** stars · **99** stars/天 · 建立 14 天前 · Swift · Apache-2.0
...
> **適合** 需要安裝特定版本 iOS 應用的開發者和普通用戶。
> **一句話重點** Pastel 的自動化功能大幅提升了獲取舊版應用的效率，特別適合開發者和測試人員。
...
> [!abstract] 核心創新
> 支持自動選擇 App Store 和多選下載功能，這在市場上是獨特的。

## 專案簡介

Pastel 是一款專為 macOS 26+ 設計的 IPA 下載工具，能夠幫助用戶輕鬆獲取舊版應用並自動捕獲數據包。用戶只需在應用內搜索所需的 App，選擇後可直接下載並透過 AirDrop 安裝到 iPhone 或 iPad。這個工具的賣點在於其支持多種語言界面，並且能夠根據用戶的 Apple 帳戶自動選擇對應的 App Store。技術上，Pastel 使用 SwiftUI 編寫，充分利用了 macOS 的 Liquid Glass 效果，提供了流暢的用戶體驗。相較於其他 IPA 下載工具，Pastel 支持多選 App 版本進行下載，並能從 Apple 獲取版本 ID，這在市場上是獨特的。

使用者在首次使用時需添加 Apple 帳戶，並完成雙重認證，這解決了許多現有工具的痛點，如無法觸發雙重認證等。這款工具的性能在於其能夠快速獲取和安裝應用，並且界面友好，適合各種用戶，包括開發者和普通用戶。未來計劃推出 Windows 版本，擴大其用戶基礎。整體而言，Pastel 是一款成熟且功能強大的工具，適合需要安裝舊版應用的用戶。
...
- 自動選擇 App Store — 根據用戶的 Apple 帳戶自動選擇對應的商店。
- AirDrop 安裝 — 下載後可直接透過 AirDrop 安裝到 iPhone 或 iPad。
- 手動輸入 App ID — 支持用戶手動輸入 App ID 獲取最新版。
...
> [!tip] 爆紅原因
> 建立 14 天就累積 1385 stars（99/天），forks 80（5.8%），顯示出穩定的增長。作者 EEliberto 在開發社群中有一定的知名度，過去參與過多個開源項目。這個工具解決了用戶在獲取舊版應用時的困難，特別是對於需要安裝特定版本的開發者來說，這是一個重要的需求。最近的更新和功能增強吸引了更多用戶的注意，並且社群的反饋也相對積極。技術上，隨著 macOS 的更新，這款工具的需求也隨之上升，尤其是在 Apple 生態系統中，對於舊版應用的需求依然存在。

## 適合誰使用

**目標受眾**：需要安裝特定版本 iOS 應用的開發者和普通用戶。

> [!example] 使用場景
> - iOS 開發者用它來快速獲取舊版應用進行測試，因為手動下載舊版應用耗時且容易出錯。
> - 普通用戶用它來安裝特定版本的應用，因為這樣可以避免新版本中的 bug，提升使用體驗。
> - 測試人員用它來捕獲應用數據包，因為自動化的數據捕獲功能能夠節省大量時間和精力。

## 架構分析

Pastel 的架構設計以 SwiftUI 為核心，利用 macOS 的 Liquid Glass 效果來提升用戶體驗。資料流方面，使用者在應用內搜索 App 時，系統會根據 Apple 帳戶自動選擇對應的 App Store，並從中獲取應用的版本 ID。這樣的設計使得用戶能夠快速找到所需的應用，並且減少了手動輸入的錯誤。選擇 SwiftUI 的好處在於其能夠提供現代化的界面，但代價是僅限於 macOS 平台，無法跨平台使用。擴展性方面，未來若要支持 Windows 版本，將需要重新考慮架構設計，這可能會增加開發成本。

## 技術深入分析

Pastel 使用 SwiftUI 作為主要開發框架，這使得其界面設計現代化且符合 macOS 的設計標準。核心功能上，應用通過 Apple 的 API 獲取應用版本 ID，並支持用戶手動輸入 ID 進行下載，這在其他同類工具中較為少見。性能上，Pastel 能夠快速處理用戶請求，並且在下載過程中提供即時反饋，這對於需要頻繁下載舊版應用的用戶來說非常重要。設計上，選擇 SwiftUI 使得開發過程中能夠快速迭代，但同時也限制了其跨平台的能力，未來若要擴展至 Windows，可能需要重構部分代碼。技術風險方面，隨著 macOS 的更新，Pastel 需要持續維護以確保兼容性，這可能會增加開發者的負擔。整合方面，Pastel 可以與其他 macOS 應用無縫協作，但對於非 macOS 用戶則無法使用，這是其一個主要的限制。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且提供了安裝步驟，對於新手來說友好。安裝過程相對順暢，但需要注意 Node 依賴的安裝。文件目前僅提供英文和中文，對於非這兩種語言的用戶可能會有障礙。

## 優缺點分析

> [!success] 優點
> - 界面友好，使用 SwiftUI 提供流暢的用戶體驗。
> - 支持多語言，適合不同地區的用戶。
> - 自動選擇 App Store 功能，減少用戶操作步驟。

> [!danger] 缺點
> - 僅支持 macOS 26+，限制了用戶群。
> - 目前沒有 Windows 版本，未來計劃不明。
> - 無法獲取付費 App 的版本 ID，對某些用戶來說是個限制。

> [!warning] 注意事項
> - 僅支持 macOS 26+ 且配備 Apple 芯片的 Mac。
> - 目前沒有 Windows 版本，未來計劃尚未確定。
> - 無法從 Apple 獲取付費 App 的版本 ID。
