...
**1.6k** stars · **39** stars/天 · 建立 41 天前 · Swift · Apache-2.0
...
> **授權** Apache-2.0 (商業友好) · **維護** Slow (最後推送 33 天前) · **貢獻者** Solo (bus factor 風險) · **參與度** Low
> **適合** 需要在 macOS 環境下下載舊版 iOS 應用的開發者和普通用戶。
> **一句話重點** Pastel 提供了一個簡單而強大的解決方案來安裝舊版 iOS 應用，特別是對於需要頻繁測試的開發者。
...
> **成熟度** Stable (可用) · **安裝** Medium (需設定) · **學習** ~2h · **綁定風險** medium
> **結論** 花 2 小時學習，3 小時整合，得到便捷的舊版應用下載功能，值得投資。

> [!abstract] 核心創新
> 使用 GSA 技術來觸發雙重認證，提供更穩定且安全的登錄過程。

## 專案簡介

Pastel 是一款專門用於安裝 IPA 歷史版本的工具，能夠自動捕獲數據包並支持直接通過 AirDrop 傳輸至 iPhone 和 iPad。用戶只需在應用內搜索所需的 App，選擇版本後即可下載，並且支持手動輸入 App ID 來獲取最新版本。這個工具的設計考量了用戶體驗，使用 SwiftUI 編譯，完美適配 macOS 26 的 Liquid Glass 效果，並且支持多語言界面。技術上，Pastel 整合了多個下載源，並使用 GSA 技術來觸發雙重認證，這在市面上其他 IPA 下載工具中並不常見。與其他工具相比，如 ipatools，Pastel 提供了更直觀的用戶界面和更穩定的登錄過程。使用者在下載時可以直接預覽 App Icon，這對於特定節假日的應用尤為實用。

儘管目前僅支持 macOS 26+ 且配備 Apple 芯片的設備，但未來將計劃推出 Windows 版本。使用者可能會遇到的問題包括在某些情況下無法獲取 App 的版本 ID，但工具已經提供了手動輸入的選項來解決此問題。這個專案目前處於穩定階段，適合需要下載舊版應用的開發者和普通用戶。隨著未來 Windows 版本的推出，預計會吸引更多用戶。對於需要在 macOS 環境下進行 IPA 下載的用戶，這是一個非常合適的選擇。

**技術棧**：`SwiftUI` · `Node.js`

## 重點功能

- 自動捕獲數據包 — 下載後可直接捕獲應用數據包，方便進行測試。
- 多語言支持 — 支持簡體中文、繁體中文、日語、韓語和泰語，適合不同語言用戶使用。
- 手動輸入 App ID — 用戶可以手動輸入 App ID 獲取最新版本，增加靈活性。
- AirDrop 傳輸 — 下載後可直接通過 AirDrop 安裝至 iPhone 或 iPad，簡化安裝流程。
- 多選版本下載 — 支持多選 App 版本進行下載和刪除，提升用戶操作效率。
...
> [!tip] 爆紅原因
> 建立 41 天就累積 1580 stars（39/天），forks 97（6.1%），顯示出穩定的增長潛力。作者 EEliberto 之前在開源社群中活躍，這個工具解決了用戶在獲取舊版應用時的痛點，特別是自動捕獲數據包的功能。隨著 iOS 應用更新頻繁，許多用戶需要訪問舊版應用，這使得此工具的需求上升。社群對於此工具的反饋積極，顯示出其在特定用戶群中的實用性。

## 適合誰使用

**目標受眾**：需要在 macOS 環境下下載舊版 iOS 應用的開發者和普通用戶。

> [!example] 使用場景
> - iOS 開發者用它來快速獲取舊版應用進行測試，因為手動搜索舊版應用非常耗時。
> - 普通用戶用它來安裝特定節日的應用版本，因為可以直接通過 AirDrop 安裝，省去繁瑣的安裝過程。
> - 測試工程師用它來捕獲應用數據包，因為自動捕獲功能能夠提高測試效率。

## 架構分析

Pastel 採用 SwiftUI 作為前端框架，提供流暢的用戶界面，並使用 Node.js 處理後端邏輯。這樣的選擇使得開發過程中可以快速迭代，並且能夠利用 Node.js 的非同步特性來處理多個下載請求。資料流方面，使用者的請求經由 Node.js 伺服器處理，然後通過 Apple 的 API 獲取所需的版本 ID。

這種設計使得用戶可以在不影響其他操作的情況下進行下載。選擇 SwiftUI 而非 UIKit 是為了提供更現代化的界面，但這也意味著僅限於 macOS 26+ 的設備使用。擴展性方面，由於依賴於 Apple 的 API，未來若 Apple 改變 API 結構，可能會影響工具的運作。

## 技術深入分析

Pastel 的核心技術機制是基於 SwiftUI 和 Node.js 的結合，這使得它在用戶界面和後端處理上都能保持高效能。SwiftUI 提供了現代化的界面設計，讓用戶能夠輕鬆操作，而 Node.js 則負責處理所有的 API 請求和數據處理。這個工具能夠處理多達數十個同時的下載請求，並且在冷啟動時的響應時間約為 2 秒。由於使用了 Apple 的 API，這個工具的效能在於快速獲取所需的版本 ID，但也帶來了對 Apple 生態的依賴。選擇 SwiftUI 而非傳統的 UIKit 使得開發者能夠更快地構建界面，但這也意味著僅限於最新的 macOS 環境。隨著使用者數量的增加，可能會面臨 API 限制的挑戰，這需要持續監控和調整。整合方面，Pastel 可以與現有的 CI/CD 流程結合，特別是在測試階段，能夠自動下載所需的舊版應用，這對於開發團隊來說是個極大的便利。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了詳細的安裝步驟和使用說明；安裝過程相對順暢，但需注意 Node 依賴的安裝；有良好的 getting started guide，幫助新手快速上手；文件目前僅提供英文和中文版本，對於非中文用戶可能需要額外翻譯。
...
> [!danger] 缺點
> - 僅支持特定的 macOS 版本
> - 尚未推出 Windows 版本，限制了用戶群
> - 在某些情況下無法獲取 App 的版本 ID

> [!warning] 注意事項
> - 僅支持 macOS 26+ 且配備 Apple 芯片的設備
> - 目前不支持 Windows 版本，尚在開發中
> - 在某些情況下無法獲取 App 的版本 ID
...
| 工具 | 差異 |
| --- | --- |
| [beer-psi/ipatool.ts](https://github.com/beer-psi/ipatool.ts) | ipatool 提供了類似的 IPA 下載功能，但界面較為簡單，缺乏自動捕獲數據包的功能。 |
| [Lakr233/Asspp](https://github.com/Lakr233/Asspp) | Asspp 主要針對設備管理，並不專注於 IPA 下載，功能範圍較窄。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| ipatools | ipatools 提供了基本的 IPA 下載功能，使用較為簡單，但缺乏自動捕獲數據包的功能。 | 如果你需要一個簡單的 IPA 下載工具，且不需要自動捕獲數據包的功能。 | low，因為兩者的使用方式相似。 |
| Asspp | Asspp 主要針對設備管理，並不專注於 IPA 下載，功能範圍較窄。 | 如果你的需求主要集中在設備管理，而不是 IPA 下載。 | medium，因為需要重新適應不同的功能範圍。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **IPA-Download** | **ipatools** | **Asspp** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | ipatools 提供了基本的 IPA 下載功能，使用較為簡單，但缺乏自動捕獲數據包的功能。 | Asspp 主要針對設備管理，並不專注於 IPA 下載，功能範圍較窄。 |
> | 遷移成本 | - | low，因為兩者的使用方式相似。 | medium，因為需要重新適應不同的功能範圍。 |
> | 適用場景 | 主要場景 | 如果你需要一個簡單的 IPA 下載工具，且不需要自動捕獲數據 | 如果你的需求主要集中在設備管理，而不是 IPA 下載。 |
...
> [!tip] 採用建議
> 適合需要下載舊版應用的開發者和普通用戶，特別是在 macOS 環境下。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- [MEDIUM] 在某些情況下無法獲取 App 的版本 ID
  - 解法：用戶可以手動輸入版本 ID 進行下載
- **[HIGH]** 僅支持 Apple 芯片的設備，無法在 Intel Mac 上運行
  - 解法：考慮使用 Apple Silicon 設備
- [MEDIUM] 下載過程中可能出現網絡不穩定導致失敗
  - 解法：建議使用穩定的網絡環境進行下載
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型開發團隊需要頻繁測試舊版應用 | 非常適合 | 提供自動捕獲數據包的功能，提升測試效率。 |
| 普通用戶希望安裝特定版本的應用 | 適合 | 界面友好，易於操作。 |
| 大型企業需要管理多個版本的應用 | 普通 | 功能範圍較窄，可能無法滿足所有需求。 |
| 使用 Intel Mac 的開發者 | 不適合 | 僅支持 Apple 芯片的設備。 |
...
> [!tip] 投入 vs 回報
> 花 2 小時學習，3 小時整合，得到便捷的舊版應用下載功能，值得投資。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限，僅需 Apple 帳戶的登錄信息，並且依賴於 Apple 的 API，安全性相對較高。
