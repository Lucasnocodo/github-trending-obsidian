...
**1.5k** stars · **52** stars/天 · 建立 29 天前 · C++ · MIT
...
> **適合** 專注於 Windows 安全漏洞的滲透測試者和安全研究員。
> **一句話重點** RoguePlanet 展示了如何利用競爭條件漏洞來實現系統權限提升，對於安全研究者來說是一個有價值的實驗工具。
...
> **結論** 花 5 小時學習、3 小時整合，得到有效的漏洞利用工具，值得在安全研究中使用。

> [!abstract] 核心創新
> 利用 Windows Defender 的競爭條件漏洞進行權限提升。

## 專案簡介

RoguePlanet 是一個針對 Windows Defender 的漏洞利用工具，利用競爭條件來獲取系統權限。該工具的核心機制是透過創建一個 race condition，當成功時會生成一個 SYSTEM shell。使用者需要在 Windows 10 或 Windows 11 上運行，並且已安裝 2026 年 6 月的補丁。這個 PoC 在某些機器上達到 100% 的成功率，但在其他機器上則可能失敗，這是因為環境差異導致的競爭條件變化。該工具的賣點在於其簡單的使用方式和高效的權限提升能力。

技術上，RoguePlanet 是用 C++ 實作，這使得它能夠直接與 Windows API 互動，並且不依賴於任何額外的庫，這樣的設計使得它在執行時的性能較為優越。與其他類似工具相比，如 0x0funky/agent-sprite-forge 和 0xGF/boneyard，RoguePlanet 的特點在於其針對 Windows Defender 的專一性，並且不需要複雜的設置。使用此工具的風險在於，若未能成功利用漏洞，可能會導致系統不穩定或無法預期的行為。該專案目前處於 alpha 階段，社群活躍度尚可，但仍需進一步的測試和驗證。適合安全研究人員和滲透測試者使用，但不建議在生產環境中使用，因為它仍然存在不穩定性和潛在的風險。
...
- 高成功率 — 在某些機器上達到 100% 的成功率，具體取決於環境。
- C++ 實作 — 直接與 Windows API 互動，無需額外依賴庫。
- 簡單的使用方式 — 只需在目標機器上運行 PoC 即可。
- 針對 Windows 10 和 11 — 專為最新版本的 Windows 系統設計。
...
```cpp
[
  "# 前置條件：需在 Windows 環境下運行",
  "void exploit() {",
  "    // 競爭條件的實作",
  "    // 具體的漏洞利用邏輯",
  "}",
  "# 預期輸出：若成功，將會生成一個 SYSTEM shell"
]
```
...
> [!tip] 爆紅原因
> 建立 29 天就累積 1517 stars（52/天），forks 601（39.6%），顯示出高度的社群參與。該專案的作者 MSNightmare 在安全領域有一定的知名度，過去也有其他相關的漏洞利用工具。RoguePlanet 解決了 Windows Defender 中一個明顯的安全漏洞，這在安全研究中是相對少見的，因為許多工具通常針對其他系統或應用程式。這個工具的出現引起了安全研究者的注意，尤其是在滲透測試和漏洞利用的領域。技術上，Windows 系統的競爭條件漏洞是相對少見且難以利用的，因此這個專案的成功吸引了許多人的興趣。forks/stars 比率高達 39.6%，顯示出許多人在實際修改和使用這個工具。

## 適合誰使用

**目標受眾**：專注於 Windows 安全漏洞的滲透測試者和安全研究員。

> [!example] 使用場景
> - 安全研究員用它來測試 Windows 系統的安全性，因為它能有效地利用 Windows Defender 的漏洞進行權限提升。
> - 滲透測試者用它來評估客戶系統的弱點，因為這個工具能在特定環境中達到高成功率。
> - 開發者用它來了解 Windows 系統的漏洞利用技術，因為它提供了一個實際的範例來學習競爭條件的應用。

## 架構分析

RoguePlanet 的架構設計主要集中在利用 Windows API 來觸發競爭條件。這種設計選擇使得它能夠直接與系統底層交互，從而提高了效率和成功率。資料流方面，工具首先會檢查系統環境，然後執行漏洞利用邏輯，最後生成一個 SYSTEM shell。

選擇 C++ 作為開發語言的好處在於其高效能和對系統資源的直接控制，但代價是開發過程中需要處理更多的底層細節。擴展性方面，這個工具的設計並不容易進行橫向擴展，因為它專注於特定的漏洞利用場景，這可能會限制其未來的功能擴展。整體來看，這個工具在特定環境中能夠發揮出色的性能，但在其他環境中可能會遇到挑戰。

## 技術深入分析

RoguePlanet 的核心技術機制是利用競爭條件來觸發系統漏洞，這種方法在安全研究中並不常見。該工具的設計使得它能夠在特定條件下獲得 SYSTEM 權限，這對於滲透測試者來說是非常有價值的。效能上，該工具能夠快速執行，並在成功時立即生成 shell，這使得它在測試過程中非常高效。選擇 C++ 作為開發語言的原因在於其對系統資源的高效控制，但這也意味著開發者需要對 Windows API 有深入的了解。技術風險方面，競爭條件的利用可能在不同環境中表現不一，這可能導致工具在某些情況下無法正常工作。整合方面，該工具的設計並不需要與其他框架或工具進行深度整合，這使得它在使用上相對獨立，降低了整合的摩擦點。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了基本的使用說明，但缺乏詳細的範例。安裝過程相對順暢，但對於新手來說，理解 C++ 編譯和執行可能需要一些時間。文件目前僅提供英文版本，缺乏多語言支持。

## 優缺點分析

> [!success] 優點
> - 高成功率，特別是在特定環境中。
> - 直接與 Windows API 互動，性能優越。
> - 簡單易用，適合快速測試和驗證。

> [!danger] 缺點
> - 僅限於 Windows 10 和 11，無法在 Windows Server 上運行。
> - 成功率受環境影響，可能不穩定。
> - 處於 alpha 階段，可能存在潛在的風險。

> [!warning] 注意事項
> - 僅在 Windows 10 和 11 上測試，Windows Server 不支援。
> - 成功率受環境影響，可能在某些機器上無法正常運行。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 該工具針對不同的 Windows 漏洞進行利用，功能範圍更廣，但不專注於 Windows Defender。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 這個工具主要針對網路漏洞，與 RoguePlanet 的專注點不同。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 這個工具專注於多種 Windows 漏洞的利用，而 RoguePlanet 專注於 Windows Defender 的特定漏洞。 | 如果需要針對不同的 Windows 漏洞進行測試，這個工具會更合適。 | medium，因為需要重新學習不同的漏洞利用邏輯。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 該工具主要針對網路漏洞，而 RoguePlanet 專注於本地系統漏洞的利用。 | 如果你的測試環境主要是網路安全，則選擇這個工具會更合適。 | high，因為需要完全轉換測試策略。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **RoguePlanet** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 這個工具專注於多種 Windows 漏洞的利用，而 RoguePlanet 專注於 Windows Defender 的特定漏洞。 | 該工具主要針對網路漏洞，而 RoguePlanet 專注於本地系統漏洞的利用。 |
> | 遷移成本 | - | medium，因為需要重新學習不同的漏洞利用邏輯。 | high，因為需要完全轉換測試策略。 |
> | 適用場景 | 主要場景 | 如果需要針對不同的 Windows 漏洞進行測試，這個工具會 | 如果你的測試環境主要是網路安全，則選擇這個工具會更合適。 |
...
- **[HIGH]** 在某些環境下無法成功運行，導致權限提升失敗
  - 解法：嘗試在不同的 Windows 版本上運行，或調整環境設置
- [MEDIUM] 需要特定的補丁版本，否則無法利用漏洞
  - 解法：確保系統已安裝最新的安全補丁
- [MEDIUM] 競爭條件的變化可能導致不穩定性
  - 解法：進行多次測試以確保成功率
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型安全測試團隊進行滲透測試 | 非常適合 | 該工具專注於 Windows 系統的漏洞，能夠提供有效的測試結果。 |
| 大型企業的安全審計 | 不適合 | 工具目前不穩定，可能無法在生產環境中使用。 |
| 學術研究或安全教育 | 適合 | 提供了一個實際的漏洞利用範例，適合學習和研究。 |
| 個人開發者進行安全實驗 | 適合 | 簡單易用，適合快速測試和學習。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習、3 小時整合，得到有效的漏洞利用工具，值得在安全研究中使用。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具本身不需要高權限運行，但可能會存取系統敏感資料。使用時需謹慎，特別是在生產環境中。依賴鏈的信任程度尚可，但需注意可能的安全風險。

## 生態系整合

> [!abstract] 如何融入你的工具鏈

RoguePlanet 通常與 Windows 系統的安全測試工具搭配使用，特別是在滲透測試的環節中。實際整合範例包括在使用 Metasploit 進行滲透測試時，將 RoguePlanet 作為一個權限提升的手段。該工具與主流的 CI/CD 工具如 Jenkins 和 GitHub Actions 的整合相對較少，因為它主要針對手動測試場景。整合的摩擦點主要在於需要手動配置環境，並確保所有依賴項正確安裝。

## 歷史脈絡

> [!info] 這個工具為什麼現在出現？

在 RoguePlanet 出現之前，Windows 系統的安全測試主要依賴於其他通用的漏洞利用工具，這些工具通常針對更廣泛的漏洞。隨著 Windows 系統的更新和安全性加強，針對特定漏洞的工具需求逐漸增加。RoguePlanet 的出現恰好填補了這一空白，提供了一個專門針對 Windows Defender 的漏洞利用方案。

這個工具代表了在安全測試中越來越專注於特定漏洞的趨勢，未來可能會有更多類似的專案出現。
