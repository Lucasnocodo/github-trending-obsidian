...
**1.6k** stars · **33** stars/天 · 建立 48 天前 · C++ · MIT
...
> **授權** MIT (商業友好) · **維護** Slow (最後推送 48 天前) · **貢獻者** Solo (bus factor 風險) · **參與度** High
> **適合** 對 Windows 安全性有深入研究需求的安全研究人員或開發者。
> **一句話重點** 這個專案展示了如何利用 Windows Defender 的競爭條件漏洞，並強調了安全研究中的環境因素對成功率的影響。
...
> **結論** 花 4 小時學習，2 小時整合，得到針對 Windows Defender 的漏洞利用，值得嘗試。
...
RoguePlanet 是一個針對 Windows Defender 的漏洞利用工具，利用競爭條件來獲取系統權限。使用者在執行該工具時，若成功，將會獲得一個 SYSTEM shell。該漏洞在 Windows 10 和 Windows 11 的某些版本中經過測試，但在 Windows Server 上則無法直接使用，因為標準用戶無法掛載 ISO 映像。這意味著雖然 Windows Server 也存在此漏洞，但需要重新設計 PoC 才能利用。此工具的賣點在於其在某些機器上可達到 100% 的成功率，這使得它在特定環境下非常有效。使用者需注意，該漏洞的成功率受環境影響，並非每次都能成功。

工具的設計考量了多種 Windows 環境，並指出了在不同系統上可能的限制。與其他類似工具相比，RoguePlanet 的獨特之處在於其專注於 Windows Defender 的特定漏洞，並且提供了針對該漏洞的具體利用方式。使用者在實際操作中可能會遇到成功率不穩定的情況，這是由於競爭條件的特性所致。維護者的活躍度相對較高，並且社群中有不少互動，顯示出對該專案的興趣和支持。整體來看，這是一個針對特定漏洞的專案，適合對 Windows 安全性有深入研究需求的安全研究人員或開發者。
...
- 高成功率 — 在某些機器上可達到 100% 的成功率，具體取決於環境。
- 多版本支持 — 在 Windows 10 和 Windows 11 中經過測試，顯示出其廣泛的適用性。
- 社群互動 — 許多使用者在 GitHub 上提出問題和建議，顯示出活躍的社群支持。
- 易於修改 — 提供了重新設計 PoC 的可能性，讓開發者能夠根據需求進行調整。
...
> [!tip] 爆紅原因
> 建立 48 天就累積 1560 stars（33/天），forks 603（38.7%），這顯示出相當高的社群參與度。作者 MSNightmare 在安全研究領域有一定的知名度，過去可能有其他相關專案。這個專案解決了 Windows Defender 的一個具體漏洞，之前的解決方案往往無法針對該漏洞進行有效利用。社群的反應熱烈，特別是對於漏洞利用的討論和合作提議，顯示出該專案的潛在價值。技術生態的變化，如對 Windows 安全性漏洞的關注增加，使得這個工具的需求上升。高達 38.7% 的 forks/stars 比率顯示出許多使用者不僅是觀望，而是積極參與修改和使用這個工具。

## 適合誰使用

**目標受眾**：對 Windows 安全性有深入研究需求的安全研究人員或開發者。

> [!example] 使用場景
> - 安全研究人員用它來測試 Windows Defender 的安全性，因為它能夠揭示系統的潛在漏洞，並提供具體的利用方式。
> - 滲透測試工程師用它來評估客戶系統的防禦能力，因為這能幫助他們了解系統在面對真實攻擊時的表現。
> - 開發者用它來研究競爭條件漏洞的工作原理，因為這能增進他們對安全漏洞的理解和防範能力。

## 架構分析

RoguePlanet 的架構設計主要是基於 C++，專注於利用 Windows Defender 的特定漏洞。該工具的核心是利用競爭條件來獲取系統權限，這意味著它需要在特定的時間窗口內執行特定的操作。資料流方面，工具會在執行時檢查系統狀態，並根據當前環境調整其行為。

選擇 C++ 作為開發語言是因為其高效能和對系統資源的直接控制，這在安全漏洞利用中至關重要。然而，這也意味著開發過程中需要對系統底層有深入的理解。擴展性方面，若要支持更多 Windows 版本或不同的攻擊向量，可能需要進行大量的重構和測試。

整體來看，這個工具的設計是為了針對特定漏洞進行高效利用，但在擴展性和通用性上可能會有所限制。

## 技術深入分析

RoguePlanet 的核心技術機制是利用 Windows Defender 的競爭條件漏洞，這是一種常見的安全漏洞類型，通常涉及到多線程或多進程環境中的資源競爭。該工具的效能特性在於其能夠在特定條件下獲得 SYSTEM 權限，這對於安全研究人員來說是非常有價值的。然而，這種利用方式的成功率會受到環境的影響，並且在某些情況下可能無法重現。選擇 C++ 作為開發語言的好處在於其高效能和對系統的直接控制，但這也意味著開發者需要對 Windows 系統有深入的了解。技術風險方面，競爭條件的特性可能在不同的系統配置中表現不一致，這可能導致在某些環境中無法成功利用。此外，該工具的設計並未考慮到 Windows Server 的特定限制，這可能會影響其在企業環境中的適用性。整合方面，該工具可以與其他安全測試工具搭配使用，但需要注意其對環境的要求，可能會增加整合的複雜度。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了基本的使用說明，但缺乏詳細的範例和安裝指導。安裝過程相對順暢，但對於新手來說，可能需要額外的背景知識。沒有提供多語言支持，僅有英文版本。整體來說，對於有一定基礎的使用者來說，能在 30 分鐘內跑起來，但新手可能會遇到困難。

## 優缺點分析

> [!success] 優點
> - 針對特定漏洞的高效利用，能夠獲得 SYSTEM 權限。
> - 活躍的社群支持，使用者能夠獲得幫助和建議。
> - 提供了重新設計 PoC 的可能性，靈活性高。

> [!danger] 缺點
> - 成功率受環境影響，並非每次都能成功。
> - 在 Windows Server 上無法直接使用，限制了適用範圍。
> - 需要對競爭條件有一定的理解才能有效利用。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於其他 Windows 漏洞的利用，並提供不同的攻擊向量，適合需要多樣化攻擊手法的安全研究人員。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 針對不同的系統漏洞進行利用，提供了更廣泛的漏洞庫，適合需要多平台支持的使用者。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於其他 Windows 漏洞的利用，並提供不同的攻擊向量，適合需要多樣化攻擊手法的安全研究人員。 | 如果你的需求是針對多種 Windows 漏洞進行測試，而不僅僅是 Windows Defender。 | medium，因為需要重新學習不同的漏洞利用方式。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 針對不同的系統漏洞進行利用，提供了更廣泛的漏洞庫，適合需要多平台支持的使用者。 | 如果你的團隊需要在多個平台上進行安全測試，而不僅限於 Windows。 | high，因為需要重新適應不同的工具和流程。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **RoguePlanet** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於其他 Windows 漏洞的利用，並提供不同的攻擊向量，適合需要多樣化攻擊手法的安全研究人員。 | 針對不同的系統漏洞進行利用，提供了更廣泛的漏洞庫，適合需要多平台支持的使用者。 |
> | 遷移成本 | - | medium，因為需要重新學習不同的漏洞利用方式。 | high，因為需要重新適應不同的工具和流程。 |
> | 適用場景 | 主要場景 | 如果你的需求是針對多種 Windows 漏洞進行測試，而不僅 | 如果你的團隊需要在多個平台上進行安全測試，而不僅限於 Win |
...
> [!tip] 採用建議
> 適合安全研究人員進行實驗，不建議用於生產環境。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- [MEDIUM] 在某些 Windows 版本上成功率不穩定，可能需要多次嘗試。
  - 解法：嘗試在不同的機器上執行，或調整執行時的環境設置。
- **[HIGH]** 在 Windows Server 上無法直接使用，限制了其適用範圍。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 安全研究人員進行漏洞測試 | 非常適合 | 專注於 Windows Defender 的漏洞，提供具體的利用方式。 |
| 企業環境的安全測試 | 不適合 | 在 Windows Server 上無法直接使用，限制了其適用性。 |
| 滲透測試工程師的工具鏈 | 適合 | 能夠幫助評估 Windows 系統的防禦能力。 |
...
> [!tip] 投入 vs 回報
> 花 4 小時學習，2 小時整合，得到針對 Windows Defender 的漏洞利用，值得嘗試。

## 安全性評估

> [!warning] 安全性快速掃描
> 中等風險：該工具需要在目標系統上執行，可能會涉及到系統權限的變更。使用時需謹慎，避免在生產環境中使用。
