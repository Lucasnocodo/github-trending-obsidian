...
**1.6k** stars · **39** stars/天 · 建立 40 天前 · C++ · MIT
...
> **授權** MIT (商業友好) · **維護** Slow (最後推送 40 天前) · **貢獻者** Solo (bus factor 風險) · **參與度** High
> **適合** 對 Windows 系統安全漏洞有興趣的安全研究者和滲透測試工程師。
> **一句話重點** RoguePlanet 提供了一個針對 Windows Defender 的專門漏洞利用工具，對於安全研究者來說具有重要的實用價值。
...
> **結論** 花 5 小時學習、3 小時整合，能夠獲得針對 Windows Defender 的漏洞利用能力，值得嘗試。
...
RoguePlanet 是一個針對 Windows Defender 的漏洞利用工具，利用競爭條件來實現系統權限提升。具體來說，當攻擊者觸發該漏洞時，系統會在不正確的狀態下執行某些操作，最終可能導致一個 SYSTEM shell 被啟動。這個 PoC 在 Windows 10 和 Windows 11 的最新更新中經過測試，但在 Windows Server 上無法直接使用，因為標準用戶無法掛載 ISO 檔案。這意味著雖然 Windows Server 版本也存在漏洞，但需要重新設計利用方式。這種設計選擇使得漏洞利用的成功率在不同機器上可能有所不同，甚至在某些情況下達到 100%。使用者需要注意，這個工具的成功率並不保證，因為它依賴於特定的執行條件。

與其他類似工具相比，RoguePlanet 的獨特之處在於其針對 Windows Defender 的專門設計，這使得它在針對 Windows 環境的攻擊中具有特定的優勢。雖然有其他工具也專注於權限提升，但大多數不會針對 Windows Defender 進行專門的攻擊。實際使用中，這個工具的效能和成功率可能會受到環境配置的影響，特別是在不同版本的 Windows 系統上。維護者的活躍度相對較高，社群對於這個工具的反饋也顯示出一定的興趣和參與度。這個專案目前處於 alpha 階段，對於安全研究者和漏洞利用者來說，值得關注和試用，但不建議在生產環境中使用。對於需要在 Windows 環境中進行安全測試的團隊來說，這是一個有趣的選擇，但使用時需謹慎評估其風險。

**技術棧**：`C++`

## 重點功能

- 競爭條件利用 — 利用 Windows Defender 的競爭條件實現權限提升。
- 跨版本支持 — 在 Windows 10 和 Windows 11 上經過測試，顯示出良好的兼容性。
- 可重現性 — 在某些機器上達到 100% 的成功率，雖然在其他機器上可能會有變化。
- 開源社群支持 — 社群活躍，提供反饋和改進建議。
...
> [!tip] 爆紅原因
> 建立 40 天就累積 1555 stars（39/天），forks 605（38.9%），這顯示出強烈的社群參與和興趣。作者 MSNightmare 以其在安全領域的專業知識而聞名，這個專案解決了 Windows 系統中權限提升的痛點，特別是針對 Windows Defender 的漏洞利用。過去，針對 Windows 系統的權限提升工具多數不夠專注，這使得 RoguePlanet 成為一個有吸引力的選擇。社群的反饋和討論也顯示出對這個工具的興趣，尤其是在安全研究和漏洞利用領域。這個專案的成功與其針對特定漏洞的專注有關，並且在技術生態中填補了某些空白。

## 適合誰使用

**目標受眾**：對 Windows 系統安全漏洞有興趣的安全研究者和滲透測試工程師。

> [!example] 使用場景
> - 安全研究員用它來測試 Windows 系統的安全性，因為它能夠揭示 Windows Defender 的漏洞，幫助改善系統防護。
> - 滲透測試工程師用它來驗證客戶系統的防禦能力，因為它提供了一個具體的漏洞利用示例，能夠測試防禦措施的有效性。
> - 開發者用它來理解 Windows 系統的安全漏洞，因為它能夠幫助他們學習如何設計更安全的應用程式。

## 架構分析

RoguePlanet 的架構基於 C++，專注於利用 Windows Defender 的競爭條件漏洞。這種設計選擇使其能夠直接與 Windows 系統交互，實現權限提升。資料流方面，當利用成功時，系統會啟動一個 SYSTEM shell，這是透過觸發特定的競爭條件來實現的。

這種方法的優勢在於其直接性和有效性，但也意味著在不同環境中可能會有變化。由於依賴於特定的系統行為，這種設計在某些情況下可能會導致不穩定性，特別是在不同版本的 Windows 系統上。整體而言，這個專案在設計上具備一定的靈活性，但在實際應用中可能會面臨一些挑戰。

## 技術深入分析

RoguePlanet 利用競爭條件漏洞來實現系統權限提升，這種方法依賴於 Windows 系統的特定行為。其核心技術機制是通過觸發競爭條件，使得系統在不正確的狀態下執行操作，最終導致 SYSTEM shell 的啟動。這種設計使得在某些環境中能夠達到 100% 的成功率，但在其他環境中則可能會失敗。從效能和規模特性來看，這個工具在 Windows 10 和 Windows 11 上表現良好，但在 Windows Server 上的應用受到限制。

設計選擇上，使用 C++ 使得其能夠直接與系統交互，但也帶來了在不同版本間的兼容性問題。這個專案的依賴樹相對簡單，主要集中在 C++ 標準庫，沒有過多的外部依賴。技術風險方面，競爭條件的存在意味著在不同環境中可能會出現不穩定性，這在未來的擴展中可能會成為一個問題。整合方面，由於專注於 Windows 環境，與主流開發工具的整合難度較低，但在 CI/CD pipeline 中的使用可能需要額外的配置。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了基本的使用說明，但缺乏詳細的範例和說明。安裝過程相對順暢，但對於新手來說可能需要一些 C++ 編譯的基礎。沒有提供多語言支持，對於非英語使用者可能會造成一定的障礙。

## 優缺點分析

> [!success] 優點
> - 專注於 Windows Defender 的漏洞，提供針對性強的利用方式。
> - 開源且社群活躍，便於獲得支持和改進。
> - 在某些環境中能達到高成功率，具備實用性。

> [!danger] 缺點
> - 成功率受環境影響，無法保證每次都能成功。
> - 在 Windows Server 上無法直接使用，限制了其應用範圍。
> - 目前仍處於 alpha 階段，可能存在不穩定性。

> [!warning] 注意事項
> - 僅在 Windows 10 和 Windows 11 上經過測試，對於其他版本的支持不明確。
> - 在 Windows Server 上無法直接使用，因為標準用戶無法掛載 ISO。
> - 成功率可能因環境不同而有所變化，並不保證每次都能成功。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於針對 Windows 環境的權限提升，但使用不同的技術路線，可能不如 RoguePlanet 專注於 Windows Defender 的漏洞。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供類似的漏洞利用功能，但針對不同的系統漏洞，使用場景和效果可能有所不同。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於 Windows 環境的權限提升，但使用不同的技術路線，可能不如 RoguePlanet 專注於 Windows Defender 的漏洞。 | 如果你的團隊需要針對特定的 Windows 環境進行權限提升測試，且不需要專注於 Windows Defender 的漏洞。 | medium，因為需要重新學習工具的使用方式和特定的漏洞利用技術。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供類似的漏洞利用功能，但針對不同的系統漏洞，使用場景和效果可能有所不同。 | 如果你的需求是針對其他系統漏洞進行測試，而不僅限於 Windows Defender。 | medium，因為需要適應不同的漏洞利用策略和工具特性。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **RoguePlanet** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於 Windows 環境的權限提升，但使用不同的技術路線，可能不如 RoguePlanet 專注於 Windows Defender 的漏洞。 | 提供類似的漏洞利用功能，但針對不同的系統漏洞，使用場景和效果可能有所不同。 |
> | 遷移成本 | - | medium，因為需要重新學習工具的使用方式和特定的漏洞利用技術。 | medium，因為需要適應不同的漏洞利用策略和工具特性。 |
> | 適用場景 | 主要場景 | 如果你的團隊需要針對特定的 Windows 環境進行權限提升 | 如果你的需求是針對其他系統漏洞進行測試，而不僅限於 Wind |
...
> [!tip] 採用建議
> 適合安全研究者進行測試和學習，但不建議在生產環境中使用。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在某些 Windows 版本上可能無法成功利用，特別是未更新的版本
  - 解法：確保系統已更新至最新版本
- [MEDIUM] 在 Windows Server 上無法直接使用，因為無法掛載 ISO
  - 解法：考慮重新設計利用方式以適應 Server 環境
- [MEDIUM] 成功率受環境影響，可能無法每次都成功
  - 解法：多次嘗試，或在不同環境中測試
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 大型企業的安全測試團隊 | 適合 | 能夠針對 Windows Defender 進行專門的漏洞測試，提升安全性。 |
| 小型開發團隊進行安全研究 | 普通 | 雖然有用，但可能需要額外的學習成本。 |
| 對 Windows Server 環境進行測試 | 不適合 | 無法直接在 Server 環境中使用，限制了應用範圍。  |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習、3 小時整合，能夠獲得針對 Windows Defender 的漏洞利用能力，值得嘗試。

## 安全性評估

> [!warning] 安全性快速掃描
> 中等風險：該工具本身不需要高權限，但可能會觸及敏感系統資源。使用時需謹慎，避免在生產環境中使用。
