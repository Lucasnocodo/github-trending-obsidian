...
**1.5k** stars · **44** stars/天 · 建立 35 天前 · C++ · MIT
...
> **授權** MIT (商業友好) · **維護** Slow (最後推送 35 天前) · **貢獻者** Solo (bus factor 風險) · **參與度** High
> **適合** 對 Windows 安全性有深入研究的安全研究人員和滲透測試工程師。
> **一句話重點** 這個專案揭示了 Windows Defender 中一個未被廣泛認識的漏洞，並提供了一個具體的利用範例。
...
> **結論** 花 10 小時學習，5 小時整合，得到直接的漏洞利用效果，值得對 Windows 安全有興趣的研究者嘗試。

> [!abstract] 核心創新
> 利用競爭條件漏洞來獲取 Windows 系統的高權限，這在現有工具中相對少見。

## 專案簡介

RoguePlanet 是一個針對 Windows Defender 的漏洞利用工具，利用了競爭條件的特性來獲取系統權限。該工具的核心機制是透過特定的操作來觸發競爭條件，導致系統在處理過程中出現錯誤，最終使攻擊者能夠執行任意命令。使用者只需在支援的 Windows 10 或 Windows 11 系統上運行該工具，若成功，將會獲得 SYSTEM 權限的 shell。這種設計的優勢在於其簡單性和直接性，但缺點是成功率因系統環境而異，可能需要多次嘗試。該工具使用 C++ 編寫，並針對 Windows 的特定版本進行了測試，這使得它在特定環境下的效能表現良好。與其他類似工具相比，如 0x0funky/agent-sprite-forge 和 0xGF/boneyard，RoguePlanet 專注於 Windows Defender 的特定漏洞，這使得它在針對性攻擊中更具優勢。

該工具的使用效果可能受到系統配置和更新的影響，使用者需要注意其適用範圍和限制。維護者的活躍度不高，社群互動主要集中在幾個熱門問題上，顯示出該工具的使用者對於其效能和可靠性仍有疑慮。總體來看，這是一個針對特定漏洞的實驗性工具，適合對 Windows 安全性有深入研究的安全研究人員使用。對於普通用戶來說，使用此工具的風險和不確定性較高，建議謹慎評估。使用者應該在測試環境中進行實驗，避免在生產環境中使用。
...
- 針對 Windows 10 和 11 測試 — 在最新的 Windows 系統上進行了測試，確保其有效性。
- 簡單的使用流程 — 只需運行該工具即可嘗試獲取權限，無需複雜的配置。
- 開源 MIT 授權 — 允許用戶自由使用和修改，促進社群貢獻。
- 活躍的社群互動 — 討論區域有多個熱門問題，顯示出使用者的關注和反饋。
...
> [!tip] 爆紅原因
> 建立 35 天內累積 1532 stars（44 stars/天），forks 603（39.4%），顯示出強烈的社群興趣。作者 MSNightmare 以其在安全領域的專業知識而聞名，這個專案解決了 Windows 系統中一個未被廣泛認識的漏洞，提供了一個具體的攻擊實例。該工具的出現引起了安全研究者的關注，尤其是在 Windows 環境中，因為這是一個相對少見的漏洞利用案例。社群的反應和互動顯示出對該工具的好奇和實驗性質，這也促進了其快速增長。

## 適合誰使用

**目標受眾**：對 Windows 安全性有深入研究的安全研究人員和滲透測試工程師。

> [!example] 使用場景
> - 安全研究人員用它來測試 Windows 系統的安全性，因為它能夠揭示 Windows Defender 的漏洞，幫助評估系統防禦能力。
> - 滲透測試工程師用它來模擬攻擊，因為該工具能夠在特定環境下成功獲取系統權限，提供真實的測試數據。
> - 開發者用它來了解競爭條件漏洞的工作原理，因為該工具提供了實際的漏洞利用範例，幫助學習安全編程的最佳實踐。

## 架構分析

RoguePlanet 的架構設計專注於利用 Windows Defender 的競爭條件漏洞。該工具的核心在於觸發一個特定的競爭條件，這需要精確的時機和系統狀態。資料流從用戶執行命令開始，然後通過 C++ 程式碼處理系統調用，最終實現權限提升。這種設計的代價在於成功率不穩定，因為它依賴於系統的具體狀態和環境配置。擴展性方面，由於其針對特定漏洞，未來的擴展可能需要重新設計 PoC 以適應不同的系統環境。

## 技術深入分析

RoguePlanet 的核心技術機制是利用 Windows Defender 的競爭條件漏洞，這是一種在多線程環境中常見的安全問題。該工具的設計使其能夠在特定條件下觸發漏洞，從而獲得系統權限。效能方面，該工具在 Windows 10 和 11 上的表現良好，但在 Windows Server 上的效能未經驗證，這可能是未來擴展的瓶頸。選擇 C++ 作為開發語言的原因在於其高效能和對系統調用的良好支持，但這也意味著使用者需要具備一定的 C++ 編程知識。技術風險方面，若未來 Windows 更新修補了此漏洞，該工具的有效性將大大降低。此外，對於外部依賴的風險相對較低，因為該工具主要依賴於 Windows 系統本身的行為。整合方面，該工具的使用需要一定的技術背景，對於普通用戶來說，可能存在一定的學習曲線。

## 新手體驗

> [!info] 上手難度評估
> README 文件簡潔明瞭，提供了基本的使用說明，但缺乏詳細的範例和安裝指導。安裝過程相對順暢，但使用者需要具備一定的 C++ 知識來編譯專案。沒有提供多語言支持，對於非英語使用者可能會造成困難。
...
> [!danger] 缺點
> - 成功率不穩定，依賴於系統環境。
> - 不支援 Windows Server，限制了應用範圍。
> - 社群活躍度不高，可能影響未來的支持和更新。

> [!warning] 注意事項
> - 成功率依賴於系統環境，可能需要多次嘗試。
> - 不支援 Windows Server，因為標準用戶無法掛載 ISO 映像。
> - 僅在特定版本的 Windows 10 和 11 上測試，其他版本的效能未經驗證。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於不同的 Windows 漏洞利用，提供更廣泛的攻擊面，但不針對 Windows Defender。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 針對多種 Windows 漏洞的綜合工具，功能更全面，但使用複雜度較高。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於不同的 Windows 漏洞利用，提供更廣泛的攻擊面，但不針對 Windows Defender。 | 如果需要一個更全面的漏洞利用工具，並且不特定針對 Windows Defender。 | medium，因為需要重新學習工具的使用方式和特性。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 針對多種 Windows 漏洞的綜合工具，功能更全面，但使用複雜度較高。 | 如果你的團隊需要多種漏洞利用功能，並且能夠處理較高的學習曲線。 | high，因為需要重新適應新的工具和操作流程。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **RoguePlanet** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於不同的 Windows 漏洞利用，提供更廣泛的攻擊面，但不針對 Windows Defender。 | 針對多種 Windows 漏洞的綜合工具，功能更全面，但使用複雜度較高。 |
> | 遷移成本 | - | medium，因為需要重新學習工具的使用方式和特性。 | high，因為需要重新適應新的工具和操作流程。 |
> | 適用場景 | 主要場景 | 如果需要一個更全面的漏洞利用工具，並且不特定針對 Windo | 如果你的團隊需要多種漏洞利用功能，並且能夠處理較高的學習曲線 |
...
- **[HIGH]** 在某些系統上可能無法成功運行，導致權限提升失敗。
  - 解法：嘗試在不同的系統配置上運行，或多次嘗試。
- [MEDIUM] 不支援 Windows Server，限制了使用範圍。
  - 解法：考慮重新設計 PoC 以適應 Windows Server 環境。
- [MEDIUM] 社群活躍度不高，可能影響未來的支持。
  - 解法：關注 GitHub 上的更新，或考慮其他活躍的替代方案。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 安全研究人員進行 Windows 系統的漏洞測試 | 非常適合 | 該工具專注於 Windows Defender 的漏洞，提供直接的測試方法。 |
| 企業環境中的滲透測試 | 不適合 | 在生產環境中使用此工具風險較高，可能導致系統不穩定。 |
| 學術研究中的安全漏洞分析 | 適合 | 該工具提供了實際的漏洞利用範例，對於學術研究有幫助。 |
| 開發者學習安全編程 | 普通 | 雖然提供了實際範例，但需要一定的 C++ 知識。 |
...
> [!tip] 投入 vs 回報
> 花 10 小時學習，5 小時整合，得到直接的漏洞利用效果，值得對 Windows 安全有興趣的研究者嘗試。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具本身不需要高權限運行，但使用時需謹慎，因為可能會影響系統穩定性。對於敏感資料的存取風險相對較低，但仍需在安全環境中進行測試。

## 生態系整合

> [!abstract] 如何融入你的工具鏈

RoguePlanet 通常與 Windows 系統的安全測試工具搭配使用，特別是在滲透測試和安全研究中。實際整合範例包括在使用 Metasploit 進行滲透測試時，利用 RoguePlanet 來測試 Windows Defender 的防禦能力。該工具與主流 CI/CD 工具的整合相對較少，主要因為其特定的用途和高風險性。與其他安全工具的相容性需要進一步測試，特別是在自動化測試環境中。整合的摩擦點主要在於使用者需要具備一定的技術背景來正確配置和運行該工具。
...
**社群活躍度**：社群互動主要集中在幾個熱門問題上，顯示出對工具的關注。
