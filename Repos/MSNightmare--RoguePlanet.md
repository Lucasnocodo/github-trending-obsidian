...
**1.6k** stars · **37** stars/天 · 建立 42 天前 · C++ · MIT
...
> **授權** MIT (商業友好) · **維護** Slow (最後推送 42 天前) · **貢獻者** Solo (bus factor 風險) · **參與度** High
> **適合** 對 Windows 安全性有興趣的滲透測試者和安全研究人員。
> **一句話重點** 這個工具展示了如何利用 Windows 系統中的競爭條件來實現權限提升，對於安全研究者來說是一個有趣的案例。
...
> **結論** 花 5 小時學習，3 小時整合，能夠獲得對 Windows 安全性漏洞的深入理解，值得一試。

> [!abstract] 核心創新
> 利用 Windows Defender 的競爭條件來實現系統權限提升的漏洞利用工具。

## 專案簡介

RoguePlanet 是一個針對 Windows Defender 的漏洞利用工具，利用了競爭條件（race condition）來獲取系統權限。用戶只需在受影響的 Windows 10 或 Windows 11 系統上運行此 PoC，若成功，將會產生一個 SYSTEM shell。這個工具的設計重點在於其可變性，因為在不同的機器上成功率有所不同，開發者報告在某些機器上達到 100% 的成功率，而在其他機器上則可能失敗。這種不穩定性是由於競爭條件的特性所致，這使得該漏洞的利用變得有趣且具有挑戰性。該工具目前不適用於 Windows Server，因為標準用戶無法掛載 ISO 映像，這限制了其在伺服器環境中的應用，但開發者相信只需重新設計 PoC 即可克服這一限制。

這個工具的賣點在於它能夠在不需要複雜設置的情況下，直接利用 Windows 系統的安全漏洞來獲取高權限。由於其設計的靈活性，開發者認為可以進一步改進以提高成功率。這個專案的活躍度不高，僅有 9 個開放問題，且只有 25% 的解決率，顯示出社群的參與度相對有限。對於安全研究人員和滲透測試者來說，這是一個值得關注的工具，但其不穩定性可能會影響實際應用的可靠性。
...
- 多版本支援 — 在 Windows 10 和 Windows 11 上測試有效，顯示出廣泛的適用性。
- 簡單的 PoC — 用戶只需運行一個命令即可嘗試利用漏洞，降低了使用門檻。
- 靈活性 — 開發者認為可以透過重新設計來提高成功率，顯示出未來的擴展潛力。
- 社群互動 — 有活躍的社群反饋，雖然解決率不高，但仍有不少用戶參與討論。
...
> [!tip] 爆紅原因
> 建立 42 天內累積 1557 stars（37/天），forks 604（38.8%），顯示出強烈的興趣和參與度。作者 MSNightmare 似乎是一位專注於安全漏洞的開發者，這個專案解決了 Windows 系統中一個未被廣泛報導的漏洞，這使得它在安全領域引起了注意。該專案的流行可能與社群對於 Windows 安全性問題的關注有關，尤其是在滲透測試和漏洞利用方面。這個工具的出現填補了市場上對於 Windows Defender 漏洞利用的需求，並且吸引了許多安全研究者的興趣。forks/stars 比率高達 38.8%，顯示出許多人對於修改和實驗的興趣。

## 適合誰使用

**目標受眾**：對 Windows 安全性有興趣的滲透測試者和安全研究人員。

> [!example] 使用場景
> - 安全研究人員用它來測試 Windows 系統的安全性，因為它能快速驗證 Windows Defender 的漏洞。
> - 滲透測試者用它來模擬攻擊，評估企業防禦系統的有效性，因為它提供了一個實用的攻擊向量。
> - 開發者用它來學習和實驗漏洞利用技術，因為它的 PoC 簡單易用，適合初學者進行實驗。

## 架構分析

RoguePlanet 的架構基於 C++ 編寫，專注於利用 Windows 系統中的競爭條件來達成權限提升。這種設計模式使得工具能夠在不同的 Windows 環境中運行，但也帶來了成功率的不穩定性。資料流方面，工具首先檢查系統環境，然後嘗試執行漏洞利用，這一過程中需要考慮到不同機器的配置差異。

選擇 C++ 作為開發語言是因為其對於系統層級操作的強大支持，但也使得工具的學習曲線相對較陡。這個工具的擴展性受到 Windows 系統本身的限制，未來可能需要針對不同版本進行調整。整體而言，這個專案在設計上兼顧了靈活性和實用性，但在穩定性上仍有待提升。

## 技術深入分析

RoguePlanet 的核心技術機制在於利用 Windows Defender 的競爭條件，這是一種在多線程環境中常見的漏洞。該工具的效能在於其能夠在不同的 Windows 版本上運行，但成功率因環境而異，這使得其在實際應用中可能會遇到挑戰。設計上選擇 C++ 是因為其對於系統層級操作的強大支持，但這也意味著使用者需要具備一定的編程基礎。該工具的依賴性相對較低，主要依賴於 Windows 系統本身的行為。技術風險方面，由於競爭條件的特性，隨著系統環境的變化，成功率可能會受到影響。整合方面，這個工具可以與其他滲透測試工具搭配使用，但在實際操作中可能需要調整以適應不同的環境。整體而言，這個專案在技術上具有挑戰性，適合對安全漏洞有興趣的開發者進行深入研究。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了基本的使用說明，但缺乏詳細的範例和安裝指導。安裝過程相對順暢，但對於不熟悉 C++ 的用戶可能會有一定的挑戰。文件沒有多語言支持，僅提供英文版本，這對於非英語使用者可能造成困難。

## 優缺點分析

> [!success] 優點
> - 能夠快速驗證 Windows 系統的安全性。
> - 提供了簡單的 PoC，降低了使用門檻。
> - 活躍的社群反饋，能夠獲得即時的幫助。
...
> - 僅針對特定 Windows 版本進行測試，其他版本的效果未知。

> [!warning] 注意事項
> - 不支援 Windows Server，因為標準用戶無法掛載 ISO。
> - 成功率因機器而異，可能需要多次嘗試。
> - 僅適用於特定的 Windows 版本，其他版本未經測試。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型企業的 IT 安全測試 | 適合 | 能夠快速驗證 Windows 系統的安全性，適合小型企業的資源。 |
| 大型企業的安全審計 | 普通 | 成功率不穩定，可能需要更多的測試和調整。 |
| 安全研究者的漏洞挖掘 | 非常適合 | 提供了一個有趣的案例來學習和實驗漏洞利用技術。 |
| 教育機構的安全課程 | 適合 | 可以作為教學工具來展示漏洞利用的原理。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，3 小時整合，能夠獲得對 Windows 安全性漏洞的深入理解，值得一試。

## 安全性評估

> [!warning] 安全性快速掃描
> 安全性評估中等，該工具本身不需要高權限運行，但可能會接觸到敏感資料。使用時需謹慎，特別是在生產環境中。
