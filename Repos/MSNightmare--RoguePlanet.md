...
star_history: "2026-06-11:913,2026-06-11:914,2026-06-12:1119,2026-06-12:1119,2026-06-13:1218,2026-06-13:1218,2026-06-14:1252,2026-06-14:1253,2026-06-15:1275,2026-06-15:1275,2026-06-16:1292,2026-06-16:1292,2026-06-17:1294,2026-06-18:1305"
tags:
  - github
  - "category/安全"
  - "lang/c++"
...
**1.3k** stars · **163** stars/天 · 建立 8 天前 · C++ · MIT
...
> **一句話重點** 這個專案展示了如何利用 Windows Defender 的漏洞來獲取系統權限，對安全研究者來說具有重要意義。
...
> **結論** 花 5 小時學、3 小時整合，得到對 Windows Defender 漏洞的深入理解，值得進行探索。

> [!abstract] 核心創新
> 這個專案利用 Windows Defender 中的競爭條件漏洞，提供了一種新的攻擊方式。

## 專案簡介

RoguePlanet 是一個針對 Windows Defender 的漏洞利用工具，利用競爭條件來獲取系統權限。這個工具的核心機制是透過特定的時序攻擊，讓攻擊者能夠在某些條件下成功執行惡意代碼，最終獲得 SYSTEM shell。使用者只需在受影響的 Windows 10 或 Windows 11 系統上執行 PoC，若成功則會開啟一個系統管理員的命令提示字元。這個工具的「一句話賣點」是它能在特定條件下以高成功率突破 Windows 的安全防護。技術上，這個漏洞利用依賴於 Windows 的 ISO 映像掛載機制，並且在 Windows Server 上無法直接使用，因為標準用戶無法掛載 ISO。

這意味著攻擊者需要重新設計 PoC 以適應伺服器環境。與其他類似工具相比，如 0x0funky/agent-sprite-forge 和 0xGF/boneyard，RoguePlanet 的獨特之處在於其針對 Windows Defender 的具體實現，並且其成功率受限於具體的系統配置。使用者在實際操作中可能會遇到成功率不穩定的情況，這取決於系統的具體狀態和配置。社群活躍度高，許多用戶對此工具表達了興趣，這顯示出其在安全研究領域的潛在價值。這個專案目前處於早期階段，建議在測試環境中使用，並注意可能的法律風險。
...
- 多平台支持 — 在 Windows 10 和 Windows 11 上運行，但不支持 Windows Server。
- 高成功率 — 在某些機器上達到 100% 的成功率。
- 命令行界面 — 使用者可透過簡單的命令行操作來執行 PoC。
...
```cpp
[
  "# 前置條件：需要在 Windows 10 或 Windows 11 上運行；",
  "g++ exploit.cpp -o exploit",
  "# 預期輸出：如果成功，將會開啟 SYSTEM shell。"
]
```
...
> [!tip] 爆紅原因
> 建立 8 天就累積 1305 stars（163/天），forks 543（41.6%），這顯示出強烈的社群參與。作者 MSNightmare 似乎在安全研究領域有一定的經驗，這個工具解決了 Windows Defender 中一個明顯的安全漏洞，之前的解決方案往往無法有效利用這個漏洞。近期的社群討論和反饋也促進了這個專案的曝光，特別是在安全研究者之間。這個工具的成功引起了許多安全研究者的關注，並且在社群中引發了討論，顯示出其潛在的實用性和重要性。
...
> [!example] 使用場景
> - 安全研究人員用它來測試 Windows Defender 的防護能力，因為這能幫助他們評估系統的安全性和潛在漏洞。
> - 滲透測試工程師用它來模擬攻擊，因為這能有效展示 Windows 系統在特定情況下的脆弱性。
> - 系統管理員用它來驗證內部安全措施，因為這能幫助他們了解防護系統的不足之處。

## 架構分析

RoguePlanet 的架構基於 C++，設計目的是為了利用 Windows Defender 的競爭條件漏洞。這種設計使得工具能夠在特定的系統環境中運行，並且能夠直接與 Windows 系統的底層 API 進行交互。資料流方面，攻擊者需要首先準備好環境，然後透過特定的時序來觸發漏洞，這樣才能成功執行惡意代碼。這種設計的代價在於，成功率可能會因系統配置而異，並且需要針對不同環境進行調整。擴展性方面，這個工具的使用範圍主要集中在 Windows 系統，若要擴展到其他平台，則需要重新設計。

## 技術深入分析

RoguePlanet 的核心技術機制是利用 Windows Defender 中的競爭條件漏洞，這種漏洞的特點是攻擊者可以在特定的時序下觸發系統行為，從而執行未經授權的代碼。這個工具的效能特性在於它能夠在某些系統上達到 100% 的成功率，但這取決於具體的系統環境和配置。設計上，選擇 C++ 作為開發語言，因為它能夠直接與 Windows 系統的底層 API 進行交互，這樣的選擇帶來了高效能和靈活性，但也增加了開發的複雜性。技術風險方面，這個工具的成功依賴於特定的系統配置，若未來 Windows 更新修補了這個漏洞，則可能會導致工具失效。整合分析方面，這個工具在安全研究和滲透測試中具有實用價值，但在實際應用中需要謹慎考量法律風險和倫理問題。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了基本的使用說明和指令。安裝過程需要一定的 C++ 環境配置，對新手來說可能有些挑戰。文件中沒有提供詳細的入門指南，可能需要使用者自行探索。整體而言，對於有一定技術背景的使用者來說，花 30 分鐘能夠跑起來。

## 優缺點分析

> [!success] 優點
> - 針對 Windows Defender 的具體漏洞，提供了有效的攻擊手段。
> - 在特定條件下能達到高成功率，對安全研究有實際價值。
> - 開源社群活躍，能夠獲得持續的支持和改進。

> [!danger] 缺點
> - 成功率不穩定，依賴於具體的系統配置。
> - 不支持 Windows Server，限制了使用範圍。
> - 需要重新設計以適應不同的環境，增加了使用的複雜性。

> [!warning] 注意事項
> - 僅支持 Windows 10 和 Windows 11，無法在 Windows Server 上直接使用。
> - 成功率依賴於具體的系統配置，可能會有不穩定的情況。
> - 需要重新設計 PoC 以適應不同的環境。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 針對不同的 Windows 安全漏洞進行攻擊，但未專注於競爭條件。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供多種漏洞利用工具，但不專注於 Windows Defender 的特定漏洞。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於多種 Windows 漏洞的利用，但不專注於競爭條件。 | 如果需要一個更全面的漏洞利用工具，且不僅限於 Windows Defender。 | medium，因為需要重新學習不同的使用方式。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供多種漏洞利用工具，涵蓋範圍更廣，但不專注於特定漏洞。 | 如果需要針對多個系統進行測試，而不僅限於 Windows Defender。 | low，因為使用方式相似。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **RoguePlanet** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於多種 Windows 漏洞的利用，但不專注於競爭條件。 | 提供多種漏洞利用工具，涵蓋範圍更廣，但不專注於特定漏洞。 |
> | 遷移成本 | - | medium，因為需要重新學習不同的使用方式。 | low，因為使用方式相似。 |
> | 適用場景 | 主要場景 | 如果需要一個更全面的漏洞利用工具，且不僅限於 Windows | 如果需要針對多個系統進行測試，而不僅限於 Windows D |
...
- [MEDIUM] 在某些系統上成功率低，可能需要多次嘗試才能成功。
  - 解法：調整系統配置或重啟後再試。
- **[HIGH]** 不支持 Windows Server，導致無法在伺服器環境中使用。
  - 解法：重新設計 PoC 以適應伺服器環境。
- [MEDIUM] 需要特定的環境設置，對新手來說可能有挑戰。
  - 解法：參考社群討論尋求幫助。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 大型企業的安全測試團隊 | 適合 | 能夠有效評估 Windows 系統的安全性和漏洞。 |
| 小型開發團隊進行滲透測試 | 普通 | 雖然有價值，但成功率不穩定可能影響測試結果。 |
| 個人開發者進行安全研究 | 非常適合 | 對於學習和實驗有很大幫助。 |
| 政府機構的安全審計 | 不適合 | 法律風險和倫理問題需要謹慎考量。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學、3 小時整合，得到對 Windows Defender 漏洞的深入理解，值得進行探索。

## 安全性評估

> [!warning] 安全性快速掃描
> 中等風險：該工具需要在受影響的 Windows 系統上運行，可能會涉及敏感資料的存取。使用時需遵循法律和倫理規範，避免未經授權的操作。
