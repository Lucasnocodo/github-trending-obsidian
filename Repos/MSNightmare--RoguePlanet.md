...
**1.6k** stars · **35** stars/天 · 建立 44 天前 · C++ · MIT
...
> **授權** MIT (商業友好) · **維護** Slow (最後推送 44 天前) · **貢獻者** Solo (bus factor 風險) · **參與度** High
> **適合** 專注於 Windows 安全漏洞研究的安全專家和滲透測試工程師。
> **一句話重點** RoguePlanet 展示了如何利用競爭條件漏洞來突破 Windows Defender 的防護，這在安全研究中具有重要意義。
...
> **結論** 花 5 小時學習，3 小時整合，得到有效的漏洞利用工具，但需注意風險。
...
RoguePlanet 是一個針對 Windows Defender 的漏洞利用工具，利用競爭條件來獲取系統權限。具體來說，這個工具在執行時會嘗試觸發一個競爭條件，這可能導致系統 shell 被啟動。使用者需要在 Windows 10 或 Windows 11 上執行，並且已安裝 2026 年 6 月的更新。該工具的賣點在於它能夠在某些機器上達到 100% 的成功率，但在其他機器上則可能會失敗，這使得它的使用變得不穩定。技術上，這個工具是用 C++ 編寫的，並且依賴於 Windows 的內部行為來實現漏洞利用。相較於其他類似工具如 0x0funky/agent-sprite-forge 和 0xGF/boneyard，RoguePlanet 的特點在於它專注於 Windows Defender 的特定漏洞，而不是一般的系統漏洞。

這使得它在針對特定環境時更具針對性。使用者在使用時需注意，該工具在 Windows Server 上無法正常運作，因為標準用戶無法掛載 ISO 映像。這意味著如果你的環境中有 Windows Server，則需要重新設計利用方式。社群的活躍度相對較高，儘管開放的問題數量不多，但有不少使用者對這個工具表示興趣。整體來看，這個專案仍處於早期階段，未來可能會有更多的改進和更新。
...
- 跨版本支持 — 在 Windows 10 和 Windows 11 上均可運行，需安裝 2026 年 6 月的更新。
- 高成功率 — 在某些機器上可達到 100% 的成功率，但在其他機器上可能會失敗。
- 簡單的使用方式 — 只需執行 PoC 即可嘗試利用漏洞。
- 社群支持 — 有活躍的社群參與和反饋，便於使用者獲得幫助。
...
```cpp
[
  "# 前置條件：需要在 Windows 10 或 Windows 11 上執行",
  "void exploit() {",
  "    // 競爭條件利用邏輯",
  "    system(\"cmd.exe\");",
  "}",
...
> [!tip] 爆紅原因
> 建立 44 天內累積 1557 stars（35/天），forks 605（38.9%），這顯示出強烈的社群參與。作者 MSNightmare 似乎專注於安全漏洞的研究，這個專案解決了 Windows Defender 的一個具體漏洞，之前的工具無法針對這個特定的防護機制進行有效的利用。雖然沒有明顯的觸發事件，但社群的反應顯示出對這個工具的興趣和潛在的應用場景。這個工具的出現正好填補了針對 Windows Defender 的漏洞利用需求，並且在技術上具有一定的挑戰性。forks/stars 比率高達 38.9%，顯示出許多人在實際修改和使用這個工具。

## 適合誰使用

**目標受眾**：專注於 Windows 安全漏洞研究的安全專家和滲透測試工程師。

> [!example] 使用場景
> - 安全研究人員用它來測試 Windows Defender 的防護能力，因為這可以幫助他們評估系統的安全性和潛在的漏洞。
> - 滲透測試工程師用它來模擬攻擊，因為這能夠提供對 Windows 環境的深入了解，並幫助修補安全漏洞。
> - 開發者用它來學習競爭條件漏洞的利用方式，因為這能夠提升他們在安全領域的技能和知識。

## 架構分析

RoguePlanet 的架構主要依賴於 C++ 的系統調用和 Windows 的內部行為。它的設計目的是利用競爭條件，這意味著它需要在特定的時間窗口內執行某些操作。資料流方面，該工具會在執行時觸發一個競爭條件，然後試圖獲取系統權限。這種設計的代價在於成功率的不穩定性，因為不同的系統環境可能導致不同的結果。擴展性方面，這個工具在 Windows 環境中運行良好，但在 Windows Server 上的限制使得它的應用範圍受到影響。

## 技術深入分析

RoguePlanet 的核心技術機制在於利用 Windows 的競爭條件，這是一種在多執行緒環境中可能導致不預期行為的漏洞。該工具的設計使其能夠在特定條件下成功獲取系統權限。效能方面，這個工具的成功率會受到系統環境的影響，可能在某些機器上表現良好，而在其他機器上則可能失敗。設計上，選擇 C++ 作為開發語言是因為它能夠直接調用系統 API，這樣可以更有效地控制執行流程。這種選擇的代價在於需要對 Windows 系統有深入的了解。技術風險方面，這個工具在不同版本的 Windows 上可能會表現不一致，這可能會導致使用者在實際應用中遇到問題。整合方面，這個工具的使用需要一定的技術背景，對於沒有經驗的使用者來說，可能會有一定的學習曲線。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了基本的使用說明。安裝過程相對順暢，但需要注意管理權限的要求。沒有提供詳細的入門指南，對於新手來說可能需要額外的學習。文件僅提供英文版本，缺乏多語言支持。

## 優缺點分析

> [!success] 優點
> - 針對 Windows Defender 的特定漏洞，提供有效的利用方式。
> - 在某些環境下可以達到 100% 的成功率。
> - 社群活躍，使用者可以獲得支持和反饋。

> [!danger] 缺點
> - 成功率不穩定，依賴於特定的系統配置。
> - 無法在 Windows Server 上運行，限制了應用範圍。
> - 需要管理權限，對於普通用戶來說不易使用。

> [!warning] 注意事項
> - 僅支援 Windows 10 和 Windows 11，無法在 Windows Server 上運行。
> - 成功率不穩定，依賴於特定的系統環境和配置。
> - 需要在具有管理權限的環境中執行，否則無法獲得系統權限。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於不同的系統漏洞利用，並不針對 Windows Defender 的特定漏洞。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供更廣泛的漏洞利用工具，但不專注於 Windows 環境的特定防護。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於多種系統漏洞的利用，並不針對 Windows Defender 的特定漏洞。 | 如果需要一個更廣泛的漏洞利用工具，並且不特定於 Windows Defender。 | medium，因為需要重新學習工具的使用方式。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供多種漏洞利用工具，適用於不同的操作系統。 | 如果你的環境不僅限於 Windows，並且需要針對多個系統進行測試。 | medium，因為需要適應不同的工具和工作流程。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **RoguePlanet** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於多種系統漏洞的利用，並不針對 Windows Defender 的特定漏洞。 | 提供多種漏洞利用工具，適用於不同的操作系統。 |
> | 遷移成本 | - | medium，因為需要重新學習工具的使用方式。 | medium，因為需要適應不同的工具和工作流程。 |
> | 適用場景 | 主要場景 | 如果需要一個更廣泛的漏洞利用工具，並且不特定於 Window | 如果你的環境不僅限於 Windows，並且需要針對多個系統進 |
...
> [!tip] 採用建議
> 適合安全研究和滲透測試的實驗性使用，不建議用於生產環境。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在某些系統上無法成功執行，導致無法獲得系統權限
  - 解法：嘗試在不同的機器上運行，或調整系統配置
- [MEDIUM] 需要管理權限，普通用戶無法執行
  - 解法：確保以管理員身份運行
- **[HIGH]** 在 Windows Server 上無法運行，限制了使用範圍
  - 解法：重新設計利用方式以適應 Windows Server

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 專注於 Windows 環境的安全測試團隊 | 非常適合 | 專門針對 Windows Defender 的漏洞利用，能提供有效的測試手段。 |
| 需要在生產環境中使用的企業 | 不適合 | 目前尚未成熟，存在高風險。 |
| 安全研究人員進行漏洞分析 | 適合 | 能夠提供對 Windows Defender 的深入理解和測試。 |
| 普通用戶希望提升系統安全 | 不適合 | 需要管理權限，普通用戶無法使用。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，3 小時整合，得到有效的漏洞利用工具，但需注意風險。

## 安全性評估

> [!warning] 安全性快速掃描
> 中等風險：該工具需要管理權限，可能會存取系統敏感資料。對於普通用戶來說，使用時需謹慎，並確保在安全的環境中進行測試。
