...
**1.5k** stars · **42** stars/天 · 建立 37 天前 · C++ · MIT
...
> **授權** MIT (商業友好) · **維護** Slow (最後推送 37 天前) · **貢獻者** Solo (bus factor 風險) · **參與度** High
> **適合** 專注於 Windows 系統安全測試的安全研究員和滲透測試者。
> **一句話重點** RoguePlanet 是一個針對 Windows Defender 的專用漏洞利用工具，能夠在特定環境中實現系統權限提升。
...
> **結論** 花 10 小時學習、5 小時整合，得到有效的 Windows 漏洞利用工具，值得安全研究者投入。

> [!abstract] 核心創新
> 這個專案利用 Windows Defender 的競爭條件漏洞來實現系統權限提升。

## 專案簡介

RoguePlanet 是一個針對 Windows Defender 的漏洞利用工具，利用競爭條件來獲取系統權限。當用戶執行此工具時，它會嘗試在特定條件下觸發一個 race condition，從而在某些機器上成功生成一個 SYSTEM shell。這個工具在 Windows 10 和 Windows 11 的最新補丁下進行了測試，但不適用於 Windows Server，因為標準用戶無法掛載 ISO 映像。這種設計的原因在於，競爭條件的特性使得它在不同的環境中表現不一，這也是為什麼作者提到需要重新設計 PoC 以提高成功率。這個工具的賣點在於它能在特定情況下達到 100% 的成功率，雖然在其他情況下可能會失敗。技術上，它使用 C++ 編寫，並依賴於 Windows 的內部機制來觸發漏洞，這意味著它的實作相對簡單，但也有其局限性。

與其他同類工具相比，如 0x0funky/agent-sprite-forge 和 0xGF/boneyard，RoguePlanet 針對的是特定的 Windows 環境，並且專注於利用 Windows Defender 的特定漏洞，而不是一般的系統漏洞。這使得它在針對 Windows 系統的安全測試中具有獨特的價值。使用者可能會遇到的問題包括在不同版本的 Windows 上的兼容性問題，這需要使用者具備一定的技術背景來進行調整和測試。這個專案目前處於 alpha 階段，適合安全研究人員和漏洞測試者使用，但不建議用於生產環境。未來的發展可能會集中在提高成功率和擴展到其他 Windows 版本上。對於那些需要在 Windows 環境中進行安全測試的開發者來說，這是一個值得關注的工具，但需謹慎使用以避免潛在的法律風險。

**技術棧**：`C++`

## 重點功能

- 競爭條件利用 — 利用 Windows Defender 的競爭條件來獲取系統權限。
- 多版本支持 — 在 Windows 10 和 Windows 11 上進行測試，展示了其兼容性。
- PoC 可重設計 — 提供了重新設計的可能性以提高成功率。
- C++ 實作 — 使用 C++ 編寫，便於在 Windows 環境中編譯和運行。
- 開源社群支持 — 高比例的 forks 顯示出社群的活躍和對工具的實際應用。
...
> [!tip] 爆紅原因
> 建立 37 天就累積 1548 stars（42/天），forks 604（39.0%），這顯示出強烈的社群興趣。作者 MSNightmare 以其在安全研究領域的經驗而聞名，這個專案解決了 Windows 環境中對於權限提升的需求，特別是在 Windows Defender 的漏洞利用方面。這個工具的出現恰逢安全研究者對於 Windows 系統安全性的關注上升，並且在社群中引發了討論。高比例的 forks 表示許多人對這個專案進行了實際的修改和使用，顯示出其實用性和潛在的擴展性。

## 適合誰使用

**目標受眾**：專注於 Windows 系統安全測試的安全研究員和滲透測試者。

> [!example] 使用場景
> - 安全研究員用它來測試 Windows 系統的防護能力，因為它能揭示 Windows Defender 的漏洞，幫助改進安全措施。
> - 滲透測試者用它來評估客戶系統的安全性，因為這個工具能有效地模擬攻擊，發現潛在的安全風險。
> - 開發者用它來設計針對 Windows 環境的安全防護工具，因為它提供了實際的漏洞利用案例，幫助理解攻擊者的視角。

## 架構分析

RoguePlanet 的架構設計基於 C++，專注於利用 Windows Defender 的漏洞。它的資料流從用戶執行 exploit 開始，然後通過觸發競爭條件來獲取系統權限。這種設計的選擇使得工具能夠在特定環境中達到高成功率，但也意味著在其他環境中可能會失敗。選擇 C++ 作為實作語言使得它能夠直接與 Windows 系統進行交互，但也增加了對於系統環境的依賴。這個工具的擴展性受到 Windows 系統版本的限制，未來可能需要針對不同版本進行調整以提高兼容性。

## 技術深入分析

RoguePlanet 的核心技術機制是利用 Windows Defender 中的競爭條件漏洞。這種漏洞的特性使得攻擊者能夠在特定的時間窗口內觸發系統行為，從而獲取更高的權限。效能上，這個工具的成功率會受到系統配置的影響，可能在某些機器上達到 100% 的成功率，但在其他機器上則可能完全失敗。C++ 的選擇使得這個工具能夠直接與 Windows 系統進行交互，這樣的設計帶來了高效能，但也使得工具對於 Windows 環境的依賴性增加。這個專案的依賴樹相對簡單，主要集中在 C++ 的標準庫，沒有引入過多的外部依賴。技術風險方面，競爭條件的特性可能在未來的 Windows 更新中被修補，這會影響工具的有效性。整合方面，由於專注於 Windows 環境，與其他主流框架的整合難度較低，但在實際使用中可能需要進行調整以適應不同的系統配置。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的漏洞利用描述，但缺乏詳細的安裝和使用範例。安裝過程相對順暢，但對於新手來說，可能需要一些額外的技術知識。沒有提供多語言支持，主要以英文為主。

## 優缺點分析

> [!success] 優點
> - 能夠揭示 Windows Defender 的安全漏洞，對於安全研究非常有價值。
> - 高成功率的潛力，特別是在特定環境下。
> - 開源社群的支持，提供了實際的修改和擴展可能性。

> [!danger] 缺點
> - 不適用於 Windows Server，限制了其應用範圍。
> - 成功率不穩定，依賴於具體的系統配置。
> - 需要較高的技術背景來進行有效的測試和調整。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於不同的 Windows 漏洞利用，並且可能不專注於 Windows Defender。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供更廣泛的漏洞利用工具，但不一定針對 Windows Defender 的特定漏洞。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於不同的 Windows 漏洞利用，並且可能不專注於 Windows Defender。 | 如果需要針對其他 Windows 漏洞進行測試，而不僅僅是 Windows Defender。 | medium，因為需要重新調整測試方法和工具。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供更廣泛的漏洞利用工具，但不一定針對 Windows Defender 的特定漏洞。 | 如果需要一個更通用的漏洞利用工具，而不僅限於 Windows Defender。 | high，因為需要全面重新設計測試流程。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **RoguePlanet** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於不同的 Windows 漏洞利用，並且可能不專注於 Windows Defender。 | 提供更廣泛的漏洞利用工具，但不一定針對 Windows Defender 的特定漏洞。 |
> | 遷移成本 | - | medium，因為需要重新調整測試方法和工具。 | high，因為需要全面重新設計測試流程。 |
> | 適用場景 | 主要場景 | 如果需要針對其他 Windows 漏洞進行測試，而不僅僅是  | 如果需要一個更通用的漏洞利用工具，而不僅限於 Windows |
...
> [!tip] 採用建議
> 適合安全研究和漏洞測試，但不建議用於生產環境的核心路徑上。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在某些 Windows 版本上可能無法成功執行，特別是未更新的版本。
  - 解法：確保系統更新到最新版本。
- [MEDIUM] 需要以管理員身份運行才能獲得預期效果。
  - 解法：確保以管理員權限執行工具。
- [MEDIUM] 競爭條件的成功率不穩定，可能需要多次嘗試。
  - 解法：在不同的系統環境中測試以提高成功率。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 大型企業的安全測試團隊 | 非常適合 | 能夠有效測試 Windows Defender 的漏洞，提升系統安全性。 |
| 小型開發團隊進行滲透測試 | 適合 | 提供了針對 Windows 環境的具體漏洞利用案例。 |
| 個人開發者進行安全研究 | 普通 | 需要較高的技術背景和系統配置調整。 |
| 非 Windows 環境的安全測試 | 不適合 | 專注於 Windows 環境，無法應用於其他系統。 |
...
> [!tip] 投入 vs 回報
> 花 10 小時學習、5 小時整合，得到有效的 Windows 漏洞利用工具，值得安全研究者投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 中等風險：需要管理員權限運行，並可能存取系統敏感資料。對於安全研究者來說，使用時需遵循法律和道德規範。
...
**社群活躍度**：社群活躍，特別是在 GitHub 上有多個熱門討論。
