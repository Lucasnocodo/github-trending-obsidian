...
**1.6k** stars · **33** stars/天 · 建立 47 天前 · C++ · MIT
...
> **授權** MIT (商業友好) · **維護** Slow (最後推送 47 天前) · **貢獻者** Solo (bus factor 風險) · **參與度** High
> **適合** 對 Windows 安全漏洞有興趣的滲透測試人員和安全研究員。
> **一句話重點** RoguePlanet 展示了如何利用 Windows Defender 的漏洞來獲取系統權限，這在安全研究中具有重要意義。
...
> **結論** 花 5 小時學、3 小時整合，得到有效的漏洞利用工具，值得投入。
...
RoguePlanet 是一個針對 Windows Defender 的漏洞利用工具，利用競爭條件來獲取系統權限。當用戶執行此工具時，若成功，將會在目標機器上啟動一個 SYSTEM shell。該漏洞已在 Windows 10 和 Windows 11 的最新更新中進行測試，但不支援 Windows Server，因為標準用戶無法掛載 ISO 映像。這個設計的核心在於利用競爭條件，這意味著在某些條件下可能會成功，而在其他情況下則不然。開發者提到，若重新設計 PoC，可能會提高成功率，但目前已不打算進行進一步的開發。此工具的賣點在於其簡單的使用方式和高效的權限提升能力，尤其在特定環境下能達到 100% 的成功率。

技術上，這個工具使用 C++ 編寫，並依賴於 Windows 的內部機制來執行攻擊，這使得它在某些情況下相對於其他工具更具有效性。與其他類似工具相比，RoguePlanet 的獨特之處在於其針對 Windows Defender 的專一性，這使得它在特定的攻擊場景中更具針對性和有效性。使用者可能會面臨的限制包括在不同版本的 Windows 上的變異性，以及對於 Windows Server 的不支援。這個專案目前處於活躍開發階段，社群反應熱烈，並且有不少人對其進行了討論和提問。整體來看，這是一個針對特定漏洞的專案，適合對 Windows 安全性有深入了解的開發者使用。

**技術棧**：`C++`

## 重點功能

- 競爭條件利用 — 利用 Windows Defender 的競爭條件漏洞來獲取系統權限。
- 高成功率 — 在特定環境下可達到 100% 的成功率。
- C++ 實作 — 使用 C++ 編寫，能直接與 Windows 系統進行交互。
- 針對性強 — 專門針對 Windows Defender，提供特定的攻擊向量。
- 社群互動 — 社群活躍，提供即時的問題解答和討論。
...
> [!tip] 爆紅原因
> 建立 47 天就累積 1559 stars（33/天），forks 603（38.7%），這顯示出強烈的社群興趣。作者 MSNightmare 是一位專注於安全漏洞的開發者，過去在類似領域有一定的貢獻。這個專案解決了 Windows Defender 中一個明顯的漏洞，之前的工具往往無法針對這一特定問題。社群中對於此漏洞的討論和關注，顯示出對於 Windows 安全性議題的高度重視。這個工具的出現，正好填補了市場上對於 Windows Defender 漏洞利用的需求，並且在技術上具有一定的創新性。forks/stars 比率高達 38.7%，表明許多人不僅在觀望，還在實際修改和使用這個工具。

## 適合誰使用

**目標受眾**：對 Windows 安全漏洞有興趣的滲透測試人員和安全研究員。

> [!example] 使用場景
> - 安全研究員用它來測試 Windows Defender 的防護能力，因為這能幫助他們評估系統的安全性和潛在的漏洞。
> - 滲透測試人員用它來模擬攻擊，評估企業環境中 Windows 系統的脆弱性，因為這能提供真實的安全評估。
> - 開發者用它來學習如何利用競爭條件漏洞，因為這能增強他們在安全領域的技能和知識。

## 架構分析

RoguePlanet 的架構基於 C++，專注於利用 Windows 系統的內部機制。這個工具的設計目的是為了簡化漏洞利用的過程，並在特定環境下達到高成功率。資料流中，工具首先檢查系統環境，然後執行漏洞利用的核心邏輯，最終啟動一個 SYSTEM shell。

這種設計使得使用者能夠在不需要複雜配置的情況下，快速達成目標。選擇 C++ 作為開發語言的原因在於其對於系統底層的控制能力，這使得工具能夠直接與 Windows API 交互。這種設計的代價是需要使用者具備一定的 C++ 知識，並且在不同版本的 Windows 系統上可能會有不同的行為。

整體來看，這個工具在特定場景下具有良好的擴展性，但在其他環境下可能會遇到限制。

## 技術深入分析

RoguePlanet 的核心技術機制是利用 Windows Defender 中的競爭條件漏洞。這種漏洞通常難以重現，但在特定條件下可以達到高成功率。該工具能夠在 Windows 10 和 Windows 11 環境中運行，並且在測試中顯示出良好的效能。由於使用 C++ 編寫，這使得它能夠直接與 Windows 系統進行交互，並利用系統底層的 API 來執行攻擊。

這種設計的好處在於能夠快速達成目標，但代價是需要使用者具備一定的 C++ 知識。技術上，這個工具的依賴相對較少，主要依賴於 Windows 的內部機制。未來在擴展性上，若能針對 Windows Server 進行改進，將會大幅提升其適用範圍。整體來看，這是一個針對特定漏洞的專案，適合對 Windows 安全性有深入了解的開發者使用。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了基本的使用指引和範例。安裝過程相對順暢，但需要使用者具備一定的 C++ 知識。文件目前僅提供英文版本，沒有多語言支持。
...
> - 高成功率 — 在特定環境下可達到 100% 的成功率。
> - 針對性強 — 專門針對 Windows Defender，提供特定的攻擊向量。
> - 社群活躍 — 討論熱烈，能獲得即時的支持和建議。

> [!danger] 缺點
> - 不支援 Windows Server，限制了使用範圍。
> - 成功率依賴於特定環境，可能在某些機器上無法運行。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 這個工具專注於其他 Windows 漏洞，並且可能不如 RoguePlanet 針對 Windows Defender 的專一性強。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard 主要針對 Linux 環境，而 RoguePlanet 專注於 Windows 系統，適用場景截然不同。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 這個工具專注於其他 Windows 漏洞，並且可能不如 RoguePlanet 針對 Windows Defender 的專一性強。 | 如果你的目標是針對其他 Windows 漏洞，而非專注於 Windows Defender，則可以選擇此工具。 | medium，因為需要重新學習不同的漏洞利用方式。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard 主要針對 Linux 環境，而 RoguePlanet 專注於 Windows 系統，適用場景截然不同。 | 如果你的環境是 Linux，則 Boneyard 會是更合適的選擇。 | high，因為需要完全轉換到不同的操作系統。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **RoguePlanet** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 這個工具專注於其他 Windows 漏洞，並且可能不如 RoguePlanet 針對 Windows Defender 的專一性強。 | Boneyard 主要針對 Linux 環境，而 RoguePlanet 專注於 Windows 系統，適用場景截然不同。 |
> | 遷移成本 | - | medium，因為需要重新學習不同的漏洞利用方式。 | high，因為需要完全轉換到不同的操作系統。 |
> | 適用場景 | 主要場景 | 如果你的目標是針對其他 Windows 漏洞，而非專注於 W | 如果你的環境是 Linux，則 Boneyard 會是更合適 |
...
- **[HIGH]** 在某些 Windows 版本上，成功率可能不穩定，導致無法獲得 SYSTEM shell。
  - 解法：嘗試在不同的機器上運行，或調整環境設置。
- [MEDIUM] 不支援 Windows Server，限制了使用範圍。
  - 解法：考慮重新設計 exploit 以適應 Windows Server 環境。
- [MEDIUM] 需要對 C++ 有一定了解，否則難以修改和調整工具。
  - 解法：學習基本的 C++ 知識以便於使用。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 安全研究員進行漏洞測試 | 非常適合 | 該工具專門針對 Windows Defender，能提供有效的測試結果。 |
| 企業內部滲透測試 | 適合 | 能夠模擬攻擊，評估系統的安全性。 |
| 個人學習 C++ 和安全漏洞利用 | 非常適合 | 能夠提供實際的漏洞利用案例，增強學習效果。 |
| 大規模企業環境的安全測試 | 不適合 | 不支援 Windows Server，限制了使用範圍。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學、3 小時整合，得到有效的漏洞利用工具，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具本身不需要高權限運行，但使用時需注意系統安全性。依賴於 Windows 的內部機制，需評估其安全性。
