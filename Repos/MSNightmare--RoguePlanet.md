...
**1.5k** stars · **48** stars/天 · 建立 32 天前 · C++ · MIT
...
> **授權** MIT (商業友好) · **維護** Slow (最後推送 32 天前) · **貢獻者** Solo (bus factor 風險) · **參與度** High
> **適合** 專注於 Windows 安全性測試的滲透測試者和安全研究人員。
> **一句話重點** 這個專案展示了如何利用 Windows 系統中的競爭條件漏洞來獲取高權限，對於安全研究者來說是個重要的學習資源。
...
> **結論** 花 5 小時學習、3 小時整合，得到針對 Windows Defender 的漏洞利用，值得一試。

> [!abstract] 核心創新
> 利用 Windows Defender 中的競爭條件漏洞來獲取系統權限。

## 專案簡介

RoguePlanet 是一個針對 Windows Defender 的漏洞利用工具，利用競爭條件來獲取 SYSTEM 權限。當用戶執行該漏洞時，系統會在特定條件下產生競爭條件，導致安全性檢查失效，最終使攻擊者能夠執行任意命令。該工具已在 Windows 10 和 Windows 11 的最新更新上進行測試，但不支援 Windows Server，因為標準用戶無法掛載 ISO 映像。這個漏洞的成功率因環境而異，某些機器上可達 100% 成功率，而其他機器則可能失敗。該專案的賣點在於其簡單的使用方式和針對特定 Windows 環境的高效能。技術上，該工具使用 C++ 編寫，並依賴於 Windows 的內部實現來觸發競爭條件，這意味著它的效能與 Windows 的版本和配置密切相關。

與其他類似工具相比，如 0x0funky/agent-sprite-forge 和 0xGF/boneyard，RoguePlanet 專注於 Windows Defender 的特定漏洞，而不是通用的漏洞利用框架。這使得它在針對特定環境的攻擊中更具針對性，但也限制了其通用性。實際使用中，使用者可能會遇到環境配置的差異導致成功率不一的問題，這需要對環境進行調整以提高成功率。該專案目前仍在活躍開發中，並有潛力在未來進一步完善。對於安全研究人員和滲透測試者來說，這是一個值得關注的工具，但在生產環境中使用時需謹慎。適合在小型團隊或個人專案中進行測試，但不建議用於大型企業環境。
...
- 高成功率 — 在某些機器上達到 100% 的成功率，具體取決於環境配置。
- 針對 Windows 10 和 Windows 11 — 專為最新版本的 Windows 系統設計。
- 簡單的使用方式 — 只需執行特定命令即可觸發漏洞。
- 開源社群支持 — 許多開發者參與修改和擴展功能。
...
> [!tip] 爆紅原因
> 建立 32 天就累積 1522 stars（48/天），forks 602（39.6%），這顯示出強烈的社群參與。作者 MSNightmare 以其在安全領域的專業知識而聞名，這個專案解決了 Windows Defender 中一個明顯的漏洞，之前的解決方案未能有效利用該漏洞。近期的社交媒體討論和安全論壇的關注也促進了這個專案的曝光。技術上，Windows 的安全性漏洞在近期的更新中成為焦點，這使得該工具的需求上升。高達 39.6% 的 forks/stars 比率表明許多開發者正在實際修改和使用這個工具，顯示出其實用性和潛在的應用價值。

## 適合誰使用

**目標受眾**：專注於 Windows 安全性測試的滲透測試者和安全研究人員。

> [!example] 使用場景
> - 安全研究人員用它來測試 Windows Defender 的防禦能力，因為它能有效揭示系統的安全漏洞。
> - 滲透測試者用它來模擬攻擊，評估企業的安全防護，因為它提供了針對 Windows 環境的具體漏洞利用。
> - 開發者用它來學習競爭條件漏洞的利用方式，因為它是一個簡單明瞭的示範，幫助理解安全性問題。

## 架構分析

RoguePlanet 的架構基於 C++ 編寫，專注於利用 Windows 的內部實現來觸發競爭條件。這種設計使得工具能夠直接與 Windows 系統交互，從而達到高效能的漏洞利用。資料流中，工具首先檢查系統環境，然後執行特定的操作來觸發競爭條件，最終獲得 SYSTEM 權限。

選擇 C++ 而非其他語言（如 Python）是因為 C++ 提供了更高的執行效率和對系統資源的直接控制。這樣的選擇雖然增加了開發的複雜性，但卻能在性能上獲得更好的表現。隨著系統環境的不同，工具的成功率也會有所變化，這是一個需要考量的設計取捨。

整體而言，這個工具在特定環境下的效能優勢明顯，但在其他環境中可能會遇到挑戰。

## 技術深入分析

RoguePlanet 的核心技術機制是利用 Windows 系統中的競爭條件漏洞，這是一種在多執行緒環境中常見的安全問題。該工具的設計使其能夠在特定條件下觸發漏洞，從而獲得 SYSTEM 權限。效能方面，該工具能夠在某些機器上達到 100% 的成功率，但這依賴於具體的系統配置和環境。這意味著在不同的硬體上，工具的表現可能會有顯著差異。

選擇 C++ 作為開發語言的原因在於其高效能和對系統資源的直接控制，這使得工具在執行時能夠快速響應。雖然這樣的選擇提高了開發的複雜性，但卻能在性能上獲得更好的表現。技術風險方面，隨著使用者對該工具的需求增加，可能會出現安全性問題，特別是在不當使用的情況下。整合到現有的安全測試流程中，該工具能夠與其他滲透測試工具協同工作，但使用者需要具備一定的技術背景來進行有效的配置和使用。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了基本的使用說明，但缺乏詳細的範例。安裝過程相對順暢，但對於不熟悉 C++ 的使用者來說，可能需要額外的學習時間。沒有提供多語言支持，僅有英文文檔。

## 優缺點分析

> [!success] 優點
> - 專注於 Windows Defender 的特定漏洞，提供針對性強的攻擊方式。
> - 在某些環境下可達到 100% 的成功率，具備實用性。
> - 開源社群活躍，易於獲得支持和擴展功能。

> [!danger] 缺點
> - 不支援 Windows Server，限制了使用範圍。
> - 成功率依賴於具體的硬體和系統配置，可能不穩定。
> - 需要一定的 C++ 知識來進行修改和擴展。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於不同的漏洞利用，並提供更廣泛的功能，但不專注於 Windows Defender。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供通用的漏洞利用框架，適用於多種環境，但缺乏針對 Windows 的專屬優化。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於多種漏洞利用，而非僅針對 Windows Defender，提供更廣泛的功能。 | 如果需要一個通用的漏洞利用框架，適合多種環境的測試。 | medium，因為需要重新調整測試策略和工具配置。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供通用的漏洞利用框架，適用於多種環境，但缺乏針對 Windows 的專屬優化。 | 如果你的團隊需要針對多種系統進行測試，而不僅限於 Windows。 | high，因為需要全面重構測試流程和工具鏈。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **RoguePlanet** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於多種漏洞利用，而非僅針對 Windows Defender，提供更廣泛的功能。 | 提供通用的漏洞利用框架，適用於多種環境，但缺乏針對 Windows 的專屬優化。 |
> | 遷移成本 | - | medium，因為需要重新調整測試策略和工具配置。 | high，因為需要全面重構測試流程和工具鏈。 |
> | 適用場景 | 主要場景 | 如果需要一個通用的漏洞利用框架，適合多種環境的測試。 | 如果你的團隊需要針對多種系統進行測試，而不僅限於 Windo |
...
- [MEDIUM] 在某些系統上成功率低，可能需要多次嘗試才能成功。
  - 解法：調整環境配置，或在不同的機器上測試。
- **[HIGH]** 不支援 Windows Server，可能導致使用者誤解其適用範圍。
  - 解法：明確標示不支援的環境，避免誤用。
- [MEDIUM] 使用過程中可能會遇到系統安全警告，影響測試結果。
  - 解法：在受控環境中進行測試，避免干擾。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型安全團隊進行 Windows 環境的滲透測試 | 非常適合 | 專注於 Windows Defender 的漏洞，能有效揭示系統弱點。 |
| 大型企業的安全測試 | 不適合 | 不支援 Windows Server，限制了使用範圍。 |
| 個人學習安全漏洞利用 | 適合 | 提供簡單明瞭的示範，適合入門學習。 |
| 需要高穩定性的商業應用 | 不適合 | 目前仍在 beta 階段，穩定性不足。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習、3 小時整合，得到針對 Windows Defender 的漏洞利用，值得一試。

## 安全性評估

> [!warning] 安全性快速掃描
> 中等風險：該工具需要在受控環境中使用，避免在生產系統上運行，因為可能會引發安全警告和系統不穩定。
