...
use_case: "利用漏洞繞過 Bitlocker 加密，獲取未授權存取。"
priority: high
ring: assess
...
> **適合** 需要評估 Windows 11 環境中 Bitlocker 安全性的安全研究人員和滲透測試者。
> **一句話重點** 這個工具揭示了 Windows 11 中一個重大的 Bitlocker 繞過漏洞，值得安全研究者深入探討。
...
> **結論** 花 2 小時學習，1 小時整合，得到快速的 Bitlocker 繞過效果，值得安全研究者試用。

> [!abstract] 核心創新
> 這個專案揭示了一個針對 Bitlocker 的重大安全漏洞，能夠在不經授權的情況下獲取存取權。

## 專案簡介

YellowKey 是一個針對 Windows 11 和 Server 2022/2025 的 Bitlocker 繞過漏洞工具。其核心機制是將特定的 FsTx 文件夾複製到 USB 隨身碟的系統分區，然後透過 Windows Recovery Environment (WinRE) 進行重啟，最終在未經授權的情況下獲取對 Bitlocker 加密磁碟的完全存取權。這種設計的背後，似乎暗示著某種程度的故意設計，因為相關的組件在正常的 Windows 安裝中並不存在。這個工具的賣點在於其簡單易用的操作流程，使用者只需簡單的幾個步驟即可實現繞過。技術上，這個工具利用了 WinRE 中的特定組件，這些組件在正常環境下並不具備相同的功能，這使得這個漏洞的存在顯得異常可疑。

與其他 Bitlocker 繞過工具相比，YellowKey 的獨特之處在於它不需要任何外部設備的連接，僅需在系統內部進行操作。這使得它在某些情況下比其他工具更具便利性。實際使用中，這個工具的效果取決於是否能夠正確進入 WinRE 環境，並且需要注意的是，僅限於 Windows 11 和 Server 2022/2025，Windows 10 不受影響。這個專案目前仍在活躍開發中，社群的反饋也顯示出潛在的使用問題，例如在特定環境下可能無法正常運作。對於安全性評估，這個工具的成熟度尚未達到穩定階段，使用時需謹慎考量其風險。

## 重點功能

- 簡單的操作流程 — 只需將 FsTx 文件夾複製到 USB 隨身碟的特定位置，並重啟進入 WinRE。
- 無需外部設備 — 可以在不連接外部存儲的情況下進行操作，提升便利性。
- 針對 Windows 11 和 Server 2022/2025 — 專門設計來繞過這些版本的 Bitlocker，加強針對性。
- 獲取未授權存取 — 成功後可獲得對 Bitlocker 加密磁碟的完全存取權。
- 社群活躍 — 目前有多個開放的問題和討論，顯示出使用者的實際需求和反饋。
...
2. 將 USB 插入目標電腦
```bash
Insert USB stick into target Windows computer with Bitlocker enabled.
```
3. 重啟進入 WinRE 環境
```bash
...
4. 進入 shell 環境
```bash
Hold CTRL after releasing SHIFT.
```

## 為什麼值得關注

> [!tip] 爆紅原因
> 建立 1 天就累積 1000 stars（1000/天），forks 226（22.6%），這顯示出極高的關注度。作者 Nightmare-Eclipse 似乎在安全研究領域有一定的背景，這次發現的漏洞是針對 Bitlocker 的一個重大安全隱患，之前並沒有類似的有效解決方案。這個工具的出現引起了社群的廣泛討論，特別是在安全研究者和滲透測試者之間。技術上，這個漏洞的發現與 Windows 系統的安全設計有關，這使得它在當前的安全生態中顯得尤為重要。高達 22.6% 的 forks/stars 比率表明，許多人正在實際修改和使用這個工具，而不是僅僅觀望。

## 適合誰使用

**目標受眾**：需要評估 Windows 11 環境中 Bitlocker 安全性的安全研究人員和滲透測試者。

> [!example] 使用場景
> - 安全研究人員用它來測試企業環境中的 Bitlocker 安全性，因為這能幫助他們識別潛在的安全漏洞。
> - 滲透測試者用它來模擬攻擊，評估客戶系統的防禦能力，因為這個工具能夠快速繞過 Bitlocker 加密。
> - 系統管理員用它來檢查 Bitlocker 配置的安全性，因為這能幫助他們確保系統不會受到未經授權的存取。

## 架構分析

這個專案的架構相對簡單，主要依賴於 Windows Recovery Environment (WinRE) 的特定組件來實現 Bitlocker 繞過。資料流方面，使用者將特定的文件複製到 USB 隨身碟，然後透過重啟進入 WinRE 環境，最終獲取未授權的存取權。這種設計的代價在於僅限於特定的 Windows 版本，且可能存在穩定性問題。由於這個工具的運作依賴於 WinRE 的特定組件，若未來 Windows 更新改變了這些組件的行為，可能會導致工具失效。擴展性方面，這個工具的使用場景相對狹窄，主要針對特定的安全測試需求。

## 技術深入分析

YellowKey 的核心技術機制是利用 Windows Recovery Environment (WinRE) 中的特定組件，這些組件在正常的 Windows 環境中並不存在，這使得該漏洞的存在顯得可疑。透過將 FsTx 文件夾複製到 USB 隨身碟的特定位置，使用者可以在重啟後進入一個具有未經授權存取權的 shell 環境。這個工具的效能在於其簡單的操作流程，但其實際使用效果取決於能否成功進入 WinRE 環境。設計上，這個工具的選擇是基於 Windows 系統的安全設計缺陷，這使得它在某些情況下比其他 Bitlocker 繞過工具更具優勢。由於這個工具的運作依賴於特定的組件，未來的 Windows 更新可能會影響其功能，這是一個潛在的技術風險。整合方面，這個工具的使用場景相對狹窄，主要針對特定的安全測試需求，與主流的安全測試工具鏈的整合難度較高。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了具體的操作步驟和預期結果。安裝過程相對順暢，但需要注意特定環境的要求。文件目前僅提供英文版本，缺乏多語言支持。

## 優缺點分析

> [!success] 優點
> - 操作簡單，無需複雜的設置。
> - 能夠快速獲取 Bitlocker 加密磁碟的存取權。
> - 針對特定 Windows 版本的有效性高。

> [!danger] 缺點
> - 僅限於 Windows 11 和 Server 2022/2025，使用範圍有限。
> - 尚未經過廣泛的實際測試，穩定性存在疑慮。
> - 可能會受到未來 Windows 更新的影響，導致失效。

> [!warning] 注意事項
> - 僅支援 Windows 11 和 Server 2022/2025，Windows 10 不受影響。
> - 需要在特定的環境下操作，否則可能無法成功進入 WinRE。
> - 尚未經過廣泛的實際測試，存在潛在的穩定性問題。
...
- **[HIGH]** 在某些環境下可能無法成功進入 WinRE，導致無法使用工具。
  - 解法：確保正確的重啟流程，並檢查系統設置。
- [MEDIUM] 未來 Windows 更新可能會影響工具的有效性。
  - 解法：定期檢查工具更新和社群討論。
- [MEDIUM] 在特定硬體上可能無法正常運作，導致無法獲取存取權。
  - 解法：測試不同的硬體配置以確保兼容性。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 大型企業的安全測試團隊 | 非常適合 | 能夠有效評估 Bitlocker 的安全性，發現潛在的漏洞。 |
| 個人使用者想要測試自家電腦的安全性 | 適合 | 操作簡單，能快速獲得結果。 |
| 小型企業的 IT 支援團隊 | 普通 | 雖然能用，但可能需要更多的測試和驗證。 |
| 使用 Windows 10 的環境 | 不適合 | 僅支援 Windows 11 和 Server 2022/2025。 |
...
> [!tip] 投入 vs 回報
> 花 2 小時學習，1 小時整合，得到快速的 Bitlocker 繞過效果，值得安全研究者試用。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：此工具本身不需要高權限，但使用時需注意系統的安全性和穩定性。
...
相關專案：[[Nightmare-Eclipse--RedSun|Nightmare-Eclipse/RedSun]] · [[V4bel--dirtyfrag|V4bel/dirtyfrag]] · [[hicode002--qualcomm_gbl_exploit_poc|hicode002/qualcomm_gbl_exploit_poc]] · [[dazzyddos--PrivHound|dazzyddos/PrivHound]]
