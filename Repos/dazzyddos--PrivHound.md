...
**464** stars · **15** stars/天 · 建立 32 天前 · PowerShell · 未標註授權
...
> **授權** 未標註授權 (風險較高) · **維護** Moderate (最後推送 21 天前) · **貢獻者** 2 人 · **參與度** Low
> **適合** 需要深入分析 Windows 環境中權限提升風險的安全專業人士。
> **一句話重點** PrivHound 讓權限提升的分析變得更直觀，突破了傳統工具的限制。
...
> **結論** 花 2 小時學習，1 小時整合，得到清晰的權限提升分析，值得採用。

> [!abstract] 核心創新
> PrivHound 將本地權限提升問題模型化為圖形結構，提供互聯的攻擊路徑分析。這在傳統工具中是前所未有的。

## 專案簡介

PrivHound 將 Windows 本地權限提升問題視為一個圖形結構，透過 BloodHound 的 OpenGraph 框架，將 29 種權限提升向量（如弱服務權限、DLL 劫持等）轉換為可查詢的互聯節點和邊。這樣的設計使得多步驟的權限提升鏈路變得可視化，使用者可以透過 Cypher 查詢這些鏈路，從而更清晰地了解如何從一個發現點到達另一個。與 WinPEAS、PowerUp 等工具不同，這些工具僅報告孤立的發現，而 PrivHound 則將這些發現串聯起來，形成可利用的攻擊路徑。這樣的設計不僅提高了分析效率，也讓使用者能夠更好地理解攻擊的潛在路徑。PrivHound 的核心賣點是它能夠自動連接不同的發現點，並提供一個完整的攻擊鏈視圖，這在傳統工具中是無法實現的。使用者只需運行簡單的 PowerShell 指令，即可收集數據並生成圖形結構，這樣的流程大大簡化了權限提升的分析過程。PrivHound 也支持多端點收集，允許在多台主機上運行，並將數據整合到同一圖形中，這對於大型環境的安全評估尤為重要。

**技術棧**：`PowerShell` · `Cypher`

## 重點功能

- 多步驟攻擊路徑建模 — 將 29 種權限提升向量轉換為可查詢的圖形結構。
- Cypher 查詢支持 — 使用 Cypher 語言查詢攻擊路徑，靈活性高。
- 跨用戶權限提升分析 — 自動分析發現的用戶憑證可達的資源。
- 多端點收集 — 支持在多台主機上運行，並將數據整合到同一圖形中。
- 自定義節點圖示 — 允許用戶為節點設置獨特的視覺效果，增強可讀性。
...
```powershell
{
  "前置條件": "在目標主機上運行 PrivHound",
  "指令": ".\\PrivHound.ps1",
  "預期輸出": "生成一個 JSON 格式的權限提升數據集"
}
```
...
> [!tip] 爆紅原因
> 建立 32 天內累積 464 stars（15/天），forks 46（9.9%），顯示出相對活躍的社群參與。作者 Signum21 和 dazzyddos 在安全領域有豐富經驗，這個專案解決了傳統工具無法有效連接權限提升發現的痛點，讓使用者能夠更直觀地理解攻擊路徑。近期的推廣和社群討論也可能促進了這個專案的曝光度，吸引了更多安全研究者的注意。工具的設計理念與當前安全研究的趨勢相符，特別是在對抗複雜攻擊時的需求。

## 適合誰使用

**目標受眾**：需要深入分析 Windows 環境中權限提升風險的安全專業人士。

> [!example] 使用場景
> - 安全研究員用它來分析 Windows 環境中的權限提升路徑，因為它能將多個發現串聯成完整的攻擊鏈，讓分析更具效率。
> - 滲透測試工程師用它來快速識別和評估潛在的權限提升漏洞，因為 PrivHound 提供了清晰的可視化圖形，便於理解和報告。
> - 系統管理員用它來檢查和加固系統配置，因為它能幫助發現隱藏的安全風險，並提供具體的修復建議。

## 架構分析

PrivHound 採用單一 PowerShell 腳本來進行數據收集，這樣的設計使得部署和使用變得簡單。數據收集過程中，腳本會檢查多種權限提升向量，並將結果以 JSON 格式輸出。這種結構使得使用者可以輕鬆整合到 BloodHound 中，並進行進一步的分析。選擇 PowerShell 作為主要語言的好處在於其與 Windows 環境的高度相容性，然而這也意味著工具的使用範圍僅限於 Windows 系統。整體架構的擴展性良好，能夠支持多端點的數據收集，但在處理大型環境時，可能會面臨性能瓶頸。

## 技術深入分析

PrivHound 的核心技術機制是將 Windows 本地權限提升的多種向量建模為圖形結構，這樣的設計使得使用者能夠透過 Cypher 語言進行查詢。這種圖形化的表示方式讓多步驟的權限提升鏈路變得清晰可見，使用者可以更容易地理解各個發現之間的關聯。效能上，PrivHound 能夠在多台主機上同時運行，並將結果整合到同一圖形中，這對於大型環境的安全評估尤為重要。選擇 PowerShell 作為主要語言的好處在於其與 Windows 環境的高度相容性，但這也限制了其在其他操作系統上的使用。整體架構的設計使得使用者能夠快速上手，但在處理大量數據時可能會面臨性能瓶頸。技術風險方面，PrivHound 依賴於 Windows 的安全模型，若未來 Windows 系統的安全機制發生變化，可能會影響其效能和準確性。整合方面，PrivHound 能夠與 BloodHound 無縫對接，使用者只需簡單的上傳步驟即可完成數據的整合，這降低了使用的摩擦點。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的指導和範例，安裝過程順暢且無明顯坑。雖然沒有專門的快速入門指南，但使用者可以透過範例指令快速上手。文件目前僅提供英文版本，可能對非英語使用者造成一定困難。

## 優缺點分析

> [!success] 優點
> - 能夠自動連接不同的權限提升發現，形成完整的攻擊鏈。
> - 提供清晰的圖形化視圖，便於分析和報告。
...
| 工具 | 差異 |
| --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard 專注於發現和利用 Active Directory 的弱點，而 PrivHound 專注於 Windows 本地權限提升的圖形化分析。 |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | Parsync 主要用於文件同步，而 PrivHound 專注於安全分析，特別是權限提升的可視化。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | Shadowbroker 主要針對漏洞利用，而 PrivHound 提供了一個更全面的權限提升路徑分析工具。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard 專注於發現 Active Directory 的弱點，而 PrivHound 專注於 Windows 本地權限提升的圖形化分析。 | 如果你的工作重點在於 Active Directory 的安全評估，Boneyard 會更適合。 | medium，因為兩者的數據結構和分析方法存在差異。 |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | Parsync 主要用於文件同步，而 PrivHound 專注於安全分析，特別是權限提升的可視化。 | 如果你的需求是文件同步而非安全分析，Parsync 會是更好的選擇。 | low，因為兩者的功能和目的完全不同。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | Shadowbroker 主要針對漏洞利用，而 PrivHound 提供了一個更全面的權限提升路徑分析工具。 | 如果你需要專注於漏洞利用的分析，Shadowbroker 會更合適。 | medium，因為兩者在數據處理上有不同的側重點。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PrivHound** | **boneyard** | **parsync** | **Shadowbroker** |
> | --- | --- | --- | --- | --- |
> | 技術路線 | 本專案 | Boneyard 專注於發現 Active Directory 的弱點，而 PrivHound 專注於 Windows 本地權限提升的圖形化分析。 | Parsync 主要用於文件同步，而 PrivHound 專注於安全分析，特別是權限提升的可視化。 | Shadowbroker 主要針對漏洞利用，而 PrivHound 提供了一個更全面的權限提升路徑分析工具。 |
> | 遷移成本 | - | medium，因為兩者的數據結構和分析方法存在差異。 | low，因為兩者的功能和目的完全不同。 | medium，因為兩者在數據處理上有不同的側重點。 |
> | 適用場景 | 主要場景 | 如果你的工作重點在於 Active Directory 的安 | 如果你的需求是文件同步而非安全分析，Parsync 會是更好 | 如果你需要專注於漏洞利用的分析，Shadowbroker 會 |
...
> [!tip] 採用建議
> 適合個人或小型團隊進行安全評估，不建議用在生產環境的核心路徑上。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在某些 Windows 環境中，PowerShell 可能需要額外的執行權限才能運行。
  - 解法：確保以管理員身份運行 PowerShell。
- [MEDIUM] 如果未正確配置 BloodHound，數據可能無法正確上傳。
  - 解法：檢查 BloodHound 的 API 設定和權限。
- [MEDIUM] 在多端點收集時，可能會出現數據重複的情況。
  - 解法：確保每台主機的輸出文件名唯一。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的小型安全團隊 | 非常適合 | PrivHound 的簡單部署和易用性使其成為小型團隊的理想選擇。 |
| 大型企業的安全評估 | 適合 | 雖然 PrivHound 支持多端點收集，但在處理大量數據時可能會面臨性能瓶頸。 |
| 需要跨平台使用的安全工具 | 不適合 | PrivHound 僅限於 Windows 環境，無法在其他操作系統上使用。  |
...
> [!tip] 投入 vs 回報
> 花 2 小時學習，1 小時整合，得到清晰的權限提升分析，值得採用。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：PrivHound 本身不需要高權限運行，且不存取敏感資料，但在使用過程中可能需要訪問系統的安全設置。依賴於 Windows 的安全模型，未來的安全更新可能影響其功能。
...
相關專案：[[0xGF--boneyard|0xGF/boneyard]] · [[AlpinDale--parsync|AlpinDale/parsync]] · [[BigBodyCobain--Shadowbroker|BigBodyCobain/Shadowbroker]] · [[Flowseal--tg-ws-proxy|Flowseal/tg-ws-proxy]] · [[PowerUp--PowerUp|PowerUp/PowerUp]] · [[WinPEAS--winPEAS|WinPEAS/winPEAS]] · [[imbue-bit--OpenClaw-PwnKit|imbue-bit/OpenClaw-PwnKit]] · [[vulhunt-re--vulhunt|vulhunt-re/vulhunt]]
