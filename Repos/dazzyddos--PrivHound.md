...
use_case: "將 Windows 本地特權提升的攻擊路徑以圖形方式建模，幫助安全研究人員識別潛在的漏洞。"
priority: medium
ring: assess
...
star_history: "2026-03-10:301,2026-03-11:302,2026-03-13:347,2026-03-14:363,2026-03-15:367,2026-03-16:370,2026-03-17:373,2026-03-18:378,2026-03-19:392,2026-03-20:408,2026-03-21:417,2026-03-22:418,2026-03-23:422,2026-03-24:428,2026-03-25:429,2026-03-26:433,2026-03-27:434,2026-03-29:435,2026-03-30:438,2026-03-31:440,2026-04-01:445"
tags:
  - github
  - "category/安全"
  - "lang/powershell"
aliases:
  - "PrivHound"
  - "dazzyddos/PrivHound"
  - "將 Windows 本地特權提升的攻擊路徑以圖形方式建模，幫助安全研究人員識別潛在的漏洞。"
---

# PrivHound

**445** stars · **18** stars/天 · 建立 25 天前 · PowerShell · 未標註授權
...
> **授權** 未標註授權 (風險較高) · **維護** Moderate (最後推送 14 天前) · **貢獻者** 2 人 · **參與度** Medium
> **適合** 需要進行 Windows 環境安全評估的安全專業人員。
> **一句話重點** PrivHound 以圖形化的方式呈現 Windows 本地特權提升的攻擊路徑，讓安全評估變得更直觀。
...
>     .where(p => p.subcategory === "安全評估工具" && p.file.name !== "dazzyddos--PrivHound" && p.status !== "archived")
...
>       p.ring || "assess"
>     ]));
>   } else { dv.paragraph("_目前 vault 中沒有其他 安全評估工具 類工具_"); }
> }
> ```

> [!question] TL;DR — 值得投入嗎？
> **成熟度** Beta (可試用) · **安裝** Medium (需設定) · **學習** ~5h · **綁定風險** medium
> **結論** 花 5 小時學習、3 小時整合，得到清晰的安全評估報告，值得投入。

> [!abstract] 核心創新
> 將 Windows 本地特權提升的攻擊路徑以圖形方式建模，提供更直觀的風險評估。

## 專案簡介

PrivHound 是一個專為 Windows 環境設計的 BloodHound OpenGraph 收集器，旨在將本地特權提升的攻擊路徑以圖形化方式呈現。使用者可以透過這個工具檢查多種潛在的安全漏洞，例如可修改的服務權限、可寫的服務二進位檔和 DLL 劫持等。這些檢查項目都與 MITRE ATT&CK 框架中的技術相對應，讓使用者能夠快速識別和理解風險。核心賣點是將複雜的攻擊路徑以圖形化的方式呈現，避免了傳統的文字描述造成的理解困難。此工具使用 PowerShell 和 Cypher 進行資料收集和查詢，並且能夠與 BloodHound 的資料結構無縫整合，提供更直觀的視覺化效果。

相較於傳統的安全檢查工具，PrivHound 將重點放在攻擊路徑的可視化，讓安全專家能夠更有效率地進行風險評估。與其他工具如 AlpinDale/parsync 和 BigBodyCobain/Shadowbroker 相比，PrivHound 更專注於 Windows 環境的特權提升攻擊，並提供更具體的檢查項目和視覺化結果。使用者在進行安全評估時，能夠快速識別出哪些服務或設定存在風險，並針對性地進行修正。這個工具的設計考量了安全專家的需求，提供了清晰的檢查報告和建議，幫助他們在實際操作中做出更明智的決策。整體而言，PrivHound 是一個針對 Windows 環境的強大安全工具，適合需要進行深入安全評估的專業團隊。
...
- 多種檢查項目 — 包含 30 種檢查項目，如可修改的服務權限、DLL 劫持等，涵蓋廣泛的安全風險。
- MITRE ATT&CK 兼容 — 每個檢查項目都對應 MITRE ATT&CK 框架，便於與其他安全工具整合。
- PowerShell 和 Cypher 支援 — 使用 PowerShell 收集資料，並用 Cypher 查詢圖形資料，提供靈活的使用方式。
- 即時報告生成 — 能夠快速生成檢查報告，幫助使用者了解當前的安全狀態。
...
> [!tip] 爆紅原因
> 建立 25 天內累積 445 stars（18 stars/天），forks 45（10.1%），顯示出相對活躍的使用者基礎。這個專案的主要貢獻者 Signum21 和 dazzyddos 在安全領域有著豐富的經驗，之前也參與過類似的開源專案。PrivHound 解決了傳統安全檢查工具在可視化和易用性上的不足，特別是在 Windows 環境中，這類工具的需求一直存在。最近的推廣活動和社群討論也促進了它的曝光率，讓更多安全專家開始關注這個工具。這個工具的成功也反映了當前對於可視化安全分析工具的需求增長，尤其是在複雜的企業環境中。

## 適合誰使用

**目標受眾**：需要進行 Windows 環境安全評估的安全專業人員。

> [!example] 使用場景
> - 安全研究員用它來分析 Windows 環境中的特權提升路徑，因為它能提供清晰的攻擊路徑圖，讓風險評估更直觀。
> - 系統管理員用它來檢查服務的安全性，因為它能快速識別可修改的服務和潛在的安全漏洞，節省了手動檢查的時間。
> - 滲透測試人員用它來制定攻擊計畫，因為它能幫助他們理解攻擊者可能利用的路徑，從而設計更有效的測試策略。

## 架構分析

PrivHound 的架構基於 PowerShell 腳本和 Cypher 查詢語言，主要用於收集和分析 Windows 環境中的安全漏洞。資料流從 PowerShell 腳本開始，收集系統資訊並生成圖形資料，然後使用 Cypher 查詢這些資料以生成可視化的攻擊路徑。這種設計使得使用者能夠快速獲取關鍵的安全資訊，但也需要使用者具備一定的 PowerShell 知識。選擇 PowerShell 作為主要語言是因為它在 Windows 環境中的廣泛使用和強大的系統訪問能力，然而這也意味著在某些受限環境中可能無法執行。整體架構的擴展性取決於資料收集的範圍和深度，對於大型企業環境，可能需要進一步的調整和優化。

## 技術深入分析

PrivHound 的核心技術機制是透過 PowerShell 腳本收集 Windows 系統的安全資訊，並使用 Cypher 查詢語言來分析這些資料，生成攻擊路徑的圖形化表示。這種設計使得使用者能夠快速識別潛在的安全漏洞，並理解它們之間的關聯。效能方面，PrivHound 能夠處理中小型企業的安全評估，但在大型企業環境中可能會遇到性能瓶頸，特別是在資料量龐大的情況下。選擇 PowerShell 作為主要語言的好處在於其強大的系統訪問能力，但這也意味著在某些受限環境中可能無法正常運行。技術風險方面，PrivHound 依賴於 Windows 環境的特性，若未來 Windows 系統的安全模型發生變化，可能會影響工具的有效性。整合方面，PrivHound 可以與其他安全工具如 BloodHound 無縫整合，提供更全面的安全分析，但這也要求使用者具備一定的技術背景。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的介紹和使用步驟，但缺乏詳細的範例。安裝過程相對順暢，但需要注意 PowerShell 的執行權限。文件目前僅提供英文版本，對於非英語使用者可能會造成一定困難。

## 優缺點分析

> [!success] 優點
> - 提供清晰的圖形化攻擊路徑，便於理解和分析。
> - 涵蓋多種安全檢查項目，適合全面的安全評估。
> - 與 MITRE ATT&CK 框架兼容，便於整合其他安全工具。

> [!danger] 缺點
> - 僅限於 Windows 環境，無法應用於其他作業系統。
> - 需要一定的 PowerShell 知識，對新手不太友好。
> - 在大型企業環境中可能需要額外的配置和調整。

> [!warning] 注意事項
> - 僅支援 Windows 環境的特權提升分析
> - 需要 PowerShell 的執行權限，可能在某些環境中受限
> - 對於大型企業環境，可能需要進一步的配置和調整

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 專注於資料同步和備份，而 PrivHound 專注於安全漏洞的識別和分析。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | 提供對於 Windows 環境的攻擊面分析，但不具備 PrivHound 的圖形化視覺化功能。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 專注於資料同步和備份，而 PrivHound 專注於安全漏洞的識別和分析。 | 如果你的需求是資料同步而非安全評估，則應選擇 AlpinDale/parsync。 | low，因為兩者的功能範圍不同，無需直接遷移。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | 提供對於 Windows 環境的攻擊面分析，但不具備 PrivHound 的圖形化視覺化功能。 | 如果你需要進行攻擊面分析而不需要圖形化展示，則可以考慮 BigBodyCobain/Shadowbroker。 | medium，因為需要重新適應不同的分析方法和報告格式。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PrivHound** | **parsync** | **Shadowbroker** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於資料同步和備份，而 PrivHound 專注於安全漏洞的識別和分析。 | 提供對於 Windows 環境的攻擊面分析，但不具備 PrivHound 的圖形化視覺化功能。 |
> | 遷移成本 | - | low，因為兩者的功能範圍不同，無需直接遷移。 | medium，因為需要重新適應不同的分析方法和報告格式。 |
> | 適用場景 | 主要場景 | 如果你的需求是資料同步而非安全評估，則應選擇 AlpinDa | 如果你需要進行攻擊面分析而不需要圖形化展示，則可以考慮 Bi |
...
- **[HIGH]** 在某些 Windows 環境中，PowerShell 的執行權限可能會受到限制。
  - 解法：確保使用者擁有足夠的權限，或在管理員模式下執行。
- [MEDIUM] 對於大型企業環境，可能需要額外的配置和調整以獲得最佳效能。
  - 解法：在使用前進行小範圍測試，並根據結果調整配置。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型企業的安全評估 | 非常適合 | 提供清晰的攻擊路徑視覺化，便於快速識別風險。 |
| 大型企業的安全評估 | 普通 | 可能需要額外的配置和調整以獲得最佳效能。 |
| 個人使用者的安全檢查 | 適合 | 能夠快速檢查個人系統中的潛在漏洞。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習、3 小時整合，得到清晰的安全評估報告，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：工具本身不需要高權限，但需要在安全的環境中執行以避免潛在的安全風險。
...
> [!note]- 直接競品（同子分類：安全評估工具）
> ```dataview
> TABLE stars, stars_per_day AS "Stars/天", install_complexity AS "難度", use_case AS "用途"
> FROM "Repos"
> WHERE subcategory = "安全評估工具" AND file.name != "dazzyddos--PrivHound"
> SORT stars DESC
> ```
