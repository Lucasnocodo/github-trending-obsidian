...
use_case: "透過圖形化方式展示 Windows 本地權限提升的攻擊路徑，讓安全分析更直觀。"
priority: medium
ring: assess
...
aliases:
  - "PrivHound"
  - "dazzyddos/PrivHound"
  - "透過圖形化方式展示 Windows 本地權限提升的攻擊路徑，讓安全分析更直觀。"
---

# PrivHound

**481** stars · **9** stars/天 · 建立 56 天前 · PowerShell · 未標註授權
...
> **一句話重點** PrivHound 讓 Windows 本地權限提升的分析變得直觀且易於操作，對於安全專家來說是一個強大的工具。
...
> **結論** 花 5 小時學習，3 小時整合，得到直觀的安全分析工具，值得一試。

> [!abstract] 核心創新
> 將 Windows 本地權限提升的攻擊路徑以圖形化方式展示，提供直觀的分析工具。

## 專案簡介

PrivHound 是一個 BloodHound OpenGraph 收集器，專注於 Windows 本地權限提升的攻擊路徑建模。它的核心機制是將各種潛在的權限提升技術以圖形化的方式呈現，讓安全專家能夠快速識別和分析攻擊路徑。使用者可以透過執行特定的 PowerShell 指令來收集系統資訊，並生成相應的圖形化結果，這樣的設計使得複雜的攻擊路徑不再是冗長的文字描述，而是直觀的視覺化圖表。這個工具的賣點在於其能夠將多種攻擊技術（如弱服務權限、DLL 劫持等）整合在一個圖中，便於分析和決策。技術上，PrivHound 使用 PowerShell 進行資料收集，並利用 Cypher 語言進行圖形資料庫查詢，這樣的選擇使得它能夠高效地處理和查詢大量的系統資料。

相比於其他工具如 BloodHound，PrivHound 更加專注於本地權限提升的攻擊路徑，並且提供了更為清晰的視覺化結果。這樣的設計使得安全分析師能夠在短時間內識別潛在的安全風險。使用者在實際運行中可能會遇到的問題包括需要適當的權限來執行 PowerShell 指令，並且要確保系統環境符合工具的需求。這個專案目前處於活躍開發階段，社群參與度良好，對於需要進行 Windows 安全評估的團隊來說，PrivHound 是一個值得考慮的工具。未來可能會增加更多的檢查項目和功能，以增強其分析能力。

**技術棧**：`PowerShell` · `Cypher`

## 重點功能

- 圖形化攻擊路徑 — 將權限提升技術以圖形方式呈現，便於分析。
- 多種檢查項目 — 包括弱服務權限、DLL 劫持、計畫任務等，涵蓋多種攻擊技術。
- PowerShell 整合 — 使用 PowerShell 收集系統資料，便於在 Windows 環境中運行。
- Cypher 查詢 — 利用 Cypher 語言進行高效的圖形資料庫查詢。
- MITRE ATT&CK 對應 — 每個檢查項目都對應 MITRE ATT&CK 框架，便於安全分析。
...
> [!tip] 爆紅原因
> 建立 56 天內累積 481 stars（約 9 stars/天），forks 46（9.6%），顯示出良好的社群參與度。作者 dazzyddos 和 Signum21 在安全領域有一定的經驗，這個工具解決了傳統文本描述的安全分析工具在視覺化和易用性上的不足。PrivHound 的推出正好填補了這一空白，讓安全專家能夠更直觀地理解和分析攻擊路徑。社群對於這個工具的需求也促進了其快速增長。這個工具的設計使得它在現有的安全工具生態中具備了獨特的價值，特別是在 Windows 環境下的安全評估。
...
> [!example] 使用場景
> - 安全分析師用它來快速識別 Windows 系統中的權限提升路徑，因為其圖形化展示能讓複雜的攻擊路徑一目了然。
> - 滲透測試人員用它來模擬攻擊路徑，因為能夠清楚地看到每個攻擊技術的相互關聯，從而制定更有效的測試策略。
> - 系統管理員用它來檢查系統的安全性，因為它提供了詳細的檢查項目，幫助發現潛在的安全風險。

## 架構分析

PrivHound 的架構設計以 PowerShell 為核心，通過執行腳本收集系統資料，然後利用 Cypher 語言將資料存儲在圖形資料庫中。這樣的設計使得資料的查詢和分析變得高效。資料流從 PowerShell 收集系統資訊開始，經過處理後轉換為圖形格式，最終呈現給使用者。選擇 PowerShell 是因為它在 Windows 環境中的普遍性和強大功能，但這也意味著需要適當的權限來執行。未來可能會面臨擴展性瓶頸，尤其是在資料量增長時，如何保持查詢效能將是一個挑戰。

## 技術深入分析

PrivHound 的核心技術機制是利用 PowerShell 收集 Windows 系統的各種資訊，並將這些資訊轉換為圖形資料庫中的節點和邊。這樣的設計使得使用者能夠透過 Cypher 查詢語言高效地分析攻擊路徑。效能方面，PrivHound 能夠處理中等規模的系統資料，但在資料量增長時可能會面臨查詢效能的挑戰。選擇 PowerShell 是因為它在 Windows 環境中的廣泛應用，但這也意味著需要使用者具備一定的 PowerShell 知識。

技術上，PrivHound 的依賴相對輕量，主要依賴於 PowerShell 和圖形資料庫的查詢能力。設計上，PrivHound 將多種攻擊技術整合在一起，這樣的選擇使得使用者能夠在一個界面中獲得全面的安全分析。未來，隨著資料量的增長，如何保持查詢效能將是一個需要關注的問題，特別是在大型企業環境中。整合方面，PrivHound 能夠與現有的安全工具鏈相容，特別是在使用 PowerShell 的環境中，這使得它能夠輕鬆融入現有的工作流程。

整體而言，PrivHound 是一個針對 Windows 系統安全分析的強大工具，適合需要進行深入安全評估的團隊使用。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的介紹和使用步驟，安裝過程相對順暢，但需要注意權限問題。文件中有基本的範例，幫助新手快速上手。整體而言，花 30 分鐘可以順利運行 PrivHound。

## 優缺點分析

> [!success] 優點
> - 圖形化展示使得複雜的攻擊路徑易於理解。
> - 涵蓋多種權限提升技術，提供全面的安全檢查。
> - 使用 PowerShell 使得在 Windows 環境中部署簡單。

> [!danger] 缺點
> - 僅限於 Windows 環境，無法跨平台使用。
> - 需要適當的權限，對於某些使用者可能造成困難。
> - 目前的檢查項目數量有限，未來需要擴展。

> [!warning] 注意事項
> - 僅支援 Windows 環境。
> - 需要適當的權限來執行 PowerShell 指令。
> - 目前檢查項目數量有限，未來可能需要擴展。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於自動化攻擊模擬，提供更全面的攻擊模擬功能，而 PrivHound 更專注於權限提升路徑的視覺化分析。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供更廣泛的攻擊面分析，但缺乏 PrivHound 的圖形化展示功能，適合需要詳細報告的使用者。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| BloodHound | BloodHound 提供更全面的攻擊面分析，涵蓋多種攻擊技術，而 PrivHound 專注於本地權限提升的視覺化分析。 | 如果需要進行全面的攻擊面分析，BloodHound 是更合適的選擇。 | medium，因為需要重新學習 BloodHound 的操作方式。 |
| PowerView | PowerView 提供強大的 PowerShell 模組來收集 Active Directory 資訊，而 PrivHound 更加專注於本地權限提升的攻擊路徑。 | 如果主要關注 Active Directory 環境的安全，PowerView 是更好的選擇。 | low，因為兩者都使用 PowerShell，轉換相對簡單。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PrivHound** | **BloodHound** | **PowerView** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | BloodHound 提供更全面的攻擊面分析，涵蓋多種攻擊技術，而 PrivHound 專注於本地權限提升的視覺化分析。 | PowerView 提供強大的 PowerShell 模組來收集 Active Directory 資訊，而 PrivHound 更加專注於本地權限提升的攻擊路徑。 |
> | 遷移成本 | - | medium，因為需要重新學習 BloodHound 的操作方式。 | low，因為兩者都使用 PowerShell，轉換相對簡單。 |
> | 適用場景 | 主要場景 | 如果需要進行全面的攻擊面分析，BloodHound 是更合適 | 如果主要關注 Active Directory 環境的安全， |
...
- **[HIGH]** 在某些系統上執行時可能會遇到權限不足的問題
  - 解法：確保以管理員身份運行 PowerShell
- [MEDIUM] 圖形化展示可能在資料量大時變得緩慢
  - 解法：考慮分批處理資料以減少一次性加載的量
- [MEDIUM] 某些檢查項目可能需要額外的系統配置才能正常運行
  - 解法：查閱官方文檔以獲取配置指導
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司進行安全評估 | 非常適合 | 工具輕量且易於部署，適合小型團隊使用。 |
| 大型企業的安全團隊進行全面的安全分析 | 適合 | 雖然功能強大，但在資料量大時可能會面臨效能挑戰。 |
| 需要跨平台使用的安全工具 | 不適合 | 目前僅支援 Windows 環境，無法滿足跨平台需求。  |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，3 小時整合，得到直觀的安全分析工具，值得一試。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：PrivHound 本身不需要高權限運行，但需要適當的權限來收集系統資料，確保安全性。
