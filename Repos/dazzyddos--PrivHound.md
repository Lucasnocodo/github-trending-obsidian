...
use_case: "將 Windows 本地權限提升的攻擊路徑以圖形方式建模，讓攻擊路徑一目了然。"
priority: medium
ring: assess
...
star_history: "2026-03-10:301,2026-03-11:302,2026-03-13:347,2026-03-14:363,2026-03-15:367,2026-03-16:370,2026-03-17:373,2026-03-18:378,2026-03-19:392,2026-03-20:408,2026-03-21:417,2026-03-22:418,2026-03-23:422,2026-03-24:428,2026-03-25:429,2026-03-26:433,2026-03-27:434,2026-03-29:435,2026-03-30:438,2026-03-31:440,2026-04-01:445,2026-04-02:449,2026-04-03:453"
tags:
  - github
  - "category/安全"
  - "lang/powershell"
aliases:
  - "PrivHound"
  - "dazzyddos/PrivHound"
  - "將 Windows 本地權限提升的攻擊路徑以圖形方式建模，讓攻擊路徑一目了然。"
---

# PrivHound

**453** stars · **17** stars/天 · 建立 27 天前 · PowerShell · 未標註授權
...
> **授權** 未標註授權 (風險較高) · **維護** Moderate (最後推送 16 天前) · **貢獻者** 2 人 · **參與度** Medium
> **適合** 需要進行 Windows 系統安全評估的安全專業人士。
> **一句話重點** PrivHound 透過圖形化的方式將 Windows 本地權限提升的攻擊路徑清晰呈現，讓安全分析變得更高效。
...
> **結論** 花 5 小時學習，2 小時整合，得到快速識別 Windows 系統漏洞的效果，值得考慮。

> [!abstract] 核心創新
> 將 Windows 本地權限提升的攻擊路徑以圖形方式建模，讓安全分析更直觀。

## 專案簡介

PrivHound 是一個 BloodHound OpenGraph 收集器，專注於 Windows 本地權限提升的攻擊路徑。它的核心機制是透過檢查多種潛在的安全漏洞，並將這些漏洞以圖形方式展示，讓使用者能夠直觀地理解攻擊路徑。使用者可以透過執行特定的 PowerShell 指令來收集與分析系統資訊，並生成可視化的攻擊路徑圖。這種設計使得安全分析師能夠快速識別系統中的弱點，並針對性地進行修補。PrivHound 檢查的項目包括弱服務權限、可寫服務二進位檔、DLL 劫持等，這些都是常見的攻擊手法，並且每個檢查項目都對應到 MITRE ATT&CK 框架中的技術編號，方便使用者進行對照和深入分析。

技術上，PrivHound 使用 PowerShell 進行系統檢查，並利用 Cypher 語言來處理和查詢圖形數據，這樣的選擇使得它能夠在 Windows 環境中高效運行。與其他工具相比，如 AlpinDale/parsync 和 BigBodyCobain/Shadowbroker，PrivHound 提供了更為直觀的視覺化效果，讓使用者能夠更清楚地看到攻擊路徑的相互關聯。實際使用中，PrivHound 能夠處理多種檢查，並且以圖形化的方式呈現，這對於需要快速評估系統安全狀態的安全專業人員來說非常有價值。這個專案目前處於活躍開發中，社群回應速度良好，適合中小型企業的安全團隊使用。未來可能會增加更多的檢查項目和功能，進一步提升其實用性。

**技術棧**：`PowerShell` · `Cypher`

## 重點功能

- 圖形化攻擊路徑 — 將 Windows 本地權限提升的攻擊路徑以圖形方式展示，便於理解和分析。
- 多種檢查項目 — 檢查弱服務權限、可寫服務二進位檔、DLL 劫持等 30 種潛在漏洞。
- MITRE 技術對應 — 每個檢查項目都對應到 MITRE ATT&CK 框架，方便進行安全分析。
- PowerShell 整合 — 使用 PowerShell 指令進行系統檢查，適合 Windows 環境。
- Cypher 查詢 — 利用 Cypher 語言處理和查詢圖形數據，提升數據處理效率。
...
> [!tip] 爆紅原因
> 建立 27 天內累積 453 stars（約 17 stars/天），forks 為 46（10.2%），顯示出社群對於這個工具的興趣。開發者 Signum21 和 dazzyddos 在安全領域有過往的貢獻，這使得這個專案在技術社群中獲得了一定的信任。PrivHound 解決了傳統安全檢查工具常常以文本形式呈現的問題，透過圖形化的方式讓使用者能夠更容易理解和分析攻擊路徑。這樣的設計在安全分析中是相對少見的，能夠吸引那些需要進行快速評估的安全專業人士。社群的活躍度和開發者的背景也為這個專案的成長提供了良好的基礎。
...
> [!example] 使用場景
> - 安全分析師用它來快速識別 Windows 系統中的權限提升漏洞，因為它能以圖形化方式展示攻擊路徑，讓分析過程更直觀。
> - IT 管理員用它來檢查系統服務的安全配置，因為它提供了清晰的檢查項目和對應的 MITRE 技術編號，方便進行對照。
> - 滲透測試人員用它來制定攻擊路徑，因為它能有效地展示不同漏洞之間的關聯，幫助制定更有效的測試策略。

## 架構分析

PrivHound 的架構設計以 PowerShell 為基礎，專注於 Windows 環境的安全檢查。其主要流程是通過執行 PowerShell 腳本來收集系統資訊，並利用 Cypher 語言將這些資訊轉換為圖形數據。這種設計使得使用者能夠快速獲取系統的安全狀態，並且以圖形化的方式展示出來，便於進行後續的分析和修補。

選擇 PowerShell 是因為其在 Windows 環境中的廣泛應用，而 Cypher 則提供了靈活的數據查詢能力。這樣的架構雖然在功能上相對單一，但能夠在特定的安全檢查場景中提供高效的解決方案。未來的擴展可能會集中在增加更多的檢查項目和提升圖形化展示的效果。

## 技術深入分析

PrivHound 的核心技術機制是利用 PowerShell 進行系統檢查，並將收集到的數據轉換為圖形格式。這樣的設計使得使用者能夠快速識別系統中的安全漏洞，並且以圖形化的方式展示出來，便於進行後續的分析和修補。效能方面，PrivHound 能夠快速處理多種檢查項目，並且在 Windows 環境中運行流暢。設計上選擇 PowerShell 是因為其在 Windows 環境中的廣泛應用，而 Cypher 則提供了靈活的數據查詢能力。這樣的選擇雖然在功能上相對單一，但能夠在特定的安全檢查場景中提供高效的解決方案。技術風險方面，PrivHound 可能在擴展性上面臨挑戰，尤其是在增加更多檢查項目時，可能需要重新考慮架構設計。整合方面，PrivHound 可以與現有的安全工具鏈進行搭配，但在使用上可能需要一定的適應期，特別是對於不熟悉 PowerShell 的使用者。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了基本的使用說明和指令範例。安裝過程相對順暢，但需要注意 PowerShell 的執行權限設定。沒有提供詳細的入門指南，但基本的使用流程已經涵蓋在 README 中。文件目前僅提供英文版本。
...
> - 圖形化展示攻擊路徑，讓安全分析更直觀。
> - 支持多種安全檢查項目，幫助識別潛在漏洞。
> - 與 MITRE ATT&CK 框架對接，方便進行安全分析。

> [!danger] 缺點
> - 僅限於 Windows 環境，無法跨平台使用。
> - 需要 PowerShell 的執行權限，可能受到限制。
> - 檢查項目數量相對有限，未來需擴展。

> [!warning] 注意事項
> - 僅支援 Windows 環境，無法在其他操作系統上運行。
> - 需要 PowerShell 的執行權限，可能受到安全策略限制。
...
| 工具 | 差異 |
| --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 專注於檔案同步和備份，與 PrivHound 的安全檢查功能不同，適合需要檔案管理的場景。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | 主要用於滲透測試和漏洞利用，與 PrivHound 的圖形化攻擊路徑展示有明顯差異，適合更深入的安全測試。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 專注於檔案同步和備份，與 PrivHound 的安全檢查功能不同。 | 如果你的需求主要是檔案管理和備份，而不是安全檢查。 | low，因為兩者的功能範疇不同，轉換不需要太多調整。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | 主要用於滲透測試和漏洞利用，與 PrivHound 的圖形化攻擊路徑展示有明顯差異。 | 如果你的重點是進行深入的滲透測試，而不僅僅是識別漏洞。 | medium，因為需要重新調整測試策略和工具使用。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PrivHound** | **parsync** | **Shadowbroker** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於檔案同步和備份，與 PrivHound 的安全檢查功能不同。 | 主要用於滲透測試和漏洞利用，與 PrivHound 的圖形化攻擊路徑展示有明顯差異。 |
> | 遷移成本 | - | low，因為兩者的功能範疇不同，轉換不需要太多調整。 | medium，因為需要重新調整測試策略和工具使用。 |
> | 適用場景 | 主要場景 | 如果你的需求主要是檔案管理和備份，而不是安全檢查。 | 如果你的重點是進行深入的滲透測試，而不僅僅是識別漏洞。 |
...
> [!tip] 採用建議
> 適合在測試環境中使用，尚不建議用於生產環境的核心路徑。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** PowerShell 執行政策可能限制腳本運行
  - 解法：需要在執行前設定 ExecutionPolicy 為 Bypass。
- [MEDIUM] 某些檢查項目可能需要管理員權限
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型企業的 IT 安全團隊 | 非常適合 | 能夠快速識別系統中的權限提升漏洞，提升安全防護能力。 |
| 大型企業的安全審計 | 適合 | 提供的圖形化展示能夠幫助分析複雜的攻擊路徑。 |
| 個人開發者進行安全測試 | 普通 | 雖然功能強大，但可能需要較高的技術門檻。 |
| 需要跨平台的安全檢查工具 | 不適合 | PrivHound 僅支援 Windows 環境，無法在其他操作系統上運行。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，2 小時整合，得到快速識別 Windows 系統漏洞的效果，值得考慮。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：PrivHound 本身不需要高權限運行，但需注意 PowerShell 的執行權限設定，並確保不會存取敏感資料。
