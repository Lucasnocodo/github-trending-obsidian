...
star_history: "2026-03-10:301,2026-03-11:302,2026-03-13:347,2026-03-14:363,2026-03-15:367,2026-03-16:370,2026-03-17:373,2026-03-18:378,2026-03-19:392,2026-03-20:408,2026-03-21:417,2026-03-22:418,2026-03-23:422,2026-03-24:428"
tags:
  - github
  - "category/安全"
  - "lang/powershell"
...
**428** stars · **25** stars/天 · 建立 17 天前 · PowerShell · 未標註授權
...
> **適合** 需要評估和修復 Windows 環境中安全漏洞的安全專家和系統管理員。
> **一句話重點** PrivHound 將 Windows 本地權限提升的複雜性以圖形化方式呈現，讓安全分析變得更加高效。
...
> **結論** 花 4 小時學習，2 小時整合，得到快速的安全評估效果，值得考慮。

> [!abstract] 核心創新
> 將 Windows 本地權限提升建模為互聯攻擊路徑，提供可視化的分析工具。

## 專案簡介

PrivHound 是一個專注於 Windows 本地權限提升的工具，通過 BloodHound 的 OpenGraph 收集器來建模攻擊路徑。用戶可以透過這個工具檢查多種潛在的權限提升漏洞，例如可修改的服務、可寫的服務二進位檔和 DLL 劫持等。這些檢查項目不僅列出問題，還以圖形化的方式展示它們之間的關聯，讓使用者能夠直觀理解攻擊面。使用者只需執行一個簡單的命令來啟動檢查，並獲得一個全面的報告，這樣的設計使得安全分析變得更加高效。PrivHound 使用 PowerShell 和 Cypher 進行實作，這使得它能夠與現有的 Windows 環境無縫整合，並利用 BloodHound 的強大功能來進行數據分析。

相較於其他工具如 AlpinDale/parsync 和 BigBodyCobain/Shadowbroker，PrivHound 提供了更為直觀的圖形化結果，並且專注於本地權限提升的具體場景，這在進行安全測試時非常重要。使用者在進行權限提升測試時，能夠快速識別和修復潛在的安全漏洞，這對於維護系統安全至關重要。該工具的社群活躍度良好，且有持續的更新，這意味著使用者可以期待未來的功能增強和漏洞修復。對於小型團隊或安全專家來說，PrivHound 是一個值得考慮的工具，特別是在需要快速評估和修復 Windows 環境中的安全漏洞時。

**技術棧**：`PowerShell` · `Cypher`

## 重點功能

- 圖形化攻擊路徑 — 以可視化方式展示 Windows 本地權限提升的攻擊路徑。
- 多種檢查項目 — 包括弱服務權限、可寫服務二進位檔、DLL 劫持等，涵蓋多種潛在漏洞。
- MITRE ATT&CK 對應 — 每個檢查項目都有相應的 MITRE 技術編號，便於安全專家進行分析。
- PowerShell 和 Cypher 支援 — 利用這兩種語言進行數據收集和分析，與 Windows 環境無縫整合。
- 社群活躍 — 定期更新和修復，保持工具的有效性和安全性。
...
> [!tip] 爆紅原因
> 建立 17 天就累積 428 stars（25/天），forks 45（10.5%），顯示出穩定的增長趨勢。開發者 Signum21 和 dazzyddos 具備相關的安全背景，這使得他們能夠針對 Windows 環境的特定需求進行開發。PrivHound 解決了傳統權限提升工具的可視化不足問題，讓安全專家能夠更直觀地理解攻擊路徑。最近的推廣活動和社群討論也促進了該工具的曝光率，增加了使用者的興趣。這個工具的出現正好填補了市場上對於 Windows 本地權限提升分析工具的需求，特別是在企業安全防護中。

## 適合誰使用

**目標受眾**：需要評估和修復 Windows 環境中安全漏洞的安全專家和系統管理員。

> [!example] 使用場景
> - 安全分析師用它來檢查 Windows 環境中的權限提升漏洞，因為它能快速提供可視化的攻擊路徑，節省了手動分析的時間。
> - 系統管理員用它來評估服務的安全性，因為它能夠識別可修改的服務和不安全的路徑，幫助他們加強系統防護。
> - 滲透測試人員用它來模擬攻擊路徑，因為它提供了詳細的檢查項目，讓他們能夠針對性地進行測試和修復。

## 架構分析

PrivHound 採用 PowerShell 作為主要執行環境，並利用 Cypher 進行數據查詢和分析。這樣的設計使得工具能夠直接在 Windows 環境中運行，並且能夠快速收集和分析系統數據。資料流方面，工具首先收集系統的服務和權限信息，然後使用 Cypher 查詢來分析這些數據，最終生成可視化的攻擊路徑圖。

這種設計的優勢在於能夠快速響應和提供即時的安全分析，但代價是對於非 Windows 環境的支持有限。擴展性方面，隨著功能的增加，可能需要進一步優化查詢性能，特別是在大型企業環境中。整體來看，PrivHound 的架構設計使其在 Windows 環境中表現出色，但在其他平台的適用性則受到限制。

## 技術深入分析

PrivHound 的核心技術機制是利用 PowerShell 進行系統數據的收集，並使用 Cypher 查詢語言進行數據分析。這樣的設計使得它能夠快速獲取系統服務、權限和可寫目錄等信息，並將這些信息轉換為可視化的攻擊路徑。效能方面，這個工具可以在短時間內處理大量的系統數據，但在大型企業環境中，查詢性能可能會受到影響。選擇 PowerShell 作為執行環境的好處在於其與 Windows 系統的深度整合，但這也限制了工具的跨平台能力。技術風險方面，PrivHound 的設計決策可能在面對大型系統時出現性能瓶頸，特別是在數據量極大的情況下。整合方面，PrivHound 可以與現有的安全工具鏈進行配合，但對於非 Windows 環境的支持則需要額外的適配工作。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的功能介紹和快速開始步驟，但缺乏詳細的範例。安裝過程相對順暢，但需要注意 PowerShell 的執行權限設置。整體來說，對於新手來說，花 30 分鐘內能夠順利運行起來。
...
> - 提供可視化的攻擊路徑，便於理解和分析。
> - 涵蓋多種權限提升檢查項目，全面性強。
> - 社群活躍，持續更新和修復，保持工具的有效性。

> [!danger] 缺點
> - 僅支援 Windows 環境，無法跨平台使用。
> - 需要較高的權限來執行，可能會受到安全策略限制。
> - 對於網絡攻擊的分析能力不足，專注於本地權限提升。

> [!warning] 注意事項
> - 僅支援 Windows 環境，無法在其他操作系統上運行。
> - 需要 PowerShell 的執行權限，可能需要調整系統安全設置。
> - 僅提供本地權限提升的檢查，對於網絡攻擊的分析不適用。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 專注於資料同步和備份，而非權限提升分析，適合不同的使用場景。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | 提供更廣泛的滲透測試工具，但缺乏 PrivHound 的可視化攻擊路徑功能。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 專注於資料同步和備份，而非權限提升分析，適合不同的使用場景。 | 如果你的需求是資料同步而非安全分析，則可以選擇此工具。 | low，因為兩者功能差異明顯，無需轉換。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | 提供更廣泛的滲透測試工具，但缺乏 PrivHound 的可視化攻擊路徑功能。 | 如果需要一個多功能的滲透測試工具，且不僅限於權限提升，則可考慮此工具。 | medium，因為需要重新學習工具的使用方式。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PrivHound** | **parsync** | **Shadowbroker** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於資料同步和備份，而非權限提升分析，適合不同的使用場景。 | 提供更廣泛的滲透測試工具，但缺乏 PrivHound 的可視化攻擊路徑功能。 |
> | 遷移成本 | - | low，因為兩者功能差異明顯，無需轉換。 | medium，因為需要重新學習工具的使用方式。 |
> | 適用場景 | 主要場景 | 如果你的需求是資料同步而非安全分析，則可以選擇此工具。 | 如果需要一個多功能的滲透測試工具，且不僅限於權限提升，則可考 |
...
> [!tip] 採用建議
> 適合在小型安全測試中使用，但不建議用於生產環境的核心系統。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在某些 Windows 系統上，PowerShell 執行策略可能會阻止腳本運行。
  - 解法：需要手動調整執行策略，或使用 -ExecutionPolicy Bypass 參數。
- [MEDIUM] 可視化結果在大型環境中可能會導致性能下降。
  - 解法：建議在小型環境中測試，或分批執行檢查。
- [MEDIUM] 某些檢查項目可能會因為系統設置不同而無法正確執行。
  - 解法：需根據具體系統環境調整檢查項目。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型企業的 Windows 系統安全評估 | 非常適合 | 專注於本地權限提升的檢查，能快速識別漏洞。 |
| 大型企業的多用戶環境 | 普通 | 在大型環境中性能可能受到影響，需要進行調整。 |
| 滲透測試團隊的工具鏈 | 適合 | 提供可視化的攻擊路徑，便於分析和報告。 |
| 非 Windows 環境的安全評估 | 不適合 | 僅支援 Windows 環境，無法使用。 |
...
> [!tip] 投入 vs 回報
> 花 4 小時學習，2 小時整合，得到快速的安全評估效果，值得考慮。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：工具本身不需要高權限運行，但需注意 PowerShell 的執行權限設置。對於敏感資料的存取需謹慎，並確保在安全的環境中運行。
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["特權提升","滲透測試","MITRE ATT&CK"];
