...
use_case: "將 Windows 本地權限升級模型化為互聯攻擊路徑的 BloodHound OpenGraph 收集器。"
priority: medium
ring: assess
...
aliases:
  - "PrivHound"
  - "dazzyddos/PrivHound"
  - "將 Windows 本地權限升級模型化為互聯攻擊路徑的 BloodHound OpenGraph 收集器。"
---

# PrivHound

**475** stars · **9** stars/天 · 建立 52 天前 · PowerShell · 未標註授權
...
> **適合** 專注於 Windows 環境安全的滲透測試人員和安全研究員。
> **一句話重點** PrivHound 透過圖形化的方式，讓 Windows 本地權限升級的檢查變得更直觀和易於理解。
...
> **結論** 花 4 小時學習，2 小時整合，得到清晰的權限升級視覺化，值得採用。

> [!abstract] 核心創新
> 提供圖形化的攻擊路徑視覺化，讓使用者能夠更直觀地理解權限升級的可能性。

## 專案簡介

PrivHound 是一個專為 Windows 本地權限升級設計的工具，透過 BloodHound 的 OpenGraph 來建模攻擊路徑。使用者可以透過執行 PowerShell 指令來收集系統資訊，並生成一個圖形化的攻擊路徑，這樣的設計使得用戶不再需要面對繁瑣的文字描述。它檢查的項目包括弱服務權限、可寫的服務二進位檔、DLL 劫持等，這些檢查項目對應到 MITRE ATT&CK 框架中的技術編號，幫助安全專業人員識別潛在的攻擊面。PrivHound 的核心賣點在於其圖形化的攻擊路徑視覺化，讓使用者能夠更直觀地理解權限升級的可能性。技術上，它使用 PowerShell 進行系統檢查，並利用 Cypher 語言來查詢和處理數據，這樣的選擇使得它能夠高效地與 Windows 系統進行互動。與其他工具相比，如 BloodHound，PrivHound 提供了一個更專注於本地權限升級的視角，而不是僅僅呈現整體的網絡結構。

這使得它在針對特定攻擊場景時更具針對性和效率。使用者在實際操作中可能會遇到需要特定權限來執行某些檢查的限制，這要求使用者在運行工具前必須具備一定的系統管理權限。PrivHound 的社群活躍度良好，最近的提交顯示出持續的維護和更新，這對於使用者來說是一個正面的信號。對於小型團隊或個人安全研究者來說，這個工具的成熟度足以支持日常使用，並且在未來幾個月內可能會持續增強功能和穩定性。總體來說，PrivHound 是一個值得考慮的工具，特別是對於那些專注於 Windows 環境安全的專業人士。

**技術棧**：`PowerShell` · `Cypher`

## 重點功能

- 圖形化攻擊路徑 — 將權限升級的攻擊路徑以圖形方式呈現，便於理解和分析。
- 多項檢查項目 — 包含 30 種檢查項目，如弱服務權限、可寫的服務二進位檔等，對應 MITRE 技術編號。
- PowerShell 和 Cypher 支援 — 使用 PowerShell 進行檢查，並利用 Cypher 查詢數據，實現高效數據處理。
- 即時更新 — 定期更新和維護，保持工具的有效性和安全性。
- 社群支持 — 活躍的社群和貢獻者，提供持續的支持和改進。
...
> [!tip] 爆紅原因
> 建立 52 天就累積 475 stars（約9.1 stars/天），forks 46（9.7%），顯示出穩定的增長潛力。這個專案由 dazzyddos 和 Signum21 共同維護，兩位貢獻者在安全領域有一定的經驗。PrivHound 解決了以往工具在本地權限升級檢查上的不足，提供了一個更直觀的圖形化界面，讓安全專業人員能夠快速識別攻擊路徑。近期的社群討論和分享也促進了其曝光率，尤其是在安全研究者之間。這個工具的出現正好填補了市場上對於 Windows 本地權限升級檢查的需求，特別是在企業環境中。forks/stars 比率接近 10% 表示有相當一部分使用者在積極修改和使用這個工具，顯示出其實用性和需求性。

## 適合誰使用

**目標受眾**：專注於 Windows 環境安全的滲透測試人員和安全研究員。

> [!example] 使用場景
> - 安全研究員用它來分析 Windows 環境中的權限升級路徑，因為它能夠清晰地視覺化攻擊路徑，幫助識別潛在的安全風險。
> - 系統管理員用它來檢查服務的安全性，因為它提供了詳細的檢查項目，能夠快速定位系統中的弱點。
> - 滲透測試人員用它來模擬攻擊路徑，因為它能夠生成圖形化的攻擊路徑，幫助制定更有效的測試策略。

## 架構分析

PrivHound 採用 PowerShell 作為主要執行環境，並利用 Cypher 進行數據查詢和處理。這樣的設計使得工具能夠高效地與 Windows 系統互動，並生成圖形化的攻擊路徑。資料流中，PowerShell 負責收集系統資訊，然後將數據傳遞給 Cypher 進行分析和視覺化。

選擇 PowerShell 是因為其在 Windows 環境中的原生支援，而 Cypher 則提供了強大的查詢能力。這樣的架構使得 PrivHound 能夠快速響應並生成結果，但也意味著需要使用者具備一定的 PowerShell 知識。擴展性方面，若未來需要增加更多檢查項目，可能需要進一步調整 Cypher 查詢的結構。

## 技術深入分析

PrivHound 的核心技術機制在於使用 PowerShell 進行系統檢查，並利用 Cypher 語言來查詢和處理數據。這樣的設計使得它能夠高效地與 Windows 系統進行互動，並生成圖形化的攻擊路徑。效能方面，PrivHound 能夠快速檢查多達 30 種潛在的安全風險，並在幾分鐘內生成結果。由於使用 PowerShell，對於 Windows 環境的整合度非常高，但這也意味著使用者需要具備一定的 PowerShell 知識。設計取捨上，選擇 PowerShell 而非其他語言（如 Python）是因為其在 Windows 環境中的原生支援，這樣的選擇使得工具能夠更好地與系統互動，但也限制了跨平台的使用。技術風險方面，若未來需要擴展功能，可能需要調整 Cypher 查詢的結構，這可能會帶來一定的維護成本。整合方面，PrivHound 能夠與現有的 Windows 安全工具鏈搭配使用，但在與其他平台的整合上可能會遇到挑戰。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且包含必要的安裝指引，但缺乏詳細的範例。安裝過程相對順暢，但需要注意權限問題。沒有提供多語言支持，主要以英文為主。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於自動化攻擊模擬，與 PrivHound 的手動檢查方式不同。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供更廣泛的攻擊面分析，PrivHound 專注於本地權限升級。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| BloodHound | BloodHound 提供全面的網絡結構分析，而 PrivHound 專注於本地權限升級的檢查。 | 如果需要全面的網絡安全分析，BloodHound 是更好的選擇。 | medium，因為需要重新學習 BloodHound 的使用方式。 |
| Cobalt Strike | Cobalt Strike 提供更全面的滲透測試工具，而 PrivHound 專注於特定的權限升級檢查。 | 如果需要一個全面的滲透測試解決方案，Cobalt Strike 是更合適的選擇。 | high，因為 Cobalt Strike 的功能和操作界面與 PrivHound 有很大不同。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PrivHound** | **BloodHound** | **Cobalt Strike** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | BloodHound 提供全面的網絡結構分析，而 PrivHound 專注於本地權限升級的檢查。 | Cobalt Strike 提供更全面的滲透測試工具，而 PrivHound 專注於特定的權限升級檢查。 |
> | 遷移成本 | - | medium，因為需要重新學習 BloodHound 的使用方式。 | high，因為 Cobalt Strike 的功能和操作界面與 PrivHound 有很大不同。 |
> | 適用場景 | 主要場景 | 如果需要全面的網絡安全分析，BloodHound 是更好的選 | 如果需要一個全面的滲透測試解決方案，Cobalt Strik |
...
- **[HIGH]** 在某些 Windows 環境中，PowerShell 的執行策略可能會阻止腳本運行。
  - 解法：需要以管理員身份運行 PowerShell 並設置執行政策。
- [MEDIUM] 某些檢查項目需要特定的系統權限，否則無法獲得正確結果。
  - 解法：確保以系統管理員身份運行工具。
- [MEDIUM] 在大型企業環境中，可能需要額外的配置來適應特定的安全政策。
  - 解法：提前與 IT 部門協調，了解安全政策。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型企業的 Windows 環境安全檢查 | 非常適合 | PrivHound 專注於本地權限升級，能有效識別安全風險。 |
| 大型企業的全面安全評估 | 不適合 | 功能較為專一，無法滿足全面的安全需求。 |
| 滲透測試專案 | 適合 | 提供具體的權限升級檢查，能幫助制定測試策略。 |
| 個人安全研究 | 非常適合 | 工具簡單易用，適合個人研究和學習。 |
...
> [!tip] 投入 vs 回報
> 花 4 小時學習，2 小時整合，得到清晰的權限升級視覺化，值得採用。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：PrivHound 本身不需要高權限運行，但某些檢查項目可能需要系統管理權限。它不會存取敏感資料，但使用者需確保在安全的環境中運行。
...
相關概念：[[滲透測試]] · [[MITRE ATT&CK]] · [[Windows 安全性]]
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["滲透測試","MITRE ATT&CK","Windows 安全性"];
