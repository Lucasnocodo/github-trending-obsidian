...
use_case: "將 Windows 本地特權提升的攻擊路徑以圖形化方式呈現，讓攻擊者思考更具體的攻擊鏈。"
priority: medium
ring: assess
...
aliases:
  - "PrivHound"
  - "dazzyddos/PrivHound"
  - "將 Windows 本地特權提升的攻擊路徑以圖形化方式呈現，讓攻擊者思考更具體的攻擊鏈。"
---

# PrivHound

**487** stars · **7** stars/天 · 建立 70 天前 · PowerShell · 未標註授權
...
> **授權** 未標註授權 (風險較高) · **維護** Moderate (最後推送 19 天前) · **貢獻者** 2 人 · **參與度** Low
> **適合** 需要進行 Windows 系統安全評估的安全專業人士和滲透測試人員。
> **一句話重點** PrivHound 讓 Windows 本地特權提升的攻擊路徑變得可視化，從而提高了安全分析的效率。
...
> **結論** 花 5 小時學習，3 小時整合，得到強大的特權提升分析能力，值得投入。

> [!abstract] 核心創新
> PrivHound 將本地特權提升視為一個圖形化的攻擊路徑，讓使用者能夠直觀地理解多步驟的攻擊鏈。

## 專案簡介

PrivHound 是一個基於 BloodHound 的 OpenGraph 收集器，專注於 Windows 本地特權提升的攻擊路徑。它通過建模 29 種特權提升向量，將這些向量以互聯的節點和邊的形式呈現，讓使用者能夠查詢和分析多步驟的攻擊鏈。這種設計的核心在於，它不僅僅列出單一的配置錯誤，而是將這些錯誤串聯起來，形成可利用的攻擊路徑。例如，當 PrivHound 發現某個用戶可以讀取另一用戶的配置文件，並且該文件中包含有效的憑證時，它會自動建立從一個用戶到另一個用戶的攻擊鏈。這樣的設計使得攻擊者能夠更清晰地理解從一個配置錯誤到最終獲得 SYSTEM 權限的整個過程。

PrivHound 使用 Cypher 查詢語言來查詢這些攻擊路徑，並且可以與現有的 Active Directory 攻擊路徑重疊，提供更全面的安全分析。相較於傳統的工具如 WinPEAS 和 PowerUp，PrivHound 提供了一種更直觀的方式來理解和利用這些特權提升的機會，讓使用者能夠快速識別和利用潛在的安全漏洞。它的多端點收集功能也讓在大型環境中進行安全評估變得更加高效，支持將多個主機的數據整合到同一圖中進行分析。整體而言，PrivHound 的設計理念是將複雜的安全問題視覺化，讓使用者能夠更有效地理解和應對本地特權提升的威脅。

**技術棧**：`PowerShell` · `Cypher`

## 重點功能

- 多步驟攻擊鏈 — 將 29 種特權提升向量以圖形化方式呈現，讓使用者能夠清晰理解攻擊路徑。
- Cypher 查詢支持 — 使用 Cypher 語言查詢攻擊路徑，提供靈活的數據分析能力。
- 自定義節點圖示 — 允許使用者為節點設定自定義圖示，提升可視化效果。
- 多端點收集 — 支持在多個主機上收集數據，並將結果整合到同一圖中進行分析。
- 自動化邊緣生成 — 根據發現的憑證和配置自動生成攻擊邊，無需手動連接。
...
> [!tip] 爆紅原因
> 建立 70 天內累積 487 stars（約 7 stars/天），forks 45（9.2%），顯示出良好的社群參與度。這個專案由 dazzyddos 和 Signum21 共同開發，解決了傳統工具無法有效連接多個特權提升向量的問題。之前的工具如 WinPEAS 和 PowerUp 雖然能找到個別的配置錯誤，但無法提供整體的攻擊鏈視圖。PrivHound 的出現讓安全分析師能夠更直觀地理解攻擊路徑，這在當前對於安全性要求日益提高的環境中顯得尤為重要。社群的活躍度和高效的問題解決率（100%）也顯示出這個專案的成熟度和實用性。

## 適合誰使用

**目標受眾**：需要進行 Windows 系統安全評估的安全專業人士和滲透測試人員。

> [!example] 使用場景
> - 安全分析師用它來評估企業內部的 Windows 系統安全性，因為它能快速識別多個特權提升路徑，從而降低潛在的安全風險。
> - 滲透測試人員用它來模擬攻擊路徑，因為它能將各種配置錯誤串聯起來，讓測試過程更具針對性和效率。
> - 系統管理員用它來檢查系統配置，因為它能幫助他們發現潛在的安全漏洞，從而提前防範可能的攻擊。

## 架構分析

PrivHound 的架構基於 BloodHound 的 OpenGraph 框架，將多種特權提升向量以節點和邊的形式組織。這種設計使得攻擊路徑不僅可視化，還能進行查詢。資料流從每個檢查開始，經過多個步驟，最終生成一個包含所有攻擊路徑的 JSON 文件。

選擇這種圖形化的方式而非傳統的文字報告，能夠讓使用者更快速地理解和識別潛在的安全風險。由於 PrivHound 需要與 BloodHound 整合，這也意味著它依賴於 BloodHound 的資料結構和查詢能力，這在某些情況下可能會造成學習曲線。整體而言，PrivHound 的架構設計旨在提高安全分析的效率和準確性。

## 技術深入分析

PrivHound 的核心技術機制在於將多種特權提升向量建模為圖形結構，這樣的設計使得使用者能夠直觀地理解攻擊路徑。它使用 PowerShell 進行數據收集，並利用 Cypher 查詢語言來分析這些數據。這種方式的效能表現在於能夠快速識別多步驟的攻擊鏈，而不僅僅是單一的配置錯誤。PrivHound 的設計選擇使其能夠與 BloodHound 整合，這樣的選擇雖然增加了學習曲線，但也帶來了更強的功能擴展性。由於 PrivHound 依賴於 Windows 環境的特性，這也意味著在其他操作系統上無法使用，這是一個明顯的限制。在技術風險方面，PrivHound 需要正確的權限來執行檢查，否則可能無法獲取完整的數據。整體而言，PrivHound 在安全分析中提供了一個創新的視角，讓使用者能夠更有效地識別和利用潛在的安全漏洞。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且提供了範例，安裝過程相對順暢，但需要注意權限問題。沒有提供多語言支持，主要以英文為主。

## 優缺點分析

> [!success] 優點
> - 提供清晰的攻擊路徑視覺化，讓使用者能夠快速理解特權提升的可能性。
> - 支持多端點數據收集，適合大型環境的安全評估。
> - 使用 Cypher 查詢語言，提供靈活的數據分析能力。

> [!danger] 缺點
> - 僅限於 Windows 環境，無法跨平台使用。
> - 需要一定的權限來執行某些檢查，可能影響數據完整性。
> - 對於大型環境，數據處理時間可能較長。

> [!warning] 注意事項
> - 僅支援 Windows 環境，無法在其他操作系統上運行。
> - 需要適當的權限來執行某些檢查，否則可能無法獲取完整數據。
> - 對於大型環境，收集和處理數據的時間可能較長。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於自動化攻擊路徑生成，但不具備 PrivHound 的圖形化視覺化功能。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供類似的特權提升檢查，但缺乏 PrivHound 的多步驟攻擊鏈分析能力。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於自動化攻擊路徑生成，但不具備 PrivHound 的圖形化視覺化功能。 | 如果需要自動化生成攻擊路徑而不需要圖形化展示，可以選擇這個工具。 | medium，因為需要重新學習其自動化流程。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供類似的特權提升檢查，但缺乏 PrivHound 的多步驟攻擊鏈分析能力。 | 如果只需要檢查單一的特權提升向量，而不需要整合多個向量，可以考慮這個工具。 | low，因為功能相似，轉換成本不高。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PrivHound** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於自動化攻擊路徑生成，但不具備 PrivHound 的圖形化視覺化功能。 | 提供類似的特權提升檢查，但缺乏 PrivHound 的多步驟攻擊鏈分析能力。 |
> | 遷移成本 | - | medium，因為需要重新學習其自動化流程。 | low，因為功能相似，轉換成本不高。 |
> | 適用場景 | 主要場景 | 如果需要自動化生成攻擊路徑而不需要圖形化展示，可以選擇這個工 | 如果只需要檢查單一的特權提升向量，而不需要整合多個向量，可以 |
...
> [!tip] 採用建議
> 適合進行安全測試和研究，但不建議用在生產環境的核心路徑上。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在某些環境中，權限不足可能導致無法收集完整數據
  - 解法：確保以具有足夠權限的用戶執行工具
- [MEDIUM] 多端點收集時，數據合併可能會導致重複節點
  - 解法：在上傳數據前進行去重處理
- [MEDIUM] 使用自定義圖示時，可能會遇到格式不正確的錯誤
  - 解法：檢查自定義圖示的格式是否符合要求
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 大型企業的 Windows 系統安全評估 | 非常適合 | 能夠有效識別多步驟的特權提升路徑，適合複雜環境。 |
| 中小型企業的安全測試 | 適合 | 提供清晰的攻擊路徑視覺化，能快速識別潛在風險。 |
| 個人開發者的安全研究 | 普通 | 雖然功能強大，但可能對於小型專案來說過於複雜。 |
| 需要跨平台使用的安全工具 | 不適合 | 僅限於 Windows 環境，無法在其他操作系統上運行。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，3 小時整合，得到強大的特權提升分析能力，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：PrivHound 本身不需要高權限運行，但需要適當的權限來執行檢查。它不會存取敏感資料，但依賴於用戶的權限設置。
