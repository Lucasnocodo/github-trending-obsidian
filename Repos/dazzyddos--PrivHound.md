...
use_case: "將 Windows 本地特權提升建模為互聯的攻擊路徑，幫助安全專家識別潛在的漏洞。"
priority: medium
ring: assess
...
aliases:
  - "PrivHound"
  - "dazzyddos/PrivHound"
  - "將 Windows 本地特權提升建模為互聯的攻擊路徑，幫助安全專家識別潛在的漏洞。"
---

# PrivHound

**466** stars · **13** stars/天 · 建立 35 天前 · PowerShell · 未標註授權
...
> **授權** 未標註授權 (風險較高) · **維護** Moderate (最後推送 24 天前) · **貢獻者** 2 人 · **參與度** Low
> **適合** 需要進行 Windows 環境安全評估的資安專業人士。
> **一句話重點** PrivHound 將複雜的 Windows 本地特權提升問題以圖形化方式呈現，讓安全分析變得更直觀。
...
> **結論** 花 5 小時學、3 小時整合，得到快速識別特權提升漏洞的能力，值得考慮。

> [!abstract] 核心創新
> 將 Windows 本地特權提升的檢查以圖形化方式呈現，便於安全專家快速識別攻擊路徑。

## 專案簡介

PrivHound 是一個 BloodHound OpenGraph 收集器，專注於 Windows 本地特權提升的攻擊路徑。它通過檢查系統中的多種潛在漏洞，將這些漏洞以圖形化方式呈現，便於安全專家分析。使用者可以透過執行特定的 PowerShell 指令來收集系統資訊，並生成可視化的攻擊路徑圖，這樣的設計使得複雜的安全問題變得更易於理解。它檢查的項目包括可修改的服務、可寫的服務二進位檔、未加引號的服務路徑等，這些都是常見的攻擊向量。這些檢查對應到 MITRE ATT&CK 框架，讓使用者能夠快速識別和修補安全漏洞。

使用者只需執行 `Invoke-PrivHound` 指令，即可開始收集數據，並生成攻擊路徑圖，這是它的賣點之一。技術上，PrivHound 使用 PowerShell 和 Cypher 語言，依賴於 BloodHound 的圖形數據庫來進行分析，這使得它在處理複雜的權限關係時具備優勢。相較於其他工具如 0xGF/boneyard 和 BigBodyCobain/Shadowbroker，PrivHound 提供了更直觀的視覺化結果，便於快速識別攻擊路徑。使用者在實際操作中可能會遇到 PowerShell 執行政策的限制，這需要額外的配置來解決。整體來看，PrivHound 是一個針對 Windows 環境的強大工具，適合需要進行安全評估的專業團隊使用。

**技術棧**：`PowerShell` · `Cypher`

## 重點功能

- 圖形化攻擊路徑 — 將 Windows 本地特權提升的潛在漏洞以圖形化方式展示，便於分析。
- 多種檢查項目 — 包括弱服務權限、可寫服務二進位檔、未加引號的服務路徑等，涵蓋多種攻擊向量。
- MITRE ATT&CK 對應 — 每個檢查項目都與 MITRE ATT&CK 框架中的技術對應，便於理解和修補。
- PowerShell 支援 — 使用 PowerShell 指令進行數據收集，易於整合到現有的安全工具鏈中。
- 社群支持 — 擁有活躍的開發者和使用者社群，能快速獲得幫助和更新。
...
> [!tip] 爆紅原因
> 建立 35 天內累積 466 stars（每天下降約 13），forks 46（9.9%），顯示出一定的社群關注度。主要貢獻者 Signum21 和 dazzyddos 在安全領域有豐富的經驗，這使得專案具有可信度。PrivHound 解決了傳統文本報告難以理解的問題，將複雜的攻擊路徑以圖形化方式呈現，讓安全專家能快速識別潛在的漏洞。這種視覺化的方式在安全評估中是相當重要的，特別是對於大型系統的分析。社群的反饋也顯示出對於這種工具的需求，尤其是在 Windows 環境下的特權提升問題上。

## 適合誰使用

**目標受眾**：需要進行 Windows 環境安全評估的資安專業人士。

> [!example] 使用場景
> - 安全分析師用它來識別 Windows 系統中的潛在特權提升漏洞，因為它提供了清晰的攻擊路徑圖，讓分析過程更有效率。
> - 滲透測試人員用它來模擬攻擊路徑，因為它能快速找出可利用的漏洞，並提供具體的修補建議。
> - 系統管理員用它來檢查服務配置的安全性，因為它能自動化檢查過程，節省手動檢查的時間。

## 架構分析

PrivHound 的架構基於 BloodHound 的圖形數據庫，使用 PowerShell 收集系統資訊，並利用 Cypher 查詢語言進行數據分析。這種設計使得工具能夠有效地處理複雜的權限關係，並將結果以圖形化方式展示。選擇 PowerShell 是因為它在 Windows 環境中的廣泛應用，而 Cypher 則提供了強大的查詢能力。這樣的架構雖然增加了學習成本，但能夠提供更深入的分析。擴展性方面，未來可以考慮加入更多的檢查項目和功能，以應對不斷變化的安全威脅。

## 技術深入分析

PrivHound 的核心技術機制在於使用 PowerShell 收集 Windows 系統的安全資訊，並利用 Cypher 語言進行圖形數據的查詢和分析。這種設計使得工具能夠有效地處理複雜的權限關係，並將結果以圖形化方式展示。效能方面，PrivHound 能夠快速檢查多達 30 種潛在的特權提升漏洞，具體的處理速度取決於系統的性能和檢查項目的數量。選擇 PowerShell 作為主要語言是因為其在 Windows 環境中的廣泛應用，而 Cypher 提供了強大的查詢能力，能夠快速檢索和分析數據。

這樣的選擇雖然增加了學習成本，但能夠提供更深入的分析。技術風險方面，PrivHound 依賴於 PowerShell 的執行政策，這可能在某些環境中造成限制。整合方面，PrivHound 可以與現有的安全工具鏈進行整合，但需要注意 PowerShell 的執行政策和安全設置。整體來看，PrivHound 是一個針對 Windows 環境的強大工具，適合需要進行安全評估的專業團隊使用。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的介紹和快速入門指引，但缺乏詳細的範例。安裝過程相對順暢，但需要注意 PowerShell 的執行政策。整體來說，對於有一定技術背景的使用者來說，能在 30 分鐘內上手。

## 優缺點分析

> [!success] 優點
> - 提供清晰的圖形化攻擊路徑，便於快速識別漏洞。
> - 涵蓋多種常見的特權提升攻擊向量，檢查全面。
> - 與 MITRE ATT&CK 框架對應，便於理解和修補。

> [!danger] 缺點
> - 僅限於 Windows 環境，無法跨平台使用。
> - 需要額外配置 PowerShell 執行政策，對新手不友好。
> - 社群支持相對較少，可能影響使用者的問題解決速度。

> [!warning] 注意事項
> - 僅支援 Windows 環境，無法在其他作業系統上運行。
> - 需要 PowerShell 執行政策允許，可能需要額外配置。
> - 目前只有一個開放的 Issue，社群支持可能有限。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供類似的權限提升檢查，但缺乏 PrivHound 的圖形化展示功能，使用者需要手動分析結果。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | 專注於 Windows 系統的滲透測試，但不如 PrivHound 直觀，使用者需具備較高的技術背景。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供類似的權限提升檢查，但缺乏 PrivHound 的圖形化展示功能，使用者需要手動分析結果。 | 如果需要更靈活的檢查和自定義功能，且不介意手動分析結果。 | medium，因為需要重新學習工具的使用方式。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | 專注於 Windows 系統的滲透測試，但不如 PrivHound 直觀，使用者需具備較高的技術背景。 | 如果你的團隊已經在使用 Shadowbroker，並希望保持一致性。 | high，因為需要重新適應不同的工具和流程。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PrivHound** | **boneyard** | **Shadowbroker** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 提供類似的權限提升檢查，但缺乏 PrivHound 的圖形化展示功能，使用者需要手動分析結果。 | 專注於 Windows 系統的滲透測試，但不如 PrivHound 直觀，使用者需具備較高的技術背景。 |
> | 遷移成本 | - | medium，因為需要重新學習工具的使用方式。 | high，因為需要重新適應不同的工具和流程。 |
> | 適用場景 | 主要場景 | 如果需要更靈活的檢查和自定義功能，且不介意手動分析結果。 | 如果你的團隊已經在使用 Shadowbroker，並希望保持 |
...
> [!tip] 採用建議
> 適合進行安全評估的專業團隊試用，但不建議用在生產環境的核心路徑上。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** PowerShell 執行政策限制，可能導致無法執行指令
  - 解法：需要手動調整 PowerShell 執行政策
- **[HIGH]** 僅支援 Windows 環境，無法在其他作業系統上運行
  - 解法：無法解決，需使用 Windows 環境

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 大型企業的安全評估團隊 | 非常適合 | 能夠快速識別和修補多種潛在的特權提升漏洞。 |
| 中小型企業的 IT 部門 | 適合 | 提供清晰的攻擊路徑圖，便於進行安全評估。 |
| 個人開發者或小型團隊 | 普通 | 功能強大但學習曲線較陡，可能需要更多時間上手。 |
| 對 Windows 安全不熟悉的使用者 | 不適合 | 需要一定的技術背景才能有效使用。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學、3 小時整合，得到快速識別特權提升漏洞的能力，值得考慮。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：工具本身不需要高權限運行，但需注意 PowerShell 的執行政策設定。對於敏感資料的存取需謹慎，特別是在企業環境中使用時。
...
相關概念：[[滲透測試]] · [[MITRE ATT&CK]] · [[Windows 安全]]
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["滲透測試","MITRE ATT&CK","Windows 安全"];
