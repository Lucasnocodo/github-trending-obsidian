...
star_history: "2026-03-10:301,2026-03-11:302,2026-03-13:347,2026-03-14:363,2026-03-15:367,2026-03-16:370,2026-03-17:373,2026-03-18:378,2026-03-19:392,2026-03-20:408,2026-03-21:417,2026-03-22:418,2026-03-23:422,2026-03-24:428,2026-03-25:429,2026-03-26:433,2026-03-27:434,2026-03-29:435,2026-03-30:438"
tags:
  - github
  - "category/安全"
  - "lang/powershell"
...
**438** stars · **19** stars/天 · 建立 23 天前 · PowerShell · 未標註授權
...
> **授權** 未標註授權 (風險較高) · **維護** Moderate (最後推送 12 天前) · **貢獻者** 2 人 · **參與度** Medium
> **適合** 需要進行 Windows 系統安全評估的安全專業人員。
> **一句話重點** PrivHound 讓 Windows 本地權限提升的安全評估變得更直觀，提升了安全專業人員的工作效率。
...
>     .where(p => p.subcategory === "安全評估" && p.file.name !== "dazzyddos--PrivHound" && p.status !== "archived")
...
>       p.ring || "assess"
>     ]));
>   } else { dv.paragraph("_目前 vault 中沒有其他 安全評估 類工具_"); }
> }
> ```

> [!question] TL;DR — 值得投入嗎？
> **成熟度** Beta (可試用) · **安裝** Medium (需設定) · **學習** ~5h · **綁定風險** medium
> **結論** 花 5 小時學習，3 小時整合，能夠有效提升 Windows 系統的安全評估效率，值得投入。

> [!abstract] 核心創新
> PrivHound 提供了一種全新的方式來視覺化 Windows 本地權限提升的攻擊路徑。

## 專案簡介

PrivHound 是一個專為 Windows 本地權限提升設計的 BloodHound OpenGraph 收集器，能夠將複雜的權限提升路徑視覺化，讓使用者更容易理解潛在的攻擊路徑。它透過檢查多種安全漏洞，如可修改的服務權限、可寫的服務二進位檔，以及 DLL 劫持等，來建立攻擊圖譜。這些檢查基於 MITRE ATT&CK 框架，提供了清晰的技術細節和對應的攻擊技術編號。使用者可以透過簡單的 PowerShell 指令來收集和分析這些數據，從而快速識別系統中的安全風險。這個工具的賣點在於其圖形化的呈現方式，讓複雜的攻擊路徑不再是冗長的文字，而是直觀的圖形化展示。技術上，PrivHound 使用 PowerShell 進行數據收集，並利用 Cypher 查詢語言來處理和展示數據，這樣的設計使得它能夠靈活地與 BloodHound 整合，並且易於擴展。

相較於其他工具，如 BloodHound 本身或是其他權限提升檢測工具，PrivHound 提供了更為直觀的圖形化界面，讓使用者能夠快速識別和理解潛在的安全漏洞。這對於安全專業人員在進行系統安全評估時，能夠節省大量的時間和精力。使用 PrivHound 的過程中，使用者可能會遇到一些特定的環境配置問題，特別是在不同版本的 Windows 系統上，這可能會影響到數據的準確性和完整性。整體來看，PrivHound 是一個值得考慮的工具，尤其適合需要進行深入安全評估的團隊和個人。隨著社群的活躍度提升，未來可能會有更多的功能和檢查項目被加入。

**技術棧**：`PowerShell` · `Cypher`

## 重點功能

- 圖形化攻擊路徑 — 將 Windows 本地權限提升的攻擊路徑以圖形方式呈現，便於理解。
- 多種檢查項目 — 包含 30 種不同的檢查項目，如可寫的服務二進位檔、DLL 劫持等。
- MITRE ATT&CK 整合 — 每個檢查項目都對應 MITRE ATT&CK 框架，提供詳細的攻擊技術編號。
- PowerShell 數據收集 — 使用 PowerShell 進行數據收集，便於在 Windows 環境中運行。
- Cypher 查詢語言 — 利用 Cypher 進行數據處理和查詢，提升數據分析的靈活性。
...
> [!tip] 爆紅原因
> 建立 23 天內累積 438 stars（約19 stars/天），forks 45（10.3%），顯示出相對穩定的增長。這個專案由 Signum21 和 dazzyddos 共同開發，兩位開發者在安全領域有著豐富的經驗。PrivHound 解決了傳統權限提升檢測工具難以視覺化和理解的痛點，讓使用者能夠快速識別攻擊路徑。該工具的推出可能與近期對 Windows 安全性評估需求的增加有關，特別是在企業環境中。forks/stars 比率為 10.3%，顯示出有相當比例的使用者在實際修改和使用這個工具。

## 適合誰使用

**目標受眾**：需要進行 Windows 系統安全評估的安全專業人員。

> [!example] 使用場景
> - 安全分析師用它來視覺化 Windows 系統中的權限提升路徑，因為這樣能更快地識別潛在的安全漏洞。
> - 滲透測試人員用它來生成攻擊路徑圖，因為這樣可以更有效地規劃測試策略，提升測試效率。
> - 系統管理員用它來檢查服務的安全配置，因為這樣能及早發現並修復安全隱患，降低風險。

## 架構分析

PrivHound 採用 PowerShell 作為主要的數據收集工具，這使得它能夠直接在 Windows 環境中運行，並且能夠利用系統的原生功能來獲取必要的數據。數據收集後，使用 Cypher 查詢語言進行處理，這樣的設計使得數據分析過程更加靈活且高效。選擇 PowerShell 的原因在於它的廣泛使用和對 Windows 系統的深度整合，這樣可以減少使用者的學習成本。

相對於其他語言，PowerShell 在 Windows 環境中的性能優勢明顯。整體架構的擴展性良好，未來可以根據需求增加更多的檢查項目和功能。由於依賴於 Windows 的特定功能，這也使得它在跨平台的擴展性上存在一定的限制。

## 技術深入分析

PrivHound 的核心技術機制在於使用 PowerShell 進行數據收集，這使得它能夠直接訪問 Windows 系統的各種安全配置和權限設置。數據收集後，使用 Cypher 查詢語言來處理和展示數據，這樣的設計不僅提升了數據處理的效率，也讓使用者能夠靈活地查詢和分析數據。根據 README，PrivHound 能夠檢查多達 30 種不同的安全漏洞，這使得它在處理複雜的安全環境時具備了良好的效能。選擇 PowerShell 作為主要語言的原因在於其對 Windows 環境的深度整合，這樣的選擇雖然帶來了良好的性能，但也限制了它的跨平台能力。技術上，PrivHound 的依賴關係相對簡單，主要依賴於 PowerShell 和 Cypher，這使得它在安裝和使用上都較為簡單。未來在擴展性方面，PrivHound 可以根據使用者的需求增加更多的檢查項目，但這也可能會增加維護的複雜度。整體來看，PrivHound 在安全評估中提供了一個有效的解決方案，但在使用過程中，使用者需注意其環境配置的兼容性問題。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的介紹和快速入門指南，安裝過程相對順暢。雖然沒有提供詳細的範例，但基本的使用指令已經列出，方便新手上手。整體來說，花 30 分鐘應該能夠順利運行起來。

## 優缺點分析

> [!success] 優點
> - 提供直觀的圖形化界面，便於理解攻擊路徑。
> - 整合 MITRE ATT&CK 框架，提供詳細的技術細節。
> - 支持多種檢查項目，涵蓋廣泛的安全風險。

> [!danger] 缺點
> - 僅限於 Windows 環境，無法跨平台使用。
> - 需要適當的權限來執行，可能會限制使用者。
> - 對於某些 Windows 版本的兼容性問題。

> [!warning] 注意事項
> - 僅支援 Windows 環境，無法在其他操作系統上運行。
> - 需要適當的權限來執行 PowerShell 腳本，否則可能無法收集所有數據。
> - 對於某些特定的 Windows 版本，可能會出現兼容性問題。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [BloodHound/BloodHound](https://github.com/BloodHound/BloodHound) | BloodHound 是一個更全面的工具，專注於 Active Directory 環境的權限提升，而 PrivHound 專注於本地權限提升的視覺化。 |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | parsync 提供了不同的數據同步方式，雖然也涉及安全性，但不專注於權限提升的視覺化。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [BloodHound/BloodHound](https://github.com/BloodHound/BloodHound) | BloodHound 專注於 Active Directory 環境的權限提升，提供更全面的視覺化和分析功能，而 PrivHound 專注於本地權限提升的視覺化。 | 如果你的環境主要是 Active Directory，BloodHound 會是更合適的選擇，因為它提供了更深入的分析。 | medium，因為需要重新學習 BloodHound 的操作方式和界面。 |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | parsync 提供了數據同步的功能，雖然也涉及安全性，但不專注於權限提升的視覺化，適合需要數據同步的場景。 | 如果你的需求是數據同步而非權限提升分析，parsync 會是更好的選擇。 | low，因為使用方式相對簡單，且不需要太多額外學習。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PrivHound** | **BloodHound** | **parsync** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | BloodHound 專注於 Active Directory 環境的權限提升，提供更全面的視覺化和分析功能，而 PrivHound 專注於本地權限提升的視覺化。 | parsync 提供了數據同步的功能，雖然也涉及安全性，但不專注於權限提升的視覺化，適合需要數據同步的場景。 |
> | 遷移成本 | - | medium，因為需要重新學習 BloodHound 的操作方式和界面。 | low，因為使用方式相對簡單，且不需要太多額外學習。 |
> | 適用場景 | 主要場景 | 如果你的環境主要是 Active Directory，Blo | 如果你的需求是數據同步而非權限提升分析，parsync 會是 |
...
> [!tip] 採用建議
> 適合在內部環境中試用，但不建議用於生產環境的核心路徑上。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在某些 Windows 版本上可能會遇到兼容性問題，導致無法正確收集數據。
  - 解法：確保使用最新的 Windows 更新，並在測試環境中進行驗證。
- [MEDIUM] 執行 PowerShell 腳本時可能會遇到權限不足的問題。
  - 解法：確保以管理員身份運行 PowerShell。
- [MEDIUM] 某些檢查項目在特定環境下可能無法正確執行。
  - 解法：檢查相關的系統設置和權限配置。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 大型企業的安全評估團隊 | 非常適合 | 能夠快速識別和視覺化複雜的權限提升路徑，提升評估效率。 |
| 中小型企業的 IT 支援團隊 | 適合 | 能夠幫助識別潛在的安全風險，雖然可能需要額外的配置。 |
| 個人開發者進行安全測試 | 普通 | 雖然功能強大，但可能對於個人使用者來說過於複雜。 |
| 對權限提升不太了解的使用者 | 不適合 | 需要一定的安全知識來理解和使用這個工具。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，3 小時整合，能夠有效提升 Windows 系統的安全評估效率，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：PrivHound 本身不需要高權限運行，但在執行時可能需要管理員權限來收集數據，需注意安全性。
...
相關概念：[[特權提升]] · [[MITRE ATT&CK]] · [[Windows 安全性]]
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["特權提升","MITRE ATT&CK","Windows 安全性"];
