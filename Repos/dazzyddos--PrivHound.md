...
use_case: "將 Windows 本地權限提升建模為互聯的攻擊路徑，幫助安全分析人員識別潛在的安全風險。"
priority: medium
ring: assess
...
star_history: "2026-03-10:301,2026-03-11:302,2026-03-13:347,2026-03-14:363,2026-03-15:367,2026-03-16:370,2026-03-17:373,2026-03-18:378,2026-03-19:392,2026-03-20:408,2026-03-21:417"
tags:
  - github
  - "category/安全"
  - "lang/powershell"
aliases:
  - "PrivHound"
  - "dazzyddos/PrivHound"
  - "將 Windows 本地權限提升建模為互聯的攻擊路徑，幫助安全分析人員識別潛在的安全風險。"
---

# PrivHound

**417** stars · **30** stars/天 · 建立 14 天前 · PowerShell · 未標註授權
...
> **適合** 需要進行 Windows 本地權限提升分析的安全專業人士。
> **一句話重點** PrivHound 透過視覺化的攻擊路徑分析，讓安全專業人士能更有效率地識別 Windows 系統中的安全漏洞。
...
> **結論** 花 5 小時學習，3 小時整合，能夠快速識別 Windows 系統中的安全漏洞，值得考慮。

> [!abstract] 核心創新
> 將本地權限提升建模為互聯的攻擊路徑，提供直觀的視覺化分析。

## 專案簡介

PrivHound 是一個 BloodHound OpenGraph 收集器，專注於 Windows 本地權限提升的攻擊路徑。使用者可以透過執行特定的檢查來識別系統中的潛在漏洞，例如可修改的服務權限和不安全的 DLL 路徑。這些檢查會生成一個圖形化的攻擊路徑，讓安全專家能快速理解各個漏洞之間的關聯。主要的 CLI 指令未在 README 中明確列出，但可以推測使用者會透過 PowerShell 執行檢查並生成圖形化報告。這個工具的賣點在於其視覺化的攻擊路徑，讓使用者不再需要閱讀冗長的文字說明，而是可以直觀地看到漏洞的關聯性。技術上，PrivHound 使用 PowerShell 和 Cypher 語言來收集和查詢數據，這使得它能夠有效地與 BloodHound 整合，並利用其圖形數據庫的強大功能。

與其他安全工具相比，如 AlpinDale/parsync 和 BigBodyCobain/Shadowbroker，PrivHound 提供了一個更為直觀的視覺化方式，幫助使用者快速識別和理解攻擊路徑。這在面對複雜的權限提升攻擊時，能大幅提高效率。使用者可能會面臨的挑戰包括需要對 PowerShell 和 BloodHound 的基本操作有一定了解，否則可能無法充分利用這個工具的潛力。這個專案目前處於活躍開發階段，未來可能會增加更多檢查項目和功能，特別是在面對不斷演變的安全威脅時。對於需要進行安全評估的團隊，PrivHound 是一個值得考慮的工具，尤其是在 Windows 環境中進行本地權限提升分析時。

**技術棧**：`PowerShell` · `Cypher`

## 重點功能

- 圖形化攻擊路徑 — 將本地權限提升的潛在攻擊路徑以圖形方式呈現，便於理解和分析。
- 多種檢查項目 — 包括弱服務權限、可寫服務二進位檔、DLL 劫持等，涵蓋多種潛在風險。
- 與 BloodHound 整合 — 利用 BloodHound 的圖形數據庫功能，提升數據查詢和視覺化能力。
- PowerShell 支援 — 使用 PowerShell 進行數據收集，方便 Windows 環境下的使用。
...
> [!tip] 爆紅原因
> 建立 14 天內累積 417 stars（30/天），forks 41（9.8%），顯示出穩定的增長潛力。這個專案由 Signum21 和 dazzyddos 共同開發，兩位貢獻者在安全領域有一定的背景。PrivHound 解決了傳統安全分析工具在視覺化和易用性上的不足，讓使用者能更快識別攻擊路徑。近期的社群討論和推廣活動也可能助長了其知名度。這個工具的出現正好符合了對於 Windows 環境中本地權限提升分析的需求，尤其是在安全漏洞不斷增加的當下。forks/stars 比率為 9.8%，顯示出使用者對於這個工具的實際修改和使用意願。

## 適合誰使用

**目標受眾**：需要進行 Windows 本地權限提升分析的安全專業人士。

> [!example] 使用場景
> - 安全分析師用它來識別 Windows 系統中的潛在權限提升漏洞，因為它提供了直觀的攻擊路徑視覺化，能快速定位問題。
> - 滲透測試人員用它來模擬攻擊路徑，因為它能幫助他們理解不同漏洞之間的關聯性，從而制定更有效的測試策略。
> - 系統管理員用它來檢查服務配置的安全性，因為它能自動化檢查過程，節省大量手動檢查的時間。

## 架構分析

PrivHound 的架構基於 PowerShell 腳本和 BloodHound 的圖形數據庫。使用 PowerShell 收集系統信息，並將數據轉換為 BloodHound 可識別的格式。這種設計使得使用者能夠快速獲取和分析數據，但也要求使用者對 PowerShell 有一定的理解。

數據流從系統檢查開始，經過 PowerShell 腳本的處理，最終生成圖形報告。選擇 PowerShell 是因為其在 Windows 環境中的普遍應用，但這也限制了其跨平台的能力。未來可能需要考慮擴展到其他平台以增加使用範圍。

## 技術深入分析

PrivHound 的核心技術機制是利用 PowerShell 腳本收集 Windows 系統的安全數據，並將其轉換為 BloodHound 所需的格式。這樣的設計使得使用者能夠快速獲取系統的安全狀態，並以圖形化方式呈現攻擊路徑。效能上，PrivHound 能夠處理中小型企業的 Windows 環境，但在大型企業中，可能會因為數據量過大而影響性能。選擇 PowerShell 作為主要語言是因為其在 Windows 環境中的廣泛應用，但這也帶來了跨平台的限制。技術風險方面，PrivHound 依賴於 BloodHound 的數據庫，如果 BloodHound 更新或改變其數據結構，可能會影響 PrivHound 的功能。整合方面，PrivHound 可以輕鬆與現有的 Windows 安全工具鏈結合，但對於不熟悉 PowerShell 的使用者來說，整合過程可能會有一定的挑戰。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了基本的使用說明，但缺乏詳細的範例和安裝指令。安裝過程相對順暢，但對於不熟悉 PowerShell 的使用者可能會有一定的困難。建議增加一個詳細的入門指南，幫助新手快速上手。

## 優缺點分析

> [!success] 優點
> - 提供直觀的攻擊路徑視覺化，便於快速識別漏洞。
> - 涵蓋多種潛在的安全檢查項目，提升安全分析的全面性。
> - 與 BloodHound 整合，利用其強大的圖形數據庫功能。

> [!danger] 缺點
> - 僅限於 Windows 環境，無法跨平台使用。
> - 需要使用者具備一定的 PowerShell 知識，學習曲線較陡。
> - 目前檢查項目數量有限，可能無法滿足所有需求。

> [!warning] 注意事項
> - 僅支援 Windows 環境，無法在其他操作系統上運行。
> - 需要對 PowerShell 有基本了解，否則難以操作。
> - 目前檢查項目數量有限，未來可能需要增加更多檢查項目。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 專注於文件同步和備份，而 PrivHound 專注於安全漏洞分析，兩者用途截然不同。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | Shadowbroker 主要用於網絡攻擊模擬，而 PrivHound 專注於本地權限提升的視覺化分析，適用場景不同。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 專注於文件同步和備份，而 PrivHound 專注於安全漏洞分析，兩者用途截然不同。 | 如果你的需求是文件同步而非安全分析，則應選擇 parsync。 | low，因為兩者的功能和用途完全不同。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | Shadowbroker 主要用於網絡攻擊模擬，而 PrivHound 專注於本地權限提升的視覺化分析，適用場景不同。 | 如果你的工作重點是網絡安全測試而非本地權限提升，則應選擇 Shadowbroker。 | medium，因為需要重新調整安全測試策略。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PrivHound** | **parsync** | **Shadowbroker** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於文件同步和備份，而 PrivHound 專注於安全漏洞分析，兩者用途截然不同。 | Shadowbroker 主要用於網絡攻擊模擬，而 PrivHound 專注於本地權限提升的視覺化分析，適用場景不同。 |
> | 遷移成本 | - | low，因為兩者的功能和用途完全不同。 | medium，因為需要重新調整安全測試策略。 |
> | 適用場景 | 主要場景 | 如果你的需求是文件同步而非安全分析，則應選擇 parsync | 如果你的工作重點是網絡安全測試而非本地權限提升，則應選擇 S |
...
- **[HIGH]** 在某些 Windows 環境中，PowerShell 執行策略可能會阻止腳本執行。
  - 解法：使用 -ExecutionPolicy Bypass 參數來繞過這個限制。
- [MEDIUM] 某些檢查項目可能因系統配置不同而無法正確執行。
  - 解法：確保系統配置符合 PrivHound 的要求，或手動檢查相關項目。
- [MEDIUM] 需要對 BloodHound 的數據結構有一定了解，否則難以解讀報告。
  - 解法：參考 BloodHound 的文檔以了解數據結構和報告格式。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型企業的安全評估 | 非常適合 | 提供直觀的攻擊路徑視覺化，能快速識別潛在漏洞。 |
| 大型企業的安全分析 | 普通 | 在面對大量數據時，性能可能會成為瓶頸。 |
| 滲透測試團隊的工具鏈 | 適合 | 能夠幫助測試人員快速識別攻擊路徑。 |
| 對 PowerShell 不熟悉的使用者 | 不適合 | 需要一定的 PowerShell 知識，否則難以操作。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，3 小時整合，能夠快速識別 Windows 系統中的安全漏洞，值得考慮。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：PrivHound 本身不需要高權限運行，但在執行檢查時可能會存取系統敏感資料。依賴鏈的信任程度較高，因為主要使用 PowerShell 和 BloodHound。
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["特權提升","MITRE ATT&CK","安全分析"];
