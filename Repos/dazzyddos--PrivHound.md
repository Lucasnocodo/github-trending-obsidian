...
use_case: "以圖形方式建模 Windows 本地特權提升的攻擊路徑，幫助安全專家識別潛在的安全風險。"
priority: medium
ring: assess
...
aliases:
  - "PrivHound"
  - "dazzyddos/PrivHound"
  - "以圖形方式建模 Windows 本地特權提升的攻擊路徑，幫助安全專家識別潛在的安全風險。"
---

# PrivHound

**465** stars · **13** stars/天 · 建立 36 天前 · PowerShell · 未標註授權
...
> **授權** 未標註授權 (風險較高) · **維護** Moderate (最後推送 25 天前) · **貢獻者** 2 人 · **參與度** Low
> **適合** 需要進行 Windows 環境安全評估的安全專家和滲透測試工程師。
> **一句話重點** PrivHound 以圖形化方式展示特權提升攻擊路徑，讓安全分析變得更加直觀和高效。
...
>     .where(p => p.subcategory === "漏洞掃描" && p.file.name !== "dazzyddos--PrivHound" && p.status !== "archived")
...
> **結論** 花 5 小時學習、3 小時整合，得到全面的 Windows 安全評估效果，值得考慮。

> [!abstract] 核心創新
> 將 Windows 本地特權提升的攻擊路徑以圖形化方式展示，提升了安全分析的直觀性。

## 專案簡介

PrivHound 是一個 BloodHound OpenGraph 收集器，專注於 Windows 本地特權提升的攻擊路徑。它通過分析系統中的弱點，將潛在的攻擊路徑以圖形方式呈現，讓使用者能夠快速識別和理解安全風險。使用者可以透過執行特定的檢查來收集數據，例如檢查可寫的服務二進位檔、弱服務權限和 DLL 劫持等。這些檢查對應到 MITRE ATT&CK 框架中的技術，幫助使用者更好地理解攻擊者可能利用的路徑。這個工具的賣點在於它不僅提供文字描述，而是將複雜的攻擊路徑以圖形化的方式展示，便於安全專家進行分析和決策。

在技術實作上，PrivHound 使用 PowerShell 進行數據收集，並利用 Cypher 查詢語言來處理和呈現數據。這樣的設計使得工具能夠高效地從 Windows 環境中提取信息，並以圖形化方式展示，從而提高了分析的直觀性。

相較於傳統的安全掃描工具，PrivHound 更加專注於特權提升的攻擊路徑，並且能夠將多個檢查結果整合在一起，形成一個完整的攻擊圖譜。

與其他工具相比，如 0xGF/boneyard 和 BigBodyCobain/Shadowbroker，PrivHound 提供了更為直觀的視覺化界面，讓使用者能夠更快地識別潛在的攻擊路徑。這些替代品可能提供類似的檢查功能，但缺乏 PrivHound 的圖形化展示能力，這使得它在安全分析過程中更具優勢。

在實際使用中，PrivHound 能夠處理多種檢查，並且在 465 顆 stars 和 46 個 forks 的支持下，顯示出社群對這個工具的認可。雖然目前只有 1 個開放的問題，但這也顯示出社群的活躍度和維護者的回應速度。

這個專案目前處於 beta 階段，適合中小型團隊進行安全評估和漏洞掃描。未來 6 個月內，隨著 Windows 安全性需求的增加，PrivHound 可能會進一步擴展其功能，增強對新型攻擊技術的支持。

**技術棧**：`PowerShell` · `Cypher`

## 重點功能

- 圖形化攻擊路徑 — 將特權提升的攻擊路徑以圖形方式展示，便於分析。
- 多種檢查項目 — 包含 30 種檢查項目，如可寫的服務二進位檔、DLL 劫持等，全面評估安全風險。
- MITRE ATT&CK 對應 — 每個檢查項目都對應到 MITRE ATT&CK 框架，幫助使用者理解攻擊技術。
- PowerShell 數據收集 — 使用 PowerShell 進行數據收集，適合 Windows 環境。
- Cypher 查詢語言 — 利用 Cypher 處理和呈現數據，提升查詢效率。
...
> [!tip] 爆紅原因
> 建立 36 天內累積 465 stars（約 13 stars/天），forks 46（9.9%），顯示出穩定的增長趨勢。這個專案的作者 Signum21 和 dazzyddos 在安全領域有一定的影響力，提供了一個針對 Windows 本地特權提升的具體解決方案，填補了市場上對於圖形化攻擊路徑分析工具的需求。PrivHound 的推出正好符合了安全專家對於可視化工具的需求，讓他們能夠更有效地識別和分析潛在的安全風險。社群的活躍度和對於問題的快速回應也促進了這個專案的成長。

## 適合誰使用

**目標受眾**：需要進行 Windows 環境安全評估的安全專家和滲透測試工程師。

> [!example] 使用場景
> - 安全分析師用它來識別 Windows 環境中的特權提升路徑，因為它能夠以圖形化方式展示複雜的攻擊路徑，讓分析更直觀。
> - 滲透測試工程師用它來評估客戶系統的安全性，因為它提供了針對特權提升的具體檢查，幫助發現潛在漏洞。
> - 系統管理員用它來檢查服務的安全配置，因為它能快速識別可寫的服務二進位檔和弱服務權限，降低風險。

## 架構分析

PrivHound 採用 PowerShell 作為主要的數據收集工具，這使得它能夠直接從 Windows 環境中提取信息，並將這些信息轉換為 Cypher 查詢語言所需的格式。這樣的設計選擇使得工具能夠高效運行，並且能夠快速適應不同的 Windows 環境。資料流方面，工具首先收集系統中的弱點信息，然後通過 Cypher 查詢語言進行處理和分析，最終以圖形化的方式展示結果。

這種設計的代價在於，對於非 Windows 環境的支持有限，並且需要使用者具備一定的 PowerShell 知識。擴展性方面，隨著檢查項目的增加，可能會導致性能瓶頸，特別是在大型環境中進行掃描時。整體來說，PrivHound 的架構設計充分考慮了 Windows 環境的特性，但在跨平台支持上仍有待加強。

## 技術深入分析

PrivHound 的核心技術機制在於使用 PowerShell 進行數據收集，並利用 Cypher 查詢語言來處理和展示數據。這樣的設計使得工具能夠高效地從 Windows 環境中提取信息，並以圖形化方式展示，提升了分析的直觀性。效能方面，PrivHound 能夠處理多種檢查項目，並在大型環境中可能會遇到性能瓶頸，特別是在進行全面掃描時。設計上選擇 PowerShell 而非其他語言，主要是因為其在 Windows 環境中的原生支持，這樣的選擇雖然提高了效率，但也限制了工具的跨平台能力。技術風險方面，隨著檢查項目的增加，可能會導致性能下降，特別是在大型環境中進行掃描時。整合方面，PrivHound 與主流的安全工具鏈相容性良好，但需要使用者具備一定的 PowerShell 知識，這可能會增加學習成本。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的介紹和使用說明，但缺乏詳細的範例。安裝過程相對順暢，克隆和執行指令簡單明瞭。沒有提供多語言支持，對於非英語使用者可能會造成一定困難。
...
> - 提供圖形化的攻擊路徑展示，便於快速分析。
> - 涵蓋多種檢查項目，全面評估安全風險。
> - 與 MITRE ATT&CK 框架對應，幫助使用者理解攻擊技術。

> [!danger] 缺點
> - 僅支援 Windows 環境，無法跨平台使用。
> - 需要一定的 PowerShell 知識，對新手不太友好。
> - 目前社群支持相對較少，可能影響未來的發展。

> [!warning] 注意事項
> - 僅支援 Windows 環境，無法在其他操作系統上運行。
> - 需要 PowerShell 執行權限，可能在某些環境中受到限制。
> - 目前僅有 1 個開放的問題，可能在未來需要更多的社群支持。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供類似的檢查功能，但缺乏 PrivHound 的圖形化展示能力，分析過程不夠直觀。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | 同樣針對特權提升進行分析，但不如 PrivHound 提供的視覺化效果明顯。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 使用 Python 實作，著重於靜態分析，缺乏 PrivHound 的動態圖形展示。 | 如果需要靜態分析和報告生成，且團隊熟悉 Python，則可選擇此工具。 | medium，因為需要重新學習工具的使用方式。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | 基於 C# 開發，專注於特權提升的漏洞掃描，但不提供圖形化界面。 | 若需要一個專注於特權提升的掃描工具，且不需要圖形化展示，可以考慮此工具。 | high，因為需要重新適應 C# 環境和工具的使用。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PrivHound** | **boneyard** | **Shadowbroker** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 使用 Python 實作，著重於靜態分析，缺乏 PrivHound 的動態圖形展示。 | 基於 C# 開發，專注於特權提升的漏洞掃描，但不提供圖形化界面。 |
> | 遷移成本 | - | medium，因為需要重新學習工具的使用方式。 | high，因為需要重新適應 C# 環境和工具的使用。 |
> | 適用場景 | 主要場景 | 如果需要靜態分析和報告生成，且團隊熟悉 Python，則可選 | 若需要一個專注於特權提升的掃描工具，且不需要圖形化展示，可以 |
...
> [!tip] 採用建議
> 適合進行安全評估的實驗性專案，但不建議用在生產環境的核心路徑上。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在某些 Windows 環境中，PowerShell 執行權限可能受到限制，導致無法正常運行。
  - 解法：確保使用者擁有足夠的權限，或使用管理員模式運行 PowerShell。
- [MEDIUM] 某些檢查項目可能因環境配置不同而無法正確執行。
  - 解法：根據具體環境調整檢查項目的參數設置。
- [low] 缺乏詳細的使用範例，可能導致新手使用時困惑。
  - 解法：參考相關文獻或社群討論尋求幫助。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 中小型企業的 Windows 環境安全評估 | 非常適合 | 提供全面的檢查項目，能有效識別潛在風險。 |
| 大型企業的跨平台安全評估 | 不適合 | 僅支援 Windows 環境，無法處理其他操作系統。 |
| 滲透測試團隊的工具鏈整合 | 適合 | 能夠與其他安全工具搭配使用，提升測試效率。 |
| 個人開發者的安全工具探索 | 普通 | 雖然功能強大，但學習曲線較陡峭。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習、3 小時整合，得到全面的 Windows 安全評估效果，值得考慮。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：工具本身不需要高權限運行，但需要訪問 Windows 系統的敏感資料，使用時需謹慎處理。
...
相關概念：[[MITRE ATT&CK]] · [[漏洞掃描]] · [[特權提升]]

相關專案：[[0xGF--boneyard|0xGF/boneyard]] · [[BigBodyCobain--Shadowbroker|BigBodyCobain/Shadowbroker]] · [[AlpinDale--parsync|AlpinDale/parsync]] · [[Flowseal--tg-ws-proxy|Flowseal/tg-ws-proxy]] · [[PowerUp--PowerUp|PowerUp/PowerUp]] · [[WinPEAS--winPEAS|WinPEAS/winPEAS]] · [[imbue-bit--OpenClaw-PwnKit|imbue-bit/OpenClaw-PwnKit]] · [[vulhunt-re--vulhunt|vulhunt-re/vulhunt]]
...
> FROM "Repos"
> WHERE subcategory = "漏洞掃描" AND file.name != "dazzyddos--PrivHound"
> SORT stars DESC
> ```
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["MITRE ATT&CK","漏洞掃描","特權提升"];
