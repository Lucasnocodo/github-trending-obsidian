...
use_case: "將 Windows 本地權限提升的攻擊路徑建模為互聯圖形，幫助安全專家發現潛在的權限提升漏洞。"
priority: medium
ring: assess
...
aliases:
  - "PrivHound"
  - "dazzyddos/PrivHound"
  - "將 Windows 本地權限提升的攻擊路徑建模為互聯圖形，幫助安全專家發現潛在的權限提升漏洞。"
---

# PrivHound

**480** stars · **9** stars/天 · 建立 55 天前 · PowerShell · 未標註授權
...
> **適合** 需要深入分析 Windows 環境安全性的滲透測試工程師和安全專家。
> **一句話重點** PrivHound 將權限提升的分析從靜態檢查轉變為動態的圖形探索，這是滲透測試中的一個重要進步。
...
> **結論** 花 4 小時學習，2 小時整合，得到一個強大的權限提升分析工具，值得投入。

> [!abstract] 核心創新
> PrivHound 將 Windows 本地權限提升建模為圖形，提供了可視化和查詢的能力，這在現有工具中是前所未有的。

## 專案簡介

PrivHound 是一個基於 BloodHound 的 OpenGraph 收集器，專注於 Windows 本地權限提升的攻擊路徑。它的核心機制是將 29 種權限提升向量（如弱服務權限、DLL 劫持等）建模為圖形，並使用 Cypher 查詢語言進行可視化和查詢。這樣的設計使得多步驟的權限提升鏈條變得可見，並且能夠在現有的 Active Directory 攻擊路徑上進行疊加。PrivHound 的賣點在於它不僅僅列出孤立的發現，而是將這些發現連接成可利用的鏈條，讓使用者能夠直觀地理解從某一點出發可以達到的最終目標。技術上，它使用 PowerShell 來收集數據，並且依賴於 BloodHound 的圖形結構來展示攻擊路徑。

與傳統工具如 WinPEAS 和 PowerUp 相比，PrivHound 提供了一個更高層次的視角，能夠回答「這個可寫的 Program Files 目錄是否實際上導致 SYSTEM 權限？」等問題。該工具的使用效果顯著，能夠在多個主機上收集數據並整合成一個圖形，這對於大型環境中的安全評估尤為重要。PrivHound 的社群活躍度良好，且有穩定的更新頻率，適合對 Windows 安全有深入需求的安全專家和滲透測試人員。未來六個月內，預計會有更多的功能和優化，特別是在圖形可視化和查詢效率上。

**技術棧**：`PowerShell` · `Cypher`

## 重點功能

- 多步驟攻擊路徑建模 — 將 29 種權限提升向量連接成可利用的鏈條。
- Cypher 查詢語言 — 允許用戶對攻擊路徑進行靈活的查詢。
- 自動化數據收集 — 使用 PowerShell 自動收集 Windows 環境中的潛在漏洞。
- 可視化圖形輸出 — 將攻擊路徑以圖形方式展示，便於理解和分析。
- 跨用戶權限提升分析 — 自動檢查其他用戶的憑證和權限，發現潛在的攻擊路徑。
...
> [!tip] 爆紅原因
> 建立 55 天內累積 480 stars（約 9 stars/天），forks 數量為 46（9.6%），顯示出相對活躍的社群參與。開發者 dazzyddos 和 Signum21 具備相關背景，這使得該專案能夠針對 Windows 權限提升問題提供創新解決方案。PrivHound 解決了傳統工具無法有效連接多個發現的痛點，這在滲透測試中是個重要需求。社群的活躍度和開發者的背景都為這個專案的未來發展提供了良好的基礎。

## 適合誰使用

**目標受眾**：需要深入分析 Windows 環境安全性的滲透測試工程師和安全專家。

> [!example] 使用場景
> - 滲透測試工程師用它來分析 Windows 環境中的權限提升路徑，因為它能夠將多個發現連接成可利用的攻擊鏈，提升測試效率。
> - 安全分析師用它來視覺化權限提升風險，因為 PrivHound 提供了圖形化的攻擊路徑，讓風險評估變得直觀。
> - 系統管理員用它來檢查服務配置的安全性，因為它能夠快速識別可寫的服務二進制檔案，降低安全風險。

## 架構分析

PrivHound 的架構基於 BloodHound 的 OpenGraph 框架，這使得它能夠將多種權限提升向量以圖形的方式展示。數據流從 PowerShell 腳本開始，收集 Windows 環境中的潛在漏洞，然後將這些數據轉換為圖形結構。這樣的設計使得使用者能夠直觀地理解不同漏洞之間的關聯。選擇使用 PowerShell 是因為它在 Windows 環境中的普遍性和強大功能，但這也限制了它在其他操作系統上的使用。整體架構輕量，便於快速部署和使用，但在處理大量數據時可能會遇到性能瓶頸，特別是在大型企業環境中。

## 技術深入分析

PrivHound 的核心技術機制是基於 BloodHound 的 OpenGraph 框架，利用圖形數據結構來表示 Windows 環境中的權限提升路徑。它使用 PowerShell 腳本來收集系統中的潛在漏洞，並將這些漏洞轉換為可視化的圖形結構。這種設計使得使用者能夠快速識別多步驟的攻擊路徑，並通過 Cypher 查詢語言進行靈活的查詢。效能方面，PrivHound 能夠處理多個主機的數據，並將其整合到同一圖形中，這對於大型環境的安全評估尤為重要。選擇 PowerShell 作為主要語言是因為它在 Windows 環境中的普遍性，但這也限制了它的跨平台能力。技術風險方面，PrivHound 依賴於 BloodHound 的穩定性，若 BloodHound 發生變更，可能會影響 PrivHound 的功能。整合方面，PrivHound 能夠與現有的安全工具鏈無縫結合，特別是在滲透測試和安全評估的工作流中。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了詳細的使用範例和指令。安裝過程相對順暢，但需要確保 PowerShell 環境正常運行。文件目前僅提供英文版本，可能對非英語使用者造成一定困難。

## 優缺點分析

> [!success] 優點
> - 提供多步驟攻擊路徑的可視化，便於理解和分析。
> - 使用 Cypher 查詢語言，靈活性高。
> - 自動化數據收集，減少手動操作的需求。

> [!danger] 缺點
> - 僅限於 Windows 環境，無法跨平台使用。
> - 對於未啟用 PowerShell 的系統無法運行。
> - 需要 BloodHound 環境的支持，增加了部署的複雜性。

> [!warning] 注意事項
> - 僅支援 Windows 環境，無法在其他操作系統上運行。
> - 需要 PowerShell 環境，對於未啟用 PowerShell 的系統無法使用。
> - 依賴於 BloodHound，需確保 BloodHound 環境正常運行。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於自動化攻擊向量的生成，而 PrivHound 更加關注於權限提升的可視化和分析。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 主要用於檢查系統配置的安全性，而 PrivHound 提供了更深入的攻擊路徑分析。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 專注於自動化攻擊向量的生成，而 PrivHound 更加關注於權限提升的可視化和分析。 | 如果你的團隊需要自動化生成攻擊向量，則可以選擇這個工具。 | low，因為兩者的使用場景有一定重疊。 |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 主要用於檢查系統配置的安全性，而 PrivHound 提供了更深入的攻擊路徑分析。 | 如果需要針對系統配置進行詳細檢查，則可以考慮使用這個工具。 | medium，因為需要調整工作流程以適應不同的分析方法。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PrivHound** | **agent-sprite-forge** | **boneyard** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於自動化攻擊向量的生成，而 PrivHound 更加關注於權限提升的可視化和分析。 | 主要用於檢查系統配置的安全性，而 PrivHound 提供了更深入的攻擊路徑分析。 |
> | 遷移成本 | - | low，因為兩者的使用場景有一定重疊。 | medium，因為需要調整工作流程以適應不同的分析方法。 |
> | 適用場景 | 主要場景 | 如果你的團隊需要自動化生成攻擊向量，則可以選擇這個工具。 | 如果需要針對系統配置進行詳細檢查，則可以考慮使用這個工具。 |
...
> [!tip] 採用建議
> 適合在測試環境中使用，不建議用於生產環境的核心路徑上。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在某些環境中，PowerShell 腳本可能因安全策略被阻止執行。
  - 解法：確保在執行前設定適當的執行策略。
- [MEDIUM] 如果 BloodHound 環境未正確配置，可能會導致數據上傳失敗。
  - 解法：檢查 BloodHound 的配置和 API 設定。
- [MEDIUM] 在大型環境中，數據收集可能需要較長時間。
  - 解法：考慮分批收集數據，並在非高峰時段執行。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 普通 | 雖然可以用於小型環境，但功能可能過於複雜。 |
| 50 人以上的企業安全團隊 | 非常適合 | 能夠有效分析大型環境中的權限提升風險。 |
| 需要跨平台支持的小型團隊 | 不適合 | 僅限於 Windows 環境，無法在其他操作系統上運行。  |
...
> [!tip] 投入 vs 回報
> 花 4 小時學習，2 小時整合，得到一個強大的權限提升分析工具，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：PrivHound 本身不需要高權限運行，但需注意收集過程中可能接觸到敏感資料。依賴於 PowerShell 的安全性，需確保環境安全。
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["特權提升","滲透測試","圖形數據庫"];
