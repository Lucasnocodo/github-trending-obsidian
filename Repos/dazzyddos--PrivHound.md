...
use_case: "將 Windows 本地特權提升建模為互聯的攻擊路徑，幫助安全專家識別潛在的安全風險。"
priority: medium
ring: assess
...
star_history: "2026-03-10:301,2026-03-11:302,2026-03-13:347,2026-03-14:363,2026-03-15:367,2026-03-16:370,2026-03-17:373,2026-03-18:378,2026-03-19:392,2026-03-20:408,2026-03-21:417,2026-03-22:418,2026-03-23:422,2026-03-24:428,2026-03-25:429,2026-03-26:433,2026-03-27:434,2026-03-29:435,2026-03-30:438,2026-03-31:440,2026-04-01:445,2026-04-02:449,2026-04-03:453,2026-04-04:454,2026-04-05:457"
tags:
  - github
  - "category/安全"
  - "lang/powershell"
aliases:
  - "PrivHound"
  - "dazzyddos/PrivHound"
  - "將 Windows 本地特權提升建模為互聯的攻擊路徑，幫助安全專家識別潛在的安全風險。"
---

# PrivHound

**457** stars · **16** stars/天 · 建立 29 天前 · PowerShell · 未標註授權
...
> **授權** 未標註授權 (風險較高) · **維護** Moderate (最後推送 18 天前) · **貢獻者** 2 人 · **參與度** Medium
> **適合** 需要快速識別 Windows 環境中安全漏洞的中小型企業安全團隊。
> **一句話重點** PrivHound 以圖形化方式呈現 Windows 本地特權提升的攻擊路徑，讓安全檢查變得更直觀。
...
> **結論** 花 5 小時學習，3 小時整合，能夠快速識別 Windows 環境中的安全風險，值得投入。

> [!abstract] 核心創新
> PrivHound 將 Windows 本地特權提升建模為互聯的攻擊路徑，提供圖形化的風險識別方式。

## 專案簡介

PrivHound 是一個 BloodHound OpenGraph 收集器，專注於 Windows 本地特權提升的攻擊路徑。它通過檢查多種潛在的安全漏洞（如可修改的服務權限、可寫的服務二進位檔、DLL 劫持等）來建立一個圖形化的攻擊路徑，這樣安全專家可以更直觀地理解和分析這些風險。使用者可以透過簡單的命令來啟動掃描，並獲得一個包含各種檢查結果的圖形化報告。這個工具的主要賣點在於將複雜的安全檢查以圖形方式展示，避免了傳統的文字描述造成的理解困難。技術上，PrivHound 使用 PowerShell 和 Cypher 來進行數據收集和查詢，這使得它能夠有效地與 Windows 環境整合。

相比於傳統的安全掃描工具，PrivHound 提供了更具視覺化的數據呈現，讓使用者能快速識別潛在的攻擊路徑。與其他工具如 AlpinDale/parsync 和 BigBodyCobain/Shadowbroker 相比，PrivHound 更加專注於本地特權提升的具體場景，並且提供了更詳細的 MITRE ATT&CK 技術映射，讓安全專家能夠針對性地進行防範。實際使用中，PrivHound 能夠處理多達 30 種不同的檢查項目，這些檢查涵蓋了從服務權限到憑證存儲的各種安全風險。社群活躍度良好，開發者在過去的 29 天內持續更新，顯示出對專案的承諾。這個工具適合中小型企業的安全團隊，尤其是那些需要快速識別和修復 Windows 環境中的安全漏洞的團隊。

**技術棧**：`PowerShell` · `Cypher`

## 重點功能

- 圖形化攻擊路徑 — 將複雜的特權提升風險以圖形方式展示，便於理解。
- 多達 30 種檢查項目 — 涵蓋從服務權限到憑證存儲的各種安全風險。
- MITRE ATT&CK 映射 — 每個檢查項目均對應 MITRE 技術，便於安全專家進行分析。
- PowerShell 和 Cypher 支援 — 利用這些技術進行數據收集和查詢，與 Windows 環境整合良好。
- 社群支持 — 積極的開發和更新，顯示出對專案的承諾和活躍度。
...
```powershell
{
  ...
  "預期輸出": "一個圖形化的攻擊路徑報告，顯示各種潛在的特權提升風險。"
}
```
...
> [!tip] 爆紅原因
> 建立 29 天內累積 457 stars（16/天），forks 46（10.1%），顯示出穩定的增長趨勢。作者 Signum21 和 dazzyddos 具備豐富的安全背景，專注於解決 Windows 環境中的特權提升問題，這在現有的安全工具中尚未得到充分關注。這個工具的出現正好填補了這一空白，並且在社群中引起了討論。技術上，PrivHound 的設計使得它能夠有效地整合到現有的安全工作流中，這也是它受到關注的原因之一。forks/stars 比率為 10.1%，顯示出使用者對這個工具的興趣和實際修改的需求。

## 適合誰使用

**目標受眾**：需要快速識別 Windows 環境中安全漏洞的中小型企業安全團隊。

> [!example] 使用場景
> - 安全工程師用它來識別 Windows 環境中的特權提升風險，因為它能以圖形化方式展示複雜的攻擊路徑，讓風險識別更直觀。
> - IT 管理員用它來檢查服務的安全配置，因為它提供了詳細的檢查項目，幫助快速定位可修改的服務權限。
> - 滲透測試人員用它來模擬攻擊路徑，因為它能夠清楚地顯示出不同攻擊向量之間的關聯，便於制定測試策略。

## 架構分析

PrivHound 採用 PowerShell 作為主要執行環境，並使用 Cypher 進行數據查詢。這樣的設計使得工具能夠在 Windows 環境中無縫運行，並且能夠快速收集和分析系統數據。資料流方面，工具首先檢查系統中的各種安全配置，然後將結果以圖形化方式呈現。

這種設計的代價在於需要用戶具備一定的 PowerShell 知識，才能有效地執行和解讀結果。擴展性方面，PrivHound 目前的架構能夠處理多達 30 種檢查項目，但隨著檢查項目的增加，可能會導致性能下降，特別是在大型企業環境中。整體而言，PrivHound 的架構設計旨在提供快速、直觀的安全檢查體驗，但在處理複雜環境時可能需要進一步優化。

## 技術深入分析

PrivHound 的核心技術機制在於使用 PowerShell 進行系統檢查，並利用 Cypher 查詢數據。這使得它能夠快速收集系統中的安全配置，並將結果以圖形化方式呈現。效能方面，PrivHound 能夠處理多達 30 種檢查項目，這些檢查涵蓋了從服務權限到憑證存儲的各種安全風險。設計上，選擇 PowerShell 是因為它在 Windows 環境中的普遍性和強大功能，但這也意味著使用者需要具備一定的 PowerShell 知識。技術風險方面，PrivHound 可能在處理大型企業環境時面臨性能瓶頸，特別是在檢查項目數量增加的情況下。此外，對於某些檢查項目，可能需要額外的系統權限才能獲取完整資訊。整合方面，PrivHound 可以輕鬆融入現有的安全工作流中，並與其他安全工具搭配使用，提升整體安全性。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了安裝和使用的基本指導。安裝過程相對順暢，但需要注意 PowerShell 的執行權限設定。文件中未提供多語言支持，可能對非英語使用者造成一定困難。

## 優缺點分析

> [!success] 優點
> - 提供圖形化的攻擊路徑，讓風險識別更直觀。
> - 涵蓋多種安全檢查項目，幫助全面評估系統安全性。
> - 社群活躍，持續更新和改進，顯示出對專案的承諾。

> [!danger] 缺點
> - 僅限於 Windows 環境，無法跨平台使用。
> - 需要 PowerShell 執行權限，對於某些用戶可能造成困難。
> - 在大型環境中可能面臨性能瓶頸，需進一步優化。

> [!warning] 注意事項
> - 僅支援 Windows 環境，無法在其他操作系統上運行。
> - 需要 PowerShell 執行權限，否則無法正確執行掃描。
> - 對於某些檢查項目，可能需要額外的系統權限才能獲取完整資訊。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 專注於檔案同步，但不提供特權提升的具體檢查項目，適合需要檔案管理的場景。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | 提供類似的安全檢查，但主要針對網路環境，而非本地特權提升。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 專注於檔案同步，並不針對特權提升進行檢查。 | 如果你的需求主要是檔案管理而非安全檢查，這個工具會更合適。 | low，因為兩者的使用場景不同，轉換需求不大。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | 針對網路環境的安全檢查，並不專注於本地特權提升。 | 如果你的環境主要是網路導向的，這個工具會更適合。 | medium，因為需要重新評估安全檢查的重點。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PrivHound** | **parsync** | **Shadowbroker** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於檔案同步，並不針對特權提升進行檢查。 | 針對網路環境的安全檢查，並不專注於本地特權提升。 |
> | 遷移成本 | - | low，因為兩者的使用場景不同，轉換需求不大。 | medium，因為需要重新評估安全檢查的重點。 |
> | 適用場景 | 主要場景 | 如果你的需求主要是檔案管理而非安全檢查，這個工具會更合適。 | 如果你的環境主要是網路導向的，這個工具會更適合。 |
...
> [!tip] 採用建議
> 適合安全專家進行測試和評估，但不建議在生產環境中使用。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在某些 Windows 版本上，PowerShell 可能無法正確執行腳本。
  - 解法：確保 PowerShell 版本為最新，並檢查執行策略設定。
- [MEDIUM] 對於某些檢查項目，可能需要管理員權限才能獲取完整資訊。
  - 解法：使用具有管理員權限的帳戶執行掃描。
- [MEDIUM] 在大型企業環境中，執行時間可能較長。
  - 解法：考慮分批執行檢查項目以提高效率。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 中小型企業的安全團隊 | 非常適合 | 提供全面的安全檢查，幫助快速識別風險。 |
| 大型企業的安全部門 | 普通 | 在大型環境中可能面臨性能瓶頸，需進一步優化。 |
| 滲透測試人員 | 非常適合 | 能夠清楚地顯示攻擊路徑，便於制定測試策略。 |
| 個人開發者 | 不適合 | 功能過於專業，對於個人使用者來說可能過於複雜。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，3 小時整合，能夠快速識別 Windows 環境中的安全風險，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：PrivHound 本身不需要高權限運行，但需注意執行時的 PowerShell 權限設定。它不會存取敏感資料，但在執行過程中可能會接觸到系統配置。
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["安全漏洞","特權提升","MITRE ATT&CK"];
