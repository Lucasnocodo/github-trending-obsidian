...
**474** stars · **10** stars/天 · 建立 48 天前 · PowerShell · 未標註授權
...
> **授權** 未標註授權 (風險較高) · **維護** Slow (最後推送 37 天前) · **貢獻者** 2 人 · **參與度** Low
> **適合** 需要評估 Windows 系統安全性並尋找權限提升路徑的安全專家。
> **一句話重點** PrivHound 讓 Windows 本地權限提升的分析變得可視化，顯著提升了安全評估的效率。
...
> **結論** 花 2 小時學習，1 小時整合，得到快速的權限提升分析，值得投入。

> [!abstract] 核心創新
> PrivHound 將 Windows 本地權限提升建模為圖形，讓攻擊路徑可視化並可查詢。

## 專案簡介

PrivHound 透過 BloodHound 的 OpenGraph 框架，將 Windows 本地權限提升的各種向量建模為圖形，讓使用者能夠直觀地查詢和分析攻擊路徑。使用者可以透過執行 PowerShell 腳本來收集系統資訊，並生成 JSON 格式的輸出，這些資訊可以被上傳至 BloodHound 進行進一步分析。這個工具的賣點在於它不僅列出單一的安全配置問題，而是將這些問題連結成可利用的攻擊鏈，讓使用者能夠理解從一個漏洞如何延伸到另一個漏洞。PrivHound 支援 29 種檢查項目，涵蓋從弱服務權限到 DLL 劫持等多種攻擊向量，並能夠查詢多跳攻擊路徑。技術上，PrivHound 使用 PowerShell 進行資料收集，並利用 Cypher 查詢語言來進行資料分析，這使得使用者能夠靈活地探索和視覺化攻擊路徑。

與傳統工具如 WinPEAS 和 PowerUp 相比，PrivHound 提供了更為整合的視角，能夠回答「這個配置問題實際上能通往什麼？」的問題，而不僅僅是列出問題清單。這使得它在進行安全評估時更具實用性和效率。使用者可以透過簡單的 PowerShell 指令來執行基本收集，或是自定義輸出路徑和跳過特定檢查，這樣的靈活性使得 PrivHound 適合各種環境的使用。整體而言，PrivHound 是一個強大的工具，能夠幫助安全專家更好地理解和利用 Windows 系統中的權限提升路徑。

**技術棧**：`PowerShell` · `Cypher`

## 重點功能

- 多跳攻擊路徑 — 將多個安全配置問題連結成可利用的攻擊鏈，提供更全面的安全評估。
- 29 種檢查項目 — 涵蓋從弱服務權限到 COM 劫持等多種攻擊向量，幫助使用者識別潛在風險。
- PowerShell 腳本收集 — 使用者可透過簡單的 PowerShell 指令來執行資料收集，方便快捷。
- Cypher 查詢語言 — 允許使用者靈活查詢和分析攻擊路徑，提升資料探索能力。
- 自定義輸出路徑 — 使用者可指定輸出路徑，並選擇跳過特定檢查，增強靈活性。
...
> [!tip] 爆紅原因
> 建立 48 天內累積 474 stars（約 10 stars/天），forks 46（9.7%），顯示出不錯的社群關注度。作者 Signum21 和 dazzyddos 具備相關背景，並針對 Windows 本地權限提升的痛點提供了一個整合性的解決方案，這在傳統工具中並不常見。PrivHound 的設計讓使用者能夠更直觀地理解攻擊路徑，這是過去工具無法做到的。社群的活躍度和開發者的回應速度也顯示出這個專案的潛力。

## 適合誰使用

**目標受眾**：需要評估 Windows 系統安全性並尋找權限提升路徑的安全專家。

> [!example] 使用場景
> - 安全分析師用它來評估企業內部系統的權限提升風險，因為它能將多個配置問題連結成攻擊路徑，讓風險評估更全面。
> - 滲透測試人員用它來快速識別可利用的權限提升路徑，因為 PrivHound 提供了可視化的攻擊鏈，節省了手動分析的時間。
> - 系統管理員用它來檢查系統配置的安全性，因為它能自動化檢查過程，並提供具體的攻擊路徑分析。

## 架構分析

PrivHound 的架構基於 BloodHound 的 OpenGraph 框架，這使得資料的收集和分析變得更為靈活。資料流從 PowerShell 腳本收集系統資訊開始，然後將這些資訊轉換為 JSON 格式，最終上傳至 BloodHound 進行可視化。這種設計使得使用者能夠快速獲得攻擊路徑的視覺化結果，而不需要手動連接各個漏洞。選擇 PowerShell 作為主要語言是因為它在 Windows 環境中的普遍性和強大功能，這使得 PrivHound 能夠充分利用 Windows 系統的特性。整體而言，這種架構設計使得 PrivHound 能夠在不需要過多依賴外部工具的情況下，提供一個完整的權限提升分析解決方案。

## 技術深入分析

PrivHound 的核心技術機制在於將 Windows 本地權限提升的各種向量建模為圖形，這使得使用者能夠直觀地探索攻擊路徑。它使用 PowerShell 腳本來收集系統資訊，並將這些資訊轉換為 JSON 格式，最終上傳至 BloodHound 進行可視化。這種設計使得資料流動變得清晰，使用者能夠快速獲得攻擊路徑的視覺化結果。PrivHound 的效能在於能夠處理多種檢查項目，並且能夠快速生成攻擊路徑，這對於安全專家來說是非常重要的。

選擇 PowerShell 作為主要語言是因為它在 Windows 環境中的普遍性，這使得 PrivHound 能夠充分利用 Windows 系統的特性。設計上，PrivHound 充分考慮了使用者的需求，提供了靈活的選項來自定義檢查項目和輸出路徑。這使得它在實際使用中能夠適應不同的環境和需求。整體而言，PrivHound 提供了一個強大的權限提升分析工具，能夠幫助安全專家更好地理解和利用 Windows 系統中的權限提升路徑。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的指導和範例，安裝過程相對順暢，使用者只需執行 PowerShell 腳本即可開始收集資料。文件中缺少多語言支持，但對於英文使用者來說，理解上沒有太大障礙。整體而言，花 30 分鐘內能夠順利運行起來。

## 優缺點分析

> [!success] 優點
> - 提供可視化的攻擊路徑，讓使用者能夠更直觀地理解權限提升風險。
> - 支援多種檢查項目，涵蓋廣泛的攻擊向量。
> - 使用 PowerShell 腳本進行資料收集，方便快捷，易於整合進現有工作流。

> [!danger] 缺點
> - 僅限於 Windows 環境，無法在其他作業系統上使用。
> - 需要適當的權限來執行，否則可能無法獲得完整資料。
> - 對於特定系統配置，可能需要手動調整檢查項目。

> [!warning] 注意事項
> - 僅支援 Windows 環境，無法在其他作業系統上運行。
> - 需要適當的權限來執行 PowerShell 腳本，否則可能無法獲得完整的資料。
> - 對於某些特定的系統配置，可能需要手動調整檢查項目。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard 專注於發現和利用 Active Directory 漏洞，而 PrivHound 專注於 Windows 本地權限提升，兩者的攻擊面不同。 |
| WinPEAS | WinPEAS 主要是列出單一的安全配置問題，而 PrivHound 將這些問題連結成攻擊鏈，提供更深層的分析。 |
| PowerUp | PowerUp 專注於權限提升的檢查，而 PrivHound 則提供可視化的攻擊路徑，讓使用者能夠更直觀地理解攻擊鏈。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | Boneyard 專注於發現和利用 Active Directory 漏洞，而 PrivHound 專注於 Windows 本地權限提升，兩者的攻擊面不同。 | 如果你的工作重心在 Active Directory 安全性分析，而非單一 Windows 系統的權限提升，則 Boneyard 更適合。 | medium，因為需要重新學習不同的工具和流程。 |
| WinPEAS | WinPEAS 主要是列出單一的安全配置問題，而 PrivHound 將這些問題連結成攻擊鏈，提供更深層的分析。 | 如果你需要快速識別單一的安全配置問題，WinPEAS 會更有效率。 | low，因為兩者的使用方式相似。 |
| PowerUp | PowerUp 專注於權限提升的檢查，而 PrivHound 則提供可視化的攻擊路徑，讓使用者能夠更直觀地理解攻擊鏈。 | 如果你需要詳細的權限提升檢查，PowerUp 會是更好的選擇。 | medium，因為需要了解不同的檢查項目和報告格式。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PrivHound** | **boneyard** | **WinPEAS** | **PowerUp** |
> | --- | --- | --- | --- | --- |
> | 技術路線 | 本專案 | Boneyard 專注於發現和利用 Active Directory 漏洞，而 PrivHound 專注於 Windows 本地權限提升，兩者的攻擊面不同。 | WinPEAS 主要是列出單一的安全配置問題，而 PrivHound 將這些問題連結成攻擊鏈，提供更深層的分析。 | PowerUp 專注於權限提升的檢查，而 PrivHound 則提供可視化的攻擊路徑，讓使用者能夠更直觀地理解攻擊鏈。 |
> | 遷移成本 | - | medium，因為需要重新學習不同的工具和流程。 | low，因為兩者的使用方式相似。 | medium，因為需要了解不同的檢查項目和報告格式。 |
> | 適用場景 | 主要場景 | 如果你的工作重心在 Active Directory 安全性 | 如果你需要快速識別單一的安全配置問題，WinPEAS 會更有 | 如果你需要詳細的權限提升檢查，PowerUp 會是更好的選擇 |
...
> [!tip] 採用建議
> 適合個人測試和小型專案，不建議用於生產環境的核心路徑上。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在某些系統上，PowerShell 腳本可能需要額外的執行權限才能正常運行。
  - 解法：確保以管理員身份運行 PowerShell。
- [MEDIUM] 某些檢查項目可能因系統配置而無法正常執行。
  - 解法：根據實際情況調整檢查項目。
- [MEDIUM] 上傳至 BloodHound 時，可能會遇到格式不正確的問題。
  - 解法：檢查 JSON 格式是否符合要求。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 能夠快速識別和分析權限提升風險，適合資源有限的小型團隊。 |
| 大型企業的安全審計 | 適合 | 提供全面的權限提升分析，但可能需要更多的手動調整。 |
| 個人學習和實驗 | 非常適合 | 易於使用和安裝，適合個人探索 Windows 安全性。 |
| 需要即時反應的生產環境 | 不適合 | 目前仍在 beta 階段，穩定性和可靠性尚未達到生產要求。  |
...
> [!tip] 投入 vs 回報
> 花 2 小時學習，1 小時整合，得到快速的權限提升分析，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：PrivHound 本身不需要高權限運行，但執行 PowerShell 腳本時需要適當的權限。它不會存取敏感資料，依賴鏈的信任程度較高，適合在 CI/CD 中使用。
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["特權提升","攻擊路徑分析","Windows 安全性"];
