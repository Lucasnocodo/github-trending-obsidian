...
use_case: "將 Windows 本地特權提升建模為互聯的攻擊路徑，幫助安全分析師識別潛在的安全風險。"
priority: medium
ring: assess
...
aliases:
  - "PrivHound"
  - "dazzyddos/PrivHound"
  - "將 Windows 本地特權提升建模為互聯的攻擊路徑，幫助安全分析師識別潛在的安全風險。"
---

# PrivHound

**472** stars · **10** stars/天 · 建立 47 天前 · PowerShell · 未標註授權
...
> **授權** 未標註授權 (風險較高) · **維護** Slow (最後推送 36 天前) · **貢獻者** 2 人 · **參與度** Low
> **適合** 需要進行 Windows 環境安全評估的安全專業人士。
> **一句話重點** PrivHound 透過圖形化的方式將 Windows 本地特權提升的攻擊路徑可視化，讓安全分析變得更有效率。
...
> **結論** 花 4 小時學習，3 小時整合，得到全面的安全評估效果，值得投入。

> [!abstract] 核心創新
> 提供了一種新的方式來視覺化 Windows 本地特權提升的攻擊路徑。

## 專案簡介

PrivHound 是一個 BloodHound OpenGraph 收集器，專注於 Windows 本地特權提升的攻擊路徑。它透過檢查多種潛在的安全漏洞，例如可修改的服務權限、可寫的服務二進制檔案及 DLL 劫持等，來建構一個互聯的攻擊路徑圖。使用者只需執行簡單的 PowerShell 指令，便可收集系統的安全配置，並將結果以圖形化方式呈現，這樣的設計使得安全分析不再是冗長的文字描述，而是直觀的視覺化圖形。這個工具的賣點在於其能夠快速識別和視覺化多種攻擊向量，從而幫助安全專家制定更有效的防禦策略。

技術上，PrivHound 使用 PowerShell 來執行檢查，並利用 Cypher 語言來查詢和呈現數據。這種設計選擇使得它能夠輕鬆整合到現有的 BloodHound 生態系統中，並且能夠利用 BloodHound 的圖形化展示功能。

相較於其他工具，PrivHound 的優勢在於其專注於 Windows 環境的特權提升，並且提供了更為直觀的視覺化結果。

與其他類似工具如 0xGF/boneyard 和 AgentSeal/codeburn 相比，PrivHound 提供了更全面的檢查項目，涵蓋了多達 30 種檢查技術，並且能夠將這些檢查結果以圖形方式呈現，這對於安全分析師來說是非常有價值的。

在實際使用中，PrivHound 能夠處理大量的系統配置數據，並且對於中小型企業來說，這是一個相對輕量級的解決方案。社群活躍度尚可，解決率為 33%，顯示出開發者對於問題的回應速度。

這個專案目前處於 beta 階段，適合需要進行 Windows 環境安全評估的團隊使用。未來可能會增加更多的檢查項目和功能，進一步提升其實用性。

**技術棧**：`PowerShell` · `Cypher`

## 重點功能

- 多達 30 種檢查項目 — 包括弱服務權限、可寫的服務二進制檔案、DLL 劫持等，提供全面的安全評估。
- 圖形化攻擊路徑 — 將檢查結果以圖形方式呈現，幫助用戶直觀理解潛在的攻擊向量。
- 基於 PowerShell 和 Cypher — 利用這兩種語言進行數據收集和查詢，方便與 BloodHound 整合。
- MITRE ATT&CK 框架對應 — 每個檢查項目都對應 MITRE 技術，幫助用戶理解攻擊技術。
- 社群支持 — 透過 GitHub Issues 進行問題反饋和功能請求，活躍的社群能夠提供即時的支持。
...
```powershell
{
  "前置條件": "需要安裝 PowerShell 和 Neo4j。",
  "指令": "powershell -ExecutionPolicy Bypass -File PrivHound.ps1",
  "預期輸出": "生成的圖形化攻擊路徑，顯示潛在的特權提升路徑。"
}
```
...
> [!tip] 爆紅原因
> 建立 47 天內累積 472 stars（10 stars/天），forks 46（9.7%），顯示出一定的社群關注度。作者 Signum21 和 dazzyddos 在安全領域有一定的經驗，這個工具解決了 Windows 環境中本地特權提升的可視化問題，之前的工具往往缺乏直觀的圖形化展示。社群的討論和需求也推動了這個工具的發展，特別是在安全測試和評估方面的需求日益增加。
...
> [!example] 使用場景
> - 安全分析師用它來視覺化本地特權提升的攻擊路徑，因為這樣能快速識別潛在的安全風險，並制定相應的防禦策略。
> - 系統管理員用它來檢查 Windows 系統的安全配置，因為它能提供詳細的檢查項目，幫助發現系統中的安全漏洞。
> - 滲透測試人員用它來模擬攻擊路徑，因為其圖形化的結果能夠幫助他們更好地理解攻擊面，並制定測試計劃。

## 架構分析

PrivHound 的架構基於 PowerShell 腳本和 Neo4j 數據庫。使用者執行 PowerShell 腳本來收集系統的安全配置，然後將這些數據存儲到 Neo4j 中。這種設計使得數據的查詢和視覺化變得更加高效。

選擇 Neo4j 作為數據庫的原因在於其強大的圖形數據處理能力，能夠輕鬆地處理複雜的數據關係。這樣的架構設計雖然增加了部署的複雜性，但卻能夠提供更為直觀的結果。未來可能的擴展方向包括增加更多的檢查項目和功能，以提升其實用性。

## 技術深入分析

PrivHound 的核心技術機制是利用 PowerShell 腳本來收集 Windows 系統的安全配置，並將這些數據存儲在 Neo4j 數據庫中。這樣的設計使得數據之間的關聯性可以被有效地查詢和視覺化。效能上，PrivHound 能夠處理大量的系統配置數據，並且在中小型企業的環境中表現良好。選擇 PowerShell 和 Cypher 的原因在於它們的靈活性和強大的查詢能力，這使得 PrivHound 能夠快速地生成圖形化結果。

設計上，這個工具的依賴性相對較低，但仍然需要 Neo4j 作為數據庫，這可能會對某些用戶造成困擾。技術風險方面，未來在擴展檢查項目時可能會面臨性能瓶頸，特別是在處理大型企業的系統時。整合方面，PrivHound 可以輕鬆與現有的 BloodHound 生態系統整合，並且能夠與其他安全工具鏈相容。整體來說，PrivHound 是一個值得關注的安全工具，特別是在 Windows 環境的安全評估方面。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的指導，並包含了基本的使用範例。安裝過程相對順暢，但需要注意 Neo4j 的安裝配置。沒有專門的 getting started guide，但使用者可以從 README 中獲得足夠的資訊。文件目前僅提供英文版本。

## 優缺點分析

> [!success] 優點
> - 提供全面的檢查項目，幫助用戶識別多種潛在的安全風險。
> - 圖形化的結果使得安全分析變得更加直觀，易於理解。
> - 社群活躍，能夠提供即時的問題反饋和支持。

> [!danger] 缺點
> - 僅支援 Windows 環境，限制了其應用範圍。
> - 需要額外安裝 Neo4j，增加了部署的複雜性。
> - 目前的檢查項目數量有限，未來需要增加更多功能。

> [!warning] 注意事項
> - 僅支援 Windows 環境，無法在其他操作系統上運行。
> - 需要安裝 Neo4j 作為數據庫，增加了部署的複雜性。
> - 目前僅有少量的檢查項目，未來可能需要增加更多功能。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於 Windows 環境的安全檢查，但檢查項目較少，無法提供圖形化展示。 |
| [AgentSeal/codeburn](https://github.com/AgentSeal/codeburn) | 提供類似的功能，但主要針對 Linux 環境，無法直接應用於 Windows 系統。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 專注於 Windows 環境的安全檢查，但檢查項目較少，無法提供圖形化展示。 | 如果需要一個簡單的安全檢查工具，且不需要圖形化展示。 | low，因為功能上有相似之處。 |
| [AgentSeal/codeburn](https://github.com/AgentSeal/codeburn) | 提供類似的功能，但主要針對 Linux 環境，無法直接應用於 Windows 系統。 | 如果你的環境主要是 Linux，且需要針對 Linux 的特權提升檢查。 | medium，因為需要重新學習工具的使用方式。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PrivHound** | **boneyard** | **codeburn** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於 Windows 環境的安全檢查，但檢查項目較少，無法提供圖形化展示。 | 提供類似的功能，但主要針對 Linux 環境，無法直接應用於 Windows 系統。 |
> | 遷移成本 | - | low，因為功能上有相似之處。 | medium，因為需要重新學習工具的使用方式。 |
> | 適用場景 | 主要場景 | 如果需要一個簡單的安全檢查工具，且不需要圖形化展示。 | 如果你的環境主要是 Linux，且需要針對 Linux 的特 |
...
> [!tip] 採用建議
> 適合進行 Windows 環境的安全評估，但不建議在生產環境的核心路徑上使用。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在某些 Windows 環境中，PowerShell 腳本可能因安全策略而無法執行。
  - 解法：需要調整 PowerShell 的執行策略。
- [MEDIUM] Neo4j 的配置可能會造成初次使用者困惑。
  - 解法：參考官方文檔進行配置。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司進行安全評估 | 非常適合 | 提供全面的檢查項目，幫助識別潛在的安全風險。 |
| 大型企業的安全團隊進行滲透測試 | 適合 | 能夠視覺化攻擊路徑，但可能需要更多的檢查項目。 |
| 需要快速檢查 Windows 系統的安全配置 | 非常適合 | 能夠快速生成檢查結果，並提供直觀的圖形展示。 |
| 對於 Linux 環境的安全評估 | 不適合 | 僅支援 Windows 環境，無法應用於 Linux 系統。 |
...
> [!tip] 投入 vs 回報
> 花 4 小時學習，3 小時整合，得到全面的安全評估效果，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：PrivHound 本身不需要高權限運行，但需要存取系統的安全配置資料。依賴的 Neo4j 也需注意其安全配置，避免未經授權的存取。
