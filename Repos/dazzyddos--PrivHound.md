...
use_case: "將 Windows 本地權限提升攻擊路徑以圖形化方式呈現，幫助安全專家識別潛在的安全漏洞。"
priority: medium
ring: assess
...
star_history: "2026-03-10:301,2026-03-11:302,2026-03-13:347,2026-03-14:363,2026-03-15:367,2026-03-16:370,2026-03-17:373,2026-03-18:378,2026-03-19:392,2026-03-20:408,2026-03-21:417,2026-03-22:418,2026-03-23:422,2026-03-24:428,2026-03-25:429"
tags:
  - github
  - "category/安全"
  - "lang/powershell"
aliases:
  - "PrivHound"
  - "dazzyddos/PrivHound"
  - "將 Windows 本地權限提升攻擊路徑以圖形化方式呈現，幫助安全專家識別潛在的安全漏洞。"
---

# PrivHound

**429** stars · **24** stars/天 · 建立 18 天前 · PowerShell · 未標註授權
...
> **一句話重點** PrivHound 透過圖形化方式讓 Windows 本地權限提升的攻擊路徑變得易於理解，顯著提升了安全分析的效率。
...
> **結論** 花 5 小時學習，3 小時整合，得到全面的安全評估工具，值得考慮。

> [!abstract] 核心創新
> PrivHound 提供了一種全新的圖形化方式來展示 Windows 本地權限提升的攻擊路徑。

## 專案簡介

PrivHound 是一個專為 Windows 環境設計的 BloodHound OpenGraph 收集器，旨在將本地權限提升的攻擊路徑以圖形化方式呈現。使用者可以透過這個工具檢查多種潛在的安全漏洞，例如可修改的服務權限、可寫的服務二進位檔和 DLL 劫持等。這些檢查項目都與 MITRE ATT&CK 框架相連結，使得安全專家能夠快速識別和評估系統中的風險。使用者只需執行簡單的 PowerShell 指令來收集數據，然後利用 Cypher 查詢語言來分析和視覺化這些數據，這樣的設計使得複雜的攻擊路徑變得易於理解。PrivHound 的核心賣點在於其圖形化的呈現方式，讓使用者不再需要面對冗長的文字描述，能夠更直觀地識別攻擊路徑。這種視覺化的方式不僅提高了效率，也降低了誤解的風險。

相較於傳統的文字報告，PrivHound 的圖形化展示能夠讓使用者更快地掌握系統的安全狀態。技術上，PrivHound 依賴 PowerShell 進行數據收集，並使用 Cypher 進行數據查詢，這樣的選擇使得它能夠輕鬆整合到現有的 Windows 環境中。與其他類似工具相比，PrivHound 提供了更為全面的檢查項目，並且其圖形化的結果使得分析變得更加直觀。使用者在實際運用中可能會遇到的問題包括如何有效地解讀圖形結果，以及如何根據檢查結果進行後續的安全加固。這個專案目前處於活躍開發中，未來可能會持續增加更多的檢查項目和功能。對於需要進行安全評估的團隊，PrivHound 是一個值得考慮的工具，特別是在處理複雜的 Windows 環境時。

**技術棧**：`PowerShell` · `Cypher`

## 重點功能

- 圖形化攻擊路徑 — 將本地權限提升的攻擊路徑以圖形化方式呈現，便於理解。
- 多種檢查項目 — 包含 30 種不同的檢查項目，涵蓋各種潛在的安全漏洞。
- MITRE ATT&CK 整合 — 每個檢查項目都與 MITRE 框架相連結，提供清晰的攻擊向量。
- PowerShell 數據收集 — 使用 PowerShell 指令輕鬆收集系統數據。
- Cypher 查詢分析 — 利用 Cypher 語言進行數據查詢和分析，提升靈活性。
...
> [!tip] 爆紅原因
> 建立 18 天就累積 429 stars（24/天），forks 45（10.5%），顯示出穩定的增長潛力。開發者 Signum21 和 dazzyddos 具備安全領域的背景，這使得他們能夠針對實際需求開發出有效的工具。PrivHound 解決了傳統安全檢查報告過於冗長且難以理解的問題，提供了一種更直觀的方式來識別和分析攻擊路徑。這個工具的推出正好契合了對於 Windows 環境安全性評估需求的增長，並且其圖形化的呈現方式吸引了許多安全專家的注意。forks/stars 比率為 10.5%，顯示出相對較高的實際使用意圖。
...
> [!example] 使用場景
> - 安全分析師用它來識別 Windows 環境中的潛在權限提升路徑，因為其圖形化的結果能快速幫助他們定位問題。
> - 系統管理員用它來檢查服務的安全性配置，因為 PrivHound 提供了多種檢查項目，能夠全面評估服務的風險。
> - 滲透測試者用它來模擬攻擊路徑，因為其與 MITRE ATT&CK 框架的整合能夠提供清晰的攻擊向量分析。

## 架構分析

PrivHound 採用 PowerShell 進行數據收集，並將數據存儲在 Neo4j 數據庫中。這樣的設計使得數據能夠以圖形化的方式進行查詢和分析。使用 Cypher 查詢語言來分析數據，能夠靈活地獲取所需的信息。

選擇 PowerShell 是因為它在 Windows 環境中的普遍性，而 Neo4j 則提供了強大的圖形數據庫功能。這種架構的代價在於需要額外的環境設置和權限配置，尤其是對於 PowerShell 腳本的執行。此外，這種設計在處理大量數據時可能會面臨性能瓶頸，特別是在複雜查詢的情況下。

整體來說，PrivHound 的架構能夠有效地支持安全分析，但在擴展性方面可能需要進一步的優化。

## 技術深入分析

PrivHound 的核心技術機制在於使用 PowerShell 進行數據收集，並將收集到的數據存儲在 Neo4j 數據庫中。這樣的設計使得數據能夠以圖形化的方式進行查詢和分析，並且能夠快速識別出潛在的安全漏洞。效能上，PrivHound 能夠處理中等規模的 Windows 環境，但在面對大量數據時，查詢的速度可能會受到影響。選擇 PowerShell 是因為它在 Windows 環境中的普遍性，而 Neo4j 則提供了強大的圖形數據庫功能，這樣的選擇使得 PrivHound 能夠在安全分析中提供更高的靈活性和效率。技術風險方面，PrivHound 的設計可能在處理複雜查詢時出現性能瓶頸，特別是在數據量增大時，這可能會影響使用者的體驗。整合方面，PrivHound 可以與其他安全工具配合使用，但在設置過程中可能會遇到一些摩擦點，例如 PowerShell 腳本的執行權限問題。整體來說，PrivHound 提供了一個強大的工具來進行 Windows 環境的安全分析，但仍需注意其在擴展性和性能方面的挑戰。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了基本的使用說明，但缺乏詳細的範例。安裝過程相對順暢，但需要注意 PowerShell 腳本的執行權限。沒有提供專門的入門指南，文件主要以英文撰寫，對於非英語使用者可能會有一定的障礙。

## 優缺點分析

> [!success] 優點
> - 提供直觀的圖形化攻擊路徑展示，便於快速理解。
> - 涵蓋多種檢查項目，能夠全面評估系統安全性。
> - 與 MITRE ATT&CK 框架整合，提供清晰的攻擊向量分析。

> [!danger] 缺點
> - 僅限於 Windows 環境，對於其他操作系統無法使用。
> - 需要額外的環境設置和權限配置，增加了使用門檻。
> - 在處理大量數據時可能會遇到性能瓶頸。

> [!warning] 注意事項
> - 僅支援 Windows 環境，無法在其他操作系統上運行。
> - 需要安裝 Neo4j 以支持數據存儲和查詢。
> - PowerShell 腳本執行需要適當的權限設置。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 提供了類似的數據收集功能，但不專注於權限提升路徑的圖形化展示。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | 專注於 Windows 系統的漏洞利用，但缺乏 PrivHound 的圖形化分析功能。 |
| [Flowseal/tg-ws-proxy](https://github.com/Flowseal/tg-ws-proxy) | 主要用於網路流量的代理和分析，與本專案的安全檢查功能不同。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | 專注於數據同步和備份，與 PrivHound 的安全檢查功能不同。 | 如果你的需求是數據同步而非安全檢查，這個工具會更合適。 | low，因為兩者的功能和目的不同，遷移不需要太多調整。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | 專注於漏洞利用的工具，與 PrivHound 的圖形化分析功能無法相比。 | 如果你的重點是漏洞利用而非安全評估，Shadowbroker 會更合適。 | medium，因為需要調整使用的工具和流程。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PrivHound** | **parsync** | **Shadowbroker** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於數據同步和備份，與 PrivHound 的安全檢查功能不同。 | 專注於漏洞利用的工具，與 PrivHound 的圖形化分析功能無法相比。 |
> | 遷移成本 | - | low，因為兩者的功能和目的不同，遷移不需要太多調整。 | medium，因為需要調整使用的工具和流程。 |
> | 適用場景 | 主要場景 | 如果你的需求是數據同步而非安全檢查，這個工具會更合適。 | 如果你的重點是漏洞利用而非安全評估，Shadowbroker |
...
> [!tip] 採用建議
> 適合用於測試和評估，但不建議在生產環境中使用。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** PowerShell 腳本執行需要特定權限，可能導致執行失敗
  - 解法：確保 PowerShell 的執行策略設置為 Bypass
- [MEDIUM] 在大型環境中，數據收集可能需要較長時間
  - 解法：考慮分批收集數據以提高效率
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 大型企業的 IT 安全團隊 | 非常適合 | 能夠全面評估 Windows 環境的安全性，提供直觀的攻擊路徑分析。 |
| 小型公司的系統管理員 | 適合 | 能夠快速識別和修復安全漏洞，但可能需要額外的學習時間。 |
| 個人開發者進行安全測試 | 普通 | 雖然可以使用，但功能可能過於複雜，且不一定符合個人需求。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，3 小時整合，得到全面的安全評估工具，值得考慮。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：PrivHound 本身不需要高權限運行，但需要適當的權限設置以執行 PowerShell 腳本。它不會存取敏感資料，主要依賴於系統的安全配置。
...
相關概念：[[特權提升]] · [[MITRE ATT&CK]] · [[PowerShell]]
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["特權提升","MITRE ATT&CK","PowerShell"];
