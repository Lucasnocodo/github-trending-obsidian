...
use_case: "將 Windows 本地特權提升模型化為互聯的攻擊路徑，幫助安全研究人員識別潛在的安全風險。"
priority: medium
ring: assess
...
star_history: "2026-03-10:301,2026-03-11:302,2026-03-13:347,2026-03-14:363,2026-03-15:367,2026-03-16:370,2026-03-17:373,2026-03-18:378,2026-03-19:392,2026-03-20:408"
tags:
  - github
  - "category/安全"
  - "lang/powershell"
...
aliases:
  - "PrivHound"
  - "dazzyddos/PrivHound"
  - "將 Windows 本地特權提升模型化為互聯的攻擊路徑，幫助安全研究人員識別潛在的安全風險。"
---

# PrivHound

**408** stars · **31** stars/天 · 建立 13 天前 · PowerShell · 未標註授權
...
> **適合** 需要快速識別 Windows 環境中潛在特權提升風險的安全研究人員和系統管理員。
> **一句話重點** PrivHound 讓安全檢查變得更直觀，幫助使用者快速識別潛在的攻擊路徑。
...
>     .where(p => p.subcategory === "安全檢查工具" && p.file.name !== "dazzyddos--PrivHound" && p.status !== "archived")
...
>       p.ring || "assess"
>     ]));
>   } else { dv.paragraph("_目前 vault 中沒有其他 安全檢查工具 類工具_"); }
> }
> ```

> [!question] TL;DR — 值得投入嗎？
> **成熟度** Alpha (不穩定) · **安裝** Easy (一行搞定) · **學習** ~2h · **綁定風險** low
> **結論** 花 2 小時學習，1 小時整合，得到快速識別安全漏洞的效果，值得考慮。

> [!abstract] 核心創新
> 將本地特權提升的潛在攻擊路徑以圖形化方式呈現，提升安全檢查的直觀性。

## 專案簡介

PrivHound 是一個專為 Windows 環境設計的 BloodHound OpenGraph 收集器，旨在將本地特權提升的潛在攻擊路徑以圖形化方式呈現。使用者可以透過執行特定的檢查來識別系統中的弱點，例如可修改的服務權限和可寫的服務二進位檔。這些檢查包括檢測不安全的服務路徑、DLL 劫持和存儲的憑證等，這些都是攻擊者常用的手法。使用者只需執行相應的 PowerShell 指令，即可獲得這些檢查的結果，並以圖形形式展示潛在的攻擊路徑，從而更直觀地理解安全風險。這個工具的賣點在於其將複雜的安全檢查以圖形化方式呈現，讓安全研究人員能夠快速識別並修復潛在的漏洞。

技術上，PrivHound 使用 PowerShell 進行檢查，並利用 Cypher 語言來建構和查詢圖形數據。這使得它能夠高效地處理和展示大量的安全數據，並且能夠與 BloodHound 生態系統無縫整合。這個工具的依賴性相對較低，主要依賴於 PowerShell 和一些基本的圖形數據庫功能。



在與其他類似工具的比較中，PrivHound 提供了更直觀的圖形化展示，與傳統的文字報告相比，能夠更快地讓使用者理解潛在的攻擊路徑。相比於 AlpinDale/parsync，PrivHound 更專注於本地特權提升的攻擊路徑，而不是一般的安全檢查；而與 BigBodyCobain/Shadowbroker 相比，PrivHound 提供了更具體的檢查項目，並且能夠以圖形方式展示結果。

實際使用中，PrivHound 能夠處理多種檢查項目，並且能夠在大型企業環境中發揮作用，幫助安全團隊識別和修復潛在的安全漏洞。使用者可能會遇到的問題包括對 PowerShell 的依賴，這可能會在某些環境中造成兼容性問題。社群活躍度方面，該專案在短時間內獲得了 408 顆星，顯示出其受到廣泛關注。

目前，PrivHound 還處於早期階段，建議在非關鍵環境中進行測試，未來可能會增加更多的檢查項目和功能。對於需要快速識別 Windows 環境中潛在特權提升風險的安全研究人員來說，這是一個值得考慮的工具。

**技術棧**：`PowerShell` · `Cypher`

## 重點功能

- 圖形化攻擊路徑展示 — 將本地特權提升的潛在攻擊路徑以圖形方式呈現，便於理解。
- 多種檢查項目 — 包括弱服務權限、可寫服務二進位檔、DLL 劫持等，涵蓋多種攻擊手法。
- MITRE ATT&CK 對應 — 每個檢查項目都對應 MITRE ATT&CK 框架，幫助使用者了解攻擊技術。
- PowerShell 支援 — 使用 PowerShell 進行檢查，方便在 Windows 環境中部署。
- Cypher 查詢 — 利用 Cypher 語言進行圖形數據查詢，提升查詢效率。
...
> [!tip] 爆紅原因
> 建立 13 天就累積 408 stars（31/天），forks 41（10.0%），顯示出強勁的增長潛力。這個專案的主要貢獻者 Signum21 和 dazzyddos 在安全領域有豐富的經驗，之前的專案也受到廣泛關注。PrivHound 解決了傳統安全檢查報告難以直觀理解的問題，讓使用者能夠更快地識別和修復安全漏洞。社群對於 Windows 本地特權提升的關注度也在上升，這使得這個工具的需求增加。forks/stars 比率為 10.0%，顯示出使用者對於進一步修改和擴展的興趣。

## 適合誰使用

**目標受眾**：需要快速識別 Windows 環境中潛在特權提升風險的安全研究人員和系統管理員。

> [!example] 使用場景
> - 安全研究員用它來識別 Windows 環境中的特權提升風險，因為它能夠以圖形方式展示複雜的攻擊路徑，讓風險更易於理解。
> - 系統管理員用它來檢查服務的安全性，因為它提供了具體的檢查項目，幫助快速定位潛在的安全漏洞。
> - 企業安全團隊用它來進行安全審計，因為它能夠整合多種檢查，並以圖形方式展示結果，提升審計效率。

## 架構分析

PrivHound 採用 PowerShell 作為主要執行環境，這使得它能夠直接在 Windows 系統上運行，並進行各種安全檢查。資料流方面，檢查結果會被收集並轉換為圖形數據，使用 Cypher 語言進行查詢和展示。這種設計使得使用者能夠快速獲得檢查結果，並以圖形方式理解潛在的攻擊路徑。選擇 PowerShell 的好處在於其與 Windows 環境的高度相容性，但代價是無法在其他作業系統上運行。擴展性方面，未來可以考慮增加更多的檢查項目和功能，以滿足不同使用者的需求。

## 技術深入分析

PrivHound 的核心技術機制是利用 PowerShell 進行安全檢查，並將結果以圖形化方式展示。這使得使用者能夠直觀地理解各種攻擊路徑，提升了安全檢查的效率。效能方面，PrivHound 能夠快速處理多種檢查項目，並在大型企業環境中運行良好。設計上選擇 PowerShell 是因為其在 Windows 環境中的廣泛應用，但這也限制了其在其他作業系統上的使用。技術風險方面，未來可能需要擴展更多的檢查項目，以滿足不斷變化的安全需求。整合方面，PrivHound 可以與 BloodHound 生態系統無縫整合，這使得它能夠在現有的安全工具鏈中發揮作用，並且相對容易上手。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的說明和快速上手指南，安裝過程簡單明瞭，沒有明顯的坑。文件中缺乏多語言支持，但對於英語使用者來說，理解上不會有太大困難。整體來說，花 30 分鐘應該能夠順利運行起來。
...
> - 圖形化展示，提升檢查結果的可讀性。
> - 涵蓋多種攻擊手法，提供全面的安全檢查。
> - MITRE ATT&CK 對應，幫助使用者理解攻擊技術。

> [!danger] 缺點
> - 僅支援 Windows 環境，限制了使用範圍。
> - 對於某些使用者可能造成兼容性問題。
> - 檢查項目數量仍然有限，未來需要擴展。

> [!warning] 注意事項
> - 僅支援 Windows 環境，無法在其他作業系統上運行。
> - 需要 PowerShell 環境，對於某些使用者可能造成兼容性問題。
> - 目前檢查項目數量有限，未來可能需要擴展更多檢查項目。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | PrivHound 專注於本地特權提升的攻擊路徑，而 parsync 更偏向於一般的安全檢查，適用範圍不同。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | PrivHound 提供圖形化的攻擊路徑展示，而 Shadowbroker 更加注重於數據的收集和分析。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [AlpinDale/parsync](https://github.com/AlpinDale/parsync) | PrivHound 專注於 Windows 本地特權提升，而 parsync 更偏向於一般的安全檢查。 | 如果你的團隊需要更全面的安全檢查，而不僅僅是針對特權提升的風險，則可以考慮 parsync。 | medium，因為需要重新調整檢查項目和報告格式。 |
| [BigBodyCobain/Shadowbroker](https://github.com/BigBodyCobain/Shadowbroker) | PrivHound 提供圖形化的攻擊路徑展示，而 Shadowbroker 更加注重數據的收集和分析。 | 如果你的需求是對數據進行深入分析而非簡單的安全檢查，則 Shadowbroker 可能更合適。 | high，因為需要重新設計整個安全檢查流程。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PrivHound** | **parsync** | **Shadowbroker** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | PrivHound 專注於 Windows 本地特權提升，而 parsync 更偏向於一般的安全檢查。 | PrivHound 提供圖形化的攻擊路徑展示，而 Shadowbroker 更加注重數據的收集和分析。 |
> | 遷移成本 | - | medium，因為需要重新調整檢查項目和報告格式。 | high，因為需要重新設計整個安全檢查流程。 |
> | 適用場景 | 主要場景 | 如果你的團隊需要更全面的安全檢查，而不僅僅是針對特權提升的風 | 如果你的需求是對數據進行深入分析而非簡單的安全檢查，則 Sh |
...
- **[HIGH]** 在某些 Windows 環境中，PowerShell 的執行策略可能會阻止腳本運行。
  - 解法：需要手動設置執行策略為 Bypass。
- [MEDIUM] 某些檢查項目可能需要額外的權限才能執行。
  - 解法：確保以管理員身份運行 PowerShell。
- [MEDIUM] 在大型環境中，檢查可能需要較長時間才能完成。
  - 解法：考慮分批執行檢查以提高效率。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 中型企業的安全審計團隊 | 非常適合 | 能夠快速識別和修復潛在的安全漏洞，提升審計效率。 |
| 小型開發團隊進行安全測試 | 適合 | 提供簡單的檢查項目，易於上手。 |
| 大型企業的安全運維團隊 | 普通 | 雖然能夠處理多種檢查，但在大型環境中可能會遇到性能瓶頸。 |
| 個人開發者進行安全測試 | 不適合 | 功能較為專業，對於個人使用者來說可能過於複雜。 |
...
> [!tip] 投入 vs 回報
> 花 2 小時學習，1 小時整合，得到快速識別安全漏洞的效果，值得考慮。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：PrivHound 本身不需要高權限運行，但在執行檢查時可能會存取敏感資料。對於依賴的 PowerShell 環境，需確保其安全性。
...
> [!note]- 直接競品（同子分類：安全檢查工具）
> ```dataview
> TABLE stars, stars_per_day AS "Stars/天", install_complexity AS "難度", use_case AS "用途"
> FROM "Repos"
> WHERE subcategory = "安全檢查工具" AND file.name != "dazzyddos--PrivHound"
> SORT stars DESC
> ```
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["安全漏洞","MITRE ATT&CK","特權提升"];
