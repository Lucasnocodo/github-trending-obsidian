...
star_history: "2026-03-10:301,2026-03-11:302,2026-03-13:347,2026-03-14:363,2026-03-15:367,2026-03-16:370,2026-03-17:373"
tags:
  - github
  - "category/安全"
  - "lang/powershell"
...
**373** stars · **37** stars/天 · 建立 10 天前 · PowerShell · 未標註授權
...
> **適合** 需要針對 Windows 環境進行特權提升分析的安全專業人士。
> **一句話重點** PrivHound 讓 Windows 本地特權提升的安全分析變得更加可視化和直觀。
...
>     .where(p => p.subcategory === "安全測試" && p.file.name !== "dazzyddos--PrivHound" && p.status !== "archived")
...
> **結論** 花 5 小時學習，3 小時整合，得到針對 Windows 環境的特權提升分析，值得嘗試。

> [!abstract] 核心創新
> PrivHound 將 Windows 本地特權提升建模為互聯攻擊路徑，提供更直觀的視覺化分析。

## 專案簡介

PrivHound 是一個專注於 Windows 本地特權提升的工具，透過圖形化方式展示攻擊路徑，讓使用者能夠更直觀地理解潛在的安全風險。它檢查多種權限提升技術，包括可修改的服務、可寫的服務二進位檔、DLL 劫持等，並將這些檢查結果以圖形方式呈現。使用者只需執行相關的 PowerShell 指令，即可獲得一個完整的攻擊路徑圖，這比傳統的文字報告更具可讀性和操作性。此工具的賣點在於其可視化的攻擊路徑，讓安全專家能夠快速識別並修補漏洞。技術上，PrivHound 使用 PowerShell 進行系統檢查，並利用 Cypher 查詢語言來處理和展示數據，這樣的選擇使得它能夠有效地與 BloodHound 整合，並利用其強大的圖形化能力。

與其他工具相比，如 BloodHound 本身或其他安全掃描工具，PrivHound 提供了一種更為專注的視角，專門針對 Windows 環境中的特權提升問題。這使得它在面對特定的安全需求時，能夠提供更具針對性的解決方案。使用 PrivHound 時，使用者可能會遇到一些常見問題，如輸出格式不正確等，這在其熱門 Issues 中已有反映。該專案目前仍在活躍開發中，適合需要針對 Windows 環境進行深入安全分析的團隊或個人。總體來看，PrivHound 是一個值得關注的工具，特別是在 Windows 安全性測試的領域。

**技術棧**：`PowerShell` · `Cypher`

## 重點功能

- 圖形化攻擊路徑 — 將 Windows 本地特權提升的各種技術以圖形方式展示，便於理解和分析。
- 多種檢查項目 — 包含 30 種不同的檢查項目，如可寫服務二進位檔、DLL 劫持等，全面評估安全風險。
- Cypher 數據處理 — 使用 Cypher 查詢語言來高效處理和展示數據，提升查詢效率。
- PowerShell 整合 — 透過 PowerShell 指令進行系統檢查，易於在 Windows 環境中部署和使用。
- 開源社群支持 — 透過 GitHub 社群的反饋和貢獻，持續改進和更新功能。
...
> [!tip] 爆紅原因
> 建立 10 天內累積 373 stars（37/天），forks 37（9.9%），顯示出穩定的增長潛力。作者 dazzyddos 在安全領域有一定的背景，這個專案解決了傳統文字報告無法直觀展示攻擊路徑的問題，讓安全專家能更有效地識別風險。這個工具的推出正值安全需求日益增加的時期，特別是在 Windows 環境中。forks/stars 比率接近 10%，顯示出不少使用者對此專案有實際修改和使用的需求。

## 適合誰使用

**目標受眾**：需要針對 Windows 環境進行特權提升分析的安全專業人士。

> [!example] 使用場景
> - 安全分析師用它來快速識別 Windows 環境中的特權提升路徑，因為可視化的圖形化結果能讓他們更快地制定修補計畫。
> - 滲透測試工程師用它來模擬攻擊路徑，因為它能幫助他們發現潛在的安全漏洞，從而提高測試的全面性。
> - 系統管理員用它來檢查服務的安全性，因為它提供了詳細的檢查項目，能有效降低系統被攻擊的風險。

## 架構分析

PrivHound 採用 PowerShell 腳本進行系統檢查，並使用 Cypher 查詢語言來處理和展示數據。這樣的架構設計使得它能夠有效整合 BloodHound 的圖形化能力，並提供針對 Windows 環境的特權提升分析。資料流中，PowerShell 腳本負責收集系統信息，然後將這些信息轉換為可供 Cypher 查詢的格式。

選擇 PowerShell 是因為它在 Windows 環境中的原生支持，這樣可以減少依賴和部署複雜度。相對於其他語言，PowerShell 更適合進行系統層級的操作，但這也意味著使用者需要具備一定的 PowerShell 知識。整體架構的擴展性有限，因為它主要依賴於 Windows 環境的特性，未來若要支持其他平台將需要重構。

## 技術深入分析

PrivHound 的核心技術機制是利用 PowerShell 腳本進行系統檢查，並透過 Cypher 查詢語言來處理和展示數據。這種設計使得它能夠快速收集系統信息並生成可視化的攻擊路徑圖。效能方面，PrivHound 能夠處理多種檢查項目，具體的性能數據尚未提供，但可以推測在小型至中型環境中運行流暢。選擇 PowerShell 作為主要語言是因為它在 Windows 環境中的原生支持，這樣可以簡化部署過程，但也限制了其跨平台的能力。技術風險方面，PrivHound 依賴於 Windows 的特性，未來若要支持其他平台可能需要進行重大重構。整合方面，PrivHound 的使用者需要具備一定的 PowerShell 知識，這可能會增加學習成本，但對於熟悉 Windows 環境的安全專業人士來說，這並不會造成太大困難。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了基本的使用說明，但缺乏詳細的範例。安裝過程相對順暢，但使用者需要具備 PowerShell 的基本知識。文件目前僅提供英文版本，對於非英語使用者可能會造成一定的障礙。

## 優缺點分析

> [!success] 優點
> - 提供直觀的圖形化攻擊路徑，便於安全分析。
> - 涵蓋多種特權提升檢查項目，全面評估安全風險。
> - 開源且免費，適合預算有限的團隊使用。
...
> - 目前僅有初步的功能，未來可能需要更多的檢查項目和功能擴展。
> - 需要一定的 PowerShell 知識才能有效使用和修改腳本。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [BloodHound/BloodHound](https://github.com/BloodHound/BloodHound) | BloodHound 提供更全面的攻擊路徑分析，但缺乏 PrivHound 的專注於本地特權提升的視角。 |
| [PowerView/PowerView](https://github.com/PowerView/PowerView) | PowerView 主要用於 Active Directory 環境的資訊收集，而 PrivHound 專注於 Windows 本地特權提升，適用場景不同。 |
| [Cobalt Strike/Cobalt Strike](https://github.com/Cobalt Strike/Cobalt Strike) | Cobalt Strike 是一個商業滲透測試工具，功能強大但成本高，而 PrivHound 是免費的開源選擇，適合預算有限的團隊。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [BloodHound/BloodHound](https://github.com/BloodHound/BloodHound) | BloodHound 提供全面的攻擊路徑分析，涵蓋多種攻擊技術，但缺乏 PrivHound 專注於本地特權提升的深度。 | 如果需要全面的攻擊路徑分析而不僅限於本地特權提升，BloodHound 是更好的選擇。 | medium，因為需要重新學習 BloodHound 的使用方法和界面。 |
| [PowerView/PowerView](https://github.com/PowerView/PowerView) | PowerView 專注於 Active Directory 環境的資訊收集，而 PrivHound 專注於 Windows 本地特權提升，適用場景不同。 | 如果主要工作在 Active Directory 環境中，PowerView 會更合適。 | low，因為 PowerView 和 PrivHound 都使用 PowerShell，轉換相對簡單。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PrivHound** | **BloodHound** | **PowerView** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | BloodHound 提供全面的攻擊路徑分析，涵蓋多種攻擊技術，但缺乏 PrivHound 專注於本地特權提升的深度。 | PowerView 專注於 Active Directory 環境的資訊收集，而 PrivHound 專注於 Windows 本地特權提升，適用場景不同。 |
> | 遷移成本 | - | medium，因為需要重新學習 BloodHound 的使用方法和界面。 | low，因為 PowerView 和 PrivHound 都使用 PowerShell，轉換相對簡單。 |
> | 適用場景 | 主要場景 | 如果需要全面的攻擊路徑分析而不僅限於本地特權提升，Blood | 如果主要工作在 Active Directory 環境中，P |
...
- **[HIGH]** 輸出格式 BloodHound-customnodes 無法生成 privhound_customnodes.json 檔案
  - 解法：目前無法解決，需等待更新
- [MEDIUM] 在某些 Windows 環境中執行時可能會遇到權限問題
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 小型企業的 IT 安全團隊 | 非常適合 | 提供針對 Windows 環境的特權提升分析，幫助團隊識別風險。 |
| 大型企業的安全測試部門 | 普通 | 雖然功能強大，但可能需要更多的擴展以滿足大型環境的需求。 |
| 個人安全研究者 | 非常適合 | 開源且免費，適合進行個人研究和實驗。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學習，3 小時整合，得到針對 Windows 環境的特權提升分析，值得嘗試。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：PrivHound 本身不需要高權限運行，但在執行時需以管理員身份運行，可能會存取敏感資料。依賴鏈的安全性尚未評估，使用時需謹慎。
...
> FROM "Repos"
> WHERE subcategory = "安全測試" AND file.name != "dazzyddos--PrivHound"
> SORT stars DESC
> ```
