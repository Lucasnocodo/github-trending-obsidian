...
use_case: "將 Windows 本地權限提升的攻擊路徑以圖形方式建模，幫助安全研究人員識別潛在的安全漏洞。"
priority: medium
ring: assess
...
aliases:
  - "PrivHound"
  - "dazzyddos/PrivHound"
  - "將 Windows 本地權限提升的攻擊路徑以圖形方式建模，幫助安全研究人員識別潛在的安全漏洞。"
---

# PrivHound

**470** stars · **12** stars/天 · 建立 40 天前 · PowerShell · 未標註授權
...
> **授權** 未標註授權 (風險較高) · **維護** Moderate (最後推送 29 天前) · **貢獻者** 2 人 · **參與度** Low
> **適合** 專注於 Windows 環境安全評估的 IT 安全專家和滲透測試工程師。
> **一句話重點** PrivHound 透過圖形化的方式，讓安全專家能更快速地識別和分析 Windows 系統中的安全漏洞。
...
> [!abstract] 核心創新
> 提供了一種圖形化的方式來展示 Windows 本地權限提升的攻擊路徑，改善了傳統文字描述的不足。

## 專案簡介

PrivHound 是一個專為 BloodHound 設計的 OpenGraph 收集器，專注於 Windows 本地權限提升的攻擊路徑。使用者可以透過執行 PowerShell 指令來收集系統的安全資訊，並將這些資訊轉換為圖形化的攻擊路徑，這樣比起傳統的文字描述更易於理解和分析。核心機制是檢查多種潛在的安全漏洞，例如可修改的服務權限、可寫的服務二進位檔和 DLL 劫持等，這些檢查對應到 MITRE ATT&CK 框架中的技術編號，幫助使用者快速識別風險。這個工具的賣點在於其圖形化的展示方式，讓安全專家能夠更直觀地理解複雜的攻擊路徑。技術上，PrivHound 使用 PowerShell 進行系統檢查，並利用 Cypher 語言來查詢和展示圖形數據，這樣的設計使得資料的處理和展示都非常高效。

與其他工具相比，如 0xGF/boneyard 和 AgentSeal/codeburn，PrivHound 提供了更直觀的視覺化界面，讓使用者能夠輕鬆地追蹤和分析攻擊路徑。實際使用中，PrivHound 可以處理大量的系統檢查，並且能夠在短時間內生成圖形化的報告，這對於需要快速評估安全狀況的團隊來說非常有價值。這個專案目前處於活躍開發階段，社群相對健康，解決率為 50%。未來幾個月可能會增加更多檢查項目和功能，進一步提升其使用價值。適合需要進行安全評估的中小型團隊，尤其是在 Windows 環境中工作的人員。
...
- 多種檢查項目 — 包含 30 種不同的安全檢查，涵蓋多種潛在的漏洞。
- MITRE ATT&CK 對應 — 每個檢查項目都對應到 MITRE ATT&CK 框架，方便安全專家進行分析。
- PowerShell 支援 — 使用 PowerShell 進行系統檢查，方便與現有的 Windows 環境整合。
- Cypher 查詢語言 — 利用 Cypher 進行圖形數據的查詢和展示，提升數據處理效率。
...
> [!tip] 爆紅原因
> 建立 40 天內累積 470 stars（12/天），forks 46（9.8%），這顯示出相對穩定的增長。作者 Signum21 和 dazzyddos 在安全領域有一定的經驗，這個專案解決了傳統文字描述在安全分析中的不足，提供了一種更直觀的方式來識別和分析 Windows 系統中的安全漏洞。這個工具的出現正好填補了市場上對於圖形化安全分析工具的需求，特別是在 BloodHound 生態系統中。其 forks/stars 比率接近 10%，顯示出有不少使用者在實際修改和使用這個工具。

## 適合誰使用

**目標受眾**：專注於 Windows 環境安全評估的 IT 安全專家和滲透測試工程師。

> [!example] 使用場景
> - 安全研究員用它來分析 Windows 系統的本地權限提升路徑，因為它提供了直觀的圖形化界面，能快速識別潛在的安全漏洞。
> - IT 安全團隊用它來定期檢查系統的安全狀態，因為它能有效整合多種檢查技術並生成可視化報告，節省了手動檢查的時間。
> - 滲透測試工程師用它來制定攻擊路徑，因為它能幫助他們更清晰地理解不同權限之間的關係，從而設計出更有效的測試策略。

## 架構分析

PrivHound 採用 PowerShell 作為主要執行環境，並利用 Cypher 查詢語言來處理和展示圖形數據。這樣的設計使得工具能夠快速收集系統資訊並轉換為可視化的攻擊路徑。資料流從 PowerShell 腳本開始，收集系統檢查結果，然後通過 Cypher 查詢將結果轉換為圖形格式。這種架構的優勢在於其高效性和直觀性，但也可能在處理大量數據時面臨性能瓶頸。整體來說，這種設計使得安全分析變得更為簡單和有效。

## 技術深入分析

PrivHound 的核心技術機制在於其使用 PowerShell 來執行系統檢查，並利用 Cypher 查詢語言來處理和展示數據。這樣的設計使得工具能夠快速收集和分析大量的安全資訊，並將其轉換為圖形化的攻擊路徑。效能方面，PrivHound 能夠處理多達 30 種不同的檢查項目，並在短時間內生成報告，這對於需要快速評估安全狀況的團隊來說非常重要。設計上，選擇 PowerShell 主要是因為它在 Windows 環境中的普遍性，而 Cypher 的使用則提高了數據查詢的效率。這種選擇的代價在於對於不熟悉這些技術的使用者來說，可能會有一定的學習曲線。技術風險方面，PrivHound 可能在處理極大量數據時面臨性能瓶頸，這可能會影響其在大型企業環境中的應用。整合方面，PrivHound 與主流的安全工具鏈相容性良好，能夠輕鬆融入現有的安全評估流程。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的介紹和快速入門指引，安裝過程相對順暢，使用者只需執行 PowerShell 腳本即可開始使用。文件中缺乏多語言支持，可能對非英語使用者造成困難。

## 優缺點分析

> [!success] 優點
> - 圖形化展示使得安全分析更直觀。
> - 多種檢查項目覆蓋廣泛，能識別多種潛在漏洞。
> - 與 MITRE ATT&CK 框架的對應提升了分析的專業性。

> [!danger] 缺點
> - 僅限於 Windows 環境，無法跨平台使用。
> - 需要一定的 PowerShell 知識，對新手有學習障礙。
> - 目前社群活躍度尚需提升，可能影響未來的支持和更新。

> [!warning] 注意事項
> - 僅支援 Windows 環境，無法在其他作業系統上運行。
> - 需要 PowerShell 環境，對於不熟悉 PowerShell 的使用者可能有學習曲線。
> - 目前僅有一個開放的 Issue，可能表示社群活躍度尚需提升。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 提供類似的安全檢查功能，但主要以文字報告形式呈現，缺乏圖形化界面。 |
| [AgentSeal/codeburn](https://github.com/AgentSeal/codeburn) | 專注於特定的攻擊向量，功能較為單一，無法提供全面的權限提升路徑分析。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0xGF/boneyard](https://github.com/0xGF/boneyard) | 主要以文字報告形式呈現，缺乏圖形化界面，對於視覺化需求的使用者不夠友好。 | 如果需要一個簡單的文字報告工具，而不需要圖形化展示。 | low，因為兩者的功能有部分重疊，轉換相對簡單。 |
| [AgentSeal/codeburn](https://github.com/AgentSeal/codeburn) | 專注於特定的攻擊向量，功能較為單一，無法提供全面的權限提升路徑分析。 | 如果你的需求非常專注於某一特定攻擊向量，而不需要全面的分析。 | medium，因為需要重新調整分析方法和工具使用習慣。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **PrivHound** | **boneyard** | **codeburn** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 主要以文字報告形式呈現，缺乏圖形化界面，對於視覺化需求的使用者不夠友好。 | 專注於特定的攻擊向量，功能較為單一，無法提供全面的權限提升路徑分析。 |
> | 遷移成本 | - | low，因為兩者的功能有部分重疊，轉換相對簡單。 | medium，因為需要重新調整分析方法和工具使用習慣。 |
> | 適用場景 | 主要場景 | 如果需要一個簡單的文字報告工具，而不需要圖形化展示。 | 如果你的需求非常專注於某一特定攻擊向量，而不需要全面的分析。 |
...
> [!tip] 採用建議
> 適合安全研究和測試使用，但不建議在生產環境中使用。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在某些 Windows 版本上可能無法正確執行 PowerShell 腳本
  - 解法：檢查 PowerShell 執行策略，確保允許執行腳本
- [MEDIUM] 圖形生成過程中可能會出現性能瓶頸
  - 解法：在小型測試環境中先行測試，避免在大型系統上直接運行

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司進行安全評估 | 非常適合 | PrivHound 提供了簡單易用的工具，適合小型團隊快速評估安全狀況。 |
| 大型企業的安全團隊進行全面性安全檢查 | 普通 | 雖然功能強大，但在處理大量數據時可能會遇到性能瓶頸。 |
| 滲透測試工程師進行攻擊路徑分析 | 非常適合 | 圖形化展示能幫助他們快速理解複雜的權限關係。 |
| 不熟悉 PowerShell 的使用者 | 不適合 | 需要一定的 PowerShell 知識，對新手有學習障礙。 |
...
> [!warning] 安全性快速掃描
> 低風險：PrivHound 本身不需要高權限運行，但會存取系統的安全資訊，使用時需注意資料的保護。
...
**社群活躍度**：社群活躍度尚需提升，目前僅有一個開放的 Issue。
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["MITRE ATT&CK","特權提升","安全漏洞分析"];
