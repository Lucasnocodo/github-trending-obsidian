...
**11.8k** stars · **538** stars/天 · 建立 22 天前 · Python · MIT
...
> [!info] 速覽
> **安裝難度** Easy · **專案狀態** Recent · **熱度** Hot (538 stars/day)
> **授權** MIT (商業友好) · **維護** Active (最後推送 1 天前) · **貢獻者** 5+ 人 · **參與度** Low
> **適合** 希望減少手動操作並提升自動化水平的開發者和團隊。
> **一句話重點** Browser Harness 的自我修復能力顯示了自動化工具在日常工作中的巨大潛力。
...
> [!abstract] 核心創新
> 這個專案的核心創新在於其自我修復的能力，能夠讓 LLM 自動生成所需的輔助代碼。

## 專案簡介

Browser Harness 是一個用於將大型語言模型（LLM）直接連接到真實瀏覽器的工具，透過一個輕量的 CDP（Chrome DevTools Protocol）介面來執行任務。用戶只需設置一次，之後 LLM 便能在執行過程中自動生成所需的輔助代碼，這樣的設計使得每次執行都能持續改進，提升自動化的靈活性和效率。核心指令 `browser-harness` 允許用戶啟動這個工具，並且在執行任務時，LLM 可以即時編寫缺失的輔助函數，確保任務的順利完成。這種自我修復的特性使得用戶幾乎不需要再手動干預，從而大幅提高了工作效率。

技術上，Browser Harness 使用 Python 實作，依賴於如 `cdp-use` 和 `websockets` 等庫來實現與 Chrome 的即時通訊。這樣的選擇使得它能夠在保持輕量的同時，提供高效的性能。

與其他類似工具相比，如 `0x0funky/agent-sprite-forge` 和 `HKUDS/OpenHarness`，Browser Harness 的特點在於其自我學習的能力，能夠根據實際執行的任務自動生成技能，這在需要頻繁變動的網頁操作中尤為重要。

在實際使用中，Browser Harness 能夠處理多種瀏覽器任務，並且支援多達 3 個並行瀏覽器的免費雲端使用，這對於需要同時執行多個任務的用戶來說非常方便。雖然目前的開發仍在進行中，但它的社群活躍度和開放的貢獻機制使得未來的功能擴展充滿潛力。

總的來說，Browser Harness 是一個適合需要高效自動化瀏覽器操作的開發者，特別是那些希望減少手動操作並提升工作流程的團隊。隨著未來的版本更新，這個工具有潛力成為自動化領域的重要選擇。

**技術棧**：`Python 3.11` · `cdp-use 1.4.5` · `websockets 15.0.1`

## 重點功能

- 自我修復能力 — LLM 能夠在執行過程中自動生成缺失的輔助代碼，提升任務執行的靈活性。
- 輕量 CDP 介面 — 透過簡單的 websocket 連接到 Chrome，無需複雜的設置。
- 免費雲端使用 — 提供 3 個並行瀏覽器的免費雲端服務，適合小型團隊和個人使用。
- 可擴展的技能生成 — 用戶可以輕鬆添加新的域技能，提升工具的適用範圍。
- 即時通訊 — 使用 websockets 實現與 Chrome 的即時通訊，確保快速響應。
...
3. 連接到 Chrome 瀏覽器
```bash
在 Chrome 中啟用遠端調試並允許連接
```

## 為什麼值得關注

> [!tip] 爆紅原因
> 建立 22 天內累積 11841 stars（538/天），forks 1072（9.1%），顯示出強勁的增長潛力。這個專案由 sauravpanda 和其他幾位貢獻者共同開發，解決了 LLM 在瀏覽器操作中缺乏靈活性和自我修復能力的痛點。之前的方案往往需要手動編寫大量的輔助代碼，效率低下，而 Browser Harness 則通過自動生成技能來改進這一點。近期的社群討論和問題反饋也顯示出用戶對於這個工具的需求和期待，進一步推動了其流行。這個工具的成功也反映了自動化和智能化技術在日常工作中的重要性，尤其是在需要高頻率和高靈活性的場景中。

## 適合誰使用

**目標受眾**：希望減少手動操作並提升自動化水平的開發者和團隊。

> [!example] 使用場景
> - 後端工程師用它來自動化網頁數據抓取，因為可以減少手動操作的時間，並提高數據準確性。
> - 產品經理用它來測試網頁功能，因為能夠快速生成測試腳本，並自動修復錯誤。
> - 行銷專員用它來自動化社交媒體發文，因為可以同時管理多個帳號，提升工作效率。

## 架構分析

Browser Harness 採用輕量級的架構，主要由四個核心文件組成，確保了簡單的安裝和使用流程。其核心是通過 CDP 與 Chrome 瀏覽器進行通訊，使用 websockets 實現即時互動。資料流方面，LLM 在執行任務時會自動生成所需的輔助代碼，這樣的設計使得用戶無需手動編寫代碼，提升了使用的便利性。

選擇 Python 作為開發語言，因其擁有豐富的庫和良好的社群支持，這使得開發和維護變得更加高效。這種架構的代價在於需要用戶在 Chrome 中啟用遠端調試，這對於某些用戶來說可能會造成不便。整體來說，這個架構設計旨在提供高效的自動化解決方案，並且能夠隨著用戶需求的變化而靈活擴展。

## 技術深入分析

Browser Harness 的核心技術機制在於其利用 CDP 與 Chrome 瀏覽器的即時通訊能力，並通過 websockets 實現高效的數據傳輸。這使得 LLM 能夠在執行任務時，根據實際情況自動生成所需的輔助代碼，這在傳統的自動化工具中是難以實現的。效能方面，這個工具能夠處理多達 3 個並行的瀏覽器實例，並且在執行過程中能夠快速響應用戶的需求。設計上選擇 Python 作為開發語言，這不僅使得開發過程更加靈活，也降低了學習曲線，因為 Python 在自動化和數據處理領域有著廣泛的應用。

這個選擇的代價在於可能需要用戶對 Python 環境有一定的了解。技術風險方面，隨著用戶需求的增加，如何保持系統的穩定性和性能將是未來需要面對的挑戰。此外，對於外部 API 的依賴程度也需謹慎評估，以避免潛在的服務中斷風險。整合方面，Browser Harness 能夠輕鬆與現有的開發工具鏈結合，並且在 CI/CD pipeline 中的友好度也相對較高，這使得團隊能夠快速上手並投入使用。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰，提供了詳細的安裝步驟和使用範例。安裝過程順暢，無明顯坑點。文件中有針對新手的指導，幫助用戶快速上手。
...
> - 需要用戶手動啟用遠端調試，對某些用戶不夠友好。

> [!warning] 注意事項
> - 僅支援 Python 3.11 以上版本。
> - 需要 Chrome 瀏覽器並啟用遠端調試功能。
> - 目前功能仍在開發中，可能存在不穩定性。
...
| 工具 | 差異 |
| --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供類似的自動化功能，但缺乏自我修復的能力，使用者需手動編寫輔助代碼。 |
| [HKUDS/OpenHarness](https://github.com/HKUDS/OpenHarness) | 雖然也支援 LLM 操作，但在技能生成方面不如 Browser Harness 靈活。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 使用 Rust 實作，記憶體用量少，但缺乏自我修復的能力，需手動編寫輔助代碼。 | 如果需要高效能且對記憶體使用有嚴格要求的環境。 | medium，因為需要重寫部分自動化邏輯。 |
| [HKUDS/OpenHarness](https://github.com/HKUDS/OpenHarness) | 提供類似的自動化功能，但在技能生成方面不如 Browser Harness 靈活。 | 如果已經在使用 HKUDS 的其他工具，並希望保持一致性。 | low，因為 API 和功能相似。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **browser-harness** | **agent-sprite-forge** | **OpenHarness** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 使用 Rust 實作，記憶體用量少，但缺乏自我修復的能力，需手動編寫輔助代碼。 | 提供類似的自動化功能，但在技能生成方面不如 Browser Harness 靈活。 |
> | 遷移成本 | - | medium，因為需要重寫部分自動化邏輯。 | low，因為 API 和功能相似。 |
> | 適用場景 | 主要場景 | 如果需要高效能且對記憶體使用有嚴格要求的環境。 | 如果已經在使用 HKUDS 的其他工具，並希望保持一致性。 |
...
> [!tip] 採用建議
> 適合個人或小型團隊的實驗性項目，不建議用於生產環境的核心路徑上。

## 已知陷阱

> [!bug] 踩坑才知道的問題

- **[HIGH]** 在某些環境下，無法正確連接到 Chrome 瀏覽器，導致無法執行任務。
  - 解法：檢查 Chrome 的遠端調試設置是否正確。
- [MEDIUM] 自動生成的技能可能不夠完善，需手動調整。
  - 解法：在生成後檢查並手動編輯技能文件。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端自動化任務 | 非常適合 | 能夠快速自動化多種瀏覽器操作，減少人力成本。 |
| 大型企業的核心業務系統 | 不適合 | 目前仍在 beta 階段，穩定性不足。 |
| 個人開發者的自動化工具 | 適合 | 輕量級設計，易於上手，適合快速實驗。 |
| 需要高效能的數據抓取任務 | 普通 | 雖然支援多並行瀏覽器，但在高負載下可能性能下降。 |
...
> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限，僅需訪問 Chrome 瀏覽器的遠端調試功能，不會存取敏感資料。
...
> | [#329](https://github.com/browser-use/browser-harness/issues/329) | Feat: Add model-compatibility.json registry for self-hostabl | 0 | 0 |
> | [#328](https://github.com/browser-use/browser-harness/issues/328) | Feat: Improve --doctor to detect and warn about Snap-based C | 0 | 0 |
