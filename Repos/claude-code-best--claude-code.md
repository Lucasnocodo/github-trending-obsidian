...
star_history: "2026-04-02:7790,2026-04-02:7914,2026-04-03:10736,2026-04-03:10850,2026-04-04:13023,2026-04-04:13026,2026-04-05:13577,2026-04-05:13580,2026-04-06:13897,2026-04-06:13908,2026-04-07:14198,2026-04-07:14224,2026-04-08:14619"
tags:
  - github
  - "category/開發工具"
  - "lang/typescript"
...
**14.6k** stars · **2.1k** stars/天 · 建立 7 天前 · TypeScript · 未標註授權
...
> **適合** 需要快速開發和測試 AI 相關功能的開發團隊，尤其是熟悉 Bun 環境的工程師。
> **一句話重點** 這個專案不僅提供了 Claude Code 的功能，還通過開源的方式讓開發者能夠自由擴展和修改。
...
> **結論** 花 5 小時學、3 小時整合，得到靈活的 AI 開發環境，值得投入。

> [!abstract] 核心創新
> 提供了一個完整的開源版本的 Claude Code CLI 工具，並修復了 TypeScript 類型。

## 專案簡介

Claude Code Best (CCB) 是一個基於 Claude Code 的 CLI 工具，旨在提供一個互動式的 AI 編程助手。用戶可以通過 `ccb` 命令直接啟動該工具，並使用 `bun run dev` 進入開發模式。這個工具的核心賣點在於其完整的功能復現和企業級的可靠性，並且支持多種 API 兼容性，讓開發者能夠輕鬆連接到不同的 AI 服務。技術上，CCB 使用 Bun 作為執行環境，並採用多文件打包的方式來構建，這樣可以有效管理依賴和提升性能。與其他類似工具相比，如 OpenAI 的 CLI 工具，CCB 提供了更靈活的 API 連接選項，並且支持自定義功能開關，這使得它在特定場景下更具優勢。

使用者在運行時需要注意環境變數的設定，特別是在網絡環境不佳的情況下，這可能會影響到安裝和運行的穩定性。這個專案目前處於活躍開發中，社群活躍度高，問題解決率達到 82%。未來的發展方向可能會集中在功能擴展和性能優化上。對於需要快速開發和測試 AI 相關功能的團隊，CCB 是一個值得考慮的選擇，但對於不熟悉 Bun 環境的開發者，可能需要額外的學習時間。

**技術棧**：`TypeScript` · `Bun` · `Node.js`

## 重點功能

- 即時啟動 — 使用 `ccb` 命令直接啟動工具，無需克隆倉庫。
- 多文件打包 — 使用 `bun run build` 進行構建，支持多個 chunk 文件的管理。
- API 兼容性 — 支持多種 AI 服務的 API 連接，方便開發者使用。
- 功能開關 — 透過環境變數啟用不同功能，如 `FEATURE_BUDDY=1`。
- 教學模式 — 提供 `/teach-me` 命令幫助用戶理解各個模塊的功能。
...
> [!tip] 爆紅原因
> 建立 7 天就累積 14619 stars（2098/天），forks 14484（99.1%），這顯示出極高的用戶參與度。這個專案的主要貢獻者來自於活躍的開源社群，並且其目標是復現 Anthropic 的 Claude Code CLI 工具，這在過去是缺乏良好開源解決方案的。社群中的熱門討論和反饋也促進了其快速發展，特別是針對功能的改進和錯誤修復。技術上，Bun 的流行使得這個工具能夠在性能上有更好的表現，並且支持多種語言的開發，這也是其受歡迎的原因之一。

## 適合誰使用

**目標受眾**：需要快速開發和測試 AI 相關功能的開發團隊，尤其是熟悉 Bun 環境的工程師。

> [!example] 使用場景
> - 後端工程師用它來快速構建和測試 AI 相關的 CLI 工具，因為它支持多種 API 並且能夠快速啟動開發環境。
> - AI 研究人員用它來調試和優化自己的 AI 模型，因為它提供了完整的開發和測試功能，並且支持即時反饋。
> - 新手開發者用它來學習如何與 AI 服務互動，因為其內建的教學功能能夠引導他們逐步掌握相關技術。

## 架構分析

CCB 採用 Bun 作為執行環境，這使得它在性能上有顯著的提升。架構上使用多文件打包的方式來管理依賴，這樣可以有效減少單一文件的大小，提升加載速度。資料流方面，使用者通過 `ccb` 命令啟動工具，然後可以進行 API 連接和功能開關的設置。

這樣的設計使得工具的擴展性和靈活性大大提高。選擇 Bun 而非傳統的 Node.js 主要是為了提升性能和開發效率，但這也要求使用者必須熟悉新的環境。擴展性方面，CCB 目前支持多種 API 連接，未來可能會進一步增加更多的功能模塊。

## 技術深入分析

CCB 的核心技術機制在於其使用 Bun 作為執行環境，這使得它在性能上有顯著的提升。Bun 的非同步處理能力和快速啟動時間使得 CCB 能夠在開發過程中提供即時反饋。效能上，CCB 能夠處理大量的 API 請求，並且在開發模式下的冷啟動時間非常短，通常在幾秒內。設計上，選擇 Bun 而非傳統的 Node.js 主要是為了提升性能和開發效率，但這也要求使用者必須熟悉新的環境。CCB 的依賴樹相對簡單，主要依賴於 Bun 和一些輔助庫，這降低了維護的複雜度。技術風險方面，未來在擴展功能時可能會遇到性能瓶頸，特別是在高並發的情況下。整合方面，CCB 可以輕鬆與現有的 CI/CD pipeline 整合，並且支持多種 IDE 的插件，這使得它在團隊協作中非常友好。

## 新手體驗

> [!info] 上手難度評估
> README 文件清晰且包含多個範例，讓新手能夠快速上手。安裝過程相對順暢，但需要注意 Bun 的版本要求。文件中有針對新手的教學指引，幫助他們理解如何使用工具。整體來說，花 30 分鐘能夠順利運行起來。
...
> - 快速啟動和開發，無需繁瑣的安裝步驟。
> - 支持多種 API 兼容性，靈活性強。
> - 社群活躍，問題解決率高，能夠快速獲得支持。

> [!danger] 缺點
> - 需要學習新的 Bun 環境，對新手來說有一定門檻。
> - 在某些地區的網絡環境不佳，可能影響使用體驗。
> - 不支持 Windows 環境，限制了使用者的範圍。

> [!warning] 注意事項
> - 需要最新版本的 Bun，否則可能會出現錯誤。
> - 在某些地區，GitHub 的網絡連接可能不穩定，影響安裝。
> - 不支持 Windows 環境，主要針對類 Unix 系統。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [openai/openai-cli](https://github.com/openai/openai-cli) | OpenAI 的 CLI 工具專注於其自家 API，功能較為單一，而 CCB 提供了多種 API 兼容性，靈活性更高。 |
| [anthropic/claude-code](https://github.com/anthropic/claude-code) | 官方的 Claude Code 提供了原始功能，但缺乏開源社群的支持和擴展性，CCB 則在此基礎上進行了改進和功能擴展。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [openai/openai-cli](https://github.com/openai/openai-cli) | OpenAI 的 CLI 工具專注於其自家 API，功能較為單一，而 CCB 提供了多種 API 兼容性，靈活性更高。 | 如果你的團隊主要使用 OpenAI 的 API，並且不需要其他服務的支持。 | low，因為兩者的 CLI 使用方式相似。 |
| [anthropic/claude-code](https://github.com/anthropic/claude-code) | 官方的 Claude Code 提供了原始功能，但缺乏開源社群的支持和擴展性，CCB 則在此基礎上進行了改進和功能擴展。 | 如果你需要官方支持的功能，並且不需要開源的靈活性。 | medium，因為需要重新配置 API 連接和功能設置。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **claude-code** | **openai-cli** | **claude-code** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | OpenAI 的 CLI 工具專注於其自家 API，功能較為單一，而 CCB 提供了多種 API 兼容性，靈活性更高。 | 官方的 Claude Code 提供了原始功能，但缺乏開源社群的支持和擴展性，CCB 則在此基礎上進行了改進和功能擴展。 |
> | 遷移成本 | - | low，因為兩者的 CLI 使用方式相似。 | medium，因為需要重新配置 API 連接和功能設置。 |
> | 適用場景 | 主要場景 | 如果你的團隊主要使用 OpenAI 的 API，並且不需要其 | 如果你需要官方支持的功能，並且不需要開源的靈活性。 |
...
- **[HIGH]** 在某些地區，GitHub 的網絡連接可能不穩定，影響安裝。
  - 解法：使用環境變數設定代理。
- [MEDIUM] 首次運行需要進行 API 配置，對新手來說可能有點困難。
  - 解法：參考 README 中的配置指引。
- [MEDIUM] 在開發模式下，某些功能可能不穩定。
  - 解法：建議在正式環境中使用構建版本。
...
| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 10 人以下的新創公司後端 API | 非常適合 | 提供快速開發和測試的環境，能夠靈活連接多種 API。 |
| 大型企業的核心系統開發 | 不適合 | 目前處於 beta 階段，穩定性和支持度不足。 |
| AI 研究團隊進行模型測試 | 適合 | 支持多種 API 連接，方便進行實驗和測試。 |
| 需要快速原型開發的開發者 | 非常適合 | 能夠快速啟動和測試新功能，提升開發效率。 |
...
> [!tip] 投入 vs 回報
> 花 5 小時學、3 小時整合，得到靈活的 AI 開發環境，值得投入。

## 安全性評估

> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限運行，且不存取敏感資料。依賴鏈中沒有已知的供應鏈風險，適合在 CI/CD pipeline 中使用。
...
> | 貢獻者 | Commits |
> | --- | --- |
> | [@claude-code-best](https://github.com/claude-code-best) | 195 |
...
> **活躍天數** 1 天 · **最新 commit** Merge branch 'refactor/ink-v2'
...
> | --- | --- | --- | --- |
> | [#13](https://github.com/claude-code-best/claude-code/issues/13) | [留影专用]: 大家想留影就到这里来吧 | 26 | 132 |
...
> | [#154](https://github.com/claude-code-best/claude-code/issues/154) | Crab Code -- 受本项目启发，正在用 Rust 重写 Claude Code | 1 | 4 |
> | [#131](https://github.com/claude-code-best/claude-code/issues/131) | 希望大佬研究pipe，这样可以复用官方agent teams land的代码实现群控. | 1 | 1 |
> | [#190](https://github.com/claude-code-best/claude-code/issues/190) | 和官方的vscode code插件不连通 | 0 | 0 |
