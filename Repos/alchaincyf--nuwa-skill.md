...
**16.3k** stars · **679** stars/天 · 建立 24 天前 · Python · MIT
...
> **適合** 需要從各領域名人中提取思維模式的創新型團隊或個人。
> **一句話重點** Nuwa 不僅是蒸餾名人的工具，更是提取認知架構的創新方法。
...
>     .where(p => p.subcategory === "思維模型提取" && p.file.name !== "alchaincyf--nuwa-skill" && p.status !== "archived")
...
>       p.ring || "assess"
>     ]));
>   } else { dv.paragraph("_目前 vault 中沒有其他 思維模型提取 類工具_"); }
> }
> ```
...
> [!abstract] 核心創新
> Nuwa 提供了一種全新的方式來提取名人的思維模式，而不是單純的角色扮演。

## 專案簡介

Nuwa 是一個可以蒸餾名人思維的工具，透過輸入一個名字，系統會自動進行調研，提煉出該名人的心智模型、決策啟發式和表達風格。這個過程包括六路並行的資料收集，涵蓋著作、訪談和社交媒體等多個來源，並且經過三重驗證，確保提煉出的模型具有預測力和排他性。使用者只需透過簡單的 CLI 指令，如 `npx skills add alchaincyf/nuwa-skill`，就可以安裝並開始使用。其核心賣點在於不僅僅是角色扮演，而是提取名人的認知架構，讓使用者能夠在複雜的問題上獲得名人的視角。技術上，Nuwa 使用 Python 和 Shell，並且依賴於多個 Agent 進行資料收集，這使得它在處理多樣化的資料來源時具備高效性。

相較於同類工具，如 [同事.skill](https://github.com/titanwings/colleague-skill)，Nuwa 的範圍更廣，能夠蒸餾各個領域的名人，而不僅限於身邊的同事。這使得它在需要多元思維和創新解決方案的場合特別有價值。使用者可以在各種情境下應用這些提煉出的思維方式，例如在商業決策、產品設計或個人發展上。儘管目前的功能已經相當強大，但仍有一些限制，例如無法捕捉名人的直覺和即時反應，這可能會影響某些情境下的準確性。整體來看，Nuwa 是一個值得探索的工具，特別適合需要多元視角的創新型團隊或個人。

**技術棧**：`Python` · `Shell`

## 重點功能

- 六路並行資料收集 — 同時從著作、播客、社交媒體等六個來源提取資訊。
- 三重驗證提煉 — 需跨多個領域出現、具預測力且排他的觀點才能被收錄。
- 多名人物技能 — 目前已蒸餾13位名人，使用者可直接安裝使用。
- 表達DNA提取 — 提取名人的語氣、節奏和用詞偏好。
- 誠實邊界標註 — 明確告知使用者技能的局限性，提升信任度。
...
> [!tip] 爆紅原因
> 建立 24 天內累積 16307 stars（679/天），forks 2596（15.9%），顯示出強烈的社群關注。作者 alchaincyf 是一位獨立開發者，過去的作品如同事.skill 已經證明了蒸餾人的可行性，這次的 Nuwa 則是進一步擴展了這一概念，讓使用者能夠從各領域的名人中提煉思維。這種創新方法解決了過去僅能依賴同事或身邊人的限制，並且在技術上利用了多元資料來源的優勢。社群的活躍度和開發者的背景都為這個專案的成功奠定了基礎。

## 適合誰使用

**目標受眾**：需要從各領域名人中提取思維模式的創新型團隊或個人。

> [!example] 使用場景
> - 產品經理用它來蒸餾馬斯克的思維，幫助分析產品路線圖，因為馬斯克的決策模型能提供獨特的視角。
> - 創業者用它來獲取保羅·格雷厄姆的建議，因為他的創業哲學能幫助他們在資金籌集上做出更好的決策。
> - 教育工作者用它來蒸餾費曼的教學方法，因為他的學習理念能夠提升學生的理解能力和興趣。

## 架構分析

Nuwa 的架構設計為多 Agent 並行運行，這樣的設計使得資料收集能夠在短時間內完成，並且能夠從多個來源獲取資訊。每個 Agent 負責不同的資料來源，並將結果整合到一起，形成最終的心智模型。這種方法的代價在於需要較高的計算資源來支持多路徑的資料收集，但這也使得最終的結果更為全面和準確。擴展性方面，隨著名人數量的增加，系統能夠輕鬆地添加新的 Agent 來處理新的資料來源，這使得 Nuwa 在未來的發展中具備良好的擴展性。

## 技術深入分析

Nuwa 的核心技術機制在於其多 Agent 的資料收集方式，這使得它能夠同時從多個來源獲取資訊，並且在資料的篩選和提煉上具備高效性。每個 Agent 專注於特定的資料來源，如著作、播客或社交媒體，這樣的設計使得資料的多樣性和全面性得以保證。效能方面，雖然多 Agent 的設計需要較高的計算資源，但這也使得最終的心智模型更為準確和可靠。設計上，選擇 Python 作為主要語言，因為其在資料處理和機器學習領域的強大生態系統，這樣的選擇雖然可能導致一定的學習曲線，但長期來看能夠帶來更好的擴展性和維護性。技術風險方面，隨著名人數量的增加，資料的質量控制將成為一個挑戰，這需要持續的監控和優化。整合方面，Nuwa 可以輕鬆與現有的開發工具鏈結合，並且在 CI/CD pipeline 中也能夠友好地運行，這使得它在實際應用中具備良好的靈活性。

## 新手體驗

> [!info] 上手難度評估
> README 文件提供了清晰的安裝指引和使用範例，讓新手能夠快速上手。安裝過程相對順暢，沒有明顯的坑。文件中有多語言版本，增加了可讀性和易用性。

## 優缺點分析

> [!success] 優點
> - 能夠從多位名人中提取思維模式，提供多元視角。
> - 資料收集過程透明，使用者可查看調研過程。
> - 使用簡單，安裝和操作都很方便。

> [!danger] 缺點
> - 目前支持的名人數量有限，可能無法滿足所有需求。
> - 無法捕捉名人的直覺和即時反應，可能影響某些情境的準確性。
> - 需要較高的計算資源來支持多 Agent 的運行。

> [!warning] 注意事項
> - 無法捕捉名人的直覺反應。
> - 只能基於公開資訊進行蒸餾，無法獲得私密見解。
> - 目前支持的名人數量有限，未來可能需要擴展。

## 類似工具比較

| 工具 | 差異 |
| --- | --- |
| [titanwings/colleague-skill](https://github.com/titanwings/colleague-skill) | 專注於蒸餾離職同事的思維，而 Nuwa 則能蒸餾各領域的名人，範圍更廣。 |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供多種 Agent 的整合，但不專注於思維模型的提取，適用場景不同。 |
...
| 工具 | 技術路線 | 選它的時機 | 遷移難度 |
| --- | --- | --- | --- |
| [titanwings/colleague-skill](https://github.com/titanwings/colleague-skill) | 專注於蒸餾離職同事的思維，使用者只能從身邊的人獲取資料，而 Nuwa 則能蒸餾各領域的名人，範圍更廣。 | 如果你的需求僅限於蒸餾身邊同事的經驗和思維，則可以選擇同事.skill。 | low，因為兩者的使用方式相似。 |
| [0x0funky/agent-sprite-forge](https://github.com/0x0funky/agent-sprite-forge) | 提供多種 Agent 的整合，但不專注於思維模型的提取，適用場景不同。 | 如果你需要更靈活的 Agent 組合來處理特定任務，則可以考慮這個工具。 | medium，因為需要重新適應不同的操作模式。 |

> [!abstract]- 功能對比矩陣
>
> | 維度 | **nuwa-skill** | **colleague-skill** | **agent-sprite-forge** |
> | --- | --- | --- | --- |
> | 技術路線 | 本專案 | 專注於蒸餾離職同事的思維，使用者只能從身邊的人獲取資料，而 Nuwa 則能蒸餾各領域的名人，範圍更廣。 | 提供多種 Agent 的整合，但不專注於思維模型的提取，適用場景不同。 |
> | 遷移成本 | - | low，因為兩者的使用方式相似。 | medium，因為需要重新適應不同的操作模式。 |
> | 適用場景 | 主要場景 | 如果你的需求僅限於蒸餾身邊同事的經驗和思維，則可以選擇同事. | 如果你需要更靈活的 Agent 組合來處理特定任務，則可以考 |
...
- [MEDIUM] 資料來源的質量可能不均，影響最終的心智模型準確性。
  - 解法：定期檢查和更新資料來源，確保其可靠性。
- **[HIGH]** 蒸餾過程中可能無法捕捉名人的即時反應。
  - 解法：使用者需理解這一點，並在需要即時反應的情境下謹慎使用。
- [MEDIUM] 對於不在支持名人列表中的人物，無法進行蒸餾。
  - 解法：使用者可自行蒐集資料並手動輸入，或等待未來的擴展。

## 使用情境適合度

| 情境 | 適合度 | 說明 |
| --- | --- | --- |
| 創業團隊需要多元視角來制定產品策略 | 非常適合 | 能夠從多位名人中提取不同的思維模型，提供創新的解決方案。 |
| 教育機構希望提升學生的學習興趣和理解能力 | 適合 | 能夠蒸餾費曼的教學方法，幫助學生更好地理解複雜概念。 |
| 個人開發者希望獲取名人的創業建議 | 普通 | 雖然可以提供名人的思維，但可能無法完全符合個人需求。 |
| 大型企業需要蒸餾內部員工的思維方式 | 不適合 | 目前主要針對名人，無法有效蒸餾內部員工的經驗。 |
...
> [!warning] 安全性快速掃描
> 低風險：該工具不需要高權限，並且不存取敏感資料，使用過程中不會暴露使用者的私密資訊。

## 生態系整合

> [!abstract] 如何融入你的工具鏈

Nuwa 最常與 AI 開發工具搭配使用，特別是在需要多元視角的情境中。典型的工作流中，它可以在開發階段提供思維模型的支持，幫助團隊在產品設計上做出更好的決策。整合方面，Nuwa 可以與 GitHub Actions 等 CI 工具相容，並且在 VS Code 等 IDE 中也能夠友好地運行。最常見的整合問題是資料來源的更新，使用者需要定期檢查資料的可靠性，以確保最終結果的準確性。

## 歷史脈絡

> [!info] 這個工具為什麼現在出現？

在 Nuwa 出現之前，蒸餾名人的思維主要依賴於書籍和訪談，這些方法雖然有效，但無法快速獲取多位名人的觀點。隨著 AI 和資料處理技術的進步，Nuwa 的出現使得從多個來源快速提取思維成為可能。這代表了思維模型提取的技術趨勢，未來可能會看到更多類似工具的出現，進一步擴展思維模型的應用範圍。
...
> [!tip] 導入策略
> 第一週：在個人項目中試用。第二週：在小型團隊的討論中進行實驗。第三週：收集反饋並調整使用方式。第四週：在正式項目中開始使用。

**成功指標**：能夠在決策過程中引入多元的思維模型，提升團隊的創新能力。

> [!warning] 退出計畫
> 如果要退出這個工具，所有設定和資料都可以導出為標準格式，方便轉換為其他工具。
...
> | --- | --- | --- | --- |
> | [#47](https://github.com/alchaincyf/nuwa-skill/issues/47) | 或许应该是nvwa.skill? | 0 | 0 |
...
> [!note]- 直接競品（同子分類：思維模型提取）
> ```dataview
> TABLE stars, stars_per_day AS "Stars/天", install_complexity AS "難度", use_case AS "用途"
> FROM "Repos"
> WHERE subcategory = "思維模型提取" AND file.name != "alchaincyf--nuwa-skill"
> SORT stars DESC
> ```
...
> [!note]- 共用概念的相關專案
> ```dataviewjs
> const concepts = ["心智模型","決策啟發式","認知架構"];
