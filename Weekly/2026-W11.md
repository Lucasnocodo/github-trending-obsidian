...
```dataviewjs
const thisWeek = dv.pages('"Repos"').where(p => p.week === "2026-W11");
...
if (lastWeek.length > 0) {
  ...
  dv.table(["分類", prevStr, "2026-W11", "變化"], rows);
} else {
...
```dataviewjs
const pages = dv.pages('"Repos"').where(p => p.week === "2026-W11" && p.install_complexity === "easy");
if (pages.length > 0) {
