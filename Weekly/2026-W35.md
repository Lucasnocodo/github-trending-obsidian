...
```dataviewjs
const pages = dv.pages('"Repos"').where(p => p.week === "2026-W35");
...
TABLE
  ...
  language AS "語言"
FROM "Repos"
WHERE week = "2026-W35" AND stars_per_day >= 500
...
TABLE
  ...
  category AS "分類"
FROM "Repos"
WHERE week = "2026-W35" AND install_complexity = "easy"
SORT stars DESC
```
...
```dataviewjs
const thisWeek = dv.pages('"Repos"').where(p => p.week === "2026-W35");
...
if (lastWeek.length > 0) {
  ...
  dv.table(["分類", prevStr, "2026-W35", "變化"], rows);
} else {
...
```dataviewjs
const pages = dv.pages('"Repos"').where(p => p.week === "2026-W35" && p.install_complexity === "easy");
if (pages.length > 0) {
