...
```dataviewjs
const pages = dv.pages('"Repos"').where(p => p.category === "資料科學");
...
```dataviewjs
const pages = dv.pages('"Repos"').where(p => p.category === "資料科學" && p.status !== "archived");
if (pages.length > 0) {
  dv.table(
...
```dataviewjs
const pages = dv.pages('"Repos"').where(p => p.category === "資料科學" && p.status !== "archived" && p.subcategory);
const subs = {};
for (const p of pages) {
...
```dataviewjs
const repos = dv.pages('"Repos"').where(p => p.category === "資料科學");
...
```dataviewjs
const pages = dv.pages('"Repos"').where(p => p.category === "資料科學" && p.ring);
...
const pages = dv.pages('"Repos"').where(p => {
  if (p.category !== "資料科學" || p.status !== "to-review" || !p.first_seen) return false;
...
```dataviewjs
const pages = dv.pages('"Repos"').where(p => p.category === "資料科學" && p.status !== "archived" && (p.contributor_count || 0) <= 1);
if (pages.length > 0) {
  dv.table(
    ["專案", "Stars", "貢獻者", "維護狀態"],
