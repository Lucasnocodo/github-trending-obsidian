...
> 所有分類為「CLI 工具」的 GitHub Trending 專案

## 總覽

```dataviewjs
const pages = dv.pages('"Repos"').where(p => p.category === "CLI 工具");
...
TABLE
  ...
  install_complexity AS "安裝"
FROM "Repos"
WHERE category = "CLI 工具" AND status = "to-review"
...
```dataviewjs
const pages = dv.pages('"Repos"').where(p => p.category === "CLI 工具" && p.status !== "archived");
if (pages.length > 0) {
  dv.table(
...
```dataviewjs
const pages = dv.pages('"Repos"').where(p => p.category === "CLI 工具" && p.status !== "archived" && p.subcategory);
const subs = {};
for (const p of pages) {
...
```dataviewjs
const repos = dv.pages('"Repos"').where(p => p.category === "CLI 工具");
...
```dataviewjs
const pages = dv.pages('"Repos"').where(p => p.category === "CLI 工具" && p.ring);
...
const pages = dv.pages('"Repos"').where(p => {
  if (p.category !== "CLI 工具" || p.status !== "to-review" || !p.first_seen) return false;
...
```dataviewjs
const pages = dv.pages('"Repos"').where(p => p.category === "CLI 工具" && p.status !== "archived" && (p.contributor_count || 0) <= 1);
if (pages.length > 0) {
  dv.table(
    ["專案", "Stars", "貢獻者", "維護狀態"],
